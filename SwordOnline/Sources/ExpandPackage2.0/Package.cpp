...
Package::Package(string PackageName) {
  try {
    ...
    strstream ErrorInfo;
    ErrorInfo << "Open Package File Error! System Error Info :" << error.what()
              << endl
              << ends;
...
DataSource &Package::GetDataSource() { return m_DataBlock; }
...
Package::Package(DataSource &Source, const string FileName) {
  ...
  PackageFileHead FileHead(Source.GetBlockCount(), IndexOffset, FileID,
                           BlockStartPosition, CRC32value);
