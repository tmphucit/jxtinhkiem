...
protected:
  ...
  DB_ENV *dbenv; // 数据库环境
  char *_search(bool bKey, const char *key_ptr, int key_size, int &size,
                int index);          // 搜索指定记录
...
public:
  ...
  bool commit();                                          // 基于事务的提交
  // 基本记录操作
  bool add(const char *key_ptr, int key_size, const char *data_ptr,
           int data_size);
  ...
  char *search_key(const char *key_ptr, int key_size, int &size,
                   int index = -1) { // 搜索指定记录，返回主键值
  ...
  char *GetRecord(int &size, CursorPointer cpMode,
                  int index = -1); // 取得按游标某一个数据
  char *GetRecord_key(int &size, CursorPointer cpMode,
                      int index = -1); // 取得按游标某一个数据的Key值

  bool GetRecordEx(char *aBuffer, int &size, char *aKeyBuffer, int &keysize,
                   CursorPointer cpMode,
                   int index = -1); // 取得按游标某一个数据和key(新版函数)
};
...
protected:
  char env_path[MAX_TABLE_NAME];
  DB_ENV *dbenv; // 数据库环境
  char *_search(bool bKey, const char *key_ptr, int key_size,
                int &size);          // 搜索指定记录
  char *_next(bool bKey, int &size); // 下一个记录
public:
  ...
  char *GetRecord_key(int &size,
                      CursorPointer cpMode); // 取得按游标某一个数据的Key值
  bool GetRecordEx(char *aBuffer, int &size, char *aKeyBuffer, int &keysize,
                   CursorPointer cpMode);

  char *search_key(const char *key_ptr, int key_size,
                   int &size) {                    // 搜索指定记录，返回主键值
