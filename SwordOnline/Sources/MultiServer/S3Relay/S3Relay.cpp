...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
  ...
  try {
    ...
    if (!gs_LogFile.Initialize(strLogDir, log_threshold)) {
      ::MessageBox(NULL, "Initializing LogFile fail", "error",
                   MB_OK | MB_ICONERROR);
      return FALSE;
    }
...
#define MAX_BUFFER_LEN 1024 * 1024
  ...
  BOOL HasWriteInfo() { return m_RW_Buffer[m_nWrite].nInfoStart > 0; }
  BOOL HasReadInfo() {
    DWORD *pLen =
        (DWORD *)(m_RW_Buffer[m_nRead].buff + m_RW_Buffer[m_nRead].nInfoStart);
    return *pLen != 0;
  }
...
public:
  ...
  BOOL WriteInfo(BYTE *szInfo, DWORD nLen) {
    ...
    if (nInfoStart < MAX_BUFFER_LEN - nMemLen) {
      ...
      memcpy(m_RW_Buffer[m_nWrite].buff + nInfoStart + sizeof(DWORD), szInfo,
             nLen);
      m_RW_Buffer[m_nWrite].nInfoStart += nMemLen;
      bCanAdd = TRUE;
      DWORD *pLen =
          (DWORD *)(m_RW_Buffer[m_nWrite].buff + nInfoStart + nMemLen);
      *pLen = 0;
    }
    ::LeaveCriticalSection(&m_switchsect);
    return bCanAdd;
...
int gTimeStamp(char *sDest, char *pTail) {
  ...
  return sprintf(sDest, "[%04d/%02d/%02d %02d:%02d:%02d]%s",
                 ...
                 ptm->tm_hour, ptm->tm_min, ptm->tm_sec, pTail);
}
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow) {
  ...
  g_listwnd = ::CreateWindow(
      "ListBox", "Out",
      WS_CHILD | WS_VISIBLE | WS_VSCROLL | WS_HSCROLL | LBS_NOINTEGRALHEIGHT, 0,
      0, rc.right, rc.bottom, g_mainwnd, NULL, hInst, NULL);
  if (!g_listwnd)
    return FALSE;
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam,
  ...
  case WM_COMMAND:
    ...
    case IDM_TRACE_HOSTSERVER:
      g_HostServer.TraceInfo();
      break;
    case IDM_TRACE_CHATSERVER:
      g_ChatServer.TraceInfo();
      break;
    case IDM_TRACE_TONGSERVER:
      g_TongServer.TraceInfo();
      break;
    case IDM_TRACE_RELAYSERVER:
      g_RelayServer.TraceInfo();
      break;

    case IDM_TRACE_ROOTCENTER:
      g_RootCenter.TraceInfo();
      break;
    case IDM_TRACE_GATEWAYCENTER:
      g_GatewayCenter.TraceInfo();
      break;
    case IDM_TRACE_DBROLECENTER:
      g_DBCenter.TraceInfo();
      break;
    case IDM_TRACE_RELAYCENTER:
      g_RelayCenter.TraceInfo();
      break;

    case IDM_TRACE_PLAYER_DETAIL:
      g_HostServer.TracePlayer();
      break;
    ...
    case IDM_TRACE_STATUS:
      rTRACE("message: [0] RootClient: %d, GatewayClient: %d, DBRoleClient: %d",
             g_RootCenter.GetClientCount(), g_GatewayCenter.GetClientCount(),
             g_DBCenter.GetClientCount());
      ...
      rTRACE("message: [2] HostConnect: %d, ChatConnect: %d, TongConnect: %d",
             g_HostServer.GetConnectCount(), g_ChatServer.GetConnectCount(),
             g_TongServer.GetConnectCount());
      break;
    case IDM_TRACE_SOCKTHREAD:
      g_threadSock.TraceInfo();
      break;
    case IDM_TRACE_DATASIZE:
      dTRACE("message: recvsum: %ld B, sendsum: %ld B", gGetRecvDataSize(),
             gGetSendDataSize());
      break;
    case IDM_TRACE_MAC: {
      {
        ...
        rTRACE("message: localMAC: %s, globalMAC: %s", descLocal.c_str(),
               descGlobal.c_str());
      }
    } break;
    case IDM_TRACE_IP:
      rTRACE("message: localIP: 0x%08X (%s), globalIP: 0x%08X (%s)",
             gGetHostIP(local_adapt),
             std::_tstring(_ip2a(gGetHostIP(local_adapt))).c_str(),
             gGetHostIP(global_adapt),
             std::_tstring(_ip2a(gGetHostIP(global_adapt))).c_str());
      break;
    case IDM_STARTUP:
      if (gs_initialized)
  ...
  case WM_TIMER:
    ...
    case timer_log:
      gShowTrace();
      break;
    case timer_rootclient:
      gReOneClient(oneclient_root);
      break;
    case timer_gatewayclient:
      gReOneClient(oneclient_gateway);
      break;
    case timer_dbclient:
      gReOneClient(oneclient_db);
      break;
    case timer_friendudtdb:
      gFriendTimingUpdateDB();
      break;
    default:
      break;
    }
...
HWND gGetMainWnd() { return g_mainwnd; }

HWND gGetListBox() { return g_listwnd; }
