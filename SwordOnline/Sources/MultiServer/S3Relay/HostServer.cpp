...
CNetConnect *CHostServer::CreateConnect(CNetServer *pNetServer,
                                        unsigned long id) {
  return new CHostConnect((CHostServer *)pNetServer, id);
}

void CHostServer::DestroyConnect(CNetConnect *pConn) { delete pConn; }
...
BOOL CHostServer::FindPlayerByAcc(CHostConnect *pConn, const std::_tstring &acc,
                                  CNetConnectDup *pConnDup,
...
BOOL CHostServer::FindPlayerByRole(CHostConnect *pConn,
                                   const std::_tstring &role,
                                   CNetConnectDup *pConnDup,
...
BOOL CHostServer::FindPlayerByIpParam(CHostConnect *pConn, DWORD ip,
                                      unsigned long param,
                                      CNetConnectDup *pConnDup,
                                      std::_tstring *pAcc, std::_tstring *pRole,
                                      DWORD *pNameID) {
  AUTOLOCKREAD(m_lockIpMap);
...
/*
BOOL CHostServer::BroadOnPlayer(BOOL byAcc, DWORD fromIP, unsigned long
fromRelayID, const void* pData, size_t size)
{
        AUTOLOCKREAD(m_lockIpMap);

        for (IP2CONNECTMAP::const_iterator it = m_mapIp2Connect.begin(); it !=
m_mapIp2Connect.end(); it++)
        {
                CHostConnect* pConnMe = (*it).second;
