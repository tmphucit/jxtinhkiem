...
BOOL CConnTrack::Uninitialize() { return TRUE; }
...
BOOL CConnTrack::ClearConnect(unsigned long conn) {
  ...
  for (FAMILYMAP::iterator itFami = m_mapFamily.begin();
       itFami != m_mapFamily.end(); itFami++) {
    METHODMAP &methods = (*itFami).second;

    for (METHODMAP::iterator itMeth = methods.begin(); itMeth != methods.end();
         itMeth++) {
      CONNSET &conns = (*itMeth).second;
      conns.erase(conn);
    }
...
CConnTrack::CONNECTS CConnTrack::DupConns(BYTE fami, BYTE meth) {
  ...
  if (itFami != m_mapFamily.end()) {
    ...
    if (itMeth != methods.end()) {
      ...
      for (CONNSET::iterator itConn = conns.begin(); itConn != conns.end();
           itConn++)
