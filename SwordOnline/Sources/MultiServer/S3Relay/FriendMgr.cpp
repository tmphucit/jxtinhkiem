...
static std::_tstring s_dbpath =
    gGetPrivateProfileStringEx(sec_database, key_path, file_friendcfg);
static std::_tstring s_dbfile =
    gGetPrivateProfileStringEx(sec_database, key_file, file_friendcfg);
...
CFriendMgr::CFriendMgr()
    : m_elapseUpdateDB(0), m_tickLast(0), m_StorePlayerPerUdtDB(0),
      m_LoadPlayerPerUdtDB(0), m_dbFriend(s_dbpath.c_str(), s_dbfile.c_str()),
      m_dbFriendRO(s_dbpath.c_str(), s_dbfile.c_str()), m_cursorBkgrndUdtDB(0) {
...
BOOL CFriendMgr::Initialize() {
  ...
  m_StorePlayerPerUdtDB = (size_t)gGetPrivateProfileIntEx(
      sec_database, key_storeplayerperudtdb, file_friendcfg, 0);
  m_LoadPlayerPerUdtDB = (size_t)gGetPrivateProfileIntEx(
      sec_database, key_loadplayerperudtdb, file_friendcfg, 0);

  m_elapseUpdateDB = (UINT)gGetPrivateProfileIntEx(
      sec_database, key_updateinterval, file_friendcfg, 0);
  ...
  {
    {
      ...
      for (PLAYERSMAP::iterator itPlayer = m_mapPlayers.begin();
           itPlayer != m_mapPlayers.end(); itPlayer++) {
...
BOOL CFriendMgr::SomeoneLogin(const std::string &role) {
  ...
  {
    {
      ...
      for (FRIENDSMAP::iterator itFriend = rPlayerInfo.friends.begin();
           itFriend != rPlayerInfo.friends.end(); itFriend++) {
        ...
        if (!*rInfo.pCheated) { // this friend is sincere for someone
          CNetConnectDup conndup2;
          DWORD nameid2 = 0;
          unsigned long param2 = 0;
          if (g_HostServer.FindPlayerByRole(NULL, dst, &conndup2, NULL,
                                            &nameid2,
                                            &param2)) { // this friend is online
  ...
  if (!mapIp2Players.empty()) {
    ...
    for (IP2PLAYERSMAP::iterator itIP = mapIp2Players.begin();
         itIP != mapIp2Players.end(); itIP++) {
      ...
      for (PLAYERLIST::iterator itPlayer = rPlayers.begin();
...
BOOL CFriendMgr::SomeoneLogout(const std::string &role) {
  ...
  for (FRIENDSMAP::iterator itFriend = rPlayerInfo.friends.begin();
    ...
    if (!*rInfo.pCheated) { // notify
      CNetConnectDup conndup2;
      DWORD nameid2 = 0;
      unsigned long param2 = -1;
      if (g_HostServer.FindPlayerByRole(NULL, dst, &conndup2, NULL, &nameid2,
                                        &param2)) { // this friend is online
        CNetConnectDup tongconndup2 =
            g_TongServer.FindTongConnectByIP(conndup2.GetIP());
        if (tongconndup2.IsValid()) {
...
BOOL CFriendMgr::SomeoneSyncFriends(const std::string &role,
                                    unsigned long param, DWORD nameid,
                                    const CNetConnectDup &conndup, BOOL full,
                                    BYTE pckgid) {
  typedef std::pair<BYTE, std::_tstring> THEFRIEND; // first: state, second:
                                                    // name
  ...
  {
    {
      ...
      for (FRIENDSMAP::iterator itFriend = rPlayerInfo.friends.begin();
        ...
        if (!rInfo.cheating) {
          BOOL online =
              g_HostServer.FindPlayerByRole(NULL, dst, NULL, NULL, NULL, NULL);
          ...
          rFriends.push_back(
              THEFRIEND(online ? stateOnline : stateOffline, dst));
        }
      }
    }
  }
  ...
  if (!mapGrp2Friends.empty()) {
    ...
    for (GRP2FRIENDSMAP::iterator itGrp = mapGrp2Friends.begin();
         itGrp != mapGrp2Friends.end(); itGrp++) {
      ...
      for (FRIENDLIST::iterator itFriend = rFriends.begin();
           itFriend != rFriends.end(); itFriend++) {
...
BOOL CFriendMgr::PlayerAddFriend(const std::_tstring &someone,
                                 const std::_tstring &dst) {
...
BOOL CFriendMgr::PlayerDelFriend(const std::_tstring &someone,
                                 const std::_tstring &dst) {
  AUTOLOCKWRITE(m_lockFriend);
...
BOOL CFriendMgr::PlayerIsFriend(const std::_tstring &someone,
                                const std::_tstring &dst, BOOL rev) {
  AUTOLOCKREAD(m_lockFriend);
...
BOOL CFriendMgr::SetFriendGroup(const std::_tstring &someone,
                                const std::_tstring &dst,
                                const std::_tstring &group) {
  AUTOLOCKWRITE(m_lockFriend);
  ...
  rTRACE("Player Group Friend: %s [%s] (%s)", someone.c_str(), dst.c_str(),
         group.c_str());

  return TRUE;
}

std::_tstring CFriendMgr::GetFriendGroup(const std::_tstring &someone,
                                         const std::_tstring &dst) {
  AUTOLOCKREAD(m_lockFriend);
...
BOOL CFriendMgr::TrackAddFriend(const std::_tstring &src,
                                const std::_tstring &dst, BYTE id) {
...
BOOL CFriendMgr::ExtractAddFriend(const std::_tstring &src,
                                  const std::_tstring &dst, BYTE id) {
  AUTOLOCKWRITE(m_lockAddFriend);
...
CFriendMgr::PLAYERSMAP::iterator
CFriendMgr::_LoadSomeone(const std::_tstring &someone) {
  // assert(m_lockFriend._IsWriting());

  std::pair<PLAYERSMAP::iterator, bool> insret =
      m_mapPlayers.insert(PLAYERSMAP::value_type(someone, PLAYERINFO()));
  ...
  // update
  for (MEM_FRIENDRECORDLIST::iterator itMemFriend = memFriends.begin();
       itMemFriend != memFriends.end(); itMemFriend++) {
    MEM_FRIENDRECORD &rmemFriendRec = *itMemFriend;

    std::pair<FRIENDSMAP::iterator, bool> insretfriend =
        rPlayerInfo.friends.insert(
            FRIENDSMAP::value_type(rmemFriendRec.name, FRIENDINFO()));
    ...
    if (insretfriend.second) // insert this friend
    {
      ...
      rInfo.orig_cheating = rmemFriendRec.cheating ? FRIENDINFO::cheatTRUE
                                                   : FRIENDINFO::cheatFALSE;
      rInfo.orig_cheated = rmemFriendRec.cheated ? FRIENDINFO::cheatTRUE
                                                 : FRIENDINFO::cheatFALSE;
...
BOOL CFriendMgr::_StoreSomeone(PLAYERSMAP::iterator itSomeone) {
  ...
  for (FRIENDSMAP::iterator itFriend = rPlayerInfo.friends.begin();
    ...
    if (!needstore &&
        ((rInfo.cheating ? FRIENDINFO::cheatTRUE : FRIENDINFO::cheatFALSE) !=
             rInfo.orig_cheating ||
         (*rInfo.pCheated ? FRIENDINFO::cheatTRUE : FRIENDINFO::cheatFALSE) !=
             rInfo.orig_cheated)) {
      needstore = TRUE;
    }
...
BOOL CFriendMgr::DB_AddSomeone(const std::_tstring &someone,
                               const MEM_FRIENDRECORDLIST &memFriends) {
...
BOOL CFriendMgr::DB_LoadSomeone(const std::_tstring &someone,
                                MEM_FRIENDRECORDLIST *pmemFriends) {
...
BOOL CFriendMgr::DB_StoreSomeone(const std::_tstring &someone,
                                 const MEM_FRIENDRECORDLIST &memFriends) {
  assert(!someone.empty());

  size_t cntFriends = memFriends.size();

  if (cntFriends <= 0) { // erase this someone record, because it's empty
    DB_DeleteSomeone(someone);
    return TRUE;
  }

  size_t sizeMax =
      sizeof(DB_FRIENDRECORD) + (sizeof(_BASIC_STR) + _NAME_LEN) * cntFriends;

  DB_FRIENDRECORDLIST *pdbFriendList = (DB_FRIENDRECORDLIST *)_alloca(sizeMax);
  pdbFriendList->count = cntFriends;

  DB_FRIENDRECORD *pCursor = (DB_FRIENDRECORD *)(pdbFriendList + 1);
  for (MEM_FRIENDRECORDLIST::const_iterator it = memFriends.begin();
       it != memFriends.end(); it++) {
    const MEM_FRIENDRECORD &rmemFriendRec = *it;
    assert(rmemFriendRec.name.size() > 0 &&
           rmemFriendRec.name.size() < _NAME_LEN);
    assert(rmemFriendRec.group.size() >= 0 &&
           rmemFriendRec.group.size() < _NAME_LEN);
  ...
  AUTOLOCKWRITE(m_lockDB);
  return m_dbFriend.add(dbkey.data(), dbkey.size(), (const char *)pdbFriendList,
                        sizeUsed);
}
      ...
      {
        {
          ...
          do {
            ...
            if (m_StorePlayerPerUdtDB != size_t(-1) &&
                listOnePlayer.size() + memPlayers.size() >
                    m_StorePlayerPerUdtDB)
              break;
            ...
            if (rPlayerInfo.loaded) {
              ...
              for (FRIENDSMAP::iterator itFriend = rPlayerInfo.friends.begin();
                ...
                if (!needstore && ((rInfo.cheating ? FRIENDINFO::cheatTRUE
                                                   : FRIENDINFO::cheatFALSE) !=
                                       rInfo.orig_cheating ||
                                   (*rInfo.pCheated ? FRIENDINFO::cheatTRUE
                                                    : FRIENDINFO::cheatFALSE) !=
                                       rInfo.orig_cheated)) {
                  needstore = TRUE;
                }

                rInfo.orig_cheating = rInfo.cheating ? FRIENDINFO::cheatTRUE
                                                     : FRIENDINFO::cheatFALSE;
                rInfo.orig_cheated = *rInfo.pCheated ? FRIENDINFO::cheatTRUE
                                                     : FRIENDINFO::cheatFALSE;
              }
      ...
      {
        {
          if (!memPlayers.empty()) {
            ...
            for (MEMPLAYERS::iterator itMemPlayer = memPlayers.begin();
                 itMemPlayer != memPlayers.end(); itMemPlayer++) {
              ...
              if (itPlayer != m_mapPlayers.end()) {
                ...
                for (FRIENDSMAP::iterator itFriend =
                         rPlayerInfo.friends.begin();
                     itFriend != rPlayerInfo.friends.end(); itFriend++) {
                  FRIENDINFO &rInfo = (*itFriend).second;
                  assert(rInfo.pCheated);
                  ...
                  if (!needstore &&
                      ((rInfo.cheating
                            ? FRIENDINFO::cheatTRUE
                            : FRIENDINFO::cheatFALSE) != rInfo.orig_cheating ||
                       (*rInfo.pCheated
                            ? FRIENDINFO::cheatTRUE
                            : FRIENDINFO::cheatFALSE) != rInfo.orig_cheated)) {
                    needstore = TRUE;
                  }
  ...
  {
    {
      ...
      for (ONEPLAYERLIST::iterator itOnePlayer = listOnePlayer.begin();
           itOnePlayer != listOnePlayer.end(); itOnePlayer++) {
...
BOOL CFriendMgr::PlayerAssociate(const std::_tstring &someone,
                                 const std::_tstring &dst,
                                 const std::_tstring &group, BYTE bidir) {
  AUTOLOCKWRITE(m_lockFriend);
  ...
  rTRACE("Player Associate: %s [%s] on <%s>, bidir: %u", someone.c_str(),
         dst.c_str(), group.c_str(), int(bidir));

  return TRUE;
}

BOOL CFriendMgr::PlayerAssociateBevy(const _BEVY &bevy,
                                     const std::_tstring &group) {
  if (bevy.size() < 2)
    return TRUE;
  ...
  for (_BEVY::const_iterator itS = bevy.begin(); itS != bevy.end(); itS++) {
    ...
    std::pair<PLAYERSMAP::iterator, bool> insit =
        m_mapPlayers.insert(PLAYERSMAP::value_type(someone, PLAYERINFO()));
    assert(!insit.second && insit.first != m_mapPlayers.end() &&
           (*insit.first).second.loaded);
