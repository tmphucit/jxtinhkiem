...
int CTongSet::Create(int nCamp, char *lpszPlayerName, char *lpszTongName,
  ...
  for (i = 0; i < m_nTongPointSize; i++) {
    if (m_pcTong[i] && (m_pcTong[i]->m_dwNameID == dwTongNameID ||
                        m_pcTong[i]->m_dwMasterID == dwPlayerNameID))
      break;
  }
  // 帮会同名错误
  ...
  m_pcTong[nPos]->m_nMasterSex = nSex;

  strcpy(m_pcTong[nPos]->m_szMasterTitle, defTONG_SET_TITLE_NAME_MASTER);

  strcpy(m_pcTong[nPos]->m_szNormalTitle, defTONG_SET_TITLE_NAME_NORMAL);

  strcpy(m_pcTong[nPos]->m_szNormalTitleF, defTONG_SET_TITLE_NAME_NORMALF);
...
int CTongSet::AddMember(char *lpszPlayerName, char *lpszTongName, int nSex) {
  if (!m_pcTong || m_nTongPointSize <= 0)
    return -1;
  if (!lpszPlayerName || !lpszPlayerName[0] || !lpszTongName ||
      !lpszTongName[0])
    return -1;
  if (strlen(lpszTongName) >= defTONG_STR_LENGTH ||
      strlen(lpszPlayerName) >= defTONG_STR_LENGTH)
  ...
  for (i = 0; i < m_nTongPointSize; i++) {
    // 找到了
    if (m_pcTong[i] && (m_pcTong[i]->m_dwNameID == dwTongNameID)) {
      if (!m_pcTong[i]->AddMember(lpszPlayerName, nSex))
        return -1;
      else {
        // 存盘，数据保存至数据库
        TMemberStruct sMember;
...
BOOL CTongSet::GetTongHeadInfo(DWORD dwTongNameID,
                               STONG_HEAD_INFO_SYNC *pInfo) {
...
BOOL CTongSet::GetTongManagerInfo(STONG_GET_MANAGER_INFO_COMMAND *pApply,
...
BOOL CTongSet::GetTongMemberInfo(STONG_GET_MEMBER_INFO_COMMAND *pApply,
...
BOOL CTongSet::Instate(STONG_INSTATE_COMMAND *pInstate,
...
BOOL CTongSet::ChangeTitleFemale(STONG_CHANGE_TITLE_FEMALE_COMMAND *pChange,
  ...
  pSync->m_dwParam = pChange->m_dwParam;

  for (int i = 0; i < m_nTongPointSize; i++) {
    if (m_pcTong[i] && m_pcTong[i]->m_dwNameID == pChange->m_dwTongNameID) {
      return m_pcTong[i]->ChangeTitleFemale(pChange, pSync);
    }
  }
...
BOOL CTongSet::ChangeTitleMale(STONG_CHANGE_TITLE_MALE_COMMAND *pChange,
  ...
  for (int i = 0; i < m_nTongPointSize; i++) {
    if (m_pcTong[i] && m_pcTong[i]->m_dwNameID == pChange->m_dwTongNameID) {
      return m_pcTong[i]->ChangeTitleMale(pChange, pSync);
    }
  }
...
BOOL CTongSet::ChangeTitle(STONG_CHANGE_TITLE_COMMAND *pChange,
  ...
  for (int i = 0; i < m_nTongPointSize; i++) {
    if (m_pcTong[i] && m_pcTong[i]->m_dwNameID == pChange->m_dwTongNameID) {
      return m_pcTong[i]->ChangeTitle(pChange, pSync);
    }
  }
...
BOOL CTongSet::InitFromDB() {
  ...
  for (i = 0; i < defTONG_SET_INIT_POINT_NUM; i++) {
...
BOOL CTongSet::GetLoginData(STONG_GET_LOGIN_DATA_COMMAND *pLogin,
...
BOOL CTongSet::SearchOne(DWORD dwTongNameID, char *lpszName,
                         STONG_ONE_LEADER_INFO *pInfo) {
  if (!m_pcTong)
    return FALSE;
