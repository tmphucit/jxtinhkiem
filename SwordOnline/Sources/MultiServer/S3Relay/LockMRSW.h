...
struct KAutoLockWrite {
  CLockMRSW &theLock;
  KAutoLockWrite(CLockMRSW &locksrc) : theLock(locksrc) {
    theLock.EnterWrite();
  }
...
#define AUTOLOCKREAD(lock) KAutoLockRead __local_autolockread_(lock);
#define AUTOLOCKREAD_(name, lock) KAutoLockRead name(lock);

#define AUTOLOCKWRITE(lock) KAutoLockWrite __local_autolockwrite_(lock);
#define AUTOLOCKWRITE_(name, lock) KAutoLockWrite name(lock);
