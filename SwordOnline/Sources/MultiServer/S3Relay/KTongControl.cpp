...
CTongControl::CTongControl(int nCamp, char *lpszPlayerName,
                           char *lpszTongName) {
  m_nCamp = 0;
  m_dwMoney = 0;
  m_nCredit = 0;
  ...
  if (!lpszTongName || !lpszTongName[0] ||
      strlen(lpszTongName) >= defTONG_STR_LENGTH)
    return;
  m_nCamp = nCamp;
...
BOOL CTongControl::AddMember(char *lpszPlayerName, int nSex) {
  ...
  if (i >= m_nMemberPointSize) {
    ...
    delete[] m_psMember;
    m_psMember = (STONG_MEMBER *)new STONG_MEMBER[m_nMemberPointSize +
                                                  defTONG_MEMBER_SIZE_ADD];
    memcpy(m_psMember, pTemp, sizeof(STONG_MEMBER) * m_nMemberPointSize);
    memset(&m_psMember[m_nMemberPointSize], 0,
           sizeof(STONG_MEMBER) * defTONG_MEMBER_SIZE_ADD);
...
BOOL CTongControl::GetTongHeadInfo(STONG_HEAD_INFO_SYNC *pInfo) {
  ...
  pInfo->m_wLength =
      sizeof(STONG_HEAD_INFO_SYNC) - sizeof(pInfo->m_sMember) +
      sizeof(STONG_ONE_LEADER_INFO) * (1 + pInfo->m_btDirectorNum);

  return TRUE;
}

BOOL CTongControl::GetTongManagerInfo(STONG_GET_MANAGER_INFO_COMMAND *pApply,
  ...
  pInfo->m_wLength = sizeof(STONG_MANAGER_INFO_SYNC) -
                     sizeof(STONG_ONE_LEADER_INFO) *
                         (defTONG_ONE_PAGE_MAX_NUM - pInfo->m_btCurNum);

  return TRUE;
}

BOOL CTongControl::GetTongMemberInfo(STONG_GET_MEMBER_INFO_COMMAND *pApply,
  ...
  for (; i < m_nMemberPointSize; i++) {
    ...
    strcpy(pInfo->m_sMember[pInfo->m_btCurNum].m_szName,
           m_psMember[i].m_szName);

    if (m_psMember[i].m_nSex == 0) {
      strcpy(pInfo->m_sMember[pInfo->m_btCurNum].m_szTitle, m_szNormalTitle);
    } else {
      strcpy(pInfo->m_sMember[pInfo->m_btCurNum].m_szTitle, m_szNormalTitleF);
    }
  ...
  pInfo->m_wLength = sizeof(STONG_MEMBER_INFO_SYNC) -
                     sizeof(STONG_ONE_MEMBER_INFO) *
                         (defTONG_ONE_PAGE_MAX_NUM - pInfo->m_btCurNum);

  return TRUE;
}

BOOL CTongControl::Instate(STONG_INSTATE_COMMAND *pInstate,
                           STONG_INSTATE_SYNC *pSync) {
  if (!pInstate || !pSync)
    return FALSE;
  if (pInstate->m_btCurFigure == pInstate->m_btNewFigure &&
      pInstate->m_btCurPos == pInstate->m_btNewPos)
    return FALSE;
  ...
  if (pInstate->m_btCurFigure == enumTONG_FIGURE_DIRECTOR) {
    for (i = 0; i < defTONG_MAX_DIRECTOR; i++) {
      if (m_dwDirectorID[i] == dwNameID &&
          strcmp(szName, m_szDirectorName[i]) == 0)
        break;
    }
    ...
    case enumTONG_FIGURE_MANAGER:
      ...
      NewSex = m_nDirectorSex[nOldPos];

      strcpy(m_szManagerName[nNewPos], szName);
      m_dwManagerID[nNewPos] = dwNameID;
      m_nManagerSex[nNewPos] = NewSex;
      strcpy(m_szManagerTitle[nNewPos], defTONG_SET_TITLE_NAME_MANAGER);

      m_szDirectorName[nOldPos][0] = 0;
      m_dwDirectorID[nOldPos] = 0;
      m_nDirectorSex[nOldPos] = 0;
      m_szDirectorTitle[nOldPos][0] = 0;
    ...
    case enumTONG_FIGURE_MEMBER:
      if (!AddMember(szName, m_nDirectorSex[nOldPos]))
        return FALSE;
      NewSex = m_nDirectorSex[nOldPos];
      m_szDirectorName[nOldPos][0] = 0;
      m_dwDirectorID[nOldPos] = 0;
      ...
      if (NewSex == 0) {
        ...
        strcpy(pSync->m_szTitle, m_szNormalTitleF);
      }
    ...
    for (i = 0; i < defTONG_MAX_MANAGER; i++) {
      if (m_dwManagerID[i] == dwNameID &&
          strcmp(szName, m_szManagerName[i]) == 0)
        break;
    }
    if (i >= defTONG_MAX_MANAGER)
      return FALSE;
    ...
    case enumTONG_FIGURE_DIRECTOR:
      ...
      if (i >= defTONG_MAX_DIRECTOR)
        return FALSE;
      nNewPos = i;
      NewSex = m_nManagerSex[nOldPos];

      strcpy(m_szDirectorName[nNewPos], szName);
      m_dwDirectorID[nNewPos] = dwNameID;
      m_nDirectorSex[nNewPos] = NewSex;
      strcpy(m_szDirectorTitle[nNewPos], defTONG_SET_TITLE_NAME_DIRECTOR);

      m_szManagerName[nOldPos][0] = 0;
      m_dwManagerID[nOldPos] = 0;
      m_nManagerSex[nOldPos] = 0;
      m_szManagerTitle[nOldPos][0] = 0;
      ...
      break;
    case enumTONG_FIGURE_MEMBER:
      if (!AddMember(szName, m_nManagerSex[nOldPos]))
        return FALSE;
  ...
  {
    {
      ...
      if (g_HostServer.FindPlayerByRole(NULL, std::_tstring(szName), &conndup,
                                        NULL, &nameid, &param)) {
        CNetConnectDup tongconndup =
            g_TongServer.FindTongConnectByIP(conndup.GetIP());
        if (tongconndup.IsValid()) {
          ...
          case enumTONG_FIGURE_MEMBER:
            ...
            if (NewSex == 0) {
              strcpy(sSync.m_szTitle, m_szNormalTitle);
            } else {
              strcpy(sSync.m_szTitle, m_szNormalTitleF);
            }
  ...
  if (channid != -1) {
    ...
    g_ChannelMgr.SayOnChannel(channid, TRUE, std::string(), std::string("公告"),
                              std::string(szMsg));
  }
...
STONG_INSTATE_SYNC *pSync)
{
        if (!pInstate || !pSync)
                return FALSE;
        if (pInstate->m_btCurFigure == pInstate->m_btNewFigure &&
pInstate->m_btCurPos == pInstate->m_btNewPos) return FALSE;
        ...
        if (pInstate->m_btCurFigure == enumTONG_FIGURE_DIRECTOR)
        {
                ...
                if (!m_szDirectorName[pInstate->m_btCurPos][0] ||
                        strcmp(szName, m_szDirectorName[pInstate->m_btCurPos])
!= 0) return FALSE; switch (pInstate->m_btNewFigure)
                {
                case enumTONG_FIGURE_DIRECTOR:
                        ...
                        m_dwDirectorID[pInstate->m_btNewPos] =
g_String2Id(szName); m_szDirectorName[pInstate->m_btCurPos][0] = 0;
                        m_dwDirectorID[pInstate->m_btCurPos] = 0;
                        strcpy(pSync->m_szTitle,
m_szDirectorTitle[pInstate->m_btNewPos]); break; case enumTONG_FIGURE_MANAGER:
                        ...
                        m_dwManagerID[pInstate->m_btNewPos] =
g_String2Id(szName); m_szDirectorName[pInstate->m_btCurPos][0] = 0;
                        m_dwDirectorID[pInstate->m_btCurPos] = 0;
                        m_nDirectorNum--;
                        m_nManagerNum++;
                        strcpy(pSync->m_szTitle,
m_szManagerTitle[pInstate->m_btNewPos]); break; case enumTONG_FIGURE_MEMBER: if
(!AddMember(szName)) return FALSE; m_szDirectorName[pInstate->m_btCurPos][0] =
0; m_dwDirectorID[pInstate->m_btCurPos] = 0; m_nDirectorNum--;
                        strcpy(pSync->m_szTitle, m_szNormalTitle);
                        break;
                default:
                        return FALSE;
        ...
        else if (pInstate->m_btCurFigure == enumTONG_FIGURE_MANAGER)
        {
                ...
                if (!m_szManagerName[pInstate->m_btCurPos][0] ||
                        strcmp(szName, m_szManagerName[pInstate->m_btCurPos]) !=
0) return FALSE; switch (pInstate->m_btNewFigure)
                {
                case enumTONG_FIGURE_DIRECTOR:
                        if (pInstate->m_btNewPos >= defTONG_MAX_DIRECTOR)
                        ...
                        strcpy(m_szDirectorName[pInstate->m_btNewPos], szName);
                        m_dwDirectorID[pInstate->m_btNewPos] =
g_String2Id(szName); m_szManagerName[pInstate->m_btCurPos][0] = 0;
...
g_String2Id(szName); m_szManagerName[pInstate->m_btCurPos][0] = 0;
                        ...
                        strcpy(pSync->m_szTitle,
m_szManagerTitle[pInstate->m_btNewPos]); break; case enumTONG_FIGURE_MEMBER: if
(!AddMember(szName)) return FALSE; m_szManagerName[pInstate->m_btCurPos][0] = 0;
                        m_dwManagerID[pInstate->m_btCurPos] = 0;
                        m_nManagerNum--;
                        strcpy(pSync->m_szTitle, m_szNormalTitle);
                        break;
        ...
        else if (pInstate->m_btCurFigure == enumTONG_FIGURE_MEMBER)
        {
                ...
                case enumTONG_FIGURE_DIRECTOR:
                        ...
                        strcpy(m_szDirectorName[pInstate->m_btNewPos], szName);
                        m_dwDirectorID[pInstate->m_btNewPos] =
g_String2Id(szName); this->m_psMember[nPos].m_szName[0] = 0;
                        this->m_psMember[nPos].m_dwNameID = 0;
                        m_nDirectorNum++;
                        m_nMemberNum--;
                        strcpy(pSync->m_szTitle,
...
g_String2Id(szName); this->m_psMember[nPos].m_szName[0] = 0;
                        ...
                        m_nMemberNum--;
                        strcpy(pSync->m_szTitle,
m_szManagerTitle[pInstate->m_btNewPos]); break; default: return FALSE;
                }
        }
        pSync->m_btSuccessFlag = 1;
        ...
        {{
                ...
                DWORD nameid = 0;
                unsigned long param = 0;
                if (g_HostServer.FindPlayerByRole(NULL, std::_tstring(szName),
&conndup, NULL, &nameid, &param))
                {
                        CNetConnectDup tongconndup =
g_TongServer.FindTongConnectByIP(conndup.GetIP()); if (tongconndup.IsValid())
                        {
                                STONG_BE_INSTATED_SYNC	sSync;
                                ...
                                {
                                case enumTONG_FIGURE_DIRECTOR:
                                        strcpy(sSync.m_szTitle,
m_szDirectorTitle[pInstate->m_btNewPos]); break; case enumTONG_FIGURE_MANAGER:
                                        strcpy(sSync.m_szTitle,
m_szManagerTitle[pInstate->m_btNewPos]); break; case enumTONG_FIGURE_MEMBER:
                                        strcpy(sSync.m_szTitle,
this->m_szNormalTitle); break;
                                }

                                tongconndup.SendPackage((const void *)&sSync,
sizeof(sSync));
                        }
                        else
                        {
        ...
        {
                if (pSync->m_szTitle[0])
                        sprintf(szMsg, "%s被任命为%s！", szName,
pSync->m_szTitle); else
                {
                        switch (pSync->m_btNewFigure)
                        {
                        case enumTONG_FIGURE_DIRECTOR:
                ...
                g_ChannelMgr.SayOnChannel(channid, TRUE, std::string(),
std::string("公告"), std::string(szMsg));
        }
...
BOOL CTongControl::Kick(STONG_KICK_COMMAND *pKick, STONG_KICK_SYNC *pSync) {
  ...
  if (pKick->m_btFigure == enumTONG_FIGURE_MANAGER) {
    ...
    for (int i = 0; i < defTONG_MAX_MANAGER; i++) {
      if (m_dwManagerID[i] == dwNameID &&
          strcmp(m_szManagerName[i], szName) == 0)
        break;
    }
    if (i >= defTONG_MAX_MANAGER)
      return FALSE;
...
BOOL CTongControl::ChangeTitleFemale(STONG_CHANGE_TITLE_FEMALE_COMMAND *pChange,
                                     STONG_CHANGE_TITLE_FEMALE_SYNC *pSync) {
  ...
  strcpy(m_szNormalTitleF, pChange->m_szTitle);
  strcpy(pSync->m_szTitle, m_szNormalTitleF);

  return TRUE;
}

BOOL CTongControl::ChangeTitleMale(STONG_CHANGE_TITLE_MALE_COMMAND *pChange,
                                   STONG_CHANGE_TITLE_MALE_SYNC *pSync) {
  ...
  strcpy(m_szNormalTitle, pChange->m_szTitle);
  strcpy(pSync->m_szTitle, m_szNormalTitle);

  return TRUE;
}

BOOL CTongControl::ChangeTitle(STONG_CHANGE_TITLE_COMMAND *pChange,
                               STONG_CHANGE_TITLE_SYNC *pSync) {

  if (!pChange || !pSync)
    return FALSE;

  int i;
  DWORD dwNameID = g_String2Id(pChange->m_szName);
  ...
  if (this->m_dwMasterID == dwNameID) {
    strcpy(m_szMasterTitle, pChange->m_szTitle);
  ...
  for (i = 0; i < defTONG_MAX_DIRECTOR; i++) {
    if (this->m_dwDirectorID[i] == dwNameID) {
      strcpy(m_szDirectorTitle[i], pChange->m_szTitle);
  ...
  for (i = 0; i < defTONG_MAX_MANAGER; i++) {
    if (this->m_dwManagerID[i] == dwNameID) {
      strcpy(m_szManagerTitle[i], pChange->m_szTitle);
...
BOOL CTongControl::Leave(STONG_LEAVE_COMMAND *pLeave, STONG_LEAVE_SYNC *pSync) {
  ...
  case enumTONG_FIGURE_MANAGER: {
    ...
    if (nPos < 0)
      return FALSE;
    if (!m_szManagerName[nPos][0] || strcmp(szName, m_szManagerName[nPos]) != 0)
      return FALSE;
...
BOOL CTongControl::AcceptMaster(STONG_ACCEPT_MASTER_COMMAND *pAccept) {
  ...
  if (pAccept->m_btAcceptFalg == 0) {
    ...
    DWORD nameid = 0;
    unsigned long param = 0;
    if (g_HostServer.FindPlayerByRole(NULL, std::_tstring(m_szMasterName),
                                      &conndup, NULL, &nameid, &param)) {
      CNetConnectDup tongconndup =
          g_TongServer.FindTongConnectByIP(conndup.GetIP());
      if (tongconndup.IsValid()) {
  ...
  if (pAccept->m_btFigure == enumTONG_FIGURE_DIRECTOR) {
    ...
    if (i >= defTONG_MAX_DIRECTOR)
      return FALSE;
    if (!AddMember(m_szMasterName, m_nDirectorSex[i]))
      return FALSE;
    ...
    if (i >= defTONG_MAX_MANAGER)
      return FALSE;
    if (!AddMember(m_szMasterName, m_nManagerSex[i]))
      return FALSE;
    strcpy(this->m_szMasterName, szName);
    this->m_dwMasterID = dwNameID;
    ...
    if (i >= m_nMemberPointSize)
      return FALSE;
    if (!AddMember(m_szMasterName, m_psMember[i].m_nSex))
      return FALSE;
  ...
  if (g_HostServer.FindPlayerByRole(NULL, std::_tstring(szOldMaster), &conndup1,
                                    NULL, &nameid, &param)) {
    CNetConnectDup tongconndup =
        g_TongServer.FindTongConnectByIP(conndup1.GetIP());
    if (tongconndup.IsValid()) {
  ...
  if (g_HostServer.FindPlayerByRole(NULL, std::_tstring(szName), &conndup2,
                                    NULL, &nameid, &param)) {
    CNetConnectDup tongconndup =
...
BOOL CTongControl::GetLoginData(STONG_GET_LOGIN_DATA_COMMAND *pLogin,
                                STONG_LOGIN_DATA_SYNC *pSync) {
  if (!pLogin || !pSync)
    return FALSE;
    ...
    for (i = 0; i < this->m_nMemberPointSize; i++) {
      if (m_psMember[i].m_dwNameID == dwNameID) {
        ...
        if (m_psMember[i].m_nSex == 0) {
          ...
          strcpy(pSync->m_szTitle, this->m_szNormalTitleF);
        }
        return TRUE;
      }
    }
