...
std::_tstring gGetPrivateProfileStringEx(LPCSTR lpAppName, LPCSTR lpKeyName,
                                         LPCSTR lpFileName, LPCSTR lpDefault) {
  std::_tstring path = gGetLocateDirectory() + lpFileName;

  TCHAR buffer[_MAX_PATH] = {0};
  ::GetPrivateProfileString(lpAppName, lpKeyName, lpDefault, buffer, _MAX_PATH,
                            path.c_str());
  return buffer;
}
INT gGetPrivateProfileIntEx(LPCSTR lpAppName, LPCSTR lpKeyName,
                            LPCSTR lpFileName, INT nDefault) {
  std::_tstring path = gGetLocateDirectory() + lpFileName;

  return ::GetPrivateProfileInt(lpAppName, lpKeyName, nDefault, path.c_str());
}

TCHAR gGetPrivateProfileCharEx(LPCSTR lpAppName, LPCSTR lpKeyName,
                               LPCSTR lpFileName, TCHAR chDefault) {
  std::_tstring theStr =
      ::gGetPrivateProfileStringEx(lpAppName, lpKeyName, lpFileName);

  return theStr.empty() ? chDefault : theStr[0];
}

const std::vector<std::_tstring> &
gGetPrivateProfileSectionEx(LPCTSTR lpAppName, LPCTSTR lpFileName) {
  std::_tstring path = gGetLocateDirectory() + lpFileName;
  ...
  DWORD cch =
      ::GetPrivateProfileSection(lpAppName, buff, MAX_BUFFSIZE, path.c_str());
...
const std::vector<std::_tstring> &
gGetPrivateProfileSectionNamesEx(LPCTSTR lpFileName) {
  std::_tstring path = gGetLocateDirectory() + lpFileName;
...
BOOL gInitialize() {
  ...
  const size_t playercntRelay =
      gGetPrivateProfileIntEx(sec_relay, key_playercnt, file_relaycfg, 0);
  const size_t precisionRelay =
      gGetPrivateProfileIntEx(sec_relay, key_precision, file_relaycfg, 0);
  const size_t freebufferRelay =
      gGetPrivateProfileIntEx(sec_relay, key_freebuffer, file_relaycfg, 0);
  const size_t buffersizeRelay =
      gGetPrivateProfileIntEx(sec_relay, key_buffersize, file_relaycfg, 0);
  const DWORD listenaddrRelay = INADDR_ANY;
  const unsigned short listenportRelay = RELAY_SIBLING_PORT;

  if (!g_RelayCenter.Initialize())
    return FALSE;
  if (!g_RelayServer.Startup(playercntRelay, precisionRelay, freebufferRelay,
                             buffersizeRelay, listenaddrRelay,
                             listenportRelay)) {
    rTRACE("FAIL: relay server");
    // return FALSE;
  }

  const size_t freebufferRoot =
      gGetPrivateProfileIntEx(sec_root, key_freebuffer, file_relaycfg, 0);
  const size_t buffersizeRoot =
      gGetPrivateProfileIntEx(sec_root, key_buffersize, file_relaycfg, 0);
  std::_tstring addrRoot =
      gGetPrivateProfileStringEx(sec_root, key_address, file_relaycfg);
  const unsigned short portRoot = RELAY_ROUTE_PORT;

  if (!g_RootCenter.Initialize())
    return FALSE;
  if (!g_RootClient.Startup(freebufferRoot, buffersizeRoot, addrRoot.c_str(),
                            portRoot)) {
  ...
  const size_t freebufferDB =
      gGetPrivateProfileIntEx(sec_dbrole, key_freebuffer, file_relaycfg, 0);
  const size_t buffersizeDB =
      gGetPrivateProfileIntEx(sec_dbrole, key_buffersize, file_relaycfg, 0);
  std::_tstring addrDB =
      gGetPrivateProfileStringEx(sec_dbrole, key_address, file_relaycfg);
  ...
  const size_t freebufferGateway =
      gGetPrivateProfileIntEx(sec_gateway, key_freebuffer, file_relaycfg, 0);
  const size_t buffersizeGateway =
      gGetPrivateProfileIntEx(sec_gateway, key_buffersize, file_relaycfg, 0);
  std::_tstring addrGateway =
      gGetPrivateProfileStringEx(sec_gateway, key_address, file_relaycfg);
  const unsigned short portGateway = GATEWAY_GAMESVR_SERVICE_PORT;

  if (!g_GatewayCenter.Initialize())
    return FALSE;
  if (!g_GatewayClient.Startup(freebufferGateway, buffersizeGateway,
                               addrGateway.c_str(), portGateway)) {
    rTRACE("FAIL: gateway client - %s", addrGateway.c_str());
    gOnOneClientClose(oneclient_gateway);
    // return FALSE;
  }

  const size_t playercntHost =
      gGetPrivateProfileIntEx(sec_host, key_playercnt, file_relaycfg, 0);
  const size_t precisionHost =
      gGetPrivateProfileIntEx(sec_host, key_precision, file_relaycfg, 0);
  const size_t freebufferHost =
      gGetPrivateProfileIntEx(sec_host, key_freebuffer, file_relaycfg, 0);
  const size_t buffersizeHost =
      gGetPrivateProfileIntEx(sec_host, key_buffersize, file_relaycfg, 0);
  const DWORD listenaddrHost = INADDR_ANY;
  const unsigned short listenportHost = RELAY_GAMESVR_SERVICE_PORT;

  if (!g_HostServer.Startup(playercntHost, precisionHost, freebufferHost,
                            buffersizeHost, listenaddrHost, listenportHost)) {
    rTRACE("FAIL: host server");
    // return FALSE;
  }

  const size_t playercntTong =
      gGetPrivateProfileIntEx(sec_tong, key_playercnt, file_relaycfg, 0);
  const size_t precisionTong =
      gGetPrivateProfileIntEx(sec_tong, key_precision, file_relaycfg, 0);
  const size_t freebufferTong =
      gGetPrivateProfileIntEx(sec_tong, key_freebuffer, file_relaycfg, 0);
  const size_t buffersizeTong =
      gGetPrivateProfileIntEx(sec_tong, key_buffersize, file_relaycfg, 0);
  const DWORD listenaddrTong = INADDR_ANY;
  const unsigned short listenportTong = RELAY_GAMESVR_TONGSERVICE_PORT;

  if (!g_TongServer.Startup(playercntTong, precisionTong, freebufferTong,
                            buffersizeTong, listenaddrTong, listenportTong)) {
    rTRACE("FAIL: tong server");
    // return FALSE;
  }

  const size_t playercntChat =
      gGetPrivateProfileIntEx(sec_chat, key_playercnt, file_relaycfg, 0);
  const size_t precisionChat =
      gGetPrivateProfileIntEx(sec_chat, key_precision, file_relaycfg, 0);
  const size_t freebufferChat =
      gGetPrivateProfileIntEx(sec_chat, key_freebuffer, file_relaycfg, 0);
  const size_t buffersizeChat =
      gGetPrivateProfileIntEx(sec_chat, key_buffersize, file_relaycfg, 0);
  const DWORD listenaddrChat = INADDR_ANY;
  const unsigned short listenportChat = RELAY_GAMESVR_CHATSERVICE_PORT;

  if (!g_ChatServer.Startup(playercntChat, precisionChat, freebufferChat,
                            buffersizeChat, listenaddrChat, listenportChat)) {
...
BOOL gIsLocalIP(DWORD ip) { return (ip & _localmask) == _localtag; }
...
const UCHAR *gGetHostMac(int adapt, size_t *pSize) {
  ...
  switch (adapt) {
  case local_adapt:
    return _macstore.macLocal;
  case global_adapt:
    return _macstore.macGlobal;
  }
...
DWORD gGetHostIP(int adapt) {
  ...
  switch (adapt) {
  case local_adapt:
    return _ipstore.ipLocal;
  case global_adapt:
    return _ipstore.ipGlobal;
  }
...
BOOL gConnectToSibling(const char *addr) {
  const size_t freebuffer =
      gGetPrivateProfileIntEx(sec_relay, key_freebuffer, file_relaycfg, 0);
  const size_t buffersize =
      gGetPrivateProfileIntEx(sec_relay, key_buffersize, file_relaycfg, 0);

  CRelayClient *pRelayClient = new CRelayClient(&g_RelayCenter, TRUE);
  if (!pRelayClient->Startup(freebuffer, buffersize, addr, RELAY_SIBLING_PORT,
                             FALSE))
    return FALSE;
...
size_t gFillClntwayPckg(DWORD nameid, unsigned long param,
                        const RELAY_ASKWAY_DATA *pAskWay, size_t sizeAskWay,
                        RELAY_ASKWAY_DATA *pClntWay, size_t sizeClntWay) {
...
size_t gFillLosewayPckg(DWORD nToIP, DWORD nToRelayID, const void *pLosePckg,
                        size_t sizeLosePckg, void *pLoseway,
                        size_t sizeLoseway) {
...
BOOL gOnOneClientClose(ONECLIENT ct) {
  ...
  case oneclient_root:
    if (!gs_timerRoot) {
      INT rootinterval = gGetPrivateProfileIntEx(sec_root, key_retryinterval,
                                                 file_relaycfg, 0);
      if (rootinterval > 0) {
        gs_timerRoot =
            ::SetTimer(mainwnd, timer_rootclient, rootinterval, NULL);
        if (!gs_timerRoot) {
  ...
  case oneclient_gateway:
    if (!gs_timerGateway) {
      INT gatewayinterval = gGetPrivateProfileIntEx(
          sec_gateway, key_retryinterval, file_relaycfg, 0);
      if (gatewayinterval > 0) {
        gs_timerGateway =
            ::SetTimer(mainwnd, timer_gatewayclient, gatewayinterval, NULL);
        if (!gs_timerGateway) {
  ...
  case oneclient_db:
    if (!gs_timerDB) {
      INT dbinterval = gGetPrivateProfileIntEx(sec_dbrole, key_retryinterval,
                                               file_relaycfg, 0);
      if (dbinterval > 0) {
  ...
  case oneclient_root: {
    {
      ...
      if (!g_RootClient.Startup(freebufferRoot, buffersizeRoot,
                                addrRoot.c_str(), portRoot)) {
  ...
  case oneclient_gateway: {
    {
      ...
      const size_t freebufferGateway = gGetPrivateProfileIntEx(
          sec_gateway, key_freebuffer, file_relaycfg, 0);
      const size_t buffersizeGateway = gGetPrivateProfileIntEx(
          sec_gateway, key_buffersize, file_relaycfg, 0);
      std::_tstring addrGateway =
          gGetPrivateProfileStringEx(sec_gateway, key_address, file_relaycfg);
      const unsigned short portGateway = GATEWAY_GAMESVR_SERVICE_PORT;
  ...
  case oneclient_db: {
    {
      ...
      if (!g_DBClient.Startup(freebufferDB, buffersizeDB, addrDB.c_str(),
                              portDB)) {
