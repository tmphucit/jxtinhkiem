...
public:
  BOOL Startup(size_t maxFreeBuffers, size_t bufferSize,
               const char *pAddressToConnectServer,
               unsigned short usPortToConnectServer, BOOL block = TRUE);
  BOOL Shutdown();
...
public:
  ...
  CNetClientDup(const CNetClientDup &netclntdup)
      : m_pClient(netclntdup.GetClient()), m_SvrIP(netclntdup.m_SvrIP) {
