...
CNetRelay::CNetRelay() : m_hDataExist(NULL) {}
...
DWORD CNetRelay::Main(LPVOID lpParam) {
  for (;;) {
    ...
    DWORD wait = ::WaitForMultipleObjects(sizeof(hdls) / sizeof(hdls[0]), hdls,
                                          FALSE, INFINITE);
...
void CNetRelay::Breathe() {
  ...
  if (m_queuePckg.empty()) {
    ...
    g_pNetServer->SendPackage(pckg.idFrom, pckg.ipTo, pckg.portTo, pckg.pData,
                              pckg.size);
...
void CNetRelay::Clearup() {
  for (PACKAGEQUEUE::iterator it = m_queuePckg.begin(); it != m_queuePckg.end();
       it++)
...
int CNetRelay::RelayPackage(unsigned long idFrom, DWORD ipTo,
                            unsigned short portTo, void *pData, unsigned size) {
  ...
  if (pRelayCmd->wLength <
      sizeof(RELAY_COMMAND) - sizeof(pRelayCmd->ProtocolType))
    return FALSE;
