...
BOOL CDealPackage::Uninitialize() { return TRUE; }

BOOL CDealPackage::DealPackage(const CNetConnect *pConn, const void *pData,
                               unsigned len) {
...
BOOL CDealPackage::Proc_c2sRegisterFamily(const CNetConnect *pConn,
                                          const void *pData, unsigned len) {
  ...
  if (pRegFami->bRegister) {
    g_pConnTrack->RegConnect(pConn->GetID(), pRegFami->nFamily,
                             pRegFami->RelayMethod);
  } else {
    g_pConnTrack->UnregConnect(pConn->GetID(), pRegFami->nFamily,
                               pRegFami->RelayMethod);
...
BOOL CDealPackage::Proc_c2sEnterGame(const CNetConnect *pConn,
                                     const void *pData, unsigned len) {
  ...
  g_pPlayerTrack->Login(pEnterGame2->szAccount, pEnterGame2->szRoleName,
                        pEnterGame2->dwNameID, pConn->GetID(),
                        pEnterGame2->nPlayerIdx);

  return TRUE;
}

BOOL CDealPackage::Proc_c2sLeaveGame(const CNetConnect *pConn,
                                     const void *pData, unsigned len) {
...
BOOL CDealPackage::Proc_c2sRelay(const CNetConnect *pConn, const void *pData,
                                 unsigned len) {
  ...
  if (g_pNetRelay->RelayPackage(pConn->GetID(), pRelayCmd->nIP, pRelayCmd->port,
                                pRelayCmdDup, len))
    return TRUE;
