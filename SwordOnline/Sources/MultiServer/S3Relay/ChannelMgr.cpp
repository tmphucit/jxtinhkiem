...
CChannelMgr::CChannelMgr()
    : m_lastChannID(0), m_channidGM(-1), m_chChannEsc(0), m_chChannSplt(0),
      m_chChannTEAM(0), m_chChannKIM(0), m_chChannFAC(0), m_chChannTONG(0),
      m_chChannSCRN(0), m_minTeamID(0), m_maxTeamID(-1), m_minKimID(0),
      m_maxKimID(-1), m_minFacID(0), m_maxFacID(-1), m_minTongID(0),
      m_maxTongID(-1), m_defCost(0), m_costTeam(0), m_costKim(0), m_costFac(0),
      m_costTong(0), m_costScrn(0), m_costBc(0) {}
...
BOOL CChannelMgr::Initialize() {
  ...
  m_chChannEsc = gGetPrivateProfileCharEx(sec_system, key_charEsc,
                                          file_channcfg, defCHANN_esc);
  m_chChannSplt = gGetPrivateProfileCharEx(sec_system, key_charSplt,
                                           file_channcfg, defCHANN_splt);

  m_chChannTEAM = gGetPrivateProfileCharEx(sec_team, key_escSpec, file_channcfg,
                                           defCHANN_TEAM);
  m_chChannKIM = gGetPrivateProfileCharEx(sec_kim, key_escSpec, file_channcfg,
                                          defCHANN_KIM);
  m_chChannFAC = gGetPrivateProfileCharEx(sec_faction, key_escSpec,
                                          file_channcfg, defCHANN_FAC);
  m_chChannTONG = gGetPrivateProfileCharEx(sec_tong, key_escSpec, file_channcfg,
                                           defCHANN_TONG);
  m_chChannSCRN = gGetPrivateProfileCharEx(sec_screen, key_escSpec,
                                           file_channcfg, defCHANN_SCRN);
  m_chChannBC = gGetPrivateProfileCharEx(sec_broadcast, key_escSpec,
                                         file_channcfg, defCHANN_BC);
  ...
  m_minKimID = gGetPrivateProfileIntEx(sec_kim, key_minID, file_channcfg, 0);
  m_maxKimID = gGetPrivateProfileIntEx(sec_kim, key_maxID, file_channcfg, -1);
  m_minFacID =
      gGetPrivateProfileIntEx(sec_faction, key_minID, file_channcfg, 0);
  m_maxFacID =
      gGetPrivateProfileIntEx(sec_faction, key_maxID, file_channcfg, -1);
  ...
  m_defCost =
      (BYTE)gGetPrivateProfileIntEx(sec_system, key_defCost, file_channcfg, 0);

  m_costTeam = (BYTE)gGetPrivateProfileIntEx(sec_team, key_cost, file_channcfg,
                                             (INT)(UINT)m_defCost);
  m_costKim = (BYTE)gGetPrivateProfileIntEx(sec_kim, key_cost, file_channcfg,
                                            (INT)(UINT)m_defCost);
  m_costFac = (BYTE)gGetPrivateProfileIntEx(
      sec_faction, key_cost, file_channcfg, (INT)(UINT)m_defCost);
  m_costTong = (BYTE)gGetPrivateProfileIntEx(sec_tong, key_cost, file_channcfg,
                                             (INT)(UINT)m_defCost);
  m_costScrn = (BYTE)gGetPrivateProfileIntEx(
      sec_screen, key_cost, file_channcfg, (INT)(UINT)m_defCost);
  m_costBc = (BYTE)gGetPrivateProfileIntEx(sec_broadcast, key_cost,
                                           file_channcfg, (INT)(UINT)m_defCost);

  std::_tstring nameChannGM =
      gGetPrivateProfileStringEx(sec_system, key_nameGM, file_channcfg);
  if (!nameChannGM.empty()) {
    ...
    rChannInfo.cost = (BYTE)gGetPrivateProfileIntEx(
        nameChannGM.c_str(), key_cost, file_relaychann, (INT)(UINT)m_defCost);

    rTRACE("Create Channel: [Stock, GM] <%08X> %s", m_channidGM,
           nameChannGM.c_str());
  }

  const std::vector<std::_tstring> &rVecChann =
      gGetPrivateProfileSectionNamesEx(file_relaychann);

  for (std::vector<std::_tstring>::const_iterator it = rVecChann.begin();
       it != rVecChann.end(); it++) {
    ...
    rChannInfo.gmsub = FALSE;
    rChannInfo.cost = (BYTE)gGetPrivateProfileIntEx(
        channname.c_str(), key_cost, file_relaychann, (INT)(UINT)m_defCost);
...
DWORD CChannelMgr::GenChannID() {
  ...
  DWORD id = 0;
  for (CHANNID2INFOMAP::iterator it = m_mapChannid2Info.begin();
       it != m_mapChannid2Info.end(); it++) {
    if ((*it).first != id)
      return id;
    id++;
  }
...
DWORD CChannelMgr::GetChannelID(const std::_tstring &channname, DWORD advIP) {
  std::_tstring advChannName =
      advIP == 0 ? channname : MakeChannelName(channname, advIP);
...
BOOL CChannelMgr::GmQueryChannelID(const std::_tstring &channel, BOOL force) {
  ...
  if (it != m_mapChann2ID.end()) { // channel exist
...

  {
    {
      size_t pckgsize =
          sizeof(RELAY_ASKWAY_DATA) + sizeof(PLAYERCOMM_NOTIFYCHANNELID);
      ...
      PLAYERCOMM_NOTIFYCHANNELID *pPlayerCommNotiChannID =
          (PLAYERCOMM_NOTIFYCHANNELID *)(pAskWayData + 1);
...
BOOL CChannelMgr::GmFreeChannID(DWORD channid) {
  ...
  if (!rChannInfo.stock && rChannInfo.mapIp2Players.empty()) {
    rTRACE("GM Destroy Channel: <%08X> %s", channid,
           rChannInfo.channname.c_str());
...
std::_tstring CChannelMgr::MakeChannelName(const std::_tstring &chann,
                                           DWORD ip) {
  if (chann.empty())
    return NULL_STR;
  ...
  if (chann[1] == m_chChannTEAM || chann[1] == m_chChannKIM ||
      chann[1] == m_chChannFAC || chann[1] == m_chChannTONG) {
    if (chann.size() <= 2)
      return NULL_STR;
    ...
    return channname;
  } else if (chann[1] == m_chChannSCRN || chann[1] == m_chChannBC) {
    if (chann.size() > 2)
      return NULL_STR;
...
std::_tstring CChannelMgr::ReduceChannelName(const std::_tstring &chann) {
  ...
  if (chann[1] == m_chChannKIM) {
    int spltpos = chann.find_last_of(m_chChannSplt);
    if (spltpos == -1)
      return NULL_STR;
    return std::_tstring(chann.c_str(), spltpos);
  }
  if (chann[1] == m_chChannFAC || chann[1] == m_chChannTONG) {
    return chann;
  }
  if (chann[1] == m_chChannSCRN || chann[1] == m_chChannBC) {
...
BYTE CChannelMgr::GainPreCost(const std::_tstring &channel) {
  if (channel.size() >= 2 && channel[0] == m_chChannEsc) {
    if (channel[1] == m_chChannTEAM)
      return m_costTeam;
    else if (channel[1] == m_chChannKIM)
      return m_costKim;
...
DWORD CChannelMgr::GetGMChannID() { return m_channidGM; }
...
BOOL CChannelMgr::PassToSpecMan(DWORD ToIP, unsigned long ToRelayID,
                                DWORD ToNameID, const void *pData,
                                size_t size) {
...
BOOL CChannelMgr::PassToGroupMan(DWORD ToIP, BOOL verify, const MANLIST &mans,
...
BOOL CChannelMgr::IsValidKimID(DWORD idKim) {
  if (m_minKimID > m_maxKimID)
    return TRUE;

  return idKim >= m_minKimID && idKim <= m_maxKimID;
}
...
BOOL CChannelMgr::SomeoneChat(DWORD srcIP, const tagPlusSrcInfo &SrcInfo,
                              const CHAT_SOMEONECHAT_CMD *pSomeoneChatCmd) {
  ...
  if (!g_HostServer.FindPlayerByIpParam(NULL, srcIP, SrcInfo.lnID, NULL, NULL,
                                        &srcrole, NULL))
    return FALSE;
  ...
  if (g_HostServer.FindPlayerByRole(NULL,
                                    std::_tstring(pSomeoneChatCmd->someone),
                                    &hostconndup, NULL, &nameid, &param)) {
    size_t syncsize = sizeof(CHAT_SOMEONECHAT_SYNC) + pSomeoneChatCmd->sentlen;
    CHAT_SOMEONECHAT_SYNC *pCscSync =
        (CHAT_SOMEONECHAT_SYNC *)_alloca(syncsize);
  ...
  {
    {
      size_t _exsize = sizeof(DWORD) + _NAME_LEN + sizeof(BYTE) +
                       pSomeoneChatCmd->sentlen; ////X
...
BOOL CChannelMgr::ChannelChat(DWORD srcIP, const tagPlusSrcInfo &SrcInfo,
                              const CHAT_CHANNELCHAT_CMD *pChannelChatCmd) {
  ...
  std::_tstring srcacc;
  std::_tstring srcrole;
  if (!g_HostServer.FindPlayerByIpParam(NULL, srcIP, SrcInfo.lnID, NULL,
                                        &srcacc, &srcrole, NULL))
    return FALSE;
  ...
  CHANNID2INFOMAP::iterator itChannid =
      m_mapChannid2Info.find(pChannelChatCmd->channelid);
  if (itChannid == m_mapChannid2Info.end())
    return FALSE;
  ...
  if (rChannInfo.god) {
    ...
    if (IsNeedRelegate(rChannInfo.channname,
                       pChannelChatCmd->filter)) { // relegate to gamesvr
      ...
      size_t syncsize =
          sizeof(CHAT_CHANNELCHAT_SYNC) + pChannelChatCmd->sentlen;
      ...
      pCccSync->figure = pChannelChatCmd->figure;
      ...
      } else if (rChannInfo.channname[1] == m_chChannKIM) {
        if (theID == -1 || channIP == 0)
          return FALSE;
    ...
    } else { // broad on channel
      //== PassToPlayer
      // do like this for optimize

      size_t syncsize =
          sizeof(CHAT_CHANNELCHAT_SYNC) + pChannelChatCmd->sentlen;
      CHAT_CHANNELCHAT_SYNC *pCccSync =
          (CHAT_CHANNELCHAT_SYNC *)_alloca(syncsize);
      pCccSync->ProtocolType = chat_channelchat;
      pCccSync->wSize = syncsize - 1;
      pCccSync->packageID = pChannelChatCmd->packageID;
      strcpy(pCccSync->someone, srcrole.c_str());
      pCccSync->channelid = pChannelChatCmd->channelid;
      pCccSync->figure = pChannelChatCmd->figure;
      // rTRACE("Test %d",pChannelChatCmd->figure);
      pCccSync->sentlen = pChannelChatCmd->sentlen;
      memcpy(pCccSync + 1, pChannelChatCmd + 1, pChannelChatCmd->sentlen);

      for (IP2PLAYERSMAP::iterator itIP = rChannInfo.mapIp2Players.begin();
           itIP != rChannInfo.mapIp2Players.end(); itIP++) {
        DWORD IP = (*itIP).first;
        PLAYERSMAP &rPlayers = (*itIP).second;

        MANLIST mans;

        for (PLAYERSMAP::iterator itPlayer = rPlayers.begin();
             itPlayer != rPlayers.end(); itPlayer++) {
...
BOOL CChannelMgr::QueryChannelID(const std::_tstring &channel, DWORD ip,
                                 unsigned long param, DWORD nameid) {
  assert(FALSE);
  return FALSE;
...
DWORD CChannelMgr::DoBlockOp(DWORD lmt) {
  ...
  for (IP2OPSMAP::iterator itIP = m_mapIp2Ops.begin();
       itIP != m_mapIp2Ops.end(); itIP++) {
    DWORD ip = (*itIP).first;
    BLOCKOPLIST &rOps = (*itIP).second;

    for (BLOCKOPLIST::iterator itOp = rOps.begin(); itOp != rOps.end();
         itOp++) {
      ...
      case _BLOCKCHANNELOP::op_querychannelid: {
        {
          ...
          Block_QueryChannelID(pTheOp->channelname, ip, pTheOp->param,
                               pTheOp->nameid);
...
BOOL CChannelMgr::Block_QueryChannelID(const std::_tstring &channel, DWORD ip,
                                       unsigned long param, DWORD nameid) {
  ...
  if (!g_HostServer.FindPlayerByIpParam(NULL, ip, param, &conndup, NULL,
                                        &rolename, NULL))
    return FALSE;
  ...
  {
    {
      ...
      size_t pckgsize = sizeof(RELAY_ASKWAY_DATA) + methodsize +
                        sizeof(PLAYERCOMM_NOTIFYCHANNELID);
      ...
      PLAYERCOMM_NOTIFYCHANNELID *pPlayerCommNID =
          (PLAYERCOMM_NOTIFYCHANNELID *)(pRoleInfo + 2);
...
BOOL CChannelMgr::Block_Subscribe(DWORD ip, unsigned long param,
                                  DWORD channelid) {
  if (channelid == -1)
    return FALSE;
...
BOOL CChannelMgr::Block_Unsubscribe(DWORD ip, unsigned long param,
                                    DWORD channelid) {
  if (channelid == m_channidGM)
    return FALSE;
...
BOOL CChannelMgr::Block_FreeChannID(DWORD channid, DWORD ip,
                                    unsigned long param) {
  if (channid == -1)
    return FALSE;
  ...
  if (rPlayers.empty()) {
    ...
    if (!rChannInfo.stock && !rChannInfo.gmuse &&
      ...
      rTRACE("Destroy Channel: <%08X> %s", channid,
             rChannInfo.channname.c_str());
...
BOOL CChannelMgr::B_QueryChannelID(const std::_tstring &channel, DWORD ip,
                                   unsigned long param, DWORD nameid) {
...
BOOL CChannelMgr::B_Unsubscribe(DWORD ip, unsigned long param,
                                DWORD channelid) {
...
BOOL CChannelMgr::SayOnSomeone(DWORD ip, unsigned long param, DWORD nameid,
                               const std::_tstring &name,
                               const std::_tstring &sent) {
  CNetConnectDup hostconndup;
  if (!g_HostServer.FindPlayerByIpParam(NULL, ip, param, &hostconndup, NULL,
                                        NULL, NULL))
    return FALSE;
...
BOOL CChannelMgr::SayOnChannel(DWORD channid, BOOL filter,
                               const std::_tstring &ids,
                               const std::_tstring &name,
                               const std::_tstring &sent) {
  DUMMY_AUTOLOCKREAD(m_lockChannel);
  ...
  if (rChannInfo.god) {
    ...
    if (IsNeedRelegate(rChannInfo.channname, filter)) { // relegate to gamesvr
      assert(rChannInfo.channname[0] == m_chChannEsc);
