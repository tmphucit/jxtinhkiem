...
CNetClient::CNetClient(CNetCenter *pNetCenter, BOOL bAutoFree)
    : m_pClient(NULL), m_ready(FALSE), m_pNetCenter(pNetCenter),
      m_bAutoFree(bAutoFree), m_SvrIP(0), m_SvrPort(0), m_hOver(NULL),
      m_bReqDisconn(FALSE), m_shutdown(0L) {
  assert(m_pNetCenter);
}
...
BOOL CNetClient::Startup(size_t maxFreeBuffers, size_t bufferSize,
                         const char *pAddressToConnectServer,
  ...
  if (FAILED(g_libRainbow.CreateInterface(
...
BOOL CNetClient::Route() { return DoRoute(); }
