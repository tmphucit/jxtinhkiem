...
public:
  BOOL B_QueryChannelID(const std::_tstring &channel, DWORD ip,
  ...
  BOOL QueryChannelID(const std::_tstring &channel, DWORD ip,
                      unsigned long param, DWORD nameid);
...
private:
  ...
  BOOL PassToSpecMan(DWORD ToIP, unsigned long ToRelayID, DWORD ToNameID,
                     const void *pData, size_t size);

  typedef std::list<tagPlusSrcInfo> MANLIST;
  BOOL PassToGroupMan(DWORD ToIP, BOOL verify, const MANLIST &mans,
                      const void *pData, size_t size);

private:
  BOOL IsValidTeamID(DWORD idTeam);
  BOOL IsValidKimID(DWORD idKim);
...
public:
  BOOL SomeoneChat(DWORD srcIP, const tagPlusSrcInfo &SrcInfo,
                   const CHAT_SOMEONECHAT_CMD *pSomeoneChatCmd);
  BOOL ChannelChat(DWORD srcIP, const tagPlusSrcInfo &SrcInfo,
                   const CHAT_CHANNELCHAT_CMD *pChannelChatCmd);

public:
  BOOL SayOnSomeone(DWORD ip, unsigned long param, DWORD nameid,
                    const std::_tstring &name, const std::_tstring &sent);
  BOOL SayOnChannel(DWORD channid, BOOL filter, const std::_tstring &ids,
                    const std::_tstring &name, const std::_tstring &sent);
...
private:
  BOOL Block_QueryChannelID(const std::_tstring &channel, DWORD ip,
                            unsigned long param, DWORD nameid);
