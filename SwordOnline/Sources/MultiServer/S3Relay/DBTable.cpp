...
bool ZDBTable::open() {
  ...
  if (!db_create(&primary_db, dbenv, 0)) {
    if (!primary_db->open(primary_db, NULL, table_name, NULL, DB_BTREE,
                          DB_CREATE | DB_AUTO_COMMIT, 0664)) { // 打开主数据库
      for (index = 0; index < index_number; index++) {
        ...
        if (!db_create(&index_db[index], dbenv, 0)) {
          if (!is_index_unique[index]) {
            if (index_db[index]->set_flags(index_db[index],
                                           DB_DUP | DB_DUPSORT))
              break;
          }
          if (index_db[index]->open(index_db[index], NULL, index_table_name,
                                    NULL, DB_BTREE, DB_CREATE | DB_AUTO_COMMIT,
                                    0664))
            break;
          if (ret = primary_db->associate(primary_db, NULL, index_db[index],
                                          get_index_funcs[index],
                                          DB_AUTO_COMMIT)) {
...
bool ZDBTable::add(const char *key_ptr, int key_size, const char *data_ptr,
                   int data_size) {
  DBT data, key;
...
char *ZDBTable::_search(bool bKey, const char *key_ptr, int key_size, int &size,
                        int index) {
  dbcp = NULL;
  DBT key, data, pkey;
...
char *ZDBTable::GetRecord(int &size, CursorPointer cpMode,
                          int index) { // 取得按游标某一个数据
  if (!dbcp) {                         // 如果数据库指针没有初始化，先初始化dbcp
    if (index_db[index]->cursor(index_db[index], NULL, &dbcp,
                                0)) { // 初始化dbcp失败
      dbcp = NULL;
      return NULL;
    }
...
bool ZDBTable::GetRecordEx(char *aBuffer, int &size, char *aKeyBuffer,
                           int &keysize, CursorPointer cpMode,
                           int index) { // 取得按游标某一个数据(新版函数)
  if (!dbcp) { // 如果数据库指针没有初始化，先初始化dbcp
    if (index_db[index]->cursor(index_db[index], NULL, &dbcp,
                                0)) { // 初始化dbcp失败
      dbcp = NULL;
...
char *CDBTableReadOnly::_search(bool bKey, const char *key_ptr, int key_size,
                                int &size) {
  dbcp = NULL;
  DBT key, data, pkey;
  memset(&key, 0, sizeof(key));
...
char *CDBTableReadOnly::GetRecord(int &size, CursorPointer cpMode) {
  if (!dbcp) { // 如果数据库指针没有初始化，先初始化dbcp
    if (primary_db->cursor(primary_db, NULL, &dbcp, 0)) { // 初始化dbcp失败
      dbcp = NULL;
      return NULL;
    }
  }
...
bool CDBTableReadOnly::GetRecordEx(
    char *aBuffer, int &size, char *aKeyBuffer, int &keysize,
    CursorPointer cpMode) { // 取得按游标某一个数据(新版函数)
  if (!dbcp) {              // 如果数据库指针没有初始化，先初始化dbcp
    if (primary_db->cursor(primary_db, NULL, &dbcp, 0)) { // 初始化dbcp失败
      dbcp = NULL;
      return false;
