...
void BroadGlobal(const void *pData, size_t size, DWORD nFromIP,
                 DWORD nFromRelayID) {
...
void BroadGame(DWORD nToIP, const void *pData, size_t size, DWORD nFromIP,
               DWORD nFromRelayID) {
...
TLua_Funcs GameScriptFuns[] = {
    {"GlobalExecute",
     LuaGlobalExecute},              // GlobalExecute通知整个服务组执行一个脚本
...
BOOL InitScript() {
  if (gs_Script.Init() &&
      gs_Script.RegisterFunctions(GameScriptFuns, g_GetGameScriptFunNum())) {
    return TRUE;
  }
