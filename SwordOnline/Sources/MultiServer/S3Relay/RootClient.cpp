...
void CRootClient::OnServerEventCreate() {
  ...
  strncpy(serlogin.Account, g_rootloginname.c_str(),
  ...
  // strncpy(serlogin.Password, g_rootloginpswd.c_str(),
  // LOGIN_USER_PASSWORD_MAX_LEN);
...
BOOL CRootClient::IsStartAutoTime() { return m_isStartAutoTime; }
...
void CRootClient::EndRoute() { AutoTime(); }
...
void CRootClient::Proc2_Relay_Data_Here(const void *pData, size_t size) {
  ...
  case (MAKEWORD(pf_gamemaster, gm_c2s_execute)): {
    ...
    memcpy(szScriptAction, ((const char *)pGMData) + sizeof(GM_EXECUTE_COMMAND),
           min(MAX_PATH - 1, pGMData->wLength));
  ...
  case (MAKEWORD(pf_gamemaster, gm_c2s_getrolelist)):
    _ASSERT(pRelayData->routeDateLength == sizeof(GM_GET_ROLE_LIST_CMD));
    {
      ...
      strncpy((char *)(pNewData + 1), pGMData->AccountName,
              sizeof(pGMData->AccountName));
...
void CRootClient::Proc1_Relay_Data(const void *pData, size_t size) {
  ...
  if (pRelayData->nToIP == 0) {
    ...
    return;
  } else if (pRelayData->nToIP == INADDR_BROADCAST) { // broadcast to gamesvrs
    pRelayData->nToIP = 0;
    g_HostServer.BroadPackage(pRelayData, size);
    return;
  } else if (pRelayData->nToIP == gGetHostIP(global_adapt)) { // relay to host
    pRelayData->nToIP = 0;
  ...
  gFillLosewayPckg(pRelayData->nFromIP, pRelayData->nFromRelayID, pRelayData,
                   size, pLoseDataDup, sizeLoseData);
...
void CRootClient::Proc1_Relay_LoseWay(const void *pData, size_t size) {
  ...
  if (pLoseData->nToIP == 0) { // my data
    //...
    return;
  } else if (pLoseData->nToIP == INADDR_BROADCAST) { // broadcast to gamesvrs
    assert(FALSE);
    return;
  } else if (pLoseData->nToIP == gGetHostIP(global_adapt)) { // relay to host
    pLoseData->nToIP = 0;
...
void CRootClient::Proc2_Relay_AskWay_AccountRoleID(const void *pData,
  ...
  if (acc ? g_HostServer.FindPlayerByAcc(NULL, strName, &conndup, NULL, &nameid,
                                         &param)
          : g_HostServer.FindPlayerByRole(NULL, strName, &conndup, NULL,
                                          &nameid, &param)) { // send to target
  ...
  gFillLosewayPckg(pAskWayData->nFromIP, pAskWayData->nFromRelayID, pAskWayData,
                   size, pLoseDataDup, sizeLoseData);
...
void CRootClient::Proc0_PlayerCommunity(const void *pData, size_t size) {
  ...
  if (pHeader->ProtocolID == playercomm_s2c_gmquerychannelid) {
    PLAYERCOMM_GMQUERYCHANNELID *pPlayerCommGmQID =
        (PLAYERCOMM_GMQUERYCHANNELID *)pData;

    g_ChannelMgr.GmQueryChannelID(pPlayerCommGmQID->channel,
                                  pPlayerCommGmQID->force);
  } else if (pHeader->ProtocolID == playercomm_s2c_gmfreechannelid) {
    PLAYERCOMM_GMFREECHANNELID *pPlayerCommGmFID =
        (PLAYERCOMM_GMFREECHANNELID *)pData;
