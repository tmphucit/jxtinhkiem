...
public:
  ...
  BOOL SomeoneSyncFriends(const std::string &role, unsigned long param,
                          DWORD nameid, const CNetConnectDup &conndup,
                          BOOL full, BYTE pckgid);
  ...
  BOOL PlayerIsFriend(const std::_tstring &someone, const std::_tstring &dst,
                      BOOL rev = FALSE);

  BOOL SetFriendGroup(const std::_tstring &someone, const std::_tstring &dst,
                      const std::_tstring &group);
  std::_tstring GetFriendGroup(const std::_tstring &someone,
  ...
  struct FRIENDINFO {
    ...
    enum {
      ...
      cheatTRUE = 1
        ...
        orig_cheated; // is valid when !-1 && loaded

    FRIENDINFO()
        : cheating(true), pCheated(NULL), orig_cheating(cheatNONE),
          orig_cheated(cheatNONE) {}
  };
...
private:
  struct ADDFRIENDKEY {
    ...
    bool operator<(const ADDFRIENDKEY &s) const {
      return pckgid < s.pckgid ||
             (pckgid == s.pckgid && _tstring_less()(src, s.src));
    }
  };
...
public:
  BOOL TrackAddFriend(const std::_tstring &src, const std::_tstring &dst,
                      BYTE id);
  BOOL ExtractAddFriend(const std::_tstring &src, const std::_tstring &dst,
                        BYTE id);
  BOOL TidyAddFriend();
...
private:
  BOOL DB_AddSomeone(const std::_tstring &someone,
                     const MEM_FRIENDRECORDLIST &memFriends);
  BOOL DB_DeleteSomeone(const std::_tstring &someone);
  BOOL DB_LoadSomeone(const std::_tstring &someone,
                      MEM_FRIENDRECORDLIST *pmemFriends);
  BOOL DB_StoreSomeone(const std::_tstring &someone,
                       const MEM_FRIENDRECORDLIST &memFriends);
...
public:
  BOOL PlayerAssociate(const std::_tstring &someone, const std::_tstring &dst,
