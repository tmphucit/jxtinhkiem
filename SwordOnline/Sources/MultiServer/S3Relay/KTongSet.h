...
public:
  ...
  BOOL SearchOne(DWORD dwTongNameID, char *lpszName,
                 STONG_ONE_LEADER_INFO *pInfo);

  // 创建一个帮会
  int Create(int nCamp, char *lpszPlayerName, char *lpszTongName, int nSex);
  // 添加一个帮会成员，if return == 0 成功 else return error id
  int AddMember(char *lpszPlayerName, char *lpszTongName, int nSex);
  // 获得帮会阵营
  ...
  BOOL GetTongManagerInfo(STONG_GET_MANAGER_INFO_COMMAND *pApply,
                          STONG_MANAGER_INFO_SYNC *pInfo);

  BOOL GetTongMemberInfo(STONG_GET_MEMBER_INFO_COMMAND *pApply,
  ...
  BOOL ChangeTitleFemale(STONG_CHANGE_TITLE_FEMALE_COMMAND *pChange,
                         STONG_CHANGE_TITLE_FEMALE_SYNC *pSync);

  BOOL ChangeTitleMale(STONG_CHANGE_TITLE_MALE_COMMAND *pChange,
                       STONG_CHANGE_TITLE_MALE_SYNC *pSync);

  BOOL ChangeTitle(STONG_CHANGE_TITLE_COMMAND *pChange,
  ...
  BOOL GetLoginData(STONG_GET_LOGIN_DATA_COMMAND *pLogin,
