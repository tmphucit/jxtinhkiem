  ...
  */
  ...
  struct PLAYERVALUE {
    ...
    bool operator<(const PLAYERVALUE &src) const {
      return index < src.index && nameid < src.nameid && role < src.role &&
             acc < src.acc;
    }
  };
...
public:
  BOOL Login(const std::_tstring &acc, const std::_tstring &role, DWORD nameid,
             unsigned long conn, int idx);
