...
CGatewayClient::~CGatewayClient() { assert(m_mapAskWayMaps.empty()); }
  ...
  {
    {
      ...
      // clearup
      for (ID2ASKWAYMAPMAP::iterator it = m_mapAskWayMaps.begin();
           it != m_mapAskWayMaps.end(); it++) {
        ASKWAYMAP &rAskWayMap = (*it).second;
        size_t sizePckg = sizeof(RELAY_ASKWAY_DATA) +
                          rAskWayMap.pAskWayMap->wMethodDataLength +
                          rAskWayMap.pAskWayMap->routeDateLength;
        ...
        if (sizeLoseData <= max_packagesize) {
          CNetConnectDup conndup =
              g_HostServer.FindHostConnectByIP(rAskWayMap.fromIP);
          if (conndup.IsValid()) {
            size_t sizeLoseData = sizeof(RELAY_DATA) + sizePckg;
            RELAY_DATA *pLoseDataDup = (RELAY_DATA *)buffer;

            gFillLosewayPckg(0, 0, rAskWayMap.pAskWayMap, sizePckg,
                             pLoseDataDup, sizeLoseData);
...
void CGatewayClient::Proc0_NotifySvrIP(const void *pData, size_t size) {
  ...
  if (!passed) { // lose
    ...
    if (conndup2.IsValid()) {
      ...
      gFillLosewayPckg(0, 0, rAskWayMap.pAskWayMap, sizePckg, pLoseDataDup,
                       sizeLoseData);
...
BOOL CGatewayClient::PassAskWayMap(DWORD fromIP,
                                   const RELAY_ASKWAY_DATA *pAskWayMap) {
  assert(pAskWayMap && pAskWayMap->seekMethod == rm_map_id);
  size_t size = sizeof(RELAY_ASKWAY_DATA) + pAskWayMap->wMethodDataLength +
                pAskWayMap->routeDateLength;
  ...
  if (IsReady()) {
    if (m_mapAskWayMaps.size() < MAX_PACKAGECOUNT) {
      for (;;) {
        std::pair<ID2ASKWAYMAPMAP::iterator, bool> insret =
            m_mapAskWayMaps.insert(
                ID2ASKWAYMAPMAP::value_type(++m_lastPckgID, ASKWAYMAP()));
        if (insret.second) { // insert, not existed
  ...
  {
    {
      ...
      if (conndup.IsValid()) {
        ...
        gFillLosewayPckg(0, 0, pAskWayMap, size, pLoseDataDup, sizeLoseData);
