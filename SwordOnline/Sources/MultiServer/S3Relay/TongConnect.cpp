...
int g_nTongPSSize[defTONG_PROTOCOL_SERVER_NUM] = {
    ...
    sizeof(STONG_CHANGE_TITLE_FEMALE_COMMAND),
    sizeof(STONG_CHANGE_TITLE_MALE_COMMAND),
    sizeof(STONG_CHANGE_TITLE_COMMAND),
};
...
BOOL CTongConnect::PassToSomeone(DWORD ip, unsigned long id, DWORD nameid,
...
void CTongConnect::Proc0_Tong(const void *pData, size_t size) {
  ...
  case enumC2S_TONG_CREATE: {
    ...
    int nSex = pTongCreate->m_nSex;

    memcpy(szTongName, &pTongCreate->m_szBuffer[0],
           pTongCreate->m_btTongNameLength);
    szTongName[pTongCreate->m_btTongNameLength] = 0;
    memcpy(szPlayerName,
           &pTongCreate->m_szBuffer[pTongCreate->m_btTongNameLength],
           pTongCreate->m_btPlayerNameLength);
    szPlayerName[pTongCreate->m_btPlayerNameLength] = 0;

    // 建立帮会
    int nErrorID = g_cTongSet.Create(nCamp, szPlayerName, szTongName, nSex);
    if (nErrorID == 0) {
      ...
      memcpy(sSync.m_szTongName, pTongCreate->m_szBuffer,
             sSync.m_btTongNameLength);
      sSync.m_wLength = sizeof(STONG_CREATE_SUCCESS_SYNC) -
                        sizeof(sSync.m_szTongName) + sSync.m_btTongNameLength;
  ...
  case enumC2S_TONG_ADD_MEMBER: {
    ...
    memcpy(szPlayerName, &pAdd->m_szBuffer[pAdd->m_btTongNameLength],
           pAdd->m_btPlayerNameLength);
    szPlayerName[pAdd->m_btPlayerNameLength] = 0;

    int nRet = g_cTongSet.AddMember(szPlayerName, szTongName, pAdd->m_nSex);
    // 成员添加成功
    if (nRet >= 0) {
    ...
    // 成员添加不成功
      ...
      this->SendPackage((const void *)&sSync,
                        sizeof(STONG_ADD_MEMBER_FAIL_SYNC));
    }
  } break;
  // 获得帮会信息
  case enumC2S_TONG_GET_HEAD_INFO: {
    STONG_GET_TONG_HEAD_INFO_COMMAND *pGet =
        (STONG_GET_TONG_HEAD_INFO_COMMAND *)pData;
  ...
  case enumC2S_TONG_GET_MANAGER_INFO: {
    STONG_GET_MANAGER_INFO_COMMAND *pGet =
        (STONG_GET_MANAGER_INFO_COMMAND *)pData;
    STONG_MANAGER_INFO_SYNC sInfo;
    if (g_cTongSet.GetTongManagerInfo(pGet, &sInfo))
      this->SendPackage((const void *)&sInfo, sInfo.m_wLength);
  } break;
  case enumC2S_TONG_GET_MEMBER_INFO: {
    STONG_GET_MEMBER_INFO_COMMAND *pGet =
        (STONG_GET_MEMBER_INFO_COMMAND *)pData;
  ...
  case enumC2S_TONG_GET_LOGIN_DATA: {
    STONG_GET_LOGIN_DATA_COMMAND *pLogin =
        (STONG_GET_LOGIN_DATA_COMMAND *)pData;
  ...
  case enumC2S_TONG_CHANGE_TITLE_FEMALE: {
    STONG_CHANGE_TITLE_FEMALE_COMMAND *pChange =
        (STONG_CHANGE_TITLE_FEMALE_COMMAND *)pData;
    STONG_CHANGE_TITLE_FEMALE_SYNC sChange;
    sChange.m_dwParam = 0;

    if (g_cTongSet.ChangeTitleFemale(pChange, &sChange) && sChange.m_dwParam)
      this->SendPackage((const void *)&sChange, sizeof(sChange));
  } break;

  case enumC2S_TONG_CHANGE_TITLE_MALE: {
    STONG_CHANGE_TITLE_MALE_COMMAND *pChange =
        (STONG_CHANGE_TITLE_MALE_COMMAND *)pData;
    STONG_CHANGE_TITLE_MALE_SYNC sChange;
    sChange.m_dwParam = 0;

    if (g_cTongSet.ChangeTitleMale(pChange, &sChange) && sChange.m_dwParam)
      this->SendPackage((const void *)&sChange, sizeof(sChange));
  } break;

  case enumC2S_TONG_CHANGE_TITLE: {
    STONG_CHANGE_TITLE_COMMAND *pChange = (STONG_CHANGE_TITLE_COMMAND *)pData;
    STONG_CHANGE_TITLE_SYNC sChange;
    sChange.m_dwParam = 0;

    if (g_cTongSet.ChangeTitle(pChange, &sChange) && sChange.m_dwParam)
      this->SendPackage((const void *)&sChange, sizeof(sChange));
  } break;
...
void CTongConnect::Proc1_Friend_AskAddFriend(const void *pData, size_t size) {
  ...
  if (!g_HostServer.FindPlayerByIpParam(NULL, GetIP(), pSrcInfo->lnID, NULL,
                                        NULL, &srcrole, NULL))
    return;
  ...
  if (g_HostServer.FindPlayerByRole(NULL, dstrole, &dsthostconndup, NULL,
                                    &dstnameid, &dstparam)) {
    if (g_FriendMgr.PlayerIsFriend(dstrole, srcrole)) {
      ...
      if (g_FriendMgr.TrackAddFriend(srcrole, dstrole, pAafCmd->pckgid)) {
        ...
        if (PassToSomeone(dsthostconndup.GetIP(), dstparam, dstnameid, &AafSync,
                          sizeof(ASK_ADDFRIEND_SYNC)))
          pass = TRUE;
        else
  ...
  if (!pass) {
    ...
    PassToSomeone(GetIP(), pSrcInfo->lnID, pSrcInfo->nameid, &RafSync,
                  sizeof(REP_ADDFRIEND_SYNC));
  }
}
  ...
  {
    {
      ...
      if (g_HostServer.FindPlayerByRole(NULL, dstrole, &dsthostconndup, NULL,
        ...
        PassToSomeone(dsthostconndup.GetIP(), dstparam, dstnameid, &RafSync,
                      sizeof(REP_ADDFRIEND_SYNC));
      } else {
        FRIEND_STATE Fs;
        ...
        PassToSomeone(GetIP(), pSrcInfo->lnID, pSrcInfo->nameid, &Fs,
                      sizeof(FRIEND_STATE));
      }
    }
  }
...
void CTongConnect::Proc1_Friend_AskSyncFriendList(const void *pData,
                                                  size_t size) {
  ...
  g_FriendMgr.SomeoneSyncFriends(role, pSrcInfo->lnID, pSrcInfo->nameid,
                                 CNetConnectDup(*this), pAsfl->full,
                                 pAsfl->pckgid);
}
...
void CTongConnect::Proc1_Friend_Associate(const void *pData, size_t size) {
  ...
  size_t maxsize =
      basesize + (pFa->bidir ? max(role1size, role2size) : role2size) + 1;
  ...
  {
    {
      ...
      DWORD nameid = 0;
      unsigned long param = 0;
      if (g_HostServer.FindPlayerByRole(NULL, role1, &conndup, NULL, &nameid,
                                        &param)) {
  ...
  if (pFa->bidir) {
    ...
    if (g_HostServer.FindPlayerByRole(NULL, role2, &conndup2, NULL, &nameid2,
                                      &param2)) {
  ...
  {
    {
      ...
      for (CFriendMgr::_BEVY::iterator itS = bevy.begin(); itS != bevy.end();
           itS++) {
        ...
        DWORD nameid = 0;
        unsigned long param = 0;
        if (!g_HostServer.FindPlayerByRole(NULL, someone, &conndup, NULL,
                                           &nameid, &param))
          continue;

        size_t cursor = 0;
        for (CFriendMgr::_BEVY::iterator itD = bevy.begin(); itD != bevy.end();
             itD++) {
          if (itD == itS)
            continue;
