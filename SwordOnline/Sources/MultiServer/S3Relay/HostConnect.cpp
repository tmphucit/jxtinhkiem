...
void CHostConnect::Proc1_Normal_EnterGame(const void *pData, size_t size) {
  ...
  rTRACE("player login: %s [%s] (%08X, %08X)", strAcc.c_str(), strRole.c_str(),
         GetIP(), pEnterGame2->lnID);
}
...
void CHostConnect::Proc1_Relay_Data(const void *pData, size_t size) {
  ...
  if (pRelayData->nToIP == 0) {
    ...
    if (g_HostServer.IsConnectReady(pRelayData->nToRelayID)) {
      CNetConnectDup conndup =
          g_HostServer.FindNetConnect(pRelayData->nToRelayID);
      if (conndup.IsValid()) {
    ...
    if (pRelayData->nToRelayID == 0 || !g_RootClient.IsReady()) { // local
  ...
  gFillLosewayPckg(0, GetID(), pRelayData, size, pLoseDataDup, sizeLoseData);
...
void CHostConnect::Proc1_Relay_LoseWay(const void *pData, size_t size) {
  ...
  if (pLoseData->nToIP == 0) {
    ...
    if (g_HostServer.IsConnectReady(pLoseData->nToRelayID)) {
      CNetConnectDup conndup =
          g_HostServer.FindNetConnect(pLoseData->nToRelayID);
      if (conndup.IsValid()) {
...
void CHostConnect::Proc2_Relay_AskWay_AccountRoleID(const void *pData,
                                                    size_t size, BOOL acc) {
  RELAY_ASKWAY_DATA *pAskWayData = (RELAY_ASKWAY_DATA *)pData;
  ...
  if (acc ? FindPlayerByAcc(strName, NULL, &nameid, &param)
          : FindPlayerByRole(strName, NULL, &nameid,
                             &param)) { // the player is at this gamesvr
    conndup = *this;

    find = TRUE;
  } else if (acc ? g_HostServer.FindPlayerByAcc(this, strName, &conndup, NULL,
                                                &nameid, &param)
                 : g_HostServer.FindPlayerByRole(
                       this, strName, &conndup, NULL, &nameid,
                       &param)) { // the player is at other gamesvr in this
                                  // group
    find = TRUE;
  }
  ...
  gFillLosewayPckg(0, GetID(), pAskWayData, size, pLoseDataDup, sizeLoseData);
...
void CHostConnect::Proc2_Relay_AskWay_GM(const void *pData, size_t size) {
  ...
  gFillLosewayPckg(0, GetID(), pData, size, pLoseDataDup, sizeLoseData);
...
void CHostConnect::Proc1_PlayerCommunity_QueryChannelID(const void *pData,
                                                        size_t size) {
  if (size < sizeof(PLAYERCOMM_QUERYCHANNELID) + sizeof(tagPlusSrcInfo))
    return;

  PLAYERCOMM_QUERYCHANNELID *pPlayerCommQID =
  ...
  g_ChannelMgr.B_QueryChannelID(pPlayerCommQID->channel, GetIP(),
                                pSrcInfo->lnID, pSrcInfo->nameid);
}

void CHostConnect::Proc1_PlayerCommunity_FreeChannelID(const void *pData,
                                                       size_t size) {
  ...
  g_ChannelMgr.B_FreeChannID(pPlayerCommFID->channelid, GetIP(),
                             pSrcInfo->lnID);
}

void CHostConnect::Proc1_PlayerCommunity_Subscribe(const void *pData,
                                                   size_t size) {
  ...
  if (pPlayerCommSub->subscribe)
    g_ChannelMgr.B_Subscribe(GetIP(), pSrcInfo->lnID,
                             pPlayerCommSub->channelid);
  else
    g_ChannelMgr.B_Unsubscribe(GetIP(), pSrcInfo->lnID,
                               pPlayerCommSub->channelid);
}
...
void CHostConnect::OnClientConnectClose() {
  ...
  size_t pckgsize =
      sizeof(RELAY_ASKWAY_DATA) + sizeof(EXTEND_HEADER) + sizeof(KServerInfo);
...
BOOL CHostConnect::FindPlayerByAcc(const std::_tstring &acc,
                                   std::_tstring *pRole, DWORD *pNameID,
                                   unsigned long *pParam,
                                   std::_tstring *pRealAcc) {
...
BOOL CHostConnect::FindPlayerByRole(const std::_tstring &role,
                                    std::_tstring *pAcc, DWORD *pNameID,
                                    unsigned long *pParam,
                                    std::_tstring *pRealRole) {
  DUMMY_AUTOLOCKREAD(m_lockPlayer);
...
BOOL CHostConnect::FindPlayerByParam(unsigned long param, std::_tstring *pAcc,
                                     std::_tstring *pRole, DWORD *pNameID) {
  DUMMY_AUTOLOCKREAD(m_lockPlayer);
...
/*
BOOL CHostConnect::BroadOnPlayer(BOOL byAcc, DWORD fromIP, unsigned long
fromRelayID, const void* pData, size_t size)
{
        ...
        for (PARAMMAP::const_iterator it = m_mapParam.begin(); it !=
m_mapParam.end(); it++)
        {
                const PARAMINFO& infoParam = (*it).second;
                strcpy(pTheName, byAcc ? infoParam.acc.c_str() :
infoParam.role.c_str()); pTheInfo[0] = infoParam.nameID; pTheInfo[1] =
(*it).first;
...
void CHostConnect::UnprepareRecvs() { g_ChannelMgr.DoBlockOp(-1); }
