...
void CChatConnect::Proc0_GroupMan(const void *pData, size_t size) {
  ...
  if (pCgc->byHasIdentify) {
    ...
    for (size_t i = 0; i < playercount; i++) {
      if (g_ChannelMgr.IsSubscribed(srcIP, pPlayers[i].lnID,
                                    pCccSync->channelid)) {
    ...
    for (size_t i = 0; i < playercount; i++) {
      if (g_ChannelMgr.IsSubscribed(srcIP, (unsigned long)pPlayers[i],
                                    pCccSync->channelid)) {
        if (pCgc->wPlayerCount < i)
          pPlayers[pCgc->wPlayerCount] = pPlayers[i];
        ++pCgc->wPlayerCount;
  ...
  size_t size2 =
      size - (playercount - pCgc->wPlayerCount) *
                 (pCgc->byHasIdentify ? sizeof(tagPlusSrcInfo) : sizeof(WORD));
  pCgc->wSize = size2 - 1;
