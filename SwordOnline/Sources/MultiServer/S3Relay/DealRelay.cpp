...
void _Proc2_Relay_AskWay_AccountRoleID(const CNetSockDupEx &NetSockDupEx,
  ...
  if (g_HostServer.FindPlayerByAcc(NULL, strAcc, &conndup, NULL, &nameid,
                                   &param)) { // find
    size_t pckgsize = sizeof(DWORD) * 2 + size;
    RELAY_ASKWAY_DATA *pClntWayDataDup = (RELAY_ASKWAY_DATA *)_alloca(pckgsize);

    gFillClntwayPckg(nameid, param, pAskWayData, size, pClntWayDataDup,
                     pckgsize);
...
void _Proc1_Relay_Data(const CNetSockDupEx &NetSockDupEx, const void *pData,
                       size_t size) {
...
void _Proc1_Relay_AskWay(const CNetSockDupEx &NetSockDupEx, const void *pData,
                         size_t size) {
  RELAY_ASKWAY_DATA *pAskWayData = (RELAY_ASKWAY_DATA *)pData;
...
void _Proc1_Relay_LoseWay(const CNetSockDupEx &NetSockDupEx, const void *pData,
                          size_t size) {
...
void _Proc0_Relay(const CNetSockDupEx &NetSockDupEx, const void *pData,
                  size_t size) {
...
void _RecvPackage(const CNetSockDupEx &NetSockDupEx, const void *pData,
                  size_t size) {
  EXTEND_HEADER *pHeader = (EXTEND_HEADER *)pData;
