...
int g_nTongPCSize[defTONG_PROTOCOL_CLIENT_NUM] = {
    ...
    sizeof(
        STONG_CHECK_GET_MASTER_POWER_SYNC), // enumS2C_TONG_CHECK_CHANGE_MASTER_POWER
...
KSwordOnLineSever::KSwordOnLineSever() {
  ...
  m_nServerPortMin = 6666;
  m_nServerPortMax = 6666;

  m_nTimeServerPortChange = 0;

  m_nServerPortRange = 0;

  m_nServerPortFowardRange1 = 0;
  m_nServerPortFowardRange2 = 0;
  m_nServerPortFowardRange3 = 0;
  ...
  m_nFowardServerPort1 = 22277;
  m_nFowardServerPort2 = 22277;
  m_nFowardServerPort3 = 22277;
...
BOOL KSwordOnLineSever::Init() {
  ...
  iniFile.GetInteger("GameServer", "Port1", 6666, &m_nServerPort1);

  iniFile.GetInteger("GameServer", "PortMin", 6666, &m_nServerPortMin);
  iniFile.GetInteger("GameServer", "PortMax", 6666, &m_nServerPortMax);

  iniFile.GetInteger("GameServer", "TimeChange", 0, &m_nTimeServerPortChange);

  iniFile.GetInteger("GameServer", "PortRange", 0, &m_nServerPortRange);

  iniFile.GetInteger("GameServer", "PortFoward1", 0, &m_nServerPortFoward1);

  iniFile.GetInteger("GameServer", "PortFoward2", 0, &m_nServerPortFoward2);

  iniFile.GetInteger("GameServer", "PortFoward3", 0, &m_nServerPortFoward3);

  iniFile.GetInteger("GameServer", "PortFowardRange1", 0,
                     &m_nServerPortFowardRange1);
  iniFile.GetInteger("GameServer", "PortFowardRange2", 0,
                     &m_nServerPortFowardRange2);
  ...
  iniFile.GetInteger("Overload", "MaxCLfromCPU", 1, &m_nMaxCLfromCPU);
  ...
  cout << "Server " << m_szIntranetIp << " | " << m_szInternetIp << " : "
       << m_nServerPort << " | " << m_nServerPort1 << endl;

  cout << "Foward " << m_szFowardServerIp1 << ":" << m_nFowardServerPort1
       << " | " << m_szFowardServerIp2 << ":" << m_nFowardServerPort2 << " | "
       << m_szFowardServerIp3 << ":" << m_nFowardServerPort3 << endl;
   ...
   */
#ifndef _STANDALONE
  pfnCreateServerInterface pFactroyFun =
      (pfnCreateServerInterface)(g_theHeavenLibrary.GetProcAddress(
          "CreateInterface"));

  IServerFactory *pServerFactory = NULL;

  if (pFactroyFun &&
      SUCCEEDED(pFactroyFun(IID_IServerFactory,
                            reinterpret_cast<void **>(&pServerFactory)))) {
    pServerFactory->SetEnvironment(m_nMaxPlayer, m_nPrecision, m_snMaxBuffer,
                                   m_snBufferSize);

    pServerFactory->CreateServerInterface(
        IID_IIOCPServer, reinterpret_cast<void **>(&m_pServer));
  ...
  m_pServer->RegisterMsgFilter(reinterpret_cast<void *>(m_pServer),
                               ServerEventNotify);

  if (FAILED(
          m_pServer->OpenService(INADDR_ANY, m_nServerPort, m_nServerPort1))) {
    return FALSE;
  }
  ...
  cout << "CoreServerShell OK" << endl;

  m_pCoreServerShell->OperationRequest(SSOI_LAUNCH, (unsigned int)pCloneServer,
                                       0);
  ...
  pfnCreateClientInterface pClientFactroyFun =
      (pfnCreateClientInterface)(g_theRainbowLibrary.GetProcAddress(
          "CreateInterface"));

  IClientFactory *pClientFactory = NULL;

  if (pClientFactroyFun &&
      SUCCEEDED(pClientFactroyFun(
          IID_IClientFactory, reinterpret_cast<void **>(&pClientFactory)))) {
    pClientFactory->SetEnvironment(1024 * 128);

    //		pClientFactory->SetEnvironment(1024 * 128 * 12);

    pClientFactory->CreateClientInterface(
        IID_IESClient, reinterpret_cast<void **>(&m_pDatabaseClient));
  ...
  cout << "Database IP : " << m_szDatabaseIP << " - Port : " << m_nDatabasePort
       << endl;

  m_pDatabaseClient->RegisterMsgFilter(
      reinterpret_cast<void *>(m_pDatabaseClient), DatabaseClientEventNotify);
  ...
  if (pClientFactroyFun &&
    ...
    pClientFactory->CreateClientInterface(
        IID_IESClient, reinterpret_cast<void **>(&m_pTransferClient));
  ...
  cout << "Transfer IP : " << m_szTransferIP << " - Port : " << m_nTransferPort
       << endl;

  m_pTransferClient->RegisterMsgFilter(
      reinterpret_cast<void *>(m_pTransferClient), TransferClientEventNotify);
  ...
  if (pClientFactroyFun &&
    ...
    pClientFactory->CreateClientInterface(
        IID_IESClient, reinterpret_cast<void **>(&m_pChatClient));
  ...
  m_pChatClient->RegisterMsgFilter(reinterpret_cast<void *>(m_pChatClient),
                                   ChatClientEventNotify);
...
//------------------------ Connect to tong ----------------------------
  ...
  if (pClientFactroyFun &&
    ...
    pClientFactory->SetEnvironment(1024 * 1024 * 4);
    pClientFactory->CreateClientInterface(
        IID_IESClient, reinterpret_cast<void **>(&m_pTongClient));
    pClientFactory->Release();
  } else {
    return FALSE;
  }
  ...
  m_pTongClient->RegisterMsgFilter(reinterpret_cast<void *>(m_pTongClient),
                                   TongClientEventNotify);
  ...
  if (pClientFactroyFun &&
    ...
    //		pClientFactory->SetEnvironment( 1024 * 1024 * 12);

    pClientFactory->CreateClientInterface(
        IID_IESClient, reinterpret_cast<void **>(&m_pGatewayClient));
  ...
  cout << "Gateway IP : " << m_szGatewayIP << " - Port : " << m_nGatewayPort
       << endl;

  m_pGatewayClient->RegisterMsgFilter(
      reinterpret_cast<void *>(m_pGatewayClient), GatewayClientEventNotify);
...
void KSwordOnLineSever::Release() {
  ...
  if (m_pServer) {
    ...
    if (m_pCoreServerShell) {
      printf("Save File Data All Spcrit !");
  ...
  for (it = m_mapIp2TransferUnit.begin(); it != m_mapIp2TransferUnit.end();
       it++) {
  ...
  if (m_pCoreServerShell && m_bIsRunning) {
    ...
    if (!m_bRunningDataBase) {
      ...
      for (;;) {
        ...
        if (pClientFactroyFun &&
            SUCCEEDED(pClientFactroyFun(
                IID_IClientFactory,
                reinterpret_cast<void **>(&pClientFactory)))) {
          pClientFactory->SetEnvironment(1024 * 128);
...
#endif
        if (!m_pDatabaseClient) {
          cout << "Initialization failed! Don't find a correct rainbow.dll"
               << endl;
        ...
        cout << "Database IP : " << m_szDatabaseIP
             << " - Port : " << m_nDatabasePort << endl;

        m_pDatabaseClient->RegisterMsgFilter(
            reinterpret_cast<void *>(m_pDatabaseClient),
            DatabaseClientEventNotify);

        if (FAILED(m_pDatabaseClient->ConnectTo(m_szDatabaseIP,
                                                m_nDatabasePort))) {
          cout << "Connect to database is failed!" << endl;
          continue;
        }
      ...
      for (i = 0; i < m_nMaxPlayer; i++) {
        ...
        m_pCoreServerShell->GetGameData(SGDI_CHARACTER_NAME,
                                        (unsigned int)reg.Name, nIndex);
        if (m_pDatabaseClient)
          m_pDatabaseClient->SendPackToServer((const void *)&reg,
                                              sizeof(tagRoleEnterGame));
      }
    ...
    if (!m_bRunningGateway) {
      ...
      for (int igt = 0; igt < 1; igt++) {
        ...
        cout << "Gateway IP : " << m_szGatewayIP
             << " - Port : " << m_nGatewayPort << endl;

        m_pGatewayClient->RegisterMsgFilter(
            reinterpret_cast<void *>(m_pGatewayClient),
            GatewayClientEventNotify);

        if (FAILED(
                m_pGatewayClient->ConnectTo(m_szGatewayIP, m_nGatewayPort))) {
          cout << "Connect to gateway is failed!" << endl;
          continue;
        }
    ...
    if (m_nGameLoop * 1000 <= m_Timer.GetElapse() * GAME_FPS) {
      ...
      if (m_nGameLoop % m_nTimeServerPortChange == 0) {

        int nTTRange = 0;
        int nServerPortOld = m_nServerPort;
        ...
        for (;;) {
          ...
          for (;;) {
            ...
            nTTRange = g_Random(
                (m_nServerPortMax - m_nServerPortMin) / m_nServerPortRange + 1);
            m_nServerPort = m_nServerPortMin + nTTRange * m_nServerPortRange;

            if (m_nServerPort != nServerPortOld)
              break;
          }

          // m_nServerPort = 22223;
          if (ChangePortServer(m_nServerPort, m_nServerPort1))
            break;
        }
        ...
        ni.nIPAddr_Foward1 = m_dwFowardServerIp1;
        ni.nIPAddr_Foward2 = m_dwFowardServerIp2;
        ni.nIPAddr_Foward3 = m_dwFowardServerIp3;

        ni.nPort = m_nServerPort;

        ni.nFowardPort1 = m_nFowardServerPort1;
        ni.nFowardPort2 = m_nFowardServerPort2;
        ni.nFowardPort3 = m_nFowardServerPort3;

        ni.wCapability = m_nMaxPlayerCount;
        m_pGatewayClient->SendPackToServer((const void *)&ni,
                                           sizeof(tagGameSvrInfo));

        //			printf("Change Port GameServer [%d]
        //[%d|%d|%d]\n",m_nServerPort,m_nFowardServerPort1,m_nFowardServerPort2,m_nFowardServerPort3);
      }
      ...
      if (m_nGameLoop % (GAME_FPS * 60) == 0) // 一分钟一次
      {
        ...
        printf("GameLoop= %06d, Time= %02d:%02d:%02d, Svr(S: %dK R: %dK), "
               "Clt(S: %dK R: %dK), Online= %d\n",
               m_nGameLoop, dwElapse / (1000 * 3600),
               (dwElapse / (1000 * 60)) % 60, (dwElapse / 1000) % 60,
               m_pServer->sendPerf.total_size / 1024,
               m_pServer->recvPerf.total_size / 1024,
               g_sendPerf.total_size / 1024, g_recvPerf.total_size / 1024,
...
void KSwordOnLineSever::MessageLoop() {
  ...
  for (i = 0; i < m_nMaxPlayer; i++) {
    ...
    if (m_pGameStatus[i].bTimeCheckCreadConnect &&
        m_pGameStatus[i].nTimeNextCreadConnect < m_nGameLoop) {

      m_pServer->ShutdownClient(i);
      m_pGameStatus[i].bTimeCheckCreadConnect = FALSE;
      m_pGameStatus[i].nTimeNextCreadConnect = 0;
      printf("Shutdown Client GameServer %d\n", i);
    }
...
void KSwordOnLineSever::ChatMessageProcess(const char *pChar, size_t nSize) {
  ...
  case chat_relegate: {
    ...
    m_pCoreServerShell->GroupChat(
        m_pChatClient, pCR->nFromIP, pCR->nFromRelayID, pCR->channelid,
        pCR->TargetCls, theID, pCR + 1, pCR->routeDateLength);
  } break;
  case chat_groupman:
  ...
  case chat_everyone: {
    {
      ...
      m_pCoreServerShell->OperationRequest(
          SSOI_BROADCASTING, (unsigned int)(pCeo + 1), pCeo->wChatLength);
    }
  } break;
  default:
    break;
  ...
  if (pHeader->ProtocolFamily == pf_extend) {
    if (pHeader->ProtocolID == extend_s2c_passtosomeone) {
      ...
      if (pEpb->playercount > 0) {
        ...
        tagPlusSrcInfo *pPlayers =
            (tagPlusSrcInfo *)((BYTE *)pExPckg + pEpb->datasize);
    ...
    case enumS2C_TONG_CREATE_SUCCESS: {
      ...
      memcpy(sSuccess.m_szTongName, pSync->m_szTongName,
             pSync->m_btTongNameLength);
      sSuccess.m_szTongName[pSync->m_btTongNameLength] = 0;

      if (m_pCoreServerShell->OperationRequest(SSOI_TONG_CREATE,
                                               (unsigned int)&sSuccess, 0)) {
      } else {
      }
    } break;
    case enumS2C_TONG_CREATE_FAIL: {
      ...
      nNetID = m_pCoreServerShell->GetGameData(SGDI_CHARACTER_NETID,
                                               pFail->m_dwParam, 0);
      if (m_pServer)
        m_pServer->PackDataToClient(nNetID, &sFail, sFail.m_wLength + 1);
    } break;
    case enumS2C_TONG_ADD_MEMBER_SUCCESS: {
      STONG_ADD_MEMBER_SUCCESS_SYNC *pSync =
          (STONG_ADD_MEMBER_SUCCESS_SYNC *)pChar;
      ...
      memcpy(sAdd.m_szMasterName, pSync->m_szMasterName,
             sizeof(sAdd.m_szMasterName));
      memcpy(sAdd.m_szTitleName, pSync->m_szTitleName,
             sizeof(sAdd.m_szTitleName));
      memcpy(sAdd.m_szTongName, pSync->m_szTongName, sizeof(sAdd.m_szTongName));

      if (m_pCoreServerShell->OperationRequest(SSOI_TONG_ADD,
                                               (unsigned int)&sAdd, 0)) {
      } else {
      }
    } break;
    ...
    case enumS2C_TONG_HEAD_INFO: {
      ...
      int nNetID = m_pCoreServerShell->GetGameData(SGDI_CHARACTER_NETID,
                                                   pInfo->m_dwParam, 0);
      if (nNetID < 0)
        break;
      ...
      memcpy(sInfo.m_szTongName, pInfo->m_szTongName,
             sizeof(pInfo->m_szTongName));
      memset(sInfo.m_sMember, 0, sizeof(sInfo.m_sMember));
      memcpy(sInfo.m_sMember, pInfo->m_sMember,
             sizeof(TONG_ONE_LEADER_INFO) * (1 + sInfo.m_btDirectorNum));
      sInfo.m_wLength =
          sizeof(sInfo) - 1 - sizeof(sInfo.m_sMember) +
          sizeof(TONG_ONE_LEADER_INFO) * (1 + sInfo.m_btDirectorNum);
      if (m_pServer)
    ...
    case enumS2C_TONG_MANAGER_INFO: {
      ...
      memset(sInfo.m_sMember, 0, sizeof(sInfo.m_sMember));
      memcpy(sInfo.m_sMember, pInfo->m_sMember,
             sizeof(TONG_ONE_LEADER_INFO) * sInfo.m_btCurNum);
      sInfo.m_wLength = sizeof(sInfo) - 1 -
                        sizeof(TONG_ONE_LEADER_INFO) *
                            (defTONG_ONE_PAGE_MAX_NUM - sInfo.m_btCurNum);
      if (m_pServer)
        m_pServer->PackDataToClient(nNetID, &sInfo, sInfo.m_wLength + 1);
    } break;
    ...
    case enumS2C_TONG_MEMBER_INFO: {
      ...
      memset(sInfo.m_sMember, 0, sizeof(sInfo.m_sMember));
      memcpy(sInfo.m_sMember, pInfo->m_sMember,
             sizeof(TONG_ONE_MEMBER_INFO) * sInfo.m_btCurNum);
      sInfo.m_wLength = sizeof(sInfo) - 1 -
                        sizeof(TONG_ONE_MEMBER_INFO) *
                            (defTONG_ONE_PAGE_MAX_NUM - sInfo.m_btCurNum);
      if (m_pServer)
        m_pServer->PackDataToClient(nNetID, &sInfo, sInfo.m_wLength + 1);
    } break;
    ...
    case enumS2C_TONG_BE_INSTATED: {
      ...
      m_pCoreServerShell->GetGameData(SGDI_TONG_BE_INSTATED,
                                      (unsigned int)&sInstated, 0);
    } break;
    ...
    case enumS2C_TONG_BE_KICKED: {
      ...
      m_pCoreServerShell->GetGameData(SGDI_TONG_BE_KICKED,
                                      (unsigned int)&sKicked, 0);
    } break;
    ...
    case enumS2C_TONG_LEAVE: {
      ...
      m_pCoreServerShell->GetGameData(SGDI_TONG_LEAVE, (unsigned int)&sLeave,
                                      0);
    } break;

    case enumS2C_TONG_CHECK_CHANGE_MASTER_POWER: {
      STONG_CHECK_GET_MASTER_POWER_SYNC *pSync =
          (STONG_CHECK_GET_MASTER_POWER_SYNC *)pChar;
      if (m_pGameStatus[pSync->m_dwParam].nPlayerIndex <= 0)
        break;
      ...
      int nRet = 0;
      if (m_pCoreServerShell)
        nRet = m_pCoreServerShell->GetGameData(SGDI_TONG_GET_MASTER_POWER,
      ...
      if (m_pTongClient)
        m_pTongClient->SendPackToServer((const void *)&sAccept,
                                        sizeof(sAccept));
    } break;

    case enumS2C_TONG_CHANGE_MASTER_FAIL: {
      STONG_CHANGE_MASTER_FAIL_SYNC *pFail =
          (STONG_CHANGE_MASTER_FAIL_SYNC *)pChar;
      ...
      if (nPlayer <= 0 || nPlayer >= MAX_PLAYER)
        break;
      int nNetID =
          m_pCoreServerShell->GetGameData(SGDI_CHARACTER_NETID, nPlayer, 0);
      if (nNetID < 0)
        break;
    ...
    case enumS2C_TONG_CHANGE_AS: {
      ...
      m_pCoreServerShell->GetGameData(SGDI_TONG_CHANGE_AS,
    ...
    case enumS2C_TONG_CHANGE_MASTER: {
      ...
      m_pCoreServerShell->GetGameData(SGDI_TONG_CHANGE_MASTER,
                                      (unsigned int)&sChange, 0);
    } break;
...
void KSwordOnLineSever::DatabaseMessageProcess(const char *pData,
                                               size_t dataLength) {
  _ASSERT(pData && dataLength);
...
void KSwordOnLineSever::DatabaseLargePackProcess(const char *pData,
                                                 size_t dataLength) {
  _ASSERT(pData && dataLength);
...
void KSwordOnLineSever::TransferMessageProcess(const char *pChar,
                                               size_t nSize) {
  // #ifndef _STANDALONE
  ...
  // 是否寻路包？
  if (pEH->ProtocolID == relay_c2c_askwaydata &&
      pEH->ProtocolFamily == pf_relay) {
    TransferAskWayMessageProcess(pChar, nSize);
    return;
  }

  RELAY_DATA *pRD = (RELAY_DATA *)pEH;
  if (nSize <= sizeof(RELAY_DATA) ||
      nSize != pRD->routeDateLength + sizeof(RELAY_DATA))
    return;

  // 是否寻路失败包
  if (pEH->ProtocolID == relay_s2c_loseway && pEH->ProtocolFamily == pf_relay) {
    TransferLoseWayMessageProcess(pChar + sizeof(RELAY_DATA),
                                  nSize - sizeof(RELAY_DATA));
    return;
  }
  ...
  if (it != m_mapIp2TransferUnit.end()) {
    ...
    m_mapIp2TransferUnit.insert(
        IP2CONNECTUNIT::value_type(pRD->nFromRelayID, pUnit));
  }
  ...
  if (cProtocol < c2c_micropackbegin) {
    TransferLargePackProcess(pChar + sizeof(RELAY_DATA), pRD->routeDateLength,
                             pUnit);
  } else if (cProtocol > c2c_micropackbegin) {
    TransferSmallPackProcess(pChar + sizeof(RELAY_DATA), pRD->routeDateLength,
                             pUnit);
  } else {
    _ASSERT(FALSE && "Error!");
  }
...
void KSwordOnLineSever::TransferLargePackProcess(const void *pData,
                                                 size_t dataLength,
                                                 KTransferUnit *pUnit) {
  _ASSERT(pData && dataLength);
  if (!pData || !dataLength)
  ...
  if (pBuffer) {
    ...
    case c2c_transferroleinfo: {
      ...
      if (pGI->datalength > 0) {
        ...
        int nIdx = 0;
        if (bCRC) {
          nIdx = m_pCoreServerShell->AddCharacter(
              pGI->nExtPoint, pGI->nChangePoint, (void *)pRoleData, &guid);
        } else {
        ...
        tagPermitPlayerExchange *pPPEO =
            (tagPermitPlayerExchange *)(szChar + sizeof(RELAY_DATA));

        pPPEO->cProtocol = c2c_permitplayerexchangein;
        memcpy(&pPPEO->guid, &guid, sizeof(GUID));

        pPPEO->dwIp =
            m_dwInternetIp; // OnlineGameLib::Win32::net_aton(m_szGameSvrIP);
        pPPEO->wPort = m_nServerPort;

        pPPEO->dwIp1 = m_dwFowardServerIp1;
        pPPEO->wPort1 = m_nFowardServerPort1;
        ...
        m_pTransferClient->SendPackToServer(
            pRD, sizeof(RELAY_DATA) + sizeof(tagPermitPlayerExchange));
...
void KSwordOnLineSever::TransferSmallPackProcess(const void *pData,
                                                 size_t dataLength,
                                                 KTransferUnit *pUnit) {
  // #ifndef _STANDALONE
  ...
  case s2s_broadcast: {
    BYTE *pS = (BYTE *)pData;
    m_pCoreServerShell->ProcessBroadcastMessage((const char *)(pS + 1),
                                                dataLength - 1);
  } break;
  case s2s_execute: {
    BYTE *pS = (BYTE *)pData;
    m_pCoreServerShell->ProcessExecuteMessage((const char *)(pS + 1),
                                              dataLength - 1);
  } break;
  case c2c_permitplayerexchangein: {
    ...
    if (m_pGameStatus[lnID].nExchangeStatus !=
        enumExchangeWaitForGameSvrRespone)
      break;
    ...
    if (pPermit->bPermit) {
      ...
      npe.nIPAddr1 = pPermit->dwIp1;
      npe.nPort1 = pPermit->wPort1;
      ...
      char szName[32];
      m_pCoreServerShell->GetGameData(SGDI_CHARACTER_ACCOUNT,
                                      (unsigned int)szName, nIndex);
      sLeaveGame.nExtPoint = m_pCoreServerShell->GetGameData(
  ...
  case c2c_notifyexchange: {
    ...
    if (pSW->dwPlayerID !=
        m_pCoreServerShell->GetGameData(SGDI_CHARACTER_ID, nIndex, 0)) {
      printf("break 4\n");
      break;
    }
    ...
    TRoleData *pData =
        (TRoleData *)m_pCoreServerShell->PreparePlayerForExchange(nIndex);
    ...
    gi.nExtPoint =
        m_pCoreServerShell->GetGameData(SGDI_CHARACTER_EXTPOINT, nIndex, 0);
    gi.nChangePoint = m_pCoreServerShell->GetGameData(
        SGDI_CHARACTER_EXTPOINTCHANGED, nIndex, 0);
    ...
    m_theSend.AddData(c2c_transferroleinfo, (const char *)&gi,
                      sizeof(tagGuidableInfo));
    m_theSend.AddData(c2c_transferroleinfo, (const char *)pData,
                      gi.datalength - 4);
...
#ifndef _STANDALONE
      memcpy(szData + sizeof(RELAY_DATA), pBuffer->GetBuffer(),
             pBuffer->GetUsed());
#else
      memcpy(szData + sizeof(RELAY_DATA), pBuffer, actLen);
#endif

      m_pTransferClient->SendPackToServer(szData, pRD->routeDateLength +
                                                      sizeof(RELAY_DATA));
#ifndef _STANDALONE
      try {
        if (pBuffer) {
...
#endif
    m_pGameStatus[pSW->lnID].nExchangeStatus =
        enumExchangeWaitForGameSvrRespone;
  } break;
  }
  // #endif
...
void KSwordOnLineSever::PlayerMessageProcess(const unsigned long lnID,
                                             const char *pData,
                                             size_t dataLength) {
  // #ifndef _STANDALONE
  _ASSERT(pData && dataLength);
  ...
  if (protocoltype >= _c2s_begin_relay && protocoltype <= _c2s_end_relay) {
    ...
    if (pClient && nIndex && dataLength > sizeof(tagExtendProtoHeader)) {
      ...
      if (protocoltype == c2s_extendchat) {
        ...
        {
          if (!m_pCoreServerShell->PayForSpeech2(nIndex, pEh->cost,
                                                 pEh->channelid))
            return;
        }

        if (!m_pCoreServerShell->CheckCastChat(nIndex, 0))
          return;
      }
  ...
  if (!m_pCoreServerShell->CheckProtocolSize(pData, dataLength)) {
    ...
    printf("Size co van de %d %d, Kick !\n", lnID, sizeof(pData));
#ifndef _STANDALONE
  ...
  case enumPlayerSyncEnd:
    if (ProcessSyncReplyProtocol(lnID, pData, dataLength)) {
      ...
      int nCamp =
          m_pCoreServerShell->GetGameData(SGDI_CHARACTER_CAMP, nIndex, 0);
      if (nCheckClc >= 4 && nCamp != 0) {
        ...
        memset(m_pGameStatus[lnID].szName, 0,
               sizeof(m_pGameStatus[lnID].szName));
#endif
        break;
      }
      ...
      m_pCoreServerShell->GetGameData(SGDI_CHARACTER_ACCOUNT,
                                      (unsigned int)eg.szAccountName, nIndex);

      if (m_pGatewayClient)
        m_pGatewayClient->SendPackToServer((const void *)&eg,
                                           sizeof(tagEnterGame));
      ...
      m_pCoreServerShell->GetGameData(
          SGDI_CHARACTER_NAME, (unsigned int)eg2.szCharacterName, nIndex);
      eg2.lnID = lnID;
      if (m_pTransferClient)
        m_pTransferClient->SendPackToServer((const void *)&eg2,
      ...
      if (nPlayerIdx > 0 && nPlayerIdx < MAX_PLAYER) {
        ...
        DWORD dwTongNameID = m_pCoreServerShell->GetGameData(
            SGDI_TONG_GET_TONG_NAMEID, 0, nPlayerIdx);
        char szName[32];
        szName[0] = 0;
        m_pCoreServerShell->GetGameData(SGDI_CHARACTER_NAME,
                                        (unsigned int)szName, nPlayerIdx);

        printf("Acc: %s - %d \n", eg.szAccountName, nPlayerIdx);

        if (dwTongNameID > 0 && szName[0]) {
          m_pCoreServerShell->GetGameData(SGDI_TONG_LOGIN, dwTongNameID,
                                          nPlayerIdx);
        }

        m_pCoreServerShell->GetGameData(SGDI_TONG_SEND_SELF_INFO, 0,
                                        nPlayerIdx);

        ExSpcritStarPlayer(nPlayerIdx);
  ...
  case enumPlayerBegin: {
    ...
    if (nIndex) {
      ...
      m_pGameStatus[lnID].nTimeNextCreadConnect = 0;
      m_pGameStatus[lnID].bTimeCheckCreadConnect = FALSE;

      tagLogicLogin *pLLClc = (tagLogicLogin *)pData;
      char szNameClc[64];
      szNameClc[0] = 0;

      if (pLLClc->szName && pLLClc->szName[0]) {
        memcpy(szNameClc, pLLClc->szName, sizeof(szNameClc));
        szNameClc[sizeof(szNameClc) - 1] = 0;
      }

      BOOL bCheckClc = TRUE;
      nCheckClc = 0;

      if (szNameClc && szNameClc[0]) {

        for (int iclc = 0; iclc < m_nMaxPlayer; iclc++) {
          if (lnID < 0 || lnID >= m_nMaxPlayer || iclc == lnID ||
              enumNetUnconnect == GetNetStatus(iclc))
            continue;

          if (strcmp(m_pGameStatus[iclc].szName, szNameClc) == 0) {

            const char *pInfoClc1 = m_pServer->GetClientInfo(lnID);
            const char *pInfoClc2 = m_pServer->GetClientInfo(iclc);

            if (!pInfoClc1 || !pInfoClc2 ||
                inet_addr(pInfoClc1) == inet_addr("113.20.115.1") ||
                inet_addr(pInfoClc2) == inet_addr("113.20.115.1") ||
                inet_addr(pInfoClc1) == inet_addr("171.244.17.16") ||
                inet_addr(pInfoClc2) == inet_addr("171.244.17.16") ||
                inet_addr(pInfoClc1) == inet_addr("116.193.73.18") ||
                inet_addr(pInfoClc2) == inet_addr("116.193.73.18"))

            {
              nCheckClc++;
            } else {
              if (!pInfoClc1 || !pInfoClc2 ||
                  inet_addr(pInfoClc1) == inet_addr(pInfoClc2)) {
                nCheckClc++;
              }
            }

            if (nCheckClc >= m_nMaxCLfromCPU) {
              printf("Nhieu hon %d Acc %s\n", m_nMaxCLfromCPU, szNameClc);
              bCheckClc = FALSE;
              break;
            }
          }
      ...
      {
        printf("Khong ton tai szName\n");
        bCheckClc = FALSE;
      }

      if (bCheckClc) {

        if (lnID >= 0 && lnID < m_nMaxPlayer && pLLClc->szName &&
            pLLClc->szName[0]) {
          memcpy(m_pGameStatus[lnID].szName, pLLClc->szName,
                 sizeof(m_pGameStatus[lnID].szName));
          m_pGameStatus[lnID].szName[sizeof(m_pGameStatus[lnID].szName) - 1] =
              0;
        }
...
void KSwordOnLineSever::GatewayMessageProcess(const char *pData,
                                              size_t dataLength) {
  _ASSERT(pData && dataLength);
  ...
  if (cProtocol < s2c_micropackbegin) {
    ...
    printf("GatewayMessageProcess Error!\n");
  }
}

void KSwordOnLineSever::GatewayLargePackProcess(const void *pData,
                                                size_t dataLength) {
#ifndef _STANDALONE
  _ASSERT(pData && dataLength);
  ...
  if (pBuffer) {
    ...
    case s2c_syncgamesvr_roleinfo_cipher: {
      ...
      if (pGI->datalength > 0) {
        ...
        BOOL bCRCCheck = TRUE;
        if (pRoleData->dwFriendOffset == pRoleData->dwDataLen - 4 &&
        ...
        for (int nindex = 1; nindex < MAX_PLAYER; nindex++) {
          if (m_pCoreServerShell->IsCharacterLiXian(nindex) == 2) {
            ...
            if (!strcmpi(pRoleData->BaseInfo.caccname, szName)) {
        ...
        if (Info.nNumPlayer < m_nMaxPlayer &&
            bCRCCheck) // 去掉注释CRC错的用户就无法进入游戏了
        {
          int nIdx = m_pCoreServerShell->AddCharacter(
              pGI->nExtPoint, pGI->nChangePoint, (void *)pRoleData, &guid);
        ...
        m_pGatewayClient->SendPackToServer((const void *)&ppl,
                                           sizeof(tagPermitPlayerLogin));
      }
    } break;
...
void KSwordOnLineSever::GatewaySmallPackProcess(const void *pData,
                                                size_t dataLength) {
  _ASSERT(pData && dataLength);
  ...
  case s2c_querymapinfo: {
    ...
    int nMapCount = m_pCoreServerShell->GetGameData(
        SGDI_LOADEDMAP_ID, (unsigned int)&sMapID, sizeof(sMapID));
    pUMI = (tagUpdateMapID
                *)new BYTE[nMapCount * sizeof(char) + sizeof(tagUpdateMapID)];
    ...
    /*			printf("-----Send \ndatalen = %u, data = ", nMapCount *
       sizeof(char) + sizeof( tagUpdateMapID )); for (int i = 0; i < 32; i++)
                                    printf("[%02X] ", ((BYTE *)pUMI)[i]);
                            printf("\n");*/
    m_pGatewayClient->SendPackToServer(
        (const void *)pUMI, nMapCount * sizeof(char) + sizeof(tagUpdateMapID));
...
void KSwordOnLineSever::GatewayBoardCastProcess(const char *pData,
                                                size_t dataLength) {
  // #ifndef _STANDALONE
  ...
  // printf("BroadCast type = %d, %08x, %08x, %d\n",pGatewayBroadCast->uCmdType,
  // (unsigned int)pGatewayBroadCast->szData, pGatewayBroadCast->szData,
  // strlen(pGatewayBroadCast->szData));
  switch (pGatewayBroadCast->uCmdType) {
  case AP_WARNING_ALL_PLAYER_QUIT: {
    m_pCoreServerShell->OperationRequest(
        SSOI_BROADCASTING, (unsigned int)pGatewayBroadCast->szData,
        strlen(pGatewayBroadCast->szData));
  ...
  case AP_NOTIFY_ALL_PLAYER: {
    printf("Safe AP_NOTIFY_ALL_PLAYER\n");
    m_pCoreServerShell->OperationRequest(
        SSOI_BROADCASTING, (unsigned int)pGatewayBroadCast->szData,
        strlen(pGatewayBroadCast->szData));
  } break;
...
#define defMAX_PING_INTERVAL 5 * 20
    ...
    if (m_pGameStatus[lnID].nReplyPingTime != 0) {
      if (m_nGameLoop - m_pGameStatus[lnID].nSendPingTime >
          defMAX_PING_INTERVAL) {
        // printf("Send ping to client....\n");
        PingClient(lnID);
      }
    } else if (m_nGameLoop - m_pGameStatus[lnID].nSendPingTime >
               defMAX_PING_TIMEOUT) {
...
void KSwordOnLineSever::PingClient(const unsigned long lnID) {
  ...
  _ASSERT(m_pGameStatus[lnID].nPlayerIndex > 0 &&
          m_pGameStatus[lnID].nPlayerIndex <= m_nMaxPlayer);
...
void KSwordOnLineSever::ProcessPingReply(const unsigned long lnID,
...
void KSwordOnLineSever::SavePlayerData() {
  ...
  for (i = 0; i < m_nMaxPlayer; i++) {
    ...
    // TRoleData* pData = (TRoleData
    // *)m_pCoreServerShell->SavePlayerData(nIndex);
...
BOOL KSwordOnLineSever::SavePlayerData(int nIndex, bool bUnLock) {
  ...
  TRoleData *pData =
      (TRoleData *)m_pCoreServerShell->SavePlayerDataAtOnce(nIndex);
  if (pData && pData->dwDataLen) {
    ...
    m_theSend.AddData(c2s_roleserver_saveroleinfo, (const char *)&sProcessData,
                      sizeof(TProcessData) - 1);
    m_theSend.AddData(c2s_roleserver_saveroleinfo, (const char *)pData,
                      pData->dwDataLen - 4);
    ...
    while (pBuffer) {
#ifndef _STANDALONE
      m_pDatabaseClient->SendPackToServer(pBuffer->GetBuffer(),
                                          pBuffer->GetUsed());
#else
...
#endif
    // printf("Saving %s(%d)'s data, size:(%d)...\n", pData->BaseInfo.caccname,
    // nIndex, sProcessData.nDataLen);
  ...
  for (i = 0; i < m_nMaxPlayer; i++) {
    ...
    if (m_pCoreServerShell->IsPlayerExchangingServer(nIndex)) {
      ...
      case enumExchangeBegin: {
        ...
        *(DWORD *)(szChar + sizeof(RELAY_ASKWAY_DATA)) =
            m_pCoreServerShell->GetExchangeMap(nIndex);
        ...
        pSW->dwPlayerID =
            m_pCoreServerShell->GetGameData(SGDI_CHARACTER_ID, pSW->nIndex, 0);
        // ask another game svr ip
        m_pTransferClient->SendPackToServer(pRAD, sizeof(RELAY_ASKWAY_DATA) +
                                                      pRAD->wMethodDataLength +
...
void KSwordOnLineSever::PlayerLogoutGateway() {
  ...
  for (int nIndex = 1; nIndex < MAX_PLAYER; nIndex++) {
    ...
    if (m_pCoreServerShell->IsPlayerLoginTimeOut(nIndex)) {
      ...
      if (m_pCoreServerShell->IsCharacterLiXian(nIndex) ==
          0) // trang thai ko UYTHAC
        SavePlayerData(nIndex, true);

      //			if (!SavePlayerData(nIndex))
      //				continue;
      if (m_pCoreServerShell->IsCharacterLiXian(nIndex) == 0 ||
          m_pCoreServerShell->IsCharacterLiXian(nIndex) ==
              1) // UYTHAC them DK nay
      {
        ...
        /*
                                m_pGameStatus[lnID].nExchangeStatus =
           enumExchangeBegin; m_pGameStatus[lnID].nGameStatus = enumPlayerBegin;
                                m_pGameStatus[lnID].nPlayerIndex = 0;
        */
      }
...
void KSwordOnLineSever::SetNetStatus(const unsigned long lnID,
                                     NetStatus nStatus) {
  ...
  if (nStatus == enumNetUnconnect) {
    if (m_pGameStatus[lnID].nGameStatus == enumPlayerPlaying ||
        (m_pGameStatus[lnID].nGameStatus == enumPlayerExchangingServer &&
         m_pGameStatus[lnID].nExchangeStatus !=
             enumExchangeCleaning)) // 跨服务器时自己处理
    {
      int nIndex = m_pGameStatus[lnID].nPlayerIndex;
      ExSpcritStopPlayer(nIndex);
  ...
  if (nStatus == enumNetConnected) {
    ...
    m_pGameStatus[lnID].nTimeNextCreadConnect = m_nGameLoop + 10 * 18;
    m_pGameStatus[lnID].bTimeCheckCreadConnect = TRUE;

    printf("enumNetConnected %d...\n", lnID);
  }
  m_pGameStatus[lnID].nNetStatus = nStatus;
}

int KSwordOnLineSever::ProcessLoginProtocol(const unsigned long lnID,
                                            const char *pData,
                                            size_t dataLength) {
...
void KSwordOnLineSever::ExSpcritStarPlayer(const int nPlayerIndex) {

  if (m_pCoreServerShell)
    m_pCoreServerShell->ExSpcritStarPlayer(nPlayerIndex);
}

void KSwordOnLineSever::ExSpcritStopPlayer(const int nPlayerIndex) {

  if (m_pCoreServerShell)
    m_pCoreServerShell->ExSpcritStopPlayer(nPlayerIndex);
}

void KSwordOnLineSever::KickAcountAndBlock(const int nPlayerIndex,
                                           const char *szNameCpuIp) {

  if (m_pCoreServerShell)
    m_pCoreServerShell->KickAcountAndBlock(nPlayerIndex, szNameCpuIp);
}

BOOL KSwordOnLineSever::SendGameDataToClient(const unsigned long lnID,
                                             const int nPlayerIndex) {
  BOOL bRet = FALSE;
  // #ifndef _STANDALONE
  _ASSERT(m_pServer);
  ...
  while (true) {
    ...
    bRet = m_pCoreServerShell->PlayerDbLoading(nPlayerIndex, bSyncEnd, nStep,
                                               nParam);
    if (!bRet) {
...
BOOL KSwordOnLineSever::ProcessSyncReplyProtocol(const unsigned long lnID,
                                                 const char *pData,
                                                 size_t dataLength) {
  const char *pBuffer = pData;
...
BOOL KSwordOnLineSever::GetRunningDataBase() { return m_bRunningDataBase; }

void KSwordOnLineSever::SetRunningGateway(BOOL bStatus) {
  m_bRunningGateway = bStatus;
}

BOOL KSwordOnLineSever::GetRunningGateway() { return m_bRunningGateway; }

BOOL KSwordOnLineSever::ConformAskWay(const void *pData, int nSize,
                                      DWORD *lpnNetID) {
  BOOL bRet = FALSE;
  ...
  case rm_account_id: {
    DWORD dwNameID;
    DWORD lnID;
    LPDWORD lpdwPTR = (DWORD *)(((char *)pData) + sizeof(RELAY_ASKWAY_DATA) +
                                pRAD->wMethodDataLength - sizeof(DWORD) * 2);
    dwNameID = *lpdwPTR;
    lnID = *(lpdwPTR + 1);
    if (lnID >= m_nMaxPlayer || m_pGameStatus[lnID].nPlayerIndex <= 0 ||
        m_pGameStatus[lnID].nPlayerIndex > m_nMaxPlayer) {
      bRet = FALSE;
      break;
    }
    DWORD dwNameIDbyIndex = m_pCoreServerShell->GetGameData(
        SGDI_CHARACTER_ID, m_pGameStatus[lnID].nPlayerIndex, 0);
...
BOOL KSwordOnLineSever::GetLocalIpAddress(DWORD *pIntranetAddr,
                                          DWORD *pInternetAddr) {
#ifndef _STANDALONE
...
#ifndef WIN32
    if (!hp || hp && hp->h_addr_list[0] &&
                   *(unsigned long *)hp->h_addr_list[0] == 0x0100007f) {
      int sock;
      struct ifreq ifr;
      char *ip = NULL;
      int err = -1;
...
void KSwordOnLineSever::TransferAskWayMessageProcess(const char *pData,
                                                     size_t dataLength) {
  char szChar[1024];
  ...
  if (!ConformAskWay(pRAD, dataLength, &lnID)) {
    ...
    case rm_map_id:
      memcpy(szChar + sizeof(RELAY_DATA),
             pData + sizeof(RELAY_ASKWAY_DATA) + pRAD->wMethodDataLength,
             pRD->routeDateLength);
      m_pTransferClient->SendPackToServer(pRD, sizeof(RELAY_DATA) +
                                                   pRD->routeDateLength);
      break;
    case rm_role_id:
    case rm_account_id: {
      if (pf_normal !=
        ...
        m_pCoreServerShell->ProcessNewClientMessage(
            m_pTransferClient, pRAD->nFromIP, pRAD->nFromRelayID,
...
void KSwordOnLineSever::TransferLoseWayMessageProcess(const char *pData,
                                                      size_t dataLength) {
  ...
  if (pInHeader->ProtocolFamily == pf_relay) {
    ...
    case relay_c2c_askwaydata:
      pSW = (tagSearchWay *)(pData + sizeof(RELAY_ASKWAY_DATA) +
                             ((RELAY_ASKWAY_DATA *)pData)->wMethodDataLength);
      break;
    default:
      return;
    }
    ...
    if (m_pCoreServerShell->GetGameData(SGDI_CHARACTER_ID, pSW->nIndex, 0) !=
        pSW->dwPlayerID)
      return;
    ...
    reg.ProtocolType = c2s_roleserver_lock;
    reg.bLock = true;
    m_pCoreServerShell->GetGameData(SGDI_CHARACTER_NAME, (unsigned int)reg.Name,
                                    nIndex);
    if (m_pDatabaseClient)
      m_pDatabaseClient->SendPackToServer((const void *)&reg,
                                          sizeof(tagRoleEnterGame));
...
void KSwordOnLineSever::ProcessPlayerTongMsg(const unsigned long nPlayerIdx,
                                             const char *pData,
                                             size_t dataLength) {
  if (nPlayerIdx <= 0 || nPlayerIdx >= MAX_PLAYER)
    return;
  ...
  case enumTONG_COMMAND_ID_APPLY_CREATE: {
    ...
    if (m_pCoreServerShell) {
      m_pCoreServerShell->GetGameData(SGDI_TONG_APPLY_CREATE,
                                      (unsigned int)&sApply, 0);
    }

  } break;
  case enumTONG_COMMAND_ID_APPLY_CHANGE: {
    TONG_APPLY_CHANGE_COMMAND *pApply = (TONG_APPLY_CHANGE_COMMAND *)pData;
    STONG_SERVER_TO_CORE_APPLY_CHANGE sApply;
    ...
    if (m_pCoreServerShell) {
      m_pCoreServerShell->GetGameData(SGDI_TONG_APPLY_CHANGE,
                                      (unsigned int)&sApply, 0);
    }
  ...
  case enumTONG_COMMAND_ID_APPLY_ADD: {
    ...
    if (m_pCoreServerShell)
      m_pCoreServerShell->GetGameData(SGDI_TONG_APPLY_ADD, (unsigned int)&sAdd,
                                      0);
  } break;
  case enumTONG_COMMAND_ID_ACCEPT_ADD: {
    ...
    if (pAccept->m_btFlag == 0) {
      ...
      m_pCoreServerShell->OperationRequest(SSOI_TONG_REFUSE_ADD,
                                           (unsigned int)&sRefuse, 0);
      break;
    } else {
      char szTongName[32];
      ...
      if (m_pCoreServerShell)
        m_pCoreServerShell->GetGameData(SGDI_TONG_CHECK_ADD_CONDITION,
                                        (unsigned int)szTongName,
                                        (unsigned int)&sAdd);
    }
  } break;
  case enumTONG_COMMAND_ID_APPLY_INFO: {
    ...
    if (pInfo->m_wLength <
        sizeof(TONG_APPLY_INFO_COMMAND) - 1 - sizeof(pInfo->m_szBuf))
      break;
    ...
    case enumTONG_APPLY_INFO_ID_SELF: {
      ...
      sGet.m_szName[0] = 0;
      if (m_pCoreServerShell)
        m_pCoreServerShell->GetGameData(SGDI_TONG_GET_INFO, (unsigned int)&sGet,
                                        0);
  ...
  case enumTONG_COMMAND_ID_APPLY_INSTATE: {
    ...
    if (m_pCoreServerShell)
      m_pCoreServerShell->GetGameData(SGDI_TONG_INSTATE_POWER,
                                      (unsigned int)pApply, nPlayerIdx);
  ...
  case enumTONG_COMMAND_ID_APPLY_KICK: {
    ...
    if (m_pCoreServerShell)
      m_pCoreServerShell->GetGameData(SGDI_TONG_KICK_POWER, (unsigned int)pKick,
                                      nPlayerIdx);
  } break;
  case enumTONG_COMMAND_ID_APPLY_LEAVE: {
    ...
    ;
    if (m_pCoreServerShell)
      m_pCoreServerShell->GetGameData(SGDI_TONG_LEAVE_POWER,
                                      (unsigned int)pLeave, nPlayerIdx);

  } break;
  case enumTONG_COMMAND_ID_APPLY_CHANGE_MASTER: {
    TONG_APPLY_CHANGE_MASTER_COMMAND *pChange =
        (TONG_APPLY_CHANGE_MASTER_COMMAND *)pData;
    if (pChange->m_wLength + 1 != sizeof(TONG_APPLY_CHANGE_MASTER_COMMAND))
      break;
    int nRet = 0;
    if (m_pCoreServerShell)
      nRet = m_pCoreServerShell->GetGameData(SGDI_TONG_CHANGE_MASTER_POWER,
                                             (unsigned int)pChange, nPlayerIdx);
    if (nRet == 0)
      break;
    ...
    printf("Loi can kiem tra ngay TONG_COMMAND_ID_APPLY_CHANGE_MASTER "
           "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
  ...
  case enumTONG_COMMAND_ID_APPLY_CHANGE_TITLE_FEMALE: {
    TONG_APPLY_CHANGE_TITLE_FEMALE *pChange =
        (TONG_APPLY_CHANGE_TITLE_FEMALE *)pData;
    if (pChange->m_wLength + 1 != sizeof(TONG_APPLY_CHANGE_TITLE_FEMALE))
      break;

    if (m_pCoreServerShell)
      m_pCoreServerShell->GetGameData(SGDI_TONG_CHANGE_TITLE_FEMALE, nPlayerIdx,
                                      (unsigned int)pChange);

  } break;

  case enumTONG_COMMAND_ID_APPLY_CHANGE_TITLE_MALE: {
    TONG_APPLY_CHANGE_TITLE_MALE *pChange =
        (TONG_APPLY_CHANGE_TITLE_MALE *)pData;
    if (pChange->m_wLength + 1 != sizeof(TONG_APPLY_CHANGE_TITLE_MALE))
      break;

    if (m_pCoreServerShell)
      m_pCoreServerShell->GetGameData(SGDI_TONG_CHANGE_TITLE_MALE, nPlayerIdx,
                                      (unsigned int)pChange);

  } break;

  case enumTONG_COMMAND_ID_APPLY_CHANGE_TITLE: {
    TONG_APPLY_CHANGE_TITLE *pChange = (TONG_APPLY_CHANGE_TITLE *)pData;
    if (pChange->m_wLength + 1 != sizeof(TONG_APPLY_CHANGE_TITLE))
      break;

    if (m_pCoreServerShell)
      m_pCoreServerShell->GetGameData(SGDI_TONG_CHANGE_TITLE, nPlayerIdx,
                                      (unsigned int)pChange);
  } break;

  case enumTONG_COMMAND_ID_APPLY_DEPOSIT_MONEY: {

    TONG_APPLY_DEPOSIT_MONEY *pChange = (TONG_APPLY_DEPOSIT_MONEY *)pData;
    if (pChange->m_wLength + 1 != sizeof(TONG_APPLY_DEPOSIT_MONEY))
      break;

    if (m_pCoreServerShell)
      m_pCoreServerShell->GetGameData(SGDI_TONG_DEPOSIT_MONEY, nPlayerIdx,
                                      (unsigned int)pChange);
  } break;

  case enumTONG_COMMAND_ID_APPLY_TAKE_MONEY: {
    TONG_APPLY_TAKE_MONEY *pChange = (TONG_APPLY_TAKE_MONEY *)pData;
    if (pChange->m_wLength + 1 != sizeof(TONG_APPLY_TAKE_MONEY))
      break;

    if (m_pCoreServerShell)
      m_pCoreServerShell->GetGameData(SGDI_TONG_TAKE_MONEY, nPlayerIdx,
                                      (unsigned int)pChange);
  } break;
...
BOOL KSwordOnLineSever::ChangePortServer(unsigned short uPort,
                                         unsigned short uPort1) {

  if (!m_pServer) {
    printf("Change Port Server Fail 1 !!!!\n");
    return TRUE;
  }

  m_pServer->CloseService();

  if (FAILED(m_pServer->OpenService(INADDR_ANY, uPort, uPort1))) {
    printf("Change Port Server Fail 2 !!!!\n");
    return FALSE;
  }
