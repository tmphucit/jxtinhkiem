...
public:
  explicit CPerConnectionData(const _tstring &address, WPARAM wParam,
                              const LPARAM &lParam)
      : m_address(address), m_wParam(wParam), m_lParam(lParam) {}

  void SetConnectionFirstParam(WPARAM wParam) { m_wParam = wParam; }

  WPARAM GetConnectionFirstParam() const { return m_wParam; }

  LPARAM GetConnectionSecondParam() const { return m_lParam; }

  _tstring GetConnectionDetails() const { return m_address; }
...
CIOCPServer::CIOCPServer(size_t nPlayerMaxCount, size_t nPrecision,
                         size_t maxFreeSockets, size_t maxFreeBuffers,
                         size_t maxFreeBuffers_Cache,
                         size_t bufferSize_Cache /*= 8192*/,
                         size_t bufferSize /*= 1024*/,
                         size_t numThreads /*= 0*/)
    : OnlineGameLib::Win32::CSocketServer(maxFreeSockets, maxFreeBuffers,
                                          bufferSize, numThreads),
      m_theCacheAllocator(bufferSize_Cache, maxFreeBuffers_Cache),
      m_lRefCount(0), m_nPlayerMaxCount(nPlayerMaxCount),
      m_nPrecision(nPrecision),
      m_nNetworkBufferMaxLen(bufferSize -
                             32 /* sizeof( protocol of network ) */),
  ...
  m_hHelperThread = (HANDLE)::_beginthreadex(0, 0, HelperThreadFunction,
                                             (void *)this, 0, &threadID);

  if (m_hHelperThread == INVALID_HANDLE_VALUE) {
    throw CWin32Exception(_T("CIOCPServer::CIOCPServer() - _beginthreadex"),
                          GetLastError());
  }
}
...
CIOCPServer::~CIOCPServer() {
...

  {
    ...
    for (it = m_theClientManager.begin(); it != m_theClientManager.end();
         it++) {
    ...
    m_theClientManager.erase(m_theClientManager.begin(),
                             m_theClientManager.end());
  }
...
STDMETHODIMP CIOCPServer::Startup() {
  ...
  try {
    ...
    Output(_T("CIOCPServer::Startup Exception: ") + e.GetWhere() + _T(" - ") +
           e.GetMessage());
  } catch (...) {
...
STDMETHODIMP CIOCPServer::Cleanup() {
  ...
  try {
    ...
    Output(_T("CIOCPServer::Cleanup Exception: ") + e.GetWhere() + _T(" - ") +
           e.GetMessage());
  } catch (...) {
...
STDMETHODIMP
CIOCPServer::OpenService(const unsigned long &ulnAddressToListenOn,
                         const unsigned short &usnPortToListenOn,
                         const unsigned short &usnPortToListenOn1) {
  HRESULT hr = E_FAIL;

  try {

    Open(ulnAddressToListenOn, usnPortToListenOn, usnPortToListenOn1);

    StartAcceptingConnections();

    hr = S_OK;
  } catch (const CException &e) {
    Output(_T("CIOCPServer::OpenService Exception: ") + e.GetWhere() +
           _T(" - ") + e.GetMessage());
  } catch (...) {
...
STDMETHODIMP CIOCPServer::CloseService() {
  ...
  try {
    ...
    Output(_T("CIOCPServer::CloseService Exception: ") + e.GetWhere() +
           _T(" - ") + e.GetMessage());
  } catch (...) {
...
STDMETHODIMP
CIOCPServer::RegisterMsgFilter(LPVOID lpParam,
...
STDMETHODIMP CIOCPServer::PreparePackSink() { return S_OK; }
  ...
  if (ulnClientID < m_nPlayerMaxCount && datalength > 0) {
    ...
    if (pCN && pCN->pSocket) {
      try {
        ...
        pWriteBuffer->AddData(static_cast<const char *const>(pData),
                              datalength);

        hr = S_OK;
      } catch (const CException &e) {
        Output(_T("CIOCPServer::PackDataToClient Exception: ") + e.GetWhere() +
               _T(" - ") + e.GetMessage());
...
STDMETHODIMP
CIOCPServer::SendPackToClient(const unsigned long &ulnClientID /* -1 */) {
  HRESULT hr = E_FAIL;
      ...
      try {
...

        {
          ...
          if (used > 0) {
            ...
            pBuffer->AddData(reinterpret_cast<const char *>(&headlength),
                             s_len_protocol);
            ...
            KSG_EncodeBuf(
                used, (unsigned char *)(pBuffer->GetBuffer() + s_len_protocol),
                &pCN->uServerKey);
        ...
        Output(_T("CIOCPServer::SendPackToClient Exception: ") + e.GetWhere() +
               _T(" - ") + e.GetMessage());
...
bool CIOCPServer::_SendDataEx(LPCLIENT_NODE pNode, const void *const pData,
                              const size_t &datalength) {
  static const size_t s_len_protocol = sizeof(WORD);
  ...
  if (pNode->pSocket) {
    ...
    try {
      ...
      Output(_T("CIOCPServer::_SendDataEx Exception: ") + e.GetWhere() +
             _T(" - ") + e.GetMessage());
...
STDMETHODIMP_(const void *)
CIOCPServer::GetPackFromClient(const unsigned long &ulnClientID,
                               size_t &datalength) {
  if (ulnClientID < m_nPlayerMaxCount) {
    LPCLIENT_NODE pCN = NULL;
    ...
    if (pCN && pCN->pSocket) {
      ...
      const char *pData =
          reinterpret_cast<const char *>(pCN->pReadBuffer->GetBuffer());
...
CIOCPServer::GetClientInfo(const unsigned long &ulnClientID) {
  ...
  if (ulnClientID < m_nPlayerMaxCount) {
    ...
    if (pCN && pCN->pSocket) {
      const CPerConnectionData *pData =
          reinterpret_cast<const CPerConnectionData *>(
              pCN->pSocket->GetUserPtr());
...
void CIOCPServer::OnConnectionEstablished(
    Socket *pSocket, OnlineGameLib::Win32::CIOBuffer *pAddress) {
  if (pSocket) {
    pSocket->AddRef();
    ...
    CSocketAddress address(
        reinterpret_cast<const sockaddr *>(pAddress->GetBuffer()));

    CPerConnectionData *pData = new CPerConnectionData(
        address.GetAsString(), (WPARAM)(-1), (LPARAM)(pSocket));
...
void CIOCPServer::OnConnectionClosed(Socket *pSocket) {
  if (pSocket) {
    ...
    CPerConnectionData *pData =
        reinterpret_cast<CPerConnectionData *>(pSocket->GetUserPtr());

    m_theDelClientQueue.Push(pData);
  }
}

void CIOCPServer::ReadCompleted(Socket *pSocket,
  ...
  try {
    const CPerConnectionData *pData =
        reinterpret_cast<const CPerConnectionData *>(pSocket->GetUserPtr());
...
CIOBuffer *CIOCPServer::ProcessDataStream(Socket *pSocket, CIOBuffer *pBuffer) {
  ...
  /*
   * not enough data in the buffer, reissue a read into the same buffer to
   * collect more data
   */
  return pBuffer;
}
...
void CIOCPServer::ProcessCommand(Socket *pSocket, const CIOBuffer *pBuffer) {
  ...
  if (used <= s_len_protocol + 4) {
    return;
  }

  const CPerConnectionData *pPCD =
      reinterpret_cast<const CPerConnectionData *>(pSocket->GetUserPtr());
  ...
  LPCLIENT_NODE pCN = NULL;
  {
    ...
    KSG_DecodeBuf(used - (s_len_protocol + 4),
                  const_cast<unsigned char *>(pData + s_len_protocol + 4),
                  &(pCN->uClientKey));

    pCN->uClientKey = (unsigned int)(*(unsigned int *)(pData + s_len_protocol));

    pCN->pReadBuffer->AddData((const BYTE *)(pData + s_len_protocol + 4),
                              used - (s_len_protocol + 4));
  }
}

CServerFactory::CServerFactory()
    : m_lRefCount(0), m_nPlayerMaxCount(0), m_nPrecision(0),
      m_maxFreeBuffers_Cache(0), m_bufferSize_Cache(0) {}
...
STDMETHODIMP CServerFactory::CreateServerInterface(REFIID riid, void **ppv) {
  ...
  if (IID_IIOCPServer == riid) {
    const size_t maxPlayerCount =
        (m_nPlayerMaxCount > 0) ? m_nPlayerMaxCount : 100;
    const size_t precision = (m_nPrecision > 0) ? m_nPrecision : 10;

    const size_t maxFreeBuffers_Cache =
        (m_maxFreeBuffers_Cache > 0) ? m_maxFreeBuffers_Cache : 10;
    const size_t bufferSize_Cache =
        (m_bufferSize_Cache > 0) ? m_bufferSize_Cache : 8192;

    const size_t maxFreeBuffers = 10240;

    try {
      CIOCPServer *pObject = new CIOCPServer(
          maxPlayerCount, precision, maxPlayerCount, maxFreeBuffers,
          maxFreeBuffers_Cache, bufferSize_Cache);
...
#pragma pack()
...

      {
        ...
        SendAccountBegin.wLen =
            s_len_protocol + sizeof(SendAccountBegin.AccountBegin);

        SendAccountBegin.AccountBegin.ProtocolType = CIPHER_PROTOCOL_TYPE;
        SendAccountBegin.AccountBegin.Mode =
            m_theClientManager[dwIndex]->uKeyMode;
        SendAccountBegin.AccountBegin.ServerKey =
            ~m_theClientManager[dwIndex]->uServerKey;
        SendAccountBegin.AccountBegin.ClientKey =
            ~m_theClientManager[dwIndex]->uClientKey;
      }
      ...
      if (m_pfnCallBackServerEvent) {
        m_pfnCallBackServerEvent(m_lpCallBackParam, dwIndex,
                                 enumClientConnectCreate);
      }

      Trace(pData->GetConnectionDetails().c_str(),
            GetTimeStamp() + _T(" Add a client [ID:") + ToString(dwIndex) +
                _T( "] - Current total is " ) +
                ToString(m_usedClientNode.size()));
...
bool CIOCPServer::_HelperDelClient() {
  ...
  if ((DWORD)(-1) != dwIndex) {
    ...
    Trace(pData->GetConnectionDetails().c_str(),
          GetTimeStamp() + _T(" Del a client [ID:") + ToString(dwIndex) +
              _T( "] - Current total is " ) +
              ToString(m_usedClientNode.size()));
...
void CIOCPServer::ProcessMessage(Socket *pSocket, CIOBuffer *pBuffer) {
  const size_t used = pBuffer->GetUsed();
  static size_t minMessageSize = sizeof(WORD) + sizeof(BYTE) + 4;
  ...
  if (used >= minMessageSize) {
    ...
    LPCLIENT_NODE pCN;
    {
      ...
      while (used >= processedSize + messageSize) {
        KSG_DecodeBuf(messageSize - (sizeof(WORD) + 4),
                      const_cast<unsigned char *>(pData + sizeof(WORD) + 4),
                      &(pCN->uClientKey));

        pCN->uClientKey =
            (unsigned int)(*(unsigned int *)(pData + sizeof(WORD)));

        pSocket->GetMessageCallback()->OnMessage(
            (VOID *)(pData + sizeof(WORD) + 4),
            messageSize - (sizeof(WORD) + 4));

        pData += messageSize;

        processedSize += messageSize;

        if (used < processedSize + sizeof(WORD) + 4) {
