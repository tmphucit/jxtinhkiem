...
STDMETHODIMP CIOCPServer::OpenService( const unsigned long &ulnAddressToListenOn,
			const unsigned short &usnPortToListenOn, const unsigned short &usnPortToListenOn1 )
{
	HRESULT hr = E_FAIL;
...
void CIOCPServer::ProcessCommand( Socket *pSocket, const CIOBuffer *pBuffer )
{
	...
	LPCLIENT_NODE pCN = NULL;
	{
		...
		pCN->pReadBuffer->AddData( ( const BYTE * )( pData + s_len_protocol + 4), used - (s_len_protocol + 4) );
	}
}
...
void CIOCPServer::ProcessMessage( Socket *pSocket, CIOBuffer *pBuffer )
{
	...
	if ( used >= minMessageSize )
	{
		...
		LPCLIENT_NODE pCN;
		{
			...
			pCN->uClientKey = ( unsigned int )( *( unsigned int * )( pData + sizeof( WORD ) ) );




				pSocket->GetMessageCallback()->OnMessage((VOID*)(pData+sizeof( WORD ) + 4), 
						messageSize - (sizeof(WORD)+4));
