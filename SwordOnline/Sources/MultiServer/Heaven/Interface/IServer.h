...
DEFINE_GUID(IID_IIOCPServer, 0xfc6b562e, 0x89c2, 0x4fbc, 0xbf, 0x5f, 0x55, 0x3c,
            0x9, 0xae, 0x93, 0xd8);
...
interface IMessageProcess { virtual HRESULT OnMessage(VOID *, size_t) = 0; };
...
DECLARE_INTERFACE_(IServer, IUnknown) {
  ...
  (const unsigned long &ulnAddressToListenOn,
   ...
   const unsigned short &usnPortToListenOn1 = 0) PURE;
  ...
  (LPVOID lpParam, CALLBACK_SERVER_EVENT pfnEventNotify) PURE;

  /*
   * Install a callback funtion is used to get message from server
   */
  STDMETHOD(RegisterMsgFilter)
  (const unsigned long ulnClientID, IMessageProcess *pfnMsgNotify) PURE;
  ...
  */
  ...
  (const unsigned long &ulnClientID, size_t &datalength) PURE;

  /*
   * Disconnect a specified connection
   */
  STDMETHOD(ShutdownClient)
  (const unsigned long &ulnClientID) PURE;
...
DEFINE_GUID(IID_IServerFactory, 0xa4984b66, 0x8cde, 0x4752, 0x9b, 0xee, 0xda,
            0xcc, 0x6, 0xa6, 0x35, 0x48);

DECLARE_INTERFACE_(IServerFactory, IUnknown) {
  STDMETHOD(SetEnvironment)
  (const size_t &nPlayerMaxCount, const size_t &nPrecision,
   const size_t &maxFreeBuffers_Cache, const size_t &bufferSize_Cache) PURE;
