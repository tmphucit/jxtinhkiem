    ...
    if (!primary_db->open(primary_db, NULL, table_name, NULL, DB_BTREE,
      ...
      for (index = 0; index < index_number; index++) {
        ...
        if (!db_create(&index_db[index], dbenv, 0)) {
          ...
          if (index_db[index]->open(
                  index_db[index], NULL, index_table_name, NULL, DB_BTREE,
...
bool ZDBTable::commit() { return true; }
...
ZCursor *ZDBTable::_search(bool bKey, const char *key_ptr, int key_size,
                           int index) {
  DBT key, data, pkey;
