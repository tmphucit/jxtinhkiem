...
CClientNode::CDataQueue::CDataQueue(size_t bufferSize /*= 1024 * 64*/,
...
CClientNode::CClientNode(IServer *pServer, size_t id)
    : m_nIndentity(id), m_theAllocator(1024 * 64, 3), m_pServer(pServer) {
...
void CClientNode::DelNode(size_t id) {
  ...
  if (CClientNode::m_theClientMap.end() !=
      (it = CClientNode::m_theClientMap.find(id))) {
    CCriticalSection::Owner lock(CClientNode::m_csCL);
...
bool CClientNode::Start(IServer *pServer) {
  if (CClientNode::m_hThread == NULL) {
    ...
    CClientNode::m_hThread = (HANDLE)::_beginthreadex(
        0, 0, ThreadFunction, (void *)pServer, 0, &threadID);
...
unsigned int __stdcall CClientNode::ThreadFunction(void *pV) {
  ...
  try {
    while (!CClientNode::m_hQuitEvent.Wait(0)) {
      {
        ...
        for (it = CClientNode::m_theClientMap.begin();
             it != CClientNode::m_theClientMap.end(); it++) {
    ...
    ::MessageBox(NULL, "CClientNode::ThreadFunction was error!", "Warning",
                 MB_OK);
  }
...
void CClientNode::_QueryRoleList(const void *pData, size_t dataLength) {
  ...
  TProcessData *pListData = reinterpret_cast<TProcessData *>(
  ...
  {
    {
      ...
      if (pRoleData->BaseInfo.szName[0]) {
        for (size_t pos = sizeof(pRoleData->BaseInfo.szName) - 1; pos >= 1;
             pos--) {
...
void CClientNode::_RelayExtend(const void *pData, size_t dataLength) {
  ...
  case MAKEWORD(pf_gamemaster, gm_c2s_getrole): {
    ...
    case gm_role_entergame_position: {
      ...
      char *pReturnBuffer = (char *)_alloca(1 + sizeof(GM_GET_ROLE_DATA_SYNC) +
                                            sizeof(GM_ROLE_DATA_SUB_ENTER_POS));

      *pReturnBuffer = (char)s2c_extend;

      GM_GET_ROLE_DATA_SYNC *pGMReturn =
          (GM_GET_ROLE_DATA_SYNC *)(pReturnBuffer + 1);
      strncpy(pGMReturn->AccountName, pGMData->AccountName,
      ...
      GetRoleInfoForGM(pGMData->wGetID, (char *)(pGMReturn + 1),
                       pGMData->AccountName, nResultBuflen);
      ASSERT(pGMReturn->wLength == nResultBuflen);
      m_pServer->SendData(m_nIndentity, pReturnBuffer,
                          1 + sizeof(GM_GET_ROLE_DATA_SYNC) +
                              sizeof(GM_ROLE_DATA_SUB_ENTER_POS));
    } break;
    default:
      ASSERT(0);
  ...
  case MAKEWORD(pf_gamemaster, gm_c2s_setrole): {
    ...
    SetRoleInfoForGM(pGMData->wSetID, (char *)(pGMData + 1),
                     pGMData->AccountName, pGMData->wLength);
  } break;
  default:
    ASSERT(0);
    break;
...
void CClientNode::_GetRoleInfo(const void *pData, size_t dataLength) {
  ...
  TProcessData *pRoleData = reinterpret_cast<TProcessData *>(
      const_cast<BYTE *>(pBuffer->GetBuffer()));
  ...
  m_theSend.AddData(s2c_roleserver_getroleinfo_result, pBuffer->GetBuffer(),
                    nUsedLength, nIdentity);

  CBuffer *pPack = m_theSend.GetHeadPack(s2c_roleserver_getroleinfo_result);

  while (pPack) {
    m_pServer->SendData(m_nIndentity, (const void *)pPack->GetBuffer(),
                        pPack->GetUsed());
...
void CClientNode::_GetGameStat(
  ...
  // 需要发送的数据包
  CBuffer *pBuffer = m_theAllocator.Allocate();
  TProcessData *pStatData = reinterpret_cast<TProcessData *>(
      const_cast<BYTE *>(pBuffer->GetBuffer()));
  ...
  m_theSend.AddData(s2c_gamestatistic_bigpackage, pBuffer->GetBuffer(),
                    nUsedLen);
...
void CClientNode::_LockOrUnlockRole(const void *pData,
                                    size_t dataLength) // 强行加解锁
{
