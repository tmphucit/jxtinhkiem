...
public:
  ...
  struct TStatData {
    ...
    TRoleList
        MoneyStatBySect[11]
                       [SECTMAXSTATNUM]; // 各门派金钱最多排名列表[门派][玩家数]
    TRoleList
        LevelStatBySect[11]
                       [SECTMAXSTATNUM]; // 各门派级别最多排名列表[门派][玩家数]
...
private:
  ...
  static void ListSort( // 对列表做特定的排序
      TRoleList *const aRoleList, const int n, const StatType aType);
  static int GetIndexByName(char *aName, TRoleList *aList, int aListSize);
  static void
  RoleDataCopy(TRoleList *Desc,
               TRoleData *Source); // 把RoleData有用的结构复制到RoleList结构中
  static char *
  LoadFilterName(); // 取得要过滤的用户名列表 二维数组 [最大名字长度20][数目]
