	...
	for(i=0;i<n;++i)
	{
		...
		for(j=i+1;j<n;++j)
		{
			...
			case stMoney:
				...
				break;
			case stLevel:
				aDest = (__int64)(tmpData->Level + tmpData->HeightLevel * 200) * 0xffffffff + (__int64)tmpData->FightExp;
				aSourse = (__int64)(aRoleList[j].Level + aRoleList[j].HeightLevel * 200) * 0xffffffff + (__int64)aRoleList[j].FightExp;
				break;
			case stKiller:
				aDest = tmpData->KillNum;
...
{//生成发送给客户端的统计数据结构
	...
	for(i=0;i<10;++i)
	{
		...
		SendStatData.LevelStat[i].nHeightValue = newGameStatData.LevelStat[i].HeightLevel;
