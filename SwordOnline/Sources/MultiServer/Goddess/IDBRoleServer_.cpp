...
static ZDBTable *db_table_tong = NULL;
static ZDBTable *db_table_tong_member = NULL;

static HANDLE hDeadLockThreadTong = NULL;  // 检查数据库有没有死锁的线程
static HANDLE hRemoveLogThreadTong = NULL; // 删除没用日志文件的线程

static HANDLE hDeadLockThreadTongMember = NULL;  // 检查数据库有没有死锁的线程
static HANDLE hRemoveLogThreadTongMember = NULL; // 删除没用日志文件的线程
...
int get_tong(DB *db, const DBT *pkey, const DBT *pdata, DBT *ikey) {
  // 给定一个完整的buffer，得到account作为索引
  memset(ikey, 0, sizeof(DBT));
  TRole_Data_Tong_Member *pRoleData = (TRole_Data_Tong_Member *)pdata->data;

  ikey->data = pRoleData->szTongName;
  ikey->size = strlen(pRoleData->szTongName) + 1;
...
DWORD WINAPI DeadlockProcTong(LPVOID lpParameter) {
  while (!db_table_tong->bStop) {
    Sleep(5 * 1000); // 5秒钟一次
    db_table_tong->deadlock();
  }
  return 0;
}

DWORD WINAPI RemoveLogProcTong(LPVOID lpParameter) {
  while (!db_table_tong->bStop) {
    db_table_tong->removeLog();
    Sleep(60 * 60 * 1000); // 1小时一次
  }
  return 0;
}

DWORD WINAPI DeadlockProcTongMember(LPVOID lpParameter) {
  while (!db_table_tong_member->bStop) {
    Sleep(5 * 1000); // 5秒钟一次
    db_table_tong_member->deadlock();
  }
  return 0;
}

DWORD WINAPI RemoveLogProcTongMember(LPVOID lpParameter) {
  while (!db_table_tong_member->bStop) {
    db_table_tong_member->removeLog();
    Sleep(60 * 60 * 1000); // 1小时一次
  }
  return 0;
}
...
BOOL InitDBInterface(size_t nMaxRoleCount) {
  BOOL bCheckDB = FALSE;
  BOOL bCheckDBTong = FALSE;
  BOOL bCheckDBTongMember = FALSE;
  ...
  if (db_table->open()) {
    ...
    /*
                    ...
                    hDeadLockThread = CreateThread(
                            NULL,				// no security
       attributes 0,					// use default stack
       size DeadlockProc,		// thread function &dwThrdParam,
       // argument to thread function 0,
       // use default creation flags &dwThreadId);		// returns the
       thread identifier if(!hDeadLockThread)
                    {
                            //创建线程失败,暂时没有处理
                    }
    */
  ...
  db_table_tong = new ZDBTable("databasetong", "roledb");

  if (db_table_tong->open()) {
    DWORD dwThreadIdTong, dwThrdParamTong = 1;
    /*
    ...
    */
    ...
    hDeadLockThreadTong =
        CreateThread(NULL,              // no security attributes
                     0,                 // use default stack size
                     RemoveLogProcTong, // thread function
                     &dwThrdParamTong,  // argument to thread function
                     0,                 // use default creation flags
                     &dwThreadIdTong);  // returns the thread identifier
    if (!hRemoveLogThreadTong) {
      // 创建线程失败,暂时没有处理
    }
  ...
  db_table_tong_member = new ZDBTable("databasetongmember", "roledb");
  db_table_tong_member->addIndex(get_tong);

  if (db_table_tong_member->open()) {
    DWORD dwThreadIdTongMember, dwThrdParamTongMember = 1;
    /*
    ...
    */
    ...
    hRemoveLogThreadTongMember =
        CreateThread(NULL,                    // no security attributes
                     0,                       // use default stack size
                     RemoveLogProcTongMember, // thread function
                     &dwThrdParamTongMember,  // argument to thread function
                     0,                       // use default creation flags
                     &dwThreadIdTongMember);  // returns the thread identifier
    if (!hRemoveLogThreadTongMember) {
      // 创建线程失败,暂时没有处理
    }

    bCheckDBTongMember = TRUE;
  }

  if (bCheckDB && bCheckDBTong && bCheckDBTongMember) {
    return TRUE;
  }
...
void ReleaseDBInterface() // 释放数据库引擎
{
  ...
  if (db_table_tong) {
    ...
    */
    ...
    if (!hRemoveLogThreadTong)
      TerminateThread(hRemoveLogThreadTong, 0);

    db_table_tong->removeLog();

    db_table_tong->close();
    delete db_table_tong;
  }

  if (db_table_tong_member) {
    db_table_tong_member->commit();
    /*
    ...
    */
    ...
    if (!hRemoveLogThreadTongMember)
      TerminateThread(hRemoveLogThreadTongMember, 0);

    db_table_tong_member->removeLog();

    db_table_tong_member->close();
    delete db_table_tong_member;
  }
}

//[wxb 2003-7-23]
void SetRoleInfoForGM(int nInfoID, char *pRoleBuffer, char *strUser,
                      int nBufLen) {
  ...
  if (size) {
    ...
    case gm_role_entergame_position:
      ...
      memcpy(&(pRoleData->BaseInfo.ientergameid), pRoleBuffer,
             sizeof(GM_ROLE_DATA_SUB_ENTER_POS));
      break;
    default:
      ASSERT(0);
      return;
...
//[wxb 2003-7-22]
void *GetRoleInfoForGM(int nInfoID, char *pRoleBuffer, char *strUser,
                       int &nBufLen) {
  int size = 0;
  ...
  if (cursor) {
    ...
    switch (nInfoID) {
    case gm_role_entergame_position:
      nBufLen = sizeof(pRole->BaseInfo.ientergameid) +
                sizeof(pRole->BaseInfo.ientergamex) +
                sizeof(pRole->BaseInfo.ientergamey);
...
// bAutoInsertWhenNoExistUser
// 设为TRUE时表示，如果需要保存的该玩家在数据库中并不存在则自动加入到数据库中，FALSE则不增加直接返回错误
// 注意INI文件只须存放将需要改动的数据，不需改动的数据将自动保存原状。
int SaveRoleInfo(char *pRoleBuffer, const char *strUser,
                 BOOL bAutoInsertWhenNoExistUser) {
  ASSERT(pRoleBuffer);
  ...
  // 输出数据======
  char aStr[1024];
  sprintf(aStr, "SaveRoleInfo:%s dwDataLen=%d", pRoleData->BaseInfo.szName,
          pRoleData->dwDataLen);
  AddOutputString(hListOutput, aStr);
  //===============

  if (pRoleData->dwDataLen >= 64 * 1024)
    return 0; // 如果数据大于64K就不添加到数据库

  if (bAutoInsertWhenNoExistUser) { // 如果是新增角色就把账号名转成小写
  ...
  sprintf(aStr, "[%s] [%s] ", pName, pRoleData->BaseInfo.caccname);
  AddOutputString(hListOutput, aStr);

  if (db_table->add(pName, strlen(pName) + 1, pRoleBuffer,
                    pRoleData->dwDataLen)) {
    return 1;
  }
...
int GetRoleListOfAccount(char *szAccountName, S3DBI_RoleBaseInfo *RoleBaseList,
                         int nMaxCount) {
  int size = 0;
  int count = 0;
  ...
  ZCursor *cursor =
      db_table->search(szAccountName, strlen(szAccountName) + 1, 0);
  //	char *buffer = db_table->search( szAccountName, strlen( szAccountName )
  //+ 1, size, 0 );
...
void AddOutputString(HWND hListCtrl, char *aStr) { // 添加操作输出文字
  if (hListCtrl && ::IsWindow(hListCtrl)) {
    ...
    int nIndex = ::SendMessage(hListCtrl, LB_INSERTSTRING, 0,
                               (LPARAM)aStr); // 把新的信息添到第一个
  }
}