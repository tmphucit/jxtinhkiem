...
#define MAX_INDEX 128       // 一个数据表最多的索引数目
#define MAX_TABLE_NAME 2048 // 最长的表名字
...
protected:
  ...
  DB_ENV *dbenv; // 数据库环境
  ZCursor *_search(bool bKey, const char *key_ptr, int key_size,
                   int index);            // 搜索指定记录
...
public:
  ...
  ZCursor *search_key(const char *key_ptr, int key_size,
