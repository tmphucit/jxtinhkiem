...
void TGameStatistic::ListSort(TRoleList *const aRoleList, const int n,
...
void TGameStatistic::RoleDataCopy(
    TRoleList *Desc,
...
bool TGameStatistic::AllStat(TStatData &aStatData) { // 全部统计
  if (!DBTable)
    return false;
  int i;
  ...
  while (aBuffer) {
    ...
    if ((pRoleData->BaseInfo.nSect <= 10) && (pRoleData->BaseInfo.nSect >= 1)) {
      tmpData = GetMin(aStatData.MoneyStatBySect[pRoleData->BaseInfo.nSect], 10,
                       stMoney);
    } else {
    ...
    // 各门派对级别排序
    if ((pRoleData->BaseInfo.nSect <= 10) && (pRoleData->BaseInfo.nSect >= 1)) {
      tmpData = GetMin(aStatData.LevelStatBySect[pRoleData->BaseInfo.nSect], 10,
                       stMoney);
    } else {
  ...
  //////////////////////////////门派统计////////////////////////////////////
  for (i = 0; i < 100; ++i) { // 财富排名前一百玩家中各门派所占比例数
