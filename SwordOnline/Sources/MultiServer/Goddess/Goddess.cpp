...
void __stdcall ServerEventNotify(LPVOID lpParam, const unsigned long &ulnID,
  ...
  while (!g_hQuitEvent.Wait(0)) {
    {
      ...
      for (it = g_thePlayer.begin(); it != g_thePlayer.end(); it++) {
        ...
        size_t datalength = 0;
        const char *pData =
            (const char *)pServer->GetPackFromClient(index, datalength);
...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
  ...
  if (!g_libFilterText.Initialize() || !g_fltRoleName.Initialize()) {
    MessageBox(NULL, "text filter's initing has failed", "error",
               MB_OK | MB_ICONERROR);
    return -1;
  }

  HWND hwndMain = ::CreateDialog(hInstance, MAKEINTRESOURCE(IDD_DLG_GODDESS),
...
BOOL CALLBACK MainWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
  ...
  case WM_TIMER: {
    if (g_nLastServiceLoop != g_nServiceLoop) {
      ...
      ::SetDlgItemText(hwnd, IDC_STATIC_SERVICE,
                       (const char *)g_szBaseInfo + g_nServiceStep);
    ...
    if (g_nLastDBEngineLoop != g_nDBEngineLoop) {
      ...
      ::SetDlgItemText(hwnd, IDC_STATIC_DATABASE,
                       (const char *)g_szBaseInfo + g_nDBEngineStep);
  ...
  case WM_CLOSE: {
    if (IDYES == ::MessageBox(hwnd, "Are you sure quit?", "Info",
      ...
      g_BackupSleepTime = ::GetDlgItemInt(hwnd, IDC_EDIT_BACKUP_SLEEP_TIME,
                                          &bTranslated, FALSE);
  ...
  case WM_COMMAND:
    ...
    case IDC_BTN_BACKUP_MANUAL:
      ...
      if (!IsBackupThreadWorking())
        MessageBox(hwnd, "Backup thread is not started yet.", "Information",
    ...
    case IDC_BTN_BACKUP_SUS_RES:
      ...
      if (!IsBackupThreadWorking())
        ...
        if (SuspendBackupTimer()) {
          ...
          MessageBox(hwnd,
                     "Unable to suspend backup thread.\nPlease try later.",
                     "Information", MB_OK | MB_ICONEXCLAMATION);
      }
      break;
    }
  ...
  case WM_CREATE_ENGINE: {
    ...
    g_nMaxRoleCount =
        ::GetDlgItemInt(hwnd, IDC_EDIT_MAXNUM_ROLE, &bTranslated, FALSE);
    g_BackupSleepTime =
        ::GetDlgItemInt(hwnd, IDC_EDIT_BACKUP_SLEEP_TIME, &bTranslated, FALSE);
    ...
    /*		DWORD dwThreadId, dwThrdParam = 1;
       ...
       size MonitorProc,		// thread function &dwThrdParam,
  ...
  case WM_DEL_CLIENT: {
    ...
    if (hCtrl && ::IsWindow(hCtrl)) {
      ...
      for (int i = 0; i < nCount, LB_ERR != nCount; i++) {
        ...
        if (wParam ==
            (nSearchID = ::SendMessage(hCtrl, LB_GETITEMDATA, i, 0))) {
...
bool CreateDatabaseEngine(HWND hwnd) {
  if (!InitDBInterface(g_nMaxRoleCount)) {
    ::MessageBox(NULL, "Setup dbserver is failed!", "Warning",
                 MB_OK | MB_ICONSTOP);
  ...
  if (pFactroyFun &&
    ...
    //		pServerFactory->SetEnvironment( 2, 2, 20, 4 * 1024 * 1024 * 12
    //);

    pServerFactory->CreateServerInterface(
        IID_IIOCPServer, reinterpret_cast<void **>(&g_pServer));

    pServerFactory->Release();
  }

  if (!g_pServer) {
    ::MessageBox(NULL, "Initialization failed! Don't find a correct heaven.dll",
  ...
  g_hThread = ::CreateThread(NULL, 0, ThreadProcess, (void *)pCloneServer, 0,
                             &dwThreadID);
...
void LoadSetting() {
  ...
  g_BackupSleepTime =
      theConfigFile.ReadInteger("Setting", "BackupSleepTime", 3);
}
