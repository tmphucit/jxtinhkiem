...
unsigned CPlayerCreator::GetRevivalID(size_t nMapID,
                                      UINT nType /*= enumRandom*/) {
  stdMapID2RID::iterator it;
...
const TRoleData *CPlayerCreator::GetRoleData(unsigned int &uDataLength,
                                             LPROLEPARAM lpParam) {
  ...
  if (m_pRoleData[nIndex]) {
    ...
    strncpy(pData->BaseInfo.szName, lpParam->szName,
            sizeof(pData->BaseInfo.szName));
    pData->BaseInfo.szName[sizeof(pData->BaseInfo.szName) - 1] = '\0';

    strncpy(pData->BaseInfo.caccname, lpParam->szAccName,
            sizeof(pData->BaseInfo.caccname));
    ...
    if (lpParam->nMapID == 3)
      pData->BaseInfo.irevivalx = 3;
    else if (lpParam->nMapID == 53)
      pData->BaseInfo.irevivalx = 19;
    else if (lpParam->nMapID == 66)
      pData->BaseInfo.irevivalx = 18;
    else if (lpParam->nMapID == 4)
      pData->BaseInfo.irevivalx = 41;
...
bool CPlayerCreator::GetRoleDataFromIni(BYTE *pData, const char *szFileName) {
  ...
  pRoleData->BaseInfo.szAlias[0] = 0;
  ...
  TDBSkillData *pSkillData =
      (TDBSkillData *)(pData + pRoleData->dwFSkillOffset);
  for (int i = 0; i < nSkillCount; i++, pSkillData++) {
    ...
    pSkillData->m_nSkillId =
        cFile.ReadInteger(SECTION_FIGHTSKILLLIST, szSkillId, 0);
    pSkillData->m_nSkillLevel =
        cFile.ReadInteger(SECTION_FIGHTSKILLLIST, szSkillLevel, 0);
  }
  ...
  for (i = 0; i < nItemCount; i++) {
    ...
    // 物件基本信息**************************************************************
    pItemData->iequipclasscode =
        cFile.ReadInteger(szSection, "iequipclasscode", 0);
    pItemData->idetailtype = cFile.ReadInteger(szSection, "idetailtype", 0);
    pItemData->iparticulartype =
        cFile.ReadInteger(szSection, "iparticulartype", 0);
    ...
    pItemData->idurability = cFile.ReadInteger(szSection, "idurability", 0);
    ...
    pItemData->ihour = 0;
    pItemData->ibind = 0;
    pItemData->iShopCost = 0;
    pItemData->iNgoaiTrang = 0;
