...
public:
  ...
  void SetRoleSvrIP(const char *pAddress) {
    if (pAddress) {
      m_sRoleSvrIP = pAddress;
    }
  }
  unsigned short GetRoleSvrPort() { return m_nRoleSvrPort; }
  void SetRoleSvrPort(unsigned short nRoleSvrPort) {
    m_nRoleSvrPort = nRoleSvrPort;
  }

  unsigned short GetClientOpenPort() { return m_nClientOpenPort; }
  void SetClientOpenPort(unsigned short nClientOpenPort) {
    m_nClientOpenPort = nClientOpenPort;
  }

  unsigned short GetGameSvrOpenPort() { return m_nGameSvrOpenPort; }
  void SetGameSvrOpenPort(unsigned short nGameSvrOpenPort) {
    m_nGameSvrOpenPort = nGameSvrOpenPort;
  }
