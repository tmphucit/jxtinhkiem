...
CIntercessor::CIntercessor(unsigned long lnMaxPlayerCount,
                           CNetwork &theNetworkConfig)
    : m_hwndViewer(NULL)
      //		, m_pAccSvrClient( NULL )
      ,
      m_pDBRoleClient(NULL), m_pGameSvrServer(NULL), m_pPlayerServer(NULL),
      m_hWorkingThread(INVALID_HANDLE_VALUE),
      m_hHelperThread(INVALID_HANDLE_VALUE),
      m_hQuitEvent(NULL, true, false, NULL /*"PM_Quit_Event"*/),
      m_hStartupWoringThreadEvent(NULL, false, false,
                                  NULL /*"PM_WORKING_EVENT"*/),
      m_hStartupHelperThreadEvent(NULL, false, false,
                                  NULL /*"PM_HELPER_EVENT"*/),
...
bool CIntercessor::Create(HWND hwndViewer /* NULL */) {
  ...
  m_hWorkingThread = ::CreateThread(NULL, 0, WorkingThreadFunction,
  ...
  m_hHelperThread = ::CreateThread(NULL, 0, HelperThreadFunction, (void *)this,
                                   0, &dwThreadID);
...
&ulnEventType )
{
        CIntercessor *pPlayerManager = reinterpret_cast< CIntercessor * >(
lpParam );
...
void CIntercessor::_AccountEventNotify( const unsigned long &ulnEventType )
{
        ...
        case enumServerConnectCreate:
                ...
                ::PostMessage( m_hwndViewer, WM_SERVER_STATUS,
ACCOUNTSERVER_NOTIFY, CONNECTED );
        ...
        case enumServerConnectClose:
                ...
                ::PostMessage( m_hwndViewer, WM_SERVER_STATUS,
ACCOUNTSERVER_NOTIFY, DICONNECTED );

                break;
        }
}
*/
void __stdcall CIntercessor::DBRoleEventNotify(
...
void CIntercessor::_DBRoleEventNotify(const unsigned long &ulnEventType) {
  ...
  case enumServerConnectCreate:
    ...
    ::PostMessage(m_hwndViewer, WM_SERVER_STATUS, DBROLESERVER_NOTIFY,
                  CONNECTED);
...
void __stdcall CIntercessor::PlayerEventNotify(
...
bool CIntercessor::StartupNetwork() {
  ...
  pfnCreateClientInterface pClientFactroyFun =
      (pfnCreateClientInterface)(m_theRainbowLib.GetProcAddress(
          _T( "CreateInterface" )));
  ...
  if (pClientFactroyFun &&
    ...
    pClientFactory->CreateClientInterface(
        IID_IESClient, reinterpret_cast<void **>(&m_pDBRoleClient));
  ...
  if (m_pDBRoleClient) {
    ...
    m_pDBRoleClient->RegisterMsgFilter(reinterpret_cast<void *>(this),
                                       DBRoleEventNotify);

    if (SUCCEEDED(
            m_pDBRoleClient->ConnectTo(m_theNetworkConfig.GetRoleSvrIP(),
                                       m_theNetworkConfig.GetRoleSvrPort()))) {
  ...
  pfnCreateServerInterface pServerFactroyFun =
      (pfnCreateServerInterface)(m_theHeavenLib.GetProcAddress(
          _T( "CreateInterface" )));

  IServerFactory *pServerFactory = NULL;

  if (pServerFactroyFun &&
      SUCCEEDED(pServerFactroyFun(
          IID_IServerFactory, reinterpret_cast<void **>(&pServerFactory)))) {
    pServerFactory->SetEnvironment(m_lnMaxPlayerCount, 10, 50, 1024 * 8);

    //		pServerFactory->SetEnvironment( m_lnMaxPlayerCount, 10, 50, 1024
    //* 8 * 12);
    ...
    pServerFactory->CreateServerInterface(
        IID_IIOCPServer, reinterpret_cast<void **>(&m_pPlayerServer));

    /*
     * For gameserver
     */
    pServerFactory->CreateServerInterface(
        IID_IIOCPServer, reinterpret_cast<void **>(&m_pGameSvrServer));
  ...
  if (m_pGameSvrServer) {
    ...
    /*
                    ...
                    m_pGameSvrServer->QueryInterface( IID_IIOCPServer,
       reinterpret_cast< void ** >( &pCloneGameServer ) );
    ...
    */
    ...
    m_pGameSvrServer->RegisterMsgFilter(reinterpret_cast<void *>(&ni),
                                        CGameServer::GameSvrEventNotify);

    if (SUCCEEDED(m_pGameSvrServer->OpenService(
            INADDR_ANY, m_theNetworkConfig.GetGameSvrOpenPort()))) {
  ...
  if (m_pPlayerServer) {
    ...
    m_pPlayerServer->RegisterMsgFilter(reinterpret_cast<void *>(this),
                                       PlayerEventNotify);

    if (SUCCEEDED(m_pPlayerServer->OpenService(
            INADDR_ANY, m_theNetworkConfig.GetClientOpenPort()))) {
  ...
  if (bPlayerServerSucceed && bDBRoleServerSucceed && bGameSvrServerSucceed) {
    ...
    m_pPlayerServer->QueryInterface(IID_IIOCPServer,
                                    reinterpret_cast<void **>(&pClonePlayer));

    IClient *pCloneDBRole = NULL;
    m_pDBRoleClient->QueryInterface(IID_IESClient,
                                    reinterpret_cast<void **>(&pCloneDBRole));

    ASSERT(/* pCloneAcc && */ pClonePlayer && pCloneDBRole);

    CGamePlayer::ATTACH_NETWORK(NULL /* pCloneAcc */, pClonePlayer,
                                pCloneDBRole);
    ...
    IServer *pCloneGameServer = NULL;
    m_pGameSvrServer->QueryInterface(
        IID_IIOCPServer, reinterpret_cast<void **>(&pCloneGameServer));
  ...
  return (bPlayerServerSucceed && bDBRoleServerSucceed &&
...
DWORD WINAPI CIntercessor::WorkingThreadFunction(void *pV) {
  ...
  try {
    ...
    ::MessageBox(NULL, "Run a working thread is failed!", "CIntercessor class",
                 MB_OK);
  }
     ...
     */
    {
      ...
      for (size_t index = 0; index < m_lnMaxPlayerCount; index++) {
        ...
        if (pPlayer && pPlayer->IsActive()) {
          ...
          const void *pData =
              m_pPlayerServer->GetPackFromClient(index, datalength);
...
DWORD WINAPI CIntercessor::HelperThreadFunction(void *pV) {
  ...
  try {
    ...
    ::MessageBox(NULL, "Startup a helper thread is failed!",
                 "CIntercessor class", MB_OK);
  }
...
bool CIntercessor::AnalyzePlayerRequire(size_t index, const void *pData,
                                        size_t datalength) {
