...
const int CGamePlayer::s_nProcessTimeoutTimer = 4 * 30 * 1000;
...
OnlineGameLib::Win32::_tstring Table_Player_Check[MAXPLAYERCHECK];
int Table_Player_Time_Check[MAXPLAYERCHECK];

/*
 * CGamePlayer Global Function
 */
bool CGamePlayer::SetupGlobalAllocator(size_t bufferSize,
 ...
 * CGamePlayer::CTask
 */
CGamePlayer::CTask::CTask() { ASSERT(FALSE); }
...
CGamePlayer::CTask::~CTask() {
  ...
  for (theIterator = m_stdCommand.begin(); theIterator != m_stdCommand.end();
       theIterator++) {
...
CGamePlayer::CTask::CTask(CGamePlayer *pReceiver, UINT nTaskID)
    : m_pReceiver(pReceiver), m_indexCmd(0), m_nTaskProgID(nTaskID) {}
 ...
 * CGamePlayer class
 */
CGamePlayer::CDataQueue::CDataQueue(size_t bufferSize /*= 1024 * 64*/,
                                    size_t maxFreeBuffers /*= 1*/)
    : m_theDQAllocator(bufferSize, maxFreeBuffers) {}

CGamePlayer::CDataQueue::~CDataQueue() { Empty(); }
...
bool CGamePlayer::CDataQueue::AddData(LONG lnID, const BYTE *pData,
                                      size_t datalength) {
  bool ok = false;
...
CGamePlayer::CGamePlayer(UINT nIdentityID /*  = ( UINT )( -1 ) */)
    : m_lnIdentityID(nIdentityID), m_theLoginTask(this, enumLogin),
      ...
      m_theSafeCloseTask(this, enumSafeClose), m_nCurrentTaskID(0),
      m_nAttachServerID(-1), m_bActiveStatus(false), m_dwTaskBeginTimer(0),
      m_dwTaskTotalTimer(0), m_bAutoUnlockAccount(false),
      m_bUseSuperPassword(false), m_nExtPoint(-1) {
...
bool CGamePlayer::Active() {
  ...
  for (int i = 0; i < MAXPLAYERACCOUNT; i++) {
    m_sPlayerName[i] = "";
  }
...
bool CGamePlayer::Inactive() {
  ...
  /*	IGServer *pGServer = CGameServer::GetServer( m_nAttachServerID );
     ...
     m_sRoleName.c_str(), m_sRoleName.size() );
          }
  */
  m_nAttachServerID = -1;
...
bool CGamePlayer::IsWorking() { return (GetCurrentTask() != enumNone); }
...
bool CGamePlayer::AppendData(UINT nOwner, const void *pData,
                             size_t dataLength) {
...
bool CGamePlayer::SmallPackProcess(UINT nOwner, const void *pData,
                                   size_t dataLength) {
  switch (nOwner) {
...
bool CGamePlayer::LargePackProcess(UINT nOwner, const void *pData,
                                   size_t dataLength) {
  switch (nOwner) {
  case enumOwnerAccSvr:
...
bool CGamePlayer::DispatchTaskForAccount(const void *pData, size_t dataLength) {
  ...
  m_theDataQueue[enumOwnerAccSvr].AddData(cProtocol, (const BYTE *)pData,
                                          dataLength);
...
bool CGamePlayer::DispatchTaskForDBRole(const void *pData, size_t dataLength) {
  ...
  m_theDataQueue[enumOwnerRoleSvr].AddData(cProtocol, (const BYTE *)pData,
                                           dataLength);
...
bool CGamePlayer::DispatchTaskForPlayer(const void *pData, size_t dataLength) {
  ...
  m_theDataQueue[enumOwnerPlayer].AddData(cProtocol, (const BYTE *)pData,
                                          dataLength);
...
UINT CGamePlayer::QueryAccPwd() {
  ...
  */
  ...
  if (m_bUseSuperPassword) {
    ...
    nMinLen =
        m_sSuperPassword.size() > nMinLen ? nMinLen : m_sSuperPassword.size();
...
UINT CGamePlayer::VerifyAccount() {
  ...
  CBuffer *pRetBuffer =
      m_theDataQueue[enumOwnerAccSvr].Attach(s2c_accountlogin);

  if (pRetBuffer) {
    KAccountUserReturnExt *pReturn =
        (KAccountUserReturnExt *)(pRetBuffer->GetBuffer() +
                                  1 /* size of a protocol byte */);
...
bool CGamePlayer::_VerifyAccount_ToPlayer(UINT nQueryResult,
  ...
  used =
      (used > m_sAccountName.length()) ? m_sAccountName.length() : (used - 1);
  ...
  m_pPlayerServer->SendData(
      m_lnIdentityID, pData,
      (sizeof(KLoginAccountInfo) + sizeof(BYTE) /* protocol id */));
...
UINT CGamePlayer::QueryRoleList() {
  ...
  TProcessData *pPlayerList = reinterpret_cast<TProcessData *>(
...
UINT CGamePlayer::ProcessRoleList() {
  ...
  if (pRetBuffer) {
    ...
    m_sPlayerCount = pPD->pDataBuffer[0];
    if (m_sPlayerCount > MAXPLAYERACCOUNT)
      m_sPlayerCount = MAXPLAYERACCOUNT;
    else if (m_sPlayerCount < 0)
      m_sPlayerCount = 0;

    if (strcmp(pPD->szAccountName, m_sAccountName.c_str()) != 0) {
      SAFE_RELEASE(pRetBuffer);
    ...
    RoleBaseInfo *PlayerListActive = (RoleBaseInfo *)&pPD->pDataBuffer[1];

    for (int i = 0; i < m_sPlayerCount; i++) {
      if (PlayerListActive->szName[0]) {
        m_sPlayerName[i] = PlayerListActive->szName;
        PlayerListActive++;
      } else {
        m_sPlayerName[i] = "";
        m_sPlayerCount = i;
    ...
    if (nRoleCount >= 0) {
      ...
      m_pPlayerServer->SendData(m_lnIdentityID, pRetBuffer->GetBuffer(),
                                pRetBuffer->GetUsed());
...
UINT CGamePlayer::SelAddDelRole() {
  ...
  if (pRetBuffer) {
    ...
    const NEW_PLAYER_COMMAND *pNPC =
        (const NEW_PLAYER_COMMAND *)(pCRPD->pDataBuffer);
   ...
   */
  pRetBuffer =
      m_theDataQueue[enumOwnerPlayer].Attach(c2s_roleserver_deleteplayer);
  ...
  if (pRetBuffer) {
    ...
    if (enumToNextTask != nNextTask) {
      ...
      m_pPlayerServer->SendData(m_lnIdentityID, (const void *)&ndrr,
                                sizeof(tagNewDelRoleResponse));
...
UINT CGamePlayer::DelRole_WaitForVerify() {
  ...
  if (pRetBuffer) {
    ...
    if (nResult == ACTION_SUCCESS) {
      ...
      TProcessData *pRoleInfo = reinterpret_cast<TProcessData *>(
          const_cast<BYTE *>(pBuffer->GetBuffer()));
...
UINT CGamePlayer::WaitForCreateResult() {
  ...
  CBuffer *pRetBuffer =
      m_theDataQueue[enumOwnerRoleSvr].Attach(s2c_roleserver_createrole_result);
...
UINT CGamePlayer::WaitForDeleteResult() {
  ...
  CBuffer *pRetBuffer =
      m_theDataQueue[enumOwnerRoleSvr].Attach(s2c_roleserver_deleterole_result);
...
bool CGamePlayer::_CreateNewPlayer_ToDBRole(const char *pRoleName,
  ...
  if (pRoleData && datalength) {
    ...
    TProcessData *pPD = reinterpret_cast<TProcessData *>(
        const_cast<BYTE *>(pData->GetBuffer()));
    ...
    m_thePackager.AddData(c2s_roleserver_createroleinfo, (const BYTE *)pPD,
                          nRoleInfoLength);

    /*
     * For create a role
     */
    m_theDataQueue[enumOwnerRoleSvr].AddData(
        s2c_roleserver_getroleinfo_result, (const BYTE *)pPD, nRoleInfoLength);
...
UINT CGamePlayer::ProcessRoleInfo() {
  ...
  CBuffer *pRetBuffer = m_theDataQueue[enumOwnerRoleSvr].Attach(
      s2c_roleserver_getroleinfo_result);
  ...
  if (pRetBuffer) {
    ...
    if (1 == cResult &&
        _SyncRoleInfo_ToGameServer((const void *)(&pPA->pDataBuffer[1]),
                                   pPA->nDataLen - 1)) {
      nNextTask = enumToNextTask;
    } else {
      tagNotifyPlayerLogin npl;
...
bool CGamePlayer::_SyncRoleInfo_ToGameServer(const void *pData,
                                             size_t dataLength) {
  bool ok = false;
...
UINT CGamePlayer::WaitForGameSvrPermit() {
  ...
  CBuffer *pRetBuffer =
      m_theDataQueue[enumOwnerPlayer].Attach(s2c_notifyplayerlogin);

  if (pRetBuffer && m_pPlayerServer) {
    UINT nNextTask = enumError;

    tagNotifyPlayerLogin *pNPL =
        (tagNotifyPlayerLogin *)pRetBuffer->GetBuffer();

#ifdef CONSOLE_DEBUG
    cprintf("CGamePlayer::WaitForGameSvrPermit Notify player to login "
            "gameserver\n");
#endif
