...
for (int i=0;i<m_sPlayerCount;i++)
{
if (m_sPlayerName[i] == pDSPC->szRoleName && m_sAccountName == pDSPC->szAccountName && m_sPassword == pDSPC->Password.szPassword)
{
CheckLogin = 1;
break;
}
...
if (CheckNewPlayer == 1)
{
	...
	if (pNPC->m_NativePlaceId == 53 || pNPC->m_NativePlaceId == 3 || pNPC->m_NativePlaceId == 66 || pNPC->m_NativePlaceId == 4)
	{
...
const tagDBDelPlayer *pDDPC = ( const tagDBDelPlayer * )pRetBuffer->GetBuffer();

//int CheckDelete = 0;

//for (int j=0;j<m_sPlayerCount;j++)
//{
//if (m_sPlayerName[j] == pDDPC->szRoleName && m_sAccountName == pDDPC->szAccountName)
//{
//CheckDelete = 1;
//break;
//}
