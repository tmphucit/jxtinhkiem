...
CNetwork::CNetwork()
    ...
    , */
      m_nAccSvrPort(0), m_nRoleSvrPort(0), m_nClientOpenPort(0),
...
bool CNetwork::Create() {
  ...
  DWORD dwLen = theConfigFile.ReadString(
      "Network", "RoleSvrIP", const_cast<char *>(m_sRoleSvrIP.c_str()),
      MAX_STRING_LEN, "");
  ...
  dwLen = theConfigFile.ReadString("Network", "AccSvrIP",
                                   const_cast<char *>(m_sAccSvrIP.c_str()),
                                   MAX_STRING_LEN, "");
  ...
  m_nGameSvrOpenPort =
      theConfigFile.ReadInteger("Network", "GameSvrOpenPort", 0);
...
//	 */
//	pfnCreateClientInterface pClientFactroyFun = ( pfnCreateClientInterface
//)( m_theRainbowLib.GetProcAddress( _T( "CreateInterface" ) ) );
//
...
/*void __stdcall CNetwork::AccSvrEventNotify( LPVOID lpParam, const unsigned
long &ulnEventType )
{
