...
CSmartClient::CSmartClient()
    : m_hWorkingThread(NULL), m_pClient(NULL), m_hwndContain(NULL),
      m_nSvrPort(0), m_theAllocator(1024, 500) {}
...
bool CSmartClient::Open(const char *const pAddressToConnectServer,
                        unsigned short usPortToConnectServer, HWND hwndContain,
...
void CSmartClient::_EventNotify(const unsigned long &ulnEventType) {
  ...
  case enumServerConnectCreate:
    ::PostMessage(m_hwndContain, WM_SERVER_STATUS, ACCOUNTSERVER_NOTIFY,
                  CONNECTED);
    break;
...
DWORD WINAPI CSmartClient::WorkingThreadFunction(void *pV) {
  ...
  try {
    ...
    ::MessageBox(NULL, "Run a working thread is failed!", "CSmartClient class",
                 MB_OK);
  }
...
void CSmartClient::_Verify(IClient *pClient, bool bDisconnect) {
  ...
  while (pClient) {
    ...
    case s2c_gatewayverify: {
      ...
      nLen--;
      if (pReturn && (g_dwServerIdentify == pReturn->Operate) &&
        ...
        if (ACTION_SUCCESS == pReturn->nReturn) {
          ...
          ::PostMessage(m_hwndContain, WM_SERVER_LOGIN_FAILED,
                        enumUsrNamePswdErr, 0L);

          SetClientStatus(enumClientError);
        } else if (E_ADDRESS_OR_PORT == pReturn->nReturn) {
          ::PostMessage(m_hwndContain, WM_SERVER_LOGIN_FAILED, enumIPPortErr,
                        0L);

          SetClientStatus(enumClientError);
        } else {
          ::PostMessage(m_hwndContain, WM_SERVER_LOGIN_FAILED, enumException,
                        0L);
  ...
  if (IsNormal()) {
    if (m_pClient) {
       ...
       */
      {
        ...
        stdList::iterator it;
        for (it = m_theHistroyList.begin(); it != m_theHistroyList.end();
             it++) {
          CBuffer *pBuffer = *it;

          if (pBuffer) {
            m_pClient->SendPackToServer((const void *)(pBuffer->GetBuffer()),
                                        pBuffer->GetUsed());
