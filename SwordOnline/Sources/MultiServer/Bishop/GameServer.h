...
public:
  ...
  CGameServer(IServer *pGameSvrServer, IClient *pAccountClient,
  ...
  virtual bool DispatchTask(UINT nTask, const void *pData, size_t datalength,
                            WORD nData);
  ...
  virtual size_t GetIP(BYTE cType) {
    if (INTERNET_IP == cType) {
      return m_nServerIP_Internet;
    }
    return m_nServerIP_Intraner;
  };
...
protected:
  ...
  bool PopAccount(const char *pAccountName, bool bUnlockAccount,
                  WORD nExtPoint);
...
private:
   ...
   */
  typedef bool (CGameServer::*ProcessProtocolFun[c2s_end])(const void *pData,
                                                           size_t datalength);
