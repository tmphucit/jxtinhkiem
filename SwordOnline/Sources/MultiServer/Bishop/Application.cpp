...
2000中并不一定是连续分配的 void getmac_one(int lana_num, ASTAT& Adapter)
{
    NCB ncb;
    UCHAR uRetCode;
...
BOOL CALLBACK CBishopApp::LoginDlgProc(HWND hwndDlg, UINT message,
  ...
  case WM_INITDIALOG:
    ...
    ::SetDlgItemInt(hwndDlg, IDC_EDIT_LOGONTO_PORT,
                    m_theNetwork.GetAccSvrPort(), FALSE);
...
void CBishopApp::EnterToAffirm(HWND hwndDlg) {
  ...
  /*
   * Start up a listening thread to wait a required confirm from the account
   * server
   */
  // DWORD dwThreadID = 0;

  // HANDLE hThread = ::CreateThread( NULL, 0, ServerLoginRoutine, ( void *
  // )hwndDlg, 0, &dwThreadID );
...
bool CBishopApp::CheckUserInfo(HWND hwndDlg) {
  ...
  if (0 == (nLen = ::GetDlgItemText(hwndDlg, IDC_EDIT_USERNAME, szBuffer,
                                    MAX_PATH))) {
    ::SetFocus(::GetDlgItem(hwndDlg, IDC_EDIT_USERNAME));

    ::MessageBox(hwndDlg, "Please input a valid username!", "Warning",
                 MB_ICONASTERISK);

    return false;
  }

  if (0 == (nLen = ::GetDlgItemText(hwndDlg, IDC_EDIT_LOGONTO_IP, szBuffer,
                                    MAX_PATH))) {
    ::SetFocus(::GetDlgItem(hwndDlg, IDC_EDIT_LOGONTO_IP));

    ::MessageBox(hwndDlg, "Please input a valid IP!", "Warning",
                 MB_ICONASTERISK);
  ...
  if (0 == (nLen = ::GetDlgItemInt(hwndDlg, IDC_EDIT_LOGONTO_PORT, &bTranslated,
                                   TRUE))) {
    ::SetFocus(::GetDlgItem(hwndDlg, IDC_EDIT_LOGONTO_PORT));

    ::MessageBox(hwndDlg, "Please input a valid port!", "Warning",
                 MB_ICONASTERISK);
...
DWORD WINAPI CBishopApp::ServerLoginRoutine(HWND hwndDlg) {
  ...
  /*	IClient *pAccSvrClient = m_theNetwork.CreateAccSvrClient( szBuffer,
     nPort );
...
BOOL CBishopApp::RegisterMainWndClass() {
  ...
  wcx.hbrBackground =
      (HBRUSH)GetStockObject(WHITE_BRUSH);      // white background brush
  wcx.lpszMenuName = NULL;                      // name of menu resource
  wcx.lpszClassName = _T( "IntercessorClass" ); // name of window class
  wcx.hIconSm = (HICON)LoadImage(
      m_hInst, // small class icon
      MAKEINTRESOURCE(IDI_LOCK), IMAGE_ICON, GetSystemMetrics(SM_CXSMICON),
...
UINT CBishopApp::MainDialog() {
  ...
  */
  HWND hwndMain = ::CreateDialog(m_hInst, MAKEINTRESOURCE(IDD_DLG_INTERCESSOR),
...
BOOL CALLBACK CBishopApp::MainWndProc(HWND hwnd, UINT msg, WPARAM wParam,
                                      LPARAM lParam) {
  ...
  case WM_CLOSE:
    ...
    if (IDYES == ::MessageBox(hwnd, "Are you sure quit?", "Info",
  ...
  case WM_COMMAND:
    ...
    case IDC_BTN_SEND_MSG: {
      BOOL bToAllGS = FALSE;
      if (BST_CHECKED ==
          ::IsDlgButtonChecked(hwnd, IDC_INDEX_SEND2GAMESERVER)) {
        bToAllGS = TRUE;
      }
      ...
      uLength =
          ::GetDlgItemText(hwnd, IDC_EDIT_ANNOUNCE_MSG, szAnnounceText, 256);
      szAnnounceText[uLength] = '\0';

      SendAnnounceText(hwnd, (const char *)szAnnounceText, uLength, uOption,
                       bToAllGS);
    } break;
    ...
    case IDC_BTN_GAMESVRINFO: {
      ...
      if (hCtrl && ::IsWindow(hCtrl)) {
        ...
        if (LB_ERR != (nSelIndex = ::SendMessage(hCtrl, LB_GETCURSEL, 0, 0))) {
          ...
          if (LB_ERR != (nSearchID = ::SendMessage(hCtrl, LB_GETITEMDATA,
                                                   nSelIndex, 0))) {
            ...
            if (pGServer) {
              ...
              bRet = ::SetDlgItemInt(hwnd, IDC_STATIC_GAMESERVER_CONTEXT, nNum,
                                     TRUE);
            }
          }
        }
...
void CBishopApp::InitMainDlg(HWND hDlg) {
  ...
  ::SetDlgItemInt(hDlg, IDC_EDIT_ACCSVRPORT, m_theNetwork.GetAccSvrPort(),
                  FALSE);

  ::SetDlgItemText(hDlg, IDC_EDIT_ROLESVRIP, m_theNetwork.GetRoleSvrIP());
  ::SetDlgItemInt(hDlg, IDC_EDIT_ROLESVRPORT, m_theNetwork.GetRoleSvrPort(),
                  FALSE);

  ::SetDlgItemInt(hDlg, IDC_EDIT_CLIENT_PORT, m_theNetwork.GetClientOpenPort(),
                  FALSE);

  ::SetDlgItemInt(hDlg, IDC_EDIT_GAMESVR_PORT,
                  m_theNetwork.GetGameSvrOpenPort(), FALSE);
...
void CBishopApp::SendAnnounceText(HWND hwndDlg, const char *pText, UINT uLength,
                                  UINT uOption, BOOL bAllGS) {
  if (!pText || 0 == uLength) {
    ::MessageBox(hwndDlg, "Please input a valid message!", "Warning",
  ...
  if (bAllGS) {
    ...
    if (hCtrl && ::IsWindow(hCtrl)) {
      ...
      if (LB_ERR != (nSelIndex = ::SendMessage(hCtrl, LB_GETCURSEL, 0, 0))) {
        ...
        if (LB_ERR !=
            (nSearchID = ::SendMessage(hCtrl, LB_GETITEMDATA, nSelIndex, 0))) {
          IGServer *pGServer = CGameServer::GetServer(nSearchID);
