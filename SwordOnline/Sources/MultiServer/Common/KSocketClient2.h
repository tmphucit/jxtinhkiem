...
public:
  ...
  ConnectTo(const char *const &pAddressToConnectServer,
            unsigned short usPortToConnectServer);

  virtual HRESULT STDMETHODCALLTYPE
  RegisterMsgFilter(LPVOID lpParam, CALLBACK_CLIENT_EVENT pfnEventNotify);

  // Send some data to the current server
  virtual HRESULT STDMETHODCALLTYPE SendPackToServer(const void *const pData,
                                                     const size_t &datalength);

  // Get some data from the current server
  virtual const void *STDMETHODCALLTYPE GetPackFromServer(size_t &datalength);
