...
CIniFile::CIniFile() { memset(m_szFileName, 0, MAX_PATH); }
...
int CIniFile::ReadInteger(LPCTSTR lpSectionName, LPCTSTR lpKeyName,
                          int nDefault) {
...
BOOL CIniFile::WriteInteger(LPCTSTR lpSectionName, LPCTSTR lpKeyName,
                            int nValue) {
  char szStrOut[TCS_INTBUFFER];

  wsprintf(szStrOut, "%d", nValue);

  return ::WritePrivateProfileString(lpSectionName, lpKeyName, szStrOut,
                                     m_szFileName);
}

DWORD CIniFile::ReadHexNum(LPCTSTR lpSectionName, LPCTSTR lpKeyName,
                           DWORD dwDefault) {
  char szStr[TCS_KEYBUFFER];
  szStr[0] = 0;
...
BOOL CIniFile::WriteHexNum(LPCTSTR lpSectionName, LPCTSTR lpKeyName,
                           DWORD dwValue) {
  char szStrOut[TCS_INTBUFFER];
...
DWORD CIniFile::ReadString(LPCTSTR lpSectionName, LPCTSTR lpKeyName,
                           LPTSTR lpString, DWORD dwSize, LPCTSTR lpDefault) {
  ASSERT(lpSectionName && lpKeyName);

  return ::GetPrivateProfileString(lpSectionName, lpKeyName, lpDefault,
                                   lpString, dwSize, m_szFileName);
}

BOOL CIniFile::WriteString(LPCTSTR lpSectionName, LPCTSTR lpKeyName,
                           LPCTSTR lpString) {
  ASSERT(lpSectionName && lpKeyName);
...
BOOL CIniFile::ReadSections(_VETSTR &stdSections) {
  ...
  if (::GetPrivateProfileString(NULL, NULL, NULL, (char *)pvData, 16384,
                                m_szFileName)) {
...
BOOL CIniFile::ReadSection(LPCTSTR lpSection, _VETSTR &stdKey) {
  ...
  if (::GetPrivateProfileString(lpSection, NULL, NULL, (char *)pvData, 16384,
                                m_szFileName)) {
