...
protected:
  ...
  CSocketServer(size_t maxFreeSockets, size_t maxFreeBuffers,
                size_t bufferSize = 1024, size_t numThreads = 0,
                bool useSequenceNumbers = true, bool postZeroByteReads = false);
...
private:
  ...
  virtual void OnConnectionError(enumConnectionErrorSource source,
                                 Socket *pSocket, CIOBuffer *pBuffer,
                                 DWORD lastError);
  ...
  void PostIoOperation(Socket *pSocket, CIOBuffer *pBuffer,
...
public:
  ...
  bool Write(const char *pData, size_t dataLength, bool throwOnFailure = false);

  bool Write(const BYTE *pData, size_t dataLength, bool throwOnFailure = false);
  ...
  void SetMessageCallback(IMessageProcess *pIM) { m_pIM = pIM; }

  IMessageProcess *GetMessageCallback() const { return m_pIM; }

private:
  Socket(CSocketServer &server, SOCKET socket, bool useSequenceNumbers);

  ~Socket();

  void Attach(SOCKET socket);

  void *GetServerDataPtr() const { return ServerData::GetUserPtr(); }

  void SetServerDataPtr(void *pData) { ServerData::SetUserPtr(pData); }
  ...
  void OnConnectionError(CSocketServer::enumConnectionErrorSource source,
                         CIOBuffer *pBuffer, DWORD lastError);
...
protected:
  ...
  virtual void HandleOperation(Socket *pSocket, CIOBuffer *pBuffer,
                               DWORD dwIoSize, bool weClosedSocket);
