...
int KSG_DecodeEncode_ASM(size_t uSize, unsigned char *pbyBuf, unsigned *puKey) {
  __asm mov eax, [uSize] __asm mov esi, [puKey] __asm mov edi,
      [pbyBuf] __asm mov ecx, eax __asm mov edx,
      [esi] // key
      ...
      2 __asm jz Next1
          ...
          Loop1 : __asm mov ebx,
                  [edi] __asm add edi,
                  4 __asm xor ebx,
                  edx __asm dec eax __asm mov[edi - 4],
                  ebx __asm jnz Loop1
                      ...
                      Next1 : __asm and ecx,
                              ...
                              __asm jz Next2
                                  ...
                                  Loop2 : __asm mov bl,
                                          [edi] __asm inc edi __asm xor bl,
                                          al __asm shr eax,
                                          8 __asm mov[edi - 1],
                                          bl __asm dec ecx __asm jnz Loop2 Next2
      :

      __asm mov eax,
      edx // NewKey = OldKey * 31 +  134775813L
      __asm shl edx,
      5 __asm sub edx,
      eax __asm add edx,
      134775813 // 08088405H
