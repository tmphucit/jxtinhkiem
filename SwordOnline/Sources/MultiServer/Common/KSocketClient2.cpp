...
KSocketClient2::~KSocketClient2() { WSACleanup(); }
...
KSocketClient2::ConnectTo(const char *const &pAddressToConnectServer,
...
HRESULT STDMETHODCALLTYPE KSocketClient2::RegisterMsgFilter(
    LPVOID lpParam, CALLBACK_CLIENT_EVENT pfnEventNotify) {
...
const void *STDMETHODCALLTYPE
KSocketClient2::GetPackFromServer(size_t &datalength) {
  static struct timeval s_timeout = {0, 0};
  int nRetCode = false;

  if (m_nReturnBufferFlag) {
    unsigned char *pbyNextInBuffer =
        (m_InBuffer + (unsigned)(*(WORD *)m_InBuffer));
  ...
  while (true) {
    ...
    if (!((m_pbyInBufferEnd < (m_InBuffer + sizeof(WORD))) ||
          (m_pbyInBufferEnd <
           (m_InBuffer + (unsigned)(*(WORD *)m_InBuffer))))) {
...
HRESULT STDMETHODCALLTYPE KSocketClient2::Shutdown() {
  ...
  if (sckSocket != INVALID_SOCKET) {
    ...
    setsockopt(sckSocket, SOL_SOCKET, SO_LINGER, (char *)&lingerStruct,
               sizeof(lingerStruct));
