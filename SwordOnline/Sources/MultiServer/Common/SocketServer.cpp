 ...
 * CSocketServer
 */
CSocketServer::CSocketServer(size_t maxFreeSockets, size_t maxFreeBuffers,
    ...
    : CIOBuffer::Allocator(bufferSize, maxFreeBuffers),
      ...
      m_listeningSocket(INVALID_SOCKET), m_listeningSocket1(INVALID_SOCKET),
      m_iocp(0), m_maxFreeSockets(maxFreeSockets),
...
void CSocketServer::Open(unsigned long addressToListenOn,
                         ...
                         unsigned short portToListenOn1) {
  m_address = addressToListenOn;
  m_port = portToListenOn;
  m_port1 = portToListenOn1;
}
...
void CSocketServer::StartAcceptingConnections() {
  ...
  if (m_port1) {
    ...
    if (m_listeningSocket == INVALID_SOCKET &&
        m_listeningSocket1 == INVALID_SOCKET) {
      OnStartAcceptingConnections();

      m_listeningSocket = CreateListeningSocket(m_address, m_port);

      m_listeningSocket1 = CreateListeningSocket(m_address, m_port1);
  ...
  if (m_port1) {
    ...
    if (m_listeningSocket != INVALID_SOCKET &&
      ...
      if (0 != ::closesocket(m_listeningSocket)) {
        OnError(
            _T("CSocketServer::StopAcceptingConnections() - closesocket - ") +
            GetLastErrorMessage(::WSAGetLastError()));
      }

      m_listeningSocket = INVALID_SOCKET;

      if (0 != ::closesocket(m_listeningSocket1)) {
        OnError(
            _T("CSocketServer::StopAcceptingConnections() - closesocket - ") +
            GetLastErrorMessage(::WSAGetLastError()));
      }
...
void CSocketServer::InitiateShutdown() {
  ...
  /*
   * signal that the dispatch thread should shut down all worker threads and
   * then exit
   */
  m_shutdownEvent.Set();
...
SOCKET CSocketServer::CreateListeningSocket(unsigned long address,
                                            unsigned short port) {
...
CSocketServer::CreateWorkerThread(CIOCompletionPort &iocp) {
...
int CSocketServer::Run() {
  try {
      ...
      if (waitResult == WAIT_OBJECT_0) {
        ...
        while (!m_shutdownEvent.Wait(0) && m_acceptConnectionsEvent.Wait(0)) {
          ...
          SOCKET acceptedSocket =
              ::WSAAccept(m_listeningSocket,
                          reinterpret_cast<sockaddr *>(
                              const_cast<BYTE *>(pAddress->GetBuffer())),
                          &addressSize, 0, 0);
    ...
    OnError(_T("CSocketServer::Run() - Exception: ") + e.GetWhere() +
            _T(" - ") + e.GetMessage());
  } catch (...) {
...
void CSocketServer::WriteCompleted(Socket * /*pSocket*/, CIOBuffer *pBuffer) {
  if (pBuffer->GetUsed() != pBuffer->GetWSABUF()->len) {
    OnError(_T("CSocketServer::WorkerThread::WriteCompleted - Socket write ")
            _T("where not all data was written"));
  }
}
...
bool CSocketServer::FilterSocketClose(Socket * /*pSocket*/) { return true; }

void CSocketServer::PostIoOperation(Socket *pSocket, CIOBuffer *pBuffer,
...
CSocketServer::Socket::Socket(CSocketServer &server, SOCKET theSocket,
    ...
    : m_server(server), m_socket(theSocket), m_ref(1), m_outstandingWrites(0),
      m_readShutdown(false), m_writeShutdown(false), m_closing(false),
      m_clientClosed(false), m_pSequenceData(0), m_pIM(0) {
  if (!IsValid()) {
...
void CSocketServer::Socket::AddRef() { ::InterlockedIncrement(&m_ref); }
...
bool CSocketServer::Socket::WritePending() {
  if (m_writeShutdown) {
    DEBUG_ONLY(Output(_T("CSocketServer::Socket::WritePending() - Attempt to ")
                      _T("write after write shutdown")));
...
void CSocketServer::Socket::AbortiveClose() {
  ...
  if (SOCKET_ERROR == ::setsockopt(m_socket, SOL_SOCKET, SO_LINGER,
                                   (char *)&lingerStruct,
                                   sizeof(lingerStruct))) {
    m_server.OnError(_T("CSocketServer::Socket::AbortiveClose() - setsockopt( ")
                     _T("SO_LINGER ) - ") +
                     GetLastErrorMessage(::WSAGetLastError()));
  }
...
void CSocketServer::Socket::InternalClose() {
  ...
  if (IsValid()) {
    if (0 != ::closesocket(m_socket)) {
      m_server.OnError(
          _T("CSocketServer::Socket::InternalClose() - closesocket - ") +
          GetLastErrorMessage(::WSAGetLastError()));
    }
...
bool CSocketServer::Socket::Read(CIOBuffer *pBuffer /* = 0 */,
                                 bool throwOnFailure /* = false*/) {
  if (!IsValid()) {
    if (throwOnFailure) {
      throw CException(_T("CSocketServer::Socket::Read()"),
                       _T("Socket is closed"));
    } else {
      return false;
    }
  ...
  /*
   * Post a read request to the iocp so that the actual socket read gets
   * performed by one of the server's IO threads...
   */
  ...
  m_server.PostIoOperation(this, pBuffer,
                           m_server.m_postZeroByteReads
...
bool CSocketServer::Socket::Write(const char *pData, size_t dataLength,
                                  bool throwOnFailure /* = false*/) {
  return Write(reinterpret_cast<const BYTE *>(pData), dataLength,
               throwOnFailure);
}

bool CSocketServer::Socket::Write(const BYTE *pData, size_t dataLength,
                                  bool throwOnFailure /* = false*/) {
  if (!IsValid()) {
    if (throwOnFailure) {
      /*
       ...
       */
      throw CException(_T("CSocketServer::Socket::Write()"),
                       _T("Socket is closed"));
    } else {
      return false;
    }
...
bool CSocketServer::Socket::Write(CIOBuffer *pBuffer,
                                  bool throwOnFailure /* = false*/) {
  if (!IsValid()) {
    if (throwOnFailure) {
      throw CException(_T("CSocketServer::Socket::Write()"),
...
void CSocketServer::Socket::OnConnectionError(
    CSocketServer::enumConnectionErrorSource source, CIOBuffer *pBuffer,
    DWORD lastError) {
...
CSocketServer::WorkerThread::WorkerThread(CSocketServer &server,
                                          CIOCompletionPort &iocp)
    : m_server(server), m_iocp(iocp) {
...
int CSocketServer::WorkerThread::Run() {
  try {
    while (true) {
      ...
      DWORD dwResult = m_iocp.GetStatus((PDWORD_PTR)&pSocket, &dwIoSize,
                                        (OVERLAPPED **)&pBuffer);
      ...
      if (S_OK != dwResult) {
        if (ERROR_NETNAME_DELETED == dwResult) {
          ...
          throw CWin32Exception(
              _T("CIOCompletionPort::GetStatus() - GetQueuedCompletionStatus"),
              dwResult);
        }
      ...
      if (!pSocket) {
        /*
         * A completion key of 0 is posted to the iocp to request us to shut
         * down...
         */
    ...
    m_server.OnError(_T("CSocketServer::WorkerThread::Run() - Exception: ") +
                     e.GetWhere() + _T(" - ") + e.GetMessage());
  } catch (...) {
    m_server.OnError(
        _T("CSocketServer::WorkerThread::Run() - Unexpected exception"));
  }

  return 0;
}

int CSocketServer::WorkerThread::Run1() { return 0; }

void CSocketServer::WorkerThread::InitiateShutdown() { m_iocp.PostStatus(0); }
...
void CSocketServer::WorkerThread::HandleOperation(Socket *pSocket,
                                                  ...
                                                  bool weClosedSocket) {
  if (pBuffer) {
    const enumIO_Operation operation =
        static_cast<enumIO_Operation>(pBuffer->GetOperation());
    ...
    case enumIO_Read_Request:
      ...
      DEBUG_ONLY(
          Output(_T("enumIO_Zero_Byte_Read_Completed | enumIO_Read_Request:") +
                 ToString(pBuffer)));
    ...
    case enumIO_Write_Request:
      ...
      DEBUG_ONLY(Output(_T("enumIO_Write_Request:") + ToString(pBuffer)));
    ...
    default:
      m_server.OnError(
          _T("CSocketServer::WorkerThread::Run() - Unexpected operation"));
      break;
    }
  } else {
    m_server.OnError(
        _T("CSocketServer::WorkerThread::Run() - Unexpected - pBuffer is 0"));
  }
}

void CSocketServer::WorkerThread::ZeroByteRead(Socket *pSocket,
  ...
  if (SOCKET_ERROR == ::WSARecv(pSocket->m_socket, pBuffer->GetWSABUF(), 1,
                                &dwNumBytes, &dwFlags, pBuffer, NULL)) {
...
void CSocketServer::WorkerThread::Read(Socket *pSocket,
                                       CIOBuffer *pBuffer) const {
  pSocket->AddRef();
  ...
  pBuffer->SetSequenceNumber(
      pSocket->GetSequenceNumber(Socket::enumReadSequenceNo));
...
void CSocketServer::WorkerThread::Write(Socket *pSocket,
                                        CIOBuffer *pBuffer) const {
  pSocket->AddRef();

  {
    ...
    while (pBuffer) {
      ...
      if (SOCKET_ERROR == ::WSASend(pSocket->m_socket, pBuffer->GetWSABUF(), 1,
                                    &dwSendNumBytes, dwFlags, pBuffer, NULL)) {
        DWORD lastError = ::WSAGetLastError();
