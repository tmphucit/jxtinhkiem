...
CIOBuffer::CIOBuffer(Allocator &allocator, size_t size)
    : m_operation(0), m_sequenceNumber(0), m_allocator(allocator), m_ref(1),
...
CIOBuffer::~CIOBuffer() { SAFE_DELETE_ARRAY(m_buffer_ptr); }
...
CIOBuffer::InOrderBufferList::InOrderBufferList(
    ...
    : m_next(0), m_criticalSection(criticalSection) {}

void CIOBuffer::InOrderBufferList::AddBuffer(CIOBuffer *pBuffer) {
  CCriticalSection::Owner lock(m_criticalSection);

  std::pair<BufferSequence::iterator, bool> result = m_list.insert(
      BufferSequence::value_type(pBuffer->GetSequenceNumber(), pBuffer));
...
CIOBuffer *CIOBuffer::InOrderBufferList::ProcessAndGetNext() {
  ...
  if (it != m_list.end()) {
    if (it->first == m_next) {
      ...
      DEBUG_ONLY(Output(ToString(this) + _T(" Got buffer : ") +
                        ToString(it->first) + _T("Want buffer : ") +
                        ToString(m_next)));
    }
  }
