...
CSocketClient::CSocketClient(const _tstring &addressToConnectServer,
                             unsigned short portToConnectServer,
                             size_t maxFreeBuffers,
                             size_t bufferSize /* = 1024 */
                             )
    : CIOBuffer::Allocator(bufferSize, maxFreeBuffers),
      m_address(addressToConnectServer), m_port(portToConnectServer),
      m_connectSocket(INVALID_SOCKET) {}

CSocketClient::CSocketClient(size_t maxFreeBuffers,
                             size_t bufferSize /* = 1024 */
                             )
    : CIOBuffer::Allocator(bufferSize, maxFreeBuffers), m_port(0),
      m_connectSocket(INVALID_SOCKET) {}

void CSocketClient::Connect(const _tstring &addressToConnectServer,
                            unsigned short portToConnectServer) {
...
bool CSocketClient::StartConnections() {
  ...
  if (INVALID_SOCKET == m_connectSocket) {
    ...
    m_eventSelect.AssociateEvent(m_connectSocket,
...
void CSocketClient::StopConnections() {
  ...
  if (INVALID_SOCKET != m_connectSocket) {
    ...
    {
            OnError( _T("CSocketClient::StopAcceptingConnections() - WSAIoctl -
    ...
    */
    ...
    if (SOCKET_ERROR == ::setsockopt(m_connectSocket, SOL_SOCKET, SO_LINGER,
                                     (char *)&lingerStruct,
                                     sizeof(lingerStruct))) {
      OnError(_T("CSocketClient::setsockopt( SO_LINGER ) - ") +
              GetLastErrorMessage(::WSAGetLastError()));
    }
    ...
    if (0 != ::closesocket(m_connectSocket)) {
       ...
       * Call to unqualified virtual function
       */
      OnError(_T("CSocketClient::StopAcceptingConnections() - closesocket - ") +
              GetLastErrorMessage(::WSAGetLastError()));
    }
  ...
  try {
    ...
    while (!m_shutdownEvent.Wait(0)) {
      ...
      if (waitResult == WAIT_OBJECT_0) {
         ...
         * Call to unqualified virtual function
         */
        OnError(_T("CSocketClient::Run() - WaitForMultipleObjects: ") +
    ...
    OnError(_T("CSocketClient::Run() - Exception: ") + e.GetWhere() +
            _T(" - ") + e.GetMessage());
...
int CSocketClient::Run1() { return 0; }

void CSocketClient::ReleaseBuffers() { Flush(); }

void CSocketClient::OnError(const _tstring &message) { Output(message); }
...
void CSocketClient::OnRead(CIOBuffer *pBuffer) {
  ...
  if (SOCKET_ERROR == ::WSARecv(m_connectSocket, pBuffer->GetWSABUF(), 1,
                                &dwNumBytes, &dwFlags, NULL, NULL)) {
...
void CSocketClient::Write(const char *pData, size_t dataLength) {
  if (INVALID_SOCKET != m_connectSocket && dataLength > 0 && pData) {
...
void CSocketClient::Write(CIOBuffer *pBuffer) {
  ...
  if (NULL == pBuffer || (0 == (uDataLength = pBuffer->GetUsed())) ||
  ...
  do {
    if (SOCKET_ERROR != nError) {
      ...
      nError = ::WSASend(m_connectSocket, &wsa, 1, &dwSendNumBytes, dwFlags,
                         NULL, NULL);
     ...
     */
    if ((ERROR_IO_PENDING != lastError) && (WSAEWOULDBLOCK != lastError)) {
      _tstring sErrorInfo = GetLastErrorMessage(lastError);
...
bool CSocketClient::WaitAndVerifyCipher() {
  ...
  while (true) {
    ...
    if (res > 0) {
      res = recv(s, (LPSTR)&m_theSendAccountBegin + dwTotalLength,
                 sizeof(m_theSendAccountBegin) - dwTotalLength, 0);
    ...
    if (dwTotalLength == sizeof(m_theSendAccountBegin)) {
      ACCOUNT_BEGIN *pAccountBegin =
          (ACCOUNT_BEGIN *)(&m_theSendAccountBegin.AccountBegin);

      if (pAccountBegin->ProtocolType == CIPHER_PROTOCOL_TYPE) {
        if (pAccountBegin->Mode != 0) {
          /*
           * Write this message and then shutdown the sending side of the
           * socket.
           */
          ...
          if (0 != ::closesocket(m_connectSocket)) {
             ...
             * Call to unqualified virtual function
             */
            OnError(_T("CSocketClient::StopAcceptingConnections() - ")
                    _T("closesocket - ") +
                    GetLastErrorMessage(::WSAGetLastError()));
          }
