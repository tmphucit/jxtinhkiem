...
CThread::CThread() : m_hThread(NULL), m_hThread1(NULL) {}
...
HANDLE CThread::GetHandle() const { return m_hThread; }

void CThread::Start() {
  if (m_hThread == NULL && m_hThread1 == NULL) {
    unsigned int threadID = 0;

    m_hThread = (HANDLE)::_beginthreadex(0, 0, ThreadFunction, (void *)this, 0,
                                         &threadID);

    unsigned int threadID1 = 0;

    m_hThread1 = (HANDLE)::_beginthreadex(0, 0, ThreadFunction1, (void *)this,
                                          0, &threadID1);

    if (m_hThread == NULL || m_hThread1 == NULL) {
      throw CWin32Exception(_T("CThread::Start() - _beginthreadex"),
                            GetLastError());
    }
  } else {
    throw CException(
        _T("CThread::Start()"),
        _T("Thread already running - you can only call Start() once!"));
  }
}

void CThread::Wait() const {
  if (!Wait(INFINITE)) {
    throw CException(_T("CThread::Wait()"),
                     _T("Unexpected timeout on infinite wait"));
  }
}
...
bool CThread::Wait(DWORD timeoutMillis) const {
  ...
  if (result == WAIT_TIMEOUT) {
    ...
    throw CWin32Exception(_T( "CThread::Wait() - WaitForSingleObject" ),
                          ::GetLastError());
  }
