...
void CSocket::Bind(const SOCKADDR_IN &address) {
  if (SOCKET_ERROR == ::bind(m_socket,
                             reinterpret_cast<struct sockaddr *>(
                                 const_cast<SOCKADDR_IN *>(&address)),
                             sizeof(SOCKADDR_IN))) {
...
void CSocket::Bind(const struct sockaddr &address, size_t addressLength) {
  if (SOCKET_ERROR == ::bind(m_socket, const_cast<struct sockaddr *>(&address),
                             addressLength)) {
    throw Exception(_T("CSocket::Bind()"), ::WSAGetLastError());
  }
}
...
CSocket::InternetAddress::InternetAddress(unsigned long address,
                                          unsigned short port) {
  /*
...
CSocket::InternetAddress::InternetAddress(
    const _tstring &addressToConnectServer, unsigned short port) {
  sin_family = AF_INET;
  sin_port = htons(port);
