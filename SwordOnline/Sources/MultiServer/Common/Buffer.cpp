...
CBuffer::CBuffer(Allocator &allocator, size_t size)
    : m_allocator(allocator), m_ref(1), m_size(size), m_used(0),
      m_packLength(0), m_offsetNextPack(0) {
...
CBuffer::~CBuffer() { SAFE_DELETE_ARRAY(m_buffer_ptr); }
...
CBuffer *CBuffer::GetNextPack() {
  if (m_offsetNextPack < m_used) {
    ...
    size_t canused = (m_offsetNextPack + m_packLength > m_used)
                         ? (m_used - m_offsetNextPack)
                         : m_packLength;
...
void CBuffer::AddData(BYTE data) { AddData(&data, 1); }
...
CBuffer *CBuffer::AllocateNewBuffer() const { return m_allocator.Allocate(); }
...
CBuffer *CBuffer::Allocator::Allocate() {
  ...
  if (!m_freeList.Empty()) {
    ...
    if (!pBuffer) {
      throw CException(_T("CBuffer::Allocator::Allocate()"),
                       _T("Out of memory"));
    }
...
void CBuffer::Allocator::Release(CBuffer *pBuffer) {
  if (!pBuffer) {
    throw CException(_T("CBuffer::Allocator::Release()"),
                     _T("pBuffer is null"));
  }
...
CPackager::CPackager(size_t bufferSize /* = 65536  */,
                     size_t maxFreeBuffers /* = 16  */)
    : m_theAllocator(bufferSize, maxFreeBuffers) {}

CPackager::~CPackager() { Empty(); }

void CPackager::AddData(BYTE cID, const char *const pData, size_t dataLength,
                        unsigned long lnUserData /* = 0 */) {
...
void CPackager::AddData(BYTE cID, const BYTE *const pData, size_t dataLength,
                        unsigned long lnUserData /* = 0 */) {
...
CBuffer *CPackager::GetHeadPack(BYTE cID, size_t packLength /* = 512 */) {
  ...
  if (m_theSend.end() != (it = m_theSend.find(cID))) {
    ...
    CBuffer *pPack =
        pBuffer->GetHeadPack(packLength - 2 /* enumPackHeader + cID */);
...
CBuffer *CPackager::PackUp(const void *pData, size_t dataLength) {
  ...
  if (_Header(cFlag)) {
    ...
    pBuffer->AddData(((const char *)pData + nDataBegin),
                     dataLength - nDataBegin);
  }
  ...
  if (_Tail(cFlag)) {
    unsigned long lnUserData =
        *(const unsigned long *)((const char *)pData + 2);
