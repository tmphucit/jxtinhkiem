...
void Output(const _tstring &message) {
  ...
  const _tstring msg =
      ToString(GetCurrentThreadId()) + _T(": ") + message + _T("\n");

  ::OutputDebugString(msg.c_str());

  /*	if (!s_debugOut.is_open())
          {
                  s_debugOut.open(s_logFileName.c_str(), std::ios_base::out |
     std::ios_base::app);
...
#ifdef _DEBUG
  const _tstring msg = ToString(GetCurrentThreadId()) + _T(" [") + progID +
                       _T("] ") + message + _T("\n");
...
_tstring GetLastErrorMessage(DWORD last_error) {
  ...
  if (!FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, 0, last_error, 0, errmsg, 511,
                     NULL)) {
    /*
...
const char *net_ntoa(unsigned long lnAddr) {
  ...
  sprintf(szAddr, "%u.%u.%u.%u", lnAddr & 0xFF, (lnAddr >> 8) & 0xFF,
          (lnAddr >> 16) & 0xFF, (lnAddr >> 24) & 0xFF);
...
_tstring GetCurrentDirectory() {
  ...
  if (0 == ::GetCurrentDirectory(size, spBuf.get())) {
    throw CException(_T("GetCurrentDirectory()"),
                     _T("Failed to get current directory"));
  }
...
_tstring GetDateStamp() {
  ...
  _stprintf(buffer, _T("%02d%02d%02d"), systime.wDay, systime.wMonth,
...
_tstring GetTimeStamp() {
  ...
  _stprintf(buffer, _T("%02d:%02d:%02d"), systime.wHour, systime.wMinute,
            systime.wSecond);
...
_tstring DumpData(const BYTE *const pData, size_t dataLength,
  ...
  while (i < dataLength) {
    ...
    if ((bytesPerLine && (i % bytesPerLine == 0 && i != 0)) ||
        i == dataLength) {
  ...
  if (verSize != 0) {
    ...
    if (::GetFileVersionInfo(pModuleFileName, 0, verSize, spBlock.get())) {
      if (::VerQueryValue(spBlock.get(),
                          ...
                          (void **)(&lpTranslate), &nTranslateSize)) {
        if ((nTranslateSize / sizeof(LCP)) > 0) {
           ...
           */
          wsprintf(szSubBlock, TEXT("\\StringFileInfo\\%04x%04x\\FileVersion"),
                   lpTranslate[0].wLanguage, lpTranslate[0].wCodePage);

          auto_ptr<BYTE> spBuffer(new BYTE[verSize]);

          if (::GetFileVersionInfo(pModuleFileName, 0, verSize,
                                   spBuffer.get())) {
            LPTSTR pVersion = 0;
            UINT verLen = 0;

            if (::VerQueryValue(spBuffer.get(), szSubBlock,
