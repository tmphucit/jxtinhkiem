...
class CPackager {
public:
  CPackager(size_t bufferSize = 65536 /* 1024*64 */,
  ...
  void AddData(BYTE cID, const char *const pData, size_t dataLength,
               unsigned long lnUserData = 0);
  void AddData(BYTE cID, const BYTE *const pData, size_t dataLength,
               unsigned long lnUserData = 0);
   ...
   */
  bool ReSet(size_t bufferSize, size_t maxFreeBuffers) {
    return m_theAllocator.ReSet(bufferSize, maxFreeBuffers);
  }
...
inline bool CBuffer::Allocator::ReSet(size_t bufferSize,
                                      size_t maxFreeBuffers) {
  memcpy(const_cast<size_t *>(&m_bufferSize), &bufferSize, sizeof(size_t));
  memcpy(const_cast<size_t *>(&m_maxFreeBuffers), &maxFreeBuffers,
         sizeof(size_t));
