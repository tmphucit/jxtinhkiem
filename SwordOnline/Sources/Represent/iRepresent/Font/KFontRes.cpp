...
*****************************************************************************/
void KFontRes::GetInfo(int &nWidth, int &nHeight, int &nHCount,
                       int &nCount) const {
  nWidth = m_nFontW;
  nHeight = m_nFontH;
...
// Argumant		: int& nCount		缓存字符的个数
*****************************************************************************/
void KFontRes::GetInfo(float &fWidth, float &fHeight, float &fHInterval,
                       float &fVInterval) const {
...
// Purpose		: 更新缓存字符
// Argumant		: unsigned short * pUpdates
更新字符编码及在贴图中位置数组 树组的结构为对应于每个更新字符有两个uint数值。
...
void KFontRes::Update(unsigned short *pUpdates, int nCount) {
  ...
  for (i = 0; i < nCount; i++) {
    ...
    // 更新贴图
    Update(pCharacterData, ((BYTE *)d3dlr.pBits + d3dlr.Pitch * nY + nX * 2),
           d3dlr.Pitch);
  }
  m_pCharTexture->UnlockRect(0);
}
...
*****************************************************************************/
void KFontRes::Update(unsigned char *pCharImage, unsigned char *pTexData,
                      int nTexPitch) {
  ...
  without_border_start_line : {
    ...
    mov edx,
        ...
        : xor eax,
          ...
          cmp eax,
          7 je without_border_alpha_equal_7

              xor eax,
          eax jmp without_border_store_pixel
        ...
        : stosw loop without_border_store_pixel

          or
          edx,
          edx jg without_border_same_alpha_block

              add edi,
          TextureOffset dec nHeight jg without_border_start_line
  }
}
}