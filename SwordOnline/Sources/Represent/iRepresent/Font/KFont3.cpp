...
// Argumant		: int nCount
字符串的长度(BYTE)，默认值为-1，表示此字符串是以'\0'结尾。
// Argumant		: int nX
字符串显示起点坐标X，如果传入值为KF_FOLLOW，
//
则此字符串紧接在上次字符串的输出位置之后。
// Argumant		: int nY      字符串显示起点坐标Y,
如果传入值为KF_FOLLOW，
//
则此字符串与前一次输出字符串在同一行的位置。
// Argumant		: uint nColor
...
void KFont3::OutputText(const char *pszText, int nCount /*= KF_ZERO_END*/,
  ...
  while (nPos < nCount) {
    ...
    if (bShowChar) {
      ...
      pCharPoly[0].z = 0.9f;
      pCharPoly[0].w = 1.0f;
      pCharPoly[0].color = nColor;
      pCharPoly[1].z = 0.9f;
      pCharPoly[1].w = 1.0f;
      pCharPoly[1].color = nColor;
      pCharPoly[2].z = 0.9f;
      pCharPoly[2].w = 1.0f;
      pCharPoly[2].color = nColor;
      pCharPoly[3].z = 0.9f;
      pCharPoly[3].w = 1.0f;
      pCharPoly[3].color = nColor;
      pCharPoly[4].z = 0.9f;
      pCharPoly[4].w = 1.0f;
      pCharPoly[4].color = nColor;
      pCharPoly[5].z = 0.9f;
      pCharPoly[5].w = 1.0f;
      pCharPoly[5].color = nColor;
    ...
    if (nDx == -1 || (nLineWidth && h + m_nOutputWidth > nLineWidth)) {
      if (nPos < nCount && lpByte[nPos] == 0x0a &&
          nDx != -1) // 处理自动换行位置后恰好紧接一个换行符号
        nPos++;
      h = 0;
      nY += m_nOutputHeight;
      nHalfIndex = 0;
    } else if (nDx > 0 && nLineWidth &&
               h + nDx + m_nOutputWidth + m_nFontHalfWidth[nHalfIndex] >=
                   nLineWidth) {
      const char *pNext = TGetSecondVisibleCharacterThisLine(
          (const char *)lpByte, nPos, nCount);
...
// Purpose		: 绘制字符串
// Argumant		: KFontVertex* pPolyVertices
用于绘制字符的多边形顶点数据
// Argumant		: unsigned int nNumPolys 用于绘制字符的多边形数目
*****************************************************************************/
void KFont3::RenderTextDirect(KFontVertex *pPolyVertices,
                              unsigned int nNumPolys) {
  //_ASSERT(ms_pd3dDevice && pPolyVertices && nNumPolys);
  // 绘制字符多边形
  ms_pd3dDevice->DrawPrimitiveUP(D3DPT_TRIANGLELIST, nNumPolys, pPolyVertices,
                                 sizeof(KFontVertex));
}
...
// Purpose		: 绘制字符串
*****************************************************************************/
void KFont3::RenderTextCmpReverse(KFontVertex *pPolyVertices,
                                  unsigned int nNumPolys) {
  //_ASSERT(ms_pd3dDevice && pPolyVertices && nNumPolys);
  unsigned int i;
  unsigned int nNumVertex = nNumPolys * 3;
...
/*!*****************************************************************************
// Function		: KFont3::RestoreDeviceObjects
// Purpose		: 在D3d Device由lost state恢复为operational
state后KFont3作
//					d3d Device相关内容的恢复操作。
*****************************************************************************/
bool KFont3::RestoreDeviceObjects() {
  ...
  for (int which = 0; which < 2; which++) {
    ...
    ms_pd3dDevice->SetTextureStageState(0, D3DTSS_TEXTURETRANSFORMFLAGS,
                                        D3DTTFF_DISABLE);
...
void KFont3::SetBorderColor(unsigned int uColor) { ms_uBorderColor = uColor; }
