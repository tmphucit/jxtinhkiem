...
bool KFontData::Load(const char *pszFontFile) {
  ...
  while (pszFontFile) {
    ...
    // 读取偏移表
    if (File.Read(m_pdwOffs, m_uCharCount * sizeof(unsigned int)) !=
        m_uCharCount * sizeof(unsigned int))
      break;
...
// Purpose		: 取得单个字符的数据区
*****************************************************************************/
unsigned char *KFontData::GetCharacterData(unsigned char cFirst,
                                           unsigned char cNext) const {

  if (m_pFontData) {
    if (cNext < m_uCharCount && m_pdwOffs[cFirst])
      return (m_pFontData + m_pdwOffs[cFirst]);
  }
