...
------------------------------------------------------------------------------------------
//	Comment :
输入的文本串中包含控制符号，不是原始的标签控制符，是经过编码的。文本串中不
//
...
public:
  ...
  int TransXYPosToCharOffset(int nX, int nY, iFont *pFont, int nFontSize,
                             KOutputTextParam *pParam);

  // 跳到指定行开始处
  int SeekToSpecialLine(int nLineIndex, KTP_CTRL &Ctrl1, KTP_CTRL &Ctrl2,
                        int &nSkipedHalfLines, int nFontSize,
  ...
  void GetCurLineHeight(int &nSpanLines, int &nHeight, int nFontSize,
                        int bPicPackInSingleLine);
...
private:
  // 获取字符串里单一性质同行的一个子串
  int GetSimplexText(int &nSimplexStartPos, KTP_CTRL &HeadCtrl,
