...
extern IInlinePicEngineSink
    *g_pIInlinePicSinkRP; // 嵌入式图片的处理接口[wxb 2003-6-20]
...
//	次再调用又将重新从文本串起始处获取子串。
//--------------------------------------------------------------------------
int KTextProcess::GetSimplexText(int &nSimplexStartPos, KTP_CTRL &HeadCtrl,
                                 KTP_CTRL &TailCtrl, int nFontSize) {
  int nBeforeLen = 0,
  ...
  while (m_nReadPos < m_nCount) {
    ...
    } else if (cCode == KTC_INLINE_PIC) //[wxb 2003-6-20]
    {
      ...
      if (g_pIInlinePicSinkRP) {
        int cx, cy;
        if (SUCCEEDED(
                g_pIInlinePicSinkRP->GetPicSize(TailCtrl.wParam, cx, cy))) {
          m_fCurrentLineLen += (float)cx * 2 / nFontSize;
          m_nCurrentLineLen = (int)(m_fCurrentLineLen + 0.9999);
          if (m_nMaxLineLen == 0 || m_nCurrentLineLen < m_nMaxLineLen) {
            m_nReadPos += 1 + sizeof(unsigned short);
          } else if (m_nCurrentLineLen == m_nMaxLineLen ||
                     m_nCurrentLineLen == 0) {
            m_nReadPos += 1 + sizeof(unsigned short);
            break;
          } else {
    ...
    if (m_nMaxLineLen) {
      if (m_nCurrentLineLen >= m_nMaxLineLen) {
        ...
        const char *pNext = TGetSecondVisibleCharacterThisLine(
            (const char *)m_pBuffer, m_nReadPos, m_nCount);
...
//--------------------------------------------------------------------------
int KTextProcess::SeekToSpecialLine(int nLineIndex, KTP_CTRL &Ctrl1,
                                    KTP_CTRL &Ctrl2, int &nSkipedHalfLines,
                                    int nFontSize, int bPicPackInSingleLine) {
  m_nReadPos = 0;
  int nSimplexTextPos;
  ...
  while (nLineIndex > 0) {
    ...
    if (0 == nLinesSpan || KTC_ENTER == TailCtrl.cCtrl) {
      GetCurLineHeight(nLinesSpan, nLineHeight, nFontSize,
...
//	返回：实际绘制的文字行数
//--------------------------------------------------------------------------
int KTextProcess::DrawTextLine(iFont *pFont, int nFontSize,
...
#define _SET_CURRENT_BORDER_COLOR(c)                                           \
  ...
  if (SeekToSpecialLine(pParam->nSkipLine, PreCtrl1, PreCtrl2, nSkipedHalfLines,
                        nFontSize, pParam->bPicPackInSingleLine) < 0)
    return 0;
  ...
  do {
    if (0 == nSpanLines || KTC_ENTER == TailCtrl.cCtrl) {
      ...
      GetCurLineHeight(nSpanLines, nLineHeight, nFontSize,
                       pParam->bPicPackInSingleLine);
      if (nSpanLines > 1) {
    ...
    if (nCount) {
      pFont->OutputText(m_pBuffer + nPos, nCount, x, y, CurFontColor.Color_dw,
                        0);
        ...
        if (SUCCEEDED(
          ...
          if (nSkipedHalfLines && cy > nFontSize) {
            ...
            if (cy > (nSpanLines - nSkipedHalfLines) * nFontSize)
              ...
              nPicY -=
                  (int)(((cy - (nSpanLines - nSkipedHalfLines) * nFontSize)) *
                        1.5);
          } else
            nPicY -= (cy - nFontSize) / 2;
          if (SUCCEEDED(
                  g_pIInlinePicSinkRP->DrawPic(TailCtrl.wParam, x, nPicY))) {
            x += cx;
          }
        }
      }
...
//	[wxb 2003-7-21]
//--------------------------------------------------------------------------
int KTextProcess::TransXYPosToCharOffset(int nX, int nY, iFont *pFont,
                                         int nFontSize,
                                         KOutputTextParam *pParam) {
  _ASSERT(pParam);
  if (pFont == NULL || pParam->nNumLine <= 0)
    ...
    if (nCount) {
      ...
      if (nOffsetXThisLine != -1) {
        if (nX <= nOldX)
          ...
          nOffsetXThisLine =
              nOldReadPos + (nX - nOldX + nFontSize / 4) * 2 / nFontSize;
      }
    }
...
//[wxb 2003-6-20]
void KTextProcess::GetCurLineHeight(int &nSpanLines, int &nHeight,
                                    int nFontSize,
  ...
  nSpanLines =
      (nHeight % nFontSize) ? nHeight / nFontSize + 1 : nHeight / nFontSize;
}
