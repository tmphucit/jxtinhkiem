...
public:
  ...
  void Reset();   // Resets back to initial conditions (doesn't try to free
                  // anything)...
  void FreeAll(); // Frees all the member vars and resets afterwards...

  bool BuildDeviceList();
  D3DDeviceInfo *PickDefaultDev(
      D3DAdapterInfo **pAdapterInfo); // Pick default device from our list...
  D3DModeInfo *PickDefaultMode(D3DDeviceInfo *pDeviceInfo,
                               uint32 iBitDepth); // Pick default mode from the
                                                  // passed devices mode list...
  ...
  D3DModeInfo *GetModeInfo(unsigned int uAdapterID, unsigned int uDeviceID,
                           unsigned int uModeID);

  void
  ListDevices(); // Go through all the Adapters/Devices and print them out...
...
private:
  D3DDISPLAYMODE
      m_DesktopFormat; // Desktop display mode (mainly for window mode)...
  vector<D3DAdapterInfo>
      m_AdapterList; // List of adaptors on this system (probably just one,
                     // unless you've got two cards)...
};

extern CD3D_Shell g_D3DShell; // The global D3D Shell...
#define PDIRECT3D                                                              \
  (g_D3DShell.m_pD3D) // Use for quick access to the D3DDevice...
