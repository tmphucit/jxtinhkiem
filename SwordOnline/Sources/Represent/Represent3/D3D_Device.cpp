...
// 创建显示设备
bool CD3D_Device::CreateDevice(D3DAdapterInfo *pAdapter, D3DDeviceInfo *pDevice,
                               D3DModeInfo *pMode) {
  ...
  HRESULT hResult = PDIRECT3D->CreateDevice(
      pAdapter->iAdapterNum, pDevice->DeviceType, g_hWnd, BehaviorFlags,
      &m_PresentationParam, &m_pD3DDevice);
  ...
  if (m_pDevice->DeviceType == D3DDEVTYPE_REF)
    g_DebugLog("[D3DRender]Warning: Couldnt' find any HAL devices, Using "
...
// Releases the device (and resets all that should be reset on device
// release)...
...
// 释放设备对象
bool CD3D_Device::ReleaseDevObjects() { return true; }

// 恢复设备对象
bool CD3D_Device::RestoreDevObjects() { return true; }
...
void CD3D_Device::SetPresentationParams(
    D3DPRESENT_PARAMETERS &PresentationParam, D3DModeInfo *pMode) {
  ...
  if (g_bRunWindowed || g_bForceSwapEffectBlt) {
  ...
  PresentationParam.BackBufferCount =
      BackBufferCount; // Number of back buffers (1 "double buffer", 2 "triple
                       // buffer", etc)
  PresentationParam.MultiSampleType =
      GetDefaultMultiSampleType(g_nAntiAliasFSOverSample);
  ...
  //	PresentationParam.Flags
  //= 0;
  PresentationParam.FullScreen_RefreshRateInHz =
      D3DPRESENT_RATE_DEFAULT; // D3DPRESENT_RATE_UNLIMITED
}

/*
// Go for the best one that works and fallback till we find one...
//	Note: Need to watch for loops here. Would be very bad.
#define D3DDEVICE_CHECKVALID_ZSFORMAT(MyFormat)
(PDIRECT3D->CheckDeviceFormat(m_pAdapter->iAdapterNum,m_pDevice->DeviceType,m_pMode->Format,D3DUSAGE_DEPTHSTENCIL,D3DRTYPE_SURFACE,MyFormat)
== D3D_OK ? true : false) D3DFORMAT
CD3D_Device::GetDefaultDepthStencilFormat(uint32 iZBitDepth,uint32
iStencilBitDepth)
{
        ...
        case 32 :
                ...
                case 8  : DepthStencilFormat = D3DFMT_D24S8;	if
(!D3DDEVICE_CHECKVALID_ZSFORMAT(DepthStencilFormat)) return
GetDefaultDepthStencilFormat(32,4); break; case 4  : DepthStencilFormat =
D3DFMT_D24X4S4;	if (!D3DDEVICE_CHECKVALID_ZSFORMAT(DepthStencilFormat)) return
GetDefaultDepthStencilFormat(24,8); break; case 1  : DepthStencilFormat =
D3DFMT_D24S8;	if (!D3DDEVICE_CHECKVALID_ZSFORMAT(DepthStencilFormat)) return
GetDefaultDepthStencilFormat(32,4); break; case 0  : DepthStencilFormat =
D3DFMT_D32;		if (!D3DDEVICE_CHECKVALID_ZSFORMAT(DepthStencilFormat))
return GetDefaultDepthStencilFormat(24,8); break; default : assert(0); return
D3DFMT_UNKNOWN; break; } break; case 24 : switch (iStencilBitDepth) { case 8  :
DepthStencilFormat = D3DFMT_D24S8;	if
(!D3DDEVICE_CHECKVALID_ZSFORMAT(DepthStencilFormat)) return
GetDefaultDepthStencilFormat(24,4); break; case 4  : DepthStencilFormat =
D3DFMT_D24X4S4;	if (!D3DDEVICE_CHECKVALID_ZSFORMAT(DepthStencilFormat)) return
GetDefaultDepthStencilFormat(24,0); break; case 1  : DepthStencilFormat =
D3DFMT_D24S8;	if (!D3DDEVICE_CHECKVALID_ZSFORMAT(DepthStencilFormat)) return
GetDefaultDepthStencilFormat(24,4); break; case 0  : DepthStencilFormat =
D3DFMT_D24X8;	if (!D3DDEVICE_CHECKVALID_ZSFORMAT(DepthStencilFormat)) return
GetDefaultDepthStencilFormat(16,8); break; default : assert(0); return
D3DFMT_UNKNOWN; break; } break; case 16 : switch (iStencilBitDepth) { case 8  :
DepthStencilFormat = D3DFMT_D15S1;	if
(!D3DDEVICE_CHECKVALID_ZSFORMAT(DepthStencilFormat)) return
GetDefaultDepthStencilFormat(16,4); break; case 4  : DepthStencilFormat =
D3DFMT_D15S1;	if (!D3DDEVICE_CHECKVALID_ZSFORMAT(DepthStencilFormat)) return
GetDefaultDepthStencilFormat(16,0); break; case 1  : DepthStencilFormat =
D3DFMT_D15S1;	if (!D3DDEVICE_CHECKVALID_ZSFORMAT(DepthStencilFormat)) return
GetDefaultDepthStencilFormat(16,4); break; case 0  : DepthStencilFormat =
D3DFMT_D16;		if (!D3DDEVICE_CHECKVALID_ZSFORMAT(DepthStencilFormat))
return D3DFMT_UNKNOWN; break; default : assert(0); return D3DFMT_UNKNOWN; break;
} break; default : assert(0); return D3DFMT_UNKNOWN; }
...
D3DMULTISAMPLE_TYPE CD3D_Device::GetDefaultMultiSampleType(uint32 Samples) {
  ...
  if (PDIRECT3D->CheckDeviceMultiSampleType(
          m_pAdapter->iAdapterNum, m_pDevice->DeviceType, m_pMode->Format,
          g_bRunWindowed, MultiSampleType, NULL) != D3D_OK) {
...
void CD3D_Device::SetDefaultRenderStates() {
  ...
  m_pD3DDevice->SetRenderState(D3DRS_FILLMODE, g_nWireframe ? D3DFILL_WIREFRAME
                                                            : D3DFILL_SOLID);
...
void CD3D_Device::SetupViewport(uint32 iLeft, uint32 iRight, uint32 iTop,
                                uint32 iBottom, float fMinZ, float fMaxZ) {
...
void CD3D_Device::ListDeviceCaps() {
  ...
  d3d_GetColorMasks(m_pMode->Format, iBitCount, iAlphaMask, iRedMask,
                    iGreenMask, iBlueMask);
  ...
  g_DebugLog("[D3DRender]VendorID: 0x%x, DeviceID: 0x%x, SubSysID: 0x%x, "
             "Revision: 0x%x",
             m_pAdapter->AdapterID.VendorId, m_pAdapter->AdapterID.DeviceId,
             m_pAdapter->AdapterID.SubSysId, m_pAdapter->AdapterID.Revision);
  g_DebugLog("[D3DRender]Width: %d, Height: %d, BitDepth: %d", m_pMode->Width,
             m_pMode->Height, iBitCount);
  g_DebugLog("[D3DRender]------------------------------------------------------"
             "---------");
}
