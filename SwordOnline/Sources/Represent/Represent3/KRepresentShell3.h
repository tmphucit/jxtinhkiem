...
public:
  ...
  void DrawPrimitivesOnImage(
      ...
      const char *pszImage,
      // ## 目标图形的id
      unsigned int uImage, short &nImagePosition);
  ...
  virtual bool SaveScreenToFile(const char *pszName, ScreenFileType eType,
...
private:
  ...
  DWORD pLightingArray[LIGHTING_GRID_WIDTH *
                       LIGHTING_GRID_HEIGHT]; // 光照图信息数组
...
private:
  ...
  // 渲染函数
  void DrawImage2D(int nPrimitiveCount, KRepresentUnit *pPrimitives,
                   int bSinglePlaneCoord);
  ...
  void DrawPlayer2D(int nPrimitiveCount, KRepresentUnit *pPrimitives,
                    int bSinglePlaneCoord);

  // 透视模式绘制主角，预先绘制到贴图再绘制到屏幕
  void DrawPlayer3D(int nPrimitiveCount, KRepresentUnit *pPrimitives,
                    int bSinglePlaneCoord);

  // 计算非透视模式下的主角外包矩形
  void GetBoundBox2D(int nPrimitiveCount, KRepresentUnit *pPrimitives,
                     int bSinglePlaneCoord, RECT &rect);

  // 计算透视模式下的主角外包矩形
  void GetBoundBox3D(int nPrimitiveCount, KRepresentUnit *pPrimitives,
                     RECTFLOAT &rect);

  void DrawSprOnTexture2D(int nPrimitiveCount, KRepresentUnit *pPrimitives,
                          int bSinglePlaneCoord, RECT &rcBound,
                          RECTFLOAT &rcRenderBound, int nTexSize,
                          bool bLighting);
  void DrawImage3D(unsigned int uGenre, int nPrimitiveCount,
                   KRepresentUnit *pPrimitives, int bSinglePlaneCoord);
  // bStretch是否缩放位图
  void DrawBitmap16(int32 nX, int32 nY, int32 nWidth, int32 nHeight,
                    TextureResBmp *pBitmap, bool bStretch = false);
  void DrawSpriteAlpha(int32 nX, int32 nY, int32 nWidth, int32 nHeight,
                       int32 nFrame, TextureResSpr *pSprite, DWORD color,
                       int32 nRenderStyle);
  void DrawSpritePartAlpha(int32 nX, int32 nY, int32 nWidth, int32 nHeight,
                           int32 nFrame, TextureResSpr *pSprite, RECT &rect);
  void DrawBitmap16Part(int32 nX, int32 nY, int32 nWidth, int32 nHeight,
                        TextureResBmp *pBitmap, RECT &rect);
  ...
  void DrawSpriteAlpha3D(RenderParam3D &param, int32 nFrame,
                         TextureResSpr *pSprite, DWORD color,
                         int32 nRenderStyle, RECT *rect);
  void DrawSpriteAlpha3DLighting(RenderParam3D &param, int32 nFrame,
                                 TextureResSpr *pSprite, DWORD color,
                                 int32 nRenderStyle, RECT *rect);

  void __fastcall SetPoint3dLighting(VERTEX3D &pDes, VERTEX3D &pSrc,
                                     DWORD color);
  inline unsigned int GetPoint3dLighting(D3DXVECTOR3 &v);
  unsigned int GetPoint3dLightingLerp(D3DXVECTOR3 &v);

  void RIO_CopyBitmap16ToBuffer(TextureResBmp *pBitmap,
                                TextureResBmp *pDestBitmap, int32 nX, int32 nY);
  void RIO_CopySprToBufferAlpha(TextureResSpr *pSpr, int32 nFrame,
                                TextureResBmp *pDestBitmap, int32 nX, int32 nY);
  ...
  // 绘制一批点
  void DrawPoint(int nPrimitiveCount, KRepresentUnit *pPrimitives,
                 int bSinglePlaneCoord);
  ...
  // 生成网格，计算亮度
  uint32 BuildMesh3D(VERTEX3D *pvbSrc, VERTEX3D *pvbDes, uint32 nNumDes,
                     DWORD color);
};
