...
static inline void DOWRDToRGB(DWORD color, DWORD &a, DWORD &r, DWORD &g,
                              DWORD &b) {
  a = color >> 24;
...
inline unsigned int KRepresentShell3::GetPoint3dLighting(D3DXVECTOR3 &v) {
  ...
  return pLightingArray[
      // 这里不能写成uY * LIGHTING_GRID_WIDTH / LIGHTING_GRID_SIZEY,因为uY /
      // LIGHTING_GRID_SIZEY这一步舍去小数部分是必须的
      (uY / LIGHTING_GRID_SIZEY * LIGHTING_GRID_WIDTH) +
      (uX / LIGHTING_GRID_SIZEX)];
}

void __fastcall KRepresentShell3::SetPoint3dLighting(VERTEX3D &pDes,
                                                     VERTEX3D &pSrc,
                                                     DWORD color) {
  pDes = pSrc;
...
static WORD g_A8ToA4[256] = {
    ...
    0x1000, 0x1000, 0x1000, 0x1000, 0x1000, 0x2000, 0x2000, 0x2000, 0x2000,
    0x2000, 0x2000, 0x2000, 0x2000, 0x2000, 0x2000, 0x2000, 0x2000, 0x2000,
    0x2000, 0x2000, 0x2000, 0x3000, 0x3000, 0x3000, 0x3000, 0x3000, 0x3000,
    0x3000, 0x3000, 0x3000, 0x3000, 0x3000, 0x3000, 0x3000, 0x3000, 0x3000,
    0x3000, 0x4000, 0x4000, 0x4000, 0x4000, 0x4000, 0x4000, 0x4000, 0x4000,
    0x4000, 0x4000, 0x4000, 0x4000, 0x4000, 0x4000, 0x4000, 0x4000, 0x5000,
    0x5000, 0x5000, 0x5000, 0x5000, 0x5000, 0x5000, 0x5000, 0x5000, 0x5000,
    0x5000, 0x5000, 0x5000, 0x5000, 0x5000, 0x5000, 0x6000, 0x6000, 0x6000,
    0x6000, 0x6000, 0x6000, 0x6000, 0x6000, 0x6000, 0x6000, 0x6000, 0x6000,
    0x6000, 0x6000, 0x6000, 0x6000, 0x7000, 0x7000, 0x7000, 0x7000, 0x7000,
    0x7000, 0x7000, 0x7000, 0x7000, 0x7000, 0x7000, 0x7000, 0x7000, 0x7000,
    0x7000, 0x7000, 0x8000, 0x8000, 0x8000, 0x8000, 0x8000, 0x8000, 0x8000,
    0x8000, 0x8000, 0x8000, 0x8000, 0x8000, 0x8000, 0x8000, 0x8000, 0x8000,
    0x9000, 0x9000, 0x9000, 0x9000, 0x9000, 0x9000, 0x9000, 0x9000, 0x9000,
    0x9000, 0x9000, 0x9000, 0x9000, 0x9000, 0x9000, 0x9000, 0xa000, 0xa000,
    0xa000, 0xa000, 0xa000, 0xa000, 0xa000, 0xa000, 0xa000, 0xa000, 0xa000,
    0xa000, 0xa000, 0xa000, 0xa000, 0xa000, 0xb000, 0xb000, 0xb000, 0xb000,
    0xb000, 0xb000, 0xb000, 0xb000, 0xb000, 0xb000, 0xb000, 0xb000, 0xb000,
    0xb000, 0xb000, 0xb000, 0xc000, 0xc000, 0xc000, 0xc000, 0xc000, 0xc000,
    0xc000, 0xc000, 0xc000, 0xc000, 0xc000, 0xc000, 0xc000, 0xc000, 0xc000,
    0xc000, 0xd000, 0xd000, 0xd000, 0xd000, 0xd000, 0xd000, 0xd000, 0xd000,
    0xd000, 0xd000, 0xd000, 0xd000, 0xd000, 0xd000, 0xd000, 0xd000, 0xe000,
    0xe000, 0xe000, 0xe000, 0xe000, 0xe000, 0xe000, 0xe000, 0xe000, 0xe000,
    0xe000, 0xe000, 0xe000, 0xe000, 0xe000, 0xe000, 0xf000, 0xf000, 0xf000,
    0xf000, 0xf000, 0xf000, 0xf000, 0xf000, 0xf000, 0xf000, 0xf000, 0xf000,
    0xf000, 0xf000, 0xf000, 0xf000};

// 将spr数据转换到A4R4G4B4缓冲区
void RenderToA4R4G4B4(WORD *pDest, uint32 nPitch, PBYTE pData, RECT &rect,
                      uint32 nWidth, uint32 nHeight, WORD *pPalette) {
...
// 将世界坐标转换到屏幕坐标
D3DXVECTOR3 *WorldToScreen(D3DXVECTOR3 *pOut, CONST D3DXVECTOR3 *pV,
...
inline D3DXVECTOR3 ChaZhi(D3DXVECTOR3 v1, D3DXVECTOR3 v2, float fs1, float fs2,
                          float fs) {
  return (v2 - v1) * (fs - fs1) / (fs2 - fs1) + v1;
}

//=========测试模块性能，判断是否推荐使用===============
extern "C" __declspec(dllexport) bool RepresentIsModuleRecommended() {
  return Test3D();
}

//=========创建一个iRepresentShell接口的实例===============
extern "C" __declspec(dllexport) iRepresentShell *CreateRepresentShell() {
  return (new KRepresentShell3);
}

IInlinePicEngineSink *g_pIInlinePicSinkRP =
    NULL; // 嵌入式图片的处理接口[wxb 2003-6-20]
...
void KRepresentShell3::SetUpProjectionMatrix() {
  ...
  if (g_renderModel == RenderModel3DOrtho)
    ...
    D3DXMatrixPerspectiveFovLH(&m_matProj, D3DX_PI / 24, fAspect, 1.0f,
                               20000.0f);
...
bool KRepresentShell3::InitDeviceObjects() {
  ...
  if (FAILED(PD3DDEVICE->CreateTexture(
          SPR_PRERENDER_TEXSIZE1, SPR_PRERENDER_TEXSIZE1, 1, 0, D3DFMT_A4R4G4B4,
          D3DPOOL_MANAGED, &m_pPreRenderTexture128, NULL)))
    return false;
  if (FAILED(PD3DDEVICE->CreateTexture(
          SPR_PRERENDER_TEXSIZE2, SPR_PRERENDER_TEXSIZE2, 1, 0, D3DFMT_A4R4G4B4,
          D3DPOOL_MANAGED, &m_pPreRenderTexture256, NULL)))
    return false;
  return true;
}
...
bool KRepresentShell3::RestoreDeviceObjects() {
  ...
  if (!m_pVB2D) {
    ...
    if (FAILED(PD3DDEVICE->CreateVertexBuffer(
            VERTEX_BUFFER_SIZE * sizeof(VERTEX2D),
  ...
  if ((g_renderModel == RenderModel3DOrtho ||
    ...
    // 创建透视状态使用的顶点缓冲区
    if (FAILED(PD3DDEVICE->CreateVertexBuffer(
            VERTEX_BUFFER_SIZE * sizeof(VERTEX3D),
            D3DUSAGE_WRITEONLY | D3DUSAGE_DYNAMIC | D3DUSAGE_SOFTWAREPROCESSING,
...
bool KRepresentShell3::CreateAFont(const char *pszFontFile,
                                   CHARACTER_CODE_SET CharaSet, int nId) {
  if (m_bDeviceLost)
    return false;
  ...
  if (pszFontFile[0] == '#') {
    ...
    for (int j = 0; j < RS2_MAX_FONT_ITEM_NUM; j++) {
      if (nFirstFree != j && m_FontTable[j].nId == nShareWithId &&
        ...
        m_FontTable[nFirstFree].pFontObj =
            (KFont3 *)m_FontTable[j].pFontObj->Clone();
        return true;
      }
    }
    return false;
  ...
  if (!m_FontTable[nFirstFree].pFontObj->Load(
          (LPSTR)pszFontFile /*, CharaSet*/)) {
    m_FontTable[nFirstFree].pFontObj->Release();
    m_FontTable[nFirstFree].pFontObj = NULL;
  }
...
unsigned int KRepresentShell3::CreateImage(const char *pszName, int nWidth,
...
void KRepresentShell3::DrawPrimitives(int nPrimitiveCount,
    ...
    for (i = 0; i < nPrimitiveCount; i++, pTemp++) {
      ...
      case ISI_T_SPR: {
        TextureResSpr *pSprite = (TextureResSpr *)m_TextureResMgr.GetImage(
            pTemp->szImage, pTemp->uImage, pTemp->nISPosition, pTemp->nFrame,
            pTemp->nType);
      ...
      case ISI_T_BITMAP16: {
        ...
        if (pBitmap) {
          ...
          DrawBitmap16Part(nX, nY, pBitmap->GetWidth(), pBitmap->GetHeight(),
                           pBitmap, rc);
        }
      } break;
      }
  ...
  case RU_T_LINE: {
    ...
    for (i = 0; i < nPrimitiveCount; i++, pTemp++) {
      ...
      DrawLine(nX1, nY1, nX2, nY2,
               D3DCOLOR_ARGB((DWORD)pTemp->Color.Color_b.a,
                             (DWORD)pTemp->Color.Color_b.r,
                             (DWORD)pTemp->Color.Color_b.g,
                             (DWORD)pTemp->Color.Color_b.b));
    }
  } break;
  case RU_T_RECT: {
    ...
    for (i = 0; i < nPrimitiveCount; i++, pTemp++) {
      ...
      DrawRectFrame(nX1, nY1, nX2, nY2,
                    D3DCOLOR_ARGB(255, (DWORD)pTemp->Color.Color_b.r,
                                  (DWORD)pTemp->Color.Color_b.g,
                                  (DWORD)pTemp->Color.Color_b.b));
    }
  } break;
  case RU_T_SHADOW: {
    ...
    for (i = 0; i < nPrimitiveCount; i++, pTemp++) {
      ...
      if (!bSinglePlaneCoord) {
        ...
        CoordinateTransform(
            nX2, nY2,
            pTemp->oEndPos.nZ); // No nEndZ? Must be Single Plane Coord?
      }
      DrawRect(nX1, nY1, nX2 - nX1, nY2 - nY1,
               D3DCOLOR_ARGB(255 - (pTemp->Color.Color_b.a << 3),
                             (DWORD)pTemp->Color.Color_b.r,
                             (DWORD)pTemp->Color.Color_b.g,
                             (DWORD)pTemp->Color.Color_b.b));
    }
...
void KRepresentShell3::DrawImage2D(int nPrimitiveCount,
                                   KRepresentUnit *pPrimitives,
                                   int bSinglePlaneCoord) {
  // 判断是否主角类
...
#define CENTERY 192
      ...
      DrawSpriteAlpha(nX, nY, pSprite->m_pFrameInfo[pTemp->nFrame].nWidth,
                      pSprite->m_pFrameInfo[pTemp->nFrame].nHeight,
                      pTemp->nFrame, pSprite, pTemp->Color.Color_dw,
                      pTemp->bRenderStyle);
    } break;
    case ISI_T_BITMAP16: {
      TextureResBmp *pBitmap = (TextureResBmp *)m_TextureResMgr.GetImage(
...
void KRepresentShell3::DrawImage2DStretch(int nPrimitiveCount,
                                          KRepresentUnit *pPrimitives) {
  int i;
  ...
  for (i = 0; i < nPrimitiveCount; i++, pTemp++) {
    ...
    DrawBitmap16(pTemp->oPosition.nX, pTemp->oPosition.nY,
                 pTemp->oEndPos.nX - pTemp->oPosition.nX,
                 pTemp->oEndPos.nY - pTemp->oPosition.nY, pBitmap, true);
  }
}

void KRepresentShell3::DrawPlayer2D(int nPrimitiveCount,
                                    KRepresentUnit *pPrimitives,
                                    int bSinglePlaneCoord) {
  RECT rcBound;
  RECTFLOAT rcBound1;
  ...
  // 如果图素超出屏幕范围则不渲染
  if (rcBound.right < 0 || rcBound.left > g_nScreenWidth ||
      rcBound.bottom < 0 || rcBound.top > g_nScreenHeight)
    return;
  ...
  if (rcBound.right - rcBound.left <= SPR_PRERENDER_TEXSIZE1 &&
    ...
    DrawSprOnTexture2D(nPrimitiveCount, pPrimitives, bSinglePlaneCoord, rcBound,
                       rcBound1, SPR_PRERENDER_TEXSIZE1, false);
    g_ntest++;
  } else if (rcBound.right - rcBound.left <= SPR_PRERENDER_TEXSIZE2 &&
             rcBound.bottom - rcBound.top <= SPR_PRERENDER_TEXSIZE2) {
    DrawSprOnTexture2D(nPrimitiveCount, pPrimitives, bSinglePlaneCoord, rcBound,
                       rcBound1, SPR_PRERENDER_TEXSIZE2, false);
    g_ntest++;
  }
}

void KRepresentShell3::GetBoundBox2D(int nPrimitiveCount,
                                     KRepresentUnit *pPrimitives,
                                     int bSinglePlaneCoord, RECT &rcBound) {
  int i;
  KRUImage *pTemp = (KRUImage *)pPrimitives;
...
void KRepresentShell3::DrawSprOnTexture2D(int nPrimitiveCount,
                                          KRepresentUnit *pPrimitives,
                                          int bSinglePlaneCoord, RECT &rcBound,
                                          RECTFLOAT &rcRenderBound,
                                          int nTexSize, bool bLighting) {
  int i;
  RECT rect;
  ...
  for (i = 0; i < nPrimitiveCount; i++, pTemp++) {
    ...
    if (!bLightGet) {
      ...
      if (pTemp->bRenderStyle == IMAGE_RENDER_STYLE_ALPHA_COLOR_ADJUST) {
        // 处理偏色
        color = ScaleColor(color, pTemp->Color.Color_b.r,
...
void KRepresentShell3::DrawImage3D(unsigned int uGenre, int nPrimitiveCount,
                                   KRepresentUnit *pPrimitives,
                                   int bSinglePlaneCoord) {
  ...
  for (i = 0; i < nPrimitiveCount; i++, pTemp++) {
    ...
    case ISI_T_SPR: {
      ...
      if (!(pTemp->bRenderFlag & RUIMAGE_RENDER_FLAG_REF_SPOT) &&
          (pTemp->oEndPos.nX == 0 || pTemp->oEndPos.nY == 0))
        break;
...
#define CENTERY 192
          ...
          fX3 = fX1 + pSprite->m_pFrameInfo[pTemp->nFrame].nWidth *
                          SCALE_RATE_SPRITE_WIDTH;
          fZ3 = fZ1 - pSprite->m_pFrameInfo[pTemp->nFrame].nHeight *
                          SCALE_RATE_SPRITE_HEIGHT;
          fY3 = fY1;
        }
        ...
        float fYOff = 0;
        if (!(pTemp->bRenderFlag & RUIMAGE_RENDER_FLAG_FRAME_DRAW)) {
          fXOff += pSprite->m_pFrameInfo[pTemp->nFrame].nOffX *
                   SCALE_RATE_SPRITE_WIDTH;
          fYOff += pSprite->m_pFrameInfo[pTemp->nFrame].nOffY *
                   SCALE_RATE_SPRITE_HEIGHT;
          if (fZ1 == fZ3) {
            fX1 += fXOff;
            fX3 += fXOff;
        ...
        if (m_bDoLighting &&
            pTemp->bRenderStyle != IMAGE_RENDER_STYLE_ALPHA_NOT_BE_LIT)
          DrawSpriteAlpha3DLighting(renderParam, pTemp->nFrame, pSprite,
                                    pTemp->Color.Color_dw, pTemp->bRenderStyle,
                                    NULL);
        else
          DrawSpriteAlpha3D(renderParam, pTemp->nFrame, pSprite,
...
void KRepresentShell3::DrawPlayer3D(int nPrimitiveCount,
                                    KRepresentUnit *pPrimitives,
                                    int bSinglePlaneCoord) {
  RECTFLOAT rcBound3D;
  RECT rcBound2D;
  ...
  // 如果图素超出屏幕范围则不渲染
  if (rcBound3D.right < 0 || rcBound3D.left > g_nScreenWidth ||
      rcBound3D.bottom < 0 || rcBound3D.top > g_nScreenHeight)
    return;

  if (rcBound3D.left - rcBound3D.right > 260 ||
      rcBound3D.right - rcBound3D.left > 260)
    return;

  // 绘制主角类
  if (rcBound2D.right - rcBound2D.left <= SPR_PRERENDER_TEXSIZE1 &&
      rcBound2D.bottom - rcBound2D.top <= SPR_PRERENDER_TEXSIZE1) {
    DrawSprOnTexture2D(nPrimitiveCount, pPrimitives, false, rcBound2D,
                       rcBound3D, SPR_PRERENDER_TEXSIZE1, true);
    g_ntest++;
  } else if (rcBound2D.right - rcBound2D.left <= SPR_PRERENDER_TEXSIZE2 &&
             rcBound2D.bottom - rcBound2D.top <= SPR_PRERENDER_TEXSIZE2) {
    DrawSprOnTexture2D(nPrimitiveCount, pPrimitives, false, rcBound2D,
                       rcBound3D, SPR_PRERENDER_TEXSIZE2, true);
    g_ntest++;
  }
}

void KRepresentShell3::GetBoundBox3D(int nPrimitiveCount,
                                     KRepresentUnit *pPrimitives,
                                     RECTFLOAT &rcBound) {
  int i;
  KRUImage *pTemp = (KRUImage *)pPrimitives;
...
#define CENTERY 192
      ...
      v2.x = v1.x + pSprite->m_pFrameInfo[pTemp->nFrame].nWidth *
                        SCALE_RATE_SPRITE_WIDTH;
      v2.z = v1.z - pSprite->m_pFrameInfo[pTemp->nFrame].nHeight *
                        SCALE_RATE_SPRITE_HEIGHT;
      v2.y = v1.y;
    }
    ...
    if (!(pTemp->bRenderFlag & RUIMAGE_RENDER_FLAG_FRAME_DRAW)) {
      fXOff +=
          pSprite->m_pFrameInfo[pTemp->nFrame].nOffX * SCALE_RATE_SPRITE_WIDTH;
      fYOff +=
          pSprite->m_pFrameInfo[pTemp->nFrame].nOffY * SCALE_RATE_SPRITE_HEIGHT;
...
void KRepresentShell3::DrawPrimitivesOnImage(
    int nPrimitiveCount, KRepresentUnit *pPrimitives, unsigned int uGenre,
  ...
  if (FAILED(pDestBitmap->m_FrameInfo.texInfo[0].pTexture->GetSurfaceLevel(
          0, &pDesSurface))) {
    pOldSurface->Release();
    return;
  }
  ...
  case RU_T_IMAGE:
    ...
    for (i = 0; i < nPrimitiveCount; i++, pTemp++) {
      ...
      case ISI_T_BITMAP16: {
        ...
        if (!pBitmap)
          break;
        RIO_CopyBitmap16ToBuffer(pBitmap, pDestBitmap, pTemp->oPosition.nX,
                                 pTemp->oPosition.nY);
      } break;
      }
    }
...
// ## 清除图形数据
void KRepresentShell3::ClearImageData(const char *pszImage, unsigned int uImage,
...
void KRepresentShell3::FreeAllImage() { m_TextureResMgr.Free(); }
...
void *KRepresentShell3::GetBitmapDataBuffer(const char *pszImage,
...
void KRepresentShell3::ReleaseBitmapDataBuffer(const char *pszImage,
                                               void *pBuffer) {
  if (!pszImage || !pszImage[0])
    return;
...
bool KRepresentShell3::GetImageParam(const char *pszImage,
                                     KImageParam *pImageData, int nType) {
...
bool KRepresentShell3::GetImageFrameParam(const char *pszImage, int nFrame,
                                          KRPosition2 *pOffset,
                                          KRPosition2 *pSize, int nType) {
  if (!pOffset || !pszImage || !pszImage[0])
    return false;

  return m_TextureResMgr.GetImageFrameParam(pszImage, nFrame, pOffset, pSize,
                                            nType);
}

int KRepresentShell3::GetImagePixelAlpha(const char *pszImage, int nFrame,
                                         int nX, int nY, int nType) {
  if (!pszImage || !pszImage[0])
    return 0;
...
void KRepresentShell3::LookAt(int nX, int nY, int nZ) {
  ...
  if (g_renderModel == RenderModel3DOrtho ||
    ...
    D3DXMatrixLookAtLH(&m_matView, &m_vCamera, &D3DXVECTOR3(fX, fY, fZ),
                       &D3DXVECTOR3(0, 0, 1));
...
void KRepresentShell3::OutputText(int nFontId, const char *psText, int nCount,
                                  int nX, int nY, unsigned int Color,
                                  int nLineWidth, int nZ,
  ...
  if (nZ != TEXT_IN_SINGLE_PLANE_COORD) {
    ...
    if (m_dwWindowStyle == RenderModel3DPerspective) {
      ...
      D3DVIEWPORT9 viewportData = g_Device.GetViewport();
      D3DXVec3Project(&vPos, &vPos, &viewportData, &m_matProj, &m_matView,
                      NULL);
...
int KRepresentShell3::LocateRichText(int nX, int nY, int nFontId,
  ...
  if (i < RS2_MAX_FONT_ITEM_NUM && m_FontTable[i].pFontObj) {
    ...
    m_FontTable[i].pFontObj->SetBorderColor(pParam->BorderColor);
    m_FontTable[i].pFontObj->SetOutputSize(nFontId, nFontId + 1);
    return tp.TransXYPosToCharOffset(nX, nY, m_FontTable[i].pFontObj, nFontId,
                                     pParam);
  }
  return -1;
}
...
bool KRepresentShell3::SaveImage(const char *pszFile, const char *pszImage,
                                 int nFileType) {
...
void KRepresentShell3::SetImageStoreBalanceParam(int nNumImage,
                                                 unsigned int uCheckPoint) {
  m_TextureResMgr.SetBalanceParam(nNumImage, uCheckPoint);
}

bool KRepresentShell3::CopyDeviceImageToImage(const char *pszName, int nDeviceX,
                                              int nDeviceY, int nImageX,
                                              int nImageY, int nWidth,
                                              int nHeight) {
  ...
  /*	float fAngleAdd = 0.03f;
          ...
          for(int i=0; i<LIGHTING_GRID_HEIGHT; i++)
                  for(int j=0; j<LIGHTING_GRID_WIDTH; j++)
                  {
                          ...
                          c22 = (1.0f - fd) * c2;
                          c33 = (1.0f - fd) * c3;
                          dwTp[i * LIGHTING_GRID_WIDTH + j] = 0xff000000 |
  (c11<<16) | (c22<<8) | c33;
                  }
...
void KRepresentShell3::ViewPortCoordToSpaceCoord(int &nX, int &nY, int nZ) {
  if (g_renderModel == RenderModel3DPerspective) {
    D3DVIEWPORT9 viewportData = g_Device.GetViewport();
    D3DXVECTOR3 vPos(
        (2.0f * (float)nX / viewportData.Width - 1.0f) / m_matProj._11,
        (1.0f - 2.0f * (float)nY / viewportData.Height) / m_matProj._22, 1.0f);
    D3DXVECTOR3 vOut1, vOut2;

    // 鼠标确定的选择射线的方向向量
    vOut2.x = vPos.x * m_matViewInverse._11 + vPos.y * m_matViewInverse._21 +
              vPos.z * m_matViewInverse._31;
    vOut2.y = vPos.x * m_matViewInverse._12 + vPos.y * m_matViewInverse._22 +
              vPos.z * m_matViewInverse._32;
    vOut2.z = vPos.x * m_matViewInverse._13 + vPos.y * m_matViewInverse._23 +
              vPos.z * m_matViewInverse._33;
...
void KRepresentShell3::DrawRect(int32 x1, int32 y1, int32 nWidth, int32 nHeight,
                                DWORD color) {
  if (FAILED(PD3DDEVICE->SetStreamSource(0, m_pVB2D, 0, sizeof(VERTEX2D))))
    return;
  PD3DDEVICE->SetFVF(D3DFVF_VERTEX2D);
...
void KRepresentShell3::DrawPoint(int nPrimitiveCount,
                                 KRepresentUnit *pPrimitives,
                                 int bSinglePlaneCoord) {
  if (FAILED(PD3DDEVICE->SetStreamSource(0, m_pVB2D, 0, sizeof(VERTEX2D))))
    return;
  ...
  VERTEX2D *pvb;
  if (FAILED(m_pVB2D->Lock(0, nPrimitiveCount * sizeof(VERTEX2D), (void **)&pvb,
                           0)))
    return;
...
void KRepresentShell3::DrawLine(int32 x1, int32 y1, int32 x2, int32 y2,
                                DWORD color) {
  if (FAILED(PD3DDEVICE->SetStreamSource(0, m_pVB2D, 0, sizeof(VERTEX2D))))
    return;
  PD3DDEVICE->SetFVF(D3DFVF_VERTEX2D);
...
void KRepresentShell3::DrawRectFrame(int32 x1, int32 y1, int32 x2, int32 y2,
                                     DWORD color) {
  if (FAILED(PD3DDEVICE->SetStreamSource(0, m_pVB2D, 0, sizeof(VERTEX2D))))
    return;
  PD3DDEVICE->SetFVF(D3DFVF_VERTEX2D);
...
void KRepresentShell3::DrawBitmap16(int32 nX, int32 nY, int32 nWidth,
                                    int32 nHeight, TextureResBmp *pBitmap,
                                    bool bStretch) {
  if (FAILED(PD3DDEVICE->SetStreamSource(0, m_pVB2D, 0, sizeof(VERTEX2D))))
    return;
  PD3DDEVICE->SetFVF(D3DFVF_VERTEX2D);
  ...
  float fU2, fV2;
  fU2 =
      (float)pBitmap->m_nWidth / (float)pBitmap->m_FrameInfo.texInfo[0].nWidth;
  fV2 = (float)pBitmap->m_nHeight /
        (float)pBitmap->m_FrameInfo.texInfo[0].nHeight;
...
void KRepresentShell3::DrawSpriteAlpha(int32 nX, int32 nY, int32 nWidth,
                                       int32 nHeight, int32 nFrame,
                                       TextureResSpr *pSprite, DWORD color,
                                       int32 nRenderStyle) {
  int i;
...
void KRepresentShell3::DrawSpritePartAlpha(int32 nX, int32 nY, int32 nWidth,
                                           int32 nHeight, int32 nFrame,
                                           TextureResSpr *pSprite, RECT &rect) {
  int i;
...
void KRepresentShell3::DrawBitmap16Part(int32 nX, int32 nY, int32 nWidth,
                                        int32 nHeight, TextureResBmp *pBitmap,
                                        RECT &rc) {
  if (FAILED(PD3DDEVICE->SetStreamSource(0, m_pVB2D, 0, sizeof(VERTEX2D))))
    return;
...
void KRepresentShell3::DrawBitmap163D(RenderParam3D &param,
                                      TextureResBmp *pBitmap) {
  float fU2, fV2;
  fU2 =
      (float)pBitmap->m_nWidth / (float)pBitmap->m_FrameInfo.texInfo[0].nWidth;
  fV2 = (float)pBitmap->m_nHeight /
...
void KRepresentShell3::DrawBitmap163DLighting(RenderParam3D &param,
                                              TextureResBmp *pBitmap) {
  float fU2, fV2;
  fU2 =
      (float)pBitmap->m_nWidth / (float)pBitmap->m_FrameInfo.texInfo[0].nWidth;
  ...
  VERTEX3D *pvb = NULL;
  if (FAILED(m_pVB3D->Lock(0, VERTEX_BUFFER_SIZE * sizeof(VERTEX3D),
                           (void **)&pvb, 0)))
    return;
...
void KRepresentShell3::DrawSpriteAlpha3D(RenderParam3D &param, int32 nFrame,
  ...
  D3DVIEWPORT9 viewportData = g_Device.GetViewport();
  D3DXVec3Project(&vPos1, &param.m_pos[0], &viewportData, &m_matProj,
                  &m_matView, NULL);
  D3DXVec3Project(&vPos2, &param.m_pos[1], &viewportData, &m_matProj,
                  &m_matView, NULL);
  D3DXVec3Project(&vPos3, &param.m_pos[2], &viewportData, &m_matProj,
                  &m_matView, NULL);
  D3DXVec3Project(&vPos4, &param.m_pos[3], &viewportData, &m_matProj,
                  &m_matView, NULL);

  if (vPos2.x < 0 || vPos1.x > g_nScreenWidth || vPos3.x < 0 ||
      vPos4.x > g_nScreenWidth)
    return;
  if (vPos3.y < 0 || vPos1.y > g_nScreenHeight || vPos4.y < 0 ||
      vPos2.y > g_nScreenHeight)
    return;
  ...
  for (i = 0; i < pSprite->m_pFrameInfo[nFrame].nTexNum; i++) {
    ...
    if (rect) {
      ...
      fRcX1 = (float)rect->left, fRcY1 = (float)rect->top;
      fRcX2 = (float)rect->right, fRcY2 = (float)rect->bottom;
      if (fRcX1 > fFrameX2 || fRcX2 < fFrameX1 || fRcY1 > fFrameY2 ||
          fRcY2 < fFrameY1) {
        // 如果这个面片的剪彩矩形在图素矩形之外则不画
        bDraw[i] = false;
        continue;
    ...
    v11 = ChaZhi(param.m_pos[0], param.m_pos[1], 0,
                 (float)pSprite->m_pFrameInfo[nFrame].nWidth,
                 (float)pSprite->m_pFrameInfo[nFrame].texInfo[i].nFrameX);
    v22 = ChaZhi(param.m_pos[3], param.m_pos[2], 0,
                 (float)pSprite->m_pFrameInfo[nFrame].nWidth,
                 (float)pSprite->m_pFrameInfo[nFrame].texInfo[i].nFrameX);
...
void KRepresentShell3::DrawSpriteAlpha3DLighting(
    RenderParam3D &param, int32 nFrame, TextureResSpr *pSprite, DWORD color,
    int32 nRenderStyle, RECT *rect) {
  int i;
  if (nFrame >= pSprite->m_nFrameNum)
    return;
...
uint32 KRepresentShell3::BuildMesh3D(VERTEX3D *pvbSrc, VERTEX3D *pvbDes,
                                     uint32 nNumDes, DWORD color) {
  uint32 nCount = 0;
  ...
  if (pvbSrc[0].position.z == pvbSrc[2].position.z) {
    ...
    ftuAdd = (pvbSrc[1].tu - pvbSrc[0].tu) * MESH_GRID_SIZE /
             (pvbSrc[1].position.x - pvbSrc[0].position.x);
    ftvAdd = (pvbSrc[2].tv - pvbSrc[0].tv) * MESH_GRID_SIZE /
             (pvbSrc[2].position.y - pvbSrc[0].position.y);
    ...
    ftuAdd = (pvbSrc[1].tu - pvbSrc[0].tu) *
             (MESH_GRID_SIZE / (pvbSrc[1].position.x - pvbSrc[0].position.x));
    ftvAdd = (pvbSrc[3].tv - pvbSrc[0].tv) *
             (MESH_GRID_SIZE / (pvbSrc[0].position.z - pvbSrc[3].position.z));
    fvAdd = (pvbSrc[1].position - pvbSrc[0].position) *
            (MESH_GRID_SIZE / (pvbSrc[1].position.x - pvbSrc[0].position.x));
...
unsigned int KRepresentShell3::GetPoint3dLightingLerp(D3DXVECTOR3 &v) {
  ...
  for (int i = 0; i < 4; i++) {
    if (pt[i].nX < m_nLightingAreaX || pt[i].nY < m_nLightingAreaY ||
        pt[i].nX >= m_nLightingAreaX + 1536 ||
        pt[i].nY >= m_nLightingAreaY + 3072)
  ...
  dwLightColor = 0xff000000 | (((DWORD)vColor[0].x) << 16) |
                 (((DWORD)vColor[0].y) << 8) | ((DWORD)vColor[0].z);

  return dwLightColor;
}

void KRepresentShell3::RIO_CopyBitmap16ToBuffer(TextureResBmp *pBitmap,
                                                TextureResBmp *pDestBitmap,
                                                int32 nX, int32 nY) {
  return;
  /*	int   nDestWidth  = pDestBitmap->GetWidth();
          ...
          if ( pBitmap->m_TextureInfo.pTexture->GetSurfaceLevel( 0,
     &pSrcSurface) != D3D_OK ) return; if (
     pDestBitmap->m_TextureInfo.pTexture->GetSurfaceLevel( 0, &pDesSurface) !=
     D3D_OK )
          {
                  pSrcSurface->Release();
                  return;
          ...
          PD3DDEVICE->CopyRects( pSrcSurface, &SrcRect, 1, pDesSurface,
     &DesPoint); pSrcSurface->Release(); pDesSurface->Release();*/
}

inline WORD C32BitTo16Bit555(BYTE *pSrc) {
  return (((WORD)(*pSrc)) >> 3) +
         ((((WORD)(*(pSrc + 1))) << 2) & MASK_GREEN_555) +
         ((((WORD)(*(pSrc + 2))) << 7) & MASK_RED_555);
}
...
inline void C32BitTo16Bit565(BYTE *pSrc, WORD *pDesColor) {
  ...
  *pDesColor = ((rs * as) & MASK_RED_565) +
               (((gs * as) >> 5) & MASK_GREEN_565) + ((bs * as) >> 11) +
               ((rd * ad) & MASK_RED_565) +
               (((gd * ad) >> 5) & MASK_GREEN_565) + ((bd * ad) >> 11);
}

void KRepresentShell3::RIO_CopySprToBufferAlpha(TextureResSpr *pSprite,
                                                int32 nFrame,
                                                TextureResBmp *pDestBitmap,
                                                int32 nX, int32 nY) {
  if (nFrame >= pSprite->m_nFrameNum)
  ...
  for (i = 0; i < pSprite->m_pFrameInfo[nFrame].nTexNum; i++) {
    ...
    if (ny >= 0)
      ...
      fV1 =
          -(float)ny / (float)pSprite->m_pFrameInfo[nFrame].texInfo[i].nHeight;
    fU2 = fU1 + (float)nTargetWidth /
                    (float)pSprite->m_pFrameInfo[nFrame].texInfo[i].nWidth;
    fV2 = fV1 + (float)nTargetHeight /
                    (float)pSprite->m_pFrameInfo[nFrame].texInfo[i].nHeight;
...
void KRepresentShell3::SetLightInfo(int nX, int nY, unsigned int *pLighting) {
  ...
  m_nLightingAreaX = nX;
  m_nLightingAreaY = nY;
  memcpy(pLightingArray, pLighting,
         LIGHTING_GRID_WIDTH * LIGHTING_GRID_HEIGHT * sizeof(DWORD));
}

bool KRepresentShell3::SaveScreenToFile(const char *pszName,
  ...
  if (g_bRunWindowed) {
    ...
    // 如果窗口客户区超出屏幕则返回
    if (ptLT.x >= nDesktopWidth || ptLT.y >= nDesktopHeight || ptRB.x <= 0 ||
        ptRB.y <= 0)
      return false;
    if (ptLT.x < 0)
      ptLT.x = 0;
  ...
  if (FAILED(PD3DDEVICE->CreateOffscreenPlainSurface(
          nDesktopWidth, nDesktopHeight, D3DFMT_A8R8G8B8, D3DPOOL_SYSTEMMEM,
          &pSurface, NULL)))
    goto error;
  ...
  pData =
      ((BYTE *)lockedRect.pBits) + nPicOffY * nDesktopWidth * 4 + nPicOffX * 4;
  BOOL bRet;
  if (eType == SCRFILETYPE_BMP)
    // 保存24位bmp文件
    bRet = KBmpFile24::SaveBuffer32((char *)pszName, pData, nDesktopWidth * 4,
                                    nPicWidth, nPicHeight);
  else
    // 保存24位jpg文件
    bRet = SaveBufferToJpgFile32((char *)pszName, pData, nDesktopWidth * 4,
                                 nPicWidth, nPicHeight, nQuality);
  if (!bRet) {
    pSurface->UnlockRect();
    goto error;
  }
