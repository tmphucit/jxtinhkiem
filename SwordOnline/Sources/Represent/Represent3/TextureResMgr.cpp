...
TextureResMgr::~TextureResMgr() { Free(); }
...
uint32 TextureResMgr::CreateImage(const char *pszName, int32 nWidth,
                                  int32 nHeight, int32 nType) {
...
bool TextureResMgr::GetImageParam(const char *pszImage, KImageParam *pImageData,
                                  int nType) {
  if (!pszImage || !pszImage[0])
    return false;
...
bool TextureResMgr::GetImageFrameParam(const char *pszImage, int nFrame,
                                       KRPosition2 *pOffset, KRPosition2 *pSize,
                                       int nType) {
  if (!pszImage || !pszImage[0])
    return false;
...
TextureRes *TextureResMgr::GetImage(const char *pszImage, unsigned int &uImage,
                                    short &nImagePosition, int nFrame,
                                    int nType, bool bPrepareTex) {
  if (!pszImage || !pszImage[0])
    return NULL;

  if (!uImage) {
    uImage =
        g_FileName2Id((LPSTR)pszImage); // const char to LPSTR, maybe problem.
  }
  ...
  if ((nImagePosition = FindImage(uImage, nImagePosition)) >= 0) {
    if (m_TextureResList[nImagePosition].m_nType == nType) {
      ...
      m_uTexCacheMemUsed -=
          m_TextureResList[nImagePosition].m_pTextureRes->m_nTexMemUsed;
  ...
  if ((m_uTexCacheMemUsed > m_nBalanceNum) &&
  ...
  if (pObject && nType == ISI_T_SPR) {
    if (!((TextureResSpr *)pObject)
             ->PrepareFrameData(pszImage, nFrame, bPrepareTex))
      return NULL;
  }
...
int32 TextureResMgr::GetImagePixelAlpha(const char *pszImage, int nFrame,
  ...
  case ISI_T_SPR:
    ...
    if (!pTemp)
      break;
    nRet = ((TextureResSpr *)pTemp)
               ->GetPixelAlpha(nFrame, nX, nY); // 在Spr的方法里已经处理了范围
    break;
  case ISI_T_BITMAP16:
    pTemp = GetImage(pszImage, uImage, nPos, nFrame, ISI_T_BITMAP16);
    if (!pTemp)
      break;
    if (nX < 0 || nY < 0 || nX >= ((TextureResBmp *)pTemp)->GetWidth() ||
        nY >= ((TextureResBmp *)pTemp)->GetHeight())
      nRet = 0;
    else
      nRet = 255;
    break;
...
TextureRes *TextureResMgr::LoadImage(const char *pszImageFile,
                                     uint32 nType) const {
...
void TextureResMgr::GetProfileString(char *str, int BufLen) {
  ...
  char cc[200];
  sprintf(cc, "CacheMemUsed: %dM\n MemDrawingUsed: %d\n TextureNumber: %d\n",
          m_uTexCacheMemUsed / 1024 / 1024, m_uMemDrawingUsed / 1024 / 1024,
          m_TextureResList.size());
