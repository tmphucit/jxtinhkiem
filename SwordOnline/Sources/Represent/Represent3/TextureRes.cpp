...
inline void RenderToA4R4G4B4(WORD *pDest, BYTE *pSrc, int width, int height,
                             BYTE *pPalette) {
  __asm
  {
...
TextureResBmp::~TextureResBmp() { Release(); }
...
bool TextureResBmp::RestoreDeviceObjects() {
  if (!m_FrameInfo.texInfo[0].pTexture) {
    if (FAILED(D3DXCreateTexture(
            PD3DDEVICE, m_FrameInfo.texInfo[0].nWidth,
            m_FrameInfo.texInfo[0].nHeight, 1, D3DUSAGE_RENDERTARGET,
            g_16BitFormat, D3DPOOL_DEFAULT, &m_FrameInfo.texInfo[0].pTexture)))
      return false;
  }

  if (m_pSysMemTexture) {
    m_pSysMemTexture->AddDirtyRect(NULL);
    PD3DDEVICE->UpdateTexture(m_pSysMemTexture,
                              m_FrameInfo.texInfo[0].pTexture);
  }
...
// 创建内存资源
bool TextureResBmp::CreateImage(const char *szImage, int nWidth, int nHeight,
                                uint32 nType) {
  if (!szImage || !szImage[0])
    return false;

  if (nWidth < 0 || nWidth > MAX_TEXTURE_SIZE || nHeight < 0 ||
  ...
  if (FAILED(D3DXCreateTexture(
          ...
          g_16BitFormat, D3DPOOL_DEFAULT, &m_FrameInfo.texInfo[0].pTexture)))
    goto error;
  if (FAILED(D3DXCreateTexture(PD3DDEVICE, m_FrameInfo.texInfo[0].nWidth,
                               m_FrameInfo.texInfo[0].nHeight, 1, 0,
                               g_16BitFormat, D3DPOOL_SYSTEMMEM,
                               &m_pSysMemTexture)))
    goto error;

  m_nTexMemUsed =
      m_FrameInfo.texInfo[0].nWidth * m_FrameInfo.texInfo[0].nHeight * 2;
  return true;
...
TextureResSpr::TextureResSpr() { ResetVar(); }

TextureResSpr::~TextureResSpr() { Release(); }
...
// 创建内存资源
bool TextureResSpr::CreateImage(const char *szImage, int nWidth, int nHeight,
                                uint32 nType) {
  return false;
}
...
bool TextureResSpr::LoadSprFile(char *szImage) {
  ...
  for (i = 0; i < pHeader->Colors; i++) {
    m_pPal16[i] =
        ARGBTO4444(0, m_pPal24[i].Red, m_pPal24[i].Green, m_pPal24[i].Blue);
  }
  ...
  if (pFrameData) {
    ...
    for (i = 0; i < m_nFrameNum; i++) {
      ...
      memcpy(m_pFrameInfo[i].pRawData, pFrame->Sprite,
             m_pFrameInfo[i].nRawDataLen);
    ...
    m_pOffset =
        (SPROFFS *)(m_pHeader + sizeof(SPRHEAD) + m_nColors * sizeof(KPAL24));
  }
...
bool TextureResSpr::PrepareFrameData(const char *szImage, int32 nFrame,
                                     bool bPrepareTex) {
  assert(szImage);
  if (!szImage)
    return false;
  ...
  if (!m_pFrameInfo[nFrame].pRawData) {
    ...
    m_pFrameInfo[nFrame].nRawDataLen =
        m_pOffset[nFrame].Length - 8; // sizeof(SPRFRAME);
...
void TextureResSpr::CreateTexture16Bit(const char *szImage, int32 nFrame) {
  ...
  BYTE *pTempData =
      new BYTE[m_pFrameInfo[nFrame].nWidth * m_pFrameInfo[nFrame].nHeight * 2];
  if (!pTempData)
    return;

  RenderToA4R4G4B4((WORD *)pTempData, m_pFrameInfo[nFrame].pRawData,
                   m_pFrameInfo[nFrame].nWidth, m_pFrameInfo[nFrame].nHeight,
                   (BYTE *)m_pPal16);

  /*
          // 填充贴图数据
          uint32 nByteCount = 0;
     // 总字节计数 uint32 nWidth = m_pFrameInfo[nFrame].nWidth; uint32 nHeight =
     m_pFrameInfo[nFrame].nHeight; uint32 nFrameLen =
     m_pFrameInfo[nFrame].nRawDataLen; BYTE *pData =
     m_pFrameInfo[nFrame].pRawData; BYTE pixelNum, alpha, pixelColor; BYTE
     *pTexLine = pTempData;						//
     贴图每一行数据 for(;;)
          {
                  pixelNum = *pData++;
                  ...
                  else
                  {
                          ...
                          {
                                  pixelColor = *pData++;
                          //	*((WORD*)pTexLine) = ARGBTO4444(alpha,
     m_pPal24[pixelColor].Red, m_pPal24[pixelColor].Green,
     m_pPal24[pixelColor].Blue);
                                  *((WORD*)pTexLine) =
     ((((WORD)alpha)<<8)&0xf000) | m_pPal16[pixelColor]; pTexLine += 2;
                          }
                          nByteCount += pixelNum;
                  }
  ...
  */
  ...
  for (i = 0; i < m_pFrameInfo[nFrame].nTexNum; i++) {
    m_nTexMemUsed += m_pFrameInfo[nFrame].texInfo[i].nWidth *
                     m_pFrameInfo[nFrame].texInfo[i].nHeight * 2;
    ...
    if (FAILED(PD3DDEVICE->CreateTexture(
            ...
            m_pFrameInfo[nFrame].texInfo[i].nHeight, 1, 0, D3DFMT_A4R4G4B4,
            D3DPOOL_MANAGED, &m_pFrameInfo[nFrame].texInfo[i].pTexture, NULL)))
      goto error;

    D3DLOCKED_RECT LockedRect;
    if (FAILED(m_pFrameInfo[nFrame].texInfo[i].pTexture->LockRect(
            0, &LockedRect, NULL, 0)))
      goto error;

    // 向贴图拷贝数据
    BYTE *pTexData = (BYTE *)LockedRect.pBits; // 贴图数据
    BYTE *pTp = pTempData + (m_pFrameInfo[nFrame].texInfo[i].nFrameY *
                                 m_pFrameInfo[nFrame].nWidth +
                             m_pFrameInfo[nFrame].texInfo[i].nFrameX) *
                                2;
...
LPDIRECT3DTEXTURE8 TextureResSpr::CreateTexture16Bit(uint32 nFrame)
{
        ...
        // 创建贴图
        if (FAILED(PD3DDEVICE->CreateTexture(m_pTextureInfo[nFrame].nWidth,
m_pTextureInfo[nFrame].nHeight, 1, 0, D3DFMT_A4R4G4B4, D3DPOOL_MANAGED,
&m_pTextureInfo[nFrame].pTexture))) return NULL;

        D3DLOCKED_RECT LockedRect;
        if (FAILED(m_pTextureInfo[nFrame].pTexture->LockRect(0, &LockedRect,
NULL, 0)))
        {
        ...
        m_nTexMemUsed += m_pTextureInfo[nFrame].nWidth *
m_pTextureInfo[nFrame].nHeight * 2; m_nMemUseful += m_pFrameInfo[nFrame].nWidth
* m_pFrameInfo[nFrame].nHeight * 2;

        // 填充贴图数据
        uint32 nByteCount = 0; // 总字节计数 uint32 nPixelCount = 0;
// 总象素计数 uint32 nWidth = m_pFrameInfo[nFrame].nWidth; uint32 nHeight =
m_pFrameInfo[nFrame].nHeight; uint32 nFrameLen =
m_pFrameInfo[nFrame].nRawDataLen; BYTE *pData = m_pFrameInfo[nFrame].pRawData;
        BYTE pixelNum, alpha, pixelColor;
        BYTE *pTexData = (BYTE*)LockedRect.pBits;		// 贴图数据
        BYTE *pTexLine = pTexData; // 贴图每一行数据 int nTp2 = 0; for(;;)
        {
                pixelNum = *pData++;
                alpha = *pData++;
                nByteCount += 2;
...
m_pPal24[pixelColor].Red, m_pPal24[pixelColor].Green,
m_pPal24[pixelColor].Blue);
                                *((WORD*)pTexLine) = ((((WORD)alpha)<<8)&0xf000)
| m_pPal16[pixelColor]; pTexLine += 2;
                        }
                        else
                                pData += pixelNum;
                        nByteCount += pixelNum;
                ...
                if(nPixelCount == nWidth)
                {
                        ...
                        if(nByteCount >= nFrameLen)
                                break;
                        if((nTp2%2) == 1)
                                pTexLine = pTexData = pTexData +
LockedRect.Pitch; nPixelCount = 0; nTp2++;
                }
        }
...
void TextureResSpr::CreateTexture32Bit(uint32 nFrame) {
  ...
  /*	if(nFrame >= m_nFrameNum)
          ...
          // 创建贴图
          if (FAILED(PD3DDEVICE->CreateTexture(m_pTextureInfo[nFrame].nWidth,
     m_pTextureInfo[nFrame].nHeight, 1, 0, D3DFMT_A8R8G8B8, D3DPOOL_MANAGED,
     &m_pTextureInfo[nFrame].pTexture))) return NULL;
     ...
     NULL, 0)))
          ...
          m_nTexMemUsed += m_pTextureInfo[nFrame].nWidth *
     m_pTextureInfo[nFrame].nHeight * 4; m_nMemUseful +=
     m_pFrameInfo[nFrame].nWidth * m_pFrameInfo[nFrame].nHeight * 4;

          // 填充贴图数据
          uint32 nByteCount = 0;
     // 总字节计数 uint32 nPixelCount = 0;
     // 总象素计数 uint32 nWidth = m_pFrameInfo[nFrame].nWidth; uint32 nHeight =
     m_pFrameInfo[nFrame].nHeight; uint32 nTotlePixelNum = nWidth * nHeight;
     // 总象素数目 uint32 nFrameLen = m_pFrameInfo[nFrame].nRawDataLen; BYTE
     *pData = m_pFrameInfo[nFrame].pRawData; BYTE pixelNum, alpha, pixelColor;
          BYTE *pTexData = (BYTE*)LockedRect.pBits;		// 贴图数据
          BYTE *pTexLine = pTexData; // 贴图每一行数据 for(;;)
          {
                  pixelNum = *pData++;
                  alpha = *pData++;
                  nByteCount += 2;
...
LPDIRECT3DTEXTURE8 TextureResSpr::CreateTexture16Bit(uint32 nFrame)
{
        ...
        // 创建贴图
        if (FAILED(D3DXCreateTexture(PD3DDEVICE, m_pTextureInfo[nFrame].nWidth,
m_pTextureInfo[nFrame].nHeight, 1, 0, D3DFMT_A4R4G4B4, D3DPOOL_SYSTEMMEM,
&pTexture))) return NULL;
...
//	uint32 nTotlePixelNum = m_pTextureInfo[nFrame].nWidth *
m_pTextureInfo[nFrame].nHeight; for(;;)
        {
                pixelNum = *pData++;
                alpha = *pData++;
                nByteCount += 2;
...
| m_pPal16[pixelColor]; pTexLine += 2;
        ...
        // 创建贴图
        if (FAILED(D3DXCreateTexture(PD3DDEVICE, m_pTextureInfo[nFrame].nWidth,
m_pTextureInfo[nFrame].nHeight, 1, 0, D3DFMT_DXT5, D3DPOOL_MANAGED,
&m_pTextureInfo[nFrame].pTexture))) goto error;

        if ( pTexture->GetSurfaceLevel( 0, &pSrcSurface) != D3D_OK )
                goto error;
        if ( m_pTextureInfo[nFrame].pTexture->GetSurfaceLevel( 0, &pDesSurface)
!= D3D_OK ) goto error;

        D3DXLoadSurfaceFromSurface( pDesSurface, NULL, NULL, pSrcSurface, NULL,
...
LPDIRECT3DTEXTURE8 TextureResSpr::CreateTexture32Bit(uint32 nFrame)
{
        ...
        // 创建贴图
        if (FAILED(D3DXCreateTexture(PD3DDEVICE, m_pTextureInfo[nFrame].nWidth,
m_pTextureInfo[nFrame].nHeight, 1, 0, D3DFMT_A8R8G8B8, D3DPOOL_SYSTEMMEM,
&pTexture))) return NULL;
        ...
        m_nTexMemUsed += m_pTextureInfo[nFrame].nWidth *
m_pTextureInfo[nFrame].nHeight * 4; m_nMemUseful += m_pFrameInfo[nFrame].nWidth
* m_pFrameInfo[nFrame].nHeight * 4;

        // 填充贴图数据
        uint32 nByteCount = 0; // 总字节计数 uint32 nPixelCount = 0;
// 总象素计数 uint32 nWidth = m_pFrameInfo[nFrame].nWidth; uint32 nHeight =
m_pFrameInfo[nFrame].nHeight; uint32 nTotlePixelNum = nWidth * nHeight;
// 总象素数目 uint32 nFrameLen = m_pFrameInfo[nFrame].nRawDataLen; BYTE *pData =
m_pFrameInfo[nFrame].pRawData; BYTE pixelNum, alpha, pixelColor; BYTE *pTexData
= (BYTE*)LockedRect.pBits;		// 贴图数据 BYTE *pTexLine = pTexData;
// 贴图每一行数据 for(;;)
        {
                pixelNum = *pData++;
                alpha = *pData++;
...
int32 TextureResSpr::GetPixelAlpha(int32 nFrame, int32 x, int32 y) {
  ...
  for (int i = 0; i < m_pFrameInfo[nFrame].nTexNum; i++) {
    ...
    // 检测象素点是否落在第i张贴图上
    if (tx >= 0 && ty >= 0 &&
        tx < m_pFrameInfo[nFrame].texInfo[i].nFrameWidth &&
      ...
      D3DLOCKED_RECT LockedRect;
      if (FAILED(m_pFrameInfo[nFrame].texInfo[i].pTexture->LockRect(
              0, &LockedRect, NULL, D3DLOCK_READONLY)))
        return nRet;
  ...
  if (m_pFrameInfo) {
    for (int i = 0; i < m_nFrameNum; i++) {
      ...
      if (m_pFrameInfo[i].texInfo[0].pTexture) {
        for (int j = 0; j < 4; j++) {
          m_nTexMemUsed -= m_pFrameInfo[i].texInfo[j].nWidth *
                           m_pFrameInfo[i].texInfo[j].nHeight * 2;
...
int TextureResSpr::SplitTexture(uint32 nFrame) {
  ...
  if (nWidth > nHalfWidth && nWidth <= nFourThreeWidth &&
      nHeight > nHalfHeight && nHeight <= nFourThreeHeight) {
    // 拆为4份
    ...
    nMem = m_pFrameInfo[nFrame].texInfo[0].nWidth *
           m_pFrameInfo[nFrame].texInfo[0].nHeight * 2;
  }
