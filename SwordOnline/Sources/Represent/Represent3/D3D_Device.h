...
public:
  ...
  // 创建设备
  bool CreateDevice(D3DAdapterInfo *pAdapter, D3DDeviceInfo *pDevice,
                    D3DModeInfo *pMode);
  ...
  void SetupViewport(uint32 iLeft, uint32 iRight, uint32 iTop, uint32 iBottom,
...
private:
  ...
  D3DAdapterInfo *m_pAdapter; // My Adaptor Info Pointer (Points into
                              // g_D3DShell's device list)
  D3DDeviceInfo *m_pDevice; // My Device Info Pointer (Points into g_D3DShell's
                            // device list)
  D3DModeInfo
      *m_pMode; // My Mode Info Pointer (Points into g_D3DShell's mode list)
  bool m_bWindowed;
...
#define PD3DDEVICE                                                             \
  (g_Device.m_pD3DDevice) // Use for quick access to the D3DDevice...
static LPDIRECT3DDEVICE9 d3d_GetD3DDevice() {
  return PD3DDEVICE;
} // For the RenderStruct...
