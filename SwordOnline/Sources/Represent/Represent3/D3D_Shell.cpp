...
bool CD3D_Shell::BuildDeviceList() {
  ...
  for (UINT iAdapter = 0; iAdapter < m_pD3D->GetAdapterCount(); ++iAdapter) {
    ...
    for (int i = 0; i < 3; i++) {
      uint32 iNumAdapterModes =
          m_pD3D->GetAdapterModeCount(iAdapter, g_PixelFormat[i]);
      ...
      for (uint32 iMode = 0; iMode < iNumAdapterModes; iMode++) {
        ...
        // 取得显示模式属性
        m_pD3D->EnumAdapterModes(iAdapter, g_PixelFormat[i], iMode,
                                 &d3dDisplayMode);
    ...
    for (UINT iDevice = 0; iDevice < iNumDeviceTypes; ++iDevice) {
      ...
      for (DWORD f = 0; f < formats.size(); ++f) {
        ...
        // 检查是否可以渲染
        if (FAILED(m_pD3D->CheckDeviceType(iAdapter, Device.DeviceType,
...
//	Pick Device Named szDevName if there's one of this name, otherwise, will
// pick what it thinks is best...
D3DDeviceInfo *CD3D_Shell::PickDefaultDev(D3DAdapterInfo **pAdapterInfo) {
  for (vector<D3DAdapterInfo>::iterator itAdapter = m_AdapterList.begin();
       itAdapter != m_AdapterList.end(); ++itAdapter) {
    for (vector<D3DDeviceInfo>::iterator itDevice = itAdapter->Devices.begin();
         itDevice != itAdapter->Devices.end(); ++itDevice) {
      if (g_bRefRast && itDevice->DeviceType != D3DDEVTYPE_REF)
        continue;
      if (!g_bRefRast && itDevice->DeviceType != D3DDEVTYPE_HAL)
        continue;
...
D3DModeInfo *CD3D_Shell::PickDefaultMode(D3DDeviceInfo *pDeviceInfo,
                                         uint32 iBitDepth) {
  for (vector<D3DModeInfo>::iterator itMode = pDeviceInfo->Modes.begin();
       itMode != pDeviceInfo->Modes.end(); ++itMode) {
    if (itMode->Width != g_nScreenWidth)
      continue;
    if (itMode->Height != g_nScreenHeight)
      continue;
    ...
    if (!g_bRunWindowed) {
      switch (iBitDepth) {
      case 32:
        if (itMode->Format != D3DFMT_X8R8G8B8 &&
            itMode->Format != D3DFMT_R8G8B8)
          continue;
        break;
      case 24:
        if (itMode->Format != D3DFMT_X8R8G8B8 &&
        ...
        break;
      case 16:
        if (itMode->Format != D3DFMT_R5G6B5 &&
            itMode->Format != D3DFMT_X1R5G5B5)
          continue;
        break;
      }
    }
...
D3DDeviceInfo *CD3D_Shell::FindDevice(const char *strDesc,
...
D3DDeviceInfo *CD3D_Shell::GetDeviceInfo(unsigned int uAdapterID,
...
D3DModeInfo *CD3D_Shell::GetModeInfo(unsigned int uAdapterID,
                                     unsigned int uDeviceID,
                                     unsigned int uModeID) {
