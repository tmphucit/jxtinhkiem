...
// 设置偏色列表
unsigned int KRepresentShell2::SetAdjustColorList(unsigned int *puColorList,
                                                  unsigned int uCount) {
...
bool KRepresentShell2::CreateAFont(const char *pszFontFile,
...
unsigned int KRepresentShell2::CreateImage(const char *pszName, int nWidth,
                                           int nHeight, int nType) {
  return m_ImageStore.CreateImage(pszName, nWidth, nHeight, nType);
}

// ##ModelId=3DB69FE401DA
void KRepresentShell2::DrawPrimitives(int nPrimitiveCount,
...
#define CENTERY 192
      ...
      case ISI_T_BITMAP16: {
        ...
        KSGImageContent *pBitmap = (KSGImageContent *)m_ImageStore.GetImage(
            pTemp->szImage, pTemp->uImage, pTemp->nISPosition, 0,
            ISI_T_BITMAP16, pFrame);
        if (pBitmap) {
          int nX = pTemp->oPosition.nX;
          int nY = pTemp->oPosition.nY;
          if (bSinglePlaneCoord == false)
            CoordinateTransform(nX, nY, pTemp->oPosition.nZ);
          m_Canvas.DrawBitmap16(nX, nY, pBitmap->nWidth, pBitmap->nHeight,
                                pBitmap->Data);
        }
      } break;
      }
  ...
  case RU_T_POINT: {
    ...
    for (i = 0; i < nPrimitiveCount; i++, pTemp++) {
      ...
      if (!bSinglePlaneCoord)
        CoordinateTransform(nX, nY, pTemp->oPosition.nZ);
      m_Canvas.DrawPixel(nX, nY,
                         g_RGB(pTemp->Color.Color_b.r, pTemp->Color.Color_b.g,
                               pTemp->Color.Color_b.b));
    }
  } break;
  case RU_T_LINE: {
    ...
    for (i = 0; i < nPrimitiveCount; i++, pTemp++) {
      ...
      if (pTemp->Color.Color_b.a >= 248) {
        m_Canvas.DrawLine(nX1, nY1, nX2, nY2,
                          g_RGB(pTemp->Color.Color_b.r, pTemp->Color.Color_b.g,
                                pTemp->Color.Color_b.b));
  ...
  case RU_T_RECT: {
    ...
    for (i = 0; i < nPrimitiveCount; i++, pTemp++) {
      ...
      int Color = g_RGB(pTemp->Color.Color_b.r, pTemp->Color.Color_b.g,
                        pTemp->Color.Color_b.b);
  ...
  case RU_T_SHADOW: {
    ...
    for (i = 0; i < nPrimitiveCount; i++, pTemp++) {
      ...
      m_Canvas.ClearAlpha(nX1, nY1, nX2 - nX1, nY2 - nY1,
                          g_RGB(pTemp->Color.Color_b.r, pTemp->Color.Color_b.g,
                                pTemp->Color.Color_b.b),
                          pTemp->Color.Color_b.a);
    }
  } break;
        ...
        if (pTemp->nType == ISI_T_BITMAP16) {
          ...
          if (pBitmap) {
            if (pSurface) {
              ...
              m_ImageStore.CreateBitmapSurface(pTemp->szImage, pTemp->uImage,
                                               pTemp->nISPosition);
            }
          }
        }
...
void KRepresentShell2::DrawPrimitivesOnImage(
  ...
  KSGImageContent *pDestBitmap =
      (KSGImageContent *)m_ImageStore.GetExistedCreateBitmap(pszImage, uImage,
                                                             nImagePosition);
  ...
  case RU_T_IMAGE:
    ...
    for (i = 0; i < nPrimitiveCount; i++, pTemp++) {
      ...
      case ISI_T_SPR: {
        ...
        case IMAGE_RENDER_STYLE_ALPHA:
        case IMAGE_RENDER_STYLE_ALPHA_NOT_BE_LIT:
          RIO_CopySprToBufferAlpha(pFrame->Sprite, pFrame->Width,
                                   pFrame->Height, pPalette, pDestBuffer,
                                   nDestWidth, nDestHeight, nX, nY);
          break;
        case IMAGE_RENDER_STYLE_3LEVEL:
          RIO_CopySprToBuffer3LevelAlpha(pFrame->Sprite, pFrame->Width,
                                         pFrame->Height, pPalette, pDestBuffer,
                                         nDestWidth, nDestHeight, nX, nY);
          break;
        case IMAGE_RENDER_STYLE_OPACITY:
          RIO_CopySprToBuffer(pFrame->Sprite, pFrame->Width, pFrame->Height,
                              pPalette, pDestBuffer, nDestWidth, nDestHeight,
                              nX, nY);
          break;
        case IMAGE_RENDER_STYLE_ALPHA_COLOR_ADJUST:
          pPalette = m_ImageStore.GetAdjustColorPalette(pTemp->nISPosition,
      ...
      case ISI_T_BITMAP16: {
        ...
        KSGImageContent *pBitmap = (KSGImageContent *)m_ImageStore.GetImage(
            ...
            pTemp->nType, pFrame);
        if (pBitmap) {
          RIO_CopyBitmap16ToBuffer(pBitmap->Data, pBitmap->nWidth,
                                   pBitmap->nHeight, pDestBitmap, nDestWidth,
                                   nDestHeight, pTemp->oPosition.nX,
                                   pTemp->oPosition.nY);
        }
      } break;
      }
...
// ##Documentation
// ## 清除图形数据
void KRepresentShell2::ClearImageData(const char *pszImage, unsigned int uImage,
...
void KRepresentShell2::FreeAllImage() { m_ImageStore.Free(); }
...
void *KRepresentShell2::GetBitmapDataBuffer(const char *pszImage,
  ...
  if (pDestBitmap) {
    ...
    if (pInfo) {
      ...
      pInfo->nPitch = nPitch;
      pInfo->pData = pBuffer;
      pInfo->eFormat = (m_DirectDraw.GetRGBBitMask16() == RGB_565)
                           ? BDBF_16BIT_565
                           : BDBF_16BIT_555;
    }
  }
...
void KRepresentShell2::ReleaseBitmapDataBuffer(const char *pszImage,
                                               void *pBuffer) {
  unsigned int uImage = 0;
  short nISPosition = -1;
  LPDIRECTDRAWSURFACE pSurface;
...
bool KRepresentShell2::GetImageParam(const char *pszImage,
...
bool KRepresentShell2::GetImageFrameParam(const char *pszImage, int nFrame,
  ...
  return m_ImageStore.GetImageFrameParam(pszImage, nType, nFrame, pOffset,
                                         pSize);
}

// ##ModelId=3DCA72620157
int KRepresentShell2::GetImagePixelAlpha(const char *pszImage, int nFrame,
...
void KRepresentShell2::OutputText(int nFontId, const char *psText, int nCount,
...
// ## 返回指定坐标在字符串中最近的字符偏移
int KRepresentShell2::LocateRichText(int nX, int nY, int nFontId,
...
bool KRepresentShell2::SaveImage(const char *pszFile, const char *pszImage,
                                 int nFileType) {
  return m_ImageStore.SaveImage(pszFile, pszImage, nFileType);
}

// ##ModelId=3DCD90F30011
void KRepresentShell2::SetImageStoreBalanceParam(int nNumImage,
                                                 unsigned int uCheckPoint) {
  m_ImageStore.SetBalanceParam(nNumImage, uCheckPoint);
}

// ##ModelId=3DD00EEE0149
bool KRepresentShell2::CopyDeviceImageToImage(const char *pszName, int nDeviceX,
                                              int nDeviceY, int nImageX,
                                              int nImageY, int nWidth,
                                              int nHeight) {
  if (nWidth > m_Canvas.GetWidth() - nDeviceX ||
      nHeight > m_Canvas.GetHeight() - nDeviceY)
    return false;

  short nISPosition = -1;
  KSGImageContent *pBitmap =
      (KSGImageContent *)m_ImageStore.GetExistedCreateBitmap(pszName, 0,
                                                             nISPosition);

  if (pBitmap) {
    if (pBitmap->nWidth >= nImageX + nWidth &&
        pBitmap->nHeight >= nImageY + nHeight) {
      int nPitch;
      void *pDevice = m_Canvas.LockCanvas(nPitch);
      if (pDevice) {
        unsigned short *pBuffer =
            &pBitmap->Data[pBitmap->nWidth * nImageY + nImageX];
...
bool KRepresentShell2::SaveScreenToFile(const char* pszName)
{
        ...
        int nPicWidth, nPicHeight, nDesktopWidth, nDesktopHeight, nPicOffX,
nPicOffY;
        {
                //全屏模式参数设定
                nPicOffX = 0;
        ...
        // 生成24位bmp文件
        if(!KBmpFile24::SaveBuffer24((char*)pszName, pTemp, nPicWidth*3,
...
bool KRepresentShell2::SaveScreenToFile(const char *pszName,
  ...
  if (eType == SCRFILETYPE_BMP)
    // 保存24位bmp文件
    bRet = KBmpFile24::SaveBuffer24((char *)pszName, pTemp, nPicWidth * 3,
                                    nPicWidth, nPicHeight);
  else
    // 保存24位jpg文件
    bRet = SaveBufferToJpgFile24((char *)pszName, pTemp, nPicWidth * 3,
