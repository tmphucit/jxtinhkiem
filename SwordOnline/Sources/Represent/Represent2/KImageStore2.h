...
public:
  ...
  unsigned int SetAdjustColorList(unsigned int *puColorList,
                                  unsigned int uCount);

  char *GetAdjustColorPalette(int nISPosition,  // 图形在iImageStore内的位置。
                              unsigned uColor); // 偏色量
...
private:
  ...
  // ##  返回值 < 0  -->
  // 表中没有给定的id，如果需要插入此id的话，插入位置应该为(-返回值-1)
  int FindImage(
      // ##Documentation
      // ## 图形的id
      unsigned int uImage,
  ...
  // 获取图形对象的某一帧的数据缓冲区
  void *GetSprFrame(const char *pszImageFile, _KISImageObj &ImgObject,
                    int nFrame);
