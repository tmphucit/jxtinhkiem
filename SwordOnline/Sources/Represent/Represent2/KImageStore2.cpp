      ...
      if (Img.pcAdjustColorPalettes) {
        for (unsigned int i = 0; i < m_uNumSprAdustColor; i++) {
          if ((uColor & 0xffffff) == m_uSprAdjustColorList[i]) {
            pPalette = Img.pcAdjustColorPalettes +
                       (2 * i * ((SPRHEAD *)Img.pObject)->Colors);
            break;
          }
        }
      }
...
char *KImageStore2::CreateAdjustColorPalette(const char *pOrigPalette,
                                             int nNumColor) {
  char *pPalette = NULL;
...
// 设置偏色列表
unsigned int KImageStore2::SetAdjustColorList(unsigned int *puColorList,
                                              unsigned int uCount) {
...
unsigned int KImageStore2::CreateImage(const char *pszName, int nWidth,
  ...
  if (nWidth > 0 && nHeight > 0 && uImage && nType == ISI_T_BITMAP16) {
    ...
    {
      pBitmap =
          (KSGImageContent *)malloc(KSG_IMAGE_CONTENT_SIZE(nWidth, nHeight));
    }
  }
...
void *KImageStore2::GetExistedCreateBitmap(const char *pszImage,
                                           unsigned int uImage,
...
LPDIRECTDRAWSURFACE
CreateDirectDrawSurfaceFromBitmap(KSGImageContent *pBitmap) {
...
bool KImageStore2::CreateBitmapSurface(const char *pszImage,
                                       unsigned int &uImage,
                                       short &nImagePosition) {
  if (uImage == 0)
    uImage = ImageNameToId(pszImage);
  ...
  if (nImagePosition >= 0) {
    ...
    if (Img.bType == ISI_T_BITMAP16 && Img.bNotCacheable == true &&
        Img.pObject) {
      if (Img.pSurface == NULL) {
        Img.pSurface =
            CreateDirectDrawSurfaceFromBitmap((KSGImageContent *)(Img.pObject));
      }
...
void *KImageStore2::GetImage(const char *pszImage, unsigned int &uImage,
                             short &nImagePosition, int nFrame, int nType,
                             void *&pFrameData) {
  pFrameData = NULL;
  if (uImage == 0) {
...
void *KImageStore2::GetSprFrame(const char *pszImageFile,
                                _KISImageObj &ImgObject, int nFrame) {
  ...
  if (nFrame >= 0 && nFrame < pSprHeader->Frames) {
    ...
    if (ImgObject.bSingleFrameLoad == false) { // 一次加载全部帧的图形
      ...
      pFrame->bRef = true;
      if ((pFrameData = pFrame->pFrameData) == NULL) { // 指定的帧数据还不存在
...
bool KImageStore2::GetImageParam(const char *pszImage, int nType,
  ...
  if (nImagePosition < 0) {
    void *pFrameData;
    if (GetImage(pszImage, uImage, nImagePosition, 0, nType, pFrameData) ==
        NULL)
      nImagePosition = -1;
  }
  if (nImagePosition >= 0) {
...
bool KImageStore2::GetImageFrameParam(const char *pszImage, int nType,
                                      int nFrame, KRPosition2 *pOffset,
                                      KRPosition2 *pSize) {
  bool bRet = false;
  short nPos = -1;
...
int KImageStore2::GetImagePixelAlpha(const char *pszImage, int nType,
                                     int nFrame, int nX, int nY) {
  int nRet = 0;
  short nPos = -1;
  unsigned int uImage = 0;
  SPRFRAME *pFrame;
  ...
  if (pImage) {
    if (nType == ISI_T_SPR) {
      ...
      if (nX >= 0 && nY >= 0 && nX < ((KSGImageContent *)pImage)->nWidth &&
...
bool KImageStore2::SaveImage(const char *pszFile, const char *pszImage,
                             int nFileType) {
  return 0;
}
...
void KImageStore2::CheckBalance() {
  ...
  for (i = 0; i < m_nNumImages; i++) {
    if (m_pObjectList[i].bNotCacheable == false) {
      ...
      if ((m_pObjectList[i].bSingleFrameLoad == true) &&
          (m_pObjectList[i].pObject) && (m_pObjectList[i].bType == ISI_T_SPR)) {
...
bool KImageStore2::ExpandSpace() {
  _KISImageObj *pNewList = (_KISImageObj *)realloc(
      m_pObjectList,
...
void KImageStore2::FreeImageObject(_KISImageObj &ImgObject,
                                   int nFrame /*=-1*/) {
...
void *KImageStore2::LoadImage(const char *pszImageFile, _KISImageObj &ImgObj,
                              int nFrame, void *&pFrameData) {
  void *pRet = NULL;
  ...
  if (ImgObj.bType == ISI_T_SPR) {
    ...
    if (pSprHeader) {
      ...
      if (ImgObj.pObject) {
        g_Pal24ToPal16((KPAL24 *)(&pSprHeader[1]), (KPAL16 *)(&pSprHeader[1]),
                       pSprHeader->Colors);
      } else {
...
KImageStore2::~KImageStore2() { Free(); }
