...
HRESULT CD3DFileObject::SetMeshGeometry(D3DVECTOR *pVertexData,
  ...
  // Allocate memory for the indices, you must call AddFace() to set the
  // vertices
    ...
    for (DWORD i = 0; i <= m_dwNumMaterials; i++) {
      ...
      if (bAlpha == m_bHasAlpha) {
        ...
        pd3dDevice->DrawIndexedPrimitive(D3DPT_TRIANGLELIST, D3DFVF_VERTEX,
                                         m_pVertices, m_dwNumVertices, pIndices,
                                         dwNumIndices, NULL);
      }
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CD3DFileObject::GetMeshGeometry(D3DVERTEX **ppVertices,
                                        DWORD *pdwNumVertices, WORD **ppIndices,
                                        DWORD *pdwNumIndices) {
  if (ppVertices)
    *ppVertices = m_pVertices;
  if (pdwNumVertices)
    *pdwNumVertices = m_dwNumVertices;
  if (ppIndices)
    *ppIndices = m_pIndices;
  if (pdwNumIndices)
...
HRESULT ParseMesh(LPDIRECTXFILEDATA pFileData, CD3DFileObject *pParentFrame) {
  ...
  DWORD dwNumVertices = *((DWORD *)pData);
  pData += 4;
  D3DVECTOR *pVertices = ((D3DVECTOR *)pData);
  pData += 12 * dwNumVertices;
  DWORD dwNumFaces = *((DWORD *)pData);
  pData += 4;
  ...
  while (SUCCEEDED(pFileData->GetNextObject(&pChildObj))) {
    ...
    if (SUCCEEDED(pChildObj->QueryInterface(IID_IDirectXFileData,
      ...
      if (TID_D3DRMMeshMaterialList == *pGUID) {
        DWORD dwNumMaterials = *((DWORD *)pData);
        pData += 4;
        DWORD dwNumMatFaces = *((DWORD *)pData);
        pData += 4;
...
//-----------------------------------------------------------------------------
BOOL CD3DFileObject::EnumObjects(BOOL (*fnCallback)(CD3DFileObject *,
                                                    D3DMATRIX *, VOID *),
...
// Desc: Enumerates all objects in the file.
//-----------------------------------------------------------------------------
VOID CD3DFile::EnumObjects(BOOL (*fnCallback)(CD3DFileObject *, D3DMATRIX *,
                                              VOID *),
                           D3DMATRIX *pmat, VOID *pContext) {
  if (m_pRoot) {
    D3DMATRIX mat;
...
BOOL ScaleMeshCB(CD3DFileObject *pFileObject, D3DMATRIX *, VOID *pContext) {
  ...
  if (SUCCEEDED(pFileObject->GetMeshGeometry(&pVertices, &dwNumVertices, NULL,
                                             NULL))) {
