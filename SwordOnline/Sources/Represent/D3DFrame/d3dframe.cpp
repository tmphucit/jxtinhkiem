...
HRESULT CD3DFramework7::Initialize(HWND hWnd, GUID *pDriverGUID,
  ...
  if (FAILED(hr =
                 CreateEnvironment(pDriverGUID, pDeviceGUID, pMode, dwFlags))) {
    DestroyObjects();
    return hr;
  }
...
HRESULT CD3DFramework7::CreateDirectDraw(GUID *pDriverGUID, DWORD dwFlags) {
  ...
  if (FAILED(DirectDrawCreateEx(pDriverGUID, (VOID **)&m_pDD, IID_IDirectDraw7,
                                NULL))) {
...
HRESULT CD3DFramework7::CreateFullscreenBuffers(DDSURFACEDESC2 *pddsd) {
  ...
  if (m_bIsStereo) {
    ...
    if (FAILED(hr = m_pddsBackBuffer->GetAttachedSurface(
                   &ddscaps, &m_pddsBackBufferLeft))) {
...
HRESULT CD3DFramework7::ShowFrame() {
  ...
  if (m_bIsFullscreen) {
    return m_pddsFrontBuffer->Blt(NULL, m_pddsBackBuffer, NULL, DDBLT_WAIT,
                                  NULL);
    ...
    return m_pddsFrontBuffer->Blt(&m_rcScreenRect, m_pddsBackBuffer, NULL,
                                  DDBLT_WAIT, NULL);
  }
}
