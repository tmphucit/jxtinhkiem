  ...
  for (DWORD device = 0; device < dwNumDevices; device++) {
    ...
    if (pDevice == pCurrentDevice) {
      ...
      for (DWORD mode = 0; mode < pDevice->dwNumModes; mode++) {
        ...
        TCHAR strMode[80];
        wsprintf(strMode, _T("%ld x %ld x %ld"), pddsdMode->dwWidth,
                 pddsdMode->dwHeight, pddsdMode->ddpfPixelFormat.dwRGBBitCount);
...
static BOOL CALLBACK ChangeDeviceProc(HWND hDlg, UINT uiMsg, WPARAM wParam,
  ...
  if (WM_INITDIALOG == uiMsg) {
    ...
    UpdateDialogControls(hDlg, pCurrentDevice, dwCurrentMode, bCurrentWindowed,
                         bCurrentStereo);
    ...
    // Keep the UI current
    UpdateDialogControls(hDlg, &pDeviceList[dwDevice], dwMode, bWindowed,
                         bStereo);
    return TRUE;
  }
...
HRESULT D3DEnum_UserChangeDevice(D3DEnum_DeviceInfo **ppDevice) {
  if (IDOK == DialogBoxParam((HINSTANCE)GetModuleHandle(NULL),
                             ...
                             GetForegroundWindow(), ChangeDeviceProc,
                             (LPARAM)ppDevice))
    return S_OK;
...
HRESULT D3DEnum_SelectDefaultDevice(D3DEnum_DeviceInfo **ppDevice,
  ...
  for (DWORD i = 0; i < dwNumDevices; i++) {
    if (bWindowed) {
      ...
      if (pDeviceList[i].ddsdFullscreenMode.dwWidth == 800 &&
          pDeviceList[i].ddsdFullscreenMode.dwHeight == 600 &&
          pDeviceList[i].ddsdFullscreenMode.dwDepth == 16) {
        if (pDeviceList[i].bHardware) {
          if ((*pDeviceList[i].pDeviceGUID) == IID_IDirect3DTnLHalDevice)
            pHardwareTnLDevice = &pDeviceList[i];
          else
