...
HRESULT CD3DApplication::Create(HINSTANCE hInst, TCHAR *strCmdLine) {
  ...
  if (FAILED(hr = D3DEnum_SelectDefaultDevice(&m_pDeviceInfo, true))) {
    DisplayFrameworkError(hr, MSGERR_APPMUSTEXIT);
    return hr;
  }
  ...
  m_hWnd = CreateWindow(_T("D3D Window"), m_strWindowTitle,
                        ...
                        CW_USEDEFAULT, 300, 300, 0L,
                        LoadMenu(hInst, MAKEINTRESOURCE(IDR_MENU)), hInst, 0L);
  UpdateWindow(m_hWnd);
...
HRESULT CD3DApplication::Initialize3DEnvironment() {
  ...
  if (SUCCEEDED(hr = m_pFramework->Initialize(
                    m_hWnd, m_pDeviceInfo->pDriverGUID,
...
VOID CD3DApplication::ShowStats() {
  ...
  TCHAR buffer[80];
  sprintf(buffer, _T("%7.02f fps (%dx%dx%d)"), fFPS, m_ddsdRenderTarget.dwWidth,
