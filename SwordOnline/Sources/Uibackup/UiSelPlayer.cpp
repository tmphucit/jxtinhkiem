...
void KUiSelPlayer::LoadScheme(const char *pScheme) {
  ...
  if (Ini.Load(Buff)) {
    ...
    Ini.GetString("SelRole", "PlayerImgPrefix", "", m_szPlayerImgPrefix,
...
void KUiSelPlayer::PaintWindow() {
  ...
  for (int i = 0; i < m_nNumPlayer; i++) {
    ...
    if (m_bJustClicked && nLoopBack && i == m_nSelPlayer) {
      char szFileName[128];
      GetRoleImageName(szFileName, m_szPlayerImgPrefix, m_Gender[m_nSelPlayer],
                       m_Attribute[m_nSelPlayer], 0);
...
void KUiSelPlayer::UpdateData() {
  ...
  if (m_nNumPlayer > 0) {
    ...
    for (i = 0; i < m_nNumPlayer; i++) {
      ...
      GetRoleImageName(szFileName, m_szPlayerImgPrefix, Info.Gender,
                       Info.Attribute, 2);
  ...
  if (pSetting) {
    pSetting->GetString("InfoString", LIFE_TIME_STRING_ID, "", szInfo,
                        sizeof(szInfo));
    pSetting->GetString("InfoString", LIFE_TIME_DAY_ID, "day(s)", szDay,
                        sizeof(szDay));
    pSetting->GetString("InfoString", LIFE_TIME_HOUR_ID, "hour(s)", szHour,
                        sizeof(szHour));
    pSetting->GetString("InfoString", LIFE_TIME_MINUTE_ID, "minute(s)",
                        szMinute, sizeof(szMinute));
    pSetting->GetString("InfoString", LIFE_TIME_SECOND_ID, "second(s)",
                        szSecond, sizeof(szSecond));
...
void KUiSelPlayer::GetRoleImageName(char *pszName, const char *pszPrefix,
                                    unsigned char bGender,
                                    unsigned char bAttribute, int nIndex) {
...
void KUiSelPlayer::OnDel() {
  if (m_nSelPlayer >= 0) {
    KUiConnectInfo::OpenWindow(CI_MI_TO_DEL_ROLE, CI_NS_SEL_ROLE_WND,
                               m_nSelPlayer);
    Hide();
  }
}
