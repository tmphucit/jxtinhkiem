...
int GetFormatedTimeString(int nTime, char *pString) {
  nTime += 60;
  return sprintf(
      pString, "%s%s Thêi gian ?s kh¾c", s_TimeName[(nTime / 120) % 12],
...
int GameWorld_DateTime::Init(KIniFile *pIniFile, const char *pSection) {
  if (KWndLabeledButton::Init(pIniFile, pSection)) {
    ...
    pIniFile->GetString(pSection, "SmoothMsg", "", m_szSmoothMsg,
                        sizeof(m_szSmoothMsg));
    pIniFile->GetString(pSection, "CrowdMsg", "", m_szCrowdMsg,
                        sizeof(m_szCrowdMsg));
...
int KImmediaItem::Init(KIniFile *pIniFile, const char *pSection) {
  ...
  if (nRet) {
    char ChName[64];
    pIniFile->GetString(pSection, "TextColor", "255,255,0", ChName,
                        sizeof(ChName));
...
void KUiPlayerBar::LoadScheme(const char *pScheme) {
  if (m_pSelf) {
    ...
    sprintf(Buff, "%s\\%s", pScheme,
            m_pSelf->m_bMiniMode ? SCHEME_INI_MINI : SCHEME_INI);
    if (Ini.Load(Buff))
...
void KUiPlayerBar::LoadScheme(KIniFile *pIni) {
  ...
  if (pIni->GetString("InputEdit", "FocusNoCanBKColor", "0,0,0", szBuffer,
                      sizeof(szBuffer))) {
...
int KUiPlayerBar::WndProc(unsigned int uMsg, unsigned int uParam, int nParam) {
  ...
  case WND_N_ITEM_PICKDROP:
    if (g_UiBase.IsOperationEnable(UIS_O_MOVE_ITEM))
      OnObjPickedDropped((ITEM_PICKDROP_PLACE *)uParam,
  ...
  case WND_N_EDIT_SPECIAL_KEY_DOWN:
    if (nParam == VK_RETURN) {
      ...
      if (m_nCurChannel >= 0) {
        int nToChannel;
        if (nParam == VK_PRIOR)
          nToChannel = (m_nCurChannel - 1 >= 0)
                           ? m_nCurChannel - 1
                           : KUiMsgCentrePad::GetChannelCount() +
                                 m_nRecentPlayerName - 1;
        else
          nToChannel =
              (DWORD(m_nCurChannel + 1) <
               (KUiMsgCentrePad::GetChannelCount() + m_nRecentPlayerName))
                  ? m_nCurChannel + 1
                  : 0;
  ...
  case WND_M_MENUITEMRIGHT_SELECTED:
    if (uParam == (unsigned int)(KWndWindow *)this) {
      if (HIWORD(nParam) == SEL_CHANNEL_MENU) {
        if (short(LOWORD(nParam)) >= 0) {
          if (KUiMsgCentrePad::CheckChannel(
                  LOWORD(nParam),
...
void KUiPlayerBar::PopupChannelMenu(int x, int y) {
  ...
  KPopupMenuData *pMenuData = (KPopupMenuData *)malloc(
      MENU_DATA_SIZE(nChannelDataCount + m_nRecentPlayerName));
  if (pMenuData == NULL) {
    return;
  }
  ...
  for (i = 0; i < nChannelDataCount; i++) {
    ...
    if (KUiMsgCentrePad::GetChannelMenuinfo(i, &nPicIndex, &nHei, &uColor,
                                            &uBKColor, szText,
                                            &nCheckPicIndex)) {
      ...
      *((WORD *)(pMenuData->Items[i].szData + 1)) = nPicIndex;
      ...
      if (nCheckPicIndex != -1) {
        pMenuData->Items[i].szData[pMenuData->Items[i].uDataLen] =
            KTC_INLINE_PIC;
        *((WORD *)(pMenuData->Items[i].szData + pMenuData->Items[i].uDataLen +
                   1)) = nCheckPicIndex;
  ...
  for (; i < nChannelDataCount + m_nRecentPlayerName; i++) {
    if (m_RecentPlayerName[i - nChannelDataCount][0] != 0) {
      ...
      KRColor uColor;
      KRColor uBKColor;
      if (KUiMsgCentrePad::GetPeopleMenuinfo(
              m_RecentPlayerName[i - nChannelDataCount], &nPicIndex, &nHei,
              &uColor, &uBKColor)) {
        pMenuData->Items[i].szData[0] = KTC_INLINE_PIC;
        *((WORD *)(pMenuData->Items[i].szData + 1)) = nPicIndex;
        pMenuData->Items[i].szData[3] = uColor.Color_b.a;
      ...
      pMenuData->Items[i].uDataLen =
          strlen(m_RecentPlayerName[i - nChannelDataCount]) + nOffset;
      pMenuData->Items[i].uBgColor = uBKColor.Color_dw;
    } else {
      break;
    }
...
BOOL KUiPlayerBar::IsCanSendMessage(const char *Buffer, int nLen,
                                    char *szDestName, int nChannelID) {
  if (m_pSelf) {
...
void KUiPlayerBar::OnSendChannelMessage(DWORD nChannelID, const char *Buffer,
                                        int nLen) // ·¢ËÍÆµµÀÁÄÌìµ½·þÎñÆ÷
{
  if (nChannelID != -1 && Buffer && nLen > 0) {
    ...
    if (nLeft < 0) {
      ...
      char szWarning[64];
      sprintf(szWarning, "Th«ng b¸o sÏ ®­îc göi sau %d gi©y!",
              (nLeft + 999) / 1000);
...
void KUiPlayerBar::OnDirectSendChannelMessage(DWORD nChannelID, BYTE cost,
                                              const char *Buffer,
                                              int nLen) // ·¢ËÍÆµµÀÁÄÌìµ½·þÎñÆ÷
{
...
//--------------------------------------------------------------------------
void KUiPlayerBar::OnObjPickedDropped(ITEM_PICKDROP_PLACE *pPickPos,
...
void KUiPlayerBar::SetCurrentChannel(int nIndex) {
  ...
  if (nIndex < nChannelDataCount) {
    ...
    WORD nPicIndex = -1;
    if (KUiMsgCentrePad::GetChannelMenuinfo(m_pSelf->m_nCurChannel,
                                            &nPicIndex)) {
      char buffer[3];
    ...
    WORD nPicIndex = -1;
    if (KUiMsgCentrePad::GetPeopleMenuinfo(
            m_pSelf->m_RecentPlayerName[m_pSelf->m_nCurChannel -
                                        nChannelDataCount],
            &nPicIndex)) {
      char buffer[3];
      buffer[0] = KTC_INLINE_PIC;
      *((WORD *)(buffer + 1)) = nPicIndex;
      ...
      m_pSelf->m_ChannelSwitchBtn.SetText(
          m_pSelf
              ->m_RecentPlayerName[m_pSelf->m_nCurChannel - nChannelDataCount]);
  }
}
...
//	¹¦ÄÜ£º±ä¸üÎïÆ·°Ú»»
//--------------------------------------------------------------------------
void KUiPlayerBar::UpdateItem(int nIndex, unsigned int uGenre,
                              unsigned int uId) {
...
//	¹¦ÄÜ£º±ä¸ü¿ì½Ý¼¼ÄÜ
//--------------------------------------------------------------------------
void KUiPlayerBar::UpdateSkill(int nIndex, unsigned int uGenre,
                               unsigned int uId) {
...
void KUiPlayerBar::Breathe() {
  ...
  while (KUiMsgCentrePad::GetChannelData(dwID, cost, Buffer, nLen)) {
    int nRet = IsHasCost(cost, nMoney, nLevel, nMana, nFullMana, nUseMoney,
                         nUseLevel, nUseMana);
    ...
    if (!bSend) {
      if (nRet == 2 && nUseLevel > 0)
        sprintf(szSystem,
                "§¼ng cÊp cña b¹n kh«ng ®ñ %d cÊp, kh«ng c¸ch nµo sö dông "
                "thiªn lý truyÒn ©m!",
                nUseLevel);

      if (nRet == 1 && nUseMoney > 0)
        sprintf(szSystem,
                "TiÒn cña b¹n kh«ng ®ñ %d l­îng, kh«ng c¸ch nµo sö dông thiªn "
                "lý truyÒn ©m!",
                nUseMoney);

      if (nRet == 3 && nUseMana > 0)
        sprintf(szSystem,
                "Néi lùc cña b¹n kh«ng ®ñ %d ®iÓm, kh«ng c¸ch nµo sö dông "
                "thiªn lý truyÒn ©m!",
                nUseMana);
    } else {
      if (nUseMoney > 0)
        sprintf(szSystem,
                "B¹n sö dông thiªn lý truyÒn ©m tæn hao ng©n l­îng %d l­îng!",
                nUseMoney);
      if (nUseMana > 0)
        sprintf(szSystem,
                "B¹n sö dông thiªn lý truyÒn ©m tæn hao néi lùc %d ®iÓm!",
                nUseMana);
    }
  ...
  if (m_nCurChannel >= 0 &&
      m_nCurChannel < (int)KUiMsgCentrePad::GetChannelCount() &&
      Wnd_GetFocusWnd() == (KWndWindow *)&m_InputEdit) {
    cost = KUiMsgCentrePad::GetChannelCost(
        KUiMsgCentrePad::GetChannelID(m_nCurChannel));
    if (IsHasCost(cost, nMoney, nLevel, nMana, nFullMana, nUseMoney, nUseLevel,
                  nUseMana) != 0)
...
int KUiPlayerBar::IsHasCost(BYTE cost, int nMoney, int nLevel, int nMana,
                            int nFullMana, int &nUseMoney, int &nUseLevel,
                            int &nUseMana) {
  if (cost == 0) // Ãâ·Ñ
  {
    nUseMoney = 0;
...
void KUiPlayerBar::InputNameMsg(char bChannel, const char *szName,
                                bool bFocus) {
  if (m_pSelf == NULL)
    return;
  ...
  if (Buffer[nName] == TEXT_CTRL_CHAT_PREFIX) {
    while (nName <= nLen) {
      if (Buffer[nName] == ' ' || Buffer[nName] == 0) {
        Buffer[nName] = 0;
        break;
      }
      nName++;
  ...
  if (bChannel) {
    ...
    if (nName > 0 && (KShortcutKeyCentre::FindCommand(Buffer + 1) >= 0 ||
                      KShortcutKeyCentre::ms_FunsMap.find(Buffer + 1) !=
                          KShortcutKeyCentre::ms_FunsMap
                              .end())) // µ±Ô­ÊäÈë´®ÖÐÓÐ"/"ÇÒÊÇº¯Êý¹Ø¼ü×ÖÊ±
    {                                  // Ö»×·¼Ó" name"
      m_pSelf->m_InputEdit.InsertString(" ", 1);
      m_pSelf->m_InputEdit.InsertString(szName, strlen(szName));
    } else {
...
char *KUiPlayerBar::GetRecentPlayerName(int nIndex) {
  ...
  if (m_pSelf && nIndex >= nChannelCount &&
      nIndex < m_pSelf->m_nRecentPlayerName + nChannelCount) {
...
void KUiPlayerBar::ReplaceSpecialName(char *szDest, size_t nDestSize,
                                      char *szSrc) {
...
void KUiPlayerBar::SetItemBtnInfo(int nBtnNo, ChatItem *pItem) {
  if (nBtnNo >= 0 && nBtnNo < MAX_ITEMBUTTON && g_pCoreShell && m_pSelf &&
    ...
    int nIdx = g_pCoreShell->GetGameData(
    ...
    if (nKind == 2)
      m_pSelf->m_ItemBtn[nBtnNo].SetColor(0xffcc33ff, 0xff000000, 0xff00ffff,
                                          0xff000000, 0xffff6699, 0xff000000);
    else if (nKind == 1)
      m_pSelf->m_ItemBtn[nBtnNo].SetColor(0xffffd94e, 0xff000000, 0xff00ffff,
                                          0xff000000, 0xffff6699, 0xff000000);
    else if (g_pCoreShell->IsBlueItem(nIdx) &&
             g_pCoreShell->GetGenre(nIdx) == 0)
      m_pSelf->m_ItemBtn[nBtnNo].SetColor(0xff6464ff, 0xff000000, 0xff00ffff,
                                          0xff000000, 0xffff6699, 0xff000000);
    else
      m_pSelf->m_ItemBtn[nBtnNo].SetColor(0xffffffff, 0xff000000, 0xff00ffff,
                                          0xff000000, 0xffff6699, 0xff000000);
  ...
  if (nBtnNo >= 0 && nBtnNo < MAX_ITEMBUTTON) {
    if (m_pSelf) {
      ...
      if (m_pSelf->m_ItemBtn[nBtnNo].m_X == -300 &&
          m_pSelf->m_ItemBtn[nBtnNo].m_Y == 100)
