...
KUiInit *KUiInit::OpenWindow(bool bStartMusic, bool bJustLaunched) {
  ...
  if (m_pSelf) {
    if (bJustLaunched) {
      ...
      KUiOptions::LoadSetting(true, false);
...
void KUiInit::PlayStartMovie() {
  ...
  if (Ini.Load(LAUNCH_GAME_INI)) {
    ...
    Ini.GetInteger("JustLaunched", "MoveEnable", 0, &bMoveEnable);

    if (szFile[0] && bMoveEnable == 1) {
