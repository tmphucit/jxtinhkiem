...
const CORBA::TypeCode_ptr _tc_ReqServer =
    CORBA::TypeCode::PR_interface_tc("IDL:ReqServer:1.0", "ReqServer");

static void _0RL_tcParser_setObjectPtr_ReqServer(tcObjrefDesc *_desc,
...
void _0RL_buildDesc_cReqServer(
    tcDescriptor &_desc,
    const _CORBA_ObjRef_tcDesc_arg<_objref_ReqServer, ReqServer_Helper> &_d) {
  ...
  if (sp == 0) {
    ...
    if (_a.PR_unpackTo(_tc_ReqServer, &tcd)) {
      if (!omniORB::omniORB_27_CompatibleAnyExtraction) {
        ((CORBA::Any *)&_a)
            ->PR_setCachedData((void *)(ReqServer_ptr)tmp,
                               _0RL_delete_ReqServer);
      }
      _s = tmp._retn();
      return 1;
    } else {
