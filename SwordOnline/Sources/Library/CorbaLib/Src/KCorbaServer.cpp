...
extern CORBA::Boolean BindObjectToName(CORBA::ORB_ptr orb,
                                       CORBA::Object_ptr objref, char *ObjName,
                                       char *ObjType);
BOOL KCorbaDataServer::ServerInit(int argc = 0, char **argv = NULL) {
  try {
...
BOOL KCorbaDataServer::StartServer(char *ServerName, char *ServerType) {
  try {
    ...
    printf("KCorbaDataServer[%s.%s] Have Started!.........\nsior is:%s\n",
...
BOOL KCorbaDataServer::ShutDownServer() { return TRUE; }