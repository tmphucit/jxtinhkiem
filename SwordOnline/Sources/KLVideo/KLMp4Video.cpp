...
BOOL KLMp4Video::HandleNotify() {
  ...
  dwEvent = WaitForMultipleObjects(2,         // How many possible events
...
// 返回:	void
//---------------------------------------------------------------------------
void KLMp4Video::Seek(int nPercent) { m_AviFile.VideoSeek(nPercent); }
//---------------------------------------------------------------------------
// 函数:	Rewind
// 功能:
// 参数:	void
// 返回:	void
//---------------------------------------------------------------------------
void KLMp4Video::Rewind() { m_AviFile.Rewind(); }
//---------------------------------------------------------------------------
// 函数:	NextFrame
// 功能:	下一帧
// 参数:	void
...
BOOL KLMp4Video::NextFrame() {
  ...
  if ((DWORD)m_AviFile.CurrentFrame() * 1000 <
      m_Timer.GetElapse() * m_nFrameRate) {
    bRender = FALSE;
    SetEvent(m_hEvent[0]);
  }
...
void KLMp4Video::VideoCopyToBuffer(void *buf, uint32 left, uint32 top,
                                   uint32 Pitch, uint32 destheight,
                                   uint32 Flags) {
...
void KLMp4Video::Play(BOOL bZoom) {
  ...
  NextFrame();
  if (m_uTimeID == 0) {
    m_uTimeID = timeSetEvent(m_nFrameTime, // delay time in ms
                             1,            // resolution
...
// 返回:	BOOL
//---------------------------------------------------------------------------
BOOL KLMp4Video::IsPlaying() { return (m_uTimeID != 0); }
