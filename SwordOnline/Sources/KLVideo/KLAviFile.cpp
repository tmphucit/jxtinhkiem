...
static unsigned long str2ulong(char *str) {
  ...
  result =
      (((unsigned long)str[0] & 0xFF) | (((unsigned long)str[1] & 0xFF) << 8) |
...
// also working on big endian machines
//---------------------------------------------------------------------------
static unsigned long str2ushort(char *str) { return (str[0] | (str[1] << 8)); }
  ...
  while (hdrl_ptr < hdrl_end) {
    ...
    if (memcmp(hdrl_ptr, "strh", 4) == 0) {
      ...
      if (lasttag == 1) {
        ...
        memcpy(&m_BitmapInfoHeader, hdrl_ptr, sizeof(BITMAPINFOHEADER));

        vids_strf_seen = 1;
      } else if (lasttag == 2) {
        // keep a copy of the WAVEFORMATEX
        memcpy(&m_WaveFormatEx, hdrl_ptr, sizeof(WAVEFORMATEX));
...
BOOL KLAviFile::GetAudioIndex() {
  ...
  m_audio_index =
      (audio_index_entry *)new char[nai * sizeof(audio_index_entry)];
...
BOOL KLAviFile::GetVideoIndex() {
  ...
  m_video_index =
      (video_index_entry *)new char[nvi * sizeof(video_index_entry)];
...
// 返回:
//---------------------------------------------------------------------------
int KLAviFile::VideoStreams() { return m_video_strn; }
//---------------------------------------------------------------------------
// 函数:	AudioStreams
...
// 返回:
//---------------------------------------------------------------------------
int KLAviFile::AudioStreams() { return m_audio_strn; }
//---------------------------------------------------------------------------
// 函数:	GetBitmapInfoHeader
// 功能:	Returns the bitmapinfoheader associated with the first video
// stream 参数: 返回:
//---------------------------------------------------------------------------
...
// 返回:
//---------------------------------------------------------------------------
int KLAviFile::CurrentFrame() { return m_video_pos; }
//---------------------------------------------------------------------------
// 函数:	TotalFrames
...
// 返回:
//---------------------------------------------------------------------------
int KLAviFile::TotalFrames() { return m_video_frames; }
//---------------------------------------------------------------------------
// 函数:	VideoSeek
