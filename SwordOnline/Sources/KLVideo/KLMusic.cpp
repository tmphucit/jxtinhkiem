...
// 返回:	TRUE－成功	FALSE－失败
//---------------------------------------------------------------------------
BOOL KLMusic::Open(LPSTR FileName) { return FALSE; }
//---------------------------------------------------------------------------
// 函数:	Init
// 功能:	初始化
// 参数:	void
...
BOOL KLMusic::InitSoundNotify() {
  ...
  // query interface
  hres = m_pSoundBuffer->QueryInterface(IID_IDirectSoundNotify,
                                        (void **)&m_pSoundNotify);
  if (hres != S_OK) {
    return FALSE;
  }
...
BOOL KLMusic::HandleNotify() {
  ...
  dwEvent = WaitForMultipleObjects(NUMEVENTS, // How many possible events
...
BOOL KLMusic::FillBufferWithSound(DWORD dwPos) {
  ...
  hres = m_pSoundBuffer->Lock(dwStartOfs,  // Offset of lock start
  ...
  if (dwBytesRead < dwBytes1) {
    memset((PBYTE)lpvPtr1 + dwBytesRead, 0, dwBytes1 - dwBytesRead);
  }
...
// 返回:	DWORD	实际读取的长度
//---------------------------------------------------------------------------
DWORD KLMusic::ReadWaveData(LPBYTE lpBuf, DWORD dwLen) { return 0; }
//---------------------------------------------------------------------------
// 函数:	Play
// 功能:	播放音乐
// 参数:	bLoop		是否循环播放
