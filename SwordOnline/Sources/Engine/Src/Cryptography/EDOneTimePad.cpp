...
//	功能：一次一密码乱本译码函数
//--------------------------------------------------------------------------
extern "C" ENGINE_API void EDOneTimePad_Encipher(char *pPlaintext,
                                                 int nPlainLen) {
  if (pPlaintext && nPlainLen > 0) {
    unsigned char *pContent = (unsigned char *)pPlaintext;
#define NUM_FIX_CIPHER_CHARA 3
    unsigned char cCipher[NUM_FIX_CIPHER_CHARA + NUM_FIX_CIPHER_CHARA] = {
        'W', 'o', 'o', 'y', 'u', 'e'};
    ...
    for (int nPos = 0; nPos < nPlainLen; nPos++) {
      pContent[nPos] =
          (pContent[nPos] - 0x20 + (0xff - cCipher[cCipher[0] % 3])) % 0xdf +
          0x20;
      cCipher[0] = cCipher[1];
      cCipher[1] = cCipher[2];
...
//	功能：一次一密码乱本解码函数
//--------------------------------------------------------------------------
extern "C" ENGINE_API void EDOneTimePad_Decipher(char *pCiphertext,
                                                 int nCiphertextLen) {
...
#define NUM_FIX_CIPHER_CHARA 3
    ...
    for (nPos = nCiphertextLen - 1; nPos >= NUM_FIX_CIPHER_CHARA; nPos--)
      pContent[nPos] =
          (pContent[nPos] - 0x20 +
           (pContent[nPos - 3 + (pContent[nPos - 3] % 3)] - 0x20)) %
              0xdf +
          0x20;
    ...
    for (; nPos >= 0; nPos--) {
      pContent[nPos] = (pContent[nPos] - 0x20 +
                        (cCipher[nPos + (cCipher[nPos] % 3)] - 0x20)) %
                           0xdf +
                       0x20;
    }
  }
