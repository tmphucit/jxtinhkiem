...
BOOL KLubCmpl_Blocker::ExchangeCurLines() {
  ...
  if (nCount == 2) {
    ...
    Buf =
        (char *)pNewReturnLine->m_pLineMem->Alloc(strlen(szNewReturnLabel) + d);
    ...
    char *Buff =
        (char *)pGotoReturnNode->m_pLineMem->Alloc(strlen(szGotoReturn) + d);
  ...
  while (1) {
    if (i != nCount - 2) {
      ...
      KStackNode *pEndNode =
          (KStackNode *)UnitsList.GetHead()->GetNext()->GetNext();

      if ((pElseIfNode->nKey != KEYELSEIF && pElseIfNode->nKey != KEYIF) ||
          pElseNode->nKey != KEYELSE || pEndNode->nKey != KEYEND)
        return FALSE;
      ...
      Buf = (char *)pNewReturnLine->m_pLineMem->Alloc(strlen(szNewReturnLabel) +
                                                      d);
      strcpy(Buf, szNewReturnLabel);
      ...
      sprintf(szNewLine,
              "%s Goto(\"AutoLabel%d\") else Goto(\"AutoLabel%d\") end;\n",
              (char *)pElseIfNode->m_pLine->m_pLineMem->GetMemPtr(),
              nLabelNum - 1, nLabelNum);
      ...
      char *Buff =
          (char *)pGotoReturnNode1->m_pLineMem->Alloc(strlen(szGotoReturn) + d);
      strcpy(Buff, szGotoReturn);

      Buff =
          (char *)pGotoReturnNode2->m_pLineMem->Alloc(strlen(szGotoReturn) + d);
      strcpy(Buff, szGotoReturn);
...
KLineNode *KLubCmpl_Blocker::Load(LPSTR FileName) {
  ...
  while (nCurPos < dwSize) {
    ...
    if (strstr(szLine, "function main()"))
      pFirstLine = pLineNode;

    if (strstr(szLine, "end;--main"))
      nState = 1;
...
void KLubCmpl_Blocker::PushKey(KStackNode *pStackNode) {
  g_DebugLog("Push (%d) %s", pStackNode->nKey,
             pStackNode->m_pLine->m_pLineMem->GetMemPtr());

  m_StackList.AddHead((KNode *)pStackNode);
}

KStackNode *KLubCmpl_Blocker::PopKey() {
  g_DebugLog("Pop  (%d) %s", ((KStackNode *)(m_StackList.GetHead()))->nKey,
             ((KStackNode *)(m_StackList.GetHead()))
                 ->m_pLine->m_pLineMem->GetMemPtr());
