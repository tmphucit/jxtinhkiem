...
struct IInlinePicEngineSink {
  ...
  virtual long AddCustomInlinePic(unsigned short &wIndex,
                                  const char *szSprPathName) = 0;
  // 动态卸载图片
...
extern "C" {
// 获取本行的下个显示字符
const char *TGetSecondVisibleCharacterThisLine(const char *pCharacter, int nPos,
                                               int nLen);
...
const char *TGetLimitLenString(const char *pOrigString, int nOrigLen,
                               char *pLimitLenString, int nLimitLen);
// 如果原(包含控制符)字符串长度（包括结尾符）超过限定的长度，则截短它并加上..后缀
const char *TGetLimitLenEncodedString(const char *pOrigString, int nOrigLen,
...
// 在编码字串寻找分割字符串的合适位置
int TSplitEncodedString(const char *pString, int nCount, int nDesirePos,
                        int bLess);
// 获得指定行的开始位置
...
// 获取编码文本的行数与最大行宽
int TGetEncodedTextLineCount(const char *pBuffer, int nCount, int nWrapCharaNum,
                             int &nMaxLineLen, int nFontSize, int nSkipLine = 0,
                             int nLineLimit = 0, int bPicSingleLine = false);
// 对已经编码的文本，从指定位置开始查找指定的控制符号的位置，返回-1表示未找到
int TFindSpecialCtrlInEncodedText(const char *pBuffer, int nCount,
                                  int nStartPos, char cControl);
// 对已经编码的文本，去除指定类型的控制符
int TClearSpecialCtrlInEncodedText(char *pBuffer, int nCount, char cControl);
// 对已经编码的文本，指定输出长度的在缓冲区中位置
int TGetEncodedTextOutputLenPos(const char *pBuffer, int nCount, int &nLen,
                                bool bLess, int nFontSize);
// 对已经编码的文本，指定的前段缓冲区中控制符，对后面的输出产生效果影响
int TGetEncodedTextEffectCtrls(const char *pBuffer, int nSkipCount,
                               KTP_CTRL &Ctrl0, KTP_CTRL &Ctrl1);

long AdviseEngine(IInlinePicEngineSink *);
long UnAdviseEngine(IInlinePicEngineSink *);

int TGetItemChatLineCount(const char *pBuffer, int nCount, int nWrapCharaNum,
                          int &nMaxLineLen, int nFontSize, int &nFace,
                          int &nLastPos, int &nTotalLen, int nSkipLine = 0,
                          int nLineLimit = 0,
                          int bPicSingleLine = false); // itemchat
}
