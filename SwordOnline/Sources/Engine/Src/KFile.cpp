...
// 函数:	Append
// 功能:
// 打开一个文件，准备在后部添加数据，如果此文件不存在，则产生这个文件。 参数:
// FileName	文件名 返回:	成功返回TRUE，失败返回FALSE。
//---------------------------------------------------------------------------
BOOL KFile::Append(LPSTR FileName) {
  ...
  /*	m_hFile = CreateFile(
                  ...
                  FILE_SHARE_READ,// share mode
                  NULL,			// pointer to security attributes
                  OPEN_ALWAYS,	// Opens the file, if it exists. If the file
     does not exist, the function creates the file as if dwCreationDisposition
     were CREATE_NEW FILE_ATTRIBUTE_NORMAL, // file attributes NULL);
     // template file*/
  // if (m_hFile == INVALID_HANDLE_VALUE)
  //	return FALSE;
