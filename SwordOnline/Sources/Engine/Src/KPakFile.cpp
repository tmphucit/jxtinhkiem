...
// 返回:	void
//---------------------------------------------------------------------------
void g_SetPakFileMode(int nFileMode) { m_nPakFileMode = nFileMode; }

#define PAK_INDEX_STORE_IN_RESERVED 0

#ifndef _SERVER

//----modify by Wooy to add Adjust color palette and to get rid of #@$%^&
//----2003.8.19
SPRHEAD *SprGetHeader(const char *pszFileName, SPROFFS *&pOffsetTable) {
  ...
  if (File.IsFileInPak()) {
    ...
    if (g_pPakList->FindElemFile(pszFileName, PakRef)) {
      ...
      if (pSpr)
        pSpr->Reserved[PAK_INDEX_STORE_IN_RESERVED] =
            (WORD)(short)PakRef.nPackIndex;
    }
  } else {
    bool bOk = false;
    SPRHEAD Header;
    ...
    while (File.Read(&Header, sizeof(SPRHEAD)) == sizeof(SPRHEAD)) {
      ...
      if (File.Read(&pSpr[1], uEntireSize) == uEntireSize) {
        //----获得图形帧索引表的开始位置---
        pOffsetTable =
            (SPROFFS *)(((char *)(pSpr)) + sizeof(SPRHEAD) + Header.Colors * 3);
...
KSGImageContent *get_jpg_image(const char cszName[], unsigned uRGBMask16) {
  ...
  pImageResult = (KSGImageContent *)malloc(
      KSG_IMAGE_CONTENT_SIZE(JpegInfo.width, JpegInfo.height));
  if (!pImageResult)
    goto Exit0;
        ...
        if (dwBlockPos)
        {
                ...
                if (dwBlockPos + dwSize <= BLOCK_SIZE)
                {
                        ...
                        if ((m_dwFilePtr & 0xffff) == 0)
                                m_dwDataPtr += (m_pBlocks[nBlock] == 0)?
BLOCK_SIZE : m_pBlocks[nBlock];

                        return dwSize;
                }

                // 要读的数据长度大于64K
                g_MemCopyMmx(pOutBuf, m_pBuffer + dwBlockPos, BLOCK_SIZE -
dwBlockPos); pOutBuf += BLOCK_SIZE - dwBlockPos; m_dwDataPtr +=
(m_pBlocks[nBlock] == 0)? BLOCK_SIZE : m_pBlocks[nBlock]; m_dwFilePtr =
(++nBlock) << 16; dwSize -= (BLOCK_SIZE - dwBlockPos);
        }
        ...
        while (dwSize > 0xffff) // 大于64K
        {
                ...
                pOutBuf += BLOCK_SIZE;
                m_dwDataPtr += (m_pBlocks[nBlock] == 0)? BLOCK_SIZE :
m_pBlocks[nBlock]; m_dwFilePtr = (++nBlock) << 16; dwSize -= BLOCK_SIZE;
        }
