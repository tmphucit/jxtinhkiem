...
KBinTree::KBinTree() { m_pTreeRoot = NULL; }
...
//---------------------------------------------------------------------------
TBinTreeNode *KBinTree::Search(TBinTreeNode *pParentTBinTreeNode,
                               TBinTreeNode *pTBinTreeNode,
                               TBinTreeNode *pKeyNode, BOOL *pResult) {
...
//---------------------------------------------------------------------------
TBinTreeNode *KBinTree::AddNode(TBinTreeNode *pNewTBinTreeNode,
                                TBinTreeNode *pTBinTreeNode) {
...
// 返回:	TBinTreeNode *
//---------------------------------------------------------------------------
TBinTreeNode *KBinTree::RemoveNode(TBinTreeNode *pTBinTreeNode,
                                   TBinTreeNode **ppRootTBinTreeNode) {
  TBinTreeNode *pFindTBinTreeNode;
  if (pTBinTreeNode == NULL)
    return NULL;

  if (pTBinTreeNode->pLeftChild == NULL &&
      pTBinTreeNode->pRightChild == NULL) // 该结点没有左右子
  {
  ...
  else if (!(pTBinTreeNode->pLeftChild &&
             pTBinTreeNode->pRightChild)) // 只有单子时
  {
      ...
      if (pFindTBinTreeNode) {
        ...
        if (pFindTBinTreeNode->pLeftChild) {
          ...
          if (pFindTBinTreeNode != pTBinTreeNode->pLeftChild) {
            pFindTBinTreeNode->pParent->pRightChild =
                pFindTBinTreeNode->pLeftChild;
...
// 返回:	DWORD
//---------------------------------------------------------------------------
DWORD KBinTree::InOrder(TBinTreeNode *pTBinTreeNode) // 中序遍历
{
  static int Count = 0;
  if (pTBinTreeNode != NULL) {
...
BOOL KBinTree::RemoveThisNode(TBinTreeNode *pNode) {
  ...
  if (pNode->pLeftChild == NULL && pNode->pRightChild == NULL &&
      pNode->pParent == NULL && pNode != m_pTreeRoot)
    return FALSE;
