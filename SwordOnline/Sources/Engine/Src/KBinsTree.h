...
// 复制构造函数
template <class T> BinSTree<T>::BinSTree(const BinSTree<T> &tree) {
...
// 析构函数
template <class T> BinSTree<T>::~BinSTree(void) { ClearList(); }

// 删除树中的所有结点
template <class T> void BinSTree<T>::ClearList(void) {
  DeleteTree(root);
  root = current = NULL;
  size = 0;
}
...
// 赋值运算符
template <class T> BinSTree<T> &BinSTree<T>::operator=(const BinSTree<T> &rhs) {
  // 不能将树复制到自身
  if (this == &rhs)
    return *this;
...
template <class T> bool BinSTree<T>::Find(T &item) {
...
// 指示树是否为空
template <class T> bool BinSTree<T>::ListEmpty(void) const {
  return (size == 0);
}

// 返回树中的数据项个数
template <class T> int BinSTree<T>::ListSize(void) const { return size; }

// 往查找树中插入数据项，若元素重复，则更新现有元素
template <class T> void BinSTree<T>::Insert(const T &item) {
...
// 如果 item 在树中，将其删除
template <class T> void BinSTree<T>::Delete(const T &item) {
...
// 插入到树中
template <class T> void BinSTree<T>::Update(const T &item) {
