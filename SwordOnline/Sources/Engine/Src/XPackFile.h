...
private:
  // 直接读取包文件数据中的数据到缓冲区
  bool DirectRead(void *pBuffer, unsigned int uOffset,
                  unsigned int uSize) const;
  // 带解压地读取包文件到缓冲区
  bool ExtractRead(void *pBuffer, unsigned int uExtractSize, long lCompressType,
                   unsigned int uOffset, unsigned int uSize) const;
  // 在索引表中查找子文件项
