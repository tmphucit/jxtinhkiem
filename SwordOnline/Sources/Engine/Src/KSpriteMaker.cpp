...
// 返回:	void
//---------------------------------------------------------------------------
KSpriteMaker::~KSpriteMaker() { Free(); }
//---------------------------------------------------------------------------
// 函数:	Load
// 功能:	载入SPR文件
// 参数:	FileName	文件名
...
BOOL KSpriteMaker::Load(LPSTR FileName) {
  ...
  for (int i = 0; i < m_nFrames; i++) {
    pNode =
        (KSprNode *)g_MemManager.Calloc(sizeof(KSprNode) + pOffset[i].Length);
...
void KSpriteMaker::Draw(int nX, int nY, int nFrame) {
  ...
  g_pCanvas->DrawSprite(nX + pFrame->OffsetX, nY + pFrame->OffsetY,
                        pFrame->Width, pFrame->Height, pFrame->Sprite, m_Pal16);
}
//---------------------------------------------------------------------------
// 函数:	AddFrame
// 功能:	追加图象帧
...
BOOL KSpriteMaker::AddFrame(KBitmap *pBitmap, BOOL bTrans) {
  ...
  // copy frame data
  KSprNode *pNode =
      (KSprNode *)g_MemManager.Calloc(sizeof(KSprNode) + Bmp2Spr.GetLength());
