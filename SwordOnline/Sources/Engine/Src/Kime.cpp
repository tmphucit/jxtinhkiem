...
// 停止对消息的响应，暂停工作
//----------------------------------------------------------------------------
void KIme::TurnOff() { m_bIsWorking = FALSE; }

//----------------------------------------------------------------------------
// 恢复对消息的响应，开始工作
//----------------------------------------------------------------------------
void KIme::TurnOn() { m_bIsWorking = TRUE; }
...
// 禁止语言切换
//----------------------------------------------------------------------------
void KIme::DisableLanguageChange() { m_bNoLanguageChange = TRUE; }
...
int KIme::WndMsg(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
  if (m_bIsWorking) {
    if (uMsg == WM_INPUTLANGCHANGEREQUEST) {
      ...
      if ((wParam == IMN_SETOPENSTATUS) && m_bNoLanguageChange) {
        CloseIME();
        return true;
        ...
        if (m_bNoLanguageChange) {
          ...
          if (hImc) {
            ...
            //)
            //{
            //     ImmSetConversionStatus(hImc, IME_CMODE_CHARCODE,
            //     IME_SMODE_NONE);
            // }
