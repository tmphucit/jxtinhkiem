...
char *KNetClient::FindStringInMem(char *pMem, DWORD dwSize, char *pString) {
  ...
  for (i = 0; i < ((int)dwSize - len); i++) {
    ...
    if (g_MemComp(pTemp, pString, len) == TRUE)
      return pTemp;
    ...
    default:
      if (Num <= RecvData.DataNum) {
        if (SysCommand.dwCurrentOffset < 63 * 1024) {
          memcpy(SysCommand.Data + SysCommand.dwCurrentOffset, RecvData.Data,
                 dwSysSize);
          SysCommand.dwCurrentOffset += dwSysSize;
          SysCommand.dwNum += Num;
        }
        if (CoreCommand.dwCurrentOffset < 63 * 1024) {
          memcpy(CoreCommand.Data + CoreCommand.dwCurrentOffset,
                 RecvData.Data + dwSysSize, RecvData.dwSize - dwSysSize);
          CoreCommand.dwCurrentOffset += RecvData.dwSize - dwSysSize;
          CoreCommand.dwNum += RecvData.DataNum - Num;
        }
      }
      g_DebugLog("Sorting Protocol...:%d,%d", RecvData.DataNum,
                 CoreCommand.dwNum);
      return;
      break;
    }
...
BOOL KNetClient::DataProcess(void) {
  ...
  pTemp = (RECV_DATA *)FindStringInMem((char *)RecvBuffer.Data,
                                       RecvBuffer.dwSize, "PACK");
  if (pTemp == NULL)
    return FALSE;
  ...
  while (TRUE) {
    pTemp = (RECV_DATA *)FindStringInMem((char *)RecvBuffer.Data + offset,
                                         RecvBuffer.dwSize - offset, "PACK");
    if (pTemp == NULL) {
      if (infoflag)
        return TRUE;
      else
...
// 返回:	char*
//---------------------------------------------------------------------------
char *KNetClient::GetHostName(void) { return m_ServerHostName; }
//---------------------------------------------------------------------------
// 函数:	GetPortName()
// 功能:
// 参数:	void
// 返回:	char*
//---------------------------------------------------------------------------
char *KNetClient::GetPortName(void) { return m_ServerPortName; }
//--------------------------------------------------------------------------------
