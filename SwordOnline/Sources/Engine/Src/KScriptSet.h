...
// 二叉树结构标准函数
extern TScript *BTSearch(TScript *pParentTScript, TScript *pTScript,
                         char *szKey, BOOL *pResult);
...
extern TScript *BTSearch(TScript *pParentTScript, TScript *pTScript, DWORD nKey,
                         BOOL *pResult);
extern DWORD BTPreorder(TScript *pTScript); // 中序遍历排序二叉树
extern TScript *BTInsert(TScript *pTScript, DWORD nKey);
extern TScript *BTDelete(TScript *pTScript, TScript **ppRootTScript,
                         int nKeyStyle);
...
public:
  ...
  KScript *GetScript(char *szKey, BOOL nNeedCreate,
                     BOOL nNeedUpdateRecent); // 根据关键字获得脚本
  KScript *GetScript(DWORD nKey, BOOL nNeedCreate, BOOL nNeedUpdateRecent);
  DWORD ListScriptsKey();
  DWORD ListRecent(int order);

  TScript *
  SearchScript(char *szKey,
               BOOL *pnResult); // 查找关键字的结点，若存在则*pnResult =
                                // 1，返回该结点；否则*pnResult = 0,返回接近点。
  TScript *
  SearchScript(DWORD nKey,
               BOOL *pnResult); // 查找关键字的结点，若存在则*pnResult =
                                // 1，返回该结点；否则*pnResult = 0,返回接近点。
  ...
  int m_nKeyStyle; // 关键字的类型 有两种 数字与字符串，值分别为 KEY_NUMER
                   // KEY_STRING
};
//---------------------------------------------------------------------------
#endif
