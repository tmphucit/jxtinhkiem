...
// 返回:	void
//---------------------------------------------------------------------------
KNetThread::~KNetThread(void) { Exit(); }
...
void KNetThread::Init(int(THREADPROC *thread_func)(KNetThread *),
                      long stack_size, void (*_wakeup_func)(KNetThread *),
                      KSafeList *_ext_msglist, void *_user_data) {
  KASSERT(thread_func);
  if (!stack_size)
...
//-------------------------------------------------------------------
void KNetThread::ThreadStart(void) { m_StartupEvent.Signal(); }
...
//  27-Apr-99   floh    + Support fuer __NEBULA_NO_THREADS__
//-------------------------------------------------------------------
bool KNetThread::ThreadStopRequested(void) { return m_bStopThread; }

//-------------------------------------------------------------------
//  KNetThread::ThreadSleep()
//  20-Oct-98   floh    created
//-------------------------------------------------------------------
void KNetThread::ThreadSleep(long ms) { Sleep(ms); }
...
//  20-Oct-98   floh    created
//-------------------------------------------------------------------
void KNetThread::ReplyMsg(KMsgNode *nd) { delete nd; }

//-------------------------------------------------------------------
//  KNetThread::WaitMsg()
//  20-Oct-98   floh    created
//-------------------------------------------------------------------
void KNetThread::WaitMsg(void) { m_pMsgList->WaitEvent(); }
...
//  20-Oct-98   floh    created
//-------------------------------------------------------------------
void KNetThread::UnlockUserData(void) { m_Mutex.Unlock(); }
//---------------------------------------------------------------------------
