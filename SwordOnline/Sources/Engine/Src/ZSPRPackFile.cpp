...
char *ZCache::getNode(unsigned long index_high, unsigned long index_low,
                      long size) { // 得到一个缓冲区数据块
  ...
  while (free_items->size < size) { // 删除后面的项目直到有足够的空间为止
    ...
    if ((char *)next - buffer >= cache_size) { // 已经到了结束的位置，回第一个
      ...
      /*			while(next->index_high && (next->index_low &
         0xFFFF0000ul)) { next = (item_info *)((char *)next + next->size);
                                      if((char *)next - buffer >= cache_size) {
                                              LeaveCriticalSection(&mutex);
                                              return 0;
         //有数据并且引用计数不为0(或者为等待状态)
                                      }
                              }*/
      /*			if(next->index_high && (next->index_low &
         0xFFFF0000ul)) {			//还有数据并且正在使用
                                      LeaveCriticalSection(&mutex);
                                      return 0;
         //有数据并且引用计数不为0(或者为等待状态)
                              }
...
void ZCache::releaseNode(char *node) {
  /*
          ...
          current->index_low = (current->index_low & 0x0000FFFF) | (ref_count <<
     16); LeaveCriticalSection(&mutex);
  */
}
...
int ZFile::read(char *buffer, unsigned long offset, int size) {
  ...
  if (ReadFile(m_hFile, buffer, size, &read_size, NULL))
    return read_size;
  return read_size;
}
...
bool ZPackFile::_readData(int node_index, char *node) {
  char *source = read(index_list[node_index].offset,
                      (index_list[node_index].compress_size & 0x00FFFFFF));
  if (!source)
    return false;
  ...
  if ((compress_type == TYPE_UCL) | (compress_type == TYPE_BZIP2)) { // 整体压缩
    if (compress_type == TYPE_UCL) {
      r = ucl_nrv2b_decompress_8(
          (BYTE *)source, index_list[node_index].compress_size & 0x00FFFFFF,
          (BYTE *)node, &dest_length, NULL);
...
ZPackFile::ZPackFile(const char *name, ZCache *cache) : ZFile(name, cache) {
  index_list = 0;
  if (!read((char *)&header, 0, sizeof(header)))
    return;
  index_list = new index_info[header.count];
  read((char *)&index_list[0], header.index_offset,
       header.count * sizeof(index_info));
}
...
void ZPackFile::releaseData(char *data) { m_Cache->releaseNode(data); }
...
SPRHEAD *ZSPRPackFile::SprGetHeader(unsigned long uNameId,
  ...
  if (nNodeIndex >= 0) {
    ...
    if ((index_list[nNodeIndex].compress_size & (TYPE_FRAME << 24)) == 0) {
      ...
      if (pSpr) {
        if (_readData(nNodeIndex, (char *)pSpr)) {
          if ((*(int *)&pSpr->Comment[0]) == SPR_COMMENT_FLAG) {
            pOffsetTable = (SPROFFS *)(((char *)pSpr) + sizeof(SPRHEAD) +
                                       pSpr->Colors * 3);
            bOk = true;
          }
        }
      ...
      SPRHEAD Header;
      if (read((char *)&Header, index_list[nNodeIndex].offset,
               sizeof(SPRHEAD)) == sizeof(SPRHEAD)) {
        if (*(int *)&(Header.Comment[0]) == SPR_COMMENT_FLAG) {
          ...
          if (pSpr) {
            if (read((char *)(&pSpr[1]),
                     index_list[nNodeIndex].offset + sizeof(SPRHEAD),
  ...
  if (pSprHeader && nFrame >= 0 && nFrame < pSprHeader->Frames) {
    ...
    if (nNodeIndex >= 0 && nNodeIndex < (int)header.count) {
      ...
      if ((uCompressType & TYPE_FRAME) != 0) {
        ...
        // 读出指定帧的信息
        frame_info *pFrameList =
            (frame_info *)((char *)pSprHeader + lTempValue);
        unsigned long uSrcOffset = index_list[nNodeIndex].offset + lTempValue +
                                   pSprHeader->Frames * sizeof(frame_info);
        while (nFrame > 0) {
