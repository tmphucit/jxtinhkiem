...
/*typedef struct
{
        ...
        int mode;			// 0=stereo 1=joint stereo 2=dual
channel 3=single channel int mode_ext;		// 0=MPG_MD_LR_LR 1=MPG_MD_LR_I
2=MPG_MD_MS_LR 3=MPG_MD_MS_I int copyright;		// 0=no 1=yes int
original;		// 0=no 1=yes int emphasis;		// 0=none
1=50/15 microsecs 2=reserved 3=CCITT J 17
}
MPEG_HEAD;
//---------------------------------------------------------------------------
typedef struct
...
typedef long (*mp3_decode_init)(MPEG_HEAD *head,
                                                                int
framebytes_arg, int reduction_code, int transform_code, int convert_code, int
freq_limit); typedef void (*mp3_decode_info)(DEC_INFO *info); typedef void
(*mp3_decode_data)(unsigned char *mp3, int *mp3_size, unsigned char *pcm, int
*pcm_size);
//---------------------------------------------------------------------------
...
BOOL LoadMp3Lib()
{
        ...
        Mp3DecodeHead = (mp3_decode_head)GetProcAddress(m_hDll,
"mp3_decode_head"); Mp3DecodeInit = (mp3_decode_init)GetProcAddress(m_hDll,
"mp3_decode_init"); Mp3DecodeInfo = (mp3_decode_info)GetProcAddress(m_hDll,
"mp3_decode_info"); Mp3DecodeData = (mp3_decode_data)GetProcAddress(m_hDll,
"mp3_decode_data"); if
((!Mp3DecodeHead)||(!Mp3DecodeInit)||(!Mp3DecodeInfo)||(!Mp3DecodeData))
        {
...
BOOL KMp3Music::Mp3Init() {
  ...
  */
  ...
  // init mp3 decoder
  if (!mp3_decode_init(&mpeg_head, mp3_frmbytes, reduction_code, transform_code,
  ...
  m_WaveFormat.nAvgBytesPerSec =
      (dec_info.channels * dec_info.samprate * dec_info.bits + 7) / 8;
...
BOOL KMp3Music::Mp3FillBuffer() {
  ...
  if (mp3_bufbytes < mp3_trigger) {
    ...
    dwRead = Mp3FileRead(mp3_buffer + mp3_bufbytes, MP3_BUFSIZE - mp3_bufbytes);
    mp3_bufbytes += dwRead;
    mp3_bufptr = mp3_buffer;
  }
