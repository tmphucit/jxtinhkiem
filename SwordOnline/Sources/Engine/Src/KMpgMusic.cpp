...
typedef long (*mp3_decode_data)(struct mpstr *mp, unsigned char *in,
                                unsigned char *out);
//---------------------------------------------------------------------------
...
void KMpgMusic::InitSoundFormat() {
  ...
  m_dwBufferSize =
      (m_dwBufferSize + MP3_FRM_SIZE - 1) / MP3_FRM_SIZE * MP3_FRM_SIZE;
...
// 返回:	BOOL
//---------------------------------------------------------------------------
BOOL KMpgMusic::Mp3FileOpen(LPSTR FileName) { return m_Mp3File.Open(FileName); }
//---------------------------------------------------------------------------
// 函数:	Mp3FileRead
// 功能:
// 参数:	pBuf
...
BOOL KMpgMusic::Mp3FillBuffer() {
  ...
  if (mp3_bufbytes < 2500) {
    ...
    readbytes =
        Mp3FileRead(mp3_buffer + mp3_bufbytes, MP3_BUF_SIZE - mp3_bufbytes);
