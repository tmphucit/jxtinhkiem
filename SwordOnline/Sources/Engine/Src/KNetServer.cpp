...
BOOL KNetServer::Create(char *port_name) {
  ...
  for (int i = 0; i < MAX_CLIENT_NUMBER; i++) {
    memset((char *)Client_data_array + i * sizeof(CLIENT_DATA), 0,
           sizeof(CLIENT_DATA));
...
void KNetServer::CheckConnect() {
  ...
  if (s > 0) {
    ...
    // 增加该连结
    Client_data_array[i].Socket =
        accept(m_Socket, (struct sockaddr *)&sockaddr, &addrlen);
    if (Client_data_array[i].Socket > 0) {
      Client_data_array[i].LogStatus = LogStatus_Connected;
      memcpy(&Client_data_array[i].m_ClientAddr, &sockaddr,
             sizeof(sockaddr_in));
      n_ClientNum++;
    }
  }
...
void KNetServer::Encipher(int nClientIndex) {
  ...
  if (nClientIndex < 0 || nClientIndex >= MAX_CLIENT_NUMBER)
    return;

  if (!pTemp->CipherVerify)
    return;
  for (i = 0; i < (int)Num; i++) {
    *pTemp1 ^= *(pTemp2 + index);
    index = (index + 1) % 16;
...
int KNetServer::RecieveData(int nClientIndex, BYTE *pBuffer, DWORD dwSize) {
  ...
  if ((*pBuffer) < 32 && pTemp->DataNum) // 系统协议放在最前面
  {
    ...
    memcpy(pTemp->Data + Client_data_array[nClientIndex].CurOffset, pBuffer,
           dwSize);
...
int KNetServer::SendMessage(int nClientIndex) {
  ...
  if (pTemp->dwSize > MIN_COMPRESS_SIZE) {
    ...
    if (((KCodecLha *)pCodec)->Encode(&tCodeInfo) == TRUE &&
        tCodeInfo.dwPackLen < pTemp->dwSize) {
...
int KNetServer::RecieveMessage(int nClientIndex, DWORD dwSize) {
  ...
  len = recv(socket, (char *)Client_data_array[nClientIndex].RecieveBuffer,
             dwSize, 0);
...
void KNetServer::CopyDataToTemp() {
  ...
  for (i = 0; i < MAX_CLIENT_NUMBER; i++) {
    ...
    if (!pTemp->Socket)
      continue;
    if (!pTemp->SendBuffer.DataNum)
      continue;
    if (pTemp->SendTemp.DataNum)
      continue;
...
void KNetServer::RecvDataToTemp() {
  ...
  for (i = 0; i < MAX_CLIENT_NUMBER; i++) {
    ...
    if ((pTemp->dwTempSize + nLeftSize) >= 8 * RECIEVE_BUFFER_SIZE)
      continue;
    Mutex[i].Lock();
    memcpy(pTemp->RecieveTemp + pTemp->dwTempSize, pTemp->RecieveBuffer,
           nLeftSize);
