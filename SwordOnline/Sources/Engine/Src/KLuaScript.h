...
public:
  ...
  void SafeCallBegin(
      int *pIndex); // 记录当前堆栈栈顶位置，并保存到pIndex指向的数据中
  ...
  inline void SetTableMember(int nIndex, const char *szMemberName,
                             Lua_CFunction CFun) {
...
private:
  BOOL CallFunction(LPSTR cFuncName, int nResults, LPSTR cFormat,
                    va_list vlist);
};
//---------------------------------------------------------------------------
#endif // KLuaScript_H
