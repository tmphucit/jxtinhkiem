...
BOOL KDirectDraw::SetDisplayMode() {
  ...
  // set display mode
  hres = m_lpDirectDraw->SetDisplayMode(m_dwScreenWidth,  // Screen Width
...
void KDirectDraw::SetWindowStyle() {
  ...
  if (GetScreenMode() == FULLSCREEN) {
    ...
    SetWindowPos(hWnd, HWND_NOTOPMOST, 0, 0, rc.right - rc.left,
                 rc.bottom - rc.top, SWP_NOACTIVATE);
  }
}
//---------------------------------------------------------------------------

void KDirectDraw::BltToFrontBuffer(LPDIRECTDRAWSURFACE pSurface,
                                   RECT *pDestRect, RECT *pSrcRect) {
  if (m_lpDDSPrimary && pSurface) {
    m_lpDDSPrimary->Blt(pDestRect, pSurface, pSrcRect, DDBLT_WAIT, NULL);
  }
}

void KDirectDraw::BltToBackBuffer(LPDIRECTDRAWSURFACE pSurface, RECT *pDestRect,
                                  RECT *pSrcRect) {
