...
KTabFileCtrl::~KTabFileCtrl() { Clear(); }
...
BOOL KTabFileCtrl::LoadPack(LPSTR FileName) { return TRUE; }
...
// 以0,0为起点
BOOL KTabFileCtrl::GetValue(int nRow, int nColumn, LPSTR &lpRString,
                            DWORD dwSize) {
  if (nRow < 0 || nColumn < 0)
    return FALSE;
  TTabLineNode *pLineNode = (TTabLineNode *)m_RowList.GetHead();
...
BOOL KTabFileCtrl::SetValue(int nRow, int nColumn, LPSTR lpString, DWORD dwSize,
                            BOOL bCanCreateNew) {
  if (nRow < 0 || nColumn < 0)
    return FALSE;
  TTabLineNode *pLineNode = (TTabLineNode *)m_RowList.GetHead();
...
BOOL KTabFileCtrl::GetString(int nRow, int nColumn, LPSTR lpDefault,
...
BOOL KTabFileCtrl::GetInteger(int nRow, int nColumn, int nDefault,
                              int *pnValue) {
  char *pData = NULL;
...
BOOL KTabFileCtrl::GetFloat(int nRow, int nColumn, float fDefault,
                            float *pfValue) {
  char *pData = NULL;
  if (!GetValue(nRow - 1, nColumn - 1, pData, 100)) {
    *pfValue = fDefault;
...
BOOL KTabFileCtrl::WriteString(int nRow, int nColumn, LPSTR lpString,
                               DWORD dwSize) {
  if (dwSize <= 0)
