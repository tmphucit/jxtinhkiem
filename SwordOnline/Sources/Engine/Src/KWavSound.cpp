...
// 返回:	void
//---------------------------------------------------------------------------
KWavSound::KWavSound() { g_MemZero(m_Buffer, sizeof(m_Buffer)); }
//---------------------------------------------------------------------------
// 函数:	~KWavSound
// 功能:	析造函数
// 参数:	void
...
BOOL KWavSound::Load(LPSTR FileName) {
  ...
  desc.dwFlags = DSBCAPS_CTRLPAN | DSBCAPS_CTRLVOLUME | DSBCAPS_LOCSOFTWARE;
  ...
  hres = m_Buffer[0]->Lock(0, WavFile.GetWaveSize(), &lpAudio, &dwBytes, NULL,
                           NULL, 0);
  if (hres != DS_OK)
    return FALSE;
