...
#define CANCOMPILETOLUB // 是否支持lub文件也就是为单机版服务的将某些指令IF
                        // 语句换成GOTO的功能
//---------------------------------------------------------------------------
...
// 返回:
//---------------------------------------------------------------------------
KStepLuaScript::~KStepLuaScript(void) { m_Memory.Free(); }
...
BOOL KStepLuaScript::Load(LPSTR szFileName) {
  ...
  if (len == 0)
    return FALSE;
  if (!GetExeBuffer(pMem->GetMemPtr(), len))
    return FALSE;
  if (!KLuaScript::LoadBuffer((PBYTE)m_Memory.GetMemPtr(), len))
    return FALSE;
  if (!ExecuteCode())
    return FALSE;
  delete pExitLine;
  delete pMem;
#else
  if (!GetExeBufferFromFile(szFileName))
    return FALSE;
...
BOOL KStepLuaScript::ExeLine(LPSTR szLine) {
  if (szLine) {
    if (lua_dostring(m_LuaState, szLine) == 0)
      return TRUE;
  }
  return FALSE;
}

BOOL KStepLuaScript::CheckLine(
    LPSTR szLine) // 检查将执行的Lua语句是否符合条件，如不能有for goto
{
  return TRUE;
}

BOOL KStepLuaScript::GetExeBufferFromFile(
    char *FileName) // 获得当前文件中执行段的语句
{
  KPakFile File;
  DWORD Size;
...
// 从Buffer中获得代码
BOOL KStepLuaScript::GetExeBuffer(void *szScriptBuffer,
                                  int nLen) // 获得执行段的语句
{
  char szLine[100];
  long nCurPos = 0;
...
int KStepLuaScript::Active() {
  ...
  while (pNode) {
    ...
    if (!CallFunction(MsgFuncName, 0, "ds", (unsigned int)pNode->StateAddr,
                      pNode->szMsgData)) {
      ;
    }
...
BOOL KStepLuaScript::SendMessage(KStepLuaScript *pSendedScript,
                                 char *szMessageName, char *szData) {
