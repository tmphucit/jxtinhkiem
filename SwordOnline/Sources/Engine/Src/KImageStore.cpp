...
unsigned int KImageStore::CreateImage(const char *pszName, int nWidth,
...
bool KImageStore::GetImageParam(const char *pszImage, KImageParam *pImageData,
                                int nType) {
  if (!pszImage || !pszImage[0])
    return false;
...
KImageRes *KImageStore::GetImage(const char *pszImage, unsigned int &uImage,
                                 int &nImagePosition, int nFrame, int nType) {
  if (!pszImage || !pszImage[0])
    return NULL;
  ...
  if (m_fCacheMemUsed > m_nBalanceNum &&
      (tmCur <= m_tmLastCheckBalance ||
       (tmCur - m_tmLastCheckBalance) > ISBP_CHECK_POINT_DEF)) {
...
int KImageStore::GetImagePixelAlpha(const char *pszImage, int nFrame, int nX,
                                    int nY, int nType) {
  if (!pszImage || !pszImage[0])
    return 0;
  ...
  case ISI_T_BITMAP16:
    ...
    if (!pTemp)
      break;
    if (nX < 0 || nY < 0 || nX >= ((ImageResBmp *)pTemp)->GetWidth() ||
        nY >= ((ImageResBmp *)pTemp)->GetHeight())
      nRet = 0;
    else
      nRet = 255;
    break;
...
KImageRes *KImageStore::LoadImage(const char *pszImageFile,
