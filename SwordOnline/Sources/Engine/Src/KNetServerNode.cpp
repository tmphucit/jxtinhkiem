      ...
      if (ims->m_ReceiveBuf[0] == '~') {
        ...
        if (strcmp(cmd, "handshake") == 0) {
          ...
          if (pname) {
            ...
            g_DebugLog("KNetServerNode::n_miniserver_tfunc(): ~handshake "
                       "command broken!");
          }
          ...
          g_DebugLog("KNetServerNode::n_miniserver_tfunc(): unknown internal "
                     "command!");
        }
...
bool KNetServerNode::Listen(void) {
  ...
  if (listen(m_ServerSocket, 5) != SOCKET_ERROR) {
    ...
    m_AcceptSocket =
        accept(m_ServerSocket, (struct sockaddr *)&(m_ClientAddr), &addr_len);
...
bool KNetServerNode::Accept(void) {
  ...
  m_ReceiveThread.Init(n_miniserver_tfunc,  // thread func
                       0,                   // stack size
                       n_miniserver_wakeup, // wakeup func
                       &(m_Server->m_MsgList), (void *)this);
  return TRUE;
}
