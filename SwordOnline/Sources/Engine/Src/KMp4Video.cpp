...
typedef struct _DEC_PARAM_ {
  ...
  unsigned long
      color_depth; // leaved for compatibility (new value must be NULL)
  int output_format;
} DEC_PARAM;
...
typedef int (*decore)(
    unsigned long
        handle, // handle - the handle of the calling entity, must be unique
    unsigned long dec_opt, // dec_opt - the option for docoding, see below
    void *param1,  // param1 - the parameter 1 (it's actually meaning depends on
                   // dec_opt
    void *param2); // param2 - the parameter 2 (it's actually meaning depends on
                   // dec_opt
...
// 返回:	void
//---------------------------------------------------------------------------
KMp4Video::KMp4Video() { m_dwHandle = 0; }
//---------------------------------------------------------------------------
// 函数:	~KMp4Video
// 功能:	析构函数
// 参数:	void
...
// 返回:	void
//---------------------------------------------------------------------------
void KMp4Video::Seek(int nPercent) { m_AviFile.VideoSeek(nPercent); }
//---------------------------------------------------------------------------
// 函数:	Rewind
// 功能:
// 参数:	void
// 返回:	void
//---------------------------------------------------------------------------
void KMp4Video::Rewind() { m_AviFile.Rewind(); }
//---------------------------------------------------------------------------
// 函数:	NextFrame
// 功能:	下一帧
// 参数:	void
