...
KScriptSet::KScriptSet(int Key_Style) {
  if (Key_Style == KEY_NUMBER || Key_Style == KEY_STRING)
    ...
    m_nKeyStyle = KEY_STRING;
    printf("UnKnown ScriptSet KeyStyle:[%d]. So Set Key As Default KeyStyle "
           "KEY_STRING.\n",
           Key_Style);
  }
  m_pRootList = NULL;
...
// 返回:	KScript *
//---------------------------------------------------------------------------
KScript *KScriptSet::GetScript(char *szKey, BOOL nNeedCreate = 1,
                               BOOL nNeedUpdateRecent = 1) {
  if (szKey == NULL)
    return NULL;
  ...
  if (m_nKeyStyle != KEY_STRING) {
    printf("The ScriptSet's Key Is Not a String. So Can Not Find A CorrRespond "
           "Script.\n");
    return NULL;
  }
...
KScript *KScriptSet::GetScript(DWORD nKey, BOOL nNeedCreate = 1,
                               BOOL nNeedUpdateRecent = 1) {
  ...
  if (m_nKeyStyle != KEY_NUMBER) {
    printf("The ScriptSet's Key Is Not a Number. So Can Not Find A CorrRespond "
           "Script.\n");
    return NULL;
  }
...
// 返回:	KScript *
//---------------------------------------------------------------------------
KScript *KScriptSet::CreateScript(char *szKey, int StackSize) { return NULL; }
...
// 返回:	KScript *
//---------------------------------------------------------------------------
KScript *KScriptSet::CreateScript(DWORD nKey, int StackSize) { return NULL; }
...
BOOL KScriptSet::DeleteScript(char *szKey) {
  ...
  if (nResult && pTScript) // 在链表中找到了该关键字
  {
    ...
    if (pTScript->pLessRecent == NULL &&
        pTScript->pMoreRecent == NULL) // 全部为空，即只有一个结点
    {
      m_pMostRecent = NULL;
      m_pLestRecent = NULL;
...
// 返回:	int
//---------------------------------------------------------------------------
int KScriptSet::GetInitStackSize(char *szKey) { return 0; }
...
// 返回:	int
//---------------------------------------------------------------------------
int KScriptSet::GetInitStackSize(DWORD nKey) { return 0; }
...
// 返回:	void
//---------------------------------------------------------------------------
DWORD KScriptSet::ListScriptsKey() { return BTPreorder(m_pRootList); }
...
BOOL KScriptSet::Run(char *szKey) { return TRUE; }

BOOL KScriptSet::Run(DWORD nKey) { return TRUE; }
...
// 返回:	TScript *
//---------------------------------------------------------------------------
TScript *BTSearch(TScript *pParentTScript, TScript *pTScript, char *szKey,
                  BOOL *pResult) {
...
// 返回:	TScript *
//---------------------------------------------------------------------------
TScript *BTSearch(TScript *pParentTScript, TScript *pTScript, DWORD nKey,
                  BOOL *pResult) {
...
TScript *BTDelete(TScript *pTScript, TScript **ppRootTScript, int nKeyStyle) {
  ...
  if (pTScript->pLeftChild == NULL &&
      pTScript->pRightChild == NULL) // 该结点没有左右子
  {
