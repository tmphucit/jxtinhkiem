...
#define CHARASET_GBK_INDEX(cH, cL)                                             \
  (((cH) - 0x81) * 190 + ((cL) - 0x40) - ((cL) >> 7))
...
void KFont::DrawFont(int nIndex, int nX, int nY) {
  ...
  if (lpData && nIndex >= 0 && nIndex < m_nCount && m_pdwOffs[nIndex]) {
    ...
    if (m_TextStyle == FONT_ALPHA)
      ...
      g_pCanvas->DrawFontSolid(nX, nY, m_nFontW, m_nFontH, m_dwColor, 31,
                               lpData);
  }
}
...
void KFont::DrawFontAlpha(int nIndex, int nX, int nY, int nAlpha) {
  ...
  if (lpData && nIndex >= 0 && nIndex < m_nCount && m_pdwOffs[nIndex]) {
    lpData += m_pdwOffs[nIndex];
    if (m_TextStyle == FONT_ALPHA)
      g_pCanvas->DrawFont(nX, nY, m_nFontW, m_nFontH, m_dwColor, nAlpha,
                          lpData);
    else
      g_pCanvas->DrawFontSolid(nX, nY, m_nFontW, m_nFontH, m_dwColor, nAlpha,
                               lpData);
  }
}
...
BOOL KFont::MakeFont(LPSTR FileName, int nWidth, int nHeight) {
  ...
  BYTE Buffer
      [32 *
       32]; // 这个数字不能随便加大，来自于字库文件编码的限制，以及GetFontLine代码的局限。
  int nSize;
  int nX, nY;

  if (!BmpFile.Load(FileName, &Bitmap) || nWidth < 0 || nWidth > 32 ||
      nHeight < 0 || nHeight > 32)
    return FALSE;
  ...
  m_dwSize = 0;
  m_nOffsetSpace =
      (Bitmap.GetWidth() / nWidth) * (Bitmap.GetHeight() / nHeight);
  m_nFontW = nWidth;
  m_nFontH = nHeight;
...
int KFont::GetFontLine(int nX, int nY, KBitmap *pBmp, PBYTE pBuf) {
  ...
  while (nWidth > 0) {
    ...
    byPixel &=
        0xE0; // 新的代码处理256阶（色）的位图，黑底白字，色盘颜色由黑到白。
    //		byPixel <<= 5;
    ////旧的代码是处理8阶（色）的位图，白底黑字，色盘颜色由白到黑。
    byPixel |= nCount;
    *pBuf = byPixel;
    pBuf++;
    nSize++;
