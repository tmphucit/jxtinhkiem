...
char *ZMapFile::map(unsigned long offset, unsigned long size) {
  ...
  if (m_Ptr) {
    if (offset > old_offset &&
        (offset + size < old_offset + MINIMIZE_MAP_SIZE)) { // 已经映射了
  ...
  if (old_offset + size > m_Size)
    size = m_Size - old_offset;
...
char *ZCache::searchNode(unsigned long index_high, unsigned long index_low) {
  ...
  for (int index = 0; index < LAST_ITEM; index++) {
    if (last_item[index].index_high == index_high &&
        last_item[index].index_low == index_low) {
      m_Mutex.Unlock();
...
bool ZPackFile::_readData(int node_index, char *node) {
  char *source =
      data_map->map(index_list[node_index].offset, index_list[node_index].size);
  if (!source) {
    return false;
  }
