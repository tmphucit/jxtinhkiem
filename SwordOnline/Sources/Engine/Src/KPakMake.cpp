...
// 返回:	void
//---------------------------------------------------------------------------
KPakMake::~KPakMake() { g_FreeCodec(&m_pCodec, m_nCompressMethod); }
//---------------------------------------------------------------------------
// 函数:	Pack
// 功能:	打包
...
//			FALSE				失败
//---------------------------------------------------------------------------
BOOL KPakMake::Pack(LPSTR lpListFileName, LPSTR lpPackFileName,
                    int nCompressMethod) {
  ...
  for (i = 0; i < m_nFileNum - 1; i++) {
    for (j = i + 1; j < m_nFileNum; j++) {
      if (pFileList[i].FileId > pFileList[j].FileId) {
        ...
        sprintf(
            WarnChar,
            "There are files have the same id: %d  File1 is  %s  File2 is %s ",
            pFileList[i].FileId, pFileList[i].FileName, pFileList[j].FileName);

        // warn<<"There are files have the same id:" << pFileList[i].FileId <<"
        // File1 is " << pFileList[i].FileName  << " File2 is " <<
        // pFileList[j].FileName;
...
BOOL KPakMake::ReadFileList(const vector<string> &FileList,
...
BOOL KPakMake::Pack(LPSTR lpPackFileName, const vector<string> &vFileList,
                    const string &sRootPath, int nCompressMethod) {
  char compress_buffer
      [COMPRESS_BUF_SIZE]; // 10M的压缩缓冲区，存放所有的帧，一次写
  ...
  for (int index = 0; index < vFileList.size(); index++) {
    ...
    for (index = 0; index < file_count; index++) {
      if (id < index_list[index].id) {
        memmove(temp_buffer, (char *)&index_list[index],
                (file_count - index) * sizeof(index_info));
        memmove((char *)&index_list[index + 1], temp_buffer,
                (file_count - index) * sizeof(index_info));
        break;
      }
    }
      ...
      if (!bSPR || map.m_Size < FRAME_SIZE) {
        if (compress_type == TYPE_UCL) {
          r = ucl_nrv2b_99_compress(
              (BYTE *)map.m_Ptr, map.m_Size, (BYTE *)ptr,
              (unsigned int *)&index_list[index].compress_size, NULL, 5, NULL,
              NULL);
        ...
        memmove(ptr, head,
                sizeof(SPRHEAD) + head->Colors * 3); // 前面的数据不压缩
        ptr += sizeof(SPRHEAD) + head->Colors * 3;
        frame_info *compress_frame_info =
            (frame_info *)ptr; // 压缩后每一帧的数据
        ptr += head->Frames * sizeof(SPROFFS);
        SPROFFS *frame_info = (SPROFFS *)(map.m_Ptr + sizeof(SPRHEAD) +
                                          head->Colors * 3); // 原来每一帧的数据
        ...
        for (frame_index = 0; frame_index < head->Frames; frame_index++) {
          ...
          if (frame_info[frame_index].Length >= 256) { // 小于256字节的不压缩
            if (compress_type == TYPE_UCL) {
              r = ucl_nrv2b_99_compress(
                  (BYTE *)frame_data + frame_info[frame_index].Offset,
                  frame_info[frame_index].Length, (BYTE *)ptr, &size, NULL, 10,
                  NULL, NULL);
            } else if (compress_type == TYPE_BZIP2) {
              //						size =
              //COMPRESS_BUF_SIZE;
            ...
            if (r)
              return false;
            compress_frame_info[frame_index].size =
                frame_info[frame_index].Length; // 记录原来的大小
          } else {
            size = frame_info[frame_index].Length;
            memmove(ptr, (BYTE *)frame_data + frame_info[frame_index].Offset,
                    size);
            compress_frame_info[frame_index].size =
                -(long)frame_info[frame_index].Length; // 记录原来的大小
          }
          compress_size += size;
        ...
        index_list[index].compress_size =
            (ptr - compress_buffer) | ((compress_type | TYPE_FRAME) << 24);
      }
    }
  }
  int result =
