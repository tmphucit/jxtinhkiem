...
int LuaLabel(Lua_State *L) { return 0; }
...
int LuaSendMessage(Lua_State *L) {
  g_StoryScriptList.SendMessage((char *)lua_tostring(L, 1),
...
int LuaNpcGoto(Lua_State *L) {
  ...
  if (pScript->IsRunMain()) {
    ...
    *(int *)&NetCommand[5] = nDesY;
    ...
    SubWorld[Npc[i].m_SubWorldIndex].Map2Mps(
        Npc[i].m_RegionIndex, Npc[i].m_MapX, Npc[i].m_MapY, Npc[i].m_OffX,
        Npc[i].m_OffY, &nSrcX, &nSrcY);
    if (abs(nSrcX - nDesX) < Npc[i].m_WalkSpeed &&
        abs(nSrcY - nDesY) < Npc[i].m_WalkSpeed)
      pScript->RunMain();
    else
      Npc[i].SendCommand(do_walk, nDesX, nDesY);
  }
...
int LuaAddNpcClient(Lua_State *L) {
  ...
  if (pIniFile[0]) {
    ...
    nNpcIdx =
        NpcSet.Add(1, 0, 0, (int)lua_tonumber(L, 4), (int)lua_tonumber(L, 5));
    g_StrCpy(Npc[nNpcIdx].Name, (char *)pName);
...
int LuaCastSkill(Lua_State *L) {
  ...
  if (nSrc == 0)
    return 0;
  Npc[nSrc].SendCommand(do_skill, (int)lua_tonumber(L, 2),
...
int LuaGetNpcTalk(Lua_State *L) {
  ...
  if (nSrc == 0)
    ...
    char *strBuf;
    if (Npc[nSrc].m_szChatBuffer[0] == 19)
      strBuf = Npc[nSrc].m_szChatBuffer + 2;
    else {
