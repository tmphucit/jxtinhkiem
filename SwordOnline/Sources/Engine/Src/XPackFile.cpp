...
bool XPackFile::Open(const char *pszPackFileName, int nSelfIndex) {
  ...
  m_hFile = ::CreateFile(pszPackFileName, GENERIC_READ, FILE_SHARE_READ, NULL,
  ...
  while (m_hFile != INVALID_HANDLE_VALUE) {
    ...
    if (m_pIndexList == NULL ||
        ::SetFilePointer(m_hFile, Header.uIndexTableOffset, NULL, FILE_BEGIN) !=
            Header.uIndexTableOffset) {
      break;
    }
...
// 返回：成功与否
//-------------------------------------------------
bool XPackFile::DirectRead(void *pBuffer, unsigned int uOffset,
                           unsigned int uSize) const {
  bool bResult = false;
  DWORD dwReaded;
...
bool XPackFile::ExtractRead(void *pBuffer, unsigned int uExtractSize,
                            long lCompressType, unsigned int uOffset,
                            unsigned int uSize) const {
  _ASSERT(pBuffer);
  bool bResult = false;
...
void *XPackFile::ReadElemFile(int nElemIndex) const {
  ...
  if (pDataBuffer) {
    if (ExtractRead(pDataBuffer, m_pIndexList[nElemIndex].lSize,
                    (m_pIndexList[nElemIndex].lCompressSizeFlag & TYPE_FILTER),
                    m_pIndexList[nElemIndex].uOffset,
                    (m_pIndexList[nElemIndex].lCompressSizeFlag &
                     (~TYPE_FILTER))) == false) {
      free(pDataBuffer);
      pDataBuffer = NULL;
    }
...
int XPackFile::AddElemFileToCache(void *pBuffer, int nElemIndex) {
  ...
  int nCacheIndex;
  if (ms_nNumElemFileCache < MAX_XPACKFILE_CACHE) { // 找到一个空位置
...
// 返回：成功读取得字节数
//-------------------------------------------------
int XPackFile::ElemFileRead(XPackElemFileRef &ElemRef, void *pBuffer,
                            unsigned uSize) {
  int nResult = 0;
  if (pBuffer && ElemRef.uId && ElemRef.nElemIndex >= 0) {
    ...
    if (ElemRef.nCacheIndex < 0 && // 在cache里未找到
      ...
      if (pDataBuffer)
        ElemRef.nCacheIndex =
            AddElemFileToCache(pDataBuffer, ElemRef.nElemIndex);
    }
    ...
    if (ElemRef.nCacheIndex >= 0 &&
      ...
      if (ElemRef.nOffset < ElemRef.nSize) {
        ...
        memcpy(pBuffer,
               (char *)ms_ElemFileCache[ElemRef.nCacheIndex].pBuffer +
                   ElemRef.nOffset,
               nResult);
...
SPRHEAD *XPackFile::GetSprHeader(XPackElemFileRef &ElemRef,
  ...
  if (ElemRef.nElemIndex < m_nElemFileCount &&
      m_pIndexList[ElemRef.nElemIndex].uId == ElemRef.uId) {
    // 首先检查这个id是什么类型压缩方式
    if ((m_pIndexList[ElemRef.nElemIndex].lCompressSizeFlag & TYPE_FRAME) ==
        0) {
      pSpr = (SPRHEAD *)ReadElemFile(ElemRef.nElemIndex);
      if (pSpr) {
        if ((*(int *)&pSpr->Comment[0]) == SPR_COMMENT_FLAG) {
          pOffsetTable =
              (SPROFFS *)(((char *)pSpr) + sizeof(SPRHEAD) + pSpr->Colors * 3);
          bOk = true;
        }
      }
    } else {
      SPRHEAD Header;
      if (DirectRead(&Header, m_pIndexList[ElemRef.nElemIndex].uOffset,
                     sizeof(SPRHEAD))) {
        if (*(int *)&(Header.Comment[0]) == SPR_COMMENT_FLAG) {
          unsigned int u2ListSize =
              Header.Colors * 3 + Header.Frames * sizeof(XPackSprFrameInfo);
          pSpr = (SPRHEAD *)malloc(sizeof(SPRHEAD) + u2ListSize);
          if (pSpr) {
            if (DirectRead((char *)(&pSpr[1]),
                           m_pIndexList[ElemRef.nElemIndex].uOffset +
                               sizeof(SPRHEAD),
                           u2ListSize)) {
    ...
    if (pSpr) {
      if (bOk) {
        *((int *)&pSpr->Reserved[NODE_INDEX_STORE_IN_RESERVED]) =
            ElemRef.nElemIndex;
      } else {
        free(pSpr);
        pSpr = NULL;
...
SPRFRAME *XPackFile::GetSprFrame(SPRHEAD *pSprHeader, int nFrame) {
  ...
  if (pSprHeader && nFrame >= 0 && nFrame < pSprHeader->Frames) {
    EnterCriticalSection(&m_ReadCritical);
    int nNodeIndex =
        *((int *)&pSprHeader->Reserved[NODE_INDEX_STORE_IN_RESERVED]);
    if (nNodeIndex >= 0 && nNodeIndex < m_nElemFileCount) {
      ...
      if ((lCompressType & TYPE_FRAME) != 0) {
        ...
        // 读出指定帧的信息
        XPackSprFrameInfo *pFrameList =
            (XPackSprFrameInfo *)((char *)pSprHeader + lTempValue);
        unsigned long uSrcOffset =
            m_pIndexList[nNodeIndex].uOffset + lTempValue +
        ...
        if (lTempValue < 0) {
          ...
          if (pFrame = (SPRFRAME *)malloc(lTempValue)) {
            bOk = ExtractRead(pFrame, lTempValue, lCompressType, uSrcOffset,
                              pFrameList->lCompressSize);
          }
        }
