...
IInlinePicEngineSink *g_pIInlinePicSink =
    NULL; // 嵌入式图片的处理接口[wxb 2003-6-19]
...
extern "C" ENGINE_API int
TIsCharacterNotAlowAtLineHead(const char *pCharacter) {
  int i;
  unsigned char cChar;
...
// 获取本行的下个显示字符
extern "C" ENGINE_API const char *
TGetSecondVisibleCharacterThisLine(const char *pCharacter, int nPos, int nLen) {
      ...
      while (nPos < nDesirePos) {
        ...
        if (cCharacter > 0x80) {
          if (bLess && (nPos + 2 > nDesirePos))
            break;
          if (nPos + 1 == nCount) { // 防止出现单BYTE的高于0x80的字符
            nPos = nCount;
            break;
          }
          nPos += 2;
...
static const KColorTable s_ColorTable[] = {
    ...
    {"Blue", 100, 100, 255},  {"Yellow", 255, 255, 0},
    {"Pink", 238, 0, 238}, // 184,0,195
    {"Cyan", 0, 255, 255},    {"Metal", 246, 255, 117},
    {"Wood", 0, 255, 120},    {"Water", 78, 124, 255},
    {"Fire", 255, 90, 0},     {"Earth", 254, 207, 179},
    {"DBlue", 120, 120, 120}, {"HBlue", 100, 100, 255},
    {"DYellow", 100, 100, 0}, {"HYellow", 255, 255, 0},
    {"DGreen", 0, 100, 0},    {"HGreen", 0, 255, 0},
    {"DPink", 97, 22, 97},    {"HPink", 255, 62, 150}, // 255, 52, 179
};
...
static bool TEncodeCtrl(char *pBuffer, int nCount, int &nReadPos,
                        int &nShortCount);
static int TEncodeCtrl(int nCtrl, char *pParamBuffer, int nParamLen,
                       char *pEncodedBuffer);

//--------------------------------------------------------------------------
//	功能：对文本串中的控制标记进行转换，去除无效字符，缩短文本串存储长度
//--------------------------------------------------------------------------
extern "C" ENGINE_API int TEncodeText(char *pBuffer, int nCount) {
  int nShortCount = 0;
  if (pBuffer) {
    ...
    while (nReadPos < nCount) {
      ...
      // moi them
      else if (cCharacter == KTC_INLINE_PIC) // them vao
      {
...
static bool TEncodeCtrl(char *pBuffer, int nCount, int &nReadPos,
  ...
  _ASSERT(pBuffer != NULL && nReadPos < nCount && nShortCount < nCount &&
          nShortCount <= nReadPos);

  int nCtrlCodeSize, nEndPos, nCtrl;

  // 寻找结束符号'='的位置或'>'的位置
  int nEqualPos = nReadPos + 1;
  for (; nEqualPos < nCount && nEqualPos <= nReadPos + KTC_CTRL_CODE_MAX_LEN;
       nEqualPos++)
  ...
  for (nCtrl = 0; nCtrl < s_nCtrlCount; nCtrl++) {
    if (nCtrlCodeSize == s_CtrlTable[nCtrl].nCtrlLen &&
        memcmp(pBuffer + nReadPos + 1, s_CtrlTable[nCtrl].szCtrl,
               nCtrlCodeSize) == 0)
      break;
  }
  ...
  case KTC_BORDER_COLOR:
    if (nParamLen == 0) {
      ...
      for (int i = 0; i < s_nColorCount; i++) {
        ...
        if (strcasecmp(Color, s_ColorTable[i].Token) == 0)
#endif
        //				if (stricmp(Color,s_ColorTable[i].Token)
        //== 0)
        {
...
// nWrapCharaNum, int& nMaxLineLen, int nFontSize, int nSkipLine = 0, int
// nNumLineLimit = 0)
extern "C" ENGINE_API int
TGetEncodedTextLineCount(const char *pBuffer, int nCount, int nWrapCharaNum,
                         int &nMaxLineLen, int nFontSize, int nSkipLine,
                         int nNumLineLimit, BOOL bPicSingleLine /* = FALSE*/) {
  ...
  while (nPos < nCount) {
    ...
    if (cCode > 0x80) // 可能是中文字符
    {
      ...
      if (g_pIInlinePicSink) {
        int nWidth, nHeight;
        if (SUCCEEDED(
                g_pIInlinePicSink->GetPicSize(wPicIndex, nWidth, nHeight))) {
          if (nHeight > nFontSize) {
            int nExtraLines = nHeight - nFontSize;
            nExtraLines =
                nExtraLines / nFontSize + ((nExtraLines % nFontSize) ? 1 : 0);
          ...
          else if (fNumChars + (((float)nWidth) * 2 / nFontSize) ==
                       nWrapCharaNum ||
                   fNumChars == 0) {
            bNextLine = true;
            fNumChars += ((float)nWidth) * 2 / nFontSize;
    ...
    if (bNextLine == false && fNumChars && fNumChars + 3 >= nWrapCharaNum) {
      const char *pNext =
          TGetSecondVisibleCharacterThisLine(pBuffer, nPos, nCount);
...
// 获得指定行的开始位置
int TGetEncodeStringLineHeadPos(const char *pBuffer, int nCount, int nLine,
                                int nWrapCharaNum, int nFontSize,
                                BOOL bPicSingleLine) {
  // 设一个极值免得出错 [wxb 2003-6-20]
...
extern "C" ENGINE_API
    ...
    const char *
    TGetLimitLenEncodedString(const char *pOrigString, int nOrigLen,
                              int nFontSize, int nWrapCharaNum,
                              char *pLimitLenString, int &nShortLen,
                              int nLineLimit,
                              int bPicPackInSingleLine /*=false*/) {
  if (pOrigString == NULL || pLimitLenString == NULL || nOrigLen <= 0 ||
      nShortLen < 2 || nLineLimit < 1 || nWrapCharaNum < 2) {
    nShortLen = 0;
    return NULL;
  }
  ...
  {
    nPreLineEndPos = TGetEncodeStringLineHeadPos(
        pOrigString, nOrigLen, nLineLimit - 1, nWrapCharaNum, nFontSize,
        bPicPackInSingleLine);
    if (nPreLineEndPos > nShortLen) {
      nShortLen =
          TSplitEncodedString(pOrigString, nOrigLen, nShortLen - 2, true);
  ...
  if (nPreLineEndPos < nOrigLen) {
    nFinalLineEndPos =
        TGetEncodeStringLineHeadPos(pOrigString + nPreLineEndPos,
                                    nOrigLen - nPreLineEndPos, 1, nWrapCharaNum,
                                    nFontSize, bPicPackInSingleLine) +
        nPreLineEndPos;
  } else
  ...
  int nDesireLen =
      (nFinalLineEndPos <= nShortLen) ? nFinalLineEndPos - 2 : nShortLen - 2;
  ...
  while (true) {
    nShortLen =
        TSplitEncodedString(pFinalLineHead, nRemainCount, nDesireLen, true);
    int nMaxLineLen;
    TGetEncodedTextLineCount(pFinalLineHead, nShortLen, 0, nMaxLineLen,
                             nFontSize, 0, 1, FALSE);
...
extern "C" ENGINE_API int TFindSpecialCtrlInEncodedText(const char *pBuffer,
                                                        int nCount,
                                                        int nStartPos,
                                                        char cControl) {
  int nFindPos = -1;
...
// 对已经编码的文本，去除指定类型的控制符
extern "C" ENGINE_API int
TClearSpecialCtrlInEncodedText(char *pBuffer, int nCount, char cControl) {
  int nFinalLen = 0;
  int nReadPos = 0;
  if (pBuffer) {
...
extern "C" ENGINE_API int TGetEncodedTextOutputLenPos(const char *pBuffer,
                                                      int nCount, int &nLen,
                                                      bool bLess,
                                                      int nFontSize) {
    ...
    while (nLenTemp < nLen) {
      ...
      if (cCharacter > 0x80) // 可能是中文文字
      {
        ...
        if (SUCCEEDED(g_pIInlinePicSink->GetPicSize(
                *((unsigned short *)(pBuffer + nIndex + 1)), nWidth,
                nHeight))) {
...
extern "C" ENGINE_API int
TGetItemChatLineCount(const char *pBuffer, int nCount, int nWrapCharaNum,
                      int &nMaxLineLen, int nFontSize, int &nFace,
                      int &nLastPos, int &nTotalLen, int nSkipLine,
                      int nNumLineLimit, BOOL bPicSingleLine /* = FALSE*/) {
  // Tinh' toan' vi tri' item post len kenh chat
  _ASSERT(nFontSize >= 4 && nFontSize < 64);
  nFontSize = max(4, nFontSize);
  nFontSize = min(64, nFontSize);
  ...
  while (nPos < nCount) {
    ...
    if (cCode > 0x80) //
    {
      ...
      // Ken Nguyen da den day [05-05-2013]
      nFace++;        //
      nTotalLen += 3; //
      WORD wPicIndex = *((WORD *)(pBuffer + nPos + 1));
      nPos += 1 + sizeof(WORD);
...
extern "C" ENGINE_API int TGetEncodedTextEffectCtrls(const char *pBuffer,
                                                     int nSkipCount,
                                                     KTP_CTRL &Ctrl0,
                                                     KTP_CTRL &Ctrl1) {
  int nIndex = 0;
