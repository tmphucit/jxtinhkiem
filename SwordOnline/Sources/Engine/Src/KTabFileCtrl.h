...
private:
  BOOL SetValue(int nRow, int nColumn, LPSTR lpRString, DWORD dwSize,
...
public:
  ...
  BOOL GetString(int nRow, int nColumn, LPSTR lpDefault, LPSTR lpRString,
                 DWORD dwSize);

  BOOL GetString(int nRow, LPSTR szColumn, LPSTR lpDefault, LPSTR lpRString,
                 DWORD dwSize, BOOL bColumnLab = TRUE) {
    if (!bColumnLab) // 该段以exl文件的a1,b1,c1字符串为
      return GetString(nRow, Str2Col(szColumn), lpDefault, lpRString, dwSize);
    else
      return GetString(nRow, FindColumn(szColumn), lpDefault, lpRString,
                       dwSize);
  };

  BOOL GetString(LPSTR szRow, LPSTR szColumn, LPSTR lpDefault, LPSTR lpRString,
                 DWORD dwSize) {
    return GetString(FindRow(szRow), FindColumn(szColumn), lpDefault, lpRString,
                     dwSize);
  };

  BOOL GetInteger(int nRow, int nColumn, int nDefault, int *pnValue);

  BOOL GetInteger(int nRow, LPSTR szColumn, int nDefault, int *pnValue,
  ...
  BOOL GetFloat(int nRow, LPSTR szColumn, float fDefault, float *pfValue,
                BOOL bColumnLab = TRUE) {
    if (!bColumnLab)
  ...
  BOOL WriteString(int nRow, LPSTR szColumn, LPSTR lpString,
                   BOOL bColumnLab = TRUE) {
    if (!bColumnLab) // 该段以exl文件的a1,b1,c1字符串为
  ...
  BOOL WriteInteger(int nRow, LPSTR szColumn, int nValue,
                    BOOL bColumnLab = TRUE) {
    if (!bColumnLab)
  ...
  BOOL WriteFloat(int nRow, LPSTR szColumn, float fValue,
                  BOOL bColumnLab = TRUE) {
    if (!bColumnLab)
