...
void CSample::Login(const char *lpszAccName, const char *lpszPassword,
  ...
  if (NULL != pTemp) {
    ...
    if (S3PDBSocketParser::Encode(pTemp, dwTempSize, NULL,
                                  ulEncodedSize)) // 计算加密后数据包尺寸
    {
      IBYTE *pSendBuf = new IBYTE[ulEncodedSize];
      if (NULL != pSendBuf) {
        if (S3PDBSocketParser::Encode(pTemp, dwTempSize, pSendBuf,
                                      ulEncodedSize)) // 加密数据包
        {
          try {
...
void CSample::DecAccountDeposit(DWORD dwGamerID, DWORD dwServerID,
                                DWORD dwDecSeconds) {}
