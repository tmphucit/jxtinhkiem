...
BOOL TestRunnerDlg::OnInitDialog() {
  ...
  listCtrl->InsertColumn(0, "Type", LVCFMT_LEFT,
                         2 * listCtrl->GetStringWidth("Type"), 1);
  listCtrl->InsertColumn(1, "Name", LVCFMT_LEFT,
                         20 * listCtrl->GetStringWidth("#"), 2);
  listCtrl->InsertColumn(2, "Failed Condition", LVCFMT_LEFT,
                         1.75 * listCtrl->GetStringWidth("Failed Condition"),
                         3);
  listCtrl->InsertColumn(3, "Line Number", LVCFMT_LEFT,
                         1.5 * listCtrl->GetStringWidth("Line Number"), 4);
  listCtrl->InsertColumn(4, "File Name", LVCFMT_LEFT,
                         4.0 * listCtrl->GetStringWidth("File Name"), 5);
...
void TestRunnerDlg::addListEntry(std::string type, TestResult *result,
                                 Test *test, CppUnitException *e) {
  char stage[80];
  LV_ITEM lvi;
  CListCtrl *listCtrl = (CListCtrl *)GetDlgItem(IDC_LIST);
...
void TestRunnerDlg::addError(TestResult *result, Test *test,
                             CppUnitException *e) {
...
void TestRunnerDlg::addFailure(TestResult *result, Test *test,
                               CppUnitException *e) {
...
void TestRunnerDlg::updateCountsDisplay() {
  ...
  argumentString.Format("Execution time: %3.3lf seconds",
