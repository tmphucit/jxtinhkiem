...
void TextTestResult::printErrors(ostream &stream) {
  if (testErrors() != 0) {
    ...
    for (vector<TestFailure *>::iterator it = errors().begin();
         it != errors().end(); ++it) {
...
void TextTestResult::printFailures(ostream &stream) {
  if (testFailures() != 0) {
    ...
    for (vector<TestFailure *>::iterator it = failures().begin();
         it != failures().end(); ++it) {
      TestFailure *failure = *it;
      CppUnitException *e = failure->thrownException();
...
void TextTestResult::printHeader(ostream &stream) {
  if (wasSuccessful())
    ...
    cout << endl
         << "!!!FAILURES!!!" << endl
         << "Test Results:" << endl
         << "Run:  " << runTests() << "   Failures: " << testFailures()
         << "   Errors: " << testErrors() << endl;
}
