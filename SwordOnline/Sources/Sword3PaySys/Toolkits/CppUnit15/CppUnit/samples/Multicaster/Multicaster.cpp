...
bool Multicaster::subscribe(MulticastObserver *observer, std::string address) {
  ...
  if (it == m_addresses.end()) {
    m_addresses.insert(
        std::pair<std::string, Subscriptions>(address, Subscriptions()));
...
bool Multicaster::unsubscribe(MulticastObserver *observer,
                              std::string address) {
  AddressSpace::iterator it = m_addresses.find(address);
...
bool Multicaster::publish(MulticastObserver *publisher, std::string address,
                          Value value) {
  AddressSpace::iterator it = m_addresses.find(address);
...
void Multicaster::unsubscribeFromAll(MulticastObserver *observer) {
  for (AddressSpace::iterator it = m_addresses.begin(); it != m_addresses.end();
       ++it)
    unsubscribe((*it).second, observer);
}

void Multicaster::unsubscribe(Subscriptions &subscriptions,
                              MulticastObserver *observerToRemove) {
