...
#else
typedef hetero_stack<REGEX_STACK_ALIGNMENT, false, true, 4096, 1024>
    unsafe_stack;
#endif
...
template <typename T> struct static_init {
...
//
// --------------------------------------------------------------------------
template <typename CI> class backref_tag : public std::pair<CI, CI> {
  struct detail_t {
    detail_t *d;
  };
  ...
  void _do_print(OSTREAM &sout, typename OSTREAM::char_type const *) const {
    sout.write(first,
               static_cast<std::streamsize>(std::distance(first, second)));
  }
...
public:
  ...
  explicit backref_tag(CI i1 = detail::static_init<CI>::value,
                       CI i2 = detail::static_init<CI>::value)
      : std::pair<CI, CI>(i1, i2), matched(false), reserved1(i1), reserved2(0),
        reserved3(false), reserved4(detail::static_init<CI>::value),
        reserved5(detail::static_init<CI>::value) {}

  CI begin() const { return first; }

  CI end() const { return second; }
...
//
// --------------------------------------------------------------------------
template <typename CI> struct basic_match_results {
  ...
  backref_type const &
  backref(size_t cbackref) const // throw( std::out_of_range )
  {
  ...
  {
    return std::distance(m_rgbackrefs.at(cbackref).first,
                         m_rgbackrefs.at(cbackref).second);
  }
...
struct basic_subst_results
  ...
  basic_subst_results() : m_pbackref_str(&m_backref_str) {}
...
template <typename CH> void reset_intrinsic_charsets(CH ch = CH(0));
...
public:
  ...
  typedef
      typename detail::basic_rpattern_base_impl<CI>::traits_type traits_type;

  typedef
      typename detail::basic_rpattern_base_impl<CI>::string_type string_type;
  typedef typename detail::basic_rpattern_base_impl<CI>::size_type size_type;

  typedef
      typename detail::basic_rpattern_base_impl<CI>::backref_type backref_type;
  typedef typename detail::basic_rpattern_base_impl<CI>::backref_vector
      backref_vector;
  ...
  void
  init(string_type const &pat, REGEX_FLAGS flags = NOFLAGS,
       REGEX_MODE mode = MODE_DEFAULT); // throw( bad_regexpr, std::bad_alloc );

  void
  init(string_type const &pat, string_type const &subst,
...
protected:
  ...
  basic_rpattern_base(basic_rpattern_base<CI, SY> const &that) // throw()
      : detail::basic_rpattern_base_impl<CI>(that.flags(), that.mode(),
                                             that.get_pat(), that.get_subst()) {
    ...
    basic_rpattern_base<CI, SY>::_parse_subst(
        *m_subst, m_fuses_backrefs,
        m_subst_list); // must come after _common_init
  }
  ...
  basic_rpattern_base(
      string_type const &pat, string_type const &subst,
      REGEX_FLAGS flags = NOFLAGS,
      REGEX_MODE mode = MODE_DEFAULT) // throw( bad_regexpr, std::bad_alloc )
  ...
  basic_rpattern_base &
  operator=(basic_rpattern_base<CI, SY> const
                &that) // throw( bad_regexpr, std::bad_alloc )
  {
  ...
  detail::match_group_base<CI> *
  _find_next_group(iter_wrap &iw, detail::match_group_base<CI> *pgroup,
                   syntax_type &sy,
                   std::vector<detail::match_group_base<CI> *> &rggroups);

  bool _find_next(iter_wrap &iw, detail::match_group_base<CI> *pgroup,
                  syntax_type &sy,
                  std::vector<detail::match_group_base<CI> *> &rggroups);

  void _find_atom(iter_wrap &iw, detail::match_group_base<CI> *pgroup,
                  syntax_type &sy);

  void _quantify(std::auto_ptr<detail::sub_expr<CI>> &pnew, iter_wrap &iw,
                 bool is_group, syntax_type &sy);

  void _add_subst_backref(detail::subst_node &snode, size_t nbackref,
                          size_t rstart, bool &uses_backrefs,
                          detail::subst_list_type &subst_list) const;

  virtual void _parse_subst(string_type &subst, bool &uses_backrefs,
                            detail::subst_list_type &subst_list) const;
...
template <typename CI,
          typename SY =
              perl_syntax<REGEX_TYPENAME std::iterator_traits<CI>::value_type>>
...
public:
  ...
  basic_rpattern &operator=(basic_rpattern<CI, SY> const
                                &that) // throw( bad_regexpr, std::bad_alloc )
  {
  ...
  backref_type const &match(CI2 ibegin, CI2 iend,
    ...
    detail::static_assert<detail::is_convertible<CI2, CI>::value> const
        iterator_types_are_not_convertible;
    (void)iterator_types_are_not_convertible;

    detail::match_param<CI> param =
        detail::matcher_helper<CI>::init_param(ibegin, iend, results);
    return impl::_do_match(param, false)
               ? results.backref(0)
               : detail::static_init<backref_type>::value;
  }

  template <typename CH>
  backref_type const &match(CH *szbegin,
                            basic_match_results<CI> &results) const {
    // If your compile breaks here, it is because CI2 is not
    // convertible to type CI. Check the declaration of your rpattern object.
    typedef CH *CI2;
    ...
    detail::match_param<CI> param =
        detail::matcher_helper<CI>::init_param(szbegin, (CH *)0, results);
    return impl::_do_match(param, szbegin)
               ? results.backref(0)
               : detail::static_init<backref_type>::value;
  }

  template <typename CH, typename TR, typename AL>
  backref_type const &match(std::basic_string<CH, TR, AL> const &str,
                            basic_match_results<CI> &results, size_type pos,
                            size_type len) const {
    // If your compile breaks here, it is because CI2 is not
    // convertible to type CI. Check the declaration of your rpattern object.
    ...
    detail::match_param<CI> param = detail::matcher_helper<CI>::init_param(
        str.begin(), str.begin(), results);
  ...
  template <typename CI2> size_t count(CI2 ibegin, CI2 iend) const {
    // If your compile breaks here, it is because CI2 is not
    // convertible to type CI. Check the declaration of your rpattern object.
    detail::static_assert<detail::is_convertible<CI2, CI>::value> const
        iterator_types_are_not_convertible;
  ...
  template <typename CH> size_t count(CH *szbegin) const {
    // If your compile breaks here, it is because CI2 is not
    // convertible to type CI. Check the declaration of your rpattern object.
    typedef CH *CI2;
    detail::static_assert<detail::is_convertible<CI2, CI>::value> const
  ...
  template <typename CH, typename TR, typename AL>
  size_t count(std::basic_string<CH, TR, AL> const &str, size_type pos,
               size_type len) const {
    // If your compile breaks here, it is because CI2 is not
    // convertible to type CI. Check the declaration of your rpattern object.
    typedef typename std::basic_string<CH, TR, AL>::const_iterator CI2;
  ...
  template <typename CH, typename TR, typename AL>
  size_t substitute(std::basic_string<CH, TR, AL> &str,
                    basic_subst_results<CH, TR, AL> &results, size_type pos,
                    size_type len) const {
    // If your compile breaks here, it is because CI2 is not
    // convertible to type CI. Check the declaration of your rpattern object.
    typedef typename std::basic_string<CH, TR, AL>::const_iterator CI2;
...
public:
  ...
  typedef
      typename basic_rpattern_base<CH const *, SY>::backref_type backref_type;
  typedef typename basic_rpattern_base<CH const *, SY>::backref_vector
      backref_vector;
  ...
  backref_type const &match(CH const *szbegin,
                            basic_match_results_c<CH> &results) const {
    detail::match_param<CH const *> param =
        detail::matcher_helper<CH const *>::init_param(szbegin, 0, results);
    return impl::_do_match(param, szbegin)
               ? results.backref(0)
               : detail::static_init<backref_type>::value;
  }
...
typedef ::regex::detail::select<
    REGEX_FOLD_INSTANTIATIONS &&
        detail::is_convertible<rechar_t const *,
                               restring::const_iterator>::value,
    restring::const_iterator, rechar_t const *>::type lpctstr_t;
...
typedef basic_rpattern<restring::const_iterator, perl_syntax<rechar_t>>
    perl_rpattern;
typedef basic_rpattern<restring::const_iterator, posix_syntax<rechar_t>>
    posix_rpattern;
...
template <typename PAT> class rpattern_destroyer {
  PAT const &m_refPat;
...
#define STATIC_RPATTERN_EX(type, var, params)                                  \
  ...
  static regex::detail::rpattern_destroyer<type> const s_des_##var(s_f_##var,  \
                                                                   var);       \
  if (!s_f_##var) {                                                            \
...
#define STATIC_RPATTERN_EX(type, var, params) static type const var params;
...
inline std::basic_ostream<CH, TR> &operator<<(std::basic_ostream<CH, TR> &sout,
...
namespace std {
template <>
inline void swap(regex::detail::regex_arena &left,
                 regex::detail::regex_arena &right) {
  left.swap(right);
}

template <typename CI, typename SY>
inline void swap(regex::basic_rpattern_base<CI, SY> &left,
                 regex::basic_rpattern_base<CI, SY> &right) {
  left.swap(right);
}
} // namespace std
