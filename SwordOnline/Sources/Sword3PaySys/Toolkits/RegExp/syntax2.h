...
#define REGEX_CHAR(CH, ch)                                                     \
  (static_cast<CH>(                                                            \
      ::regex::detail::literal<CH>::template character<ch, L##ch>::value))

#ifdef _MSC_VER
namespace std {
template <> struct iterator_traits<char *> { // get traits from iterator _Iter
...
template <>
struct iterator_traits<char const *> { // get traits from iterator _Iter
  typedef random_access_iterator_tag iterator_category;
  typedef char value_type;
  typedef ptrdiff_t difference_type;
  typedef difference_type distance_type; // retained
...
template <>
struct iterator_traits<wchar_t *> { // get traits from iterator _Iter
  typedef random_access_iterator_tag iterator_category;
...
template <>
struct iterator_traits<wchar_t const *> { // get traits from iterator _Iter
  typedef random_access_iterator_tag iterator_category;
  typedef wchar_t value_type;
  typedef ptrdiff_t difference_type;
  typedef difference_type distance_type; // retained
...
template <typename T> T const &max(T const &l, T const &r) {
  return _cpp_max(l, r);
}
template <typename T> T const &min(T const &l, T const &r) {
  return _cpp_min(l, r);
}
...
public:
  explicit bad_regexpr(std::string const &s) : std::invalid_argument(s) {}
...
template <> struct literal<char> {
  ...
  template <char ch, wchar_t> struct character {
    enum { value = ch };
  };
};
...
template <> struct literal<wchar_t> {
  ...
  template <char, wchar_t ch> struct character {
    enum { value = ch };
  };
};
...
template <typename CH> struct charset_map_node {
  std::basic_string<CH> m_str;
  charset const *m_rgcharsets[2]; // 0==case, 1==nocase

  charset_map_node() { m_rgcharsets[0] = m_rgcharsets[1] = NULL; }

  charset_map_node(charset_map_node const &node) { *this = node; }
...
template <typename CH> class charset_map {
...
inline bool regex_isspace(char ch) { return 0 != isspace(ch); }

inline bool regex_isspace(wchar_t wch) { return 0 != iswspace(wch); }

} // namespace detail

//
// The perl_syntax class encapsulates the Perl 5 regular expression syntax. It
// is used as a template parameter to basic_rpattern.  To customize regex
// syntax, create your own syntax class and use it as a template parameter
// instead.
//
...
//
// --------------------------------------------------------------------------
template <typename CH> class perl_syntax : protected perl_syntax_base {
public:
  ...
  template <typename CH2> struct rebind {
    typedef perl_syntax<CH2> other;
  };
...
private:
  ...
  const_iterator eat_whitespace(iterator &icur, const_iterator iend) {
    if (m_flags & EXTENDED) {
      while (iend != icur &&
             (REGEX_CHAR(CH, '#') == *icur || detail::regex_isspace(*icur))) {
  ...
  bool min_quant(iterator &icur, const_iterator iend) {
    return (iend != eat_whitespace(++icur, iend) && REGEX_CHAR(CH, '?') == *icur
                ? (++icur, true)
                : false);
  }

public:
  perl_syntax(REGEX_FLAGS flags) : m_flags(flags) {}

  perl_syntax(perl_syntax<CH> const &sy) : m_flags(sy.m_flags) {}

  void set_flags(REGEX_FLAGS flags) { m_flags = flags; }

  REGEX_FLAGS get_flags() const { return m_flags; }
    ...
    case REGEX_CHAR(CH, '['):
      if (REGEX_CHAR(CH, ':') == *(++icur)--) {
        for (size_t i = 0; !tok && i < detail::g_cposix_charsets; ++i) {
          if (detail::is_posix_charset<const_iterator>(
                  icur, iend, detail::g_rgposix_charsets[i].szcharset)) {
        ...
        default:
          ...
          do {
            ...
            if (REGEX_CHAR(CH, '-') == *icur && finclude)
              ...
              m_flags = (REGEX_FLAGS)(finclude ? (m_flags | NOCASE)
                                               : (m_flags & ~NOCASE));
            else if (REGEX_CHAR(CH, 'm') == *icur)
              m_flags = (REGEX_FLAGS)(finclude ? (m_flags | MULTILINE)
                                               : (m_flags & ~MULTILINE));
            else if (REGEX_CHAR(CH, 's') == *icur)
              m_flags = (REGEX_FLAGS)(finclude ? (m_flags | SINGLELINE)
                                               : (m_flags & ~SINGLELINE));
            else if (REGEX_CHAR(CH, 'x') == *icur)
              m_flags = (REGEX_FLAGS)(finclude ? (m_flags | EXTENDED)
                                               : (m_flags & ~EXTENDED));
            else
              break;
  ...
  static bool invalid_charset(CH ch) { return _invalid_charset(ch); }
  static void register_intrinsic_charset(
      CH ch,
      std::basic_string<CH> const &str) // throw( bad_regexpr, std::bad_alloc )
  {
    perl_syntax sy(NOFLAGS);
    if (invalid_charset(ch))
      throw bad_regexpr(
          "invalid character specified to register_intrinsic_charset");
...
template <typename CH> detail::charset_map<CH> perl_syntax<CH>::s_charset_map;
...
//
// --------------------------------------------------------------------------
template <typename CH> class posix_syntax {
  REGEX_FLAGS m_flags;
...
public:
  ...
  typedef CH char_type;
  template <typename CH2> struct rebind {
    typedef posix_syntax<CH2> other;
  };

  posix_syntax(REGEX_FLAGS flags) : m_flags(flags) {}

  posix_syntax(posix_syntax<CH> const &sy) : m_flags(sy.m_flags) {}
  ...
  TOKEN subst_token(iterator &icur, const_iterator iend) {
    ...
    if (REGEX_CHAR(CH, '\\') == *icur) {
      tok = SUBST_ESCAPE;
      ++icur;
      if (iend != icur && REGEX_CHAR(CH, '0') <= *icur &&
          REGEX_CHAR(CH, '9') >= *icur) {
        tok = SUBST_BACKREF;
      }
    }
    return tok;
  }
  TOKEN ext_token(iterator &, const_iterator) { return NO_TOKEN; }
...
private:
  static bool _invalid_charset(char ch) {
    ...
    return NULL !=
           std::char_traits<CH>::find(s_invalid, ARRAYSIZE(s_invalid) - 1, ch);
  }
  static bool _invalid_charset(wchar_t ch) {
    return UCHAR_MAX >= ch && _invalid_charset(static_cast<char>(ch));
  }
...
template <typename CH> detail::charset_map<CH> posix_syntax<CH>::s_charset_map;
