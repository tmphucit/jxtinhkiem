...
template <bool F> struct bool2type {
  enum { value = F };
};
...
//              Passed by reference to all recursive_match_all_ and
//              recursive_match_this routines.
//
...
template <typename CI> struct match_param {
  ...
  match_param(CI _istart, CI _istop, std::vector<backref_tag<CI>> *_prgbackrefs)
      : prgbackrefs(_prgbackrefs), ibegin(_istart), istart(_istart),
        istop(_istop), icur(_istart), pstack(NULL), next(NULL), no0len(false) {}
  match_param(CI _ibegin, CI _istart, CI _istop,
              std::vector<backref_tag<CI>> *_prgbackrefs)
      : prgbackrefs(_prgbackrefs), ibegin(_ibegin), istart(_istart),
        istop(_istop), icur(_istart), pstack(NULL), next(NULL), no0len(false) {}
};
...
//              recursive_match_all_      - match this sub-expression and all
//              following
//                               sub-expression
//
// History:     8/14/2000 - ericne - Created
...
template <typename CI> struct sub_expr_base {
  ...
  virtual bool recursive_match_all_(match_param<CI> &,
                                    CI) const = 0; // throw() = 0;
  virtual bool recursive_match_all_c(match_param<CI> &,
                                     CI) const = 0; // throw() = 0;
  virtual bool
  iterative_match_this_(match_param<CI> &) const = 0; // throw() = 0;
  virtual bool
  iterative_match_this_c(match_param<CI> &) const = 0; // throw() = 0;
  virtual bool
  iterative_rematch_this_(match_param<CI> &) const = 0; // throw() = 0;
  virtual bool
  iterative_rematch_this_c(match_param<CI> &) const = 0; // throw() = 0;
  ...
  // For choosing an appropriate virtual function based on a compile time
  // constant
  bool recursive_match_all_(match_param<CI> &param, CI icur,
                            false_t) const // throw()
  {
...
template <typename CI> inline sub_expr_base<CI>::~sub_expr_base() {}
...
//              get_width               - get min/max nbr chars this pattern can
//              match loops                   - if false, we only need to try to
//              match at 1st position cgroups                 - number of
//              visible groups _cgroups_total          - total number of groups,
//              including hidden ( ?: ) groups get_pat                 - get
//              string representing the pattern get_subst               - get
//              string representing the substitution string get_subst_list - get
//              the list of subst nodes _normalize_string       - perform
//              character escaping
//
// Members:     m_fuses_backrefs        - true if subst string refers to
// backrefs
//              m_floop                 - false if pat only needs to be matched
//              in one place m_cgroups               - total count of groups
...
protected:
  ...
  explicit basic_rpattern_base_impl(
      REGEX_FLAGS flags = NOFLAGS, REGEX_MODE mode = MODE_DEFAULT,
      string_type const &pat = string_type(),
      string_type const &subst = string_type()) // throw()
      : m_arena(DEFAULT_BLOCK_SIZE()), m_fuses_backrefs(false), m_floop(true),
        m_fok_to_recurse(true), m_cgroups(0), m_cgroups_visible(0),
        m_flags(flags), m_mode(mode), m_nwidth(uninit_width()),
        m_pat(new string_type(pat)), m_subst(new string_type(subst)),
        m_subst_list(), m_pfirst(NULL), m_invisible_groups() {}
  ...
  bool m_floop; // false if m_pfirst->recursive_match_all_ only needs to be
                // called once
  ...
  std::auto_ptr<sub_expr_base<CI> const>
      m_pfirst; // first subexpression in pattern
...
template <typename CI> struct matcher_helper {
  ...
  static match_param<CI> init_param(CI ibegin, CI iend,
  ...
  static bool _Do_match_iterative(sub_expr_base<CI> const *expr,
                                  match_param<CI> &param, CI icur, CSTRINGS) {
  ...
  static bool _Do_match_iterative_helper(sub_expr_base<CI> const *expr,
                                         match_param<CI> &param, CI icur) {
    return _Do_match_iterative(expr, param, icur, false_t());
  }

  static bool _Do_match_iterative_helper_c(sub_expr_base<CI> const *expr,
                                           match_param<CI> &param, CI icur) {
    return _Do_match_iterative(expr, param, icur, true_t());
  }

  static bool _Do_match_recursive(sub_expr_base<CI> const *expr,
                                  match_param<CI> &param, CI icur) {
    return expr->recursive_match_all_(param, icur);
  }

  static bool _Do_match_recursive_c(sub_expr_base<CI> const *expr,
                                    match_param<CI> &param, CI icur) {
    return expr->recursive_match_all_c(param, icur);
  }

  static bool _Do_match_impl(rpattern_type const &pat, match_param<CI> &param,
                             bool const use_null) {
    typedef std::list<size_t>::const_iterator LCI;
    typedef bool (*pfndomatch_t)(sub_expr_base<CI> const *expr,
                                 match_param<CI> &param, CI icur);
    ...
    // If the pstack parameter is not NULL, we should do a safe, iterative
    // match. Otherwise, we should do a fast, recursive match.
    ...
    std::fill(param.prgbackrefs->begin(), param.prgbackrefs->end(),
              backref_type());
      ...
      if (!use_null) {
        ...
        if (nwidth.m_min <= (size_t)std::distance(param.istart, param.istop)) {
          ...
          if (RIGHTMOST & flags) {
            ...
            for (CI icur = local_istop; icur >= param.istart;
                 --icur, param.no0len = false)
            ...
            // Continue to the end
            for (CI icur = param.istart; icur <= local_istop;
                 ++icur, param.no0len = false)
    ...
    if ((*param.prgbackrefs)[0].matched) {
      ...
      for (LCI curr = l.begin(), next = l.begin(); curr != l.end();
           curr = next, ++dropped) {
        if (++next == l.end()) {
          std::copy(param.prgbackrefs->begin() + *curr + 1,
      ...
      param.prgbackrefs->resize(param.prgbackrefs->size() -
                                pat.m_invisible_groups.size());
    }

    return (*param.prgbackrefs)[0].matched;
  }

  static bool _Do_match_with_stack(rpattern_type const &pat,
                                   match_param<CI> &param, bool const use_null);

  static bool _Do_match(rpattern_type const &pat, match_param<CI> &param,
                        bool const use_null) {
  ...
  static size_t
  _Do_subst_internal(std::basic_string<CH, TR, AL> &str,
                     basic_subst_results<CH, TR, AL> const &results,
                     rpattern_type const &pat, size_type strpos,
                     size_type strlen) {
    ...
    for (LCI isubst = pat.m_subst_list.begin();
         isubst != pat.m_subst_list.end(); ++isubst) {
      size_t sublen;
      typename std::basic_string<CH, TR, AL>::const_iterator
          itsubpos1; // iter into str
      typename std::basic_string<CH, TR, AL>::const_iterator itsublen1;
      typename std::basic_string<char_type>::const_iterator
          itsubpos2; // iter into subst string
    ...
    // If *first* is still true, then we never called str.replace, and the
    // substitution string is empty. Erase the part of the string that the
    // pattern matched.
    if (first)
  ...
  template <typename CH, typename TR, typename AL>
  static size_t _Do_subst(rpattern_type const &pat,
    ...
    results.m_pbackref_str =
        pat._save_backrefs() ? &(results.m_backref_str = str) : &str;
    ...
    match_param<CI> param(results.ibegin, results.ibegin,
                          &results.m_rgbackrefs);
    ...
    if (GLOBAL & pat.flags()) {
      ...
      while (_Do_match(pat, param, false)) {
        ...
        size_type subst_length = _Do_subst_internal(
            str, results, pat, pos + pos_offset, match_length);
        ...
        std::advance(param.istart,
                     pos); // ineffecient for bidirectional iterators.

        param.istop = results.ibegin;
        std::advance(param.istop,
                     stop_offset); // ineffecient for bidirectional iterators.
      }
      ...
      backref_type const &br = results.m_rgbackrefs[0];
      ++csubst;
      _Do_subst_internal(str, results, pat,
...
template <typename CI>
REGEX_NOINLINE bool matcher_helper<CI>::_Do_match_with_stack(
    rpattern_type const &pat, match_param<CI> &param, bool const use_null) {
  unsafe_stack s;
  param.pstack = &s;
  return _Do_match_impl(pat, param, use_null);
}
...
//
template <typename CH> inline bool regex_isxdigit(CH ch) {
  return (REGEX_CHAR(CH, '0') <= ch && REGEX_CHAR(CH, '9') >= ch) ||
         (REGEX_CHAR(CH, 'a') <= ch && REGEX_CHAR(CH, 'f') >= ch) ||
         (REGEX_CHAR(CH, 'A') <= ch && REGEX_CHAR(CH, 'F') >= ch);
}

template <typename CH> inline int regex_xdigit2int(CH ch) {
...
// --------------------------------------------------------------------------
template <typename CH, typename TR, typename AL>
inline void process_escapes(std::basic_string<CH, TR, AL> &str,
                            bool fPattern) // throw()
{
  ...
  while (npos != (i = str.find(REGEX_CHAR(CH, '\\'), i))) {
    ...
    case REGEX_CHAR(CH, '\\'):
      if (fPattern) {
        if (i + 3 < str.size() && REGEX_CHAR(CH, '\\') == str[i + 2] &&
            REGEX_CHAR(CH, '\\') == str[i + 3])
          str.erase(i, 2);
        ++i;
      } else
        str.erase(i, 1);
    ...
    case REGEX_CHAR(CH, '7'):
      if (!fPattern) {
        ...
        for (; j - i < 4 && j < str.size() && REGEX_CHAR(CH, '0') <= str[j] &&
               REGEX_CHAR(CH, '7') >= str[j];
             ++j)
    ...
    case REGEX_CHAR(CH, 'x'):
      if (!fPattern) {
        CH ch = 0;
        size_t j = i + 2;
        for (; j - i < 4 && j < str.size() && detail::regex_isxdigit(str[j]);
             ++j)
