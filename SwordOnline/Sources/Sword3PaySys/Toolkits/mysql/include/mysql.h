...
#define INTERNAL_NUM_FIELD(f)                                                  \
  (((f)->type <= FIELD_TYPE_INT24 &&                                           \
    ((f)->type != FIELD_TYPE_TIMESTAMP || (f)->length == 14 ||                 \
     (f)->length == 8)) ||                                                     \
...
#endif /* HAVE_OPENSSL */
MYSQL *STDCALL mysql_connect(MYSQL *mysql, const char *host, const char *user,
                             const char *passwd);
...
int STDCALL mysql_send_query(MYSQL *mysql, const char *q, unsigned int length);
...
char *STDCALL mysql_odbc_escape_string(
    MYSQL *mysql, char *to, unsigned long to_length, const char *from,
