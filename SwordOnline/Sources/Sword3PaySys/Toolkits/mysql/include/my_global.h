...
#endif
/* was #if defined(HAVE_LINUXTHREADS) || defined(HAVE_DEC_THREADS) ||
 * defined(HPUX) */
#if !defined(SCO)
...
#define FIX_GCC_LINKING_PROBLEM                                                \
  extern "C" {                                                                 \
  int __cxa_pure_virtual() { return 0; }                                       \
  }
#else
#define FIX_GCC_LINKING_PROBLEM
#endif
...
#define set_bits(type, bit_count)                                              \
  (sizeof(type) * 8 <= (bit_count) ? ~(type)0                                  \
...
#define uint5korr(A)                                                           \
  ((ulonglong)(((uint32)((uchar)(A)[0])) + (((uint32)((uchar)(A)[1])) << 8) +  \
               (((uint32)((uchar)(A)[2])) << 16) +                             \
               (((uint32)((uchar)(A)[3])) << 24)) +                            \
...
#endif /* doubleget */
#define longlongget(V, M)                                                      \
  memcpy_fixed((byte *)&V, (byte *)(M), sizeof(ulonglong))
#define longlongstore(T, V)                                                    \
