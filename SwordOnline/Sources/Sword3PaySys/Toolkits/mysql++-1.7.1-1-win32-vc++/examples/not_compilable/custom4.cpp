...
sql_create_5(stock,
  ...
  try { // its in one big try block
    ...
    cout.setf(ios::left);
    cout << setw(17) << "Item" << setw(4) << "Num" << setw(7) << "Weight"
         << setw(7) << "Price"
         << "Date" << endl
         << endl;
    ...
    for (i = res.begin(); i != res.end(); i++) {
      cout << setw(17) << i->item.c_str() << setw(4)
           << (int)(i->num)
           // VC++ gives an error when using cout << on a long long, so
           // it's cast to an int here
           << setw(7) << i->weight << setw(7) << i->price << i->sdate << endl;
    }

    i = res.find(stock("Hamburger Buns"));
    if (i != res.end())
      cout << "Hamburger Buns found.  Currently " << (int)(i->num)
           << " in stock.\n";
    // VC++ gives an error when using cout << on a long long, so
    // it's cast to an int here
    else
