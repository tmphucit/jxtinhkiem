...
/* Trap all occurences of my_...() in source and use our wrapper around this
 * function */
...
File my_raid_create(const char *FileName, int CreateFlags, int access_flags,
                    ...
                    myf MyFlags);
File my_raid_open(const char *FileName, int Flags, uint raid_type,
                  uint raid_chunks, ulong raid_chunksize, myf MyFlags);
...
private:
  ...
  inline void Calculate() {
    ...
    DBUG_PRINT("info", ("_total_block: %d  this_block: %d  _remaining_bytes:%d",
