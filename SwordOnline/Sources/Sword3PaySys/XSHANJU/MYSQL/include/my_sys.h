...
#define my_b_tell(info)                                                        \
  ((info)->pos_in_file + ((info)->rc_pos - (info)->rc_request_pos))
...
extern File my_create(const char *FileName, int CreateFlags, int AccsesFlags,
                      myf MyFlags);
...
extern gptr _myrealloc(gptr pPtr, uint uSize, const char *sFile, uint uLine,
                       myf MyFlag);
extern gptr my_multi_malloc _VARARGS((myf MyFlags, ...));
extern void _myfree(gptr pPtr, const char *sFile, uint uLine, myf MyFlag);
extern int _sanity(const char *sFile, unsigned int uLine);
extern gptr _my_memdup(const byte *from, uint length, const char *sFile,
...
extern void soundex(my_string out_pntr, my_string in_pntr,
...
extern byte *key_cache_read(File file, my_off_t filepos, byte *buff,
...
extern int my_block_write(IO_CACHE *info, const byte *Buffer, uint Count,
                          my_off_t pos);
...
extern my_bool set_dynamic(DYNAMIC_ARRAY *array, gptr element,
                           uint array_index);
