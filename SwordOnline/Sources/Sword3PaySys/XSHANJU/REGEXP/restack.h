...
// warning C4610: struct can never be instantiated - user defined constructor
// required
#pragma warning(push)
...
public:
  enum {
    value = sizeof(helper) - sizeof(T) < sizeof(T) ? sizeof(helper) - sizeof(T)
                                                   : sizeof(T)
  };
};
...
template <typename T> struct unique_type_info {
  static std::type_info const *const value;

  static bool equals(std::type_info const *const *ppti) {
...
template <bool F, typename T, typename U> class select {
  template <bool> struct select_helper {
    typedef U type;
  };
  template <> struct select_helper<true> {
    typedef T type;
  };
...
#else
// For use in conditional typedefs
template <bool F, typename T, typename U> struct select {
  typedef U type;
};
template <typename T, typename U> struct select<true, T, U> {
  typedef T type;
};
...
template <
    ...
    bool AssumePOD =
        false, // assume non-throwing copy/assign/destroy for better perf
...
class hetero_stack {
public:
  typedef hetero_stack<Alignment, RuntimeTypeCheck, AssumePOD, DynamicBlockSize,
                       StaticBlockSize>
      stack_type;
...
private:
  static void _check_align() {
    ...
    detail::static_assert<
        ...
        Alignment == 16 || Alignment == 32 || Alignment == 128 ||
        Alignment == 256 || Alignment == 512 || Alignment == 1024 ||
        Alignment == 2048 || Alignment == 4096> const align_check;
    (void)align_check;
  }
  ...
  enum {
    DYNAMIC_BLOCK_SIZE = DynamicBlockSize > sizeof(stack_node)
                             ? DynamicBlockSize
                             : sizeof(stack_node)
  };
  ...
  void *_grow(size_t size) // throw(std::bad_alloc)
  {
    ...
    if (m_current_node->m_head.m_next) {
      ...
      m_current = m_end = new_node->m_head.m_current = new_node->m_head.m_end =
          new_node->m_mem + size;
    ...
    size_t new_size = (std::max)(size, (size_t)DYNAMIC_BLOCK_SIZE -
                                           offsetof(stack_node, m_mem));
    stack_node *new_node = static_cast<stack_node *>(
...
public:
  ...
  public:
    type_error(std::type_info const &requested_type,
        ...
        : std::logic_error(s + " (requested type: " + requested_type.name() +
          ...
          prequested_type_(&requested_type), pactual_type_(&actual_type) {}
  ...
  hetero_stack() // throw()
    ...
    m_begin = m_current = m_first_node.m_node.m_head.m_current =
        m_first_node.m_node.m_mem;
    m_end = m_first_node.m_node.m_head.m_end =
        m_first_node.m_buf + sizeof(m_first_node);
  }
  ...
  inline void push(T const &t) // throw(std::bad_alloc,...)
  {
    ...
    detail::static_assert<(Alignment >= detail::alignof<T>::value)> const
        align_test;
    (void)align_test;

    // If T won't throw in copy c'tor, and if RuntimeTypeCheck is false,
    // then we don't need to use an unwinder object.
    typedef typename ::regex::detail::select<
        AssumePOD, // || detail::has_trivial_copy<T>::value,
  ...
  inline void pop(T &t) // throw(...)
  {
    ...
    if (RuntimeTypeCheck) {
      ...
      if (!detail::unique_type_info<T>::equals(
              *static_cast<std::type_info const *const **>(pti)))
        throw type_error(typeid(T),
                         ***static_cast<std::type_info const *const **>(pti));
    }
  ...
  {
    detail::static_assert<RuntimeTypeCheck> const type_check;
    (void)type_check;
    void *pti = m_current - size_of<std::type_info const *const *>::aligned;
  ...
  template <typename T> struct scoped_pop_t : public scoped_pop_base {
