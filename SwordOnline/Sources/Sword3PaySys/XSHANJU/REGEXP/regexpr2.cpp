...
public:
  explicit const_mem_fun1_t(R (Ty::*p)(A) const) : m_p(p) {}
  R operator()(Ty const *p, A arg) const { return (p->*m_p)(arg); }
};
...
// Global critical section used to synchronize the creation of static const
// patterns
...
inline size_t parse_int(II &istr, CI iend, size_t const max_ = unsigned(-1)) {
  ...
  size_t retval = 0;
  while (iend != istr && REGEX_CHAR(CH, '0') <= *istr &&
         REGEX_CHAR(CH, '9') >= *istr && max_ > retval) {
    retval *= 10;
...
REGEXPR_H_INLINE regex_arena::~regex_arena() { deallocate(); }
...
REGEXPR_H_INLINE void *regex_arena::allocate(size_t size) {
  ...
  m_pfirst->m_offset +=
      ((size + alignof<not_pod>::value - 1) & ~(alignof<not_pod>::value - 1));

  return pnew;
}

REGEXPR_H_INLINE size_t regex_arena::max_size() const { return size_t(-1); }
...
template <typename T> inline void regex_destroy(T *pt) {
  pt;
  pt->~T();
}
...
// gets destroyed.
template <typename T> struct regex_allocator {
  ...
  regex_allocator(regex_arena &arena) : m_arena(arena) { align_check(); }
#if !defined(_MSC_VER) | _MSC_VER >= 1300
  regex_allocator(regex_allocator const &alloc) : m_arena(alloc.m_arena) {
    align_check();
  }
#endif
  template <typename U>
  regex_allocator(regex_allocator<U> const &alloc) : m_arena(alloc.m_arena) {
    align_check();
  }
  ...
  void construct(pointer p, T const &t) { new ((void *)p) T(t); }
  void destroy(pointer p) { regex_destroy(p); }
...
private:
  ...
  static void align_check() {
    ...
    static_assert<((size_t)alignof<T>::value <=
                   (size_t)alignof<not_pod>::value)> const align_check;
    (void)align_check;
  }
};

template <typename T, typename U>
inline bool operator==(regex_allocator<T> const &rhs,
                       regex_allocator<U> const &lhs) {
...
template <typename AL> struct allocator_factory {
  template <typename T>
...
template <> struct allocator_factory<regex_allocator<char>> {
  template <typename T> static regex_allocator<T> create(regex_arena &arena) {
    return regex_allocator<T>(arena);
  }
};
#define MAKE_ALLOCATOR(type, arena)                                            \
  allocator_factory<REGEX_ALLOCATOR<char>>::template create<type>(arena)
#endif
...
class ascii_bitvector {
  ...
  enum {
    ...
    CELEMS = (UCHAR_MAX + 1) / CBELEM
  }; // number of element in array
  elem_type m_rg[CELEMS];

  // Used to inline operations like: bv1 |= ~bv2; without creating temp bit
  // vectors.
  struct not_ascii_bitvector {
    ascii_bitvector const &m_ref;
    not_ascii_bitvector(ascii_bitvector const &ref) : m_ref(ref) {}
...
public:
  ascii_bitvector() { zero(); }

  void zero() { memset(m_rg, 0, CELEMS * sizeof(elem_type)); }
...
template <typename A1, typename A2, typename A3> struct charset_t {
  bool m_fcompliment;
  ...
  charset_t(A1 const &a1 = A1(), A2 const &a2 = A2(), A3 const &a3 = A3())
      : m_fcompliment(false), m_fskip_extended_check(false),
        m_ascii_bitvector(), m_posixcharson(0), m_range_vector(a1),
        m_posixcharsoff(a2), m_nestedcharsets(a3) {}
  ...
  charset_t &operator|=(charset const &that) {
    if (that.m_fcompliment) {
      ...
      if (0 == that.m_posixcharson && that.m_posixcharsoff.empty() &&
        ...
        for (VCI prg = that.m_range_vector.begin();
             prg != that.m_range_vector.end(); ++prg) {
      ...
      m_ascii_bitvector |= that.m_ascii_bitvector;
      m_range_vector.insert(m_range_vector.end(), that.m_range_vector.begin(),
                            that.m_range_vector.end());

      m_posixcharson |= that.m_posixcharson;
      std::copy(that.m_posixcharsoff.begin(), that.m_posixcharsoff.end(),
                std::back_inserter(m_posixcharsoff));

      std::copy(that.m_nestedcharsets.begin(), that.m_nestedcharsets.end(),
  ...
  void set_bit_range(char ch1, char ch2, bool const fnocase) {
    ...
    if (fnocase) {
      ...
      for (unsigned int i = static_cast<unsigned char>(ch1);
        ...
        m_ascii_bitvector.set(
            static_cast<unsigned char>(regex_toupper((char)i)));
        m_ascii_bitvector.set(
            static_cast<unsigned char>(regex_tolower((char)i)));
      }
    } else {
      // i is unsigned int to prevent overflow if ch2 is UCHAR_MAX
      for (unsigned int i = static_cast<unsigned char>(ch1);
  ...
  void set_bit_range(wchar_t ch1, wchar_t ch2, bool const fnocase) {
    ...
    if (UCHAR_MAX >= ch1)
      set_bit_range(
          ...
          static_cast<char>((std::min)(static_cast<wchar_t>(UCHAR_MAX), ch2)),
          fnocase);

    if (UCHAR_MAX < ch2)
      add_range(range_type((std::max)(static_cast<wchar_t>(UCHAR_MAX + 1), ch1),
                           ch2));
  }
  ...
  void optimize(type2type<wchar_t>) {
    ...
    m_fskip_extended_check =
        m_posixcharsoff.empty() && m_nestedcharsets.empty();
  }
...
#define DECLARE_EXTENDED_CHECK(FUN, CHAR, CTYPE, PMF)                          \
  ...
  DECLARE_EXTENDED_CHECK(extended_check_narrow, char, _isctype,
                         &charset::in_narrow)
  DECLARE_EXTENDED_CHECK(extended_check_wide_with_case, wchar_t, iswctype,
                         &charset::in_wide_with_case)
  DECLARE_EXTENDED_CHECK(extended_check_wide_no_case, wchar_t, iswctype,
                         &charset::in_wide_no_case)
#undef DECLARE_EXTENDED_CHECK
  ...
  bool in_wide_with_case(wchar_t ch) const {
    ...
    return m_fcompliment !=
           (((UCHAR_MAX >= ch)
                 ? (m_ascii_bitvector[static_cast<unsigned char>(ch)])
                 : ((!m_range_vector.empty() &&
                     in_range_vector_with_case(ch)) ||
  ...
  bool in(char ch, true_t) const { return in_narrow(ch); }

  bool in(char ch, false_t) const { return in_narrow(ch); }

  bool in(wchar_t ch, true_t) const { return in_wide_with_case(ch); }

  bool in(wchar_t ch, false_t) const { return in_wide_no_case(ch); }
...
// Intrinsic character sets are allocated on the heap with the standard
// allocator. They are either the built-in character sets, or the user-defined
// ones.
struct charset
    : public charset_t<std::allocator<range_type>, std::allocator<wctype_t>,
...
REGEXPR_H_INLINE void free_charset(charset const *pcharset) { delete pcharset; }
...
struct custom_charset
    : public charset_t<REGEX_ALLOCATOR<range_type>, REGEX_ALLOCATOR<wctype_t>,
...
template <typename CH> class intrinsic_charsets {
...
public:
  static charset const &get_word_charset() { return _get_word_charset(); }
  static charset const &get_digit_charset() { return _get_digit_charset(); }
  static charset const &get_space_charset() { return _get_space_charset(); }
...
template <typename CSTRINGS> struct bol_t {
  ...
  template <typename U> struct rebind {
    typedef bol_t<U> other;
  };
};

// Evaluates end-of-string condition for string's
template <typename CSTRINGS> struct eos_t {
  template <typename CI>
  static bool eval(match_param<CI> const &param, CI iter) {
    return param.istop == iter;
  }
  template <typename U> struct rebind {
    typedef eos_t<U> other;
  };
};
template <> struct eos_t<true_t> {
  template <typename CI> static bool eval(match_param<CI> const &, CI iter) {
    typedef typename std::iterator_traits<CI>::value_type CH;
    typedef std::char_traits<CH> traits_type;
...
// newline character.
template <typename CSTRINGS> struct eol_t {
  template <typename CI>
  static bool eval(match_param<CI> const &param, CI iter) {
    typedef typename std::iterator_traits<CI>::value_type CH;
    typedef std::char_traits<CH> traits_type;

    return param.istop == iter || traits_type::eq(REGEX_CHAR(CH, '\n'), *iter);
  }
  template <typename U> struct rebind {
    typedef eol_t<U> other;
  };
};
template <> struct eol_t<true_t> {
  template <typename CI> static bool eval(match_param<CI> const &, CI iter) {
    typedef typename std::iterator_traits<CI>::value_type CH;
    typedef std::char_traits<CH> traits_type;
...
// newline character followed by end of string. ( Only used by $ and /Z
// assertions )
template <typename CSTRINGS> struct peos_t {
  template <typename CI>
  static bool eval(match_param<CI> const &param, CI iter) {
    typedef typename std::iterator_traits<CI>::value_type CH;
    typedef std::char_traits<CH> traits_type;
  ...
  template <typename U> struct rebind {
    typedef peos_t<U> other;
  };
};
template <> struct peos_t<true_t> {
  template <typename CI> static bool eval(match_param<CI> const &, CI iter) {
    typedef typename std::iterator_traits<CI>::value_type CH;
    typedef std::char_traits<CH> traits_type;
...
template <typename CH> struct ch_neq_t {
...
template <typename CH> struct ch_neq_nocase_t {
  typedef CH char_type;
  typedef std::char_traits<char_type> traits_type;
...
inline width_type operator+(width_type const &rhs, width_type const &lhs) {
  width_type width = {width_add(rhs.m_min, lhs.m_min),
                      width_add(rhs.m_max, lhs.m_max)};
  return width;
}

inline width_type operator*(width_type const &rhs, width_type const &lhs) {
  width_type width = {width_mult(rhs.m_min, lhs.m_min),
                      width_mult(rhs.m_max, lhs.m_max)};
  return width;
}
...
template <typename CI> struct width_param {
  int cookie;
  ...
  width_param(int cookie, width_type const &total_width,
              std::vector<match_group_base<CI> *> &rggroups,
              std::list<size_t> const &invisible_groups)
      : cookie(cookie), total_width(total_width), rggroups(rggroups),
...
//              recursive_match_this_ - does this sub_expr match at the given
//              location width_this           - what is the width of this
//              sub_expr ~sub_expr            - recursively delete the sub_expr
//              graph next                 - pointer to the next node in the
//              graph next                 - pointer to the next node in the
//              graph recursive_match_next_ - match the rest of the graph
//              recursive_match_all_  - recursive_match_this_ and
//              recursive_match_next_ is_assertion         - true if this
//              sub_expr is a zero-width assertion get_width            - find
//              the width of the graph at this sub_expr
//
...
//
// --------------------------------------------------------------------------
template <typename CI> class sub_expr : public sub_expr_base<CI> {
  sub_expr *m_pnext;

protected:
  // Only derived classes can instantiate sub_expr's
  sub_expr() : m_pnext(NULL) {}
...
public:
  ...
  virtual ~sub_expr() { delete m_pnext; }

  sub_expr **pnext() { return &m_pnext; }

  sub_expr const *next() const { return m_pnext; }
  ...
  // If recursive_match_all_ returns false, it must not change any of param's
  // state
  virtual bool recursive_match_all_(match_param<CI> &param, CI icur) const {
    return (recursive_match_this_(param, icur) &&
            recursive_match_next_(param, icur, false_t()));
  }

  virtual bool recursive_match_all_c(match_param<CI> &param,
                                     CI icur) const // for C-style strings
  {
    return (recursive_match_this_c(param, icur) &&
            recursive_match_next_(param, icur, true_t()));
  }
  ...
  virtual bool recursive_match_this_c(match_param<CI> &,
                                      CI &) const // for C-style strings
  {
    return true;
  }
  ...
  virtual bool
  iterative_match_this_c(match_param<CI> &param) const // for C-style strings
  {
    param.next = next();
    return true;
  }
  ...
  virtual bool
  iterative_rematch_this_c(match_param<CI> &) const // for C-style strings
  {
    return false;
  }

  virtual bool is_assertion() const { return false; }
...
template <typename CI> class assertion : public sub_expr<CI> {
public:
  typedef typename sub_expr<CI>::const_iterator const_iterator;
  typedef typename sub_expr<CI>::char_type char_type;

  virtual bool is_assertion() const { return true; }
  virtual width_type width_this(width_param<CI> &) { return zero_width; }
};

template <typename CI, typename OP> class assert_op : public assertion<CI> {
public:
  virtual bool recursive_match_all_(match_param<CI> &param, CI icur) const {
    return (assert_op::recursive_match_this_(param, icur) &&
            recursive_match_next_(param, icur, false_t()));
  }
  virtual bool recursive_match_all_c(match_param<CI> &param, CI icur) const {
    return (assert_op::recursive_match_this_c(param, icur) &&
            recursive_match_next_(param, icur, true_t()));
  }
...
public:
  match_wrapper(SUB_EXPR *psub) : m_psub(psub) {}

  virtual ~match_wrapper() { _cleanup(); }
...
//     __forceinline static bool iterative_match_this_( SUB_EXPR const * psub,
//     match_param<CI> & param )
//     {
...
#define DECLARE_RECURSIVE_MATCH_ALL(cstrings, ext)                             \
  virtual bool recursive_match_all##ext(match_param<CI> &param, CI icur)       \
      const {                                                                  \
    ...
    if (m_ubound &&                                                            \
        m_psub->SUB_EXPR::recursive_match_this##ext(param, icur)) {            \
  ...
  virtual bool iterative_match_this##ext(match_param<CI> &param) const {       \
    ...
    if (m_ubound && m_psub->SUB_EXPR::iterative_match_this##ext(param)) {      \
      if (0 == std::distance(istart, param.icur)) {                            \
        ...
        while (++cmatches < m_ubound &&                                        \
               m_psub->SUB_EXPR::iterative_match_this##ext(param))             \
          ;                                                                    \
      }                                                                        \
    }                                                                          \
...
#define DECLARE_ITERATIVE_REMATCH_THIS(ext)                                    \
  virtual bool iterative_rematch_this##ext(match_param<CI> &param) const {     \
    size_t &cmatches =                                                         \
        param.pstack->top(static_init<std::pair<CI, size_t>>::value).second;   \
    if (m_lbound != cmatches) {                                                \
      --cmatches;                                                              \
  ...
  //         {
  //             while( ++cmatches < m_ubound &&
  //             inliner<CSTRINGS>::iterative_match_this_( m_psub, param ) );
  //         }
  //     }
  //     if( cmatches >= m_lbound )
  //     {
...
template <typename CI> class match_char : public sub_expr<CI> {
  match_char &operator=(match_char const &);

public:
  typedef typename sub_expr<CI>::char_type char_type;

  match_char(char_type const &ch) : m_ch(ch) {}
...
template <typename CI> class match_char_t : public match_char<CI> {
  match_char_t &operator=(match_char_t const &);

public:
  typedef typename match_char<CI>::char_type char_type;

  match_char_t(char_type ch) : match_char<CI>(ch) {}

  virtual sub_expr<CI> *quantify(size_t lbound, size_t ubound, bool greedy,
                                 regex_arena &arena) {
    if (greedy)
      return new (arena)
          max_atom_quantifier<CI, match_char_t<CI>>(this, lbound, ubound);
    else
      return new (arena)
          min_atom_quantifier<CI, match_char_t<CI>>(this, lbound, ubound);
  }
  virtual bool recursive_match_all_(match_param<CI> &param, CI icur) const {
    return (match_char_t::recursive_match_this_(param, icur) &&
            recursive_match_next_(param, icur, false_t()));
  }
  virtual bool recursive_match_all_c(match_param<CI> &param, CI icur) const {
    return (match_char_t::recursive_match_this_c(param, icur) &&
            recursive_match_next_(param, icur, true_t()));
  }
  virtual bool recursive_match_this_(match_param<CI> &param, CI &icur) const {
...
template <typename CI> class match_char_nocase_t : public match_char<CI> {
...
public:
  ...
  virtual sub_expr<CI> *quantify(size_t lbound, size_t ubound, bool greedy,
                                 regex_arena &arena) {
    if (greedy)
      return new (arena) max_atom_quantifier<CI, match_char_nocase_t<CI>>(
          this, lbound, ubound);
    else
      return new (arena) min_atom_quantifier<CI, match_char_nocase_t<CI>>(
          this, lbound, ubound);
  }
  virtual bool recursive_match_all_(match_param<CI> &param, CI icur) const {
    return (match_char_nocase_t::recursive_match_this_(param, icur) &&
            recursive_match_next_(param, icur, false_t()));
  }
  virtual bool recursive_match_all_c(match_param<CI> &param, CI icur) const {
    return (match_char_nocase_t::recursive_match_this_c(param, icur) &&
            recursive_match_next_(param, icur, true_t()));
  }
  virtual bool recursive_match_this_(match_param<CI> &param, CI &icur) const {
    return _do_match_this(param, icur, false_t());
...
private:
  ...
  bool _do_match_this(match_param<CI> &param, CI &icur, CSTRINGS) const {
    if (eos_t<CSTRINGS>::eval(param, icur) ||
        (!traits_type::eq(*icur, m_ch) && !traits_type::eq(*icur, m_ch_lower)))
      return false;
    ++icur;
    return true;
  }
...
template <typename CI>
inline match_char<CI> *
create_char(typename std::iterator_traits<CI>::value_type ch, REGEX_FLAGS flags,
            regex_arena &arena) {
  typedef typename std::iterator_traits<CI>::value_type CH;
  typedef std::char_traits<CH> traits_type;
...
template <typename CI> class match_literal : public sub_expr<CI> {
...
public:
  ...
  match_literal(const_iterator istart, const_iterator istop)
      : m_istart(istart), m_istop(istop),
        m_dist(std::distance(m_istart, m_istop)) {}
...
template <typename CI> class match_literal_t : public match_literal<CI> {
...
public:
  ...
  virtual sub_expr<CI> *quantify(size_t lbound, size_t ubound, bool greedy,
    ...
    if (greedy)
      return new (arena)
          max_atom_quantifier<CI, match_literal_t<CI>>(this, lbound, ubound);
    else
      return new (arena)
          min_atom_quantifier<CI, match_literal_t<CI>>(this, lbound, ubound);
  }
  virtual bool recursive_match_all_(match_param<CI> &param, CI icur) const {
    return (match_literal_t::recursive_match_this_(param, icur) &&
            recursive_match_next_(param, icur, false_t()));
  }
  virtual bool recursive_match_all_c(match_param<CI> &param, CI icur) const {
    return (match_literal_t::recursive_match_this_c(param, icur) &&
            recursive_match_next_(param, icur, true_t()));
  }
  virtual bool recursive_match_this_(match_param<CI> &param, CI &icur) const {
    return _do_match_this(param, icur, false_t());
...
template <typename CI> class match_literal_nocase_t : public match_literal<CI> {
...
public:
  ...
  match_literal_nocase_t(iterator istart, const_iterator istop,
                         regex_arena &arena)
      : match_literal<CI>(istart, istop),
    ...
    regex_tolower(m_szlower,
                  static_cast<char_type const *>(m_szlower + m_dist));
  }

  virtual sub_expr<CI> *quantify(size_t lbound, size_t ubound, bool greedy,
                                 regex_arena &arena) {
    if (greedy)
      return new (arena) max_atom_quantifier<CI, match_literal_nocase_t<CI>>(
          this, lbound, ubound);
    else
      return new (arena) min_atom_quantifier<CI, match_literal_nocase_t<CI>>(
          this, lbound, ubound);
  }
  virtual bool recursive_match_all_(match_param<CI> &param, CI icur) const {
    return (match_literal_nocase_t::recursive_match_this_(param, icur) &&
            recursive_match_next_(param, icur, false_t()));
  }
  virtual bool recursive_match_all_c(match_param<CI> &param, CI icur) const {
    return (match_literal_nocase_t::recursive_match_this_c(param, icur) &&
            recursive_match_next_(param, icur, true_t()));
  }
  virtual bool recursive_match_this_(match_param<CI> &param, CI &icur) const {
    return _do_match_this(param, icur, false_t());
...
inline sub_expr<CI> *create_literal(II1 istart, II2 istop, REGEX_FLAGS flags,
                                    regex_arena &arena) {
...
template <typename CI> class match_any : public sub_expr<CI> {
public:
  virtual width_type width_this(width_param<CI> &) {
    width_type width = {1, 1};
    return width;
...
template <typename CI, typename EOS> class match_any_t : public match_any<CI> {
  template <typename CSTRINGS>
...
public:
  virtual sub_expr<CI> *quantify(size_t lbound, size_t ubound, bool greedy,
    ...
    if (greedy)
      return new (arena)
          max_atom_quantifier<CI, match_any_t<CI, EOS>>(this, lbound, ubound);
    else
      return new (arena)
          min_atom_quantifier<CI, match_any_t<CI, EOS>>(this, lbound, ubound);
  }

  virtual bool recursive_match_all_(match_param<CI> &param, CI icur) const {
    return (match_any_t::recursive_match_this_(param, icur) &&
            recursive_match_next_(param, icur, false_t()));
  }
  virtual bool recursive_match_all_c(match_param<CI> &param, CI icur) const {
    return (match_any_t::recursive_match_this_c(param, icur) &&
            recursive_match_next_(param, icur, true_t()));
  }
  virtual bool recursive_match_this_(match_param<CI> &param, CI &icur) const {
    return _do_match_this(param, icur, false_t());
...
template <typename CI> class match_charset : public sub_expr<CI> {
public:
  virtual width_type width_this(width_param<CI> &) {
    width_type width = {1, 1};
    return width;
...
public:
  match_charset_t(charset const &cs) : m_cs(cs) {}

  virtual sub_expr<CI> *quantify(size_t lbound, size_t ubound, bool greedy,
                                 regex_arena &arena) {
    if (greedy)
      return new (arena) max_atom_quantifier<CI, match_charset_t<CI, CASE>>(
          this, lbound, ubound);
    else
      return new (arena) min_atom_quantifier<CI, match_charset_t<CI, CASE>>(
          this, lbound, ubound);
  }
  virtual bool recursive_match_all_(match_param<CI> &param, CI icur) const {
    return (match_charset_t::recursive_match_this_(param, icur) &&
            recursive_match_next_(param, icur, false_t()));
  }
  virtual bool recursive_match_all_c(match_param<CI> &param, CI icur) const {
    return (match_charset_t::recursive_match_this_c(param, icur) &&
            recursive_match_next_(param, icur, true_t()));
  }
  virtual bool recursive_match_this_(match_param<CI> &param, CI &icur) const {
    return _do_match_this(param, icur, false_t());
...
class match_custom_charset_t : public match_charset<CI> {
  std::auto_ptr<custom_charset const>
      m_pcs; // ptr to a custom charset alloc'ed in regex arena

  template <typename CSTRINGS>
  bool _do_match_this(match_param<CI> &param, CI &icur, CSTRINGS) const {
    if (eos_t<CSTRINGS>::eval(param, icur) ||
        !m_pcs->in(*icur, bool2type<CASE>()))
      return false;
    ++icur;
    return true;
  }
...
public:
  match_custom_charset_t(custom_charset const *pcs) : m_pcs(pcs) {}

  virtual sub_expr<CI> *quantify(size_t lbound, size_t ubound, bool greedy,
                                 regex_arena &arena) {
    if (greedy)
      return new (arena)
          max_atom_quantifier<CI, match_custom_charset_t<CI, CASE>>(
              this, lbound, ubound);
    else
      return new (arena)
          min_atom_quantifier<CI, match_custom_charset_t<CI, CASE>>(
              this, lbound, ubound);
  }
  virtual bool recursive_match_all_(match_param<CI> &param, CI icur) const {
    return (match_custom_charset_t::recursive_match_this_(param, icur) &&
            recursive_match_next_(param, icur, false_t()));
  }
  virtual bool recursive_match_all_c(match_param<CI> &param, CI icur) const {
    return (match_custom_charset_t::recursive_match_this_c(param, icur) &&
            recursive_match_next_(param, icur, true_t()));
  }
  virtual bool recursive_match_this_(match_param<CI> &param, CI &icur) const {
    return _do_match_this(param, icur, false_t());
...
template <typename CI>
inline match_charset<CI> *
create_charset(charset const &charset, REGEX_FLAGS flags, regex_arena &arena) {
  switch (NOCASE & flags) {
  case 0:
...
template <typename CI>
inline match_charset<CI> *create_custom_charset(custom_charset const *pcharset,
                                                REGEX_FLAGS flags,
                                                regex_arena &arena) {
  switch (NOCASE & flags) {
  case 0:
...
template <typename CI> class word_assertion_t : public assertion<CI> {
...
template <typename CI> class word_boundary_t : public word_assertion_t<CI> {
  word_boundary_t &operator=(word_boundary_t const &);

  template <typename CSTRINGS>
  bool _do_match_this(match_param<CI> &param, CI icur, CSTRINGS) const {
    bool const fthisword =
        !eos_t<CSTRINGS>::eval(param, icur) && m_isword.in(*icur, true_t());
    bool const fprevword =
        !bos_t<CSTRINGS>::eval(param, icur) && m_isword.in(*--icur, true_t());

    return (m_fisboundary == (fprevword != fthisword));
  }

public:
  word_boundary_t(bool const fisboundary) : m_fisboundary(fisboundary) {}

  virtual bool recursive_match_all_(match_param<CI> &param, CI icur) const {
    return (word_boundary_t::recursive_match_this_(param, icur) &&
            recursive_match_next_(param, icur, false_t()));
  }
  virtual bool recursive_match_all_c(match_param<CI> &param, CI icur) const {
    return (word_boundary_t::recursive_match_this_c(param, icur) &&
            recursive_match_next_(param, icur, true_t()));
  }
  virtual bool recursive_match_this_(match_param<CI> &param, CI &icur) const {
    return _do_match_this(param, icur, false_t());
...
template <typename CI> class word_start_t : public word_assertion_t<CI> {
...
public:
  virtual bool recursive_match_all_(match_param<CI> &param, CI icur) const {
    return (word_start_t::recursive_match_this_(param, icur) &&
            recursive_match_next_(param, icur, false_t()));
  }
  virtual bool recursive_match_all_c(match_param<CI> &param, CI icur) const {
    return (word_start_t::recursive_match_this_c(param, icur) &&
            recursive_match_next_(param, icur, true_t()));
  }
  virtual bool recursive_match_this_(match_param<CI> &param, CI &icur) const {
    return _do_match_this(param, icur, false_t());
...
template <typename CI> class word_stop_t : public word_assertion_t<CI> {
...
public:
  virtual bool recursive_match_all_(match_param<CI> &param, CI icur) const {
    return (word_stop_t::recursive_match_this_(param, icur) &&
            recursive_match_next_(param, icur, false_t()));
  }
  virtual bool recursive_match_all_c(match_param<CI> &param, CI icur) const {
    return (word_stop_t::recursive_match_this_c(param, icur) &&
            recursive_match_next_(param, icur, true_t()));
  }
  virtual bool recursive_match_this_(match_param<CI> &param, CI &icur) const {
    return _do_match_this(param, icur, false_t());
...
template <typename CI>
inline assertion<CI> *create_word_boundary(bool const fisboundary, REGEX_FLAGS,
                                           regex_arena &arena) {
...
template <typename CI> class match_group_base : public sub_expr<CI> {
protected:
  typedef std::list<sub_expr<CI> *, REGEX_ALLOCATOR<sub_expr<CI> *>>
      alt_list_type;
...
private:
  ...
  bool _recursive_match_all(match_param<CI> &param, CI icur, CSTRINGS) const {
    ...
    if (size_t(-1) != m_cgroup) // could be -1 if this is a lookahead_assertion
    {
      ...
      for (LCI ialt = m_rgalternates.begin(); ialt != m_rgalternates.end();
           ++ialt) {
...
public:
  ...
  void end_alternate() { *m_pptail = _get_end_group(); }
  ...
  virtual width_type width_this(width_param<CI> &param) {
    ...
    for (LCI ialt = m_rgalternates.begin();
         worst_width != width && ialt != m_rgalternates.end(); ++ialt) {
...
template <typename CI> inline match_group_base<CI>::~match_group_base() {}
...
template <typename CI> class match_group : public match_group_base<CI> {
  match_group &operator=(match_group const &);

public:
  match_group(size_t cgroup, regex_arena &arena)
      : match_group_base<CI>(cgroup, arena), m_end_group(this) {}

  virtual ~match_group() { _cleanup(); }
...
protected:
  ...
  bool _call_back(match_param<CI> &param, CI icur, CSTRINGS) const {
    if (size_t(-1) != m_cgroup) {
      ...
      (*param.prgbackrefs)[m_cgroup].first =
          (*param.prgbackrefs)[m_cgroup].reserved1;
  ...
  public:
    end_group(match_group<CI> const *pgroup = NULL) : m_pgroup(pgroup) {}
    virtual bool recursive_match_all_(match_param<CI> &param, CI icur) const {
  ...
  virtual sub_expr<CI> *_get_end_group() { return &m_end_group; }
};

template <typename CI>
inline void save_backrefs(std::vector<backref_tag<CI>> const &rgbackrefs,
                          CI *prgci) {
  typedef typename std::vector<backref_tag<CI>>::const_iterator VI;
  for (VI iter = rgbackrefs.begin(); iter != rgbackrefs.end();
       ++iter, ++prgci) {
...
template <typename CI>
inline void restore_backrefs(std::vector<backref_tag<CI>> &rgbackrefs,
                             CI *prgci) {
...
template <typename CI> class group_wrapper : public sub_expr<CI> {
  match_group_base<CI> const *const m_pgroup;

  group_wrapper &operator=(group_wrapper const &);

public:
  group_wrapper(match_group_base<CI> const *pgroup) : m_pgroup(pgroup) {}
  virtual bool iterative_match_this_(match_param<CI> &param) const {
...
class independent_group_base : public match_group_base<CI> {
  ...
  bool _recursive_match_all(match_param<CI> &param, CI icur, CSTRINGS) const {
    ...
    if (m_extent.second) {
      prgbr = static_cast<backref_tag<CI> *>(
          alloca(m_extent.second * sizeof(backref_tag<CI>)));
      std::copy(
          param.prgbackrefs->begin() + m_extent.first,
    ...
    // if match_group::recursive_match_all_ returned true, the backrefs must be
    // restored
    if (m_extent.second && fdomatch)
      std::copy(prgbr, prgbr + m_extent.second,
                param.prgbackrefs->begin() + m_extent.first);
  ...
  bool _iterative_match_this(match_param<CI> &param, CSTRINGS) const {
    ...
    bool const fdomatch = matcher_helper<CI>::_Do_match_iterative(
        &expr, param, param.icur, CSTRINGS());
...
public:
  independent_group_base(size_t cgroup, regex_arena &arena)
      : match_group_base<CI>(cgroup, arena), m_fexpected(true), m_extent(0, 0) {
  }
  virtual void set_extent(extent const &ex) { m_extent = ex; }
  virtual bool recursive_match_all_(match_param<CI> &param, CI icur) const {
    return _recursive_match_all(param, icur, false_t());
  }
  virtual bool recursive_match_all_c(match_param<CI> &param, CI icur) const {
...
public:
  ...
  virtual ~independent_group() { _cleanup(); }
...
protected:
  independent_group(bool const fexpected, regex_arena &arena)
      : independent_group_base<CI>(fexpected, arena), m_end_group(this) {}
  ...
  public:
    end_group(independent_group<CI> const *pgroup = NULL) : m_pgroup(pgroup) {}
    virtual bool recursive_match_all_(match_param<CI> &param, CI icur) const {
...
class lookbehind_assertion : public independent_group_base<CI> {
  ...
  bool _recursive_match_all(match_param<CI> &param, CI icur, CSTRINGS) const {
    ...
    if (m_nwidth.m_min > room)
      return m_fexpected ? false
                         : recursive_match_next_(param, icur, CSTRINGS());
    ...
    match_param<CI> local_param(param.ibegin, param.istart, icur,
                                param.prgbackrefs);

    // Find the rightmost match that ends at icur.
    for (CI local_icur = local_istart; local_icur != local_istop;
         ++local_icur) {
      // Match until the end of this group and then return
      // Note that we're calling recursive_match_all_ regardless of the CSTRINGS
      // switch. This is because for the lookbehind assertion, the termination
      // condition is when icur == param.iend, not when *icur == '\0'
      bool const fmatched =
          match_group_base<CI>::recursive_match_all_(local_param, local_icur);
      ...
      // if match_group::recursive_match_all_ returned true, the backrefs must
      // be restored
      if (fmatched) {
        if (m_extent.second)
          std::copy(prgbr, prgbr + m_extent.second,
  ...
  bool _iterative_match_this(match_param<CI> &param, CSTRINGS) const {
    ...
    // Create a local param struct that has icur as param.iend
    match_param<CI> local_param(param.ibegin, param.istart, param.icur,
                                param.prgbackrefs);
    ...
    for (CI local_icur = local_istart; local_icur != local_istop;
         ++local_icur) {
      // Match until the end of this group and then return
      // Note that we're calling _Do_match_iterative_helper regardless of the
      // CSTRINGS switch. This is because for the lookbehind assertion, the
      // termination condition is when icur == param.iend, not when *icur ==
      // '\0'
      bool const fmatched = matcher_helper<CI>::_Do_match_iterative_helper(
          &expr, local_param, local_icur);
...
public:
  ...
  virtual ~lookbehind_assertion() { _cleanup(); }
...
template <typename CI> class group_quantifier : public match_quantifier<CI> {
...
public:
  group_quantifier(match_group_base<CI> *psub, size_t lbound, size_t ubound,
                   sub_expr<CI> *pend_quant)
      : match_quantifier<CI>(psub, lbound, ubound), m_group(*psub) {
...
template <typename CI> inline group_quantifier<CI>::~group_quantifier() {}
...
inline void fixup_backref(size_t &cbackref,
                          std::list<size_t> const &invisible_groups) {
...
template <typename CI> class match_backref : public sub_expr<CI> {
...
public:
  match_backref(size_t cbackref) : m_cbackref(cbackref) {}
...
public:
  match_backref_t(size_t cbackref) : match_backref<CI>(cbackref) {}
  virtual sub_expr<CI> *quantify(size_t lbound, size_t ubound, bool greedy,
                                 regex_arena &arena) {
    if (greedy)
      return new (arena) max_atom_quantifier<CI, match_backref_t<CMP, CI>>(
          this, lbound, ubound);
    else
      return new (arena) min_atom_quantifier<CI, match_backref_t<CMP, CI>>(
          this, lbound, ubound);
  }
  virtual bool recursive_match_all_(match_param<CI> &param, CI icur) const {
    return (match_backref_t::recursive_match_this_(param, icur) &&
            recursive_match_next_(param, icur, false_t()));
  }
  virtual bool recursive_match_all_c(match_param<CI> &param, CI icur) const {
    return (match_backref_t::recursive_match_this_c(param, icur) &&
            recursive_match_next_(param, icur, true_t()));
  }
  virtual bool recursive_match_this_(match_param<CI> &param, CI &icur) const {
    return _do_match_this(param, icur, false_t());
...
template <typename CI>
inline match_backref<CI> *create_backref(size_t cbackref, REGEX_FLAGS flags,
                                         regex_arena &arena) {
  typedef typename std::iterator_traits<CI>::value_type char_type;
  ...
  case NOCASE:
    return new (arena)
        match_backref_t<ch_neq_nocase_t<char_type>, CI>(cbackref);
  default:
    __assume(0); // tells the compiler that this is unreachable
  }
}

template <typename CI> class match_recurse : public sub_expr<CI> {
  ...
  template <typename CSTRINGS>
  bool _recursive_match_all(match_param<CI> &param, CI icur, CSTRINGS) const {
    // Prevent infinite recursion. If icur == ( *param.prgbackrefs )[ 0
    // ].reserved1, then the pattern has eaten 0 chars to date, and we would
    // recurse forever.
    ...
    CI *prgci =
        static_cast<CI *>(alloca(param.prgbackrefs->size() * sizeof(CI)));
    ...
    if (param.first->recursive_match_all_(param, icur, CSTRINGS())) {
      ...
      return recursive_match_next_(param, (*param.prgbackrefs)[0].second,
                                   CSTRINGS());
    }
  ...
  bool _iterative_match_this(match_param<CI> &param, CSTRINGS) const {
    ...
    if (matcher_helper<CI>::_Do_match_iterative(param.first, param, param.icur,
                                                CSTRINGS())) {
      _pop_frame(param);
      param.next = next();
      return true;
...
public:
  ...
  virtual width_type width_this(width_param<CI> &param) {
    ...
    this_width.m_max =
        width_mult(this_width.m_max, size_t(-1)); // could recurse forever
    return this_width;
  }
};
...
template <typename CI> struct backref_condition {
  size_t m_cbackref;

  backref_condition(size_t cbackref) : m_cbackref(cbackref) {}

  template <typename CSTRINGS>
  bool recursive_match_this_(match_param<CI> &param, CI, CSTRINGS) const {
    return m_cbackref < param.prgbackrefs->size() &&
           (*param.prgbackrefs)[m_cbackref].matched;
  }
  template <typename CSTRINGS>
  bool iterative_match_this_(match_param<CI> &param, CSTRINGS) const {
    return m_cbackref < param.prgbackrefs->size() &&
...
template <typename CI> struct assertion_condition {
  std::auto_ptr<match_group_base<CI>> m_passert;

  assertion_condition(match_group_base<CI> *passert) : m_passert(passert) {}
...
private:
  ...
  bool _recursive_match_all(match_param<CI> &param, CI icur, CSTRINGS) const {
    ...
    if (m_condition.recursive_match_this_(param, icur, CSTRINGS()) ||
        ++ialt != m_rgalternates.end()) {
...
public:
  ...
  match_conditional(size_t cgroup, condition_type condition, regex_arena &arena)
      : match_group<CI>(cgroup, arena), m_condition(condition) {}
...
template <typename CI>
inline match_conditional<CI, backref_condition<CI>> *
create_backref_conditional(size_t cgroup, size_t cbackref, regex_arena &arena) {
  backref_condition<CI> cond(cbackref);
  return new (arena)
      match_conditional<CI, backref_condition<CI>>(cgroup, cond, arena);
}

template <typename CI>
inline match_conditional<CI, assertion_condition<CI>> *
create_assertion_conditional(size_t cgroup, match_group_base<CI> *passert,
                             regex_arena &arena) {
  assertion_condition<CI> cond(passert);
  return new (arena)
      match_conditional<CI, assertion_condition<CI>>(cgroup, cond, arena);
}
...
template <typename AL> struct skip_dtor_calls {
  enum { value = false };
};
template <> struct skip_dtor_calls<regex_allocator<char>> {
  enum { value = true };
};
...
template <typename CI>
REGEXPR_H_INLINE bool
basic_rpattern_base_impl<CI>::_do_match(match_param<CI> &param,
                                        bool use_null) const {
  if (GLOBAL & flags()) // do a global find
  {
    ...
    while (matcher_helper<CI>::_Do_match(*this, param, use_null)) {
      ...
      if (fFirst)
        ...
        rgtempbackrefs.insert(rgtempbackrefs.end(), param.prgbackrefs->begin(),
...
template <typename CI>
REGEXPR_H_INLINE size_t basic_rpattern_base_impl<CI>::_do_count(
    match_param<CI> &param, bool use_null) const {
  size_t cmatches = 0;
...
template <typename CI>
REGEXPR_H_INLINE size_t basic_rpattern_base_impl<CI>::_do_count(
    match_param<CI> &param, char_type const *szbegin) const {
  if (RIGHTMOST & flags()) {
    // We need to know the end of the string if we're doing a
    // RIGHTMOST count
    param.istop = param.istart;
...
struct deallocation_helper {
  deallocation_helper(regex_arena &arena)
      : m_arena(arena), m_fparse_successful(false) {}
  ...
  void dismiss() { m_fparse_successful = true; }
...
REGEXPR_H_INLINE void basic_rpattern_base<CI, SY>::init(string_type const &pat,
                                                        REGEX_FLAGS flags,
                                                        REGEX_MODE mode) {
...
template <typename CI, typename SY>
REGEXPR_H_INLINE void
basic_rpattern_base<CI, SY>::init(string_type const &pat,
                                  string_type const &subst, REGEX_FLAGS flags,
                                  REGEX_MODE mode) {
...
template <typename CI, typename SY>
REGEXPR_H_INLINE void
basic_rpattern_base<CI, SY>::_common_init(REGEX_FLAGS flags) {
  m_cgroups = 0;
  ...
  //
  // determine if we can get away with only calling
  // m_pfirst->recursive_match_all_ only once
  //
  ...
  if (MULTILINE != (MULTILINE & m_flags) && 1 == pgroup->calternates() &&
      icur != m_pat->end() && BEGIN_LINE == sy.reg_token(icur, m_pat->end())) {
  ...
  if (RIGHTMOST != (RIGHTMOST & m_flags) &&
      SINGLELINE == (SINGLELINE & m_flags) && 1 == pgroup->calternates() &&
      icur != m_pat->end() && MATCH_ANY == sy.reg_token(icur, m_pat->end()) &&
      icur != m_pat->end()) {
...
template <typename CI, typename SY>
REGEXPR_H_INLINE void
basic_rpattern_base<CI, SY>::set_substitution(string_type const &subst) {
  ...
  basic_rpattern_base<CI, SY>::_parse_subst(*temp_subst, uses_backrefs,
                                            temp_subst_list);
...
template <typename CI, typename SY>
inline detail::match_group_base<CI> *
basic_rpattern_base<CI, SY>::_find_next_group(
    iter_wrap &iw, detail::match_group_base<CI> *pgroup_enclosing,
    syntax_type &sy, std::vector<detail::match_group_base<CI> *> &rggroups) {
  ...
  if (m_pat->end() != iw.ipat &&
      NO_TOKEN != (tok = sy.ext_token(iw.ipat, m_pat->end()))) {
    ...
    if (END_GROUP != sy.reg_token(itemp = iw.ipat, m_pat->end())) {
      ...
      case EXT_NOBACKREF:
        ...
        detail::assign_auto_ptr(pgroup, new (m_arena) detail::match_group<CI>(
                                            _get_next_group_nbr(), m_arena));
        break;

      case EXT_INDEPENDENT:
        m_invisible_groups.push_back(m_cgroups);
        detail::assign_auto_ptr(pgroup,
                                new (m_arena) detail::independent_group<CI>(
                                    _get_next_group_nbr(), m_arena));
        break;
      ...
      case EXT_POS_LOOKAHEAD:
        detail::assign_auto_ptr(
            pgroup,
            new (m_arena) detail::lookahead_assertion<CI>(true, m_arena));
        break;

      case EXT_NEG_LOOKAHEAD:
        detail::assign_auto_ptr(
            pgroup,
            new (m_arena) detail::lookahead_assertion<CI>(false, m_arena));
        break;

      case EXT_POS_LOOKBEHIND:
        detail::assign_auto_ptr(
            pgroup,
            new (m_arena) detail::lookbehind_assertion<CI>(true, m_arena));
        break;

      case EXT_NEG_LOOKBEHIND:
        detail::assign_auto_ptr(
            pgroup,
            new (m_arena) detail::lookbehind_assertion<CI>(false, m_arena));
        break;
      ...
      case EXT_CONDITION:
        ...
        if (size_t cbackref =
                detail::parse_int(iw.ipat, m_pat->end()) &&
...
template <typename CI> inline void check_iter(CI icur, CI istop) {
  if (istop == icur)
    throw bad_regexpr("expecting end of character set");
}

template <typename II, typename CI>
inline typename std::iterator_traits<CI>::value_type
get_escaped_char(II &icur, CI iend, bool normalize) {
  typedef typename std::iterator_traits<CI>::value_type CH;
  CH ch = 0, i;
  ...
  case REGEX_CHAR(CH, '0'):
  case REGEX_CHAR(CH, '1'):
  case REGEX_CHAR(CH, '2'):
  case REGEX_CHAR(CH, '3'):
  case REGEX_CHAR(CH, '4'):
  case REGEX_CHAR(CH, '5'):
  case REGEX_CHAR(CH, '6'):
  case REGEX_CHAR(CH, '7'):
    ch = CH(*icur++ - REGEX_CHAR(CH, '0'));
    for (i = 0;
         i < 2 && REGEX_CHAR(CH, '0') <= *icur && REGEX_CHAR(CH, '7') >= *icur;
         check_iter<CI>(++icur, iend))
  ...
  case REGEX_CHAR(CH, 'x'):
    for (++icur, ch = i = 0; i < 2 && detail::regex_isxdigit(*icur);
         check_iter<CI>(++icur, iend))
...
inline void parse_charset(std::auto_ptr<CS> &pnew,
  ...
  do {
    ...
    if (CHARSET_RANGE == tok && fhave_prev) {
      ...
      case CHARSET_ESCAPE: // BUGBUG user-defined charset?
        pnew->set_bit_range(ch_prev, get_escaped_char(icur, iend, normalize),
                            fnocase);
        continue;
...
inline void basic_rpattern_base<CI, SY>::_find_atom(
    iter_wrap &iw, detail::match_group_base<CI> *pgroup, syntax_type &sy) {
  ...
  do {
    if (itemp != iw.ipat) // Is there whitespace to skip?
    {
      ...
      if (m_pat->end() ==
          (itemp = iw.ipat)) // are we at the end of the pattern?
        break;
    }
    switch (sy.quant_token(itemp, m_pat->end())) {
      // if {, } can't be interpreted as quantifiers, treat them as regular
      // chars
    case BEGIN_RANGE:
    ...
    quantify:
      if (istart != --iw.ipat)
        pgroup->add_item(detail::create_literal<CI>(istart, iw.ipat,
                                                    sy.get_flags(), m_arena));
      std::auto_ptr<detail::sub_expr<CI>> pnew(
          detail::create_char<CI>(*iw.ipat++, sy.get_flags(), m_arena));
      _quantify(pnew, iw, false, sy);
      pgroup->add_item(pnew.release());
      return;
    }
  } while (m_pat->end() != ++iw.ipat &&
           !sy.reg_token(itemp = iw.ipat, m_pat->end()));

  std::advance(istart = m_pat->begin(), nstart);
  assert(iw.ipat != istart);
  pgroup->add_item(
      detail::create_literal<CI>(istart, iw.ipat, sy.get_flags(), m_arena));
}
...
inline bool basic_rpattern_base<CI, SY>::_find_next(
  ...
  case BEGIN_LINE:
    detail::assign_auto_ptr(pnew,
                            detail::create_bol<CI>(sy.get_flags(), m_arena));
    break;

  case END_LINE:
    detail::assign_auto_ptr(pnew,
                            detail::create_eol<CI>(sy.get_flags(), m_arena));
    break;

  case BEGIN_CHARSET:
    detail::assign_auto_ptr(pcs, new (m_arena) detail::custom_charset(m_arena));
    detail::parse_charset<char_type, detail::custom_charset>(pcs, iw.ipat,
                                                             m_pat->end(), sy);
    detail::assign_auto_ptr(pnew, detail::create_custom_charset<CI>(
                                      pcs.get(), sy.get_flags(), m_arena));
    pcs.release();
    break;

  case MATCH_ANY:
    detail::assign_auto_ptr(pnew,
                            detail::create_any<CI>(sy.get_flags(), m_arena));
    break;

  case ESC_WORD_BOUNDARY:
    detail::assign_auto_ptr(
        pnew, detail::create_word_boundary<CI>(true, sy.get_flags(), m_arena));
    break;

  case ESC_NOT_WORD_BOUNDARY:
    detail::assign_auto_ptr(
        pnew, detail::create_word_boundary<CI>(false, sy.get_flags(), m_arena));
    break;

  case ESC_WORD_START:
    detail::assign_auto_ptr(
        pnew, detail::create_word_start<CI>(sy.get_flags(), m_arena));
    break;

  case ESC_WORD_STOP:
    detail::assign_auto_ptr(
        pnew, detail::create_word_stop<CI>(sy.get_flags(), m_arena));
    break;

  case ESC_DIGIT:
    detail::assign_auto_ptr(
        pnew, detail::create_charset<CI>(
                  detail::intrinsic_charsets<char_type>::get_digit_charset(),
                  sy.get_flags(), m_arena));
    break;
  ...
  case ESC_NOT_DIGIT:
    detail::assign_auto_ptr(
        pnew,
        detail::create_charset<CI>(
            detail::intrinsic_charsets<char_type>::get_not_digit_charset(),
            sy.get_flags(), m_arena));
    break;
  ...
  case ESC_WORD:
    detail::assign_auto_ptr(
        pnew, detail::create_charset<CI>(
                  detail::intrinsic_charsets<char_type>::get_word_charset(),
                  sy.get_flags(), m_arena));
    break;
  ...
  case ESC_NOT_WORD:
    detail::assign_auto_ptr(
        pnew, detail::create_charset<CI>(
                  detail::intrinsic_charsets<char_type>::get_not_word_charset(),
                  sy.get_flags(), m_arena));
    break;
  ...
  case ESC_SPACE:
    detail::assign_auto_ptr(
        pnew, detail::create_charset<CI>(
                  detail::intrinsic_charsets<char_type>::get_space_charset(),
                  sy.get_flags(), m_arena));
    break;
  ...
  case ESC_NOT_SPACE:
    detail::assign_auto_ptr(
        pnew,
        detail::create_charset<CI>(
            detail::intrinsic_charsets<char_type>::get_not_space_charset(),
            sy.get_flags(), m_arena));
    break;

  case ESC_BEGIN_STRING:
    detail::assign_auto_ptr(pnew,
                            detail::create_bos<CI>(sy.get_flags(), m_arena));
    break;

  case ESC_END_STRING:
    detail::assign_auto_ptr(pnew,
                            detail::create_eos<CI>(sy.get_flags(), m_arena));
    break;

  case ESC_END_STRING_z:
    detail::assign_auto_ptr(pnew,
                            detail::create_eoz<CI>(sy.get_flags(), m_arena));
    break;
  ...
  case ESCAPE:
    if (m_pat->end() == iw.ipat) {
      ...
      detail::assign_auto_ptr(
          pnew, detail::create_char<CI>(*--iw.ipat, sy.get_flags(), m_arena));
      ++iw.ipat;
    } else if (REGEX_CHAR(CH, '0') <= *iw.ipat &&
               REGEX_CHAR(CH, '9') >= *iw.ipat) {
      ...
      if (REGEX_CHAR(CH, '0') != *iw.ipat &&
          (10 > nbackref || nbackref < _cgroups_total())) {
        detail::assign_auto_ptr(pnew, detail::create_backref<CI>(
                                          nbackref, sy.get_flags(), m_arena));
        iw.ipat = itemp;
      } else {
        ...
        char_type ch = 0, i = 0;
        for (;
             i < 3 && m_pat->end() != iw.ipat &&
             REGEX_CHAR(CH, '0') <= *iw.ipat && REGEX_CHAR(CH, '7') >= *iw.ipat;
             ++i, ++iw.ipat)
          ch = CH(ch * 8 + (*iw.ipat - REGEX_CHAR(CH, '0')));
        detail::assign_auto_ptr(
            pnew, detail::create_char<CI>(ch, sy.get_flags(), m_arena));
      }
    } else if (REGEX_CHAR(CH, 'e') == *iw.ipat) {
      ++iw.ipat;
      detail::assign_auto_ptr(
          pnew, detail::create_char<CI>(CH(27), sy.get_flags(), m_arena));
    } else if (REGEX_CHAR(CH, 'x') == *iw.ipat) {
      char_type ch = 0, i = 0;
      for (++iw.ipat;
           i < 2 && m_pat->end() != iw.ipat && detail::regex_isxdigit(*iw.ipat);
           ++i, ++iw.ipat)
      ...
      detail::assign_auto_ptr(
          pnew,
          detail::create_char<CI>(CH(ch ^ 0x40), sy.get_flags(), m_arena));
    } else if (REGEX_CHAR(CH, 'a') == *iw.ipat && normalize) {
      ++iw.ipat;
      detail::assign_auto_ptr(pnew,
                              detail::create_char<CI>(REGEX_CHAR(CH, '\a'),
                                                      sy.get_flags(), m_arena));
    } else if (REGEX_CHAR(CH, 'f') == *iw.ipat && normalize) {
      ++iw.ipat;
      detail::assign_auto_ptr(pnew,
                              detail::create_char<CI>(REGEX_CHAR(CH, '\f'),
                                                      sy.get_flags(), m_arena));
    } else if (REGEX_CHAR(CH, 'n') == *iw.ipat && normalize) {
      ++iw.ipat;
      detail::assign_auto_ptr(pnew,
                              detail::create_char<CI>(REGEX_CHAR(CH, '\n'),
                                                      sy.get_flags(), m_arena));
    } else if (REGEX_CHAR(CH, 'r') == *iw.ipat && normalize) {
      ++iw.ipat;
      detail::assign_auto_ptr(pnew,
                              detail::create_char<CI>(REGEX_CHAR(CH, '\r'),
                                                      sy.get_flags(), m_arena));
      ...
      detail::charset const *pcharset =
          detail::get_altern_charset(*iw.ipat, sy);
      if (NULL != pcharset)
        detail::assign_auto_ptr(pnew, detail::create_charset<CI>(
                                          *pcharset, sy.get_flags(), m_arena));
      else
        detail::assign_auto_ptr(
            pnew, detail::create_char<CI>(*iw.ipat, sy.get_flags(), m_arena));
      ++iw.ipat;
  ...
  case ESC_QUOTE_META_ON:
    for (istart = itemp = iw.ipat, fdone = false;
         !fdone && m_pat->end() != iw.ipat;) {
      switch (sy.reg_token(iw.ipat, m_pat->end())) {
    ...
    if (itemp != istart)
      pgroup->add_item(
          detail::create_literal<CI>(istart, itemp, sy.get_flags(), m_arena));
...
inline void basic_rpattern_base<CI, SY>::_quantify(
    std::auto_ptr<detail::sub_expr<CI>> &pnew, iter_wrap &iw, bool is_group,
    syntax_type &sy) {
  if (m_pat->end() != iw.ipat && !pnew->is_assertion()) {
    ...
    if ((size_t)-1 != lbound) {
      ...
      std::auto_ptr<detail::sub_expr<CI>> pquant(
          pnew->quantify(lbound, ubound, !fmin, m_arena));
      pnew.release();
...
inline void basic_rpattern_base<CI, SY>::_add_subst_backref(
    detail::subst_node &snode, size_t nbackref, size_t rstart,
    bool &uses_backrefs, detail::subst_list_type &subst_list) const {
  uses_backrefs = true;
...
inline void basic_rpattern_base<CI, SY>::_parse_subst(
    string_type &subst, bool &uses_backrefs,
  ...
  while (subst.end() != icur) {
    ...
    case SUBST_MATCH:
      _add_subst_backref(snode, 0, std::distance(subst.begin(), icur),
                         uses_backrefs, subst_list);
      break;

    case SUBST_PREMATCH:
      _add_subst_backref(snode, (size_t)detail::subst_node::PREMATCH,
                         std::distance(subst.begin(), icur), uses_backrefs,
                         subst_list);
      break;

    case SUBST_POSTMATCH:
      _add_subst_backref(snode, (size_t)detail::subst_node::POSTMATCH,
                         std::distance(subst.begin(), icur), uses_backrefs,
                         subst_list);
      break;

    case SUBST_BACKREF:
      nbackref = detail::parse_int(icur, subst.end(),
                                   cgroups() - 1); // always at least 1 group
      if (0 == nbackref)
        throw bad_regexpr("invalid backreference in substitution");

      _add_subst_backref(snode, nbackref, std::distance(subst.begin(), icur),
                         uses_backrefs, subst_list);
      break;
    ...
    case SUBST_QUOTE_META_ON:
      ...
      snode.subst_string.rlength =
          std::distance(subst.begin(), itemp) - snode.subst_string.rstart;
      if (snode.subst_string.rlength)
        subst_list.push_back(snode);
...
template <typename CH> REGEXPR_H_INLINE void reset_intrinsic_charsets(CH) {
  detail::intrinsic_charsets<CH>::reset();
}

typedef ::regex::detail::select<
    REGEX_FOLD_INSTANTIATIONS &&
        detail::is_convertible<char const *,
                               std::string::const_iterator>::value,
    std::string::const_iterator, char const *>::type lpcstr_t;

typedef ::regex::detail::select<
    REGEX_FOLD_INSTANTIATIONS &&
        detail::is_convertible<wchar_t const *,
                               std::wstring::const_iterator>::value,
    std::wstring::const_iterator, wchar_t const *>::type lpcwstr_t;

namespace {
// Used to fake the compiler into implicitly instantiating the templates we need
bool g_regex_false;

template <typename CI, typename SY> struct rpattern_instantiator {
  ...
  rpattern_instantiator() {
    if (g_regex_false) {
      ...
      // These force VC6 to create COMDATs for set_substitution and the two init
      // methods
      void (*preset)(char_type) = &reset_intrinsic_charsets;
      (*preset)(char_type());

      void (rpattern_type::*psetsub)(string_type const &) =
          &rpattern_type::set_substitution;
      (pat.*psetsub)(str);

      void (rpattern_type::*pinit1)(string_type const &, REGEX_FLAGS,
                                    REGEX_MODE) = &rpattern_type::init;
      (pat.*pinit1)(str, NOFLAGS, MODE_DEFAULT);

      void (rpattern_type::*pinit2)(string_type const &, string_type const &,
                                    REGEX_FLAGS, REGEX_MODE) =
          &rpattern_type::init;
...
struct typelist {
  typedef cons<T1,
               typename typelist<T2, T3, T4, T5, T6, T7, T8, null_type>::type>
      type;
};

template <>
struct typelist<null_type, null_type, null_type, null_type, null_type,
...
template <> struct recursive_instantiator<null_type> {
  template <typename SY> struct inner {};
};
...
#define REGEX_TO_INSTANTIATE                                                   \
  std::string::const_iterator, std::wstring::const_iterator, lpcstr_t, lpcwstr_t
#else
#define REGEX_TO_INSTANTIATE restring::const_iterator, lpctstr_t
#endif
#endif

// Create the perl instantiations
#ifndef REGEX_NO_PERL
recursive_instantiator<typelist<REGEX_TO_INSTANTIATE>::type>::inner<
    perl_syntax<char>>
    _dummy1;
#endif
