...
static BOOL IsEnterKey(LPCTSTR lpChar, BOOL *pEnterType = NULL) {
  BOOL bRet = FALSE;
...
static BOOL GetStrInStrPos(LPCTSTR lpszSouce, LPCTSTR lpszSearch,
                           long &liOffset) {
  BOOL bRet = FALSE;
  if ((NULL == lpszSouce) || (NULL == lpszSearch)) {
    bRet = FALSE;
  } else {
...
//----------------------------------------------------------
static int GetPrivateProfileKeyPos(LPCTSTR lpszBuf, LPCTSTR lpszAppName,
                                   LPCTSTR lpszKeyName, long &liOffset,
                                   long &liSize) {
  int iRet = -1;
  if ((NULL == lpszBuf) || (NULL == lpszAppName) || (NULL == lpszKeyName)) {
    iRet = -1;
  } else {
    ...
    if (NULL != lpszModifiedAppName) {
      ...
      bSearchFlag =
          GetStrInStrPos(lpszBuf, lpszModifiedAppName, liAppNameOffset);
      if (bSearchFlag) {
        lpszAt += liAppNameOffset + liAppName + uiLBracket + uiRBracket;
        long liKeyNameOffset;
        bSearchFlag = GetStrInStrPos(lpszAt, lpszKeyName, liKeyNameOffset);
        if (bSearchFlag) {
          liOffset = liAppNameOffset + liKeyNameOffset + liAppName +
                     uiLBracket + uiRBracket;
          liSize = GetFirstLineLen(lpszAt + liKeyNameOffset);
          iRet = 1;
        } else {
          liOffset = liAppNameOffset + liAppName + uiLBracket + uiRBracket;
          liSize = 0;
          iRet = 2;
        }
      } else {
      ...
      for (i += uiEqual, lpszAt += uiEqual; i < liKeySize;) {
        if (0 == _tcsncmp(lpszAt, szSpaceKey, uiSpaceKey)) {
          ...
          if ((lpszAt - lpszKeyLine) < liKeySize) {
            lpszRet = lpszAt;
            liKeyValueSize = liKeySize - (lpszAt - lpszKeyLine);
          }
          break;
        }
      }
...
//	Platform	: Unconcerned.
//----------------------------------------------------------
BOOL KPIWritePrivateProfileString(
    LPCTSTR lpszAppName, /* section name */
    LPCTSTR lpszKeyName, /* key name */
    LPCTSTR lpszString,  /* string to add */
    LPCTSTR lpszFileName /* initialization file */) {
  BOOL bRet = TRUE;
  if ((NULL == lpszAppName) || (NULL == lpszKeyName) || (NULL == lpszString) ||
      (NULL == lpszFileName)) {
    bRet = FALSE;
  } else {
    try {
      ...
      if (NULL != pFile) {
        if (0 == liFileSize) {
          ...
          int iResult = GetPrivateProfileKeyPos(lpszFileBuf, lpszAppName,
                                                lpszKeyName, liOffset, liSize);
          if (0 == iResult) {
            ...
            long liKeyLen = GetFirstLineLen(&(lpszFileBuf[liOffset]));
            LPTSTR lpszKeyTemp = new TCHAR[liKeyLen + 1];
            memcpy(lpszKeyTemp, &(lpszFileBuf[liOffset]),
                   liKeyLen * sizeof(TCHAR));
            lpszKeyTemp[liKeyLen] = 0;
            long liKeyValueSize;
            if (NULL != GetKeyValue(lpszKeyTemp, liKeyValueSize)) {
              fwrite(lpszKeyTemp, 1, liKeyLen - liKeyValueSize, pFile);
              fwrite(lpszString, 1, _tcslen(lpszString), pFile);
              fwrite(&(lpszFileBuf[liOffset + liKeyLen]), 1,
                     liFileSize - liOffset - liKeyLen, pFile);
            } else {
...
//	Platform	: Unconcerned.
//----------------------------------------------------------
DWORD KPIGetPrivateProfileString(
    LPCTSTR lpszAppName,       /* section name(Input) */
  ...
  if ((NULL == lpszAppName) || (NULL == lpszKeyName) || (NULL == lpszDefault) ||
      (NULL == lpszFileName)) {
    return dwRet;
  }
        ...
        if (NULL != lpszFileBuf) {
          ...
          if (1 == iResult) {
            ...
            if (NULL != lpszKeyValue) {
              ...
              DWORD dwCopyLen = dwSize <= dwRet ? dwSize : dwRet;
              if (NULL != lpszReturnedString) {
...
HANDLE
KPICreateThread(LPTHREAD_START_ROUTINE lpStartAddress, /* thread function */
  ...
  hRet = (HANDLE)_beginthreadex(NULL, 0, (start_address)lpStartAddress,
                                lpParameter, 0, (unsigned *)lpThreadId);
...
BOOL KPIReleaseMutex(HANDLE hMutex) { return ReleaseMutex(hMutex); }

unsigned long KPIHash(char *pStr, unsigned long ulModel,
                      unsigned long ulBaseNum) {
...
void KPIPrintToFile(const TCHAR *lpszFilePath, const TCHAR *lpszFmt, ...) {
  if ((NULL == lpszFilePath) || (NULL == lpszFmt)) {
    return;
  }
