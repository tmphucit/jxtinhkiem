...
std::string S3PEquipment::GetAutoIncrementField() { return "iid"; }
...
S3PTableObjList<S3PEquipRequireInfo> *S3PEquipment::GetEquipRequireInfoList() {
  ...
  if (!m_pEquipRequireInfoList) {
    m_pEquipRequireInfoList = new S3PTableObjList<S3PEquipRequireInfo>(
        "EquipRequireInfo", "iid", "iMainID", m_properties["iid"]);
  }
...
S3PTableObjList<S3PEquipBaseInfo> *S3PEquipment::GetEquipBaseInfoList() {
  ...
  if (!m_pEquipBaseInfoList) {
    m_pEquipBaseInfoList = new S3PTableObjList<S3PEquipBaseInfo>(
        "EquipBaseInfo", "iid", "iMainID", m_properties["iid"]);
  }
...
S3PTableObjList<S3PEquipEfficInfo> *S3PEquipment::GetEquipEfficInfoList() {
  ...
  if (!m_pEquipEfficInfoList) {
    m_pEquipEfficInfoList = new S3PTableObjList<S3PEquipEfficInfo>(
        "EquipEfficInfo", "iid", "iMainID", m_properties["iid"]);
  }
  return m_pEquipEfficInfoList;
}

int S3PEquipment::Delete() {

  GetEquipBaseInfoList()->DeleteAll();
  GetEquipEfficInfoList()->DeleteAll();
  GetEquipRequireInfoList()->DeleteAll();
  return S3PManipulator::Delete();
}