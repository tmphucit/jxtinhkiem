...
S3PDBSocketPool::S3PDBSocketPool()
    : m_hProcessor(NULL), m_dwProcessorThreadId(0), m_piRunSignal(NULL),
      m_hEnable(NULL), m_Socket(INVALID_SOCKET) {
...
void S3PDBSocketPool::PushDataPacket(_LPDBSOCKETPOOLDATAPACKET lpData) {
  ...
  if (NULL == g_hPoolMutex) {
    g_hPoolMutex = KPICreateMutex(NULL, FALSE, "Socket pool's mutex");
  }
  if (NULL != g_hPoolMutex) {
    DWORD dwWaitResult;
    dwWaitResult = KPIWaitForSingleObject(g_hPoolMutex, INFINITE);
    if (1 == dwWaitResult) {
      __try {
        //<------END
    ...
    if (1 == dwWaitResult) {
      __try {
        ...
        if (!(m_UDPs.IsEmpty())) {
          ...
          if ((m_UDPs.IsEmpty()) && (NULL != m_hEnable)) {
...
static DWORD WINAPI ProcessData(LPVOID lpParam) {
  ...
  if ((NULL == lpPoolParam->piRunSignal) ||
      (NULL == lpPoolParam->hEnableEvent) || (NULL == lpPoolParam->pPool)) {
    dwRet = 0;
    return dwRet;
  }
...
HANDLE S3PDBSocketPool::Start() {
  HANDLE hRet = NULL;
  if ((NULL != m_hProcessor) || (NULL == m_hEnable) ||
      (NULL == m_piRunSignal) || (INVALID_SOCKET == m_Socket)) {
    hRet = NULL;
    return hRet;
  }
...
BOOL S3PDBSocketPool::ProMsgWhenLocked(DWORD dwTargetIP, int iTargetPort,
  ...
  if (NULL != lpcParsedData) {
    ...
    if ((LOGIN_ACCOUNT_MAX_LEN + 2 + LOGIN_PASSWORD_MAX_LEN + 2) ==
      ...
      DWORD dwOffset = 0;
      memcpy(szAccount, lpcParsedData->m_lpData + dwOffset,
             LOGIN_ACCOUNT_MAX_LEN);
      dwOffset += LOGIN_ACCOUNT_MAX_LEN + 2;
      memcpy(szPassword, lpcParsedData->m_lpData + dwOffset,
      ...
      memcpy(&feedBuf[1 + sizeof(KLoginStructHead)], szAccount,
      ...
      if (TRUE ==
          S3PDBSocketParser::Encode(feedBuf, ulSize, NULL, ulEncodedSize)) {
        IBYTE *pEncodedBuf = new IBYTE[ulEncodedSize];
        if (NULL != pEncodedBuf) {
          S3PDBSocketParser::Encode(feedBuf, ulSize, pEncodedBuf,
                                    ulEncodedSize);
...
BOOL S3PDBSocketPool::ProAddAccount(DWORD dwTargetIP, int iTargetPort,
                                    _LPCPARSEDDATAPACKET lpcParsedData) {
  ...
  if ((LOGIN_ACCOUNT_MAX_LEN + 2 + LOGIN_PASSWORD_MAX_LEN + 2 +
       LOGIN_REALNAME_MAX_LEN + 2) == lpcParsedData->m_dwSize) {
    char szAccount[LOGIN_ACCOUNT_MAX_LEN + 2];
    szAccount[LOGIN_ACCOUNT_MAX_LEN] = 0;
    char szPassword[LOGIN_PASSWORD_MAX_LEN + 2];
    szPassword[LOGIN_PASSWORD_MAX_LEN] = 0;
    ...
    dwOffset += LOGIN_PASSWORD_MAX_LEN + 2;
    memcpy(szRealName, lpcParsedData->m_lpData + dwOffset,
    ...
    // bRet = SendUDP( dwTargetIP, iTargetPort, feedBuf, 1 +
    // KLOGINSTRUCTHEADSIZE );
    unsigned long ulEncodedSize;
    if (TRUE == S3PDBSocketParser::Encode(feedBuf, 1 + KLOGINSTRUCTHEADSIZE,
                                          NULL, ulEncodedSize)) {
      ...
      if (NULL != pEncodedFeedBuf) {
        ...
        bRet = SendUDP(dwTargetIP, iTargetPort, pEncodedFeedBuf, ulEncodedSize);
...
BOOL S3PDBSocketPool::ProLoginFromGame(DWORD dwTargetIP, int iTargetPort,
                                       _LPCPARSEDDATAPACKET lpcParsedData) {
  BOOL bRet = FALSE;
  if (NULL != lpcParsedData) {
    IBYTE feedBuf[def_UDPSIZE];
...
BOOL S3PDBSocketPool::ProLogoutFromGame(DWORD dwTargetIP, int iTargetPort,
                                        _LPCPARSEDDATAPACKET lpcParsedData) {
  assert(NULL != lpcParsedData);
  BOOL bRet = FALSE;
  ...
  if ((LOGIN_ACCOUNT_MAX_LEN + 2 + LOGIN_PASSWORD_MAX_LEN + 2 +
       sizeof(DWORD)) == lpcParsedData->m_dwSize) {
    S3PAccount account;
    ...
    memcpy(szPassword, lpcParsedData->m_lpData + dwOffset,
           LOGIN_PASSWORD_MAX_LEN);
    dwOffset += LOGIN_PASSWORD_MAX_LEN + 2;
    memcpy(&dwGameID, lpcParsedData->m_lpData + dwOffset, sizeof(DWORD));
...
BOOL S3PDBSocketPool::ProReportFromGame(DWORD dwTargetIP, int iTargetPort,
                                        _LPCPARSEDDATAPACKET lpcParsedData) {
  assert(NULL != lpcParsedData);
  BOOL bRet = FALSE;
  IBYTE feedBuf[def_UDPSIZE];
  ...
  if ((LOGIN_ACCOUNT_MAX_LEN + 2 + LOGIN_PASSWORD_MAX_LEN + 2 +
    ...
    memcpy(&dwGameID,
           &(lpcParsedData
                 ->m_lpData[LOGIN_ACCOUNT_MAX_LEN + LOGIN_PASSWORD_MAX_LEN]),
           sizeof(DWORD));
    dwOffset += sizeof(DWORD);
...
BOOL S3PDBSocketPool::ProQueryGameserverList(
    DWORD dwTargetIP, int iTargetPort, _LPCPARSEDDATAPACKET lpcParsedData) {
  BOOL bRet = FALSE;
  if (NULL != lpcParsedData) {
    ...
    if ((LOGIN_ACCOUNT_MAX_LEN + 2 + LOGIN_PASSWORD_MAX_LEN + 2) ==
      ...
      szPassword[LOGIN_PASSWORD_MAX_LEN] = 0;
      memcpy(szAccount, lpcParsedData->m_lpData, LOGIN_ACCOUNT_MAX_LEN);
      memcpy(szPassword, &(lpcParsedData->m_lpData[LOGIN_ACCOUNT_MAX_LEN]),
             LOGIN_PASSWORD_MAX_LEN);
      S3PAccount account;
      DWORD dwSizeTemp = 0;
      ...
      memcpy(&pFeedBuf[1 + sizeof(KLoginStructHead)], szAccount,
             LOGIN_ACCOUNT_MAX_LEN + 2);
      memcpy(
          &pFeedBuf[1 + sizeof(KLoginStructHead) + LOGIN_ACCOUNT_MAX_LEN + 2],
          szPassword, LOGIN_PASSWORD_MAX_LEN + 2);
      unsigned long ulSize = dwSize + 1;
      unsigned long ulEncodedSize;
      if (TRUE ==
...
BOOL S3PDBSocketPool::ProBeginGame(DWORD dwTargetIP, int iTargetPort,
                                   _LPCPARSEDDATAPACKET lpcParsedData) {
  BOOL bRet = FALSE;
  if (NULL != lpcParsedData) {
    IBYTE feedBuf[def_UDPSIZE];
...
BOOL S3PDBSocketPool::ProDBLogin(DWORD dwTargetIP, INT iTargetPort,
                                 _LPCPARSEDDATAPACKET lpcParsedData) {
  BOOL bRet = FALSE;
  if (NULL != lpcParsedData) {
    if ((sizeof(GUID) + sizeof(_DBLOGINSTRUCT) - sizeof(KLoginStructHead)) ==
      ...
      GUID msgID;
      memcpy(szUserName, lpcParsedData->m_lpData, def_DBUSERNAME_MAX_LEN);
      memcpy(szPassword, &(lpcParsedData->m_lpData[def_DBUSERNAME_MAX_LEN]),
             def_DBPASSWORD_MAX_LEN);
      memcpy(&msgID,
             &(lpcParsedData
                   ->m_lpData[def_DBUSERNAME_MAX_LEN + def_DBPASSWORD_MAX_LEN]),
             sizeof(GUID));
      ...
      memcpy(&feedBuf[1], &dwSize, sizeof(DWORD));
      memcpy(&feedBuf[1 + sizeof(DWORD) * 2], &szUserName,
      ...
      int iResult = S3PAccount::DBLogin(szUserName, szPassword, lpszTargetIP);
      ...
      if (TRUE ==
        ...
        IBYTE *pEncodedBuf = new IBYTE[ulEncodedSize];
        if (NULL != pEncodedBuf) {
          S3PDBSocketParser::Encode(feedBuf, ulFeedSize, pEncodedBuf,
                                    ulEncodedSize);
          bRet = SendUDP(dwTargetIP, iTargetPort, pEncodedBuf, ulEncodedSize);
          delete[] pEncodedBuf;
          pEncodedBuf = NULL;
...
BOOL S3PDBSocketPool::ProDBLogout(DWORD dwTargetIP, int iTargetPort,
                                  _LPCPARSEDDATAPACKET lpcParsedData) {
  BOOL bRet = FALSE;
  if (NULL != lpcParsedData) {
    if ((sizeof(GUID) + sizeof(_DBLOGINSTRUCT) - sizeof(KLoginStructHead)) ==
      ...
      int iResult = S3PAccount::DBLogout(szUserName, szPassword);
      if (1 == iResult) // Successful
      {
        dwParam |= DBLOGIN_R_SUCCESS;
...
BOOL S3PDBSocketPool::ProDBQueryUserList(DWORD dwTargetIP, int iTargetPort,
                                         _LPCPARSEDDATAPACKET lpcParsedData) {
  BOOL bRet = FALSE;
  if (NULL != lpcParsedData) {
    if ((sizeof(GUID) + sizeof(_DBLOGINSTRUCT) - sizeof(KLoginStructHead)) ==
      ...
      int iResult = S3PAccount::DBQueryUserList(szUserName, szPassword, NULL,
                                                dwUserListSize);
      ...
      if (1 == iResult) {
        if (0 == dwUserListSize) {
          ...
          pLastFeedBuf = new IBYTE[1 + sizeof(_DBLOGINSTRUCT) + sizeof(GUID) +
                                   dwUserListSize];
          if (NULL != pLastFeedBuf) {
            bFeedBufIsFixed = FALSE;
            dwSize += dwUserListSize;
            memcpy(pLastFeedBuf, feedBuf,
                   1 + sizeof(_DBLOGINSTRUCT) + sizeof(GUID));
            memcpy(&pLastFeedBuf[1], &dwSize, sizeof(DWORD));
            dwParam |= DBLOGIN_R_SUCCESS;
            S3PAccount::DBQueryUserList(
      ...
      unsigned long ulEncodedSize;
      if (TRUE == S3PDBSocketParser::Encode(pLastFeedBuf, ulFeedSize, NULL,
                                            ulEncodedSize)) {
        IBYTE *pEncodedBuf = new IBYTE[ulEncodedSize];
        if (NULL != pEncodedBuf) {
          S3PDBSocketParser::Encode(pLastFeedBuf, ulFeedSize, pEncodedBuf,
                                    ulEncodedSize);
          bRet = SendUDP(dwTargetIP, iTargetPort, pEncodedBuf, ulEncodedSize);
          delete[] pEncodedBuf;
          pEncodedBuf = NULL;
...
BOOL S3PDBSocketPool::ProDBLock(DWORD dwTargetIP, int iTargetPort,
                                _LPCPARSEDDATAPACKET lpcParsedData) {
  BOOL bRet = FALSE;
  if (NULL != lpcParsedData) {
    if ((sizeof(GUID) + sizeof(_DBLOGINSTRUCT) - sizeof(KLoginStructHead)) ==
      ...
      int iResult = S3PAccount::DBLock(szUserName, szPassword);
      if (1 == iResult) // Successful
      {
...
BOOL S3PDBSocketPool::ProDBActivate(DWORD dwTargetIP, int iTargetPort,
                                    _LPCPARSEDDATAPACKET lpcParsedData) {
  BOOL bRet = FALSE;
  if (NULL != lpcParsedData) {
    if ((sizeof(GUID) + sizeof(_DBLOGINSTRUCT) - sizeof(KLoginStructHead)) ==
      ...
      int iResult = S3PAccount::DBActivate(szUserName, szPassword);
      if (1 == iResult) // Successful
      {
        m_bAccDBIsLocked = TRUE;
        dwParam |= DBLOGIN_R_SUCCESS;
...
BOOL S3PDBSocketPool::ProDBAddUser(DWORD dwTargetIP, int iTargetPort,
                                   _LPCPARSEDDATAPACKET lpcParsedData) {
  BOOL bRet = FALSE;
  if (NULL != lpcParsedData) {
    if ((sizeof(GUID) + sizeof(_DBLOGINSTRUCT) + def_DBUSERNAME_MAX_LEN +
         def_DBPASSWORD_MAX_LEN + sizeof(short int) -
         sizeof(KLoginStructHead)) == lpcParsedData->m_dwSize) {
      ...
      memcpy(szNewUserName, lpcParsedData->m_lpData + dwOffset,
             def_DBUSERNAME_MAX_LEN);
      dwOffset += def_DBUSERNAME_MAX_LEN;
      memcpy(szNewUserPassword, lpcParsedData->m_lpData + dwOffset,
             def_DBPASSWORD_MAX_LEN);
      dwOffset += def_DBPASSWORD_MAX_LEN;
      memcpy(&siNewUserPriority, lpcParsedData->m_lpData + dwOffset,
             sizeof(short int));
      dwOffset += sizeof(short int);
      memcpy(&msgID, lpcParsedData->m_lpData + dwOffset, sizeof(GUID));
      IBYTE feedBuf[def_UDPSIZE];
      feedBuf[0] = LOGIN_R_TAG;
      DWORD dwSize = sizeof(_DBLOGINSTRUCT) + sizeof(GUID);
      memcpy(&feedBuf[1], &dwSize, sizeof(DWORD));
      ...
      int iResult = S3PAccount::DBAddUser(szUserName, szPassword, szNewUserName,
                                          szNewUserPassword, siNewUserPriority);
      if (1 == iResult) // Successful
      {
        dwParam |= DBLOGIN_R_SUCCESS;
      } else if (-6 == iResult) {
...
BOOL S3PDBSocketPool::ProDBDeleteUser(DWORD dwTargetIP, int iTargetPort,
                                      _LPCPARSEDDATAPACKET lpcParsedData) {
  BOOL bRet = FALSE;
  if (NULL != lpcParsedData) {
    if ((sizeof(GUID) + sizeof(_DBLOGINSTRUCT) + def_DBUSERNAME_MAX_LEN -
      ...
      memcpy(szPassword, lpcParsedData->m_lpData + dwOffset,
             def_DBPASSWORD_MAX_LEN);
      dwOffset += def_DBPASSWORD_MAX_LEN;
      memcpy(szSelUserName, lpcParsedData->m_lpData + dwOffset,
             def_DBUSERNAME_MAX_LEN);
      dwOffset += def_DBUSERNAME_MAX_LEN;
      memcpy(&msgID, lpcParsedData->m_lpData + dwOffset, sizeof(GUID));
      IBYTE feedBuf[def_UDPSIZE];
...
BOOL S3PDBSocketPool::ProDBCreateAccount(DWORD dwTargetIP, int iTargetPort,
                                         _LPCPARSEDDATAPACKET lpcParsedData) {
  BOOL bRet = FALSE;
  if (NULL != lpcParsedData) {
    if ((sizeof(GUID) + sizeof(_DBLOGINSTRUCT) + LOGIN_REALNAME_MAX_LEN +
         LOGIN_PASSWORD_MAX_LEN + LOGIN_ACCOUNT_MAX_LEN -
         sizeof(KLoginStructHead)) == lpcParsedData->m_dwSize) {
      char szUserName[def_DBUSERNAME_MAX_LEN + 2];
      szUserName[def_DBUSERNAME_MAX_LEN] = 0;
      char szPassword[def_DBPASSWORD_MAX_LEN + 2];
      ...
      memcpy(szPassword, lpcParsedData->m_lpData + dwOffset,
             def_DBPASSWORD_MAX_LEN);
      dwOffset += def_DBPASSWORD_MAX_LEN;
      memcpy(szAccRealName, lpcParsedData->m_lpData + dwOffset,
             LOGIN_REALNAME_MAX_LEN);
      dwOffset += LOGIN_REALNAME_MAX_LEN;
      memcpy(szAccPassword, lpcParsedData->m_lpData + dwOffset,
             LOGIN_PASSWORD_MAX_LEN);
      dwOffset += LOGIN_PASSWORD_MAX_LEN;
      memcpy(szAccName, lpcParsedData->m_lpData + dwOffset,
             LOGIN_ACCOUNT_MAX_LEN);
      ...
      int iResult = S3PAccount::DBCreateAccount(
          szUserName, szPassword, szAccRealName, szAccPassword, szAccName);
      if (1 == iResult) {
        dwParam |= DBLOGIN_R_SUCCESS;
      } else if (-6 == iResult) {
        dwParam |= DBLOGIN_R_HASNOENOUGHPRIORITY;
...
BOOL S3PDBSocketPool::Distribute(DWORD dwTargetIP, int iTargetPort,
                                 _LPCPARSEDDATAPACKET lpcParsedData) {
  assert(NULL != lpcParsedData);
  BOOL bRet = FALSE;
...
BOOL S3PDBSocketPool::SendUDP(DWORD dwTargetIP, int iTargetPort, IBYTE *pBuf,
                              DWORD dwSize) {
  BOOL bRet = FALSE;
  ...
  targetAddr.sin_port = htons((short)(iTargetPort));
  targetAddr.sin_addr.s_addr = dwTargetIP;
  int iResult = sendto(m_Socket, pBuf, dwSize, 0, (SOCKADDR *)(&targetAddr),
                       sizeof(targetAddr));
