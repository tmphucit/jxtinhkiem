...
S3PTableDAO *S3PRole::GetTableDAO() { return new S3PRoleInfoDAO(m_pConn); }

std::string S3PRole::GetAutoIncrementField() { return "iid"; }

std::string S3PRole::GetMandatoryField() { return "cusercode"; }
...
S3PTableObjList<S3PTask> *S3PRole::GetTaskList() {
  ...
  if (m_pTaskList == NULL) {
    m_pTaskList = new S3PTableObjList<S3PTask>("Task_List", "iid", "cusercode",
                                               m_properties["cusercode"]);
  }
  return m_pTaskList;
}
...
S3PTableObjList<S3PFriend> *S3PRole::GetFriendList() {
  ...
  if (m_pFriendList == NULL) {
    m_pFriendList = new S3PTableObjList<S3PFriend>(
        "Friend_List", "iid", "cusercode", m_properties["cusercode"]);
  }
...
S3PTableObjList<S3PFightSkill> *S3PRole::GetFightSkillList() {
  ...
  if (m_pFightSkillList == NULL) {
    m_pFightSkillList = new S3PTableObjList<S3PFightSkill>(
        "FightSkill", "iid", "cusercode", m_properties["cusercode"]);
  }
...
S3PTableObjList<S3PLifeSkill> *S3PRole::GetLifeSkillList() {
  ...
  if (m_pLifeSkillList == NULL) {
    m_pLifeSkillList = new S3PTableObjList<S3PLifeSkill>(
        "LifeSkill", "iid", "cusercode", m_properties["cusercode"]);
  }
...
S3PTableObjList<S3PEquipment> *S3PRole::GetEquipmentList() {
  ...
  if (m_pEquipmentList == NULL) {
    m_pEquipmentList = new S3PTableObjList<S3PEquipment>(
        "Equipments", "iid", "cusercode", m_properties["cusercode"]);
  }
