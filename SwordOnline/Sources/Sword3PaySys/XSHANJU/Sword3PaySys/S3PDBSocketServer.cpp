...
S3PDBSocketServer::S3PDBSocketServer(SOCKET s, S3PDBSocketPool *pPool)
    : m_piRunSignal(NULL), m_hServer(NULL), m_dwServerThreadId(0) {
  m_Socket = s;
...
S3PDBSocketServer::S3PDBSocketServer(int iPort, S3PDBSocketPool *pPool)
    : m_Socket(INVALID_SOCKET), m_piRunSignal(NULL), m_hServer(NULL),
...
static DWORD WINAPI StartServer(LPVOID lpParam) {
  ...
  _LPDBSOCKETSERVERPARAM lpServerParam = (_LPDBSOCKETSERVERPARAM)lpParam;
  ...
  while (0 != *(lpServerParam->piRunSignal)) {
    ...
    iResult = recvfrom(s, recvBuf, def_UDPSIZE, 0, (SOCKADDR *)(&fromAddr),
                       &iFromLen);
...
HANDLE S3PDBSocketServer::Start() {
  HANDLE hRet = NULL;
  if ((NULL != m_hServer) || (NULL == m_piRunSignal) ||
      (NULL == m_pSocketPool) || (NULL == m_hEnablePool)) {
    hRet = NULL;
    return hRet;
  }
  ...
  *m_piRunSignal = 1;
  ...
  m_hServer = KPICreateThread(StartServer, &m_ServerParam, &m_dwServerThreadId);
...
BOOL S3PDBSocketServer::CreateSocket() {
  ...
  // local.sin_port = htons( ( short )( KPIHash( "AccountPort", 1000, 7000 ) )
  // );// Not Completed...
...
BOOL S3PDBSocketServer::SendUDP(SOCKET s, DWORD dwTargetIP, int iTargetPort,
  ...
  int iResult =
      sendto(s, buf, dwSize, 0, (SOCKADDR *)(&targetAddr), sizeof(targetAddr));
  if (SOCKET_ERROR == iResult) {
    bRet = FALSE;
  }
...
SOCKET S3PDBSocketServer::CreateSocket(int iPort) {
  ...
  local.sin_addr.s_addr = htonl(INADDR_ANY);
  if (bind(socketRet, (SOCKADDR *)(&local), sizeof(local)) == SOCKET_ERROR) {
