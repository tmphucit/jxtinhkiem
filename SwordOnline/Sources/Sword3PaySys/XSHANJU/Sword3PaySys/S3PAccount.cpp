...
S3PAccount::S3PAccount() : m_bLocked(TRUE) {}
...
//----------------------------------------------------------
int S3PAccount::Login(const std::string strAccName,
                      const std::string strPassword, int iGameId) {
  int iRet = 1;
  if (strAccName.empty()) {
    iRet = -1;
    return iRet;
...
//----------------------------------------------------------
int S3PAccount::Logout(const std::string strAccName,
                       const std::string strPassword, int iGameId) {
  int iRet = 1;
  if (strAccName.empty()) {
    iRet = -1;
...
//----------------------------------------------------------
int S3PAccount::Report(const std::string strAccName,
                       const std::string strPassword, int iGameId) {
  int iRet = 1;
  if (strAccName.empty()) {
    iRet = -1;
        ...
        if (NULL != pServers) {
          ...
          for (int i = 0; i < iRowNum; i++) {
            ...
            memcpy(&(pServers[i * sizeof(KLoginGameServer) + offset]),
                   strName.c_str(), GAMESERVERNAME_MAX_LEN);
            offset += GAMESERVERNAME_MAX_LEN + 2;
            memcpy(&(pServers[i * sizeof(KLoginGameServer) + offset]), &dwIP,
                   sizeof(DWORD));
            offset += sizeof(DWORD);
            memcpy(&(pServers[i * sizeof(KLoginGameServer) + offset]), &iPort,
                   sizeof(short));
            offset += sizeof(short);
            memcpy(&(pServers[i * sizeof(KLoginGameServer) + offset]), &dwID,
                   sizeof(DWORD));
          }
          iRet = 1;
        }
...
int S3PAccount::DBQueryUserList(const std::string strUserName,
                                const std::string strPassword, IBYTE *pUsers,
                                DWORD &dwSize) {
  int iRet = 1;
  if (strUserName.empty()) {
    iRet = -1;
          ...
          if (NULL != pUsers) {
            if (dwSizeTemp > dwSize) {
              ...
              for (int i = 0; i < iRowNum; i++) {
                ...
                memcpy(&pUsers[i * sizeof(_USERINFO) + offset], &siPriority,
                       sizeof(short));
                offset += sizeof(short);
                BOOL bLoggedin;
                ...
                memcpy(&pUsers[i * sizeof(_USERINFO) + offset], &bLoggedin,
                       sizeof(BOOL));
                offset += sizeof(BOOL);
                memcpy(&pUsers[i * sizeof(_USERINFO) + offset],
...
//----------------------------------------------------------
int S3PAccount::DBLock(const std::string strUserName,
                       const std::string strPassword) {
  int iRet = 1;
  if (strUserName.empty()) {
    iRet = -1;
...
//			  = others	---- Not defined.
//	Description	: Check whether the specified user who carry out
//			      activating account database operation is
// connected.
//----------------------------------------------------------
int S3PAccount::DBActivate(const std::string strUserName,
                           const std::string strPassword) {
  int iRet = 1;
  if (strUserName.empty()) {
    iRet = -1;
...
//----------------------------------------------------------
int S3PAccount::DBAddUser(const std::string strUserName,
                          const std::string strPassword,
  ...
  if (NULL != pAccountCon) {
    ...
    if (1 == iRowNum) {
      ...
      if (0 == stricmp("Write", strPriority.c_str())) {
        if ((0 < siNewUserPriority) && (1 >= siNewUserPriority)) {
          bHasPriority = TRUE;
        }
      } else if (0 == stricmp("Read-Write", strPriority.c_str())) {
        if ((0 < siNewUserPriority) && (2 >= siNewUserPriority)) {
          bHasPriority = TRUE;
        }
      } else if (0 == stricmp("Administrator", strPriority.c_str())) {
        bHasPriority = TRUE;
