...
S3PDBConnection::S3PDBConnection() : m_pRes(NULL) { Init(); }

S3PDBConnection::~S3PDBConnection() {}

void S3PDBConnection::Close() {
  S3PDBConnectionPool::Instance()->SetDBConFree(m_info.iDBIdentifier,
                                                m_info.dwConID);
...
DWORD S3PDBConnection::GetRes() { return m_pRes; }

BOOL S3PDBConnection::QueryBySql(LPCTSTR lpszSql) {
  BOOL bRet = FALSE;
  m_pRes = NULL;
  if ((NULL != lpszSql) && (NULL != m_info.pCon)) {
    try {
...
BOOL S3PDBConnection::Connect(int iDBIdentifier) {
  ...
  if (NULL == m_info.pCon) {
    ...
    if ((def_ERRORCONID != con_ID.dwID) && (NULL != con_ID.pCon)) {
