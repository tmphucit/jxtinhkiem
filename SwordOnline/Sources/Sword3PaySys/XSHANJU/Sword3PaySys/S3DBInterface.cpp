...
S3PManipulator *GetTableFromString(S3_DB_TABLENAME tablename,
                                   std::string strUser);
...
void Item_BaseDataConversion(IN TDBItemData *pItemData,
                             ColumnAndValue &ItemValue) {
...
void Role_BaseDataConversion(IN TRoleBaseInfo *pRoleData,
                             IN ColumnAndValue &RoleValue) {
...
S3DBI_API void *S3DBI_GetRoleInfo(IN OUT BYTE *pRoleBuffer, IN char *strUser,
                                  IN OUT int &nBufLen) {
  ...
  for (int t = 0; t < nFightSkillCount; t++, pFSkillData++) {
    ...
    pFSkillData->m_nSkillLevel =
        atoi(FightSkillValue["ifightskilllevel"].c_str());
  }
  //*************************************************
  ...
  for (t = 0; t < nLifeSkillCount; t++, pLSkillData++) {
    ...
    pLSkillData->m_nSkillLevel =
        atoi(LifeSkillValue["ilifeskilllevel"].c_str());
  }
  //*************************************************
  ...
  TDBTaskData *pTaskData =
      (TDBTaskData *)((BYTE *)pRoleData + pRoleData->dwTaskOffset);
  ...
  for (int j = 0; j < nEquiptListCount; j++) {
    ...
    // 基本属性加载
    S3PTableObjList<S3PEquipBaseInfo> *EquipBaseInfoList =
        (*EquipList)[j]->GetEquipBaseInfoList();
    ...
    //	效果数据加载
    S3PTableObjList<S3PEquipEfficInfo> *EquipEfficInfoList =
        (*EquipList)[j]->GetEquipEfficInfoList();
    ...
    // 需求属性加载
    S3PTableObjList<S3PEquipRequireInfo> *EquipRequireInfoList =
        (*EquipList)[j]->GetEquipRequireInfoList();
    ...
    for (k = 0; k < nEquipRequireInfoListCount; k++, pMagicAttrib++) {
      ColumnAndValue &RequireValue =
          (*EquipRequireInfoList)[k]->GetProperties();
...
void Item_SaveBaseInfo(ColumnAndValue &ItemValue, TDBItemData *pItemData) {
  char szValue[100];
  // ItemValue["iid"]				   = itoa(pItemData->iid,
  // szValue, 10);
...
S3DBI_API void S3DBI_ShowDebugInfo(IN BYTE *pRoleBuffer, char *pShowString) {
  ...
  if (pRoleData->dwFSkillOffset > 0) {
    ...
    TDBSkillData *pSkillData =
        (TDBSkillData *)((BYTE *)pRoleData + pRoleData->dwFSkillOffset);
  ...
  if (pRoleData->dwLSkillOffset > 0) {
    TDBSkillData *pSkillData =
        (TDBSkillData *)((BYTE *)pRoleData + pRoleData->dwLSkillOffset);
  ...
  if (pRoleData->dwTaskOffset > 0) {
    ...
    for (int nT = 0; nT < nTaskCount; nT++, pTaskData++) {
      sprintf(pShowString, "%sTask%d Value%d\n", pShowString,
              pTaskData->m_nTaskId, pTaskData->m_nTaskValue);
    }
  }
      ...
      for (int j = 0; j < nEquipListCount; j++) {
        ...
        for (int t = 0; t < nEquipBaseInfoListCount; t++, pMagicAttrib++) {
          ...
          sprintf(pShowString, "%sBaseInfo[%d] %d, %d, %d, %d\n", pShowString,
                  t, pMagicAttrib->nAttribType, pMagicAttrib->nValue[0],
                  pMagicAttrib->nValue[1], pMagicAttrib->nValue[2]);
        }
        ...
        for (t = 0; t < nEquipRequireInfoListCount; t++, pMagicAttrib++) {
          ...
          sprintf(pShowString, "%sRequireInfo[%d] %d, %d, %d, %d\n",
                  pShowString, t, pMagicAttrib->nAttribType,
                  pMagicAttrib->nValue[0], pMagicAttrib->nValue[1],
                  pMagicAttrib->nValue[2]);
        }
        pItemData = (TDBItemData *)pMagicAttrib;
#else
        pItemData++;
  ...
  if (pRoleData->dwFriendOffset > 0) {
    ...
    char *pFriendBuffer =
        (char *)((BYTE *)pRoleData + pRoleData->dwFriendOffset);
...
S3DBI_API int S3DBI_SaveRoleInfo(IN BYTE *pRoleBuffer, IN char *strUser,
  ...
  if (pRoleData->dwFSkillOffset > 0) {
    ...
    for (int nT = 0; nT < nFightCount; nT++, pSkillData++) {
      ...
      FightSkillValue["ifightskill"] =
          itoa(pSkillData->m_nSkillId, szValue, 10);
      FightSkillValue["ifightskilllevel"] =
          itoa(pSkillData->m_nSkillLevel, szValue, 10);
  ...
  if (pRoleData->dwLSkillOffset > 0) {
    ...
    for (int nT = 0; nT < nLifeCount; nT++, pSkillData++) {
      ...
      LifeSkillValue["ilifeskilllevel"] =
          itoa(pSkillData->m_nSkillLevel, szValue, 10);
  ...
  if (pRoleData->dwItemOffset > 0) {
    ...
    if (pEquipList) {
      ...
      for (int j = 0; j < nEquipListCount; j++) {
        ...
        Item_SaveBaseInfo(InfoValue, pItemData);
        if (pEquipList->Add(InfoValue) < 0)
          continue; // 如果保存失败，则该物件的其它数据不用再更新了
        std::string szIid =
            (*pEquipList)[pEquipList->Size() - 1]->GetProperties()["iid"];
        ...
        for (int t = 0; t < nEquipBaseInfoListCount; t++, pMagicAttrib++) {
          ...
          (*pEquipList)[pEquipList->Size() - 1]->GetEquipBaseInfoList()->Add(
              BaseValue);
        }
        ...
        for (t = 0; t < nEquipEfficInfoListCount; t++, pMagicAttrib++) {
          ...
          (*pEquipList)[pEquipList->Size() - 1]->GetEquipEfficInfoList()->Add(
              EfficValue);
        }
        ...
        for (t = 0; t < nEquipRequireInfoListCount; t++, pMagicAttrib++) {
          ...
          (*pEquipList)[pEquipList->Size() - 1]->GetEquipRequireInfoList()->Add(
              RequireValue);
        }
        pItemData = (TDBItemData *)pMagicAttrib;
#else
...
S3DBI_API int S3DBI_DeleteRole(char *strUser) {
  ...
  if (nLastResult < 0)
    g_DebugLog("Delete[%s]Player FriendList Error!,Result = %d", strUser,
               nLastResult);
  //*************************************************
  ...
  if (nLastResult < 0)
    g_DebugLog("Delete[%s]Player FightSkillList Error!,Result = %d", strUser,
               nLastResult);
  //*************************************************
  ...
  if (nLastResult < 0)
    g_DebugLog("Delete[%s]Player LifeSkillList Error!,Result = %d", strUser,
               nLastResult);
  ...
  if (nLastResult < 0)
    g_DebugLog("Delete[%s]Player Item List Error!,Result = %d", strUser,
               nLastResult);
  ...
  if (nLastResult < 0)
    g_DebugLog("Delete[%s]Player TaskList Error!,Result = %d", strUser,
               nLastResult);
  //*************************************************
  ...
  if (nLastResult < 0)
    g_DebugLog("Delete[%s]Player InfoData Error!,Result = %d", strUser,
               nLastResult);
  return nLastResult;
}

S3DBI_API int S3DBI_GetRoleListOfAccount(char *szAccountName,
                                         S3DBI_RoleBaseInfo *RoleBaseList,
                                         int nMaxCount) {
