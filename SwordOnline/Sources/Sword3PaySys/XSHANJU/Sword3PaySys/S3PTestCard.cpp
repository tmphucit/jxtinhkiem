...
void S3PTestCard::testRemove() {
  ...
  cout << "testRemove: Card code is ==>" << cavInfo["ccordcode"].c_str()
       << endl;
  cavInfo.clear();
...
void S3PTestCard::testCardSetInfo() {
  ...
  cout << "testSetInfo: Card code first is ==>" << cavInfo["ccardcode"].c_str()
       << endl;
  card->setCardProp(cav);
  cavInfo = card->getCardProp();
  cout << "testSetInfo: Card code last is ==>" << cavInfo["ccardcode"].c_str()
       << endl;
  cout << "             Card iHoldsecond is ===>"
       << cavInfo["iholdsecond"].c_str() << endl;
  cavInfo.clear();
  delete card;
}
...
Test *S3PTestCard::suite() {
  ...
  // testSuite->addTest (new TestCaller <S3PTestCard> ("testCardInit",
  // testCardInit));
  // testSuite->addTest (new TestCaller <S3PTestCard> ("testCardGetInfo",
  // testCardSetInfo)); testSuite->addTest (new TestCaller <S3PTestCard>
  // ("testRemove", testRemove));
  testSuite->addTest(
      new TestCaller<S3PTestCard>("testGetCardList", testGetCardList));
