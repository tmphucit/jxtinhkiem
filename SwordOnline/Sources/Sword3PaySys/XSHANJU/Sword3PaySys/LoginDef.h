...
// 数据库传输消息类型定义
//------>BEGIN
#define DBLOGIN_A_LOGIN 0x0a0000            // 登录数据库
#define DBLOGIN_A_LOGOUT 0x0b0000           // 从数据库注销
#define DBLOGIN_A_ADDDBUSER 0x0c0000        // 添加数据库管理员
#define DBLOGIN_A_DELDBUSER 0x0d0000        // 删除数据库管理员
#define DBLOGIN_A_QUERYUSERLIST 0x0e0000    // 获取数据库管理员列表
#define DBLOGIN_A_LOCKDATABASE 0x0f0000     // 锁住账号数据库
#define DBLOGIN_A_ACTIVATEDATABASE 0x100000 // 解锁账号数据库
#define DBLOGIN_A_CREATEACCOUNT 0x110000    // 生成新账号
#define DBLOGIN_A_DELETEACCOUNT 0x120000    // 删除账号
//<------END
...
#define LOGIN_R_REQUEST                                                        \
  0x0 // when the login request is send from client to server
...
struct KLoginStructHead {
  DWORD Size;  // size of the struct, if the struct is been inherit,the size is
               // ref the the derive struct
  DWORD Param; // be one of the LOGIN_R_* define value combin with a LOGIN_A_*
               // value.
};
...
#define KLOGINGAMESERVERSIZE                                                   \
  44 // = GAMESERVERNAME_MAX_LEN + 2 * sizeof( DWORD ) + sizeof( int )
