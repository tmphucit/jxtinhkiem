...
void S3PTestCardInfoDAO::testUpdate() {
  /*	S3PDBConnection* pCardCon =		// 连接角色数据库
          ...
          if (NULL != pCardCon)
          {
                  ...
                  S3PResult * res;
                  std::string strQuery;
                  strQuery = "select * from cardinfo where ccardcode like
     \'001\' "; card.GetCardInfoResult(strQuery,res); if ( res->rows() != 1)
                  {
                          assert(false);
                  }
                  else
...
Test *S3PTestCardInfoDAO::suite() {
  ...
  testSuite->addTest(
      new TestCaller<S3PTestCardInfoDAO>("testCardInfoAdd", testAdd));
  testSuite->addTest(
      new TestCaller<S3PTestCardInfoDAO>("testCardInfoUpdate", testUpdate));
  testSuite->addTest(
      new TestCaller<S3PTestCardInfoDAO>("testCardInfoDelete", testDelete));
