...
public:
  ...
  static int
  AddAccount(const std::string strAccName, const std::string strPassword,
             const std::string strRealName, const std::string strIDNum = "",
             const std::string strBirthDay = "", const std::string strArea = "",
             const std::string strPhone = "");
  static int Login(const std::string strAccName /* Input */,
                   const std::string strPassword /* Input */, int iGameId = 0);
  static int Logout(const std::string strAccName, const std::string strPassword,
                    int iGameId);
  static int Report(const std::string strAccName, const std::string strPassword,
                    int iGameId);
...
protected:
  BOOL
      m_bLocked; // 用于标识数据库是否被管理员锁住停止对公共用户的访问请求提供服务
};
