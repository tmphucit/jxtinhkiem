...
S3PManipulator::~S3PManipulator() { Clear(); }

ColumnAndValue &S3PManipulator::GetProperties() { return m_properties; }

int S3PManipulator::Load() {
  if (!m_bInit)
    return (m_bLastResult = -1);

  S3PRow where(m_pTableDAO->GetTableName(), &m_primaryFields, m_pConn);

  std::string strWhere;
  std::string strQuery;
  if (where.GetExpLikeWhereAnd(strWhere) > 0) {
    strQuery =
        "select * from " + m_pTableDAO->GetTableName() + " where " + strWhere;
    S3PResult result;
...
int S3PManipulator::Load(ColumnAndValue &cav) {
  ...
  S3PRow where(m_pTableDAO->GetTableName(), &cav, m_pConn);
