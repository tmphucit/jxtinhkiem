  ...
  try { // its in one big try block
    ...
    for (unsigned int i = 0; i < res.names().size(); i++) {
      cout << setw(2) << i << setw(15)
           ...
           // Result::types(unsigned int) returns a mysql_type_info which in
           // many ways is like type_info except that it has additional sql type
           // information in it. (with one of the methods being sql_name())
           << setw(20)
           << res.types(i).name()
           // this is the C++ identifier name which most closely resembles
           // the sql name (its is implementation defined and often not very
           // readable)
           << endl;
    }
