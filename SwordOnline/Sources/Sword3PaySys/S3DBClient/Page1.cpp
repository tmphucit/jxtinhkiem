...
CPage1::CPage1() : CPropertyPage(CPage1::IDD), m_bHasLockDatabase(FALSE) {
...
void CPage1::OnLockdatabase() {
  ...
  if (NULL != theApp.m_pClientSocket) {
    ...
    BOOL bResult;
    if (!m_bHasLockDatabase) {
      bResult = theApp.m_pClientSocket->RemoteQuery(
          &dbLoginStruct, LockDBCallbackFun, (DWORD)(&iResult),
          theApp.m_serverInfo.dwWaitTime);
    } else {
      bResult = theApp.m_pClientSocket->RemoteQuery(
          &dbLoginStruct, ActivateDBCallbackFun, (DWORD)(&iResult),
          theApp.m_serverInfo.dwWaitTime);
    }
    if (bResult) {
      CString cstrMsg;
      if (1 == iResult) // Log in successfully
      {
        ...
        if (!m_bHasLockDatabase) {
          cstrMsg = _T("Failed to lock database.\r\nMaybe the net is busy, ")
                    _T("nplease try again later.");
        } else {
          cstrMsg = _T("Failed to activate database.\r\nMaybe the net is ")
...
void CPage1::OnCreateaccount() {
  ...
  if (NULL != theApp.m_pClientSocket) {
    ...
    memcpy(dbLoginStruct.szPassword, theApp.m_cstrCurPassword,
           theApp.m_cstrCurPassword.GetLength());
    dbLoginStruct.Size = sizeof(_DBLOGINSTRUCT) + LOGIN_ACCOUNT_MAX_LEN +
                         LOGIN_REALNAME_MAX_LEN + LOGIN_PASSWORD_MAX_LEN;
    ...
    if (NULL != pSendBuf) {
      ...
      memcpy(pSendBuf + dwOffset, m_cstrDefaultAccRealName,
             m_cstrDefaultAccRealName.GetLength());
      dwOffset += LOGIN_REALNAME_MAX_LEN;
      memcpy(pSendBuf + dwOffset, m_cstrDefaultPassword,
             m_cstrDefaultPassword.GetLength());
      ...
      for (DWORD i = 0; i < m_dwNumOfCreating; i++) {
        ...
        BOOL bResult = theApp.m_pClientSocket->RemoteQuery(
            (_PDBLOGINSTRUCT)pSendBuf, CreateAccountCallBack, (DWORD)(&iResult),
            theApp.m_serverInfo.dwWaitTime);
        if (bResult) {
          CString cstrMsg;
          if (1 == iResult) {
            if (FALSE == bHasPrintTitle) {
              ...
              KPIPrintToFile(
                  ...
                  time.GetHour(), time.GetMinute(), time.GetYear(),
                  time.GetMonth(), time.GetDay());
              bHasPrintTitle = TRUE;
            }
            KPIPrintToFile(pLogFile, _T("%s\t%s\t%s\r\n"), szAccount,
                           m_cstrDefaultPassword, m_cstrDefaultAccRealName);
      ...
      if (0 < i) {
        ...
        KPIPrintToFile(
            ...
            i, time.GetHour(), time.GetMinute(), time.GetYear(),
            time.GetMonth(), time.GetDay());
      }
      CString cstrMsg;
...
void CPage1::OnDestroy() {
  ...
  KPIWritePrivateProfileString(_T("Account"), _T("logpath"), theApp.m_szLogPath,
                               szSetupFilePath);
}
...
void CPage1::OnChangeNumofcreating() {
  ...
  // CPropertyPage::OnInitDialog() function and call
