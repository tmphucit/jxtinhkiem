...
CClientSocket *CClientSocket::Instance(short *psiPort /* = NULL */) {
  ...
  if (NULL != m_pInstance) {
    if ((!(m_pInstance->HasValidSocket())) && (NULL != psiPort)) {
...
CClientSocket *CClientSocket::GetInstance() { return m_pInstance; }
...
BOOL CClientSocket::CreateSocket(short siPort) {
  ...
  else
  {
          ...
          {
                  closesocket( m_s );	// Asserts that m_s can be close
  rightly. m_s = INVALID_SOCKET; bRet = FALSE;
          }
  }
  */
  return bRet;
...
BOOL CClientSocket::Start() {
          ...
          && ( NULL == m_hServer ) )
  {
          ...
          DWORD dwThreadId;
          m_hServer =
                  CreateThread( 0, 0, CClientSocket::RunServer, &m_serverParam,
  0, &dwThreadId ); bRet = TRUE;
  }*/
  return bRet;
}
...
BOOL CClientSocket::RemoteQuery(_PDBLOGINSTRUCT pParam,
                                ...
                                DWORD dwCustomParam, DWORD dwMilliseconds) {
  BOOL bRet = FALSE;
  if (NULL != pParam) {
    IBYTE iTag = 0;
    unsigned long ulSendSize = pParam->Size + sizeof(iTag) + sizeof(GUID);
    ...
    if (NULL != pSendBuf) {
      ...
      if (GUID_NULL != msgId) {
        pSendBuf[0] = iTag;
        memcpy(&(pSendBuf[1]), pParam,
               pParam->Size /*sizeof( _DBLOGINSTRUCT )*/);
        ...
        if (TRUE == S3PDBSocketParser::Encode(pSendBuf, ulSendSize, NULL,
                                              ulEncodedSize)) {
          IBYTE *pEncodedBuf = new IBYTE[ulEncodedSize];
          if (NULL != pEncodedBuf) {
            S3PDBSocketParser::Encode(pSendBuf, ulSendSize, pEncodedBuf,
                                      ulEncodedSize);
            try {
              m_pS->Write(pEncodedBuf,
                          ulEncodedSize); // douhao 调用刘鹏的socket发送
              ...
              if (WAIT_OBJECT_0 == dwResult) {
                ...
                if (TRUE == parser.Parse((char *)(m_pS->m_pRecvBuf),
                  ...
                  GUID revMsgId;
                  memcpy(&revMsgId,
                         &(parser.m_lpData[def_DBUSERNAME_MAX_LEN +
...
static DWORD WINAPI gRecvfrom(LPVOID lParam) {
  DWORD dwRet = 0;
  if (NULL != lParam) {
    CClientSocket::_PRECVPARAM pParam = (CClientSocket::_PRECVPARAM)lParam;
    pParam->iResult =
        recvfrom(pParam->s, pParam->pRecvBuf, pParam->iRecvBufSize, 0,
                 (struct sockaddr *)(&pParam->from), &(pParam->fromlen));
  }
  return dwRet;
}
//-------------------------------------------------------------------------
