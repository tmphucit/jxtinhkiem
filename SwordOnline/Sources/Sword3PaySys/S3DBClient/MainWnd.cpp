...
void CMainWnd::OnLogin() {
  ...
  if (NULL != theApp.m_pClientSocket) {
    ...
    if (IDOK == dlg.DoModal()) {
      ...
      memcpy(dbLoginStruct.szUserName, dlg.m_cstrUserName,
             dlg.m_cstrUserName.GetLength());
      theApp.m_cstrCurUsername = dlg.m_cstrUserName;
      memcpy(dbLoginStruct.szPassword, dlg.m_cstrPassword,
      ...
      int iResult = 0;
      BOOL bResult = theApp.m_pClientSocket->RemoteQuery(
          &dbLoginStruct, LoginCallback, (DWORD)(&iResult),
          theApp.m_serverInfo.dwWaitTime);
      if (bResult) {
        CString cstrMsg;
        ...
        {
          // cstrMsg = _T("The same user have connected the database server from
          // other machine."); UpdateMenuState( FALSE );
          cstrMsg = _T("Successful to log in.");
          UpdateMenuState(TRUE);
          OnRefresh();
        } else {
          cstrMsg = _T("Failed to log in.\r\nMaybe the username or password ")
                    _T("you input is wrong.");
        }
        MessageBox(cstrMsg);
      } else {
...
void CMainWnd::OnLogout() {
  ...
  if (NULL != theApp.m_pClientSocket) {
    _DBLOGINSTRUCT dbLoginStruct;
    memset(&dbLoginStruct, 0, sizeof(_DBLOGINSTRUCT));
    memcpy(dbLoginStruct.szUserName, theApp.m_cstrCurUsername,
           theApp.m_cstrCurUsername.GetLength());
    memcpy(dbLoginStruct.szPassword, theApp.m_cstrCurPassword,
    ...
    int iResult = 0;
    BOOL bResult = theApp.m_pClientSocket->RemoteQuery(
        &dbLoginStruct, LogoutCallback, (DWORD)(&iResult),
        theApp.m_serverInfo.dwWaitTime);
    if (bResult) {
      CString cstrMsg;
      if (1 == iResult) // Log in successfully
      {
        ...
        cstrMsg = _T("Failed to log out.\r\nMaybe the net is busy, please try ")
                  _T("again later.");
        MessageBox(cstrMsg);
      }
    } else {
...
static DWORD CALLBACK AddUserCallback(DWORD dwPara1, DWORD dwPara2) {
  ...
  if ((NULL != pRecvMsg) || (NULL != piRet)) {
    ...
    if ((DBLOGIN_R_ADDDBUSER | DBLOGIN_R_SUCCESS) == dwMsgParam) {
      *piRet = 1;
    } else if ((DBLOGIN_R_ADDDBUSER | DBLOGIN_R_HASNOENOUGHPRIORITY) ==
               dwMsgParam) {
      *piRet = -1;
  ...
  if (NULL != theApp.m_pClientSocket) {
    ...
    if (IDOK == dlg.DoModal()) {
      ...
      if (NULL != pSendData) {
        ...
        memcpy(pSendData + dwOffset, dlg.m_cstrName,
               dlg.m_cstrName.GetLength());
        dwOffset += def_DBUSERNAME_MAX_LEN;
        memcpy(pSendData + dwOffset, dlg.m_cstrPassword,
               dlg.m_cstrPassword.GetLength());
        ...
        int iResult = 0;
        BOOL bResult = theApp.m_pClientSocket->RemoteQuery(
            (_PDBLOGINSTRUCT)pSendData, //&dbLoginStruct,
        ...
        if (bResult) {
          ...
          if (1 == iResult) // Log in successfully
          {
            ...
            cstrMsg = _T("Has no enough priority to carry out this operation.");
            MessageBox(cstrMsg);
          } else if (-2 == iResult) {
            cstrMsg.Format(_T("User \"%s\" has existed."), dlg.m_cstrName);
          } else {
            cstrMsg = _T("Failed to access database.\r\nMaybe the net is ")
                      _T("busy, please try again later.");
            MessageBox(cstrMsg);
          }
        } else {
          MessageBox(_T("Failed to access database.\r\nMaybe the net is busy, ")
...
static DWORD CALLBACK DeleteUserCallBack(DWORD dwPara1, DWORD dwPara2) {
  ...
  if ((NULL != pRecvMsg) || (NULL != piRet)) {
    ...
    if ((DBLOGIN_R_DELDBUSER | DBLOGIN_R_SUCCESS) == dwMsgParam) {
      *piRet = 1;
    } else if ((DBLOGIN_R_DELDBUSER | DBLOGIN_R_HASNOENOUGHPRIORITY) ==
               dwMsgParam) {
      *piRet = -1;
    } else {
      *piRet = 0;
...
void CMainWnd::OnDeleteuser() {
  // TODO: Add your command handler code here
  if ((&m_page2 == GetActivePage()) && theApp.m_bConnected) {
    ...
    if (1 == uSelectedCount) {
      ...
      if (NULL != theApp.m_pClientSocket) {
        ...
        if (NULL != pSendBuf) {
          ...
          memcpy(pSendBuf + sizeof(_DBLOGINSTRUCT), cstrSelName,
                 cstrSelName.GetLength());
          CString cstrMsg;
          int iResult = 0;
          BOOL bResult = theApp.m_pClientSocket->RemoteQuery(
              (_PDBLOGINSTRUCT)pSendBuf, DeleteUserCallBack, (DWORD)(&iResult),
              theApp.m_serverInfo.dwWaitTime);
          if (bResult) {
            if (1 == iResult) {
              ...
              cstrMsg = (_T("You have no enough priority to carry out this ")
                         _T("operation."));
              MessageBox(cstrMsg);
            } else {
...
void CMainWnd::OnPreferences() {
  ...
  if (IDOK == dlg.DoModal()) {
    ...
    BYTE byField[4];
    dlg.GetAddress(byField[0], byField[1], byField[2], byField[3]);
    sprintf(theApp.m_serverInfo.szDBServerIP, "%d.%d.%d.%d", byField[0],
            byField[1], byField[2], byField[3]);
...
void CMainWnd::OnDestroy() {
  ...
  KPIWritePrivateProfileString(_T("Local"), _T("port"), szPort,
                               szSetupFilePath);
  sprintf(szPort, "%d", theApp.m_serverInfo.dwWaitTime);
  KPIWritePrivateProfileString(_T("Local"), _T("waittime"), szPort,
                               szSetupFilePath);
  KPIWritePrivateProfileString(_T("DBServer"), _T("addr"),
                               theApp.m_serverInfo.szDBServerIP,
                               szSetupFilePath);
  sprintf(szPort, "%d", theApp.m_serverInfo.siDBServerPort);
  KPIWritePrivateProfileString(_T("DBServer"), _T("port"), szPort,
                               szSetupFilePath);
...
void CMainWnd::InitClientSocket() {
  ...
  strcat(szSetupFilePath, "setup.ini");
  char szPort[32];
  KPIGetPrivateProfileString(_T("Local"), _T("port"), _T("8000"), szPort, 32,
                             szSetupFilePath);
  theApp.m_serverInfo.siLocalPort = atoi(szPort);
  KPIGetPrivateProfileString(_T("Local"), _T("waittime"), _T("6000"), szPort,
                             32, szSetupFilePath);
  theApp.m_serverInfo.dwWaitTime = atoi(szPort);
  KPIGetPrivateProfileString(_T("DBServer"), _T("addr"), _T(""),
                             theApp.m_serverInfo.szDBServerIP,
                             16 * sizeof(TCHAR), szSetupFilePath);
  KPIGetPrivateProfileString(_T("DBServer"), _T("port"), _T(""), szPort, 32,
                             szSetupFilePath);
  theApp.m_serverInfo.siDBServerPort = atoi(szPort);

  theApp.m_pClientSocket =
      CClientSocket::Instance(&(theApp.m_serverInfo.siLocalPort));
  if (NULL != theApp.m_pClientSocket) {
...
static DWORD CALLBACK QueryUserlistCallback(DWORD dwPara1, DWORD dwPara2) {
  ...
  if ((NULL != pRecvBuf) && (NULL != pListCtrl)) {
    if (pListCtrl->GetSafeHwnd()) {
      ...
      if (0 == dwUserListSize % sizeof(CClientSocket::_USERINFO)) {
        int iItemNum = dwUserListSize / sizeof(CClientSocket::_USERINFO);
        pListCtrl->DeleteAllItems();
        for (int i = 0; i < iItemNum; i++) {
          memset(&item, 0, sizeof(CClientSocket::_USERINFO));
          memcpy(&item,
                 &pRecvBuf[1 + sizeof(_DBLOGINSTRUCT) + sizeof(GUID) +
                           i * sizeof(CClientSocket::_USERINFO)],
...
BOOL CMainWnd::QueryUserlist(const char *lpszUserName,
  ...
  if ((0 == lpszUserName[0]) || (0 == lpszPassword[0])) {
    bRet = FALSE;
    return bRet;
  }
  if (NULL != theApp.m_pClientSocket) {
    ...
    BOOL bResult = theApp.m_pClientSocket->RemoteQuery(
        &dbLoginStruct, QueryUserlistCallback, (DWORD)(pListCtrl),
        theApp.m_serverInfo.dwWaitTime);
    if (bResult) {
      bRet = TRUE;
    } else {
...
void CMainWnd::OnTurntoPage2(WPARAM wParam, LPARAM lParam) { OnRefresh(); }
