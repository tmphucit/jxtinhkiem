...
BOOL CPage2::OnInitDialog() {
  ...
  static const CString itemNameArray[] = {
      _T("User name"), _T("Host addr"),       _T("Priority"),
      _T("State"),     _T("Last login time"), _T("Last logout time")};
  static const int itemWidthArray[] = {70, 75, 60, 55, 120, 120};
  for (int i = 0; i < 6; i++) {
    m_dbUserListCtrl.InsertColumn(i, itemNameArray[i], LVCFMT_LEFT,
                                  itemWidthArray[i]);
  }
