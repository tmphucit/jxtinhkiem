...
CS3DBClientApp::CS3DBClientApp()
    : m_bConnected(FALSE), m_pClientSocket(NULL), m_cstrCurUsername(_T("")),
...
void CS3DBClientApp::InitAccOperation() {
  ...
  KPIGetPrivateProfileString(_T("Account"), _T("password"), _T("888888"),
                             m_szDefaultAccPassword, LOGIN_PASSWORD_MAX_LEN,
                             szSetupFilePath);
  m_szDefaultAccRealName[LOGIN_REALNAME_MAX_LEN] = 0;
  KPIGetPrivateProfileString(_T("Account"), _T("realname"), _T("kingsoft"),
                             m_szDefaultAccRealName, LOGIN_PASSWORD_MAX_LEN,
                             szSetupFilePath);
  m_szLogPath[MAX_PATH] = 0;
  KPIGetPrivateProfileString(_T("Account"), _T("logpath"),
                             _T("C:\\account.log"), m_szLogPath, MAX_PATH,
                             szSetupFilePath);
}
