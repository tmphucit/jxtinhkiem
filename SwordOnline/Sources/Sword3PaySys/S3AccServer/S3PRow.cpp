...
S3PRow::S3PRow(const std::string &tableName, ColumnAndValue *pCAV,
...
int S3PRow::FixValue(const std::string &key, const std::string &value,
                     std::string &outValue) {
...
BOOL S3PRow::IsNeedQuote(const std::string &column) {
  ...
  rpattern pat("^\\s*(bigint|decimal|double|enum|float|int|mediumint|numeric|"
               "real|set|smallint|tinyint)",
               NOCASE);
