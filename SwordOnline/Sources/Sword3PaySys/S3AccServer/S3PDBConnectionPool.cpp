...
DWORD GetGMID() { return g_nGMID; }

S3PDBConnectionPool::S3PDBConnectionPool() { m_dwConLimits = 0; }
...
BOOL S3PDBConnectionPool::Init(const std::string &strINIPath,
  ...
  if (!(strINIPath.empty())) {
    ...
    DWORD dwLen = KPIGetPrivateProfileString(
        strSection.c_str(), def_SERVERKEYNAME, "", szServer, def_KEYNAMELEN,
        strINIPath.c_str());
    dwLen = KPIGetPrivateProfileString(strSection.c_str(), def_DATABASEKEYNAME,
                                       "", szDataBase, def_KEYNAMELEN,
                                       strINIPath.c_str());
    dwLen =
        KPIGetPrivateProfileString(strSection.c_str(), def_USERKEYNAME, "",
                                   szUser, def_KEYNAMELEN, strINIPath.c_str());
    dwLen = KPIGetPrivateProfileString(strSection.c_str(), def_PASSWORDKEYNAME,
...
BOOL S3PDBConnectionPool::CreateConnection(_LPDATABASEINFO pDatabase,
                                           S3PDBConVBC **ppInfo) {
