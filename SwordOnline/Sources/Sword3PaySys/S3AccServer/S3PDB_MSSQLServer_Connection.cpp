// S3PDB_MSSQLServer_Connection.cpp: implementation of the
// S3PDB_MSSQLServer_Connection class.
//
//////////////////////////////////////////////////////////////////////
  ...
  if (NULL == m_pCon) {
    ...
    if ((SUCCEEDED(hr)) && (NULL != pCon)) {
      ...
      try {
        ...
        hr = pCon->Open(strSource.c_str(), lpDBIdentifier->strUser.c_str(),
        ...
        gTrace("[S3PDBConnectionPool::OpenConnect]COM error: %s",
               e.ErrorMessage());
      }
    }
  }
...
bool S3PDB_MSSQLServer_Connection::DoCmd(const char *lpszSql) {
  ...
  if (lpszSql && strlen(lpszSql) <= 128) {
    gTrace("[Debug] SQL CMD [%s]", lpszSql);
  } else {
    gTrace("[Debug] SQL CMD NOT [%d]", strlen(lpszSql));
  }
...
bool S3PDB_MSSQLServer_Connection::DoResult(const char *lpszSql) {
  ...
  if (lpszSql && strlen(lpszSql) <= 128) {
    gTrace("[Debug] SQL Result [%s]", lpszSql);
  } else {
    gTrace("[Debug] SQL Result NOT [%d]", strlen(lpszSql));
  }
  ...
  try {
    try {
      ...
      hr = pResult->Open(lpszSql, _variant_t((IDispatch *)(m_pCon), true),
                         adOpenDynamic, adLockOptimistic, adCmdText);
      if (SUCCEEDED(hr)) {
        bRet = TRUE;
      }
    } catch (_com_error &e) {
      gTrace("[S3PDB_MSSQLServer_Connection::Do COM error: %s",
             e.ErrorMessage());
    }
  } catch (_com_error &e) {
    gTrace("[S3PDB_MSSQLServer_Connection::Do COM error: %s", e.ErrorMessage());
...
bool S3PDB_MSSQLServer_Connection::QuerySql(const char *lpszSql,
                                            S3PResultVBC **ppResult) {
  *ppResult = NULL;
  BOOL bRet = FALSE;
...
bool S3PDB_MSSQLServer_Connection::QueryResult(const char *lpszSql,
                                               S3P_MSSQLServer_Result *pRes) {

  if (lpszSql && strlen(lpszSql) <= 128) {
    gTrace("[Debug] SQL Query Result [%s]", lpszSql);
  } else {
    gTrace("[Debug] SQL Query Result NOT [%d]", strlen(lpszSql));
  }

  BOOL bRet = false;
  if ((NULL != m_pCon) && (NULL != lpszSql) && (NULL != pRes)) {
    try {
      _RecordsetPtr pResult;
      HRESULT hr = pResult.CreateInstance("ADODB.Recordset");
      hr = pResult->Open(lpszSql, _variant_t((IDispatch *)(m_pCon), true),
...
#endif
      gTrace("[S3PDB_MSSQLServer_Connection::DoSql COM error: %s",
             e.ErrorMessage());
      bRet = FALSE;
    }
  }
...
bool S3PDB_MSSQLServer_Connection::QueryCmd(const char *lpszSql,
                                            S3P_MSSQLServer_Result *pRes) {

  if (lpszSql && strlen(lpszSql) <= 128) {
    gTrace("[Debug] SQL Query CMD [%s]", lpszSql);
  } else {
    gTrace("[Debug] SQL Query CMD NOT [%d]", strlen(lpszSql));
  }
...
bool S3PDB_MSSQLServer_Connection::GetFreeResult(
    S3P_MSSQLServer_Result **ppResult) {
  assert(ppResult);
