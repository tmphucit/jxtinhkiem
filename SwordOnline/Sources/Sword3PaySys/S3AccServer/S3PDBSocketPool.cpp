  ...
  while (i != m_clientIDs.end()) {
    if (*i) {
      ...
      case KGatewayDataProcess::gdp_work: {
        ...
        gTrace("Game %d: %s(%s:%d) is working", (*i)->m_nGameID,
               (*i)->m_ServerName, inet_ntoa(in_addr(add)), (*i)->m_Port);
      } break;
      case KGatewayDataProcess::gdp_again: {
        in_addr add;
        add.s_addr = (*i)->m_Address;
        gTrace("Game %d: %s(%s:%d) is waiting", (*i)->m_nGameID,
               (*i)->m_ServerName, inet_ntoa(in_addr(add)), (*i)->m_Port);
      } break;
...
void KGatewayDataProcess::AutoTime() {
  ...
  if (m_nLeftStatusTime > 0 && (nNow - m_nStatusTime > m_nLeftStatusTime) &&
      IsWorkAgain()) {
  ...
  if (m_nLeftPingTime > 0 && (nNow - m_LastPingTime) >= m_nLeftPingTime &&
      IsWork()) {
    m_pServer->ShutdownClient(m_nConnectID);
  }
...
void KGatewayDataProcess::ProcessClientData(const void *pData,
  ...
  if (pData && dwDataSize > 0) {
    ...
    if (nProtocol == c2s_ping) {
      ...
      in_addr ar;
      ar.s_addr = m_Address;
      gTrace("Gateway %s(%s): Ping %d(s)", m_ServerName, inet_ntoa(ar),
  ...
  if (nProtocol == c2s_gatewayverify || nProtocol == c2s_gatewayverifyagain) {
    if (GetStatus() != gdp_verify && GetStatus() != gdp_verifyagain) {
      ...
      if (nProtocol == c2s_gatewayverifyagain && IsVerifyTimeout()) {
        ...
        if (bRet) {
          if (nProtocol ==
              c2s_gatewayverify) // gatewayverify成功后要根据状态清理现场
          {
    ...
    if (GetStatus() == gdp_work) {
      if (nProtocol >= c2s_accountbegin && nProtocol < c2s_multiserverbegin &&
        ...
        bRet = (this->*ProcessFunc[nProtocol - c2s_accountbegin])(
            pConn, ((char *)pData) + 1, dwDataSize - 1);
      } else
...
BOOL KGatewayDataProcess::ProAccountLogin(S3PDBConVBC *pConn,
                                          const IBYTE *lpData,
  ...
  if (NULL != lpData && dwDataSize == sizeof(KAccountUserLoginInfo)) {
    ...
    if (pAccInfo->Version == ACCOUNT_CURRENT_VERSION &&
        (pAccInfo->Type == AccountUserLoginInfo ||
         pAccInfo->Type == AccountUserVerify)) {
      ...
      if (m_UserNames.find(*((UserName *)pAccInfo->Account)) ==
          m_UserNames.end()) // 不在黑名单上
      {
        if (pAccInfo->Type == AccountUserLoginInfo)
          aReturn.nReturn = S3PAccount::Login(
              pConn, pAccInfo->Account, pAccInfo->Password, m_nGameID,
              aReturn.nExtPoint, aReturn.nLeftTime);
        else
          aReturn.nReturn = S3PAccount::VerifyUserModifyPassword(
              pConn, m_nGameID, pAccInfo->Account, pAccInfo->Password);
      } else
      ...
      if (pAccInfo->Type == AccountUserLoginInfo)
        gTrace("Gateway %s(%s): User %s Login%s", m_ServerName, inet_ntoa(ar),
               pAccInfo->Account, szDetail);
      else
        gTrace("Gateway %s(%s): User %s SecondPassword%s", m_ServerName,
...
BOOL KGatewayDataProcess::ProGameLogin(S3PDBConVBC *pConn, const IBYTE *lpData,
  ...
  if (NULL != lpData && dwDataSize == sizeof(KAccountUser)) {
    ...
    if (pAccInfo->Version == ACCOUNT_CURRENT_VERSION &&
      ...
      memcpy(aReturn.Account, pAccInfo->Account, LOGIN_USER_ACCOUNT_MAX_LEN);
      aReturn.nReturn =
          S3PAccount::LoginGame(pConn, m_nGameID, pAccInfo->Account);
      ...
      gTrace("Gateway %s(%s): User %s Enter Game%s", m_ServerName,
             inet_ntoa(ar), pAccInfo->Account, szDetail);
...
BOOL KGatewayDataProcess::ProAccountLogout(S3PDBConVBC *pConn,
  ...
  if (NULL != lpData && dwDataSize == sizeof(KAccountUserLogout)) {
    ...
    if (pAccInfo->Version == ACCOUNT_CURRENT_VERSION &&
      ...
      aReturn.nReturn = S3PAccount::Logout(pConn, m_nGameID, pAccInfo->Account,
                                           pAccInfo->nExtPoint);
      ...
      gTrace("Gateway %s(%s): User %s Logout%s", m_ServerName, inet_ntoa(ar),
             pAccInfo->Account, szDetail);
...
BOOL KGatewayDataProcess::ProGetwayVerify(S3PDBConVBC *pConn,
                                          const IBYTE *lpData,
                                          const DWORD dwDataSize) {
  BOOL bRet = FALSE;
  if (NULL != lpData && dwDataSize == sizeof(KServerAccountUserLoginInfo)) {
    ...
    if (pAccInfo->Version == ACCOUNT_CURRENT_VERSION &&
      ...
      szmac[14] = 0;
      if (bRet)
        gTrace("Gateway %s(%s)%s: Verify OK !", m_ServerName, inet_ntoa(ar),
               szmac);
      else {
        if (aReturn.nReturn == E_ACCOUNT_EXIST)
          gTrace("Gateway %s(%s)%s: Already login !", m_ServerName,
                 inet_ntoa(ar), szmac);
        else if (aReturn.nReturn == E_ACCOUNT_OR_PASSWORD)
          gTrace("Gateway %s(%s)%s: Name, Password !", m_ServerName,
                 inet_ntoa(ar), szmac);
        else if (aReturn.nReturn == E_ADDRESS_OR_PORT)
          gTrace("Gateway %s(%s)%s: Address, Port !", m_ServerName,
                 inet_ntoa(ar), szmac);
      }
...
BOOL KGatewayDataProcess::ConnectErrorWork(unsigned long nID,
                                           unsigned long Address, short Port) {
  BOOL bRet = FALSE;
  Lock();

  if (m_Status == gdp_verify || // 只有这几个状态下的m_Address有效
      m_Status == gdp_work || m_Status == gdp_verifyagain ||
...
BOOL KGatewayDataProcess::ConnectFreeWork(unsigned long nID,
                                          unsigned long Address, short Port) {
  BOOL bRet = FALSE;
  Lock();
...
BOOL KGatewayDataProcess::ConnectAgainWork(unsigned long nID,
                                           unsigned long Address, short Port) {
  BOOL bRet = FALSE;
  Lock();
...
BOOL KGatewayDataProcess::ConnectFreeForErrorWork(unsigned long nID,
...
BOOL KGatewayDataProcess::IsWantData() {
  BOOL bRet = FALSE;
  Lock();
  if (m_Status == gdp_work || m_Status == gdp_again || m_Status == gdp_verify ||
      m_Status == gdp_verifyagain) {
    bRet = TRUE;
  }
  Unlock();
...
bool KGatewayDataProcess::SetStatus(int nNews) {
  ...
  if (m_Status == gdp_free && nNews == gdp_verify) {
    ...
    bRet = true;
             ...
             (nNews == gdp_verifyagain || nNews == gdp_free)) {
    m_Status = nNews;
    bRet = true;
  } else if (m_Status == gdp_verifyagain &&
             (nNews == gdp_work || nNews == gdp_again)) {
    m_Status = nNews;
    bRet = true;
...
bool KGatewayDataProcess::IsVerifyTimeout() {
  ...
  if (m_Status == gdp_verify || m_Status == gdp_verifyagain) {
    if ((GetTickCount() - m_nStatusTime) <=
        m_nLeftStatusTime) { // 正确状态的有限时间内,才叫不超时
      bRet = false;
    }
  }
  Unlock();
