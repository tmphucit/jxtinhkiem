...
public:
  ...
  static int Login(S3PDBConVBC *pConn, const char *strAccName,
                   const char *strPassword, DWORD ClientID, WORD &nExtPoint,
                   DWORD &nLeftTime);
  static int LoginGame(S3PDBConVBC *pConn, DWORD ClientID,
                       const char *strAccName);
  static int Logout(S3PDBConVBC *pConn, DWORD ClientID, const char *strAccName,
                    WORD nExtPoint);
  static int ElapseTime(S3PDBConVBC *pConn, DWORD ClientID,
                        const char *strAccName, const DWORD dwDecSecond);
  static int QueryTime(S3PDBConVBC *pConn, DWORD ClientID,
                       const char *strAccName, DWORD &dwSecond);
  static int ServerLogin(S3PDBConVBC *pConn, const char *strAccName,
  ...
  static int CheckAddress(S3PDBConVBC *pConn, const DWORD Address,
                          const short Port);
  static int GetAccountGameID(S3PDBConVBC *pConn, const char *strAccName,
                              DWORD &ClientID);
  static int VerifyUserModifyPassword(S3PDBConVBC *pConn, DWORD ClientID,
                                      const char *strAccName,
                                      const char *strPassword);
  static int GetServerID(S3PDBConVBC *pConn, const char *strAccName,
                         unsigned long &nGameID);

protected:
  static int GetLeftSecondsOfDeposit(S3PDBConVBC *pConn, const char *strAccName,
                                     long &liLeft, long &liExp,
                                     long &liNumLogin);
};
