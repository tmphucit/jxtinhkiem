...
#ifdef TRACE2FILE
  if (!s_debugOut.is_open()) {
    s_debugOut.open(s_logFileName.c_str(),
                    std::ios_base::out | std::ios_base::app);

    s_debugOut << _T( "************ OnlineGame newest log *************" )
               << endl;
  }
...
_tstring GetLastErrorMessage(DWORD last_error) {
  ...
  if (!FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, 0, last_error, 0, &errmsg[len],
                     511, NULL)) {
    /*
  ...
  if (verSize != 0) {
    ...
    if (::GetFileVersionInfo(pModuleFileName, 0, verSize, spBuffer.get())) {
      ...
      if (::VerQueryValue(spBuffer.get(),
                          const_cast<LPTSTR>(
                              _T("\\StringFileInfo\\080904b0\\ProductVersion")),
