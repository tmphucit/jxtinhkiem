...
bool tagSockAddrIn::IsEqual(const tagSockAddrIn &sin) {
   ...
   */
  return (memcmp(&this->sockAddrIn, &sin.sockAddrIn,
                 Size() - sizeof(sockAddrIn.sin_zero)) == 0);
}
 ...
 * CSocketServer
 */
CSocketServer::CSocketServer(unsigned long addressToListenOn,
    ...
    : CIOBuffer::Allocator(bufferSize, maxFreeBuffers),
      ...
      m_listeningSocket(INVALID_SOCKET), m_iocp(0),
      m_address(addressToListenOn), m_port(portToListenOn),
...
void CSocketServer::ReleaseBuffers() { Flush(); }
...
void CSocketServer::Read(Socket *pSocket, CIOBuffer *pBuffer) {
   ...
   * performed by one of our IO threads...
   */
...
void CSocketServer::Write(Socket *pSocket, const char *pData, size_t dataLength,
                          bool thenShutdown) {
  if (!pSocket || !pData || dataLength <= 0) {
    return;
  }
  /*
   * Post a write request to the iocp so that the actual socket write gets
   * performed by one of our IO threads...
   */
...
#ifdef NETWORK_DEBUG
  //{
  PreWrite(pSocket, pBuffer, pData,
           dataLength + PACK_HEADER_LEN + sizeof(BYTE));
//}
#else
  //{
...
void CSocketServer::Write(Socket *pSocket, CIOBuffer *pBuffer,
                          bool thenShutdown) {
  /*
   * Post a write request to the iocp so that the actual socket write gets
   * performed by one of our IO threads...
...
void CSocketServer::OnError(const _tstring &message) { Output(message); }

/*
 * CSocketServer::Socket
 */
CSocketServer::Socket::Socket(CSocketServer &server, SOCKET theSocket)
    : m_server(server), m_socket(theSocket), m_ref(1) {
...
void CSocketServer::Socket::Close() {
  ...
  if (INVALID_SOCKET != m_socket) {
    if (0 != ::closesocket(m_socket)) {
      m_server.OnError(_T("CSocketServer::Socket::Close() - closesocket - ") +
                       GetLastErrorMessage(::WSAGetLastError()));
    }
...
void CSocketServer::Socket::Write(const char *pData, size_t dataLength,
                                  bool thenShutdown /* = false */) {
  m_server.Write(this, pData, dataLength, thenShutdown);
}

void CSocketServer::Socket::Write(CIOBuffer *pBuffer,
                                  bool thenShutdown /* = false */) {
...
int CSocketServer::WorkerThread::Run() {
  try {
    while (true) {
      ...
      try {
        m_iocp.GetStatus((PDWORD_PTR)&pSocket, &dwIoSize,
                         (OVERLAPPED **)&pBuffer);
      ...
      if (pBuffer) {
        const IO_Operation operation =
            static_cast<IO_Operation>(pBuffer->GetUserData());
        ...
        case IO_Read_Completed:
          ...
          if (0 != dwIoSize) {
            ...
            // pBuffer->GetWSABUF()->buf), dwIoSize, 40) ) );
        ...
        case IO_Write_Completed:
          ...
          // DumpData(reinterpret_cast<const BYTE*>( pBuffer->GetWSABUF()->buf),
          // dwIoSize, 40) ) );
         ...
         * Call to unqualified virtual function
         */
        OnError(_T("CSocketServer::WorkerThread::Run() - Unexpected - pBuffer ")
                _T("is 0"));
      }
     ...
     * Call to unqualified virtual function
     */
    OnError(_T("CSocketServer::WorkerThread::Run() - Exception: ") +
...
void CSocketServer::WorkerThread::Read(Socket *pSocket,
  ...
  if (SOCKET_ERROR == ::WSARecv(pSocket->m_socket, pBuffer->GetWSABUF(), 1,
    ...
    if (ERROR_IO_PENDING != lastError) {
      Output(_T("CSocketServer::Read() - WSARecv: ") +
...
void CSocketServer::WorkerThread::Write(Socket *pSocket,
  ...
  if (SOCKET_ERROR == ::WSASend(pSocket->m_socket, pBuffer->GetWSABUF(), 1,
    ...
    if (ERROR_IO_PENDING != lastError) {
      Output(_T("CSocketServer::Write() - WSASend: ") +
             GetLastErrorMessage(lastError));
...
void CSocketServer::WorkerThread::WriteCompleted(Socket * /*pSocket*/,
                                                 CIOBuffer *pBuffer) {
  if (pBuffer->GetUsed() != pBuffer->GetWSABUF()->len) {
    /*
     * Call to unqualified virtual function
     */
    _ASSERT(_T("CSocketServer::WorkerThread::WriteCompleted - Socket write ")
...
void CSocketServer::WorkerThread::AbortiveClose(Socket *pSocket) {
  ...
  if (SOCKET_ERROR == ::setsockopt(pSocket->m_socket, SOL_SOCKET, SO_LINGER,
                                   (char *)&lingerStruct,
                                   sizeof(lingerStruct))) {
    /*
     * Call to unqualified virtual function
     */
    OnError(_T("CSocketServer::Socket::AbortiveClose() - ")
            _T("setsockopt(SO_LINGER) - ") +
            GetLastErrorMessage(::WSAGetLastError()));
  }
