...
S3PRelockAccount::S3PRelockAccount() { Init(); }
  ...
  if (NULL != pAccountCon) {
    while (TRUE) {
      ...
      } else if (2 == dwResult) // Time out
      {
        ...
        //------>BEGIN
        pAccountCon->Do("update account_info set iGameId = 0 where iGameId <> "
                        "0 and iTimeCount = 0");
        pAccountCon->Do("update account_info set iTimeCount = 0 where iGameId "
                        "<> 0 and iTimeCount <> 0");
        //<------END
      }
    }
...
HANDLE S3PRelockAccount::Start() {
  ...
  m_hRelocker =
      KPICreateThread(S3PRelockAccount::Relock, &m_param, &m_dwRelockerID);
  hRet = m_hRelocker;
