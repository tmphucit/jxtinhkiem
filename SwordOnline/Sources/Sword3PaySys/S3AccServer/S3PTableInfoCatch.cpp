...
S3PTableInfoCatch::GetColumnInfo(std::string tableName,
      ...
      if (pResult) {
        ...
        if (col_count > 0) {
          ...
          for (unsigned int i = 0; i < col_count; i++) {
            ...
            columnInfoMap->insert(
                std::map<std::string, std::string>::value_type(column, type));
          }
      ...
      if (pResult) {
        ...
        if (col_count > 0) {
          ...
          for (long i = 0; i < col_count; i++) {
            ...
            strType = szDataType;
            columnInfoMap->insert(
                std::map<std::string, std::string>::value_type(strName,
                                                               strType));
          }
          m_initedMap[tableName] = columnInfoMap;
          return columnInfoMap;
