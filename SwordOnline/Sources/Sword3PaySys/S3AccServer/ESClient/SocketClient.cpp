...
void CSocketClient::ConnectTo(const _tstring &addressToConnectServer,
                              unsigned short portToConnectServer) {
...
void CSocketClient::OnRead(CIOBuffer *pBuffer) {
  ...
  if (SOCKET_ERROR == ::WSARecv(m_connectSocket, pBuffer->GetWSABUF(), 1,
    ...
    if (ERROR_IO_PENDING != lastError) {
      Output(_T("CSocketClient::OnRead() - WSARecv: ") +
...
void CSocketClient::Write(const char *pData, size_t dataLength) {
  if (INVALID_SOCKET != m_connectSocket && dataLength > 0 && pData) {
    ...
    if (SOCKET_ERROR == ::WSASend(m_connectSocket, pBuffer->GetWSABUF(), 1,
                                  &dwSendNumBytes, dwFlags, pBuffer, NULL)) {
      DWORD lastError = ::WSAGetLastError();

      if (ERROR_IO_PENDING != lastError) {
        Output(_T("CSocketClient::Write() - WSASend: ") +
               GetLastErrorMessage(lastError));
    ...
    if (pBuffer->GetUsed() != pBuffer->GetWSABUF()->len) {
       ...
       * Call to unqualified virtual function
       */
      // OnError(_T("CSocketClient::WriteCompleted - Socket write where not all
      // data was written"));
    }
