	...
	if (pConn->QuerySql(strSQL, &pResult) )
	{
		...
		else
		{
			...
			if (clientID.vt == VT_NULL || clientID.lVal == 0)
			{
				...
				long iExp = 0;
				long NumLogin = 0;
				if (GetLeftSecondsOfDeposit(pConn, strAccName, iLeft, iExp, NumLogin) == ACTION_SUCCESS && iLeft > 1800)	//最小剩余时间30分钟
				{
					...
					if (pConn->Do(strSQL))
					{
						...
						if (iExp != 0)
						{	
							sprintf(strSQL, "update View_AccountMoney set nExtPoint = 0,iNumLogin = %d where (cAccName = '%s')",iExp, strAccName);
							pConn->Do(strSQL);
						}
					}
					else
...
int S3PAccount::LoginGame(S3PDBConVBC* pConn, DWORD ClientID, const char* strAccName)
{
	...
	if (iRet == ACTION_SUCCESS)
	{
		if (NewClientID == ClientID)
		{
			...
			{
				iRet = ACTION_SUCCESS;
				sprintf(strSQL, "update View_AccountMoney set iNumLogin = 0 where (cAccName = '%s')", strAccName);
				pConn->Do(strSQL);
			}
			else
				iRet = ACTION_FAILED;
...
int S3PAccount::Logout(S3PDBConVBC* pConn, DWORD ClientID, const char* strAccName, WORD nExtPoint)
{
	...
	sprintf(strSQL, "update View_AccountMoney set iLeftSecond = iLeftSecond - (datediff(second, dLoginDate, getdate())) where (datediff(second, getdate(), dEndDate) <= 0)",
		 strAccName);
	pConn->Do(strSQL);	//扣点,即使被的冻结帐户

	if (nExtPoint != 0)
	{
		//sprintf(strSQL, "update View_AccountMoney set nExtPoint = CASE WHEN nExtPoint - %d >= 0 THEN nExtPoint - %d WHEN nExtPoint - %d < 0 THEN 0 END where cAccName = '%s'",
...
//		{
			...
			{
				iRet = ACTION_SUCCESS;
				sprintf(strSQL, "update View_AccountMoney set nExtPoint = iNumLogin, iNumLogin = 0 where (cAccName = '%s') and (iNumLogin > 0)",strAccName);
				pConn->Do(strSQL);
										...
										long& liNumLogin)
{
	...
	char strSQL[MAX_PATH];
	//sprintf(strSQL, "select datediff(second, getdate(), dEndDate), iLeftSecond, nExtPoint, iNumLogin from View_AccountMoney where (cAccname = '%s')", strAccName); // old qry
	sprintf(strSQL, "select datediff(second, getdate(), dEndDate), iLeftSecond, nExtPoint from View_AccountMoney where (cAccname = '%s')", strAccName); //new fix
	S3PResultVBC* pResult = NULL;
	if (pConn->QuerySql(strSQL, &pResult))
	{
		if (pResult->num_rows() <= 0)
