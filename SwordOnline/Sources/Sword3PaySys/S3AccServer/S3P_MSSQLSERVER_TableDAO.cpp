// S3P_MSSQLSERVER_TableDAO.cpp: implementation of the S3P_MSSQLSERVER_TableDAO
// class.
//
//////////////////////////////////////////////////////////////////////
...
int S3P_MSSQLServer_TableDAO::AddGroup(
    const std::list<ColumnAndValue *> &group) {
  BOOL bFirst = TRUE;
  std::string strQuery;
  ...
  if (NULL != pCon) {
    if (NULL != pCon->m_info.pCon) {
      try {
        ...
        _RecordsetPtr pRes;
        HRESULT hr = pRes.CreateInstance("ADODB.Recordset");
        if (SUCCEEDED(hr)) {
          std::string strTableName = GetTableName();
          char szSource[256];
          sprintf(szSource, "SELECT IDENT_CURRENT('%s')", strTableName.c_str());
          hr = pRes->Open(szSource, (IDispatch *)(pCon->m_info.pCon),
...
int S3P_MSSQLServer_TableDAO::Query(S3PDBConnection *pConn, string q,
  ...
  if (pConn) {
    if (pConn->QueryBySql(q.c_str())) {
      S3PRawResult pResult = (S3PRawResult)((IDispatch *)(pConn->GetRes()));
      if (NULL != pResult) {
        // Success
