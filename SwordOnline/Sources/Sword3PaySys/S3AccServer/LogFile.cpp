...
CLogFile::~CLogFile() { assert(m_threshold == 0); }
...
BOOL CLogFile::PrepareFile() {
  ...
  sprintf(szTime, "\\%02d_%02d_%02d_%03d", systm.wHour, systm.wMinute,
...
BOOL CLogFile::OpenUniDirectory(const std::string &dirname,
                                std::string *pNewdir) {
...
HANDLE CLogFile::CreateUniFile(const std::string &filename,
                               std::string *pNewfile) {
  ...
  HANDLE hFile = ::CreateFile(file.c_str(), GENERIC_WRITE, FILE_SHARE_READ,
