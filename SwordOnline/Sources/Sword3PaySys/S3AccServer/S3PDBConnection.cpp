...
S3PDB_mySQL_Connection::S3PDB_mySQL_Connection() : m_pRes(NULL), m_pCon(NULL) {}
...
bool S3PDB_mySQL_Connection::QuerySql(const char *lpszSql,
                                      S3PResultVBC **ppResult) {
  BOOL bRet = FALSE;
  m_pRes = NULL;
  if ((NULL != lpszSql) && (NULL != m_pCon)) {
    try {
