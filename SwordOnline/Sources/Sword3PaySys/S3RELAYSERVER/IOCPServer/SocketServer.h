...
public:
  ...
  bool IsNull() const {
    return ((sockAddrIn.sin_addr.s_addr == 0L) && (sockAddrIn.sin_port == 0));
  }
...
protected:
  CSocketServer(unsigned long addressToListenOn, unsigned short portToListenOn,
...
private:
  ...
  void Write(Socket *pSocket, const char *pData, size_t dataLength,
             bool thenShutdown);
  void Write(Socket *pSocket, CIOBuffer *pBuffer, bool thenShutdown);

  virtual void PreWrite(Socket *pSocket, CIOBuffer *pBuffer, const char *pData,
                        size_t dataLength) {};
