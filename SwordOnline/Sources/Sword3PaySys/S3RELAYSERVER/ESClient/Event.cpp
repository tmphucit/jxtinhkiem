 ...
 */
static HANDLE Create(LPSECURITY_ATTRIBUTES lpEventAttributes, bool bManualReset,
...
CEvent::CEvent(LPSECURITY_ATTRIBUTES lpEventAttributes, bool bManualReset,
               bool bInitialState)
    : m_hEvent(Create(lpEventAttributes, bManualReset, bInitialState, 0)) {}

CEvent::CEvent(LPSECURITY_ATTRIBUTES lpEventAttributes, bool bManualReset,
               bool bInitialState, const _tstring &name)
    : m_hEvent(Create(lpEventAttributes, bManualReset, bInitialState,
                      name.c_str())) {}

CEvent::~CEvent() { ::CloseHandle(m_hEvent); }

HANDLE CEvent::GetEvent() const { return m_hEvent; }

void CEvent::Wait() const {
  if (!Wait(INFINITE)) {
    throw CException(_T("CEvent::Wait()"),
...
bool CEvent::Wait(DWORD timeoutMillis) const {
  ...
  if (result == WAIT_TIMEOUT) {
    ...
    throw CWin32Exception(_T("CEvent::Wait() - WaitForSingleObject"),
                          ::GetLastError());
  }
