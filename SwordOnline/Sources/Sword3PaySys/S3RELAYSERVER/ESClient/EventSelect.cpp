...
CEventSelect::CEventSelect() : m_event(WSA_INVALID_EVENT) {
...
void CEventSelect::AssociateEvent(SOCKET s, long lNetworkEvents) {
  if (WSA_INVALID_EVENT == (m_event = ::WSACreateEvent())) {
    throw CWin32Exception(_T("CEventSelect::AssociateEvent() - WSACreateEvent"),
                          ::WSAGetLastError());
  }

  if (SOCKET_ERROR == ::WSAEventSelect(s, m_event, lNetworkEvents)) {
    throw CWin32Exception(_T("CEventSelect::AssociateEvent() - WSAEventSelect"),
                          ::WSAGetLastError());
  }
}
...
bool CEventSelect::WaitForEnumEvent(SOCKET s, DWORD dwTimeout) {
  ...
  DWORD dwRet =
      ::WSAWaitForMultipleEvents(1, &m_event, FALSE, dwTimeout, FALSE);

  if (WSA_WAIT_TIMEOUT == dwRet || WSA_WAIT_FAILED == dwRet) {
    return false;
  }

  ::WSAEnumNetworkEvents(s, m_event, &m_networkEvents);
