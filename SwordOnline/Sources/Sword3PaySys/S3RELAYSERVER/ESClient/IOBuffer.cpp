...
CIOBuffer::CIOBuffer(Allocator &allocator, size_t size)
    : m_operation(0), m_allocator(allocator), m_ref(1), m_size(size),
      m_used(0) {
...
WSABUF *CIOBuffer::GetWSABUF() const { return const_cast<WSABUF *>(&m_wsabuf); }

size_t CIOBuffer::GetUsed() const { return m_used; }

size_t CIOBuffer::GetSize() const { return m_size; }

const BYTE *CIOBuffer::GetBuffer() const { return &m_buffer[0]; }
...
void CIOBuffer::Use(size_t dataUsed) { m_used += dataUsed; }
...
void CIOBuffer::AddData(const char *const pData, size_t dataLength) {
  if (dataLength > m_size - m_used) {
    throw CException(_T("CIOBuffer::AddData"),
                     _T("Not enough space in buffer"));
  }
...
void CIOBuffer::AddData(BYTE data) { AddData(&data, 1); }

void CIOBuffer::AddRef() { ::InterlockedIncrement(&m_ref); }
...
size_t CIOBuffer::GetOperation() const { return m_operation; }

void CIOBuffer::SetOperation(size_t operation) { m_operation = operation; }
...
CIOBuffer *CIOBuffer::Allocator::Allocate() {
  ...
  if (!m_freeList.Empty()) {
    ...
    if (!pBuffer) {
      throw CException(_T("CIOBuffer::Allocator::Allocate()"),
                       _T("Out of memory"));
    }
...
void CIOBuffer::Allocator::Release(CIOBuffer *pBuffer) {
  if (!pBuffer) {
    throw CException(_T("CIOBuffer::Allocator::Release()"),
                     _T("pBuffer is null"));
  }
