...
int S3PAccount::GetAccountCount(S3PDBConVBC *pConn, DWORD nGameID, BOOL bOnline,
                                DWORD &dwCount) {
  int iRet = ACTION_FAILED;
  if (NULL == pConn) {
    return iRet;
...
int S3PAccount::UnlockAccount(S3PDBConVBC *pConn, const char *strAccName) {
  ...
  char strSQL[MAX_PATH];
  sprintf(strSQL,
          "update Account_info set iClientID = 0 where (cAccName = '%s')",
          strAccName);
  S3PResultVBC *pResult = NULL;
  if (pConn->Do(strSQL))
    iRet = ACTION_SUCCESS;
...
int S3PAccount::FreezeAccount(S3PDBConVBC *pConn, const char *strAccName) {
  ...
  char strSQL[MAX_PATH];
  sprintf(strSQL,
          "update Account_info set iClientID = %d where (cAccName = '%s')",
          GetGMID(), strAccName);
  S3PResultVBC *pResult = NULL;
  if (pConn->Do(strSQL))
    iRet = ACTION_SUCCESS;
...
int S3PAccount::UnlockServer(S3PDBConVBC *pConn, unsigned long nGameID) {
  ...
  char strSQL[MAX_PATH];
  sprintf(strSQL,
          "update Account_info set iClientID = 0 where (iClientID = %d)",
          nGameID);
  S3PResultVBC *pResult = NULL;
  if (pConn->Do(strSQL))
    iRet = ACTION_SUCCESS;
...
int S3PAccount::GetAccountsTime(S3PDBConVBC *pConn, DWORD ClientID,
                                DWORD dwMinSecond, AccountTimeList &List) {
  int iRet = ACTION_FAILED;
  if (NULL == pConn) {
    return iRet;
  }
  ...
  sprintf(
      strSQL,
      "select datediff(second, getdate(), dEndDate), iLeftSecond, cAccName, "
      "datediff(second, getdate(), dLoginDate) from View_AccountMoney where "
      "(iClientID = %d) and (dLoginDate is not null)",
      ClientID);
  S3PResultVBC *pResult = NULL;
  if (pConn->QuerySql(strSQL, &pResult)) {
    for (int iRow = 0; iRow < pResult->num_rows(); iRow++) {
      ...
      } else if (usesencond.vt == VT_I4 &&
                 usesencond.lVal < 0) // 点卡扣点起作用了
      {
        liLeft += usesencond.lVal;
      }
      aReturn.nTime = max(0, liLeft);
      memcpy(aReturn.Account, (const char *)_bstr_t(accname.bstrVal, true),
