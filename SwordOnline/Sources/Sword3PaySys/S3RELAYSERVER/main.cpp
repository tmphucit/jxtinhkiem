...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
  ...
  DWORD dwResult = KPIGetPrivateProfileString("Local", "port", szPort, szPort,
                                              31, szINIPath);
  short siPort = atoi(szPort);
  dwResult = KPIGetPrivateProfileString("Local", "maxconnect", "10", szPort, 31,
                                        szINIPath);
  int snMaxPlayerCount = atoi(szPort);
  dwResult = KPIGetPrivateProfileString("Local", "precision", "1", szPort, 31,
                                        szINIPath);
  int snPrecision = atoi(szPort);
  dwResult = KPIGetPrivateProfileString("Local", "maxbuffer", "15", szPort, 31,
                                        szINIPath);
  int snMaxBuffer = atoi(szPort);
  dwResult = KPIGetPrivateProfileString("Local", "sizebuffer", "1048576",
                                        szPort, 31, szINIPath);
  int snBufferSize = atoi(szPort);

  dwResult = KPIGetPrivateProfileString("Local", "timelog", "3000", szPort, 31,
                                        szINIPath);
  ...
  if (hModule) {
    pfnCreateServerInterface pFactroyFun =
        (pfnCreateServerInterface)GetProcAddress(hModule, "CreateInterface");

    IServerFactory *pServerFactory = NULL;

    if (SUCCEEDED(pFactroyFun(IID_IServerFactory,
                              reinterpret_cast<void **>(&pServerFactory)))) {
      pServerFactory->SetEnvironment(snMaxPlayerCount, snPrecision, snMaxBuffer,
                                     snBufferSize);

      pServerFactory->CreateServerInterface(
          IID_IIOCPServer, reinterpret_cast<void **>(&pServer));
  ...
  dwResult = KPIGetPrivateProfileString("Local", "DBConnects", "5", szPort, 31,
                                        szINIPath);
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow, int nLogTime) {
  ...
  g_listwnd = ::CreateWindow(
      ...
      0, rc.right, rc.bottom, hWnd, NULL, hInstance, NULL);
  if (!g_listwnd)
    return FALSE;
