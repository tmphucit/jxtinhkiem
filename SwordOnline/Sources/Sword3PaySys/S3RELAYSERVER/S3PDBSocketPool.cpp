...
S3PDBSocketPool::S3PDBSocketPool() { m_pServer = NULL; }
...
BOOL S3PDBSocketPool::SendData(unsigned long uID, const void *const pData,
...
BOOL S3PDBSocketPool::CustomSend(unsigned long uID, char nProtocol,
                                 KAccountHead *pSend) {
  if (m_pServer == NULL)
    return FALSE;
...
void S3PDBSocketPool::SendRelayData(unsigned long uID, char *szAccount,
                                    const void *const pSrcData1,
                                    const size_t &datalength1,
                                    const void *const pSrcData2,
                                    const size_t &datalength2) {
  ...
  relay.nFromRelayID =
      0; // 由于是服务器主动发给客户端,所以无法填nFromRelayID,只好填0
  ...
  BYTE *pData = (BYTE *)_alloca(sizeof(relay) + relay.wMethodDataLength +
                                relay.routeDateLength);
  ...
  m_pServer->SendData(uID, (const void *)pDataOrg,
                      sizeof(relay) + relay.wMethodDataLength +
                          relay.routeDateLength);
}

void S3PDBSocketPool::SendRelayData(unsigned long uID,
                                    const void *const pSrcData1,
                                    const size_t &datalength1,
                                    const void *const pSrcData2,
                                    const size_t &datalength2) {
  ...
  m_pServer->SendData(uID, (const void *)pDataOrg,
                      sizeof(relayIP) + relayIP.routeDateLength);
}
...
BOOL S3PDBSocketPool::AddUserClientID(unsigned long uID) {
  ...
  p->Start(m_pServer);
  gTrace("Add Relay(%s) Client %d ! -- Count %d", pInfo, uID,
         m_clientIDs.size());
  return TRUE;
}
...
BOOL S3PDBSocketPool::ShowAllClientInfo() {
  ...
  while (i != m_clientIDs.end()) {
    ...
    if (p) {
      ...
      gTrace("Client %d: %s(%s)", n, p->m_ServerName.c_str(),
             inet_ntoa(in_addr(add)));
    }
    i++;
    n++;
  }
...
BOOL S3PDBSocketPool::FindGatewayByServerName(const char *szServerName,
                                              DWORD &nGameID) {
  if (szServerName == NULL || szServerName[0] == 0) {
    return FALSE;
  }
  Lock();
  BOOL b = FALSE;
  ...
  while (i != m_clientIDs.end()) {
    if (i->second &&
        strcmp(i->second->m_ServerName.c_str(), szServerName) == 0) {
...
BOOL S3PDBSocketPool::FindGatewayClientByAddress(DWORD nAddress,
                                                 DWORD &nClientID) {
  Lock();
  BOOL b = FALSE;
  GatewayIDMap::iterator i = m_clientIDs.begin();
...
void S3PDBSocketPool::BroadGMData(const IBYTE *lpData, const DWORD dwDataSize) {
  ...
  GatewayIDMap::iterator i = m_clientIDs.begin();
  while (i != m_clientIDs.end()) {
    if (i->second && i->second->m_ServerName[0] == 'g' &&
...
void S3PDBSocketPool::NotifyRelayLoginInfo(DWORD nGameID, DWORD nClientID,
                                           DWORD Address,
  ...
  while (i != m_clientIDs.end()) {
    if (i->second) {
      if (i->second->m_nGameID != nGameID) {
        ...
        strncpy(aInfo.Account, i->second->m_ServerName.c_str(),
                LOGIN_USER_ACCOUNT_MAX_LEN);
...
void S3PDBSocketPool::NotifyRelayLogoutInfo(DWORD Address,
                                            const std::string &ServerName) {
  KServerInfo aInfo;
  aInfo.Size = sizeof(KServerInfo);
  aInfo.Type = ServerInfo;
...
KGatewayDataProcess::KGatewayDataProcess(unsigned long nID,
                                         unsigned long Address, short Port) {
  m_nClientID = nID;
  m_Address = Address;
  m_Port = Port;
...
KGatewayDataProcess::~KGatewayDataProcess() { assert(m_pServer == NULL); }
...
void KGatewayDataProcess::ProcessClientData(const void *pData,
                                            DWORD dwDataSize) {
...
#pragma pack(pop)
      ...
      in_addr ar;
      ar.s_addr = m_Address;
      gTrace("Relay %s(%s): Ping %d(s)", m_ServerName.c_str(), inet_ntoa(ar),
             (m_LastPingTime - nSecond) / 1000);
    } else if (nFamily == pf_relay && (nProtocol == relay_c2c_data ||
                                       nProtocol == relay_s2c_loseway)) {
...
DWORD KGatewayDataProcess::ProcessData(S3PDBConVBC *pConn, const void *pData,
                                       DWORD dwDataSize) {
  BOOL bRet = FALSE;
  ...
  if (nFamily == pf_normal && nProtocol == c2s_gatewayverify) {
    if (m_nGameID > 0) {
      ...
      aReturn.Operate =
          ((KAccountUserLoginInfo *)(((char *)pData) + 1))->Operate;
      ...
      if (bRet) {
        ...
        pSocket->NotifyRelayLoginInfo(m_nGameID, m_nClientID, m_Address,
                                      m_ServerName);
      } else
        bStopConn = TRUE;
    }
  ...
  } else if (nFamily == pf_normal &&
             nProtocol == c2s_accountlogout) // unlock account
  {
    if (m_nGameID == 0) {
      bStopConn = TRUE;
    } else {
      bRet = ProAccountLogout(pConn, ((char *)pData) + 2, dwDataSize - 2);
             ...
             nProtocol == c2s_gamelogin) // freeze account
  {
    if (m_nGameID == 0) {
      bStopConn = TRUE;
    } else {
...
BOOL KGatewayDataProcess::ProRelayVerify(S3PDBConVBC *pConn,
                                         const IBYTE *lpData,
                                         const DWORD dwDataSize) {
  BOOL bRet = FALSE;
  if (NULL != lpData && dwDataSize == sizeof(KServerAccountUserLoginInfo)) {
    KServerAccountUserLoginInfo *pAccInfo =
        (KServerAccountUserLoginInfo *)(lpData);
    if (pAccInfo->Version == ACCOUNT_CURRENT_VERSION &&
      ...
      DWORD nGameID = 0;
      aReturn.nReturn = S3PAccount::ServerLogin(
          pConn, pAccInfo->Account, pAccInfo->Password, m_Address, m_Port,
          pAccInfo->MacAddress, nGameID);
      ...
      char szmac[15];
      sprintf(szmac, "%02X%02X-%02X%02X-%02X%02X", pAccInfo->MacAddress[0],
              pAccInfo->MacAddress[1], pAccInfo->MacAddress[2],
              pAccInfo->MacAddress[3], pAccInfo->MacAddress[4],
              pAccInfo->MacAddress[5]);
      szmac[14] = 0;
      if (bRet)
        gTrace("Relay %s(%s)%s: Verify OK !", m_ServerName.c_str(),
               inet_ntoa(ar), szmac);
      else {
        if (aReturn.nReturn == E_ACCOUNT_EXIST)
          gTrace("Relay %s(%s)%s: Already login !", m_ServerName.c_str(),
                 inet_ntoa(ar), szmac);
        else if (aReturn.nReturn == E_ACCOUNT_OR_PASSWORD)
          gTrace("Relay %s(%s)%s: Name, Password !", m_ServerName.c_str(),
                 inet_ntoa(ar), szmac);
        else if (aReturn.nReturn == E_ADDRESS_OR_PORT)
          gTrace("Relay %s(%s)%s: Address, Port !", m_ServerName.c_str(),
                 inet_ntoa(ar), szmac);
      }
...
BOOL KGatewayDataProcess::ProRelayClose(S3PDBConVBC *pConn, const IBYTE *lpData,
                                        const DWORD dwDataSize) {
  BOOL bRet = FALSE;
...
BOOL KGatewayDataProcess::ProRelayInfo(S3PDBConVBC *pConn, const IBYTE *lpData,
                                       const DWORD dwDataSize) {
  BOOL bRet = FALSE;
...
BOOL KGatewayDataProcess::ProAccountLogout(S3PDBConVBC *pConn,
                                           const IBYTE *lpData,
                                           const DWORD dwDataSize) {
  BOOL bRet = FALSE;
  if (NULL != lpData && dwDataSize == sizeof(KAccountUserLogout)) {
    ...
    if (pAccInfo->Version == ACCOUNT_CURRENT_VERSION &&
      ...
      gTrace("Gateway %s(%s): User %s Unlock%s", m_ServerName.c_str(),
             inet_ntoa(ar), pAccInfo->Account, szDetail);
...
BOOL KGatewayDataProcess::ProGameLogin(S3PDBConVBC *pConn, const IBYTE *lpData,
                                       const DWORD dwDataSize) {
  BOOL bRet = FALSE;
  if (NULL != lpData && dwDataSize == sizeof(KAccountUser)) {
    KAccountUser *pAccInfo = (KAccountUser *)(lpData);
    if (pAccInfo->Version == ACCOUNT_CURRENT_VERSION &&
      ...
      if (aReturn.nReturn == ACTION_SUCCESS) {
        bRet = TRUE;
        char szS[200];
        sprintf(szS, "GlobalExecute(\"dw KickOutAccount([[%s]])\")",
                pAccInfo->Account);
      ...
      gTrace("Gateway %s(%s): User %s Freeze Account%s", m_ServerName.c_str(),
             inet_ntoa(ar), pAccInfo->Account, szDetail);
...
BOOL KGatewayDataProcess::ProRelayIPData(const IBYTE *lpData,
                                         const DWORD dwDataSize) {
  BOOL bRet = FALSE;
...
BOOL KGatewayDataProcess::ProRelayAskData(S3PDBConVBC *pConn,
                                          const IBYTE *lpData,
                                          const DWORD dwDataSize) {
  BOOL bRet = FALSE;
  if (NULL != lpData && dwDataSize >= sizeof(RELAY_ASKWAY_DATA)) {
    ...
    if (pIPData->routeDateLength + pIPData->wMethodDataLength +
            sizeof(RELAY_ASKWAY_DATA) !=
        dwDataSize)
      return FALSE;

    if (pIPData->seekMethod == rm_account_id || pIPData->seekMethod == rm_gm) {
      if (pIPData->nFromIP == 0) // 刚进入Relay体系，填写nFromIP和nFromRelayID
      {
        pIPData->nFromIP = m_Address;
        pIPData->nFromRelayID = m_nClientID;
      ...
      if (pIPData->seekMethod == rm_account_id) {
        ...
        if (ACTION_SUCCESS ==
            S3PAccount::GetAccountGameID(pConn, name, nGameID))
...
BOOL KGatewayDataProcess::ProGMFamily(S3PDBConVBC *pConn, const IBYTE *lpData,
                                      const DWORD dwDataSize) {
  ...
  case gm_c2s_unlock: {
    DWORD nGameID = 0;
    S3PAccount::GetServerID(pConn, ((GM_UNLOCK_ACCOUNT *)lpData)->AccountName,
                            nGameID);
    if (nGameID > 0) {
...
BOOL KGatewayDataProcess::ProAutoTime(S3PDBConVBC *pConn) {
  ...
  if (dwMin > 0) {
    ...
    if (S3PAccount::GetAccountsTime(pConn, m_nGameID, dwMin, TimeList) ==
        ACTION_SUCCESS) {
      ...
      while (i != TimeList.end()) {
        ...
        } else // 催促充值
        {
          int nLen = 0;
          if (i->nTime >= 60)
            nLen = sprintf(szMessage, "您在游戏中还可以停留%d分钟, 请赶快充值!",
                           i->nTime / 60);
          else
            nLen = sprintf(szMessage, "您在游戏中还可以停留%d秒, 请赶快充值!",
                           i->nTime);
...
void KGatewayDataProcess::ExecuteAction(char *szAccount, char *szScript) {
  ...
  pSocket->SendRelayData(m_nClientID, szAccount, &exe, sizeof(exe), szScript,
                         nszLen);
}
...
void KGatewayDataProcess::SendSystemInfo(unsigned long uID, char *szAccount,
                                         char *lpszSendName, char *lpszSentence,
                                         int nSentenceLength) {
