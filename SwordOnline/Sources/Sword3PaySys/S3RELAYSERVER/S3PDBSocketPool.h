...
protected:
  ...
  BOOL ProRelayVerify(S3PDBConVBC *pConn, const IBYTE *lpData,
                      const DWORD dwDataSize);
  BOOL ProRelayClose(S3PDBConVBC *pConn, const IBYTE *lpData,
                     const DWORD dwDataSize);
  BOOL ProRelayInfo(S3PDBConVBC *pConn, const IBYTE *lpData,
                    const DWORD dwDataSize);
  BOOL ProAccountLogout(S3PDBConVBC *pConn, const IBYTE *lpData,
                        const DWORD dwDataSize);
  BOOL ProGameLogin(S3PDBConVBC *pConn, const IBYTE *lpData,
                    const DWORD dwDataSize);
  BOOL ProAutoTime(S3PDBConVBC *pConn);

  BOOL ProGMFamily(S3PDBConVBC *pConn, const IBYTE *lpData,
                   const DWORD dwDataSize);
  BOOL ProRelayAskData(S3PDBConVBC *pConn, const IBYTE *lpData,
                       const DWORD dwDataSize);
  ...
  void SendSystemInfo(unsigned long uID, char *szAccount, char *lpszSendName,
...
public:
  ...
  void NotifyRelayLoginInfo(DWORD nGameID, DWORD nClientID, DWORD Address,
                            const std::string &ServerName);
  void NotifyRelayLogoutInfo(DWORD Address, const std::string &ServerName);

  BOOL SendData(unsigned long uID, const void *const pData,
                const size_t &datalength);
  BOOL CustomSend(unsigned long uID, char nProtocol, KAccountHead *pSend);
  void SendRelayData(unsigned long uID, char *szAccount,
                     const void *const pSrcData1, const size_t &datalength1,
                     const void *const pSrcData2, const size_t &datalength2);
  void SendRelayData(unsigned long uID, const void *const pSrcData1,
                     const size_t &datalength1, const void *const pSrcData2,
