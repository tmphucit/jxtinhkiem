...
enum {
  ...
  WORDA =
      12, //	no		Match "" at wordchar, where prev is nonword
  WORDZ =
      13,    //	no		Match "" at nonwordchar, where prev is word
...
void CRegErrorHandler::regerror(int id) const { regerror(FindErr(id)); }

const CString &CRegErrorHandler::GetErrorString() const { return m_szError; }

void CRegErrorHandler::ClearErrorString() const { m_szError.resize(0); }

LPCTSTR CRegErrorHandler::FindErr(int id) {
  for (struct regErr *perr = errors; perr->m_id != REGERR_SENTINEL_VALUE;
       perr++)
    if (perr->m_id == id)
      return perr->m_err;
...
public:
  static inline TCHAR OP(LPTSTR p) { return (*(p)); }
  static inline LPTSTR OPERAND(LPTSTR p) { return p + 3; }
...
protected:
  ...
  void regc(int b) { *regcode++ = (TCHAR)b; }
...
void CRegCompiler::regtail(LPTSTR p, LPTSTR val) {
  ...
  *((short *)(scan + 1)) =
      (short)((OP(scan) == BACK) ? scan - val : val - scan);
}
...
CRegCompilerBase::CRegCompilerBase(LPCTSTR parse)
    : regparse((LPTSTR)parse), regnpar(1) {}
...
regexp::regexp(LPCTSTR exp, BOOL iCase)
    : regstart(0), reganch(0), regmust(0), regmlen(0), program(0),
      m_programSize(0) {
#if _DEBUG
...
regexp::regexp(const regexp &orig)
    : regstart(orig.regstart), reganch(orig.reganch), regmlen(orig.regmlen),
      m_programSize(orig.m_programSize), numSubs(orig.numSubs), regmust(0) {
#if _DEBUG
...
regexp *regexp::getCopy() { return new regexp(*this); }
...
CRegExecutor::CRegExecutor(regexp *p, LPTSTR string)
    : regbol(string), regstartp(p->startp), regendp(p->endp), prog(p) {}
...
bool CRegExecutor::regmatch(LPTSTR prog) {
  ...
  for (scan = prog; scan != NULL; scan = next) {
    ...
    case ANYOF:
      if (*reginput == '\0' || _tcschr(OPERAND(scan), *reginput) == NULL)
        return false;
      reginput++;
      break;
    case ANYBUT:
...
Regexp::Regexp() : rc(0), string(0) {}

Regexp::Regexp(LPCTSTR exp, BOOL iCase)
    : rc(new regexp(exp, iCase)), string(0) {}

Regexp::Regexp(const Regexp &r)
    : rc(r.rc), m_szError(r.m_szError), string(r.string) {
  if (rc)
    rc->count++;
}
...
bool Regexp::CompiledOK() const { return rc ? rc->Status() : false; }
  ...
  while ((c = *src++) != _T('\0')) {
    ...
    if (no < 0) {
      ...
      *buf++ = c;
      ...
      buf += len;
      if (len != 0 && *(buf - 1) == _T('\0')) { /* strncpy hit NUL. */
...
CString Regexp::GetErrorString() const {
  ...
  // ASSERT( ( ! CompiledOK() ) ? ( rc ? rc->GetErrorString() :
  // m_szError).GetLength() != 0 : 1 );
  ASSERT((!CompiledOK()) ? (rc ? rc->GetErrorString() : m_szError).size() != 0
                         : 1);
