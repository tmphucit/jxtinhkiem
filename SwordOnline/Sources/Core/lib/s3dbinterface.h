...
struct KMagicAttrib {
  ...
  int nValue[4];   // 属性参数
  KMagicAttrib() {
    nValue[0] = nValue[1] = nValue[2] = nValue[3] = nAttribType = 0;
  };
};
#else
struct KMagicAttrib;
...
struct TDBTaskData {
  int m_nTaskId; // fix gioi han id task
  short m_nTaskValue;
};
...
TDBFriendArray; typedef std::vector<TDBTaskData>
TDBTaskArray;
...
// 获得角色的信息并以Ini文件的格式放置
// S3DBI_API int	S3DBI_GetRoleInfo(IN char * strUser, IN OUT KIniFile *
// pIniFile);
...
// S3DBI_API int	S3DBI_GetListData(S3DBI_LISTNAME listname, void * pList,
// int i , ColumnAndValue &cav);
S3DBI_API int S3DBI_GetCountOfList(S3DBI_LISTNAME listname, void *pList);
// S3DBI_API int	S3DBI_UpdateDataAtList(S3DBI_LISTNAME listname, void *
// pList, int i , ColumnAndValue &cav);
S3DBI_API int S3DBI_DeleteDataAtList(S3DBI_LISTNAME listname, void *pList,
                                     int i);
// S3DBI_API int	S3DBI_InsertDataAtList(S3DBI_LISTNAME listname, void *
// pList, ColumnAndValue cav);
...
// S3DBI_API int	S3DBI_GetData(S3_DB_TABLENAME tablename, int nId,
// ColumnAndValue *pcav); S3DBI_API int	S3DBI_GetData(S3_DB_TABLENAME tablename,
// char * strUser , ColumnAndValue * pCav); S3DBI_API int
// S3DBI_UpdateData(S3_DB_TABLENAME tablename, int nId, ColumnAndValue *pcav);
// S3DBI_API int	S3DBI_UpdateData(S3_DB_TABLENAME tablename, char *
// strUser, ColumnAndValue *pcav);
...
// S3DBI_API int	S3DBI_InsertData(S3_DB_TABLENAME tablename, int nId,
// ColumnAndValue cav); S3DBI_API int	S3DBI_InsertData(S3_DB_TABLENAME
// tablename, char * strUser, ColumnAndValue cav);

S3DBI_API int S3DBI_GetListData(S3DBI_LISTNAME listname, void *pList, int i,
                                KTabFileCtrl *pTabFile);
S3DBI_API int S3DBI_DeleteDataAtList(S3DBI_LISTNAME listname, void *pList,
                                     int i);
S3DBI_API int S3DBI_InsertDataAtList(S3DBI_LISTNAME listname, void *pList,
                                     KTabFileCtrl *pTabFile);
S3DBI_API int S3DBI_GetData(S3_DB_TABLENAME tablename, int nId,
                            KTabFileCtrl *pTabFile);
S3DBI_API int S3DBI_GetData(S3_DB_TABLENAME tablename, char *strUser,
                            KTabFileCtrl *pTabFile);
S3DBI_API int S3DBI_UpdateData(S3_DB_TABLENAME tablename, int nId,
                               KTabFileCtrl *pTabFile);
S3DBI_API int S3DBI_UpdateData(S3_DB_TABLENAME tablename, char *strUser,
                               KTabFileCtrl *pTabFile);
S3DBI_API int S3DBI_InsertData(S3_DB_TABLENAME tablename,
                               KTabFileCtrl *pTabFile);
#endif
