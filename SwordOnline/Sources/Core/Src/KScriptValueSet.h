...
#if !defined(                                                                  \
    AFX_KSCRIPTVALUESET_H__00036A2D_425B_4C7D_AD6F_4941B0DD1735__INCLUDED_)
...
public:
  char ValueName[40];
  char strValue[100];
  TStringValueNode(char *pValueName) {
    strcpy(ValueName, pValueName);
    strcpy(strValue, "");
  };
  TStringValueNode(char *pValueName, char *pValue) {
    strcpy(ValueName, pValueName);
    strcpy(strValue, pValue);
  };
};
...
public:
  char ValueName[40];
  int intValue;
  TIntegerValueNode(char *pName) {
    strcpy(ValueName, pName);
    intValue = 0;
  };
  TIntegerValueNode(char *pName, int iValue) {
    strcpy(ValueName, pName);
    intValue = iValue;
  };
};
