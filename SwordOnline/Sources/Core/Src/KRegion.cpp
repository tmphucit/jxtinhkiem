...
BOOL KRegion::LoadObject(int nSubWorld, int nX, int nY) {
  ...
  sprintf(szFile, "%s\\%03d_%s", szFilePath, nX,
          REGION_COMBIN_FILE_NAME_SERVER);
  if (cData.Open(szFile)) {
    ...
    // 载入障碍数据
    cData.Seek(dwHeadSize + sElemFile[REGION_OBSTACLE_FILE_INDEX].uOffset,
               FILE_BEGIN);
    LoadServerObstacle(&cData, sElemFile[REGION_OBSTACLE_FILE_INDEX].uLength);

    // 载入trap信息
    cData.Seek(dwHeadSize + sElemFile[REGION_TRAP_FILE_INDEX].uOffset,
               FILE_BEGIN);
...
BOOL KRegion::LoadClientNpc(KPakFile *pFile, DWORD dwDataSize) {
  ...
  for (i = 0; i < sNpcFileHead.uNumNpc; i++) {
    ...
    nNpcNo = NpcSet.SearchClientID(sTempID);
    if (nNpcNo == 0) {
      int nIdx = NpcSet.AddClientNpc(sNpcCell.nTemplateID, LOWORD(m_RegionID),
                                     HIWORD(m_RegionID), sNpcCell.nPositionX,
                                     sNpcCell.nPositionY, i);
      if (nIdx > 0) {
        Npc[nIdx].m_Kind = sNpcCell.shKind;
        Npc[nIdx].SendCommand(do_stand);
...
//	功能：载入障碍数据给小地图
//----------------------------------------------------------------------
void KRegion::LoadLittleMapData(int nX, int nY, char *lpszPath,
                                BYTE *lpbtObstacle) {
  if (!lpbtObstacle)
    return;
...
void KRegion::Activate() {
  ...
  while (pNode) {
    ...
    if ((nCounter == m_nNpcSyncCounter / 2) && (m_nNpcSyncCounter & 1)) {
      ...
      //	else
      //	{
      //	   Npc[nNpcIdx].m_nCurConutTimeSyn++;
      //	}
    }
    nCounter++;
#endif
    // printf("Region [%03d:%03d] NPC %s active...\n", m_nRegionX, m_nRegionY,
    // Npc[nNpcIdx].Name);	//[wxb 2003-7-29]
  ...
  while (pNode) {
    ...
    if ((nCounter == m_nObjSyncCounter / 2) && (m_nObjSyncCounter & 1)) {
      ...
      //	else
      //	{
      //	   Object[pNode->m_nIndex].m_nCurConutTimeSyn++;
      //	}
    }
    nCounter++;
#endif
...
#ifndef _SERVER
  if (Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].m_RegionIndex ==
      m_nIndex) // 是Player所在的Region
  {
...
#ifdef _SERVER
  if (nMapX < 0 || nMapY < 0 || nMapX >= REGION_GRID_WIDTH ||
      nMapY >= REGION_GRID_HEIGHT) {
    return 0;
  }
...
void KRegion::AddTrap(int nMapX, int nMapY, char *szSpcrit) {
  ...
  m_dwTrap[nMapX][nMapY] = g_FileName2Id(szSpcrit);
#endif
}
...
//----------------------------------------------------------------------------
BYTE KRegion::GetBarrierMin(int nGridX, int nGridY, int nOffX, int nOffY,
                            BOOL bCheckNpc) {
#ifdef _SERVER
  _ASSERT(0 <= nGridX && nGridX < REGION_GRID_WIDTH && 0 <= nGridY &&
          nGridY < REGION_GRID_HEIGHT);
  long lType;
...
void KRegion::BroadCast(const void *pBuffer, DWORD dwSize, int &nMaxCount,
                        int nOX, int nOY) {
#define MAX_SYNC_RANGE 25
  ...
  while (pNode && nMaxCount > 0) {
    ...
    if (Player[pNode->m_nIndex].m_nNetConnectIdx >= 0 &&
      ...
      g_pServer->PackDataToClient(Player[pNode->m_nIndex].m_nNetConnectIdx,
                                  (BYTE *)pBuffer, dwSize);
    nMaxCount--;
    pNode = (KIndexNode *)pNode->GetNext();
  }
