...
private:
  ...
  int m_nBackHand;
  int m_EquipItem
      [itempart_num]; // 玩家装备的道具（对应游戏世界中道具数组的索引）

  int m_PGBoxItem[MAX_PGBOX_ITEM];

  PlayerItem m_Items
      [MAX_PLAYER_ITEM]; // 玩家拥有的所有道具（包括装备着的和箱子里放的，对应游戏世界中道具数组的索引）
  PlayerItem m_sBackItems[MAX_PLAYER_ITEM]; // 交易过程中 m_Items 的备份
  static int ms_ActiveEquip[itempart_num]
                           [MAX_ITEM_ACTIVE]; // 某个部分的装备所激活的装备
  static int ms_ActivedEquip[itempart_num]
                            [MAX_ITEM_ACTIVE]; // 某个部分的装备被什么装备激活
...
private:
  ...
  int GetActiveEquipPlace(
      int nPlace, int nCount); // 取得nPlace的装备激活的第nCount个装备位置
  ...
  BOOL FindSameDetailTypeInEquipment(int nGenre, int nDetail, int *pnIdx,
                                     int *pnX, int *pnY);
...
public:
  ...
  int Add(int nIdx, int nPlace, int nX,
          int nY);                            // nIdx指游戏世界中道具数组的编号
  ...
  void TradeMoveMoney(
      int nMoney); // 调用此接口必须保证传入的nMoney是一个有效数(正数且不超过所有钱数)
  ...
  int GetItemInCheckBoxNum(int nGenre, int nDetailType, int nParticulType,
                           int nLevel);
  int CheckItemCountAll(int nGenre, int nDetailType, int nParticulType,
                        int nLevel);
  int CheckItemBlockInCheckBox(int nGenre, int nDetailType, int nParticulType,
                               int nLevel);
  ...
  int GetItemIdxInCheckBoxByNum(int nGenre, int nDetailType, int nParticulType,
                                int nLevel, int nNum);
  int SetAutoRepair();
  ...
  void GetTradeRoomItemInfo(); // 交易中把 trade room 中的 item 的 idx width
                               // height 信息写入 itemset 中的 m_psItemInfo 中去
  BOOL TradeCheckCanPlace();   // 交易中判断买进的物品能不能完全放进自己的物品栏
  BOOL CheckCanPlaceInEquipment(int nWidth, int nHeight, int *pnX,
                                int *pnY); // 判断一定长宽的物品能否放进物品栏
  BOOL EatMecidine(int nPlace, int nX, int nY); // 吃什么地方的药
  // 自动把一个药品从room_equipment移动到room_immediacy
  BOOL AutoMoveMedicine(int nItemIdx, int nSrcX, int nSrcY, int nDestX,
                        int nDestY);
  // 丢失随身物品
