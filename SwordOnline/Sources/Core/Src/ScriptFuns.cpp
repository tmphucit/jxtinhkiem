...
int LuaMsgToTeam(Lua_State *L) {
  ...
  if (nPlayerIndex > 0) {
    if (Player[nPlayerIndex].m_cTeam.m_nID >= 0) {
      ...
      if (nTeamLeaderId > 0)
        KPlayerChat::SendSystemInfo(1, nTeamLeaderId, "×é¶ÓÏûÏ¢", (char *)szMsg,
                                    strlen(szMsg));
      ...
      for (int i = 0; i < MAX_TEAM_MEMBER; i++) {
        ...
        if (nMemberId > 0) {
          if (szMsg)
            KPlayerChat::SendSystemInfo(1, nMemberId, "×é¶ÓÏûÏ¢", (char *)szMsg,
                                        strlen(szMsg));
        }
      }
    }
...
// ·þÎñÆ÷¶Ë½Å±¾Ö¸Áî
    ...
    {"SetTimer", LuaSetTimer}, // SetTimer(Ê±¼äÁ¿,
