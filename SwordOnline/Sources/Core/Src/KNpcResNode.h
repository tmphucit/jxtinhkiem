...
#define MAX_NPC_IMAGE_NUM                                                      \
  (1 + MAX_PART + 18 + 1 + 1) // 部件加阴影加状态加特殊加menustate
...
public:
  ...
  BOOL GetFile(int nActionNo, int *pnFrame, int *pnDir, int *pnInterval,
               int *pnCgX, int *pnCgY, char *lpszName);
...
public:
  ...
  BOOL Init();        // 初始化
  // 功能:	得到某种状态的光影效果的信息
  void GetInfo(int nNo, char *lpszGetName, int *pnType, int *pnPlayType,
               int *pnBackStart, int *pnBackEnd, int *pnTotalFrame,
...
public:
  ...
  // 根据动作编号、装备编号得到资源文件名
  BOOL GetName(int nActionNo, int nEquipNo, char *lpszDefault,
...
// 装备、状态与动作对照表，KNpcResNode
// 中有两个，一个用于没骑马的，一个用于骑马的
...
public:
  ...
  // 从表格中得到一组排序信息
  BOOL GetSort(int nActNo, int nDir, int nFrameNo, int *lpnTable,
               int nTableLen);
...
// 注：表格前面 MAX_SORT_DIR
// 行是所有动作在默认方式下的排序方式，当动作指明使用默认
//     方式而且不在特殊桢的情况下使用这些信息；跟着是所有动作的特殊排序信息，每一个
//     动作一个单元，每个单元分为两部分：第一部分是所有 MAX_SORT_DIR
//     个方向的排序信 息，如果此动作指明不用默认方式，则这个部分存在并且占
//     MAX_SORT_DIR 行；第二部
//     分是特殊桢的排序方式，特殊桢的数量由动作指明（当存在第一部分时为 nLineNum
//     - MAX_SORT_DIR，当不存在第一部分时为
//     nLineNum），如果没有特殊桢，则这部分不存在
...
public:
  ...
  ~KNpcResNode();
  //	初始化，载入所有相应数据
  BOOL Init(char *lpszNpcName, CActionName *cActionName,
  ...
  BOOL GetFileName(int nPartNo, int nActionNo, int nEquipNo, char *lpszDefault,
                   char *lpszGetName, int nStrLen);
