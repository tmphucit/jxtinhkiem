...
enum NPC_ENCHANT
{
        ...
        enchant_selfresist,					//
自己的五行属性的抗性为100% enchant_conquerresist, // 相生的五行属性的抗性为100%
        enchant_num,
};
*/
...
public:
  ...
  int Add(int nNpcSetingIdxInfo, int nSubWorld, int nRegion, int nMapX,
  ...
  // 把所有npc的 bActivateFlag 设为 FALSE
  ...
  int AddClientNpc(int nTemplateID, int nRegionX, int nRegionY, int nMpsX,
                   int nMpsY, int nNo);
  ...
  void CheckBalance();
  int GetAroundPlayerForTeamInvite(
      KUiPlayerItem *pList, int nCount); // 获得周围玩家列表(用于队伍邀请列表)
  void GetAroundOpenCaptain(int nCamp);  // 获得周围同阵营的已开放队伍队长列表
  int GetAroundPlayer(KUiPlayerItem *pList,
                      int nCount); // 获得周围玩家列表(用于列表)

  // 设定是否全部显示玩家的名字  bFlag ==	TRUE 显示，bFlag == FALSE 不显示
  // zroc add
  void SetShowNameFlag(BOOL bFlag);
  // 判断是否全部显示玩家的名字  返回值 TRUE 显示，FALSE 不显示
  BOOL CheckShowName();
  // 设定是否全部显示玩家的聊天  bFlag ==	TRUE 显示，bFlag == FALSE 不显示
  // zroc add
  void SetShowChatFlag(BOOL bFlag);
  // 判断是否全部显示玩家的聊天  返回值 TRUE 显示，FALSE 不显示
  BOOL CheckShowChat();
  // 设定是否全部显示玩家的血  bFlag ==	TRUE 显示，bFlag == FALSE 不显示 zroc
  // add
  void SetShowLifeFlag(BOOL bFlag);
  // 判断是否全部显示玩家的血  返回值 TRUE 显示，FALSE 不显示
  BOOL CheckShowLife();
  // 设定是否全部显示玩家的内力  bFlag ==	TRUE 显示，bFlag == FALSE 不显示
  // zroc add
...
private:
  ...
  NPC_RELATION GenOneRelation(NPCKIND Kind1, NPCKIND Kind2, NPCCAMP Camp1,
                              NPCCAMP Camp2);
};
