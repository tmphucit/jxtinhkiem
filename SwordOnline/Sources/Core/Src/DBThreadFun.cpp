...
void *DBThreadFun(void *pParam) {
  ...
  while (1) {
    ...
    g_Mutex.Lock();
    if (g_AccessDBMsgList(DBMSG_POP, &nPlayerIndex, (DBI_COMMAND *)&nCommand,
                          &pParam1, &pParam2)) {
      switch (nCommand) {
      case DBI_PLAYERSAVE:
      ...
      case DBI_PLAYERLOAD:
        if (nPlayerIndex > 0 && pParam1) {
          ...
          void *pBuffer = S3DBI_GetRoleInfo(
              bysRoleInfo, Player[nPlayerIndex].GetPlayerName(), nBufferLen);
          if (pBuffer && nBufferLen > 0) {
            ...
            Player[nPlayerIndex].m_pStatusLoadPlayerInfo =
                (BYTE *)pMemClass; // pRoleInfo;

            // testcodes for get test role
            /*KFile File;
            char szFileName[100];
            sprintf(szFileName, "c:\\Role_%s.dat",
            Player[nPlayerIndex].GetPlayerName()); File.Create(szFileName);
      ...
      case DBI_GETPLAYERLISTFROMACCOUNT: {
        if (pParam1 && ((char *)pParam1)[0] && pParam2) {
          ...
          pData->nRoleCount = S3DBI_GetRoleListOfAccount(
              (char *)pParam1, (S3DBI_RoleBaseInfo *)pData->PlayerBaseInfo,
...
int CORE_API g_ClearOutOfDateDBMsgInList(int nPlayerIndex) {
  ...
  while (pNode) {
    ...
    if (pNode->nPlayerIndex == nPlayerIndex &&
        pNode->Command != DBI_PLAYERSAVE) {
...
BOOL CORE_API g_AccessDBMsgList(DBMESSAGE Msg, int *pnPlayerIndex,
                                DBI_COMMAND *pnDBICommand, void **ppParam1,
                                void **ppParam2) {
