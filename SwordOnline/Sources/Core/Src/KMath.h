...
typedef int __cdecl g_InternalDirSinCosFunction(int pSinCosTable[], int nDir,
                                                int nMaxDir);

inline int g_DirSin(int nDir, int nMaxDir) {
  return (*(g_InternalDirSinCosFunction *)(&(g_InternalDirSinCosCode[0])))(
      g_nSin, nDir, nMaxDir);
}
...
typedef int __cdecl g_InternalIsAccrueConquerFunction(int pAccrueConquerTable[],
                                                      int nSrcSeries,
                                                      int nDesSeries);

inline int g_IsAccrue(int nSrcSeries, int nDesSeries) {
  return (*(g_InternalIsAccrueConquerFunction *)(&(
      g_InternalIsAccrueConquerCode[0])))(g_nAccrueSeries, nSrcSeries,
                                          nDesSeries);
}

inline int g_IsConquer(int nSrcSeries, int nDesSeries) {
  return (*(g_InternalIsAccrueConquerFunction *)(&(
      g_InternalIsAccrueConquerCode[0])))(g_nConquerSeries, nSrcSeries,
                                          nDesSeries);
}
