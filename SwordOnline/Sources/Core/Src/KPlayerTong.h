...
public:
  ...
  void GetTongRank(char *lpszGetRank);
  DWORD GetTongNameID();
  ...
  BOOL ApplyChangeTongCamp(int nCamp, char *lpszTongName);
  ...
  // 加入帮会，成为普通帮众
  BOOL AddTong(int nCamp, char *lpszTongName, char *lpszTitle,
               char *lpszMaster);
  // 申请任命
  BOOL ApplyInstate(int nCurFigure, int nCurPos, int nNewFigure, int nNewPos,
                    char *lpszName);
  // 申请踢人
  ...
  BOOL ApplyChangeTitleFeMale(char *lpszName);

  BOOL ApplyChangeTitleMale(char *lpszName);

  BOOL ApplyChangeTitle(char *lpszName, char *lpszNameChange);

  BOOL ApplyDepositMoney(char *lpszMoney);

  BOOL ApplyTakeMoney(char *lpszMoney);
  ...
  if nInfoID == enumTONG_APPLY_INFO_ID_MANAGER nParam1 帮会名id nParam2
  从开始有效第几个 nParam3 连续几个 if nInfoID == enumTONG_APPLY_INFO_ID_MEMBER
  nParam1 帮会名id nParam2 从开始有效第几个 nParam3 连续几个 if nInfoID ==
  enumTONG_APPLY_INFO_ID_ONE lpszName 人名 if nInfoID ==
  enumTONG_APPLY_INFO_ID_TONG_HEAD nParam1 NpcIndex
  */
  BOOL ApplyInfo(int nInfoID, int nParam1, int nParam2, int nParam3,
                 char *lpszName = NULL);
  // 设定自身帮会信息
  ...
  BOOL Create(int nCamp, char *lpszTongName, char *lpszTitle);
  void Delete();
  // 头上是否需要顶找人标志
  BOOL GetOpenFlag();
  // 转发加入帮会申请给对方客户端
  ...
  void Login(int nFigure, int nCamp, char *szName, char *szTitle,
             char *szTongName, char *szMaster);

  void LoadLogin(DWORD dwID);

  void LoadTongInfo(int nInfoID, int nPlayerSlect, int dwTongName);
  ...
  void DBSetTongNameID(DWORD dwID) {
    m_dwTongNameID = dwID;
    if (dwID)
      m_nFlag = 1;
  };

  void ApplyAddMember(int nTagetPlayer);

  void MemberAddTong(int nCamp, char *szTitle, char *lpszTongName,
                     char *lpszTongMaster);

  void ChangeTitleFeMale(DWORD dwTongID, char *szTitle);

  void ChangeTitleMale(DWORD dwTongID, char *szTitle);

  void ChangeTitle(DWORD dwTongID, char *szName, char *szTitle);

  void DepositMoney(DWORD dwTongID, int nMoney);
  void TakeMoney(DWORD dwTongID, int nMoney);
