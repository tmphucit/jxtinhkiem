...
private:
  ...
  int m_nLevelExp1[MAX_LEVEL];
  int m_nLevelExp2[MAX_LEVEL];

  int m_nLevelExp3[MAX_LEVEL];
  int m_nLevelExp4[MAX_LEVEL];
  int m_nLevelExp5[MAX_LEVEL];
...
public:
  ...
  int GetLevelExp1(int nLevel);
  int GetLevelExp2(int nLevel);
  int GetLevelExp3(int nLevel);
  int GetLevelExp4(int nLevel);
  int GetLevelExp5(int nLevel);
...
public:
  KTeamLeadExp();
  BOOL Init(); // 初始化
  int GetLevel(DWORD dwExp,
               int nCurLeadLevel = 0); // 传入经验获得等级数
                                       // (如果传入当前等级，可以加快搜索速度)
