...
#define GAMEVALUE_TO_HUNDREDTHS_OF_DECIBELS                                    \
  (-10000 + (m_nGameVolume *                                                   \
             m_pMusicInfo[m_nCurInfoNo].m_sMusic[m_nCurMusicNo].m_nVolume *    \
             m_nWeatherVolume) /                                               \
                100)
...
void KMapMusic::LoadSetFile() {
  ...
  for (i = 0; i < m_nInfoSize; i++) {
    ...
    for (j = 0; j < defONE_MAP_MAX_MUSIC; j++) {
      cFile.GetString(i + 2, 2 + j * 5, "",
                      m_pMusicInfo[i].m_sMusic[j].m_szFileName,
                      FILE_NAME_LENGTH);
      cFile.GetInteger(i + 2, 2 + j * 5 + 1, 100,
                       &m_pMusicInfo[i].m_sMusic[j].m_nVolume);
      cFile.GetInteger(i + 2, 2 + j * 5 + 2, 0,
                       &m_pMusicInfo[i].m_sMusic[j].m_nStartTime);
      cFile.GetInteger(i + 2, 2 + j * 5 + 3, defGAME_TIME_ONE_DAY - 1,
                       &m_pMusicInfo[i].m_sMusic[j].m_nEndTime);
    }
  }
  ...
  for (i = 0; i < nStage; i++) {
    ...
    for (j = 0; j < nMusicNum; j++) {
      ...
      cFightFile.GetString(szSection, szKey, NULL,
                           m_sFightMusic[i].m_szMusicName[j], FILE_NAME_LENGTH);
...
void KMapMusic::Play(int nMapID, int nGameTime, BOOL bFightMode) {
  ...
  if (!bFightMode) {
    ...
    if (nMapID != m_nCurMapID || m_bFightMode) {
      ...
      // 如果当前正在播放应该播放的音乐，继续
      if (g_pMusic->IsPlaying() &&
          strcmp(m_szCurName, m_pMusicInfo[m_nCurInfoNo]
                                  .m_sMusic[m_nCurMusicNo]
                                  .m_szFileName) == 0) {
        g_pMusic->SetVolume(GAMEVALUE_TO_HUNDREDTHS_OF_DECIBELS);
        return;
      }
      // 播放应该播放的音乐
      strcpy(m_szCurName,
             m_pMusicInfo[m_nCurInfoNo].m_sMusic[m_nCurMusicNo].m_szFileName);
      if (!m_szCurName[0])
        return;
      g_pMusic->Stop();
      //			g_SetFilePath("\\");
    ...
    // 正常播放，循环之
    if (m_pMusicInfo[m_nCurInfoNo].m_sMusic[m_nCurMusicNo].m_nStartTime <=
            nGameTime &&
        nGameTime <=
            m_pMusicInfo[m_nCurInfoNo].m_sMusic[m_nCurMusicNo].m_nEndTime) {
      if (g_pMusic->IsPlaying()) {
        g_pMusic->SetVolume(GAMEVALUE_TO_HUNDREDTHS_OF_DECIBELS);
      } else {
        g_pMusic->Stop();
      ...
      if (nFind) {
        ...
        for (i = 0; i < defMUSIC_STATE_MAX_MUSIC; i++) {
          if (m_sFightMusic[m_nCurStage].m_szMusicName[i][0] &&
              strcmp(m_sFightMusic[m_nCurStage].m_szMusicName[i],
                     m_szCurName) == 0) {
            nFind = 1;
            m_nCurMusicNo = i;
            break;
        ...
        strcpy(m_szCurName,
               m_sFightMusic[m_nCurStage].m_szMusicName[m_nCurMusicNo]);
        //				g_SetFilePath("\\");
        if (m_szCurName[0] && g_pMusic->Open(m_szCurName)) {
          g_pMusic->SetVolume(GAMEVALUE_TO_HUNDREDTHS_OF_DECIBELS);
          g_pMusic->Play(FALSE);
