...
BOOL KFaction::Init() {
  ...
  char szCamp[camp_num][16] = {"新手", "正派", "邪派", "中立",
                               "杀手", "野兽", "路人"};
  int i, j, k, nArrayPos;
  ...
  for (i = 0; i < MAX_FACTION; i++) {
    ...
    Ini.GetString(szSection, "Name", "少林派", m_sAttribute[nArrayPos].m_szName,
                  sizeof(m_sAttribute[nArrayPos].m_szName));
...
int KFaction::GetID(int nSeries, char *lpszName) {
  ...
  if (!lpszName || !lpszName[0])
    return -1;
  for (int i = nSeries * FACTIONS_PRR_SERIES;
       i < (nSeries + 1) * FACTIONS_PRR_SERIES; i++) {
