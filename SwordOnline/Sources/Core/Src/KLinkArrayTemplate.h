...
template <class T, size_t ulSize> class KLinkArrayTemplate {
private:
...
public:
  ...
  KLinkArrayTemplate() {
    m_ulTotalSize = m_ulFreeSize =
        ulSize - 1; // 由于index0是无效的，所以实际数量要减一个
  ...
  unsigned long GetFreeCount() const { return m_ulFreeSize; };
  unsigned long GetUsedCount() const {
    _ASSERT(m_ulTotalSize >= m_ulFreeSize);
    return m_ulTotalSize - m_ulFreeSize;
  };
  unsigned long GetTotalCount() const { return m_ulTotalSize; };
