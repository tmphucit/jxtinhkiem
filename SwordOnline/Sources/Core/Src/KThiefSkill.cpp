...
int KThiefSkill::GetSkillId() { return 400; }

const char *KThiefSkill::GetSkillName() { return m_szSkillName; }

int KThiefSkill::GetSkillStyle() { return SKILL_SS_Thief; }
...
int KThiefSkill::DoSkill(KNpc *pNpc, int nX, int nY) {
  ...
  Show.nNpcIndex = nY;
  // KMissle::CreateMissleForShow("\\spr\\skill\\NPC\\mag_npc_毒水消失.spr",
  // "30,1,1", "\\sound\\skill\\狂雷震地.wav", &Show);
  KMissle::CreateMissleForShow(m_szTargetMovie, m_szTargetMovieParam,
                               m_szSkillSound, &Show);
#endif
...
int KThiefSkill::Cast_Redress(KPlayer *pLaucherPlayer, KPlayer *pTargetPlayer) {
  ...
  if (nTotalMoney <= 0 || nRand < nOdds) {
    ...
    sprintf(szMsg, "%s 从你身上偷到%d两银子捐给前线!",
            Npc[pLaucherPlayer->m_nIndex].Name, nThiefMoney);
    KPlayerChat::SendSystemInfo(1, pTargetPlayer->GetPlayerIndex(), "被偷成功 ",
                                (char *)szMsg, strlen(szMsg));

    sprintf(szMsg, "你成功的从%s 那里偷到%d两银子捐给前线!",
            Npc[pTargetPlayer->m_nIndex].Name, nThiefMoney);
    KPlayerChat::SendSystemInfo(1, pLaucherPlayer->GetPlayerIndex(),
                                "偷窃成功 ", (char *)szMsg, strlen(szMsg));
    return TRUE;
  }

  sprintf(szMsg, "%s 试图偷你的钱,但未成功!",
          Npc[pLaucherPlayer->m_nIndex].Name);
  KPlayerChat::SendSystemInfo(1, pTargetPlayer->GetPlayerIndex(), "被偷未遂",
                              (char *)szMsg, strlen(szMsg));

  sprintf(szMsg, "你偷%s 失败!", Npc[pTargetPlayer->m_nIndex].Name);
  KPlayerChat::SendSystemInfo(1, pLaucherPlayer->GetPlayerIndex(), "偷窃未遂",
                              (char *)szMsg, strlen(szMsg));
  return TRUE;
}

int KThiefSkill::Cast_ShowLife(KPlayer *pLauncherPlayer,
                               KPlayer *pTargetPlayer) {
  _ASSERT(pLauncherPlayer && pTargetPlayer);
  char szMsg[200];
  sprintf(szMsg, "%s 当前生命值情况: %d / %d ",
          Npc[pTargetPlayer->m_nIndex].Name,
          Npc[pTargetPlayer->m_nIndex].m_CurrentLife,
          Npc[pTargetPlayer->m_nIndex].m_CurrentLifeMax);
  KPlayerChat::SendSystemInfo(1, pLauncherPlayer->GetPlayerIndex(), "对方情报",
                              (char *)szMsg, strlen(szMsg));
  return TRUE;
}

int KThiefSkill::Cast_ShowMana(KPlayer *pLauncherPlayer,
                               KPlayer *pTargetPlayer) {
  _ASSERT(pLauncherPlayer && pTargetPlayer);
  char szMsg[200];
  sprintf(szMsg, "%s 当前内力值情况: %d / %d ",
          Npc[pTargetPlayer->m_nIndex].Name,
          Npc[pTargetPlayer->m_nIndex].m_CurrentMana,
          Npc[pTargetPlayer->m_nIndex].m_CurrentManaMax);
  KPlayerChat::SendSystemInfo(1, pLauncherPlayer->GetPlayerIndex(), "对方情报",
                              (char *)szMsg, strlen(szMsg));
  return TRUE;
}

int KThiefSkill::Cast_ShowMoney(KPlayer *pLauncherPlayer,
                                KPlayer *pTargetPlayer) {
  _ASSERT(pLauncherPlayer && pTargetPlayer);
  char szMsg[200];
  sprintf(szMsg, "%s 当前现金情况: %d 两 ", Npc[pTargetPlayer->m_nIndex].Name,
          pTargetPlayer->m_ItemList.GetMoney(room_equipment));
  KPlayerChat::SendSystemInfo(1, pLauncherPlayer->GetPlayerIndex(), "对方情报",
                              (char *)szMsg, strlen(szMsg));
  return TRUE;
}

int KThiefSkill::Cast_RunScript(KPlayer *pLauncherPlayer,
                                KPlayer *pTargetPlayer) {
  _ASSERT(pLauncherPlayer && pTargetPlayer);
  if (m_dwParam1)
    pTargetPlayer->ExecuteScript(m_dwParam1, "main",
                                 pLauncherPlayer->GetPlayerIndex());
  return TRUE;
}
#endif
...
int KThiefSkill::OnSkill(KNpc *pNpc) {
  ...
  if (pNpc->m_bRideHorse)
    pNpc->m_SkillList.SetNextCastTime(
        pNpc->m_ActiveSkillID, SubWorld[pNpc->m_SubWorldIndex].m_dwCurrentTime +
                                   GetDelayPerCastHorse());
  else
    pNpc->m_SkillList.SetNextCastTime(
        pNpc->m_ActiveSkillID,
        SubWorld[pNpc->m_SubWorldIndex].m_dwCurrentTime + GetDelayPerCast());
...
BOOL KThiefSkill::CanCastSkill(int nLauncher, int &nParam1,
                               int &nParam2) const {
  if (nParam1 != -1)
    return FALSE;
...
#ifndef _SERVER
void KThiefSkill::GetDesc(unsigned long ulSkillId, unsigned long ulLevel,
                          unsigned long ulCurLevel, char *pszMsg,
                          int nOwnerIndex, bool bGetNextLevelDesc) {
...
BOOL KThiefSkill::LoadSetting(char *szSettingFile) {
  ...
  TabFile.GetInteger(2, "TimePerCastHorse", 100, (int *)&m_ulDelayPerCastHorse);
  ...
  TabFile.GetString(2, "SkillIcon", "\\spr\\skill\\图标\\通用.spr",
                    m_szSkillIcon, sizeof(m_szSkillIcon));
  TabFile.GetString(2, "Movie", "", m_szSkillSpecialFile,
                    sizeof(m_szSkillSpecialFile));
  TabFile.GetString(2, "TargetMovie", "", m_szTargetMovie,
                    sizeof(m_szTargetMovie));
  TabFile.GetString(2, "TargetMovieInfo", "1,1,1", m_szTargetMovieParam,
                    sizeof(m_szTargetMovieParam));
  TabFile.GetString(2, "SkillSound", "", m_szSkillSound,
                    sizeof(m_szSkillSound));
#endif
  return TRUE;
}
