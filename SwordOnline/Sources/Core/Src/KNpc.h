...
#define SPRSTUNSTATE "\\spr\\skill\\补充\\mag_spe_眩晕.spr"
...
public:
  ...
  // 用于Npc模板库中，当FALSE表示该Npc数据当前是无效的
  // ，数值未经过脚本计算，需要生成. TRUE表示有效数据
  ...
  KMagicAttrib
      m_PhysicsDamage; // Npc的当前伤害(由力量敏捷与武器伤害决定，不考虑直接加伤害的魔法属性)
  ...
  KMagicAttrib
      m_CurrentPhysicsDamage2; // Npc的当前伤害(由力量敏捷与武器伤害决定，不考虑直接加伤害的魔法属性)
  KMagicAttrib m_CurrentFireDamage2;   // Npc的当前火伤害
  KMagicAttrib m_CurrentColdDamage2;   // Npc的当前冰伤害
  KMagicAttrib m_CurrentLightDamage2;  // Npc的当前电伤害
  KMagicAttrib m_CurrentPoisonDamage2; // Npc的当前毒伤害
  ...
  int m_CurrentRWSpeed; // Npc的当前跑动速度
  ...
  int m_RateBoss;
  int m_AiSkillRadiusLoadFlag;      // 战斗npc技能范围是否已经初始化
                                    // 只需要在构造的时候初始化一次
...
private:
  ...
  void PlayerDeadCreateMoneyObj(
      int nMoneyNum);        // 玩家死的时候掉出来的钱生成一个object
...
public:
  ...
  BOOL Cost(
      NPCATTRIB nType, int nCost,
      BOOL bOnlyCheckCanCast =
          FALSE); // 消耗内力体力等,如果OnlyCheckCanCost为TRUE,表示只是检查当前的内力等是否够消耗，并不实际的扣
  ...
  void SetStateSkillEffect(int nLauncher, int nSkillID, int nLevel, void *pData,
                           int nDataNum,
                           int nTime = -1); // 主动辅助技能与被动技能
  ...
  void AppendSkillEffect(int nIdSkillActive, BOOL bIsSkillPhysical,
                         BOOL bIsSkillMagic, BOOL bIsPhysical, BOOL bIsMelee,
                         void *pSrcData, void *pDesData);
  ...
  void BroadCastShop();
  int GetPlayerIdx();
  void CalcDamage(int nAttacker, int nMin, int nMax, DAMAGE_TYPE nType,
                  BOOL bIsMelee, BOOL bDoHurt = TRUE, BOOL bReturn = FALSE,
                  int nngu_hanh_tuong_khac = 0, int nloai_ngu_hanh = -1,
                  int nhut_sinh_luc = 0, int nhut_noi_luc = 0);
  BOOL ReceiveDamage(int nLauncher, BOOL bIsMelee, void *pData, BOOL bUseAR,
                     BOOL bDoHurt, int nDameXG = 0);
...
#ifndef _SERVER
  void SetSleepMode(BOOL bSleep) {
    m_nSleepFlag = bSleep;
    m_DataRes.SetSleepState(bSleep);
  };
  void SetNpcState(BYTE *pNpcState);
  void RemoveRes();
  void ProcNetCommand(NPCCMD cmd, int x = 0, int y = 0, int z = 0);
  void Paint();
  int PaintInfo(int nHeightOffset, bool bSelect, int nFontSize = 12,
                DWORD dwBorderColor = 0);
  int PaintChat(int nHeightOffset);
  int SetChatInfo(const char *Name, const char *pMsgBuff,
  ...
  void SetMenuState(int nState, char *lpszSentence = NULL,
                    int nLength = 0); // 设定头顶状态
