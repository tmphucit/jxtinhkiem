...
public:
  void SaveSentenceSingle(char *lpszSelfName, char *lpszTarget,
                          char *lpszTalkerName, char *lpszSentence);
  void SaveSentenceChannel(char *lpszSelfName, char *lpszTalkerName,
                           char *lpszSentence);
};
#endif
...
public:
  ...
  void Release();
#ifdef _SERVER
  void DataBaseAddOne(int nSelfIdx, DWORD dwID,
                      char *lpszName); // 玩家登陆的时候从数据库获得数据时专用
#endif
};
...
public:
#ifdef _SERVER
  int m_nAddFriendList[CHAT_APPLY_ADD_FRIEND_LIST]; // 当前申请添加的好友 npc id
                                                    // 列表
...
public:
...
#ifdef _SERVER
  void ServerSendChat(int nPlayerIdx,
                      BYTE *pProtocol); // 服务器收到客户端发来的聊天信息
  void DataBaseAddOne(int nSelfIdx, DWORD dwID,
                      char *lpszName); // 玩家登陆的时候从数据库获得数据时专用
  ...
  BOOL SyncFriendData(
      int nPlayerIdx,
      unsigned int &
          nParam); // 玩家登录时加载数据库玩家好友数据完成后自动依次发送好友数据给客户端
  void
  StartSendFriendData(); // 玩家登录时加载数据库玩家好友数据完成后调用次方法开始发送给客户端好友数据
  void DeleteFriend(DWORD dwID,
                    DWORD dwSelfID); // 删除某个好友数据（会通知对方相应处理）
  void DeleteFriendData(DWORD dwID,
                        int nSelfIndex); // 删除某个好友数据（会通知自己客户端）
  ...
  static void SendSystemInfo(int nType, int nTargetIdx, char *lpszSendName,
                             char *lpszSentence, int nSentenceLength);
  static void SendGlobalSystemInfo(char *lpszSendName, char *lpszSentence,
                                   int nSentenceLength);
  typedef std::list<std::string> STRINGLIST;
  static void MakeBrother(const STRINGLIST &brothers);
  static void MakeEnemy(char *szPlayer, char *szEnemy);
  static void SendInfoToGM(char *lpszAccName, char *lpszRoleName,
                           char *lpszSentence, int nSentenceLength);
  static void SendInfoToIP(DWORD nIP, DWORD nID, char *lpszAccName,
                           char *lpszRoleName, char *lpszSentence,
                           int nSentenceLength);
#endif

#ifndef _SERVER
  void SetCurChannel(int nChannelNo, DWORD dwID = 0,
                     int nIdx = 0); // 设定当前发送频道
  void AddChannel(int nChannelNo);  // 增加订阅频道
  void SubChannel(int nChannelNo);  // 取消订阅频道
  void SendTakeChannel();           // 客户端发送订阅频道给服务器

  void SendSentence(KUiMsgParam *pMsg,
                    char *lpszSentence); // 客户端发送聊天语句给服务器
  void QQSendSentence(DWORD dwID, int nIdx, KUiMsgParam *pMsg,
                      char *lpszSentence); // 客户端发送聊天语句给服务器(QQ专用)

  void GetChat(PLAYER_SEND_CHAT_SYNC *pChat); // 客户端收到聊天语句

  void ApplyAddFriend(DWORD dwNpcID,
                      char *lpszSentence = NULL); // 申请添加聊天好友
  void ApplyDeleteFriend(DWORD dwID, int nTeamNo,
                         char *lpszSelfName); // 客户端申请删除某好友

  BOOL GetApplyOneInfo(int *lpnPlayerIdx, char *lpszName,
                       char *lpszInfo); // 界面获取聊天好友添加申请人信息

  void SaveTeamInfo(char *lpszName); // 保存所有好友分组信息
  int CheckTeamNo(char *lpszName,
                  char *lpszGetName); // 判断某个ID的好友在哪个组里
  void LoadTeamName(
      char *lpszPlayerName); // 载入组名(初始化的时候调用，其他时候不允许调用)

  void AddOne(DWORD dwID, int nPlayerIdx, char *lpszName,
              int nTeamNo);           // 添加一个成员
  BOOL CheckIsFriend(int nPlayerIdx); // 判断某个playeridx是否好友
  int AddTeam(char *lpszPlayerName,
              char *lpszTeamName); // 添加组，返回第几组，if < 0 失败
  BOOL RenameTeam(int nTeamNo, char *lpszTeamName,
                  char *lpszPlayerName);              // 组改名
  BOOL DeleteTeam(int nTeamNo, char *lpszPlayerName); // 删除组，返回成功与否
  BOOL OneFriendChangeTeam(DWORD dwID, int nSrcTeam,
                           int nDestTeam); // 把一个好友从一个组移动到另一个组
  ...
  // 用于界面
  int SendSelectChannelList(KUiChatChannel *pList,
                            int nCount); // 发送可选择聊天频道列表给界面
  int SendTakeChannelList(KUiChatChannel *pList,
                          int nCount);       // 发送可订阅聊天频道列表给界面
  BOOL GetCurChannel(KUiChatChannel *pList); // 获得当前使用聊天频道信息
  int IGetTeamInfo(
      int nTeamNo,
      KUiChatGroupInfo *
          psTeamInfo); // 获得某个分组信息，返回下一个有效组编号(如果是最后一个有效组，返回-1)
  int IGetTeamFriendInfo(int nTeamNo,
                         KUiPlayerItem *pList); // 获得某个分组的所有成员的信息
#endif
};
