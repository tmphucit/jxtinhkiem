...
public:
  ...
  bool
      m_bWaitingPlayerFeedBack; // 当前是否正等待玩家在客户端的反馈。该状态下，当前脚本不置空.类式对话选择情况
  //	DWORD			m_dwOutOfDateFeedBackTime;
  ////当前等待玩家与服务器脚本交互的最大限定时刻，到了这个游戏时间如果玩家仍未反馈则视作废。
...
public:
  ...
  BOOL NewPlayerGetBaseAttribute(
      int Series); // 新玩家登陆时根据五行属性产生 力量 敏捷 活力 精力 四项数值
  void AddBaseLucky(int nData); // 增加基本运气

  void AddExp(int nExp, int nTarLevel,
              BOOL bCheck = FALSE); // 增加经验(原始数据，还未经过处理)
  void AddSelfExp(
      int nExp,
      int nTarLevel); // 增加经验(不需要再经过队伍分配的处理，但需要考虑被砍死npc的等级)
  ...
  void ApplyTeamChangeCaptain(
      DWORD dwNpcID);        // 队长向服务器申请把自己的队长身份交给别的队员
  ...
  BOOL ApplyUseItem(
      int nItemID,
      ItemPos SrcPos); // 向服务器申请使用某个物品（鼠标右键点击该物品）
  void
  PickUpObj(int nObjIndex); // 客户端鼠标点击obj检起某个物品，向服务器发消息
  void ObjMouseClick(int nObjIndex); // 客户端鼠标点击obj，向服务器发消息
  void MoveItem(ItemPos DownPos,
                ItemPos UpPos); // DownPos 不能是手，UpPos 必须是手
  int ThrowAwayItem();
  ...
  void
  TradeDecision(int nDecision); // if nDecision == 0 推出交易  if nDecision == 1
                                // 确定交易  if nDecision == 2 取消交易确定
  ...
  int FindAroundPlayer(
      DWORD dwNpcID); // 寻找玩家周围的某个指定npc id的player index
  ...
  void S2CSendTeamInfo(
      BYTE *
          pProtocol); // 收到客户端查询某个npc组队信息的申请后，向这个客户端发送队伍信息
  void SendSelfTeamInfo();          // 服务器向客户端发送队伍信息
  BOOL CreateTeam(BYTE *pProtocol); // 收到客户端请求创建一支队伍
  BOOL SetTeamState(
      BYTE *
          pProtocol); // 收到客户端请求开放、关闭本player队伍是否允许队员加入状态
  ...
  void ChatTransmitApplyAddFriend(
      BYTE *pProtocol);                   // 收到客户端请求转发聊天添加好友信息
  ...
  void
  ChatRedeleteFriend(BYTE *pProtocol);   // 收到客户端申请再次删除某个聊天好友
  ...
  void
  SendEquipItemInfo(int nTargetPlayer); // 发送自己装备在身上的装备信息给别人看
...
private:
  ...
  //// 计算当前力量 	void			CalcCurDexterity();
  //// 计算当前敏捷 	void			CalcCurVitality();
  //// 计算当前活力 	void			CalcCurEngergy();
  //// 计算当前精力
  ...
  //// 由当前敏捷计算对应npc的行走速度 	void SetNpcRunSpeed();
  //// 由当前敏捷计算对应npc的跑步速度
  ...
  void OnButtonDown(int x, int y, int Key,
                    MOUSE_BUTTON nButton); // 处理鼠标键按下
  void OnButtonMove(int x, int y, int Key,
                    MOUSE_BUTTON nButton); // 处理鼠标键按下后移动
...
public:
  ...
  void DoScriptAction(
      PLAYER_SCRIPTACTION_SYNC *pUIInfo); // 通知该客户端显示某个UI界面
  void ProcessPlayerSelectFromUI(
      BYTE *pProtocol); // 处理当玩家从选择菜单选择某项时的操作
#ifndef _SERVER
  void DialogNpc(int nIndex);
  void
  OnSelectFromUI(PLAYER_SELECTUI_COMMAND *pSelectUI,
                 UIInfo eUIInfo); // 当玩家从选择框中选择某项后，将向服务器发送
  ...
  int DeletePlayer(
      char *szPlayerName = NULL); // 注意：本函数是清除玩家帐号！！！，不能乱用
  void LaunchPlayer();
  BOOL Pay(int nMoney);
