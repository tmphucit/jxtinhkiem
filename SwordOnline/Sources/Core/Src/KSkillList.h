...
public:
  ...
  int m_nNpcIndex;
  int m_CurrentListSkillAddDame[MAX_SKILL];
  int m_CurrentListSkillAddLevel[MAX_SKILL];
...
public:
  ...
  int SetSkillLevelDirectlyUsingIndex(
      unsigned long ulSkillIdx,
      unsigned long
          ulLevel) /*请慎用该函数，因为目前未加任何被动技能降级对数值的影响*/
  {
    if (ulSkillIdx >= MAX_NPCSKILL || ulSkillIdx == 0 ||
        ulLevel >= MAX_SKILLLEVEL)
      return 0;
  ...
  int SetSkillLevelDirectlyUsingId(
      unsigned long ulSkillId,
      unsigned long
          ulLevel) /*请慎用该函数，因为目前未加任何被动技能降级对数值的影响*/
  {
  ...
  void SetBeginCastTime(int nSkillID, DWORD dwTime);
  DWORD GetNextCastTime(int nSkillID);
  DWORD GetBeginCastTime(int nSkillID);
  ...
  int GetSkillIdx(int nSkillId);

#ifdef _SERVER
  int UpdateDBSkillList(BYTE *);
  void
  SetNpcSkill(int nSkillNo, int nSkillId,
              int nSkillLevel); // sometime , npc's AI SkillList may have same
                                // skills.IT ONLY BE USED FOR NPC(NO PLAYER)
