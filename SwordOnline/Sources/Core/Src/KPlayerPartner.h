...
private: // 成员函数
  void SetType(ePartnerType eType) { m_ePartnerType = eType; };
  void SetNpcIndex(unsigned long ulNpcIndex) { m_ulNpcIndex = ulNpcIndex; };
  KNpc *GetNpc() const {
    if (m_ulNpcIndex != 0) {
  ...
  ePartnerType GetType() const { return m_ePartnerType; };
...
public: // 成员函数
  ...
  BOOL Init(int nNpcTemplateId, int nLevel, int nSubWorld, int nRegion,
            int nMapX, int nMapY, int nOffX, int nOffY, tPartnerProp Prop);
  BOOL Init(int nNpcTemplateId, int nLevel, int nSubWorld, int nMpsX, int nMpsY,
            tPartnerProp Prop);
