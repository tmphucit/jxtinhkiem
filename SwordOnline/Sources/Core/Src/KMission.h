...
typedef _KMissionPlayerArray<TMissionPlayerInfo, MAX_PLAYER>
    KMissionPlayerArray;

// 管理Mission时间触发器集合的模板类
typedef KTimerFunArray<KTimerTaskFun, MAX_TIMER_PERMISSION,
                       g_MissionTimerCallBackFun>
    KMissionTimerArray;
...
public:
  ...
  unsigned long GetMissionId() { return m_ulMissionId; }
    ...
    while (1) {
      ...
      if (m_MissionPlayer.m_Data[nIdx].m_ucPlayerGroup == ucGroup) {
        KPlayerChat::SendSystemInfo(
            1, m_MissionPlayer.m_Data[nIdx].m_ulPlayerIndex,
            MESSAGE_SYSTEM_ANNOUCE_HEAD, (char *)strMsg, strlen(strMsg));
        nCount++;
      }
    }
    return nCount;
  ...
  unsigned long Msg2Player(const char *strMsg, unsigned long ulPlayerIndex) {
    if (!strMsg || !strMsg[0])
      return 0;
    KPlayerChat::SendSystemInfo(1, ulPlayerIndex, MESSAGE_SYSTEM_ANNOUCE_HEAD,
                                (char *)strMsg, strlen(strMsg));
  };

  unsigned long GetNextPlayer(unsigned long ulIdx, unsigned char ucGroup,
                              unsigned long &ulPlayerIndex) {
    ulPlayerIndex = 0;
    while (1) {
  ...
  /*	unsigned long FindSame(unsigned long ulPlayerIndex )
          {
                  ...
                  while(1)
                  {
                          ...
                          if (m_MissionPlayer[nIdx].ulPlayerIndex ==
     ulPlayerIndex) return nIdx;
                  }
                  return 0;
          };
  */

  unsigned long AddPlayer(unsigned long ulPlayerIndex, unsigned long ulPlayerID,
                          unsigned char ucPlayerGroup, int ulJoinTime = 0,
  ...
  BOOL RemovePlayer(unsigned long ulPlayerIndex, unsigned long ulPlayerID = 0) {
    ...
    if (m_MissionPlayer.Remove(&Info)) {
      if (m_ulMissionId) {
        ...
        g_MissionTabFile.GetString(m_ulMissionId + 1, 2, "", szScript,
                                   MAX_PATH);
        if (szScript[0]) {
