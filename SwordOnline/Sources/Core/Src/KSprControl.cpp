...
//	功能：	构造函数
//---------------------------------------------------------------------------
KSprControl::KSprControl() { Release(); }
...
//	功能：	设定控制对象
//---------------------------------------------------------------------------
void KSprControl::SetSprFile(char *lpszName, int nTotalFrame, int nTotalDir,
                             int nInterVal) {
  if (!lpszName)
    return;
  if (!lpszName[0]) {
...
//	功能：	设定重心位置
//---------------------------------------------------------------------------
void KSprControl::SetCenterPos(int x, int y) { m_nCgXpos = x, m_nCgYpos = y; }

//---------------------------------------------------------------------------
//	功能：	获得单方向帧数
//---------------------------------------------------------------------------
int KSprControl::GetOneDirFrames() { return m_nTotalFrame / m_nTotalDir; }
...
//---------------------------------------------------------------------------
//	功能：	取得当前方向的下一帧
//	参数：bLoop		是否循环播放 TRUE 循环播放 FALSE
//播放到最后一帧停止 	返回值：FALSE
//文件名错误或者bLoop==FALSE时播放到最后一帧(可以通知调用者这个动画播放完了)
//			TRUE otherwise
//---------------------------------------------------------------------------
BOOL KSprControl::GetNextFrame(BOOL bLoop) {
  ...
  if (SubWorld[0].m_dwCurrentTime - m_dwTimer >= m_dwInterval) {
    ...
    m_nCurFrame = (m_nTotalFrame / m_nTotalDir) * m_nCurDir +
                  (m_nTotalFrame / m_nTotalDir) *
                      (SubWorld[0].m_dwCurrentTime - m_dwTimer) / m_dwInterval;
    bRetVal = TRUE;
  }
...
//	功能：	设定帧间隔
//---------------------------------------------------------------------------
void KSprControl::SetInterVal(DWORD dwInterval) { m_dwInterval = dwInterval; }
/*
//---------------------------------------------------------------------------
//	功能：	绘制
//---------------------------------------------------------------------------
void	KSprControl::DrawAlpha(
                                                           int nX,
// 屏幕坐标 x int nY,			// 屏幕坐标 y int nExAlpha)	// alpha
度
{
        KCacheNode	*pSprNode = NULL;
        KSprite		*pSprite;

        pSprNode = (KCacheNode *)g_SpriteCache.GetNode(m_szName, (KCacheNode
*)pSprNode); pSprite = (KSprite*)pSprNode->m_lpData; if (pSprite)
                pSprite->DrawAlpha( nX - m_nCgXpos, nY - m_nCgYpos, m_nCurFrame,
nExAlpha );
}
...
void	KSprControl::DrawBorder(
                                                                int nX,
// 屏幕坐标 x int nY,			// 屏幕坐标 y int nColor)
// 颜色
{
        KCacheNode	*pSprNode = NULL;
        KSprite		*pSprite;
        ...
        pSprNode = (KCacheNode *)g_SpriteCache.GetNode(m_szName, (KCacheNode
*)pSprNode); pSprite = (KSprite*)pSprNode->m_lpData; if (pSprite)
                pSprite->DrawBorder( nX - m_nCgXpos, nY - m_nCgYpos,
m_nCurFrame, nColor );
}
*/
#endif