...
public:
  ...
  int m_addskillsdame[6][2];
  int m_addskillmisslesrun;
  ...
  eSkillLRInfo
      m_eLRSkillInfo; // 0
                      // 表示左右键皆可，1表示只可以作右键技能，2表示左右键都不可作
#endif
...
private:
  ...
  int m_nMinTimePerCastHorse;  //	发该技能的最小间阁时间
  ...
  KMagicAttrib m_MissleAttribs
      [MAXSKILLLEVELSETTINGNUM]; // 生成子弹的相关数据。不同等级的技能，子弹的相关属性可以变化。
  int m_nMissleAttribsNum;
  KMagicAttrib
      m_DamageAttribs[MAX_MISSLE_DAMAGEATTRIB]; // 应用到非状态相关的数据
  int m_nDamageAttribsNum;
  KMagicAttrib m_ImmediateAttribs
      [MAXSKILLLEVELSETTINGNUM]; // 非状态类数据,立即伤害，技能发的子弹打中对方就立即生效的伤害,不随技能等级变化而改变.
  ...
  int m_nChildSkillId; //	技能引发的子技能Id;
                       ////当该技能为基本技能时，这项无用
  ...
  eMisslesGenerateStyle
      m_eMisslesGenerateStyle; // 同时生成的多个子弹，DoWait的时间顺序
...
public:
  ...
  unsigned long GetSkillLevelUpScriptId() const {
    return m_dwSkillLevelUpScriptID;
  };
  ...
  const char *GetPreCastSoundFile(BOOL bIsFeMale) const {
  ...
  BOOL Load(int nCol);
  BOOL
  Cast(int nLauncher, int nParam1, int nParam2, int nWaitTime = 0,
       eSkillLauncherType eLauncherType = SKILL_SLT_Npc) const; // 发出时调用
  int GetChildSkillNum(int nLevel) const;
  int GetAttackRadius() const { return m_nAttackRadius; };
  int CalculateDamage() const;
  void LoadSkillLevelData(
      unsigned long ulLevel,
      int nParam); // 从技能升级文件中获得nLevel等级的属性变动，并设置
  ...
  int GetDelayPerCastHorse() const { return m_nMinTimePerCastHorse; };
  ...
  BOOL IsMelee() const { return m_bIsMelee; };
  ...
  int GetStateAttribsNum() { return m_nStateAttribsNum; };

private:
  void Vanish(KMissle *) const;
  void FlyEvent(KMissle *) const;
  void Collidsion(KMissle *) const;
  BOOL CastMissles(int nLauncher, int nParam1, int nParam2, int nWaitTime = 0,
                   eSkillLauncherType eLauncherType = SKILL_SLT_Npc) const;
  BOOL CastInitiativeSkill(int nLauncher, int nParam1, int nParam2,
                           int nWaitTime) const;
  BOOL CastPassivitySkill(int nLauncher, int nParam1, int nParam2,
                          int nWaitTime) const;

  int CastWall(TOrdinSkillParam *pSkillParam, int nDir, int nRefPX,
               int nRefPY) const;
  int CastLine(TOrdinSkillParam *pSkillParam, int nDir, int nRefPX,
               int nRefPY) const;
  int CastSpread(TOrdinSkillParam *pSkillParam, int nDir, int nRefPX,
                 int nRefPY) const;
  int CastCircle(TOrdinSkillParam *pSkillParam, int nDir, int nRefPX,
                 int nRefPY) const;
  int CastZone(TOrdinSkillParam *pSkillParam, int nDir, int nRefPX,
               int nRefPY) const;
  int CastZoneZ(TOrdinSkillParam *pSkillParam, int nDir, int nRefPX,
                int nRefPY) const;
  int CastExtractiveLineMissle(TOrdinSkillParam *pSkillParam, int nDir,
                               int nSrcX, int nSrcY, int nXOffset, int nYOffset,
                               int nDesX, int nDesY) const;
  int Param2PCoordinate(int nLauncher, int nParam1, int nParam2, int *npPX,
                        int *npPY,
                        eSkillLauncherType eLauncherType = SKILL_SLT_Npc) const;

  void CreateMissle(int nLauncher, int ChildSkillId, int nMissleIndex) const;

  BOOL ParseString2MagicAttrib(unsigned long ulLevel, char *szMagicAttribName,
                               char *szValue);
...
public:
  ...
  static void GetDesc(unsigned long ulSkillId, unsigned long ulLevel,
