...
KSubWorld::KSubWorld() {
  ...
  m_pWeatherMgr = NULL;
  // MissionArry中0虽然无效，但还是加上去，容错.TotalCount
  // 为空间减1，所以不会越界
...
int KSubWorld::FindFreeRegion(int nX, int nY) {
  if (nX == 0 && nY == 0) {
    ...
    for (int i = 0; i < m_nTotalRegion; i++) {
      ...
      if ((nX - nRegoinX) * (nX - nRegoinX) +
              (nY - nRegoinY) * (nY - nRegoinY) >
          2) // 不在附近
        return i;
    }
  }
...
int KSubWorld::GetDistance(int nRx1, int nRy1, int nRx2, int nRy2) {
  return (int)sqrt((nRx1 - nRx2) * (nRx1 - nRx2) +
                   (nRy1 - nRy2) * (nRy1 - nRy2));
}

void KSubWorld::Map2Mps(int nR, int nX, int nY, int nDx, int nDy, int *nRx,
                        int *nRy) {
/*
#ifdef TOOLVERION
        *nRx = nX;
...
void KSubWorld::Map2Mps(int nRx, int nRy, int nX, int nY, int nDx, int nDy,
                        int *pnX, int *pnY) {
  *pnX = (nRx * REGION_GRID_WIDTH + nX) * defLOGIC_CELL_WIDTH + (nDx >> 10);
  *pnY = (nRy * REGION_GRID_HEIGHT + nY) * defLOGIC_CELL_HEIGHT + (nDy >> 10);
}

void KSubWorld::Mps2Map(int Rx, int Ry, int *nR, int *nX, int *nY, int *nDx,
                        int *nDy) {
  if (m_nCellWidth == 0 || m_nCellHeight == 0 || m_nRegionWidth == 0 ||
      m_nRegionHeight == 0)
    return;
...
#ifdef _SERVER
  // 非法的坐标
  if (x >= m_nWorldRegionWidth + m_nRegionBeginX ||
      y >= m_nWorldRegionHeight + m_nRegionBeginY || x < m_nRegionBeginX ||
      y < m_nRegionBeginY) {
    *nR = -1;
    return;
  }
...
BYTE KSubWorld::TestBarrier(int nRegion, int nMapX, int nMapY, int nDx, int nDy,
                            int nChangeX, int nChangeY) {
  int nOldMapX = nMapX;
  int nOldMapY = nMapY;
...
BYTE KSubWorld::TestBarrierMin(int nRegion, int nMapX, int nMapY, int nDx,
                               int nDy, int nChangeX, int nChangeY) {
  int nOldMapX = nMapX;
  int nOldMapY = nMapY;
  int nOldRegion = nRegion;
  ...
  int nMpsX, nMpsY;
  Map2Mps(nRegion, nMapX, nMapY, nDx, nDy, &nMpsX, &nMpsY);
  BYTE bRet = (BYTE)g_ScenePlace.GetObstacleInfoMin(
      nMpsX, nMpsY, nDx & 0x000003ff, nDy & 0x000003ff);
  if (bRet != Obstacle_NULL)
    return bRet;
...
void KSubWorld::AddTrap(int nMpsX, int nMpsY, char *szSpcrit) {
  ...
  m_Region[nRegion].AddTrap(nMapX, nMapY, szSpcrit);
}

void KSubWorld::LoadTong(char *szMapTongName, int nMapTongId, int nMapTongT,
                         int nMapTongVG, char *szMapTongNameBC,
                         int nMapTongBCId, int nCheckMap) {
  if (nCheckMap) {
...
BYTE KSubWorld::GetBarrier(int nMpsX, int nMpsY) {
  ...
  if (nRegion == -1)
    return 0xff;
  return m_Region[nRegion].GetBarrier(nMapX, nMapY, (nOffX >> 10),
                                      (nOffY) >> 10);
#else
...
BOOL KSubWorld::LoadMap(int nId) {
  ...
  if (m_nTotalRegion <= 0 || m_nWorldRegionWidth <= 0 ||
      m_nWorldRegionHeight <= 0)
    return FALSE;
  ...
  for (nY = 0; nY < m_nWorldRegionHeight; nY++) {
    for (nX = 0; nX < m_nWorldRegionWidth; nX++) {
      ...
      for (int i = 0; i < 8; i++) {
        short nTmpX, nTmpY;
        nTmpX =
            (short)LOWORD(m_Region[nIdx].m_nConRegionID[i]) - m_nRegionBeginX;
        nTmpY =
            (short)HIWORD(m_Region[nIdx].m_nConRegionID[i]) - m_nRegionBeginY;
        if (nTmpX < 0 || nTmpY < 0 || nTmpX >= m_nWorldRegionWidth ||
            nTmpY >= m_nWorldRegionHeight) {
  ...
  for (nY = 0; nY < m_nWorldRegionHeight; nY++) {
    for (nX = 0; nX < m_nWorldRegionWidth; nX++) {
      g_SetFilePath(szPath);
      m_Region[nY * m_nWorldRegionWidth + nX].LoadObject(
          m_nIndex, nX + m_nRegionBeginX, nY + m_nRegionBeginY);
    }
  }
...
BOOL KSubWorld::LoadMap(int nId, int nRegion) {
  ...
  for (int i = 0; i < 8; i++) {
    ...
    if (nConIdx < 0) {
      ...
      if (m_Region[nConIdx].Load(nX + nXOff[i], nY + nYOff[i])) {
        ...
        m_Region[nConIdx].LoadObject(0, nX + nXOff[i], nY + nYOff[i],
                                     m_szMapPath);
      } else {
  ...
  if (m_Region[nIdx].m_nConnectRegion[0] >= 0) {
    ...
    m_Region[m_Region[nIdx].m_nConnectRegion[0]].m_nConnectRegion[2] =
        m_Region[nIdx].m_nConnectRegion[1];
    m_Region[m_Region[nIdx].m_nConnectRegion[0]].m_nConnectRegion[3] =
        m_Region[nIdx].m_nConnectRegion[2];
    m_Region[m_Region[nIdx].m_nConnectRegion[0]].m_nConnectRegion[5] =
        m_Region[nIdx].m_nConnectRegion[6];
    m_Region[m_Region[nIdx].m_nConnectRegion[0]].m_nConnectRegion[6] =
        m_Region[nIdx].m_nConnectRegion[7];
  }
  ...
  if (m_Region[nIdx].m_nConnectRegion[1] >= 0) {
    ...
    m_Region[m_Region[nIdx].m_nConnectRegion[1]].m_nConnectRegion[4] =
        m_Region[nIdx].m_nConnectRegion[2];
    ...
    m_Region[m_Region[nIdx].m_nConnectRegion[1]].m_nConnectRegion[6] =
        m_Region[nIdx].m_nConnectRegion[0];
  }

  if (m_Region[nIdx].m_nConnectRegion[2] >= 0) {
    m_Region[m_Region[nIdx].m_nConnectRegion[2]].m_nConnectRegion[0] =
        m_Region[nIdx].m_nConnectRegion[1];
    m_Region[m_Region[nIdx].m_nConnectRegion[2]].m_nConnectRegion[1] = -1;
    m_Region[m_Region[nIdx].m_nConnectRegion[2]].m_nConnectRegion[7] =
        m_Region[nIdx].m_nConnectRegion[0];
    m_Region[m_Region[nIdx].m_nConnectRegion[2]].m_nConnectRegion[4] =
        m_Region[nIdx].m_nConnectRegion[3];
    ...
    m_Region[m_Region[nIdx].m_nConnectRegion[2]].m_nConnectRegion[5] =
        m_Region[nIdx].m_nConnectRegion[4];
    m_Region[m_Region[nIdx].m_nConnectRegion[2]].m_nConnectRegion[6] = nIdx;
  }

  if (m_Region[nIdx].m_nConnectRegion[3] >= 0) {
    m_Region[m_Region[nIdx].m_nConnectRegion[3]].m_nConnectRegion[0] =
        m_Region[nIdx].m_nConnectRegion[2];
    ...
    m_Region[m_Region[nIdx].m_nConnectRegion[3]].m_nConnectRegion[6] =
        m_Region[nIdx].m_nConnectRegion[4];
  ...
  if (m_Region[nIdx].m_nConnectRegion[4] >= 0) {
    ...
    m_Region[m_Region[nIdx].m_nConnectRegion[4]].m_nConnectRegion[1] =
        m_Region[nIdx].m_nConnectRegion[2];
    m_Region[m_Region[nIdx].m_nConnectRegion[4]].m_nConnectRegion[2] =
        m_Region[nIdx].m_nConnectRegion[3];
    ...
    m_Region[m_Region[nIdx].m_nConnectRegion[4]].m_nConnectRegion[6] =
        m_Region[nIdx].m_nConnectRegion[5];
    m_Region[m_Region[nIdx].m_nConnectRegion[4]].m_nConnectRegion[7] =
        m_Region[nIdx].m_nConnectRegion[6];
  }

  if (m_Region[nIdx].m_nConnectRegion[5] >= 0) {
    m_Region[m_Region[nIdx].m_nConnectRegion[5]].m_nConnectRegion[0] =
        m_Region[nIdx].m_nConnectRegion[6];
    m_Region[m_Region[nIdx].m_nConnectRegion[5]].m_nConnectRegion[1] = nIdx;
    m_Region[m_Region[nIdx].m_nConnectRegion[5]].m_nConnectRegion[2] =
        m_Region[nIdx].m_nConnectRegion[4];
  ...
  if (m_Region[nIdx].m_nConnectRegion[6] >= 0) {
    m_Region[m_Region[nIdx].m_nConnectRegion[6]].m_nConnectRegion[0] =
        m_Region[nIdx].m_nConnectRegion[7];
    m_Region[m_Region[nIdx].m_nConnectRegion[6]].m_nConnectRegion[1] =
        m_Region[nIdx].m_nConnectRegion[0];
    m_Region[m_Region[nIdx].m_nConnectRegion[6]].m_nConnectRegion[2] = nIdx;
    m_Region[m_Region[nIdx].m_nConnectRegion[6]].m_nConnectRegion[3] =
        m_Region[nIdx].m_nConnectRegion[4];
    m_Region[m_Region[nIdx].m_nConnectRegion[6]].m_nConnectRegion[4] =
        m_Region[nIdx].m_nConnectRegion[5];
  ...
  if (m_Region[nIdx].m_nConnectRegion[7] >= 0) {
    ...
    m_Region[m_Region[nIdx].m_nConnectRegion[7]].m_nConnectRegion[2] =
        m_Region[nIdx].m_nConnectRegion[0];
    m_Region[m_Region[nIdx].m_nConnectRegion[7]].m_nConnectRegion[3] = nIdx;
    m_Region[m_Region[nIdx].m_nConnectRegion[7]].m_nConnectRegion[4] =
        m_Region[nIdx].m_nConnectRegion[6];
...
void KSubWorld::ProcessMsg(KWorldMsgNode *pMsg) {
  ...
  case GWM_NPC_DEL:
    ...
    if (Npc[pMsg->m_nParam[0]].m_RegionIndex >= 0) {
      ...
      SubWorld[nSubWorld].m_Region[nRegion].DecRef(Npc[nIdx].m_MapX,
...
#else
  {
    ...
    /*	case GWM_NPC_CHANGE_REGION:
                    NpcChangeRegion(pMsg->m_nParam[0], pMsg->m_nParam[1],
       pMsg->m_nParam[2]); break;*/
  case GWM_OBJ_DEL:
#ifndef _SERVER
    if (Object[pMsg->m_nParam[0]].m_nRegionIdx >= 0)
      SubWorld[Object[pMsg->m_nParam[0]].m_nSubWorldID]
          .m_Region[Object[pMsg->m_nParam[0]].m_nRegionIdx]
          .RemoveObj(pMsg->m_nParam[0]);
  ...
  case GWM_MISSLE_DEL:
    ...
    if (Missle[pMsg->m_nParam[0]].m_nRegionId >= 0) {
      SubWorld[Missle[pMsg->m_nParam[0]].m_nSubWorldId]
          .m_Region[Missle[pMsg->m_nParam[0]].m_nRegionId]
          .RemoveMissle(pMsg->m_nParam[0]);
    ...
    /*	case GWM_PLAYER_CHANGE_REGION:
                    PlayerChangeRegion(pMsg->m_nParam[0], pMsg->m_nParam[1],
       pMsg->m_nParam[2]); break;*/
  default:
    break;
  }
...
void KSubWorld::GetMps(int *nX, int *nY, int nSpeed, int nDir,
                       int nMaxDir /* = 64 */) {
...
BOOL KSubWorld::SendSyncData(int nIdx, int nClient) {
  ...
  if (SUCCEEDED(g_pServer->PackDataToClient(nClient, (BYTE *)&WorldSync,
                                            sizeof(WORLD_SYNC)))) {
    return TRUE;
  } else {
...
void KSubWorld::Paint() {
  ...
  Map2Mps(Npc[nIdx].m_RegionIndex, Npc[nIdx].m_MapX, Npc[nIdx].m_MapY,
          Npc[nIdx].m_OffX, Npc[nIdx].m_OffY, &nX, &nY);
...
int KSubWorld::GetRegionIndex(int nRegionID) {
  ...
  if (nX < 0 || nY < 0 || nX >= m_nWorldRegionWidth ||
      nY >= m_nWorldRegionHeight)
    return -1;
...
void KSubWorld::LoadObject(char *szPath, char *szFileName) {
  ...
  for (int i = 0; i < nObjNumber; i++) {
    ...
    IniChange.GetString("MAIN", "mapedit_templatesection", "", szName,
                        sizeof(szName));
...
void KSubWorld::BroadCast(const char *pBuffer, size_t uSize) {
  ...
  while (nIdx) {
    if (Player[nIdx].m_nNetConnectIdx >= 0 && Player[nIdx].m_nIndex > 0 &&
        Npc[Player[nIdx].m_nIndex].m_SubWorldIndex == m_nIndex) {
      g_pServer->PackDataToClient(Player[nIdx].m_nNetConnectIdx, pBuffer,
                                  uSize);
    }
...
void KSubWorld::BroadCastRegion(const void *pBuffer, size_t uSize,
                                int &nMaxCount, int nRegionIndex, int nOX,
                                int nOY) {
  const POINT POff[8] = {
  ...
  for (unsigned int i = 0; i < 8; i++) {
    ...
    if (nConnectRegion != -1) {
      m_Region[nConnectRegion].BroadCast(pBuffer, uSize, nMaxCount,
                                         nOX - POff[i].x, nOY - POff[i].y);
    }
  }
}
#endif
