...
public:
  ...
  BOOL m_bAutoRefuseInviteFlag; // 是否自动拒绝别人的加入队伍的邀请 TRUE
                                // 自动拒绝   FALSE 手动
  KTeamApplyList
...
public:
  ...
  void CreateTeam(int nCaptainNpcID, char *lpszCaptainName, int nCaptainLevel,
                  DWORD nTeamServerID);
  BOOL AddMember(DWORD dwNpcID, int nLevel,
                 char *lpszNpcName); // 添加一个队伍成员
  void DeleteMember(DWORD dwNpcID);  // 客户端删除一个队伍成员
  int GetMemberInfo(KUiPlayerItem *pList,
                    int nCount); // 输出队员信息(按界面的要求，给界面)
#endif
