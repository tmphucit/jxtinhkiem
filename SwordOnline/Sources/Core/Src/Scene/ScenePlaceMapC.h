...
public:
  ...
  void SetGreenLine(int nX, int nY, BOOL bSearch, BOOL bDelete);
  void DrawGreenLine(int nX, int nY, int nX2, int nY2);
  void PaintPointYellow(int nX, int nY, int nNpcX, int nNpcY, char szName[64]);
...
private:
  ...
  enum SCENE_PLACE_MAP_WORK_PARAM {
    ...
    MAP_MAX_SUPPORT_WIDTH =
        ...
        MAP_CELL_MAP_WIDTH, // 小地图的最大支持显示宽（单位：像素点）
    MAP_MAX_SUPPORT_HEIGHT =
        (MAP_CELL_MAX_RANGE - 1) *
        MAP_CELL_MAP_HEIGHT, // 小地图的最大支持显示高（单位：像素点）

    MAP_A_REGION_NUM_MAP_PIXEL_H =
        32, // 场景一个区域在地图上的横向跨度（单位：像素点）
    MAP_A_REGION_NUM_MAP_PIXEL_V =
        32, // 场景一个区域在地图上的纵向跨度（单位：像素点）
  };
...
private:
  ...
  //--绘制显示时参数预先计算与保留的--
  RECT
      m_MapCoverArea; // 地图绘制部分覆盖的范围（以地图像素点为单位，场景坐标点0,0对应点为原点）
