...
KWeather::~KWeather() { m_ParticleList.clear(); }
...
void KWeatherRain::Breath() {
  ...
  for (int j = 0; j < nCreateNum; j++) {
    ...
    particle.m_vPos.nZ =
        500.0f + (particle.m_vPos.nY - m_nFocusPosY) / 3 - g_Random(nZOff);
    particle.m_vSpeed.nX = m_nWindSpeed / 20;
    particle.m_vSpeed.nZ = m_nDownSpeed / 20;
    particle.m_color.Color_b.a = 0x20 + g_Random(10);
    particle.m_color.Color_b.r = particle.m_color.Color_b.g =
        particle.m_color.Color_b.b = 0xff;
  ...
  if (m_nFlareTime > 0 && !m_bIsShutingDown) {
    ...
    case 3:
      if (tmCur - m_nFlareStateStart > m_nFlareStateDelay) {
        ...
        KCacheNode *pSndNode = NULL;
        g_SetFilePath("\\");
        pSndNode = (KCacheNode *)g_SoundCache.GetNode(
            m_szFlareSounds[g_Random(3)], pSndNode);
...
void KWeatherRain::Render(iRepresentShell *pRepresent) {
  ...
  for (i = m_ParticleList.begin(); i != m_ParticleList.end(); i++) {
    ...
    Primitives[nPrimitiveCount].oEndPos.nX =
        i->m_vPos.nX + (m_nWindSpeed * nLen) / m_nDownSpeed;
...
void KWeatherSnow::Render(iRepresentShell *pRepresent) {
  ...
  for (i = m_ParticleList.begin(); i != m_ParticleList.end(); i++) {
    ...
    nPrimitiveCount++;
    if (nPrimitiveCount >= 100) {
      pRepresent->DrawPrimitives(nPrimitiveCount, Primitives, RU_T_SHADOW,
                                 false);
      nPrimitiveCount = 0;
    }
  }
