...
public:
  ...
  // ##Documentation
  // ## 绘制高空物体
  static void PaintAboveHeadObj(KBuildinObj *pObj, RECT *pRepresentArea);
  ...
  // ## 获取区域内内建未初始化的对象的列表。
  void GetBIOSBuildinObjs(KBuildinObj *&pObjsList, unsigned int &nNumObjs);

  // 获得内建光源的信息
  unsigned int GetBuildinLights(KBuildInLightInfo *&pLights);
