...
#define LOCAL_MAX_IMG_NUM 80 // Mac dinh 80
...
KScenePlaceRegionC::~KScenePlaceRegionC() { Clear(); }
...
bool KScenePlaceRegionC::Load(const char *pszBaseFolderName) {
  if (m_Status != REGION_S_TO_LOAD || pszBaseFolderName == NULL ||
      pszBaseFolderName[0] == 0)
    return false;

  m_Status = REGION_S_LOADING;

  char RegionPathPrefix[180];
  sprintf(RegionPathPrefix, "%s\\v_%03d\\%03d_", pszBaseFolderName,
          m_RegionIndex.y, m_RegionIndex.x);
  ...
  if (Data.Open(File)) {
    ...
    if (uMaxElemFile > REGION_ELEM_FILE_COUNT) {
      Data.Read(&ElemFile[0],
                sizeof(KCombinFileSection) * REGION_ELEM_FILE_COUNT);
      Data.Seek(sizeof(KCombinFileSection) *
                    (uMaxElemFile - REGION_ELEM_FILE_COUNT),
                FILE_CURRENT);
    } else {
      Data.Read(&ElemFile[0], sizeof(KCombinFileSection) * uMaxElemFile);
    }

    unsigned int uOffsetAhead =
        sizeof(unsigned int) + sizeof(KCombinFileSection) * uMaxElemFile;

    //--地表层--
    if (ElemFile[REGION_GROUND_LAYER_FILE_INDEX].uLength) {
      Data.Seek(uOffsetAhead + ElemFile[REGION_GROUND_LAYER_FILE_INDEX].uOffset,
                FILE_BEGIN);
      LoadGroundLayer(&Data, ElemFile[REGION_GROUND_LAYER_FILE_INDEX].uLength);
    }
    //--内建对象--
    if (ElemFile[REGION_BUILDIN_OBJ_FILE_INDEX].uLength) {
      Data.Seek(uOffsetAhead + ElemFile[REGION_BUILDIN_OBJ_FILE_INDEX].uOffset,
                FILE_BEGIN);
      LoadAboveGroundObjects(&Data,
                             ElemFile[REGION_BUILDIN_OBJ_FILE_INDEX].uLength);
    }
    //--障碍--
    if (ElemFile[REGION_OBSTACLE_FILE_INDEX].uLength) {
      Data.Seek(uOffsetAhead + ElemFile[REGION_OBSTACLE_FILE_INDEX].uOffset,
                FILE_BEGIN);
    ...
    if (ElemFile[REGION_TRAP_FILE_INDEX].uLength) {
      Data.Seek(uOffsetAhead + ElemFile[REGION_TRAP_FILE_INDEX].uOffset,
                FILE_BEGIN);
...
bool KScenePlaceRegionC::PrerenderGround(bool bForce) {
  ...
  for (nIndex = 0; nIndex < m_GroundLayerData.uNumGrunode; nIndex++) {
    ...
    pGrunode = (KSPRCrunode *)(((char *)pGrunode) +
                               sizeof(KSPRCrunode::KSPRCrunodeParam) +
                               pGrunode->Param.nFileNameLen);
    nNum++;
    if (nNum < LOCAL_MAX_IMG_NUM) {
      pGi++;
    } else {
      g_pRepresent->DrawPrimitivesOnImage(
          LOCAL_MAX_IMG_NUM, &ImgList[0], RU_T_IMAGE,
  ...
  if (nNum) {
    g_pRepresent->DrawPrimitivesOnImage(
        nNum, &ImgList[0], RU_T_IMAGE, m_pPrerenderGroundImg->szImage,
...
m_pPrerenderGroundImg->nImagePosition);
        ...
        KRUImage	gi;
        unsigned int nIndex;
        int			CellWidth  = RWPP_AREGION_WIDTH /
RWP_NUM_GROUND_CELL_H; int			CellHeight = RWPP_AREGION_HEIGHT
/ 2 / RWP_NUM_GROUND_CELL_V;
        ...
        for (nIndex = 0; nIndex < m_GroundLayerData.uNumGrunode; nIndex++)
        {
                ...
                memcpy(gi.szImage, pGrunode->szImgName,
pGrunode->Param.nFileNameLen); gi.szImage[pGrunode->Param.nFileNameLen] = 0;
                gi.nFrame = pGrunode->Param.nFrame;
                g_pRepresent->DrawPrimitivesOnImage(1, &gi, RU_T_IMAGE,
                        m_pPrerenderGroundImg->szImage,
m_pPrerenderGroundImg->uImage, m_pPrerenderGroundImg->nImagePosition); gi.uImage
= 0; gi.szImage[0] = 0; gi.nImagePosition = IMAGE_IS_POSITION_INIT; pGrunode =
(KSPRCrunode*)(((char*)pGrunode) + sizeof(KSPRCrunode::KSPRCrunodeParam) +
pGrunode->Param.nFileNameLen);
        }
        ...
        KSPRCoverGroundObj* pObj = m_GroundLayerData.pObjects;
        for (nIndex = 0; nIndex < m_GroundLayerData.uNumObject; nIndex++,
pObj++)
        {
                ...
                g_pRepresent->DrawPrimitivesOnImage(1, &gi, RU_T_IMAGE,
                                m_pPrerenderGroundImg->szImage,
m_pPrerenderGroundImg->uImage, m_pPrerenderGroundImg->nImagePosition); gi.uImage
= 0; gi.szImage[0] = 0; gi.nImagePosition = IMAGE_IS_POSITION_INIT;
        }
...
void KScenePlaceRegionC::SetNestRegion(KScenePlaceRegionC *pNest) {
  ...
  for (i = 0; i < pNest->m_GroundLayerData.uNumObject; i++, pObj++) {
    if (pObj->bRelateRegion == OBJ_IS_SELF_OWNED &&
        pObj->nPositionX < rcthis.right &&
        pObj->nPositionX + pObj->nWidth > rcthis.left &&
  ...
  pObj = (KSPRCoverGroundObj *)realloc(
      m_GroundLayerData.pObjects,
...
bool KScenePlaceRegionC::LoadGroundLayer(KPakFile *pDataFile,
                                         unsigned int uSize) {
  //_ASSERT(pDataFile);
...
void KScenePlaceRegionC::LoadTrap(KPakFile *pDataFile, unsigned int uSize) {
  ...
  for (DWORD i = 0; i < sTrapFileHead.uNumTrap; i++) {
    ...
    if (sTrapCell.cY >= RWP_NUM_GROUND_CELL_V * 2 ||
        sTrapCell.cX + sTrapCell.cNumCell - 1 >= RWP_NUM_GROUND_CELL_H)
      continue;
...
bool KScenePlaceRegionC::LoadAboveGroundObjects(KPakFile *pDataFile,
                                                unsigned int uSize) {
  _ASSERT(pDataFile);
  pDataFile->Read(&m_BiosData.Numbers, sizeof(KBuildinObjFileHead));
  if (uSize <
      m_BiosData.Numbers.nNumBios * sizeof(KBuildinObj) +
...
void KScenePlaceRegionC::PaintGroundDirect() {
  ...
  for (nIndex = 0; nIndex < m_GroundLayerData.uNumGrunode; nIndex++) {
    ...
    pGi->nType = ISI_T_SPR;
    pGi->oPosition.nX =
        pGrunode->Param.h * CellWidth + m_LeftTopCornerScenePos.x;
    pGi->oPosition.nY =
        (pGrunode->Param.v * CellHeight) * 2 + m_LeftTopCornerScenePos.y;
    memcpy(pGi->szImage, pGrunode->szImgName, pGrunode->Param.nFileNameLen);
    pGi->nFrame = pGrunode->Param.nFrame;
    pGi->uImage = 0;
    pGi->nISPosition = IMAGE_IS_POSITION_INIT;
    ...
    if (nNum < LOCAL_MAX_IMG_NUM) {
      ...
      g_pRepresent->DrawPrimitives(LOCAL_MAX_IMG_NUM, &ImgList[0], RU_T_IMAGE,
                                   false);
      nNum = 0;
      pGi = &ImgList[0];
    }
  ...
  for (nIndex = 0; nIndex < m_GroundLayerData.uNumObject; nIndex++, pObj++) {
    ...
    if (pObj->bRelateRegion == OBJ_IS_SELF_OWNED &&
      ...
      if (pObj->nPositionY < m_LeftTopCornerScenePos.y)
        ImgPart.oImgLTPos.nY =
            (m_LeftTopCornerScenePos.y - pObj->nPositionY) / 2;
      else
  ...
  for (i = 0; i < 16; i++, nX += RWP_OBSTACLE_WIDTH) {
    ...
    for (j = 0; j < 32; j++, nY += RWP_OBSTACLE_HEIGHT) {
      ...
      if (nTrap > 0) {
        ...
        g_pRepresent->OutputText(12, szDebugTrap, KRF_ZERO_END, nX, nY,
                                 0xffffffff, 0, 0);
      }

      int nIdx =
          SubWorld[0].FindRegion(MAKELONG(m_RegionIndex.x, m_RegionIndex.y));
      if (nIdx >= 0) {
        int nRef = SubWorld[0].m_Region[nIdx].GetRef(i, j, obj_npc);
        char szDebugNpcRef[32];
        if (nRef) {
          sprintf(szDebugNpcRef, "%d", nRef);
          g_pRepresent->OutputText(12, szDebugNpcRef, KRF_ZERO_END, nX, nY,
                                   0xffffffff, 0, 0);
        }
      }
      ...
      Line[2].oEndPos.nX = Line[1].oEndPos.nX = Line[0].oEndPos.nX =
          Line[2].oPosition.nX = Line[1].oPosition.nX = Line[0].oPosition.nX =
              nX;
      Line[2].oEndPos.nY = Line[1].oEndPos.nY = Line[0].oEndPos.nY =
          Line[2].oPosition.nY = Line[1].oPosition.nY = Line[0].oPosition.nY =
              nY;
      nNumLine = 0;
      switch (nType) {
      case Obstacle_Full:
...
void KScenePlaceRegionC::PaintAboveHeadObj(KBuildinObj *pObj,
                                           RECT *pRepresentArea) {
  //_ASSERT(pObj);
  ...
  if (pObj->nAniSpeed == 0) {
    ...
    if (SM_IsOutsideRepresentArea(
            pRepresentArea, pObj->ImgPos1.x, pObj->ImgPos1.y, pObj->ImgPos1.z,
...
// ## 获取高空对象最大的绘图次序编号。
unsigned int
KScenePlaceRegionC::GetAboveHeadLayer(KBuildinObj *&pObjsAboveHead) {
  pObjsAboveHead = NULL;
  unsigned int uNum = 0;
...
void KScenePlaceRegionC::GetBuildinObjs(KIpotBuildinObj *&pObjsPointList,
...
// ## 获取区域内内建未初始化的对象的列表。
void KScenePlaceRegionC::GetBIOSBuildinObjs(KBuildinObj *&pObjsList,
...
void KScenePlaceRegionC::EnterProcessArea(KRUImage *pImage) {
  ...
  if (m_pPrerenderGroundImg != pImage) {
    ...
    if (m_pPrerenderGroundImg = pImage) {
      ...
      m_pPrerenderGroundImg->oEndPos.nX =
          m_pPrerenderGroundImg->oPosition.nX + RWPP_AREGION_WIDTH;
      m_pPrerenderGroundImg->oEndPos.nY =
          m_pPrerenderGroundImg->oPosition.nY + RWPP_AREGION_HEIGHT;
      m_pPrerenderGroundImg->oEndPos.nZ = m_pPrerenderGroundImg->oPosition.nZ =
          0;
    }
  }
  ...
  if (m_BiosData.pLeafs == NULL && m_BiosData.pBios &&
      m_BiosData.Numbers.nNumBios > m_BiosData.Numbers.nNumBiosAbove) {
    m_BiosData.pLeafs =
        (KIpotBuildinObj *)malloc(sizeof(KIpotBuildinObj) * nCount);
    if (m_BiosData.pLeafs) {
...
long KScenePlaceRegionC::GetObstacleInfo(int nX, int nY) {
  ...
  _ASSERT(nMapX >= 0 && nMapX < RWP_NUM_GROUND_CELL_H && nMapY >= 0 &&
          nMapY < RWP_NUM_GROUND_CELL_V * 2);
...
long KScenePlaceRegionC::GetObstacleInfoMin(int nX, int nY, int nOffX,
                                            int nOffY) {
