...
#define REGION_COMBIN_FILE_NAME_SERVER                                         \
  "Region_S.dat" // 场景地图区域数据的服务器端版合并文件的文件名
#define REGION_COMBIN_FILE_NAME_CLIENT                                         \
  "Region_C.dat" // 场景地图区域数据的客户端版合并文件的文件名
...
struct KSPRCrunode // ## 地表格图形信息。
{
  struct KSPRCrunodeParam {
    ...
    unsigned short
        nFileNameLen; // 图形文件名长度（包括结束符），取值不可大于MAX_RESOURCE_FILE_NAME_LEN
  } Param;
...
struct KBuildinObjFileHead {
  // ## 区域中内建对象的数目
  unsigned int nNumBios; // nNumBios = nNumBiosTree + nNumBiosLine +
                         // nNumBiosPoint + nNumBiosAbove
  // ## 以树方式排序的内建对象的数目
...
struct KSPNpc {
  ...
  unsigned short
      nScriptNameLen; // Npc脚本文件名长度（包括结束符），取值不可大于MAX_RESOURCE_FILE_NAME_LEN
  char szScript
      [MAX_RESOURCE_FILE_NAME_LEN]; // Npc脚本文件名//此结构实际存储时为变长，实际长度为此结构中的nScriptNameLen数值
};
...
struct KSPObj {
  ...
  unsigned short
      nScriptNameLen; // Obj脚本文件名长度（包括结束符），取值不可大于MAX_RESOURCE_FILE_NAME_LEN
  char szScript
      [MAX_RESOURCE_FILE_NAME_LEN]; // Obj脚本文件名//实际存储时为变长，实际长度为此结构中的nScriptNameLen数值
};
...
struct KLColor {
  ...
  DWORD GetColor() { return 0xff000000 | (r << 16) | (g << 8) | b; }
