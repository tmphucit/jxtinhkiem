...
void KIpotLeaf_EnumerateObjects(KIpotLeaf *pLeaf, void *p,
                                ObjectsCallbackFn CallbackFn) {
  while (pLeaf) {
    if (pLeaf->pLChild)
  ...
  while (pLeaf) {
    ...
    if (pLeaf->eLeafType == KIpotLeaf::IPOTL_T_RUNTIME_OBJ) {
      ...
      if ((pObj->eLayerParam & IPOT_RL_OBJECT) &&
        ...
        // g_DebugLog("TEST %d %d %d
        // %d",pObj->uGenre,pObj->nId,pObj->oPosition.x,pObj->oPosition.y);
...
void KIpotLeaf_PaintNoneObjectLayer(KIpotLeaf *pLeaf, RECT *pRepresentArea,
                                    int eLayer) {
  while (pLeaf) {
    if (pLeaf->pLChild)
      ...
      if (pLeaf->oPosition.x >= pRepresentArea->left &&
          pLeaf->oPosition.x < pRepresentArea->right &&
          pLeaf->oPosition.y >= pRepresentArea->top &&
          pLeaf->oPosition.y < pRepresentArea->bottom) {
        KIpotRuntimeObj *pObj = (KIpotRuntimeObj *)pLeaf;
        CoreDrawGameObj(pObj->uGenre, pObj->nId, pObj->oPosition.x,
    ...
    /*		if (nFrame != pBio->nFrame)
                    {
                            ...
                            {
                                    if
       (g_pRepresent->GetImageFrameParam(pBio->szImage, pBio->nFrame, &Offset1,
       &Size1, ISI_T_SPR) && g_pRepresent->GetImageFrameParam(pBio->szImage,
       nFrame, &Offset2, &Size2, ISI_T_SPR))
                                    {
                                            float fEntireZRange =
       (float)(pBio->ImgPos3.z - pBio->ImgPos1.z);

                                            pBio->ImgPos3.x = pBio->ImgPos1.x +
                                                    (Size2.nX + Offset2.nX -
       Offset1.nX); pBio->ImgPos1.x += (Offset2.nX - Offset1.nX);

                                            if (Offset1.nY != Offset2.nY)
                                                    pBio->ImgPos1.z +=
       (int)((Offset1.nY - Offset2.nY) * ONE_OF_COS_30); if (Size1.nY +
       Offset1.nY != Size2.nY + Offset2.nY) pBio->ImgPos3.z += (int)((Offset1.nY
       + Size1.nY - Offset2.nY - Size2.nY) * ONE_OF_COS_30); pBio->nFrame =
       nFrame;
                                    }
                            }
                            else
