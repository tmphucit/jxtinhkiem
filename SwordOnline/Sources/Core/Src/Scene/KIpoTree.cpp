...
void KSelfBreathLight::Breath() {
  ...
  fRadius += (dwCurTime - dwLastBreathTime) / fCycle * (fMaxRange - fMinRange) *
             nDir * 2;
...
KIpoTree::KIpoTree() {
  ...
  m_pbyLColorBase = new unsigned char[(sizeof(KLColor) * LIGHTING_GRID_WIDTH *
  ...
  m_pby_ptpBase = new unsigned char[(sizeof(KLColor) * LIGHTING_GRID_WIDTH *
                                     LIGHTING_GRID_HEIGHT) +
                                    255];
...
void KIpoTree::Paint(RECT *pRepresentArea, IPOT_RENDER_LAYER eLayer) {
  if (eLayer == IPOT_RL_COVER_GROUND && m_bDynamicLighting) {
    ...
    RenderLightMap();
    // 设置表现模块的光照信息
    g_pRepresent->SetLightInfo(m_nLeftTopX, m_nLeftTopY,
                               (unsigned int *)pLightingArray);
  }
...
void KIpoTree::ObjectsCallback(void *p, KIpotLeaf *pLeaf) {
  ...
  if ((pBio->Props & SPBIO_P_BLOCK_LIGHT_MASK) == SPBIO_P_BLOCK_LIGHT_BOTTOM) {
    // 按线段处理遮挡
    pTree->FillLineObstacle(pObj->oPosition.x - pTree->m_nLeftTopX,
                            pObj->oPosition.y - pTree->m_nLeftTopY,
                            pObj->oEndPos.x - pTree->m_nLeftTopX,
                            pObj->oEndPos.y - pTree->m_nLeftTopY, nObstacleID);
  } else if ((pBio->Props & SPBIO_P_BLOCK_LIGHT_MASK) ==
             SPBIO_P_BLOCK_LIGHT_CIRCLE) {
    // 按圆处理光线遮挡，忽略半径，都按一个格子处理
    int nX = pObj->oPosition.x;
    int nY = pObj->oPosition.y;
    int nIdx;
    if (nX < pTree->m_nLeftTopX || nY < pTree->m_nLeftTopY ||
        nX >= pTree->m_nLeftTopX + AREGION_WIDTH * 3 ||
        nY >= pTree->m_nLeftTopY + AREGION_HEIGHT * 3)
      return;
    else {
...
void KIpoTree::FillLineObstacle(int x1, int y1, int x2, int y2,
                                int nObstacleID) {
  if (x1 >= x2)
    return;
  ...
  if (fdx >= fdy) {
    ...
    while (1) {
      ...
      if (gx > 0 && gx < LIGHTING_GRID_WIDTH - 1 && gy > 0 &&
          gy < LIGHTING_GRID_HEIGHT - 1) {
        nIdx = gy * LIGHTING_GRID_WIDTH + gx;
        pObstacle[nIdx].nObstacle = nObstacleID;
        pObstacle[nIdx].vDir = vDir;

        fFlope2 = (float)(fabs((gy + gStepY) * 32.0f - fy1) /
                          fabs((gx + gStepX) * 32.0f - fx1));
        if (fFlope2 > fFlope1) {
...
void KIpoTree::RenderLightMap()
{
        ...
        for (i = m_LightList.begin(); i != m_LightList.end(); ++i)
        {
                ...
                // 光源所在格子中心的坐标
                int nLightGridCenterX = nLightGridX * LIGHTING_GRID_SIZEX +
LIGHTING_GRID_SIZEX / 2; int nLightGridCenterY = nLightGridY *
LIGHTING_GRID_SIZEY + LIGHTING_GRID_SIZEY / 2;
                ...
                {
                        int j, x, y, gx, gy;
                        int nGridRadius = pLight->m_nRadius /
LIGHTING_GRID_SIZEX; for(int r=0; r<nGridRadius; r++)
                        {
                                ...
                                for(j=0; j<(r * 2 + 1); j++)
                                {
                                        float f = sqrt((x - nLightPosX) * (x -
nLightPosX) + (y - nLightPosY) * (y - nLightPosY)) / pLight->m_nRadius; if(f
> 1.0f) f = 1.0f; f = 1.0f - f; f = f*f;

                                        nIdx = gy * LIGHTING_GRID_WIDTH + gx;
                                        pLColor[nIdx].r += (unsigned int)(nr *
f); pLColor[nIdx].g += (unsigned int)(ng * f); pLColor[nIdx].b += (unsigned
int)(nb * f);
...
int)(nb * f);
                                ...
                                x = nLightGridCenterX - r * LIGHTING_GRID_SIZEX;
                                y = nLightGridCenterY - (r - 1) *
LIGHTING_GRID_SIZEY; gx = nLightGridX - r; gy = nLightGridY - (r - 1); for(j=0;
j<(r * 2 - 1); j++)
                                {
                                        float f = sqrt((x - nLightPosX) * (x -
nLightPosX) + (y - nLightPosY) * (y - nLightPosY)) / pLight->m_nRadius; if(f
...
int)(nb * f);
        ...
        for(j=0; j<LIGHTING_GRID_WIDTH*LIGHTING_GRID_HEIGHT; j++)
        {
                ...
                if(pLColor[j].b > 0xff)
                        pLColor[j].b = 0xff;
                pLightingArray[j] = 0xff000000 | (pLColor[j].r<<16) |
(pLColor[j].g<<8) | pLColor[j].b;
        }
}*/
...
LIGHTING_GRID_SIZEY + LIGHTING_GRID_SIZEY / 2;
                ...
                for(int r=0; r<nGridRadius; r++)
                {
                        ...
                        for(j=0; j<(r * 2 + 1); j++)
                        {
                                ...
                                {
                                        float f = (float)(sqrt((x - nLightPosX)
* (x - nLightPosX) + (y - nLightPosY) * (y - nLightPosY)) / pLight->m_nRadius);
                                        if(f > 1.0f)
                                                f = 1.0f;
                                        f = 1.0f - f;
...
int)(nb * f);
        ...
        for(j=0; j<LIGHTING_GRID_WIDTH*LIGHTING_GRID_HEIGHT; j++)
        {
                ...
                if(ptp[j].b > 0xff)
                        ptp[j].b = 0xff;
                pLightingArray[j] = 0xff000000 | (ptp[j].r<<16) | (ptp[j].g<<8)
| ptp[j].b;
        }
}
*/
...
void KIpoTree::RenderLightMap() {
  ...
  for (i = m_LightList.begin(); i != m_LightList.end(); ++i) {
    ...
    // 光源所在格子中心的坐标
    int nLightGridCenterX =
        nLightGridX * LIGHTING_GRID_SIZEX + LIGHTING_GRID_SIZEX / 2;
    int nLightGridCenterY =
        nLightGridY * LIGHTING_GRID_SIZEY + LIGHTING_GRID_SIZEY / 2;
    ...
    for (int r = 0; r < nGridRadius; r++) {
      ...
      if (!((gx < 0) || (gy < 0))) {
        ...
        for (j = 0; j < (r * 2 + 1); j++) {
          if (CanLighting(x, y, nLightPosX, nLightPosY)) {
            ...
            //));
            float f =
                65536.0 - ((float)(sqrt((x - nLightPosX) * (x - nLightPosX) +
                                        (y - nLightPosY) * (y - nLightPosY)) *
                                   fLightRadiusDenom * 65536.0));
      ...
      gx = nLightGridX - r;
      gy = nLightGridY + r;
      if (!((gx < 0) || (gy >= LIGHTING_GRID_HEIGHT))) {
        x = nLightGridCenterX - r * LIGHTING_GRID_SIZEX;
        y = nLightGridCenterY + r * LIGHTING_GRID_SIZEY;
        pCurPos = &pLColor[gy * LIGHTING_GRID_WIDTH + gx];
        for (j = 0; j < (r * 2 + 1); j++) {
      ...
      gx = nLightGridX + r;
      gy = nLightGridY - (r - 1);
      if (!((gx >= LIGHTING_GRID_WIDTH) || (gy < 0))) {
        x = nLightGridCenterX + r * LIGHTING_GRID_SIZEX;
        y = nLightGridCenterY - (r - 1) * LIGHTING_GRID_SIZEY;
        pCurPos = &pLColor[gy * LIGHTING_GRID_WIDTH + gx];
        for (j = 0; j < (r * 2 - 1); j++) {
  ...
  for (m = 1; m < LIGHTING_GRID_HEIGHT - 1; m++) {
    ...
    for (n = 1; n < LIGHTING_GRID_WIDTH - 1; n++) {
      ...
      *pdwLight = 0xff000000 | (((pDest->r > 0xff) ? 0xff : pDest->r) << 16) |
  ...
  //		//if(ptp[j].b > 0xff)
  //		//	ptp[j].b = 0xff;
  //		//pLightingArray[j] = 0xff000000 | (ptp[j].r<<16) |
  //(ptp[j].g<<8) | ptp[j].b;
  //
  //        pLightingArray[j] = 0xff000000 |
  ...
  for (m = 0; m < LIGHTING_GRID_HEIGHT; m++) {
    ...
    pLightingArray[m * LIGHTING_GRID_WIDTH + LIGHTING_GRID_WIDTH - 1] =
        m_dwAmbient | 0xff000000;
  }

  for (n = 0; n < LIGHTING_GRID_WIDTH; n++) {
    pLightingArray[n] = m_dwAmbient | 0xff000000;
    pLightingArray[(LIGHTING_GRID_HEIGHT - 1) * LIGHTING_GRID_WIDTH + n] =
        m_dwAmbient | 0xff000000;
  }
...
bool KIpoTree::CanLighting(int x, int y, int nLightPosX, int nLightPosY) {
  int nGridX, nGridY, nGridLightPosX, nGridLightPosY, nGridStep, nCurrentGridX,
      nCurrentGridY, nIdx;
  ...
  fx = (float)x, fy = (float)y, fLightPosX = (float)nLightPosX,
  fLightPosY = (float)nLightPosY;
  ...
  if (fdx >= fdy) {
    ...
    while (1) {
      ...
      // 检查是否碰到挡光障碍
      if (pObstacle[nIdx].nObstacle != 0 &&
          pObstacle[nIdx].nObstacle != nObstacle)
        return false;
    }
  }
  // 斜率大于45度的情况
...
bool KIpoTree::CanLighting(int x1, int y1, int x2, int y2)
{
        ...
        if(fdx >= fdy)
        {
                ...
                while(1)
                {
                        ...
                        {
                                //
没有碰到障碍，根据这点本身是否障碍和障碍方向判断是否受光 if(nObstacle && vDir.fX
!= 0.0f)
                                {
                                        if(vDir.fX * vLight.fY - vDir.fY *
vLight.fX < 0.0f) return true; else return false;
                                }
                                else
                                        return true;
                        }
                        ...
                        // 检查是否碰到挡光障碍
                        if(pObstacle[nIdx].nObstacle != 0 &&
pObstacle[nIdx].nObstacle != nObstacle) return false;
                }
        }
        // 斜率大于45度的情况
        else
