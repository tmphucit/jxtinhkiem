...
public:
  ...
  unsigned int AddObject(
      ...
      // ##Documentation
      // ## 加入的位置坐标。
      int x, int y, int z, int eLayerParam = IPOT_RL_OBJECT);
  ...
  unsigned int MoveObject(
      ...
      unsigned int &uRtoid, int eLayerParam = IPOT_RL_OBJECT);
  ...
  void DrawGreenLine(int nX, int nY, BOOL bSearch, BOOL bDelete); // toa do
  void FindPos(int nX, int nY, BOOL bSearch, BOOL bDelete);       // toa do
  void SearchPos();
...
private:
  ...
  // ##Documentation
  // ## 加载区域数据的过程，具体内容见活动图：
  // ## Use Case View/场景/客户端/场景地图加载/State/Activity
  // Model/加载区域的机制 中子线程空间的执行内容
  void LoadProcess();
...
private:
  ...
  KPrevLoadFileNameAndFrame
      m_PrevLoadFileNameAndFrames[MAX_PREV_LOAD_FILE_COUNT];
};
