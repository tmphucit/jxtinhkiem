...
void KScenePlaceMapC::FillCellsPicInfo() {
  ...
  for (v = 0, y = nStartY; v < MAP_CELL_MAX_RANGE;
    ...
    bool bCleared = false;
    if (y < 0 || nToY > PicEntireSize.cy) { // 纵向没有全部落在图内
    ...
    if (y < PicEntireSize.cy && nToY > 0) {
      ...
      for (h = 0, x = nStartX; h < MAP_CELL_MAX_RANGE;
        ...
        if (bCleared == false &&
            (x < 0 || nToX > PicEntireSize.cx)) { // 横向没有全部落在图内
          g_pRepresent->ClearImageData(m_ElemsList[v][h].szImageName,
                                       m_ElemsList[v][h].uImageId,
                                       m_ElemsList[v][h].sISPosition);
        }
...
void KScenePlaceMapC::PaintMap(int nX, int nY) {
  ...
  if (m_bHavePicMap && g_pRepresent) {
    ...
    if (nNpcIdx >= 0 && Npc[nNpcIdx].m_RegionIndex >= 0) {
      ...
      // DrawGreenLine(nX + m_GreenLineDes.x - m_MapCoverArea.left - 1,nY +
      // m_GreenLineDes.y - m_MapCoverArea.top  - 1, nX + nNpcX -
      // m_MapCoverArea.left - 1, nY + nNpcY - m_MapCoverArea.top  - 1); // toa
      // do
...
void KScenePlaceMapC::SetHidePicMap(bool bHide) { m_bHidePicMap = bHide; }
// 绘制队友位置
...
void KScenePlaceMapC::PaintFindPos(int nX, int nY, int nNpcPosX, int nNpcPosY) {
  ...
  int nNpcIdx = 0;
  int nNpcX =
      LOWORD(Npc[nNpcPosIdx].m_dwRegionID) * MAP_A_REGION_NUM_MAP_PIXEL_H +
      Npc[nNpcPosIdx].m_MapX * 2;
  int nNpcY =
      HIWORD(Npc[nNpcPosIdx].m_dwRegionID) * MAP_A_REGION_NUM_MAP_PIXEL_V +
      Npc[nNpcPosIdx].m_MapY;
  bool nPaintFindSymbol = true;

  if (m_FlagPos.x - nNpcX > -5 && m_FlagPos.x - nNpcX < 5 &&
      m_FlagPos.y - nNpcY > -5 && m_FlagPos.y - nNpcY < 5)
  ...
  if (nNpcPosIdx >= 0 && Npc[nNpcPosIdx].m_RegionIndex >= 0) {
    ...
    int nNamePosLeftE = m_LineFindPos.oPosition.nX +
                        (g_StrLen(MapPos) * nFontWidth) + nFontWidth;
    ;
    ...
    if (m_bLine) {
      ...
      g_pRepresent->OutputText(nFontSize, MapPos, KRF_ZERO_END, nNamePosLeftB,
                               nNamePosY, 0xffff00);
      if (AutoMove) {
...
#define MAX_NUM_CHARACTER 100
  ...
  while (nNpcIdx = NpcSet.GetNextIdx(nNpcIdx)) {
          ...
          (
              ...
              (nIsInTeam && // 主角是组队状态
               ((DWORD)g_Team[0].m_nCaptain ==
                    Npc[nNpcIdx].m_dwID || // 是主角所在队伍的队长
                g_Team[0].FindMemberID(Npc[nNpcIdx].m_dwID) >= 0))

              ||

              (Npc[nNpcIdx].m_CurrentCamp ==
                   Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].m_CurrentCamp &&
               Player[CLIENT_PLAYER_INDEX].m_cTask.GetSaveVal(128) > 0)) &&
    ...
    if (bValidNpc) {
      ...
      if (Npc[nNpcIdx].m_InvisibilityState.nTime <= 0 ||
          nNpcIdx == Player[CLIENT_PLAYER_INDEX].m_nIndex) {
