...
unsigned int KIpotBranch::m_BranchFlag[2] = {IPOT_BF_HAVE_LEFT_BRANCH,
                                             IPOT_BF_HAVE_RIGHT_BRANCH};
...
void KIpotBranch::PaintABranchObject(KIpotBuildinObj *pObject,
  ...
  if (pBio->ImgPos3.y > pRepresentArea->top &&
    ...
    if (pObject->bImgPart == false) {
      ...
      Image.oPosition.nX =
          pBio->ImgPos1.x + (Image.oSecondPos.nX * nStartPart) / nEntireXRange;
      Image.oPosition.nY =
          pBio->ImgPos1.y + (Image.oSecondPos.nY * nStartPart) / nEntireXRange;
      Image.oPosition.nZ =
          pBio->ImgPos1.z + (Image.oSecondPos.nZ * nStartPart) / nEntireXRange;

      Image.oSecondPos.nX =
          pBio->ImgPos1.x + (Image.oSecondPos.nX * nEndPart) / nEntireXRange;
      Image.oSecondPos.nY =
          pBio->ImgPos1.y + (Image.oSecondPos.nY * nEndPart) / nEntireXRange;
      Image.oSecondPos.nZ =
          pBio->ImgPos1.z + (Image.oSecondPos.nZ * nEndPart) / nEntireXRange;

      Image.oThirdPos.nX =
          pBio->ImgPos4.x + (Image.oEndPos.nX * nStartPart) / nEntireXRange;
      Image.oThirdPos.nY =
          pBio->ImgPos4.y + (Image.oEndPos.nY * nStartPart) / nEntireXRange;
      Image.oThirdPos.nZ =
          pBio->ImgPos4.z + (Image.oEndPos.nZ * nStartPart) / nEntireXRange;

      Image.oEndPos.nX =
          pBio->ImgPos4.x + (Image.oEndPos.nX * nEndPart) / nEntireXRange;
      Image.oEndPos.nY =
          pBio->ImgPos4.y + (Image.oEndPos.nY * nEndPart) / nEntireXRange;
      Image.oEndPos.nZ =
          pBio->ImgPos4.z + (Image.oEndPos.nZ * nEndPart) / nEntireXRange;
...
#define SPACE_EXPAND_STEP 8
  ...
  if (m_pFirstObject == NULL) {
    ...
    if (m_nNumObjects == m_nNumObjectSpace) {
      KIpotBuildinObj **pNewList = (KIpotBuildinObj **)realloc(
          m_pObjectList,
...
void KIpotBranch::AddBranch(KIpotBuildinObj *pBranchObj) {
  ...
  if (m_pFirstObject->fAngleXY == pBranchObj->fAngleXY &&
    ...
    eRelate =
        SM_Relation_LineLine_CheckCut(p1, p2, m_oHeadPoint, m_oEndPoint, poi);
  }
...
void KIpotBranch::AddLeafLine(KIpotBuildinObj *pLeaf) {
  ...
  if (SM_Distance_PointLine(pLeaf->oPosition, m_oHeadPoint, m_oEndPoint) >
    ...
    eRelate =
        SM_Relation_PointLine(pLeaf->oPosition, m_oHeadPoint, m_oEndPoint);
  } else
...
void KIpotBranch::AddPointLeafToList(KIpotLeaf *&pFirst, KIpotLeaf *pLeaf,
  ...
  for (pL = pFirst; pL; pL = pL->pBrother) {
    if (pL->eLeafType != KIpotLeaf::IPOTL_T_BUILDIN_OBJ)
      continue;
    if ((((KIpotBuildinObj *)pL)->pBio->Props & SPBIO_P_SORTMANNER_MASK) !=
        SPBIO_P_SORTMANNER_LINE)
      continue;
    op1 = pL->oPosition;
...
#define HEIGHT_DOWN_EXPAND 128
    ...
    if (lp.x > rcArea.left && lp.x < rcArea.right && lp.y >= rcArea.top &&
        lp.y <= rcArea.bottom) {
      pMatchL = pL;
  ...
  for (pL = pFirst; pL; pPreLeaf = pL, pL = pL->pBrother) {
    if (pL->eLeafType == KIpotLeaf::IPOTL_T_BUILDIN_OBJ &&
      ...
      if (lp.y < pL->oPosition.y && lp.y < ((KIpotBuildinObj *)pL)->oEndPos.y)
        break;
...
void KIpotBranch::AddLineLeafToList(KIpotLeaf *&pFirst,
  ...
  for (pL = (KIpotBuildinObj *)pFirst; pL;
       pL = (KIpotBuildinObj *)(pL->pBrother)) {
...
#define fsize 7
    ...
    if (lp1.x >= rcArea.left && lp1.x <= rcArea.right && lp1.y >= rcArea.top &&
        lp1.y <= rcArea.bottom && lp2.x >= rcArea.left &&
        lp2.x <= rcArea.right && lp2.y >= rcArea.top &&
        lp2.y <= rcArea.bottom) {
      if (SM_Distance_PointLine(lp1, op1, op2) >
          SM_Distance_PointLine(lp2, op1, op2))
  ...
  if (lp1.y > lp2.y)
    lp1.y = lp2.y;
  for (pL = (KIpotBuildinObj *)pFirst; pL;
       pPreLeaf = pL, pL = (KIpotBuildinObj *)(pL->pBrother)) {
