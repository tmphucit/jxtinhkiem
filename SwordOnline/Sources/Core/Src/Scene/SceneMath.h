...
enum RELATION_ENUM { RELATION_UP, RELATION_ON, RELATION_DOWN, RELATION_CROSS };
...
// void	SM_GetLineEndPointAndNodicalX(float x1, float y1, int nAngleXY, int
// nWidth, float& x2, float& y2, int& nNodicalX);
...
inline int SM_Relation_LineLine(const POINT &LP11, const POINT &pLP12,
                                const POINT &LP21, const POINT &pLP22) {
  int nRet;
  if (LP11.y < LP21.y)
    nRet = -1;
