...
// 点到直线的距离
int SM_Distance_PointLine(const POINT &Point, const POINT &LP1,
                          const POINT &LP2) {
  int x1, y1, x2, y2;
  x1 = LP1.x - Point.x;
  y1 = LP1.y - Point.y;
...
// 点和直线的位置关系
RELATION_ENUM SM_Relation_PointLine(const POINT &Point, const POINT &LP1,
                                    const POINT &LP2) {
  //	_ASSERT(LP1.x <= LP2.x);
  POINT p1, p2;
  p1.x = LP1.x - Point.x;
  ...
  if (p1.x * p1.x <= TOGGLE_VALUE_0_1 &&
      p1.y * p1.y <= TOGGLE_VALUE_0_1) // fabs(p1.x) < 3.2 && fabs(p1.y) < 3.2
    return RELATION_ON;
...
// 点和直线的位置关系
RELATION_ENUM SM_Relation_PointLine_New(const POINT &Point, const POINT &LP1,
                                        const POINT &LP2) {
  POINT p1 = {LP1.x - Point.x, LP1.y - Point.y};

  if ((abs(p1.x) <= TOGGLE_VALUE_0_1_SQRT) &&
      (abs(p1.y) <= TOGGLE_VALUE_0_1_SQRT))
    return RELATION_ON;

  POINT p2 = {LP2.x - Point.x, LP2.y - Point.y};

  if ((abs(p2.x) <= TOGGLE_VALUE_0_1_SQRT) &&
...
////点和直线的位置关系
// RELATION_ENUM	SM_Relation_PointLine(const POINT& Point, const POINT&
// LP1, const POINT& LP2)
//{
//     //g_pRepresent->OutputText(12, Name, KRF_ZERO_END, nMpsX - 12 *
//     g_StrLen(Name) / 4, nMpsY, dwColor, 0, m_Height + nHeightOff); if
//     (!gs_nTimerStartFlag)
//     {
//         gs_Timer.Start();
...
RELATION_ENUM SM_Relation_LineLine_CheckCut(const POINT &LP11,
                                            const POINT &LP12,
                                            const POINT &LP21,
                                            const POINT &LP22, POINT &POI) {
  ...
  if (nRPL1 == nRPL2)
    ...
    if (nRPL1 == RELATION_ON)
      ...
      //	y = -( (x1*y2 - x2*y1)(y4 - y3) - (x3*y4 - x4*y3)(y2 - y1) ) /
