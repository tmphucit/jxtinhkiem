...
public:
  ...
  void ChangeAlpha();
  void ClearCurNo();
  void SetFile(int nNo, char *lpszFileName, int nSprID, int nFrameNo, int nXpos,
               int nYpos, int nZpos);
  void Draw(int nIdx);
  BOOL NowGetBlur();
  void AddObj();
...
public:
  enum {
    ...
    adjustcolor_invisibility,
  };
  KSprControl m_cNpcImage[MAX_PART]; // 所有动作的所有spr文件名
  KSprControl m_cNpcShadow;          // npc阴影
  KStateSpr m_cStateSpr[18]; // 状态特效，0 1 为头顶 2 3 为脚底 4 5 为身上
  ...
  KRUImage m_cDrawFile[MAX_NPC_IMAGE_NUM]; // 绘制列表 身体部件 + 阴影 +
                                           // 魔法状态 + 特殊动画 + 头顶状态
...
public:
  ...
  BOOL SetArmor(int nArmorType, int nFifongType,
                BOOL bFifongState); // 设定盔甲类型
  ...
  void SetPos(int nNpcIdx, int x, int y, int z = 0, BOOL bFocus = FALSE,
              BOOL bMenu = FALSE); // 设定 npc 位置
  ...
  void SetAdjustColorId(unsigned long ulColorId) {
    m_ulAdjustColorId = ulColorId;
  }; // 设置偏色情况，如果为0表示不偏色.
  int GetAction() { return m_nAction; };
  void SetMenuState(int nState, char *lpszSentence = NULL,
                    int nSentenceLength = 0); // 设定头顶状态
