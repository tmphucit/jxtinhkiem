...
public:
	...
	void			Draw(int nNpcIdx, int nDir, int nAllFrame, int nCurFrame, BOOL bInMenu = FALSE, BOOL bOn = FALSE);		// 绘制
	void			DrawBorder();
	int				DrawMenuState(int nHeightOffset);
	BOOL			SetHelm(int nHelmType);								// 设定头盔类型
	BOOL			SetArmor(int nArmorType, int nFifongType, BOOL bFifongState);							// 设定盔甲类型
