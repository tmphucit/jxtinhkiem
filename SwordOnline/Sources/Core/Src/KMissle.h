...
#define MAX_MISSLE_COLI 20
#define MAX_MISSLE_NPC 20

#ifdef _SERVER

#define MAX_MISSLE 200000 // 90k

// #define MAX_MISSLE  30000
...
public:
  ...
  BOOL m_bAutoExplode; //  Is Missle Would AutoExpode ItSelf For Collision When
                       //  It's LiftTime Is Over;
  BOOL m_bTargetSelf;
  ...
  BOOL
      m_bMustBeHit; //  必中，即子弹无特殊情况一定能打中对方，无论是否实际碰撞到对方.
  //	技能获得的数据
  ...
  int m_nDameNpcId[MAX_MISSLE_NPC];
  int m_nColiNpcId[MAX_MISSLE_COLI];
...
private:
  BOOL Init(int nLauncher, int nMissleId, int nXFactor, int nYFactor,
            int nLevel);
  void OnVanish();    // 即将消失
  void OnCollision(); // 碰撞
  void OnFly();       // 飞行过程中
  ...
  void Release();
  int CheckCollision(); // 检测是否碰撞//
                        // 1表示正常碰撞到物体，0表示未碰撞到任何物体,
                        // -1表示落地
  BOOL CheckBeyondRegion(
      int nDOffsetX, int nDOffsetY, int nTagetX,
      int nTagetY); // 检测是否越界   //FALSE表示越到一个无效的位置，TRUE表示OK
  ...
  int ProcessCollision(int nLauncherIdx, int nRegionId, int nMapX, int nMapY,
                       int nRange, int eRelation);
...
public:
  ...
  int Activate();
  static BOOL GetOffsetAxis(int nSubWorld, int nSrcRegionId, int nSrcMapX,
  ...
  BOOL CreateSpecialEffect(eMissleStatus eStatus, int nPX, int nPY, int nPZ,
                           int nNpcIndex = 0);
  static BOOL CreateMissleForShow(char *szMovie, char *szSprInfo, char *szSound,
