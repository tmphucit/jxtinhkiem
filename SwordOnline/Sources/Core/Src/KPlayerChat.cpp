...
void KPlayerChat::SendSentence(KUiMsgParam *pMsg, char *lpszSentence) {
  ...
  if (pMsg->eGenre == MSG_G_CHAT) {
    ...
    sChat.m_wLength = sizeof(PLAYER_SEND_CHAT_COMMAND) - 1 -
                      sizeof(sChat.m_szSentence) + nLength +
                      pMsg->cChatPrefixLen;
    ...
    memcpy(&sChat.m_szSentence[0], (char *)pMsg->cChatPrefix,
           pMsg->cChatPrefixLen);
    ...
    if (m_nCurChannel == CHAT_CUR_CHANNEL_SINGLE) {
      ...
      if (GetName(m_dwTargetID, szName)) {
        if (szName[0])
          m_cRecorder.SaveSentenceSingle(
              Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].Name, szName,
              Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].Name, lpszSentence);
      }
    } else {
      this->m_cRecorder.SaveSentenceChannel(
...
//---------------------------------------------------------------------------
void KPlayerChat::QQSendSentence(DWORD dwID, int nIdx, KUiMsgParam *pMsg,
                                 char *lpszSentence) {
  if (!lpszSentence || !pMsg || pMsg->nMsgLength <= 0)
    return;
  int nLength = pMsg->nMsgLength;
  ...
  sChat.ProtocolType = c2s_playersendchat;
  sChat.m_wLength = sizeof(PLAYER_SEND_CHAT_COMMAND) - 1 -
                    sizeof(sChat.m_szSentence) + nLength + pMsg->cChatPrefixLen;
...
void KPlayerChat::SetTakeChannel(DWORD dwChannel) {
  ...
  m_dwTakeChannel = dwChannel | g_dwChannelOr[CHAT_CUR_CHANNEL_ALL] |
                    g_dwChannelOr[CHAT_CUR_CHANNEL_SCREEN] |
                    g_dwChannelOr[CHAT_CUR_CHANNEL_SYSTEM];
}
...
void KPlayerChat::ServerSendChat(int nPlayerIdx, BYTE *pProtocol) {
  ...
  if (pChat->m_btCurChannel <= CHAT_CUR_CHANNEL_NONE ||
      pChat->m_btCurChannel >= CHAT_CUR_CHANNEL_NUM)
    return;
  if (pChat->m_btChatPrefixLen > CHAT_MSG_PREFIX_MAX_LEN ||
      pChat->m_wSentenceLen >= MAX_SENTENCE_LENGTH)
    return;
  if (pChat->m_wLength !=
      sizeof(PLAYER_SEND_CHAT_COMMAND) - 1 - sizeof(pChat->m_szSentence) +
          pChat->m_btChatPrefixLen + pChat->m_wSentenceLen) {
  ...
  if (pChat->m_btType == MSG_G_CHAT) {
    ...
    case CHAT_CUR_CHANNEL_ALL: {
      ...
      sChat.m_dwSourceID =
          Player[nPlayerIdx]
              .m_dwID; // 发送者的player id 可能需要根据玩家身份改动
      sChat.m_wLength = sizeof(PLAYER_SEND_CHAT_SYNC) - 1 -
                        sizeof(sChat.m_szSentence) + sChat.m_btNameLen +
                        sChat.m_btChatPrefixLen + sChat.m_wSentenceLen;
      memcpy(&sChat.m_szSentence[0], Npc[Player[nPlayerIdx].m_nIndex].Name,
             sChat.m_btNameLen); // 可能需要根据玩家身份改动
      ...
      memcpy(&sChat.m_szSentence[sChat.m_btNameLen + sChat.m_btChatPrefixLen],
             &pChat->m_szSentence[pChat->m_btChatPrefixLen],
             sChat.m_wSentenceLen); // 可能需要根据玩家身份改动
      int nTargetIdx;
      nTargetIdx = PlayerSet.GetFirstPlayer();
      while (nTargetIdx) {
        g_pServer->PackDataToClient(Player[nTargetIdx].m_nNetConnectIdx,
                                    (BYTE *)&sChat, sChat.m_wLength + 1);
    ...
    case CHAT_CUR_CHANNEL_SCREEN: {
      ...
      sChat.m_wSentenceLen = pChat->m_wSentenceLen;
      sChat.m_dwSourceID =
          Npc[Player[nPlayerIdx].m_nIndex].m_dwID; // 此处传的是发送者的 npc id
      sChat.m_wLength = sizeof(PLAYER_SEND_CHAT_SYNC) - 1 -
                        sizeof(sChat.m_szSentence) + sChat.m_btNameLen +
                        sChat.m_btChatPrefixLen + sChat.m_wSentenceLen;
      memcpy(&sChat.m_szSentence[0], Npc[Player[nPlayerIdx].m_nIndex].Name,
      ...
      if (TextGMFilter(
              nPlayerIdx,
              (const char *)(pChat->m_szSentence + pChat->m_btChatPrefixLen),
              sChat.m_wSentenceLen))
        return;
#endif
      Npc[Player[nPlayerIdx].m_nIndex].SendDataToNearRegion(
          (BYTE *)&sChat, sChat.m_wLength + 1);
    } break;
    case CHAT_CUR_CHANNEL_SINGLE: {
      DWORD dwCheckID = 0;
      ...
      if (pChat->m_nTargetIdx <= 0 || pChat->m_nTargetIdx >= MAX_PLAYER) {
        ...
        nIndex = SubWorld[nSubWorld].m_Region[nRegion].FindPlayer(
            pChat->m_dwTargetID);
        if (nIndex <= 0) {
          for (int i = 0; i < 8; i++) {
            int nConRegion =
                SubWorld[nSubWorld].m_Region[nRegion].m_nConnectRegion[i];
            if (nConRegion == -1)
              continue;
            nIndex = SubWorld[nSubWorld].m_Region[nConRegion].FindPlayer(
                pChat->m_dwTargetID);
            if (nIndex > 0)
              break;
          }
      ...
      if (Player[nIndex].m_nIndex == 0 || Player[nIndex].m_dwID != dwCheckID)
        return;
      PLAYER_SEND_CHAT_SYNC sChat;
      sChat.ProtocolType = s2c_playersendchat;
      sChat.m_btCurChannel = pChat->m_btCurChannel;
    ...
    case CHAT_CUR_CHANNEL_TEAM: {
      ...
      if (nTargetIdx != nPlayerIdx &&
          (Player[nTargetIdx].m_cChat.m_dwTakeChannel &
           g_dwChannelOr[CHAT_CUR_CHANNEL_TEAM]))
        g_pServer->PackDataToClient(Player[nTargetIdx].m_nNetConnectIdx,
                                    (BYTE *)&sChat, sChat.m_wLength + 1);
      // 给队员发
      for (int i = 0; i < MAX_TEAM_MEMBER; i++) {
        ...
        if (nTargetIdx < 0 || nTargetIdx == nPlayerIdx)
          continue;
        if (Player[nTargetIdx].m_cChat.m_dwTakeChannel &
            g_dwChannelOr[CHAT_CUR_CHANNEL_TEAM])
          g_pServer->PackDataToClient(Player[nTargetIdx].m_nNetConnectIdx,
                                      (BYTE *)&sChat, sChat.m_wLength + 1);
      }
    ...
    case CHAT_CUR_CHANNEL_FACTION: {
      ...
      nTargetIdx = PlayerSet.GetFirstPlayer();
      while (nTargetIdx) {
        if (Player[nTargetIdx].m_cFaction.m_nCurFaction == nFaction &&
            (Player[nTargetIdx].m_cChat.m_dwTakeChannel &
             g_dwChannelOr[CHAT_CUR_CHANNEL_FACTION]))
          g_pServer->PackDataToClient(Player[nTargetIdx].m_nNetConnectIdx,
                                      (BYTE *)&sChat, sChat.m_wLength + 1);
        nTargetIdx = PlayerSet.GetNextPlayer();
      }
    ...
    case CHAT_CUR_CHANNEL_SCREENSINGLE: {
      ...
      if (nTargetIdx == -1)
        return;
      if (!Player[nTargetIdx].m_cChat.CheckTalkChannel(
        ...
        g_pServer->PackDataToClient(Player[nPlayerIdx].m_nNetConnectIdx,
                                    (BYTE *)&sError, sError.m_wLength + 1);
        return;
      }
      PLAYER_SEND_CHAT_SYNC sChat;
      sChat.ProtocolType = s2c_playersendchat;
    ...
    case CHAT_CUR_CHANNEL_SYSTEM:
      ...
      if (pChat->m_dwTargetID == 0) // to all
      {
        ...
        memcpy(szSentence, &pChat->m_szSentence[pChat->m_btChatPrefixLen],
               nLen); // 可能需要根据玩家身份改动
        szSentence[nLen] = 0;
...
void KPlayerChat::GetChat(PLAYER_SEND_CHAT_SYNC *pChat) {
  ...
  memcpy(szBuf,
         &pChat->m_szSentence[pChat->m_btNameLen + pChat->m_btChatPrefixLen],
         pChat->m_wSentenceLen);
  ...
  case CHAT_CUR_CHANNEL_ALL: {
    if (pChat->m_dwSourceID == Player[CLIENT_PLAYER_INDEX].m_dwID)
      return;
    memcpy(Param.cChatPrefix, &pChat->m_szSentence[pChat->m_btNameLen],
           pChat->m_btChatPrefixLen);
  ...
  case CHAT_CUR_CHANNEL_SINGLE: {
    ...
    if (pFriend == NULL) {
      ...
      /*				CHAT_REDELETE_FRIEND_COMMAND	sApply;
                                      sApply.ProtocolType =
         c2s_chatredeletefriend; sApply.m_dwID = pChat->m_dwSourceID; if
         (g_pClient) g_pClient->SendPackToServer(&sApply,
         sizeof(CHAT_REDELETE_FRIEND_COMMAND)); break;*/
      ...
      if (pChat->m_btChatPrefixLen == 4) {
        ...
        ((char *)(&sMsg.uColor))[2] =
            pChat->m_szSentence[pChat->m_btNameLen + 1];
        ((char *)(&sMsg.uColor))[1] =
            pChat->m_szSentence[pChat->m_btNameLen + 2];
        ((char *)(&sMsg.uColor))[0] =
            pChat->m_szSentence[pChat->m_btNameLen + 3];
      } else {
        sMsg.uColor = 0;
      }
  ...
  if (pChat->m_btCurChannel == CHAT_CUR_CHANNEL_SINGLE) {
    ...
    m_cRecorder.SaveSentenceSingle(
        Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].Name, szName, szName,
        szSentence);
  } else {
    char szName[32], szSentence[MAX_SENTENCE_LENGTH];
    memcpy(szName, &pChat->m_szSentence[0], pChat->m_btNameLen);
    ...
    memcpy(szSentence,
           ...
           pChat->m_wSentenceLen);
    szSentence[pChat->m_wSentenceLen] = 0;
    m_cRecorder.SaveSentenceChannel(
        Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].Name, szName, szSentence);
  }
}
#endif
...
//	功能：申请添加聊天好友
//---------------------------------------------------------------------------
void KPlayerChat::ApplyAddFriend(DWORD dwNpcID,
                                 char *lpszSentence /* = NULL*/) {
  if (dwNpcID <= 0)
    return;
  ...
  sAdd.m_wLength = sizeof(CHAT_APPLY_ADD_FRIEND_COMMAND) - 1 -
                   sizeof(sAdd.m_szInfo) + strlen(sAdd.m_szInfo);
  if (g_pClient)
...
//	功能：界面获取聊天好友添加申请人信息
//---------------------------------------------------------------------------
BOOL KPlayerChat::GetApplyOneInfo(int *lpnPlayerIdx, char *lpszName,
                                  char *lpszInfo) {
...
void KPlayerChat::LoadTakeChannelInfo(char *lpszName) {
  ...
  if (!cChannel.Load(szBuf)) {
    m_dwTakeChannel |= g_dwChannelOr[CHAT_CUR_CHANNEL_TEAM] |
                       g_dwChannelOr[CHAT_CUR_CHANNEL_FACTION] |
                       g_dwChannelOr[CHAT_CUR_CHANNEL_SCREENSINGLE];
    this->SaveTakeChannelInfo(lpszName);
    this->SendTakeChannel();
    return;
  }
  cChannel.GetInteger("Channel", "Channel", this->m_dwTakeChannel,
                      (int *)&dwTakeChannel);
...
//	功能：添加一个成员
//---------------------------------------------------------------------------
void KPlayerChat::AddOne(DWORD dwID, int nPlayerIdx, char *lpszName,
                         int nTeamNo) {
  if (!lpszName || !lpszName[0])
    return;
...
void KPlayerChat::AddFriendData(int nSelfIdx, int nDestIdx) {
  ...
  g_pServer->PackDataToClient(Player[nSelfIdx].m_nNetConnectIdx, (BYTE *)&sAdd,
                              sizeof(CHAT_ADD_FRIEND_SYNC));
}
#endif
...
void KPlayerChat::ResendOneFriendData(DWORD dwID, int nPlayerIdx) {
  ...
  g_pServer->PackDataToClient(Player[nPlayerIdx].m_nNetConnectIdx,
                              (BYTE *)&sData,
                              sizeof(CHAT_ONE_FRIEND_DATA_SYNC));
}
#endif
...
BOOL KPlayerChat::SyncFriendData(int nPlayerIdx, unsigned int &nParam) {
  ...
  if (m_nSyncState == 1) // 玩家刚登录，发送不带名字的聊天好友数据
  {
    ...
    if (SUCCEEDED(g_pServer->PackDataToClient(
            Player[nPlayerIdx].m_nNetConnectIdx, (BYTE *)&sFriend,
            sizeof(CHAT_LOGIN_FRIEND_NONAME_SYNC)))) {
      bRet = TRUE;
    } else {
  ...
  if (m_nSyncState ==
      2) // 玩家刚登录，发送带名字的聊天好友数据(玩家客户端没有好友分组和名字信息)
  {
    if (!m_pStateSendPos) {
      m_nSyncState = 0;
      nParam = 2;
    ...
    sFriend.m_wLength = sizeof(CHAT_LOGIN_FRIEND_NAME_SYNC) - 1 -
                        sizeof(sFriend.m_szName) + strlen(sFriend.m_szName);
    if (SUCCEEDED(g_pServer->PackDataToClient(
            Player[nPlayerIdx].m_nNetConnectIdx, (BYTE *)&sFriend,
            sFriend.m_wLength + 1))) {
      bRet = TRUE;
...
void KPlayerChat::DeleteFriend(DWORD dwID, DWORD dwSelfID) {
  ...
  while (pFriend) {
    if (pFriend->m_dwID == dwID) {
      ...
      // 通知被删除方执行相应处理
      if (pFriend->m_nPlayerIdx) { // 如果对方在线，id 正确
        if (pFriend->m_nPlayerIdx < MAX_PLAYER &&
            Player[pFriend->m_nPlayerIdx].m_nIndex &&
            Player[pFriend->m_nPlayerIdx].m_dwID == dwID) {
          Player[pFriend->m_nPlayerIdx].m_cChat.DeleteFriendData(
              dwSelfID, pFriend->m_nPlayerIdx);
        }
      }
...
void KPlayerChat::DeleteFriendData(DWORD dwID, int nSelfIndex) {
  ...
  while (pFriend) {
    if (pFriend->m_dwID == dwID) {
      ...
      sDelete.m_dwID = dwID;
      g_pServer->PackDataToClient(Player[nSelfIndex].m_nNetConnectIdx,
                                  (BYTE *)&sDelete,
                                  sizeof(CHAT_DELETE_FRIEND_SYNC));
...
void KPlayerChat::OffLine(DWORD dwSelfID) {
  ...
  while (pNode) {
    if (pNode->m_nPlayerIdx) {
      Player[pNode->m_nPlayerIdx].m_cChat.GetMsgOffLine(dwSelfID,
                                                        pNode->m_nPlayerIdx);
    }
    pNode = (CChatFriend *)pNode->GetNext();
  }
  ...
  while (pNode) {
    if (pNode->m_nPlayerIdx && pNode->m_dwID == dwID) {
      ...
      g_pServer->PackDataToClient(Player[nSelfIdx].m_nNetConnectIdx,
                                  (BYTE *)&sSync,
                                  sizeof(CHAT_FRIEND_OFFLINE_SYNC));
      return;
    }
    pNode = (CChatFriend *)pNode->GetNext();
  }
...
//	功能：聊天时间计数加一
//---------------------------------------------------------------------------
void KPlayerChat::TimerAdd() { this->m_nTimer++; }
#endif
...
//	功能：组改名
//---------------------------------------------------------------------------
BOOL KPlayerChat::RenameTeam(int nTeamNo, char *lpszTeamName,
                             char *lpszPlayerName) {
  if (!lpszPlayerName || !lpszPlayerName[0])
    return FALSE;
  if (!lpszTeamName || !lpszTeamName[0])
...
//	功能：客户端申请删除某好友
//---------------------------------------------------------------------------
void KPlayerChat::ApplyDeleteFriend(DWORD dwID, int nTeamNo,
                                    char *lpszSelfName) {
  ...
  while (pFriend) {
    if (pFriend->m_dwID == dwID) {
      ...
      sDelete.m_dwID = dwID;
      if (g_pClient)
        g_pClient->SendPackToServer(&sDelete,
                                    sizeof(CHAT_DELETE_FRIEND_COMMAND));
...
//  lpszSendName:  发送者名字，可能是 系统消息、通知、警告、注意 等等 长度不超过
//  32 lpszSentence:  需要发送的语句 nSentenceLength:  需要发送语句的长度
//---------------------------------------------------------------------------
#ifdef _SERVER
void KPlayerChat::SendSystemInfo(int nType, int nTargetIdx, char *lpszSendName,
                                 char *lpszSentence, int nSentenceLength) {
  ...
  strncpy(pCccSync->someone, lpszSendName,
          _NAME_LEN - 1); // 可能需要根据玩家身份改动
  ...
  if (bAll) // 给全体玩家发送
  {
    ...
    if (nTargetIdx > 0) {
      int nNpcIdx = Player[nTargetIdx].m_nIndex;
      ...
      if (nNpcIdx > 0) {
        ...
        while (nTargetIdx) {
          ...
          int nNpcIdxMap = Player[nTargetIdx].m_nIndex;

          if (nNpcIdxMap > 0 &&
              Npc[nNpcIdxMap].m_SubWorldIndex == Npc[nNpcIdx].m_SubWorldIndex) {
            g_pServer->PackDataToClient(Player[nTargetIdx].m_nNetConnectIdx,
                                        pExHeader, pckgsize);
          }
          nTargetIdx = PlayerSet.GetNextPlayer();
    ...
    while (nTargetIdx) {
      ...
      if (nNpcIdxMap > 0 && Npc[nNpcIdxMap].m_SubWorldIndex == nTargetMapIdx) {
        g_pServer->PackDataToClient(Player[nTargetIdx].m_nNetConnectIdx,
                                    pExHeader, pckgsize);
      }
      nTargetIdx = PlayerSet.GetNextPlayer();
    ...
    if (nTargetIdx > 0) {
      int nNpcIdx = Player[nTargetIdx].m_cTong.GetTongNameID();
      if (Player[nTargetIdx].m_nIndex > 0) {
        ...
        while (nTargetIdx) {
          ...
          int nNpcIdxMap = Player[nTargetIdx].m_cTong.GetTongNameID();

          if (Player[nTargetIdx].m_nIndex > 0 && nNpcIdxMap == nNpcIdx) {
            g_pServer->PackDataToClient(Player[nTargetIdx].m_nNetConnectIdx,
                                        pExHeader, pckgsize);
          }
          nTargetIdx = PlayerSet.GetNextPlayer();
    ...
    if (nTargetIdx > 0) {
      ...
      if (nNpcIdx > 0) {
        ...
        while (nTargetIdx) {
          ...
          int nNpcIdxMap = Player[nTargetIdx].m_nIndex;
          int NpcPlayerX, NpcPlayerY, NpcWorldX, NpcWorldY;
          Npc[nNpcIdx].GetMpsPos(&NpcPlayerX, &NpcPlayerY);
          Npc[nNpcIdxMap].GetMpsPos(&NpcWorldX, &NpcWorldY);

          if (nNpcIdxMap > 0 &&
              Npc[nNpcIdxMap].m_SubWorldIndex == Npc[nNpcIdx].m_SubWorldIndex &&
              (NpcPlayerX - NpcWorldX) * (NpcPlayerX - NpcWorldX) <=
                  384 * 384 &&
              (NpcPlayerY - NpcWorldY) * (NpcPlayerY - NpcWorldY) <=
                  768 * 768) {
            g_pServer->PackDataToClient(Player[nTargetIdx].m_nNetConnectIdx,
                                        pExHeader, pckgsize);
          }
  ...
  else // 给某个特定玩家发送
  {
    ...
    g_pServer->PackDataToClient(Player[nTargetIdx].m_nNetConnectIdx, pExHeader,
                                pckgsize);
  }
  ...
  /*
          ...
          if (nType == 0)	// 给全体玩家发送
          {
                  ...
                  sChat.m_dwSourceID = 0;
  // 发送者的player id 可能需要根据玩家身份改动 sChat.m_btNameLen =
  strlen(lpszSendName); if (sChat.m_btNameLen >= 32) sChat.m_btNameLen = 31; if
  (lpszPrefix && nPrefixLen > 0)
                  {
                          sChat.m_btChatPrefixLen = nPrefixLen;
                          if (sChat.m_btChatPrefixLen >=
  CHAT_MSG_PREFIX_MAX_LEN) sChat.m_btChatPrefixLen = CHAT_MSG_PREFIX_MAX_LEN -
  1;
                  }
                  else
                  {
                  ...
                  sChat.m_wLength = sizeof(PLAYER_SEND_CHAT_SYNC) - 1 -
  sizeof(sChat.m_szSentence) + sChat.m_btNameLen + sChat.m_btChatPrefixLen +
  sChat.m_wSentenceLen; memcpy(&sChat.m_szSentence[0], lpszSendName,
  sChat.m_btNameLen); // 可能需要根据玩家身份改动 if (lpszPrefix)
                          memcpy(&sChat.m_szSentence[sChat.m_btNameLen],
  lpszPrefix, sChat.m_btChatPrefixLen);
                  memcpy(&sChat.m_szSentence[sChat.m_btNameLen +
  sChat.m_btChatPrefixLen], lpszSentence, sChat.m_wSentenceLen); //
  可能需要根据玩家身份改动 int nTargetIdx; nTargetIdx =
  PlayerSet.GetFirstPlayer(); while (nTargetIdx)
                  {
                          g_pServer->PackDataToClient(Player[nTargetIdx].m_nNetConnectIdx,
  (BYTE*)&sChat, sChat.m_wLength + 1); nTargetIdx = PlayerSet.GetNextPlayer();
                  }
          }
          else			// 给某个特定玩家发送
          {
...
#ifdef _SERVER
void KPlayerChat::SendGlobalSystemInfo(char *lpszSendName, char *lpszSentence,
                                       int nSentenceLength) {
  if (!lpszSendName || !lpszSentence)
    return;
  if (nSentenceLength >= MAX_SENTENCE_LENGTH)
...
#ifdef _SERVER
void KPlayerChat::SendInfoToGM(char *lpszAccName, char *lpszRoleName,
                               char *lpszSentence, int nSentenceLength) {
  if (!lpszAccName || !lpszRoleName || !lpszSentence)
    return;
  if (nSentenceLength >= MAX_SENTENCE_LENGTH)
    nSentenceLength = MAX_SENTENCE_LENGTH;
...
#ifdef _SERVER
void KPlayerChat::SendInfoToIP(DWORD nIP, DWORD nID, char *lpszAccName,
                               char *lpszRoleName, char *lpszSentence,
                               int nSentenceLength) {
  if (!lpszAccName || !lpszRoleName || !lpszSentence)
    return;
  if (nSentenceLength >= MAX_SENTENCE_LENGTH)
...
void KPlayerChat::MakeBrother(const STRINGLIST &brothers) {
  ...
  for (STRINGLIST::const_iterator itFriend = brothers.begin();
       itFriend != brothers.end(); itFriend++) {
...
void KPlayerChat::MakeEnemy(char *szPlayer, char *szEnemy) {
  if (!szPlayer || szPlayer[0] == 0 || !szEnemy || szEnemy[0] == 0)
    return;
...
int KPlayerChat::SendSelectChannelList(KUiChatChannel *pList, int nCount) {
  ...
  if (nTempCount < nCount)
    memset(&pList[nTempCount], 0,
           sizeof(KUiChatChannel) * (nCount - nTempCount));
...
void CChatFriendTeam::DataBaseAddOne(int nSelfIdx, DWORD dwID, char *lpszName) {
  ...
  if (pFriend->m_nPlayerIdx > 0)
    Player[pFriend->m_nPlayerIdx].ChatFriendOnLine(Player[nSelfIdx].m_dwID,
                                                   nSelfIdx);
}
#endif
...
void KCHAT_RECORDER::SetTime() {
  ...
  sprintf(m_szTime, "%02d:%02d:%02d", sTime.wHour, sTime.wMinute,
          sTime.wSecond);
}

void KCHAT_RECORDER::SaveSentenceSingle(char *lpszSelfName, char *lpszTarget,
                                        char *lpszTalkerName,
                                        char *lpszSentence) {
  // 暂时关闭
  return;
  ...
  if (!lpszSelfName || !lpszTarget || !lpszTalkerName || !lpszSentence)
    return;
  if (!lpszSelfName[0] || !lpszTarget[0] || !lpszTalkerName[0] ||
      !lpszSentence[0])
    return;
...
void KCHAT_RECORDER::SaveSentenceChannel(char *lpszSelfName,
                                         char *lpszTalkerName,
                                         char *lpszSentence) {
  // 暂时关闭
  return;
