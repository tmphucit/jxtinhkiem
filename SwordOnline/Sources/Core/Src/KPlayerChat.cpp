...
void	KPlayerChat::SendSystemInfo(int nType, int nTargetIdx, char *lpszSendName, char *lpszSentence, int nSentenceLength)
{
	...
	while (nTargetIdx)
		...
		if (nNpcIdxMap > 0 && Npc[nNpcIdxMap].m_SubWorldIndex == Npc[nNpcIdx].m_SubWorldIndex)
		{
			g_pServer->PackDataToClient(Player[nTargetIdx].m_nNetConnectIdx, pExHeader, pckgsize);
		}
			nTargetIdx = PlayerSet.GetNextPlayer();
	...
	while (nTargetIdx)
		...
		if (nNpcIdxMap > 0 && Npc[nNpcIdxMap].m_SubWorldIndex == Npc[nNpcIdx].m_SubWorldIndex && (NpcPlayerX - NpcWorldX) *  (NpcPlayerX - NpcWorldX) <= 768*768 && (NpcPlayerY - NpcWorldY) *  (NpcPlayerY - NpcWorldY) <= 1535*1535 )
		{
			g_pServer->PackDataToClient(Player[nTargetIdx].m_nNetConnectIdx, pExHeader, pckgsize);
		}
			nTargetIdx = PlayerSet.GetNextPlayer();
