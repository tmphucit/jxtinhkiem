...
#define MAX_PLAYER_ITEM                                                        \
  (MAX_EQUIPMENT_ITEM + MAX_REPOSITORY_ITEM + MAX_TRADE_ITEM +                 \
   MAX_TRADE1_ITEM + MAX_IMMEDIACY_ITEM + itempart_num + MAX_HAND_ITEM +       \
   MAX_PLAYER_ITEM_RESERVED + MAX_CHECKBOX_ITEM + MAX_PGBOX_ITEM +             \
   MAX_BUYSHOP_ITEM + MAX_RPNEW_ITEM)
...
#define MAX_NPC_RECORDER_STATE 20 // 最大Npc可以记录下来的状态数量;
...
#define GOD_MAX_OBJ_TITLE_LEN                                                  \
  2048 // 128临时改为1024为了兼容旧代码 to be modified
...
struct KGameObjDesc {
  ...
  char szProp
      [GOD_MAX_OBJ_PROP_LEN]; // 属性，每行可以tab划分为靠左与靠右对齐两部分
...
struct KSystemMessage {
  ...
  unsigned char
      byParamSize; // 伴随GDCNI_SYSTEM_MESSAGE消息的pParamBuf所指参数缓冲区空间的大小。
};
...
struct KUiPlayerRuntimeInfo {
  ...
  unsigned char
      byActionDisable; // 是否不可进行各种动作，为枚举PLAYER_ACTION_LIST取值的组合
  unsigned char
      byAction; // 正在进行的行为动作，为枚举PLAYER_ACTION_LIST取值的组合
...
enum NPC_RELATION {
  ...
  relation_enemy = 8,
  relation_dialog = 16,
  relation_all = relation_none | relation_ally | relation_enemy |
                 relation_self | relation_dialog,
  relation_num,
};
...
struct KRankMessage {
  ...
  short
      cSortFlag; // 旗标值，QOO_RANK_DATA的时候表示出升降，负值表示降，正值表示升，0值表示位置未变
...
struct KMissionRecord {
  char sContent
      [256]; // 存储任务提示信息的缓冲区，（字符串为控制符已经编码的字符串）
...
struct KTongOperationParam {
  ...
  int nData[4];
  char szPassword
      [32]; // 操作项需要的密码，如果对应的操作项不需要密码，则直接填密码为空。
};
