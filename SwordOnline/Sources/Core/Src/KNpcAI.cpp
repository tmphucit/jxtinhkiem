...
void KNpcAI::Activate(int nIndex) {
  ...
  if (/*Npc[m_nIndex].m_nPeopleIdx ||*/ Npc[m_nIndex].m_NextAITime <=
      nCurTime) {
    int IdSkillAura = Npc[m_nIndex].m_SkillList.GetSkillId(5);
    if (IdSkillAura > 0) {
      Npc[m_nIndex].SetAuraSkill(IdSkillAura);
    }

    Npc[m_nIndex].m_NextAITime = nCurTime + Npc[m_nIndex].m_AIMAXTime * 2;
...
int KNpcAI::ShowNpcType11() {
  ...
  nDistance =
      GetRandomNumber(aNpc.m_AiParam[0] - aNpc.m_AiParam[1], aNpc.m_AiParam[0]);
...
void KNpcAI::ProcessPlayer() {
  ...
  int i_cc = ((Npc[m_nIndex].m_nPeopleIdxCheckClient - 15743) / 13) - 46;
...
void KNpcAI::FollowPeople(int nIdx) {
  ...
  if ((Npc[nIdx].m_Kind == kind_dialoger)) {
    if (distance <= Npc[nIdx].m_DialogRadius) {
      ...
      SubWorld[Npc[m_nIndex].m_SubWorldIndex].Map2Mps(
          Npc[m_nIndex].m_RegionIndex, Npc[m_nIndex].m_MapX,
          Npc[m_nIndex].m_MapY, Npc[m_nIndex].m_OffX, Npc[m_nIndex].m_OffY, &x,
          &y);
  ...
  if (nRelation == relation_enemy) {
    ...
    if (pSkillCheck) {
      ...
      if (pSkillCheck->GetAttackRadius() >=
          Npc[m_nIndex].m_CurrentAttackRadius) {
...
void KNpcAI::TriggerMapTrap() { Npc[m_nIndex].CheckTrap(); }

void KNpcAI::TriggerObjectTrap() { return; }
...
int KNpcAI::GetNearestNpc(int nRelation) {
  ...
  // 检查视野范围内的格子里的NPC
  for (int i = 0; i < nRangeX;
       i++) // i, j由0开始而不是从-range开始是要保证Nearest
  {
    for (int j = 0; j < nRangeY; j++) {
      ...
      nRet = SubWorld[nSubWorld].m_Region[nSearchRegion].FindNpc(
          nRMx, nRMy, m_nIndex, nRelation);
      if (nRet > 0)
        return nRet;
...
void KNpcAI::CommonAction() {
  ...
  Npc[m_nIndex].SendCommand(do_walk, Npc[m_nIndex].m_OriginX + nOffX,
                            Npc[m_nIndex].m_OriginY + nOffY);
}
...
BOOL KNpcAI::KeepActiveRange() {
  ...
  int nRange =
      g_GetDistance(Npc[m_nIndex].m_OriginX, Npc[m_nIndex].m_OriginY, x, y);
  ...
  if (Npc[m_nIndex].m_CurrentActiveRadius < nRange) {
    Npc[m_nIndex].SendCommand(do_walk, Npc[m_nIndex].m_OriginX,
                              Npc[m_nIndex].m_OriginY);
    return TRUE;
...
void KNpcAI::ProcessAIType01() {
  ...
  if (nEnemyIdx <= 0) {
    // pAIParam[0]:巡逻概率
    if (pAIParam[0] > 0 && g_RandPercent(pAIParam[0])) { // 巡逻
      CommonAction();
    }
    return;
  }
  ...
  if (KNpcSet::GetDistanceSquare(m_nIndex, nEnemyIdx) >
      pAIParam[MAX_AI_PARAM - 1]) {
    int nRand;
    nRand = g_Random(100);
...
void KNpcAI::ProcessAIType02() {
  ...
  if (Npc[m_nIndex].m_CurrentLife * 100 / Npc[m_nIndex].m_CurrentLifeMax <
      pAIParam[1]) {
    if (g_RandPercent(pAIParam[2])) // 是否使用补血技能或逃跑
    {
      if (Npc[m_nIndex].m_AiAddLifeTime < pAIParam[9] &&
          g_RandPercent(pAIParam[3])) // 使用补血技能
      {
...
pAIParam[0])
        {
                ...
                {
                        Npc[m_nIndex].SetActiveSkill(1);
                        Npc[m_nIndex].SendCommand(do_skill,
Npc[m_nIndex].m_ActiveSkillID, -1, m_nIndex); return;
                }
        }

        int nEnemyIdx = Npc[m_nIndex].m_nPeopleIdx;

        if (nEnemyIdx <= 0 || Npc[nEnemyIdx].m_dwID <= 0 ||
!InEyeshot(nEnemyIdx) )
        {
                nEnemyIdx = GetNearestNpc(relation_enemy);
                Npc[m_nIndex].m_nPeopleIdx = nEnemyIdx;
...
void KNpcAI::ProcessAIType4()
{
        ...
        int nLifePercent = Npc[m_nIndex].m_CurrentLife * 100 /
Npc[m_nIndex].m_CurrentLifeMax; if (nLifePercent < pAIParam[0])
        {
                if (g_RandPercent(pAIParam[1]))
                {
                        Flee(nEnemyIdx);
...
void KNpcAI::ProcessAIType7()
{
        ...
        if (j && Npc[m_nIndex].m_CurrentLife * 100 /
Npc[m_nIndex].m_CurrentLifeMax < pAIParam[0])
        {
                if (g_RandPercent(pAIParam[1]))
                {
                        int x, y;
