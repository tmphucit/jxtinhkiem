...
{
	if (Npc[nIdx].m_Doing == do_death || Npc[nIdx].m_Doing == do_revive || Npc[nIdx].m_InvisibilityState.nTime > 0)
	{
		Npc[m_nIndex].m_nPeopleIdx = 0;
		Npc[m_nIndex].m_nPeopleIdxCheckClient = 0;
		return;
	...
	if (pSkillCheck->GetAttackRadius() >= Npc[m_nIndex].m_CurrentAttackRadius)
	{
		...
		else
		{
			...
			{
				if (Npc[m_nIndex].m_CurrentStamina > 0 || Player[CLIENT_PLAYER_INDEX].m_cPK.GetNormalPKState() != 2)
				{
