...
bool KWeatherMgr::InitFromIni(KIniFile &IniFile) {
  ...
  for (int i = 0; i < m_nWeatherNum; i++) {
    ...
    IniFileWeather.GetInteger(cc, "LifeTimeMin", 200,
                              &m_weatherInfo[i].m_nLifeTimeMin);
    IniFileWeather.GetInteger(cc, "LifeTimeMax", 600,
                              &m_weatherInfo[i].m_nLifeTimeMax);
  ...
  m_nNextHappentime = m_nLastHappentime + m_nHappenTimeMin +
                      g_Random(m_nHappenTimeMax - m_nHappenTimeMin);
...
int KWeatherMgr::Activate() {
  ...
  if (!m_bInWeather) {
    if (tmCur > m_nNextHappentime) {
      ...
      for (i = 0; i < m_nWeatherNum; i++) {
        if (nOdds < m_weatherInfo[i].m_nOdds) {
          ...
          m_nWeatherDuration = m_weatherInfo[i].m_nLifeTimeMin +
                               g_Random(m_weatherInfo[i].m_nLifeTimeMax -
                                        m_weatherInfo[i].m_nLifeTimeMin);
          break;
        } else
      ...
      m_nNextHappentime = m_nLastHappentime + m_nWeatherDuration +
                          m_nHappenTimeMin +
                          g_Random(m_nHappenTimeMax - m_nHappenTimeMin);
      m_bInWeather = true;
    }
