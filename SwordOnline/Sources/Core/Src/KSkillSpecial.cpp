...
void KSkillSpecial::Draw(int nCurLifeFrame) {
  ...
  if (nCurLifeFrame > (int)m_nEndTime)
    return;
  ...
  if (m_nNpcIndex > 0 && Npc[m_nNpcIndex].IsMatch(m_dwMatchID)) {
    int nSrcX = 0;
    int nSrcY = 0;
    SubWorld[0].Map2Mps(Npc[m_nNpcIndex].m_RegionIndex, Npc[m_nNpcIndex].m_MapX,
                        Npc[m_nNpcIndex].m_MapY, Npc[m_nNpcIndex].m_OffX,
                        Npc[m_nNpcIndex].m_OffY, &nSrcX, &nSrcY);
  ...
  if (m_pMissleRes->nInterval > 0) {
    ...
    if (nImageDir1 >= 32 / m_pMissleRes->nDir)
      nImageDir++;
    if (nImageDir >= m_pMissleRes->nDir)
      nImageDir = 0;

    m_RUImage.nFrame =
        (short)(nCurLifeFrame - m_nBeginTime) / m_pMissleRes->nInterval +
        nImageDir * m_pMissleRes->nTotalFrame / m_pMissleRes->nDir;
#ifdef TOOLVERSION
    KSprite *pSprite = NULL;
    (KCacheNode *)m_pSprNode = (KCacheNode *)g_SpriteCache.GetNode(
        m_RUImage.szImage, (KCacheNode *)m_pSprNode);
...
void KSkillSpecial::SetAnimation(char *szFile, unsigned int uImage) {
  if (szFile)
    strcpy(m_RUImage.szImage, szFile);
