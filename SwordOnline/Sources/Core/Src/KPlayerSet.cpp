...
BOOL KPlayerSet::Init() {
  ...
  if (cTongFile.Load(defPLAYER_TONG_PARAM_FILE)) {
    ...
    cTongFile.GetInteger("TongCreate", "LeadLevel", 10,
                         &m_sTongParam.m_nLeadLevel);
...
int KPlayerSet::FindFree() { return m_FreeIdx.GetNext(0); }
...
int KPlayerSet::Broadcasting(char *pMessage, int nLen) {
  ...
  KPlayerChat::SendSystemInfo(0, 0, MESSAGE_SYSTEM_ANNOUCE_HEAD, pMessage,
                              nLen);
...
void KPlayerSet::SendWelcomeMsg(int nPlayerIndex) {
  ...
  KPlayerChat::SendSystemInfo(0, 0, MESSAGE_SYSTEM_ANNOUCE_HEAD, szBuffer,
                              nLength);
  ...
  if (m_pWelcomeMsg) {
    ...
    for (int i = 0; i < nCount; i++) {
      int nLen = *pAMsg;
      KPlayerChat::SendSystemInfo(1, nPlayerIndex, "吴府管家",
                                  (((char *)pAMsg) + sizeof(int)), nLen);
  ...
  *(int *)m_pWelcomeMsg = 0;
  ...
  while (pMsgHeader = (char *)memchr(pSearchBegin, ':', nLen)) {
    ...
    if (nMsgLen > 0) {
      *pLineHeader = nMsgLen;
      pLineHeader =
          (int *)(((char *)pLineHeader) + sizeof(int) + (*pLineHeader));
      (*(int *)m_pWelcomeMsg)++;
    }
...
void KPlayerSet::PrepareRemove(int nIndex) {
  ...
  g_DebugLog("SERVER:Player[%s] has been removed!",
             Npc[Player[nIndex].m_nIndex].Name);
  // 通知聊天好友自己下线了
...
void KPlayerSet::PrepareExchange(int i) {
  if (!Player[i].m_bExchangeServer ||
      !Npc[Player[i].m_nIndex].m_bExchangeServer)
    return;
...
void KPlayerSet::RemoveQuiting(int nIndex) {
  if (Player[nIndex].m_nNetConnectIdx == -1 || Player[nIndex].m_dwID == 0 ||
      Player[nIndex].m_nLiXian) // UYTHAC
  {
    return;
  }
...
void KPlayerSet::RemoveLoginTimeOut(int nIndex) {
  if (Player[nIndex].IsLoginTimeOut()) {
    if (Player[nIndex].m_nIndex > 0) {
      ...
      /*		if (nSubWorld >= 0)
                              SubWorld[nSubWorld].RemovePlayer(nRegion,
         nIndex);*/
...
#ifdef _SERVER
void KPlayerSet::ProcessClientMessage(int nIndex, const char *pChar,
                                      int nSize) {
  if (nIndex <= 0 && nIndex >= MAX_PLAYER)
    return;
...
BOOL KLevelAdd::Init() {
  ...
  for (i = 0; i < MAX_LEVEL; i++) {
    ...
    LevelExp.GetInteger(i + 2, 3, 0, &m_nLevelExp1[i]);
    LevelExp.GetInteger(i + 2, 4, 0, &m_nLevelExp2[i]);

    LevelExp.GetInteger(i + 2, 5, 0, &m_nLevelExp3[i]);
    LevelExp.GetInteger(i + 2, 6, 0, &m_nLevelExp4[i]);
    LevelExp.GetInteger(i + 2, 7, 0, &m_nLevelExp5[i]);
  }
...
int KLevelAdd::GetLevelExp1(int nLevel) {
  if (nLevel < 1 || nLevel > MAX_LEVEL)
    return 0;
  return m_nLevelExp1[nLevel - 1];
}

int KLevelAdd::GetLevelExp2(int nLevel) {
  if (nLevel < 1 || nLevel > MAX_LEVEL)
    return 0;
  return m_nLevelExp2[nLevel - 1];
}

int KLevelAdd::GetLevelExp3(int nLevel) {
  if (nLevel < 1 || nLevel > MAX_LEVEL)
    return 0;
  return m_nLevelExp3[nLevel - 1];
}

int KLevelAdd::GetLevelExp4(int nLevel) {
  if (nLevel < 1 || nLevel > MAX_LEVEL)
    return 0;
  return m_nLevelExp4[nLevel - 1];
}

int KLevelAdd::GetLevelExp5(int nLevel) {
  if (nLevel < 1 || nLevel > MAX_LEVEL)
    return 0;
  return m_nLevelExp5[nLevel - 1];
}
...
int KLevelAdd::GetFireResist(int nSeries, int nLevel) {
  ...
  if (nLevel <= 0 || nLevel > MAX_LEVEL)
    return 0;
  return (m_nFireResistPerLevel[nSeries] * nLevel / 200);
}
...
int KLevelAdd::GetColdResist(int nSeries, int nLevel) {
  ...
  if (nLevel <= 0 || nLevel > MAX_LEVEL)
    return 0;
  return (m_nColdResistPerLevel[nSeries] * nLevel / 200);
}
...
int KLevelAdd::GetPoisonResist(int nSeries, int nLevel) {
  ...
  if (nLevel <= 0 || nLevel > MAX_LEVEL)
    return 0;
  return (m_nPoisonResistPerLevel[nSeries] * nLevel / 200);
}
...
int KLevelAdd::GetLightResist(int nSeries, int nLevel) {
  ...
  if (nLevel <= 0 || nLevel > MAX_LEVEL)
    return 0;
  return (m_nLightResistPerLevel[nSeries] * nLevel / 200);
}
...
int KLevelAdd::GetPhysicsResist(int nSeries, int nLevel) {
  ...
  if (nLevel <= 0 || nLevel > MAX_LEVEL)
    return 0;
  return (m_nPhysicsResistPerLevel[nSeries] * nLevel / 200);
}
...
BOOL KNewPlayerAttribute::Init() {
  ...
  for (int i = 0; i < series_num; i++) {
    ...
    AttributeFile.GetInteger(szSeries[i], "Lucky", PLAYER_FIRST_LUCKY,
                             &m_nLucky[i]);
  }
...
#ifdef _SERVER
int KPlayerSet::GetPlayerCamp(int nIndex) {
  //	int i = FindClient(nClient);
  ...
  return Npc[Player[nIndex].m_nIndex].m_CurrentCamp;
}
#endif
...
void KPlayerSet::AutoSave() {
  ...
  if (uTime >= m_ulNextSaveTime) {
    int nUseIdx = m_UseIdx.GetNext(0);
    while (nUseIdx) {
      if (Player[nUseIdx].CanSave() &&
          uTime - Player[nUseIdx].m_ulLastSaveTime >=
              m_ulMaxSaveTimePerPlayer) {
        if (Player[nUseIdx].Save()) {
          Player[nUseIdx].m_uMustSave = SAVE_REQUEST;
          //					Player[nUseIdx].m_ulLastSaveTime
          //= uTime;
