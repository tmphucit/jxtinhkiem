...
void	KNpcRes::DrawStun()
{
	...
	RUIconImageStun.nFrame = (SubWorld[0].m_dwCurrentTime*10/18)%10;
			...
			else  if (m_ulAdjustColorId > 0 && m_ulAdjustColorId <= g_ulAdjustColorCount)
			{
...
void	KNpcRes::Draw(int nNpcIdx, int nDir, int nAllFrame, int nCurFrame, BOOL bInMenu, BOOL bOn)
{
		...
		if ( m_pcResNode->CheckPartExist(4) )
		{
			if (bFifongState == TRUE)
			{
				if (m_nFifongType != nFifongType)
				{
					...
					m_pcResNode->GetFileName(4, m_nAction, m_nFifongType, "", szBuffer, sizeof(szBuffer));
					m_cNpcImage[4].SetSprFile(szBuffer, m_pcResNode->GetTotalFrames(4, m_nAction, m_nFifongType, 16), m_pcResNode->GetTotalDirs(4, m_nAction, m_nFifongType, 16), m_pcResNode->GetInterval(4, m_nAction, m_nFifongType, 0));
				}
			}
			else
			{
		...
		if ( m_pcResNode->CheckPartExist(i) )
		{
			if (i == 4)
			{
				if (bFifongState == TRUE)
				{
					m_bFifongState = bFifongState;
					m_nFifongType = nFifongType;
					m_pcResNode->GetFileName(i, m_nAction, m_nFifongType, "", szBuffer, sizeof(szBuffer));
					m_cNpcImage[i].SetSprFile(szBuffer, m_pcResNode->GetTotalFrames(i, m_nAction, m_nFifongType, 16), m_pcResNode->GetTotalDirs(i, m_nAction, m_nFifongType, 16), m_pcResNode->GetInterval(i, m_nAction, m_nFifongType, 0));
				}
				else
				{
					m_bFifongState = FALSE;
