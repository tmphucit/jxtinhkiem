...
BOOL KNpcRes::Init(char *lpszNpcName, KNpcResList *pNpcResList) {
  ...
  for (i = 0; i < MAX_PART; i++) {
    if (m_pcResNode->CheckPartExist(i)) {
      ...
      m_cNpcImage[i].SetSprFile(
          szBuffer, m_pcResNode->GetTotalFrames(i, m_nAction, 0, 16),
          m_pcResNode->GetTotalDirs(i, m_nAction, 0, 16),
          m_pcResNode->GetInterval(i, m_nAction, 0, 0));
    }
    // 如果此部件不存在，对应的文件名都填空
    else {
  ...
  if (m_pcResNode->m_cShadowInfo.GetFile(m_nAction, &nShadowFrame, &nShadowDir,
                                         &nShadowInterval, &nShadowCgX,
                                         &nShadowCgY, szBuffer)) {
    this->m_cNpcShadow.SetSprFile(szBuffer, nShadowFrame, nShadowDir,
                                  nShadowInterval);
...
void KNpcRes::DrawStun() {
  ...
  KRUImage RUIconImageStun;
  RUIconImageStun.nType = ISI_T_SPR;
  RUIconImageStun.uImage = 0;
  RUIconImageStun.nISPosition = IMAGE_IS_POSITION_INIT;
  RUIconImageStun.bRenderFlag = RUIMAGE_RENDER_FLAG_REF_SPOT;
  strcpy(RUIconImageStun.szImage, SPRSTUNSTATE);
  RUIconImageStun.oPosition.nX = m_nXpos;
  RUIconImageStun.oPosition.nY = m_nYpos;
  RUIconImageStun.oPosition.nZ = nHeightOffset + m_nZpos;
  RUIconImageStun.nFrame = (SubWorld[0].m_dwCurrentTime * 10 / 18) % 10;

  if (m_ulAdjustColorId == adjustcolor_invisibility) {
    RUIconImageStun.bRenderStyle = IMAGE_RENDER_STYLE_ALPHA;
    RUIconImageStun.Color.Color_b.a = 80;
  } else if (m_ulAdjustColorId > 0 &&
             m_ulAdjustColorId <= g_ulAdjustColorCount) {
    RUIconImageStun.bRenderStyle = IMAGE_RENDER_STYLE_ALPHA_COLOR_ADJUST;
    RUIconImageStun.Color.Color_dw = g_pAdjustColorTab[m_ulAdjustColorId - 1];
  } else {
    RUIconImageStun.bRenderStyle = IMAGE_RENDER_STYLE_ALPHA;
    RUIconImageStun.Color.Color_b.a = 255;
  }

  g_pRepresent->DrawPrimitives(1, &RUIconImageStun, RU_T_IMAGE, 0);
}
...
void KNpcRes::Draw(int nNpcIdx, int nDir, int nAllFrame, int nCurFrame,
  ...
  if (nAllFrame > 0) {
    ...
    nCurFrameNo = nCurDirNo * (nGetFrame / nGetDir) +
                  (nGetFrame / nGetDir) * nCurFrame / nAllFrame;
...
#ifdef TOOLVERSION
          KImageParam sImage;
          if (g_pRepresent->GetImageParam(m_cNpcImage[i].m_szName, &sImage,
                                          ISI_T_SPR)) {
...
#ifdef TOOLVERSION
        KImageParam sImage;
        g_pRepresent->GetImageParam(m_cNpcImage[i].m_szName, &sImage,
                                    ISI_T_SPR);
        {
          m_cNpcImage[i].m_nTotalDir = sImage.nNumFramesGroup;
          m_cNpcImage[i].m_nTotalFrame = sImage.nNumFrames;
  ...
  // 得到部件排列顺序
  if (!m_pcResNode->GetSort(m_nAction, nCurDirNo, nCurFrameNo, m_nSortTable,
                            MAX_PART))
    return;
  ...
  for (i = 0; i < MAX_PART; i++) {
    if (m_nSortTable[i] >= 0 && m_nSortTable[i] < MAX_PART) {
      ...
      if (m_ulAdjustColorId == adjustcolor_invisibility) {
        m_cDrawFile[i].bRenderStyle = IMAGE_RENDER_STYLE_ALPHA;
        m_cDrawFile[i].Color.Color_b.a = 80;
      } else if (m_ulAdjustColorId > 0 &&
                 m_ulAdjustColorId <= g_ulAdjustColorCount) {
        m_cDrawFile[i].bRenderStyle = IMAGE_RENDER_STYLE_ALPHA_COLOR_ADJUST;
        m_cDrawFile[i].Color.Color_dw =
            g_pAdjustColorTab[m_ulAdjustColorId - 1];
      } else {
        m_cDrawFile[i].bRenderStyle = IMAGE_RENDER_STYLE_ALPHA;
        m_cDrawFile[i].Color.Color_b.a = 255;
      }
  ...
  if (m_nBlurState == TRUE && m_cNpcBlur.NowGetBlur()) {
    ...
    for (i = 0, j = 0; i < MAX_PART; i++) {
      if (m_nSortTable[i] >= 0 && m_nSortTable[i] < MAX_PART) {
        m_cNpcBlur.SetFile(j, m_cNpcImage[m_nSortTable[i]].m_szName,
                           m_cNpcImage[m_nSortTable[i]].m_dwNameID,
                           m_cNpcImage[m_nSortTable[i]].m_nCurFrame, nScreenX,
                           nScreenY, nScreenZ);
        j++;
      }
    }
...
BOOL KNpcRes::SetHelm(int nHelmType) {
  ...
  for (i = MAX_BODY_PART_SECT * 0;
       i < MAX_BODY_PART_SECT * 0 + MAX_BODY_PART_SECT; i++) {
    if (m_pcResNode->CheckPartExist(i)) {
      if (nHelmType != 1000) {
        m_pcResNode->GetFileName(i, m_nAction, nHelmType, "", szBuffer,
                                 sizeof(szBuffer));
        m_cNpcImage[i].SetSprFile(
            szBuffer, m_pcResNode->GetTotalFrames(i, m_nAction, nHelmType, 16),
            m_pcResNode->GetTotalDirs(i, m_nAction, nHelmType, 16),
            m_pcResNode->GetInterval(i, m_nAction, nHelmType, 0));
      }
...
//	功能：	设定盔甲类型
//---------------------------------------------------------------------------
BOOL KNpcRes::SetArmor(int nArmorType, int nFifongType, BOOL bFifongState) {
  int i;
  ...
  if (m_nArmorType == nArmorType) {
    if (m_pcResNode->CheckPartExist(4)) {
      if (bFifongState == TRUE) {
        if (m_nFifongType != nFifongType) {
          char szBuffer[80];
          m_bFifongState = bFifongState;
          m_nFifongType = nFifongType;
          m_pcResNode->GetFileName(4, m_nAction, m_nFifongType, "", szBuffer,
                                   sizeof(szBuffer));
          m_cNpcImage[4].SetSprFile(
              szBuffer,
              m_pcResNode->GetTotalFrames(4, m_nAction, m_nFifongType, 16),
              m_pcResNode->GetTotalDirs(4, m_nAction, m_nFifongType, 16),
              m_pcResNode->GetInterval(4, m_nAction, m_nFifongType, 0));
        }
      } else {
        m_bFifongState = FALSE;
        m_nFifongType = 0;
  ...
  char szBuffer[80];
  for (i = MAX_BODY_PART_SECT * 1;
       i < MAX_BODY_PART_SECT * 1 + MAX_BODY_PART_SECT; i++) {
    if (m_pcResNode->CheckPartExist(i)) {
      if (i == 4) {
        if (bFifongState == TRUE) {
          m_bFifongState = bFifongState;
          m_nFifongType = nFifongType;
          m_pcResNode->GetFileName(i, m_nAction, m_nFifongType, "", szBuffer,
                                   sizeof(szBuffer));
          m_cNpcImage[i].SetSprFile(
              szBuffer,
              m_pcResNode->GetTotalFrames(i, m_nAction, m_nFifongType, 16),
              m_pcResNode->GetTotalDirs(i, m_nAction, m_nFifongType, 16),
              m_pcResNode->GetInterval(i, m_nAction, m_nFifongType, 0));
        } else {
          m_bFifongState = FALSE;
          m_nFifongType = 0;
          m_cNpcImage[4].Release();
        ...
        m_pcResNode->GetFileName(i, m_nAction, nArmorType, "", szBuffer,
                                 sizeof(szBuffer));
        m_cNpcImage[i].SetSprFile(
            szBuffer, m_pcResNode->GetTotalFrames(i, m_nAction, nArmorType, 16),
            m_pcResNode->GetTotalDirs(i, m_nAction, nArmorType, 16),
            m_pcResNode->GetInterval(i, m_nAction, nArmorType, 0));
      }
    } else {
      m_cNpcImage[i].Release();
    }
...
BOOL KNpcRes::SetWeapon(int nWeaponType) {
  ...
  if (m_pcResNode->m_cShadowInfo.GetFile(m_nAction, &nFrame, &nDir, &nInterval,
                                         &nCgX, &nCgY, szBuffer)) {
  ...
  for (i = MAX_BODY_PART_SECT * 0;
       i < MAX_BODY_PART_SECT * 0 + MAX_BODY_PART_SECT; i++) {
    if (m_pcResNode->CheckPartExist(i)) {
      m_pcResNode->GetFileName(i, m_nAction, m_nHelmType, "", szBuffer,
                               sizeof(szBuffer));
      m_cNpcImage[i].SetSprFile(
          szBuffer, m_pcResNode->GetTotalFrames(i, m_nAction, m_nHelmType, 16),
          m_pcResNode->GetTotalDirs(i, m_nAction, m_nHelmType, 16),
          m_pcResNode->GetInterval(i, m_nAction, m_nHelmType, 0));
    } else {
      m_cNpcImage[i].Release();
    }
  }
  for (i = MAX_BODY_PART_SECT * 1;
    ...
    if (m_pcResNode->CheckPartExist(i)) {
      if (i == 4) {
        ...
        m_pcResNode->GetFileName(i, m_nAction, m_nArmorType, "", szBuffer,
                                 sizeof(szBuffer));
        m_cNpcImage[i].SetSprFile(
            szBuffer,
            m_pcResNode->GetTotalFrames(i, m_nAction, m_nArmorType, 16),
            m_pcResNode->GetTotalDirs(i, m_nAction, m_nArmorType, 16),
            m_pcResNode->GetInterval(i, m_nAction, m_nArmorType, 0));
      }
    } else {
      m_cNpcImage[i].Release();
    }
  }
  for (i = MAX_BODY_PART_SECT * 2;
       i < MAX_BODY_PART_SECT * 2 + MAX_BODY_PART_SECT; i++) {
    if (m_pcResNode->CheckPartExist(i)) {
      m_pcResNode->GetFileName(i, m_nAction, m_nWeaponType, "", szBuffer,
                               sizeof(szBuffer));
      m_cNpcImage[i].SetSprFile(
          szBuffer,
          m_pcResNode->GetTotalFrames(i, m_nAction, m_nWeaponType, 16),
          m_pcResNode->GetTotalDirs(i, m_nAction, m_nWeaponType, 16),
          m_pcResNode->GetInterval(i, m_nAction, m_nWeaponType, 0));
    } else {
      m_cNpcImage[i].Release();
    }
  }
  for (i = MAX_BODY_PART_SECT * 3;
       i < MAX_BODY_PART_SECT * 3 + MAX_BODY_PART_SECT; i++) {
    if (m_pcResNode->CheckPartExist(i)) {
      m_pcResNode->GetFileName(i, m_nAction, m_nHorseType, "", szBuffer,
                               sizeof(szBuffer));
      m_cNpcImage[i].SetSprFile(
          szBuffer, m_pcResNode->GetTotalFrames(i, m_nAction, m_nHorseType, 16),
          m_pcResNode->GetTotalDirs(i, m_nAction, m_nHorseType, 16),
          m_pcResNode->GetInterval(i, m_nAction, m_nHorseType, 0));
    } else {
      m_cNpcImage[i].Release();
    }
  }
...
//	功能：	设定 npc 位置
//---------------------------------------------------------------------------
void KNpcRes::SetPos(int nNpcIdx, int x, int y, int z, BOOL bFocus,
                     BOOL bMenu) {
  m_nXpos = x;
  m_nYpos = y;
  m_nZpos = z;
  ...
  if (!bMenu) {
    ...
    g_ScenePlace.MoveObject(CGOG_NPC, nNpcIdx, x, y, z, m_SceneID,
                            IPOT_RL_OBJECT | IPOT_RL_INFRONTOF_ALL |
                                IPOT_RL_LIGHT_PROP);
  }
}
...
void KNpcRes::SetState(KList *pNpcStateList, KNpcResList *pNpcResList) {
  ...
  while (pNode) {
    ...
    if (nFind > 0)
      goto WHILE_END;
    szBuffer[0] = 0;
    pNpcResList->m_cStateTable.GetInfo(pNode->m_StateGraphics, szBuffer, &nType,
                                       &nPlayType, &nBackStart, &nBackEnd,
                                       &nTotalFrame, &nTotalDir, &nInterVal);
    if (!szBuffer[0])
      goto WHILE_END;

    if (nType < 0 || nType >= STATE_MAGIC_TYPE_NUM)
      goto WHILE_END;
    for (i = nType * 6; i < nType * 6 + 6; i++) {
      if (m_cStateSpr[i].m_nID == 0) {
        ...
        m_cStateSpr[i].m_SprContrul.SetSprFile(szBuffer, nTotalFrame, nTotalDir,
                                               nInterVal);
        break;
      }
    }
...
void KNpcRes::SetSpecialSpr(char *lpszSprName) {
  ...
  m_cSpecialSpr.SetSprFile(
      lpszSprName, sImage.nNumFrames, sImage.nNumFramesGroup,
      (sImage.nNumFrames / sImage.nNumFramesGroup) * sImage.nInterval / 50);
}
...
void KNpcRes::SetMenuStateSpr(int nMenuState) {
  if (nMenuState < PLAYER_MENU_STATE_NORMAL ||
      nMenuState >= PLAYER_MENU_STATE_NUM) {
  ...
  if (szName[0]) {
    ...
    if (sImage.nNumFrames < sImage.nNumFramesGroup)
      sImage.nNumFrames = sImage.nNumFramesGroup;
    m_cMenuStateSpr.SetSprFile(
        szName, sImage.nNumFrames, sImage.nNumFramesGroup,
        (sImage.nNumFrames / sImage.nNumFramesGroup) * sImage.nInterval / 50);
  } else {
    this->m_cMenuStateSpr.Release();
  }
...
//	功能：设定头顶状态
//---------------------------------------------------------------------------
void KNpcRes::SetMenuState(int nState, char *lpszSentence,
                           int nSentenceLength) {
...
//	功能：获得睡眠状态
//---------------------------------------------------------------------------
BOOL KNpcRes::GetSleepState() { return (m_nSleepState ? 1 : 0); }
...
int KNpcRes::GetNormalNpcStandDir(int nFrame) {
  ...
  int nTotalFrames = m_pcResNode->GetTotalFrames(NORMAL_NPC_PART_NO, cdo_stand,
                                                 m_nHelmType, 16);
  if (nTotalFrames <= 0)
    return 0;
...
void KNpcBlur::SetMapPos(int x, int y, int z, int nNpcIdx) {
  ...
  g_ScenePlace.MoveObject(CGOG_NPC_BLUR_DETAIL(m_nCurNo), nNpcIdx, x, y, z,
                          m_SceneID[m_nCurNo]);
}
...
//	功能：	设定当前某一项的内容
//---------------------------------------------------------------------------
void KNpcBlur::SetFile(int nNo, char *lpszFileName, int nSprID, int nFrameNo,
                       int nXpos, int nYpos, int nZpos) {
...
void KNpcBlur::Remove() {
  for (int i = 0; i < MAX_BLUR_FRAME; i++) {
    if (m_SceneID[i]) {
      g_ScenePlace.RemoveObject(CGOG_NPC_BLUR_DETAIL(i), m_SceneIDNpcIdx[i],
                                m_SceneID[i]);
      m_SceneID[i] = 0;
    }
  }
