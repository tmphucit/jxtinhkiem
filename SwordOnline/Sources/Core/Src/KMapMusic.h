...
struct KMapMusicInfo {
  ...
  char m_szFileName
      [FILE_NAME_LENGTH]; // 音乐文件名(含从游戏跟目录开始的完整游戏路径)
};
...
private:
  ...
  int m_nWeatherVolume; // 当前天气导致的音乐声音大小的变化(0(无声) --
                        // 100(正常))
...
public:
  ...
  void Start(int nMapID, int nGameTime,
             BOOL bFightMode);     // 进入游戏或者音乐设定打开的时候，播放音乐
