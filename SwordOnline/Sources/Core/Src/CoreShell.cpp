...
nResistMaxCS += Player[CLIENT_PLAYER_INDEX].m_cTask.GetSaveVal(139)*2;
	...
	//		Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].m_bIsHidePlayer = true;
		...
		Player[CLIENT_PLAYER_INDEX].PlayerSetAuto(pInfo->bStart,pInfo->nLife,pInfo->nMana,pInfo->nReturn,pInfo->nAtack,pInfo->nRange,pInfo->nNAtack,pInfo->bMoneyObj,pInfo->bItemObj,pInfo->bPropObj,pInfo->bItemSelect, pInfo->nNgaMyBuff,pInfo->nPhamViTuVe);
		}	
		break;
	...
	case GDI_ITEM_CHAT:			//tao ra itemchat
		...
		if(nParam)
			...
			ChatItem* pItem = (ChatItem*)nParam;
				...
				nRet = ItemSet.Add(pItem->nItemGenre, pItem->nSeries, pItem->nLevel,pItem->nLuck,
				pItem->nDetailType, pItem->nParticularType, nVirtualMA, 1, 0);
			if(nRet)
				{
     ...
     case GOI_BLOCK_ITEM:
		{
			...
			if (nIdx > 0 && nIdx < MAX_ITEM)
			{
				...
				{
					KSystemMessage	sMsg;
					sprintf(sMsg.szMessage, "Kh«ng thÓ dïng tÝnh n¨ng nµy víi vËt phÈm kh«ng ph¶i lµ trang bÞ.!");
					sMsg.eType = SMT_NORMAL;
					sMsg.byConfirmType = SMCT_NONE;
					sMsg.byPriority = 0;
					sMsg.byParamSize = 0;
...
/*					KSystemMessage	sMsg;
				...
				{
					KSystemMessage	sMsg;
					sprintf(sMsg.szMessage, "Kh«ng thÓ söa ch÷a th«ng th­êng. H·y ®Õn <color=green>BiÖn Kinh 214/193<color> gÆp <color=yellow>Thî RÌn CÊp Cao<color> ®Ó phôc håi");
					sMsg.eType = SMT_NORMAL;
					sMsg.byConfirmType = SMCT_NONE;
					sMsg.byPriority = 0;
					sMsg.byParamSize = 0;
