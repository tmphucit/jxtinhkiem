...
BOOL KSubWorldSet::Load(LPSTR szFileName) {
  ...
  for (int i = 0; i < nWorldCount; i++) {
    sprintf((char *)szKeyName, "World%02d",
            i); // %02 chi load dc 99 map %03 se load 999 map sao tui thay co 70
                // map la max roi
...
void KSubWorldSet::MainLoop() {
  ...
  //		SendClientCmdPing();
  // 游戏音乐的处理
  this->m_cMusic.Play(SubWorld[0].m_SubWorldID, SubWorld[0].m_dwCurrentTime,
...
BOOL KSubWorldSet::SendMessage(int nSubWorldID, DWORD dwMsgType, int nParam1,
...
#ifndef _SERVER
void KSubWorldSet::Paint() { SubWorld[0].Paint(); }
#endif

#ifdef _SERVER
void KSubWorldSet::GetRevivalPosFromId(DWORD dwSubWorldId, int nRevivalId,
                                       POINT *pPos) {
  if (!pPos)
    return;
