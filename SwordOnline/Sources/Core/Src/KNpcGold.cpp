...
void KNpcGold::RandChangeGold() {
  ...
  if (pInfo->m_dwSkill5ID && pInfo->m_szSkill5Level[0]) {
    Npc[m_nNpcIdx].m_SkillList.SetNpcSkill(
        defNPC_GOLD_SKILL_NO, pInfo->m_dwSkill5ID,
        KNpcTemplate::GetNpcLevelDataFromScript(g_pNpcLevelScript, "Level5",
                                                Npc[m_nNpcIdx].m_Level,
                                                pInfo->m_szSkill5Level));
  ...
  Npc[m_nNpcIdx].SendDataToNearRegion((LPVOID)&sSync,
                                      sizeof(NPC_GOLD_CHANGE_SYNC));
#endif
}
...
BOOL KNpcGoldTemplate::Init() {
  ...
  for (i = 0; i < defMAX_NPC_GOLD_TYEP; i++) {
    ...
    cFile.GetString(i + 2, 15, "", m_sInfo[i].m_szSkill5Level,
                    sizeof(m_sInfo[i].m_szSkill5Level));
