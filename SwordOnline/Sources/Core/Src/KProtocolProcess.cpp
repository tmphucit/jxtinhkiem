...
KProtocolProcess::KProtocolProcess() {
  ...
  ProcessFunc[s2c_playerpointsync] = s2cPlayerPointSync;
  // ProcessFunc[s2c_binditem] = s2cBindItem;
  ProcessFunc[s2c_autoplay] = s2cAutoPlaySync; // protocol aut
  ProcessFunc[s2c_timebox] = s2cTimeBox;
  ProcessFunc[s2c_exitgame] = s2c_ExitGame;
  ProcessFunc[s2c_imagenpc] = s2cImageNpcSync; // protocol quest finish
  ProcessFunc[s2c_teamchangestate] = s2cSetTeamState;
  ...
  ProcessFunc[c2s_teamapplyopenclose] =
      &KProtocolProcess::PlayerApplyTeamOpenClose;
  ...
  ProcessFunc[c2s_teamapplykickmember] =
      &KProtocolProcess::PlayerApplyTeamKickMember;
  ProcessFunc[c2s_teamapplychangecaptain] =
      &KProtocolProcess::PlayerApplyTeamChangeCaptain;
  ...
  ProcessFunc[c2s_playerapplyfactiondata] =
      &KProtocolProcess::PlayerApplyFactionData;
  ProcessFunc[c2s_playersendchat] = &KProtocolProcess::PlayerSendChat;
  ProcessFunc[c2s_playeraddbaseattribute] =
      &KProtocolProcess::PlayerAddBaseAttribute;
  ProcessFunc[c2s_playerapplyaddskillpoint] =
      &KProtocolProcess::PlayerApplyAddSkillPoint;
  ...
  ProcessFunc[c2s_chatapplyresendallfriendname] =
      &KProtocolProcess::ChatApplyReSendAllFriendName;
  ProcessFunc[c2s_chatapplysendonefriendname] =
      &KProtocolProcess::ChatApplySendOneFriendName;
  ...
  ProcessFunc[c2s_pkapplychangenormalflag] =
      &KProtocolProcess::c2sPKApplyChangeNormalFlag;
  ...
  ProcessFunc[c2s_playerloginpwstorebox] =
      &KProtocolProcess::PlayerLoginPWStoreBox;
...
void KProtocolProcess::ProcessNetMsg(BYTE *pMsg) {
  if (!pMsg || pMsg[0] <= s2c_clientbegin || pMsg[0] >= s2c_end ||
      ProcessFunc[pMsg[0]] == NULL) {
...
void KProtocolProcess::s2cChatScreenSingleError(BYTE *pMsg) {
  ...
  memcpy(szName, pError->m_szName,
         pError->m_wLength + 1 + sizeof(pError->m_szName) -
             sizeof(CHAT_SCREENSINGLE_ERROR_SYNC));
...
void KProtocolProcess::NetCommandJump(BYTE *pMsg) {
  ...
  if (Player[CLIENT_PLAYER_INDEX].ConformIdx(nIdx)) {
    Npc[nIdx].SendCommand(do_jump, pNetCommandJump->nMpsX,
                          pNetCommandJump->nMpsY);
    Npc[nIdx].m_SyncSignal = SubWorld[0].m_dwCurrentTime;
  }
}
...
void KProtocolProcess::PlayerUpdateShop(BYTE *pMsg) {
  ...
  if (nIdx > 0 && nIdx != Player[CLIENT_PLAYER_INDEX].m_nIndex) {
    ...
    if (pSync->szName[0]) {
      memcpy(Npc[nIdx].m_szNameOpenShop, pSync->szName,
             sizeof(Npc[nIdx].m_szNameOpenShop));
    }
  }
}
...
void KProtocolProcess::SysNcPosMin(BYTE *pMsg) {
  ...
  if (nIdx) {
    ...
    SubWorld[0].Mps2Map(NpcSync->MapX, NpcSync->MapY, &nRegion, &nMapX, &nMapY,
                        &nOffX, &nOffY);

    Npc[nIdx].m_sSyncPos.m_dwRegionID =
        SubWorld[0].m_Region[nRegion].m_RegionID;
    ...
    if (Npc[nIdx].m_RegionIndex == -1) {
      if (nRegion == -1) {
        ...
        SubWorld[0].m_Region[nRegion].AddRef(Npc[nIdx].m_MapX, Npc[nIdx].m_MapY,
                                             obj_npc);
      }
...
void KProtocolProcess::SysItemBuyShop(BYTE *pMsg) {
  ...
  int nIndex = ItemSet.Add(pSync->m_Genre, pSync->m_Series, pSync->m_Level,
                           pSync->m_Luck, pSync->m_Detail, pSync->m_Particur,
                           pnMagicParam, pSync->m_Version, pSync->m_RandomSeed);
  ...
  if (nIndex > 0) {
    ...
    Player[CLIENT_PLAYER_INDEX].m_ItemList.Add(nIndex, pSync->m_btPlace,
                                               pSync->m_btX, pSync->m_btY);
  }
}
...
void KProtocolProcess::PlayerUpdateTong(BYTE *pMsg) {
  ...
  if (nIdx > 0 && nIdx != Player[CLIENT_PLAYER_INDEX].m_nIndex) {
    ...
    if (pSync->m_TongName[0]) {
      memcpy(Npc[nIdx].m_TongName, pSync->m_TongName,
             sizeof(pSync->m_TongName));
    }

    memset(Npc[nIdx].m_TongRank, 0, sizeof(Npc[nIdx].m_TongRank));

    if (pSync->m_TongRank[0]) {
      memcpy(Npc[nIdx].m_TongRank, pSync->m_TongRank,
             sizeof(pSync->m_TongRank));
    }
  }
}
...
void KProtocolProcess::PlayerOpenStringBox(BYTE *pMsg) {
  ...
  CoreDataChanged(GDCNI_OPEN_STRING_BOX, (unsigned int)pSync->m_ID,
                  (unsigned int)&pSync->m_Title);
}

void KProtocolProcess::PlayerOpenCheckItemBox(BYTE *pMsg) {
  PLAYER_OPEN_CHECK_ITEM_BOX *pSync = (PLAYER_OPEN_CHECK_ITEM_BOX *)pMsg;

  CoreDataChanged(GDCNI_OPEN_CHECK_ITEM_BOX, (unsigned int)&pSync->m_TitleBox,
                  (unsigned int)&pSync->m_TitleTex);
}
...
void KProtocolProcess::PlayerSendTopTKNew(BYTE *pMsg) {
  ...
  g_StrCpyLen(pDataTop.m_szTop1Name, pSync->m_DataTop.m_szTop1Name,
              sizeof(pDataTop.m_szTop1Name));
  ...
  g_StrCpyLen(pDataTop.m_szTop2Name, pSync->m_DataTop.m_szTop2Name,
              sizeof(pDataTop.m_szTop2Name));
  ...
  g_StrCpyLen(pDataTop.m_szTop3Name, pSync->m_DataTop.m_szTop3Name,
              sizeof(pDataTop.m_szTop3Name));
  ...
  g_StrCpyLen(pDataTop.m_szTop4Name, pSync->m_DataTop.m_szTop4Name,
              sizeof(pDataTop.m_szTop4Name));
  ...
  g_StrCpyLen(pDataTop.m_szTop5Name, pSync->m_DataTop.m_szTop5Name,
              sizeof(pDataTop.m_szTop5Name));
  ...
  g_StrCpyLen(pDataTop.m_szTop6Name, pSync->m_DataTop.m_szTop6Name,
              sizeof(pDataTop.m_szTop6Name));
  ...
  g_StrCpyLen(pDataTop.m_szTop7Name, pSync->m_DataTop.m_szTop7Name,
              sizeof(pDataTop.m_szTop7Name));
  ...
  g_StrCpyLen(pDataTop.m_szTop8Name, pSync->m_DataTop.m_szTop8Name,
              sizeof(pDataTop.m_szTop8Name));
  ...
  g_StrCpyLen(pDataTop.m_szTop9Name, pSync->m_DataTop.m_szTop9Name,
              sizeof(pDataTop.m_szTop9Name));
  ...
  g_StrCpyLen(pDataTop.m_szTop10Name, pSync->m_DataTop.m_szTop10Name,
              sizeof(pDataTop.m_szTop10Name));
...
void KProtocolProcess::PlayerTKSync(BYTE *pMsg) {
  ...
  Player[CLIENT_PLAYER_INDEX].m_cTask.SetSaveVal(pInfo->nValueIndex,
                                                 pInfo->nValue);
  // if(pInfo->bCheck)
...
void KProtocolProcess::S2CSyncItemParam(BYTE *pMsg) {
  ...
  if (nIdx > 0 && nIdx < MAX_ITEM) {
    Item[nIdx].SetTime(pInfo->m_Time.bYear, pInfo->m_Time.bMonth,
                       pInfo->m_Time.bDay, pInfo->m_Time.bHour);
  }
}
...
void KProtocolProcess::NetCommandSkill(BYTE *pMsg) {
  ...
  // µ±Ö¸¶¨Ä³¸öÄ¿±êÊ±(MapX ==
  // -1),MapYÎªÄ¿±êµÄNpcdwID£¬ÐèÒª×ª»»³É±¾µØµÄNpcIndex²ÅÐÐ
  if (MapX < 0) {
    if (MapX != -1)
      return;
...
void KProtocolProcess::s2cApplyCreateTeamFalse(BYTE *pMsg) {
  PLAYER_SEND_CREATE_TEAM_FALSE *pCreateFalse =
      (PLAYER_SEND_CREATE_TEAM_FALSE *)pMsg;
  KSystemMessage sMsg;
...
void KProtocolProcess::s2cChatAddFriend(BYTE *pMsg) {
  ...
  Player[CLIENT_PLAYER_INDEX].m_cChat.AddOne(pFriend->m_dwID, pFriend->m_nIdx,
                                             pFriend->m_szName, 0);
  Player[CLIENT_PLAYER_INDEX].m_cChat.SaveTeamInfo(
...
void KProtocolProcess::s2cChatAddFriendFail(BYTE *pMsg) {
  ...
  pList = (CChatApplyListNode *)Player[CLIENT_PLAYER_INDEX]
              .m_cChat.m_cApplyAddList.GetHead();
  while (pList) {
...
void KProtocolProcess::s2cChatAddOneFriend(BYTE *pMsg) {
  ...
  nTeamNo = Player[CLIENT_PLAYER_INDEX].m_cChat.CheckTeamNo(
      Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].Name, pData->m_szName);
  if (nTeamNo < 0)
    nTeamNo = 0;
  Player[CLIENT_PLAYER_INDEX].m_cChat.AddOne(pData->m_dwID, pData->m_nPlayerIdx,
                                             pData->m_szName, nTeamNo);
  Player[CLIENT_PLAYER_INDEX].m_cChat.SaveTeamInfo(
      Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].Name);
...
void KProtocolProcess::s2cChatBeRefusedAddFriend(BYTE *pMsg) {
  ...
  memset(szName, 0, sizeof(szName));
  memcpy(szName, pRefuse->m_szName,
         pRefuse->m_wLength + 1 + sizeof(pRefuse->m_szName) -
             sizeof(CHAT_REFUSE_FRIEND_SYNC));
...
void KProtocolProcess::s2cChatFriendOnline(BYTE *pMsg) {
  ...
  Player[CLIENT_PLAYER_INDEX].ChatFriendOnLine(pFriend->m_dwID,
                                               pFriend->m_nPlayerIdx);
}
...
void KProtocolProcess::s2cChatGetApplyAddFriend(BYTE *pMsg) {
  ...
  if (Player[CLIENT_PLAYER_INDEX].m_cChat.CheckIsFriend(
          pApply->m_nSrcPlayerIdx))
    return;
...
void KProtocolProcess::s2cChatLoginFriendName(BYTE *pMsg) {
  ...
  memset(szName, 0, sizeof(szName));
  memcpy(szName, pFriend->m_szName,
         pFriend->m_wLength + 1 + sizeof(pFriend->m_szName) -
             sizeof(CHAT_LOGIN_FRIEND_NAME_SYNC));
  nTeamNo = Player[CLIENT_PLAYER_INDEX].m_cChat.CheckTeamNo(
      Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].Name, szName);
  if (nTeamNo < 0)
    nTeamNo = 0;
  Player[CLIENT_PLAYER_INDEX].m_cChat.AddOne(
      pFriend->m_dwID, pFriend->m_nPlayerIdx, szName, nTeamNo);
}
...
void KProtocolProcess::s2cChatLoginFriendNoName(BYTE *pMsg) {
  CHAT_LOGIN_FRIEND_NONAME_SYNC *pFriend =
      (CHAT_LOGIN_FRIEND_NONAME_SYNC *)pMsg;
  char szName[32];
  int nTeamNo;

  szName[0] = 0;
  nTeamNo = Player[CLIENT_PLAYER_INDEX].m_cChat.CheckTeamNo(
      Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].Name, NULL);
  if (nTeamNo < 0) {
    if (Player[CLIENT_PLAYER_INDEX].m_cChat.m_nLoginGetFirstOneFriendFlag ==
        0) // ÉêÇëÖØÐÂ·¢ËÍËùÓÐºÃÓÑÊý¾Ý£¬´øÃû×Ö
    {
      ...
      if (g_pClient)
        g_pClient->SendPackToServer(
            (BYTE *)&sApply, sizeof(CHAT_APPLY_RESEND_ALL_FRIEND_NAME_COMMAND));
    } else // ÉêÇëÖØÐÂ·¢ËÍÕâ¸öºÃÓÑÊý¾Ý£¬´øÃû×Ö
    {
      ...
      if (g_pClient)
        g_pClient->SendPackToServer(
            (BYTE *)&sApply, sizeof(CHAT_APPLY_SEND_ONE_FRIEND_NAME_COMMAND));
    }
    return;
  }
  Player[CLIENT_PLAYER_INDEX].m_cChat.AddOne(
...
void KProtocolProcess::s2cCreateTeam(BYTE *pMsg) {
  PLAYER_SEND_CREATE_TEAM_SUCCESS *pCreateSuccess =
      (PLAYER_SEND_CREATE_TEAM_SUCCESS *)pMsg;

  g_Team[0].CreateTeam(Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].m_dwID,
                       Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].Name,
                       Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].m_Level,
                       pCreateSuccess->nTeamServerID);
  ...
  Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].SetInstantSpr(
...
void KProtocolProcess::s2cFactionSkillOpen(BYTE *pMsg) {
  ...
  if (Player[CLIENT_PLAYER_INDEX].m_cFaction.m_nCurFaction < 0 ||
      Player[CLIENT_PLAYER_INDEX].m_cFaction.m_nCurFaction !=
          pSkill->m_btCurFactionID) {
    // ÉêÇë¸üÐÂÃÅÅÉÐÅÏ¢
  ...
  //	{
  //		if (Player[CLIENT_PLAYER_INDEX].m_cFaction.m_sSkillOpen[i].m_nID
  //== Player[CLIENT_PLAYER_INDEX].m_cFaction.m_nCurFaction)
  //		{
...
void KProtocolProcess::s2cGetLeadExp(BYTE *pMsg) {
  ...
  Player[CLIENT_PLAYER_INDEX].m_dwLeadLevel =
      PlayerSet.m_cLeadExp.GetLevel(Player[CLIENT_PLAYER_INDEX].m_dwLeadExp,
                                    Player[CLIENT_PLAYER_INDEX].m_dwLeadLevel);
  Player[CLIENT_PLAYER_INDEX].m_dwNextLevelLeadExp =
      PlayerSet.m_cLeadExp.GetLevelExp(
          Player[CLIENT_PLAYER_INDEX].m_dwLeadLevel);

  if (dwLevel < Player[CLIENT_PLAYER_INDEX].m_dwLeadLevel) {
    KSystemMessage sMsg;
    sprintf(sMsg.szMessage, MSG_LEADER_LEVEL_UP,
            Player[CLIENT_PLAYER_INDEX].m_dwLeadLevel);
...
void KProtocolProcess::s2cGetSkillLevel(BYTE *pMsg) {
  ...
  if (pSkill->m_nSkillLevel > MAX_SKILLLEVEL)
    return;
  nSkillIndex = Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].m_SkillList.FindSame(
      pSkill->m_nSkillID);
  ...
  if (!nSkillIndex) {
    ...
    if (pSkill->m_nSkillLevel == 0) {
      if (Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].m_SkillList.Add(
              pSkill->m_nSkillID, 0) == 0)
        return;
    } else {
      if (Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].m_SkillList.Add(
              pSkill->m_nSkillID, pSkill->m_nSkillLevel) == 0)
        return;
    }
  } else {
    Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].m_SkillList.SetLevel(
        nSkillIndex, pSkill->m_nSkillLevel);
  }

  // Í¨Öª½çÃæ¸üÐÂ¼¼ÄÜÊý¾Ý
  CoreDataChanged(GDCNI_FIGHT_SKILL_POINT, 0,
                  Player[CLIENT_PLAYER_INDEX].m_nSkillPoint);
  ...
  SkillData.nLevel =
      Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].m_SkillList.GetLevel(
          pSkill->m_nSkillID);
  SkillData.nCurrentLevel =
      Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].m_SkillList.GetCurrentLevel(
          pSkill->m_nSkillID);

  int nSkillPosition =
      Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].m_SkillList.GetSkillPosition(
          pSkill->m_nSkillID);
  if (nSkillPosition < 0)
    return;
...
void KProtocolProcess::s2cLeaveTeam(BYTE *pMsg) {
  ...
  if (pLeaveTeam->m_dwNpcID ==
    ...
    if (Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].GetMenuState() ==
      ...
      Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].SetMenuState(
...
void KProtocolProcess::s2cSetFactionData(BYTE *pMsg) {
  ...
  Player[CLIENT_PLAYER_INDEX].m_cFaction.m_nCurFaction =
      (char)pData->m_btCurFaction;
  Player[CLIENT_PLAYER_INDEX].m_cFaction.m_nFirstAddFaction =
      (char)pData->m_btFirstFaction;
...
void KProtocolProcess::s2cSyncItem(BYTE *pMsg) {
  ...
  int nIndex = ItemSet.Add(
      pItemSync->m_Genre, pItemSync->m_Series, pItemSync->m_Level,
      pItemSync->m_Luck, pItemSync->m_Detail, pItemSync->m_Particur,
      pnMagicParam, pItemSync->m_Version, pItemSync->m_RandomSeed,
      pItemSync->m_Time.bYear, pItemSync->m_Time.bMonth, pItemSync->m_Time.bDay,
  ...
  if (nIndex > 0) {
    ...
    Player[CLIENT_PLAYER_INDEX].m_ItemList.Add(
        nIndex, pItemSync->m_btPlace, pItemSync->m_btX, pItemSync->m_btY);
  }
...
void KProtocolProcess::s2cTeamChangeCaptain(BYTE *pMsg) {
  ...
  // ×Ô¼º±»ÈÎÃüÎª¶Ó³¤
  if (Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].m_dwID ==
      pChange->m_dwCaptainID) {
    KSystemMessage sMsg;
    sprintf(sMsg.szMessage, MSG_TEAM_CHANGE_CAPTAIN_SELF,
            g_Team[0].m_szMemName[0]);
    sMsg.eType = SMT_TEAM;
  ...
  {
    KSystemMessage sMsg;
    sprintf(sMsg.szMessage, MSG_TEAM_CHANGE_CAPTAIN,
            g_Team[0].m_szMemName[nMemNo + 1]);
    sMsg.eType = SMT_TEAM;
    sMsg.byConfirmType = SMCT_UI_TEAM;
    sMsg.byPriority = 3;
    sMsg.byParamSize = 0;
  ...
  if (Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].m_dwID ==
    ...
    Player[CLIENT_PLAYER_INDEX].m_cTeam.m_nFigure = TEAM_CAPTAIN;
    Player[CLIENT_PLAYER_INDEX].m_cTeam.ReleaseList();
  } else if (Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].m_dwID ==
             pChange->m_dwMemberID) {
...
void KProtocolProcess::SyncCurPlayerHeight(BYTE *pMsg) {
  ...
  if (Player[CLIENT_PLAYER_INDEX].m_cTask.GetSaveVal(139) == 1) {
    ...
    if (nIndex > 0)
      Player[CLIENT_PLAYER_INDEX].m_nNextLevelExp =
          PlayerSet.m_cLevelAdd.GetLevelExp1(Npc[nIndex].m_Level);
  } else if (Player[CLIENT_PLAYER_INDEX].m_cTask.GetSaveVal(139) == 2) {
    int nIndex = Player[CLIENT_PLAYER_INDEX].m_nIndex;
    if (nIndex > 0)
      Player[CLIENT_PLAYER_INDEX].m_nNextLevelExp =
          PlayerSet.m_cLevelAdd.GetLevelExp2(Npc[nIndex].m_Level);
  }
    ...
    if (nIndex > 0)
      Player[CLIENT_PLAYER_INDEX].m_nNextLevelExp =
          PlayerSet.m_cLevelAdd.GetLevelExp3(Npc[nIndex].m_Level);
  } else if (Player[CLIENT_PLAYER_INDEX].m_cTask.GetSaveVal(139) == 4) {
    int nIndex = Player[CLIENT_PLAYER_INDEX].m_nIndex;
    if (nIndex > 0)
      Player[CLIENT_PLAYER_INDEX].m_nNextLevelExp =
          PlayerSet.m_cLevelAdd.GetLevelExp4(Npc[nIndex].m_Level);
  } else if (Player[CLIENT_PLAYER_INDEX].m_cTask.GetSaveVal(139) == 5) {
    int nIndex = Player[CLIENT_PLAYER_INDEX].m_nIndex;
    if (nIndex > 0)
      Player[CLIENT_PLAYER_INDEX].m_nNextLevelExp =
          PlayerSet.m_cLevelAdd.GetLevelExp5(Npc[nIndex].m_Level);
  }
    ...
    if (nIndex > 0)
      Player[CLIENT_PLAYER_INDEX].m_nNextLevelExp =
          PlayerSet.m_cLevelAdd.GetLevelExp(Npc[nIndex].m_Level);
  }
}
...
void KProtocolProcess::SyncCurNormalData(BYTE *pMsg) {
  ...
  Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].m_SyncSignal =
...
void KProtocolProcess::SyncNpc(BYTE *pMsg) {
    ...
    if (Npc[nIdx].m_RegionIndex >= 0) {
      if (Npc[nIdx].m_RegionIndex != nRegion) {
        SubWorld[0].NpcChangeRegion(
            SubWorld[0].m_Region[Npc[nIdx].m_RegionIndex].m_RegionID,
            SubWorld[0].m_Region[nRegion].m_RegionID, nIdx);
        Npc[nIdx].m_RegionIndex = nRegion;
      }
      SubWorld[0].m_Region[Npc[nIdx].m_RegionIndex].AddRef(
          Npc[nIdx].m_MapX, Npc[nIdx].m_MapY, obj_npc);
    } else {
      SubWorld[0].m_Region[nRegion].AddNpc(nIdx);
      Npc[nIdx].m_RegionIndex = nRegion;
  ...
  if (NpcSync->CurrentLife > 0 &&
      NpcSync->CurrentLifeMax >= NpcSync->CurrentLife) {
  ...
  if (Npc[nIdx].m_Doing != do_death ||
      Npc[nIdx].m_Doing != do_revive) // need check later -- spe 03/05/27
    Npc[nIdx].SendCommand((NPCCMD)NpcSync->m_Doing, NpcSync->MapX,
                          NpcSync->MapY);

  Npc[nIdx].m_SyncSignal = SubWorld[0].m_dwCurrentTime;
  Npc[nIdx].SetMenuState(NpcSync->m_btMenuState);
  memset(Npc[nIdx].Name, 0, sizeof(Npc[nIdx].Name));
  memcpy(Npc[nIdx].Name, NpcSync->m_szName,
         NpcSync->m_wLength -
             (sizeof(NPC_SYNC) - 1 - sizeof(NpcSync->m_szName)));
}
    ...
    if (Npc[nIdx].m_RegionIndex == -1 &&
      ...
      if (Npc[nIdx].m_RegionIndex != nRegion &&
        ...
        if (nRegion >= 0) {
          SubWorld[0].m_Region[nRegion].AddNpc(nIdx);
          Npc[nIdx].m_dwRegionID = SubWorld[0].m_Region[nRegion].m_RegionID;
          SubWorld[0].m_Region[nRegion].AddRef(Npc[nIdx].m_MapX,
                                               Npc[nIdx].m_MapY, obj_npc);
        }
      }
    }
...
void KProtocolProcess::SyncNpcMinPlayer(BYTE *pMsg) {
  ...
  SubWorld[0].Mps2Map(pSync->m_dwMapX, pSync->m_dwMapY, &nRegion, &nMapX,
                      &nMapY, &nOffX, &nOffY);
  ...
  if (Npc[nNpcIdx].m_RegionIndex == -1) {
    ...
    SubWorld[0].NpcChangeRegion(-1, SubWorld[0].m_Region[nRegion].m_RegionID,
                                nNpcIdx);
  ...
  if (nRegion == -1) {
    SubWorld[0].m_Region[Npc[nNpcIdx].m_RegionIndex].DecRef(
        Npc[nNpcIdx].m_MapX, Npc[nNpcIdx].m_MapY, obj_npc);

    int nRegionX = pSync->m_dwMapX /
                   (SubWorld[0].m_nCellWidth * SubWorld[0].m_nRegionWidth);
    int nRegionY = pSync->m_dwMapY /
                   (SubWorld[0].m_nCellHeight * SubWorld[0].m_nRegionHeight);
    ...
    SubWorld[0].m_Region[Npc[nNpcIdx].m_RegionIndex].AddRef(
        Npc[nNpcIdx].m_MapX, Npc[nNpcIdx].m_MapY, obj_npc);
    memset(&Npc[nNpcIdx].m_sSyncPos, 0, sizeof(Npc[nNpcIdx].m_sSyncPos));
    Npc[nNpcIdx].m_SyncSignal = SubWorld[0].m_dwCurrentTime;
  ...
  BYTE byBarrier = SubWorld[0].m_Region[Npc[nNpcIdx].m_RegionIndex].GetBarrier(
      Npc[nNpcIdx].m_MapX, Npc[nNpcIdx].m_MapY, Npc[nNpcIdx].m_OffX,
      Npc[nNpcIdx].m_OffY);
...
void KProtocolProcess::SyncObjectAdd(BYTE *pMsg) {
  ...
  sInfo.m_dwNpcId =
      pObjSyncAdd->m_dwNpcId; // them nhat trang bi loc xong ko nhat lai
  sInfo.m_nMovieFlag = ((pObjSyncAdd->m_btFlag & 0x02) > 0 ? 1 : 0);
  sInfo.m_nSoundFlag = ((pObjSyncAdd->m_btFlag & 0x01) > 0 ? 1 : 0);
  //   sInfo.m_dwNpcId = ((pObjSyncAdd->m_btFlag & 0x04) > 0 ? 1 : 0);
  memset(sInfo.m_szName, 0, sizeof(sInfo.m_szName));
  memcpy(sInfo.m_szName, pObjSyncAdd->m_szName,
         pObjSyncAdd->m_wLength + 1 + sizeof(pObjSyncAdd->m_szName) -
             sizeof(OBJ_ADD_SYNC));

  nObjIndex = ObjSet.ClientAdd(pObjSyncAdd->m_nID, pObjSyncAdd->m_nDataID,
                               pObjSyncAdd->m_btState, pObjSyncAdd->m_btDir,
                               pObjSyncAdd->m_wCurFrame, pObjSyncAdd->m_nXpos,
                               pObjSyncAdd->m_nYpos, sInfo);
  ...
  if (Object[nObjIndex].m_nKind == Obj_Kind_Money) {
    ...
    if (FALSE == Player[CLIENT_PLAYER_INDEX].m_ItemList.SearchPosition(
                     ...
                     Object[nObjIndex].m_nItemHeight, &sItemPos))
      return;
    sPickUp.ProtocolType = c2s_playerpickupitem;
    sPickUp.m_nObjID = Object[nObjIndex].m_nID;
...
void KProtocolProcess::SyncObjectDir(BYTE *pMsg) {
  ...
  if (nObjIndex <= 0) {
    ...
    if (g_pClient)
      g_pClient->SendPackToServer(&sObjClientSyncAdd,
                                  sizeof(sObjClientSyncAdd));
  } else { // Í¬²½·½Ïò
...
void KProtocolProcess::SyncPlayer(BYTE *pMsg) {
  ...
  if (Npc[nIdx].m_HorseType == -1) {
    ...
    if (nIdx == Player[CLIENT_PLAYER_INDEX].m_nIndex) {
      if (Npc[nIdx].m_bRideHorse == TRUE) {
        int ItemActiveIdx =
            Player[CLIENT_PLAYER_INDEX].m_ItemList.GetEquipment(itempart_horse);
        if (ItemActiveIdx != 0) {
          int ActiveMagic =
              Player[CLIENT_PLAYER_INDEX].m_ItemList.GetEquipEnhanceP(
                  itempart_horse);
  ...
  if (pPlaySync->m_TongName[0]) {
    memcpy(Npc[nIdx].m_TongName, pPlaySync->m_TongName,
           sizeof(pPlaySync->m_TongName));
  }

  memset(Npc[nIdx].m_TongRank, 0, sizeof(Npc[nIdx].m_TongRank));

  if (pPlaySync->m_TongRank[0]) {
    memcpy(Npc[nIdx].m_TongRank, pPlaySync->m_TongRank,
           sizeof(pPlaySync->m_TongRank));
  }
  ...
  if (pPlaySync->szName[0]) {
    memcpy(Npc[nIdx].m_szNameOpenShop, pPlaySync->szName,
           sizeof(Npc[nIdx].m_szNameOpenShop));
  }
}
...
void KProtocolProcess::SyncWorld(BYTE *pMsg) {
  ...
  if (SubWorld[0].m_SubWorldID != WorldSync->SubWorld) {
    ...
    /*		SubWorld[0].m_Region[0].Load(WorldSync->SubWorld,
       WorldSync->Region);
                    SubWorld[0].m_Region[0].Init(SubWorld[0].m_nRegionWidth,
       SubWorld[0].m_nRegionHeight); for (int i = 0; i < 8; i++)
                    {
                            if (SubWorld[0].m_Region[0].m_nConRegionID[i] == -1)
                                    continue;
                            SubWorld[0].m_Region[i+1].Load(WorldSync->SubWorld,
       SubWorld[0].m_Region[0].m_nConRegionID[i]);
                            SubWorld[0].m_Region[i+1].Init(SubWorld[0].m_nRegionWidth,
       SubWorld[0].m_nRegionHeight);
                    }*/
  }
  ...
  for (int i = 0; i < MAX_NPCSKILL; i++) {
    if (Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex]
            .m_SkillList.m_Skills[i]
            .SkillId > 0) {
      Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].m_SkillList.SetBeginCastTime(
          Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex]
              .m_SkillList.m_Skills[i]
              .SkillId,
          SubWorld[0].m_dwCurrentTime);
      Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].m_SkillList.SetNextCastTime(
          Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex]
              .m_SkillList.m_Skills[i]
              .SkillId,
          SubWorld[0].m_dwCurrentTime);
  ...
  memcpy(SubWorld[0].m_szTongName, WorldSync->TongName,
         sizeof(SubWorld[0].m_szTongName));
  memcpy(SubWorld[0].m_szTongNameBC, WorldSync->TongNameBC,
         sizeof(SubWorld[0].m_szTongNameBC));
...
void KProtocolProcess::s2cChatDeleteFriend(BYTE *pMsg) {
  ...
  for (int i = 0; i < MAX_FRIEND_TEAM; i++) {
    pFriend = (CChatFriend *)Player[CLIENT_PLAYER_INDEX]
                  .m_cChat.m_cFriendTeam[i]
                  .m_cEveryOne.GetHead();
    while (pFriend) {
...
void KProtocolProcess::s2cSyncAllSkill(BYTE *pMsg) {
  ...
  int nSkillCount =
      (pSync->m_wProtocolLong - 2) / sizeof(SKILL_SEND_ALL_SYNC_DATA);
  ...
  for (int i = 0; i < nSkillCount; i++) {
    if (pSync->m_sAllSkill[i].SkillId) {
      ...
      Npc[nNpcIndex].m_SkillList.Add(pSync->m_sAllSkill[i].SkillId,
                                     pSync->m_sAllSkill[i].SkillLevel, 0);
    }
  }
}
...
void KProtocolProcess::SyncEnd(BYTE *pMsg) {
  ...
  // ¿ªÊ¼²¥ÒôÀÖ
  g_SubWorldSet.m_cMusic.Start(
      SubWorld[0].m_SubWorldID, SubWorld[0].m_dwCurrentTime,
      Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].m_FightMode);
...
void KProtocolProcess::s2cNpcSetMenuState(BYTE *pMsg) {
  ...
  if (pState->m_wLength >
      sizeof(NPC_SET_MENU_STATE_SYNC) - 1 - sizeof(pState->m_szSentence))
    Npc[nNpc].SetMenuState(pState->m_btState, pState->m_szSentence,
                           pState->m_wLength -
                               (sizeof(NPC_SET_MENU_STATE_SYNC) - 1 -
                                sizeof(pState->m_szSentence)));
  else
...
void KProtocolProcess::s2cTeamInviteAdd(BYTE *pMsg) {
  Player[CLIENT_PLAYER_INDEX].m_cTeam.ReceiveInvite(
      (TEAM_INVITE_ADD_SYNC *)pMsg);
}

void KProtocolProcess::s2cTradePressOkSync(BYTE *pMsg) {
  TRADE_STATE_SYNC *pPress = (TRADE_STATE_SYNC *)pMsg;
  Player[CLIENT_PLAYER_INDEX].m_cTrade.SetTradeState(
      pPress->m_btSelfLock, pPress->m_btDestLock, pPress->m_btSelfOk,
      pPress->m_btDestOk);
}
...
void KProtocolProcess::s2cDirectlyCastSkill(BYTE *pMsg) {
  ...
  if (MapX == -1) {
    ...
    nX = Npc[MapY].m_MapX;
    nY = Npc[MapY].m_MapY;
    SubWorld[0].Map2Mps(Npc[MapY].m_RegionIndex, nX, nY, 0, 0, (int *)&MapX,
                        (int *)&MapY);
  }
...
void KProtocolProcess::s2cShowMsg(BYTE *pMsg) {
  ...
  case enumMSG_ID_TEAM_KICK_One: {
    ...
    memset(szName, 0, sizeof(szName));
    memcpy(szName, pMsg + sizeof(SHOW_MSG_SYNC) - sizeof(LPVOID),
           pShowMsg->m_wLength + 1 + sizeof(LPVOID) - sizeof(SHOW_MSG_SYNC));
  ...
  case enumMSG_ID_TEAM_DISMISS: {
    ...
    if (Player[CLIENT_PLAYER_INDEX].m_cTeam.m_nFigure == TEAM_CAPTAIN) {
      ...
      sprintf(sMsg.szMessage, MSG_TEAM_DISMISS_MEMBER,
              g_Team[0].m_szMemName[0]);
    }
    sMsg.eType = SMT_TEAM;
    sMsg.byConfirmType = SMCT_CLICK;
  ...
  case enumMSG_ID_TEAM_LEAVE: {
    ...
    if (Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].m_dwID == dwID) {
      sprintf(sMsg.szMessage, MSG_TEAM_LEAVE_SELF_MSG,
              g_Team[0].m_szMemName[0]);
    } else {
  ...
  case enumMSG_ID_TEAM_REFUSE_INVITE: {
    char szName[32];
    memset(szName, 0, sizeof(szName));
    memcpy(szName, &pShowMsg->m_lpBuf,
           pShowMsg->m_wLength + 1 + sizeof(LPVOID) - sizeof(SHOW_MSG_SYNC));
  ...
  case enumMSG_ID_TEAM_CHANGE_CAPTAIN_FAIL: {
    ...
    CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&sMsg, 0);
    sprintf(sMsg.szMessage, MSG_TEAM_CHANGE_CAPTAIN_FAIL2,
            g_Team[0].m_szMemName[nMember + 1]);
    CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&sMsg, 0);
  } break;
  ...
  case enumMSG_ID_TRADE_DEST_ROOM_FULL: {
    KSystemMessage sMsg;
    sprintf(sMsg.szMessage, MSG_TRADE_DEST_ROOM_FULL,
            Player[CLIENT_PLAYER_INDEX].m_cTrade.m_szDestName);
    sMsg.eType = SMT_NORMAL;
    sMsg.byConfirmType = SMCT_NONE;
    sMsg.byPriority = 0;
    sMsg.byParamSize = 0;
  ...
  case enumMSG_ID_BLOCK_PK: {
    KSystemMessage sMsg;
    sMsg.eType = SMT_NORMAL;
    sMsg.byConfirmType = SMCT_NONE;
    sMsg.byPriority = 0;
    sMsg.byParamSize = 0;
    sprintf(sMsg.szMessage, MSG_NEW_BLOCK);
    CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&sMsg, 0);
  } break;
  case enumMSG_ID_CHANGE_STATE_PK10: {
    KSystemMessage sMsg;
    sMsg.eType = SMT_NORMAL;
    sMsg.byConfirmType = SMCT_NONE;
    sMsg.byPriority = 0;
    sMsg.byParamSize = 0;
    sprintf(sMsg.szMessage, MSG_NEW_CHANGESTATEPK10);
    CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&sMsg, 0);
  } break;
...
void KProtocolProcess::SyncStateEffect(BYTE *pMsg) {
  ...
  Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].SetStateSkillEffect(
      Player[CLIENT_PLAYER_INDEX].m_nIndex, pSync->m_dwSkillID, pSync->m_nLevel,
      pSync->m_MagicAttrib, nDataNum, pSync->m_nTime);
}
...
void KProtocolProcess::s2cExtend(BYTE *pMsg) {
  EXTEND_HEADER *pHeader =
      (EXTEND_HEADER *)(pMsg + sizeof(tagExtendProtoHeader));

  if (pHeader->ProtocolFamily == pf_playercommunity) {
    if (pHeader->ProtocolID == playercomm_s2c_notifychannelid) {
      PLAYERCOMM_NOTIFYCHANNELID *pNChann =
          (PLAYERCOMM_NOTIFYCHANNELID *)pHeader;

      l_pDataChangedNotifyFunc->NotifyChannelID(
          pNChann->channel, pNChann->channelid, pNChann->cost);
    }
  }
}
...
void KProtocolProcess::s2cExtendChat(BYTE *pMsg) {
  ...
  if (protocol == chat_someonechat) {
    ...
    l_pDataChangedNotifyFunc->ChannelMessageArrival(
        pCccSync->figure, pCccSync->channelid, pCccSync->someone,
    ...
    if (*pChannelid == -1) { // someone
      ...
      void *pSent = pSentlen + 1;
      l_pDataChangedNotifyFunc->ChannelMessageArrival(
          0, *pChannelid, Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].Name,
          (const char *)pSent, *pSentlen, pCfb->code != codeFail);
    }
    ////X
  }
...
static BOOL sParseUGName(const std::string &name, std::string *pUnit,
                         std::string *pGroup) {
...
void KProtocolProcess::s2cExtendFriend(BYTE *pMsg) {
  ...
  if (pHeader->ProtocolFamily == pf_tong) {
        ...
        on_newgroup:
          ...
          for (char *pRoleTag = pGroup + strlen(pGroup) + 1;;) {
            if (*pRoleTag == specOver)
              ...
              char *pRole = pState + 1;
              l_pDataChangedNotifyFunc->FriendInfo(
                  pRole, (char *)theUnit.c_str(), (char *)theGroup.c_str(),
                  (BYTE)*pState);
    ...
    on_over:
      ...
      for (char *szRole = szGroup + group.size() + 1; *szRole;
           szRole += strlen(szRole) + 1)
...
void KProtocolProcess::s2cExtendTong(BYTE *pMsg) {
  ...
  case enumTONG_SYNC_ID_CREATE_FAIL: {
    ...
    sprintf(sMsg.szMessage, "°ï»á´´½¨Ê§°Ü£¡");
    CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&sMsg, 0);
  ...
  case enumTONG_SYNC_ID_TRANSFER_ADD_APPLY: {
    ...
    memset(szName, 0, sizeof(szName));
    memcpy(szName, pApply->m_szName,
           pApply->m_wLength + 1 + sizeof(pApply->m_szName) -
               sizeof(TONG_APPLY_ADD_SYNC));
    dwNameID = g_FileName2Id(szName);
    nPlayerIdx = pApply->m_nPlayerIdx;

    // ¸ø½çÃæ·¢ÏûÏ¢£¬ÊÕµ½ÉêÇë£¬ÊÇ·ñÍ¬Òâ
    KSystemMessage sMsg;
    sprintf(sMsg.szMessage, "%sÉêÇë¼ÓÈë°ï»á£¡", szName);
  ...
  case enumTONG_SYNC_ID_ADD: {
    ...
    Player[CLIENT_PLAYER_INDEX].m_cTong.AddTong(pAdd->m_btCamp, szName, szTitle,
                                                szMaster);
    // Í¨Öª½çÃæ¸üÐÂÊý¾Ý
    KUiGameObjectWithName sUi;
  ...
  case enumTONG_SYNC_ID_HEAD_INFO: {
    ...
    CoreDataChanged(GDCNI_TONG_MEMBER_LIST, (unsigned int)&sObj,
                    (unsigned int)sItem);
  } break;
  ...
  case enumTONG_SYNC_ID_INSTATE: {
    ...
    CoreDataChanged(GDCNI_TONG_ACTION_RESULT, (unsigned int)&sUi,
                    pInstate->m_btSuccessFlag);
  } break;
  ...
  case enumTONG_SYNC_ID_CHANGE_MASTER_FAIL: {
    ...
    case 0: // ¶Ô·½²»ÔÚÏß
    {
      ...
      sMsg.byPriority = 0;
      sMsg.byParamSize = 0;
      sprintf(sMsg.szMessage, "´«Î»Ê§°Ü£¬¶Ô·½²»ÔÚÏß£¡");
      CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&sMsg, 0);
    } break;
    case 1: // ¶Ô·½ÄÜÁ¦²»¹»£¡
    {
      ...
      sMsg.byPriority = 0;
      sMsg.byParamSize = 0;
      sprintf(sMsg.szMessage, "´«Î»Ê§°Ü£¬¶Ô·½ÄÜÁ¦²»¹»£¡");
      CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&sMsg, 0);
    } break;
    case 2: // Ãû×Ö²»¶Ô£¡
    {
      ...
      sMsg.byPriority = 0;
      sMsg.byParamSize = 0;
      sprintf(sMsg.szMessage, "´«Î»Ê§°Ü£¬±¾°ïÖÐÎÞ´ËÈË£¡");
      CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&sMsg, 0);
    } break;
...
void KProtocolProcess::s2cPKSyncEnmityState(BYTE *pMsg) {
  ...
  memset(szName, 0, sizeof(szName));
  memcpy(szName, pState->m_szName,
         pState->m_wLength + 1 + sizeof(pState->m_szName) -
             sizeof(PK_ENMITY_STATE_SYNC));
  Player[CLIENT_PLAYER_INDEX].m_cPK.SetEnmityPKState(pState->m_btState,
                                                     pState->m_dwNpcID, szName);
}
...
void KProtocolProcess::s2cPKSyncExerciseState(BYTE *pMsg) {
  ...
  memcpy(szName, pState->m_szName,
         pState->m_wLength + 1 + sizeof(pState->m_szName) -
             sizeof(PK_EXERCISE_STATE_SYNC));
  Player[CLIENT_PLAYER_INDEX].m_cPK.SetExercisePKState(
      pState->m_btState, pState->m_dwNpcID, szName);
}
...
void KProtocolProcess::s2cViewEquip(BYTE *pMsg) { g_cViewItem.GetData(pMsg); }
...
void KProtocolProcess::PlayerBlockItemReturn(BYTE *pMsg) {
  PLAYER_BLOCK_ITEM_RETURN_COMMAND *pIDC =
      (PLAYER_BLOCK_ITEM_RETURN_COMMAND *)pMsg;
  ...
  if (nIdx) {
    ...
    sMsg.byPriority = 1;
    sMsg.byParamSize = 0;
    sprintf(sMsg.szMessage, "%s%s%s", "§· ®Ýnh b¶o hiÓm trang bÞ ",
            Item[nIdx].GetName(), " !");
    CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&sMsg, 0);
  }
}
...
void KProtocolProcess::PlayerUnBlockItemReturn(BYTE *pMsg) {
  PLAYER_UNBLOCK_ITEM_RETURN_COMMAND *pIDC =
      (PLAYER_UNBLOCK_ITEM_RETURN_COMMAND *)pMsg;
  ...
  if (nIdx) {
    ...
    sMsg.byPriority = 1;
    sMsg.byParamSize = 0;
    sprintf(sMsg.szMessage, "%s%s%s", "§· x¸c nhËn th¸o b¶o hiÓm trang bÞ",
            Item[nIdx].GetName(), " !");
    CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&sMsg, 0);
  }
}
...
void KProtocolProcess::PlayerOpenShopReturn(BYTE *pMsg) {
  ...
  PLAYER_SHOP_OPEN_RETURN_COMMAND *pIDC =
      (PLAYER_SHOP_OPEN_RETURN_COMMAND *)pMsg;

  memcpy(Player[CLIENT_PLAYER_INDEX].m_szNameOpenShop, pIDC->szName,
         sizeof(Player[CLIENT_PLAYER_INDEX].m_szNameOpenShop));

  memcpy(Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].m_szNameOpenShop,
         pIDC->szName,
         sizeof(Npc[Player[CLIENT_PLAYER_INDEX].m_nIndex].m_szNameOpenShop));
...
void KProtocolProcess::PlayerShopPriceReturn(BYTE *pMsg) {
  PLAYER_SHOP_PRICE_RETURN_COMMAND *pIDC =
      (PLAYER_SHOP_PRICE_RETURN_COMMAND *)pMsg;
...
void KProtocolProcess::s2cPlayerPointSync(BYTE *pMsg) {
...
void KProtocolProcess::PlayerUpdateShopPrice(BYTE *pMsg) {
  PLAYER_UPDATE_SHOP_PRICE_COMMAND *pIDC =
      (PLAYER_UPDATE_SHOP_PRICE_COMMAND *)pMsg;
...
void KProtocolProcess::PlayerBlockItemNow(BYTE *pMsg) {
  PLAYER_UNBLOCK_ITEM_NOW_COMMAND *pIDC =
      (PLAYER_UNBLOCK_ITEM_NOW_COMMAND *)pMsg;
...
void KProtocolProcess::LadderList(BYTE *pMsg) {
  ...
  if (pLadderListIndex) {
    ...
    CoreDataChanged(GDCNII_RANK_INDEX_LIST_ARRIVE, pList->nCount,
                    (int)pLadderListIndex);
...
void KProtocolProcess::LadderResult(BYTE *pMsg) {
  ...
  if (pLadderMessage) {
    ...
    CoreDataChanged(GDCNII_RANK_INFORMATION_ARRIVE, uParam,
                    (int)pLadderMessage);
...
void KProtocolProcess::s2cRefestItem(BYTE *pMsg) {
  ...
  if (nIdx > 0 && nIdx < MAX_ITEM) {
    ...
    Item[nIdx].SetTime(pItemSync->m_Time.bYear, pItemSync->m_Time.bMonth,
                       pItemSync->m_Time.bDay, pItemSync->m_Time.bHour);
...
void KProtocolProcess::NpcSkillCommand(int nIndex, BYTE *pProtocol) {
  ...
  if (ParamY < 0) {
    ...
    if (nNpcIndex > 0) {
      ParamZ = nNpcIndex;
      Npc[Player[nIndex].m_nIndex].SendCommand(do_skill, ParamX, ParamY,
                                               ParamZ);
    }
  } else
...
void KProtocolProcess::PlayerApplyTeamChangeCaptain(int nIndex,
                                                    BYTE *pProtocol) {
...
void KProtocolProcess::PlayerEatItem(int nIndex, BYTE *pProtocol) {
  if (Player[nIndex].CheckTrading() || Player[nIndex].CheckShopOpen() ||
      Player[nIndex].CheckShopBuyOpen())
    return;
...
void KProtocolProcess::ChatApplyReSendAllFriendName(int nIndex,
                                                    BYTE *pProtocol) {
...
void KProtocolProcess::TeamInviteAdd(int nIndex, BYTE *pProtocol) {
  Player[nIndex].m_cTeam.InviteAdd(nIndex,
                                   (TEAM_INVITE_ADD_COMMAND *)pProtocol);
}

void KProtocolProcess::ChangeAuraSkill(int nIndex, BYTE *pProtocol) {
  SKILL_CHANGEAURASKILL_COMMAND *pCommand =
      (SKILL_CHANGEAURASKILL_COMMAND *)pProtocol;
...
void KProtocolProcess::TeamReplyInvite(int nIndex, BYTE *pProtocol) {
  ...
  if (!pProtocol || pReply->m_nIndex <= 0 || pReply->m_nIndex >= MAX_PLAYER ||
      Player[pReply->m_nIndex].m_nIndex <= 0)
    return;
  if (pReply->m_btResult &&
      Player[nIndex].m_cTeam.GetCreatTeamFlag() == FALSE) {
    SHOW_MSG_SYNC sMsg;
    sMsg.ProtocolType = s2c_msgshow;
    sMsg.m_wMsgID = enumMSG_ID_CANNOT_ADD_TEAM;
    sMsg.m_wLength = sizeof(SHOW_MSG_SYNC) - 1 - sizeof(LPVOID);
    g_pServer->PackDataToClient(Player[nIndex].m_nNetConnectIdx, &sMsg,
                                sMsg.m_wLength + 1);
    return;
  }
  Player[pReply->m_nIndex].m_cTeam.GetInviteReply(pReply->m_nIndex, nIndex,
                                                  pReply->m_btResult);
}
...
void KProtocolProcess::ObjMouseClick(int nIndex, BYTE *pProtocol) {
  ...
  if (nRegionIdx < 0)
    return;
  nObjIdx =
      SubWorld[nSubWorldIdx].m_Region[nRegionIdx].FindObject(pObj->m_nObjID);
  if (nObjIdx <= 0)
    return;

  SubWorld[nSubWorldIdx].Map2Mps(
      Npc[Player[nIndex].m_nIndex].m_RegionIndex,
      Npc[Player[nIndex].m_nIndex].m_MapX, Npc[Player[nIndex].m_nIndex].m_MapY,
      Npc[Player[nIndex].m_nIndex].m_OffX, Npc[Player[nIndex].m_nIndex].m_OffY,
      &nPlayerX, &nPlayerY);
  SubWorld[nSubWorldIdx].Map2Mps(
      nRegionIdx, Object[nObjIdx].m_nMapX, Object[nObjIdx].m_nMapY,
      Object[nObjIdx].m_nOffX, Object[nObjIdx].m_nOffY, &nObjX, &nObjY);
  /*
          ...
          Obj_Kind_Light,				//
     ¹âÔ´£¨3DÄ£Ê½ÖÐ·¢¹âµÄ¶«Î÷£© Obj_Kind_Door,				// ÃÅÀà
          Obj_Kind_Trap,				// ÏÝÚå
  ...
  */
  ...
  case Obj_Kind_Box:
    if (g_GetDistance(nPlayerX, nPlayerY, nObjX, nObjY) >
        defMAX_EXEC_OBJ_SCRIPT_DISTANCE)
      break;
...
void KProtocolProcess::StoreMoneyCommand(int nIndex, BYTE *pProtocol) {
  ...
  if (pCommand->m_byDir) // È¡Ç®
    Player[nIndex].m_ItemList.ExchangeMoney(room_repository, room_equipment,
                                            pCommand->m_dwMoney);
  else // ´æÇ®
    Player[nIndex].m_ItemList.ExchangeMoney(room_equipment, room_repository,
                                            pCommand->m_dwMoney);
}

void KProtocolProcess::NpcReviveCommand(int nIndex, BYTE *pProtocol) {
  //	NPC_REVIVE_COMMAND*		pCommand = (NPC_REVIVE_COMMAND
  //*)pProtocol;
...
void KProtocolProcess::c2sPKApplyChangeNormalFlag(int nIndex, BYTE *pProtocol) {
  PK_APPLY_NORMAL_FLAG_COMMAND *pApply =
      (PK_APPLY_NORMAL_FLAG_COMMAND *)pProtocol;
...
void KProtocolProcess::c2sViewEquip(int nIndex, BYTE *pProtocol) {
  if (g_SubWorldSet.GetGameTime() - Player[nIndex].m_nViewEquipTime <
      defMAX_VIEW_EQUIP_TIME)
    return;
...
void KProtocolProcess::UiCommandScript(int nIndex, BYTE *pProtocol) {
  ...
  case 1:
    ...
    break;
  case 2:
    g_DebugLog("%s", pUiCmd->szFunc);
    Player[nIndex].ExecuteScript(Player[nIndex].m_dwTimeBoxId, pUiCmd->szFunc,
                                 "");
    break;
  case 3:
...
void KProtocolProcess::OpenShopGold(int nIndex, BYTE *pProtocol) {
  ...
  if (nIndex > 0 && nIndex < MAX_PLAYER) {
    if (!Npc[Player[nIndex].m_nIndex].m_FightMode) {
      ...
      // KPlayerChat::SendSystemInfo(1, nIndex, MESSAGE_SYSTEM_ANNOUCE_HEAD,
      // "HiÖn t¹i kh«ng th?s?dông. K?tr©n c¸c s?m?vµo 9 gi?ngµy 25/1/2013 !",
      // strlen("HiÖn t¹i kh«ng th?s?dông. K?tr©n c¸c s?m?vµo 9 gi?ngµy
      // 25/1/2013 !") );
...
void KProtocolProcess::C2SClientOpenShopCommand(int nIndex, BYTE *pProtocol) {
  ...
  BuySell.AutoBuyItem(nIndex, pDaTau1Cmd->nItemGenre, pDaTau1Cmd->nDetailType,
                      pDaTau1Cmd->nLevel, pDaTau1Cmd->num);
}
