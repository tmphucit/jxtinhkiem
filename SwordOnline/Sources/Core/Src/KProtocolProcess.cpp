...
void	KProtocolProcess::PlayerBlockItemReturn(BYTE* pMsg)
{
	...
	if (nIdx)
	{
		...
		Player[CLIENT_PLAYER_INDEX].m_ItemList.UnlockOperation();
			...
			sMsg.byPriority = 1;
			sMsg.byParamSize = 0;
			sprintf(sMsg.szMessage,"%s%s%s", "§· ®Ýnh b¶o hiÓm trang b?", Item[nIdx].GetName()," !");
			CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&sMsg, 0);
...
void	KProtocolProcess::PlayerUnBlockItemReturn(BYTE* pMsg)
{
	...
	if (nIdx)
	{
		 ...
		 Player[CLIENT_PLAYER_INDEX].m_ItemList.UnlockOperation();
			...
			sMsg.byPriority = 1;
			sMsg.byParamSize = 0;
			sprintf(sMsg.szMessage,"%s%s%s", "§· x¸c nhËn th¸o b¶o hiÓm trang b?", Item[nIdx].GetName()," !");
			CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&sMsg, 0);
...
void	KProtocolProcess::PlayerBlockItemNow(BYTE* pMsg)
{
	...
	if (nIdx)
	{
		 Item[nIdx].m_GeneratorParam.nVersion = 1;
			...
			sMsg.byPriority = 1;
			sMsg.byParamSize = 0;
			sprintf(sMsg.szMessage,"%s%s%s", "§· x¸c nhËn th¸o b¶o hiÓm trang bÞ ", Item[nIdx].GetName()," !");
			CoreDataChanged(GDCNI_SYSTEM_MESSAGE, (unsigned int)&sMsg, 0);
...
void KProtocolProcess::OpenShopGold(int nIndex, BYTE* pProtocol)
{
	...
	if (nIndex > 0 && nIndex < MAX_PLAYER)
	{
	    ...
	    //KPlayerChat::SendSystemInfo(1, nIndex, MESSAGE_SYSTEM_ANNOUCE_HEAD, "HiÖn t¹i kh«ng th?s?dông. K?tr©n c¸c s?m?vµo 9 gi?ngµy 25/1/2013 !", strlen("HiÖn t¹i kh«ng th?s?dông. K?tr©n c¸c s?m?vµo 9 gi?ngµy 25/1/2013 !") );
