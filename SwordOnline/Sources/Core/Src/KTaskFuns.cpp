...
char *g_GetStringRes(int nStringID, char *szString, int nMaxLen) {
  ...
  g_StringResourseTabFile.GetString(szStringId, "STRING", "", szString,
                                    nMaxLen);
  return szString;
}
#endif
...
int g_WayPointPriceUnit =
    1; // WayPoint表格中价格的单位量，WayPoint价格 = 单位量 * 表格数值
int g_StationPriceUnit =
    1; // Station表格中价格的单位量，Station价格 = 单位量 * 表格数值
...
int g_GetPriceToWayPoint(int nStationId, int nWayPoint) {
  ...
  if (*(g_pWayPointPriceTab +
        (nStationId - 1) * g_WayPointPriceTabFile.GetWidth() + nWayPoint - 1) <
      0) {
    g_WayPointPriceTabFile.GetInteger(
        nWayPoint + 1, nStationId + 1, 0,
        (g_pWayPointPriceTab +
         (nStationId - 1) * g_WayPointPriceTabFile.GetWidth() + nWayPoint - 1));
  }
  return g_WayPointPriceUnit *
         (*(g_pWayPointPriceTab +
            (nStationId - 1) * g_WayPointPriceTabFile.GetWidth() + nWayPoint -
            1));
}

int g_GetPriceToStation(int nStationId, int nNextStationId) {
  if (nStationId == nNextStationId)
    return 0;
  ...
  if (*(g_pStationPriceTab +
        (nStationId - 1) * g_StationPriceTabFile.GetWidth() + nNextStationId -
        1) < 0) {
    int t;
    g_StationPriceTabFile.GetInteger(nNextStationId + 1, nStationId + 1, 0, &t);
    *(g_pStationPriceTab + (nStationId - 1) * g_StationPriceTabFile.GetWidth() +
      nNextStationId - 1) = t;
  }
  return g_StationPriceUnit *
         (*(g_pStationPriceTab +
            (nStationId - 1) * g_StationPriceTabFile.GetWidth() +
            nNextStationId - 1));
}
...
int g_GetPriceToDock(int nCurDockId, int nNextDockId) {
  ...
  if (*(g_pDockPriceTab + (nCurDockId - 1) * g_DockPriceTabFile.GetWidth() +
        nNextDockId - 1) < 0) {
    int t;
    g_DockPriceTabFile.GetInteger(nNextDockId + 1, nCurDockId + 1, 0, &t);
    *(g_pDockPriceTab + (nCurDockId - 1) * g_DockPriceTabFile.GetWidth() +
      nNextDockId - 1) = t;
  }
  return g_DockPriceUnit *
         (*(g_pDockPriceTab + (nCurDockId - 1) * g_DockPriceTabFile.GetWidth() +
            nNextDockId - 1));
}
...
void KTimerTaskFun::GetTimerTaskScript(char *szScriptFileName,
                                       unsigned short usTimerTaskId,
                                       size_t nScriptFileLen) {
  if (!szScriptFileName)
    return;
  ...
  m_TimerTaskTab.GetString(szTaskId, "SCRIPT", "", szScriptFileName,
                           nScriptFileLen);
}
