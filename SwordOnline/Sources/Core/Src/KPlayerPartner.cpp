...
BOOL KPlayerPartner::Init(int nNpcTemplateId, int nLevel, int nSubWorld, int
nRegion, int nMapX, int nMapY, int nOffX, int nOffY , tPartnerProp Prop)
{
        ...
        int	nNpcIdxInfo = MAKELONG(nLevel, nNpcTemplateId);//(nNpcTemplateId
<< 7) + nLevel; int nNpcIdx = NpcSet.Add(nNpcIdxInfo, nSubWorld, nRegion, nMapX,
nMapY, nOffX, nOffY); if (nNpcIdx <= 0) return FALSE;
...
BOOL KPlayerPartner::Init(int nNpcTemplateId, int nLevel, int nSubWorld, int
nMpsX, int nMpsY, tPartnerProp Prop)
{
        if (nNpcTemplateId < 0)
                return FALSE;
...
{
        if (m_Prop.m_ulTimeOutVanish && m_ulCurLifeTime >=
m_Prop.m_ulTimeOutVanish) GetNpc()->SendCommand(do_death); m_ulCurLifeTime ++;
        return TRUE;
}
#endif*/