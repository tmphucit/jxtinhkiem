...
void KPlayerPK::BlockPKState(BOOL bFlag)
{
m_bBlock = bFlag;
}
...
void	KPlayerPK::SetNormalPKState(int bFlag)
{
	...
	else if (Npc[Player[m_nPlayerIndex].m_nIndex].m_Level >= 135) // Level Do Sat
	{
...
BOOL	KPlayerPK::EnmityPKOpen(int nAim)
{
	...
	// aaa
	if (m_bBlock && Npc[Player[m_nPlayerIndex].m_nIndex].m_SubWorldIndex != g_SubWorldSet.SearchWorld(33) && Npc[Player[m_nPlayerIndex].m_nIndex].m_SubWorldIndex != g_SubWorldSet.SearchWorld(61))
		return FALSE;
