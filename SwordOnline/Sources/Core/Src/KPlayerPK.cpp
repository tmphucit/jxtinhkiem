...
void KPlayerPK::Active() { EnmityPKCountDown(); }

//-------------------------------------------------------------------------
//	功能：设定正常PK状态 TRUE 打开，可以砍人  FALSE 关闭，不可以砍人
//-------------------------------------------------------------------------

void KPlayerPK::BlockPKState(BOOL bFlag) { m_bBlock = bFlag; }
...
void KPlayerPK::SetNormalPKState(int bFlag) {
  ...
  */
  ...
  sFlag.m_btFlag = m_nNormalPKFlag;
  g_pServer->PackDataToClient(Player[m_nPlayerIndex].m_nNetConnectIdx,
                              (BYTE *)&sFlag, sizeof(PK_NORMAL_FLAG_SYNC));
}

void KPlayerPK::SetNormalPKStateDB(int bFlag) {
...
//-------------------------------------------------------------------------
int KPlayerPK::GetNormalPKState() { return this->m_nNormalPKFlag; }
...
void KPlayerPK::EnmityPKClose() {
  ...
  g_pServer->PackDataToClient(Player[m_nPlayerIndex].m_nNetConnectIdx,
                              (BYTE *)&sState, sState.m_wLength + 1);

  if (m_nEnmityPKAim > 0 && m_nEnmityPKAim < MAX_PLAYER) {
    g_pServer->PackDataToClient(Player[m_nEnmityPKAim].m_nNetConnectIdx,
                                (BYTE *)&sState, sState.m_wLength + 1);
...
BOOL KPlayerPK::EnmityPKOpen(int nAim) {
  ...
  sState.m_wLength = sizeof(PK_ENMITY_STATE_SYNC) - 1 -
                     sizeof(sState.m_szName) + strlen(sState.m_szName);
  g_pServer->PackDataToClient(Player[nAim].m_nNetConnectIdx, (BYTE *)&sState,
                              sState.m_wLength + 1);
...
//-------------------------------------------------------------------------
int KPlayerPK::GetEnmityPKState() { return this->m_nEnmityPKState; }

//-------------------------------------------------------------------------
//	功能：获得仇杀目标 if 目标==0 不在仇杀状态 else 得到仇杀目标
//-------------------------------------------------------------------------
int KPlayerPK::GetEnmityPKAim() { return this->m_nEnmityPKAim; }
...
void KPlayerPK::ExercisePKClose() {
  ...
  sState.m_dwNpcID = 0;
  sState.m_wLength =
      sizeof(PK_EXERCISE_STATE_SYNC) - 1 - sizeof(sState.m_szName);
  g_pServer->PackDataToClient(Player[m_nPlayerIndex].m_nNetConnectIdx,
                              (BYTE *)&sState, sState.m_wLength + 1);

  if (m_nExercisePKAim > 0 && m_nExercisePKAim < MAX_PLAYER) {
    g_pServer->PackDataToClient(Player[m_nExercisePKAim].m_nNetConnectIdx,
                                (BYTE *)&sState, sState.m_wLength + 1);
...
BOOL KPlayerPK::ExercisePKOpen(int nAim) {
  ...
  sState.m_dwNpcID = Npc[Player[m_nPlayerIndex].m_nIndex].m_dwID;
  strcpy(sState.m_szName, Npc[Player[m_nPlayerIndex].m_nIndex].Name);
  sState.m_wLength = sizeof(PK_EXERCISE_STATE_SYNC) - 1 -
                     sizeof(sState.m_szName) + strlen(sState.m_szName);
  g_pServer->PackDataToClient(Player[nAim].m_nNetConnectIdx, (BYTE *)&sState,
                              sState.m_wLength + 1);
...
//-------------------------------------------------------------------------
int KPlayerPK::GetExercisePKAim() { return this->m_nExercisePKAim; }
...
void KPlayerPK::SetPKValue(int nValue) {
  ...
  g_pServer->PackDataToClient(Player[m_nPlayerIndex].m_nNetConnectIdx,
                              (BYTE *)&sValue, sizeof(PK_VALUE_SYNC));
}

//-------------------------------------------------------------------------
//	功能：获得PK值
//-------------------------------------------------------------------------
int KPlayerPK::GetPKValue() { return m_nPKValue; }
...
void KPlayerPK::SetNormalPKState(int bFlag, BOOL bShowMsg /* = TRUE*/) {
  ...
  if (bShowMsg) {
    ...
    if (bFlag == 1)
      ...
      sprintf(sMsg.szMessage, MSG_PK_NORMAL_FLAG_OPEN_NEXT);
    else
...
//-------------------------------------------------------------------------
void KPlayerPK::ApplySetNormalPKState(int bFlag) {
...
//	功能：设定仇杀PK状态
//-------------------------------------------------------------------------
void KPlayerPK::SetEnmityPKState(int nState, int nNpcID /* = 0*/,
                                 char *lpszName /* = NULL*/) {
...
//	功能：设定切磋状态
//-------------------------------------------------------------------------
void KPlayerPK::SetExercisePKState(int nState, int nNpcID /* = 0*/,
                                   char *lpszName /* = NULL*/) {
