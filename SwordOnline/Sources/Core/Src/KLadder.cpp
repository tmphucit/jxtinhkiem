...
KLadder::KLadder() { ZeroMemory(&GameStatData, sizeof(TGAME_STAT_DATA)); }
...
const TRoleList *KLadder::TopTenKiller() { return GameStatData.KillerStat; }

const TRoleList *KLadder::TopTenMasterHand() { return GameStatData.LevelStat; }

const TRoleList *KLadder::TopTenRich() { return GameStatData.MoneyStat; }

int KLadder::GetFacMasterHandPercent(int nFac) {
  // g_DebugLog("Fac: %d %d",nFac,FACTIONS_PRR_SERIES);
  if (nFac < -1 || nFac >= series_num * FACTIONS_PRR_SERIES)
    return 0;
