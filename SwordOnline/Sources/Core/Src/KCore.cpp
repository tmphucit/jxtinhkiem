...
// #ifdef _DEBUG
CORE_API BOOL
    g_bDebugScript; // When True , Testing Debug, Script Will be ReLoaded
                    // EveryTime When to be Executed, Not Using ScriptCach.
// #endif
...
CORE_API void g_InitCore() {
...

  {
    ...
    sprintf(szFileName, "%s\\DebugData\\Script\\ScriptOut_%d%d%d%d%d.txt",
            szPathName, newtime->tm_mon, newtime->tm_mday, newtime->tm_hour,
            newtime->tm_min, newtime->tm_sec);
    sprintf(szFileName1, "%s\\DebugData\\Script\\ScriptErr_%d%d%d%d%d.txt",
            szPathName, newtime->tm_mon, newtime->tm_mday, newtime->tm_hour,
            newtime->tm_min, newtime->tm_sec);
  ...
  if (Weapon_PhysicsSkillIdFile.Load(WEAPON_PHYSICSSKILLFILE)) {
    ...
    int nParticularCol =
        Weapon_PhysicsSkillIdFile.FindColumn(WEAPON_PARTICULARTYPE);
    ...
    for (int i = 0; i < nHeight; i++) {
      ...
      Weapon_PhysicsSkillIdFile.GetInteger(i + 2, nParticularCol, -1,
                                           &nParticular);
      Weapon_PhysicsSkillIdFile.GetInteger(i + 2, nPhysicsSkillCol, -1,
                                           &nPhysicsSkill);
      ...
      if (nDetail == 0) {
        ...
        if (nParticular >= 0 &&
            nParticular < MAX_RANGEWEAPON_PARTICULARTYPE_NUM &&
            nPhysicsSkill > 0 && nPhysicsSkill < MAX_SKILL)
...
BOOL InitTaskSetting() {
  ...
  if (!g_EventItemTab.Load(QUESTITEM_TABFILE)) {
    g_DebugLog("[TASK]CAN NOT LOAD EventItem Setting File %s",
               QUESTITEM_TABFILE);
  }
  if (!g_WayPointTabFile.Load(WORLD_WAYPOINT_TABFILE)) {
    g_DebugLog("[TASK]CAN NOT LOAD WayPoint Setting File %s",
               WORLD_WAYPOINT_TABFILE);
  }

  if (!g_StationTabFile.Load(WORLD_STATION_TABFILE)) {
    g_DebugLog("[TASK]CAN NOT LOAD WayPoint Setting File %s",
               WORLD_STATION_TABFILE);
  }

  if (!g_StationPriceTabFile.Load(WORLD_STATIONPRICE_TABFILE)) {
    g_DebugLog("[TASK]CAN NOT LOAD STATION PRICE TabFile %s",
               WORLD_STATIONPRICE_TABFILE);
  }

  if (!g_WayPointPriceTabFile.Load(WORLD_WAYPOINTPRICE_TABFILE)) {
    g_DebugLog("[TASK]CAN NOT LOAD WAYPOINT PRICE TabFile %s",
               WORLD_WAYPOINTPRICE_TABFILE);
  }
  ...
  if (!g_DockPriceTabFile.Load(WORLD_DOCKPRICE_TABFILE)) {
    g_DebugLog("[TASK]CAN NOT LOAD DOCK PRICE TabFile %s",
               WORLD_DOCKPRICE_TABFILE);
  }
...
BOOL InitNpcSetting() {
  ...
  // g_pNpcTemplate = new KNpcTemplate[nNpcTemplateNum * MAX_NPC_LEVEL];
  // //0,0为起点
  ...
  g_DebugLog("NpcTempleSize is %d * %d * %d = %d", sizeof(KNpcTemplate),
             nNpcTemplateNum, MAX_NPC_LEVEL,
             sizeof(KNpcTemplate) * nNpcTemplateNum * MAX_NPC_LEVEL);
  ...
  if (!g_pNpcLevelScript) {
    g_DebugLog("%sNpc等级设定脚本无法加载，可能文件不存在或语法错误",
...
unsigned int InitAdjustColorTab() {
  ...
  for (int i = 0; i < nHeight; i++) {
    ...
    nBlue &= 0xff;
    unsigned long ulAdjustColor =
        nAlpha << 24 | nRed << 16 | nGreen << 8 | nBlue;
