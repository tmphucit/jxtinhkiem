...
char g_szObjKind[Obj_Kind_Num][32] = {
    "MapObj",    "Body",  "Box",  "Item", "Money", "LoopSound",
    "RandSound", "Light", "Door", "Trap", "Prop"};

DWORD g_dwObjKindNum[Obj_Kind_Num] = {
    0x4f70614d, 0x79646f42, 0x00786f42, 0x6d657449, 0x656e6f4d, 0x706f6f4c,
    0x646e6152, 0x6867694c, 0x726f6f44, 0x70617254, 0x706f7250};

KObjSet::KObjSet() { m_nObjID = OBJ_WORLD_ID_START; }

KObjSet::~KObjSet() { m_nObjID = 0; }
...
#ifdef _SERVER
int KObjSet::AddPropObj(KMapPos MapPos, int nDataID, int nColorID) {
  KObjItemInfo sInfo;
  sInfo.m_nItemID = 0;
  sInfo.m_nItemWidth = 0;
  sInfo.m_nItemHeight = 0;
...
int KObjSet::Add(int nDataID, KMapPos MapPos, KObjItemInfo sItemInfo) {
  ...
  nAddNo = AddData(nDataID, MapPos, sItemInfo.m_nMoneyNum, sItemInfo.m_nItemID,
                   sItemInfo.m_nItemWidth, sItemInfo.m_nItemHeight);
  if (nAddNo < 0)
    return -1;
  ...
  Object[nAddNo].m_bByPlayer = sItemInfo.m_bByPlayer;
  ...
  SubWorld[MapPos.nSubWorld].Map2Mps(MapPos.nRegion, MapPos.nMapX, MapPos.nMapY,
                                     MapPos.nOffX, MapPos.nOffY, &nTempX,
                                     &nTempY);
  ...
  SubWorld[MapPos.nSubWorld].m_Region[MapPos.nRegion].BroadCast(
      (BYTE *)&cObjAdd, cObjAdd.m_wLength + 1, nMaxCount,
      Object[nAddNo].m_nMapX, Object[nAddNo].m_nMapY);
  int nConRegion;
  for (i = 0; i < 8; i++) {
    nConRegion =
        SubWorld[MapPos.nSubWorld].m_Region[MapPos.nRegion].m_nConnectRegion[i];
    if (nConRegion == -1)
      continue;
    SubWorld[MapPos.nSubWorld].m_Region[nConRegion].BroadCast(
        (BYTE *)&cObjAdd, cObjAdd.m_wLength + 1, nMaxCount,
        Object[nAddNo].m_nMapX - POff[i].x, Object[nAddNo].m_nMapY - POff[i].y);
  }

  SubWorld[MapPos.nSubWorld].m_Region[MapPos.nRegion].AddObj(
      nAddNo); // m_WorldMessage.Send(GWM_OBJ_ADD, MapPos.nRegion, nAddNo);
...
//	功能：从obj数据文件中载入相应数据
//---------------------------------------------------------------------------
int KObjSet::AddData(int nDataID, KMapPos MapPos, int nMoneyNum, int nItemID,
  ...
  m_cTabFile.GetString(nDataID + 1, ObjDataField_Kind, g_szObjKind[0], szBuffer,
                       sizeof(szBuffer));
  ...
  // 从 ObjData 文件中读取数据
  m_cTabFile.GetInteger(nDataID + 1, ObjDataField_LifeTime, 0,
                        &Object[nFreeNo].m_nLifeTime);
  m_cTabFile.GetString(nDataID + 1, ObjDataField_Name, "",
                       Object[nFreeNo].m_szName,
                       sizeof(Object[nFreeNo].m_szName));
  m_cTabFile.GetString(nDataID + 1, ObjDataField_ScriptName, "", szBuffer,
                       sizeof(szBuffer));
  Object[nFreeNo].SetScriptFile(szBuffer);

  int nTotalFrame, nTotalDir, nCurFrame, nCurDir;
  m_cTabFile.GetInteger(nDataID + 1, ObjDataField_ImageTotalFrame, 1,
                        &nTotalFrame);
  ...
  m_cTabFile.GetInteger(nDataID + 1, ObjDataField_ImageInterval, 0,
                        (int *)(&Object[nFreeNo].m_cImage.m_dwInterval));
  ...
  Object[nFreeNo].m_nDir = (Object[nFreeNo].m_cImage.m_nCurDir * 64 /
                            Object[nFreeNo].m_cImage.m_nTotalDir) %
                           64;
...
//---------------------------------------------------------------------------
BOOL KObjSet::SetObjItem(int nObjIndex, int nItemID, int nItemWidth,
                         int nItemHeight) {
...
//	功能：客户端添加一个obj
//---------------------------------------------------------------------------
int KObjSet::ClientAdd(int nID, int nDataID, int nState, int nDir,
                       int nCurFrame, int nXpos, int nYpos,
                       KObjItemInfo sInfo) {
  if (nDataID <= 0 || nDataID >= m_cTabFile.GetHeight())
    return -1;
...
//---------------------------------------------------------------------------
BOOL KObjSet::ClientLoadRegionObj(char *lpszMapPath, int nRegionX, int nRegionY,
  ...
  sprintf(szFile, "\\%s\\v_%03d\\%03d_%s", lpszMapPath, nRegionY, nRegionX,
          REGION_OBJ_FILE_CLIENT);
  ...
  for (int i = 0; (DWORD)i < sHead.uNumObj; i++) {
    ...
    ClientAdd(0, sData.nTemplateID, sData.nState, sData.nDir, 0, sData.Pos.x,
              sData.Pos.y, sInfo);
  }
  cDataFile.Close();
...
//---------------------------------------------------------------------------
BOOL KObjSet::ServerLoadRegionObj(char *lpszMapPath, int nRegionX, int nRegionY,
                                  int nSubWorld) {
  if (!lpszMapPath || !lpszMapPath[0])
    return FALSE;
  ...
  sprintf(szFile, "\\%s\\%03d_%s", lpszMapPath, nRegionX,
          REGION_OBJ_FILE_SERVER);
  if (cDataFile.Open(szFile) != TRUE)
    return FALSE;
  if (cDataFile.Size() < sizeof(KObjFileHead)) {
    cDataFile.Close();
  ...
  for (int i = 0; i < sHead.uNumObj; i++) {
    ...
    SubWorld[nSubWorld].Mps2Map(sData.Pos.x, sData.Pos.y, &sPos.nRegion,
                                &sPos.nMapX, &sPos.nMapY, &sPos.nOffX,
                                &sPos.nOffY);
...
//	功能：服务器端载入一个Region的所有Obj
//---------------------------------------------------------------------------
BOOL KObjSet::ServerLoadRegionObj(int nSubWorld, KPakFile *pFile,
                                  DWORD dwDataSize) {
...
//---------------------------------------------------------------------------
int KObjSet::AddData(int nDataID, int nSubWorld, int nRegion, int nMapX,
                     int nMapY, int nOffX, int nOffY) {
  if (nDataID <= 0 || nDataID >= m_cTabFile.GetHeight())
    return -1;
  ...
  Object[nFreeNo].m_nKind = i;
  // 从 ObjData 文件中读取数据
  m_cTabFile.GetInteger(nDataID + 1, ObjDataField_Layer, 1,
                        &Object[nFreeNo].m_nLayer);
  m_cTabFile.GetInteger(nDataID + 1, ObjDataField_Height, 0,
                        &Object[nFreeNo].m_nHeight);
  m_cTabFile.GetInteger(nDataID + 1, ObjDataField_LifeTime, 0,
                        &Object[nFreeNo].m_nLifeTime);
  m_cTabFile.GetString(nDataID + 1, ObjDataField_Name, "",
                       Object[nFreeNo].m_szName,
  ...
  m_cTabFile.GetString(nDataID + 1, ObjDataField_ScriptName, "", szBuffer,
                       sizeof(szBuffer));
  Object[nFreeNo].SetScriptFile(szBuffer);
  m_cTabFile.GetString(nDataID + 1, ObjDataField_SoundName, "",
                       Object[nFreeNo].m_szSoundName,
                       sizeof(Object[nFreeNo].m_szSoundName));

  m_cTabFile.GetInteger(nDataID + 1, ObjDataField_LightRadius, 0,
                        &Object[nFreeNo].m_sObjLight.m_nRadius);
  m_cTabFile.GetInteger(nDataID + 1, ObjDataField_LightRed, 255,
                        &Object[nFreeNo].m_sObjLight.m_nRed);
  m_cTabFile.GetInteger(nDataID + 1, ObjDataField_LightGreen, 255,
                        &Object[nFreeNo].m_sObjLight.m_nGreen);
  m_cTabFile.GetInteger(nDataID + 1, ObjDataField_LightBlue, 255,
                        &Object[nFreeNo].m_sObjLight.m_nBlue);
  m_cTabFile.GetInteger(nDataID + 1, ObjDataField_LightAlpha, 255,
                        &Object[nFreeNo].m_sObjLight.m_nAlpha);
  m_cTabFile.GetInteger(nDataID + 1, ObjDataField_LightReflectType, 0,
                        &Object[nFreeNo].m_sObjLight.m_nReflectType);

  int nTotalFrame, nTotalDir, nCurFrame, nCurDir;
  m_cTabFile.GetString(nDataID + 1, ObjDataField_ImageName, "",
                       Object[nFreeNo].m_szImageName,
                       sizeof(Object[nFreeNo].m_szImageName));
  m_cTabFile.GetInteger(nDataID + 1, ObjDataField_ImageTotalFrame, 1,
                        &nTotalFrame);
  m_cTabFile.GetInteger(nDataID + 1, ObjDataField_ImageTotalDir, 1, &nTotalDir);
  m_cTabFile.GetInteger(nDataID + 1, ObjDataField_ImageCurFrame, 0, &nCurFrame);
  ...
  m_cTabFile.GetInteger(nDataID + 1, ObjDataField_ImageInterval, 0,
                        (int *)(&Object[nFreeNo].m_cImage.m_dwInterval));
  m_cTabFile.GetInteger(nDataID + 1, ObjDataField_ImageCgXpos, 0,
                        &Object[nFreeNo].m_cImage.m_nCgXpos);
  m_cTabFile.GetInteger(nDataID + 1, ObjDataField_ImageCgYpos, 0,
  ...
  Object[nFreeNo].m_cImage.SetCurFrame(nCurFrame);
  Object[nFreeNo].m_cImage.SetCurDir(nCurDir);
  Object[nFreeNo].SetDir(Object[nFreeNo].m_cImage.m_nCurDir * 64 /
                         Object[nFreeNo].m_cImage.m_nTotalDir);

  int nDropTotalFrame, nDropTotalDir, nDropCurFrame, nDropCurDir;
  m_cTabFile.GetString(nDataID + 1, ObjDataField_ImageDropName,
                       Object[nFreeNo].m_szImageName,
                       Object[nFreeNo].m_szImageDropName,
                       sizeof(Object[nFreeNo].m_szImageDropName));
  m_cTabFile.GetInteger(nDataID + 1, ObjDataField_ImageDropTotalFrame,
                        nTotalFrame, &nDropTotalFrame);
  m_cTabFile.GetInteger(nDataID + 1, ObjDataField_ImageDropTotalDir, nTotalDir,
                        &nDropTotalDir);
  m_cTabFile.GetInteger(nDataID + 1, ObjDataField_ImageDropCurFrame, nCurFrame,
                        &nDropCurFrame);
  m_cTabFile.GetInteger(nDataID + 1, ObjDataField_ImageDropCurDir, nCurDir,
                        &nDropCurDir);
  m_cTabFile.GetInteger(nDataID + 1, ObjDataField_ImageDropInterval,
                        Object[nFreeNo].m_cImage.m_dwInterval,
                        (int *)(&Object[nFreeNo].m_cImageDrop.m_dwInterval));
  m_cTabFile.GetInteger(nDataID + 1, ObjDataField_ImageDropCgXpos,
                        Object[nFreeNo].m_cImage.m_nCgXpos,
                        &Object[nFreeNo].m_cImageDrop.m_nCgXpos);
  m_cTabFile.GetInteger(nDataID + 1, ObjDataField_ImageDropCgYpos,
                        Object[nFreeNo].m_cImage.m_nCgYpos,
  ...
  m_cTabFile.GetInteger(nDataID + 1, ObjDataField_DrawFlag, 0,
                        &Object[nFreeNo].m_bDrawFlag);
...
//	功能：找没有使用的空闲obj
//---------------------------------------------------------------------------
int KObjSet::FindFree() { return m_FreeIdx.GetNext(0); }
...
int KObjSet::GetID() {
  m_nObjID++;
  if (m_nObjID > 2000000000)
    printf("Max OBJ DWORD !!!");
...
int KObjSet::SearchObjAt(int nX, int nY, int nRange) {
  ...
  while (1) {
    ...
    int x, y;
    SubWorld[0].Map2Mps(Object[nIdx].m_nRegionIdx, Object[nIdx].m_nMapX,
                        Object[nIdx].m_nMapY, Object[nIdx].m_nOffX,
                        Object[nIdx].m_nOffY, &x, &y);
