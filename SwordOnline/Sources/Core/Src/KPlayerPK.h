...
public:
  ...
  void SetNormalPKState(int bFlag);   // 设定正常PK状态
  void SetNormalPKStateDB(int bFlag); // 设定正常PK状态
...
public:
  void Init();                           // 初始化
  void ApplySetNormalPKState(int bFlag); // 向服务器申请打开、关闭正常PK状态
  ...
  void SetEnmityPKState(int nState, int nNpcID = 0,
                        char *lpszName = NULL);        // 设定仇杀PK状态
  ...
  void EnmityPKCountDown();                                // 仇杀倒计时
  void SetExercisePKState(int nState, int nNpcID = 0,
                          char *lpszName = NULL);        // 设定切磋状态
