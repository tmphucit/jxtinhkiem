  ...
  if (eStatus == MS_DoFly) {
    ...
    if (nSprDir && nSprFrames) {
      ...
      if (Player[CLIENT_PLAYER_INDEX].m_DebugMode) {
        ...
        sprintf(MissleId, "%d_%d", m_nMissleId,
                Missle[m_nMissleId].m_nCurrentLife);
        g_pRepresent->OutputText(12, MissleId, KRF_ZERO_END, nX, nY, 0xffffffff,
                                 3, 5);
      }
...
void KMissleRes::PlaySound(int eStatus, int nX, int nY, int nLoop) {
  ...
  m_pSndNode = (KCacheNode *)g_SoundCache.GetNode(
      m_MissleRes[eStatus].SndFileName, (KCacheNode *)m_pSndNode);
