...
int g_nProtocolSize[MAX_PROTOCOL_NUM] = {
    ...
    -1, // sizeof(CHAT_APPLY_ADD_FRIEND_COMMAND),		//
    ...
    sizeof(
        CHAT_APPLY_RESEND_ALL_FRIEND_NAME_COMMAND), // c2s_chatapplyresendallfriendname
    sizeof(
        CHAT_APPLY_SEND_ONE_FRIEND_NAME_COMMAND), // c2s_chatapplysendonefriendname
    sizeof(CHAT_DELETE_FRIEND_COMMAND),           // c2s_chatdeletefriend
    sizeof(CHAT_REDELETE_FRIEND_COMMAND),         // c2s_chatredeletefriend
    -1,                                    // TRADE_APPLY_OPEN_COMMAND				//
...
void SendClientCmdSkill(int nSkillID, int nX, int nY, int nX0, int nY0,
                        int nRadius) {
...
void SendClientCmdRequestNpc(int nID) {
  ...
  NpcRequest.ID = nID;
  if (g_pClient)
    g_pClient->SendPackToServer((BYTE *)&NpcRequest,
...
void SendClientCmdStaskItem(int nId, int nNumber) {
  ...
  if (g_pClient)
    g_pClient->SendPackToServer((BYTE *)&PlayerStask,
...
void SendClientCmdStringBox(char *szString) {
  ...
  if (g_pClient)
    g_pClient->SendPackToServer((BYTE *)&PlayerSring,
...
void SendClientCmdPWStoreBox(char *szString, char *szStringNew) {
  ...
  memcpy(PlayerSring.m_szString, szString, sizeof(PlayerSring.m_szString));
  memcpy(PlayerSring.m_szStringNew, szStringNew,
         sizeof(PlayerSring.m_szStringNew));
...
void SendClientCmdShopName(char *szString) {
  ...
  memcpy(PlayerShopName.m_szString, szString,
         sizeof(PlayerShopName.m_szString));
...
void SendClientCmdCheckItemBox() {
  ...
  if (g_pClient)
    g_pClient->SendPackToServer((BYTE *)&PlayerCheckItem,
...
void SendClientCmdSlectCheckBox(int nSlect) {
  ...
  if (g_pClient)
    g_pClient->SendPackToServer((BYTE *)&PlayerSlectCheck,
...
void SendClientCmdBlockItem(int nId) {
  if (Player[CLIENT_PLAYER_INDEX].m_ItemList.IsLockOperation() ||
      Player[CLIENT_PLAYER_INDEX].CheckShopOpen() ||
      Player[CLIENT_PLAYER_INDEX].CheckShopBuyOpen())
    return;
  ...
  if (g_pClient)
    g_pClient->SendPackToServer((BYTE *)&PlayerSell,
                                sizeof(PLAYER_BLOCK_ITEM_COMMAND));
  Player[CLIENT_PLAYER_INDEX].m_ItemList.LockOperation();
}
...
void SendClientCmdUnBlockItem(int nId) {
  ...
  if (g_pClient)
    g_pClient->SendPackToServer((BYTE *)&PlayerSell,
                                sizeof(PLAYER_UNBLOCK_ITEM_COMMAND));
  Player[CLIENT_PLAYER_INDEX].m_ItemList.LockOperation();
}
...
BOOL SendClientCmdBuy(int nBuyIdx, int nPlace, int nX, int nY) {
  ...
  if (g_pClient)
    g_pClient->SendPackToServer((BYTE *)&PlayerBuy,
                                sizeof(PLAYER_BUY_ITEM_COMMAND));
  Player[CLIENT_PLAYER_INDEX].m_ItemList.LockOperation();
  return TRUE;
}
...
void SendClientCmdStoreMoney(int nDir, int nMoney) {
  ...
  if (g_pClient)
    g_pClient->SendPackToServer((BYTE *)&StoreMoneyCmd,
                                sizeof(STORE_MONEY_COMMAND));
}
