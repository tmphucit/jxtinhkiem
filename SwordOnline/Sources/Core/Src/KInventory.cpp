...
BOOL KInventory::PlaceItem(int nX, int nY, int nIdx, int nWidth, int nHeight) {
  if (!m_pArray)
    return FALSE;
  if (nX < 0 || nY < 0 || nWidth < 1 || nHeight < 1 || nX + nWidth > m_nWidth ||
      nY + nHeight > m_nHeight)
    return FALSE;
  if (nIdx <= 0) {
    _ASSERT(0);
    return FALSE;
    ...
    for (j = nY; j < nY + nHeight; j++) {
      ...
      if (nOldIdx) {
        ...
        if (nOldIdx != m_pArray[j * m_nWidth + i])
          return FALSE;
        /*#pragma
           message(ATTENTION("需要在道具系统完成后去检查是否可叠放"))
                                        // 检查是否为可叠放物品
                                        if (Item[nIdx].CanStack(nOldIdx) && i ==
           nX && j == nY)
                                        {
                                                return
           Item[nOldIdx].Stack(nIdx); } // 检查是否是容器 else if
           (Item[nOldIdx].IsReceptacle())
                                        {
                                                return
           Item[nOldIdx].HoldItem(nIdx);
                                        }
                                        */
        return FALSE;
      }
...
// #define	_DEBUG_ITEM	1
  ...
  for (int kk = 0; kk < 10; kk++) {
    g_DebugLog("%d%d%d%d%d%d", m_pArray[kk * m_nWidth],
               m_pArray[kk * m_nWidth + 1], m_pArray[kk * m_nWidth + 2],
               m_pArray[kk * m_nWidth + 3], m_pArray[kk * m_nWidth + 4],
...
BOOL KInventory::PickUpItem(int nIdx, int nX, int nY, int nWidth, int nHeight) {
  if (nX < 0 || nY < 0 || nWidth < 1 || nHeight < 1 ||
      nX + nWidth > this->m_nWidth || nY + nHeight > this->m_nHeight)
    return FALSE;
...
int KInventory::FindItem(int nX, int nY) {
  ...
  if ((nPos > 0 && *(pArray - 1) == nIdx) ||
      (nPos >= m_nWidth && *(pArray - m_nWidth) == nIdx))
    return -1;
...
int KInventory::GetNextItem(int nStartIdx, int nXpos, int nYpos, int *pX,
                            int *pY) {
  if (!m_pArray)
    return 0;
  if (nXpos < 0 || nYpos < 0 || nXpos >= m_nWidth || nYpos >= m_nHeight ||
      !pX || !pY)
    return 0;
...
int KInventory::CalcSameDetailType(int nGenre, int nDetail) {
  ...
  for (int i = 0; i < nSize; i++) {
    ...
    if (i < m_nWidth || pArray[-m_nWidth] != *pArray) {
      nCurIdx = *pArray;
      if (Item[nCurIdx].GetGenre() == nGenre &&
          Item[nCurIdx].GetDetailType() == nDetail)
        nNum++;
    }
    pArray++;
  }
...
BOOL KInventory::FindEmptyPlace(int nWidth, int nHeight, POINT *pPos) {
  if (!m_pArray)
    return FALSE;
  if (!pPos || nWidth <= 0 || nHeight <= 0 || nWidth > m_nWidth ||
      nHeight > m_nHeight)
    return FALSE;
...
int KInventory::FindNumberArrayItem(int nIdx) {
  if (!m_pArray)
    return 0;
...
BOOL KInventory::FindSameDetailType(int nGenre, int nDetail, int *pnIdx,
                                    int *pnX, int *pnY) {
  if (!m_pArray)
    return FALSE;
  if (!pnIdx || !pnX || !pnY)
          ...
          for (i = 0; i < this->m_nHeight; i++)
          {
                  for (j = 0; j < this->m_nWidth; j++)
                  {
                          if (!m_pArray[i * m_nWidth + j])
                                  continue;
                          if (Item[m_pArray[i * m_nWidth + j]].GetGenre() !=
     nGenre) continue; if (Item[m_pArray[i * m_nWidth + j]].GetDetailType() !=
     nDetail) continue; *pnIdx = m_pArray[i * m_nWidth + j]; *pnX = j; *pnY = i;
                          return TRUE;
                  }
          }
