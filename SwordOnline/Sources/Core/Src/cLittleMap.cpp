...
int cLittleMap::Create() {
  ASSERT(GetShell());
  m_nImageID = GetShell()->CreateImage(IDS_SHELL_MAP, max_littmap_width,
                                       max_littmap_height, ISI_T_BITMAP16);
...
int cLittleMap::SetMapSize(unsigned int left, unsigned int top,
...
int cLittleMap::RemoveNpc(void *pNpc) {
  for (std::list<stNpcOnPixel>::iterator it = m_listNpc.begin();
       it != m_listNpc.end(); ++it) {
    if ((*it).pNpc = pNpc) {
      m_listNpc.erase(it);
      return true;
...
void cLittleMap::RenderTexture() {
  ...
  rd.rcSrc = CRect(m_ptLoad.x, m_ptLoad.y, m_ptLoad.x + max_littmap_width,
                   m_ptLoad.y + max_littmap_height);
  rd.ptDes.x = 0;
  rd.ptDes.y = 0;

  pSrc += int(rd.rcSrc.top * m_pImage->GetWidth() * c_nColorBytes +
              rd.rcSrc.left * c_nColorBytes);
...
BOOL cLittleMap::VerifyTexture(BOOL bForceUpdate) {
  ...
  ScreenToGround(m_param.wScreen, m_param.hScreen, rcGroundNow.right,
                 rcGroundNow.bottom);

  CRect rcGround = CRect(
      m_rcRegion.left * c_nWidthRegion, m_rcRegion.top * c_nHeightRegion,
      m_rcRegion.right * c_nWidthRegion, m_rcRegion.bottom * c_nHeightRegion);
  CRect rcImage = CRect(
      m_rcRegion.left * GetImageSize(), m_rcRegion.top * GetImageSize(),
  ...
  GroundToScreen(rcGroundNow.right, rcGroundNow.bottom, m_rcMap.right,
                 m_rcMap.bottom);
  ...
  m_rcImage.left =
      (rcGroundNow.left - rcGround.left) * GetImageSize() / c_nWidthRegion;
  m_rcImage.top =
      (rcGroundNow.top - rcGround.top) * GetImageSize() / c_nHeightRegion;
  m_rcImage.right =
      m_rcImage.left + (rcGroundNow.Width()) * GetImageSize() / c_nWidthRegion;
  m_rcImage.bottom =
      m_rcImage.top + (rcGroundNow.Height()) * GetImageSize() / c_nHeightRegion;
  if (!bUpdate) {
    if (m_rcImage.left < m_ptLoad.x ||
        m_rcImage.right > m_ptLoad.x + c_nTextureWidth ||
        m_rcImage.top < m_ptLoad.y ||
        m_rcImage.bottom > m_ptLoad.y + c_nTextureHeight) {
      bUpdate = TRUE;
    }
  }
...
HRESULT cLittleMap::DrawTexture() {
  ...
  if (VerifyTexture()) {
    CRect rc(m_stMap.ptOffset.x, m_stMap.ptOffset.y,
             m_stMap.ptOffset.x + m_stMap.wScreen,
             m_stMap.ptOffset.y + m_stMap.hScreen);
...
HRESULT cLittleMap::Draw() {
  ...
  cRect rc(m_param.ptOffset.x, m_param.ptOffset.y, m_param.ptOffset.x + 8,
           m_param.ptOffset.y + 8);
...
void cLittleMap::ScreenToGround(CPoint ptScreen, cPoint &ptGround) {
  ...
  cPoint ptOffsetGround =
      cPoint(ptOffsetScreen.x * c_nWidthRegion / GetImageSize(),
             ptOffsetScreen.y * c_nHeightRegion / GetImageSize());
