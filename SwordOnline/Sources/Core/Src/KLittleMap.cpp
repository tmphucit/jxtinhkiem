...
void KLittleMap::Init(int nLeft, int nTop, int nRight, int nBottom) {
  ...
  if (m_bColorLoadFlag == FALSE) {
    ...
    if (cColorIni.Load(defLITTLE_MAP_SET_FILE)) {
      cColorIni.GetString("MapColor", "MapColor", "255,255,255", szTemp,
                          sizeof(szTemp));
      ...
      cColorIni.GetString("MapColor", "SelfColor", "255,255,255", szTemp,
                          sizeof(szTemp));
      ...
      cColorIni.GetInteger("MapColor", "TeammateShow", 1,
                           &this->m_bTeammateShowFlag);
      cColorIni.GetString("MapColor", "TeammateColor", "255,255,255", szTemp,
                          sizeof(szTemp));
      ...
      cColorIni.GetInteger("MapColor", "PlayerShow", 1,
                           &this->m_bPlayerShowFlag);
      cColorIni.GetString("MapColor", "PlayerColor", "255,255,255", szTemp,
                          sizeof(szTemp));
      ...
      cColorIni.GetInteger("MapColor", "FightNpcShow", 1,
                           &this->m_bFightNpcShowFlag);
      cColorIni.GetString("MapColor", "FightNpcColor", "255,255,255", szTemp,
                          sizeof(szTemp));
      ...
      cColorIni.GetInteger("MapColor", "NormalNpcShow", 1,
                           &this->m_bNormalNpcShowFlag);
      cColorIni.GetString("MapColor", "NormalNpcColor", "255,255,255", szTemp,
                          sizeof(szTemp));
...
BYTE *KLittleMap::GetBarrierBuf(int nX, int nY) {
  if (nX < m_sMapRect.left || nX > m_sMapRect.right || nY < m_sMapRect.top ||
  ...
  if (!m_ppbtBarrier[nRegion]) {
    m_ppbtBarrier[nRegion] =
        (BYTE *)new BYTE[REGION_GRID_WIDTH * REGION_GRID_HEIGHT];
    memset(m_ppbtBarrier[nRegion], 0,
           sizeof(BYTE) * REGION_GRID_WIDTH * REGION_GRID_HEIGHT);
  }
...
void KLittleMap::SetHaveLoad(int nX, int nY) {
  if (nX < m_sMapRect.left || nX > m_sMapRect.right || nY < m_sMapRect.top ||
    ...
    return;
  if (m_pbyLoadFlag)
    m_pbyLoadFlag[(nY - m_sMapRect.top) * m_nMapWidth + nX - m_sMapRect.left] =
        1;
}
...
void KLittleMap::Draw(int nX, int nY) {
  ...
  for (int i = 0; i < m_nShowWidth * REGION_GRID_WIDTH; i++) {
    for (int j = 0; j < m_nShowHeight * REGION_GRID_HEIGHT; j++) {
      ...
      if (nRx < m_sMapRect.left || nRx > m_sMapRect.right ||
          nRy < m_sMapRect.top || nRy > m_sMapRect.bottom)
        continue;
      nRx -= m_sMapRect.left;
      nRy -= m_sMapRect.top;
      if (!m_pbyLoadFlag[nRy * m_nMapWidth + nRx] ||
          !m_ppbtBarrier[nRy * m_nMapWidth + nRx])
        continue;
      nOx = (nSx + i) % REGION_GRID_WIDTH;
      nOy = (nSy + j) % REGION_GRID_HEIGHT;
      if (m_ppbtBarrier[nRy * m_nMapWidth + nRx]
                       [nOy * REGION_GRID_WIDTH + nOx]) {
  ...
  while (1) {
    ...
    if (Npc[nNpcIdx].m_Kind == kind_normal) {
      ...
      if (nNpcIdx == Player[CLIENT_PLAYER_INDEX].m_nIndex)
        continue;
      if (m_bTeammateShowFlag && Player[CLIENT_PLAYER_INDEX].m_cTeam.m_nFlag &&
  ...
  if (nRx < m_sMapRect.left || nRx > m_sMapRect.right || nRy < m_sMapRect.top ||
...
//	功能：其它玩家显示开关
//---------------------------------------------------------------------------
void KLittleMap::SetPlayerShow(BOOL bFlag) { this->m_bPlayerShowFlag = bFlag; }
...
//	功能：小地图显示开关
//---------------------------------------------------------------------------
void KLittleMap::Show(int nFlag) { this->m_nShowFlag = nFlag; }
