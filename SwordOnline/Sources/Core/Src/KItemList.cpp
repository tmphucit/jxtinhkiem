...
int g_nEquipPower[itempart_num] = {2, 4, 2, 4, 1, 1, 1, 1, 1, 1, 0};
#endif
...
int KItemList::GetWeaponType() {
  if (m_EquipItem[itempart_weapon] &&
      Item[m_EquipItem[itempart_weapon]].GetDurability())
...
void KItemList::GetWeaponDamage(int *nMin, int *nMax) {
  ...
  {
    /*
    int nDamageBase = Player[m_PlayerIdx].m_nCurStrength *
    Player[m_PlayerIdx].m_nCurDexterity; *nMin = nDamageBase >> 9; *nMax =
    nDamageBase >> 8;
    */
...
int KItemList::Add(int nIdx, int nPlace, int nX, int nY) {
  ...
  case pos_equiproom:
    if (!m_Room[room_equipment].PlaceItem(nX, nY, nIdx, Item[nIdx].GetWidth(),
                                          Item[nIdx].GetHeight()))
      return 0;
  ...
  case pos_trade1:
    if (!Player[CLIENT_PLAYER_INDEX].CheckTrading())
      return 0;
    if (!m_Room[room_trade1].PlaceItem(nX, nY, nIdx, Item[nIdx].GetWidth(),
                                       Item[nIdx].GetHeight()))
      return 0;
  ...
  case pos_buyshop:
    if (Player[CLIENT_PLAYER_INDEX].CheckTrading())
      return 0;
    if (!m_Room[room_buyshop].PlaceItem(nX, nY, nIdx, Item[nIdx].GetWidth(),
                                        Item[nIdx].GetHeight()))
      return 0;
  ...
  case pos_repositoryroom:
    if (!m_Room[room_repository].PlaceItem(nX, nY, nIdx, Item[nIdx].GetWidth(),
                                           Item[nIdx].GetHeight()))
      return 0;
  ...
  case pos_rpnew:
    if (!m_Room[room_rpnew].PlaceItem(nX, nY, nIdx, Item[nIdx].GetWidth(),
                                      Item[nIdx].GetHeight()))
      return 0;
  ...
  case pos_immediacy:
    if (!m_Room[room_immediacy].PlaceItem(nX, nY, nIdx, Item[nIdx].GetWidth(),
                                          Item[nIdx].GetHeight()))
      return 0;
    ...
    break;
  case pos_checkbox:
    if (!m_Room[room_checkbox].PlaceItem(nX, nY, nIdx, Item[nIdx].GetWidth(),
                                         Item[nIdx].GetHeight()))
      return 0;
  ...
  while (nLoopIdx) {
    g_DebugLog("[ITEM]ItemListIdx:%d, Item:%d, ItemId:%d", nLoopIdx,
               m_Items[nLoopIdx].nIdx, Item[m_Items[nLoopIdx].nIdx].GetID());
    nLoopIdx = m_UseIdx.GetNext(nLoopIdx);
  }
#endif
#endif
...
BOOL KItemList::Remove(int nGameIdx, BOOL bcheck) {
  ...
  if (!bcheck) {
    ...
    if (Item[nGameIdx].GetGenre() == item_equip &&
        Item[nGameIdx].m_GeneratorParam.nVersion > 1) {
  ...
  case pos_immediacy:
    m_Room[room_immediacy].PickUpItem(nGameIdx, m_Items[nIdx].nX,
                                      m_Items[nIdx].nY,
                                      Item[m_Items[nIdx].nIdx].GetWidth(),
                                      Item[m_Items[nIdx].nIdx].GetHeight());
    break;
  case pos_equiproom:
    m_Room[room_equipment].PickUpItem(nGameIdx, m_Items[nIdx].nX,
                                      m_Items[nIdx].nY,
                                      Item[m_Items[nIdx].nIdx].GetWidth(),
                                      Item[m_Items[nIdx].nIdx].GetHeight());
    break;
  case pos_repositoryroom:
    m_Room[room_repository].PickUpItem(nGameIdx, m_Items[nIdx].nX,
                                       m_Items[nIdx].nY,
                                       Item[m_Items[nIdx].nIdx].GetWidth(),
                                       Item[m_Items[nIdx].nIdx].GetHeight());
    break;
  ...
  case pos_rpnew:
    m_Room[room_rpnew].PickUpItem(nGameIdx, m_Items[nIdx].nX, m_Items[nIdx].nY,
                                  Item[m_Items[nIdx].nIdx].GetWidth(),
                                  Item[m_Items[nIdx].nIdx].GetHeight());
    break;

  case pos_traderoom:
    m_Room[room_trade].PickUpItem(nGameIdx, m_Items[nIdx].nX, m_Items[nIdx].nY,
                                  Item[m_Items[nIdx].nIdx].GetWidth(),
                                  Item[m_Items[nIdx].nIdx].GetHeight());
    break;
#ifndef _SERVER
  case pos_trade1:
    ...
    m_Room[room_trade1].PickUpItem(nGameIdx, m_Items[nIdx].nX, m_Items[nIdx].nY,
                                   Item[m_Items[nIdx].nIdx].GetWidth(),
                                   Item[m_Items[nIdx].nIdx].GetHeight());
    break;
  ...
  case pos_buyshop:
    ...
    m_Room[room_buyshop].PickUpItem(nGameIdx, m_Items[nIdx].nX,
                                    m_Items[nIdx].nY,
                                    Item[m_Items[nIdx].nIdx].GetWidth(),
                                    Item[m_Items[nIdx].nIdx].GetHeight());
    break;
  ...
  case pos_checkbox:
    m_Room[room_checkbox].PickUpItem(nGameIdx, m_Items[nIdx].nX,
                                     m_Items[nIdx].nY,
                                     Item[m_Items[nIdx].nIdx].GetWidth(),
                                     Item[m_Items[nIdx].nIdx].GetHeight());
    break;
  ...
  if (m_Items[nIdx].nPlace != pos_trade1 &&
      m_Items[nIdx].nPlace != pos_buyshop) {
...
#ifdef _SERVER
  g_pServer->PackDataToClient(Player[m_PlayerIdx].m_nNetConnectIdx,
                              (BYTE *)&sRemove, sizeof(ITEM_REMOVE_SYNC));
#endif
  return TRUE;
}
...
// Author		: Spe
*****************************************************************************/
int KItemList::FindFree() { return m_FreeIdx.GetNext(0); }
...
#ifndef _SERVER
  m_Room[room_trade1].Init(
      TRADE_ROOM_WIDTH,
      TRADE_ROOM_HEIGHT); // Õâ¸öµÄ´óÐ¡±ØÐëÓë room_trade µÄ´óÐ¡Ò»Ñù
  m_Room[room_buyshop].Init(EQUIPMENT_ROOM_WIDTH, EQUIPMENT_ROOM_HEIGHT);
#endif
  ZeroMemory(
      m_EquipItem,
      sizeof(m_EquipItem)); // Íæ¼Ò×°±¸µÄµÀ¾ß£¨¶ÔÓ¦ÓÎÏ·ÊÀ½çÖÐµÀ¾ßÊý×éµÄË÷Òý£©
  ZeroMemory(m_PGBoxItem, sizeof(m_PGBoxItem));
  ZeroMemory(
      m_Items,
      sizeof(
          m_Items)); // Íæ¼ÒÓµÓÐµÄËùÓÐµÀ¾ß£¨°üÀ¨×°±¸×ÅµÄºÍÏä×ÓÀï·ÅµÄ£¬¶ÔÓ¦ÓÎÏ·ÊÀ½çÖÐµÀ¾ßÊý×éµÄË÷Òý£©
...
BOOL KItemList::CanEquip(int nIdx, int nPlace) {
  if (m_PlayerIdx <= 0 || nIdx <= 0 || nIdx >= MAX_ITEM ||
      Item[nIdx].GetGenre() != item_equip)
    return FALSE;
...
// Return		: BOOL
// Argumant		: int
nIdx£¬ÓÎÏ·ÊÀ½çÖÐµÄµÀ¾ßÊý×é±àºÅ£¬ÒªÇóÒ»¶¨ÊÇ×°±¸ÀàµÀ¾ß
...
BOOL KItemList::Equip(int nIdx, int nPlace /* = -1 */) {
  ...
  if (m_PlayerIdx <= 0 || nIdx <= 0 || nNpcIdx <= 0 ||
      item_equip != Item[nIdx].GetGenre())
    return FALSE;
  ...
  switch (nEquipPlace) {
  case itempart_head:
    Npc[nNpcIdx].m_HelmType = g_ItemChangeRes.GetHelmRes(
        Item[nIdx].GetParticular(), Item[nIdx].GetLevel());
    Npc[nNpcIdx].m_HelmTypeTemp = Npc[nNpcIdx].m_HelmType;
    break;
  case itempart_body: {
    Npc[nNpcIdx].m_ArmorType = g_ItemChangeRes.GetArmorRes(
        Item[nIdx].GetParticular(), Item[nIdx].GetLevel());
    ...
    if (nIdx != 0) {
      ...
      if (Npc[nNpcIdx].m_nSex == 0) {
        ...
        g_DebugLog("Check Ao: %d %d %d", Item[nIdx].GetDetailType(),
                   Item[nIdx].GetParticular(), Npc[nNpcIdx].m_FifongType);
      } else {
        if (Item[nIdx].GetLevel() < 6)
    ...
    // g_DebugLog("Kiem tra %d %d %d",
    // Npc[nNpcIdx].m_FifongType,nIdx,Item[nIdx].GetKind());
    if (Npc[nNpcIdx].m_FifongType > 0 && nIdx != 0 &&
        m_EquipItem[itempart_fifong] != 0) {
  ...
  case itempart_weapon:
    Npc[nNpcIdx].m_WeaponType = g_ItemChangeRes.GetWeaponRes(
        Item[nIdx].GetDetailType(), Item[nIdx].GetParticular(),
        Item[nIdx].GetLevel());
  ...
  case itempart_fifong:
    // Npc[nNpcIdx].m_FifongType =
    // g_ItemChangeRes.GetFifongRes(Item[nIdx].GetParticular(),
    // Item[nIdx].GetLevel()); g_DebugLog("Test Kind %d
    // ",Item[m_EquipItem[itempart_body]].GetKind());
    if (m_EquipItem[itempart_body] != 0) {
      if (Npc[nNpcIdx].m_nSex == 0) {
        if (Item[m_EquipItem[itempart_body]].GetLevel() < 6)
          Npc[nNpcIdx].m_FifongType = 24; // Do
        else if (Item[m_EquipItem[itempart_body]].GetLevel() >= 6 &&
                 Item[m_EquipItem[itempart_body]].GetLevel() < 9)
          Npc[nNpcIdx].m_FifongType = 22;
        else
          Npc[nNpcIdx].m_FifongType = 23; // Rach 24 Do 23 Den 22

        if (Item[m_EquipItem[itempart_body]].GetKind() == 1 &&
            Item[m_EquipItem[itempart_body]].GetDetailType() == 2) {
    ...
    if (Npc[nNpcIdx].m_FifongType > 0 && m_EquipItem[itempart_body] != 0 &&
        nIdx != 0) {
      Npc[nNpcIdx].m_bFifongState = TRUE;
    } else {
      Npc[nNpcIdx].m_FifongType = 0;
    ...
    break;
  case itempart_horse:
    Npc[nNpcIdx].m_HorseType = g_ItemChangeRes.GetHorseRes(
        Item[nIdx].GetParticular(), Item[nIdx].GetLevel());
...
void KItemList::CheckDoGroupGoldItem() {
  ...
  for (int l = 0; l < itempart_num; l++) {
    ...
    else if (l == itempart_head || l == itempart_body || l == itempart_belt ||
             l == itempart_weapon || l == itempart_foot || l == itempart_cuff) {
    ...
    int GroupItemActiveCount =
        Item[m_EquipItem[l]].m_SpecialParam.uItemGroupCount;
    ...
    if (GroupItemActive > 0 && GroupItemActiveCount > 0) {
      ...
      for (int m = l + 1; m < itempart_num; m++) {
        ...
        else if (m == itempart_head || m == itempart_body ||
                 m == itempart_belt || m == itempart_weapon ||
                 m == itempart_foot || m == itempart_cuff) {
        ...
        if (GroupItemActive == GroupItemFind &&
            (ItemParticularFind != ItemParticularActive ||
             ItemDetailActive != ItemDetailFind)) {
          ...
          if (ItemDetailFind == 3) {
            for (int u = l + 1; u < m; u++) {
              ...
              else if (u == itempart_head || u == itempart_body ||
                       u == itempart_belt || u == itempart_weapon ||
                       u == itempart_foot || u == itempart_cuff) {
              ...
              int GroupItemFindC =
                  Item[m_EquipItem[u]].m_SpecialParam.uItemGroup;
              int ItemDetailFindC = Item[m_EquipItem[u]].GetDetailType();
              int ItemParticularFindC = Item[m_EquipItem[u]].GetParticular();

              if (GroupItemFindC == GroupItemFind &&
                  ItemDetailFindC == ItemDetailFind &&
                  ItemParticularFindC == ItemParticularFind) {
                bCheckC = TRUE;
                break;
              }
            }
  ...
  if (m_KickAllMacatric != KickAllMacatricActive) {
    ...
    for (int i = 0; i < itempart_num; i++) {
      ...
      if (i == itempart_horse) {
        if (Npc[nNpcIdx].m_bRideHorse) {
          Item[m_EquipItem[i]].RemoveMagicAttribFromNPC(&Npc[nNpcIdx],
                                                        GetEquipEnhance(i));
        }
      }

      else if (i == itempart_head || i == itempart_body || i == itempart_belt ||
               i == itempart_weapon || i == itempart_foot ||
               i == itempart_cuff) {
    ...
    for (int j = 0; j < itempart_num; j++) {
      ...
      if (j == itempart_horse) {
        if (Npc[nNpcIdx].m_bRideHorse) {
          Item[m_EquipItem[j]].ApplyMagicAttribToNPC(&Npc[nNpcIdx],
                                                     GetEquipEnhance(j));
        }
      }

      else if (j == itempart_head || j == itempart_body || j == itempart_belt ||
               j == itempart_weapon || j == itempart_foot ||
               j == itempart_cuff) {
  ...
  for (int p = 0; p < itempart_num; p++) {
    ...
    else if (p == itempart_head || p == itempart_body || p == itempart_belt ||
             p == itempart_weapon || p == itempart_foot || p == itempart_cuff) {

      if (!Item[m_EquipItem[p]].GetDurability())
        continue;
    }

    for (int q = 0; q < Item[m_EquipItem[p]].m_SpecialParam.uItemGroupCount;
         q++) {
      ...
      for (int k = 0; k < itempart_num; k++) {
        if (!m_EquipItem[k] ||
            Item[m_EquipItem[k]].m_SpecialParam.uItemType != 1)
          continue;
        ...
        else if (k == itempart_head || k == itempart_body ||
                 k == itempart_belt || k == itempart_weapon ||
                 k == itempart_foot || k == itempart_cuff) {

          if (!Item[m_EquipItem[k]].GetDurability())
            continue;
        }

        if (Item[m_EquipItem[p]].m_GroupItemNameInfo[q][1] ==
                Item[m_EquipItem[k]].GetDetailType() &&
            Item[m_EquipItem[p]].m_GroupItemNameInfo[q][2] ==
                Item[m_EquipItem[k]].GetParticular()) {
...
void KItemList::InfectionNextEquip(int nEquipPlace, BOOL bEquip /* = FALSE */) {
  ...
  for (int i = 0; i < MAX_ITEM_ACTIVE; i++) {
    ...
    else if (nInfectionEquip == itempart_head ||
             nInfectionEquip == itempart_body ||
             nInfectionEquip == itempart_belt ||
             nInfectionEquip == itempart_weapon ||
             nInfectionEquip == itempart_foot ||
             nInfectionEquip == itempart_cuff) {
    ...
    if (g_IsAccrue(Item[m_EquipItem[nEquipPlace]].GetSeries(),
      ...
      _ASSERT(nActive > 0);
      if (bEquip)
        Item[m_EquipItem[nInfectionEquip]].ApplyHiddenMagicAttribToNPC(
            &Npc[nNpcIdx], nActive);
      else
        Item[m_EquipItem[nInfectionEquip]].RemoveHiddenMagicAttribFromNPC(
            &Npc[nNpcIdx], nActive);
    }
  }
}
...
BOOL KItemList::UnEquip(int nIdx, int nPos /* = -1*/) {
  ...
  if (i == itempart_head || i == itempart_body || i == itempart_belt ||
...
int KItemList::GetEquipEnhance(int nPlace) {
  ...
  for (int i = 0; i < MAX_ITEM_ACTIVE; i++) {
    ...
    if (nActivedEquip) {
      ...
      else if (ms_ActivedEquip[nPlace][i] == itempart_head ||
               ms_ActivedEquip[nPlace][i] == itempart_body ||
               ms_ActivedEquip[nPlace][i] == itempart_belt ||
               ms_ActivedEquip[nPlace][i] == itempart_weapon ||
               ms_ActivedEquip[nPlace][i] == itempart_foot ||
               ms_ActivedEquip[nPlace][i] == itempart_cuff) {
...
int KItemList::SearchID(int nID) {
  ...
  while (1) {
    ...
    if (!nIdx)
      break;
    if (Item[m_Items[nIdx].nIdx].GetID() > 0 &&
        Item[m_Items[nIdx].nIdx].GetID() == (DWORD)nID)
...
void KItemList::ExchangeMoney(int pos1, int pos2, int nMoney) {
  ...
  if (Player[m_PlayerIdx].m_bBlock && pos1 == room_repository &&
      pos2 == room_equipment)
    return;

  // #ifdef _SERVER
  //    if (Npc[Player[m_PlayerIdx].m_nIndex].m_SubWorldIndex ==
...
int KItemList::GetMoneyAmount() {
  return (m_Room[room_equipment].GetMoney() +
          m_Room[room_repository].GetMoney());
}

//----------------------------------------------------------------------
//	¹¦ÄÜ£ºµÃµ½ÎïÆ·À¸ºÍ´¢ÎïÏäµÄÇ®Êý
//----------------------------------------------------------------------
int KItemList::GetEquipmentMoney() { return m_Room[room_equipment].GetMoney(); }
...
void KItemList::TradeMoveMoney(int nMoney) {
  ...
  g_pServer->PackDataToClient(
      ...
      (BYTE *)&sMoney, sizeof(TRADE_MONEY_SYNC));
}
#endif
...
void KItemList::SendMoneySync() {
  ...
  g_pServer->PackDataToClient(Player[m_PlayerIdx].m_nNetConnectIdx,
                              (BYTE *)&sMoney, sizeof(PLAYER_MONEY_SYNC));
}
#endif
...
void KItemList::ExchangeItem(ItemPos *SrcPos, ItemPos *DesPos) {
  ...
  if (Player[m_PlayerIdx].m_bBlock) {
    KPlayerChat::SendSystemInfo(
        1, m_PlayerIdx, MESSAGE_SYSTEM_ANNOUCE_HEAD,
        "Ch­a më khãa r­¬ng, kh«ng thÓ di chuyÓn vËt phÈm !",
        strlen("Ch­a më khãa r­¬ng, kh«ng thÓ di chuyÓn vËt phÈm !"));
  ...
  case pos_hand:
    g_DebugLog("%s exchange item error",
               Npc[Player[m_PlayerIdx].m_nIndex].Name);
    return;
    break;
  ...
  case pos_equip:
    if (Player[this->m_PlayerIdx].CheckTrading()) // Èç¹ûÕýÔÚ½»Ò×
      return;
    if (SrcPos->nX < 0 || SrcPos->nX >= itempart_num || DesPos->nX < 0 ||
        DesPos->nX >= itempart_num)
      return;

#ifdef _SERVER
    if (SrcPos->nX != DesPos->nX) {
      printf("Hack ExchangeItem pos_equip 1 [%s] [%s]\n",
             Player[m_PlayerIdx].m_AccoutName,
             Player[m_PlayerIdx].m_PlayerName);
      return;
    }
#endif
  ...
  case pos_pgbox:
    ...
    if (Player[this->m_PlayerIdx].CheckTrading()) // Èç¹ûÕýÔÚ½»Ò×
      return;
    if (SrcPos->nX < 0 || SrcPos->nX >= MAX_PGBOX_ITEM || DesPos->nX < 0 ||
        DesPos->nX >= MAX_PGBOX_ITEM)
      return;
    ...
    if (m_Hand) {
      ...
      if (SrcPos->nX >= 0 && SrcPos->nX <= 2) {
        ...
        if (Item[m_Hand].GetGenre() != item_task ||
          ...
          KPlayerChat::SendSystemInfo(
              1, m_PlayerIdx, MESSAGE_SYSTEM_ANNOUCE_HEAD,
              "VËt phÈm kh«ng thÝch hîp, kh«ng thÓ ®Æt !",
              strlen("VËt phÈm kh«ng thÝch hîp, kh«ng thÓ ®Æt !"));
          return;
        }
        ...
        if (Item[m_Hand].GetGenre() != item_equip
            ...
            (Item[m_Hand].GetDetailType() != 2 &&
             ...
             Item[m_Hand].GetDetailType() != 8 &&
             Item[m_Hand].GetDetailType() != 7 &&
             Item[m_Hand].GetDetailType() != 0 &&
             Item[m_Hand].GetDetailType() != 1) ||
        ...
        if (Item[m_Hand].GetGenre() != item_task ||
            Item[m_Hand].GetDetailType() < 28 ||
            (Item[m_Hand].GetDetailType() > 30 &&
             Item[m_Hand].GetDetailType() < 173) ||
      ...
      /*
                              ...
                              if (Item[m_Hand].GetGenre() != item_equip
                                      ||  Item[m_Hand].GetVersion() > 1
                                      || (Item[m_Hand].GetDetailType() != 2 &&
         Item[m_Hand].GetDetailType() != 6 && Item[m_Hand].GetDetailType() != 5
         && Item[m_Hand].GetDetailType() != 8 && Item[m_Hand].GetDetailType() !=
         7  && Item[m_Hand].GetDetailType() != 0 && Item[m_Hand].GetDetailType()
         != 1)
                                      || Item[m_Hand].m_SpecialParam.uItemType
         != 2
                                      )
                              {

                  KPlayerChat::SendSystemInfo(1, m_PlayerIdx,
         MESSAGE_SYSTEM_ANNOUCE_HEAD, "VËt phÈm kh«ng thÝch hîp, kh«ng thÓ ®Æt
         !", strlen("VËt phÈm kh«ng thÝch hîp, kh«ng thÓ ®Æt !") ); return;
                              }
...
#ifdef _SERVER
        printf("Loi ExchangeItem pos_pgbox hand khong thuoc nguoi choi [%s] "
               "[%s]\n",
               Player[m_PlayerIdx].m_AccoutName,
               Player[m_PlayerIdx].m_PlayerName);
#endif
  ...
  case pos_immediacy:
    ...
    if (m_Hand) {
      if (m_Room[room_immediacy].CheckSameDetailType(
              Item[m_Hand].GetGenre(), Item[m_Hand].GetDetailType())) {
#ifdef _SERVER
    ...
    // ÏÈ°Ñ¶«Î÷¼ðÆðÀ´
    if (nEquipIdx1) {
      if (!m_Room[room_immediacy].PickUpItem(nEquipIdx1, SrcPos->nX, SrcPos->nY,
                                             Item[nEquipIdx1].GetWidth(),
                                             Item[nEquipIdx1].GetHeight()))
        return;
    }

    // Èç¹ûÊó±êÉÏÓÐ¶«Î÷£¬ÏÈÊÔÊÔÄÜ²»ÄÜ°ÑËû·ÅÏÂÈ¥£¬Èç¹û²»ÐÐµÄ»°£¬»¹µÃ°ÑÔ­À´µÄ¶«Î÷·Å»ØÈ¥
    if (m_Hand) {
      if ((Item[m_Hand].GetGenre() == item_medicine ||
           Item[m_Hand].GetGenre() == item_townportal ||
           (Item[m_Hand].GetGenre() == item_task && !Item[m_Hand].IsStack())) &&
          m_Room[room_immediacy].PlaceItem(DesPos->nX, DesPos->nY, m_Hand,
                                           Item[m_Hand].GetWidth(),
                                           Item[m_Hand].GetHeight())) {
        int nListIdx = FindSame(m_Hand);
      ...
      // °ÑÔ­À´µÄ¶«Î÷·Å»ØÈ¥
        ...
        m_Room[room_immediacy].PlaceItem(SrcPos->nX, SrcPos->nY, nEquipIdx1,
                                         Item[nEquipIdx1].GetWidth(),
                                         Item[nEquipIdx1].GetHeight());
      }
    }
  ...
  case pos_repositoryroom:
    ...
    //	            if (g_pServer)
    //		        g_pServer->PackDataToClient(Player[m_PlayerIdx].m_nNetConnectIdx,
    //&IDC, sizeof(PLAYER_ITEM_UPDATE_VERSION));
    ...
    // ÏÈ°Ñ¶«Î÷¼ðÆðÀ´
    if (nEquipIdx1) {
      if (!m_Room[room_repository].PickUpItem(
              nEquipIdx1, SrcPos->nX, SrcPos->nY, Item[nEquipIdx1].GetWidth(),
              Item[nEquipIdx1].GetHeight()))
        return;
    }
    ...
    // Èç¹ûÊó±êÉÏÓÐ¶«Î÷£¬ÏÈÊÔÊÔÄÜ²»ÄÜ°ÑËû·ÅÏÂÈ¥£¬Èç¹û²»ÐÐµÄ»°£¬»¹µÃ°ÑÔ­À´µÄ¶«Î÷·Å»ØÈ¥
    if (m_Hand) {
      if (m_Room[room_repository].PlaceItem(DesPos->nX, DesPos->nY, m_Hand,
                                            Item[m_Hand].GetWidth(),
                                            Item[m_Hand].GetHeight())) {
      ...
      // °ÑÔ­À´µÄ¶«Î÷·Å»ØÈ¥
        ...
        m_Room[room_repository].PlaceItem(SrcPos->nX, SrcPos->nY, nEquipIdx1,
                                          Item[nEquipIdx1].GetWidth(),
                                          Item[nEquipIdx1].GetHeight());
      }
    }
  ...
  case pos_rpnew:
    ...
    // ÏÈ°Ñ¶«Î÷¼ðÆðÀ´
    if (nEquipIdx1) {
      if (!m_Room[room_rpnew].PickUpItem(nEquipIdx1, SrcPos->nX, SrcPos->nY,
                                         Item[nEquipIdx1].GetWidth(),
                                         Item[nEquipIdx1].GetHeight()))
        return;
    }
    ...
    // Èç¹ûÊó±êÉÏÓÐ¶«Î÷£¬ÏÈÊÔÊÔÄÜ²»ÄÜ°ÑËû·ÅÏÂÈ¥£¬Èç¹û²»ÐÐµÄ»°£¬»¹µÃ°ÑÔ­À´µÄ¶«Î÷·Å»ØÈ¥
    if (m_Hand) {
      if (m_Room[room_rpnew].PlaceItem(DesPos->nX, DesPos->nY, m_Hand,
                                       Item[m_Hand].GetWidth(),
                                       Item[m_Hand].GetHeight())) {
      ...
      // °ÑÔ­À´µÄ¶«Î÷·Å»ØÈ¥
        ...
        m_Room[room_rpnew].PlaceItem(SrcPos->nX, SrcPos->nY, nEquipIdx1,
                                     Item[nEquipIdx1].GetWidth(),
                                     Item[nEquipIdx1].GetHeight());
      }
    }
  ...
  case pos_checkbox:
    ...
    // ÏÈ°Ñ¶«Î÷¼ðÆðÀ´
    if (nEquipIdx1) {
      if (!m_Room[room_checkbox].PickUpItem(nEquipIdx1, SrcPos->nX, SrcPos->nY,
                                            Item[nEquipIdx1].GetWidth(),
                                            Item[nEquipIdx1].GetHeight()))
        return;
    }
    ...
    // Èç¹ûÊó±êÉÏÓÐ¶«Î÷£¬ÏÈÊÔÊÔÄÜ²»ÄÜ°ÑËû·ÅÏÂÈ¥£¬Èç¹û²»ÐÐµÄ»°£¬»¹µÃ°ÑÔ­À´µÄ¶«Î÷·Å»ØÈ¥
    if (m_Hand) {
      if (m_Room[room_checkbox].PlaceItem(DesPos->nX, DesPos->nY, m_Hand,
                                          Item[m_Hand].GetWidth(),
                                          Item[m_Hand].GetHeight())) {
      ...
      // °ÑÔ­À´µÄ¶«Î÷·Å»ØÈ¥
        ...
        m_Room[room_checkbox].PlaceItem(SrcPos->nX, SrcPos->nY, nEquipIdx1,
                                        Item[nEquipIdx1].GetWidth(),
                                        Item[nEquipIdx1].GetHeight());
      }
    }
  ...
  case pos_equiproom:
    ...
    // ÏÈ°Ñ¶«Î÷¼ðÆðÀ´
    if (nEquipIdx1) {
      if (!m_Room[room_equipment].PickUpItem(nEquipIdx1, SrcPos->nX, SrcPos->nY,
                                             Item[nEquipIdx1].GetWidth(),
                                             Item[nEquipIdx1].GetHeight()))
        return;
    }
    // Èç¹ûÊó±êÉÏÓÐ¶«Î÷£¬ÏÈÊÔÊÔÄÜ²»ÄÜ°ÑËû·ÅÏÂÈ¥£¬Èç¹û²»ÐÐµÄ»°£¬»¹µÃ°ÑÔ­À´µÄ¶«Î÷·Å»ØÈ¥
    if (m_Hand) {
      if (m_Room[room_equipment].PlaceItem(DesPos->nX, DesPos->nY, m_Hand,
                                           Item[m_Hand].GetWidth(),
                                           Item[m_Hand].GetHeight())) {
      ...
      // °ÑÔ­À´µÄ¶«Î÷·Å»ØÈ¥
        ...
        m_Room[room_equipment].PlaceItem(SrcPos->nX, SrcPos->nY, nEquipIdx1,
                                         Item[nEquipIdx1].GetWidth(),
                                         Item[nEquipIdx1].GetHeight());
      }
    }
  ...
  case pos_traderoom:
    ...
    if (m_Hand &&
        (Item[m_Hand].GetGenre() == item_equip ||
         Item[m_Hand].GetGenre() == item_medicine) &&
    ...
    if (m_Hand) {
      ...
      {
        if (!m_Room[room_trade].PickUpItem(nEquipIdx1, SrcPos->nX, SrcPos->nY,
                                           Item[nEquipIdx1].GetWidth(),
                                           Item[nEquipIdx1].GetHeight()))
          return;
      }

      if (m_Room[room_trade].PlaceItem(DesPos->nX, DesPos->nY, m_Hand,
                                       Item[m_Hand].GetWidth(),
                                       Item[m_Hand].GetHeight())) {
        int nListIdx = FindSame(m_Hand);
        ...
        if (nEquipIdx1) // ½»Ò×À¸ÓÐ¶«Î÷±»¼ñÆð
        {
          ...
          g_pServer->PackDataToClient(
              Player[Player[m_PlayerIdx].m_cTrade.m_nTradeDest]
                  .m_nNetConnectIdx,
              (BYTE *)&sRemove, sizeof(ITEM_REMOVE_SYNC));
        }
        // ÊÖÉÏµÄ¶«Î÷·ÅÔÚÁË½»Ò×À¸
        ...
        // End
        for (int j = 0; j < 6; j++)
          sItem.m_MagicLevel[j] =
              Item[m_Hand].m_GeneratorParam.nGeneratorLevel[j];
      ...
      // °ÑÔ­À´µÄ¶«Î÷·Å»ØÈ¥
        ...
        m_Room[room_trade].PlaceItem(SrcPos->nX, SrcPos->nY, nEquipIdx1,
                                     Item[nEquipIdx1].GetWidth(),
                                     Item[nEquipIdx1].GetHeight());
      }
    }
...
BOOL KItemList::AutoMoveItem(ItemPos SrcPos, ItemPos DesPos) {
  ...
  case pos_equiproom: {
    ...
    case pos_immediacy: {
      ...
      if (!m_Room[room_equipment].PickUpItem(nIdx, SrcPos.nX, SrcPos.nY,
                                             Item[nIdx].GetWidth(),
                                             Item[nIdx].GetHeight()))
        return FALSE;
      m_Room[room_immediacy].PlaceItem(DesPos.nX, DesPos.nY, nIdx,
                                       Item[nIdx].GetWidth(),
                                       Item[nIdx].GetHeight());
...
BOOL KItemList::EatMecidine(int nPlace, int nX, int nY) {
  ...
  case pos_immediacy:
    ...
    if (nItemIdx > 0) {
      ...
      if (FALSE == FindSameDetailTypeInEquipment(nGenre, nDetailType, &nIdx,
                                                 &nXpos, &nYpos))
        return bEat;
...
int KItemList::FindNumberInAll(int nIdx) {
  ...
  nNumberArray +=
      m_Room[room_immediacy].FindNumberArrayItem(nIdx) * nWidth * nHeight;
...
void KItemList::BackupTrade() {
  if (!m_Room[room_tradeback].m_pArray)
    m_Room[room_tradeback].Init(m_Room[room_equipment].m_nWidth,
                                m_Room[room_equipment].m_nHeight);
  memcpy(m_Room[room_tradeback].m_pArray, m_Room[room_equipment].m_pArray,
         sizeof(int) * m_Room[room_tradeback].m_nWidth *
             m_Room[room_tradeback].m_nHeight);

  memcpy(this->m_sBackItems, this->m_Items,
         sizeof(PlayerItem) * MAX_PLAYER_ITEM);

  m_nBackHand = m_Hand;
}

void KItemList::RecoverTrade() {
  memcpy(m_Room[room_equipment].m_pArray, m_Room[room_tradeback].m_pArray,
         sizeof(int) * m_Room[room_tradeback].m_nWidth *
             m_Room[room_tradeback].m_nHeight);
...
#ifdef _SERVER
int KItemList::GetItemIdxInCheckBoxByNum(int nGenre, int nDetailType,
                                         int nParticulType, int nLevel,
                                         int nNum) {
  int nNo = 0;
  int nIdx = 0;
  while ((nIdx = m_UseIdx.GetNext(nIdx))) {
...
#ifdef _SERVER
int KItemList::CheckDelItemPoison(int nGenre, int nDetailType,
                                  int nParticulType, int nLevel) {
  int nNo = 0;
  int nIdx = 0;
  while ((nIdx = m_UseIdx.GetNext(nIdx))) {
...
int KItemList::CheckTrangSucInBox() {
  ...
  while ((nIdx = m_UseIdx.GetNext(nIdx))) {
    ...
    if (Item[nGameIdx].GetGenre() == 0 &&
        (Item[nGameIdx].GetDetailType() == 3 ||
         Item[nGameIdx].GetDetailType() == 4 ||
         Item[nGameIdx].GetDetailType() == 9))
      continue;
...
#ifdef _SERVER
int KItemList::GetItemInCheckBoxNum(int nGenre, int nDetailType,
                                    int nParticulType, int nLevel) {
  int nNo = 0;
  int nIdx = 0;
  while ((nIdx = m_UseIdx.GetNext(nIdx))) {
...
#ifdef _SERVER
int KItemList::CheckItemCountAll(int nGenre, int nDetailType, int nParticulType,
                                 int nLevel) {
  int nNo = 0;
  int nIdx = 0;
  while ((nIdx = m_UseIdx.GetNext(nIdx))) {
...
int KItemList::CheckTimeItemCountAndGold(int nTimeItem) {
  ...
  while ((nIdx = m_UseIdx.GetNext(nIdx))) {
    ...
    if ((Item[nGameIdx].GetDetailType() == 4 &&
         Item[nGameIdx].GetParticular() < 6) ||
        (Item[nGameIdx].GetDetailType() == 2 &&
         Item[nGameIdx].GetParticular() < 33) ||
        (Item[nGameIdx].GetDetailType() == 6 &&
         Item[nGameIdx].GetParticular() < 3) ||
        (Item[nGameIdx].GetDetailType() == 5 &&
         Item[nGameIdx].GetParticular() < 5) ||
        (Item[nGameIdx].GetDetailType() == 8 &&
         Item[nGameIdx].GetParticular() < 3) ||
        (Item[nGameIdx].GetDetailType() == 7 &&
         Item[nGameIdx].GetParticular() < 16) ||
        (Item[nGameIdx].GetDetailType() == 9 &&
         Item[nGameIdx].GetParticular() < 6) ||
        (Item[nGameIdx].GetDetailType() == 3 &&
         Item[nGameIdx].GetParticular() > 27) ||
        (Item[nGameIdx].GetDetailType() == 3 &&
         Item[nGameIdx].GetParticular() < 7))
      continue;
...
#ifdef _SERVER
int KItemList::CheckItemBlockInCheckBox(int nGenre, int nDetailType,
                                        int nParticulType, int nLevel) {
  int nNo = 0;
  int nIdx = 0;
  while ((nIdx = m_UseIdx.GetNext(nIdx))) {
...
void KItemList::UpdateItem(int nIdx) {
  ...
  if (g_pServer)
    g_pServer->PackDataToClient(Player[m_PlayerIdx].m_nNetConnectIdx,
                                (BYTE *)&pParam, sizeof(S2C_SYNC_ITEM_PARAM));
}
...
void KItemList::GetTradeRoomItemInfo() {
  ...
  while (1) {
    nItemIdx =
        m_Room[room_trade].GetNextItem(nItemIdx, nXpos, nYpos, &nXpos, &nYpos);
    if (nItemIdx == 0)
      break;
  ...
  for (int i = nPos - 1; i >= 0; i--) {
    for (int j = 0; j < i; j++) {
      if (ItemSet.m_psItemInfo[j].m_nWidth * ItemSet.m_psItemInfo[j].m_nHeight <
          ItemSet.m_psItemInfo[j + 1].m_nWidth *
              ItemSet.m_psItemInfo[j + 1].m_nHeight) {
...
BOOL KItemList::TradeCheckCanPlace() {
  ...
  memcpy(pnTempRoom, m_Room[room_equipment].m_pArray,
         sizeof(int) * EQUIPMENT_ROOM_WIDTH * EQUIPMENT_ROOM_HEIGHT);
  ...
  for (nPos = 0; nPos < TRADE_ROOM_WIDTH * TRADE_ROOM_HEIGHT; nPos++) {
    ...
    nFind = 0;
    for (i = 0;
         i < EQUIPMENT_ROOM_HEIGHT - ItemSet.m_psItemInfo[nPos].m_nHeight + 1;
         i++) {
      for (j = 0;
           j < EQUIPMENT_ROOM_WIDTH - ItemSet.m_psItemInfo[nPos].m_nWidth + 1;
           j++) {
        nNext = 0;
        ...
        if (!nNext) {
          ...
          for (a = 0; a < ItemSet.m_psItemInfo[nPos].m_nHeight; a++) {
            for (b = 0; b < ItemSet.m_psItemInfo[nPos].m_nWidth; b++)
              pnTempRoom[(i + a) * EQUIPMENT_ROOM_WIDTH + j + b] =
...
#ifdef _SERVER
//--------------------------------------------------------------------------
//	¹¦ÄÜ£ºÅÐ¶ÏÒ»¶¨³¤¿íµÄÎïÆ·ÄÜ·ñ·Å½øÎïÆ·À¸
...
BOOL KItemList::CheckItemEquipMateBoy() {
  ...
  for (int i = 0; i < MAX_PLAYER_ITEM; i++) {
    ...
    if (nIdx > 0 && nIdx < MAX_ITEM && nPlace == pos_equip && nItemGenre == 0 &&
        nDetailType == 2 && nParticularType == 28 && nLevel == 1)
      return TRUE;
  }
...
BOOL KItemList::CheckItemEquipMateGirl() {
  ...
  for (int i = 0; i < MAX_PLAYER_ITEM; i++) {
    ...
    if (nIdx > 0 && nIdx < MAX_ITEM && nPlace == pos_equip && nItemGenre == 0 &&
        nDetailType == 2 && nParticularType == 28 && nLevel == 4)
      return TRUE;
  }
...
BOOL KItemList::CheckItemEquipMateGuest() {
  ...
  for (int i = 0; i < MAX_PLAYER_ITEM; i++) {
    ...
    if (nIdx > 0 && nIdx < MAX_ITEM && nPlace == pos_equip && nItemGenre == 0 &&
        nDetailType == 2 && nParticularType == 28 &&
        (nLevel == 7 || nLevel == 8))
      return TRUE;
  }
...
BOOL KItemList::CheckCanPlaceInEquipment(int nWidth, int nHeight, int *pnX,
                                         int *pnY) {
...
//------------------------------------------------------------------------------
BOOL KItemList::FindSameDetailTypeInEquipment(int nGenre, int nDetail,
                                              int *pnIdx, int *pnX, int *pnY) {
  return m_Room[room_equipment].FindSameDetailType(nGenre, nDetail, pnIdx, pnX,
                                                   pnY);
}
...
//------------------------------------------------------------------------------
BOOL KItemList::AutoMoveMedicine(int nItemIdx, int nSrcX, int nSrcY, int nDestX,
                                 int nDestY) {
  if (!m_Room[room_equipment].m_pArray || !m_Room[room_immediacy].m_pArray)
    return FALSE;
  if (nSrcX < 0 || nSrcX >= m_Room[room_equipment].m_nWidth || nSrcY < 0 ||
      nSrcY >= m_Room[room_equipment].m_nHeight)
    return FALSE;
  if (nDestX < 0 || nDestX >= m_Room[room_immediacy].m_nWidth || nDestY < 0 ||
      nDestY >= m_Room[room_immediacy].m_nHeight)
    return FALSE;
  if (nItemIdx !=
          m_Room[room_equipment]
              .m_pArray[nSrcY * m_Room[room_equipment].m_nWidth + nSrcX] ||
      0 != m_Room[room_immediacy]
               .m_pArray[nDestY * m_Room[room_immediacy].m_nWidth + nDestX])
    return FALSE;

  _ASSERT(Item[nItemIdx].GetWidth() == 1 && Item[nItemIdx].GetHeight() == 1);
  if (!m_Room[room_equipment].PickUpItem(nItemIdx, nSrcX, nSrcY,
                                         Item[nItemIdx].GetWidth(),
                                         Item[nItemIdx].GetHeight()))
    return FALSE;
  if (!m_Room[room_immediacy].PlaceItem(nDestX, nDestY, nItemIdx,
                                        Item[nItemIdx].GetWidth(),
                                        Item[nItemIdx].GetHeight())) {
    m_Room[room_equipment].PlaceItem(nSrcX, nSrcY, nItemIdx,
                                     Item[nItemIdx].GetWidth(),
                                     Item[nItemIdx].GetHeight());
    return FALSE;
  }
  ...
  if (g_pServer)
    g_pServer->PackDataToClient(Player[m_PlayerIdx].m_nNetConnectIdx,
                                (BYTE *)&sMove, sizeof(ITEM_AUTO_MOVE_SYNC));
...
void KItemList::RemoveAllInOneRoom(int nRoom) {
  ...
  while (1) {
    nItemIdx =
        m_Room[nRoom].GetNextItem(nItemIdx, nXpos, nYpos, &nXpos, &nYpos);
    if (nItemIdx == 0)
      break;
...
void KItemList::AutoLoseItemFromEquipmentRoom(int nRate) {
  ...
  while (1) {
    nItemIdx = m_Room[room_equipment].GetNextItem(nItemIdx, nXpos, nYpos,
                                                  &nXpos, &nYpos);
    if (nItemIdx == 0)
      break;
  ...
  for (int i = 0; i < nPos; i++) {
    ...
    if (Item[nItemIdx].m_nPriceShop) {
      ...
      if (g_pServer)
        g_pServer->PackDataToClient(Player[m_PlayerIdx].m_nNetConnectIdx, &IDC,
                                    sizeof(PLAYER_UPDATE_SHOP_PRICE_COMMAND));
    }
    ...
    if (Remove(nItemIdx)) {
      ...
      sMsg.m_wLength =
          sizeof(SHOW_MSG_SYNC) - 1 - sizeof(LPVOID) + sizeof(sInfo.m_szName);
      ...
      memcpy((char *)sMsg.m_lpBuf + sizeof(SHOW_MSG_SYNC) - sizeof(LPVOID),
             sInfo.m_szName, sizeof(sInfo.m_szName));
      g_pServer->PackDataToClient(Player[m_PlayerIdx].m_nNetConnectIdx,
...
void KItemList::AutoLoseEquip() {
  ...
  for (i = 0; i < itempart_num; i++) {
    ...
    if (((Item[m_EquipItem[i]].GetGenre() == item_equip ||
          Item[m_EquipItem[i]].GetGenre() == item_medicine) &&
         Item[m_EquipItem[i]].m_GeneratorParam.nVersion > 1) ||
        (Item[m_EquipItem[i]].GetGenre() == item_task &&
         Item[m_EquipItem[i]].GetBindItem() >= 1))
      continue;
...
int KItemList::GetSameDetailItemNum(int nImmediatePos) {
  if (nImmediatePos < 0 ||
      nImmediatePos >= IMMEDIACY_ROOM_WIDTH * IMMEDIACY_ROOM_HEIGHT)
    return 0;
  int nIdx = m_Room[room_immediacy].FindItem(nImmediatePos, 0);
  if (nIdx <= 0)
    return 0;
  return m_Room[room_equipment].CalcSameDetailType(Item[nIdx].GetGenre(),
                                                   Item[nIdx].GetDetailType()) +
         1;
}
#endif
      ...
      if (nDur == 0) {
...

        {
...

          {
            ...
            if (i == itempart_head || i == itempart_body ||
                i == itempart_belt || i == itempart_weapon ||
                i == itempart_foot || i == itempart_cuff) {
              Item[nItemIdx].RemoveMagicAttribFromNPC(&Npc[nNpcIdx],
                                                      GetEquipEnhance(i));
              InfectionNextEquip(i, FALSE);
        ...
        if (g_pServer)
          g_pServer->PackDataToClient(Player[m_PlayerIdx].m_nNetConnectIdx,
                                      &sIDC, sizeof(ITEM_DURABILITY_CHANGE));
...
void KItemList::CheckItemTime() {
  ...
  for (int i = 0; i < MAX_PLAYER_ITEM; i++) {
    ...
    if (Item[m_Items[i].nIdx].GetTime()->bYear)
      ...
      } else if (Item[m_Items[i].nIdx].GetTime()->bYear - 1900 ==
                 timeinfo->tm_year)
        if (Item[m_Items[i].nIdx].GetTime()->bMonth < timeinfo->tm_mon + 1) {
          Remove(m_Items[i].nIdx);
          ItemSet.Remove(m_Items[i].nIdx);
        } else if (Item[m_Items[i].nIdx].GetTime()->bMonth ==
                   timeinfo->tm_mon + 1)
