...
#define TASKVALUE_LIXIAN 102
#define TASK_CHAT_TONGKIM 128
...
public:
  ...
  KTimerTaskFun(void *pOwner) { m_pTimerOwner = pOwner; };
  ...
  unsigned long GetRestTime() const {
    ...
    return (dwGameTime > m_dwTimeTaskTime) ? 0
                                           : (m_dwTimeTaskTime - dwGameTime);
  };

  void GetTimerTaskScript(char *szScriptFileName, unsigned short usTimerTaskId,
