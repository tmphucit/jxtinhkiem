...
public:
  ...
  // 载入障碍数据给小地图
  static void LoadLittleMapData(int nX, int nY, char *lpszPath,
                                BYTE *lpbtObstacle);
#endif
  void Close();
  void Activate();
  ...
  BYTE GetBarrierMin(int nGridX, int nGridY, int nOffX, int nOffY,
                     BOOL bCheckNpc);

  DWORD GetTrap(int MapX, int MapY); //	得到Trap编号
  void AddTrap(int nMapX, int nMapY, char *szSpcrit);
  ...
  void BroadCast(const void *pBuffer, DWORD dwSize, int &nMaxCount, int nX,
                 int nY);
...
inline int KRegion::FindNpc(int nMapX, int nMapY, int nNpcIdx, int nRelation) {
  ...
  while (pNode) {
    if (Npc[pNode->m_nIndex].m_MapX == nMapX &&
        Npc[pNode->m_nIndex].m_MapY == nMapY) {
