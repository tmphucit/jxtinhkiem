...
static KItemDropRate *g_GenItemDropRate(char *szDropIniFile) {
  ...
  if (IniFile.Load(szDropIniFile)) {
    ...
    IniFile.GetInteger("Main", "PropAllScaleMin", 0,
                       &pnewDrop->nPropAllScaleMin);
    IniFile.GetInteger("Main", "PropAllScaleMax", 0,
                       &pnewDrop->nPropAllScaleMax);
    IniFile.GetInteger("Main", "PropAllByTeam", 0, &pnewDrop->nPropAllByTeam);

    IniFile.GetInteger("Main", "MinItemLevelScale", 20,
                       &pnewDrop->nMinItemLevelScale);
    IniFile.GetInteger("Main", "MaxItemLevelScale", 10,
                       &pnewDrop->nMaxItemLevelScale);
    ...
    for (int i = 0; i < pnewDrop->nCount; i++, pItemParam++) {
      ...
      IniFile.GetInteger(szSection, "Particular", 0,
                         &(pItemParam->nParticulType));
...
void KNpcTemplate::InitNpcBaseData(int nNpcTemplateId) {
  ...
  g_NpcSetting.GetInteger(nNpcTempRow, "PoisonResistMax", 0,
                          &m_PoisonResistMax);

  g_NpcSetting.GetInteger(nNpcTempRow, "PhysicsResistMax", 0,
                          &m_PhysicsResistMax);
  ...
  char szDropFile[200];
  g_NpcSetting.GetString(nNpcTempRow, "DropRateFile", "", szDropFile,
                         sizeof(szDropFile));
  strlwr(szDropFile);
  ...
  g_NpcSetting.GetString(nNpcTempRow, "ActionScript", "", ActionScript,
                         sizeof(ActionScript));
#endif
}

void KNpcTemplate::InitNpcLevelData(KTabFile *pKindFile, int nNpcTemplateId,
                                    int nLevel) {
  if (nNpcTemplateId < 0 || nLevel <= 0)
    return;
  int nNpcTempRow = nNpcTemplateId + 2;
...
int KNpcTemplate::GetNpcLevelDataFromScript(KLuaScript *pScript,
                                            char *szDataName, int nLevel,
                                            char *szParam) {
  int nTopIndex = 0;
  int nReturn = 0;
  ...
  pScript->CallFunction("GetNpcLevelData", 1, "dss", nLevel, szDataName,
                        szParam);
...
int KNpcTemplate::GetNpcLevelDataFromScript(KLuaScript *pScript,
                                            char *szDataName, int nLevel,
                                            double nParam1, double nParam2,
                                            double nParam3) {
  int nTopIndex = 0;
  int nReturn = 0;

  pScript->SafeCallBegin(&nTopIndex);
  pScript->CallFunction("GetNpcKeyData", 1, "dsnnn", nLevel, szDataName,
...
int KNpcTemplate::SkillString2Id(char *szSkillString) {
  ...
  for (int i = 0; i < nSkillNum; i++) {
    g_OrdinSkillsSetting.GetString(i + 2, "SkillName", "", szSkillName,
                                   sizeof(szSkillName));
