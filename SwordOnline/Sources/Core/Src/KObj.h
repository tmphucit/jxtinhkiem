...
public:
  int m_nID;           // 物件的世界唯一ID，客户端与服务器端一一对应，如果值为 0
                       // ，此为客户端 obj ，服务器端没有对应的 obj
  int m_nDataID;       // 此物件在物件数据表中的位置(就是在模板中的编号)
  int m_nKind;         // 类型	对应 Obj_Kind_XXXX
  int m_nIndex;        // 在 Object 数组中的位置
  int m_nBelongRegion; // 本物体属于哪个region，不是坐标，坐标可以变动，这个是随产生时那个region，客户端存储的是
                       // region id
...
public:
  ...
  // 凸多边形转换为障碍信息
  void PolygonChangeToBar(KPolygon Polygon,   // 凸多边形
                          int nGridWidth,     // 格子长
                          int nGridHeight,    // 格子宽
                          int nTableWidth,    // 表格长
...
地图动画：保存在地图文件的 obj 部分中，服务器不进行处理，其障碍信息预先存储
          在地图的 obj 障碍信息层里面。在客户端，根据其坐标、spr 文件名进行
                  换帧处理，换帧处理的根据（帧数、方向等信息）存储于 obj 的 ini
文件 中以便动态加载。 循环音效、随机音效：保存在地图文件的 obj
部分中，服务器不进行处理，无障碍信
