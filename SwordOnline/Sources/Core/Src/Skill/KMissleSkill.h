...
{
private:
  int (*CastMissleFunc[SKILL_MF_COUNT])(int nSrcPX, int nSrcPY, int nDesPX,
                                        int nDesPY, TSkillParam *pSkillParam);
...
public:
  ...
  BOOL ParseString2MagicAttrib(MAGIC_ATTRIB MagicAttrib, int nValue1,
                               int nValue2, int nValue3);
  BOOL IsNeedShadow() {
    CheckAndGenSkillData();
    return m_bNeedShadow;
  };
  int GetMaxTimes() {
    CheckAndGenSkillData();
    return m_nMaxTimes;
  ...
  int GetChildSkillNum() {
    CheckAndGenSkillData();
    return m_nChildSkillNum;
  };
  int GetChildSkillId() {
    CheckAndGenSkillData();
    return m_nChildSkillId;
  };
  int GetChildSkillLevel() {
    CheckAndGenSkillData();
    return m_nChildSkillLevel;
  };
  void SetSkillId(int nId) { m_nId = nId; };

  BOOL IsPhysical() {
    CheckAndGenSkillData();
    return m_bIsPhysical;
  };
...
private:
  ...
  int CastWall(int nSrcPX, int nSrcPY, int nRefPX, int nRefPY,
               TSkillParam *pSkillParam);
  int CastLine(int nSrcPX, int nSrcPY, int nRefPX, int nRefPY,
               TSkillParam *pSkillParam);
  int CastSpread(int nSrcPX, int nSrcPY, int nRefPX, int nRefPY,
                 TSkillParam *pSkillParam);
  int CastCircle(int nSrcPX, int nSrcPY, int nRefPX, int nRefPY,
                 TSkillParam *pSkillParam);
  int CastAtTarget(int nSrcPX, int nSrcPY, int nRefPX, int nRefPY,
                   TSkillParam *pSkillParam);
  int CastAtFirer(int nSrcPX, int nSrcPY, int nRefPX, int nRefPY,
                  TSkillParam *pSkillParam);
  int CastZone(int nSrcPX, int nSrcPY, int nRefPX, int nRefPY,
               TSkillParam *pSkillParam);
  int CastExtractiveLineMissle(int nSrcPX, int nSrcPY, int nRefPX, int nRefPY,
                               TSkillParam *pSkillParam);
  void CreateMissle(TSkillParam *pSkillParam, int ChildSkillId,
                    int nMissleIndex);
