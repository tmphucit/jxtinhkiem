...
BOOL KFightSkill::GetInfoFromTabFile(KITabFile *pSkillsSettingFile, int nRow) {
  ...
  // pSkillsSettingFile->GetInteger(nRow, "CharClass",		0,
  // &m_nCharClass, TRUE);
...
// Author		: RomanDou
*****************************************************************************/
BOOL KFightSkill::ParseString2MagicAttrib(MAGIC_ATTRIB MagicAttrib, int nValue1,
                                          int nValue2, int nValue3) {
...
KFightSkill::CreateMissleMagicAttribsData(TGameActorInfo GameActor) {
  if (GameActor.nActorIndex <= 0 || m_bClientSend)
    return NULL;
  ...
  if (GameActor.nActorType == Actor_Npc)
    Npc[GameActor.nActorIndex].AppendSkillEffect(m_DamageAttribs,
                                                 pDamageAttribs);
  else
...
KMissleMagicAttribsData *
KFightSkill::CreateMissleMagicAttribsData(TSkillParam *pSkillParam) {
  if (!pSkillParam)
    return NULL;
...
void KFightSkill::SetSkillLevelData(int nLevel /* =0*/) {
  ...
  char szResult[300];
  // Question 必须顺序一至才行
  g_SkillsSetting.GetString(GetRowIdInSkillTabFile(), "LvlSetScript", "",
  ...
  for (int i = 0; i < MAXSKILLLEVELSETTINGNUM; i++) {
    ...
    g_SkillsSetting.GetString(GetRowIdInSkillTabFile(), szSettingName, "",
                              szSettingNameValue, 100);
    g_SkillsSetting.GetString(GetRowIdInSkillTabFile(), szSettingData, "",
                              szSettingDataValue, 100);

    if (szSettingNameValue[0] == 0 || szSettingScriptName[0] == '0')
      continue;
    {
      Script.CallFunction("GetSkillLevelData", 1, "ssd", szSettingNameValue,
                          szSettingDataValue, nLevel);
      const char *szType =
          lua_typename(Script.m_LuaState, Lua_GetTopIndex(Script.m_LuaState));
      if (Lua_IsNumber(Script.m_LuaState, Lua_GetTopIndex(Script.m_LuaState)) ==
          1) {
        int nResult = (int)Lua_ValueToNumber(
            Script.m_LuaState, Lua_GetTopIndex(Script.m_LuaState));
        sprintf(szResult, "%d", nResult);
      } else if (Lua_IsString(Script.m_LuaState,
                              Lua_GetTopIndex(Script.m_LuaState)) == 1) {
        strcpy(szResult,
               (char *)Lua_ValueToString(Script.m_LuaState,
                                         Lua_GetTopIndex(Script.m_LuaState)));
      } else {
        char szMsg[300];
        sprintf(szMsg, "当获得该技能等级为%d（%s,%s）时，出错，请检查脚本!,",
...
BOOL KFightSkill::ParseString2MagicAttrib(char *szMagicAttribName,
                                          char *szValue) {
  int nValue1 = 0;
  int nValue2 = 0;
  int nValue3 = 0;
  ...
  for (int i = 0; i <= magic_normal_end; i++) {
    if (!strcmp(szMagicAttribName, MagicAttrib2String(i))) {
      ...
      if (!ParseString2MagicAttrib((MAGIC_ATTRIB)i, nValue1, nValue2,
                                   nValue3)) {
