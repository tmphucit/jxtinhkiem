...
// Author		: RomanDou
*****************************************************************************/
void KMissleRes::LoadResource(int nStatus, char *MissleImage,
                              char *MissleSound) {
...
void KMissleRes::Clear() {
  ...
  while (m_SkillSpecialList.GetHead()) {
    KSkillSpecialNode *pNode =
        (KSkillSpecialNode *)m_SkillSpecialList.GetHead();
    pNode->Remove();
    delete pNode;
  }
...
// Author		: RomanDou
*****************************************************************************/
int KMissleRes::Draw(int eStatus, int nX, int nY, int nZ, int nDir,
                     int nAllFrame, int nCurLifeFrame) {
    ...
    if (nSprDir && nSprFrames) {
...

      {
        if (m_bLoopAnim) // 如果是循环播放的话则每帧都换帧
        {
          ...
          {
            nFrame =
                (nCurLifeFrame / m_MissleRes[eStatus].nInterval) % nTotalFrame;
          } else {
            //  未显示到循环播放的开始帧
            if ((nCurLifeFrame / m_MissleRes[eStatus].nInterval) < m_nSubStart)
              ...
              if (m_nSubStart == m_nSubStop)
                ...
                nFrame = m_nSubStart + ((nCurLifeFrame - m_nSubStart) /
                                        m_MissleRes[eStatus].nInterval) %
                                           (m_nSubStop - m_nSubStart);
            }
          }
        } else {
      ...
      m_pSprNode = (KCacheNode *)g_SpriteCache.GetNode(
          m_RUImage[eStatus].szImage, (KCacheNode *)m_pSprNode);
      pSprite = (KSprite *)m_pSprNode->m_lpData;
      if (pSprite) {

        pSprite->DrawAlpha(nX - pSprite->GetCenterX(),
                           nY - pSprite->GetCenterY(), nFrame);
      }
#else
      ...
      /*			if (m_bNeedShadow)
      {
        ...
        pDrawShadowNode = (KShadowNode*)m_ShadowList.GetHead();
              ...
              SubWorld[Missle[m_nMissleId].m_nSubWorldId].Mps2Screen(&nSrcX,
      &nSrcY); pSprite->DrawAlpha(nSrcX- pSprite->GetCenterX(), nSrcY-
      pSprite->GetCenterY(), pDrawShadowNode->Frame, nCurAlpha ); nCurAlpha +=
      (32 / m_nMaxShadowNum) ; pDrawShadowNode =
      (KShadowNode*)pDrawShadowNode->GetNext();
              }
      ...
      */
      ...
      // pSprite->DrawAlpha(nSrcX - pSprite->GetCenterX(), nSrcY -
      // pSprite->GetCenterY(), nFrame);
      /*			if (m_pObj)
      {
      cPoint			stPoint;
...
void KMissleRes::PlaySound(int eStatus, int nLoop, int nPan, int nVal) {
  if (m_MissleRes[eStatus].SndFileName[0] == 0)
    return;
  ...
  m_pSndNode =
      (KCacheNode *)g_SoundCache.GetNode(SndFileName, (KCacheNode *)m_pSndNode);
...
void KMissleRes::StopSound() {
  if (m_MissleRes[m_nLastSndIndex].SndFileName[0] == 0)
    return;

  KWavSound *pSound = NULL;
  m_pSndNode = (KCacheNode *)g_SoundCache.GetNode(
      m_MissleRes[m_nLastSndIndex].SndFileName, (KCacheNode *)m_pSndNode);
  pSound = (KWavSound *)m_pSndNode->m_lpData;
  if (pSound) {
    pSound->Stop();
  }
