...
#define LeftRegion                                                             \
  SubWorld[m_nSubWorldId].m_Region[m_nRegionId].m_nConnectRegion[2]
#define RightRegion                                                            \
  SubWorld[m_nSubWorldId].m_Region[m_nRegionId].m_nConnectRegion[6]
#define UpRegion                                                               \
  SubWorld[m_nSubWorldId].m_Region[m_nRegionId].m_nConnectRegion[4]
#define DownRegion                                                             \
  SubWorld[m_nSubWorldId].m_Region[m_nRegionId].m_nConnectRegion[0]
#define LeftUpRegion                                                           \
  SubWorld[m_nSubWorldId].m_Region[m_nRegionId].m_nConnectRegion[3]
#define LeftDownRegion                                                         \
  SubWorld[m_nSubWorldId].m_Region[m_nRegionId].m_nConnectRegion[1]
#define RightUpRegion                                                          \
  SubWorld[m_nSubWorldId].m_Region[m_nRegionId].m_nConnectRegion[5]
#define RightDownRegion                                                        \
  SubWorld[m_nSubWorldId].m_Region[m_nRegionId].m_nConnectRegion[7]
...
#pragma message(                                                               \
    ATTENTION("子弹消亡时，需更新发送者使用该技能时的当前使用次数，使之减一"))
#ifndef _SERVER
...
BOOL KMissle::GetInfoFromTabFile(KITabFile *pMisslesSetting, int nMissleId) {
  ...
  pMisslesSetting->GetInteger(nRow, "AutoExplode", 0, (int *)&m_bAutoExplode,
                              TRUE);
  ...
  for (int i = 0; i < MAX_MISSLE_STATUS; i++) {
    ...
    sscanf(szAnimFileInfo, "%d,%d,%d", &m_MissleRes.m_MissleRes[i].nTotalFrame,
...
BOOL KMissle::CheckCollision(int nBarrierKind) {
  ...
  pNode = (KIndexNode *)SubWorld[m_nSubWorldId]
              .m_Region[m_nRegionId]
              .m_NpcList.GetHead();
  int nAbsX = 0;
  int nAbsY = 0;
  while (pNode) {
    if (this->m_nCollideRange == 1) {
      if ((nAbsX = abs(Npc[pNode->m_nIndex].m_MapX - m_nCurrentMapX)) <= 1 &&
          (nAbsY = abs(Npc[pNode->m_nIndex].m_MapY - m_nCurrentMapY)) <= 1 &&
        ...
        if (nAbsX || nAbsY) {
          ...
          int itest = 2 * nDY * CellHeight + nDY * nDY + 2 * nDX * CellWidth +
                      nDX * nDX;
          if (2 * nDY * CellHeight + nDY * nDY + 2 * nDX * CellWidth +
                  nDX * nDX >=
              0)
            bCollision = TRUE;
        } else
        ...
        if (bCollision &&
            IsRelationSuitable(NpcSet.GetRelation(pNode->m_nIndex, nLauncher),
                               m_eRelation)) {
          if (m_nDamageRange == 1) //
            ProcessCollision(
                nLauncher, m_nRegionId, Npc[pNode->m_nIndex].m_MapX,
                Npc[pNode->m_nIndex].m_MapY, m_nDamageRange, m_eRelation);
          else
            ProcessCollision(); // 处理碰撞
      ...
      if ((nAbsX = abs(Npc[pNode->m_nIndex].m_MapX - m_nCurrentMapX)) <=
              m_nCollideRange &&
          (nAbsY = abs(Npc[pNode->m_nIndex].m_MapY - m_nCurrentMapY)) <=
              m_nCollideRange &&
          pNode->m_nIndex != nLauncher) {
        if (IsRelationSuitable(NpcSet.GetRelation(pNode->m_nIndex, nLauncher),
                               m_eRelation)) {
          ProcessCollision(); // 处理碰撞
          DoCollision();      // 子弹作碰撞后的效果
          return TRUE;
...
void KMissle::OnFly() {
  ...
  case MISSLE_MMK_Stand: //	原地
  {
    ...
    */
    ...
    int nBarrierKind = SubWorld[m_nSubWorldId].TestBarrier(
        m_nRegionId, m_nCurrentMapX, m_nCurrentMapY, m_nXOffset, m_nYOffset, 0,
        0);
  ...
  case MISSLE_MMK_Line:     //	直线飞行
  {
    ...
    CheckBeyondRegion();
    CheckCollision(nBarrierKind);
    g_DebugLog("r%d x%d, y%d, dx%d, dy%d", m_nRegionId, m_nCurrentMapX,
  ...
  case MISSLE_MMK_Circle: //	环行飞行（围绕在身边，暗黑二刺客的集气）
  {
    ...
    } else // 围绕着发送者转
    {
      ...
      if (!Npc[m_SkillParam.tLauncher.tActorInfo.nActorIndex].IsMatch(
              m_SkillParam.tLauncher.tActorInfo.dwActorID)) {
        DoVanish();
        return;
      }
      m_nCurrentMapX =
          Npc[m_SkillParam.tLauncher.tActorInfo.nActorIndex].m_MapX;
      m_nCurrentMapY =
          Npc[m_SkillParam.tLauncher.tActorInfo.nActorIndex].m_MapY;
      m_nXOffset =
          Npc[m_SkillParam.tLauncher.tActorInfo.nActorIndex].m_MapX + dx;
      m_nYOffset =
          Npc[m_SkillParam.tLauncher.tActorInfo.nActorIndex].m_MapY + dy;
    }
  ...
  case MISSLE_MMK_Follow: //	跟踪目标飞行
  {
    ...
    m_nXOffset, m_nYOffset, &nSrcMpsX, &nSrcMpsY);
    SubWorld[m_nSubWorldId].Map2Mps(Npc[m_nFollowNpcIdx].m_RegionIndex,
    Npc[m_nFollowNpcIdx].m_MapX, Npc[m_nFollowNpcIdx].m_MapY,
    Npc[m_nFollowNpcIdx].m_OffX, Npc[m_nFollowNpcIdx].m_OffY, &nDesMpsX,
    &nDesMpsY); nDistance = SubWorld[m_nSubWorldId].GetDistance(nSrcMpsX,
    nSrcMpsY, nDesMpsX, nDesMpsY); int nXFactor = ((nDesMpsX - nSrcMpsY ) <<10)
    / nDistance; int nYFactor = ((nDesMpsY - nSrcMpsY ) <<10) / nDistance; int
    dx				= nXFactor * m_nSpeed; int dy
    = nYFactor * m_nSpeed; if (m_nZAcceleration)
    {
            m_nHeight += m_nHeightSpeed;
    ...
    int nBarrierKind = SubWorld[m_nSubWorldId].TestBarrier(m_nRegionId,
    m_nCurrentMapX, m_nCurrentMapY, m_nXOffset, m_nYOffset, 0, 0); if
    (nBarrierKind == Obstacle_Normal || nBarrierKind == Obstacle_Jump)
    {
            DoVanish();
            return;
    }
  ...
  case MISSLE_MMK_Motion: //	玩家动作类
  {
    ...
    int x	= m_nXOffset;
    int y	= m_nYOffset;
    int dx	= (m_nSpeed * g_DirCos(m_nDirIndex,MaxMissleDir))  ;//*
    m_nCurrentLife; int dy	= (m_nSpeed *
    g_DirSin(m_nDirIndex,MaxMissleDir)) ;//* m_nCurrentLife;
...
BOOL KMissle::ProcessDamage(int nNpcId) {
  ...
  if (Npc[m_SkillParam.tLauncher.tActorInfo.nActorIndex].IsMatch(
    ...
    if (m_pMagicAttribsData) {
      ...
      Npc[nNpcId].ReceiveDamage(m_SkillParam.tLauncher.tActorInfo.nActorIndex,
                                m_bIsMelee,
                                m_pMagicAttribsData->m_pDamageMagicAttribs);

      if (m_pMagicAttribsData->m_nStateMagicAttribsNum > 0)
        Npc[nNpcId].SetStateSkillEffect(
            m_SkillParam.tLauncher.tActorInfo.nActorIndex, m_nSkillId, m_nLevel,
      ...
      if (m_pMagicAttribsData->m_nImmediateMagicAttribsNum > 0)
        Npc[nNpcId].SetImmediatelySkillEffect(
            m_SkillParam.tLauncher.tActorInfo.nActorIndex,
            m_pMagicAttribsData->m_pImmediateAttribs,
...
int KMissle::ProcessCollision(int nLauncherIdx, int nRegionId, int nMapX,
                              int nMapY, int nRange,
                              MISSLE_RELATION eRelation) {
#ifdef TOOLVERSION
  return 0;
#endif
  if (nLauncherIdx <= 0)
  ...
  for (int i = -nRangeX; i < nRangeX; i++) {
    for (int j = -nRangeY; j < nRangeY; j++) {
      ...
      int nNpcIdx = SubWorld[nSubWorld].m_Region[nSearchRegion].FindNpc(
          nRMx, nRMy, nLauncherIdx, relation_all);
      if (nNpcIdx > 0) {
        nRet++;
        switch (eRelation) {
        ...
        case MissleRelation_All:
          if (NpcSet.GetRelation(nLauncherIdx, nNpcIdx) == relation_enemy ||
              NpcSet.GetRelation(nLauncherIdx, nNpcIdx) == relation_ally) {
#ifndef _SERVER
            int nSrcX = 0;
            int nSrcY = 0;
            SubWorld[0].Map2Mps(nSearchRegion, Npc[nNpcIdx].m_MapX,
...
int KMissle::ProcessCollision() {
  ...
  return ProcessCollision(nLauncher, m_nRegionId, m_nCurrentMapX,
...
void KMissle::DoWait() {
  ...
  int nBeginTime  = SubWorld[0].m_dwCurrentTime + 	m_nStartLifeTime -
  Skill[m_nSkillId][m_nLevel].m_nWaitTime; int nEndTime = nBeginTime +
  m_MissleRes.m_MissleRes[MS_DoWait].nInterval *
  m_MissleRes.m_MissleRes[MS_DoWait].nTotalFrame /
  m_MissleRes.m_MissleRes[MS_DoWait].nDir ; if (nEndTime <= nBeginTime) return ;

    SubWorld[0].Map2Mps(m_nRegionId, m_nCurrentMapX, m_nCurrentMapY,m_nXOffset,
  m_nYOffset, &nSrcX, &nSrcY); KSkillSpecialNode * pNode = new
  KSkillSpecialNode; KSkillSpecial * pSkillSpecial = new KSkillSpecial;
    pNode->m_pSkillSpecial = pSkillSpecial;
    pSkillSpecial->m_nPX = nSrcX;
...
void KMissle::GetMpsPos(int *pPosX, int *pPosY) {
  SubWorld[m_nSubWorldId].Map2Mps(m_nRegionId, m_nCurrentMapX, m_nCurrentMapY,
                                  this->m_nXOffset, m_nYOffset, pPosX, pPosY);
};