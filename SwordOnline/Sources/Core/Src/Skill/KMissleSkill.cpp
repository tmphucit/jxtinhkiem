...
KMissleSkill::KMissleSkill() {
  ...
  m_nWaitTime = 0;
  m_nRowIdInSkillTabFile = m_nImmediateAttribsNum = m_nStateAttribsNum =
      m_nMissleAttribsNum = m_nDamageAttribsNum = 0;
#ifndef _SERVER
  m_szSkillDesc[0] = 0;
...
KMissleSkill::~KMissleSkill() {
  while (m_MissleLevelUpSettingList.GetTail()) {
    KEffectDataNode *pNode =
        (KEffectDataNode *)m_MissleLevelUpSettingList.GetTail();
    delete pNode;
...
void KMissleSkill::TrigerMissleEventSkill(int nMissleId, int nSkillId,
                                          int nSkillLevel) {
  CheckAndGenSkillData();

  if (nMissleId > MAX_MISSLE || nMissleId < 0)
    return;

  KMissle *pMissle = &(Missle[nMissleId]);

  if (pMissle->m_nMissleId < 0)
    return;

  if (pMissle->m_SkillParam.dwSkillParams & PARAM_LAUNCHER) {
    switch (pMissle->m_SkillParam.tLauncher.tActorInfo.nActorType) {
    case Actor_Npc:
      if (!Npc[pMissle->m_SkillParam.tLauncher.tActorInfo.nActorIndex].IsMatch(
              pMissle->m_SkillParam.tLauncher.tActorInfo.dwActorID))
        return;
      break;
    case Actor_Obj:
      //	if
      //(!Object[pMissle->m_SkillParam.tLauncher.tActorInfo.nActorIndex].IsMatch(pMissle->m_SkillParam.tLauncher.tActorInfo.dwActorID))
      return;
      break;
    case Actor_Missle:
      if (!Missle[pMissle->m_SkillParam.tLauncher.tActorInfo.nActorIndex]
               .IsMatch(pMissle->m_SkillParam.tLauncher.tActorInfo.dwActorID))
        return;
      break;
    default:
      return;
  ...
  if (nSkillId > 0) // 如果有消亡技能则发消亡技能
  {
    ...
    if (m_bByMissle) {
      ...
      SkillParam.dwSkillParams |=
          (pMissle->m_SkillParam.dwSkillParams & PARAM_LAUNCHER != 0);
      SkillParam.tLauncher.tActorInfo =
          pMissle->m_SkillParam.tLauncher.tActorInfo;
    }

    pMissle->GetMpsPos(&SkillParam.tTarget.tPoint.nX,
                       &SkillParam.tTarget.tPoint.nY);
...
// nValue2 = 0
int KMissleSkill::CastZone(int nSrcPX, int nSrcPY, int nRefPX, int nRefPY,
                           TSkillParam *pSkillParam) {
  ...
  KMissleMagicAttribsData *pNewMagicAttribsData =
      CreateMissleMagicAttribsData(pSkillParam);
#endif //_SERVER
  ...
  for (int i = 0; i < m_nChildSkillNum; i++)
    for (int j = 0; j < m_nChildSkillNum; j++) {
      if (m_bBaseSkill) {
        ...
        nMissleIndex = MissleSet.Add(
            nSubWorldId, nBeginPX + j * SubWorld[nSubWorldId].m_nCellWidth,
            nBeginPY + i * SubWorld[nSubWorldId].m_nCellHeight);
        if (nMissleIndex < 0)
          continue;
        Missle[nMissleIndex].m_nDir =
            g_GetDirIndex(nSrcPX, nSrcPY, nRefPX, nRefPY);
        Missle[nMissleIndex].m_nDirIndex =
            g_DirIndex2Dir(Missle[nMissleIndex].m_nDir, MaxMissleDir);
        ...
        nCastMissleNum++;
#ifndef _SERVER
        Missle[nMissleIndex].m_MissleRes.PlaySound(
            Missle[nMissleIndex].m_eMissleStatus, 0, 0, 0);
#endif
        ...
        ChildSkillParam.dwSkillParams &=
            ((pSkillParam->dwSkillParams & PARAM_LAUNCHER) != 0);
        ChildSkillParam.tLauncher.tActorInfo =
            pSkillParam->tLauncher.tActorInfo;

        ChildSkillParam.tTarget.tPoint.nX =
            nBeginPX + j * pSkillParam->GetCellWidth();
        ChildSkillParam.tTarget.tPoint.nY =
            nBeginPY + i * pSkillParam->GetCellHeight();
        ChildSkillParam.nWaitTime =
            pSkillParam->nWaitTime +
            GetMissleGenerateTime(i * m_nChildSkillNum + j);
...
// Value1 子弹之间的间距
// Value2
int KMissleSkill::CastLine(int nSrcPX, int nSrcPY, int nRefPX, int nRefPY,
                           TSkillParam *pSkillParam) {
  _ASSERT(pSkillParam);
  int nDesSubX = 0;
  int nDesSubY = 0;
...
/*
int		KMissleSkill::CastExtractiveLineMissle(int nSrcX, int nSrcY, int
nDesX, int nDesY, int nXOffset, int nYOffset, int nDesX, int nDesY, TSkillParam
* pSkillParam)
{
        int nDesSubX		= 0;
        int nDesSubY		= 0;
...
#ifdef _SERVER
        KMissleMagicAttribsData * pNewMagicAttribsData =
CreateMissleMagicAttribsData(nLauncher); #endif //_SERVER
        ...
        //分别生成多少子弹
        {
                ...
                if (m_bBaseSkill)
                {
                        ...
                        nMissleIndex = MissleSet.Add(nSubWorldId, nSrcPX,
nSrcPY);

                        if (nMissleIndex < 0)	goto exit;

                        Missle[nMissleIndex].m_nDir
= g_GetDirIndex(nSrcPX, nSrcPY, nDesPX, nDesPY);
                        Missle[nMissleIndex].m_nDirIndex		=
g_DirIndex2Dir(Missle[nMissleIndex].m_nDir, MaxMissleDir);
                        CreateMissle(pSkillParam, m_nChildSkillId,
nMissleIndex);

                        if (Missle[nMissleIndex].m_eMoveKind ==
MISSLE_MMK_Parabola)
                        {
                                int nLength = g_GetDistance(nSrcPX, nSrcPY,
nDesX, nDesY); int nTime = nLength / Missle[nMissleIndex].m_nSpeed;
                                Missle[nMissleIndex].m_nHeightSpeed	=
Missle[nMissleIndex].m_nZAcceleration * (nTime - 1) / 2;
                        }

                        Missle[nMissleIndex].m_dwBornTime		=
SubWorld[nSubWorldId].m_dwCurrentTime; Missle[nMissleIndex].m_nSubWorldId
= nSubWorldId; Missle[nMissleIndex].m_SkillParam		= *pSkillParam;
                        Missle[nMissleIndex].m_nStartLifeTime	=
pSkillParam->nWaitTime + GetMissleGenerateTime(0);
                        Missle[nMissleIndex].m_nLifeTime		+=
Missle[nMissleIndex].m_nStartLifeTime; if (Missle[nMissleIndex].m_eMoveKind ==
MISSLE_MMK_Line || Missle[nMissleIndex].m_eMoveKind == MISSLE_MMK_Parabola)
                        {
...
#ifndef _SERVER
                        Missle[nMissleIndex].m_MissleRes.PlaySound(Missle[nMissleIndex].m_eMissleStatus,
0, 0, 0)		; #endif nCastMissleNum ++;
                }
                else
                {
...
pSkillParam.tLauncher.tActorInfo;
                                ...
                                ChildSkillParam.nWaitTime =
pSkillParam->nWaitTime + GetMissleGenerateTime(0); ChildSkillParam.usSkillID =
m_nChildSkillId; ChildSkillParam.usSkillLevel = m_nChildSkillLevel;
                                nCastMissleNum += g_CastSkill(&ChildSkillParam);
...
int KMissleSkill::CastWall(int nSrcPX, int nSrcPY, int nDesPX, int nDesPY,
                           TSkillParam *pSkillParam)
...
#ifdef _SERVER
  KMissleMagicAttribsData *pNewMagicAttribsData =
      CreateMissleMagicAttribsData(pSkillParam->tLauncher.tActorInfo);
#endif //_SERVER

  // 分别生成多少子弹
  for (int i = 0; i < m_nChildSkillNum; i++) {
    nDesSubX =
        nDesPX + ((nCurMSDistance * g_DirCos(nDirIndex, MaxMissleDir)) >> 10);
    nDesSubY =
        nDesPY + ((nCurMSDistance * g_DirSin(nDirIndex, MaxMissleDir)) >> 10);
...
int KMissleSkill::CastCircle(int nSrcPX, int nSrcPY, int nDesPX, int nDesPY,
                             TSkillParam *pSkillParam) {
  int nDesSubPX = 0;
  int nDesSubPY = 0;
...
//  传来的X/Y参数为格子坐标
int KMissleSkill::CastSpread(int nSrcPX, int nSrcPY, int nDesPX, int nDesPY,
                             TSkillParam *pSkillParam) {
  int nDesSubMapX = 0;
  int nDesSubMapY = 0;
  int nFirstStep = m_nValue2; // 第一步的长度，子弹在刚发出去时离玩家的距离
  ...
  if (pSkillParam->dwSkillParams & PARAM_TARGET) {
    int nDistance = 0;
    nDistance = (int)sqrt((nDesPX - nSrcPX) * (nDesPX - nSrcPX) +
                          (nDesPY - nSrcPY) * (nDesPY - nSrcPY));
  ...
  for (int i = 0; i < m_nChildSkillNum; i++) {
    ...
    if (pSkillParam->dwSkillParams & PARAM_TARGET) {
      ...
      nSinAB = (nYFactor * g_DirCos(nDSubDir, MaxMissleDir) -
                nXFactor * g_DirSin(nDSubDir, MaxMissleDir)) >>
               10;
      nCosAB = (nXFactor * g_DirCos(nDSubDir, MaxMissleDir) +
                nYFactor * g_DirSin(nDSubDir, MaxMissleDir)) >>
               10;
    } else {
...
// Author		: RomanDou
*****************************************************************************/
void KMissleSkill::CreateMissle(TSkillParam *pSkillParam, int nChildSkillId,
                                int nMissleIndex) {
  _ASSERT(pSkillParam);
  ...
  if (!(pSkillParam->dwSkillParams & PARAM_LAUNCHER) &&
      pSkillParam->tLauncher.tActorInfo.nActorType == Actor_Npc) {
    if (m_bIsMelee)
      pMissle->m_nLifeTime = Npc[pSkillParam->tLauncher.tActorInfo.nActorIndex]
                                 .ModifyMissleLifeTime(pMissle->m_nLifeTime);
    else {
      pMissle->m_nSpeed =
          Npc[pSkillParam->tLauncher.tActorInfo.nActorIndex].ModifyMissleSpeed(
              pMissle->m_nSpeed);
      pMissle->m_bCollideVanish =
          Npc[pSkillParam->tLauncher.tActorInfo.nActorIndex]
              .ModifyMissleCollsion(pMissle->m_bCollideVanish);
    }
  }
}
...
BOOL KMissleSkill::ParseString2MagicAttrib(char *szMagicAttribName,
                                           char *szValue) {
  int nValue1 = 0;
  int nValue2 = 0;
  int nValue3 = 0;
  ...
  for (int i = 0; i <= magic_normal_end; i++) {
    if (!strcmp(szMagicAttribName, MagicAttrib2String(i))) {
      ...
      if (KFightSkill::ParseString2MagicAttrib((MAGIC_ATTRIB)i, nValue1,
                                               nValue2, nValue3))
        continue;
...
BOOL KMissleSkill::ParseString2MagicAttrib(MAGIC_ATTRIB MagicAttrib,
...
void KMissleSkill::GetDesc(char *pszMsg, int nOwnerIndex) {
  ...
  if (g_MisslesLib[m_nChildSkillId].m_bRangeDamage)
    strcat(pszMsg, "区域伤害 ");
  ...
  if (g_MisslesLib[m_nChildSkillId].m_nDamageRange > 1) {
    sprintf(szTemp, "伤害范围:%d ",
            g_MisslesLib[m_nChildSkillId].m_nDamageRange);
    strcat(pszMsg, szTemp);
  }
  ...
  if (m_bIsAura)
    strcat(pszMsg, "光环 ");
  if (this->m_bIsPhysical)
    strcat(pszMsg, "物理 ");
  if (this->m_bIsMelee)
    strcat(pszMsg, "近身 ");
  if (this->m_bTargetOnly)
    strcat(pszMsg, "必中 ");
  if (this->m_bTargetAlly)
    strcat(pszMsg, "对友 ");
  if (this->m_bTargetEnemy)
    strcat(pszMsg, "对敌 ");
  if (this->m_bTargetObj)
    strcat(pszMsg, "对物 ");
  if (this->m_bTargetSelf)
    strcat(pszMsg, "对已 ");
  if (this->m_bUseAttackRate)
    strcat(pszMsg, "考虑命中率 ");
