...
public:
  ...
  // int					m_nFollowNpcIdx;
  // //	跟随谁 int					m_nLauncher;
  // //	发射者在NpcSet中的Index DWORD				m_dwLauncherId;
  // //	发射者的唯一ID eSkillLauncherType	m_eSkillLauncherType;
  //	自生成的动态数据
...
public:
  ...
  BOOL IsRelationSuitable(NPC_RELATION npc_relation,
                          MISSLE_RELATION msl_relation) {
    if (msl_relation == MissleRelation_All)
      return TRUE;
    else if (msl_relation == MissleRelation_AllyOnly &&
             npc_relation == relation_ally)
      return TRUE;
    else if (msl_relation == MissleRelation_EnemyOnly &&
             npc_relation == relation_enemy)
      return TRUE;
    else
      return FALSE;
...
public:
  ...
  int ProcessCollision(int nLauncherIdx, int nRegionId, int nMapX, int nMapY,
                       int nRange, MISSLE_RELATION eRelation);
