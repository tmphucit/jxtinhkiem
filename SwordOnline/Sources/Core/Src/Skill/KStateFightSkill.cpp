...
// 服务器端需要获得并通知客户端
BOOL KStateFightSkill::GetInfoFromTabFile(KITabFile *pSkillsSettingFile,
                                          int nRow) {
...
*/
  ...
  if (pAttribsData) {
    Npc[pSkillParam->tTarget.tActorInfo.nActorIndex].ReceiveDamage(
        pSkillParam->tLauncher.tActorInfo.nActorIndex, 0,
        pAttribsData->m_pDamageMagicAttribs);

    if (pAttribsData->m_nStateMagicAttribsNum > 0)
      Npc[pSkillParam->tTarget.tActorInfo.nActorIndex].SetStateSkillEffect(
          pSkillParam->tLauncher.tActorInfo.nActorIndex, m_nId, m_nLevel,
    ...
    if (pAttribsData->m_nImmediateMagicAttribsNum > 0)
      Npc[pSkillParam->tTarget.tActorInfo.nActorIndex]
          .SetImmediatelySkillEffect(
              pSkillParam->tLauncher.tActorInfo.nActorIndex,
...
BOOL KStateFightSkill::CastPassivitySkill(TSkillParam *pSkillParam) {
  ...
  if ((pSkillParam->dwSkillParams & PARAM_LAUNCHER) &&
    ...
    KMissleMagicAttribsData *pAttribsData =
        (KMissleMagicAttribsData *)m_StateAttribs;
    if (m_nStateAttribsNum > 0)
      ;
    {
      Npc[pSkillParam->tTarget.tActorInfo.nActorIndex].SetStateSkillEffect(
          pSkillParam->tLauncher.tActorInfo.nActorIndex, m_nId, m_nLevel,
          m_StateAttribs, m_nStateAttribsNum, -1);
    }
    return TRUE;
  }
#endif
