...
int g_CastSkill(TSkillParam *pSkillParam) {
  if (pSkillParam->usSkillID > g_ulSkillCount ||
      pSkillParam->usSkillLevel > MAX_SKILLLEVEL)
    return FALSE;
  if (!g_pSkill[pSkillParam->usSkillID][pSkillParam->usSkillLevel]) {
    if (!g_GenSkillData(pSkillParam->usSkillID, pSkillParam->usSkillLevel))
      return FALSE;
  }

  return g_pSkill[pSkillParam->usSkillID][pSkillParam->usSkillLevel]->Cast(
      pSkillParam);
}

BOOL g_InstanceSkill(eSKillStyle eStyle, unsigned long ulSkillID,
                     unsigned long ulSkillLevel) {
...
ISkill *g_GetSkill(unsigned long ulSkillID, unsigned long ulSkillLevel) {
  ...
  if (!g_pSkill[ulSkillID][ulSkillLevel]) {
    ...
    if (!g_InstanceSkill(g_pSkill[ulSkillID][0].GetSkillStyle(), ulSkillID,
                         ulSkillLevel))
      return NULL;
    g_pSkill[ulSkillID][ulSkillLevel]->SetRowIdInSkillTabFile(
        g_pSkill[ulSkillID][0]->GetRowIdInSkillTabFile());
...
int TSkillParam::GetSubWorldIndex() {
  if (dwSkillParams & PARAM_LAUNCHER) {
    ...
    case Actor_Npc: {
      if (Npc[tLauncher.tActorInfo.nActorIndex].IsMatch(
              tLauncher.tActorInfo.dwActorID))
        return Npc[tLauncher.tActorInfo.nActorIndex].m_SubWorldIndex;
    } break;
    case Actor_Obj: {
      if (Object[tLauncher.tActorInfo.nActorIndex].m_nID ==
          tLauncher.tActorInfo.dwActorID)
        return Object[tLauncher.tActorInfo.nActorIndex].m_nSubWorldID;
    } break;
    case Actor_Missle: {
      if (Missle[tLauncher.tActorInfo.nActorIndex].m_dwID ==
          tLauncher.tActorInfo.dwActorID)
...
int ISkill::GetSkillIdFromName(char *szSkillName) {
  ...
  for (int i = 0; i < MAX_SKILL; i++) {
    if (!strcmp(Skill[i][1].m_szName, szSkillName))
      return i;
  }
  return -1;
}
