...
public:
  ...
  BOOL IsTargetOnly() {
    CheckAndGenSkillData();
    return m_bTargetOnly;
  };
  BOOL IsTargetEnemy() {
    CheckAndGenSkillData();
    return m_bTargetEnemy;
  };
  BOOL IsTargetAlly() {
    CheckAndGenSkillData();
    return m_bTargetAlly;
  };
  BOOL IsTargetObj() {
    CheckAndGenSkillData();
    return m_bTargetObj;
  ...
  NPCATTRIB GetSkillCostType() {
    CheckAndGenSkillData();
    return m_nSkillCostType;
  };
  CLIENTACTION GetActionType() {
    CheckAndGenSkillData();
    return m_nCharActionId;
  };
  int GetDelayPerCast() {
    CheckAndGenSkillData();
...
#ifndef _SERVER
  const char *GetPreCastEffectFile() {
    CheckAndGenSkillData();
  ...
  CreateMissleMagicAttribsData(TGameActorInfo GameActor);
  KMissleMagicAttribsData *
  CreateMissleMagicAttribsData(TSkillParam *pSkillParam);
#endif
...
#endif
  virtual BOOL ParseString2MagicAttrib(MAGIC_ATTRIB MagicAttrib, int nValue1,
