...
/*
 * The still variable better than dynamic variable, because of don't to think
 * that it can call through to unqualified virtual function
 */

static CGameServer g_theJXServer(MAX_SOCKET_KEEPINPOOL, MAX_BUFFER_KEEPINPOOL,
                                 EXTEND_BUFFER_SIZE);

static CClientManager g_theClientManager(MAX_BUFFER_KEEPINPOOL,
                                         EXTEND_BUFFER_SIZE);

/*
 * Function
 */
bool ServerStartup(JX_SERVER_ unsigned long ulnAddressToListenOn,
                   unsigned short usnPortToListenOn, bool bStartAtOnce) {
  bool ok = false;
...
void Server_Start() { g_theJXServer.StartAcceptingConnections(); }

void Server_Pause() { g_theJXServer.StopAcceptingConnections(); }

void Server_Begin() { g_theClientManager.PrepareToSend(); }

void Server_End() { g_theClientManager.BeginToSend(); }

inline void SendToClient(JX_SERVER_ const unsigned long ulnID, void *pData,
                         size_t dataLength) {
  g_theClientManager.SendToClient(ulnID, reinterpret_cast<const char *>(pData),
                                  dataLength);
}

void ShutdownClient(JX_SERVER_ const unsigned long ulnID) {
  g_theClientManager.ShutdownClient(ulnID);
}

CClientManager::CClientManager(size_t maxFreeBuffers,
...
unsigned long
CClientManager::AddClient(OnlineGameLib::Win32::CSocketServer::Socket *pSocket,
  ...
  if (!m_freeClientStack.empty() && pSocket) {
    ...
    Output(GetTimeStamp() + _T("Add a client [ID:") + ToString(index) +
           _T( "] current total is " ) + ToString(m_usedClientStack.size()));
  }
...
unsigned long CClientManager::DelClient(
  ...
  if (!m_usedClientStack.empty() && pSocket) {
    ...
    Output(GetTimeStamp() + _T("Del a client [ID:") + ToString(index) +
           _T( "] current total is " ) + ToString(m_usedClientStack.size()));
  }
...
void CClientManager::SendToClient(const unsigned long ulnID, const char *pData,
                                  size_t dataLength) {
...
void CClientManager::RecvFromClient(const unsigned long ulnID,
...
const char *CClientManager::GetFromClient(const unsigned long ulnID,
                                          size_t &dataLength) {
  if (ulnID >= 0 && ulnID < MAX_CLIENT_CANBELINKED) {
    OnlineGameLib::Win32::CSocketServer::Socket *pSocket =
        m_clientContext[ulnID].pSocket;
    if (pSocket) {
      ...
      m_clientContext[ulnID].pRecvBuffer =
          ProcessDataStream(ulnID, m_clientContext[ulnID].pRecvBuffer);
...
void CClientManager::BeginToSend() {
  ...
  for (LIST::iterator i = m_usedClientStack.begin();
    ...
    if (used > 0) {
      ...
      m_clientContext[ulnID].pWriteBuffer->AddData(
          reinterpret_cast<const char *>(&dwEOSFlag), sc_nFlagLen);

      m_clientContext[ulnID].pSocket->Write(
          reinterpret_cast<const char *>(pPackData), used + sc_nFlagLen);

/*	#define NETWORK_DEBUG_TRACE2FILE
        #undef NETWORK_DEBUG_TRACE2FILE
        #ifdef NETWORK_DEBUG_TRACE2FILE
        //{
                Trace2File( _T("Package[ID:") + ToString( gs_dwPackID - 1 ) +
   _T("]\n") + DumpData( pPackData, used, 40 ) );
        //}
        #else
        //{

                g_DebugLog( "[NETWORK] NO.[ID:%d]Pack - Len:%d", gs_dwPackID -
   1, used );
        //}
...
//}
      ...
      //{
      m_clientContext[ulnID].pSocket->Write(
          reinterpret_cast<const char *>(pPackData), used);
//}
#endif // NETWORK_DEBUG
       //]
...
inline size_t GetClientCount() { return g_theClientManager.GetClientCount(); }
...
AddClient(OnlineGameLib::Win32::CSocketServer::Socket *pSocket,
          OnlineGameLib::Win32::CIOBuffer *pAddress) {
  return g_theClientManager.AddClient(pSocket, pAddress);
}
inline unsigned long
DelClient(OnlineGameLib::Win32::CSocketServer::Socket *pSocket) {
...
OnlineGameLib::Win32::CIOBuffer *CClientManager::ProcessDataStream(
    const unsigned long ulnID, OnlineGameLib::Win32::CIOBuffer *pBuffer) const {
  const size_t used = pBuffer->GetUsed();
...
inline void RecvFromClient(JX_SERVER_ const unsigned long ulnID,
                           const char *pData, size_t dataLength) {
  g_theClientManager.RecvFromClient(ulnID, pData, dataLength);
}

inline const char *GetFromClient(const unsigned long ulnID,
                                 size_t &dataLength) {
