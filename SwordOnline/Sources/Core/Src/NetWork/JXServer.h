...
public:
  ...
  void SendToClient(const unsigned long ulnID, const char *pData,
                    size_t dataLength);

  const char *GetFromClient(const unsigned long ulnID, size_t &dataLength);

  void RecvFromClient(const unsigned long ulnID, const char *pData,
                      size_t dataLength);
  ...
  void InstallCallBack(CALLBACK_SERVER_EVENT pfn) {
    m_pfnCallBackServerEvent = pfn;
  };
...
protected:
  ...
  ProcessDataStream(const unsigned long ulnID,
                    OnlineGameLib::Win32::CIOBuffer *pBuffer) const;

  void ProcessCommand(const unsigned long ulnID,
 ...
 * Server usage
 */
extern inline void RecvFromClient(JX_SERVER_ const unsigned long ulnID,
                                  const char *pData, size_t dataLength);

extern inline const char *GetFromClient(const unsigned long ulnID,
                                        size_t &dataLength);

extern inline void SendToClient(JX_SERVER_ const unsigned long ulnID,
