...
public:
  ...
  void InstallCallBack(CALLBACK_CLIENT_EVENT pfn) {
    m_pfnCallbackClientEvent = pfn;
  };
...
protected:
  ...
  ProcessDataStream(OnlineGameLib::Win32::CIOBuffer *pBuffer);
 ...
 * Client usage
 */
extern inline void RecvFromServer(JX_CLIENT_ const char *pData,
                                  size_t dataLength);

extern inline const char *GetFromServer(size_t &dataLength);

extern inline void SendToServer(JX_CLIENT_ void *pData, size_t dataLength);
