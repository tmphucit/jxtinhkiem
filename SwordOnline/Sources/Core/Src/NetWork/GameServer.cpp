...
CGameServer::CGameServer(unsigned long addressToListenOn,
                         unsigned short portToListenOn, size_t maxFreeSockets,
                         size_t maxFreeBuffers, size_t bufferSize /* = 1024 */,
                         size_t numThreads /* = 0 */)
    : OnlineGameLib::Win32::CSocketServer(addressToListenOn, portToListenOn,
                                          maxFreeSockets, maxFreeBuffers,
                                          bufferSize, numThreads) {}

CGameServer::CGameServer(size_t maxFreeSockets, size_t maxFreeBuffers,
...
CGameServer::~CGameServer() {
  /*
   * If we want to be informed of any buffers or sockets being destroyed at
   * destruction time then we need to release these resources now, whilst we,
   * the derived class, still exists. Once our destructor exits and the base
   * destructor takes over we wont get any more notifications...
   */
...
SOCKET CGameServer::CreateListeningSocket(unsigned long address,
                                          unsigned short port) {
...
OnlineGameLib::Win32::CSocketServer::WorkerThread *
CGameServer::CreateWorkerThread(OnlineGameLib::Win32::CIOCompletionPort &iocp) {
...
void CGameServer::OnShutdownInitiated() { Output(_T("OnShutdownInitiated")); }

void CGameServer::OnShutdownComplete() { Output(_T("OnShutdownComplete")); }
...
void CGameServer::OnConnectionCreated() { Output(_T("OnConnectionCreated")); }
...
void CGameServer::PreWrite(OnlineGameLib::Win32::CSocketServer::Socket *pSocket,
