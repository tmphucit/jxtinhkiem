...
CConnectManager::CConnectManager(size_t maxFreeBuffers,
...
const char *CConnectManager::GetFromClient(size_t &dataLength) {
  ...
  if (dataLength > 0) {
    static const int sc_nFlagLen =
        sizeof(DWORD) * 2 + PACK_HEADER_LEN + sizeof(BYTE);
...
void CConnectManager::ProcessCommand(
...
CConnectManager::ProcessDataStream(OnlineGameLib::Win32::CIOBuffer *pBuffer) {
  const size_t used = pBuffer->GetUsed();
...
bool ConnectTo(
    JX_CLIENT_ const OnlineGameLib::Win32::_tstring &addressToConnectServer,
...
void Client_Start() { g_theJXClient.StartConnections(); }

void Client_Pause() { g_theJXClient.StopConnections(); }
 ...
 * Client usage
 */
inline void RecvFromServer(JX_CLIENT_ const char *pData, size_t dataLength) {
...
inline void SendToServer(JX_CLIENT_ void *pData, size_t dataLength) {
...
void ConnectServer(JX_CLIENT_) { g_theManager.ConnectServer(); }

void DisConnetServer(JX_CLIENT_) { g_theManager.DisConnetServer(); }