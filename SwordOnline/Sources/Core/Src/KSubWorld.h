...
public:
  ...
  void Map2Mps(int nR, int nX, int nY, int nDx, int nDy, int *nRx,
               int *nRy); // 格子坐标转像素坐标
  static void Map2Mps(int nRx, int nRy, int nX, int nY, int nDx, int nDy,
                      int *pnX, int *pnY); // 格子坐标转像素坐标
  void Mps2Map(int Rx, int Ry, int *nR, int *nX, int *nY, int *nDx,
               int *nDy); // 像素坐标转格子坐标
  void GetMps(int *nX, int *nY, int nSpeed, int nDir,
              int nMaxDir = 64); // 取得某方向某速度下一点的坐标
  BYTE TestBarrier(int nMpsX, int nMpsY);
  BYTE TestBarrier(int nRegion, int nMapX, int nMapY, int nDx, int nDy,
                   int nChangeX, int nChangeY); // 检测下一点是否为障碍
  BYTE TestBarrierMin(int nRegion, int nMapX, int nMapY, int nDx, int nDy,
                      int nChangeX, int nChangeY); // 检测下一点是否为障碍
  BYTE GetBarrier(int nMpsX, int nMpsY);           // 取得某点的障碍信息
  DWORD GetTrap(int nMpsX, int nMpsY);

  void AddTrap(int nMpsX, int nMpsY, char *szSpcrit);

  void LoadTong(char *szMapTongName, int nMapTongId, int nMapTongT,
                int nMapTongVG, char *szMapTongNameBC, int nMapTongBCId,
                int nCheckMap);
  ...
  void BroadCastRegion(const void *pBuffer, size_t uSize, int &nMaxCount,
                       int nRegionIndex, int nOX, int nOY);
#endif
#ifndef _SERVER
