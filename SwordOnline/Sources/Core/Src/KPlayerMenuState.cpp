...
KPlayerMenuState::KPlayerMenuState() { Release(); }
...
#ifdef _SERVER
void KPlayerMenuState::SetState(int nPlayerIdx, int nState,
                                char *lpszSentence /*=NULL*/,
                                int nSentenceLength /*=0*/,
                                DWORD dwDestNpcID /*=0*/) {
  ...
  case PLAYER_MENU_STATE_NORMAL: {
    if (m_nBackState == PLAYER_MENU_STATE_TEAMOPEN) {
      if (Player[nPlayerIdx].m_cTeam.m_nFlag &&
          g_Team[Player[nPlayerIdx].m_cTeam.m_nID].IsOpen())
    ...
    g_pServer->PackDataToClient(Player[nPlayerIdx].m_nNetConnectIdx,
                                (BYTE *)&sTrade,
                                sizeof(TRADE_CHANGE_STATE_SYNC));
  ...
  case PLAYER_MENU_STATE_TRADEOPEN: {
    ...
    sSync.m_wLength = sizeof(NPC_SET_MENU_STATE_SYNC) - 1 -
                      sizeof(sSync.m_szSentence) + nSentenceLength;
    Npc[Player[nPlayerIdx].m_nIndex].SendDataToNearRegion((LPVOID)&sSync,
                                                          sSync.m_wLength + 1);
  } break;
...
void KPlayerMenuState::RestoreBackupState(int nPlayerIdx) {
  ...
  case PLAYER_MENU_STATE_TRADEOPEN: {
    ...
    this->m_nState = PLAYER_MENU_STATE_NORMAL;
    this->m_szSentence[0] = 0;
    this->SetState(nPlayerIdx, PLAYER_MENU_STATE_TRADEOPEN, szBuffer,
                   strlen(szBuffer));
  } break;
  }
}
