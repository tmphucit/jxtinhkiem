...
//	功能：构造函数
//---------------------------------------------------------------------------
KPlayerFaction::KPlayerFaction() { Release(); }
...
BOOL KPlayerFaction::AddFaction(int nSeries, int nFactionID) {
  ...
  if (nFactionID < nSeries * FACTIONS_PRR_SERIES ||
      nFactionID >= (nSeries + 1) * FACTIONS_PRR_SERIES)
    return FALSE;
...
//	功能：离开当前门派
//---------------------------------------------------------------------------
void KPlayerFaction::LeaveFaction() { m_nCurFaction = -1; }
...
BOOL KPlayerFaction::OpenCurSkillLevel(int nLevel, KSkillList *pSkillList) {
  ...
  /*
          ...
          for (i = 0; i <= nLevel; i++)
          {
                  ...
                  {
                          if
     (g_Faction.m_sAttribute[m_nCurFaction].m_nSkill[i][j] <= 0) continue;
                          pSkillList->Add(g_Faction.m_sAttribute[m_nCurFaction].m_nSkill[i][j],
     0);
                  }
          }
...
//	功能：获得当前门派
//---------------------------------------------------------------------------
int KPlayerFaction::GetCurFactionNo() { return m_nCurFaction; }
