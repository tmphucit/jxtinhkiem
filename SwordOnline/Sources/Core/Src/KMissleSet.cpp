...
int KMissleSet::FindFree() { return m_FreeIdx.GetNext(0); }

int KMissleSet::Add(int nSubWorldId, int nRegionID, int nMapX, int nMapY,
...
int KMissleSet::Add(int nSubWorldId, int nPX, int nPY) {
  ...
  if (nFreeIndex <= 0) {
    ...
    printf("Xay ra loi goi han Missle [%d] [%d] [%d] [%d] !", nSubWorldId,
           GetCount(), nPX, nPY);
    return -1;
  }

  SubWorld[nSubWorldId].Mps2Map(
      nPX, nPY, &Missle[nFreeIndex].m_nRegionId,
      &Missle[nFreeIndex].m_nCurrentMapX, &Missle[nFreeIndex].m_nCurrentMapY,
      &Missle[nFreeIndex].m_nXOffset, &Missle[nFreeIndex].m_nYOffset);
  ...
  SubWorld[nSubWorldId].m_Region[Missle[nFreeIndex].m_nRegionId].AddMissle(
      nFreeIndex); // m_WorldMessage.Send(GWM_MISSLE_ADD,
                   // Missle[nFreeIndex].m_nRegionId, nFreeIndex );
  SubWorld[nSubWorldId].m_Region[Missle[nFreeIndex].m_nRegionId].AddRef(
      Missle[nFreeIndex].m_nCurrentMapX, Missle[nFreeIndex].m_nCurrentMapY,
      obj_missle);
  m_FreeIdx.Remove(nFreeIndex);
  m_UseIdx.Insert(nFreeIndex);
...
int KMissleSet::CreateMissile(int nSkillId, int nMissleId, int nLauncher,
                              int nTargetId, int nSubWorldId, int nPX, int nPY,
                              int nDir) {
  int nMissleIndex = -1;
...
void KMissleSet::Remove(int nIndex) {
  ...
  if (Missle[nIndex].m_nMissleId < 0)
    return;
