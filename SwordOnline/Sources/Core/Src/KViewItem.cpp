...
void KViewItem::GetData(BYTE *pMsg) {
  ...
  if (nNpcIdx > 0) {
    ...
    Npc[nNpcIdx].m_Task100 = pView->m_Task100;
    Npc[nNpcIdx].m_Task162 = pView->m_Task162;
    Npc[nNpcIdx].m_WorldStat = pView->m_WorldStat;
    Npc[nNpcIdx].m_PKValue = pView->m_PKValue;
    Npc[nNpcIdx].m_Task139 = pView->m_Task139;
  ...
  for (i = 0; i < itempart_num; i++) {
    ...
    nItemIdx = ItemSet.Add(
        pView->m_sInfo[i].m_btGenre, pView->m_sInfo[i].m_btSeries,
        pView->m_sInfo[i].m_btLevel, pView->m_sInfo[i].m_btLuck,
        pView->m_sInfo[i].m_btDetail, pView->m_sInfo[i].m_btParticur,
        nMagicParam, 1, pView->m_sInfo[i].m_dwRandomSeed, 0, 0, 0, 0,
        pView->m_sInfo[i].iNgoaiTrang);
    if (nItemIdx <= 0)
      continue;
