...
#include "KEngine.h"
#include "KItem.h"
#define CHANGERES_FIFONG_FILE "\\settings\\item\\phiphongres.txt"
...
BOOL KItemChangeRes::Init() {
  ...
  if (!m_Horse.Load(CHANGERES_HORSE_FILE))
    return FALSE;
  if (!m_Fifong.Load(CHANGERES_FIFONG_FILE))
    return FALSE;
  return TRUE;
}
...
int KItemChangeRes::GetFifongRes(int nParti, int nLevel) {
  int nRet;
  if (nLevel == 0) {
    return -1;
  }
  int nRow = nParti * 10 + nLevel + 2;
  m_Fifong.GetInteger(nRow, 2, 2, &nRet);
  return nRet - 2;
}