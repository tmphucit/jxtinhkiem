...
typedef struct
{
        int			m_nPos; // 前缀还是后缀 char
m_szName[SZBUFLEN_0];		// 名称 int			m_nClass;
// 五行要求 int			m_nLevel; // 等级要求 char
m_szIntro[SZBUFLEN_1];		// 说明文字 KMACP		m_MagicAttrib;
// 核心参数 int			m_DropRate; // 出现概率 } KMAGICATTRIB;
*/
...
protected:
  ...
  KBPT_Equipment m_BPTMask; // MASK
  KBPT_Equipment m_BPTPhiPhong;
...
public:
  ...
  // Add by Freeway Chen in 2003.5.30
  const KBPT_ClassMAIT *GetCMIT(IN int nPrefixPostfix, IN int nType,
                                IN int nSeries, int nLevel) const;
  ...
  // END
  const KBASICPROP_EQUIPMENT *GetFifongRecord(IN int) const;
  const int GetFifongRecordNumber() const;

  const KBASICPROP_EQUIPMENT_UNIQUE *FindEquipmentUnique(IN int, IN int,
                                                         IN int) const;
