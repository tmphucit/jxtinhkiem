...
BOOL KNpcResNode::Init(char *lpszNpcName, CActionName *cActionName,
                       CActionName *cNpcAction) {
  if (lpszNpcName == NULL || lpszNpcName[0] == 0)
    return FALSE;

  KTabFile KindFile;
  int i, j, k, nFindNo;
  char szBuffer[FILE_NAME_LENGTH], szTemp[FILE_NAME_LENGTH],
  ...
  if (strcmp(szBuffer, KIND_NAME_SPECIAL) == 0)
    ...
    m_nNpcKind = NPC_RES_NORMAL;
  // 得到资源文件路径
  KindFile.GetString(nFindNo, KIND_FILE_SECT5, "", m_szResPath,
                     sizeof(m_szResPath));
  ...
  if (m_nNpcKind == NPC_RES_SPECIAL) {
    ...
    // 得到部件说明文件名
    KindFile.GetString(nFindNo, KIND_FILE_SECT1, "", szBuffer,
                       sizeof(szBuffer));
    if (!szBuffer[0])
      return FALSE;
    //		g_SetFilePath(RES_INI_FILE_PATH);
    ...
    for (i = 0; i < MAX_PART; i++) {
      if (m_nSectInfo[i].nFlag) {
        KindFile.GetString(nFindNo, m_nSectInfo[i].szSectName, "",
        ...
        if (m_nSectInfo[i].szSectResName[0]) {
          g_StrCpyLen(m_nSectInfo[i].szSectSprInfoName,
                      m_nSectInfo[i].szSectResName,
    ...
    for (i = 0; i < MAX_PART; i++) {
      ...
      if (m_nSectInfo[i].nFlag) {
        ...
        if (!SectFile.Load(szBuf))
          ...
          g_UnitePathAndName(RES_INI_FILE_PATH,
                             m_nSectInfo[i].szSectSprInfoName, szBuf);
        ...
        if (!SectFile.Load(szBuf))
          continue;
        g_UnitePathAndName(RES_INI_FILE_PATH, m_nSectInfo[i].szSectSprInfoName,
                           szBuf);
        ...
        for (j = 0; j < nGetEquipNo; j++) {
          for (k = 0; k < nActionCount; k++) {
            ...
            ComposePathAndName(
                m_cResInfo[i].m_cSprInfo[j * nActionCount + k].szFileName,
                m_szResPath, szTemp);
            SectInfoFile.GetString(j + 2, k + 2, "16,16,0", szTemp,
                                   sizeof(szTemp));
    ...
    KindFile.GetString(nFindNo, KIND_FILE_SECT2, "", szNoHorseTableName,
                       sizeof(szNoHorseTableName));
    KindFile.GetString(nFindNo, KIND_FILE_SECT3, "", szOnHorseTableName,
                       sizeof(szOnHorseTableName));
    // 未骑马对应表
    ...
    // 得到贴图顺序表文件名
    if (KindFile.GetString(nFindNo, KIND_FILE_SECT4, "", szBuffer,
                           sizeof(szBuffer))) {
    ...
    if (nFindNo > 0 && nActionCount > 0) {
      for (i = 0; i < nActionCount; i++) {
        ...
        if (szTemp[0])
          this->ComposePathAndName(m_cShadowInfo.m_psInfo[i].m_szName,
                                   m_szResPath, szTemp);
        ShadowName.GetString(nFindNo, 2 + i * 2 + 1, "16,8,1", szTemp,
                             sizeof(szTemp));

        pcszTemp = szTemp;
        m_cShadowInfo.m_psInfo[i].m_nTotalFrame =
            KSG_StringGetInt(&pcszTemp, 16);
        KSG_StringSkipSymbol(&pcszTemp, ',');
        ...
        // &m_cShadowInfo.m_psInfo[i].m_nTotalDir,
        // &m_cShadowInfo.m_psInfo[i].m_nInterval);
    ...
    if (nFindNo > 0 && nActionCount > 0) {
      for (i = 0; i < nActionCount; i++) {
        ...
        if (szTemp[0])
          this->ComposePathAndName(this->m_szSoundName[i], RES_SOUND_FILE_PATH,
                                   szTemp);
      }
    }
  ...
  // 普通npc
    ...
    for (i = 0; i < nActionCount; i++) {
      ...
      ComposePathAndName(
          m_cResInfo[NORMAL_NPC_PART_NO].m_cSprInfo[i].szFileName, m_szResPath,
          szTemp);
      NormalNpcSprInfo.GetString(nFindNo, szBuffer, "16,8,0", szTemp,
                                 sizeof(szTemp));
      ...
      KNpcResNode::GetShadowName(
          m_cShadowInfo.m_psInfo[i].m_szName,
...
//---------------------------------------------------------------------------
void KNpcResNode::ComposePathAndName(char *lpszGet, char *lpszPath,
                                     char *lpszName) {
...
//---------------------------------------------------------------------------
BOOL KNpcResNode::GetFileName(int nPartNo, int nActionNo, int nEquipNo,
                              char *lpszDefault, char *lpszGetName,
                              int nStrLen) {
  if (nPartNo < 0 || nPartNo >= MAX_PART)
    return FALSE;
  if (m_cResInfo[nPartNo].GetName(nActionNo, nEquipNo, "", lpszGetName,
                                  nStrLen))
    return TRUE;
  return FALSE;
}
...
//---------------------------------------------------------------------------
int KNpcResNode::GetInterval(int nPartNo, int nActionNo, int nEquipNo,
                             int nDefault) {
  if (nPartNo < 0 || nPartNo >= MAX_PART)
    return nDefault;
...
// 功能:	获得某个部件的某个装备在某个动作下的 spr 文件信息
//---------------------------------------------------------------------------
int KNpcResNode::GetTotalFrames(int nPartNo, int nActionNo, int nEquipNo,
                                int nDefault) {
  if (nPartNo < 0 || nPartNo >= MAX_PART)
    return nDefault;
...
// 功能:	获得某个部件的某个装备在某个动作下的 spr 文件信息
//---------------------------------------------------------------------------
int KNpcResNode::GetTotalDirs(int nPartNo, int nActionNo, int nEquipNo,
                              int nDefault) {
  if (nPartNo < 0 || nPartNo >= MAX_PART)
    return nDefault;
...
// 功能:	从表格中得到一组排序信息
//---------------------------------------------------------------------------
BOOL KNpcResNode::GetSort(int nActNo, int nDir, int nFrameNo, int *lpnTable,
                          int nTableLen) {
  // 普通npc
  if (m_nNpcKind == NPC_RES_NORMAL) {
...
// 功能:	得到动作种类数
//---------------------------------------------------------------------------
int CActionName::GetActionCount() { return m_nCurActionNo; }
...
//---------------------------------------------------------------------------
BOOL CShadowFileName::GetFile(int nActionNo, int *pnFrame, int *pnDir,
                              int *pnInterval, int *pnCgX, int *pnCgY,
                              char *lpszName) {
...
//---------------------------------------------------------------------------
void CStateMagicTable::GetInfo(int nNo, char *lpszGetName, int *pnType,
                               int *pnPlayType, int *pnBackStart,
                               int *pnBackEnd, int *pnTotalFrame,
                               int *pnTotalDir, int *pnInterVal) {
  if (!lpszGetName)
    return;
...
//---------------------------------------------------------------------------
BOOL CRESINFO::GetName(int nActionNo, int nEquipNo, char *lpszDefault,
                       char *lpszGetName, int nStrLen) {
  if (!lpszGetName)
    return FALSE;
  if (m_cSprInfo == NULL)
    goto FALSE_LAB;
  if (nActionNo < 0 || nActionNo >= m_nActionKind || nEquipNo < 0 ||
      nEquipNo >= m_nEquipKind)
    goto FALSE_LAB;

  strcpy(lpszGetName,
         m_cSprInfo[nEquipNo * m_nActionKind + nActionNo].szFileName);
...
// 功能:	析构函数
//---------------------------------------------------------------------------
CEquipStyleTable::~CEquipStyleTable() { AutoDelete(); }
...
//---------------------------------------------------------------------------
// 功能:	从字符串中把一系列数字取出来
// 参数:	lpszStr : 字符串  lpnTable : 接受数字的数组  nTableSize :
// 数组的大小
//---------------------------------------------------------------------------
...
// 返回:	读取成功与否
//---------------------------------------------------------------------------
BOOL CSortTable::GetTable(char *lpszFileName, CActionName *cActionName,
                          int nPartNum) {
  if (!lpszFileName || !lpszFileName[0])
    return FALSE;
...
BOOL CSortTable::GetTable(KIniFile *pIni, CActionName *cActionName,
                          int nPartNum) {
  //	if ( !lpszFileName || !lpszFileName[0])
  //		return FALSE;
  if (nPartNum > 1)
  ...
  for (i = 0; i < MAX_SORT_DIR; i++) {
    sprintf(szKey, "Dir%d", i + 1);
    if (pIni->GetString("DEFAULT", szKey, "", szBuffer, sizeof(szBuffer))) {
      SortStrToNum(szBuffer, &m_lpnSortTable[nTablePos * (m_nPartNum + 1)],
                   m_nPartNum + 1);
    }
    nTablePos++;
  }
  ...
  for (i = 0; i < nActNo; i++) {
    ...
    if (m_sActTableOff[i].bUseDefault == FALSE) {
      for (j = 0; j < MAX_SORT_DIR; j++) {
        ...
        if (pIni->GetString(szActName, szKey, "", szBuffer, sizeof(szBuffer))) {
          ...
          memcpy(&m_lpnSortTable[nTablePos * (m_nPartNum + 1)],
                 &m_lpnSortTable[j * (m_nPartNum + 1)],
    ...
    if ((m_sActTableOff[i].bUseDefault == FALSE &&
         m_sActTableOff[i].nLineNum > MAX_SORT_DIR) ||
        (m_sActTableOff[i].bUseDefault == TRUE &&
         m_sActTableOff[i].nLineNum > 0)) {
      j = 1;
      while (1) {
        sprintf(szKey, "Line%d", j);
        if (!pIni->GetString(szActName, szKey, "", szBuffer, sizeof(szBuffer)))
...
// 返回:	读取成功与否
//---------------------------------------------------------------------------
BOOL CSortTable::GetSort(int nActNo, int nDir, int nFrameNo, int *lpnTable,
                         int nTableLen) {
  if (!lpnTable)
    return FALSE;
  ...
  if (m_sActTableOff[nActNo].nActOff == 0) {
    if (nTableLen <= m_nPartNum)
      memcpy(lpnTable, &m_lpnSortTable[nDir * (m_nPartNum + 1) + 1],
    ...
    if (m_sActTableOff[nActNo].bUseDefault == TRUE) {
      ...
      for (i = 0; i < m_sActTableOff[nActNo].nLineNum; i++) {
        // 找到特殊桢
        if (nFrameNo == m_lpnSortTable[(m_sActTableOff[nActNo].nActOff + i) *
                                       (m_nPartNum + 1)]) {
          if (nTableLen <= m_nPartNum)
            memcpy(lpnTable,
                   &m_lpnSortTable[(m_sActTableOff[nActNo].nActOff + i) *
                                       (m_nPartNum + 1) +
                                   1],
                   sizeof(int) * nTableLen);
          else {
    ...
    // 方向不使用默认
      ...
      if (nTableLen <= m_nPartNum)
        memcpy(lpnTable,
               &m_lpnSortTable[(m_sActTableOff[nActNo].nActOff + nDir) *
                                   (m_nPartNum + 1) +
                               1],
               sizeof(int) * nTableLen);
      else {
