...
KNpcSet::KNpcSet() { m_dwIDCreator = 1000; }
...
void KNpcSet::Init() {
  ...
  if (cPKIni.Load(PLAYER_PK_RATE_FILE)) {
    ...
    cPKIni.GetInteger("PK", "FactionPKFaction", 1,
                      &m_nFactionPKFactionAddPKValue);
    cPKIni.GetInteger("PK", "KillerPKFaction", 1,
                      &m_nKillerPKFactionAddPKValue);
...
void KNpcSet::LoadPlayerBaseValue(LPSTR szFile) {
  ...
  File.GetInteger("Common", "AttackFrame", 20,
                  &m_cPlayerBaseValue.nAttackFrame);
#ifndef _SERVER
  ...
  File.GetInteger("Female", "StandFrame", 15,
                  &m_cPlayerBaseValue.nStandFrame[1]);
#endif
  File.Clear();
}
...
// Add by Freeway Chen in 2003.6.29
NPC_RELATION KNpcSet::GenOneRelation(NPCKIND Kind1, NPCKIND Kind2,
                                     NPCCAMP Camp1, NPCCAMP Camp2) {
  // 路人NPC没有战斗关系
    ...
    for (nKind2 = 0; nKind2 < kind_num; nKind2++) {
      for (nCamp1 = 0; nCamp1 < camp_num; nCamp1++) {
        for (nCamp2 = 0; nCamp2 < camp_num; nCamp2++) {
          m_RelationTable[nKind1][nKind2][nCamp1][nCamp2] =
              GenOneRelation((NPCKIND)nKind1, (NPCKIND)nKind2, (NPCCAMP)nCamp1,
                             (NPCCAMP)nCamp2);
        }
      }
    }
...
int KNpcSet::FindFree() { return m_FreeIdx.GetNext(0); }

#ifndef _SERVER
//---------------------------------------------------------------------------
//	功能：添加一个客户端npc（需要设定ClientNpcID）
//---------------------------------------------------------------------------
int KNpcSet::AddClientNpc(int nTemplateID, int nRegionX, int nRegionY,
                          int nMpsX, int nMpsY, int nNo) {
  ...
  if (nRegion < 0)
    return 0;
  nNpcNo =
      this->Add(nNpcSettingIdxInfo, 0, nRegion, nMapX, nMapY, nOffX, nOffY);
  if (nNpcNo > 0) {
    ...
    SubWorld[0].m_Region[nRegion].DecRef(Npc[nNpcNo].m_MapX, Npc[nNpcNo].m_MapY,
                                         obj_npc);
  }
...
int KNpcSet::Add(int nSubWorld, void *pNpcInfo) {
  ...
  int nNpcSettingIdxInfo =
      MAKELONG(pKSNpcInfo->nLevel, pKSNpcInfo->nTemplateID);
  ...
  if (nRet) {
    ...
    if (pKSNpcInfo->szScript[0]) {
      if (pKSNpcInfo->szScript[0] == '.')
        g_StrCpyLen(Npc[nRet].ActionScript, &pKSNpcInfo->szScript[1],
                    sizeof(Npc[nRet].ActionScript));
      else
        g_StrCpyLen(Npc[nRet].ActionScript, pKSNpcInfo->szScript,
                    sizeof(Npc[nRet].ActionScript));
      // 保持小写，保证脚本对应关系
      g_StrLower(Npc[nRet].ActionScript);
      Npc[nRet].m_ActionScriptID = g_FileName2Id(Npc[nRet].ActionScript);
      g_DebugLog("[Script]Npc %s,%d", Npc[nRet].ActionScript,
                 Npc[nRet].m_ActionScriptID);
    } else {
...
int KNpcSet::Add(int nNpcSettingIdxInfo, int nSubWorld, int nMpsX, int nMpsY) {
  ...
  if (nSubWorld < 0 || nSubWorld >= MAX_SUBWORLD)
    return 0;
  SubWorld[nSubWorld].Mps2Map(nMpsX, nMpsY, &nRegion, &nMapX, &nMapY, &nOffX,
                              &nOffY);
  if (nRegion < 0)
    return 0;
  return Add(nNpcSettingIdxInfo, nSubWorld, nRegion, nMapX, nMapY, nOffX,
             nOffY);
}

int KNpcSet::Add(int nNpcSettingIdxInfo, int nSubWorld, int nRegion, int nMapX,
                 int nMapY, int nOffX /* = 0 */, int nOffY /* = 0 */) {
  int i = FindFree();

  if (i == 0) {
    printf("Xay ra loi gioi han NPC !\n");
    return 0;
  }
  ...
  Npc[i].Remove();
  for (int j = 0; j < MAX_FLOOD_SHOW; j++) {
    Npc[i].ClearBlood(j);
  }
#endif
  ...
  SubWorld[nSubWorld].Map2Mps(nRegion, nMapX, nMapY, nOffX, nOffY,
                              &Npc[i].m_OriginX, &Npc[i].m_OriginY);
  Npc[i].m_PrevX = Npc[i].m_OriginX;
  Npc[i].m_PrevY = Npc[i].m_OriginY;

#ifdef _SERVER
  SetID(i);

  m_nNumberCountNpcSet++;
#endif
  // 修改可用与使用表
  m_FreeIdx.Remove(i);
  m_UseIdx.Insert(i);
  SubWorld[nSubWorld].m_Region[nRegion].AddNpc(
      i); // m_WorldMessage.Send(GWM_NPC_ADD, nRegion, i);
...
void KNpcSet::Remove(int nIdx) {
  ...
  if (nSubWorld >= 0 && nSubWorld <= MAX_SUBWORLD && nRegion >= 0 &&
      nRegion <= SubWorld[nSubWorld].m_nTotalRegion) {
    POINT POff[8] = {
        {0, 32},  {-16, 32}, {-16, 0}, {-16, -32},
        {0, -32}, {16, -32}, {16, 0},  {16, 32},
    };
    int nMaxCount = MAX_BROADCAST_COUNT;
    SubWorld[nSubWorld].m_Region[nRegion].BroadCast(
        &NetCommand, sizeof(NetCommand), nMaxCount, Npc[nIdx].m_MapX,
        Npc[nIdx].m_MapY);
    ...
    for (int i = 0; i < 8; i++) {
      ...
      if (nConRegion == -1)
        continue;
      SubWorld[nSubWorld].m_Region[nConRegion].BroadCast(
          &NetCommand, sizeof(NetCommand), nMaxCount,
          Npc[nIdx].m_MapX - POff[i].x, Npc[nIdx].m_MapY - POff[i].y);
    }
  }
...
void KNpcSet::InsertNpcToRegion(int nRegionIdx) {
  ...
  while (1) {
    ...
    if (Npc[nIdx].m_sClientNpcID.m_dwRegionID > 0 &&
        Npc[nIdx].m_dwRegionID ==
            (DWORD)SubWorld[0].m_Region[nRegionIdx].m_RegionID) {
...
int KNpcSet::GetDistance(int nIdx1, int nIdx2) {
  ...
  if (Npc[nIdx1].m_RegionIndex == Npc[nIdx2].m_RegionIndex) {
    int XOff = (Npc[nIdx1].m_MapX - Npc[nIdx2].m_MapX) *
               SubWorld[Npc[nIdx1].m_SubWorldIndex].m_nCellWidth;
    XOff += (Npc[nIdx1].m_OffX - Npc[nIdx2].m_OffX) >> 10;

    int YOff = (Npc[nIdx1].m_MapY - Npc[nIdx2].m_MapY) *
               SubWorld[Npc[nIdx1].m_SubWorldIndex].m_nCellHeight;
    YOff += (Npc[nIdx1].m_OffY - Npc[nIdx2].m_OffY) >> 10;

    nRet = (int)sqrt(XOff * XOff + YOff * YOff);
  } else {
    int X1, Y1;
    SubWorld[Npc[nIdx1].m_SubWorldIndex].Map2Mps(
        Npc[nIdx1].m_RegionIndex, Npc[nIdx1].m_MapX, Npc[nIdx1].m_MapY,
        Npc[nIdx1].m_OffX, Npc[nIdx1].m_OffY, &X1, &Y1);
    int X2, Y2;
    SubWorld[Npc[nIdx2].m_SubWorldIndex].Map2Mps(
        Npc[nIdx2].m_RegionIndex, Npc[nIdx2].m_MapX, Npc[nIdx2].m_MapY,
        Npc[nIdx2].m_OffX, Npc[nIdx2].m_OffY, &X2, &Y2);
...
BOOL KNpcSet::SyncNpc(DWORD dwID, int nClientIdx) {
  ...
  if (nFindIndex <= 0) {
    ...
    if (g_pServer)
      g_pServer->PackDataToClient(nClientIdx, &RequestFail,
                                  sizeof(NPC_REQUEST_FAIL));
    return FALSE;
  }
...
void KNpcSet::CheckBalance() {
  ...
  while (nIdx) {
    ...
    else if (nIdx != Player[CLIENT_PLAYER_INDEX].m_nIndex &&
  ...
  while (nIdx) {
    ...
    if (SubWorld[0].m_dwCurrentTime - m_RequestNpc[nIdx].dwRequestTime > 100) {
      ...
      g_DebugLog("[Request]Remove %d from %d on %d timeout", dwID, nIdx,
...
//-------------------------------------------------------------------------
BOOL KNpcSet::CheckShowName() { return m_nShowPateFlag & PATE_NAME; }
#endif
...
//-------------------------------------------------------------------------
BOOL KNpcSet::CheckShowLife() { return m_nShowPateFlag & PATE_LIFE; }
#endif
...
//-------------------------------------------------------------------------
BOOL KNpcSet::CheckShowChat() { return m_nShowPateFlag & PATE_CHAT; }
#endif
...
//-------------------------------------------------------------------------
BOOL KNpcSet::CheckShowMana() { return m_nShowPateFlag & PATE_MANA; }
#endif
...
int KNpcSet::SearchNpcAt(int nX, int nY, int nRelation, int nRange) {
  ...
  while (1) {
    ...
    if (Npc[nIdx].m_InvisibilityState.nTime > 0)
      continue;
    ...
    int x, y;
    SubWorld[0].Map2Mps(Npc[nIdx].m_RegionIndex, Npc[nIdx].m_MapX,
                        Npc[nIdx].m_MapY, Npc[nIdx].m_OffX, Npc[nIdx].m_OffY,
                        &x, &y);
...
BOOL KNpcSet::InsertNpcRequest(DWORD dwID) {
  ...
  g_DebugLog("[Request]Insert %d at %d on %d", dwID, nIndex,
             SubWorld[0].m_dwCurrentTime);
...
void KNpcSet::RemoveNpcRequest(DWORD dwID) {
  ...
  g_DebugLog("[Request]Remove %d from %d on %d", dwID, nIndex,
             SubWorld[0].m_dwCurrentTime);
}
...
NPC_RELATION KNpcSet::GetRelation(int nId1, int nId2) {
  ...
  _ASSERT(
      ...
      ((Npc[nId1].m_CurrentCamp >= 0) &&
       (Npc[nId1].m_CurrentCamp < camp_num)) &&
  ...
  if (Player[CLIENT_PLAYER_INDEX].m_nIndex != nId1 &&
    ...
    if ((Npc[nId1].m_Kind == kind_player && Npc[nId2].m_Kind != kind_player &&
        ...
        (Npc[nId2].m_Kind == kind_player && Npc[nId1].m_Kind != kind_player &&
         !Npc[nId2].m_FightMode))
      return relation_none;

    if (Npc[nId1].m_Kind == kind_player && Npc[nId2].m_Kind == kind_partner)
      return relation_none;

    if (Npc[nId1].m_TypeNpc == 3 && Npc[nId2].m_TypeNpc != 3)
      return relation_none;

    if (Npc[nId2].m_TypeNpc == 3 &&
    ...
    if (Npc[nId2].m_Kind == kind_partner)
      return relation_none;
  ...
  } else // if (Player[CLIENT_PLAYER_INDEX].m_nIndex == nId2)
  {
    ...
    if (Npc[nId1].m_Kind != kind_player && Npc[nId1].m_Kind != kind_dialoger &&
        !Npc[nId2].m_FightMode)
      return relation_none;

    if (Npc[nId1].m_Kind == kind_partner)
      return relation_none;

    if (Npc[nId1].m_TypeNpc == 3)
      return relation_none;
  ...
  if (Npc[nId1].m_Kind != kind_player || Npc[nId2].m_Kind != kind_player) {
    ...
    if ((Npc[nId1].m_Kind == kind_player && !Npc[nId1].m_FightMode) ||
    ...
    if (Player[Npc[nId1].m_nPlayerIdx].m_cPK.GetExercisePKAim() ==
        Npc[nId2].m_nPlayerIdx)
      return relation_enemy;
    if (Player[Npc[nId1].m_nPlayerIdx].m_cPK.GetEnmityPKState() ==
            enumPK_ENMITY_STATE_PKING &&
...
void KInstantSpecial::LoadSoundName() {
  ...
  for (i = 0; i < MAX_INSTANT_SOUND; i++) {
    sprintf(szTemp, "%d", i);
    cSoundName.GetString("Game", szTemp, "", this->m_szSoundName[i],
                         sizeof(m_szSoundName[i]));
  }
}
...
void KInstantSpecial::PlaySound(int nNo) {
  ...
  m_pSoundNode = (KCacheNode *)g_SoundCache.GetNode(m_szSoundName[nNo],
