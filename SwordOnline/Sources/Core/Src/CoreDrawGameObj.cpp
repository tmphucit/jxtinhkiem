...
void CoreDrawGameObj(unsigned int uObjGenre, unsigned int uId, int x, int y,
...
#define ITEM_CELL_HEIGHT 26
  ...
  case CGOG_MENU_NPC:
    if (nParam) {
      ((KNpcRes *)nParam)
          ->SetPos(0, x + Width / 2, y + Height / 2 + 28, 0, FALSE, TRUE);
...
void CoreGetGameObjLightInfo(unsigned int uObjGenre, unsigned int uId,
                             KLightInfo *pLightInfo) {
  switch (uObjGenre) {
  case CGOG_NPC:
    if (uId > 0 && uId < MAX_NPC) {
      ...
      pLightInfo->dwColor = 0xff000000 | Npc[uId].m_RedLum << 16 |
                            Npc[uId].m_GreenLum << 8 | Npc[uId].m_BlueLum;
