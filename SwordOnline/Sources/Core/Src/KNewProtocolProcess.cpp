...
KNewProtocolProcess::KNewProtocolProcess() : m_theAllocator(1024, 500) {
  m_ProtocolTable[MAKEWORD(pf_gamemaster, gm_c2s_execute)] = P_ProcessGMExecute;
  m_ProtocolTable[MAKEWORD(pf_gamemaster, gm_c2s_findplayer)] =
      P_ProcessGMFindPlayer;

  m_ProtocolTable[MAKEWORD(pf_playercommunity, playercomm_channelchat)] =
      P_ProcessPlayerCommExtend;
  m_ProtocolTable[MAKEWORD(pf_playercommunity, playercomm_someonechat)] =
      P_ProcessPlayerCommExtend;
  m_ProtocolTable[MAKEWORD(pf_playercommunity,
...
void KNewProtocolProcess::ProcessNetMsg(IClient *pTransfer, DWORD dwFromIP,
                                        DWORD dwFromRelayID, int nPlayerIndex,
                                        BYTE *pData, int nDataSize) {
...
void KNewProtocolProcess::SendNetMsgToTransfer(IClient *pClient) {
  if (pClient) {
    ...
    for (it = m_theTransList.begin(); it != m_theTransList.end(); ++it) {
      ...
      if (pBuffer) {
        pClient->SendPackToServer((const void *)(pBuffer->GetBuffer()),
                                  pBuffer->GetUsed());
...
void KNewProtocolProcess::PushMsgInTransfer(const void *const pData,
...
void KNewProtocolProcess::PushMsgInChat(const void *const pData,
                                        const size_t datalength) {
  if (pData && datalength > 0) {
    CBuffer *pBuffer = m_theAllocator.Allocate();
...
void KNewProtocolProcess::PushMsgInTong(const void *const pData,
                                        const size_t datalength) {
  if (pData && datalength > 0) {
    CBuffer *pBuffer = m_theAllocator.Allocate();
...
void KNewProtocolProcess::BroadcastGlobal(const void *const pData,
                                          const size_t datalength) {
  if (pData && datalength > 0) {
    CBuffer *pBuffer = m_theAllocator.Allocate();
...
void KNewProtocolProcess::BroadcastLocalServer(const void *const pData,
                                               const size_t datalength) {
  if (pData && datalength > 0) {
    int nTargetIdx;
    nTargetIdx = PlayerSet.GetFirstPlayer();
    while (nTargetIdx) {
      g_pServer->PackDataToClient(Player[nTargetIdx].m_nNetConnectIdx, pData,
                                  datalength);
...
void KNewProtocolProcess::P_ProcessPlayerCommExtend(
    IClient *pTransfer, DWORD dwFromIP, DWORD dwFromRelayID, int nPlayerIndex,
    BYTE *pData, int nDataSize) {
  ...
  g_pServer->PackDataToClient(Player[nPlayerIndex].m_nNetConnectIdx, pExHeader,
                              pckgsize);
#ifndef WIN32
