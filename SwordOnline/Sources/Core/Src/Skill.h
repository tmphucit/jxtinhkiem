...
public:
  ...
  virtual unsigned short GetSkillReqLevel() const { return 0; };
  virtual void LoadSkillLevelData(unsigned long ulLevel, int nParam) = 0;
  virtual BOOL CanCastSkill(int nLauncher, int &nParam1,
                            int &nParam2) const = 0;
  ...
  virtual BOOL IsSkillPhysical() const { return FALSE; };
  virtual BOOL IsMelee() const { return FALSE; };
  virtual BOOL IsUseAR() const { return FALSE; };
  virtual BOOL IsSkillMagic() const { return FALSE; };
