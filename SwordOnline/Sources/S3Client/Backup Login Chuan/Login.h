...
struct KLoginServer {
  ...
  unsigned char Address_1[4]; // 服务器ip地址
  unsigned char Address_2[4]; // 服务器ip地址
};
...
public:
  ...
  // 传入帐号密码，账号登陆
  int AccountLogin(const char *pAccount, const KSG_PASSWORD &crPassword,
                   bool bOrignPassword = true);
  // 选中游戏角色
...
private:
  //====各操作的网络响应的返回处理====
  void ProcessAccountLoginResponse(
      KLoginStructHead *pResponse);                      // 处理账号登陆的响应
  void ProcessRoleListResponse(TProcessData *pResponse); // 角色列表返回
  void ProcessDeleteRoleResponse(
      tagNewDelRoleResponse *pResponse); // 删除角色列表的响应
  void ProcessCreateRoleResponse(
      tagNewDelRoleResponse *pResponse); // 新建角色列表的响应
  void ProcessToLoginGameServResponse(
      tagNotifyPlayerLogin *pResponse); // 准备登陆游戏服务器的响应
  ...
  void SetAccountPassword(const char *pszAccount,
