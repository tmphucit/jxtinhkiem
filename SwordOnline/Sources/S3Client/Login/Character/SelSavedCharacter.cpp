...
KSelSavedCharacter::~KSelSavedCharacter() { FreeData(); }
...
//--------------------------------------------------------------------------
int KSelSavedCharacter::GetStatus() { return ((int)m_Status); }
...
//	功能：获取角色的数目
//--------------------------------------------------------------------------
int KSelSavedCharacter::GetCharacterNum() { return m_nNumCharacter; }
...
int KSelSavedCharacter::NewCharacter(KNewCharacterInfo *pData) {
  if (!pData)
    return false;
  if (pData->Gender < 0 || pData->Gender >= ROLE_NO || pData->Attribute < 0 ||
      pData->Attribute >= series_num)
    return false;
  ...
  pNetCommand->nDataLen = sizeof(NEW_PLAYER_COMMAND) - sizeof(pInfo->m_szName) +
                          nNameLen + 1 /* sizeof( '\0' ) */;
...
int KSelSavedCharacter::DeleteCharacter(int nIndex, const char *pszPassword) {
  ...
  strncpy(NetCommand.szRoleName, m_BaseInfo[nIndex].szName,
...
void KSelSavedCharacter::SetCharacterBaseInfo(int nNum,
                                              const RoleBaseInfo *pInfo) {
  ...
  if (m_Status != SSC_S_STANDBY && m_Status != SSC_S_IDLE) {
    ...
    case s2c_roleserver_getrolelist_result:
      if (m_Status == SSC_S_LOADING_DATA) {
        ...
        SetCharacterBaseInfo(pProtocol->pDataBuffer[0],
                             (RoleBaseInfo *)&pProtocol->pDataBuffer[1]);
    ...
    case s2c_notifyplayerlogin:
      if (m_Status == SSC_S_LOADING_CHARACTER) {
        ...
        if (strcmp((const char *)pNPL->szRoleName, m_szProcessingRoleName) ==
                0 &&
            pNPL->bPermit) {
          // 开始与GameSvr进行连接
          if (g_NetConnectAgent.ConnectToGameSvr(
                  (const unsigned char *)&pNPL->nIPAddr, pNPL->nPort,
                  &pNPL->guid)) {
            g_NetConnectAgent.UpdateClientRequestTime(false);
    ...
    case s2c_rolenewdelresponse:
      ...
      //m_szProcessingRoleName) == 0)	/* 刘鹏调试版 */
      {
