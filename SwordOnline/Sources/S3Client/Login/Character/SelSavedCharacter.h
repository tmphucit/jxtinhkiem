...
表示成功)，而不是操作的实际结果。通过GetLastActionResult方法可以知道最后一次操作的执行结果。
        ...
        2
调用LoadData方法请求载入存档角色数据，载入角色数据时处于'载入角色数据中'状态(SSC_S_LOADING_DATA)，
        如果成功则切换为'角色已经载入'状态(SSC_S_STANDBY)，如果失败则返回'空闲'状态(SSC_S_IDLE)。
        3
处于SSC_S_STANDBY状态的时候可以执行NewCharacter、DeleteCharacter、SelCharacter操作。
        4
成功执行NewCharacter后转入'正在新建角色'状态(SSC_S_CREATING_CHARACTER)，操作结束后，
        无论成功与否都转入SSC_S_STANDBY状态。操作的执行结果通过GetLastActionResult方法获得。
        5
成功执行DeleteCharacter后转入'正在删除角色'状态(SSC_S_DELETING_CHARACTER)，操作结束后，
        无论成功与否都转入SSC_S_STANDBY状态。操作的执行结果通过GetLastActionResult方法获得。
        6
成功执行SelCharacter后转入'把角色载入游戏中'状态(SSC_S_LOADING_CHARACTER)，操作结束后，
...
public:
  ...
  int GetCharacterInfo(int nIndex,
                       KNewCharacterInfo *pInfo); // 获取某个角色的信息
