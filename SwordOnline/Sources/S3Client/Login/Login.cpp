...
bool GetIpAddress(const char *szAddress, unsigned char *pcAddress) {
  _ASSERT(pcAddress);
  int nValue[4];
  int nRet = sscanf(szAddress, "%d.%d.%d.%d", &nValue[0], &nValue[1],
                    &nValue[2], &nValue[3]);
  if (nRet == 4 && nValue[0] >= 0 && nValue[0] < 256 && nValue[1] >= 0 &&
      nValue[1] < 256 && nValue[2] >= 0 && nValue[2] < 256 && nValue[3] >= 0 &&
      nValue[3] < 256) {
...
//	功能：析构函数
//--------------------------------------------------------------------------
KLogin::~KLogin() { _ASSERT(m_Status == LL_S_IDLE); }
...
int KLogin::CreateConnection(const unsigned char *pAddress,
                             const unsigned char *pAddress_1,
                             const unsigned char *pAddress_2, int nSlect) {
  // m_Slect = nSlect;
  // g_DebugLog("Test %d",nSlect);
  int nRet;
  if (m_Status == LL_S_IDLE && pAddress &&
      (ConnectAccountServer(pAddress) || ConnectAccountServer(pAddress_1) ||
       ConnectAccountServer(pAddress_2))) {
    RegistNetAgent();
...
//			  失败 状态保持
//--------------------------------------------------------------------------
int KLogin::AccountLogin(const char *pszAccount, const KSG_PASSWORD &crPassword,
                         bool bOrignPassword) {
  int nRet;
  if (m_Status == LL_S_WAIT_INPUT_ACCOUNT && pszAccount &&
...
int KLogin::SelectRole(int nIndex) {
  ...
  if (m_Status == LL_S_ROLE_LIST_READY && nIndex >= 0 && nIndex < m_nNumRole) {
    ...
    GetAccountPassword(NetCommand.szAccountName, &NetCommand.Password);
...
int KLogin::CreateRole(KRoleChiefInfo *pCreateInfo) {
  ...
  if (m_Status == LL_S_ROLE_LIST_READY && pCreateInfo &&
      m_nNumRole < MAX_PLAYER_PER_ACCOUNT && pCreateInfo->Attribute >= 0 &&
      pCreateInfo->Attribute < series_num) {
    ...
    if (nNameLen >= 1 && nNameLen < sizeof(pCreateInfo->Name)) {
      ...
      NEW_PLAYER_COMMAND *pInfo =
          (NEW_PLAYER_COMMAND *)pNetCommand->pDataBuffer;
      ...
      pNetCommand->nDataLen = sizeof(NEW_PLAYER_COMMAND) -
                              sizeof(pInfo->m_szName) + nNameLen +
                              1 /* sizeof( '\0' ) */;
      pNetCommand->ulIdentity = 0;

      g_NetConnectAgent.SendMsg(&Data, sizeof(TProcessData) -
                                           sizeof(pNetCommand->pDataBuffer) +
...
int KLogin::DeleteRole(int nIndex, const KSG_PASSWORD &crSupperPassword) {
  ...
  if (m_Status == LL_S_ROLE_LIST_READY && nIndex >= 0 && nIndex < m_nNumRole) {
    ...
    RandMemSet(sizeof(tagDBDelPlayer),
               (BYTE *)&NetCommand); // random memory for make a cipher
    ...
    strncpy(NetCommand.szRoleName, m_RoleList[nIndex].Name,
            sizeof(NetCommand.szRoleName));
...
void KLogin::AutoLogin() {
  ...
  if (IsAutoLoginEnable()) {
    ...
    // if (nRandomConect == 1)
    //	{
    CreateConnection(m_Choices.AccountServer.Address_1,
                     m_Choices.AccountServer.Address_2,
                     m_Choices.AccountServer.Address, m_Slect);
    //		}
    /*	else if (nRandomConect == 2)
            {
            --CreateConnection(m_Choices.AccountServer.Address_2,m_Choices.AccountServer.Address,m_Choices.AccountServer.Address_1,m_Slect);
            }
            else
            {
            CreateConnection(m_Choices.AccountServer.Address,m_Choices.AccountServer.Address_1,m_Choices.AccountServer.Address_2,m_Slect);
            }
...
int KLogin::IsAutoLoginEnable() {
  return ((~m_Choices.Account[0]) && (~m_Choices.Password.szPassword[0]) &&
...
void KLogin::ProcessAccountLoginResponse(KLoginStructHead *pResponse) {
  ...
  if (((pResponse->Param & LOGIN_ACTION_FILTER) ==
       LOGIN_A_LOGIN) &&                            // 操作性为要匹配
      pResponse->Size >= sizeof(KLoginAccountInfo)) // 数据内容的大小也要匹配
  {
    ...
    if (strcmp(pInfo->Account, szAccount) == 0 &&
      ...
      if (nResult == LOGIN_R_SUCCESS) {
        ...
        if (m_Status != LL_S_WAIT_INPUT_ACCOUNT || m_bInAutoProgress) {
          ReturnToIdle();
        }
        m_Result = eResult;
      }
...
void KLogin::ProcessCreateRoleResponse(tagNewDelRoleResponse *pResponse) {
  ...
  if (pResponse->cProtocol == s2c_rolenewdelresponse) {
    //	if (strcmp(pResponse->szRoleName, m_Choices.szProcessingRoleName) == 0)
    ////to be check/* 刘鹏调试版 */
    {
...
void KLogin::ProcessToLoginGameServResponse(tagNotifyPlayerLogin *pResponse) {
  ...
  if (pResponse->cProtocol == s2c_notifyplayerlogin) {
    if (strcmp((const char *)pResponse->szRoleName,
               m_Choices.szProcessingRoleName) == 0) {
      g_NetConnectAgent.UpdateClientRequestTime(true);

      // 开始与GameSvr进行连接

      g_DebugLog("Test 3: %d", m_Slect);

      if (m_Slect == 0 || m_Slect == 10 || m_Slect == 20) {
        if (g_NetConnectAgent.ConnectToGameSvr(
                (const unsigned char *)&pResponse->nIPAddr2, pResponse->nPort2,
                &pResponse->guid)) {
          g_DebugLog("Test 3x: 1");
        ...
        if (g_NetConnectAgent.ConnectToGameSvr(
                (const unsigned char *)&pResponse->nIPAddr1, pResponse->nPort1,
                &pResponse->guid)) {
          m_Status = LL_S_ENTERING_GAME;
          m_Result = LL_R_NOTHING;
        } else {
...
void KLogin::AcceptNetMsg(void *pMsgData) {
  ...
  case LL_S_ACCOUNT_CONFIRMING:
    ProcessAccountLoginResponse(
        (KLoginStructHead *)(((char *)pMsgData) + PROTOCOL_MSG_SIZE));
    break;
...
void KLogin::LoadLoginChoice() {
  ...
  if (pSetting) {
    ...
    if (szAccount[0]) {
      ...
      pSetting->GetStruct($LOGIN, $LAST_PASSWORD, Password.szPassword,
                          sizeof(Password.szPassword));
    ...
    if (szAccount[0]) {
      ...
      if (pPrivate) {
        if (pPrivate->GetString("Main", "LastSelCharacter", "",
                                ...
                                sizeof(m_Choices.szProcessingRoleName))) {
          EDOneTimePad_Decipher(m_Choices.szProcessingRoleName,
                                strlen(m_Choices.szProcessingRoleName));
        }
      }
  ...
  if (pSetting) {
    ...
    if (m_Choices.bRememberAccount) {
      ...
      if (m_Choices.bRememberAll) {
        ...
        pSetting->WriteStruct($LOGIN, $LAST_PASSWORD, Password.szPassword,
                              sizeof(Password.szPassword));
      }
...
//	功能：登陆服务器列表获取
//--------------------------------------------------------------------------
KLoginServer *KLogin::GetServerList(int nRegion, int &nCount,
                                    int &nAdviceChoice) {
  KLoginServer *pServers = NULL;
  nCount = 0;
  nAdviceChoice = 0;
      ...
      if (nReadCount > 0) {
        ...
        if (pServers) {
          for (i = 0; i < nReadCount; i++) {
            ...
            if (!File.GetString(szSection, szKey, "", szBuffer,
                                sizeof(szBuffer)) ||
            ...
            sprintf(szKey, "%d_Address_1", i);
            if (!File.GetString(szSection, szKey, "", szBuffer,
                                sizeof(szBuffer)) ||
                GetIpAddress(szBuffer, pServers[nCount].Address_1) == false) {
              continue;
            }

            sprintf(szKey, "%d_Address_2", i);
            if (!File.GetString(szSection, szKey, "", szBuffer,
                                sizeof(szBuffer)) ||
                GetIpAddress(szBuffer, pServers[nCount].Address_2) == false) {
              continue;
            }
  ...
  if (nCount) {
    ...
    m_Choices.AccountServer.Address_1[0] = pServers[nAdviceChoice].Address_1[0];
    m_Choices.AccountServer.Address_1[1] = pServers[nAdviceChoice].Address_1[1];
    m_Choices.AccountServer.Address_1[2] = pServers[nAdviceChoice].Address_1[2];
    m_Choices.AccountServer.Address_1[3] = pServers[nAdviceChoice].Address_1[3];

    m_Choices.AccountServer.Address_2[0] = pServers[nAdviceChoice].Address_2[0];
    m_Choices.AccountServer.Address_2[1] = pServers[nAdviceChoice].Address_2[1];
    m_Choices.AccountServer.Address_2[2] = pServers[nAdviceChoice].Address_2[2];
    m_Choices.AccountServer.Address_2[3] = pServers[nAdviceChoice].Address_2[3];

    ////////////

    m_Slect = nAdviceChoice + nRegion * 10;
  }
  return pServers;
}
...
int KLogin::ConnectAccountServer(const unsigned char *pIpAddress) {
  g_DebugLog("Test %d", m_Slect);
  KIniFile IniFile;
  if (pIpAddress && IniFile.Load("\\Config.ini")) {
    int nPort;

    char GameServPort[32];
    sprintf(GameServPort, "%s%d", "GameServPort", m_Slect);
    IniFile.GetInteger("Server", GameServPort, 8888, &nPort);
    g_DebugLog("Test %s", GameServPort);
...
void KLogin::RegistNetAgent() {
  ...
  g_NetConnectAgent.RegisterMsgTargetObject(s2c_roleserver_getrolelist_result,
                                            this);
...
void KLogin::SetAccountPassword(const char *pszAccount,
                                const KSG_PASSWORD *pcPassword) {
  int i;
  if (pszAccount) {
