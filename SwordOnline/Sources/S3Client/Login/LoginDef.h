...
一些情况来说，捆绑的是一个以KLoginStructHead为第一个成员的更大的结构。
    ...
    一个完整的与账号服务器建立连接过程中消息往返如下：
        发送请求：c2s_login & KLoginAccountInfo; KLoginInfo::Param =
LOGIN_A_LOGIN | LOGIN_R_REQUEST; 服务器返回登陆结果：s2c_login &
KLoginAccountInfo; KLoginInfo::Param = LOGIN_A_LOGIN | (LOGIN_R_SUCCESS or
LOGIN_R_FAILED or LOGIN_R_ACCOUNT_OR_PASSWORD_ERROR); 登陆过程结束。

        申请账号过程过程中消息往返如下：(注意：只有在调试版本才可以提供此操作。)
        发送请求：c2s_login & KLoginAccountInfo; KLoginInfo::Param =
LOGIN_A_NEWACCOUNT | LOGIN_R_REQUEST; 服务器返回的申请结果：s2c_login &
KLoginAccountInfo; KLoginInfo::Param = LOGIN_A_NEWACCOUNT | (LOGIN_R_SUCCESS or
LOGIN_R_FAILED or LOGIN_R_ACCOUNT_OR_PASSWORD_ERROR or LOGIN_R_ACCOUNT_EXIST);
        申请过程结束。
  ...
  一个完整的从账号服务器获取游戏服务器列表过程中消息往返如下：
        ...
        服务器传送账号列表：s2c_login & KLoginServerList;
                KLoginServerList::Param 的值为 LOGIN_A_SERVERLIST |
(LOGIN_R_SUCCESS or LOGIN_R_VALID or LOGIN_R_INVALID or  LOGIN_R_FAILED);
                当列表包含的服务器项数目很多时，可以分多次分批发送，KLoginServerList::Param
的值为 (LOGIN_A_SERVERLIST |
LOGIN_R_VALID)时表示列表尚未传送完毕，下个数据包将继续传送未完列表，
                KLoginServerList::Param的值为LOGIN_A_SERVERLIST |
LOGIN_R_SUCCESS时候，表示列表传送结束。
...
#define LOGIN_R_INVALID_PROTOCOLVERSION                                        \
  10 // 协议版本不兼容，详见 KProtocolDef.h's KPROTOCOL_VERSION
#define LOGIN_R_FREEZE 11 // 被冻结

struct KLoginStructHead {
  unsigned short Size; // size of the struct, if the struct is been inherit, the
                       // size is ref the the derive struct
  int Param; // be one of the LOGIN_R_* define value combin with a LOGIN_A_*
             // value.
};
