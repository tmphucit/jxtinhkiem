...
//			  失败 状态保持
//--------------------------------------------------------------------------
int KLogin::CreateConnection(const unsigned char* pAddress,const unsigned char* pAddress_1,const unsigned char* pAddress_2,int nSlect)
{
	m_Slect = nSlect;
	int nRet;
	if (m_Status == LL_S_IDLE && pAddress && (
		ConnectAccountServer(pAddress) || 
		ConnectAccountServer(pAddress_1) || 
...
void KLogin::AutoLogin()
{
	...
	if (IsAutoLoginEnable())
	{
		...
		int nRandomConect = rand() % 3;
			...
			if (nRandomConect == 1)
			{
			CreateConnection(m_Choices.AccountServer.Address_1,m_Choices.AccountServer.Address_2,m_Choices.AccountServer.Address,m_Slect);	
			}
		...
		if (strcmp((const char*)pResponse->szRoleName, m_Choices.szProcessingRoleName) == 0)
		{
			...
			if (!(m_Slect%4))
			{
					...
					if (  g_NetConnectAgent.ConnectToGameSvr(
						(const unsigned char*)&pResponse->nIPAddr2,
						pResponse->nPort2, &pResponse->guid)
						)
					{
			...
			else if ((m_Slect%4) == 1)
			{
				...
				if (  g_NetConnectAgent.ConnectToGameSvr(
					(const unsigned char*)&pResponse->nIPAddr1,
					pResponse->nPort1, &pResponse->guid) ||
			...
			else if ((m_Slect%4) == 2)
			{
				...
				if (  g_NetConnectAgent.ConnectToGameSvr(
					(const unsigned char*)&pResponse->nIPAddr3,
					pResponse->nPort3, &pResponse->guid) ||
					g_NetConnectAgent.ConnectToGameSvr(
					(const unsigned char*)&pResponse->nIPAddr3,
					pResponse->nPort3, &pResponse->guid) ||
					g_NetConnectAgent.ConnectToGameSvr(
