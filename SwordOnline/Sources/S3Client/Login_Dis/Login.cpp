...
void KLogin::AutoLogin() {
  ...
  if (IsAutoLoginEnable()) {
    ...
    if (nRandomConect == 1) {
      ...
      CreateConnection(m_Choices.AccountServer.Address_2,
                       m_Choices.AccountServer.Address,
                       m_Choices.AccountServer.Address_1, m_Slect);
    } else {
      CreateConnection(m_Choices.AccountServer.Address,
                       m_Choices.AccountServer.Address_1,
                       m_Choices.AccountServer.Address_2, m_Slect);
    }
  }
}
...
void KLogin::ProcessToLoginGameServResponse(tagNotifyPlayerLogin *pResponse) {
  ...
  if (pResponse->cProtocol == s2c_notifyplayerlogin) {
    if (strcmp((const char *)pResponse->szRoleName,
      ...
      g_DebugLog("TEST CONNECT: %d ", m_Slect % 4);
      if (!(m_Slect % 4)) {
        ...
        g_DebugLog("TA DA VAO 2");
        if (g_NetConnectAgent.ConnectToGameSvr(
                (const unsigned char *)&pResponse->nIPAddr3, pResponse->nPort3,
                &pResponse->guid) ||
            g_NetConnectAgent.ConnectToGameSvr(
                (const unsigned char *)&pResponse->nIPAddr3, pResponse->nPort3,
                &pResponse->guid) ||
...
int KLogin::ConnectAccountServer(const unsigned char *pIpAddress) {
  ...
  if (pIpAddress && IniFile.Load("\\Config.ini")) {
    int nPort;
    int NewPort = m_Slect + m_NewRegion * 10;
    char GameServPort[32];
    sprintf(GameServPort, "%s%d", "GameServPort", NewPort);
