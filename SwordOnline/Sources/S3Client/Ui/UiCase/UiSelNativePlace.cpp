...
void KUiSelNativePlace::LoadList(int nPlaceId) {
  ...
  for (int i = 0; i < nCount; i++) {
    ...
    if (!File.GetString(szSection, "Name", "", m_pPlaceList[nNumValid].szName,
                        sizeof(m_pPlaceList->szName)))
      continue;
    ...
    if (File.GetString(szSection, "Desc", "", sBuffer, sizeof(sBuffer))) {
      ...
      m_pPlaceList[nNumValid].nDescLen =
          TEncodeText(sBuffer, m_pPlaceList[nNumValid].nDescLen);
      if (m_pPlaceList[nNumValid].nDescLen <
          sizeof(m_pPlaceList[nNumValid].sDesc))
        memcpy(m_pPlaceList[nNumValid].sDesc, sBuffer,
               m_pPlaceList[nNumValid].nDescLen);
      else
        m_pPlaceList[nNumValid].nDescLen = 0;
    }
...
//	功能：窗口函数
//--------------------------------------------------------------------------
int KUiSelNativePlace::WndProc(unsigned int uMsg, unsigned int uParam,
                               int nParam) {
  int nRet = 0;
  switch (uMsg) {
...
void KUiSelNativePlace::UpdateData() {
  ...
  m_PlaceDesc.SetText(m_pPlaceList[m_nLastSelPlace].sDesc,
                      m_pPlaceList[m_nLastSelPlace].nDescLen);
}