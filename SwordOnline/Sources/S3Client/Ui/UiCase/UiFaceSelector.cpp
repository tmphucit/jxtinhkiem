...
KUiFaceSelector *KUiFaceSelector::OpenWindow(KWndWindow *pCaller,
                                             void *pvParam) {
  if (m_pSelf == NULL) {
...
int KUiFaceSelector::ConvertFaceText(char *pDest, const char *pSrc,
                                     int nCount) {
...
//	功能：转换表情符
//--------------------------------------------------------------------------
int KUiFaceSelector::ConvertFace(char *pDest, int &nConvertCount,
                                 const char *pSrc, int nCount, int &nReadPos) {
...
void KUiFaceSelector::OnSelFace() {
  if (m_pCallerWnd && m_nCurrIndex >= 0 && m_nCurrIndex < ms_nNumFaces) {
    m_pCallerWnd->WndProc(WND_M_OTHER_WORK_RESULT,
                          (unsigned int)m_pvCallerParam,
                          (int)ms_pFaceList[m_nCurrIndex].szFaceText);
  }
  CloseWindow(false);
}

int KUiFaceSelector::WndProc(unsigned int uMsg, unsigned int uParam,
                             int nParam) {
  int nResult = false;
  switch (uMsg) {
  case WM_LBUTTONDOWN:
...
void KUiFaceSelector::LoadScheme(const char *pScheme) {
  ...
  if (Ini.Load(Buff)) {
    ...
    /*		if (ms_nNumFaces <= m_nNormColuCount)
                    {
                            m_Width = (MAX_SPACE + m_nBtnWidth) * ms_nNumFaces +
       MAX_SPACE; m_Height = 2 * MAX_SPACE + m_nBtnHeight;
                    }
                    else
                    {
                            int Lines = 0;
                            m_Width = (MAX_SPACE + m_nBtnWidth) *
       m_nNormColuCount + MAX_SPACE;
                            //if (ms_nNumFaces % m_nNormColuCount == 0)
                            //	Lines = ms_nNumFaces / m_nNormColuCount;
                            //else
                            //	Lines = ms_nNumFaces / m_nNormColuCount + 1;
                            m_Height = (MAX_SPACE + m_nBtnHeight) * m_nRowCount
       + MAX_SPACE;
                    }
    */
    if (m_bIsExpandMode == FALSE) {
...
void KUiFaceSelector::UpdateFaceTip(int x, int y) {
  if (m_nCurrIndex >= 0) {
    ...
    g_MouseOver.SetMouseHoverInfo((void *)(KWndWindow *)this, m_nCurrIndex, x,
                                  y, false, true);
  ...
  for (int nbRight = 0; nbRight <= 1; nbRight++) {
    ...
    for (v = 0; v < m_nRowCount; v++) {
      ...
      for (h = 0; h < nNumH; h++) {
        ...
        if (nIndex == m_nCurrIndex) { //===绘制选中表情图标的底色===
...
int KUiFaceSelector::GetFaceAtPos(int x, int y) {
  ...
  if (PtInWindow(x, y)) {
    ...
    if (x < m_nNormColuCount) {
      ...
      nIndex = m_nNormColuCount * m_nRowCount + m_nExpColuCount * y +
               (x - m_nNormColuCount);
    }
