...
KUiTongGetString::KUiTongGetString() { m_pRequester = NULL; }
...
KUiTongGetString *KUiTongGetString::OpenWindow(const char *pszTitle,
                                               ...
                                               unsigned int uParam, int nMinLen,
                                               int nMaxLen) {
  if (m_pSelf == NULL) {
...
//	功能：窗口消息函数
//--------------------------------------------------------------------------
int KUiTongGetString::WndProc(unsigned int uMsg, unsigned int uParam,
                              int nParam) {
  int nRet = 0;
  switch (uMsg) {
...
void KUiTongGetString::OnOk() {
  ...
  if (nLen >= m_nMinLen && nLen <= m_nMaxLen) {
    ...
    UIMessageBox(((nLen < m_nMinLen) ? "输入的字串太少！"
                                     : "输入的字数超过允许的最大长度！"));
  }
}
