...
void KUiTeamManage::UpdateData(KUiPlayerTeam *pInfo) {
  ...
  g_pCoreShell->GetGameData(GDI_PLAYER_LEADERSHIP, (unsigned int)&LeaderShip,
                            0);
  m_LeaderAbility.SetIntText(LeaderShip.nLeaderShipLevel);

  if (pInfo)
    m_Info = *pInfo;
  else if (!g_pCoreShell->TeamOperation(TEAM_OI_GD_INFO, (unsigned int)&m_Info,
                                        0)) {
  ...
  if (m_Info.cNumMember > 0) {
    m_pPlayersList =
        (KUiPlayerItem *)malloc(sizeof(KUiPlayerItem) * (m_Info.cNumMember));
    if (m_pPlayersList) {
      int nCount = g_pCoreShell->TeamOperation(TEAM_OI_GD_MEMBER_LIST,
                                               (unsigned int)m_pPlayersList,
                                               m_Info.cNumMember);
      _ASSERT(nCount == m_Info.cNumMember);
      m_TeamMember.SetContent(
          (unsigned char *)m_pPlayersList, m_Info.cNumMember,
...
/*void KUiTeamManage::UpdateNearbyTeams(KUiTeamItem* pList, int nCount)
{
        ...
        if (m_pSelf->m_NearbyTeamTab.IsButtonChecked())
        {
                ...
                if (pList && nCount)
                {
                        m_pSelf->m_pNearbyTeamsList =
(KUiTeamItem*)malloc(sizeof(KUiTeamItem) * nCount); if
(m_pSelf->m_pNearbyTeamsList)
                        {
                                memcpy(m_pSelf->m_pNearbyTeamsList, pList,
sizeof(KUiTeamItem) * nCount); m_pSelf->m_NearbyList.SetContent((unsigned
char*)m_pSelf->m_pNearbyTeamsList, nCount, sizeof(KUiTeamItem),
(int)&(((KUiTeamItem*)0)->Leader.Name));
                        }
                }
        }
}
...
void KUiTeamManage::OnAppoint() {
  ...
  if (nCurSel >= 0)
    g_pCoreShell->TeamOperation(TEAM_OI_APPOINT,
                                (unsigned int)&m_pPlayersList[nCurSel], 0);
}
...
void KUiTeamManage::OnKick() {
  int nCurSel = m_TeamMember.GetCurSel();
  if (nCurSel >= 0)
    g_pCoreShell->TeamOperation(TEAM_OI_KICK,
                                (unsigned int)&m_pPlayersList[nCurSel], 0);
}
...
void KUiTeamManage::OnInvite() {
  ...
  if (nCurSel >= 0) {
    ...
    g_pCoreShell->TeamOperation(
        TEAM_OI_INVITE, (unsigned int)(&m_pNearbyPlayersList[nCurSel]), 0);
  }
}
...
void KUiTeamManage::OnRefresh() {
  ...
  //	if (m_NearbyPlayerTab.IsButtonChecked())
  {
    ...
    if (nCount) {
      ...
      if (m_pNearbyPlayersList) {
        g_pCoreShell->GetGameData(GDI_NEARBY_IDLE_PLAYER_LIST,
                                  (unsigned int)m_pNearbyPlayersList, nCount);
        m_NearbyList.SetContent((unsigned char *)m_pNearbyPlayersList, nCount,
                                sizeof(KUiPlayerItem),
                                ((char *)&m_pNearbyPlayersList->Name -
                                 (char *)m_pNearbyPlayersList));
      }
    }
  }
  //	else
