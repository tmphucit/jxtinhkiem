...
int ServerNotify::RenameUnitGroup(const char *Unit, const char *Name,
                                  const char *NewName,
...
int ServerNotify::DeleteUnitGroup(const char *Unit, const char *Name,
                                  const STRINGLIST &friends) {
...
int ServerNotify::MoveUnitGroup(const char *Unit, const char *Name,
...
int ServerNotify::MoveUnitMember(const char *Unit, const char *Name,
                                 const char *Group) {
...
void ServerNotify::SendNotifyGroupFriend(const char *Unit,
  ...
  for (STRINGLIST::const_iterator itFriend = friends.begin();
       itFriend != friends.end(); itFriend++) {
...
KUiChatCentre *KUiChatCentre::OpenWindow(bool bShow) {
  ...
  if (m_pSelf && bShow) {
    ...
    m_pSelf->UpdateAll(m_pSelf->m_AllFriendUnits[m_pSelf->m_nCurentFriendUnit]
                           .m_nCurExpandGroup);
...
void KUiChatCentre::LoadScheme(class KIniFile *pIni) {
  ...
  if (pIni->GetString("MemberList", "OfflineColor", "0,0,0", Buff,
                      sizeof(Buff)))
    m_uFriendOffLineColor = GetColor(Buff);
  else
    m_uFriendOffLineColor = 0;

  pIni->GetInteger2("MemberList", "NumLine", &m_nMinVisibleLine,
                    &m_nMaxVisibleLine);

  if (nVisibleLine < m_nMinVisibleLine)
    pIni->GetInteger("MemberList", "InitLine", m_nMinVisibleLine,
                     &nVisibleLine);
  ...
  if (m_pSelf) {
    ...
    for (int i = 0; i < m_pSelf->m_nFriendUnitCount; i++) {
      ...
      if (m_pSelf->m_AllFriendUnits[i].m_pNotify &&
        ...
        m_pSelf->m_AllFriendUnits[i].m_pNotify->CreateUnit(
            m_pSelf->m_AllFriendUnits[i].m_UnitName.c_str());
      }
    }
  }
}
  ...
  if (m_pSelf) {
    ...
    for (int j = 0; j < m_pSelf->m_nFriendUnitCount; j++) {
      for (int i = 1; i < MAX_GROUP_NUM; i++) {
        ...
        pFile->GetString(m_pSelf->m_AllFriendUnits[j].m_UnitName.c_str(), key,
                         "", GroupName, 128);
  ...
  if (m_pSelf) {
    ...
    for (int j = 0; j < m_pSelf->m_nFriendUnitCount; j++) {
      for (int i = 1; i < MAX_GROUP_NUM; i++) {
        ...
        sprintf(key, "group%d", i);
        pFile->WriteString(
            m_pSelf->m_AllFriendUnits[j].m_UnitName.c_str(), key,
            m_pSelf->m_AllFriendUnits[j].m_GroupInfo[i].strGroupName.c_str());
      }
    }
  }
  return 1;
...
int KUiChatCentre::WndProc(unsigned int uMsg, unsigned int uParam, int nParam) {
  ...
  case WND_M_MENUITEM_SELECTED:
    if (uParam == (unsigned int)(KWndWindow *)this) {
      m_bPopupMenu = false;
      if (HIWORD(nParam) >= SEL_UNIT_MENU_SELECT_BASE &&
          (short)(LOWORD(nParam)) >= 0) {
        OnSelUnit(LOWORD(nParam));
      } else if (HIWORD(nParam) >= SEL_GROUP_MENU_SELECT_BASE &&
                 (short)(LOWORD(nParam)) >= 0) {
        OnMoveFriendToGroup(HIWORD(nParam) - SEL_GROUP_MENU_SELECT_BASE,
                            LOWORD(nParam));
        CancelMenu();
      } else if (HIWORD(nParam) >= FRIEND_MENU_SELECT_BASE)
        OnSelFriendMenu(HIWORD(nParam) - FRIEND_MENU_SELECT_BASE,
                        LOWORD(nParam));
      else
      ...
      if (uParam == ISP_NEW_GROUP) {
        ...
        if (nGroupIndex >= 0 && nGroupIndex < MAX_GROUP_NUM) {
          ReplaceGroupName(m_nCurentFriendUnit,
                           m_AllFriendUnits[m_nCurentFriendUnit]
                               .m_GroupInfo[nGroupIndex]
                               .strGroupName.c_str(),
                           (const char *)nParam);
        }
      }
  ...
  if (nFriendIndex >= 0 && nFriendIndex < m_nNumFriendInList) {
    for (int nNewGroupIndex = 0;
      ...
      if (m_bValidGroup[nNewGroupIndex] &&
          nNewGroupIndex !=
              m_AllFriendUnits[m_nCurentFriendUnit].m_nCurExpandGroup) {
        if ((nIndexInMenu--) == 0) {
          MoveFriendToGroup(m_nCurentFriendUnit,
                            m_pFriendList[nFriendIndex].Name,
                            m_AllFriendUnits[m_nCurentFriendUnit]
                                .m_GroupInfo[nNewGroupIndex]
                                .strGroupName.c_str());
        }
      }
    }
...
void KUiChatCentre::OnSelGroupMenu(int nGroupIndex, int nCmd) {
  ...
  case GROUP_MENU_ITEM_RENAME: {
    ...
    KUiGetString::OpenWindow("Thay ®æi tªn", Name, (KWndWindow *)this,
                             ISP_RENAME_GROUP + nGroupIndex);
  } break;
  ...
  case GROUP_MENU_ITEM_NEW:
    KUiGetString::OpenWindow("Thªm vµo danh s¸ch", "", (KWndWindow *)this,
                             ISP_NEW_GROUP);
    break;
  }
}
...
void KUiChatCentre::SendFriendMsg(int nFriendIndex, bool bDBClick) {
  if (m_pFriendList && nFriendIndex >= 0 && nFriendIndex < m_nNumFriendInList) {
    KUiPlayerBar::InputNameMsg(false, m_pFriendList[nFriendIndex].Name,
                               bDBClick);
  }
}

void KUiChatCentre::UpdateData(UICHATCENTRE_UPDATE eFlag, unsigned int uParam,
                               int nGroupIndex) {
  if (m_pSelf) {
    switch (eFlag) {
    case UICC_U_ALL:
      ...
      break;
    case UICC_U_GROUP:
      if (nGroupIndex == m_pSelf->m_AllFriendUnits[m_pSelf->m_nCurentFriendUnit]
                             .m_nCurExpandGroup)
...
bool KUiChatCentre::IsMyFriend(char *Name) {
  ...
  if (m_pSelf) {
    for (int i = 0; i < m_pSelf->m_nFriendUnitCount; i++) {
      FRIENDINFOLIST::iterator iF =
          m_pSelf->m_AllFriendUnits[i].m_FriendMap.find(Name);
...
bool KUiChatCentre::AddFriendInfo(int nUnitIndex, char *Name, char *group) {
  ...
  if (m_pSelf) {
    ...
    if (m_pSelf->m_AllFriendUnits[nUnitIndex].m_FriendMap.find(Name) !=
        m_pSelf->m_AllFriendUnits[nUnitIndex]
            .m_FriendMap.end()) { // ÒÑ´æÔÚÓÚÁÐ±íÖÐ
    ...
    int nCount = 0;
    int i = 0;
    if (m_pSelf->m_AllFriendUnits[nUnitIndex].m_GroupInfo[i].strGroupName ==
        strGroup) {
      nCount =
          m_pSelf->m_AllFriendUnits[nUnitIndex].m_GroupInfo[i].nGroupMemberNum +
          1;
    } else {
      for (i = 1; i < MAX_GROUP_NUM; i++) {
        if (m_pSelf->m_AllFriendUnits[nUnitIndex]
                .m_GroupInfo[i]
                .strGroupName.empty())
          break;
        if (m_pSelf->m_AllFriendUnits[nUnitIndex].m_GroupInfo[i].strGroupName ==
            strGroup) {
          nCount = m_pSelf->m_AllFriendUnits[nUnitIndex]
                       .m_GroupInfo[i]
                       .nGroupMemberNum +
                   1;
          break;
    ...
    if (i < MAX_GROUP_NUM) {
      ...
      {
        m_pSelf->m_AllFriendUnits[nUnitIndex].m_GroupInfo[i].strGroupName =
            strGroup;
        m_pSelf->m_AllFriendUnits[nUnitIndex].m_GroupInfo[i].nGroupMemberNum =
            1;
      }
    ...
    {
      strGroup =
          m_pSelf->m_AllFriendUnits[nUnitIndex].m_GroupInfo[0].strGroupName;
  ...
  if (m_pSelf) {
    ...
    if (Name && Name[0] != 0) {
      ...
      if (m_pSelf->m_nCurentFriendUnit == nUnitIndex) {
        FRIENDINFOLIST::iterator i =
            m_pSelf->m_AllFriendUnits[nUnitIndex].m_FriendMap.find(Name);
        if (i != m_pSelf->m_AllFriendUnits[nUnitIndex].m_FriendMap.end()) {
          int nNewGroupIndex =
              m_pSelf->m_AllFriendUnits[nUnitIndex].FindGroupIndex(i->second);
          if (nNewGroupIndex ==
              m_pSelf->m_AllFriendUnits[m_pSelf->m_nCurentFriendUnit]
                  .m_nCurExpandGroup) {
...
void KUiChatCentre::NewGroup(int nUnitIndex, const char *szGroup) {
  if (m_pSelf) {
    ...
    if (m_pSelf->m_AllFriendUnits[nUnitIndex].m_GroupInfo[0].strGroupName ==
        strGroup) {
      return;
    }
    ...
    for (int i = 1; i < MAX_GROUP_NUM; i++) {
      if (m_pSelf->m_AllFriendUnits[nUnitIndex].m_GroupInfo[i].strGroupName ==
          strGroup) {
        break;
      } else if (m_pSelf->m_AllFriendUnits[nUnitIndex]
                     .m_GroupInfo[i]
                     .strGroupName.empty()) {
        m_pSelf->m_AllFriendUnits[nUnitIndex].m_GroupInfo[i].strGroupName =
            strGroup;
        ...
        if (m_pSelf->m_AllFriendUnits[nUnitIndex].m_pNotify)
          m_pSelf->m_AllFriendUnits[nUnitIndex].m_pNotify->CreateUnitGroup(
              m_pSelf->m_AllFriendUnits[nUnitIndex].m_UnitName.c_str(),
              strGroup.c_str());

        if (nUnitIndex == m_pSelf->m_nCurentFriendUnit) {
          m_pSelf->UpdateAll(
              m_pSelf->m_AllFriendUnits[nUnitIndex].m_nCurExpandGroup);
          m_pSelf->UpdateInterface();
        }
        break;
      }
...
void KUiChatCentre::ReplaceGroupName(int nUnitIndex, const char *sz,
                                     const char *szGroup) {
  if (m_pSelf) {
    if (nUnitIndex < 0 || nUnitIndex >= m_pSelf->m_nFriendUnitCount)
      return;
    ...
    STRINGLIST list;
    FRIENDINFOLIST::iterator iF =
        m_pSelf->m_AllFriendUnits[nUnitIndex].m_FriendMap.begin();
    for (; iF != m_pSelf->m_AllFriendUnits[nUnitIndex].m_FriendMap.end();
         iF++) {
      if (iF->second == str) {
        iF->second = strGroup;
    ...
    if (m_pSelf->m_AllFriendUnits[nUnitIndex].m_pNotify)
      m_pSelf->m_AllFriendUnits[nUnitIndex].m_pNotify->RenameUnitGroup(
          m_pSelf->m_AllFriendUnits[nUnitIndex].m_UnitName.c_str(),
          strOld.c_str(), strGroup.c_str(), list);
...
void KUiChatCentre::MoveFriendToGroup(int nUnitIndex, const char *Name,
                                      const char *group) {
  if (m_pSelf) {
    if (nUnitIndex < 0 || nUnitIndex >= m_pSelf->m_nFriendUnitCount)
      return;
    ...
    int nNewGroupIndex =
        m_pSelf->m_AllFriendUnits[nUnitIndex].FindGroupIndex(group);
    if (nNewGroupIndex < 0)
      return;
    ...
    if (iF != m_pSelf->m_AllFriendUnits[nUnitIndex].m_FriendMap.end()) {
      int nSrcIndex =
          m_pSelf->m_AllFriendUnits[nUnitIndex].FindGroupIndex(iF->second);
      if (nSrcIndex >= 0)
        m_pSelf->m_AllFriendUnits[nUnitIndex]
            .m_GroupInfo[nSrcIndex]
            .nGroupMemberNum--;
      iF->second = group;
      m_pSelf->m_AllFriendUnits[nUnitIndex]
          .m_GroupInfo[nNewGroupIndex]
...
void KUiChatCentre::MoveGroupToGroup(int nUnitIndex, const char *group,
                                     const char *group2, bool bDelgroup) {
  if (m_pSelf) {
    if (nUnitIndex < 0 || nUnitIndex >= m_pSelf->m_nFriendUnitCount)
      return;
    ...
    if (nSrcIndex < 0)
      return;
    int nNewGroupIndex =
        m_pSelf->m_AllFriendUnits[nUnitIndex].FindGroupIndex(group2);
    if (nNewGroupIndex < 0)
      return;
    ...
    if (m_pSelf->m_AllFriendUnits[nUnitIndex].m_pNotify) {
      if (bDelgroup) {
        m_pSelf->m_AllFriendUnits[nUnitIndex].m_pNotify->DeleteUnitGroup(
            m_pSelf->m_AllFriendUnits[nUnitIndex].m_UnitName.c_str(), group,
            list);
      } else {
    ...
    if (bDelgroup) {
      if (nSrcIndex != 0) // 0ÊÇÃ¿¸öUnitµÄÄ¬ÈÏ×é,²»ÄÜÉ¾³ý
      {
        for (; nSrcIndex < MAX_GROUP_NUM - 1; nSrcIndex++) {
          m_pSelf->m_AllFriendUnits[nUnitIndex].m_GroupInfo[nSrcIndex] =
              m_pSelf->m_AllFriendUnits[nUnitIndex].m_GroupInfo[nSrcIndex + 1];
        }
        m_pSelf->m_AllFriendUnits[nUnitIndex]
            .m_GroupInfo[nSrcIndex]
            .strGroupName = "";
...
int KUiChatCentre::AddAddinUnit(const char *unit, FriendNotify *pNotify) {
  ...
  if (nUnit < 0) {
    if (m_pSelf && m_pSelf->m_nFriendUnitCount < MAX_UNIT_NUM) {
      ...
      m_pSelf->m_AllFriendUnits[m_pSelf->m_nFriendUnitCount].m_pNotify =
          pNotify;
...
void KUiChatCentre::PopupupGroupMenu(int nGroupIndex) {
  ...
  pMenuData->nNumItem =
      nGroupIndex ? GROUP_MENU_ITEM_COUNT : DEF_GROUP_MENU_ITEM_COUNT;
...
void KUiChatCentre::PopupSelGroupMenu(int nFriendIndex) {
  if (m_nNumValidGroup <= 1)
    return;
  m_pSelGrupMenu =
      (KPopupMenuData *)malloc(MENU_DATA_SIZE(m_nNumValidGroup - 1));
  if (m_pSelGrupMenu == NULL)
    return;
  KPopupMenu::InitMenuData(m_pSelGrupMenu, m_nNumValidGroup - 1);
  m_pSelGrupMenu->nNumItem = 0;
  for (int i = 0;
       i < MAX_GROUP_NUM && m_pSelGrupMenu->nNumItem < m_nNumValidGroup - 1;
       i++) {
    if (m_bValidGroup[i] &&
        i != m_AllFriendUnits[m_nCurentFriendUnit].m_nCurExpandGroup) {
      m_GroupBtn[i].GetLabel(
          m_pSelGrupMenu->Items[m_pSelGrupMenu->nNumItem].szData,
          sizeof(m_pSelGrupMenu->Items[0].szData));
      m_pSelGrupMenu->Items[m_pSelGrupMenu->nNumItem].uDataLen =
          strlen(m_pSelGrupMenu->Items[m_pSelGrupMenu->nNumItem].szData);
      m_pSelGrupMenu->nNumItem++;
    }
  }
  KPopupMenu::Popup(m_pSelGrupMenu, this,
                    SEL_GROUP_MENU_SELECT_BASE + nFriendIndex);
}

void KUiChatCentre::PopupSelUnitMenu() {
  struct KPopupMenuData *pSelUnitMenu =
      (KPopupMenuData *)malloc(MENU_DATA_SIZE(m_nFriendUnitCount));
  ...
  for (int i = 0; i < m_nFriendUnitCount; i++) {
    strncpy(pSelUnitMenu->Items[i].szData,
            m_AllFriendUnits[i].m_UnitName.c_str(), 63);
...
void KUiChatCentre::UpdateAll(int nCurGroup) {
  ...
  for (nIndex = 1; nIndex < MAX_GROUP_NUM; nIndex++) {
    if (m_AllFriendUnits[m_nCurentFriendUnit]
            .m_GroupInfo[nIndex]
            .strGroupName.empty()) {
      m_bValidGroup[nIndex] = false;
    } else {
      m_bValidGroup[nIndex] = true;
      m_GroupBtn[nIndex].SetLabel(m_AllFriendUnits[m_nCurentFriendUnit]
                                      .m_GroupInfo[nIndex]
                                      .strGroupName.c_str());
      m_nNumValidGroup++;
    }
  ...
  if (m_bValidGroup[m_AllFriendUnits[m_nCurentFriendUnit].m_nCurExpandGroup] ==
      false) {
    m_AllFriendUnits[m_nCurentFriendUnit].m_nCurExpandGroup = 0;
...
void KUiChatCentre::UpdateGroup(int nGroupIndex) {
  ...
  Group.nNumFriend = m_AllFriendUnits[m_nCurentFriendUnit]
                         .m_GroupInfo[nGroupIndex]
                         .nGroupMemberNum;
  if (Group.nNumFriend) {
    m_pFriendList =
        (KUiPlayerItem *)malloc(sizeof(KUiPlayerItem) * Group.nNumFriend);
    if (m_pFriendList) {
      FRIENDINFOLIST onlineList;
      FRIENDINFOLIST offlineList;
      FRIENDINFOLIST::iterator i =
          m_AllFriendUnits[m_nCurentFriendUnit].m_FriendMap.begin();
      for (; i != m_AllFriendUnits[m_nCurentFriendUnit].m_FriendMap.end();
           i++) {
        if (strcmp(i->second.c_str(), groupname) == 0) {
          if (m_AllFriendUnits[m_nCurentFriendUnit].m_FriendOnlineMap.find(
                  i->first) !=
              m_AllFriendUnits[m_nCurentFriendUnit].m_FriendOnlineMap.end()) {
            if (m_AllFriendUnits[m_nCurentFriendUnit]
                    .m_FriendOnlineMap[i->first] == stateOnline)
      ...
      m_FriendList.SetContent((unsigned char *)m_pFriendList,
                              m_nNumFriendInList, sizeof(KUiPlayerItem),
...
void KUiChatCentre::UpdateFriend(KUiPlayerItem *pFriend, int nGroupIndex) {
  if (pFriend == NULL)
    return;
  if (nGroupIndex == m_AllFriendUnits[m_nCurentFriendUnit].m_nCurExpandGroup &&
      m_pFriendList) {
...
void KUiChatCentre::UpdateInterface() {
  ...
  for (i = 0; i <= m_AllFriendUnits[m_nCurentFriendUnit].m_nCurExpandGroup;
       i++) {
    if (m_bValidGroup[i]) {
      nNumGroupAhead++;
    }
  ...
  // Ç°ÃæÌø¹ý²»ÏÔÊ¾µÄ×é°´Å¥
  for (;
       nLineIndex < m_nFirstVisibleLine &&
       nGroupIndex <= m_AllFriendUnits[m_nCurentFriendUnit].m_nCurExpandGroup &&
       nGroupIndex < MAX_GROUP_NUM;
       nGroupIndex++) {
  ...
  for (;
    ...
    if (m_bValidGroup[nGroupIndex]) {
      ...
      m_GroupBtn[nGroupIndex].SetPosition(m_nFirstGroupBtnLeft,
                                          nTop + m_nFirstGroupBtnTop);
      nTop += nBtnHeight;
...
int KUiChatCentre::PtInWindow(int x, int y) {
  ...
  if (KWndWindow::PtInWindow(x, y)) {
    if (x > m_nAbsoluteLeft + m_ListBgImgLeft &&
        ...
        y < m_nAbsoluteTop + m_nFirstGroupBtnTop +
                m_nNumVisibleLine * m_nLineHeight) {
      nRet = 1;
    }
    if (nRet == 0) {
