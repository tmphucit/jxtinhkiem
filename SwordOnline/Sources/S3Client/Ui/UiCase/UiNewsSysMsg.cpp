...
#define NEWS_SYSMSG_INI "消息系统.ini"
...
int KUiNewsSysMsg::GetSystemMessageCount() { return ms_nSystemMessageCount; }
 ...
 *功能：两个字 - 穿透！！！
 **********************************************************************************/
int KUiNewsSysMsg::PtInWindow(int x, int y) { return 0; }

// 新闻消息真的来了
void KUiNewsSysMsg::MessageArrival(const char *pMsg,
                                   unsigned short nMsgLength) {
  if (m_pSelf && pMsg && nMsgLength > 0) {
    KNewsSysMsg pSys;
...
void KUiNewsSysMsg::PaintWindow() {
  if (ms_nSystemMessageCount && g_pRepresentShell) {
    ...
    for (i = 0; i < ms_nSystemMessageCount; i++) {
      ...
      g_pRepresentShell->OutputRichText(m_nFontSize, &Param, pMsg->sMsg,
                                        pMsg->nMsgLen, 0);
