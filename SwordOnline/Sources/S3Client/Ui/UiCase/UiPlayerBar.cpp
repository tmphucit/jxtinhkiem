...
{
	nTime += 60;
	return sprintf(pString, "%s%sThêi gian ?s kh¾c",
...
void KUiPlayerBar::OnSend(BOOL bDirectSend)
{
	...
	{
		char szWarning[] = "Xin vui lßng kh«ng s?dông t?ng?kh«ng thÝch hîp trong th?"; 
...
void KUiPlayerBar::OnSendChannelMessage(DWORD nChannelID, const char* Buffer, int nLen)				//·¢ËÍÆµµÀÁÄÌìµ½·þÎñÆ÷
{
	if (nChannelID != -1 && Buffer && nLen > 0)
	{
		...
		if (nLeft < 0)
		{
			char szWarning[] = "Thiªn l?truyÒn ©m kh«ng c¸ch nµo s?dông liªn tôc, mêi b¹n lÊy h¬i chèc l¸t!";
			KUiMsgCentrePad::SystemMessageArrival(szWarning, sizeof(szWarning));
		}
		else if (nLeft > 0)
		{
			char szWarning[64];
			sprintf(szWarning, "Th«ng b¸o s?®­îc göi sau %d gi©y!", (nLeft + 999 ) / 1000);
	...
	while(KUiMsgCentrePad::GetChannelData(dwID, cost, Buffer, nLen))
	{
		...
		if (!bSend)
		{
			if (nRet == 2 && nUseLevel > 0)
				sprintf(szSystem, "§¼ng cÊp cña b¹n kh«ng ®ñ %d cÊp, kh«ng c¸ch nµo s?dông thiªn l?truyÒn ©m!", nUseLevel);

			if (nRet == 1 && nUseMoney > 0)
				sprintf(szSystem, "TiÒn cña b¹n kh«ng ®ñ %d l­îng, kh«ng c¸ch nµo s?dông thiªn l?truyÒn ©m!", nUseMoney);

			if (nRet == 3 && nUseMana > 0)
				sprintf(szSystem, "Néi lùc cña b¹n kh«ng ®ñ %d ®iÓm, kh«ng c¸ch nµo s?dông thiªn l?truyÒn ©m!", nUseMana);
		}
		else
		{
			if (nUseMoney > 0)
				sprintf(szSystem, "B¹n s?dông thiªn l?truyÒn ©m tæn hao ng©n l­îng %d l­îng!", nUseMoney);
			if (nUseMana > 0)
				sprintf(szSystem, "B¹n s?dông thiªn l?truyÒn ©m tæn hao néi lùc %d ®iÓm!", nUseMana);
		}
...
{
	if(!uId) return; //uh thank. di ngu thoi 2 ruoi roibb
	if (m_pSelf)
	{
...
void KUiPlayerBar::SetItemBtnPos(int nBtnNo,int X, int Y)
{
	...
	else m_pSelf->m_ItemBtn[nBtnNo].Show();
	}
	}
}
