...
KUiHelper2::KUiHelper2() { m_nIndexCount = 0; }

KUiHelper2::~KUiHelper2() { Clear(); }
...
void KUiHelper2::LoadIndexList(const char *pScheme) {
  ...
  for (int i = 0; i < nCount; i++) {
    ...
    ListIni.GetString("List", szIniSection, "", m_IndexData[i].szIniSection,
                      128);

    m_IndexList.SetContent(
        (unsigned char *)&m_IndexData[0].szName, nCount, sizeof(m_IndexData[i]),
