...
//	功能：如果窗口正被显示，则返回实例指针
//--------------------------------------------------------------------------
KUiMarket *KUiMarket::GetIfVisible() { return m_pSelf; }
...
void KUiMarket::Initialize() {
  ...
  //	m_ItemsBox.SetContainerId((int)UOC_MARKET);
  Wnd_AddWindow(this);
  char Scheme[256];
...
void KUiMarket::LoadScheme(const char *pScheme) {
  if (m_pSelf) {
    ...
    if (Ini.Load(Buff)) {
      ...
      m_pSelf->m_PhanLoai.Init(&Ini, "PhanLoai");
...
void KUiMarket::CancelTrade() { CloseWindow(); }
...
void KUiMarket::OnBuyItem(KUiDraggedObject *pItem, bool bDoImmed) {
...
void KUiMarket::OnClickButton(KWndButton *pWnd, int bCheck) {
