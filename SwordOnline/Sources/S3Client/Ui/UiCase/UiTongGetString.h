...
public:
  //----界面面板统一的接口函数----
  static KUiTongGetString *
  OpenWindow(const char *pszTitle, const char *pszInitString,
             KWndWindow *pRequester, unsigned int uParam, int nMinLen = 4,
             int nMaxLen = 16); // 打开窗口，返回唯一的一个类对象实例
