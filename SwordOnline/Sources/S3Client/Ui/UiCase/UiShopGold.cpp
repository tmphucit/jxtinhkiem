...
//	功能：如果窗口正被显示，则返回实例指针
//--------------------------------------------------------------------------
KUiShopGold *KUiShopGold::GetIfVisible() { return m_pSelf; }
...
void KUiShopGold::LoadScheme(const char *pScheme) {
  if (m_pSelf) {
    ...
    if (Ini.Load(Buff)) {
      ...
      m_pSelf->m_nPage.Init(&Ini, "TextPage");
...
void KUiShopGold::CancelTrade() { CloseWindow(); }
...
void KUiShopGold::SetPage(int nIndex) {
  if (nIndex >= 0 && nIndex < m_nPageCount && m_pObjsList) {
    ...
    m_nPage.Set2IntText(m_nCurrentPage + 1, m_nPageCount, '/');
  }
}
