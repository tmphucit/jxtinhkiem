...
#define NEWS_MESSAGE_INI "UiNewsMessage1.ini"
...
KUiNewsMessage1 *KUiNewsMessage1::m_pSelf = NULL;
BOOL KUiNewsMessage1::ms_bEnableIdleMsg = FALSE;

/*********************************************************************************
 *功能：构造
 **********************************************************************************/
KUiNewsMessage1::KUiNewsMessage1() {
  m_pHead = NULL;
  m_pHandling = NULL;
 ...
 *功能：增加一条消息在队列中
 **********************************************************************************/
bool KUiNewsMessage1::AddMessage(KNewsMessage1 *pMsg, unsigned int uTime,
                                 unsigned int uTimeParam) {
  KNewsMessageNode1 *pNew = NULL;
  if (pMsg) {
    pNew = (KNewsMessageNode1 *)malloc(sizeof(KNewsMessageNode1));
    if (pNew) {
      *(KNewsMessage1 *)pNew = *pMsg;
      pNew->uTime = uTime;
 ...
 **********************************************************************************/
KNewsMessageNode1 *KUiNewsMessage1::SeparateMsg() {
...
// 把pNode所指的消息加到链表末端！
void KUiNewsMessage1::AddToTail(KNewsMessageNode1 *pNode) {
  if (pNode) {
    if (m_pHead) {
 ...
 *功能：打开窗口
 **********************************************************************************/
KUiNewsMessage1 *KUiNewsMessage1::OpenWindow() {
  if (m_pSelf == NULL) {
    m_pSelf = new KUiNewsMessage1;
    if (m_pSelf) {
      m_pSelf->Initialize();
    }
  }
 ...
 *功能：如果窗口正被显示，则返回实例指针
 **********************************************************************************/
KUiNewsMessage1 *KUiNewsMessage1::GetIfVisible() {
 ...
 *功能：关闭窗口
 **********************************************************************************/
void KUiNewsMessage1::CloseWindow(BOOL bDestory) {
  if (m_pSelf) {
    if (bDestory == TRUE) {
      m_pSelf->Clear();
      m_pSelf->Destroy();
 ...
 *功能：初始化
 **********************************************************************************/
void KUiNewsMessage1::Initialize() {
  if (ms_bEnableIdleMsg)
 ...
 *功能：载入界面方案
 **********************************************************************************/
void KUiNewsMessage1::LoadScheme(const char *pszScheme) {
  if (m_pSelf && pszScheme && pszScheme[0]) {
    ...
    if (Ini.Load(szBuf)) {
      ...
      Ini.GetInteger("Main", "MaxIdleTime", MAX_IDLE_TIME,
                     (int *)&m_pSelf->m_uMaxIdleTime);
      Ini.GetInteger("Main", "ShowInterval", SHOW_INTERVAL,
                     (int *)&m_pSelf->m_uShowInterval);
      Ini.GetInteger("Main", "ScrollInterval", SCROLL_INTERVAL,
                     (int *)&m_pSelf->m_uScrollInterval);
    }
  }
}
 ...
 *功能：两个字 - 穿透！！！
 **********************************************************************************/
int KUiNewsMessage1::PtInWindow(int x, int y) { return 0; }

// 新闻消息真的来了
void KUiNewsMessage1::MessageArrival1(KNewsMessage1 *pMsg, SYSTEMTIME *pTime) {
  unsigned int uTime;
  if (m_pSelf && pMsg && pMsg->nMsgLen > 0 &&
      pMsg->nMsgLen <= sizeof(pMsg->sMsg)) {
    switch (pMsg->nType) {
    case NEWSMESSAGE_NORMAL_1: // 一般消息
      m_pSelf->AddMessage(pMsg, 0, 0);
      break;
    case NEWSMESSAGE_COUNTING_1: // 倒计（秒）数消息
    ...
    case NEWSMESSAGE_TIMEEND_1: // 定时消息
      ...
      if (uTime)
        ...
        pMsg->nType = NEWSMESSAGE_NORMAL_1;
        m_pSelf->AddMessage(pMsg, 0, 0);
        pMsg->nType = NEWSMESSAGE_TIMEEND_1;
      }
      break;
    }
  }
 ...
 **********************************************************************************/
void KUiNewsMessage1::PaintWindow() {
  if (m_pHandling && g_pRepresentShell) {
    ...
    if (m_bJustIncoming) { // 消息正在进入,(头)还未开始离开显示区域
      g_pRepresentShell->OutputRichText(m_nFontSize, &Param, m_CurrentMsg.sMsg,
                                        m_nCharIndex, 0);
    } else { // 消息头已经离开显示区域
      ...
      if (nRemainEffectCtrlsSize == 0) {
        g_pRepresentShell->OutputRichText(
            m_nFontSize, &Param, &m_CurrentMsg.sMsg[m_nCharIndex],
            m_CurrentMsg.nMsgLen - m_nCharIndex, m_nVisionWidth);
      } else {
        memcpy(sBuffer + nRemainEffectCtrlsSize,
               m_CurrentMsg.sMsg + m_nCharIndex,
               m_CurrentMsg.nMsgLen - m_nCharIndex);
        g_pRepresentShell->OutputRichText(m_nFontSize, &Param, sBuffer,
                                          m_CurrentMsg.nMsgLen - m_nCharIndex +
                                              nRemainEffectCtrlsSize,
                                          m_nVisionWidth);
      }
    }
  }
...
bool KUiNewsMessage1::PickAMessage() {
  ...
  if (m_pHead == NULL) {
    ...
    while (pNode) {
      ...
      if (pNode->nType == NEWSMESSAGE_NORMAL_1) {
        m_pHandling = pNode;
        if ((++pNode->uShowTimes) > MAX_NORMAL_SHOW_TIMES)
          bTobeFree = true;
      } else if (pNode->nType == NEWSMESSAGE_COUNTING_1) {
        m_pHandling = pNode;
 ...
 **********************************************************************************/
void KUiNewsMessage1::PickFromIni() {
  int i, nValue;
  char szNum[16];
  KNewsMessage1 Msg;
  ...
  if (nValue > 0) {
    ...
    m_IniFile.GetString("Main", itoa(i, szNum, 10), "", Msg.sMsg,
                        sizeof(Msg.sMsg));
    Msg.nMsgLen = TEncodeText(Msg.sMsg, strlen(Msg.sMsg));
    if (Msg.nMsgLen > 0) {
      Msg.nType = NEWSMESSAGE_NORMAL_1;
      MessageArrival1(&Msg, NULL);
    }
  }
}
...
void KUiNewsMessage1::ConvertMsg() {
  //_ASSERT(m_pHandling);
  // 填充当前显示消息的内容
  m_CurrentMsg.nMsgLen = 0;

  m_nInsertPlace = NOT_NEED_INSERT;
  if (m_pHandling->nType == NEWSMESSAGE_COUNTING_1) {
    m_nInsertPlace = TFindSpecialCtrlInEncodedText(
        m_pHandling->sMsg, m_pHandling->nMsgLen, 0, '%');

    if (m_nInsertPlace < 0 && m_nInsertPlace > m_pHandling->nMsgLen - 2) {
      m_nInsertPlace = NOT_NEED_INSERT;
...
bool KUiNewsMessage1::MakeCountingMsg() {
  //_ASSERT(m_pHandling &&
  ...
  if (m_pHandling->nMsgLen > m_nInsertPlace) {
    memcpy(sBuffer + m_CurrentMsg.nMsgLen, m_pHandling->sMsg + m_nInsertPlace,
...
bool KUiNewsMessage1::ScrollMsg() {
  ...
  if (m_bJustIncoming) {
    ...
    if (m_nTextPosX < m_nAbsoluteLeft + m_nIndentH) {
      ...
      nLen = 1;
      m_nCharIndex = TGetEncodedTextOutputLenPos(
          m_CurrentMsg.sMsg, m_CurrentMsg.nMsgLen, nLen, false, m_nFontSize);
      if (m_nCharIndex >= m_CurrentMsg.nMsgLen) {
        snSkipTimes = 0;
        return true;
      }
