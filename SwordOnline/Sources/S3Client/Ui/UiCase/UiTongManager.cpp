...
void KUiTongManager::ArrangeComposition(char *pszPlayerName) {
  if (g_pCoreShell) {
    ...
    if (m_nIndex >= 0) {
      ...
      g_pCoreShell->TongOperation(GTOI_REQUEST_PLAYER_TONG,
                                  (unsigned int)&Player, (int)TRUE);
    } else {
      CloseWindow();
    }
  }
...
void KUiTongManager::CloseWindow(bool bDestory) {
  if (ms_pSelf) {
    ms_pSelf->Hide();
    if (bDestory) {
      KUiTongStringName::CloseWindow(TRUE);
      KUiTongAssignBox::CloseWindow(TRUE);
      KUiTongGetString::CloseWindow(TRUE);
      ms_pSelf->Destroy();
      ms_pSelf = NULL;
    }
  }
...
void KUiTongManager::Initialize() {
  ...
  AddChild(&m_TongCamp1);
  AddChild(&m_TongCamp2);
  AddChild(&m_TongCamp3);
  ...
  AddChild(&m_BtnTitle);
  AddChild(&m_BtnMaleTitle);
  AddChild(&m_BtnFemaleTitle);
...
void KUiTongManager::LoadScheme(const char *pScheme) {
  if (ms_pSelf) {
    ...
    if (Ini.Load(Buff)) {
      ...
      ms_pSelf->m_TongCamp1.Init(&Ini, "BtChinhPhai");
      ms_pSelf->m_TongCamp2.Init(&Ini, "BtTaPhai");
      ms_pSelf->m_TongCamp3.Init(&Ini, "BtTrungLap");

      ms_pSelf->m_MemberCount.Init(&Ini, "MemberCount");
      ms_pSelf->m_Money.Init(&Ini, "Money");
      ms_pSelf->m_Level.Init(&Ini, "Level");
      ...
      ms_pSelf->m_List.Init(&Ini, "ListLeft");
      ...
      ms_pSelf->m_BtnMaleTitle.Init(&Ini, "BtnMaleTitle");
      ms_pSelf->m_BtnFemaleTitle.Init(&Ini, "BtnFemaleTitle");

      Ini.GetString("Main", "StringDismiss", "", ms_pSelf->m_szDismiss,
                    sizeof(ms_pSelf->m_szDismiss));
      Ini.GetString("Main", "StringAssign", "", ms_pSelf->m_szAssign,
                    sizeof(ms_pSelf->m_szAssign));
      Ini.GetString("Main", "StringDemise", "", ms_pSelf->m_szDemise,
                    sizeof(ms_pSelf->m_szDemise));
      Ini.GetString("Main", "StringLeave", "", ms_pSelf->m_szLeave,
                    sizeof(ms_pSelf->m_szLeave));

      Ini.GetString("Main", "StringJustice", "", ms_pSelf->m_szJustice,
                    sizeof(ms_pSelf->m_szJustice));
      Ini.GetString("Main", "StringBalance", "", ms_pSelf->m_szBalance,
                    sizeof(ms_pSelf->m_szBalance));
      Ini.GetString("Main", "StringEvil", "", ms_pSelf->m_szEvil,
                    sizeof(ms_pSelf->m_szEvil));

      Ini.GetString("Main", "StringConfirm", "", ms_pSelf->m_szConfirm,
                    sizeof(ms_pSelf->m_szConfirm));
      Ini.GetString("Main", "StringCancel", "", ms_pSelf->m_szCancel,
                    sizeof(ms_pSelf->m_szCancel));
    }
  }
}
 ...
 * 功能：窗口函数
 **********************************************************************/
int KUiTongManager::WndProc(unsigned int uMsg, unsigned int uParam,
                            int nParam) {
  switch (uMsg) {
  ...
  case WND_N_BUTTON_CLICK:
    if (uParam == (unsigned int)&m_BtnDismiss) {
    ...
    } else if (uParam == (unsigned int)&m_TongCamp1 ||
               uParam == (unsigned int)&m_TongCamp2 ||
               uParam == (unsigned int)&m_TongCamp3) {
      KUiTongChangeCamp::OpenWindow(m_TongData.szName);
      ...
      KUiTongAssignBox::ArrangeData(m_nRelation, pNode->Name, pNode->szAgname,
                                    m_nCurrentCheckBox, m_nIndex);
    }

    else if (uParam == (unsigned int)&m_BtnFemaleTitle) {

      KUiTongStringName::OpenWindow(1, "");
      KUiTongStringName::LinkToMainWindow(this);

    }

    else if (uParam == (unsigned int)&m_BtnMaleTitle) {

      KUiTongStringName::OpenWindow(2, "");
      KUiTongStringName::LinkToMainWindow(this);
      ...
      if (pNode->Name && pNode->Name[0])
        KUiTongStringName::OpenWindow(3, pNode->Name);
      else
        KUiTongStringName::OpenWindow(3, m_TongData.szMasterName);
      ...
      OnAction(TONG_ACTION_APPLY);
               ...
               uParam == (unsigned int)&m_BtnClose) {
      CloseWindow();
      ...
      KUiTongStringName::OpenWindow(4, "G鉷 ti襫 bang");
      KUiTongStringName::LinkToMainWindow(this);
      ...
      KUiTongStringName::OpenWindow(5, "R髏 ti襫 bang");
      KUiTongStringName::LinkToMainWindow(this);
    }
...
void KUiTongManager::UpdateBtnTheme(int nType, BOOL IsDissable) {
  ...
  if (g_pCoreShell && nType >= enumTONG_FIGURE_MANAGER &&
      nType <= enumTONG_FIGURE_MASTER) {
    KUiPlayerItem Player;
    memset(&Player, 0, sizeof(KUiPlayerItem));
    strcpy(Player.Name, m_szPlayerName);
    m_BtnRecruit.CheckButton(g_pCoreShell->TongOperation(
        GTOI_TONG_IS_RECRUIT, (unsigned int)&Player, 0));
  }
}
...
void KUiTongManager::UpdateTongInfoView() {
  ...
  m_MemberCount.SetText(itoa(m_TongData.nMemberCount +
                                 m_TongData.nManagerCount +
                                 m_TongData.nDirectorCount,
                             Buff, 10));
  int van = m_TongData.nMoney / 10000;

  int luong = m_TongData.nMoney % 10000;
  // int van = m_TongData.nStatus;
  char ngan_luong[32];
  sprintf(ngan_luong, "%d %s %d", van, "v筺", luong);
  m_Money.SetText(ngan_luong);

  char dang_cap[32];
  sprintf(dang_cap, "%d/5", m_TongData.nLevel);

  m_Level.SetText(dang_cap);
}
    ...
    if (nDataCount > nViewCount) {
      ...
      if (pNode) {
        ...
        for (int i = nViewCount; i < nDataCount; i++) {
          ...
          //		continue;
          //	}
          int thang = pNode->nPlayerTime / 1000;
          int ngay = pNode->nPlayerTime % 1000;
          int thangend = pNode->nPlayerTimeEnd / 1000;
          int ngayend = pNode->nPlayerTimeEnd % 1000;
          /*if (pNode->nStatus)
                  sprintf(Buff, "On %s %s   [Lv:%d| In:%d/%d| End:%d/%d|
          CH:%d]", pNode->szAgname, pNode->Name,
          pNode->nPlayerLevel,ngay,thang,ngayend,thangend ,pNode->nPlayerPoint);
          else*/
          sprintf(Buff, "%s %s   [Lv:%d| In:%d/%d| End:%d/%d| CH:%d]",
                  pNode->szAgname, pNode->Name, pNode->nPlayerLevel, ngay,
                  thang, ngayend, thangend, pNode->nPlayerPoint);

          if (pNode->nStatus)
            m_List.AddString(i, Buff);
          else
            m_List.AddString(i, Buff, GetColor("32,123,123"));
 ...
 * 功能：弹出确认窗口
 **********************************************************************/
void KUiTongManager::PopupConfirmWindow(const char *pszInfo,
                                        unsigned int uHandleID) {
  // UIMessageBox(pszInfo, this, m_szConfirm, m_szCancel, uHandleID);
  KPilgarlicItem *pPlayer = GetSelectedPlayer();
  if (pPlayer)
    KUiTongGetString::OpenWindow(pszInfo, pPlayer->Name, this, uHandleID, 1,
                                 31);
  else
    KUiTongGetString::OpenWindow(pszInfo, m_szPlayerName, this, uHandleID, 1,
                                 31);
  Hide();
}
 ...
 * 功能：接受新的数据的数据接口
 **********************************************************************/
void KUiTongManager::NewDataArrive(KUiGameObjectWithName *ParamInfo,
                                   KTongMemberItem *pIncome) {
 ...
 **********************************************************************/
int KUiTongManager::IsPlayerExist(KPilgarlicItem *MeToCmp,
                                  KLinkStruct<KPilgarlicItem> *pData) {
  unsigned int nID;
 ...
 * 功能：接受帮会信息
 **********************************************************************/
void KUiTongManager::TongInfoArrive(KUiPlayerRelationWithOther *Relation,
                                    KTongInfo *pTongInfo) {
  if (ms_pSelf && !strcmp(Relation->Name, ms_pSelf->m_szPlayerName)) {
    ...
    if (Relation->nRelation <= enumTONG_FIGURE_MASTER &&
        Relation->nRelation >= enumTONG_FIGURE_MEMBER) {
 ...
 * 功能：把新到的列表信息存到临时文件
 **********************************************************************/
void KUiTongManager::SaveNewData(KUiGameObjectWithName *ParamInfo,
                                 KTongMemberItem *pIncome) {
  char szSection[32];
  KIniFile Ini;
  ...
  if (Ini.Load(TONG_DATA_TEMP_FILE)) {
    ...
    for (int i = nCount; i < nDataCount; i++) {
      ...
      Ini.WriteInteger(szSection, szKey,
                       StringToHash(pIncome[i - nCount].Name));
    }
...
int KUiTongManager::LoadDataHandler(KLinkStruct<KPilgarlicItem> *pData,
  ...
  sprintf(szFileName, "%s\\%s", g_UiBase.GetUserTempDataFolder(),
          TONG_DATA_TEMP_FILE);
  if (Ini.Load(szFileName) && m_TongData.szName[0] &&
      Ini.IsSectionExist(szSection)) {
    int nCount, i;
    char szKey[32];
    ...
    for (i = 0; i < nCount; i++) {
      ...
      sprintf(szKey, "%d_AgName", i);
      Ini.GetString(szSection, szKey, "", AddItem.szAgname,
                    sizeof(AddItem.szAgname));
...
void KUiTongManager::RequestData(int nType, int nIndex) {
  if (g_pCoreShell) {
    ...
    g_pCoreShell->TongOperation(GTOI_REQUEST_TONG_DATA,
                                (unsigned int)&ParamInfo, 0);
  }
}
...
int KUiTongManager::RemoveTempFile() {
  ...
  sprintf(szFile, "%s\\%s", g_UiBase.GetUserTempDataFolder(),
          TONG_DATA_TEMP_FILE);
...
void KUiTongManager::OnAction(int nType) {
  if (g_pCoreShell) {
    ...
    g_pCoreShell->TongOperation(GTOI_TONG_ACTION, (unsigned int)&Param,
                                (int)&Player);
    memset(m_szPassword, 0, sizeof(m_szPassword));
  }
}
 ...
 * 功能：对结果的响应入口
 **********************************************************************/
void KUiTongManager::ResponseResult(KUiGameObjectWithName *pResult,
                                    int nbIsSucceed) {
  if (ms_pSelf) {
 ...
 * 功能：响应离帮后的操作
 **********************************************************************/
void KUiTongManager::ResponseLeave(KUiGameObjectWithName *pResult,
                                   int nbIsSucceed) {
  if (nbIsSucceed) {
    RemoveTempFile();
    CloseWindow();
 ...
 * 功能：响应踢人后的操作
 **********************************************************************/
void KUiTongManager::ResponseDismiss(KUiGameObjectWithName *pResult,
                                     int nbIsSucceed) {
  if (nbIsSucceed) {
 ...
 * 功能：响应传位后的操作
 **********************************************************************/
void KUiTongManager::ResponseDemise(KUiGameObjectWithName *pResult,
                                    int nbIsSucceed) {
  if (nbIsSucceed) {
    RemoveTempFile();
 ...
 * 功能：响应传位后的操作
 **********************************************************************/
void KUiTongManager::ResponseAssign(KUiGameObjectWithName *pResult,
                                    int nbIsSucceed) {
  if (nbIsSucceed) {
 ...
 * 功能：响应申请后的操作
 **********************************************************************/
void KUiTongManager::ResponseApply(KUiGameObjectWithName *pResult,
                                   int nbIsSucceed) {
  if (nbIsSucceed) {
...
KUiTongManager::HandleFind(KLinkStruct<KPilgarlicItem> *pData, char *pszName) {
