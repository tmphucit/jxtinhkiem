...
int KUiTongManager::WndProc(unsigned int uMsg, unsigned int uParam, int nParam)
{
	...
	case WND_N_BUTTON_CLICK:
		...
		else if (uParam == (unsigned int)&m_TongCamp1 || uParam == (unsigned int)&m_TongCamp2 || uParam == (unsigned int)&m_TongCamp3)
		{
		...
		if(nDataCount > nViewCount)
		{
			...
			if(pNode)
			{
				...
				for(int i = nViewCount;i < nDataCount;i++)
				{
					...
					/*if (pNode->nStatus)
						sprintf(Buff, "On %s %s   [Lv:%d| In:%d/%d| End:%d/%d| CH:%d]", pNode->szAgname, pNode->Name, pNode->nPlayerLevel,ngay,thang,ngayend,thangend ,pNode->nPlayerPoint);
					else*/
						sprintf(Buff, "%s %s   [Lv:%d| In:%d/%d| End:%d/%d| CH:%d]", pNode->szAgname, pNode->Name, pNode->nPlayerLevel,ngay,thang,ngayend,thangend ,pNode->nPlayerPoint);
