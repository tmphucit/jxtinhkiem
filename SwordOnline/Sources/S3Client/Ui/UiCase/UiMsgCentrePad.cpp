...
///////////////////////////////////////////////////////////////////////////////////////////
int KSysMsgCentrePad::WndProc(unsigned int uMsg, unsigned int uParam,
                              int nParam) {
  ...
  if (uMsg == WND_N_BUTTON_CLICK) {
    ...
    if (uParam == (unsigned int)(KWndWindow *)&m_AllChanel) {
      m_AllChanel.CheckButton(true);
      m_PriChanel.CheckButton(false);
      m_MenpaiChanel.CheckButton(false);
      m_TongChanel.CheckButton(false);
      m_OtherChanel.CheckButton(false);
      ...
      m_AllChanel.CheckButton(false);
      m_PriChanel.CheckButton(false);
      m_MenpaiChanel.CheckButton(true);
      m_TongChanel.CheckButton(false);
      m_OtherChanel.CheckButton(false);
      ...
      m_MenpaiChanel.CheckButton(false);
      m_TongChanel.CheckButton(false);
      m_OtherChanel.CheckButton(true);
    ...
    if (uParam == (unsigned int)(KWndWindow *)&m_DownButton) {
      if (nDownSpeed % 5 == 0) {
        m_SysRoom.ClearHideLine();
        int nLine = m_SysRoom.GetItemLineCount(m_SysRoom.GetMsgCount()) -
                    m_SysRoom.GetMaxShowLine();
...
KUiMsgCentrePad *KUiMsgCentrePad::OpenWindow() {
  ...
  if (m_pSelf) {
    ...
    m_pSelf->nIDChatRoom = 0;
    m_pSelf->m_ChatRoom.GetMessageListBox()->m_bItemActived = TRUE;
    m_pSelf->m_ChatRoom.Show();
    m_pSelf->m_ChatRoom1.Hide();
    m_pSelf->m_ChatRoom2.Hide();
    m_pSelf->m_ChatRoom3.Hide();
    m_pSelf->m_ChatRoom4.Hide();
  }
  return m_pSelf;
}
...
// 加入一条消息
void KUiMsgCentrePad::SystemMessageArrival(const char *pMsgBuff,
  ...
  if (m_pSelf && pMsgBuff && nMsgLength > 0) {
    if (m_pSelf->m_nDefaultChannelResource >= 0 &&
      ...
      m_pSelf->ChannelMessageArrival(
          0,
          m_pSelf->FindActivateChannelIndex(
              m_pSelf->m_ChannelsResource[m_pSelf->m_nDefaultChannelResource]
                  .cTitle),
          ...
          m_pSelf->m_DefaultChannelSendName, pMsgBuff, nMsgLength,
          &m_pSelf->m_Sys.m_SysRoom, false);
...

{
  ...
  if (m_pActivateChannel[nChannelIndex].ResourceIndex >= 0) {
    ...
    *((WORD *)(Buffer + nOffset)) =
        m_ChannelsResource[m_pActivateChannel[nChannelIndex].ResourceIndex]
            .nTextPicIndex;
    nOffset += 2;

    if (nChannelIndex == 5 && figure == 3) {
      KRColor uTextColor;
      uTextColor.Color_dw = GetColor("255, 69, 0");
      uTextColor.Color_b.a = KTC_COLOR;

      uColor = uTextColor;
    } else if (nChannelIndex == 5 && figure == 2) {
      KRColor uTextColor;
      uTextColor.Color_dw = GetColor("0, 255, 0");
      uTextColor.Color_b.a = KTC_COLOR;
    ...
    uColor = m_ChannelsResource[m_pActivateChannel[nChannelIndex].ResourceIndex]
                 .uTextBorderColor;
    Buffer[nOffset++] = uColor.Color_b.a;
    Buffer[nOffset++] = uColor.Color_b.r;
    Buffer[nOffset++] = uColor.Color_b.g;
    ...
    if (m_ChannelsResource[m_pActivateChannel[nChannelIndex].ResourceIndex]
            .szSoundFileName[0] != 0)
      UiSoundPlay(
          m_ChannelsResource[m_pActivateChannel[nChannelIndex].ResourceIndex]
              .szSoundFileName);
  ...
  for (i = 0; i < 12; i++) // day la lay 12 dau
  {
    ZeroMemory(szNum, sizeof(szNum));
    nLeng = 0;
    pszCheck1++;
...
NextStep:
         ...
         // Item------------------------------------------
  {
    ...
    if (uMsgLen1) {
      ...
      int nLastPos = 0;
      int nTotalLen = 0;
      nLinePos = TGetItemChatLineCount(Temp, nL, nW2, nMaxLineL, 12, nFace,
                                       nLastPos, nTotalLen, 0, 0, true);
      nPos = nLastPos + nFace * 3;
      if (nPos == 0 &&
          (nTotalLen % nMaxLineL > nMaxLineL - 5 || nTotalLen % nMaxLineL < 5))
        nLinePos++;
    ...
    nOffset += FilterTextColor(Buffer + nOffset,
                               nMsgLength - (uMsgLen1 + uMsgLen2), uColor);
  } //------------------------------------------------------------------------------------------
...
void KUiMsgCentrePad::MSNMessageArrival(char *szSourceName, char *szSendName,
                                        const char *pMsgBuff,
                                        unsigned short nMsgLength) {
  KRColor uColor;
  KRColor uBKColor;
  ...
  ShowMSNMessage(bSelf ? szSendName : szSourceName, pMsgBuff, nMsgLength,
                 uColor, uBKColor, uBorderColor, nPic);
}

void KUiMsgCentrePad::ShowMSNMessage(char *szName, const char *pMsgBuff,
                                     unsigned short nMsgLength, KRColor uColor,
                                     KRColor uBKColor, KRColor uBorderColor,
                                     WORD nPic) {
  if (g_pCoreShell == NULL)
    return; //
  char Buffer[560];
...
NextStep:
  ...
  /***********The
   * gioi-mat************************************************************/
  m_ChatRoom.GetMessageListBox()->AddOneMessage(
      Buffer, nOffset, &CItem, bOk, nPos, nLinePos, nFace, uBKColor.Color_dw);
  m_ChatRoom.GetScrollBar()->SetScrollPos(
      m_ChatRoom.GetScrollBar()->GetMaxValue());
  /***********************Add message vao kenh
   * mat*************************************/
  m_ChatRoom1.GetMessageListBox()->AddOneMessage(
      Buffer, nOffset, &CItem, bOk, nPos, nLinePos, nFace, uBKColor.Color_dw);
  m_ChatRoom1.GetScrollBar()->SetScrollPos(
      m_ChatRoom1.GetScrollBar()->GetMaxValue());
  /*************************************************************************************/
  if (nIdx)
    g_pCoreShell->GetGameData(GDI_ITEM_CHAT, false, nIdx);
  /*************************************************************************************/
...
int KUiMsgCentrePad::NewChannelMessageArrival(int figure, DWORD nChannelID,
                                              char *szSendName,
                                              const char *pMsgBuff,
                                              unsigned short nMsgLength)
// int KUiMsgCentrePad::NewChannelMessageArrival(DWORD nChannelID, char*
  ...
  if (m_pSelf && pMsgBuff) {
    ...
    if (nChannelIndex >= 0) {
      ...
      if (nChannelIndex == 2) {
        m_pSelf->ChannelMessageArrival(
            figure, nChannelIndex, szSendName, pMsgBuff, nMsgLength,
            m_pSelf->m_ChatRoom2.GetMessageListBox(), true);
        m_pSelf->m_ChatRoom2.GetScrollBar()->SetScrollPos(
            m_pSelf->m_ChatRoom2.GetScrollBar()->GetMaxValue());

      }

      else if (nChannelIndex == 5) {
        m_pSelf->ChannelMessageArrival(
            figure, nChannelIndex, szSendName, pMsgBuff, nMsgLength,
            m_pSelf->m_ChatRoom3.GetMessageListBox(), true);
        m_pSelf->m_ChatRoom3.GetScrollBar()->SetScrollPos(
            m_pSelf->m_ChatRoom3.GetScrollBar()->GetMaxValue());

      }

      else if (nChannelIndex == 6 || nChannelIndex == 3) {
        m_pSelf->ChannelMessageArrival(
            figure, nChannelIndex, szSendName, pMsgBuff, nMsgLength,
            m_pSelf->m_ChatRoom4.GetMessageListBox(), true);
        m_pSelf->m_ChatRoom4.GetScrollBar()->SetScrollPos(
            m_pSelf->m_ChatRoom4.GetScrollBar()->GetMaxValue());
      }
    }
...
void KUiMsgCentrePad::NewMSNMessageArrival(char *szSourceName, char *szSendName,
                                           const char *pMsgBuff,
                                           unsigned short nMsgLength) {
  if (m_pSelf && pMsgBuff) {
...
void KUiMsgCentrePad::ShowSomeoneMessage(char *szSourceName,
                                         const char *pMsgBuff,
                                         unsigned short nMsgLength) {
  if (m_pSelf && pMsgBuff) {
    KRColor uColor;
    ...
    m_pSelf->ShowMSNMessage(szSourceName, pMsgBuff, nMsgLength, uColor,
                            uBKColor, uBorderColor, nPic);
  }
}

void KUiMsgCentrePad::OpenChannel(char *channelName, DWORD nChannelID,
                                  BYTE cost) {
  if (nChannelID == -1)
    return;
  ...
  if (m_pSelf) {
    ...
    for (int n = 0; n < m_pSelf->m_nChannelsResource; n++) {
      ...
      if (m_pSelf->m_ChannelsResource[n].cFormatName[0] == 0)
        return;
      if (!m_pSelf->ReplaceSpecialField(
              ChannelName, m_pSelf->m_ChannelsResource[n].cFormatName))
        continue;
      if (ChannelName[0] == 0)
        continue;
...
void KUiMsgCentrePad::ReplaceChannelName(char *szDest, size_t nDestSize,
                                         char *szSrc) {
  if (m_pSelf && szSrc[0] != 0) {
    for (int n = 0; n < m_pSelf->m_nChannelsResource; n++) {
      for (int m = 0; m < MAX_CHANNELRESOURCE; m++) {
        if (m_pSelf->m_ChannelsResource[n].cShortTitle[m][0] != 0) {
          if (strcmpi(szSrc, m_pSelf->m_ChannelsResource[n].cShortTitle[m]) ==
              0) {
...
bool KUiMsgCentrePad::ReplaceSpecialField(char *szDest, char *szSrc) {
  ...
  if (self.nCurTong != 0) {
    int nCurTong = self.nCurTong;

    if (nCurTong < 0)
      nCurTong = -nCurTong;

    sprintf(s_keyDests[2], "%d", nCurTong);
  }
  int nTaskValue = g_pCoreShell->GetGameData(GDI_IS_GET_TASK_VALUE, 0, 0);
  if (nTaskValue > 0) {
    sprintf(s_keyDests[3], "%d", nTaskValue);
  }
...
void KUiMsgCentrePad::CloseSelfChannel(SelfChannel type) {
  if (m_pSelf) {
    ...
    case ch_Screen:
      // case ch_GM:	//GM不能退
      {
        m_pSelf->CloseActivateChannel(
            m_pSelf->FindActivateChannelIndexByKey(s_keys[type]));
      }
      break;
    default:
      return;
...
bool KUiMsgCentrePad::GetChannelSubscribe(int nChannelIndex) {
  if (m_pSelf && nChannelIndex >= 0 &&
      nChannelIndex < m_pSelf->m_nActivateChannels) {
...
bool KUiMsgCentrePad::IsChannelType(int nChannelIndex, SelfChannel type) {
  if (m_pSelf && nChannelIndex >= 0 &&
    ...
    case ch_Screen:
    case ch_GM: {
      return m_pSelf->FindActivateChannelIndexByKey(s_keys[type]) ==
             nChannelIndex;
    } break;
    default:
      return false;
...
int KUiMsgCentrePad::FindActivateChannelResourceIndex(char *cTitle) {
  ...
  for (int n = 0; n < m_nActivateChannels; n++) {
    if (m_pActivateChannel[n].ResourceIndex < 0)
      continue;
    if (strcmp(cTitle, m_ChannelsResource[m_pActivateChannel[n].ResourceIndex]
                           .cTitle) == 0)
...
int KUiMsgCentrePad::FindActivateChannelIndexByKey(char *cKey) {
  ...
  for (int n = 0; n < m_nActivateChannels; n++) {
    if (m_pActivateChannel[n].ResourceIndex >= 0) {
      if (m_ChannelsResource[m_pActivateChannel[n].ResourceIndex]
              .cFormatName[0] != 0) {
        char *pdest = strstr(
            m_ChannelsResource[m_pActivateChannel[n].ResourceIndex].cFormatName,
            cKey);
        if (pdest)
          return n;
      }
...
bool KUiMsgCentrePad::GetChannelMenuinfo(int nChannelIndex, WORD *pnPicIndex,
                                         WORD *pPicHei, KRColor *puTextColor,
                                         KRColor *puBkColor, char *pszMenuText,
                                         short *pnCheckPicIndex) {
  if (m_pSelf && nChannelIndex >= 0 &&
      nChannelIndex < m_pSelf->m_nActivateChannels) {
    ...
    if (n >= 0) {
      if (m_pSelf->m_ChannelsResource[n].nNeverClose) {
        ...
        if (pnCheckPicIndex)
          *pnCheckPicIndex =
              m_pSelf->m_pActivateChannel[nChannelIndex].bSubscribe
                  ? m_pSelf->m_nCheckOnPicIndex
                  : m_pSelf->m_nCheckOffPicIndex;
      }
...
char *KUiMsgCentrePad::GetChannelTitle(int nChannelIndex) {
  if (m_pSelf && nChannelIndex >= 0 &&
    ...
    if (m_pSelf->m_pActivateChannel[nChannelIndex].ResourceIndex >= 0)
      return m_pSelf
          ->m_ChannelsResource[m_pSelf->m_pActivateChannel[nChannelIndex]
                                   .ResourceIndex]
          .cTitle;
  }
  return NULL;
...
int KUiMsgCentrePad::PushChannelData(DWORD dwID, const char *Buffer, int nLen) {
  ...
  if (nIndex >= 0 && m_pSelf && Buffer && nLen > 0) {
    ...
    if (nRes >= 0 && m_pSelf->m_pActivateChannel[nIndex].uLeftSendMsgNum <
                         m_pSelf->m_ChannelsResource[nRes].uSendMsgNum) {
      ...
      memcpy(m_pSelf->m_pActivateChannel[nIndex].Buffer + nOffset, &nSendTime,
             sizeof(nSendTime));
      nOffset += sizeof(nSendTime);
      memcpy(m_pSelf->m_pActivateChannel[nIndex].Buffer + nOffset, &nLen,
             sizeof(nLen));
      nOffset += sizeof(nLen);
      memcpy(m_pSelf->m_pActivateChannel[nIndex].Buffer + nOffset, Buffer,
             nLen);
      nOffset += nLen;
...
int KUiMsgCentrePad::GetChannelData(DWORD &dwID, BYTE &cost, char *&Buffer,
                                    int &nLen) {
  if (m_pSelf) {
    ...
    for (int i = 0; i < m_pSelf->m_nActivateChannels; i++) {
      if (m_pSelf->m_pActivateChannel[i].uLeftSendMsgNum > 0) {
        unsigned int *pST =
            (unsigned int *)(m_pSelf->m_pActivateChannel[i].Buffer);
        if (*pST <= nSendTime) {
          dwID = m_pSelf->m_pActivateChannel[i].nChannelID;
          cost = m_pSelf->m_pActivateChannel[i].cost;
          nLen = *((int *)(m_pSelf->m_pActivateChannel[i].Buffer +
                           sizeof(unsigned int)));
          Buffer = m_pSelf->m_pActivateChannel[i].Buffer +
  ...
  if (m_pSelf) {
    ...
    for (int i = 0; i < m_pSelf->m_nActivateChannels; i++) {
      if (m_pSelf->m_pActivateChannel[i].nChannelID == dwID &&
        ...
        int nLen = *((int *)(m_pSelf->m_pActivateChannel[i].Buffer +
                             sizeof(unsigned int)));
...
bool KUiMsgCentrePad::GetPeopleMenuinfo(char *szDest, WORD *pnPicIndex,
                                        WORD *pPicHei, KRColor *puTextColor,
                                        KRColor *puBkColor) {
  if (m_pSelf) {
...
void KUiMsgCentrePad::SendQueryChannelID(int nChannelResourceIndex) {
  ...
  if (FindActivateChannelResourceIndex(
          m_ChannelsResource[nChannelResourceIndex].cTitle) >= 0) // 已申请过了
    return;
  ...
  if (m_ChannelsResource[nChannelResourceIndex].cFormatName[0] == 0)
    return;
  if (!ReplaceSpecialField(
          ChannelName, m_ChannelsResource[nChannelResourceIndex].cFormatName))
    return;
  if (ChannelName[0] == 0)
    return;

  char ProtocolBuffer[sizeof(tagExtendProtoHeader) +
                      sizeof(PLAYERCOMM_QUERYCHANNELID)];
  ...
  PLAYERCOMM_QUERYCHANNELID *pdata =
      (PLAYERCOMM_QUERYCHANNELID *)(ProtocolBuffer + nProtocolOffset);
...
void KUiMsgCentrePad::SendChannelSubscribe(int nChannelIndex, bool b) {
  ...
  char ProtocolBuffer[sizeof(tagExtendProtoHeader) +
                      sizeof(PLAYERCOMM_SUBSCRIBE)];
  int nProtocolOffset = 0;
  tagExtendProtoHeader *pHead = (tagExtendProtoHeader *)ProtocolBuffer;
  pHead->ProtocolType = c2s_extend;
  nProtocolOffset += sizeof(tagExtendProtoHeader);

  PLAYERCOMM_SUBSCRIBE *pdata =
      (PLAYERCOMM_SUBSCRIBE *)(ProtocolBuffer + nProtocolOffset);
  pdata->ProtocolFamily = pf_playercommunity;
...
int KUiMsgCentrePad::AddActivateChannel(const KChannelActivateInfo &Item) {
  ...
  if (n >= 0)
    return n;
  // 新增频道
  KChannelActivateInfo *pChannels =
      new KChannelActivateInfo[m_nActivateChannels + 1];
  if (m_nActivateChannels > 0 && m_pActivateChannel) {
    memcpy(pChannels, m_pActivateChannel,
           m_nActivateChannels * sizeof(KChannelActivateInfo));
  ...
  *pChannels = Item;
  pChannels->uLastSendMsgTime =
      IR_GetCurrentTime() - m_ChannelsResource[Item.ResourceIndex]
                                .uSendMsgInterval; // 确保第一条肯定能发
...
int KUiMsgCentrePad::ReleaseActivateChannel(int nChannelIndex) {
  ...
  if (nChannelIndex + 1 < m_nActivateChannels) {
    memmove(m_pActivateChannel + nChannelIndex,
            m_pActivateChannel + nChannelIndex + 1,
            (m_nActivateChannels - nChannelIndex - 1) *
                sizeof(KChannelActivateInfo));
  }
...
void KUiMsgCentrePad::Initialize() {
  ...
  AddChild(&m_ChatRoom1);
  AddChild(&m_ChatRoom2);
  AddChild(&m_ChatRoom3);
  AddChild(&m_ChatRoom4);
  ...
  m_Sys.AddChild(&m_Sys.m_AllChanel);
  m_Sys.AddChild(&m_Sys.m_PriChanel);
  m_Sys.AddChild(&m_Sys.m_MenpaiChanel);
  m_Sys.AddChild(&m_Sys.m_TongChanel);
  m_Sys.AddChild(&m_Sys.m_OtherChanel);
...
void KUiMsgCentrePad::Breathe() {
  if (m_uAutoDelMsgInterval &&
    ...
    m_ChatRoom1.GetMessageListBox()->HideNextLine();
    m_ChatRoom2.GetMessageListBox()->HideNextLine();
    m_ChatRoom3.GetMessageListBox()->HideNextLine();
    m_ChatRoom4.GetMessageListBox()->HideNextLine();
...
void KUiMsgCentrePad::HideAllMessage() {
  if (m_pSelf) {
    ...
    m_pSelf->m_ChatRoom1.GetMessageListBox()->HideAllLine();
    m_pSelf->m_ChatRoom2.GetMessageListBox()->HideAllLine();
    m_pSelf->m_ChatRoom3.GetMessageListBox()->HideAllLine();
    m_pSelf->m_ChatRoom4.GetMessageListBox()->HideAllLine();
...
void KUiMsgCentrePad::ShowAllMessage() {
  if (m_pSelf) {
    ...
    m_pSelf->m_ChatRoom1.GetMessageListBox()->ClearHideLine();
    m_pSelf->m_ChatRoom2.GetMessageListBox()->ClearHideLine();
    m_pSelf->m_ChatRoom3.GetMessageListBox()->ClearHideLine();
    m_pSelf->m_ChatRoom4.GetMessageListBox()->ClearHideLine();

    m_pSelf->m_Sys.m_SysRoom.ClearHideLine();
  }
}

void KUiMsgCentrePad::SetTypeLB(int nType) {
  if (m_pSelf) {

    if (nType == 1) {
      m_pSelf->nIDChatRoom = 1;
      m_pSelf->m_ChatRoom.Hide();
      m_pSelf->m_ChatRoom1.Show();
      m_pSelf->m_ChatRoom2.Hide();
      m_pSelf->m_ChatRoom3.Hide();
      m_pSelf->m_ChatRoom4.Hide();
      m_pSelf->m_ChatRoom4.GetMessageListBox()->m_bItemActived = FALSE;
      m_pSelf->m_ChatRoom.GetMessageListBox()->m_bItemActived = FALSE;
      m_pSelf->m_ChatRoom1.GetMessageListBox()->m_bItemActived = TRUE;
      m_pSelf->m_ChatRoom3.GetMessageListBox()->m_bItemActived = FALSE;
      m_pSelf->m_ChatRoom2.GetMessageListBox()->m_bItemActived = FALSE;
      m_pSelf->m_ChatRoom4.GetMessageListBox()->UpdateChatItem();
      m_pSelf->m_ChatRoom.GetMessageListBox()->UpdateChatItem();
      m_pSelf->m_ChatRoom3.GetMessageListBox()->UpdateChatItem();
      m_pSelf->m_ChatRoom2.GetMessageListBox()->UpdateChatItem();
      m_pSelf->m_ChatRoom1.GetMessageListBox()->UpdateChatItem();
    } else if (nType == 2) {
      m_pSelf->nIDChatRoom = 2;
      m_pSelf->m_ChatRoom.Hide();
      m_pSelf->m_ChatRoom2.Show();
      m_pSelf->m_ChatRoom1.Hide();
      m_pSelf->m_ChatRoom3.Hide();
      m_pSelf->m_ChatRoom4.Hide();
      m_pSelf->m_ChatRoom4.GetMessageListBox()->m_bItemActived = FALSE;
      m_pSelf->m_ChatRoom.GetMessageListBox()->m_bItemActived = FALSE;
      m_pSelf->m_ChatRoom1.GetMessageListBox()->m_bItemActived = FALSE;
      m_pSelf->m_ChatRoom3.GetMessageListBox()->m_bItemActived = FALSE;
      m_pSelf->m_ChatRoom2.GetMessageListBox()->m_bItemActived = TRUE;
      m_pSelf->m_ChatRoom4.GetMessageListBox()->UpdateChatItem();
      m_pSelf->m_ChatRoom.GetMessageListBox()->UpdateChatItem();
      m_pSelf->m_ChatRoom3.GetMessageListBox()->UpdateChatItem();
      m_pSelf->m_ChatRoom1.GetMessageListBox()->UpdateChatItem();
      ...
      m_pSelf->nIDChatRoom = 3;
      m_pSelf->m_ChatRoom.Hide();
      m_pSelf->m_ChatRoom3.Show();
      m_pSelf->m_ChatRoom1.Hide();
      m_pSelf->m_ChatRoom2.Hide();
      m_pSelf->m_ChatRoom4.Hide();
      m_pSelf->m_ChatRoom4.GetMessageListBox()->m_bItemActived = FALSE;
      m_pSelf->m_ChatRoom.GetMessageListBox()->m_bItemActived = FALSE;
      m_pSelf->m_ChatRoom1.GetMessageListBox()->m_bItemActived = FALSE;
      m_pSelf->m_ChatRoom3.GetMessageListBox()->m_bItemActived = TRUE;
      m_pSelf->m_ChatRoom2.GetMessageListBox()->m_bItemActived = FALSE;
      m_pSelf->m_ChatRoom4.GetMessageListBox()->UpdateChatItem();
      m_pSelf->m_ChatRoom.GetMessageListBox()->UpdateChatItem();
      m_pSelf->m_ChatRoom1.GetMessageListBox()->UpdateChatItem();
      ...
      m_pSelf->nIDChatRoom = 4;
      m_pSelf->m_ChatRoom.Hide();
      m_pSelf->m_ChatRoom4.Show();
      m_pSelf->m_ChatRoom1.Hide();
      m_pSelf->m_ChatRoom3.Hide();
      m_pSelf->m_ChatRoom2.Hide();
      m_pSelf->m_ChatRoom4.GetMessageListBox()->m_bItemActived = TRUE;
      m_pSelf->m_ChatRoom.GetMessageListBox()->m_bItemActived = FALSE;
      m_pSelf->m_ChatRoom1.GetMessageListBox()->m_bItemActived = FALSE;
      m_pSelf->m_ChatRoom3.GetMessageListBox()->m_bItemActived = FALSE;
      m_pSelf->m_ChatRoom2.GetMessageListBox()->m_bItemActived = FALSE;
    ...
    //	m_pSelf->m_ChatRoom.GetMessageListBox()->SetType(nType);
    //	m_pSelf->m_ChatRoom.GetMessageListBox()->UpdateDataP();
    //	m_pSelf->m_ChatRoom.GetScrollBar()->SetScrollPos(
    //	m_pSelf->m_ChatRoom.GetScrollBar()->GetMaxValue());
  }
}

void KUiMsgCentrePad::UpdateDelMsgTime() {
  if (m_pSelf) {
    m_pSelf->m_uLastDelMsgTime = IR_GetCurrentTime();
  }
}
...
void KUiMsgCentrePad::LoadScheme(KIniFile *pIni) {
  ...
  m_bSizeUp = (nV != 0);
  if (m_bSizeUp)
    m_SizeBtn.SetStyle(m_SizeBtn.GetStyle() & ~WND_S_MOVE_WITH_R_EDGE &
                       ~WND_S_MOVE_WITH_B_EDGE);
  ...
  if (pIni->GetString("Main", "CheckOnImage", "", szImage, MAX_PATH)) {
    if (g_pIInlinePicSink) {
      g_pIInlinePicSink->AddCustomInlinePic((WORD &)m_nCheckOnPicIndex,
                                            szImage);
    }
  }
  m_nCheckOffPicIndex = -1;
  if (pIni->GetString("Main", "CheckOffImage", "", szImage, MAX_PATH)) {
    if (g_pIInlinePicSink) {
      g_pIInlinePicSink->AddCustomInlinePic((WORD &)m_nCheckOffPicIndex,
                                            szImage);
    }
  }
  ...
  m_ChatRoom1.Init(pIni, "ChatRoom");
  m_ChatRoom2.Init(pIni, "ChatRoom");
  m_ChatRoom3.Init(pIni, "ChatRoom");
  m_ChatRoom4.Init(pIni, "ChatRoom");
  ...
  char TabChannelName[32];
  char TabChannelColor[32];
  pIni->GetString("TabButton", "Color", "0,0,0", TabChannelColor,
                  sizeof(TabChannelColor));

  if (pIni->GetString("ChatTab", "ChatTabLabel_0", "", TabChannelName,
                      sizeof(TabChannelName))) {
    m_Sys.m_AllChanel.Init(pIni, "AllChanel");
    m_Sys.m_AllChanel.SetText(TabChannelName);
    m_Sys.m_AllChanel.CheckButton(true);
  }
  if (pIni->GetString("ChatTab", "ChatTabLabel_1", "", TabChannelName,
                      sizeof(TabChannelName))) {
    m_Sys.m_PriChanel.Init(pIni, "PriChanel");
    m_Sys.m_PriChanel.SetText(TabChannelName);
    m_Sys.m_PriChanel.CheckButton(false);
  }
  if (pIni->GetString("ChatTab", "ChatTabLabel_2", "", TabChannelName,
                      sizeof(TabChannelName))) {
    m_Sys.m_MenpaiChanel.Init(pIni, "MenpaiChanel");
    m_Sys.m_MenpaiChanel.SetText(TabChannelName);
    m_Sys.m_MenpaiChanel.CheckButton(false);
  }
  if (pIni->GetString("ChatTab", "ChatTabLabel_3", "", TabChannelName,
                      sizeof(TabChannelName))) {
    m_Sys.m_TongChanel.Init(pIni, "TongChanel");
    m_Sys.m_TongChanel.SetText(TabChannelName);
    m_Sys.m_TongChanel.CheckButton(false);
  }
  if (pIni->GetString("ChatTab", "ChatTabLabel_4", "", TabChannelName,
                      sizeof(TabChannelName))) {
    m_Sys.m_OtherChanel.Init(pIni, "OtherChanel");
    m_Sys.m_OtherChanel.SetText(TabChannelName);
    m_Sys.m_OtherChanel.CheckButton(false);
  }
  ...
  if (pIni->GetString("MSNRoom", "Friend_MenuImage", "", szImage, MAX_PATH)) {
    if (g_pIInlinePicSink) {
      ...
      g_pIInlinePicSink->AddCustomInlinePic(m_MSNInfo.nFriendMenuPicIndex,
                                            szImage);
    }
  }
  ...
  if (pIni->GetString("MSNRoom", "Friend_TextImage", "", szImage, MAX_PATH)) {
    if (g_pIInlinePicSink) {
      g_pIInlinePicSink->AddCustomInlinePic(m_MSNInfo.nFriendTextPicIndex,
                                            szImage);
    }
  }
  pIni->GetString("MSNRoom", "Friend_MenuBkColor", "0,0,0", ChName,
                  sizeof(ChName));
  m_MSNInfo.uFriendMenuBkColor.Color_dw = GetColor(ChName);
  m_MSNInfo.uFriendMenuBkColor.Color_b.a = 0;

  pIni->GetString("MSNRoom", "Friend_Sound", "",
                  m_MSNInfo.szFriendSoundFileName,
  ...
  if (pIni->GetString("MSNRoom", "Stranger_MenuImage", "", szImage, MAX_PATH)) {
    if (g_pIInlinePicSink) {
      ...
      g_pIInlinePicSink->AddCustomInlinePic(m_MSNInfo.nStrangerMenuPicIndex,
                                            szImage);
    }
  }
  ...
  if (pIni->GetString("MSNRoom", "Stranger_TextImage", "", szImage, MAX_PATH)) {
    if (g_pIInlinePicSink) {
      g_pIInlinePicSink->AddCustomInlinePic(m_MSNInfo.nStrangerTextPicIndex,
                                            szImage);
    }
  }
  ...
  if (pIni->GetString("MSNRoom", "Self_TextImage", "", szImage, MAX_PATH)) {
    if (g_pIInlinePicSink) {
      g_pIInlinePicSink->AddCustomInlinePic(m_MSNInfo.nSelfTextPicIndex,
                                            szImage);
    }
  }

  pIni->GetString("MSNRoom", "Stranger_MenuBkColor", "0,0,0", ChName,
                  sizeof(ChName));
  ...
  for (int nCh = 0; nCh < MAX_CHANNELRESOURCE; nCh++) {
    ...
    if (pIni->GetString("Channels", ChName, "", m_ChannelsResource[nCh].cTitle,
                        32)) {
      pIni->GetString(m_ChannelsResource[nCh].cTitle, "TextColor", "0,0,0",
      ...
      pIni->GetString(m_ChannelsResource[nCh].cTitle, "TextBorderColor",
                      "0,0,0", ChName, sizeof(ChName));
      ...
      for (int m = 0; m < MAX_CHANNELRESOURCE; m++) {
        ...
        pIni->GetString(m_ChannelsResource[nCh].cTitle, ChName, "",
                        m_ChannelsResource[nCh].cShortTitle[m],
                        sizeof(m_ChannelsResource[nCh].cShortTitle[m]));
      }

      pIni->GetString(m_ChannelsResource[nCh].cTitle, "FormatName", "",
                      m_ChannelsResource[nCh].cFormatName,
                      sizeof(m_ChannelsResource[nCh].cFormatName));

      pIni->GetString(m_ChannelsResource[nCh].cTitle, "MenuText", "",
                      m_ChannelsResource[nCh].cMenuText,
                      sizeof(m_ChannelsResource[nCh].cMenuText));
      pIni->GetString(m_ChannelsResource[nCh].cTitle, "DeactivateMenuText",
                      m_ChannelsResource[nCh].cMenuText,
      ...
      if (pIni->GetString(m_ChannelsResource[nCh].cTitle, "MenuImage", "",
                          szImage, MAX_PATH)) {
        if (g_pIInlinePicSink) {
          ...
          g_pIInlinePicSink->AddCustomInlinePic(
              m_ChannelsResource[nCh].nMenuPicIndex, szImage);
        }
      }
      m_ChannelsResource[nCh].nMenuDeactivatePicIndex =
          m_ChannelsResource[nCh].nMenuPicIndex;
      m_ChannelsResource[nCh].nMenuDeactivatePicHeight =
          m_ChannelsResource[nCh].nMenuPicHeight;
      if (pIni->GetString(m_ChannelsResource[nCh].cTitle, "DeactivateMenuImage",
                          "", szImage, MAX_PATH)) {
        if (g_pIInlinePicSink) {
          ...
          g_pIInlinePicSink->AddCustomInlinePic(
              m_ChannelsResource[nCh].nMenuDeactivatePicIndex, szImage);
        }
      }
      m_ChannelsResource[nCh].nTextPicIndex = -1;
      if (pIni->GetString(m_ChannelsResource[nCh].cTitle, "TextImage", "",
                          szImage, MAX_PATH)) {
        if (g_pIInlinePicSink) {
          g_pIInlinePicSink->AddCustomInlinePic(
              m_ChannelsResource[nCh].nTextPicIndex, szImage);
        }
      }
      pIni->GetString(m_ChannelsResource[nCh].cTitle, "MenuBkColor", "0,0,0",
                      ChName, sizeof(ChName));
      m_ChannelsResource[nCh].uMenuBkColor.Color_dw = GetColor(ChName);
      m_ChannelsResource[nCh].uMenuBkColor.Color_b.a = 0;

      pIni->GetInteger(m_ChannelsResource[nCh].cTitle, "NeverClose", 0,
                       &m_ChannelsResource[nCh].nNeverClose);
      pIni->GetString(m_ChannelsResource[nCh].cTitle, "Sound", "",
                      m_ChannelsResource[nCh].szSoundFileName,
                      sizeof(m_ChannelsResource[nCh].szSoundFileName));

      pIni->GetInteger(m_ChannelsResource[nCh].cTitle, "SendMsgInterval", 0,
                       (int *)(&m_ChannelsResource[nCh].uSendMsgInterval));
      pIni->GetInteger(m_ChannelsResource[nCh].cTitle, "SendMsgNum", 0,
                       (int *)(&m_ChannelsResource[nCh].uSendMsgNum));
  ...
  pIni->GetString("Channels", "DefaultChannelSendName", "系统",
                  m_DefaultChannelSendName, 32);
  ...
  pIni->GetString("Main", "NameTextBorderColor", "0,0,0", ChName,
                  sizeof(ChName));
...
int KUiMsgCentrePad::PtInWindow(int x, int y) {
  ...
  if (KWndWindow::PtInWindow(x, y)) {
    ...
    if (nIDChatRoom == 1) {
      ...
      nRet = (m_MoveImg.PtInWindow(x, y) || m_SizeBtn.PtInWindow(x, y) ||
              m_ChatRoom1.PtInWindow(x, y) || m_BgShadowBtn.PtInWindow(x, y));
    }

    else if (nIDChatRoom == 2) {

      nRet = (m_MoveImg.PtInWindow(x, y) || m_SizeBtn.PtInWindow(x, y) ||
              m_ChatRoom2.PtInWindow(x, y) || m_BgShadowBtn.PtInWindow(x, y));
    }

    else if (nIDChatRoom == 3) {

      nRet = (m_MoveImg.PtInWindow(x, y) || m_SizeBtn.PtInWindow(x, y) ||
              m_ChatRoom3.PtInWindow(x, y) || m_BgShadowBtn.PtInWindow(x, y));
    }

    else if (nIDChatRoom == 4) {

      nRet = (m_MoveImg.PtInWindow(x, y) || m_SizeBtn.PtInWindow(x, y) ||
              m_ChatRoom4.PtInWindow(x, y) || m_BgShadowBtn.PtInWindow(x, y));
    }

    else {

      nRet = (m_MoveImg.PtInWindow(x, y) || m_SizeBtn.PtInWindow(x, y) ||
              m_ChatRoom.PtInWindow(x, y) || m_BgShadowBtn.PtInWindow(x, y));
    }
  }
  return nRet;
}
...
// 窗口函数
int KUiMsgCentrePad::WndProc(unsigned int uMsg, unsigned int uParam,
                             int nParam) {
  switch (uMsg) {
    ...
    if (uParam == (unsigned int)(KWndWindow *)this) {
      if (HIWORD(nParam) == SEL_CHANNEL_MENU) {
        if (short(LOWORD(nParam)) >= 0) {
          CheckChannel(LOWORD(nParam),
                       !m_pActivateChannel[LOWORD(nParam)].bSubscribe);
  ...
  case WND_M_POPUPMENU: {
    char szCurText[256];
    szCurText[0] = 0;
    int nLen = GetMessageSendName((KWndWindow *)uParam, LOWORD(nParam),
                                  HIWORD(nParam), szCurText);
...
int KUiMsgCentrePad::GetMessageSendName(KWndWindow *pWnd, int nIndex,
                                        char *szCurText) {
  if (nIndex < 0)
    return 0;
  ...
  if (pWnd == (KWndWindow *)&m_ChatRoom) {
    nLen = m_ChatRoom.GetMessageListBox()->GetOneMessage(nIndex, szCurText, 256,
                                                         TRUE);
  } else if (pWnd == (KWndWindow *)&m_ChatRoom1) {
    nLen = m_ChatRoom1.GetMessageListBox()->GetOneMessage(nIndex, szCurText,
                                                          256, TRUE);
  } else if (pWnd == (KWndWindow *)&m_ChatRoom2) {
    nLen = m_ChatRoom2.GetMessageListBox()->GetOneMessage(nIndex, szCurText,
                                                          256, TRUE);
  } else if (pWnd == (KWndWindow *)&m_ChatRoom3) {
    nLen = m_ChatRoom3.GetMessageListBox()->GetOneMessage(nIndex, szCurText,
                                                          256, TRUE);
  } else if (pWnd == (KWndWindow *)&m_ChatRoom4) {
    nLen = m_ChatRoom4.GetMessageListBox()->GetOneMessage(nIndex, szCurText,
                                                          256, TRUE);
  }
...
int KUiMsgCentrePad::GetMessageSendName(KWndWindow *pWnd, int x, int y,
                                        char *szCurText) {
  int nIndex = -1;
  if (pWnd == (KWndWindow *)&m_ChatRoom) {
    nIndex = m_ChatRoom.GetMessageListBox()->HitTextAtPoint(x, y);
  } else if (pWnd == (KWndWindow *)&m_ChatRoom1) {
    nIndex = m_ChatRoom1.GetMessageListBox()->HitTextAtPoint(x, y);
  } else if (pWnd == (KWndWindow *)&m_ChatRoom2) {
    nIndex = m_ChatRoom2.GetMessageListBox()->HitTextAtPoint(x, y);
  } else if (pWnd == (KWndWindow *)&m_ChatRoom3) {
    nIndex = m_ChatRoom3.GetMessageListBox()->HitTextAtPoint(x, y);
  }

  else if (pWnd == (KWndWindow *)&m_ChatRoom4) {
    nIndex = m_ChatRoom4.GetMessageListBox()->HitTextAtPoint(x, y);
  }
...
void KUiMsgCentrePad::PaintWindow() {
  ...
  if ((m_bSizingWnd || m_bShowShadow) && g_pRepresentShell) {
    ...
    if (nIDChatRoom == 1) {
      m_ChatRoom1.GetMessageListBox()->GetAbsolutePos(&bg.oPosition.nX,
                                                      &bg.oPosition.nY);
      bg.Color.Color_dw = 0x16000000;
      m_ChatRoom1.GetMessageListBox()->GetSize(&bg.oEndPos.nX, &bg.oEndPos.nY);
    } else if (nIDChatRoom == 2) {
      m_ChatRoom2.GetMessageListBox()->GetAbsolutePos(&bg.oPosition.nX,
                                                      &bg.oPosition.nY);
      bg.Color.Color_dw = 0x16000000;
      m_ChatRoom2.GetMessageListBox()->GetSize(&bg.oEndPos.nX, &bg.oEndPos.nY);
    } else if (nIDChatRoom == 3) {
      m_ChatRoom3.GetMessageListBox()->GetAbsolutePos(&bg.oPosition.nX,
                                                      &bg.oPosition.nY);
      bg.Color.Color_dw = 0x16000000;
      m_ChatRoom3.GetMessageListBox()->GetSize(&bg.oEndPos.nX, &bg.oEndPos.nY);
    } else if (nIDChatRoom == 4) {
      m_ChatRoom4.GetMessageListBox()->GetAbsolutePos(&bg.oPosition.nX,
                                                      &bg.oPosition.nY);
      bg.Color.Color_dw = 0x16000000;
      m_ChatRoom4.GetMessageListBox()->GetSize(&bg.oEndPos.nX, &bg.oEndPos.nY);
    } else {

      m_ChatRoom.GetMessageListBox()->GetAbsolutePos(&bg.oPosition.nX,
                                                     &bg.oPosition.nY);
      bg.Color.Color_dw = 0x16000000;
...
void KUiMsgCentrePad::DragWndSize(int nMoveOffset) {
  if (nMoveOffset == 0) {
    ...
    if (ChanageHeight(m_nSizeBtnDragOffsetY,
                      m_bSizeUp)) // 改变Height成功后，就不用积累了
      m_nSizeBtnDragOffsetY = 0;
  }
}
...
int KUiMsgCentrePad::ChanageHeight(int nOffsetY, bool bAtTop) {
  ...
  if (nIDChatRoom == 1) {
    ...
    if (bAtTop)
      nOffsetY = m_ChatRoom1.HeightToLineHeight(-nOffsetY);
    else
      nOffsetY = m_ChatRoom1.HeightToLineHeight(nOffsetY);
    ...
    if (nOffsetY != 0) {
      int x, y;
      GetSize(&x, &y);
      if (y + nOffsetY >= m_ChatRoom1.GetMinHeight()) {
        if (bAtTop) {
    ...
    if (bAtTop)
      nOffsetY = m_ChatRoom2.HeightToLineHeight(-nOffsetY);
    else
      nOffsetY = m_ChatRoom2.HeightToLineHeight(nOffsetY);
    ...
    if (nOffsetY != 0) {
      int x, y;
      GetSize(&x, &y);
      if (y + nOffsetY >= m_ChatRoom2.GetMinHeight()) {
        if (bAtTop) {
          if (m_Top - nOffsetY < m_nMinTopPos)
            SetPosition(m_Left, m_nMinTopPos);
          else
    ...
    if (bAtTop)
      nOffsetY = m_ChatRoom3.HeightToLineHeight(-nOffsetY);
    else
      nOffsetY = m_ChatRoom3.HeightToLineHeight(nOffsetY);
    ...
    if (nOffsetY != 0) {
      int x, y;
      GetSize(&x, &y);
      if (y + nOffsetY >= m_ChatRoom3.GetMinHeight()) {
        if (bAtTop) {
          if (m_Top - nOffsetY < m_nMinTopPos)
            SetPosition(m_Left, m_nMinTopPos);
          else
    ...
    if (bAtTop)
      nOffsetY = m_ChatRoom4.HeightToLineHeight(-nOffsetY);
    else
      nOffsetY = m_ChatRoom4.HeightToLineHeight(nOffsetY);
    ...
    if (nOffsetY != 0) {
      int x, y;
      GetSize(&x, &y);
      if (y + nOffsetY >= m_ChatRoom4.GetMinHeight()) {
        if (bAtTop) {
          if (m_Top - nOffsetY < m_nMinTopPos)
            SetPosition(m_Left, m_nMinTopPos);
          else
...
void KUiMsgCentrePad::Clear() {
  if (m_pSelf) {
    ...
    m_pSelf->m_ChatRoom1.GetMessageListBox()->Clear();
    m_pSelf->m_ChatRoom2.GetMessageListBox()->Clear();
    m_pSelf->m_ChatRoom3.GetMessageListBox()->Clear();
    m_pSelf->m_ChatRoom4.GetMessageListBox()->Clear();
...
void KUiMsgCentrePad::PopupChannelMenu(int x, int y) {
  ...
  KPopupMenuData *pMenuData =
      (KPopupMenuData *)malloc(MENU_DATA_SIZE(nChannelDataCount));
  if (pMenuData == NULL) {
    return;
  }
  ...
  for (int i = 0; i < nChannelDataCount; i++) {
    ...
    if (GetChannelMenuinfo(iC, &nPicIndex, &nHei, &uColor, &uBKColor, szText,
                           &nCheckPicIndex) &&
        nCheckPicIndex != -1) {
...
int KUiMsgCentrePad::CheckChannel(int nChannelIndex, bool b) {
  ...
  if (nChannelIndex < 0 || nChannelIndex >= m_pSelf->m_nActivateChannels)
    return 0;
  if (m_pSelf->IsNeverCloseActivateChannel(
          nChannelIndex)) // 不能关闭的频道不能改变状态
    return 0;
  ...
  if (n >= 0) {
    char Buffer[32];
    strncpy(Buffer,
            b ? m_pSelf->m_ChannelsResource[n].cMenuText
              : m_pSelf->m_ChannelsResource[n].cMenuDeactivateText,
            32);
...
int KUiMsgCentrePad::FilterTextColor(char *pMsgBuff, unsigned short nMsgLength,
                                     const KRColor &uColor) {
  nMsgLength = TClearSpecialCtrlInEncodedText(pMsgBuff, nMsgLength, KTC_COLOR);
  nMsgLength =
      TClearSpecialCtrlInEncodedText(pMsgBuff, nMsgLength, KTC_COLOR_RESTORE);
...
void KUiMsgCentrePad::SetFontSize(int nFontSize) {
  if (m_pSelf) {
    ...
    m_pSelf->m_ChatRoom1.GetMessageListBox()->SetFontSize(nFontSize);
    m_pSelf->m_ChatRoom2.GetMessageListBox()->SetFontSize(nFontSize);
    m_pSelf->m_ChatRoom3.GetMessageListBox()->SetFontSize(nFontSize);
    m_pSelf->m_ChatRoom4.GetMessageListBox()->SetFontSize(nFontSize);
  }
}

int KUiMsgCentrePad::GetFontSize() {
  if (m_pSelf) {
    if (m_pSelf->nIDChatRoom == 1) {
      m_pSelf->m_ChatRoom1.GetMessageListBox()->GetFontSize();
    } else if (m_pSelf->nIDChatRoom == 2) {
      m_pSelf->m_ChatRoom2.GetMessageListBox()->GetFontSize();
    } else if (m_pSelf->nIDChatRoom == 3) {
      m_pSelf->m_ChatRoom3.GetMessageListBox()->GetFontSize();
    } else if (m_pSelf->nIDChatRoom == 4) {
      m_pSelf->m_ChatRoom4.GetMessageListBox()->GetFontSize();
    } else {
...
int KUiMsgCentrePad::SetChannelTextColor(char *cTitle, DWORD uTextColor,
                                         DWORD uTextBorderColor) {
  if (m_pSelf) {
    ...
    if (nRIndex >= 0) {
      ...
      m_pSelf->m_ChannelsResource[nRIndex].uTextBorderColor.Color_dw =
          uTextBorderColor;
      m_pSelf->m_ChannelsResource[nRIndex].uTextBorderColor.Color_b.a =
          KTC_BORDER_COLOR;
    }
...
char *KUiMsgCentrePad::GetChannelTextColor(int nIndex, DWORD &uTextColor,
                                           DWORD &uTextBorderColor) {
  if (m_pSelf && nIndex >= 0 && nIndex < m_pSelf->m_nChannelsResource) {
    uTextColor = m_pSelf->m_ChannelsResource[nIndex].uTextColor.Color_dw;
    uTextBorderColor =
        m_pSelf->m_ChannelsResource[nIndex].uTextBorderColor.Color_dw;
...
void KUiMsgCentrePad::SetMSNTextColor(int nType, DWORD uTextColor,
                                      DWORD uTextBorderColor) {
  if (m_pSelf) {
    if (nType == 0) {
...
// nType 0 is me, 1 is friend, 2 is stranger
void KUiMsgCentrePad::GetMSNTextColor(int nType, DWORD &uTextColor,
                                      DWORD &uTextBorderColor) {
  if (m_pSelf) {
    if (nType == 0) {
