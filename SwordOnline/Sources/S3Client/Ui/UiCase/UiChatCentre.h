...
public:
  //----界面面板统一的接口函数----
  static KUiChatCentre *
  OpenWindow(bool bShow);               // 打开窗口，返回唯一的一个类对象实例
  ...
  static void UpdateData(UICHATCENTRE_UPDATE eFlag, unsigned int uParam,
                         int nGroupIndex);
  ...
  static void ReplaceGroupName(int nUnitIndex, const char *sz,
                               const char *szGroup);
  static void MoveFriendToGroup(int nUnitIndex, const char *Name,
                                const char *group);
  static void MoveGroupToGroup(int nUnitIndex, const char *group,
