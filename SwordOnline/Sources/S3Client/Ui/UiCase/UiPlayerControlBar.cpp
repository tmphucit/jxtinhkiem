	...
	if (g_pCoreShell)
	{
		if (m_BuffListCount)
		{
			...
			if (nCount)
			{
				...
				{
					g_pCoreShell->GetGameData(GDI_NPC_STATE_SKILL, (unsigned int)m_pStateSkill, nCount);//单线程执行，nCount值不变
					int nSkillID;
...
void KUiPlayerControlBar::Add(int nSkillPos, KStateControl* pStateSkill)
{
	...
	{
		strcpy(szTemp, "N/A");
		strcat(pszDesc, "Duy tr?v躰h vi詎 ");
	}
	else
	{
		sprintf(szTemp, "Duy tr?%02dh:%02dm:%02ds", pStateSkill->nLeftTime/18/60/60, pStateSkill->nLeftTime/18/60%60, pStateSkill->nLeftTime/18%60);
		strcat(pszDesc, szTemp);
