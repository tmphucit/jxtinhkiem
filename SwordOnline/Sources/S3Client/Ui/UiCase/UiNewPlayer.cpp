...
void KUiNewPlayer::Initialize() {
  ...
  if (Ini.Load(SERIES_INI)) {
    for (int i = 0; i < series_num; i++) {
      Ini.GetString(s_szSeriesSectionList[i], "PropText", "", Buff,
                    sizeof(Buff));
      m_propTypeInfoTable[i].nShowTextLen = strlen(Buff);
      m_propTypeInfoTable[i].nShowTextLen =
          TEncodeText(Buff, m_propTypeInfoTable[i].nShowTextLen);
      memcpy(m_propTypeInfoTable[i].propertyShow, Buff,
             m_propTypeInfoTable[i].nShowTextLen);

      Ini.GetString(s_szSeriesSectionList[i], "SelSound_m", "",
                    m_propTypeInfoTable[i].szMaleSound,
                    sizeof(m_propTypeInfoTable[i].szMaleSound));
      Ini.GetString(s_szSeriesSectionList[i], "SelSound_f", "",
                    m_propTypeInfoTable[i].szFemaleSound,
                    sizeof(m_propTypeInfoTable[i].szFemaleSound));
    }
  }
...
void KUiNewPlayer::SelGender() {
  ...
  KUiSelPlayer::GetRoleImageName(szFile, m_szPlayerImgPrefix, false,
                                 m_Info.Attribute, bMale ? 1 : 2);
  m_Male.CheckButton(bMale);
  m_Male.SetImage(ISI_T_SPR, szFile);
  m_Male.SetFrame(0);

  KUiSelPlayer::GetRoleImageName(szFile, m_szPlayerImgPrefix, true,
                                 m_Info.Attribute, bMale ? 2 : 1);
...
void KUiNewPlayer::LoadScheme(const char *pScheme) {
  ...
  if (Ini.Load(Buff)) {
    ...
    Ini.GetString("NewPlayer", "PlayerImgPrefix", "", m_szPlayerImgPrefix,
                  sizeof(m_szPlayerImgPrefix));
    ...
    for (int i = 0; i < series_num; i++) {
      ...
      Ini.GetString(s_szSeriesSectionList[i], "MaleImg", "",
                    m_propTypeInfoTable[i].szMaleImg,
                    sizeof(m_propTypeInfoTable[i].szMaleImg));
      Ini.GetString(s_szSeriesSectionList[i], "FemaleImg", "",
                    m_propTypeInfoTable[i].szFemaleImg,
                    sizeof(m_propTypeInfoTable[i].szFemaleImg));
    }
  }
}
...
void KUiNewPlayer::UpdateProperty() {
  for (int i = 0; i < series_num; i++)
    ((KWndButton *)m_propTypeInfoTable[i].pBtn)
        ->CheckButton(i == m_Info.Attribute);
...
void KUiNewPlayer::OnOk() {
  if (GetInputInfo()) {
    ...
    KUiConnectInfo::OpenWindow(CI_MI_CREATING_ROLE, LL_S_IN_GAME,
                               m_Info.NativePlaceId);
    CloseWindow(false);
  }
}
...
int KUiNewPlayer::GetInputInfo() {
  ...
  KUiConnectInfo::OpenWindow(CI_MI_INVALID_LOGIN_INPUT2, CI_NS_NEW_ROLE_WND,
                             m_Info.NativePlaceId);
  return false;
}
...
void KUiNewPlayer::Breathe() {
  ...
  if (m_bJustClicked) {
    ...
    if ((m_Info.Gender == OBJ_G_MALE) && nLoopBackMale)
      ...
      pBtn = &m_Female;
    if (pBtn) {
      KUiSelPlayer::GetRoleImageName(szFileName, m_szPlayerImgPrefix,
                                     m_Info.Gender, m_Info.Attribute, 0);
