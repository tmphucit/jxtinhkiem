...
//	功能：如果窗口正被显示，则返回实例指针
//--------------------------------------------------------------------------
KUiShop *KUiShop::GetIfVisible() { return m_pSelf; }

//--------------------------------------------------------------------------
//	功能：打开窗口，返回唯一的一个类对象实例
//--------------------------------------------------------------------------
KUiShop *KUiShop::OpenWindow(int nSlect) {
  if (m_pSelf == NULL) {
    m_pSelf = new KUiShop;
    if (m_pSelf)
      m_pSelf->Initialize();
...
void KUiShop::CancelTrade() { CloseWindow(); }
...
void KUiShop::OnBuyItem(KUiDraggedObject *pItem, bool bDoImmed) {
  ...
  if (bDoImmed == false) {
    ...
    if (m_nSlect == 2) {
      if (g_pCoreShell->GetGameData(GDI_SHOP_ITEM_PRICE_TL,
                                    (unsigned int)(&Obj), (int)(&Price)) == 1) {
        KUiTradeConfirm::OpenWindow(&Obj, &Price, TCA_BUY, 2);
      }
    } else if (m_nSlect == 3) {
      if (g_pCoreShell->GetGameData(GDI_SHOP_ITEM_PRICE_VD,
                                    (unsigned int)(&Obj), (int)(&Price)) == 1) {
        KUiTradeConfirm::OpenWindow(&Obj, &Price, TCA_BUY, 3);
      }
    } else {

      int nCheckPrice = g_pCoreShell->GetGameData(
          GDI_TRADE_ITEM_PRICE, (unsigned int)(&Obj), (int)(&Price));

      if (nCheckPrice == 1 || nCheckPrice == 2) {
...
void KUiShop::UpdateData() {
  ...
  if (m_pObjsList = (KUiObjAtContRegion *)malloc(sizeof(KUiObjAtContRegion) *
                                                 m_nObjCount)) {
