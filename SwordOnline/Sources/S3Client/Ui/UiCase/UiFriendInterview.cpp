...
KUiFriendInterview *KUiFriendInterview::FindWindow(KUiPlayerItem *pFriend,
                                                   bool bCreate) {
...
void KUiFriendInterview::LoadScheme(const char *pScheme) {
  ...
  KIniFile *pSetting = g_UiBase.GetPrivateSettingFile();
  if (pSetting) {
    if (pSetting->GetString("UiConfig", "ChatColor", "0,0,0", Buff,
                            sizeof(Buff)))
      m_uLastTextColor = (::GetColor(Buff) & 0xFFFFFF);
...
//	功能：窗口函数
//--------------------------------------------------------------------------
int KUiFriendInterview::WndProc(unsigned int uMsg, unsigned int uParam,
                                int nParam) {
  int nRet = 0;
  switch (uMsg) {
...
void KUiFriendInterview::OnSend() {
  ...
  Msg.nContentLen =
      m_InputWnd.GetText(&Msg.szContent[0], sizeof(Msg.szContent), true);
