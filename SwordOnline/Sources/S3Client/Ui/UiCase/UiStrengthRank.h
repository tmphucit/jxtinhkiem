...
#if !defined(                                                                  \
    AFX_KUISTRENGTHORDER_H__7801F7C7_BAE8_4A49_BEA8_63B5F8FA0BB1__INCLUDED_)
...
private:
  ...
  static void
  StoreNewDataToTempFile(unsigned int uParam, // 把新收到的排名资料存到临时文件
                         KRankMessage *pInData);
  static void
  StoreNewDataToTempFile(unsigned int uParam, // 把新收到的索引资料存到临时文件
                         KRankIndex *pInData);
...
private:
  ...
  unsigned char *
      m_pIDontThinkSo; // 我是为了WndList而存在，好伟大的爱情啊~~(这是传给WndList的SetContent的数组指针)
