...
public:
  ...
  KWndPureTextBtn m_AllChanel;
  KWndPureTextBtn m_PriChanel;
  KWndPureTextBtn m_MenpaiChanel;
  KWndPureTextBtn m_TongChanel;
  KWndPureTextBtn m_OtherChanel;
...
public:
  ...
  static void
  CloseWindow(bool bDestroy); // 关闭窗口，同时可以选则是否删除对象实例
  static void Clear();
  ...
  static void SetTypeLB(int nTyp); // 显示所有输出

  static void UpdateDelMsgTime();

  static void SystemMessageArrival(const char *pMsgBuff,
                                   unsigned short nMsgLength);

  // 用于新聊天协议到达的处理
  static int NewChannelMessageArrival(int figure, DWORD nChannelID,
  ...
  static void NewMSNMessageArrival(char *szSourceName, char *szSendName,
                                   const char *pMsgBuff,
                                   unsigned short nMsgLength);
  static void OpenChannel(char *channelName, DWORD nChannelID,
                          BYTE cost); // 同时订阅
  // 用于外挂的聊天
  static void ShowSomeoneMessage(char *szSourceName, const char *pMsgBuff,
                                 unsigned short nMsgLength);
  enum SelfChannel {
    ch_Team = 0,
    ch_Faction,
  ...
  static bool GetChannelMenuinfo(int nChannelIndex, WORD *pnPicIndex = NULL,
                                 WORD *pPicHei = NULL,
                                 KRColor *puTextColor = NULL,
                                 KRColor *puBkColor = NULL,
                                 char *pszMenuText = NULL,
                                 short *pnCheckPicIndex = NULL);
  static bool GetPeopleMenuinfo(char *szDest, WORD *pnPicIndex = NULL,
                                WORD *pPicHei = NULL,
                                KRColor *puTextColor = NULL,
                                KRColor *puBkColor = NULL);
  ...
  static int SetChannelTextColor(char *cTitle, DWORD uTextColor,
                                 DWORD uTextBorderColor);
  static char *GetChannelTextColor(int nIndex, DWORD &uTextColor,
                                   DWORD &uTextBorderColor);
  static void SetMSNTextColor(
      int nType, DWORD uTextColor,
      DWORD uTextBorderColor); // nType 0 is me, 1 is friend, 2 is stranger
  static void GetMSNTextColor(int nType, DWORD &uTextColor,
                              DWORD &uTextBorderColor);
...
private:
  ...
  void ChannelMessageArrival(int figure, int nChannelIndex, char *szSendName,
                             const char *pMsgBuff, unsigned short nMsgLength,
                             KWndMessageListBox *pM, bool bName);
  // void	ChannelMessageArrival(int nChannelIndex, char* szSendName, const
  // char* pMsgBuff, unsigned short nMsgLength, KWndMessageListBox* pM, bool
  // bName);
  void MSNMessageArrival(char *szSourceName, char *szSendName,
                         const char *pMsgBuff, unsigned short nMsgLength);
  void ShowMSNMessage(char *szName, const char *pMsgBuff,
                      unsigned short nMsgLength, KRColor uColor,
                      KRColor uBKColor, KRColor uBorderColor, WORD nPic);
  void SendQueryChannelID(int nChannelResourceIndex);
  void SendChannelSubscribe(int nChannelIndex, bool b);
  int AddActivateChannel(
      const KChannelActivateInfo &Item);         // 返回增加Item的Index
...
#define SECOND_AUTODELMSG 20000
  ...
  KScrollMessageListBox m_ChatRoom; // tat ca

  KScrollMessageListBox m_ChatRoom1; // mat
  KScrollMessageListBox m_ChatRoom2; // mon phai
  KScrollMessageListBox m_ChatRoom3; // bang
  KScrollMessageListBox m_ChatRoom4; // khac
