...
void KUiOptions::LoadScheme(KIniFile *pIni) {
  ...
  for (i = 0; i < OPTION_INDEX_COUNT; i++) {
    ...
    pIni->GetString("ToggleOptionsName", itoa(i, szKey, 10), "",
                    m_ToggleItemList[i].szName,
                    sizeof(m_ToggleItemList[i].szName));
  }

  m_nFirstControlableIndex = 0;
  m_nToggleBtnValidCount = (OPTION_INDEX_COUNT <= MAX_TOGGLE_BTN_COUNT)
                               ? OPTION_INDEX_COUNT
                               : MAX_TOGGLE_BTN_COUNT;
  UpdateAllToggleBtn();
  UpdateAllStatusImg();
}
...
int KUiOptions::WndProc(unsigned int uMsg, unsigned int uParam, int nParam) {
  ...
  case WND_M_MENUITEM_SELECTED:
    if (uParam == (unsigned int)(KWndWindow *)this) {
      ...
      // if (m_pSkinMenu && short(LOWORD(nParam)) >= 0 && LOWORD(nParam) <
      // m_pSkinMenu->nNumItem)
      //{
      //	if
      //(g_UiBase.LoadScheme(m_pSkinMenu->Items[LOWORD(nParam)].szData))
  ...
  if (pSetting) {
    ...
    if (nCount > 0) {
      KPopupMenuData *pMenuData =
          (KPopupMenuData *)malloc(MENU_DATA_SIZE(nCount));
      if (pMenuData) {
        KPopupMenu::InitMenuData(pMenuData, nCount);
        for (i = 0; i < nCount; i++) {
          sprintf(szKey, "%d_Name", i);
          pSetting->GetString("ShortcutSet", szKey, "",
                              pMenuData->Items[i].szData,
...
/*void KUiOptions::PopupSkinMenu()
{
        ...
        for (int i = 0; i < nCount; i++)
        {
                ...
                m_pSkinMenu->Items[i].uDataLen =
strlen(m_pSkinMenu->Items[i].szData);
        }
        int x, y;
...
void KUiOptions::OnScrollBarPosChanged(KWndWindow *pWnd, int nPos) {
  if (pWnd == (KWndWindow *)&m_BGMValue)
    ...
    if (m_nToggleItemCount < m_nToggleItemCount &&
        nPos != m_nFirstControlableIndex && nPos < m_nToggleItemCount &&
        nPos >= 0) {
  ...
  for (int i = 0; i < m_nToggleBtnValidCount; i++) {
    if (m_ToggleItemList[m_nFirstControlableIndex + i].bInvalid == false) {
      nFrame = m_ToggleItemList[m_nFirstControlableIndex + i].bEnable
                   ? m_nStatusEnableFrame
                   : m_nStatusDisableFrame;
    } else {
...
void KUiOptions::LoadSetting(bool bReload, bool bUpdateOption) {
  ...
  if (bReload == false && m_pSelf) {
    ...
    if (pSetting) {
      pSetting->GetInteger(OPTIONS_SAVE_SECTION, "Brightness", 50,
                           &nBrightness);
      pSetting->GetInteger(OPTIONS_SAVE_SECTION, "MusicValue", 100,
                           &nMusicValue);
      pSetting->GetInteger(OPTIONS_SAVE_SECTION, "SoundValue", 100,
                           &nSoundValue);
      pSetting->GetInteger(OPTIONS_SAVE_SECTION, "ShortcutSet", 0,
                           &nSettingSet);
      for (i = 0; i < OPTION_INDEX_COUNT; i++)
        pSetting->GetInteger(OPTIONS_SAVE_SECTION, ls_ToggleOptionName[i], true,
                             &bOptionsEnable[i]);
  ...
  if (bUpdateOption && g_pCoreShell) {
    if (g_bRepresent3) {
      ...
      g_pCoreShell->OperationRequest(GOI_OPTION_SETTING, OPTION_DYNALIGHT,
                                     bOptionsEnable[OPTION_I_DYNALIGHT]);
      g_pCoreShell->OperationRequest(GOI_OPTION_SETTING, OPTION_BRIGHTNESS,
                                     nBrightness);
    }

    g_pCoreShell->OperationRequest(GOI_OPTION_SETTING, OPTION_MUSIC_VALUE,
                                   nMusicValue);
    g_pCoreShell->OperationRequest(GOI_OPTION_SETTING, OPTION_SOUND_VALUE,
                                   nSoundValue);
...
void KUiOptions::ToggleOption(int nIndex) {
  ...
  if (m_ToggleItemList[nIndex].bInvalid)
    return;
  bool bEnable = m_ToggleItemList[nIndex].bEnable =
      !m_ToggleItemList[nIndex].bEnable;
  switch (nIndex) {
