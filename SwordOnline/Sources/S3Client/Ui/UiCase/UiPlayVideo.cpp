...
// 载入界面方案
void KUiPlayVideo::LoadScheme(const char *pszSettingFile,
                              const char *pszSection) {
  if (m_pSelf && pszSettingFile && pszSection && pszSettingFile[0] &&
      pszSection[0]) {
    KIniFile Ini;
...
int KUiPlayVideo::Initialize() {
  ...
  m_KLVideoSoundFn =
      (KLVideoSoundFn)GetProcAddress(m_hVideoDll, "KLVideoSetSoundSystem");
  m_KLVideoSndtrackFn =
      (KLVideoSndtrackFn)GetProcAddress(m_hVideoDll, "KLVideoSetSoundTrack");
  ...
  m_KLVideoToBuffer =
      (KLVideoToBufferFn)GetProcAddress(m_hVideoDll, "KLVideoCopyToBuffer");
  m_KLVideoDoFrame =
      (KLVideoDoFrameFn)GetProcAddress(m_hVideoDll, "KLVideoDoFrame");
  m_KLVideoNextFrame =
      (KLVideoNextFrameFn)GetProcAddress(m_hVideoDll, "KLVideoNextFrame");
  m_KLVideoGetCurrentFrame = (KLVideoGetCurrentFrameFn)GetProcAddress(
      m_hVideoDll, "KLVideoGetCurrentFrame");
  m_KLVideoSetSoundVolume = (KLVideoSetSndVolumeFn)GetProcAddress(
      m_hVideoDll, "KLVideoSetSoundVolume");

  // make sure functions connected //
  if (!m_KLVideoSoundFn || !m_KLVideoOpen || !m_KLVideoClose ||
      !m_KLVideoWait || !m_KLVideoToBuffer || !m_KLVideoDoFrame ||
      !m_KLVideoNextFrame || !m_KLVideoSndtrackFn) {
...
int KUiPlayVideo::OpenVideo(const char *pszVideoFile) {
  ...
  for (int i = 0; i < 10000; i++) {
    ...
    m_uBitmapId = g_pRepresentShell->CreateImage(m_szBitmapName, m_bnk->Width,
                                                 m_bnk->Height, ISI_T_BITMAP16);
    if (m_uBitmapId)
      break;
  }
  ...
  if (m_uBitmapId) {
    m_uBitmapIsPosition = IMAGE_IS_POSITION_INIT;
    g_pRepresentShell->ClearImageData(m_szBitmapName, m_uBitmapId,
                                      m_uBitmapIsPosition);
