...
void KUiSkillTree::LoadConfig(KIniFile *pIni) {
    ...
    if (ms_ShortcutSkills[i].uGenre != CGOG_NOTHING) {
      ...
      for (j = 0; j < i; j++) {
        if (ms_ShortcutSkills[i].uId == ms_ShortcutSkills[j].uId &&
            ...
            ms_ShortcutSkills[i].IS_LEFT_SKILL ==
                ms_ShortcutSkills[j].IS_LEFT_SKILL) {
  ...
  do {
    ...
    int nNum = g_pCoreShell->GetGameData(bLeft ? GDI_LEFT_ENABLE_SKILLS
                                               : GDI_RIGHT_ENABLE_SKILLS,
                                         (unsigned int)&Skills, 0);
    ...
    pIni->GetStruct("Player", bLeft ? "LeftSkill" : "RightSkill", &Skill,
...
void KUiSkillTree::SaveConfig(KIniFile *pIni) {
  ...
  if (g_pCoreShell) {
    ...
    g_pCoreShell->GetGameData(GDI_PLAYER_IMMED_ITEMSKILL, (unsigned int)&Skill,
                              0);

    if (Skill.IMmediaSkill[0].uGenre != CGOG_NOTHING)
      pIni->WriteStruct("Player", "LeftSkill", &Skill.IMmediaSkill[0],
                        sizeof(KUiGameObject));
    else
      pIni->EraseKey("Player", "LeftSkill");
    if (Skill.IMmediaSkill[1].uGenre != CGOG_NOTHING)
      pIni->WriteStruct("Player", "RightSkill", &Skill.IMmediaSkill[1],
                        sizeof(KUiGameObject));
    else
...
void KUiSkillTree::UpdateData() {
  m_nNumSkills = g_pCoreShell->GetGameData(m_bLeft ? GDI_LEFT_ENABLE_SKILLS
                                                   : GDI_RIGHT_ENABLE_SKILLS,
                                           (unsigned int)&m_Skills, 0);
...
void KUiSkillTree::LoadScheme(const char *pScheme) {
  ...
  if (m_pSelf && Ini.Load(Buff)) {
    Ini.GetInteger2("Main", "LeftBtnPos", &m_pSelf->m_nLeftLeft,
                    &m_pSelf->m_nLeftTop);
    Ini.GetInteger2("Main", "RightBtnPos", &m_pSelf->m_nRightLeft,
                    &m_pSelf->m_nRightTop);
    Ini.GetInteger2("Main", "BtnSize", &m_pSelf->m_nWidthPerSkill,
                    &m_pSelf->m_nHeightPerSkill);
...
int KUiSkillTree::WndProc(unsigned int uMsg, unsigned int uParam, int nParam) {
  ...
  case WM_LBUTTONDOWN:
    ...
    if ((nIndex = GetSkillAtPos(x, y)) >= 0) {
      ...
      g_pCoreShell->OperationRequest(GOI_SET_IMMDIA_SKILL, (unsigned int)&Skill,
                                     m_bLeft ? 0 : 1);
    }
    Hide();
    break;
...
void KUiSkillTree::HandleShortcutKey(int nIndex) {
  ...
  if (GetIfVisible()) {
    ...
    for (int i = 0; i < SKILLTREE_SHORTCUT_SKILL_COUNT; i++) {
      if (i != nIndex) {
        if (ms_ShortcutSkills[nIndex].uId == ms_ShortcutSkills[i].uId &&
            ms_ShortcutSkills[nIndex].IS_LEFT_SKILL ==
                ms_ShortcutSkills[i].IS_LEFT_SKILL &&
    ...
    g_pCoreShell->OperationRequest(
        GOI_SET_IMMDIA_SKILL, (unsigned int)&ms_ShortcutSkills[nIndex],
...
void KUiSkillTree::PaintWindow() {
  ...
  int nDX =
      -m_nWidthPerSkill; // m_bLeft ? m_nWidthPerSkill : (-m_nWidthPerSkill);
  ...
  for (int i = 0; i < m_nNumSkills; i++) {
    ...
    g_pCoreShell->DrawGameObj(m_Skills[i].uGenre, m_Skills[i].uId, nLeft, nTop,
                              m_nWidthPerSkill, m_nHeightPerSkill, 0);
    ...
    for (int j = 0; j < SKILLTREE_SHORTCUT_SKILL_COUNT; j++) {
      if (m_bLeft == (unsigned)ms_ShortcutSkills[j].IS_LEFT_SKILL &&
        ...
        char *pszKey = NULL;
        if (nIndexC >= 0) {
          pszKey = (char *)KShortcutKeyCentre::GetKeyName(
              KShortcutKeyCentre::GetCommandKey(nIndexC));
        } else {
        ...
        if (pszKey) {
          g_pRepresentShell->OutputText(
              m_nFont, pszKey, KRF_ZERO_END, nLeft + m_nWidthPerSkill - m_nFont,
