...
void KUiFightSkillSubPage::UpdateSkill(KUiSkillData* pSkill, int nIndex)
{
	...
	m_FightSkills[nIndex].HoldObject(pSkill->uGenre, pSkill->uId, pSkill->nCurrentLevel, 1);
	else
	m_FightSkills[nIndex].HoldObject(pSkill->uGenre, pSkill->uId, pSkill->nCurrentLevel, 0);
}
...
void KUiFightSkillSubPage::UpdateData(KUiSkillData* pSkills)
{
	...
	for (int i = 0; i < FIGHT_SKILL_COUNT_PER_PAGE; i++)
	{
		...
		{
		m_FightSkills[i].HoldObject(pSkills[i].uGenre, pSkills[i].uId, pSkills[i].nCurrentLevel, 1);
		}
		else
		{
        m_FightSkills[i].HoldObject(pSkills[i].uGenre, pSkills[i].uId, pSkills[i].nCurrentLevel, 0);
		}
