...
void KUiFightSkillSubPage::LoadScheme(const char *pScheme) {
  ...
  if (Ini.Load(Buff)) {
    ...
    Ini.GetInteger2("SkillText", "Offset", (int *)&m_SkillTextParam.Offset.cx,
...
void KUiFightSkillSubPage::UpdateSkill(KUiSkillData *pSkill, int nIndex) {
  ...
  if (pSkill->nLevel != pSkill->nCurrentLevel)
    m_FightSkills[nIndex].HoldObject(pSkill->uGenre, pSkill->uId,
                                     pSkill->nCurrentLevel, 1);
  else
    m_FightSkills[nIndex].HoldObject(pSkill->uGenre, pSkill->uId,
                                     pSkill->nCurrentLevel, 0);
}
...
void KUiFightSkillSubPage::UpdateData(KUiSkillData *pSkills) {
  _ASSERT(pSkills);
  for (int i = 0; i < FIGHT_SKILL_COUNT_PER_PAGE; i++) {
    if (pSkills[i].nLevel != pSkills[i].nCurrentLevel) {
      m_FightSkills[i].HoldObject(pSkills[i].uGenre, pSkills[i].uId,
                                  pSkills[i].nCurrentLevel, 1);
    } else {
      m_FightSkills[i].HoldObject(pSkills[i].uGenre, pSkills[i].uId,
                                  pSkills[i].nCurrentLevel, 0);
    }
  }
}

// 窗口函数
int KUiFightSkillSubPage::WndProc(unsigned int uMsg, unsigned int uParam,
                                  int nParam) {
  if (uMsg == WND_N_LEFT_CLICK_ITEM && uParam && m_nRemainSkillPoint) {
    ...
    if (pObj->uGenre != CGOG_NOTHING) {
      ...
      g_pCoreShell->OperationRequest(GOI_TONE_UP_SKILL, CGOG_SKILL_FIGHT,
                                     pObj->uId);
    }
    return 0;
  }
...
void KUiFightSkillSubPage::PaintWindow() {
  ...
  for (int i = 0; i < FIGHT_SKILL_COUNT_PER_PAGE; i++) {
    ...
    if (Obj.uGenre != CGOG_NOTHING) {
      ...
      g_pCoreShell->DrawGameObj(CGOG_SKILL_LIVE, Obj.uId, nLeft, nTop, nWidth,
                                nHeight, Obj.DataW);
      if (Obj.DataW) {
        char szLevel[8];
        int nLen;
        ...
        if (Obj.DataH) {
          g_pRepresentShell->OutputText(m_SkillTextParam.nFont, szLevel, nLen,
                                        nLeft, nTop, 0xFF5454FF, 0);
        } else {
          g_pRepresentShell->OutputText(m_SkillTextParam.nFont, szLevel, nLen,
                                        nLeft, nTop, m_SkillTextParam.Color, 0);
        }
      }
    }
  }
...
void KUiFightSkill::UpdateData() {
  int nRemainSkillPoint =
      g_pCoreShell->GetGameData(GDI_FIGHT_SKILL_POINT, 0, 0);
...
KUiLiveSkill::KUiLiveSkill() { m_nRemainSkillPoint = 0; }
...
int KUiLiveSkill::WndProc(unsigned int uMsg, unsigned int uParam, int nParam) {
  if (uMsg == WND_N_LEFT_CLICK_ITEM && uParam && m_nRemainSkillPoint) {
    ...
    if (pObj->uGenre != CGOG_NOTHING) {
      m_nRemainSkillPoint--; // 使用技能点数
      g_pCoreShell->OperationRequest(GOI_TONE_UP_SKILL, CGOG_SKILL_LIVE,
                                     pObj->uId);
    }
    return 0;
  }
...
void KUiLiveSkill::UpdateSkill(KUiSkillData *pSkill, int nIndex) {
  if (pSkill && nIndex >= 0 && nIndex < LIVE_SKILL_COUNT) {
    m_LiveSkill[nIndex].HoldObject(pSkill->uGenre, pSkill->uId, pSkill->nLevel,
                                   0);
  }
}
...
void KUiLiveSkill::UpdateData() {
  ...
  for (int i = 0; i < LIVE_SKILL_COUNT; i++)
    m_LiveSkill[i].HoldObject(Skills[i].uGenre, Skills[i].uId, Skills[i].nLevel,
                              0);
}
...
//	功能：更新生活技能公共数据
//--------------------------------------------------------------------------
void KUiSkills::UpdateLiveBaseData() { m_LiveSkillPad.UpdateBaseData(); }

// 窗口函数
int KUiSkills::WndProc(unsigned int uMsg, unsigned int uParam, int nParam) {
  if (uMsg == WND_N_BUTTON_CLICK &&
      (KWndWindow *)uParam == (KWndWindow *)&m_Close) {
    Hide(); // 关闭装备框
    return 0;
  }
