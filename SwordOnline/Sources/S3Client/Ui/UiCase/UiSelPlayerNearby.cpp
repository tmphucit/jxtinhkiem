...
// 窗口函数
int KUiSelPlayerNearby::WndProc(unsigned int uMsg, unsigned int uParam,
                                int nParam) {
  int nRet = 0;
  switch (uMsg) {
...
void KUiSelPlayerNearby::UpdateData() {
  ...
  if (nCount) {
    ...
    if (m_pDataList) {
      KUiPlayerItem Item;
      m_nPlayerCount = g_pCoreShell->GetGameData(
          GDI_NEARBY_PLAYER_LIST, (unsigned int)m_pDataList, nCount);
      _ASSERT(m_nPlayerCount == nCount);
      m_PlayerList.SetContent((unsigned char *)m_pDataList, m_nPlayerCount,
                              sizeof(KUiPlayerItem),
                              (char *)(&Item.Name) - (char *)&Item);
    }
  }
}
