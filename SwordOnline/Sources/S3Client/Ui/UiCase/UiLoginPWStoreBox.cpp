...
//	¹¦ÄÜ£º´°¿ÚÏûÏ¢º¯Êý
//--------------------------------------------------------------------------
int KUiLoginPWStoreBox::WndProc(unsigned int uMsg, unsigned int uParam, int nParam)
{
	int nRet = 0;
	switch(uMsg)
	{
...
{ 
	...
	if (g_pCoreShell)
	{
		 ...
		 else
		 {
			...
			{
		g_pCoreShell->OperationRequest(GOI_LOGIN_PW_STORE_BOX,(unsigned int)(&Buff), 0);
			}
...
void KUiLoginPWStoreBox::OnCancel()
{
	CloseWindow(false);
}
