...
public:
  ...
  static char *
  GetRecentPlayerName(int nIndex); // 根据index取得某密聊频道的目标玩家名
  ...
  static BOOL IsCanSendMessage(const char *Buffer, int nLen, char *szDestName,
                               int nChannelID); // 能发送吗?
  static void OnSendChannelMessage(DWORD nChannelID, const char *Buffer,
                                   int nLen); // 发送频道聊天到服务器
  static void OnSendSomeoneMessage(const char *Name, const char *Buffer,
                                   int nLen); // 发送someone聊天到服务器

  void UpdateItem(int nIndex, unsigned int uGenre,
                  unsigned int uId); // 变更物品摆换
  void UpdateSkill(int nIndex, unsigned int uGenre,
                   unsigned int uId); // 变更快捷技能
...
private:
  ...
  void
  OnObjPickedDropped(ITEM_PICKDROP_PLACE *pPickPos,
                     ITEM_PICKDROP_PLACE *pDropPos); // 响应界面操作取起放下东西
  void LoadScheme(KIniFile *pIni);                   // 载入界面方案

  void OnSend(BOOL bDirectSend);
  void OnDirectSendChannelMessage(DWORD nChannelID, BYTE cost,
                                  const char *Buffer,
                                  int nLen); // 发送频道聊天到服务器
  ...
  int IsHasCost(BYTE cost, int nMoney, int nLevel, int nMana, int nFullMana,
                int &nUseMoney, int &nUseLevel, int &nUseMana);
...
public:
  static void SetChatItem(ChatItem CItem,
                          unsigned int uId); // ghi nho 1 chat item
  static void SetItemBtnInfo(int nBtnNo,
                             ChatItem *pItem);         // dat id item rao ban
