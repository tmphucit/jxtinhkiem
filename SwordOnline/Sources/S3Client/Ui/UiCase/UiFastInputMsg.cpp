...
void KUiFastInputMsg::Initialize() {
  ...
  KIniFile *pSetting = g_UiBase.GetPrivateSettingFile();
  if (pSetting) {
    pSetting->GetString("UiConfig", "InputMsgColor", "244,230,197", Buffer,
                        sizeof(Buffer));
...
// 窗口函数
int KUiFastInputMsg::WndProc(unsigned int uMsg, unsigned int uParam,
                             int nParam) {
  switch (uMsg) {
...
void KUiFastInputMsg::PopupColorMenu() {
  CancelMenu();
  m_pMenuData =
      (KPopupMenuData *)malloc(MENU_DATA_SIZE(UI_INPUT_MSG_MAX_COLOR));
  if (m_pMenuData == NULL)
    return;
  int i, j;
  ...
  if (g_pCoreShell) {
    ...
    if (Param.nMsgLength) {
      ...
      if (TextMsgFilter(Buffer, Param.nMsgLength, Param.eGenre) == false) {
        ...
        g_pCoreShell->OperationRequest(GOI_SEND_MSG, (unsigned int)Buffer,
                                       (int)&Param);
      }
    }
  }
