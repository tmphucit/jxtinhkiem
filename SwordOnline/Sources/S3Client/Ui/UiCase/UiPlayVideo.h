...
typedef HKLVIDEO (*KLVideoOpenFn)(const char PTR4 *name, u32 flags,
                                  void PTR4 *hWnd);
typedef void (*KLVideoCloseFn)(HKLVIDEO bnk);
typedef u32 (*KLVideoWaitFn)(HKLVIDEO bnk);
typedef void (*KLVideoToBufferFn)(HKLVIDEO bnk, void PTR4 *buf, u32 left,
                                  u32 top, u32 Pitch, u32 destheight,
                                  u32 Flags);
...
public:
  ...
  static void LoadScheme(const char *pszSettingFile,
                         const char *pszSection); // 载入界面方案

  void Setting(bool bProcessInput, bool bCloseWhenOver, KWndWindow *pCaller,
