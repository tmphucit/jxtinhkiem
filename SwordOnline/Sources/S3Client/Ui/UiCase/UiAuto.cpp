...
#define SCHEME_INI "auto800.ini"
// #define	SCHEME_INI		"auto1024.ini"
...
// 载入界面方案
void KUiAuto::LoadScheme(const char *pScheme) {
  char Buff[128];
  KIniFile Ini;
  if (m_pSelf) {
...
// 重新初始化界面
void KUiAuto::DefaultScheme(const char *pScheme) {
  char Buff[128];
  KIniFile Ini;
  if (m_pSelf) {
    sprintf(Buff, "%s\\" SCHEME_INI, pScheme);
...
void KUiAuto::Breathe() { UpdateData(); }
