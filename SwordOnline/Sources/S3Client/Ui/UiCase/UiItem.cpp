...
void KUiItem::OnClickItem(KUiDraggedObject *pItem, bool bDoImmed) {
  ...
  if (bDoImmed == false) {
    ...
    if (eStatus == UIS_S_TRADE_SALE || eStatus == UIS_S_TRADE_NPC) {
      g_pCoreShell->OperationRequest(GOI_TRADE_NPC_SELL, (unsigned int)(&Obj),
                                     0);
    } else if (g_UiBase.IsOperationEnable(UIS_O_USE_ITEM)) {
      g_pCoreShell->OperationRequest(GOI_USE_ITEM, (unsigned int)(&Obj),
                                     UOC_ITEM_TAKE_WITH);
    }
  }
}
...
void KUiItem::OnRepairItem(KUiDraggedObject *pItem) {
  ...
  KUiItemBuySelInfo Price = {0};
  if (g_pCoreShell->GetGameData(GDI_REPAIR_ITEM_PRICE, (unsigned int)(&Obj),
                                (int)(&Price))) {
  ...
  case WND_N_BUTTON_CLICK:
    if (uParam == (unsigned int)(KWndWindow *)&m_CloseBtn)
      ...
      if (KUiStoreBox::GetIfVisible())
        KUiGetMoney::OpenWindow(0, m_nMoney, this, UIITEM_WAIT_GETMONEY,
                                &m_Money);
    }

    else if (uParam == (unsigned int)(KWndWindow *)&m_MarkPrice &&
             !m_MakeStall.IsButtonChecked()) {
      if (g_UiBase.GetStatus() == UIS_S_PRICE_ITEM) {
        g_UiBase.SetStatus(UIS_S_IDLE);
    ...
    else if (uParam == (unsigned int)(KWndWindow *)&m_MakeAdv &&
             !m_MakeStall.IsButtonChecked()) {
...
void KUiItem::OnItemPickDrop(ITEM_PICKDROP_PLACE *pPickPos,
  ...
  if (pPickPos) {
    ...
    if (eStatus == UIS_S_TRADE_SALE) {
      g_pCoreShell->OperationRequest(GOI_TRADE_NPC_SELL, (unsigned int)(&Pick),
                                     0);
      return;
