...
KUiSysMsgCentre::~KUiSysMsgCentre() { Clear(); }
...
void KUiSysMsgCentre::Initialize() {
  ...
  if (pSetting) {
    pSetting->GetInteger("Main", "SysMsgMoveInterval", POPUP_MOVE_INTERVAL_DEF,
                         (int *)(&m_uMoveInterval));
    pSetting->GetInteger("Main", "SysMsgDisappearInterval",
                         MSG_DISAPPEAR_INTERVAL_DEF,
...
void KUiSysMsgCentre::LoadScheme(KIniFile *pIni) {
  ...
  for (int i = 0; i < MAX_SYS_MSG_TYPE; i++) {
    ...
    pIni->GetString(szBuf, "Image", "", m_MsgIcon[i].szImage,
                    sizeof(m_MsgIcon[i].szImage));
    int nValue;
    pIni->GetInteger(szBuf, "Frame", 0, &nValue);
    m_MsgIcon[i].nFrame = m_nMsgIconFrame[i] = nValue;
  }

  pIni->GetString("Main", "NormalMsgColor", "255,255,255", szBuf,
                  sizeof(szBuf));
...
//	¹¦ÄÜ£º´°¿Úº¯Êý
//--------------------------------------------------------------------------
int KUiSysMsgCentre::WndProc(unsigned int uMsg, unsigned int uParam,
                             int nParam) {
  int i, nRet = 0;
  switch (uMsg) {
...
void KUiSysMsgCentre::OnConfirmOperFinished(unsigned int uParam,
                                            int nSelAction) {
  ...
  case SMCT_UI_TEAM_INVITE: // ´ðÓ¦»òTõ chèi¼ÓÈë¶ÓÎéµÄÑûÇë
    if (m_pHandlingMsg) {
      ...
      g_pCoreShell->TeamOperation(
          TEAM_OI_INVITE_RESPONSE, (unsigned int)pPlayer,
          (nSelAction == 0)); // nSelAction=0£º´ðÓ¦, nSelAction=1£ºTõ chèi
    }
    break;
  case SMCT_UI_TEAM_APPLY: // ´ðÓ¦»òTõ chèi¼ÓÈë¶ÓÎéµÄÉêÇë,
    if (m_pHandlingMsg) {
      ...
      pPlayer = (KUiPlayerItem *)(&m_pHandlingMsg[1]);
      g_pCoreShell->TeamOperation(
          TEAM_OI_APPLY_RESPONSE, (unsigned int)pPlayer,
          (nSelAction == 0)); // nSelAction=0£º´ðÓ¦, nSelAction=1£ºTõ chèi
    }
    break;
  case SMCT_UI_FRIEND_INVITE: // Åú×¼»òTõ chèi±ðÈË¼Ó×Ô¼ºÎªºÃÓÑ
    if (m_pHandlingMsg) {
      ...
      pPlayer = (KUiPlayerItem *)(&m_pHandlingMsg[1]);
      g_pCoreShell->OperationRequest(
          GOI_CHAT_FRIEND_INVITE, (unsigned int)pPlayer,
          (nSelAction == 0)); // nSelAction=0£º´ðÓ¦, nSelAction=1£ºTõ chèi
      if (nSelAction == 0) {
        int nUnit =
            KUiChatCentre::FindUnitIndexByRoleNameAtServerUnit(pPlayer->Name);
        if (nUnit >= 0) {
  ...
  case SMCT_UI_TRADE:
    if (m_pHandlingMsg) {
      ...
      pPlayer = (KUiPlayerItem *)(&m_pHandlingMsg[1]);
      g_pCoreShell->OperationRequest(
          GOI_TRADE_INVITE_RESPONSE, (unsigned int)pPlayer,
          (nSelAction == 0)); // nSelAction=0£º´ðÓ¦, nSelAction=1£ºTõ chèi
    }
    break;
  case SMCT_UI_TONG_JOIN_APPLY:
    if (m_pHandlingMsg) {
      //_ASSERT(m_pHandlingMsg->byParamSize >= sizeof(KUiPlayerItem))
      pPlayer = (KUiPlayerItem *)(&m_pHandlingMsg[1]);
      g_pCoreShell->TongOperation(
          GTOI_TONG_JOIN_REPLY, (unsigned int)pPlayer,
          (nSelAction == 0)); // nSelAction=0£º´ðÓ¦, nSelAction=1£ºTõ chèi
    }
    break;
  }
  if (m_pHandlingMsg) {
    if (m_MsgHeap[RECYCLE_MSG_HEAP_INDEX].nNumValid >=
            MAX_MSG_IN_RECYCLE_HEAP ||
  ...
  for (i = 0; i < MAX_SYS_MSG_TYPE; i++) {
    ...
    for (j = m_MsgHeap[i].nNumValid - 1; j >= 0; j--) {
      ...
      if (pMsg->byConfirmType == SMCT_CLICK &&
          IR_IsTimePassed(m_uDisappearInterval * pMsg->byPriority +
                              m_uDisappearInterval,
                          pMsg->uReservedForUi)) {
...
bool KUiSysMsgCentre::AMessageArrival(KSystemMessage *pMsg, void *pParam) {
  ...
  if (pMsg->eType == SMT_NORMAL) {
    KUiMsgCentrePad::SystemMessageArrival(
        pMsg->szMessage, strlen(pMsg->szMessage)); // ¼ÓÈëÒ»ÌõÏûÏ¢
    return true;
  }
...
void KUiSysMsgCentre::SetPopupMsgDest() {
  if (m_MsgHeap[POPUPING_MSG_HEAP_INDEX].nNumValid) {
    unsigned char ePopupType =
        m_MsgHeap[POPUPING_MSG_HEAP_INDEX].pMsgList[0]->eType;
...
void KUiSysMsgCentre::DeleteMsgInHeap(int nHeapIndex, int nMsgIndex,
                                      bool bImmedDel, bool bTobeConfirm) {
  int i;
  ...
  if (nMsgIndex >= 0 && nMsgIndex < pHeap->nNumValid) {
    ...
    } else if (m_MsgHeap[RECYCLE_MSG_HEAP_INDEX].nNumValid >=
                   MAX_MSG_IN_RECYCLE_HEAP ||
               !AddAMsgToHeap(pHeap->pMsgList[nMsgIndex],
                              RECYCLE_MSG_HEAP_INDEX, false)) {
...
bool KUiSysMsgCentre::AddAMsgToHeap(KSystemMessage *pMsg, int nHeapIndex,
                                    bool bSort) {
  //_ASSERT(pMsg);
  //_ASSERT(nHeapIndex >= 0 && nHeapIndex <= RECYCLE_MSG_HEAP_INDEX);
  SYS_MSG_HEAP *pHeap = &m_MsgHeap[nHeapIndex];
  if (pHeap->nNumValid == pHeap->nListSpace) {
    KSystemMessage **pNewList = (KSystemMessage **)realloc(
        pHeap->pMsgList,
        sizeof(KSystemMessage *) * (pHeap->nListSpace + MSG_LIST_EXPAND_STEP));
    if (pNewList) {
      memset(&pNewList[pHeap->nNumValid], 0,
             sizeof(KSystemMessage *) * MSG_LIST_EXPAND_STEP);
...
void KUiSysMsgCentre::PaintWindow() {
  ...
  if (m_MsgHeap[POPUPING_MSG_HEAP_INDEX].nNumValid) {
    ...
    if (m_MsgHeap[POPUPING_MSG_HEAP_INDEX].pMsgList[0]->byConfirmType !=
        SMCT_CLICK) {
