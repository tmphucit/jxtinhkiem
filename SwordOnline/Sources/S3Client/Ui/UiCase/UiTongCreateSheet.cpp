...
KUiTongCreateSheet::KUiTongCreateSheet() { m_nSelectSide = -1; }
...
void KUiTongCreateSheet::LoadScheme(const char *pScheme) {
  if (ms_pSelf) {
    ...
    if (Ini.Load(Buff)) {
      ...
      Ini.GetString("ErrorBox", "WithoutName", "", ms_pSelf->m_szNameNullString,
                    sizeof(ms_pSelf->m_szNameNullString));
      Ini.GetString("ErrorBox", "WithoutFaction", "",
                    ms_pSelf->m_szAlignmentNullString,
                    sizeof(ms_pSelf->m_szAlignmentNullString));
    }
    /*--------------------*/
  }
  /*--------------------*/
 ...
 * 功能：窗口函数
 **********************************************************************/
int KUiTongCreateSheet::WndProc(unsigned int uMsg, unsigned int uParam,
                                int nParam) {
  switch (uMsg) {
...
void KUiTongCreateSheet::OnDone() {
  ...
  if (Buff[0] && g_pCoreShell) {
    if (m_nSelectSide != -1) {
      g_pCoreShell->TongOperation(GTOI_TONG_CREATE, (unsigned int)Buff,
                                  (int)m_nSelectSide);
      CloseWindow();
    } else {
      m_TextError.SetText(m_szAlignmentNullString,
                          strlen(m_szAlignmentNullString));
    }
  } else {
