...
int KUiUpdataPatch::Initialize() {
  ...
  m_fnSwordUpdateGetResult = (fnSwordUpdateGetResult)GetProcAddress(
      m_PatchShell, PATCHSHELL_FN_GETRESULT);
...
void KUiUpdataPatch::Breathe() {
  if (m_fnSwordUpdateGetResult) {
    ...
    int nRet =
        m_fnSwordUpdateGetResult(sInformation, &nMsgLen, &nProgressPercent, 0);
    // 添加新的消息
    if (nMsgLen > 0)
...
//	功能：窗口函数
//--------------------------------------------------------------------------
int KUiUpdataPatch::WndProc(unsigned int uMsg, unsigned int uParam,
                            int nParam) {
  switch (uMsg) {
...
void KUiUpdataPatch::OnFinished(bool bRestart) {
  if (bRestart && m_PatchShell) {
    ...
    fnRegApp = (fnRegisterAppLaunchedAfterUpdate)GetProcAddress(
        m_PatchShell, PATCHSHELL_FN_REGLAUNCHEAPPNAME);
    if (fnRegApp != NULL) {
      char sAppName[255];
      ...
      SetCloseParam("升级完毕，点确定按钮，游戏将退出。请手工重启游戏。", true,
                    true);
  } else
