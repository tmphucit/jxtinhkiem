...
KUiTeamApply::~KUiTeamApply() { Clear(); }
...
void KUiTeamApply::UpdateData(KUiTeamItem *pList, int nCount) {
  ...
  if (pList && nCount) {
    ...
    if (m_pSelf->m_pDataList) {
      ...
      m_pSelf->m_TeamList.SetContent(
          (unsigned char *)m_pSelf->m_pDataList, nCount, sizeof(KUiPlayerItem),
          (char *)&(m_pSelf->m_pDataList->Leader.Name) -
              (char *)m_pSelf->m_pDataList);
