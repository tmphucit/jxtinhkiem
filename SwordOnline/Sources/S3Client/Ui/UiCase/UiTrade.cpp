...
//--------------------------------------------------------------------------
KUiTrade *KUiTrade::GetIfVisible() { return m_pSelf; }
...
void KUiTrade::Initialize() {
  ...
  m_OtherMoney.SetText(ngan_luong);
  m_TakewithMoney.SetText(ngan_luong);
...
void KUiTrade::LoadScheme(KIniFile *pIni) {
  ...
  char szColor[16];
  pIni->GetString("InfoText", "WaitTradeMsgColor", "255,255,255", szColor,
                  sizeof(szColor));
  m_uWaitTradeMsgColor = (::GetColor(szColor) & 0xffffff);
  pIni->GetString("InfoText", "LockMsgColor", "255,255,255", szColor,
                  sizeof(szColor));
  m_uOtherLockMsgColor = (::GetColor(szColor) & 0xffffff);
  pIni->GetString("InfoText", "UnlockMsgColor", "255,255,255", szColor,
                  sizeof(szColor));
...
void KUiTrade::UpdateTakewithItems() {
  ...
  int van = m_nTakewithMoney / 10000;
  int luong = m_nTakewithMoney % 10000;
  char ngan_luong[32] = "";
  if (van > 0)
    sprintf(ngan_luong, "%d %s ", van, "v¹n");
  ...
  if (pObjs = (KUiObjAtRegion *)malloc(sizeof(KUiObjAtRegion) * nCount)) {
    g_pCoreShell->GetGameData(GDI_ITEM_TAKEN_WITH, (unsigned int)pObjs,
                              nCount); // µ¥Ïß³ÌÖ´ÐÐ£¬nCountÖµ²»±ä
...
void KUiTrade::OnOppositeChangedItem(KUiObjAtRegion *pObj, int bAdd) {
  _ASSERT(pObj);
  if (pObj->Obj.uGenre == CGOG_MONEY) {
    int van = pObj->Obj.uId / 10000;
    int luong = pObj->Obj.uId % 10000;
    char ngan_luong[32] = "";
    if (van > 0)
      sprintf(ngan_luong, "%d %s ", van, "v¹n");
...
void KUiTrade::UpdateOperData() {
  ...
  if (g_pCoreShell->GetGameData(GDI_TRADE_OPER_DATA, UTOD_IS_TRADING, 0)) {
    ...
    int bOtherLock =
        g_pCoreShell->GetGameData(GDI_TRADE_OPER_DATA, UTOD_IS_OTHER_LOCKED, 0);
    m_Trade.CheckButton(false);
    m_Trade.Enable(bLock && bOtherLock);
    m_Info.SetText(bOtherLock ? STR_OTHER_OK : STR_OTHER_NOT_OK, -1);
    m_Info.SetTextColor(bOtherLock ? m_uOtherLockMsgColor
                                   : m_uOtherNotLockMsgColor);
  }
...
void KUiTrade::OnItemPickDrop(ITEM_PICKDROP_PLACE *pPickPos,
...
void KUiTrade::OnAdjustMoney(bool bAdd) {
  ...
  int van1 = m_nTakewithMoney / 10000;
  int luong1 = m_nTakewithMoney % 10000;
  char ngan_luong1[32] = "";
  if (van1 > 0)
    sprintf(ngan_luong1, "%d %s ", van1, "v¹n");

  sprintf(ngan_luong1, "%s%d %s", ngan_luong1, luong1, "l­îng");

  m_TakewithMoney.SetText(ngan_luong1);
}
...
void KUiTrade::OnAdjustedMoney() {
  ...
  if (g_pCoreShell) {
    ...
    g_pCoreShell->OperationRequest(GOI_TRADE_DESIRE_ITEM, (unsigned int)&Obj,
                                   0);
  }
}
...
// -------------------------------------------------------------------------
void KUiTrade::UpdateTradeEnd(int bTraded) { CloseWindow(); }
