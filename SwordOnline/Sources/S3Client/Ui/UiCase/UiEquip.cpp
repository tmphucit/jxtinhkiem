...
void KUiHorsePage::UpdateEquip(KUiObjAtRegion *pEquip, int bAdd) {
  if (pEquip && pEquip->Region.v == UIEP_HORSE) {
    ...
    if (bAdd && pEquip->Obj.uGenre != CGOG_NOTHING) {
      ...
      char Desc[1024];
      int nLen = 0;
      g_pCoreShell->GetGameData(GDI_GAME_OBJ_DESC, (unsigned int)pEquip,
                                (int)&Desc);
      if (Desc[0])
...
int KUiHorsePage::WndProc(unsigned int uMsg, unsigned int uParam, int nParam) {
  ...
  } else if (uMsg == WND_N_BUTTON_CLICK &&
             (KWndWindow *)uParam == (KWndWindow *)&m_UpDownHorseBtn) {
...
//	功能：响应界面操作引起装备的改变
//--------------------------------------------------------------------------
void KUiHorsePage::OnEquiptChanged(ITEM_PICKDROP_PLACE *pPickPos,
...
//	功能：响应界面操作引起装备的改变
//--------------------------------------------------------------------------
void KUiEquipPage::OnEquiptChanged(ITEM_PICKDROP_PLACE *pPickPos,
                                   ITEM_PICKDROP_PLACE *pDropPos) {
  KUiObjAtContRegion Drop, Pick;
  KUiDraggedObject Obj;
