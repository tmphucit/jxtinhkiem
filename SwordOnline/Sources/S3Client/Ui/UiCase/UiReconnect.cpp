...
bool KReconnectWnd::IsReconnecttingGoingOn() { return (m_pSelf != NULL); }
...
void KReconnectWnd::Breathe() {
  ...
  char szInfo[256];
  int nInfoLen;
  if (m_bWaitToReconnect) { //====µÈ´ı½øĞĞÏÂ´Î×Ô¶¯ÖØÁ¬====
    ...
    if (uRemain = IR_GetRemainTime(m_uToWaitTime, m_uWaitStartTime)) {
      if (m_nHideTimes == 0) {
        ...
        g_UiInformation.Show(szInfo, RECONNECT_QUIT_BTN_LABEL,
                             RECONNECT_START_AT_ONCE, this, 0, nInfoLen);
      }
    } else { //====¿ªÊ¼ÏÂ´Î×Ô¶¯ÖØÁ¬====
      StartReconnect();
    }
  ...
  if (m_bStop) {
    ...
    nInfoLen =
        sprintf(szInfo, MSG_RECONNECTING, m_nReconnectTimes + 1, pReconnectMsg);
    nInfoLen = TEncodeText(szInfo, nInfoLen);
    g_UiInformation.Show(szInfo, RECONNECT_QUIT_BTN_LABEL, NULL, this, 0,
                         nInfoLen);
  }
}

int KReconnectWnd::WndProc(unsigned int uMsg, unsigned int uParam,
                           int nParam) // ´°¿Úº¯Êı
{
