...
void KUiConnectInfo::LoadScheme(const char *pScheme) {
  ...
  if (Ini.Load(Buff)) {
    ...
    Ini.GetInteger("Message", "ColorChangeInterval", 0,
                   (int *)&m_uMsgColorChangeInterval);
    ...
    Ini.GetString("RuningImgBg", "LoginBg", "", m_szLoginBg,
                  sizeof(m_szLoginBg));
  }
}
...
int KUiConnectInfo::WndProc(unsigned int uMsg, unsigned int uParam,
                            int nParam) {
  switch (uMsg) {
  case WND_N_BUTTON_CLICK:
    ...
    break;
  case WM_KEYDOWN:
    if (m_ConfirmBtn.GetParent() &&
        (uParam == VK_RETURN || uParam == VK_ESCAPE))
      OnClickConfirmBtn();
...
bool KUiConnectInfo::OnDelRole() {
  ...
  if (m_InputPwdWnd.GetText(szSupperPassword, sizeof(szSupperPassword),
                            false)) {
  ...
  if (m_nMsgLen && g_pRepresentShell) {
    ...
    if (m_nColor2CharacterStartIndex) {
      g_pRepresentShell->OutputText(
          m_nFont, m_szMessage, m_nColor2CharacterStartIndex, nX, nY,
          m_uMsgColor, 0, TEXT_IN_SINGLE_PLANE_COORD, m_uMsgBorderColor);
      nX += m_nColor2CharacterStartIndex * m_nFont / 2;
    }
    g_pRepresentShell->OutputText(
        m_nFont, &m_szMessage[m_nColor2CharacterStartIndex],
        m_nColor2CharacterEndIndex - m_nColor2CharacterStartIndex, nX, nY,
        m_uMsgColor2, 0, TEXT_IN_SINGLE_PLANE_COORD, m_uMsgBorderColor2);
    nX += (m_nColor2CharacterEndIndex - m_nColor2CharacterStartIndex) *
          m_nFont / 2;
    if (m_nColor2CharacterEndIndex < m_nMsgLen) {
      g_pRepresentShell->OutputText(
          m_nFont, &m_szMessage[m_nColor2CharacterEndIndex],
          m_nMsgLen - m_nColor2CharacterEndIndex, nX, nY, m_uMsgColor, 0,
          TEXT_IN_SINGLE_PLANE_COORD, m_uMsgBorderColor);
    }
  }
}