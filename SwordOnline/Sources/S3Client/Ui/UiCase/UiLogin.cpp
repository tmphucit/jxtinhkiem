...
//	功能：构造函数
//--------------------------------------------------------------------------
KUiLogin::KUiLogin() { m_szLoginBg[0] = 0; }
...
void KUiLogin::CloseWindow(bool bDestroy) {
  if (m_pSelf) {
    g_LoginLogic.SetRememberAccountFlag(
        m_pSelf->m_RememberAccount.IsButtonChecked() != 0);
    if (bDestroy) {
      m_pSelf->Destroy();
      m_pSelf = NULL;
    } else {
...
int KUiLogin::GetInputInfo(char *pszAccount, char *pszPassword) {
  ...
  /*	int nLen = m_Account.GetText(m_szAccount, 32, false);
          ...
          KUiConnectInfo::OpenWindow(CI_MI_ACCOUNT_PWD_ERROR,
     CI_AE_BACK_TO_LOGIN); CloseWindow(false); return false;*/
}
  ...
  if (pszPassword) {
    ...
    if (nLen >= LOGIN_PASSWORD_MIN_LEN && nLen <= LOGIN_PASSWORD_MAX_LEN) {
      for (int i = 0; i < nLen; i++) {
        ...
        if ((cCode < 0x30 || cCode > 0x39) && (cCode < 0x41 || cCode > 0x5a) &&
            (cCode < 0x61 || cCode > 0x7a)) {
          break;
        }
        if (i == nLen)
