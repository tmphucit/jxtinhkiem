	...
	if(m_pCurrentIndexNode && (m_pCurrentIndexNode->bValueAppened || m_pCurrentIndexNode->bSortFlag) )
	{
    	...
    	for(i = 0;i < nCount;i++)
	    {
    		...
    		if(m_pCurrentIndexNode->bValueAppened)
			{
				...
				if (m_pCurrentIndexNode->usIndexId == 1 || (m_pCurrentIndexNode->usIndexId > 3 && m_pCurrentIndexNode->usIndexId <= 14  ))
				{

				if (pNode->HeightValua)
    			sprintf(szBuf, "%d Chuy觧 %d", pNode->HeightValua,pNode->nValueAppend);
				else
                 sprintf(szBuf, "%d",pNode->nValueAppend);
			

				}
				else if (m_pCurrentIndexNode->usIndexId == 2 || m_pCurrentIndexNode->usIndexId > 15)
				{
