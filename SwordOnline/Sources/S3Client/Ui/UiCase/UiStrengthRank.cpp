...
#define INDEX_AND_RANK_DATA_INI "UiStrengthRank.ini"
#define INDEX_AND_RANK_MAPPING "\\Ui\\RankID.ini"
 ...
 *功能：响应窗口消息的窗口函数
 *****************************************************************************/
int KUiStrengthRank::WndProc(unsigned int uMsg, unsigned int uParam,
                             int nParam) {
  int nRet = 0;
  switch (uMsg) {
...
void KUiStrengthRank::LoadScheme(const char *pScheme) {
  if (m_pSelf) {
    ...
    if (Ini.Load(Buff)) {
      ...
      Ini.GetString("Main", "UpPic", "", m_pSelf->m_szUpPicPath,
                    sizeof(m_pSelf->m_szUpPicPath));
      Ini.GetString("Main", "DownPic", "", m_pSelf->m_szDownPicPath,
                    sizeof(m_pSelf->m_szDownPicPath));
      Ini.GetString("Main", "SteadyPic", "", m_pSelf->m_szSteadyPicPath,
                    sizeof(m_pSelf->m_szSteadyPicPath));
...
void KUiStrengthRank::SendGetRankToCore(int nID) {
  if (g_pCoreShell && m_pLastGettimes &&
      IR_GetCurrentTime() - m_pLastGettimes[m_nLastIndex] > GET_RANK_TIME_OUT) {
    m_pLastGettimes[m_nLastIndex] = IR_GetCurrentTime();
    g_pCoreShell->OperationRequest(GOI_QUERY_RANK_INFORMATION,
                                   (unsigned int)nID, 0);
  }
}
...
void KUiStrengthRank::ConstructRankView() {
  ...
  if (m_pCurrentIndexNode &&
      (m_pCurrentIndexNode->bValueAppened || m_pCurrentIndexNode->bSortFlag)) {
    char szBuf[32];
    for (i = 0; i < nCount; i++) {
      ...
      if (m_pCurrentIndexNode->bValueAppened) {
        ...
        if (m_pCurrentIndexNode->usIndexId == 1 ||
            (m_pCurrentIndexNode->usIndexId > 3 &&
             m_pCurrentIndexNode->usIndexId <= 14)) {

          if (pNode->HeightValua)
            sprintf(szBuf, "%d Chuy觧 %d", pNode->HeightValua,
                    pNode->nValueAppend);
          else
            sprintf(szBuf, "%d", pNode->nValueAppend);

        } else if (m_pCurrentIndexNode->usIndexId == 2 ||
                   m_pCurrentIndexNode->usIndexId > 15) {
          int nvan = pNode->nValueAppend / 10000;
          int nluong = pNode->nValueAppend % 10000;

          if (nvan)
            sprintf(szBuf, "%d V筺 %d", nvan, nluong);
          else
            sprintf(szBuf, "%d", nluong);
        } else
          sprintf(szBuf, "%d", pNode->nValueAppend);
 ...
 *功能：新的索引消息来了
 *****************************************************************************/
void KUiStrengthRank::NewIndexArrive(unsigned int uParam,
                                     struct KRankIndex *pIndex) {
  if (m_pSelf) {
 ...
 *功能：新的排名信息来了
 *****************************************************************************/
void KUiStrengthRank::NewRankArrive(unsigned int uParam,
                                    KRankMessage *pRankData) {
...
void KUiStrengthRank::AddIndex(unsigned int uParam, struct KRankIndex *pIndex) {
  ...
  for (i = 0; i < nCount; i++) {
    ...
    m_IndexIDIni.GetString("Index", szKey, "", Node.szUnit,
                           sizeof(Node.szUnit));
...
void KUiStrengthRank::AddRank(unsigned int uParam, KRankMessage *pRankData) {
  ...
  if (m_pSelf && m_pSelf->m_pCurrentIndexNode &&
      nID == m_pSelf->m_pCurrentIndexNode->usIndexId) {
    for (i = 0; i < nCount; i++) {
      if (pIncome->szMsg[0]) {
 ...
 *功能：把新收到的索引资料存到临时文件
 *****************************************************************************/
void KUiStrengthRank::StoreNewDataToTempFile(unsigned int uParam,
                                             KRankIndex *pInData) {
  KIniFile Ini;
...
int TDrawOutString(char *szBuf, int nBufLen, int nLineLen, int nFontsize) {
  ...
  if (nTabPos > 0) {
    ...
    if (nOldLineLen < nLineLen) {
      memmove(szBuf + nTabPos + nLineLen - nOldLineLen + 1, szBuf + nTabPos + 1,
              nBufLen - nTabPos - 1);
