...
int KUiGameSpace::WndProc(unsigned int uMsg, unsigned int uParam, int nParam) {
  ...
  case WM_LBUTTONDOWN: {
    if (uParam & MK_SHIFT) {
      ...
      int nKind = -1;
      if (g_pCoreShell->FindSelectNPC(LOWORD(nParam), HIWORD(nParam),
                                      relation_all, true, &SelectPlayer,
                                      nKind)) {
        if (nKind == kind_player &&
            g_pCoreShell->CheckOpenShop(SelectPlayer.nIndex)) {
          g_pCoreShell->OpenBuyShop(SelectPlayer.uId);
          bDefault = false;
          break;
        }
      }
  ...
  case WM_RBUTTONDOWN: {
    ...
    if (KShortcutKeyCentre::HandleMouseInput(
            uMsg == WM_LBUTTONDOWN ? VK_LBUTTON : VK_RBUTTON, nModifier,
  ...
  case WM_RBUTTONDBLCLK: {
    ...
    if (KShortcutKeyCentre::HandleMouseInput(
            uMsg == WM_LBUTTONDBLCLK ? VK_LDBUTTON : VK_RDBUTTON, nModifier,
            LOWORD(nParam), HIWORD(nParam)))
      bDefault = false;
  } break;
  ...
  case WM_MOUSEHOVER:
    if (uParam & MK_LBUTTON) {
      ...
      KShortcutKeyCentre::HandleMouseInput(0, VK_RBUTTON, LOWORD(nParam),
                                           HIWORD(nParam));
    } else if (uParam & MK_SHIFT) {

      OnMouseMoveCursorShop(LOWORD(nParam), HIWORD(nParam));
      bDefault = false;
      break;
    }
...
//	¹¦ÄÜ£º»æÖÆÓÎÏ·ÊÀ½ç
//--------------------------------------------------------------------------
void KUiGameSpace::PaintWindow() { g_pCoreShell->DrawGameSpace(); }
...
void KUiGameSpace::OnMouseMoveCursor(int x, int y) {
  ...
  int nKind = -1;
  int nObjectIdx = -1;
  if (g_pCoreShell->FindSelectNPC(x, y, relation_all, true, &SelectPlayer,
                                  nKind)) {
    if (nKind == kind_dialoger)
      Wnd_SwitchCursor(MOUSE_CURSOR_DIALOG);
    else if (g_pCoreShell->GetNPCRelation(SelectPlayer.nIndex) ==
             relation_enemy)
...
void KUiGameSpace::OnMouseMoveCursorShop(int x, int y) {
  if (g_pCoreShell == NULL)
    return;
  ...
  if (g_pCoreShell->FindSelectNPC(x, y, relation_all, true, &SelectPlayer,
    ...
    if (nKind == kind_dialoger)
      Wnd_SwitchCursor(MOUSE_CURSOR_DIALOG);
             ...
             relation_enemy)
      Wnd_SwitchCursor(MOUSE_CURSOR_FIGHT);
    else if (nKind == kind_player &&
             g_pCoreShell->CheckOpenShop(SelectPlayer.nIndex))
      Wnd_SwitchCursor(MOUSE_CURSOR_SHOP_ITEM);
    else
      Wnd_SwitchCursor(MOUSE_CURSOR_NORMAL);
  } else if (g_pCoreShell->FindSelectObject(x, y, true, nObjectIdx, nKind)) {
    if (nKind == Obj_Kind_MapObj)
...
void ProcessPeople(KUiPlayerItem *pDest, int nAction) {
  ...
  case ACTION_TRADE: // ÒªÓÃnIndex
    if (g_pCoreShell && pDest->nData == PLAYER_MENU_STATE_TRADEOPEN &&
        pDest->nIndex != -1)
  ...
  case ACTION_REVENGE: // ÒªÓÃuId
    if (g_pCoreShell && pDest->uId != 0)
      g_pCoreShell->OperationRequest(GOI_REVENGE_SOMEONE, (unsigned int)pDest,
                                     0);
    break;
  case ACTION_MAKEFRIEND: // ÒªÓÃName
    if (g_pCoreShell && !KUiChatCentre::IsMyFriend(pDest->Name))
      g_pCoreShell->OperationRequest(GOI_CHAT_FRIEND_ADD, (unsigned int)pDest,
                                     0);
    break;
  ...
  case ACTION_FOLLOW: // ÒªÓÃnIndex
    if (g_pCoreShell && pDest->nIndex != -1)
      g_pCoreShell->OperationRequest(GOI_FOLLOW_SOMEONE, (unsigned int)pDest,
                                     0);
    break;
  case ACTION_VIEWITEM: // ÒªÓÃuId
    if (g_pCoreShell && pDest->uId != 0)
      g_pCoreShell->OperationRequest(GOI_VIEW_PLAYERITEM,
                                     (unsigned int)pDest->uId, 0);
    break;
...
void ProcessEmote(char *szDest, char *szDestChannel, int nEmoteIndex) {
  if (g_pCoreShell && szDest) {
    ...
    if (nChannelIndex != -1) {
      ...
      if (szDest[0])
        nStringLen =
            g_UiChatPhrase.GetEmote(nEmoteIndex, szBuff, sizeof(szBuff));
      else
        nStringLen =
            g_UiChatPhrase.GetEmote(nEmoteIndex, szBuff, sizeof(szBuff), 1);
      // »ñÈ¡ºÍ×ª»»×Ö´®ÍæÁË

      // ·¢³öÈ¥
      if (KUiPlayerBar::IsCanSendMessage(szBuff, nStringLen, szDest,
                                         nChannelID)) {
        char szBuffConverted[256];
        nStringLen = KUiFaceSelector::ConvertFaceText(szBuffConverted, szBuff,
                                                      nStringLen);
        nStringLen = g_UiChatPhrase.ConvertEmoteString(
            szBuffConverted, nStringLen, MyInfo.Name, szDest);
        if (nChannelIndex >= nChannelCount) {
          if (szDest[0]) {
            KUiPlayerBar::OnSendSomeoneMessage(szDest, szBuffConverted,
                                               nStringLen);
            int nIndex = KUiPlayerBar::AddRecentPlayer(szDest);
            KUiPlayerBar::SetCurrentChannel(nIndex);
          }
        } else {
          KUiPlayerBar::OnSendChannelMessage(nChannelID, szBuffConverted,
                                             nStringLen);
        }
        ///////////////////////////////////////////
      }
...
char g_ActionName[][32] = {
    "Nãi chuyÖn", "H÷u h¶o", "Giao dÞch", "NhËp ®äi", "Tæ ®éi",
    "Theo sau",   "Cõu s¸t", "Tin tøc",   "Sæ ®en",   "Bang héi",
};
...
void PopUpContextPeopleMenu(const KUiPlayerItem &SelectPlayer, int x, int y) {
  ...
  int nbIsRecruit = g_pCoreShell->TongOperation(GTOI_TONG_IS_RECRUIT,
  ...
  for (i = 0; i < nActionDataCount; i++) {
    if ((i == ACTION_JOINTEAM && SelectPlayer.nIndex != -1 &&
         SelectPlayer.nData ==
             PLAYER_MENU_STATE_TEAMOPEN) || //"ÉêÇëÈë¶Ó",
                                            //¶Ô·½Î´´ò¿ª¶ÓÎéÊ±²»ÄÜ¼ÓÈë
        (i == ACTION_TRADE && SelectPlayer.nIndex != -1 &&
         SelectPlayer.nData ==
             PLAYER_MENU_STATE_TRADEOPEN) || //"½»Ò×ÎïÆ·",
                                             //¶Ô·½Î´´ò¿ª½»Ò×Ê±²»ÄÜ¼ÓÈë
        (i == ACTION_MAKEFRIEND &&
         !KUiChatCentre::IsMyFriend(
             (char *)SelectPlayer
                 .Name)) || //"¼ÓÎªºÃÓÑ", ¶Ô·½ÒÑÊÇÎÒµÄºÃÓÑÊ±²»ÄÜÔÙÑûÇë
        (i == ACTION_INVITETEAM && SelectPlayer.uId != 0 &&
         TeamInfo.nCaptainPower >
             0) || //"ÑûÇë¼ÓÈë", ¶ÓÎé²»¿ÉÒÔ¼ÓÈËÁË¾Í²»ÄÜÑûÇë¼ÓÈë
...
void PopUpContextEmoteMenu(char *szDest, int x, int y) {
  ...
  int nEmoteCount =
      g_UiChatPhrase.GetEmoteCount((KUiChatPhrase::EMOTE_TYPE)(1));

  KPopupMenuData *pMenuData =
      (KPopupMenuData *)malloc(MENU_DATA_SIZE(nEmoteCount));
  if (pMenuData == NULL) {
    return;
  }
