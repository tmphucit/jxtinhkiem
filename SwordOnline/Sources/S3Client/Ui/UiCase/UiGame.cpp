...
if (uParam & MK_SHIFT)
{
	...
	int nKind = -1;
	if (g_pCoreShell->FindSelectNPC(LOWORD(nParam), HIWORD(nParam), relation_all, true, &SelectPlayer, nKind))
	{
	if (nKind == kind_player && g_pCoreShell->CheckOpenShop(SelectPlayer.nIndex))
	{
...
void KUiGameSpace::OnMouseMoveCursor(int x, int y)
{
	...
	else if (g_pCoreShell->FindSelectObject(x, y, true, nObjectIdx, nKind))
	{
		if (nKind == Obj_Kind_MapObj)
			Wnd_SwitchCursor(MOUSE_CURSOR_DIALOG);
		else if (nKind == Obj_Kind_Item || nKind == Obj_Kind_Money || nKind == Obj_Kind_Prop)
...
void KUiGameSpace::OnMouseMoveCursorShop(int x, int y)
{
	...
	if (g_pCoreShell->FindSelectNPC(x, y, relation_all, true, &SelectPlayer, nKind))
	{
		if (nKind == kind_dialoger)
			...
			Wnd_SwitchCursor(MOUSE_CURSOR_FIGHT);
		else if (nKind == kind_player && g_pCoreShell->CheckOpenShop(SelectPlayer.nIndex))
