...
// 显示对话内容
void KUiInformation2::SpeakWords(KUiInformationParam *pWordDataList,
                                 int nCount) {
  ...
  if (m_nNumWordData > m_nCurrentWord) {
    memcpy(pNewList, m_pWordDataList,
           sizeof(KUiInformationParam) * (m_nNumWordData - m_nCurrentWord));
    m_nCurrentWord = m_nNumWordData - m_nCurrentWord;
  } else
    m_nCurrentWord = 0;
  memcpy(&pNewList[m_nCurrentWord], pWordDataList,
         sizeof(KUiInformationParam) * nCount);
  ...
  if (IsVisible() == false) {
    KWndWindow *pCaller =
        m_pWordDataList->bNeedConfirmNotify ? ((KWndWindow *)WND_GAMESPACE) : 0;
...
//	功能：显示窗口
//--------------------------------------------------------------------------
void KUiInformation2::Show(const char *pInformation, int nInforLen,
                           const char *pBtnLabel, KWndWindow *pCallerWnd,
                           unsigned int uParam) {
  if (pInformation) {
...
//	功能：窗口函数
//--------------------------------------------------------------------------
int KUiInformation2::WndProc(unsigned int uMsg, unsigned int uParam,
                             int nParam) {
  int nRet = 0;
  switch (uMsg) {
...
KUiInformation2::~KUiInformation2() { Close(); }