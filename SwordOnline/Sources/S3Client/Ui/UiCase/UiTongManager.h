...
#if !defined(                                                                  \
    AFX_UITONGMANAGER_H__13BA213D_11EC_4F24_BF98_F51C3F414D6D__INCLUDED_)
...
public:
  ...
  static void
  CloseWindow(bool bDestory = TRUE); // 关闭窗口，同时可以选则是否删除对象实例
  static void LoadScheme(const char *pScheme); // 载入界面方案

  // 接受新的数据
  static void NewDataArrive(KUiGameObjectWithName *ParamInfo,
                            KTongMemberItem *pIncome);

  // 接受帮会信息
  static void TongInfoArrive(KUiPlayerRelationWithOther *Relation,
                             KTongInfo *pTongInfo);
...
private:
  ...
  int LoadDataHandler(      // 载入资料的处理函数
      KLinkStruct<KPilgarlicItem> *pData, const char *szSection);
...
private:
  KWndLabeledButton // 踢人，任命，传位按钮
      m_BtnDismiss,
      m_BtnAssign, m_BtnDemise;

  KWndLabeledButton m_BtnLeave, m_BtnRecruit; // 离帮，招人按钮

  KWndLabeledButton m_BtnApply, m_BtnCancel; // 申请，取消按钮

  KWndLabeledButton // 长老、队长、成员列表
      m_BtnElderList,
      m_BtnCaptainList, m_BtnMemberList;

  KWndButton m_BtnClose, m_BtnRefresh;

  KWndButton m_BtnDepositMoney, m_BtnTakeMoney;

  KWndText32 m_TongName, m_MasterName;              // 帮会名字，老大名字(五月)
  KWndButton m_TongCamp1, m_TongCamp2, m_TongCamp3; // 阵营名字
  KWndText32 m_MemberCount, m_Money, m_Level;       // 人数，资金

  KWndButton m_BtnTitle, m_BtnMaleTitle, m_BtnFemaleTitle;
