...
------------------------------------------------------------------------------------------
    ...
    都是遵从iKUiWindow接口。界面系统可以支持让窗口有不同的外表(skin)，在系统中称为方案(Scheme)
        ...
        界面系统的实现类为KUiBase。界面系统的应用：
                Init -->
初始化界面系统与各界面窗口，执行此操作后，界面窗口可以执行窗口操作功能了。
                LoadConfig --> 载入窗口的可定制的配置信息，如窗口的位置。
                LoadScheme -->
载入窗口系统的界面方案，在执行这个操作后，界面窗口才知道用什么图来 装扮自己。
...
public:
  ...
  int GetUserPrivateDataFolder(char *pBuffer,
                               int nSize); // 获得当前玩家私有数据的存储目录
  ...
  void DeletePrivateDataFile(
      const char *pszFileName); // 清除当前玩家私有数据的某个文件

  int LoadScheme(const char *pSchemeName); // 按照某种方案载入界面(Skin)
  int SchemeCount();                       // 获得界面方案的数目
  int GetScheme(int nIndex, char *pName,
                char *pPath); // 获得某个界面方案的名称与路径
...
private:
  ...
  void ClearFolder(const char *pszFolder,
                   bool bDeleteFolder); // 清除当前玩家私有数据的存储目录
