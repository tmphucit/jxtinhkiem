...
static inline bool __x_isgraph(char c) { return c < 0 || isgraph(c); }
...
int KShortcutKeyCentre::HandleMouseInput(unsigned int uKey, int nModifier,
                                         int x, int y) {
  if (ms_bMouse)
    return false;
...
void KShortcutKeyCentre::Enable(bool b) { ms_Enable = b; }
...
int LuaSwitchStatus(Lua_State *L) {
  ...
  if (strStatus) {
    ...
    case 3: // PK
      if (g_pCoreShell) {
        int TypePk = g_pCoreShell->GetGameData(GDI_PK_SETTING, 0, 0);
        if (TypePk == 0) {
...
std::string DescHotKey(DWORD hk) {
  ...
  static const char *vkeydesc_table[] = {
      ...
      "", // 1
      "",           "",        "",           "ESC",      "Convert",
      "NonConvert", "Accept",  "ModeChange", "Space",    "PageUp",
      "PageDown",   "End",     "Home",       "Left",     "Up",
      "Right", // 2
      "Down",       "Select",  "Print",      "Execute",  "PrintScreen",
      "Insert",     "Delete",  "Help",       "0",        "1",
      "2",          "3",       "4",          "5",        "6",
      "7", // 3
      "8",          "9",       "",           "",         "",
      "",           "",        "",           "",         "A",
  ...
  static const size_t count_moditbl =
      sizeof(modidesc_table) / sizeof(modidesc_table[0]);
  static const size_t count_vkeytbl =
      sizeof(vkeydesc_table) / sizeof(vkeydesc_table[0]);

  static const char STR_DELIMITER[] = " + ";
  static const size_t LEN_DELIMITER =
      sizeof(STR_DELIMITER) - sizeof(STR_DELIMITER[0]);
...
DWORD ParseHotKey(const std::string &desc) {
  static const struct PATTERNMAP {
    ...
    PATTERNMAP() {
      ...
      for (size_t i = 0; i < sizeof(pattern_table) / sizeof(pattern_table[0]);
           i++) {
  ...
  for (; *szToken; szToken = szNext) {
    ...
    PATTERNMAP::DESC2HKPMAP::const_iterator it =
        s_mapPattern.theMap.find(std::string(szToken, toklen));
...
int LuaRemoveCommand(Lua_State *L) {
  ...
  if (cs.uKey != 0)
    ...
    strncpy(cs.szCommand, strName, 31);
    cs.szCommand[31] = 0;
    KShortcutKeyCentre::RemoveCommand(
        KShortcutKeyCentre::FindCommand(cs.szCommand));
  } else // 清除所有命令
  {
...
int LuaJoinTeam(Lua_State *L) {
  ...
  if (strName && strName[0] != 0) {
    ...
    int nKind = -1;
    if (g_pCoreShell &&
        g_pCoreShell->FindSpecialNPC(strName, &SelectPlayer, nKind) &&
        nKind == kind_player) {
...
int LuaChat(Lua_State *L) {
  ...
  if (strChannelName && strChannelName[0] != 0) {
    ...
    if (nChannelID != -1) {
      if (KUiPlayerBar::IsCanSendMessage(strMessage, nLen, strChannelName,
                                         nChannelID)) {
        char Buffer[1536];
        nLen = KUiFaceSelector::ConvertFaceText(Buffer, strMessage, nLen);
        nLen = TEncodeText(Buffer, nLen);
...
int LuaSayPhrase(Lua_State *L) {
  ...
  if (nLen > 0) {
    ...
    if (nChannelID != -1) {
      if (KUiPlayerBar::IsCanSendMessage(
              szPhrase, nLen,
              KUiMsgCentrePad::GetChannelTitle(
                  KUiMsgCentrePad::GetChannelIndex(nChannelID)),
              nChannelID)) {
        char Buffer[1536];
...
int LuaSetEmote(Lua_State *L) {
  ...
  g_UiChatPhrase.SetEmote(pszCmd, pszName, pszStringTarget, pszStringSelf,
                          nIndex);
  return 0;
}
...
int LuaSet3D(Lua_State *L) {
  ...
  if (nBool) {
    IniString.GetString("Config", "3D", "游戏设为3D模式，重新启动游戏后生效。",
                        Msg.szMessage, sizeof(Msg.szMessage));
    Ini.WriteInteger("Client", "Represent", SCREEN_MODE_3D);
  } else {
    IniString.GetString("Config", "2D", "游戏设为2D模式，重新启动游戏后生效。",
                        Msg.szMessage, sizeof(Msg.szMessage));
...
int LuaSetFullWindow(Lua_State *L) {
  ...
  IniString.Load(GAME_CONFIG_STRING_INI);
  if (nBool) {
    IniString.GetString("Config", "FullWindow",
                        "游戏设定改变为全屏运行，将在下次启动游戏的时候生效。",
                        Msg.szMessage, sizeof(Msg.szMessage));
    Ini.WriteInteger("Client", "FullScreen", 1);
  } else {
    IniString.GetString("Config", "Window",
                        "游戏设定改变为窗口运行，将在下次启动游戏的时候生效。",
                        Msg.szMessage, sizeof(Msg.szMessage));
...
int Mouse_Action(Lua_State *L) {
  ...
  KUiPlayerItem SelectPlayer;
  int nNPCKind = -1;
  if (g_pCoreShell->FindSelectNPC(KShortcutKeyCentre::ms_MouseX,
                                  KShortcutKeyCentre::ms_MouseY, relation_all,
                                  false, &SelectPlayer, nNPCKind)) {
    ...
    if (nRelation == relation_enemy) {
      ...
      g_pCoreShell->GetGameData(GDI_PLAYER_IMMED_ITEMSKILL,
                                (int)&immedItemSkillInfo, 0);

      int nCheckAT = g_pCoreShell->LockSomeoneUseSkill(
          SelectPlayer.nIndex, immedItemSkillInfo.IMmediaSkill[0].uId);
      if (!nCheckAT) // 锁定失败
      {
        g_pCoreShell->LockSomeoneAction(0); // 解锁
      } else {
        g_pCoreShell->LockSomeoneUseSkillCC(nCheckAT * 13 + 15743);
      }
  ...
  int nObjKind = -1;
  int nObjectIdx = 0;
  if (g_pCoreShell->FindSelectObject(KShortcutKeyCentre::ms_MouseX,
                                     KShortcutKeyCentre::ms_MouseY, false,
                                     nObjectIdx, nObjKind)) {
    g_pCoreShell->LockObjectAction(nObjectIdx);
  } else
    g_pCoreShell->LockObjectAction(0);

  g_pCoreShell->GotoWhere(KShortcutKeyCentre::ms_MouseX,
                          KShortcutKeyCentre::ms_MouseY, 0);
...
int Mouse_Force0(Lua_State *L) {
  ...
  g_pCoreShell->UseSkill(KShortcutKeyCentre::ms_MouseX,
...
int Mouse_MoveTo(Lua_State *L) {
  ...
  g_pCoreShell->GotoWhere(KShortcutKeyCentre::ms_MouseX,
                          KShortcutKeyCentre::ms_MouseY, nM);
...
int Mouse_Menu(Lua_State *L) {
  ...
  if (!(g_pCoreShell->FindSelectNPC(
            KShortcutKeyCentre::ms_MouseX, KShortcutKeyCentre::ms_MouseY,
            relation_none | relation_ally | relation_enemy, false,
            &SelectPlayer, nKind) &&
        kind_player == nKind))
    return 0;

  PopUpContextPeopleMenu(SelectPlayer, KShortcutKeyCentre::ms_MouseX,
                         KShortcutKeyCentre::ms_MouseY);
...
int Mouse_Emote_Menu(Lua_State *L) {
  ...
  PopUpContextEmoteMenu(SelectPlayer.Name, KShortcutKeyCentre::ms_MouseX,
                        KShortcutKeyCentre::ms_MouseY);
...
int LuaSetChannelTextColor(Lua_State *L) {
  ...
  if (strName && strColor) {
    KUiMsgCentrePad::SetChannelTextColor(strName, GetColor(strColor),
                                         GetColor(strBColor));
  }
...
int LuaSetMeTextColor(Lua_State *L) {
  ...
  if (strColor) {
    KUiMsgCentrePad::SetMSNTextColor(0, GetColor(strColor),
                                     GetColor(strBColor));
  }
...
int LuaSetFriendTextColor(Lua_State *L) {
  ...
  if (strColor) {
    KUiMsgCentrePad::SetMSNTextColor(1, GetColor(strColor),
                                     GetColor(strBColor));
  }
...
int LuaSetStrangerTextColor(Lua_State *L) {
  ...
  if (strColor) {
    KUiMsgCentrePad::SetMSNTextColor(2, GetColor(strColor),
                                     GetColor(strBColor));
  }
...
TLua_Funcs GameScriptFuns[] = {
    ...
    {"ShortcutSkill",
     LuaShortcutSkill}, // int nIdex [0, SKILLTREE_SHORTCUT_SKILL_COUNT)
    {"DirectShortcutSkill",
     LuaDirectShortcutSkill}, // int nIdex [0, SKILLTREE_SHORTCUT_SKILL_COUNT)
    {"ShortcutUseItem",
     LuaShortcutUseItem},          // int nIdex [0, UPB_IMMEDIA_ITEM_COUNT)
    ...
    {"SayEmote",
     LuaSayEmote}, // char* pszDestName, char* pszDestChannel, int nEmoteIndex
    ...
    {"SetChannelTextColor",
     LuaSetChannelTextColor}, // char* strname, char* strColor, char*
                              // strBorderColor
    {"SetMeTextColor",
     LuaSetMeTextColor}, // char* strColor, char* strBorderColor
    {"SetFriendTextColor",
     LuaSetFriendTextColor}, // char* strColor, char* strBorderColor
    {"SetStrangerTextColor",
     LuaSetStrangerTextColor},         // char* strColor, char* strBorderColor
    ...
    {"ShowSomeoneMessage",
     LuaShowSomeoneMessage}, // char* strName, char* szMessage
    {"GetCurrentChannelName",
     LuaGetCurrentChannelName}, // return the Current Channel Name if isn't
                                // private channel, or null in private channel
    {"GetRecentPlayerName",
     LuaGetRecentPlayerName}, // return the telling player name if is private
                              // channel, or null in public channel
    ...
    {"RenameUnitGroup",
     LuaRenameUnitGroup}, // char* strName, char* strGroup, char* strNewGroup
    {"MoveUnitGroup",
     LuaMoveUnitGroup}, // char* strName, char* strGroup, char* strNewGroup
    {"DeleteUnitGroup", LuaDeleteUnitGroup}, // char* strName, char* strGroup
    {"MoveUnitMember",
     LuaMoveUnitMember}, // char* strName, char* strMember, char* strGroup
    {"CreateAddinUnitMember",
     LuaCreateAddinUnitMember}, // char* strName, char* strMember, char*
                                // strGroup
    {"DeleteAddinUnitMember",
     LuaDeleteAddinUnitMember}, // char* strName, char* strMember
    {"SetAddinUnitMemberStatus",
     LuaSetAddinUnitMemberStatus}, // char* strName, char* strMember, char*
                                   // strStatus
    {"ConvertEmotes",
     LuaConvertEmotes}, // Convert the WHOLE Emote File into Lua file
    {"SetEmote",
     LuaSetEmote}, // Set a emote expression, give 5 param,1 = command,2 = emote
                   // name,3 = emote expression with target,4 = emote expression
                   // without target,5 = emote index,if 5 set give,willl change
                   // the command,name,strings of emote with this index,if not
                   // give,this function will search the emote index by the
                   // command
    {"SetScreenShotFolder",
     LuaSetScreenShotFolder}, // Set the ScreenShot save folder
};
...
BOOL KShortcutKeyCentre::InitScript() {
  if (ms_Script.Init() &&
      ms_Script.RegisterFunctions(GameScriptFuns, g_GetGameScriptFunNum())) {
...
BOOL KShortcutKeyCentre::UninitScript() { return ClearScript(); }
...
BOOL KShortcutKeyCentre::SavePrivateSetting(KIniFile *pFile) {
  ...
  for (; i < ms_nCommands; i++) {
    sprintf(szKey, "%d", i);
    sprintf(szLine, "AddCommand(\"%s\", \"%s\", \"%s\")",
            GetKeyName(ms_pCommands[i].uKey), ms_pCommands[i].szCommand,
            ms_pCommands[i].szDo);
  ...
  while (iFun != ms_FunsMap.end()) {
    sprintf(szKey, "%d", i);
    int nLen = sprintf(szLine, "RegisterFunctionAlias(\"%s\", \"%s\", %d",
                       iFun->first.c_str(), iFun->second.strName.c_str(),
  ...
  if (pWin) {
    ...
    sprintf(szKey, "%d", i);
    sprintf(szLine, "ShowWindow(\"%s\", %d)", "statustool",
            pWin->IsVisible() ? 1 : 0);
    pFile->WriteString(SCRIPT_SECTION, szKey, szLine);
    i++;
  }
  ...
  if (pWin) {
    ...
    sprintf(szKey, "%d", i);
    sprintf(szLine, "ShowWindow(\"%s\", %d)", "normaltool",
            pWin->IsVisible() ? 1 : 0);
    pFile->WriteString(SCRIPT_SECTION, szKey, szLine);
    i++;
  }
  ...
  if (pWin) {
    ...
    sprintf(szKey, "%d", i);
    sprintf(szLine, "ShowWindow(\"%s\", %d)", "chatroom",
            pWin->IsVisible() ? 1 : 0);
    pFile->WriteString(SCRIPT_SECTION, szKey, szLine);
    i++;
    ...
    while (1) {
      ...
      sprintf(szLine, "SetChannelTextColor(\"%s\", \"%s\", \"%s\")", pTitle,
              std::string(GetColorString(uColor)).c_str(),
    ...
    sprintf(szKey, "%d", i);
    sprintf(szLine, "SetMeTextColor(\"%s\", \"%s\")",
            std::string(GetColorString(uColor)).c_str(),
            std::string(GetColorString(uBColor)).c_str());
    pFile->WriteString(SCRIPT_SECTION, szKey, szLine);
    i++;
    ...
    sprintf(szKey, "%d", i);
    sprintf(szLine, "SetFriendTextColor(\"%s\", \"%s\")",
            std::string(GetColorString(uColor)).c_str(),
            std::string(GetColorString(uBColor)).c_str());
    pFile->WriteString(SCRIPT_SECTION, szKey, szLine);
    i++;
    ...
    sprintf(szKey, "%d", i);
    sprintf(szLine, "SetStrangerTextColor(\"%s\", \"%s\")",
            std::string(GetColorString(uColor)).c_str(),
            std::string(GetColorString(uBColor)).c_str());
    pFile->WriteString(SCRIPT_SECTION, szKey, szLine);
    i++;
...
size_t Compile(const char *src, char *dst, size_t dstlen) {
  ...
  static const size_t len_param_begin =
      sizeof(str_param_begin) - sizeof(str_param_begin[0]);
  static const size_t len_param_end =
      sizeof(str_param_end) - sizeof(str_param_end[0]);
  static const size_t len_param_split =
      sizeof(str_param_split) - sizeof(str_param_split[0]);
  static const char len_quote_begin =
      sizeof(str_quote_begin) - sizeof(str_quote_begin[0]);
  static const char len_quote_end =
      sizeof(str_quote_end) - sizeof(str_quote_end[0]);
  ...
  for (;;) {
    ...
    if (cntToken > 0) {   // param
      if (cntToken > 1) { // non first param
        if (!__x_memcpy_n(dst + uselen, dstlen - uselen, str_param_split,
                          len_param_split))
          return 0;
        uselen += len_param_split;
      }

      if (!__x_memcpy_n(dst + uselen, dstlen - uselen, str_quote_begin,
                        len_quote_begin))
        return 0;
      ...
      if (!__x_memcpy_n(dst + uselen, dstlen - uselen, str_quote_end,
                        len_quote_end))
        return 0;
      ...
      if (iFun == KShortcutKeyCentre::ms_FunsMap.end()) {
        ...
        if (!__x_memcpy_n(dst + uselen, dstlen - uselen, name.c_str(),
                          name.size()))
          return 0;
        uselen += name.size();
      }

      if (!__x_memcpy_n(dst + uselen, dstlen - uselen, str_param_begin,
                        len_param_begin))
        return 0;
  ...
  if (cntToken > 0) { // is func, fill ')'
    if (iFun != KShortcutKeyCentre::ms_FunsMap.end()) {
      ...
      if (iFun->second.nParamNum - nParamCount > 0) { // 补默认参数
        ...
        for (int nDefault = iFun->second.nParamNum - nParamCount; nDefault > 0;
             nDefault--) {
          if (nParamCount > 0) {
            if (!__x_memcpy_n(dst + uselen, dstlen - uselen, str_param_split,
                              len_param_split))
    ...
    if (!__x_memcpy_n(dst + uselen, dstlen - uselen, str_param_end,
                      len_param_end))
      return 0;
...
int KShortcutKeyCentre::AddCommand(
    COMMAND_SETTING *
        pAdd) // 复制Add数据并增加到Commands中，如果uKey!=0则覆盖原uKey，否则如szCommand[0]!=0则覆盖szCommand相同者
{
  int nIndex = -1;
  if (pAdd == NULL)
    return nIndex;
  ...
  if (nIndex >= 0) {
    ...
    if (ms_pCommands == NULL) {
      ...
      ms_pCommands = (COMMAND_SETTING *)realloc(
          ms_pCommands, sizeof(COMMAND_SETTING) * (ms_nCommands + 1));
...
int KShortcutKeyCentre::RemoveCommand(int nIndex) // 返回剩余Command的总数
{
  if (ms_pCommands && nIndex >= 0 && nIndex < ms_nCommands) {
    if (nIndex != ms_nCommands - 1) {
      memmove(ms_pCommands + nIndex, ms_pCommands + nIndex + 1,
              sizeof(COMMAND_SETTING));
    }
...
void KShortcutKeyCentre::RemoveCommandAll() { ms_nCommands = 0; }
...
BOOL KShortcutKeyCentre::RegisterFunctionAlias(const char *strFunAlias,
                                               const char *strFun, int nParam,
                                               const PARAMLIST &List) {
  if (strFunAlias && strFunAlias[0] != 0 && strFun && strFun[0] != 0) {
    ShortFuncInfo info;
    info.strName = strFun;
