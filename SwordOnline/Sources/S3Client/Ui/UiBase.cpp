	...
	if (pConfigFile)
	{
		...
		if (pConfigFile->GetString("Main", "Scheme", "", SchemeName, 32))
		{
			...
			//----逐个窗口载入配置设定----
				...
				KUiAutoI::LoadPrivateSetting(pConfigFile);	//在KShortcutKeyCentre之后,因为会有脚本生成Unit
				KUiTeamAuto::LoadPrivateSetting(pConfigFile);	//在KShortcutKeyCentre之后,因为会有脚本生成Unit
			//----逐个窗口载入配置设定结束----
			}
		}
		ClosePrivateSettingFile(false);
