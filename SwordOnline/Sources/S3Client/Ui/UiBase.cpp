...
void KUiBase::DeletePrivateDataFile(const char *pszFileName) {
  ...
  if (pszFileName && pszFileName[0]) {
    sprintf(szBuffer, "%s\\%s\\%s", UI_USER_DATA_FOLDER, m_UserAccountId,
            pszFileName);
    ...
    if (g_pRepresentShell &&
      ...
      for (i = 0; i < nCount; i++) {
        ...
        if (Ini.GetInteger(FONT_SECTION, Section, 0, &nId)) {
          ...
          if (Ini.GetString(FONT_SECTION, Section, "", Buffer,
                            sizeof(Buffer)) &&
              Buffer[0]) {
...
void KUiBase::LoadSchemeForEachWnd() {
  ...
  KUiBuyShop::LoadScheme(m_CurSchemePath);
  KUiSkills::LoadScheme(m_CurSchemePath);
  KUiStoreBox::LoadScheme(m_CurSchemePath);
  KUiRPNewBox::LoadScheme(m_CurSchemePath);
  KUiCheckItemBox::LoadScheme(m_CurSchemePath);
  KUiPGBoxItem::LoadScheme(m_CurSchemePath);
  KUiSlectBoxCheck::LoadScheme(m_CurSchemePath);
...
int KUiBase::GetSchemePath(const char *pScheme) {
  ...
  if (pScheme == NULL || (pIni = GetCommConfigFile()) == NULL)
    return false;
...
void KUiBase::SetCurrentCursor() {
  ...
  case UIS_S_TRADE_REPAIR: // （与npc）交易 修理
    ...
    break;
  case UIS_S_BLOCK_ITEM: // （与npc）交易 修理
    nCursorIndex = CURSOR_BLOCK_ITEM;
    break;
  case UIS_S_UNBLOCK_ITEM: // （与npc）交易 修理
    nCursorIndex = CURSOR_UNBLOCK_ITEM;
    break;

  case UIS_S_PRICE_ITEM: // （与npc）交易 修理
    nCursorIndex = CURSOR_PRICE_ITEM;
    break;
...
int KUiBase::IsOperationEnable(UISYS_OPERATION uOper) {
  ...
  case UIS_O_TRADE_ITEM:
    ...
    break;
  case UIS_O_BLOCK_UNBLOCK_ITEM:
    bEnable = (m_Status == UIS_S_BLOCK_ITEM || m_Status == UIS_S_UNBLOCK_ITEM);
    break;

  case UIS_O_S_PRICE_ITEM:
    bEnable = (m_Status == UIS_S_PRICE_ITEM);
    break;
  }
  return bEnable;
}
...
int KUiBase::NotifyOneEvent(
    const char *szName,
    const char *szEvent) // 0为已经处理, 不要再处理了, 非0为未处理或错误
{
  ...
  if (i->second) {
    ...
    nRet = ::SendMessage((HWND)i->second, WM_COPYDATA, (WPARAM)g_GetMainHWnd(),
                         (LPARAM)&cp);
  }
    ...
    const char *szEvent) // 0为已经处理, 不要再处理了, 非0为未处理或错误
{
  ...
  while (i != m_EventWndList.end()) {
    if (i->second) {
      ...
      cp.cbData = nLen;
      cp.lpData = (void *)szEvent;
      nRet = ::SendMessage((HWND)i->second, WM_COPYDATA,
                           (WPARAM)g_GetMainHWnd(), (LPARAM)&cp);
      if (!nRet)
        return nRet;
    }
    i++;
...
int AddinNotify::RenameUnitGroup(const char *Unit, const char *Name,
                                 const char *NewName,
...
int AddinNotify::DeleteUnitGroup(const char *Unit, const char *Name,
                                 const STRINGLIST &friends) {
  if (Unit && Unit[0] && Name) {
    char szEvent[512];
...
int AddinNotify::MoveUnitGroup(const char *Unit, const char *Name,
...
int AddinNotify::CreateUnitMember(const char *Unit, const char *Name,
                                  const char *Group) {
...
int AddinNotify::MoveUnitMember(const char *Unit, const char *Name,
                                const char *Group) {
  if (Unit && Unit[0] && Name && Group) {
    char szEvent[256];
...
int AddinNotify::SetUnitMemberStatus(const char *Unit, const char *Name,
                                     const char *status) {
