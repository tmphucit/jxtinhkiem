...
int OutputTabSplitText(const char *pBuffer, int nCount, int nLineWidth,
  ...
  if (pBuffer == NULL || nCount < 0 || g_pRepresentShell == NULL ||
      nFontId <= 0)
    return 0;
  ...
  while (nCurrentPos < nCount && nOutputLineCount < nMaxLineCount) {
    int nLineEndPos =
        TFindSpecialCtrlInEncodedText(pBuffer, nCount, nCurrentPos, KTC_ENTER);
    int nTabPos =
        TFindSpecialCtrlInEncodedText(pBuffer, nCount, nCurrentPos, KTC_TAB);
    ...
    g_pRepresentShell->OutputRichText(nFontId, pParam, pBuffer + nCurrentPos,
                                      nLen);
    ...
    if (nTabPos >= 0) {
      ...
      if (nLen > 0) {
        int nOutputLen;
        TGetEncodedTextLineCount(pBuffer + nCurrentPos, nLen, 0, nOutputLen,
                                 nFontId);
        pParam->nX += nLineWidth - nOutputLen * nFontId / 2;
        g_pRepresentShell->OutputRichText(nFontId, pParam,
                                          pBuffer + nCurrentPos, nLen);
        pParam->nX = nOldX;
      }
      nCurrentPos += nLen;
    }
...
bool SetScrPicPath(const char *szPath) {
  if (!szPath || !szPath[0] || strlen(szPath) >= MAX_PATH) {
    if (!SHGetSpecialFolderPath(NULL, szScrPicPath, CSIDL_DESKTOPDIRECTORY,
                                false))
      return false;
