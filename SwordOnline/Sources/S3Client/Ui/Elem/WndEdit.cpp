...
int KWndEdit::Init(KIniFile *pIniFile, const char *pSection) {
  if (KWndWindow::Init(pIniFile, pSection)) {
    ...
    pIniFile->GetInteger(pSection, "Boder", 0, &m_bBoder);
...
void KWndEdit::SetColor(unsigned int uColor) { m_TextColor = uColor; }
...
int KWndEdit::OnKeyDown(int nKeyCode, int nModifiers) {
  ...
  case VK_DELETE:
    if (m_nCaretPos < m_nTextLen) {
      ...
      m_nTextLen -= nNextPos;
      memmove(m_pText + m_nCaretPos, m_pText + m_nCaretPos + nNextPos,
              m_nTextLen - m_nCaretPos + 1);
      UpdateData();
    }
    break;
  case VK_LEFT:
  ...
  case VK_UP:
    if (MULTI_LINE && m_nCaretLineIndex) {
      ...
      for (nRet = 0; nRet < m_nCaretLineIndex; nRet++) {
        nLineHead = nLineEnd;
        nLineEnd =
            nLineHead + SplitStringLine(&m_pText[nLineHead], 1, nCharPerLine);
      }
      ...
      nRet = m_pParentWnd->WndProc(WND_N_EDIT_SPECIAL_KEY_DOWN,
                                   (unsigned int)(KWndWindow *)this, VK_UP);
    break;
  case VK_DOWN:
    if (MULTI_LINE) {
      int nCharPerLine = (m_Width * 2) / m_nFontSize;
      ...
      nRet = m_pParentWnd->WndProc(WND_N_EDIT_SPECIAL_KEY_DOWN,
                                   (unsigned int)(KWndWindow *)this, VK_DOWN);
    break;
  case VK_TAB:
  case VK_PRIOR:
  case VK_NEXT:
  case VK_ESCAPE:
    if (m_pParentWnd)
      nRet = m_pParentWnd->WndProc(WND_N_EDIT_SPECIAL_KEY_DOWN,
                                   (unsigned int)(KWndWindow *)this, nKeyCode);
    break;
  case VK_RETURN:
    if (m_pParentWnd) {
      if ((m_Flag & WNDEDIT_ES_MULTI_LINE) == 0)
        nRet =
            m_pParentWnd->WndProc(WND_N_EDIT_SPECIAL_KEY_DOWN,
                                  (unsigned int)(KWndWindow *)this, VK_RETURN);
...
int KWndEdit::InsertChar(int nChar) {
  ...
  if (nChar == VK_BACK) // 退格
  {
    if (m_nCaretPos) {
      ...
      memmove(m_pText + nPrioPos, m_pText + m_nCaretPos,
              m_nTextLen - m_nCaretPos + 1);
    ...
    if ((m_Flag & WNDEDIT_ES_CHAR_TYPE_FILTER) ||
        (nChar >= '0' && nChar <= '9'))
...
int KWndEdit::InsertChar(char cLow, char cHigh) {
  ...
  if (m_nCaretPos + nLen <= m_nLimitText) {
    if (m_nTextLen + nLen > m_nLimitText) {
      m_nTextLen =
          m_nCaretPos + TSplitString(m_pText + m_nCaretPos,
                                     m_nLimitText - nLen - m_nCaretPos, true);
      m_pText[m_nTextLen] = 0;
    }
    memmove(m_pText + m_nCaretPos + nLen, m_pText + m_nCaretPos,
            m_nTextLen - m_nCaretPos + 1);
    if (cHigh)
...
void KWndEdit::UpdateData() {
  ...
  if (m_pParentWnd) { // 发送更新消息
    m_pParentWnd->WndProc(WND_N_EDIT_CHANGE, (unsigned int)(KWndWindow *)this,
                          0);
  }
}
...
void KWndEdit::FmtForShow() {
  ...
  //============================
  {
    ...
    if (m_nCaretPos < m_nSkipAhead)
      ...
      m_nSkipAhead =
          TSplitString(m_pText, m_nCaretPos + nJumpLen - nLineLen, false);
    if (m_nSkipAhead + nLineLen >= m_nTextLen)
      m_nSkipBehind = -1;
    else
      m_nSkipBehind =
          m_nSkipAhead + TSplitString(m_pText + m_nSkipAhead, nLineLen, true);
  }
  //============================
  //		多行输入窗口处理
  //============================
    ...
    if (m_nCaretPos < m_nSkipAhead) {
      ...
      do {
        ...
        m_nSkipBehind =
            m_nSkipAhead + SplitStringLine(m_pText + m_nSkipAhead, 1, nLineLen);
      } while (m_nSkipBehind < m_nCaretPos);
    }
    m_nSkipBehind =
        m_nSkipAhead + SplitStringLine(m_pText + m_nSkipAhead, nLine, nLineLen);
    while (m_nCaretPos > m_nSkipBehind) {
      m_nSkipAhead =
          m_nSkipAhead + SplitStringLine(m_pText + m_nSkipAhead, 1, nLineLen);
      m_nSkipBehind = m_nSkipAhead +
                      SplitStringLine(m_pText + m_nSkipAhead, nLine, nLineLen);
    }
    m_nCaretLineIndex = 0;
    int nLineHead = 0, nLineEnd = 0;
    while (true) {
    ...
    if (m_nCaretPos > 0 && m_pText[m_nCaretPos - 1] == KTC_ENTER) {
      m_nCaretLineIndex++;
      int nTemp = m_nSkipAhead +
                  SplitStringLine(m_pText + m_nSkipAhead, nLine - 1, nLineLen);
      if (nTemp < m_nSkipBehind) {
        m_nSkipAhead =
            m_nSkipAhead + SplitStringLine(m_pText + m_nSkipAhead, 1, nLineLen);
        m_nSkipBehind = m_nSkipAhead + SplitStringLine(m_pText + m_nSkipAhead,
                                                       nLine, nLineLen);
      }
    }
  }
...
void KWndEdit::PaintWindow() {
  ...
  if (m_bBoder) {
    ...
    KRURect Rect;
    Rect.Color.Color_dw = m_TextColor;
  ...
  if (MULTI_LINE == 0) {
    ...
    if ((m_Flag & WNDEDIT_ES_MASK_CHARACTER) == 0) {
      g_pRepresentShell->OutputText(m_nFontSize, &m_pText[m_nSkipAhead], nLen,
                                    m_nAbsoluteLeft, m_nAbsoluteTop,
                                    m_TextColor, 0, TEXT_IN_SINGLE_PLANE_COORD,
                                    m_TextBorderColor);
    } else {
      int x = m_nAbsoluteLeft;
      for (; nLen > 1; nLen -= 2, x += m_nFontSize)
        g_pRepresentShell->OutputText(
            m_nFontSize, "**", 2, x, m_nAbsoluteTop, m_TextColor, 0,
            TEXT_IN_SINGLE_PLANE_COORD, m_TextBorderColor);
      if (nLen)
        g_pRepresentShell->OutputText(
            m_nFontSize, "*", 1, x, m_nAbsoluteTop, m_TextColor, 0,
            TEXT_IN_SINGLE_PLANE_COORD, m_TextBorderColor);
    }
    if (bShowCaret) {
      int nCaretX =
          m_nAbsoluteLeft + (m_nCaretPos - m_nSkipAhead) * m_nFontSize / 2 + 2;
      int nCaretY = m_nAbsoluteTop;

      if ((m_nCaretX != nCaretX) || (m_nCaretY != nCaretY)) {
        m_nCaretX = nCaretX;
        m_nCaretY = nCaretY;
        SetCaretPos(m_nCaretX, m_nCaretY);
        s_Ime.SetCaretPos(m_nCaretX, m_nCaretY);
      ...
      g_pRepresentShell->OutputText(
          m_nFontSize, "|", 2, nCaretX - m_nFontSize / 4, nCaretY, m_TextColor,
    ...
    do {
      nLineEnd =
          nLineAhead + SplitStringLine(m_pText + nLineAhead, 1, nLineBytes);

      g_pRepresentShell->OutputText(
          m_nFontSize, m_pText + nLineAhead, nLineEnd - nLineAhead,
          m_nAbsoluteLeft, y, m_TextColor, m_Width, TEXT_IN_SINGLE_PLANE_COORD,
          m_TextBorderColor);

      if (bShowCaret && m_nCaretPos >= nLineAhead && m_nCaretPos <= nLineEnd) {
        int nCaretX, nCaretY;

        if (m_nCaretPos == nLineEnd && m_nCaretPos > 0 &&
            m_pText[m_nCaretPos - 1] == KTC_ENTER) {
          nCaretX = m_nAbsoluteLeft - m_nFontSize / 2 + 2;
          nCaretY = y + m_nFontSize + 1;
        } else {
          nCaretX = m_nAbsoluteLeft +
                    (m_nCaretPos - nLineAhead - 1) * m_nFontSize / 2 + 2;
          nCaretY = y;
        }
        ...
        g_pRepresentShell->OutputText(
            m_nFontSize, "│", 2, nCaretX, nCaretY, m_TextColor, 0,
            TEXT_IN_SINGLE_PLANE_COORD, m_TextBorderColor);
        bShowCaret = false;
      }
      y += m_nFontSize + 1;
