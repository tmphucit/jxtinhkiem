...
KWndToolBar::~KWndToolBar() { FreeAllWindow(); }
...
// 窗体绘制
void KWndToolBar::PaintWindow() { KWndImage::PaintWindow(); }

void KWndToolBar::AddOneWindow(KIniFile *pIniFile, const char *pSection) {
  m_pButtons = (KButtonInfo *)realloc(m_pButtons,
                                      sizeof(KButtonInfo) * (m_nButtonNum + 1));
  if (m_pButtons && m_nButtonNum >= 0) {
    ...
    if (pIniFile->GetString(pSection, "ClassType", "", szType, 32)) {
      ...
      m_pButtons[m_nButtonNum].m_pButton =
          gGetComClassFactory()->CreateComObject(szType);
