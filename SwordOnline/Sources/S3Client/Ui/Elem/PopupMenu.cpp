...
unsigned int KPopupMenu::m_uDefBorderLineColor =
...
unsigned int KPopupMenu::m_uDefSelItemBgColor =
    0x0a202020; // 被选中菜单项的背景色
...
void KPopupMenu::Popup(KPopupMenuData *pMenu, KWndWindow *pCaller,
                       unsigned int uParam) {
  Cancel();
  ...
  for (int i = 0; i < m_pMenu->nNumItem; i++) {
    nLenHei = TGetEncodedTextLineCount(m_pMenu->Items[i].szData,
                                       m_pMenu->Items[i].uDataLen, -1, nLenWid,
                                       m_pMenu->byFontSize, 0, 1);
    ...
    if (bCalcRightWidth) {
      int nTabPos = TFindSpecialCtrlInEncodedText(
          m_pMenu->Items[i].szData, m_pMenu->Items[i].uDataLen, 0, KTC_TAB);
      if (nTabPos >= 0) {
        TGetEncodedTextLineCount(m_pMenu->Items[i].szData + nTabPos + 1,
                                 m_pMenu->Items[i].uDataLen - nTabPos - 1, 0,
                                 nLenWid, m_pMenu->byFontSize);
  ...
  if (m_pMenu->nItemHeight == MENU_ITEM_DEFAULT_HEIGHT) {
    m_pMenu->nItemHeight =
        m_pMenu->byFontSize * nMaxItemHei + 2 * m_pMenu->byItemTitleUpSpace;
...
void KPopupMenu::PaintMenu() {
      ...
      if (m_pMenu->uTextColor) {
        ...
        if ((m_pMenu->usMenuFlag & PM_F_TAB_SPLIT_ITEM_TEXT) == 0) {
          g_pRepresentShell->OutputRichText(m_pMenu->byFontSize, &param,
                                            m_pMenu->Items[i].szData,
                                            m_pMenu->Items[i].uDataLen);
        } else {
          OutputTabSplitText(m_pMenu->Items[i].szData,
                             m_pMenu->Items[i].uDataLen, m_pMenu->nItemWidth,
                             m_pMenu->byFontSize, &param);
        }
      }
    }
  ...
  if (m_pMenu->nSelectedItem >= 0) {
    Shadow.oPosition.nY -=
        m_pMenu->nItemHeight * (m_pMenu->nNumItem - m_pMenu->nSelectedItem);
    Shadow.oEndPos.nY = Shadow.oPosition.nY + m_pMenu->nItemHeight;
...
// 窗口函数
int KPopupMenu::HandleInput(unsigned int uMsg, unsigned int uParam,
                            int nParam) {
  int nRet = 0;
  if (m_pMenu) {
    switch (uMsg) {
...
void KPopupMenu::LoadTheme(const char *pScheme) {
  ...
  if (Ini.Load(Buff)) {
    ...
    if (Ini.GetString("Menu", "SelTextBorderColor", "0,0,0", Buff,
                      sizeof(Buff)))
