...
void KWndMessageListBox::UpdateChatItem() {
  if (m_bItemActived) {
    KUiPlayerBar::ClearItemBtn();
    for (int j = 0; j < m_nCapability; j++) {
      if (m_pMessages[j]) {
        if (m_pMessages[j]->nHaveItem)
          KUiPlayerBar::SetItemBtnInfo(j, &m_pMessages[j]->sItem);
      }
    }
  } else
    KUiPlayerBar::ClearItemBtn();
...
// 功能	: 析构函数
// -------------------------------------------------------------------------
KWndMessageListBox::~KWndMessageListBox() { SetCapability(0); }
...
int KWndMessageListBox::SetCapability(int nNumMessage) {
  ...
  if (nNumMessage && m_nNumMessage) {
    if (m_nNumMessage <= nNumMessage) {
      ...
      memcpy(pNew, &m_pMessages[m_nNumMessage - nNumMessage],
             sizeof(KOneMsgInfo *) * nNumMessage);
      memset(&m_pMessages[m_nNumMessage - nNumMessage], 0,
             sizeof(KOneMsgInfo *) * nNumMessage);
...
// 功能	: 新添一条信息
// -------------------------------------------------------------------------
int KWndMessageListBox::AddOneMessage(const char *pText, int nLen,
                                      ChatItem *pItem /*= NULL*/,
                                      bool bHaveItem /*= false*/,
                                      int nPos /*= 0*/, int nLinePos /*= 1*/,
                                      int nFace /*= 0*/,
  ...
  if (m_nCapability > 0 && nLen) {
    ...
    if (pItem && bHaveItem) {
      memcpy(&pMsg->sItem, pItem, sizeof(ChatItem));
      pMsg->nHaveItem = 1;
      pMsg->nPos = nPos;
      pMsg->nLinePos = nLinePos;
      pMsg->nFace = nFace;
    } else {
      memset(&pMsg->sItem, 0, sizeof(ChatItem));
      pMsg->nHaveItem = 0;
      pMsg->nPos = 0;
      pMsg->nLinePos = 0;
      pMsg->nFace = 0;
    ...
    if (g_pRepresentShell) {
      pMsg->nLines =
          TGetEncodedTextLineCount(pMsg->Msg, nLen, m_nNumBytesPerLine,
                                   pMsg->nCharWidth, m_nFontSize, 0, 0, true);
    } else {
      pMsg->nLines = 0;
      pMsg->nCharWidth = 0;
    }
...
int KWndMessageListBox::Init(KIniFile *pIniFile, const char *pSection) {
  if (KWndWindow::Init(pIniFile, pSection)) {
    ...
    char Buffer[32];
    if (pIniFile->GetString(pSection, "TextLineShadow", "", Buffer,
                            sizeof(Buffer))) {
    ...
    if (nValue) {
      ...
      if (pIniFile->GetString(pSection, "SelBgColor", "", Buffer,
                              sizeof(Buffer))) {
...
void KWndMessageListBox::SetSize(int nWidth, int nHeight) {
  ...
  if (nLines != m_nNumMaxShowLine || m_nNumBytesPerLine != nNumBytesPerLine) {
    ...
    if (m_Style & MSGLISTBOX_S_TEXTBOTTOM) {
      nTo = GetItemLineCount(m_nNumMessage) -
            GetItemLineCount(m_nStartShowMsg) - m_nStartMsgSkipLine -
            m_nNumMaxShowLine;
      nTo = max(nTo, 0);
    }
    ...
    for (int i = 0; i < m_nNumMessage; i++) {
      if (g_pRepresentShell) {
        m_pMessages[i]->nLines = TGetEncodedTextLineCount(
            m_pMessages[i]->Msg, m_pMessages[i]->nLen, m_nNumBytesPerLine,
            m_pMessages[i]->nCharWidth, m_nFontSize, 0, 0, true);
      } else
        m_pMessages[i]->nLines = 0;
    }

    if (m_Style & MSGLISTBOX_S_TEXTBOTTOM)
      SetFirstShowLine(GetItemLineCount(m_nNumMessage) - nTo -
                       m_nNumMaxShowLine);
...
void KWndMessageListBox::SetType(int nType) { m_nType = nType; }
...
void KWndMessageListBox::UpdateDataP() { UpdateData(); }
...
void KWndMessageListBox::PaintWindow() {
  ...
  if (nCurMsg && m_bItemActived)
    for (int i = nCurMsg - 1; i >= 0; i--) {
      if (m_pMessages[i])
        KUiPlayerBar::SetItemBtnPos(i, INVISIBLE_POS);
    }
  //-------------------------------------------------------------------------------------------------------
  ...
  while (nViewLines > 0 && nCurMsg < m_nNumMessage) {
    ...
    if (Param.nNumLine > nViewLines) {
      ...
      if (pCurMsg->nLinePos > Param.nNumLine)
        bPaintItem = FALSE;
    }
    ...
    if (bDraw) {
      ...
      if (pCurMsg->uTextBKColor) // 绘制文字衬底
      {
        ...
        bg.oPosition.nY = Param.nY;
        bg.oEndPos.nX =
            bg.oPosition.nX + (pCurMsg->nCharWidth * m_nFontSize / 2);
        bg.oEndPos.nY = bg.oPosition.nY + Param.nNumLine * (m_nFontSize + 1);
        g_pRepresentShell->DrawPrimitives(1, &bg, RU_T_SHADOW, true);
      }
      ...
      if (m_bItemActived) {
        if ((m_pMessages[nCurMsg]->nLinePos) > Param.nSkipLine && bPaintItem)
          KUiPlayerBar::SetItemBtnPos(
              nCurMsg,
              Param.nX +
                  (m_pMessages[nCurMsg]->nPos) * m_nFontSize /
                      2 /*- (int)((m_pMessages[nCurMsg]->nLinePos==1)?3:0)*/
                  + (m_pMessages[nCurMsg]->nFace) * 2,
              Param.nY +
                  ((m_pMessages[nCurMsg]->nLinePos) - 1 - Param.nSkipLine) *
                      m_nFontSize);
        else
...
//	功能：窗口函数
//--------------------------------------------------------------------------
int KWndMessageListBox::WndProc(unsigned int uMsg, unsigned int uParam,
                                int nParam) {
  switch (uMsg) {
  case WM_MOUSEMOVE:
  ...
  if (n >= 0 && n < m_nNumMessage) {
    if (m_bHitText) {
      ...
      if (x > 0 && x <= (m_pMessages[n]->nCharWidth / 2 * m_nFontSize)) {
        ...
        if (nPos >= 0) {
          int nNameLen = 0;
          TGetEncodedTextLineCount(m_pMessages[n]->Msg, nPos,
                                   m_nNumBytesPerLine, nNameLen, m_nFontSize, 0,
                                   0, true);
...
void KWndMessageListBox::OnLButtonDown(int x, int y) {
  ...
  if (nSel >= 0 && m_pParentWnd)
    m_pParentWnd->WndProc(WND_N_LIST_ITEM_ACTIVE,
                          (unsigned int)(KWndWindow *)this, m_nSelMsgIndex);
  SetCurSel(nSel);
}
...
int KWndMessageListBox::SetCurSel(int nIndex) {
  ...
  if (m_pParentWnd)
    m_pParentWnd->WndProc(WND_N_LIST_ITEM_SEL, (unsigned int)(KWndWindow *)this,
                          m_nSelMsgIndex);
...
void KWndMessageListBox::OnMouseMove(int x, int y) {
  ...
  if (nSel != m_nHLMsgIndex) {
    m_nHLMsgIndex = nSel;
    if (m_pParentWnd) {
      m_pParentWnd->WndProc(WND_N_LIST_ITEM_HIGHLIGHT,
                            (unsigned int)(KWndWindow *)this, m_nHLMsgIndex);
    }
  }
}
...
//--------------------------------------------------------------------------
int KWndMessageListBox::GetOneMessage(int nIndex, char *pBuffer, int nLen,
                                      bool bExcludeCtrl) {
  if (pBuffer) {
    if (nIndex < 0)
...
unsigned int KWndMessageListBox::SplitData() {
  ...
  if (m_nCapability) {
    ...
    if (pData) {
      ...
      m_nStartShowMsg = 0;
      m_pMessages =
          (KOneMsgInfo **)malloc(sizeof(KOneMsgInfo *) * m_nCapability);
      if (m_pMessages)
...
void KWndMessageListBox::ClearHideLine() {
  m_nHideNumLine = 0;
  m_nFisrtVisId = m_nStartShowMsg; //
}

void KWndMessageListBox::HideNextLine() {
  int nCurMsg = m_nStartShowMsg;
  if (m_pMessages && m_nHideNumLine < m_nNumMaxShowLine && m_nNumMessage > 0 &&
      nCurMsg < m_nNumMessage) {
...
int KWndMessageListBox::GetMinHeight() { return max(m_nFontSize + 1, 0); }
...
int KScrollMessageListBox::Init(KIniFile *pIniFile,
                                const char *pSection) // 初始化
{
  if (KWndWindow::Init(pIniFile, pSection)) {
    char Buffer[128];
    ...
    pIniFile->GetInteger2(pSection, "MsgLineCount", &nMinLineCount,
                          &nMaxLineCount);
...
// 窗口函数
int KScrollMessageListBox::WndProc(unsigned int uMsg, unsigned int uParam,
                                   int nParam) {
  switch (uMsg) {
  ...
  case WND_M_POPUPMENU:
    if (m_pParentWnd) {
      m_pParentWnd->WndProc(WND_M_POPUPMENU, (unsigned int)(KWndWindow *)this,
                            nParam);
    }
    break;
  default:
