...
// 功能	: 新添一条信息
// -------------------------------------------------------------------------
int KWndMessageListBox::AddOneMessage(const char* pText, int nLen, ChatItem* pItem /*= NULL*/, bool bHaveItem /*= false*/, int nPos /*= 0*/, int nLinePos /*= 1*/, int nFace /*= 0*/,unsigned int uTextBKColor)
{
	if (pText == NULL)
		return 0;
	if (nLen < 0)
...
void KWndMessageListBox::SetType(int nType)
{
	m_nType = nType;
}
...
void KWndMessageListBox::UpdateDataP()
{
...
//		if (m_nType == 0 || ((m_pMessages[i]->nType == 7 || m_pMessages[i]->nType == 4) && m_nType == 4) || (m_pMessages[i]->nType == 3 && m_nType == 2) || (m_pMessages[i]->nType == 6 && m_nType == 3) || (m_pMessages[i]->nType == 0 && m_nType == 1))
//		{
...
//-------------------------------An~ Item chat phia' tren khung------------------------------------------
	if(nCurMsg && m_bItemActived)
	for(int i = nCurMsg-1; i >= 0; i--)
	{
		if(m_pMessages[i]) KUiPlayerBar::SetItemBtnPos(i,INVISIBLE_POS);
	}
//-------------------------------------------------------------------------------------------------------
	...
	while (nViewLines > 0 && nCurMsg < m_nNumMessage)
	{
		...
		if (Param.nNumLine > nViewLines)
			...
			if(pCurMsg->nLinePos > Param.nNumLine) bPaintItem = FALSE;
			}
...
//-----------------------------Ve~ button cho item chat-Ken Nguyen---------------------------------------------------------------
			if(m_bItemActived)
			{
			if ((m_pMessages[nCurMsg]->nLinePos) > Param.nSkipLine && bPaintItem)
			KUiPlayerBar::SetItemBtnPos(nCurMsg,Param.nX + (m_pMessages[nCurMsg]->nPos)*m_nFontSize/2 /*- (int)((m_pMessages[nCurMsg]->nLinePos==1)?3:0)*/
										+ (m_pMessages[nCurMsg]->nFace)*2 ,
										Param.nY + ((m_pMessages[nCurMsg]->nLinePos)-1 - Param.nSkipLine)*m_nFontSize);
			else KUiPlayerBar::SetItemBtnPos(nCurMsg,INVISIBLE_POS);
			}
//---------------------------------------------------------------------------------------------------------------------------------
			y += Param.nNumLine * (m_nFontSize + 1);
		}
		else if(m_bItemActived) KUiPlayerBar::SetItemBtnPos(nCurMsg,INVISIBLE_POS);
		nSkipLine = 0;
		nViewLines -= Param.nNumLine;

		nCurMsg++;
	}
//--------------------------An~ button phia duoi khung-----------------------------------------------
