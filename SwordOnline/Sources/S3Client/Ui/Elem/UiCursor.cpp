...
void KUiCursor::SetImage(int nIndex, const char *pImgFile) {
...
#else // KUI_USE_HARDWARE_MOUSE
  {
    if (nIndex >= 0 && nIndex < MAX_CURSOR_IMAGE && pImgFile) {
      ...
      g_pRepresentShell->GetImageParam(pCurrentImg->szImage, &Param,
                                       pCurrentImg->nType);
...
//	功能：重新激活鼠标
//--------------------------------------------------------------------------
void KUiCursor::RestoreCursor() { SwitchImage(m_nCurImage); }
...
void KUiCursor::Paint() {
...
#else // KUI_USE_HARDWARE_MOUSE
  {
    if (m_bShow && g_pRepresentShell) {
      ...
      g_pRepresentShell->DrawPrimitives(1, &m_CursorImages[m_nCurImage],
                                        RU_T_IMAGE, true);
