...
void SetMouseHoverObjectDesc(void *pWnd, int nObj, unsigned int uGenre,
  ...
  if (g_pCoreShell) {
    ...
    if (Desc.szTitle[0]) {
      ...
      g_MouseOver.SetMouseHoverTitle(Desc.szTitle, nLenTitle,
                                     s_uHoverObjDestTextColor);
    }
    if (Desc.szProp[0]) {
      nLenProp = TEncodeText(Desc.szProp, strlen(Desc.szProp));
      g_MouseOver.SetMouseHoverProp(Desc.szProp, nLenProp,
                                    s_uHoverObjDestTextColor);
    }
    if (Desc.szDesc[0]) {
      nLenDesc = TEncodeText(Desc.szDesc, strlen(Desc.szDesc));
      g_MouseOver.SetMouseHoverDesc(Desc.szDesc, nLenDesc,
                                    s_uHoverObjDestTextColor);
    }
  }
}

int DrawDraggingGameObjFunc(int x, int y, const KUiDraggedObject &Obj,
 ...
 ************************************************************************/
void KMouseOver::SetMouseHoverTitle(const char *pTitleText, int nTitleTextLen,
                                    UINT uColor) {
  if (nTitleTextLen > 0 && pTitleText &&
      nTitleTextLen <= GOD_MAX_OBJ_TITLE_LEN) {
 ...
 *功能：设置鼠标提示窗口的物件属性
 ************************************************************************/
void KMouseOver::SetMouseHoverProp(const char *pPropText, int nPropTextLen,
                                   UINT uColor) {
 ...
 *功能：设置鼠标提示窗口的物件说明
 ************************************************************************/
void KMouseOver::SetMouseHoverDesc(const char *pDescText, int nDescTextLen,
                                   UINT uColor) {
...
void KMouseOver::Update(int nX, int nY) {
  ...
  if (m_nTitleLen > 0) {
    m_nTitleLineNum = TGetEncodedTextLineCount(m_ObjTitle, m_nTitleLen, 0,
                                               nMaxTitleLen, m_nFontSize);
    m_nMaxLineLen = nMaxTitleLen;
  } else {
    m_nTitleLineNum = 0;
  }
  if (m_nPropLen > 0) {
    m_nPropLineNum = TGetEncodedTextLineCount(m_ObjProp, m_nPropLen, 0,
                                              nMaxPropLen, m_nFontSize);
  ...
  if (m_nDescLineNum > 0) {
    m_nDescLineNum = TGetEncodedTextLineCount(m_ObjDesc, m_nDescLen, 0,
                                              nMaxDescLen, m_nFontSize);
...
void KMouseOver::LoadScheme(const char *pScheme) {
  ...
  if (Ini.Load(Buff)) {
    ...
    Ini.GetString("Main", "Image", "", m_Image.szImage,
                  sizeof(m_Image.szImage));
...
void KMouseOver::PaintMouseHoverInfo() {
  ...
  if (m_nPropLen > 0) {
    ...
    Param.nY = Shadow.oPosition.nY;
    OutputTabSplitText(m_ObjProp, m_nPropLen, m_nWndWidth - m_nIndent * 2,
                       m_nFontSize, &Param);
  }
  ...
  if (m_nDescLen > 0) {
    ...
    Param.nX = m_nLeft + m_nIndent;
    Param.nY = Shadow.oPosition.nY;
    g_pRepresentShell->OutputRichText(m_nFontSize, &Param, m_ObjDesc,
                                      m_nDescLen, 0);
  }
...
/*void KMouseOver::FormatProp()
{
    if(m_ObjProp[0])
        {
                char szBuffer[MAX_OBJ_PROP_LEN], *pPos = NULL, *pHead = NULL,
*pTail = NULL; int nLeftLen = 0, nRightLen = 0, i, j, k;
        //初始准备
