...
void IR_GetReferenceSpot(KUiImageRef &Img, int &h,
                         int &v); // 获得图像参考点（一般所说为重心）
void IR_UpdateImagePart(KUiImagePartRef &Img, int nPartValue,
                        int nFullValue); // 设置绘制图的局部
int IR_IsTimePassed(unsigned int uInterval,
                    unsigned int &uLastTimer); // 判断时间是否已经到了
unsigned int IR_GetRemainTime(
    unsigned int uInterval,
    unsigned int
        uLastTimer); // 获取剩余时间，如果时间已经到了/过了，返回值都为0
