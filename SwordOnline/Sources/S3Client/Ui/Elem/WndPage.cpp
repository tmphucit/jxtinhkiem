...
bool KWndPageSet::AddPage(KWndPage *pPage, KWndButton *pPageBtn) {
  if (pPage && pPageBtn) {
    AddChild(pPage);
    KWndPageBtnPair *pNewList = (KWndPageBtnPair *)realloc(
        m_pPageBtnPairList, sizeof(KWndPageBtnPair) * (m_nNumPage + 1));
    if (pNewList) {
...
bool KWndPageSet::ActivePage(int nPageIndex) {
  if (nPageIndex >= 0 && nPageIndex < m_nNumPage &&
      nPageIndex != m_nAcitvePage) {
    if (m_nAcitvePage >= 0)
  ...
  for (int i = 0; i < m_nNumPage; i++) {
    if (m_pPageBtnPairList[i].pPageBtn == pBtn) {
      ...
      for (i = 0; i < m_nNumPage; i++)
        m_pPageBtnPairList[i].pPageBtn->CheckButton(
            m_pPageBtnPairList[i].pPageBtn == pBtn);
      break;
    }
  }
}
