...
int KWndScrollBar::WndProc(unsigned int uMsg, unsigned int uParam, int nParam) {
  ...
  case WM_LBUTTONDOWN:
    ...
    break;
  case WM_MOUSEMOVE:
    if ((uParam & MK_LBUTTON) && (m_Flag & WNDSCROLL_F_DRAGGING_SLIDE))
...
void KWndScrollBar::SetScrollPos(int nPosition) {
  if (nPosition != m_nCurValue) {
    ...
    if (nPosition != m_nCurValue) {
      ...
      SetSlideBtnPos();
      if (m_pParentWnd) {
        m_pParentWnd->WndProc(WND_N_SCORLLBAR_POS_CHANGED,
                              (unsigned int)(KWndWindow *)this, m_nCurValue);
      }
    }
  }
}
...
void KWndScrollBar::OnDraggingSlide(int x, int y) {
  ...
  if (nValueRange != 0) {
    if ((m_Flag & WNDSCROLL_ES_VERTICAL) == 0)
      nValue += (x - m_nAbsoluteLeft - m_nMinPosition +
                 nSpaceRange / nValueRange / 2) *
                nValueRange / nSpaceRange;
    else
      nValue += (y - m_nAbsoluteTop - m_nMinPosition +
                 nSpaceRange / nValueRange / 2) *
                nValueRange / nSpaceRange;
  }
  SetScrollPos(nValue);
