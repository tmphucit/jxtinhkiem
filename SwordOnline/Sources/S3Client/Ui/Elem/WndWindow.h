...
#define WND_S_SIZE_WITH_ALL_CHILD                                              \
  0x00100000 // 以子窗口的范围测试的集合作为自己的范围测试的集合
...
public:
  ...
  void
  GetAllChildLayoutRect(RECT *pRect); // 取得包含所有子窗口分布区域的最小区域
  ...
  virtual KWndWindow *TopChildFromPoint(
      int x, int y); // 得到处于指定坐标位置的最上层窗口，传入的坐标为绝对坐标
