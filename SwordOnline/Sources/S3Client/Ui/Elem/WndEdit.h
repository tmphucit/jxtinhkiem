...
#define WNDEDIT_ES_CHAR_TYPE_FILTER                                            \
  (WNDEDIT_ES_ENGLISH_AVAILABLE | WNDEDIT_ES_IME_AVAILABLE)
...
private:
  int CharFromPos(
      int x, int y); // retrieves information about the character closest to a
                     // specified point in the client area of an edit control
  void UpdateData();
  void FmtForShow();
  int Paste();
