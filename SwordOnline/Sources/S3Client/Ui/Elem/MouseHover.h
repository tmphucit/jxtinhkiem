...
public:
  ...
  void SetMouseHoverInfo(void *pWnd, int nObj, int x, int y, bool bHeadTailImg,
                         bool bFollowCursor);
  void SetMouseHoverTitle(const char *pcszTitleText, int nTitleTextLen,
                          UINT uColor);
  void SetMouseHoverProp(const char *pcszPropText, int nPropTextLen,
                         UINT uColor);
  void SetMouseHoverDesc(const char *pcszDescText, int nDescTextLen,
                         UINT uColor);
...
private:
  ...
  bool
      m_bFollowCursor; // 是否跟踪鼠标指针的位置，一起移动，（同时没有横宽最短限制）
  BOOL m_bShow;
};
