...
#define WNDLIST_ES_HALIGN_FILTER                                               \
  (WNDLIST_ES_HALIGN_RIGHT | WNDLIST_ES_HALIGN_CENTRE)
...
public:
  ...
  // 设置列表显示的内容
  void SetContent(BYTE *pContent, int nNumItem, int nItemSize,
                  int nStringOffset);
...
private:
  ...
  unsigned int
      m_uSelItemBgColor; // 选中项的背景色，如果值为0xffffffff表示无背景色
  ...
  unsigned int
      m_uNumSelStatusList; // m_upItemSelStatusList包含的unsigned int空间的数目
};