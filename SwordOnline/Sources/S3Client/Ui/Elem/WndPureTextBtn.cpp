...
KWndPureTextBtn::KWndPureTextBtn() {
  ...
  memset(&m_Item, 0, sizeof(ChatItem)); //
  m_bHaveItem = false;                  //
  m_X = 0;
  m_Y = 0;
}
...
void KWndPureTextBtn::PaintWindow() {
  ...
  if (m_Flag & WNDPTB_F_BE_PRESSEDDOWN || m_Flag & WNDPTB_F_CHECK) {
...
//	功能：窗口函数（处理消息）
//--------------------------------------------------------------------------
int KWndPureTextBtn::WndProc(unsigned int uMsg, unsigned int uParam,
                             int nParam) {
  if (IsDisable())
  ...
  case WM_LBUTTONUP:
    if (m_Flag & WNDPTB_F_BE_PRESSEDDOWN) {
      ...
      if (m_bHaveItem && g_pCoreShell) // click vao item tren kenh chat
      {
        if (KUiChatItem::GetIfVisible())
...
// dinh mau chu~
void KWndPureTextBtn::SetColor(unsigned int uNormal, unsigned int uNormalBorder,
                               unsigned int uOver, unsigned int uOverBorder,
                               unsigned int uSel, unsigned int uSelBorder) {
  m_NormalColor = uNormal;
  m_NormalBorderColor = uNormalBorder;
  m_OverColor = uOver;
  m_OverBorderColor = uOverBorder;
  m_PressedColor = uSel;
  m_PressedBorderColor = uSelBorder;
}
// Ken Nguyen released
void KWndPureTextBtn::SetItemInfo(ChatItem *pItem) {
  if (!pItem)
    return;
  memcpy(&m_Item, pItem, sizeof(ChatItem));
}