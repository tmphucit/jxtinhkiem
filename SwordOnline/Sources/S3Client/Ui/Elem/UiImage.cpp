...
unsigned int IR_GetCurrentTime() { return l_Time; }

//--------------------------------------------------------------------------
//	功能：更新图形换帧计算用时钟
//--------------------------------------------------------------------------
void IR_UpdateTime() { l_Time = GetTickCount(); }
...
void IR_UpdateImagePart(KUiImagePartRef &Img, int nPartValue, int nFullValue) {
  ...
  if (nPartValue < 0) {
    Img.oImgLTPos.nX = Img.oImgLTPos.nY = Img.oImgRBPos.nX = Img.oImgRBPos.nY =
        0;
    return;
  }
...
int IR_GetSpritePixcelAlpha(KUiImageRef &Img, int h, int v) {
  if (g_pRepresentShell)
    return g_pRepresentShell->GetImagePixelAlpha(Img.szImage, Img.nFrame, h, v,
                                                 Img.nType);
  return 0;
}
