...
void KWndObjectBox::SetContainerId(int nId) { m_nContainerId = nId; }
...
//	功能：清除对象物品
//--------------------------------------------------------------------------
void KWndObjectBox::Celar() { m_Object.uGenre = CGOG_NOTHING; }
...
//	功能：设置容纳的对象
//--------------------------------------------------------------------------
void KWndObjectBox::HoldObject(unsigned int uGenre, unsigned int uId,
                               int nDataW, int nDataH) {
  ...
  if (g_MouseOver.IsMoseHoverWndObj(this, 0)) {
    if (m_Object.uGenre != CGOG_NOTHING) {
      ...
      SetMouseHoverObjectDesc(this, 0, m_Object.uGenre, m_Object.uId,
                              m_nContainerId, x, y);
    } else {
...
void KWndObjectBox::PaintWindow() {
  ...
  if (m_Object.uGenre != CGOG_NOTHING && g_pRepresentShell) {
    ...
    if (m_Style & OBJCONT_F_MOUSE_HOVER)
      ...
      ITEM_IN_ENVIRO_PROP eProp =
          (ITEM_IN_ENVIRO_PROP)g_pCoreShell->GetGameData(
              GDI_ITEM_IN_ENVIRO_PROP, (unsigned int)&Obj, 0);
      ...
      else if (eProp == IIEP_SHOP)
        Shadow.Color.Color_dw = l_BgColors[5];
    } else if (m_Object.uGenre == CGOG_ITEM &&
               g_pCoreShell->GetGameData(GDI_ITEMDATAINFO, m_Object.uId, 2) ==
                   0) {
    ...
    if (Shadow.Color.Color_dw) {
      ...
      if (m_Object.uGenre == CGOG_ITEM) {
        if (g_pCoreShell->GetGameData(GDI_ITEMDATAINFO, m_Object.uId, 1) == 1) {

          int Time_Paint = timeGetTime() / 15;
          int Color_Exp = 0;

          if (Time_Paint % 180 < 90) {
            Color_Exp = (Time_Paint % 180) / 3;
          } else {
            Color_Exp = (180 - Time_Paint % 180) / 3;
          }

          KRURect rect;
          rect.Color.Color_b.a = 255;
          rect.Color.Color_b.b = 0;
          rect.Color.Color_b.g = 60 + Color_Exp;
          rect.Color.Color_b.r = 60 + Color_Exp;
          ...
          int x_star = rect.oPosition.nX;
          int x_finish = rect.oEndPos.nX;
          int y_star = rect.oPosition.nY;
          int y_finish = rect.oEndPos.nY;
          int a_rect = x_finish - x_star;
          int b_rect = y_finish - y_star;
          int p_rect = 2 * (a_rect + b_rect);
          int x_point = 0;
          int y_point = 0;
          ...
          if (p_rect != 0) {
            ...
            for (int i = 0; i < 104; i++) {
              if (i < 52) {
                x_point = x_star + (i + Time_Paint / 3 + 175) % p_rect;
                y_point = y_star;
              } else {
                x_point = x_star + (i + Time_Paint / 3) % p_rect;
                y_point = y_star;
              }

              if (x_point <= x_finish) {
              } else if (x_point <= (x_finish + b_rect)) {
                y_point = y_star + x_point - x_finish;
                x_point = x_finish;
              } else if (x_point <= (x_finish + b_rect + a_rect)) {
                x_point = x_star + a_rect - (x_point - (x_finish + b_rect));
                y_point = y_finish;
              } else {
                y_point =
                    y_star + b_rect - (x_point - (x_finish + b_rect + a_rect));
                x_point = x_star;
              }

              point[i].oPosition.nX = x_point;
              point[i].oPosition.nY = y_point;
              point[i].Color.Color_b.a = 255;
              point[i].Color.Color_b.b = 0;
              point[i].Color.Color_b.g = 80 + i % 52 * 175 / 51;
              point[i].Color.Color_b.r = 80 + i % 52 * 175 / 51;
            }
            g_pRepresentShell->DrawPrimitives(104, point, RU_T_POINT, true);
          }
        }
        ...
        if (g_pCoreShell->GetGameData(GDI_ITEMDATAINFO, m_Object.uId, 5) >=
          ...
          KRURect rect;
          rect.Color.Color_b.a = 255;
          rect.Color.Color_b.b = 60 + Color_Exp;
          rect.Color.Color_b.g = 0;
          rect.Color.Color_b.r = 60 + Color_Exp;
          rect.oPosition.nX = m_nAbsoluteLeft;
          rect.oPosition.nY = m_nAbsoluteTop;
          rect.oEndPos.nX = m_nAbsoluteLeft + m_Width;
          ...
          if (p_rect != 0) {
            ...
            for (int i = 0; i < 104; i++) {
              ...
              point[i].oPosition.nY = y_point;
              point[i].Color.Color_b.a = 255;
              point[i].Color.Color_b.b = 80 + i % 52 * 175 / 51;
              point[i].Color.Color_b.g = 0;
              point[i].Color.Color_b.r = 80 + i % 52 * 175 / 51;
            }
            g_pRepresentShell->DrawPrimitives(104, point, RU_T_POINT, true);
          }
    ...
    g_pCoreShell->DrawGameObj(m_Object.uGenre, m_Object.uId, m_nAbsoluteLeft,
                              m_nAbsoluteTop, m_Width, m_Height, 0);
  }
}
...
int KWndObjectBox::WndProc(unsigned int uMsg, unsigned int uParam, int nParam) {
      ...
      if ((m_Style & OBJCONT_S_DISABLE_PICKPUT) == 0) {
        ...
        if (m_nContainerId == UOC_EQUIPTMENT) {
          if (GetKeyState(VK_CONTROL) & 0x8000) {
            ChatItem CItem;
            if (g_pCoreShell->GetGameData(GDI_GET_ITEM_PARAM,
                                          (unsigned int)&CItem, m_Object.uId)) {
        ...
        Obj = m_Object;
        m_pParentWnd->WndProc(WND_N_LEFT_CLICK_ITEM, (unsigned int)&Obj,
                              (int)(KWndWindow *)this);
      } else if (m_Style & OBJCONT_S_ENABLE_CLICK_EMPTY) {
        m_pParentWnd->WndProc(WND_N_LEFT_CLICK_ITEM, NULL,
                              (int)(KWndWindow *)this);
      }
    }
    break;
  case WM_RBUTTONDOWN:
    if (m_pParentWnd) {
      if (m_Object.uGenre != CGOG_NOTHING) {
        KUiDraggedObject Obj;
        Obj = m_Object;
        m_pParentWnd->WndProc(WND_N_RIGHT_CLICK_ITEM, (unsigned int)&m_Object,
                              (int)(KWndWindow *)this);
      } else if (m_Style & OBJCONT_S_ENABLE_CLICK_EMPTY) {
        m_pParentWnd->WndProc(WND_N_LEFT_CLICK_ITEM, NULL,
                              (int)(KWndWindow *)this);
  ...
  case WM_MOUSEMOVE:
    ...
    if (m_Object.uGenre != CGOG_NOTHING &&
        g_MouseOver.IsMoseHoverWndObj(this, 0) == 0)
      SetMouseHoverObjectDesc(this, 0, m_Object.uGenre, m_Object.uId,
                              m_nContainerId, LOWORD(nParam), HIWORD(nParam));
    break;
...
int KWndObjectBox::DropObject(bool bTestOnly) {
  ...
  if (m_uAcceptableGenre != CGOG_NOTHING) {
    if ((m_uAcceptableGenre != DragObj.uGenre) &&
            ((m_uAcceptableGenre & 0xFFFF) != (DragObj.uGenre & 0xFFFF)) ||
        ((m_uAcceptableGenre & 0xFFFF0000) != 0))
      return false;
  }
  if (bTestOnly)
    return true;
...
void KWndObjectMatrix::SetContainerId(int nId) { m_nContainerId = nId; }

KWndObjectMatrix::~KWndObjectMatrix() { Clear(); }
...
void KWndObjectMatrix::PaintWindow() {
  ...
  for (int i = 0; i < m_nNumObjects; i++) {
    ...
    Shadow.oPosition.nX =
        m_nAbsoluteLeft + m_nUnitWidth * pObj->DataX + m_nUnitBorder;
    Shadow.oPosition.nY =
        m_nAbsoluteTop + m_nUnitHeight * pObj->DataY + m_nUnitBorder;
    if (Shadow.Color.Color_dw) {
      ...
      if (pObj->uGenre == CGOG_ITEM) {
        if (g_pCoreShell->GetGameData(GDI_ITEMDATAINFO, pObj->uId, 1) == 1) {
          ...
          rect.oPosition.nY =
              m_nAbsoluteTop + m_nUnitHeight * pObj->DataY + m_nUnitBorder;
          rect.oEndPos.nX = rect.oPosition.nX + width;
          rect.oEndPos.nY = rect.oPosition.nY + height;
          g_pRepresentShell->DrawPrimitives(1, &rect, RU_T_RECT, true);
    ...
    g_pCoreShell->DrawGameObj(pObj->uGenre, pObj->uId, Shadow.oPosition.nX,
                              Shadow.oPosition.nY, width, height, 0);
    if (pObj->uGenre == CGOG_ITEM) {
      if (g_pCoreShell->GetGameData(GDI_ITEMDATAINFO, pObj->uId, 2) == 4 &&
          g_pCoreShell->GetGameData(GDI_ITEMDATAINFO, pObj->uId, 3) == 1) {

        int nFontSize = 12;
        int nNum = g_pCoreShell->GetGameData(GDI_ITEMDATAINFO, pObj->uId, 4);
        char szNum[4];
        int nLen = sprintf(szNum, "%d", nNum);
        szNum[3] = 0;
        g_pRepresentShell->OutputText(
            nFontSize, szNum, KRF_ZERO_END,
            Shadow.oPosition.nX + width - nLen * (nFontSize + 1) / 2,
            Shadow.oPosition.nY + height - nFontSize - 1,
            GetColor("255,246,108"));
      }
    }
  }
  if (m_nPutPosX >= 0) {
    Shadow.oPosition.nX =
        m_nAbsoluteLeft + m_nUnitWidth * m_nPutPosX + m_nUnitBorder;
    Shadow.oPosition.nY =
        m_nAbsoluteTop + m_nUnitHeight * m_nPutPosY + m_nUnitBorder;
    Shadow.oEndPos.nX =
        Shadow.oPosition.nX + m_nUnitWidth * m_nPutWidth - m_nUnitBorder * 2;
    Shadow.oEndPos.nY =
        Shadow.oPosition.nY + m_nUnitHeight * m_nPutHeight - m_nUnitBorder * 2;
    Shadow.Color.Color_dw = l_BgColors[4];
    g_pRepresentShell->DrawPrimitives(1, &Shadow, RU_T_SHADOW, true);
  }
}
...
int KWndObjectMatrix::AddObject(KUiDraggedObject *pObject, int nCount) {
  if (pObject && nCount > 0) {
    ...
    if (nValidCount) {
      KUiDraggedObject *pNewList = (KUiDraggedObject *)realloc(
          m_pObjects, sizeof(KUiDraggedObject) * (m_nNumObjects + nValidCount));
      if (pNewList) {
        m_pObjects = pNewList;
        for (i = 0; i < nCount; i++) {
          if (pObject[i].uGenre) {
...
//	功能：窗口函数
//--------------------------------------------------------------------------
int KWndObjectMatrix::WndProc(unsigned int uMsg, unsigned int uParam,
                              int nParam) {
  switch (uMsg) {
  case WM_LBUTTONDOWN:
    if ((m_Style & OBJCONT_S_DISABLE_PICKPUT) == 0) {
      // dinh' item vao khung soan thao chat kplayerbar
      if (m_nContainerId == UOC_ITEM_TAKE_WITH ||
      ...
      if (uParam & MK_SHIFT) {
        ...
        if (nObj >= 0) {
          KUiDraggedObject Obj;
          Obj = m_pObjects[nObj];
          m_pParentWnd->WndProc(WND_N_LEFT_CLICK_SHIFT_ITEM, (unsigned int)&Obj,
                                (int)(KWndWindow *)this);
        }
        break;
      }
  ...
  case WM_RBUTTONDOWN:
    ...
    if (m_pParentWnd) {
      ...
      if (nObj >= 0) {
        KUiDraggedObject Obj;
        Obj = m_pObjects[nObj];
        m_pParentWnd->WndProc(WND_N_RIGHT_CLICK_ITEM, (unsigned int)&Obj,
                              (int)(KWndWindow *)this);
      }
    }
    break;
...
//	功能：尝试放置物品
//--------------------------------------------------------------------------
int KWndObjectMatrix::TryDropObjAtPos(const RECT &dor,
                                      KUiDraggedObject *&pOverlaped) {
  pOverlaped = NULL;
  for (int i = 0; i < m_nNumObjects; i++) {
    KUiDraggedObject *pHolded = &m_pObjects[i];
...
//	功能：放下物品
//--------------------------------------------------------------------------
void KWndObjectMatrix::DropObject(int x, int y,
                                  KUiDraggedObject *pToPickUpObj) {
