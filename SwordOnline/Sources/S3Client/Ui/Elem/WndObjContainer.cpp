...
void KWndObjectBox::PaintWindow()
{
	...
	if (m_Object.uGenre != CGOG_NOTHING && g_pRepresentShell)
	{
		...
		else if (m_Object.uGenre == CGOG_ITEM && g_pCoreShell->GetGameData(GDI_ITEMDATAINFO,m_Object.uId, 2) == 0)
		{
...
{
y_point = y_star + b_rect - (x_point - (x_finish+b_rect+a_rect));
x_point = x_star;
}
...
rect.Color.Color_b.g = 60 + Color_Exp;
rect.Color.Color_b.r = 60 + Color_Exp;
rect.oPosition.nX = m_nAbsoluteLeft + m_nUnitWidth * pObj->DataX + m_nUnitBorder;
rect.oPosition.nY = m_nAbsoluteTop + m_nUnitHeight * pObj->DataY + m_nUnitBorder;
...
else if (g_pCoreShell->GetGameData(GDI_ITEMDATAINFO,pObj->uId, 1) == 2)
{
...
if (pObj->uGenre == CGOG_ITEM)
{
if (g_pCoreShell->GetGameData(GDI_ITEMDATAINFO,pObj->uId, 2) == 4 && g_pCoreShell->GetGameData(GDI_ITEMDATAINFO,pObj->uId, 3) == 1)
{
...
int nNum = g_pCoreShell->GetGameData(GDI_ITEMDATAINFO,pObj->uId, 4);
			...
			g_pRepresentShell->OutputText(nFontSize, szNum, KRF_ZERO_END,
				...
				Shadow.oPosition.nY + height - nFontSize - 1, GetColor("255,246,108"));
...
int KWndObjectMatrix::WndProc(unsigned int uMsg, unsigned int uParam, int nParam)
{
	...
	case WM_LBUTTONDOWN:
		...
		{
		//dinh' item vao khung soan thao chat kplayerbar
		if(m_nContainerId == UOC_ITEM_TAKE_WITH   || m_nContainerId == UOC_STORE_BOX)
		{
		if(GetKeyState(VK_CONTROL) & 0x8000)
			{
		...
		if (uParam & MK_SHIFT)
		{
			...
			if (nObj >= 0)
			{
				KUiDraggedObject	Obj;
				Obj = m_pObjects[nObj];
				m_pParentWnd->WndProc(WND_N_LEFT_CLICK_SHIFT_ITEM,
					(unsigned int)&Obj, (int)(KWndWindow*)this);
			}
			break;	
		}
