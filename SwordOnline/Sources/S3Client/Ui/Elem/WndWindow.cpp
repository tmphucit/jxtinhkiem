...
//	功能：显示窗口
//--------------------------------------------------------------------------
void KWndWindow::Show() { m_Style |= WND_S_VISIBLE; }
...
//	功能：判断窗口是否被显示
//--------------------------------------------------------------------------
int KWndWindow::IsVisible() { return (m_Style & WND_S_VISIBLE); }
...
void KWndWindow::Destroy() { m_Style |= WND_S_TOBEDESTROY; }
...
void KWndWindow::AddChild(KWndWindow *pChild) {
  if (pChild) {
    ...
    // 调整绝对坐标
    pChild->AbsoluteMove(
        m_nAbsoluteLeft + pChild->m_Left - pChild->m_nAbsoluteLeft,
...
void KWndWindow::AddBrother(KWndWindow *pBrother) {
  if (pBrother) {
    ...
    // 调整绝对坐标
    if (m_pParentWnd) {
      pBrother->AbsoluteMove(m_pParentWnd->m_nAbsoluteLeft + pBrother->m_Left -
                                 pBrother->m_nAbsoluteLeft,
                             m_pParentWnd->m_nAbsoluteTop + pBrother->m_Top -
                                 pBrother->m_nAbsoluteTop);
    }
...
int KWndWindow::WndProc(unsigned int uMsg, unsigned int uParam, int nParam) {
  ...
  case WM_LBUTTONUP:
    if (m_bMoving) {
      ...
      m_bMoving = false;
      if (m_pParentWnd) {
        m_pParentWnd->WndProc(WND_N_CHILD_MOVE,
...
void KWndWindow::OnMoveWnd() {
  ...
  if (x != m_nLastMouseHoldPosX || y != m_nLastMouseHoldPosY) {
    ...
    if (x && m_pParentWnd) {
      m_pParentWnd->WndProc(WND_N_CHILD_MOVE, (unsigned int)(KWndWindow *)this,
                            x);
    }
  }
}
...
void KWndWindow::PaintDebugInfo() {
...
#ifdef _DEBUG
  sprintf(szInfo, "Name:%s, Pos:%d,%d,Size:%d,%d", m_Caption, m_nAbsoluteLeft,
          m_nAbsoluteTop, m_Width, m_Height);
  nInfoLen = strlen(szInfo);
#else
  sprintf(szInfo, "Pos:%d,%d,Size:%d,%d", m_nAbsoluteLeft, m_nAbsoluteTop,
          m_Width, m_Height);
  nInfoLen = strlen(szInfo);
#endif
  if (m_Style & WND_S_SIZE_WITH_ALL_CHILD) {
    ...
    sprintf(&szInfo[nInfoLen], ",Rect:%d,%d-%d,%d", rc.left, rc.top, rc.right,
            rc.bottom);
    nInfoLen = strlen(szInfo);
  }
  int x, y, w, h;
  ...
  if (y + 25 > h)
    ...
    y += 12;
  g_pRepresentShell->OutputText(12, szInfo, nInfoLen, x, y, 0xFFFF0000, 0,
                                TEXT_IN_SINGLE_PLANE_COORD, 0xffffffff);
}
...
#ifdef _DEBUG
    g_pRepresentShell->OutputText(12, m_Caption, -1, m_nAbsoluteLeft,
                                  m_nAbsoluteTop, 0xFFFFFFFF);
#endif
  }
}
