...
// nParam = 取值为WND_OPER_RESULT_0或者WND_OPER_RESULT(n),
// n指代任意一个整数值，其含义依据具体的界面应用自己确认
...
#define WND_N_LEFT_CLICK_SHIFT_ITEM WND_M_BASE + 21
...
#define WND_N_SCORLLBAR_POS_CHANGED                                            \
  WND_M_SCORLLBAR_BASE + 1 // 滚动条滑块位置发生变化
...
#define WND_N_EDIT_SPECIAL_KEY_DOWN                                            \
  WND_M_EDIT_BASE + 2 // 文本编辑窗口获得特定的按键操作
//(KWndWindow*)uParam = 文本编辑框窗口的指针
// nParam = 按键的visual key code
// 特定按键包括
// 回车键、Esc键、制表符tab键、向上键头键、向下键头键、前翻页键，后翻页键
...
// nParam = 新选择的项的索引
#define WND_N_LIST_ITEM_ACTIVE                                                 \
  WND_M_LIST_BASE + 2 // 列表框选中项被激活（点左键或按enter）
...
//(KWndWindow*)uParam = 列表框窗口的指针
// nParam = 右键点击的项的索引
#define WND_N_LIST_ITEM_HIGHLIGHT                                              \
  WND_M_LIST_BASE + 5 // 列表框选鼠标指针指向的项发生变换
//(KWndWindow*)uParam = 列表框窗口的指针
