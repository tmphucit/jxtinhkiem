...
//	功能：构造函数
//--------------------------------------------------------------------------
KWndImagePart::KWndImagePart() { IR_InitUiImagePartRef(m_Image); }
...
int KWndImagePart::Init(KIniFile *pIniFile, const char *pSection) {
  if (KWndWindow::Init(pIniFile, pSection)) {
    ...
    if (m_Image.nDivideFashion < IDF_LEFT_TO_RIGHT ||
        m_Image.nDivideFashion > IDF_BOTTOM_TO_TOP)
...
void KWndImagePart::PaintWindow() {
  ...
  if (g_pRepresentShell) {
    ...
    if (m_Image.szImage[0] == 0 && WND_SHOW_DEBUG_FRAME_TEXT) {
      g_pRepresentShell->OutputText(16, m_Caption, -1, m_nAbsoluteLeft + 1,
                                    m_nAbsoluteTop + 1, 0xFF000000);
      g_pRepresentShell->OutputText(16, m_Caption, -1, m_nAbsoluteLeft,
