...
struct KPopupMenuData {
  ...
  unsigned short
      usMenuFlag; // 菜单特性的一些标记，为枚举类型POPUPMENU_FLAG取值的组合
...
#define MENU_DATA_SIZE(nNumItem)                                               \
  (sizeof(KPopupMenuData) + (nNumItem - 1) * sizeof(KPopupMenuItem))

class KWndWindow;
class KPopupMenu {
public:
  static void Popup(KPopupMenuData *pMenu, KWndWindow *pCaller,
                    unsigned int uParam);
  static void Cancel();
  static void PaintMenu();
  static void LoadTheme(const char *pScheme); // 载入界面方案
  static int HandleInput(unsigned int uMsg, unsigned int uParam,
                         int nParam); // 窗口函数
