...
unsigned int KWndButton::ms_uBtnTipTextColor =
    0xffffffff;                            // 按钮提示名称文字的颜色
...
void KWndButton::Clone(KWndButton *pCopy) {
  if (pCopy) {
    ...
    pCopy->m_nDownNextFrame = m_nDownNextFrame;
...
int KWndButton::Init(KIniFile *pIniFile, const char *pSection) {
  if (KWndImage::Init(pIniFile, pSection)) {
    ...
    m_nDownFrame = nValue;
    pIniFile->GetInteger(pSection, "DownNext", 0, &nValue);
    m_nDownNextFrame = nValue;
    ...
    pIniFile->GetInteger(pSection, "CheckBoxRec", 0, &m_bCheckBoxRec);

    char Buff[16];
    pIniFile->GetString(pSection, "CheckBoxRecColor", "", Buff, 16);
    m_CheckBoxRecColor = GetColor(Buff);
...
//	功能：按钮是否出于按下状态
//--------------------------------------------------------------------------
int KWndButton::IsButtonChecked() { return (m_Flag & WNDBTN_F_CHECKED); }
...
int KWndButton::IsButtonActive() { return (m_Flag & WNDBTN_F_OVER); }
...
int KWndButton::WndProc(unsigned int uMsg, unsigned int uParam, int nParam) {
  ...
  case WM_MOUSEMOVE:
    ...
    if ((m_Flag & WNDBTN_F_OVER) == 0 && m_pPressedDownBtn != this) {
      if (m_pParentWnd)
        m_pParentWnd->WndProc(WND_N_BUTTON_OVER,
  ...
  case WND_M_MOUSE_LEAVE:
    ...
    break;
  case WM_MOUSEHOVER:
    if (m_pPressedDownBtn == this && (m_Flag & WNDBTN_ES_SEND_HOLD_MSG) &&
        m_pParentWnd) {
      m_pParentWnd->WndProc(WND_N_BUTTON_HOLD, (unsigned int)(KWndWindow *)this,
                            0);
      ...
      g_MouseOver.SetMouseHoverInfo((void *)(KWndWindow *)this, 0,
                                    LOWORD(nParam), HIWORD(nParam), false,
                                    true);
...
void KWndButton::PaintWindow() {
  ...
  if (m_bCheckBoxRec && g_pRepresentShell && m_CheckBoxRecColor) {

    if (IsButtonChecked()) {
      KRURect Rect;
      Rect.Color.Color_dw = m_CheckBoxRecColor;
  ...
  if (WND_SHOW_DEBUG_FRAME_TEXT && g_pRepresentShell &&
    ...
    g_pRepresentShell->OutputText(16, "[C]", 3, x + 1, m_nAbsoluteTop + 1,
                                  0xFF000000, 0);
    g_pRepresentShell->OutputText(16, "[C]", 3, x, m_nAbsoluteTop, 0xFFFFFFFF,
                                  0);
  }
#endif
}
...
void KWndButton::OnLBtnDown(bool bDoubleClick) {
  ...
  if ((m_Flag & WNDBTN_ES_CHECKBOX) == 0) {
    ...
    SetFrame(m_nDownFrame);
    if (m_pParentWnd) {
      m_pParentWnd->WndProc(bDoubleClick ? WND_N_BUTTON_DB_CLICK
                                         : WND_N_BUTTON_DOWN,
                            (unsigned int)(KWndWindow *)this, 0);
    }
  } else {
    if (m_Flag & WNDBTN_F_CHECKED) {
      ...
      m_Flag |= WNDBTN_F_CHECKED;
      SetFrame((m_Flag & WNDBTN_ES_ANIMATION) ? m_nCheckOverFrame
                                              : m_nDownFrame);
    }
    OnButtonClick();
    if (m_pParentWnd)
      m_pParentWnd->WndProc(WND_N_BUTTON_CLICK,
                            (unsigned int)(KWndWindow *)this,
                            (m_Flag & WNDBTN_F_CHECKED));
  }
}
...
KWndImageTextButton::KWndImageTextButton() { bPart = true; }
...
void KWndImageTextButton::Set2IntText(int nNumber1, int nNumber2,
                                      char Separator) {
