...
void Wnd_RenderWindows() {
  ...
  if (s_WndStation.DragInfo.bDragging && s_WndStation.DragInfo.fnDrawObjFnc) {
    int h, v;
    s_WndStation.Cursor.GetPosition(h, v);
    if (s_WndStation.DragInfo.fnDrawObjFnc(
            h, v, s_WndStation.DragInfo.DraggedObj,
...
void Wnd_ShowHideGameSpace(bool bShow) { s_WndStation.bPaintGameSpace = bShow; }
...
//	功能：开始拖动物体
//--------------------------------------------------------------------------
int Wnd_DragBegin(const KUiDraggedObject *pObj,
                  fnDrawDraggedObj fnDrawObjFunc) {
  // 原来有个对象正在被拖动
...
//	功能：结束拖动状态
//--------------------------------------------------------------------------
void Wnd_DragFinished() { s_WndStation.DragInfo.bDragging = false; }
...
void Wnd_ProcessInput(unsigned int uMsg, unsigned int uParam, int nParam) {
  ...
  if ((uMsg >= WM_MOUSEFIRST && uMsg <= WM_MOUSELAST) ||
...

    {
      ...
      if (pActiveWnd)
        pTopWnd = pActiveWnd->TopChildFromPoint(
            x, y); // 只传给位于鼠标指针下方的最上层的窗口
      if (pTopWnd != s_WndStation.pMouseOverWnd) {
        ...
        if (pTopWnd) {
          ...
          if (pOverParent) {
            pOverParent->WndProc(WND_N_MOUSE_ENTER_CHILD, (unsigned int)pTopWnd,
                                 0);
          }
        }
    ...
    if (s_WndStation.pFocusWnd &&
        s_WndStation.pFocusWnd->WndProc(uMsg, uParam, nParam))
      return;
    if (s_WndStation.pExclusiveWnd[0]) {
...
void Wnd_TransmitInputToGameSpace(unsigned int uMsg, unsigned int uParam,
                                  int nParam) {
  s_WndStation.pGameSpaceWnd->WndProc(uMsg, uParam, nParam);
}
...
// 改变鼠标指针显示状态
void Wnd_ShowCursor(int bShow) { s_WndStation.Cursor.Show(bShow); }
...
//	功能：获取当前挂接的输入焦点窗口
//--------------------------------------------------------------------------
KWndWindow *Wnd_GetFocusWnd() { return s_WndStation.pFocusWnd; }
...
void Wnd_SetExclusive(KWndWindow *pWnd) {
  ...
  if (pWnd) {
    if (s_WndStation.pFocusWnd &&
        s_WndStation.pFocusWnd->GetOwner() != pWnd->GetOwner())
      Wnd_SetFocusWnd(NULL);
...
// 还原鼠标图形
void Wnd_RestoreCursor() { s_WndStation.Cursor.RestoreCursor(); }