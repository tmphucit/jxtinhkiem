  ...
  if (m_pText && g_pRepresentShell) {
    if (pText && pText[0]) {
      ...
      if (m_Style & WNDTEXT_ES_MULTILINE) {
        int nLineLen;
        m_nLineCount = TGetEncodedTextLineCount(m_pText, m_nTextLen,
                                                (m_Width * 2) / m_nFontSize,
                                                nLineLen, m_nFontSize);
      }
    } else {
      m_pText[0] = 0;
      m_nTextLen = 0;
...
void KWndText::PaintWindow() {
  ...
  g_pRepresentShell->OutputRichText(m_nFontSize, &Param, m_pText, m_nTextLen,
                                    m_Width);
}
...
//	功能：设置文本文字颜色
//--------------------------------------------------------------------------
void KWndText::SetTextColor(unsigned int Color) { m_TextColor = Color; }
...
void KWndText::Set2IntTextExp(char AmDuong, int nNumber1, int nNumber2,
  ...
  int nLen;
  if (m_pText) {
    nLen = sprintf(Buff, "%c%d%c%d", AmDuong, nNumber1, Separator, nNumber2);
    SetText(Buff, nLen);
  }
}
