...
KInlinePicSink::KInlinePicSink() { m_pRepShell = NULL; }
...
#define CHAR_BUFFER_LEN 32
  ...
  if (!ini.Load("\\Ui\\ChatPics.ini"))
    return FALSE;
  ini.GetString("Main", "Path", "\\spr\\Ui\\聊天\\表情", szPath,
                CHAR_BUFFER_LEN);
...
long KInlinePicSink::AddCustomInlinePic(unsigned short &rwIndex,
                                        const char *szSprPathName) {
  KUiImageRef CurrentImg;
  KImageParam Param;
  int i = 0, nSize = 0;
