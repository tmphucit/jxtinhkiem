...
//	功能：析构函数
//--------------------------------------------------------------------------
KWndList2::~KWndList2() { ResetContent(); }
...
int KWndList2::AddString(int nIndex, const char *pString,
                         unsigned int nItemColor) {
  if (pString == NULL)
    return WNDLIST_ERROR;
  if (m_nItemSpace >= m_nNumItem) {
    void **pNew =
        (void **)realloc(m_pContent, sizeof(void *) * (m_nItemSpace + 10));
    if (pNew) {
      m_pContent = pNew;
      m_nItemSpace += 10;
    } else
  ...
  KWndList2Item *pItem =
      (KWndList2Item *)malloc(sizeof(KWndList2Item) + nStrLen);
  if (pItem) {
    pItem->Data = 0;
    pItem->Color = nItemColor;
