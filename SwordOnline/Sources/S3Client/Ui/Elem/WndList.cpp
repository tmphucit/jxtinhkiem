...
KWndList::~KWndList() { EnableMultiSel(0); }
...
int KWndList::Init(KIniFile *pIniFile, const char *pSection) {
  if (KWndWindow::Init(pIniFile, pSection)) {
    ...
    if (pIniFile->GetString(pSection, "SelItemBgColor", "", Buff, 16) &&
        Buff[0])
...
void KWndList::EnableMultiSel(int bEnable) {
  if (bEnable && IS_MULTI_SEL == 0) {
    ...
    if (m_nNumItem > 32) {
      ...
      m_puItemSelStatusList =
          (unsigned int *)malloc(sizeof(unsigned int) * m_uNumSelStatusList);
      if (m_puItemSelStatusList)
        memset(m_puItemSelStatusList, 0,
               sizeof(unsigned int) * m_uNumSelStatusList);
      else {
        m_uNumSelStatusList = 0;
        m_Style &= ~WNDLIST_ES_MULTI_SEL;
      }
...
int KWndList::GetIemIndexAtPoint(int x, int y) {
  int nSel =
      m_nTopItemIndex + (y - m_nAbsoluteTop) / (m_nFontSize + 1) * m_nNumColumn;
  if (m_nNumColumn > 1) {
...
void KWndList::UpdateData() {
  ...
  if (m_nTopItemIndex > m_nSelItemIndex)
    ...
    m_nTopItemIndex =
        m_nNumColumn * (m_nSelItemIndex / m_nNumColumn + 1) - nVisibleItem;
...
void KWndList::PaintWindow() {
  ...
  if (g_pRepresentShell) {
    ...
    for (i = m_nTopItemIndex, nColumn = 0; i < EndIndex; i++, nColumn++) {
      ...
      const char *pShowString = TGetLimitLenString(
          (const char *)(&pItem[m_nStringOffset]), -1, szBuffer, nMaxLen);
      ...
      g_pRepresentShell->OutputText(m_nFontSize, pShowString, nLen, nPaintX, y,
                                    uColor, 0, TEXT_IN_SINGLE_PLANE_COORD,
                                    uBorderColor);
...
//	功能：设置列表内容
//--------------------------------------------------------------------------
void KWndList::SetContent(BYTE *pContent, int nNumItem, int nItemSize,
                          int nStringOffset) {
...
int KWndList::FindString(int nPrecedingStart, const char *pString) {
  if (pString) {
    ...
    for (int i = nPrecedingStart + 1; i < m_nNumItem;
         i++, pItem += m_nItemSize) {
...
int KWndList::GetItemDataInt(int nItemIndex, int nDataOffset) {
  if (nItemIndex >= 0 && nItemIndex < m_nNumItem && nDataOffset >= 0 &&
      ((int)(nDataOffset + sizeof(int)) <= m_nItemSize)) {
...
void KWndList::GetString(int nItemIndex, char *pBuffer) {
  if (pBuffer) {
    if (nItemIndex >= 0 && nItemIndex < m_nNumItem) {
      strcpy(pBuffer,
             (char *)&m_pContent[m_nItemSize * nItemIndex + m_nStringOffset]);
    } else
      pBuffer[0] = 0;
  }
}
