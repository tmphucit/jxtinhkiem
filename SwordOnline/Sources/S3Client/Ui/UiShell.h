...
void UiProcessInput(unsigned int uMsg, unsigned int uParam,
                    int nParam);        // 处理输入
...
class Player_AT : public KWndButton {
public:
  DECLARE_COMCLASS(Player_AT)
  void OnButtonClick();
  void UpdateData();
  const char *GetShortKey();
};

class Player_KTC : public KWndButton {
public:
  DECLARE_COMCLASS(Player_KTC)
  void OnButtonClick();
  void UpdateData();
  const char *GetShortKey();
};
