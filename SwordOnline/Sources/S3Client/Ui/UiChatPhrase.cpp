...
#define EMOTE_INI_FILE "\\Ui\\UiChatPrase.ini"
...
int KUiChatPhrase::GetPhraseCount() { return MAX_PHRASE_COUNT; }

int KUiChatPhrase::SavePrivateSetting(KIniFile *pFile, LPCSTR lpSection,
                                      int nStart) {
  int i = nStart;
  char szKey[10];
  char szLine[256];
  for (; i - nStart < MAX_PHRASE_COUNT; i++) {
    sprintf(szKey, "%d", i);
    sprintf(szLine, "SetPhrase(%d, \"%s\")", i - nStart,
            m_PhraseList[i - nStart].szString);
 ...
 *************************************************************************/
int KUiChatPhrase::SetEmote(char *szCmd, char *szName, char *szStringTarget,
                            char *szStringSelf, int nIndex) {
 ...
 * 功能：载入数据的文件
 *************************************************************************/
int KUiChatPhrase::HandleLoad(KIniFile *pIni, EMOTE_PHRASE *pData, int nCount,
  ...
  if (pData) {
    for (i = 0; i < nCount; i++) {
      ...
      pData[i].nStringLen =
          TEncodeText(pData[i].szString, strlen(pData[i].szString));

      sprintf(szKey, "%d_StringOnlyMe", i);
      pIni->GetString(szSection, szKey, "", pData[i].szStringMe,
                      sizeof(pData[i].szStringMe));

      pData[i].nStringMeLen =
          TEncodeText(pData[i].szStringMe, strlen(pData[i].szStringMe));
...
int KUiChatPhrase::ConstructFunctionAlias() {
  ...
  for (i = 0; i < m_nMenuEmoteCount; i++) {
    ...
    KShortcutKeyCentre::RegisterFunctionAlias(m_pMenuEmote[i].szCmd, "SayEmote",
                                              3, PmList);
  }
  for (i = 0; i < m_nEmotePhraseCount; i++) {
    ...
    KShortcutKeyCentre::RegisterFunctionAlias(m_pEmotePhrase[i].szCmd,
                                              "SayEmote", 3, PmList);
  }
 ...
 *       这里是包括了MENU和PHRASE的
 *************************************************************************/
int KUiChatPhrase::GetEmote(const char *szCmd, char *pBuff, int nBuffLen,
                            int nType) {
  int nIndex;
...
int KUiChatPhrase::GetEmote(int nIndex, char *pBuff, int nBuffLen, int nType) {
  if (nIndex >= 0 && nIndex < (m_nEmotePhraseCount + m_nMenuEmoteCount) &&
    ...
    if (nIndex < m_nMenuEmoteCount) {
      ...
      if (nType == 0) {
        memcpy(pBuff, m_pEmotePhrase[nIndex].szString,
               m_pEmotePhrase[nIndex].nStringLen);
        return m_pEmotePhrase[nIndex].nStringLen;
      } else {
        memcpy(pBuff, m_pEmotePhrase[nIndex].szStringMe,
               m_pEmotePhrase[nIndex].nStringMeLen);
 ...
 ***************************************************************************/
int KUiChatPhrase::GetMenuEmote(const char *szCmd, char *pBuff, int nBuffLen,
                                int nType) {
 ...
 ***************************************************************************/
int KUiChatPhrase::GetMenuEmote(int nIndex, char *pBuff, int nBuffLen,
                                int nType) {
  if (nIndex >= 0 && nIndex < m_nMenuEmoteCount && m_pMenuEmote &&
      nBuffLen >= m_pMenuEmote[nIndex].nStringLen) {
    if (nType == 0) {
      memcpy(pBuff, m_pMenuEmote[nIndex].szString,
             m_pMenuEmote[nIndex].nStringLen);
      return m_pMenuEmote[nIndex].nStringLen;
    } else {
      memcpy(pBuff, m_pMenuEmote[nIndex].szStringMe,
             m_pMenuEmote[nIndex].nStringMeLen);
 ...
 * 功能：制作字符串
 *************************************************************************/
int KUiChatPhrase::ConvertEmoteString(char *szString, int nStringLen,
                                      const char *szMyName,
