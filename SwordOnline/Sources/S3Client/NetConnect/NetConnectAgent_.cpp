...
void KNetConnectAgent::Breathe(DWORD nGameCounter)
{
	...
	{
//		MessageBox(0,"tan cong roi 007 !","",0);
		g_bDisconnect = true;
	...
	{
		m_bTobeDisconnect = false;
//		MessageBox(0,"tan cong roi 006 !","",0);
...
{
//		MessageBox(0,"tan cong roi 008 !","",0);
	    g_bDisconnect = true;
		m_bTobeDisconnect = false;
		DisconnectClient();
		DisconnectGameSvr();
...
///////
		...
		while (true)
		{
			...
			while(pMsg < (PROTOCOL_MSG_TYPE*)(pBuffer + nSize))
			{
				...
				{
//					MessageBox(0,"tan cong roi 001 !","",0);
...
MessageBox(0,"hehehe tan cong roi !","",0);
	...
	{
//		MessageBox(0,"tan cong 2 !","",0);
		// 开始与GameSvr进行连接
		g_pCoreShell->OperationRequest(GOI_EXIT_GAME, 0, 0);
		if (ConnectToGameSvr((const unsigned char*)&dwIp, nPort, &guid) || ConnectToGameSvr((const unsigned char*)&dwIp1, nPort1, &guid))
		{
//			MessageBox(0,"tan cong 3 !","",0);
