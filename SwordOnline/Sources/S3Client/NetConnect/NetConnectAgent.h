...
public:
  ...
  // 建立连接
  int ClientConnectByNumericIp(const unsigned char *pIpAddress,
                               unsigned short pszPort);
  // 关闭连接
  void DisconnectClient();

  int ConnectToGameSvr(const unsigned char *pIpAddress, unsigned short uPort,
                       GUID *pGuid);
  ...
  void UpdateClientRequestTime(bool bCancel,
                               unsigned int uTimeLimit = DEF_TIMEOUT_LIMIT);

  // 注册抵达消息响应函数
  void RegisterMsgTargetObject(PROTOCOL_MSG_TYPE Msg,
