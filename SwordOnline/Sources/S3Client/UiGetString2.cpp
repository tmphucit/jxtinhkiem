...
#define SCHEME_INI "ÊäÈë×Ö´®½çÃæ.ini"

extern iRepresentShell *g_pRepresentShell;
extern iCoreShell *g_pCoreShell;
KUiGetString *KUiGetString::m_pSelf = NULL;

KUiGetString::KUiGetString() { m_pRequester = NULL; }

//--------------------------------------------------------------------------
//	¹¦ÄÜ£º´ò¿ª´°¿Ú£¬·µ»ØÎ¨Ò»µÄÒ»¸öÀà¶ÔÏóÊµÀý
//--------------------------------------------------------------------------
KUiGetString *KUiGetString::OpenWindow(GET_STRING_ACTION eAction,
                                       const char *pszTitle,
                                       const char *pszInitString,
                                       KWndWindow *pRequester,
                                       unsigned int uParam, int nMaxNum,
...
void KUiGetString::OnSend() {
  ...
  if (nLen >= m_nMinLen && nLen <= m_nMaxLen) {
    ...
    UIMessageBox(((nLen < m_nMinLen) ? "V¨n tù nhËp vµo qu¸ ng¾n"
                                     : "V¨n tù nhËp vµo qu¸ dµi"));
  }
}
