...
int get_account(DB *db, const DBT *pkey, const DBT *pdata,
                DBT *ikey) { // 给定一个完整的buffer，得到account作为索引
...
S3DBI_API int S3DBI_SaveRoleInfo(IN BYTE *pRoleBuffer, IN char *strUser,
  ...
  if (strUser[0] == 0)
    strUser = pRoleData->BaseInfo.szName;
  if (db_table->add(strUser, strlen(strUser) + 1, (char *)pRoleBuffer,
                    pRoleData->dwDataLen))
    return 1;
  else
    return 0;
...
S3DBI_API int S3DBI_GetRoleListOfAccount(char *szAccountName,
  ...
  int count = 0;
  char *buffer =
      db_table->search(szAccountName, strlen(szAccountName) + 1, size, 0);
  while (buffer) {
    if (count < nMaxCount) {
...
// 删除角色
S3DBI_API int S3DBI_DeleteRole(char *strUser) { return 1; }

S3DBI_API void *S3DBI_GetList(S3DBI_LISTNAME listname, char *strUser) {
  return 0;
}

S3DBI_API void *S3DBI_GetList(S3DBI_LISTNAME listname, int iid) { return 0; }
...
S3DBI_API int S3DBI_DeleteData(S3_DB_TABLENAME tablename, int nId) { return 0; }
