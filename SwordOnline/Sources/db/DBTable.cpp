...
ZDBTable::ZDBTable(const char *path, const char *name) {
  ...
  /*#ifdef WIN32
          ...
          if(!db_env_create(&dbenv, 0)) {
                          ...
                          if(!dbenv->open(dbenv, env_path, DB_CREATE |
  DB_INIT_LOG | DB_INIT_LOCK | DB_INIT_MPOOL | DB_INIT_TXN | DB_RECOVER, 0)) {
      ...
      for (index = 0; index < index_number; index++) {
        ...
        if (!db_create(&index_db[index], dbenv, 0)) {
          ...
          if (index_db[index]->open(index_db[index], NULL, index_table_name,
            ...
            break;
          if (primary_db->associate(primary_db, NULL, index_db[index],
                                    get_index_funcs[index], 0)) {
