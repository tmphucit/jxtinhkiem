...
pktru = GetPK()-1
if GetTask(4) > 0 then
	SayNew("<color=green>Qu¶n Ngôc<color>: B¹n ®ang bÞ t¹m giam bëi Admin. Thêi gian cßn l¹i: <color=red>"..GetTask(4).."<color> phót ",3,
...
if GetTaskTemp(142) == 0 then
	...
	SayNew("<color=green>Qu¶n Ngôc<color> §iÓm PK hiÖn t¹i cña b¹n lµ: <color=red>"..GetPK().."<color> ®iÓm. B¹n sÏ mÊt 2 giê ë ®©y ®Ó gi¶m 1 ®iÓm PK",3,
	"B¾t ®Çu ngåi nhµ lao /batdau",
	"Ta muèn ra ngoµi/rangoai",
	"Thoat/no")
	else
	SayNew("<color=green>Qu¶n Ngôc<color> B¹n ®ang trong giai ®o¹n x¸m hèi. Thêi gian ®Ó gi¶m ®iÓm PK xuèng <color=red>"..pktru.."<color> lµ: <color=yellow>"..GetTask(142).."<color> phót",2,
...
thoigian = nam * 1000000 + thang * 10000 + ngay * 100 + gio
tinhtime = GetTask(916) - thoigian

if tinhtime > 0 then
Talk(1,"","Thêi gian ngåi tï cßn l¹i: "..tinhtime.." giê, hÕt thêi gian míi cã thÓ ra ngoµi !")
return
end

tien = GetPK()*100000
if tien > 0 then
SayNew("<color=green>Qu¶n Ngôc<color>: LÖ phÝ ra khái nhµ lao lµ: : <color=yellow>"..tien.."<color> l­îng. B¹n sÏ bÞ reset thêi gian ngåi tï. B¹n cã ch¾c ch¾n kh«ng? ",2,
"Ta ®ång ý tr¶ lÖ phÝ ®Ó ra khái ®©y/rangoai2",
"Ta sÏ tiÕp tôc ngåi nhµ lao/no")
else
SayNew("<color=green>Qu¶n Ngôc<color>: B¹n cÇn <color=yellow>10 v¹n<color> l­îng ®Ó ra khái nhµ lao ?",2,
