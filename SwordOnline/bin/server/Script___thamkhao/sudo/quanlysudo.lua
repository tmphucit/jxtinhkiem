...
SayNew("Ng­¬i muèn ta gióp g× ?",8,
"Th¨ng chøc Vâ S­ /thangchuc",
"Ta muèn nhËn §Ö Tö /nhandetu",
"Hç trî nh©n ®«i kinh nghiÖm /hotronhandoi",
"Ta muèn nhËn phÇn th­ëng Vâ S­ /nhanthuongkinhnghiem",
...
for i=1,GetCountPlayerMax() do
	...
	if GetTeam() == team and GetName() ~= name then
		...
		if GetLevel() >= 120 then
			...
			Msg2Player("B¹n nhËn ®­îc 5 ThÇn BÝ §¹i Hång Bao")
			...
			Talk(1,"","§¼ng cÊp cña ng­¬i ch­a ®¹t 120, kh«ng thÓ xuÊt s­ ")
			checktientrinh = 3
		end
	end
end
...
Msg2Player("B¹n nhËn ®­îc 10 ThÇn BÝ §¹i Hång Bao")
...
Talk(1,"","Ng­¬i kh«ng mang ®ñ 5 Kim Nguyªn B¶o, kh«ng thÓ t¨ng sè l­îng")
return
end

if getn(DE_TU[vtsp]) >= 10 then
Talk(1,"","Danh s¸ch ®Ö tö ®· ®¹t ®ñ sè l­îng giíi h¹n: 5 ng­êi, kh«ng thÓ t¨ng thªm !")
return
end
DelItem(17)
DelItem(17)
...
function nhandetuxx()
Talk(1,"","TÝnh n¨ng nhËn ®Ö tö t¹m khãa tõ ngµy 24/04/2018 - 17h00 ngµy 05/05/2018")
end
function nhandetu()
if GetTeamSize() ~= 2 then
Talk(1,"","Ng­¬i chØ ®­îc lËp tæ ®éi 2 ng­êi cïng víi ®Ö tö mµ m×nh muèn nhËn ")
...
for i=1,GetCountPlayerMax() do
	...
	if GetTeam() == team and GetName() ~= name then
		if GetLevel() < 95 or GetLevel() >= 120 then
			Talk(1,"","§¼ng cÊp 95 - 119 míi cã thÓ b¸i s­ !")
			kiemtratientrinh = 3
			break
		end
		if GetTask(139) == 0 then
			Talk(1,"","Nh©n vËt chuyÓn sinh 0, kh«ng thÓ b¸i s­ ")
			kiemtratientrinh = 4
			break
		end
		vtdetu1,vtdetu2 = checkdetu()
...
Talk(1,"","Ng­¬i nµy cã ®¼ng cÊp 120 trë lªn hoÆc 95 trë xuèng, kh«ng thÓ nhËn")
...
function thangchuc()
if GetTask(139) == 0 then
Talk(1,"","C¸c h¹ chuyÓn sinh 0, kh«ng thÓ lµm Vâ S­ ")
return
end

if GetLevel() < 120 then
Talk(1,"","VÞ ®¹i hiÖp nµy ch­a ®¹t ®¼ng cÊp 120, kh«ng thÓ lµm Vâ S­ ")
return
end

if GetItemCount(17) < 3 then
Talk(1,"","PhÝ th¨ng cÊp Vâ S­: <color=yellow>3 Kim Nguyªn B¶o<color>. Vui lßng chuÈn bÞ ®ñ råi quay l¹i gÆp ta")
return
end
...
Msg2SubWorld("<color=pink>Chóc mõng "..GetName().." tö h«m nay chÝnh thøc ®­îc lµm Vâ S­, cã thÓ thu nhËn 1 ®Ö tö ")
SaveBang()
end
