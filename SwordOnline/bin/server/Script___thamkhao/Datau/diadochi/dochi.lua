...
if GetTeam() == nil then
	if GetTask(84) == 1 and GetTask(85) == 1 and GetTask(88) > 0 then -- Dang nhan nhiem vu do chi
		if w == DoChi_Map[GetTask(88)][1] then
			...
			Msg2Player("B¹n nhËn ®­îc 1 tÊm ThÇn Bİ §å Chİ. Sè l­îng  thu thËp hiÖn t¹i lµ <color=yellow>"..GetTask(87).." / "..GetTask(86).." tÊm. ")
			if GetTask(87) > GetTask(86) then
				SetTask(89,GetTask(89)+1)
				if (mod(GetTask(89),10) == 0) then
				Msg2Player("B¹n nhËn ®­îc 1 m·nh b¶n ®å SHXT, sè l­îng hiÖn t¹i lµ: "..floor(GetTask(89)/10).." tÊm")
			end
			end
		else
			SetTask(89,GetTask(89)+1)
