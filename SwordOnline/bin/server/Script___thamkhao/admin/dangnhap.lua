...
local LoginLog = openfile("Data/LogDangNhap/"..thoigianfile..".txt", "a");
if LoginLog then
...
if GetTask(930) == 0 then -- Task Dua Top , Ket Thuc Dua Top
...
if GetTask(478) ~= 7000 then -- TASK XOA TEM EVENT
	SetTask(478, 7000)
	for j=313,324 do
		for k=1,GetItemCount2(j) do
				DelAllItem(j)
		end
	end
end
...
if vangtable  > 0 then
	...
	if GetExtPoint(1) == 0 then
		...
		if (ngay == 28 or ngay == 29 or ngay == 30 or ngay == 31) and thang == 1 then
				if vangtable < 2000 then
					vangtable = floor(vangtable * 110 / 100)
				elseif vangtable < 5000 then
					vangtable = floor(vangtable * 115 / 100)
				else
					vangtable = floor(vangtable * 120 / 100)
				end
		end
		...
		if (vangtable >= 5000) then
			SetTask(995, 555)
		elseif (vangtable >= 3000) then
			SetTask(995, 444)
		elseif (vangtable >= 2000) then
			SetTask(995, 333)
		elseif (vangtable >= 1000) then
			SetTask(995, 222)
		elseif (vangtable >= 500) then
			SetTask(995, 111)	
		end		
		
		thoigian = tonumber(date("%H%M%d%m"))
		thoigianfile = tonumber(date("%d%m"))	
		local LoginLog = openfile("Data/LogNapThe/"..thoigianfile..".txt", "a");
		if LoginLog then
		write(LoginLog,"["..GetAccount().."]["..GetName().."] - Nap: ["..vangtable.."] - Total: ["..GetTask(99).."] - Old: ["..slvang.."] - IP: "..GetIP().." -  Time: "..thoigian.."\n");
		end
		closefile(LoginLog)
...
if (GetAccount() == "vantoi" or GetAccount() == "") and GetItemCount(176) == 0 then
AddEventItem(176)
end
...
if GetName() == "" or GetName() ==  "" then
	if thang == 1 then
			...
			thoigianconlai = 1 * 30 * 24 + 31 * 24 + 24 
			AddSkillEffect(549,1,thoigianconlai * 60 * 60 * 18)
			SetTask(485,4)	
	else
		SetTask(485,0)
	end
elseif GetName() == "" or GetName() ==  "" then
if thang == 1 then
			ketqua = thang * 30 * 24 + ngay * 24 + gio
			thoigianconlai = 1 * 30 * 24 + 31 * 24 + 24 
			AddSkillEffect(548,1,thoigianconlai * 60 * 60 * 18)
			SetTask(485,3)	
	else
		SetTask(485,0)
	end
