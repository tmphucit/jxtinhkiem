...
Include("\\script\\loidai\\dsloidai.lua")
	...
	if chenhlech < GO_TIME then
		...
		for i=1,GetCountPlayerMax() do
			...
			if w == 209 then
				...
				Msg2Player("Thêi gian b¸o danh l«i ®µi cßn l¹i: <color=yellow>"..(GO_TIME-chenhlech).." phót <color=red>n÷a !")
			end
		end
		PlayerIndex = idxp
	elseif chenhlech == GO_TIME then
		...
		for i=1,GetCountPlayerMax() do
			...
			if w == 209 and GetCurCamp() ~= 0 then
				...
				Msg2Player("§· hÕt thêi gian b¸o danh l«i ®µi, trËn ®Êu sÏ b¾t ®Çu sau <color=yellow>1 phót <color=red>n÷a.")
			end
		end
		PlayerIndex = idxp
		szTeamName[3][3] = 2
	...
	if chenhlech == GO_TIME + 1 then
		...
		for i=1,GetCountPlayerMax() do
			...
			if w == 209 and GetCurCamp() ~= 0 then
				SetFightState(1)
				SetPunish(1);
				Msg2Player("<color=yellow>Thi ®Êu chÝnh thøc b¾t ®Çu. Thêi gian thi ®Êu: 10 phót.")
			end
		end
		PlayerIndex = idxp
		szTeamName[3][3] = 3
...
function bw_state_joined_in(Camp)
	...
	SetCreateTeam(0);
	SetRevPos(15,10)
	SetDeathScript("\\script\\loidai\\player_death.lua");
