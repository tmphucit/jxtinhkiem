...
if GetItemCount(iditem) >= 1 then
	...
	if GetItemCount(iditem) == (n-1) then
			...
			if GetGlbMissionV(37) == 8888 then
					...
					AddGlobalCountNews("Chóc mõng "..GetName().." ®· më R­¬ng HiÖp Cèt nhËn ®­îc <color=yellow>"..GetNameItemBox(idxitem).."",1)
					Msg2SubWorld("Chóc mõng "..GetName().." ®· më R­¬ng HiÖp Cèt nhËn ®­îc: "..GetNameItemBox(idxitem).."")
					return
			end
