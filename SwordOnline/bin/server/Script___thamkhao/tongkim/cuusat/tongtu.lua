		...
		--if kill_dh - die_dh < 2 then
			if CheckTrungNhanVat(OrgPlayer) == 0 then
				...
				--end
						...
						Msg2Player("<color=yellow>B¹n ®· ®¸nh träng th­¬ng Tèng Binh "..danhhieu[die_dh][1].." "..die_name.." nhËn ®­îc "..tongdiem.." ®iÓm tÝch luü, liªn tr¶m "..GetTask(21)..", sè ng­êi giÕt ®­îc "..GetTask(11)..", tÝch luü hiÖn t¹i "..GetTask(31).." ®iÓm")
						UpdateTopTKNew()
			else
				Msg2Player("<color=yellow>HÖ thèng chèng Post §iÓm: trïng nh©n vËt")
			end
	--	else
	--		Msg2Player("<color=yellow>B¹n võa tiªu diÖt 1 ng­êi thua "..(kill_dh - die_dh).." cÊp bËc danh hiÖu, kh«ng nhËn ®­îc phÇn th­ëng")
	--	end
	end
...
s = tonumber(date("%S"))
	...
	UpdateTopTKNew()
				...
				if LoginLog then
				write(LoginLog," [KIMKILL] ["..giet.."] [DH: "..kill_dh.."] [TL: "..kill_tichluy.."]      -     [TONGDIE] ["..GetName().."] [Level: "..GetLevel().."] [DH: "..GetTask(19).."] [TL: "..GetTask(31).."]  - IP: "..GetIP().." -  Time: "..thoigian.."\n");
				end
				closefile(LoginLog)	
end
end
