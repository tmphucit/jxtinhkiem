...
vt = checkds()
if vt == 0 then
Talk(1,"","B¹n kh«ng cã tªn trong danh s¸ch ®Òn bï, nÕu cã sai xãt vui lßng liªn hÖ Fanpage ®Ó chóng t«i kiÓm tra l¹i")
return
end


if DENBU[vt][2] == 0 then
				...
				Msg2Player("B¹n nhËn ®­îc ®Òn bï: 1 ThÇn Hµnh Phï [7 ngµy] vµ 1 Tói M¸u VIP [7 ngµy]")
				BANG = TaoBang(DENBU,"DENBU")			
				LuuBang("script/danhsach/danhsachdenbu.lua",BANG)
else
	Talk(1,"","B¹n ®· nhËn ®Òn bï vµo lóc: "..DENBU[vt][2].."")
end
end
...
for k=675,676 do
	num = GetItemCount(k)
	if num > 1 then
		for i=1,num do DelItem(k) end
		for i=1,num do AddEventItem(k) end
	end
end
...
elseif GetTask(485) == 1 then
	...
	if GetTask(484) >= diem then
			if GetItemCount(17) >= 1 then
				...
				AddOwnExp(20000000)
				Msg2Player("B¹n nhËn ®­îc 3 Thñy  Tinh + 20.000.000 kinh nghiÖm")
				Msg2SubWorld("<color=green>Chóc mõng "..GetName().." ®· th¨ng cÊp lªn VIP "..GetTask(485).."")
				inlogvip("Thang Cap VIP "..GetTask(485).." - "..GetAccount().." - "..GetName().."")
			else
				Talk(1,"","Ng­¬i kh«ng mang theo 1 Kim Nguyªn B¶o, kh«ng thÓ th¨ng cÊp")
...
elseif GetTask(485) == 2 then
	...
	if GetTask(484) >= diem then
			if GetItemCount(17) >= 1 then
				...
				SetTask(196,0)
				AddOwnExp(30000000)
				Msg2Player("B¹n nhËn ®­îc 1 LÇn TÈy Tñy MiÔn PhÝ + 30.000.000 kinh nghiÖm")
				Msg2SubWorld("<color=green>Chóc mõng "..GetName().." ®· th¨ng cÊp lªn VIP "..GetTask(485).."")
				inlogvip("Thang Cap VIP "..GetTask(485).." - "..GetAccount().." - "..GetName().."")
			else
				Talk(1,"","Ng­¬i kh«ng mang theo 1 Kim Nguyªn B¶o, kh«ng thÓ th¨ng cÊp")
...
elseif GetTask(485) == 3 then
	...
	if GetTask(484) >= diem then
			if GetItemCount(17) >= 1 then
				...
				AddOwnExp(40000000)
				Msg2Player("B¹n nhËn ®­îc 5 BÝ Kip Th¨ng CÊp An Bang CÊp 2 + 40.000.000 kinh nghiÖm")
				Msg2SubWorld("<color=green>Chóc mõng "..GetName().." ®· th¨ng cÊp lªn VIP "..GetTask(485).."")
				inlogvip("Thang Cap VIP "..GetTask(485).." - "..GetAccount().." - "..GetName().."")
			else
				Talk(1,"","Ng­¬i kh«ng mang theo 1 Kim Nguyªn B¶o, kh«ng thÓ th¨ng cÊp")
...
elseif GetTask(485) == 4 then
	...
	if GetTask(484) >= diem then
			if GetItemCount(17) >= 1 then
				...
				SetTask(485 ,5)
				AddOwnExp(50000000)
				Msg2Player("B¹n nhËn ®­îc Vßng S¸ng VIP 5 + 50.000.000 kinh nghiÖm")
				Msg2SubWorld("<color=green>Chóc mõng "..GetName().." ®· th¨ng cÊp lªn VIP "..GetTask(485).."")
				inlogvip("Thang Cap VIP "..GetTask(485).." - "..GetAccount().." - "..GetName().."")
			else
				Talk(1,"","Ng­¬i kh«ng mang theo 1 Kim Nguyªn B¶o, kh«ng thÓ th¨ng cÊp")
...
if ngay == 2 and thang == 1 then
	...
	"§æi 2.000.000 kinh nghiÖm [1000 ®iÓm]/doidiemkinhnghiem",
	"1 LÖnh Bµi 1000 TÝch Lòy Tèng Kim [1200 ®iÓm]/lbtichluy",
	"Tho¸t./no")
else
SayNew("<color=green>Kim Qu©n Nhu: <color> B¹n ®ang cã: \n+ <color=yellow>"..GetTask(15).."<color> ®iÓm tÝch luü Tèng Kim.\nNg­¬i muèn ®æi phÇn th­ëng g×?",3,
	--"§æi Tu LuyÖn Ch©u [4000 ®iÓm]/doitongkim1",
	--"§æi Tiªn Th¶o Lé 3 Giê [10000 ®iÓm]/doitongkim1",
	--"§æi N÷ Nhi Hång 3 Giê [10000 ®iÓm]/doitongkim1",
	"§æi 4.000.000 kinh nghiÖm [1000 ®iÓm]/doidiemkinhnghiem",
	"1 LÖnh Bµi 1000 TÝch Lòy Tèng Kim [1200 ®iÓm]/lbtichluy",
	"Tho¸t./no")
end
end
...
if GetTask(15) >= cost then
		...
		if GetTask(15) == task - cost then
			AddEventItem(378)
			Msg2Player("B¹n nhËn ®­îc 1 LÖnh Bµi TÝch Lòy Tèng Kim")
		else
			Talk(1,"","Hack ha em")
		end
else
...
Talk(1,"","B¹n ®· ®æi v­ît giíi h¹n 20 lÇn h«m nay")
return
end
...
function dlrv()
SayNew("<color=green>Tèng Qu©n Nhu: <color> HiÖn t¹i ng­¬i ®ang cã <color=fire>"..GetTask(15).."<color> ®iÓm tÝch luü, tuú ng­¬i sö dông",3,
