...
if gio == 9 or gio == 12 or gio == 15 or gio == 18 or gio == 21 then
	if phut <= 5 then
		...
		SayNew("<color=green>Hoa §¨ng: <color> B¹n ®· tr¶ lêi c©u hái thø: <color=red>"..GetTask(121).."<color> / "..soluonghoadang.." c©u.\nB¹n cã muèn tiÕp tôc kh«ng ?",2,
...
"Thêi gian mçi lÇn ®o¸n hoa ®¨ng lµ 5 phót. Tèi ®a "..soluonghoadang.." c©u 1 ngµy !")
end
end
...
if GetLevel() < 80 then
	...
	Msg2Player("B¹n nhËn ®­îc "..expnhanduoc.." kinh nghiÖm")
	tieptuc()
else
	s = random(1,1000)
	if s < 400 then
