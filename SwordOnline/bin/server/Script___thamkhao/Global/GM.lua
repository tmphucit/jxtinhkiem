...
function system()
	SayNew("GM:",7,
	"Xem Thong Tin Toan Nhan Vat/Show",
	"Tat ca nhan vat tu level(1~100)/Show100",
	"Tat ca nhan vat tu level(101~200)/Show200",
	"Tat ca nhan vat tu level(201~300)/Show300",
	"Tat ca nhan vat tu level(301~400)/Show400",
	"Kick va luu tat ca nhan vat dang uy thac/KickNhanVatUyThac",
	"Cap Cao/chaozuo",
	"Thoat/no")
end;
...
function KickNhanVatUyThac()
	for i=1,GetPlayerCount() do
		gmidx=PlayerIndex
		PlayerIndex=i
		local nLixian = GetLiXian()
		if (nLixian == LIXIAN_DOING) then
			local nSaveResult = SaveData2();
			local szName = GetName();
			SetLiXian(LIXIAN_EXIT);
			Msg2Player("Kick nhan vat dang uy thac: "..GetName()..", Save: "..nSaveResult);
			--nen ghi log cho nay de debug
		end
		PlayerIndex=gmidx
	end
end
...
function chaozuo()
SayNew(wel,12,fir,sec,thr,fou,fif,six1,sev,eig,nin,ten,nex,can)
end;

function next()
SayNew(wel,12,fir10,sec10,thr10,fou10,fif10,six10,sev10,eig10,nin10,ten10,nex2,can)
end;

function next2()
SayNew(wel,12,fir20,sec20,thr20,fou20,fif20,six20,sev20,eig20,nin20,ten20,nex3,can)
end;

function next3()
SayNew(wel,11,fir30,sec30,thr30,fou30,fif30,six30,sev30,eig30,nin30,ten30,can)
end;
------------------------------------------------------------------------------------------------------------------------------------------

function first()
SayNew(wel, 11, on, tw, th, fo, fi, si, se, ei, ni, ze, can);
SetTaskTemp(331,0)
end;
...
SayNew(wel1, 10, it, sk,cl,mov,bip,mr,kic, kil,inv, can);
end
end;
...
PlayerIndex=GetTaskTemp(331)
PlayerIndex=gmidx
SayNew("Lua Chon", 7,"1 Cap/tishen1","5 Cap/tishen5","10 Cap/tishen10","20 Cap/tishen20","50 Cap/tishen50","100 Cap/tishen100","Chuyen Sinh./chuyensinh",can);
end;
...
PlayerIndex=GetTaskTemp(331)
PlayerIndex=gmidx
SayNew("Lua chon",5,"+1./taohua","+5./taohua2","+10./taohua3","+20./taohua4",can)
end;
...
PlayerIndex=GetTaskTemp(331)
PlayerIndex=gmidx
SayNew("Nhan vat", 3, "1 Trieu/jinbi1", "10 Trieu/jinbi10", can);
end;
...
PlayerIndex=GetTaskTemp(331)
PlayerIndex=gmidx
SayNewEx({"Lua chon lay trang bi","Tam thoi tat roi./no"})
end;
...
w,x,y=GetWorldPos()
PlayerIndex=gmidx
--SayNew("Nhan vat Nhan vat Nhan vat 型", 6,"Nhan vat Nhan vat /jinyan", "Nhan vat /jiejin", "Nhan vat Nhan vat /invest1", "Nhan vat Nhan vat /invest2", "Nhan vat IPNhan vat /ipxinx", can);
SayNew("Nhan vat Nhan vat Nhan vat 型", 4, "Nhan vat Nhan vat /invest1", "Nhan vat Nhan vat /invest2", "Nhan vat IPNhan vat /ipxinx", can);
...
function nhandoi()
SayNewEx({"Lua chon","Nhan 2./sxsx","Nhan 3./sxsx","Nhan 4./sxsx","Nhan 5./sxsx","Dung lai./sxsx","Thoat./Exit"})
end;
