...
if w == 86 then
	if GetLevel() < 80 then
		Talk(1,"","§¼ng cÊp nhÆt h¹t: > 80")
	return
	end
HideNpc(idx,5*60*18)
Msg2Player("C©y sÏ xuÊt hiÖn l¹i sau 3 phót n÷a !")
elseif w == 87 then
	if GetLevel() < 110 then
		Talk(1,"","§¼ng cÊp nhÆt h¹t: > 110 ")
	return
	end
HideNpc(idx,7*60*18)
Msg2Player("C©y sÏ xuÊt hiÖn l¹i sau 5 phót n÷a !")
elseif w == 92 then
	if GetLevel() < 120  then
		Talk(1,"","§¼ng cÊp nhÆt h¹t: > 120 ")
	return
	end

HideNpc(idx,10*60*18)
Msg2Player("C©y sÏ xuÊt hiÖn l¹i sau 8 phót n÷a !")
elseif w == 91 then
	if GetLevel() < 130 then
		Talk(1,"","§¼ng cÊp nhÆt h¹t: > 130 ")
	return
	end
	if GetPKState() ~= 1 then
		Talk(1,"","Trang th¸i luyÖn c«ng kh«ng nhÆt ®­îc H¹t Hoµng Kim !")
	return
	end
HideNpc(idx,12*60*18)
Msg2Player("C©y sÏ xuÊt hiÖn l¹i sau 10 phót n÷a !")
end
...
a = random(1,4)
if a == 1 then
SayNew("<color=green>"..GetNpcName(idx).."<color>: "..CAUHOI[i][1].."",4,
"A. "..CAUHOI[i][2].."/dung",
"B. "..CAUHOI[i][3].."/sai",
"C. "..CAUHOI[i][4].."/sai",
"D. "..CAUHOI[i][5].."/sai")
...
function inlognl(string)
thoigian = tonumber(date("%H%M%d%m"))
LoginLog = openfile("Data/SuKien/Hack.txt", "a");
if LoginLog then
...
if CheckFreeBoxItem(4,192,1,1) == 0 then
Talk(1,"","Hµnh trang kh«ng cßn chç trèng, ko thÓ nhÆt !")
return
end
sl1 = GetItemCount(189)
sl2 = GetItemCount(190)
sl3 = GetItemCount(191)
sl4 = GetItemCount(192)
w,x,y = GetWorldPos()
if w == 86 then
	AddEventItem(189)
	Talk(1,"","Chóc mõng b¹n ®· tr¶ lêi chÝnh x¸c. B¹n nhËn ®­îc <color=yellow>H¹t May M¾n")
	if GetItemCount(189) ~= sl1 + 1 then
		inlognl(""..GetAccount().." - "..GetName().." da xoa "..GetItemCount(189).." Hat May Man")
		for i=1,2000 do DelItem(189) end
		Talk(1,"","Hack ha em ")
	end
	Talk(0,"")
elseif w == 87 then
	AddEventItem(190)
	Talk(1,"","Chóc mõng b¹n ®· tr¶ lêi chÝnh x¸c. B¹n nhËn ®­îc <color=yellow>H¹t Hoa Hång")
	if GetItemCount(190) ~= sl2 + 1 then
		inlognl(""..GetAccount().." - "..GetName().." da xoa "..GetItemCount(190).." Hat Hoa Hong")
		for i=1,2000 do DelItem(190) end
		Talk(1,"","Hack ha em ")
	end
	Talk(0,"")
elseif w == 92 then
	AddEventItem(191)
	Talk(1,"","Chóc mõng b¹n ®· tr¶ lêi chÝnh x¸c. B¹n nhËn ®­îc <color=yellow>H¹t Thñy Tinh")
	if GetItemCount(191) ~= sl3 + 1 then
		inlognl(""..GetAccount().." - "..GetName().." da xoa "..GetItemCount(191).." Hat Thuy Tinh")
		for i=1,2000 do DelItem(191) end
		Talk(1,"","Hack ha em ")
	end
	Talk(0,"")
elseif w == 91 then
	AddEventItem(192)
	Talk(1,"","Chóc mõng b¹n ®· tr¶ lêi chÝnh x¸c. B¹n nhËn ®­îc <color=yellow>H¹t Hoµng Kim")
	if GetItemCount(192) ~= sl4 + 1 then
		inlognl(""..GetAccount().." - "..GetName().." da xoa "..GetItemCount(192).." Hat Hoang Kim")
		for i=1,2000 do DelItem(192) end
		Talk(1,"","Hack ha em ")
	end
	Talk(0,"")
end
...
function sai()
Talk(1,"","<color=red>B¹n ®· tr¶ lêi sai c©u hái, c©y ®· biÕn mÊt !")
end
...
{"Skill trÊn ph¸i C¸i B+ang tªn g× ? ","Tóy §iÖp Cuång Vò ","Phi Long Tai Thien ","Hoa Diem Vi Di ","Hoat Bat Luu Thu ",1},
...
{"Hoa hång lµ quèc hoa cña n­íc nµo  ","Bungari  ","Hungary  ","PhÇn Lan  ","TÊt c¶ ®Òu sai ",3},
...
{"TuyÕn ®­êng nµo dµi nhÊt n­íc ta  ","Quèc lé 1A  ","§­êng Hå ChÝ Minh  ","Quèc lé 50  ","Quèc lé 51 ",1},
{"Lôc  ®Þa giµ lµ ®Ó ¸m chØ ch©u lôc nµo  ","Ch©u ¢u  ","Ch©u ¸  ","Ch©u Mü  ","Ch©u Phi ",1},
{"Khu du lÞch §¹i Nam thuéc tØnh nµo  ","B×nh D­¬ng  ","Hå ChÝ Minh  ","Vòng Tµu  ","Ikebana ",1},
...
{"Th¸nh ®Þa Mü S¬n thuéc tØnh nµo ","Qu¶ng Nam ","Qu¶ng B×nh ","Qu¶ng TrÞ ","Qu¶ng NgÜa ",2},
