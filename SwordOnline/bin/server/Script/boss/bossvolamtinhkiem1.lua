...
Msg2SubWorld("<color=green>Chóc mõng ®¹i hiÖp ["..GetName().."] ®· tiªu diÖt Boss M¸y Chñ Håi øc vµ nhËn ®­îc:")
	Msg2SubWorld("<color=yellow>- 2 KNB")
	--Msg2SubWorld("<color=yellow>- 1 BÝ KiÕp 120")
	Msg2SubWorld("<color=yellow>- 2 Lam Thñy Tinh")
	Msg2SubWorld("<color=yellow>- 2 Lôc Thñy Tinh")
	Msg2SubWorld("<color=yellow>- 2 Tö Thñy Tinh")
	Msg2SubWorld("<color=yellow>- 1 Vâ L©m MËt TÞch - 1 TÉy Tñy Kinh")
	-- Msg2SubWorld("<color=yellow>- 3 M·nh Ngùa")
	-- Msg2SubWorld("<color=yellow>- 10 Hoµn Mü Tinh Th¹ch")
	 Msg2SubWorld("<color=yellow>- 5 B¶o R­¬ng Håi øc")
	 Msg2SubWorld("<color=yellow>- 10 M¶nh GhÐp Ngùa XÝch Thè")
	 Msg2SubWorld("<color=yellow>- 10 M¶nh GhÐp Ngùa ChiÕu D¹")
	-- Msg2SubWorld("<color=yellow>- 10 Bao L× X× Lín")
	AddEventItem(17)
	AddEventItem(17)
	--AddEventItem(19)
    AddEventItem(1)
...
--if GetTeam() == nill then
	...
	AddOwnExp(50000000)
	Msg2Player("B¹n ®· kÕt thóc Boss, nhËn ®­îc 50.000.000 kinh nghiÖm")
			...
			if GetTongName() == tong then		
				...
				if GetName() ~= name then
					if w == w1 and (x > x1-40 and x < x1+40) and (y > y-80 and y < y+80) then
						AddOwnExp(50000000)
						Msg2Player("Bang cuñ b¹n ë trong khu vùc s¨n boss, nhËn ®­îc 50.000.000 kinh nghiÖm")
					end
				end
			else
				--w,x,y = GetWorldPos()
				if GetName() ~= name then
					if w == w1 and (x > x1-40 and x < x1+40) and (y > y-80 and y < y+80) then
						AddOwnExp(25000000)
						Msg2Player("B¹n ë trong khu vùc s¨n boss, nhËn ®­îc 25.000.000 kinh nghiÖm")
					end
				end
			end
		end
				...
				--if GetTongName() == tong then	
					...
					--if GetName() ~= name then
						--if w == w1 and (x > x1-40 and x < x1+40) and (y > y-80 and y < y+80) then
							--for k=1,20000 do AddOwnExp(5000) end
							--AddOwnExp(50000000)
							--Msg2Player("B¹n thuéc Bang tiªu diÖt Boss, nhËn ®­îc 50.000.000 kinh nghiÖm")
						--end
					--end
				--else
					--if GetName() ~= name then
...
Say("Ta qu¶n lý vËt phÈm s¨n Boss cña Bang Héi: ",8,
...
Say("Ta qu¶n lý vËt phÈm s¨n Boss cña Bang Héi: ",6,
