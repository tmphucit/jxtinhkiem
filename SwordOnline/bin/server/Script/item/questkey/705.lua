...
if GetTask(Task_GH_THUYENRONGNHO) >= GH_BanhDauXanh then
	Talk(1,"","B¹n ®· sö dông tèi ®a "..GH_BanhDauXanh.." sè l­îng Ng­êi tuyÕt choµng kh¨n xanh (th­êng), kh«ng thÓ nhËn thªm !")
	return
	end
		...
		if sxitem == Rate_Item_EventThuong then
				...
				for k=1,getn(MANG_ITEMEVENTTHUONG) do
					if nType < MANG_ITEMEVENTTHUONG[k][1] then
							...
							-- Msg2SubWorld("Chóc mõng <color=green>"..GetName().."<color> sö dông Ng­êi tuyÕt choµng kh¨n xanh (th­êng) nhËn ®­îc <color=yellow>1 "..MANG_ITEMEVENTTHUONG[k][3].."")
							break
					end
				end
		end
