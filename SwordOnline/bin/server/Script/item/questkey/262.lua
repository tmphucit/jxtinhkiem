...
-- if GetLevel() < 200 then
	-- Talk(1,"","Event hÕt h¹n sö dông kh«ng thÓ sö dông !")
	-- return
	-- end
...
Talk(1,"","B¹n ®· sö dông tèi ®a "..GH_BanhThapCam.." sè l­îng ThuyÒn Rång TruyÒn Thèng (®Æc biÖt), kh«ng thÓ nhËn thªm !")
	return
	end
		...
		if sxitem == Rate_Item_ThapCam then
				...
				for k=1,getn(MANG_ITEM) do
					if nType < MANG_ITEM[k][1] then
							...
							Msg2SubWorld("Chóc mõng <color=green>"..GetName().."<color> sö dông ThuyÒn Rång TruyÒn Thèng (®Æc biÖt) nhËn ®­îc <color=yellow>1 "..MANG_ITEM[k][3].."")
							break
					end
				end
		end
