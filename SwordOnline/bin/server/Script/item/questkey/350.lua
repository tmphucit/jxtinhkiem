...
function main(idx)
if GetLevel() < 90 then
	Talk(1,"","Ng­¬i ch­a ®at cÊp 90 kh«ng thÓ sö dông !")
	return
	end
...
Talk(1,"","B¹n ®· sö dông tèi ®a "..GH_BanhThapCam.." sè l­îng B¸nh Ch­ng (®Æc biÖt), kh«ng thÓ nhËn thªm !")
	return
	end
		...
		if sxitem == Rate_Item_ThapCam then
				...
				for k=1,getn(MANG_ITEM) do
					if nType < MANG_ITEM[k][1] then
							...
							Msg2SubWorld("Chóc mõng <color=green>"..GetName().."<color> sö dông B¸nh Ch­ng (®Æc biÖt) nhËn ®­îc <color=yellow>1 "..MANG_ITEM[k][3].."")
							break
					end
				end
		end
