tennpc = "Nga My S­ Th¸i"
tenphai = "Nga My Ph¸i"
nguhanh = 2
tennguhanh = "hÖ thñy "
tenfaction = "Nga My Phai"
idmonphai = 5


function npctalk(id1,id2,id3,id4,id5,id6)
tennpc = id1
tenphai = id2
nguhanh = id3
tennguhanh = id4
	...
	elseif GetFaction() == tenfaction then
			if GetSeries() == nguhanh then
				if GetTask(103) >= 8 then
					JoinFaction(idmonphai,10)
				end
				if GetTask(103) >= 11 then
					JoinFaction(idmonphai,20)
				end
				if GetTask(103) >= 16 then
					JoinFaction(idmonphai,30)
				end
				if GetTask(103) >= 21 then
					JoinFaction(idmonphai,40)
				end
				if GetTask(103) >= 26 then
					JoinFaction(idmonphai,50)
				end
				if GetTask(103) >= 33 then
					JoinFaction(idmonphai,60)
				end
				if GetTask(103) >= 45 then
					JoinFaction(idmonphai,80)
				end


				if GetTask(103) < 9 then
					if GetLevel() >= 10 then
						SayNew("<color=green>"..tennpc.."<color>: Ng­¬i cã muèn tham gia vµo m«n ph¸i <color="..colorS(3)..">"..tenphai.."<color> hay kh«ng?",3,
...
{36,"§Õn Ph­îng T­êng gÆp Chóc Tó Thanh 198/191"},
...
function nhapphai() -- nv lv 10
	if GetTask(103) == 0 then
		SetFaction(tenfaction)
		JoinFaction(idmonphai,1)
		SetTask(103,1)
		SetTask(104,0) -- so luong nhiem vu
		Talk(2,"","<color=green>"..tennpc.."<color>: Tõ h«m nay ng­êi ®· chİnh thøc gia nhËp <color=yellow>"..tenphai.."<color>, h·y cè g¾ng luyÖn tËp nhĞ. ",
		"Lªn <color=yellow>Hoa S¬n C¶nh Kü Ch­ëng<color> tiªu diÖt <color=red>10 con nhİm<color> råi quay l¹i gÆp ta !")
...
if GetTask(103) == 9 then
	SetTask(103,10)
	Talk(2,"","<color=red>"..GetName().."<color>: Ta ®· tu luyÖn bÊy l©u giê muèn n©ng cao søc m¹nh cña m×nh, xin s­ phô chØ b¶o thªm ",
...
elseif GetTask(103) == 19 then
	Talk(1,"","<color=green>"..tennpc.."<color>: Kh¸ l¾m ®Ö tö, h·y nhanh ch©n ®em mËt hµm ®Õn <color=yellow>Ph­îng T­êng<color> ®­a cho <color=red>Tiªu T­íng Quan 217/184")
...
elseif GetTask(103) == 45 then
	...
	"<color=red>"..GetName().."<color>: §a t¹ s­ phô, xin b¸i biÖt !")
