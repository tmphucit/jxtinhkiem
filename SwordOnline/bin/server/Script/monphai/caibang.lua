...
if GetTask(103) >= 1 then
		...
		if HaveMagic(116) < 0 then
		AddMagic(116)
		AddMagic(115)
		end
				end
				if GetTask(103) >= 11 then
		if HaveMagic(129) < 0 then
		AddMagic(129)
		end
				end
				if GetTask(103) >= 16 then
		if HaveMagic(274) < 0 then
		AddMagic(274)
		AddMagic(124)
		end
				end
				if GetTask(103) >= 21 then
		if HaveMagic(277) < 0 then
		AddMagic(277)
		end
				end
				if GetTask(103) >= 26 then
		if HaveMagic(128) < 0 then
		AddMagic(128)
		AddMagic(125)
		end
				end
				if GetTask(103) >= 33 then
		if HaveMagic(130) < 0 then
		AddMagic(130)
		AddMagic(335)
		end
				end
		...
		-- if HaveMagic(362) > 0 then
			-- Talk(1,"","<color=green>"..tennpc.."<color>: Ng­êi ®· häc kü n¨ng 90 m«n ph¸i ta råi mµ !")
			-- return
		-- end
		-- nhiemvu90()
		-- return
...
W,X,Y = GetWorldPos()
	...
	elseif GetFaction() == tenfaction then
			if GetSeries() == nguhanh then
				SayNew("<color=green>"..tennpc.."<color>: Ng­¬i quay l¹i gÆp ta ®Ó lµm g×?",3,
...
function nhapphai() -- nv lv 10
	...
	elseif GetTask(103) == 3 then
		SetTask(103,4)
		Talk(2,"","<color=green>"..tennpc.."<color>: Nhµ ng­¬i còng rÊt kh¸ ®Êy, hi väng ng­¬i sÏ dïng tµi trİ cña m×nh ®Ó lµm r¹ng danh "..tenphai.."",
...
if GetTask(103) == 9 then
	SetTask(103,10)
	Talk(2,"","Ta ®· tu luyÖn bÊy l©u giê muèn n©ng cao søc m¹nh cña m×nh, xin s­ phô chØ b¶o thªm ",
...
if GetTask(103) == 12 then
	Talk(2,"","<color=green>"..tennpc.."<color>: §Õn <color=yellow>T­¬ng D­¬ng Phñ <color>t×m <color=red>Tèng ViÔn Thµnh 191/193<color>",
	" ¤ng ta sÏ kÓ cho ng­êi nghe vÒ chuyÖn mét con HuyÕt Ma Hæ ")
