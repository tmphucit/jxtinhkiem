...
h,m,s = GetTimeNew()
Msg2Player("<color=green>Chó ý: Admin sÏ lu©n phiªn kiÓm tra Post §iÓm Tèng Kim Liªn Tôc. Xö lý: <color=yellow>Block Account<color=green> khi ph¸t hiÖn")
...
if time < GetTask(1) then
	Talk(1,"","<color=red>HÖ Thèng<color>: B¹n ®ang trong thêi gian bÞ cÊm tham gia Tèng Kim. Vui lßng quay l¹i vµo ngµy mai !")
return
end
SetTask(1,0)
	...
	if GetGlbMissionV(60) == 0 then
		...
		if GetTask(123) == GetGlbMissionV(61) and GetTask(518) < 5 then
			if GetTask(12) == 1 then
				SayNew("Trèng trËn ®· rÒn vang! Sao ng­¬i ch­a tham gia chiÕn ?\nHiÖn giê <color=yellow>qu©n sè phe Tèng lµ "..GetGlbMissionV(83).." ng­êi\nqu©n sè phe Kim lµ "..GetGlbMissionV(84).." ng­êi.",2,"H·y ®­a ta trë l¹i chiÕn tr­êng/trolaiTK1","§Ó ta suy nghÜ l¹i/no")
			elseif GetTask(12) == 2 then	
				SayNew("Tªn Kim binh kia, c¶ gan th©m nhËp vµo l·nh ®Þa ®¹i Tèng, râ rµng lµ tù t×m c¸i chÕt!",1,"Tho¸t./no")
				Msg2Player("Ng­¬i ®· ®Çu qu©n cho Kim quèc, h·y ®Õn gÆp Mé binh quan xin nhËp chiÕn tr­êng!")
			else
				SayNew("Ng­¬i ®· tù ý rêi khái chiÕn tr­êng lóc chiÕn sù v× thÕ kh«ng thÓ tiÕp tôc tham gia trËn chiÕn nµy. Vui lßng quay trë l¹i vµo nh÷ng trËn tiÕp theo nhÐ!",1,"Tho¸t./no")
			...
			if GetTask(518) >= 5 then
				SetTask(123,0)
				Msg2Player("<color=pink>B¹n bÞ reset trËn ®Êu h«m nay do bÞ xö lý AFK 5 lÇn")
			end
			SayNew(""..returnType()..", mäi ng­êi h·y v× tö d©n ®¹i tèng, tôc ng÷ cã c©u: Thiªn h¹ h­ng vong, thÊt phu h÷u tr¸ch. Nay ng­êi kim x©m ph¹m bê câi chóng ta, ®©y lµ lóc b¸o hiÕu ®Êt n­íc, nµo ng­êi anh em cßn chÇn chõ g× n÷a!",2,"Ta tham gia! §iÓm tÝch lòy trë vÒ 0/baodanh1","Tho¸t/no")
		end
	end;
end
...
function baodanh1()
	...
	if GetTask(123) ~= GetGlbMissionV(61) then
		SayNew("Trèng trËn ®· rÒn vang! Sao ng­¬i ch­a tham gia chiÕn ?\nHiÖn giê <color=yellow>qu©n sè phe Tèng lµ "..GetGlbMissionV(83).." ng­êi\nqu©n sè phe Kim lµ "..GetGlbMissionV(84).." ng­êi.",2,"H·y cho ta tham gia/thamgiaTK1","§Ó ta suy nghÜ l¹i/no")
	end
	end
end
...
phut = tonumber(date("%M"))
	...
	if GetTask(123) ~= GetGlbMissionV(61) then
		-- if tongkim_checkphe(1) == 1 then
			-- Talk(1,"","IP cña b¹n ®· cã nh©n vËt theo phe Kim, b¹n kh«ng thÓ gia nhËp phe Tèng")
			-- return
		-- end
		-- if tongkim_checksoluong() >= 2 then
			-- Talk(1,"","IP cña b¹n ®· cã 2 nh©n vËt ®¨ng ký Tèng Kim h«m nay, kh«ng thÓ ®¨ng ký thªm !")
			-- return
		-- end
...
Talk(1,"","HiÖn t¹i binh lùc phe ta lµ <color=yellow>"..GetGlbMissionV(83).." ng­êi<color>, ®Þch ph­¬ng lµ <color=yellow>"..GetGlbMissionV(84).." ng­êi<color>, c¸ch biÖt "..slcb.." ng­êi<color>. Qu©n lùc cña ta hiÖn giê ®· d­ søc tiªu diÖt Kim binh! Tr¸ng sÜ xin ®îi trËn sau nhÐ!")
return
end
end
...
Talk(1,"","Sè l­îng qu©n Tèng ®· ®ñ 150 ng­êi, kh«ng thÓ tham gia thªm !")
return
end
...
--	end
--	Msg2Player("B¹n nhËn ®­îc 5 LÔ Bao Ngò Hoa Ngäc Lé Hoµn MiÔn PhÝ !")
	
	Msg2Player("<color=yellow>L­u ý: Vµo AutoPlay -> Tab Tèi ¦u -> Tick vµo Ng­êi Ch¬i ®Ó kh«ng bÞ giÊt khi PK chç ®«ng ng­êi !")
