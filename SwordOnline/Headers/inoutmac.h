...
typedef struct {
  char String[4 * 4];
} K_IP_ADDRESS_STRING, *K_PIP_ADDRESS_STRING, K_IP_MASK_STRING,
    *K_PIP_MASK_STRING;
...
// 此函数会确保两个地址均有
// 返回值为网卡数量
inline int gGetMacAndIPAddress(BYTE *pMacAddressIn, DWORD *pIPAddressIn,
                               BYTE *pMacAddressOut, DWORD *pIPAddressOut,
                               DWORD nMask = 0x0000a8c0, DWORD nMacLength = 6) {
  typedef DWORD(CALLBACK * PGAINFO)(K_PIP_ADAPTER_INFO,
                                    PULONG); // GetAdaptersInfo
  ...
  while (pInfo && (!bIn || !bOut)) {
    ...
    while (pAddTemp) /*遍历IP列表中的每一个元素*/
    {
      ...
      if (!bIn && ((nAddress & 0x0000FFFF) == nMask)) {
        bIn = true;
      ...
      if (!bOut && ((nAddress & 0x0000FFFF) != nMask)) {
        bOut = true;
