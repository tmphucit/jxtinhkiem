...
DEFINE_GUID(IID_IESClient, 0xd38249a9, 0x4565, 0x4336, 0x9f, 0x70, 0x13, 0x74,
            0xf1, 0xe1, 0x58, 0xe7);
...
DECLARE_INTERFACE_(IClient, IUnknown) {
  ...
  (const char *const &pAddressToConnectServer,
   unsigned short usPortToConnectServer) PURE;

  /*
   *
   */
  STDMETHOD(RegisterMsgFilter)
  (LPVOID lpParam, CALLBACK_CLIENT_EVENT pfnEventNotify) PURE;
  /*
   * Send some data to the current server
   */
  STDMETHOD(SendPackToServer)
  (const void *const pData, const size_t &datalength) PURE;

  /*
   * Get some data from the current server
   */
  STDMETHOD_(const void *, GetPackFromServer)
  (size_t & datalength) PURE;
...
DEFINE_GUID(IID_IClientFactory, 0x9b2f150c, 0x3d59, 0x40db, 0xa7, 0xb, 0xe9,
            0x99, 0xd3, 0x83, 0xd0, 0x44);

DECLARE_INTERFACE_(IClientFactory, IUnknown) {
  STDMETHOD(SetEnvironment)
  (const size_t &bufferSize) PURE;
