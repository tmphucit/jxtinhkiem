 ...
 * Please note that if you use STLport iostreams (compiled library) then you
 * have to use consistent settings when you compile STLport library and your
 * project. Those settings are defined in _site_config.h and have to be the same
 * for a given STLport installation.
 *
 */
 ...
 * (e.g libstlport_gcc_stldebug.a) when you set this flag in STLport iostreams
 * mode.
 */
...
/*
 *
 *  _STLP_NO_CUSTOM_IO : define this if you do not instantiate basic_xxx
 * iostream classes  with custom types (which is most likely the case). Custom
 * means types other than char, wchar and char_traits<>, like
 * basic_ostream<my_char_type, my_traits<my_char_type> > When this option is on,
 * most non-inline template functions definitions for iostreams are not seen by
 * the client. Default is off, just not to break compilation for those who do
 * use those types. which saves a lot of compile time for most compilers, also
 * object and executable size for some. That also guarantees that you still use
 * optimized standard i/o when you compile your program without optimization.
 *  Option does not affect STLport library build; you may use the same binary
 * library with and without this option, on per-project basis.
 *
 */
 ...
 * Note : if the compiler do not support namespaces, those operators are not be
 * provided by default, to simulate hiding them into rel_ops. This was proved to
 * resolve many compiler bugs with ambiguity.
 */

// #define _STLP_NO_RELOPS_NAMESPACE 1

/*
 * If _STLP_USE_OWN_NAMESPACE is in effect, STLport will try to rename std:: for
 * the user to _STL::. If you don't want this feature, or if it does not quite
 * work for your setup (like you are using third-party library that has to use
 * vendor's STL), please define the following switch :
 */
// # define _STLP_DONT_REDEFINE_STD 1

/*
 * _STLP_WHOLE_NATIVE_STD : only meaningful in _STLP_USE_OWN_NAMESPACE mode.
 * Normally, STLport only imports necessary components from native std::
 * namespace - those not yet provided by STLport (<iostream>, <complex>, etc.)
 * and their dependencies (<string>, <stdexcept>).
 * You might want everything from std:: being available in std:: namespace when
 * you include corresponding STLport header (like STLport <map> provides
 * std::map as well, etc.), if you are going to use both stlport:: and std::
 * components in your code. Otherwise this option is not recommended as it
 * increases the size of your object files and slows down compilation.
 */
...
/*
 *  This option is for gcc users only and only affects systems where native
 * linker does not let gcc to implement automatic instantiation of static
 * template data members/ It is being put in this file as there is no way to
 * check if we are using GNU ld automatically, so it becomes user's
 * responsibility.
 *
 */
...
/*
 *  Define this macro to disable STLport extensions (for example, to make sure
...
// boris : this setting is here as we cannot detect precense of new Platform SDK
// automatically If you are using new PSDK with VC++ 6.0 or lower, please define
// this to get correct prototypes for InterlockedXXX functions # define
