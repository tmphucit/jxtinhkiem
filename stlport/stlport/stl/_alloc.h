...
#if !defined(_STLP_DEBUG_H) &&                                                 \
    (defined(_STLP_DEBUG) || defined(_STLP_ASSERTIONS))
...
#define __RESTRICT
    ...
    (defined(_STLP_OWN_IOSTREAMS) && !defined(_STLP_NO_THREADS) &&             \
     !defined(_NOTHREADS))
...
template <int __inst> class __malloc_alloc {
private:
...
public:
  ...
  static __oom_handler_type _STLP_CALL
...
template <class _Alloc> class __debug_alloc : public _Alloc {
public:
...
template <bool __threads, int __inst> class __node_alloc {
  _STLP_PRIVATE : static inline size_t _STLP_CALL _S_round_up(size_t __bytes) {
...
public:
  ...
  // __n is permitted to be 0.  The C++ standard says nothing about what the
  // return value is when __n == 0.
  _Tp *allocate(size_type __n, const void * = 0) const {
    return __n != 0
               ? __REINTERPRET_CAST(value_type *, __sgi_alloc::allocate(
  ...
  void construct(pointer __p, const _Tp &__val) const {
    _STLP_STD::_Construct(__p, __val);
  }
...
// The version for the default allocator, for rare occasion when we have partial
// spec w/o member template classes
 ...
 * not using MEMBER_TEMPLATE_CLASSES as it should work given template
 * constructor  */
#if defined(_STLP_MEMBER_TEMPLATES) ||                                         \
    !defined(_STLP_CLASS_PARTIAL_SPECIALIZATION)
/* if _STLP_NO_TEMPLATE_CONVERSIONS is set, the member template constructor is
 * not used implicitly to convert allocator parameter, so let us do it
 * explicitly */
#if defined(_STLP_MEMBER_TEMPLATE_CLASSES) &&                                  \
...
inline _STLP_TYPENAME_ON_RETURN_TYPE _Alloc_traits<_Tp, _Alloc>::allocator_type
    _STLP_CALL
...
inline allocator<_Tp2> &_STLP_CALL __stl_alloc_rebind(allocator<_Tp1> &__a,
                                                      const _Tp2 *) {
  return (allocator<_Tp2> &)(__a);
}
template <class _Tp1, class _Tp2>
inline allocator<_Tp2> _STLP_CALL __stl_alloc_create(const allocator<_Tp1> &,
                                                     const _Tp2 *) {
...
public:
  ...
  inline _Tp *allocate(size_t __n) {
    return __stl_alloc_rebind(__STATIC_CAST(_Base &, *this), (_Tp *)0)
        .allocate(__n, 0);
  }
  inline void deallocate(_Tp *__p, size_t __n) {
    __stl_alloc_rebind(__STATIC_CAST(_Base &, *this), (_Tp *)0)
        .deallocate(__p, __n);
  }
#endif
};
