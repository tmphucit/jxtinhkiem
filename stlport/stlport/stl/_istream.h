...
// this block is included by _ostream.h, we include it here to lower #include
// level
...
ios_base::iostate _STLP_CALL _M_get_num(basic_istream<_CharT, _Traits> &__that,
                                        _Number &__val);

#if defined(_STLP_USE_TEMPLATE_EXPORT)
template <class _CharT, class _Traits> class _Isentry;
#endif
...
public:
  ...
  explicit sentry(basic_istream<_CharT, _Traits> &__is, bool __noskipws = false)
      : _M_ok((__noskipws || !(__is.flags() & ios_base::skipws))
                  ? _M_init_noskip(__is)
                  : _M_init_skip(__is))
  /* , _M_buf(__is.rdbuf()) */
  {}
...
_M_get_istreambuf(basic_istream<_CharT, _Traits> &__is) {
  return __is.rdbuf();
}
