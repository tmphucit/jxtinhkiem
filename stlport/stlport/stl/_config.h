...
#if defined(__unix) || defined(__linux__) || defined(__QNX__) ||               \
    defined(_AIX) || defined(__NetBSD__) || defined(__Lynx__)
#define _STLP_UNIX 1
...
#define _STLP_MAC 1
#elif defined(_WIN32) || defined(__WIN32) || defined(WIN32) ||                 \
    defined(__WIN32__)
...
/* For some compilers, C headers like <stdio.h> are located in separate
 * directory */
...
#define _STLP_NATIVE_C_HEADER(header)                                          \
  _STLP_MAKE_HEADER(_STLP_NATIVE_C_INCLUDE_PATH, header)
#endif

/* For some compilers, C-library headers like <cstdio> are located in separate
 * directory */
...
#define _STLP_NATIVE_CPP_C_HEADER(header)                                      \
  _STLP_MAKE_HEADER(_STLP_NATIVE_CPP_C_INCLUDE_PATH, header)
#endif
...
#endif
#define _STLP_NATIVE_OLD_STREAMS_HEADER(header)                                \
  _STLP_MAKE_HEADER(_STLP_NATIVE_OLD_STREAMS_INCLUDE_PATH, header)
#endif

/* Some compilers locate basic C++ runtime support headers (<new>, <typeinfo>,
 * <exception>) in separate directory */
...
#endif
#define _STLP_NATIVE_CPP_RUNTIME_HEADER(header)                                \
  _STLP_MAKE_HEADER(_STLP_NATIVE_CPP_RUNTIME_INCLUDE_PATH, header)
#endif
...
/* default parameters as template types derived from arguments ( not always
 * supported ) */
...
#define _STLP_DEFAULT_ALLOCATOR_SELECT(_Tp)                                    \
  __DFL_TYPE_PARAM(_Alloc, __sgi_alloc)
#define _STLP_DEFAULT_PAIR_ALLOCATOR(_Key, _Tp) __sgi_alloc
#define _STLP_DEFAULT_PAIR_ALLOCATOR_SELECT(_Key, _Tp)                         \
  __DFL_TYPE_PARAM(_Alloc, __sgi_alloc)
#if defined(_STLP_LIMITED_DEFAULT_TEMPLATES) &&                                \
    !defined(_STLP_DEFAULT_TYPE_PARAM)
#define _STLP_USE_WRAPPER_FOR_ALLOC_PARAM 1
#endif
#endif
 ...
 * This section is guarded by _STLP_HAS_NO_NAMESPACES, not by
 * _STLP_USE_NAMESPACES, since it depends only on the native features, not on
 * user's preference whether to use namespace for STLport or not.
 */
#if !defined(_STLP_HAS_NO_NAMESPACES)
/* Import some vendor's headers into corresponding STLport ones if they might be
 * needed (if we wrap native iostreams and use namepace other than std::) */
#if defined(_STLP_WHOLE_NATIVE_STD)
#define _STLP_IMPORT_VENDOR_STD 1
#undef _STLP_MINIMUM_IMPORT_STD
#elif (defined(_STLP_USE_OWN_NAMESPACE) ||                                     \
       (defined(_STLP_DEBUG) && defined(_STLP_USE_NAMESPACES))) &&             \
 ...
 * try importing 'em.
 * MSVC has ambiguity problem when we try to import C-style std:: stuff back
...
#define _STLP_BEGIN_RELOPS_NAMESPACE                                           \
  _STLP_BEGIN_NAMESPACE namespace rel_ops {}
...
/* boris : it was found out that _STLP_USE_SEPARATE_RELOPS_NAMESPACE
   causes less problems than having relational operator templates in global
   namespace Please define _STLP_NO_RELOPS_NAMESPACE in stl_user_config.h if
   your code rely on them. */
...
/* if we are going to use native new iostreams, use native <string> and
 * <stdexcept> */
...
#endif
/* For your own iterators, please use inheritance from iterator<> instead of
 * these obsolete queries. */
#if (defined(_STLP_NESTED_TYPE_PARAM_BUG) ||                                   \
...
#define _STLP_NO_ARROW_OPERATOR
    ...
    !defined(_STLP_NO_MSVC50_COMPATIBILITY) &&                                 \
...
#define _STLP_DECLARE_REVERSE_ITERATORS(__reverse_iterator)                    \
  typedef typename _STLP_STD ::reverse_iterator<const_iterator>                \
      const_reverse_iterator;                                                  \
  typedef typename _STLP_STD ::reverse_iterator<iterator> reverse_iterator
#elif (defined(__sgi) && !defined(__GNUC__)) || defined(__SUNPRO_CC) ||        \
    defined(__xlC__)
#define _STLP_DECLARE_REVERSE_ITERATORS(__reverse_iterator)                    \
  typedef _STLP_STD::_STLP_TEMPLATE reverse_iterator<const_iterator>           \
      const_reverse_iterator;                                                  \
...
#define _STLP_DECLARE_REVERSE_ITERATORS(__reverse_iterator)                    \
  typedef _STLP_STD::__reverse_iterator<const_iterator, value_type,            \
                                        const_reference, const_pointer,        \
                                        difference_type>                       \
      const_reverse_iterator;                                                  \
  typedef _STLP_STD::__reverse_iterator<iterator, value_type, reference,       \
...
#define _STLP_DECLSPEC                                                         \
  _STLP_IMPORT_DECLSPEC /* Other modules, importing STLport exports */
...
#define _STLP_EXPORT_TEMPLATE_CLASS                                            \
  _STLP_EXPORT template class _STLP_CLASS_DECLSPEC
...
/* We only need to expose details of streams implementation
   if we use non-standard i/o or are building STLport*/
    ...
    !defined(_STLP_NO_CUSTOM_IO)
#define _STLP_EXPOSE_STREAM_IMPLEMENTATION 1
#endif

/* We only need to expose details of global implementation if we are building
   STLport or have not instantiated everything in the lib */
#if defined(__BUILDING_STLPORT) || defined(_STLP_NO_FORCE_INSTANTIATE)
...
#else
/* when we are not using SGI iostreams, we must expose globals, but not streams
 * implementation */
...
#define _STLP_RELOPS_OPERATORS(_TMPL, _TP)                                     \
  _TMPL inline bool _STLP_CALL operator!=(const _TP &__x, const _TP &__y) {    \
    return !(__x == __y);                                                      \
  }                                                                            \
  _TMPL inline bool _STLP_CALL operator>(const _TP &__x, const _TP &__y) {     \
    return __y < __x;                                                          \
