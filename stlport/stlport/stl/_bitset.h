...
template <class _Dummy> class _Bs_G {
public:
...
struct _Base_bitset<1UL> {
  ...
  bool _M_is_equal(const _Self &__x) const { return _M_w == __x._M_w; }
...
inline size_t _Base_bitset<1UL>::_M_do_find_first(size_t __not_found) const {
  //  typedef unsigned long _WordT;
...
inline size_t _Base_bitset<1UL>::_M_do_find_next(size_t __prev,
...
public:
  ...
  bitset(unsigned long __val) : _Base_bitset<_Words>(__val) {
    _M_do_sanitize();
  }
  ...
  template <class _CharT, class _Traits, class _Alloc>
  bitset(const basic_string<_CharT, _Traits, _Alloc> &__s, size_t __pos,
         size_t __n)
      : _Base_bitset<_Words>() {
    if (__pos > __s.size())
      __stl_throw_out_of_range("bitset");
  ...
  explicit bitset(const string &__s, size_t __pos = 0, size_t __n = (size_t)-1)
      : _Base_bitset<_Words>() {
    if (__pos > __s.size())
      __stl_throw_out_of_range("bitset");
    _M_copy_from_string(__s, __pos, __n);
  ...
  bool _Unchecked_test(size_t __pos) const {
    return (this->_M_getword(__pos) & this->_S_maskbit(__pos)) !=
           __STATIC_CAST(_WordT, 0);
  }
  ...
  size_t _Find_first() const { return this->_M_do_find_first(_Nb); }
...
template <size_t _Nb>
inline bitset<_Nb> _STLP_CALL operator&(const bitset<_Nb> &__x,
...
template <size_t _Nb>
_ISTREAM_DLL &_STLP_CALL operator>>(_ISTREAM_DLL &__is, bitset<_Nb> &__x);

template <size_t _Nb>
inline _OSTREAM_DLL &_STLP_CALL operator<<(_OSTREAM_DLL &__os,
