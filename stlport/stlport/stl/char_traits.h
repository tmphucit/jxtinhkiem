...
// #elif defined (__unix) && defined (_STLP_HAS_NO_NEW_C_HEADERS) /* Other
// version of UNIX */ typedef off_t     streamoff;
#else  /* __unix */
// boris : here, it's not ptrdiff_t as some Solaris systems have confusing
...
public:
  ...
  static const _CharT *_STLP_CALL find(const _CharT *__s, size_t __n,
                                       const _CharT &__c) {
    for (; __n > 0; ++__s, --__n)
      if (eq(*__s, __c))
        return __s;
  ...
  static _CharT *_STLP_CALL move(_CharT *__s1, const _CharT *__s2, size_t _Sz) {
    return (_Sz == 0 ? __s1
                     : (_CharT *)memmove(__s1, __s2, _Sz * sizeof(_CharT)));
  }
  ...
  static bool _STLP_CALL eq_int_type(const int_type &__c1,
                                     const int_type &__c2) {
    return __c1 == __c2;
  }
...
public:
  ...
  static int _STLP_CALL compare(const char *__s1, const char *__s2,
                                size_t __n) {
