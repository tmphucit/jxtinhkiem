...
class reverse_bidirectional_iterator {
  typedef reverse_bidirectional_iterator<_BidirectionalIterator, _Tp,
      ...
      _Self;
  //  friend inline bool operator== _STLP_NULL_TMPL_ARGS (const _Self& x, const
  //  _Self& y);
protected:
...
template <class _BidirectionalIterator, class _Tp, class __Reference,
          class _Distance>
inline bidirectional_iterator_tag _STLP_CALL iterator_category(
...
operator==(const reverse_bidirectional_iterator<_BidirectionalIterator, _Tp,
...
inline bool _STLP_CALL operator!=(
    const reverse_bidirectional_iterator<_BiIter, _Tp, Reference__, _Distance>
        &__x,
    const reverse_bidirectional_iterator<_BiIter, _Tp, Reference__, _Distance>
        &__y) {
  return !(__x == __y);
...
#if defined(__MSL__) && (__MSL__ >= 0x2405) || defined(__MRC__) ||             \
    defined(__SC__) //*ty 03/22/2001 - give the default to the secont param
                    // under MPW.
                    // I believe giving the default will cause any harm even
                    // though the 2nd type parameter still have to be provided
                    // for T* type iterators.
          __DFL_TMPL_PARAM(_Tp,
...
public:
  ...
  _Self operator+(_Distance __n) const { return _Self(__current - __n); }
...
#ifdef _STLP_USE_OLD_HP_ITERATOR_QUERIES
template <class _RandomAccessIterator, class _Tp, class __Reference,
          class _Distance>
inline random_access_iterator_tag _STLP_CALL iterator_category(
    const reverse_iterator<_RandomAccessIterator, _Tp, Reference__, _Distance>
        &) {
  return random_access_iterator_tag();
}
template <class _RandomAccessIterator, class _Tp, class __Reference,
          class _Distance>
inline _Tp *_STLP_CALL value_type(
    const reverse_iterator<_RandomAccessIterator, _Tp, Reference__, _Distance>
        &) {
  return (_Tp *)0;
}
...
template <class _RandomAccessIterator, class _Tp, class __Reference,
          class _Distance>
inline _Distance _STLP_CALL operator-(
    const reverse_iterator<_RandomAccessIterator, _Tp, Reference__, _Distance>
        &__x,
    const reverse_iterator<_RandomAccessIterator, _Tp, Reference__, _Distance>
        &__y) {
...
template <class _RandomAccessIterator, class _Tp, class __Reference,
          class _Distance>
inline reverse_iterator<_RandomAccessIterator, _Tp, Reference__, _Distance>
    _STLP_CALL operator+(_Distance __n,
                         const reverse_iterator<_RandomAccessIterator, _Tp,
                                                Reference__, _Distance> &__x) {
  return reverse_iterator<_RandomAccessIterator, _Tp, Reference__, _Distance>(
      __x.base() - __n);
}
