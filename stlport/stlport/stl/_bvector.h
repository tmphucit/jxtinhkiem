...
struct _Bit_iterator_base {
  ...
  //  _Bit_iterator_base( const _Bit_iterator_base& __x) : _M_p(__x._M_p),
  //  _M_offset(__x._M_offset) {} _Bit_iterator_base& operator = ( const
  //  _Bit_iterator_base& __x) { _M_p = __x._M_p ; _M_offset = __x._M_offset ;
  //  return *this; }
...
inline bool _STLP_CALL operator==(const _Bit_iterator_base &__x,
...
template <class _Ref, class _Ptr> struct _Bit_iter : public _Bit_iterator_base {
  ...
  //   { (_Bit_iterator_base&)*this = (const _Bit_iterator_base&)__x; return
  //   *this; }

  reference operator*() const { return _Bit_reference(_M_p, 1UL << _M_offset); }
  _Self &operator++() {
    _M_bump_up();
    return *this;
  }
...
inline _Bit_iter<_Ref, _Ptr>
    _STLP_CALL operator+(ptrdiff_t __n, const _Bit_iter<_Ref, _Ptr> &__x) {
  return __x + __n;
}

#ifdef _STLP_USE_OLD_HP_ITERATOR_QUERIES
inline random_access_iterator_tag
iterator_category(const _Bit_iterator_base &) {
  return random_access_iterator_tag();
}
inline ptrdiff_t *distance_type(const _Bit_iterator_base &) {
  return (ptrdiff_t *)0;
}
inline bool *value_type(const _Bit_iter<_Bit_reference, _Bit_reference *> &) {
  return (bool *)0;
}
inline bool *value_type(const _Bit_iter<bool, const bool *> &) {
  return (bool *)0;
}
#endif
...
#else
#define __BVECTOR_QUALIFIED                                                    \
  __WORKAROUND_DBG_RENAME(vector)<bool, allocator<bool>>
#endif
...
public:
  ...
  typedef _STLP_STD::reverse_iterator<const_iterator, value_type,
...
protected:
  ...
  void _M_insert_aux(iterator __position, bool __x) {
    if (this->_M_finish._M_p != this->_M_end_of_storage._M_data) {
      __copy_backward(__position, this->_M_finish, this->_M_finish + 1,
      ...
      *__i++ = __x;
      ...
      this->_M_deallocate();
      this->_M_end_of_storage._M_data =
          __q + (__len + __WORD_BIT - 1) / __WORD_BIT;
      this->_M_start = iterator(__q, 0);
    }
  }
  ...
  void _M_initialize_range(_ForwardIterator __first, _ForwardIterator __last,
    ...
    copy(__first, __last,
         this->_M_start); // dwa 12/22/99 -- resolving ambiguous reference.
  }
  ...
  void _M_insert_range(iterator __position, _ForwardIterator __first,
    ...
    if (__first != __last) {
      ...
      if (capacity() - size() >= __n) {
        __copy_backward(__position, end(),
...
public:
  ...
  size_type capacity() const {
    return size_type(const_iterator(this->_M_end_of_storage._M_data, 0) -
                     begin());
  }
  ...
  __BVECTOR(size_type __n, bool __val,
      ...
      : _Bvector_base<_Alloc>(__a) {
    _M_initialize(__n);
    fill(this->_M_start._M_p, (__chunk_type *)(this->_M_end_of_storage._M_data),
         __val ? ~0 : 0);
  }

  explicit __BVECTOR(size_type __n) : _Bvector_base<_Alloc>(allocator_type()) {
    _M_initialize(__n);
    fill(this->_M_start._M_p, (__chunk_type *)(this->_M_end_of_storage._M_data),
         0);
  }
  ...
  void reserve(size_type __n) {
    if (capacity() < __n) {
      ...
      this->_M_start = iterator(__q, 0);
      this->_M_end_of_storage._M_data =
          __q + (__n + __WORD_BIT - 1) / __WORD_BIT;
    }
  }
  ...
  void _M_insert_dispatch(iterator __pos, _InputIterator __first,
                          _InputIterator __last, const __false_type &) {
  ...
  void _M_fill_insert(iterator __position, size_type __n, bool __x) {
    if (__n == 0)
      return;
    if (capacity() - size() >= __n) {
      __copy_backward(__position, end(), this->_M_finish + difference_type(__n),
                      random_access_iterator_tag(), (difference_type *)0);
  ...
  void flip() {
    for (unsigned int *__p = this->_M_start._M_p;
         __p != this->_M_end_of_storage._M_data; ++__p)
      *__p = ~*__p;
  }
...
__BVEC_TMPL_HEADER
inline bool _STLP_CALL operator==(const __BVECTOR_QUALIFIED &__x,
                                  const __BVECTOR_QUALIFIED &__y) {
