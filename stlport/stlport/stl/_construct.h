...
template <class _Tp> inline void _Destroy(_Tp *__pointer) {
#if _MSC_VER >= 1010
  __pointer;
#endif // _MSC_VER >= 1000
#ifdef _STLP_TRIVIAL_DESTRUCTOR_BUG
...
template <class _T1> inline void _Construct(_T1 *__p) {
#ifdef _STLP_DEBUG_UNINITIALIZED
  memset((char *)__p, _STLP_SHRED_BYTE, sizeof(_T1));
#endif
...
_STLP_INLINE_LOOP void __destroy_aux(_ForwardIterator __first,
...
template <class _ForwardIterator>
inline void __destroy_aux(_ForwardIterator, _ForwardIterator,
                          const __true_type &) {}
...
template <class _Tp> inline void destroy(_Tp *__pointer) {
  _Destroy(__pointer);
}
template <class _ForwardIterator>
