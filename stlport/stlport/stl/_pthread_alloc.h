...
template <size_t _Max_size> struct _Pthread_alloc_per_thread_state {
  ...
  // this data member is only to be used by per_thread_allocator, which returns
  // memory to the originating thread.
  _STLP_mutex _M_lock;
};
...
public:
  ...
  static void *allocate(size_t __n, __state_type *__a) {
    ...
    // boris : here, we have to lock per thread state, as we may be getting
    // memory from different thread pool.
  ...
  static void deallocate(void *__p, size_t __n, __state_type *__a) {
    ...
    // boris : here, we have to lock per thread state, as we may be returning
    // memory from different thread.
    _STLP_mutex_lock __lock(__a->_M_lock);
...
template <class _Tp> class pthread_allocator {
...
public:
  ...
  pthread_allocator(const pthread_allocator<_OtherType> &) _STLP_NOTHROW {}
#endif
  ...
  _Tp *allocate(size_type __n, const void * = 0) {
    return __n != 0
               ? __STATIC_CAST(_Tp *, _S_Alloc::allocate(__n * sizeof(_Tp)))
               : 0;
  }
  ...
  size_type max_size() const _STLP_NOTHROW { return size_t(-1) / sizeof(_Tp); }
...
inline pthread_allocator<_Tp2> &__stl_alloc_rebind(pthread_allocator<_Tp1> &__x,
                                                   const _Tp2 *) {
  return (pthread_allocator<_Tp2> &)__x;
}

template <class _Tp1, class _Tp2>
inline pthread_allocator<_Tp2> __stl_alloc_create(pthread_allocator<_Tp1> &,
                                                  const _Tp2 *) {
...
//
// per_thread_allocator<> : this allocator always return memory to the same
// thread it was allocated from.
//

template <class _Tp> class per_thread_allocator {
  typedef pthread_alloc _S_Alloc; // The underlying allocator.
...
public:
  ...
  per_thread_allocator(const per_thread_allocator<_Tp> &__a) _STLP_NOTHROW
      : _M_state(__a._M_state) {}
                                       ...
                                       (_STLP_FUNCTION_PARTIAL_ORDER) */
  template <class _OtherType>
  per_thread_allocator(const per_thread_allocator<_OtherType> &__a)
