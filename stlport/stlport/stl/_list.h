...
template <class _Dummy> class _List_global {
public:
...
template <class _Tp, class _Traits>
inline _Tp *value_type(const _List_iterator<_Tp, _Traits> &) {
  return 0;
}
...
public:
  ...
  _List_base(const allocator_type &__a)
      : _M_node(_STLP_CONVERT_ALLOCATOR(__a, _Node), (_Node *)0) {
...
template <class _Tp, _STLP_DEFAULT_ALLOCATOR_SELECT(_Tp)> class list;
...
protected:
  _Node *_M_create_node(const _Tp &__x) {
    ...
    _STLP_TRY { _Construct(&__p->_M_data, __x); }
  ...
  _Node *_M_create_node() {
    _Node *__p = this->_M_node.allocate(1);
    _STLP_TRY { _Construct(&__p->_M_data); }
    _STLP_UNWIND(this->_M_node.deallocate(__p, 1));
    return __p;
  }
...
public:
  ...
  iterator begin() {
    return iterator((_Node *)(this->_M_node._M_data->_M_next));
  }
  const_iterator begin() const {
    return const_iterator((_Node *)(this->_M_node._M_data->_M_next));
  }
  ...
  bool empty() const {
    return this->_M_node._M_data->_M_next == this->_M_node._M_data;
  }
  ...
  void _M_insert_dispatch(iterator __position, _InputIter __first,
                          _InputIter __last, const __false_type &)
  ...
  explicit list(size_type __n) : _List_base<_Tp, _Alloc>(allocator_type()) {
  ...
  list(const list<_Tp, _Alloc> &__x)
      : _List_base<_Tp, _Alloc>(__x.get_allocator()) {
...
public:
  void splice(iterator __position, _Self &__x) {
    if (!__x.empty())
      _List_global_inst::_Transfer(__position._M_node, __x.begin()._M_node,
                                   __x.end()._M_node);
  }
  ...
  void splice(iterator __position, _Self &, iterator __first, iterator __last) {
    if (__first != __last)
      _List_global_inst::_Transfer(__position._M_node, __first._M_node,
                                   __last._M_node);
  }
  ...
  void unique() { _S_unique(*this, equal_to<_Tp>()); }

  void merge(_Self &__x) { _S_merge(*this, __x, less<_Tp>()); }
  ...
  void sort() { _S_sort(*this, less<_Tp>()); }
...
_STLP_INLINE_LOOP bool _STLP_CALL operator==(const list<_Tp, _Alloc> &__x,
...
template <class _Tp, class _Alloc>
inline void _STLP_CALL swap(list<_Tp, _Alloc> &__x, list<_Tp, _Alloc> &__y) {
  __x.swap(__y);
}
