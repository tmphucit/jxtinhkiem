...
inline ptrdiff_t *_STLP_CALL distance_type(const _Slist_iterator_base &) {
  return 0;
}
...
#ifdef _STLP_USE_OLD_HP_ITERATOR_QUERIES
template <class _Tp, class _Traits>
inline _Tp *_STLP_CALL value_type(const _Slist_iterator<_Tp, _Traits> &) {
  return (_Tp *)0;
}
#endif /* OLD_QUERIES */
...
template <class _Tp, class _Alloc> struct _Slist_base {
...
public:
  allocator_type get_allocator() const {
    return _STLP_CONVERT_ALLOCATOR((const _M_node_allocator_type &)_M_head,
                                   _Tp);
  }
...
public:
  ...
  size_type size() const {
    return _Sl_global_inst::size(this->_M_head._M_data._M_next);
  }
  ...
  void swap(_Self &__x) { _STLP_STD::swap(this->_M_head, __x._M_head); }
...
public:
  ...
  void push_front() {
    __slist_make_link(&this->_M_head._M_data, _M_create_node());
  }
#endif
  ...
  iterator previous(const_iterator __pos) {
    return iterator((_Node *)_Sl_global_inst::__previous(&this->_M_head._M_data,
                                                         __pos._M_node));
  }
  const_iterator previous(const_iterator __pos) const {
    return const_iterator((_Node *)_Sl_global_inst::__previous(
        &this->_M_head._M_data, __pos._M_node));
  }
...
private:
  ...
  void _M_insert_after_fill(_Node_base *__pos, size_type __n,
  ...
  void _M_insert_after_range(_Node_base *__pos, _InIter __first,
                             _InIter __last) {
  ...
  void _M_insert_after_range(_Node_base *__pos, const_iterator __first,
  ...
  void _M_insert_after_range(_Node_base *__pos, const value_type *__first,
...
public:
  ...
  void insert_after(iterator __pos, const_iterator __first,
                    const_iterator __last) {
    _M_insert_after_range(__pos._M_node, __first, __last);
  }
  void insert_after(iterator __pos, const value_type *__first,
                    const value_type *__last) {
    _M_insert_after_range(__pos._M_node, __first, __last);
  }
  ...
  void insert(iterator __pos, size_type __n, const value_type &__x) {
    _M_insert_after_fill(
        _Sl_global_inst::__previous(&this->_M_head._M_data, __pos._M_node), __n,
        __x);
  }
...
public:
  ...
  iterator erase_after(iterator __before_first, iterator __last) {
    return iterator(
        (_Node *)this->_M_erase_after(__before_first._M_node, __last._M_node));
  }
  ...
  iterator erase(iterator __first, iterator __last) {
    return iterator((_Node *)this->_M_erase_after(
        _Sl_global_inst::__previous(&this->_M_head._M_data, __first._M_node),
        __last._M_node));
  }

  void resize(size_type new_size, const _Tp &__x);
  void resize(size_type new_size) { resize(new_size, _Tp()); }
  void clear() { this->_M_erase_after(&this->_M_head._M_data, 0); }

public:
  // Moves the range [__before_first + 1, __before_last + 1) to *this,
  //  inserting it immediately after __pos.  This is constant time.
  void splice_after(iterator __pos, iterator __before_first,
  ...
  void splice_after(iterator __pos, iterator __prev) {
    _Sl_global_inst::__splice_after(__pos._M_node, __prev._M_node,
...
public:
  void reverse() {
    if (this->_M_head._M_data._M_next)
      this->_M_head._M_data._M_next =
          _Sl_global_inst::__reverse(this->_M_head._M_data._M_next);
  }
  ...
  void merge(slist<_Tp, _Alloc> &__x, _StrictWeakOrdering __comp) {
    ...
    while (__n1->_M_next && __x._M_head._M_data._M_next) {
      if (__comp(((_Node *)__x._M_head._M_data._M_next)->_M_data,
        ...
        _Sl_global_inst::__splice_after(__n1, &__x._M_head._M_data,
                                        __x._M_head._M_data._M_next);
      __n1 = __n1->_M_next;
    }
    if (__x._M_head._M_data._M_next) {
  ...
  template <class _StrictWeakOrdering> void sort(_StrictWeakOrdering __comp) {
    if (this->_M_head._M_data._M_next &&
        this->_M_head._M_data._M_next->_M_next) {
      slist __carry;
      slist __counter[64];
      int __fill = 0;
      while (!empty()) {
        _Sl_global_inst::__splice_after(&__carry._M_head._M_data,
...
inline bool _STLP_CALL operator<(const slist<_Tp, _Alloc> &_SL1,
                                 const slist<_Tp, _Alloc> &_SL2) {
  return lexicographical_compare(_SL1.begin(), _SL1.end(), _SL2.begin(),
                                 _SL2.end());
}
...
template <class _Tp, class _Alloc> class insert_iterator<slist<_Tp, _Alloc>> {
protected:
