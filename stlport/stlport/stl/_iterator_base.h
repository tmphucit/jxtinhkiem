...
#define _STLP_VALUE_TYPE(_It, _Tp)                                             \
  (typename iterator_traits<_Tp>::value_type *)0
#define _STLP_DISTANCE_TYPE(_It, _Tp)                                          \
  (typename iterator_traits<_Tp>::difference_type *)0
...
#else
#define _STLP_ITERATOR_CATEGORY(_It, _Tp)                                      \
  typename iterator_traits<_Tp>::iterator_category()
#endif
#else
#define _STLP_ITERATOR_CATEGORY(_It, _Tp)                                      \
  __iterator_category(_It, _IsPtrType<_Tp>::_Ret())
...
template <class _Tp> struct iterator_traits<_Tp *const> {
  typedef random_access_iterator_tag iterator_category;
  typedef _Tp value_type;
  typedef ptrdiff_t difference_type;
  typedef const _Tp *pointer;
  typedef const _Tp &reference;
    ...
    (defined(_STLP_SIMULATE_PARTIAL_SPEC_FOR_TYPE_TRAITS) &&                   \
     !defined(_STLP_NO_ARROW_OPERATOR))
...
template <class _Iter>
inline random_access_iterator_tag __iterator_category(const _Iter &,
                                                      const __true_type &) {
...
template <class _Iter>
inline ptrdiff_t *_STLP_CALL __distance_type(const _Iter &) {
  return (ptrdiff_t *)(0);
}
...
template <class _Tp>
inline _Tp *__value_type(const _Tp *, const __true_type &) {
...
#else  /* old queries */
template <class _Category, class _Tp, class _Distance, class _Pointer,
          class _Reference>
inline _Category _STLP_CALL iterator_category(
    const iterator<_Category, _Tp, _Distance, _Pointer, _Reference> &) {
  return _Category();
}
template <class _Category, class _Tp, class _Distance, class _Pointer,
          class _Reference>
inline _Tp *_STLP_CALL
value_type(const iterator<_Category, _Tp, _Distance, _Pointer, _Reference> &) {
  return (_Tp *)(0);
}
template <class _Category, class _Tp, class _Distance, class _Pointer,
          class _Reference>
inline _Distance *_STLP_CALL distance_type(
    const iterator<_Category, _Tp, _Distance, _Pointer, _Reference> &) {
  return (_Distance *)(0);
}
template <class _Tp>
...
template <class _Tp> inline _Tp *_STLP_CALL value_type(const _Tp *) {
  return (_Tp *)(0);
}
template <class _Tp> inline ptrdiff_t *_STLP_CALL distance_type(const _Tp *) {
  return (ptrdiff_t *)(0);
}
...
template <class _Tp, class _Distance>
struct bidirectional_iterator : public iterator<bidirectional_iterator_tag, _Tp,
                                                _Distance, _Tp *, _Tp &> {};
template <class _Tp, class _Distance>
struct random_access_iterator : public iterator<random_access_iterator_tag, _Tp,
                                                _Distance, _Tp *, _Tp &> {};
...
template <class _Tp, class _Distance>
inline _Tp *_STLP_CALL
value_type(const bidirectional_iterator<_Tp, _Distance> &) {
  return (_Tp *)(0);
}
template <class _Tp, class _Distance>
inline _Tp *_STLP_CALL
value_type(const random_access_iterator<_Tp, _Distance> &) {
  return (_Tp *)(0);
}
template <class _Tp, class _Distance>
inline _Distance *_STLP_CALL
distance_type(const input_iterator<_Tp, _Distance> &) {
  return (_Distance *)(0);
}
template <class _Tp, class _Distance>
inline _Distance *_STLP_CALL
distance_type(const forward_iterator<_Tp, _Distance> &) {
  return (_Distance *)(0);
}
template <class _Tp, class _Distance>
inline _Distance *_STLP_CALL
...
inline void _STLP_CALL __distance(const _InputIterator &__first,
...
inline void _STLP_CALL __distance(const _ForwardIterator &__first,
...
_STLP_INLINE_LOOP void _STLP_CALL __distance(
    const _BidirectionalIterator &__first, const _BidirectionalIterator &__last,
...
inline void _STLP_CALL distance(const _InputIterator &__first,
  ...
  __distance(__first, __last, __n,
             _STLP_ITERATOR_CATEGORY(__first, _InputIterator));
}
#endif
...
_STLP_INLINE_LOOP _STLP_DIFFERENCE_TYPE(_BidirectionalIterator)
_STLP_CALL
...
inline _STLP_DIFFERENCE_TYPE(_RandomAccessIterator) _STLP_CALL
    __distance(const _RandomAccessIterator &__first,
               const _RandomAccessIterator &__last,
               const random_access_iterator_tag &) {
...
template <class _Traits> struct __cnst_traits_aux : private _Traits {
...
inline void _STLP_CALL _Distance(_InputIterator __first, _InputIterator __last,
                                 _Dist &__n) {
  __distance(__first, __last, __n,
             _STLP_ITERATOR_CATEGORY(__first, _InputIterator));
}
...
_STLP_INLINE_LOOP void _STLP_CALL __advance(_InputIter &__i, _Distance __n,
                                            const input_iterator_tag &) {
  while (__n--)
    ++__i;
}
...
#if defined(_STLP_NONTEMPL_BASE_MATCH_BUG)
template <class _ForwardIterator, class _Distance>
_STLP_INLINE_LOOP void _STLP_CALL __advance(_ForwardIterator &i, _Distance n,
                                            const forward_iterator_tag &) {
  while (n--)
    ++i;
}
