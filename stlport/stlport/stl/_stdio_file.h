...
// Solaris interface looks to be identical.
    ...
    (defined(__sgi) || (defined(__sun) && !defined(_LP64)) ||                  \
     defined(__osf__) || defined(__DECCXX) || defined(_STLP_MSVC) ||           \
     defined(__ICL) || defined(__MINGW32__) || defined(__DJGPP) ||             \
     defined(_AIX))
...
inline char *_FILE_I_begin(const FILE *__f) { return (char *)__f->__pad[1]; }
inline char *_FILE_I_next(const FILE *__f) { return (char *)__f->__pad[0]; }
inline char *_FILE_I_end(const FILE *__f) {
...
inline char *_FILE_I_end(const FILE *__f) { return (char *)__f->_p + __f->_r; }
...
inline char *_FILE_O_end(const FILE *__f) { return (char *)__f->_p + __f->_w; }
...
#define _bufend(__p)                                                           \
  (*(((__p)->__flag & _IOEXT)                                                  \
         ? &((__REINTERPRET_CAST(_FILEX *, (__p)))->__bufendp)                 \
         : &(__bufendtab[__REINTERPRET_CAST(FILE *, (__p)) - __iob])))
#endif

inline int _FILE_fd(const FILE *__f) {
  return fileno(__CONST_CAST(FILE *, __f));
}
inline char *_FILE_I_begin(const FILE *__f) {
  return (__REINTERPRET_CAST(char *, __f->__base));
}
inline char *_FILE_I_next(const FILE *__f) {
  return (__REINTERPRET_CAST(char *, __f->__ptr));
}
inline char *_FILE_I_end(const FILE *__f) {
  return (__REINTERPRET_CAST(char *, __f->__ptr + __f->__cnt));
}
...
inline void _FILE_I_set(FILE *__f, char *__begin, char *__next, char *__end) {
  ...
  if ((unsigned long)(__f - &__iob[0]) > _NFILE)
    __f->__flag |=
        _IOEXT; // used by stdio's _bufend macro and goodness knows what else...
#endif
  __f->__cnt = __end - __next;
...
#else
inline int _FILE_fd(const FILE *__f) {
  return ::_fileno(__CONST_CAST(FILE *, __f));
}
#endif

//       Returns a pointer to the beginning of the buffer.
inline char *_FILE_I_begin(const FILE *__f) {
  return __REINTERPRET_CAST(char *, __f->buffer);
}
//       Returns the current read/write position within the buffer.
inline char *_FILE_I_next(const FILE *__f) {
  return __REINTERPRET_CAST(char *, __f->buffer_ptr);
}

//       Returns a pointer immediately past the end of the buffer.
inline char *_FILE_I_end(const FILE *__f) {
  return __REINTERPRET_CAST(char *, __f->buffer_ptr + __f->buffer_len);
}
...
inline ptrdiff_t _FILE_I_avail(const FILE *__f) { return __f->__fp->__countIn; }
...
inline void _FILE_I_bump(FILE *__f, int __n) {
  __f->__fp->__bufPtr += __n;
  __f->__fp->__countIn -= __n;
}
...
inline char *_FILE_O_begin(const FILE *__f) {
  return (char *)__f->__fp->__bufPtr;
}
inline char *_FILE_O_next(const FILE *__f) {
  return (char *)__f->__fp->__bufPtr;
}
inline char *_FILE_O_end(const FILE *__f) {
  return (char *)__f->__fp->__bufPtr + __f->__fp->__countOut;
}
inline ptrdiff_t _FILE_O_avail(const FILE *__f) {
  return __f->__fp->__countOut;
}
...
inline int _FILE_fd(const FILE *__f) { return __f->_handle; }
inline char *_FILE_I_begin(const FILE *__f) {
  return __REINTERPRET_CAST(char *, __f->_link);
}
inline char *_FILE_I_next(const FILE *__f) {
  return __REINTERPRET_CAST(char *, __f->_ptr);
}
inline char *_FILE_I_end(const FILE *__f) {
  return __REINTERPRET_CAST(char *, __f->_ptr + __f->_cnt);
}
inline ptrdiff_t _FILE_I_avail(const FILE *__f) { return __f->_cnt; }
...
#else
inline int _FILE_fd(const FILE *__f) {
  return (int)::_fileno(__CONST_CAST(FILE *, __f));
}
#endif /* _STLP_WINCE */
