...
#define _STLP_ATOMIC_EXCHANGE(__p, __q)                                        \
  __test_and_set((unsigned long *)__p, (unsigned long)__q)
#endif
...
// HPUX variants have (on some platforms optional) non-standard "DCE" pthreads
// impl
...
_STLP_IMPORT_DECLSPEC long _STLP_STDCALL InterlockedExchange(long volatile *,
                                                             long);
#else
// boris : for the latest SDK, you may actually need the other version of the
// declaration (above) even for earlier VC++ versions. There is no way to tell
// SDK versions apart, sorry ...
...
_STLP_IMPORT_DECLSPEC void _STLP_STDCALL
...
#define _STLP_ATOMIC_EXCHANGE(__x, __y)                                        \
  InterlockedExchange((long *)__x, (long)__y)
#endif
...
APIRET _System DosCreateMutexSem(PCSZ pszName, PHEV phev, ULONG flAttr,
                                 BOOL32 fState);
...
template <int __inst> struct _STLP_mutex_spin {
...
struct _STLP_CLASS_DECLSPEC _STLP_mutex_base {
  ...
  void _M_acquire_lock() { _STLP_mutex_spin<0>::_M_do_lock(&_M_lock); }
...
#elif defined(_STLP_SPARC_SOLARIS_THREADS)
#if defined(__WORD64) || defined(__arch64__) || defined(__sparcv9) ||          \
  ...
  inline void _M_destroy() { pthread_mutex_destroy(&_M_lock); }
...
#elif defined(_STLP_UITHREADS)
  mutex_t _M_lock;
  inline void _M_initialize() { mutex_init(&_M_lock, 0, NULL); }
  inline void _M_destroy() { mutex_destroy(&_M_lock); }
...
struct _STLP_CLASS_DECLSPEC _STLP_mutex_indirect {
  ...
  inline void _M_release_lock() {
    pthread_mutex_unlock((pthread_mutex_t *)_M_lock);
  }
...
// Locking class.  The constructor initializes the lock, the destructor destroys
// it. Well - behaving class, does not need static initializer
...
#if defined(_STLP_THREADS) && defined(_STLP_ATOMIC_EXCHANGE)
inline __stl_atomic_t _Atomic_swap(volatile __stl_atomic_t *__p,
                                   __stl_atomic_t __q) {
  return (__stl_atomic_t)_STLP_ATOMIC_EXCHANGE(__p, __q);
}
#elif defined(_STLP_PTHREADS) || defined(_STLP_UITHREADS) ||                   \
...
template <int __inst> struct _STLP_beos_static_lock_data {
  static bool is_init;
  struct mutex_t : public _STLP_mutex {
    mutex_t() { _STLP_beos_static_lock_data<0>::is_init = true; }
    ~mutex_t() { _STLP_beos_static_lock_data<0>::is_init = false; }
  };
  static mutex_t mut;
};

template <int __inst> bool _STLP_beos_static_lock_data<__inst>::is_init = false;
template <int __inst>
typename _STLP_beos_static_lock_data<__inst>::mutex_t
    _STLP_beos_static_lock_data<__inst>::mut;
