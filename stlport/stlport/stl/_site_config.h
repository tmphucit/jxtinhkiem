...
// compatibility section
    ...
    defined(_STLP_NO_NEW_IOSTREAMS) && !defined(_STLP_NO_OWN_IOSTREAMS)
...
#if (defined(_STLP_NOTHREADS) || defined(_STLP_NO_THREADS) ||                  \
     defined(NOTHREADS))
...
/*
 * Turn _STLP_USE_STATIC_LIB to enforce use of static version of STLport
 * library. NOTE : please do that only if you know what you are doing ! Changing
 * default will require you to change makefile in "src" accordingly and to
 * rebuild STLPort library ! On UNIX, this has no effect.
 *
 */
 ...
 * The reason you have to use separate namespace in wrapper mode is that
 * new-style IO compiled library may have its own idea about STL stuff (string,
 * vector, etc.), so redefining them in the same namespace would break ODR and
 * may cause undefined behaviour. Rule of thumb is - if new-style iostreams are
 * available, there WILL be a conflict. Otherwise you should be OK.
 * In STLport iostreams mode, there is no need for this flag other than to
 * facilitate link with third-part libraries compiled with different standard
...
/*
 * Comment this out to enable throwing exceptions from default
 * __stl_debug_terminate() instead of calling _STLP_ABORT().
 */
...
/*
 * Use obsolete overloaded template functions iterator_category(), value_type(),
 * distance_type() for querying iterator properties. Please note those names are
 * non-standard and are not guaranteed to be used by every implementation.
 * However, this setting is on by default when partial specialization is not
 * implemented in the compiler and cannot be sumulated (only if
 * _STLP_NO_ANACHRONISMS is not set). Use of those interfaces for user-defined
 * iterators is strongly discouraged: please use public inheritance from
 * iterator<> template to achieve desired effect. Second form is to disable
 * old-style queries in any case.
 */
