...
template <class _Tp> struct _Deque_iterator_base {
  ...
  enum _Constants {
    ...
    __buffer_size =
        (sizeof(_Tp) < (size_t)_blocksize ? ((size_t)_blocksize / sizeof(_Tp))
                                          : size_t(1))
  };
  ...
  _Deque_iterator_base(value_type *__x, _Map_pointer __y)
      : _M_cur(__x), _M_first(*__y), _M_last(*__y + __buffer_size),
        _M_node(__y) {}
  ...
  void _M_set_node(_Map_pointer __new_node) {
    _M_last =
        (_M_first = *(_M_node = __new_node)) + difference_type(__buffer_size);
  }
};
...
struct _Deque_iterator : public _Deque_iterator_base<_Tp> {
  ...
  reference operator*() const { return *this->_M_cur; }
...
template <class _Tp, class _Traits>
inline _Tp *_STLP_CALL value_type(const _Deque_iterator<_Tp, _Traits> &) {
  return (_Tp *)0;
}
template <class _Tp, class _Traits>
inline random_access_iterator_tag _STLP_CALL
iterator_category(const _Deque_iterator<_Tp, _Traits> &) {
  return random_access_iterator_tag();
}
template <class _Tp, class _Traits>
inline ptrdiff_t *_STLP_CALL
distance_type(const _Deque_iterator<_Tp, _Traits> &) {
  return 0;
}
#endif
...
public:
  ...
  static size_t _STLP_CALL buffer_size() {
    return (size_t)_Deque_iterator_base<_Tp>::__buffer_size;
  }
...
public: // Constructor, destructor.
  ...
  explicit deque(size_type __n)
      : _Deque_base<_Tp, _Alloc>(allocator_type(), __n) {
  ...
  void _M_range_initialize(_ForwardIterator __first, _ForwardIterator __last,
    ...
    _STLP_TRY {
      for (__cur_node = this->_M_start._M_node;
           __cur_node < this->_M_finish._M_node; ++__cur_node) {
        _ForwardIterator __mid = __first;
...
protected: // Internal insert functions
  ...
  template <class _ForwardIterator>
  void insert(iterator __pos, _ForwardIterator __first, _ForwardIterator __last,
              const forward_iterator_tag &) {
    size_type __n = distance(__first, __last);
    ...
    if (__elemsbefore < difference_type(__length / 2)) {
      ...
      _STLP_TRY {
        if (__elemsafter > difference_type(__n)) {
          ...
          __uninitialized_copy_copy(__mid, __last, __pos, this->_M_finish,
...
protected: // Allocation of _M_map and nodes
  ...
  void _M_reserve_map_at_back(size_type __nodes_to_add = 1) {
    if (__nodes_to_add + 1 >
        this->_M_map_size._M_data -
            (this->_M_finish._M_node - this->_M_map._M_data))
...
template <class _Tp, class _Alloc>
inline void _STLP_CALL swap(deque<_Tp, _Alloc> &__x, deque<_Tp, _Alloc> &__y) {
  __x.swap(__y);
}
