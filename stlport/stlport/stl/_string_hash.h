...
__stl_string_hash(const basic_string<_CharT, _Traits, _Alloc> &__s) {
  unsigned long __h = 0;
  typedef
      typename basic_string<_CharT, _Traits, _Alloc>::const_pointer const_ptr;
...
_STLP_TEMPLATE_NULL struct _STLP_CLASS_DECLSPEC hash<string> {
  size_t operator()(const string &__s) const { return __stl_string_hash(__s); }
};

#if defined(_STLP_HAS_WCHAR_T)
_STLP_TEMPLATE_NULL struct _STLP_CLASS_DECLSPEC hash<wstring> {
  size_t operator()(const wstring &__s) const { return __stl_string_hash(__s); }
};
#endif
