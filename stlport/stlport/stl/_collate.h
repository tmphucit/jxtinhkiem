...
public:
  ...
  int compare(const char *__low1, const char *__high1, const char *__low2,
...
protected:
  ...
  virtual int do_compare(const char *, const char *, const char *,
                         const char *) const;
...
protected:
  ...
  virtual int do_compare(const wchar_t *, const wchar_t *, const wchar_t *,
                         const wchar_t *) const;
...
bool __locale_do_operator_call(
  ...
  collate<_CharT> *__f =
      (collate<_CharT> *)__that->_M_get_facet(collate<_CharT>::id);
  if (!__f)
    __that->_M_throw_runtime_error();
  return __f->compare(__x.data(), __x.data() + __x.size(), __y.data(),
                      __y.data() + __y.size());
}
