...
template <class _P1, class _P2, class _P3> struct _Land3 {
  typedef __false_type _Ret;
};
...
// Boris : simulation technique is used here according to Adobe Open Source
// License Version 1.0. Copyright 2000 Adobe Systems Incorporated and others.
// All rights reserved. Authors: Mat Marcus and Jesse Jones The original version
// of this source code may be found at http://opensource.adobe.com.
...
template <class _Tp1, class _Tp2> struct _IsSame {
  static _Tp1 &__null_rep1();
  static _Tp2 &__null_rep2();
  enum {
    _Ret = (sizeof(_IsSameFun(false, __null_rep1(), __null_rep2())) ==
            sizeof(char))
  };
};
...
template <class _Tp> struct _IsSame<_Tp, _Tp> {
  enum { _Ret = 1 };
};
#endif
...
_STLP_TEMPLATE_NULL struct __type_traits<unsigned short>
    : __type_traits_aux<1> {};
...
_STLP_TEMPLATE_NULL struct __type_traits<_STLP_LONG_LONG>
    : __type_traits_aux<1> {};
_STLP_TEMPLATE_NULL struct __type_traits<unsigned _STLP_LONG_LONG>
    : __type_traits_aux<1> {};
...
template <class _Tp1, class _Tp2> struct _OKToMemCpy {
...
template <class _Tp> struct _IsPOD {
  typedef typename __type_traits<_Tp>::is_POD_type _Type;
  static _Type _Ret() { return _Type(); }
};

template <class _Tp> inline _IsPOD<_Tp> _Is_POD(_Tp *) { return _IsPOD<_Tp>(); }

#ifdef _STLP_CLASS_PARTIAL_SPECIALIZATION
#if defined(__BORLANDC__) || defined(__SUNPRO_CC) ||                           \
    (defined(__MWERKS__) && (__MWERKS__ <= 0x2303)) ||                         \
    (defined(__sgi) && defined(_COMPILER_VERSION))
#define _IS_POD_ITER(_It, _Tp)                                                 \
  __type_traits<typename iterator_traits<_Tp>::value_type>::is_POD_type()
#else
#define _IS_POD_ITER(_It, _Tp)                                                 \
  typename __type_traits<                                                      \
      typename iterator_traits<_Tp>::value_type>::is_POD_type()
#endif
#else
...
template <class _Tp> inline _Tp __default_constructed(_Tp *__p) {
