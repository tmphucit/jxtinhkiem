...
public:
  pair<iterator, bool> insert(const value_type &__obj) {
    ...
    return pair<iterator, bool>(__REINTERPRET_CAST(const iterator &, __p.first),
                                __p.second);
  }
#ifdef _STLP_MEMBER_TEMPLATES
  template <class _InputIterator>
  ...
  pair<iterator, bool> insert_noresize(const value_type &__obj) {
    pair<_ht_iterator, bool> __p = _M_ht.insert_unique_noresize(__obj);
    return pair<iterator, bool>(__p.first, __p.second);
  }

#if defined(_STLP_MEMBER_TEMPLATES) && !defined(_STLP_NO_EXTENSIONS)
  template <class _KT> iterator find(const _KT &__key) const {
...
operator==(const hash_multiset<_Value, _HashFcn, _EqualKey, _Alloc> &__hs1,
  ...
  return hash_multiset<_Value, _HashFcn, _EqualKey, _Alloc>::_M_equal(__hs1,
                                                                      __hs2);
}
