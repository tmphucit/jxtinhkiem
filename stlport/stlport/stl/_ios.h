...
public: // Members from clause 27.4.4.2
  basic_ostream<_CharT, _Traits> *tie() const { return _M_tied_ostream; }
...
public: // Members from 27.4.4.3.  These four functions
  ...
  void clear(iostate __state = goodbit) {
    _M_clear_nothrow(this->rdbuf() ? __state
                                   : iostate(__state | ios_base::badbit));
...
inline char basic_ios<_CharT, _Traits>::narrow(_CharT __c,
                                               char __default) const {
  return ((const ctype<_CharT> *)this->_M_ctype_facet())
      ->narrow(__c, __default);
}

template <class _CharT, class _Traits>
inline _CharT basic_ios<_CharT, _Traits>::widen(char __c) const {
...
_STLP_TEMPLATE_NULL
inline char basic_ios<char, char_traits<char>>::narrow(char __c, char) const {
  return __c;
}

_STLP_TEMPLATE_NULL
inline char basic_ios<char, char_traits<char>>::widen(char __c) const {
  return __c;
}
