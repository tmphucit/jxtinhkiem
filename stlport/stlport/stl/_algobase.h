...
template <class _Tp> inline const _Tp &(max)(const _Tp &__a, const _Tp &__b) {
  return __a < __b ? __b : __a;
    ...
    (__BORLANDC__ < 0x530 || defined(_STLP_USE_OWN_NAMESPACE))
...
template <class _InputIter, class _OutputIter, class _Distance>
inline _OutputIter
__copy(_InputIter __first, _InputIter __last, _OutputIter __result,
       const bidirectional_iterator_tag &, _Distance *__dis) {
  for (; __first != __last; ++__result, ++__first)
    *__result = *__first;
  return __result;
}
...
inline void *__copy_trivial(const void *__first, const void *__last,
                            void *__result) {
...
__copy_backward(_RandomAccessIter __first, _RandomAccessIter __last,
                _BidirectionalIter __result, const random_access_iterator_tag &,
                _Distance *) {
  for (_Distance __n = __last - __first; __n > 0; --__n)
    *--__result = *--__last;
  return __result;
...
inline void *__copy_trivial_backward(const void *__first, const void *__last,
  ...
  return (_Num > 0) ? memmove((char *)__result - _Num, __first, _Num)
                    : __result;
}
...
inline _OutputIter __copy_aux(_InputIter __first, _InputIter __last,
  ...
  return __copy_ptrs(__first, __last, __result,
                     _IsOKToMemCpy(_STLP_VALUE_TYPE(__first, _InputIter),
                                   _STLP_VALUE_TYPE(__result, _OutputIter))
                         ._Ret());
}
...
inline _OutputIter copy(_InputIter __first, _InputIter __last,
  ...
  return __copy_aux(__first, __last, __result,
                    _BothPtrType<_InputIter, _OutputIter>::_Ret());
}
...
#define _STLP_DECLARE_COPY_TRIVIAL(_Tp)                                        \
  ...
  inline _Tp *copy_backward(const _Tp *__first, const _Tp *__last,             \
                            _Tp *__result) {                                   \
...
inline pair<_RAIter, _OutputIter> __copy_n(_RAIter __first, _Size __count,
                                           _OutputIter __result,
...
template <class _InputIter, class _Size, class _OutputIter>
inline pair<_InputIter, _OutputIter> copy_n(_InputIter __first, _Size __count,
                                            _OutputIter __result) {
  _STLP_FIX_LITERAL_BUG(__first)
  return __copy_n(__first, __count, __result,
                  _STLP_ITERATOR_CATEGORY(__first, _InputIter));
...
_STLP_INLINE_LOOP void fill(_ForwardIter __first, _ForwardIter __last,
                            const _Tp &__val) {
  _STLP_DEBUG_CHECK(__check_range(__first, __last))
...
_STLP_INLINE_LOOP _OutputIter fill_n(_OutputIter __first, _Size __n,
                                     const _Tp &__val) {
  _STLP_FIX_LITERAL_BUG(__first)
...
template <class _Size>
inline signed char *fill_n(char *__first, _Size __n, const signed char &__val) {
  fill(__first, __first + __n, __val);
  return __first + __n;
}
...
_STLP_INLINE_LOOP pair<_InputIter1, _InputIter2>
...
_STLP_INLINE_LOOP bool equal(_InputIter1 __first1, _InputIter1 __last1,
  ...
  _STLP_FIX_LITERAL_BUG(__first2) _STLP_DEBUG_CHECK(__check_range(
      __first1,
...
// find and find_if. Note find may be expressed in terms of find_if if
// appropriate binder was available.
template <class _InputIter, class _Tp>
...
// replace
template <class _ForwardIter, class _Tp>
_STLP_INLINE_LOOP void replace(_ForwardIter __first, _ForwardIter __last,
