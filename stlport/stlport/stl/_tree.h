...
public:
  ...
  static void _STLP_CALL _Rebalance(_Rb_tree_node_base *__x,
                                    _Rb_tree_node_base *&__root);
  static _Rb_tree_node_base *_STLP_CALL _Rebalance_for_erase(
  ...
  static void _STLP_CALL _Rotate_left(_Rb_tree_node_base *__x,
                                      _Rb_tree_node_base *&__root);
  static void _STLP_CALL _Rotate_right(_Rb_tree_node_base *__x,
                                       _Rb_tree_node_base *&__root);
};
...
struct _Rb_tree_iterator : public _Rb_tree_base_iterator {
  ...
  _Rb_tree_iterator(
      const _Rb_tree_iterator<_Value, _Nonconst_traits<_Value>> &__it) {
...
template <class _Value, class _Traits>
inline _Value *value_type(const _Rb_tree_iterator<_Value, _Traits> &) {
  return (_Value *)0;
}
...
inline ptrdiff_t *distance_type(const _Rb_tree_base_iterator &) {
  return (ptrdiff_t *)0;
}
...
template <class _Tp, class _Alloc> struct _Rb_tree_base {
  ...
  ~_Rb_tree_base() { _M_header.deallocate(_M_header._M_data, 1); }
...
protected:
  _Link_type _M_create_node(const value_type &__x) {
    ...
    _STLP_TRY { _Construct(&__tmp->_M_value_field, __x); }
...
public:
  ...
  typedef _Rb_tree_iterator<value_type, _Const_traits<value_type>>
      const_iterator;
...
public:
  ...
  _Rb_tree()
      : _Rb_tree_base<_Value, _Alloc>(allocator_type()), _M_node_count(0),
        _M_key_compare(_Compare()) {
    _M_empty_initialize();
  }
  ...
  _Rb_tree(const _Compare &__comp, const allocator_type &__a)
      : _Rb_tree_base<_Value, _Alloc>(__a), _M_node_count(0),
        _M_key_compare(__comp) {
    _M_empty_initialize();
  }

  _Rb_tree(const _Rb_tree<_Key, _Value, _KeyOfValue, _Compare, _Alloc> &__x)
      : _Rb_tree_base<_Value, _Alloc>(__x.get_allocator()), _M_node_count(0),
...
private:
  void _M_empty_initialize() {
    _S_color(this->_M_header._M_data) =
        _S_rb_tree_red; // used to distinguish header from
...
public:
  ...
  void erase(iterator __position) {
    _Link_type __y = (_Link_type)_Rb_global_inst::_Rebalance_for_erase(
        __position._M_node, this->_M_header._M_data->_M_parent,
        this->_M_header._M_data->_M_left, this->_M_header._M_data->_M_right);
...
public:
  ...
  template <class _KT> iterator find(const _KT &__x) {
    return iterator(_M_find(__x));
  }
  template <class _KT> const_iterator find(const _KT &__x) const {
    return const_iterator(_M_find(__x));
  }
...
private:
  ...
  {
    _Link_type __y =
        this->_M_header._M_data; // Last node which is not less than __k.
  ...
  _Link_type _M_upper_bound(const key_type &__k) const {
    _Link_type __y =
        this->_M_header._M_data; /* Last node which is greater than __k. */
    _Link_type __x = _M_root();  /* Current node. */
