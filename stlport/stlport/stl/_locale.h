...
bool __locale_do_operator_call(
    const locale *__that,
...
public:
  ...
  protected:
    explicit facet(size_t __no_del = 0)
        : _Refcount_Base(1), _M_delete(__no_del == 0) {}
    virtual ~facet();
    friend class locale;
  ...
  private: // Invalidate assignment and copying.
    facet(const facet &__f)
        : _Refcount_Base(1), _M_delete(__f._M_delete == 0) {};
...
public:
...
#if !(defined(_STLP_NO_MEMBER_TEMPLATES) ||                                    \
  ...
  bool operator!=(const locale &) const;
    ...
    defined(_STLP_INLINE_MEMBER_TEMPLATES) ||                                  \
...
template <class _Facet>
inline const _Facet &_Use_facet<_Facet>::operator*() const
#else
template <class _Facet>
...
#ifdef _STLP_NO_EXPLICIT_FUNCTION_TMPL_ARGS
template <class _Facet> struct has_facet {
  const locale &__loc;
