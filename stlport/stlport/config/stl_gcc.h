...
/* Tru64 Unix, AIX, HP : gcc there by default uses uses native ld and hence
   cannot auto-instantiate static template data. If you are using GNU ld, please
   say so in stl_user_config.h header */
#if (__GNUC__ < 3) && !(_STLP_GCC_USES_GNU_LD) &&                              \
    ((defined(__osf__) && defined(__alpha__)) || defined(_AIX) ||              \
     defined(__hpux) || defined(__amigaos__))
...
#define _STLP_NO_CUSTOM_IO
    ...
    !(defined(_STLP_USE_GLIBC) || defined(__sun))
#ifndef __MINGW32__
...
// If your installation use "g++-3" include directory for any reason (pre-2.95.2
// or Win binary kit), please change the macro below to point to your directory.
...
#define _STLP_NATIVE_INCLUDE_PATH ../ include / g++ - v3
       ...
       !(defined(__FreeBSD__) || defined(__NetBSD__) || defined(__sgi)))
#define _STLP_NATIVE_INCLUDE_PATH ../ g++ - 3
#elif (__GNUC_MINOR__ > 8) && (__GNUC_MINOR__ < 95) && (__GNUC__ < 3) &&       \
    !defined(__Lynx__)
// this really sucks, as GNUpro does not really identifies itself, so we have to
// guess depending on a platform
#ifdef __hpux
#define _STLP_NATIVE_INCLUDE_PATH ../ g++ - 3
#else
