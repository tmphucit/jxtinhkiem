...
#if (!defined(_DEBUG))
#error                                                                         \
    "For static link with STLport library, _DEBUG setting MUST be on when _STLP_DEBUG is on. (/MTd forces _DEBUG)"
#endif
#pragma comment(lib, _STLP_LIB_BASENAME "_stldebug"_STLP_LIB_STATIC_SUFFIX     \
                                        ".lib")
#elif (defined(_DEBUG) || defined(__DEBUG)) && defined(_STLP_USE_DEBUG_LIB)
#pragma comment(lib, _STLP_LIB_BASENAME "_debug"_STLP_LIB_STATIC_SUFFIX        \
                                        ".lib")
#else
#pragma comment(lib, _STLP_LIB_BASENAME ""_STLP_LIB_STATIC_SUFFIX              \
                                        ".lib")
#endif
