...
#ifdef qMacApp
#ifndef __CONDITIONALMACROS__ // skip including ConditionalMacros_AC.h if
                              // ConditionalMacros.h is already included
...
#define _STLP_DEBUG_MESSAGE_POST                                               \
  DebugStr("\pSTL diagnosis issued. See 'stderr' for detail.");
#define _STLP_ASSERT_MSG_TRAILER " "

#ifdef _STLP_DEBUG
#define _STLP_THROW(x)                                                         \
  (DebugStr("\pSTL is about to throw exception: " #x), throw x)
#endif

#if defined(__MRC__)
#ifndef __spillargs
#define __spillargs                                                            \
  1 // MrCpp requires this symbol to be defined as 1 to properly handle
    // va_start; ref.[ file stdarg.h; line 26 ]
#endif
#endif
...
#define _STLP_NATIVE_INCLUDE_PATH                                              \
  ../ CIncludes // expects the alias to {CIncludes} under the same folder as
                // {STL}
#else
#define _STLP_NATIVE_INCLUDE_PATH                                              \
  ::CIncludes // expects the alias to {CIncludes} under the same folder as {STL}
#endif
#endif
#if !defined(_STLP_MAKE_HEADER)
#if !__option(unix_includes)
#define _STLP_MAKE_HEADER(path, header)                                        \
  <path : header> // Mac uses ":" for directory delimiter
#endif
#endif

#define _STLD                                                                  \
  _DBG // to keep the length of generated symbols within the compiler limitation
...
#define _STLP_VENDOR_GLOBAL_STD // mpw's c++ libs do not utilize namespace std
                                // yet
...
#define _STLP_MPWFIX_TRY                                                       \
  try { //*TY 06/01/2000 - exception handling bug workaround
#define _STLP_MPWFIX_CATCH                                                     \
  }                                                                            \
  catch (...) {                                                                \
    throw;                                                                     \
  } //*TY 06/01/2000 - exception handling bug workaround
