...
#pragma aux __stl_InterlockedIncrement parm[ecx] = ".586"                      \
                                                   "mov eax, 1"                \
                                                   "lock xadd [ecx], eax"      \
                                                   "inc eax" value[eax];

#pragma aux __stl_InterlockedDecrement parm[ecx] = ".586"                      \
                                                   "mov eax, 0FFFFFFFFh"       \
                                                   "lock xadd [ecx], eax"      \
                                                   "dec eax" value[eax];
#else
...
// xchg has auto-lock
#pragma aux __stl_InterlockedExchange parm[ecx]                                \
    [eax] = ".586"                                                             \
            "xchg eax, [ecx]" value[eax];
#else
...
#define _STLP_ATOMIC_EXCHANGE(__x, __y)                                        \
  __stl_InterlockedExchange((long *)__x, (long)__y)
