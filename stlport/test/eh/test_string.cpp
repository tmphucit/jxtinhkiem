...
typedef EH_STD::basic_string<char, EH_STD::char_traits<char>,
                             eh_allocator(char)>
    TestString;

inline sequence_container_tag container_category(const TestString &) {
...
void test_string() {
  ...
  while (testString.size() < ropeSize) {
    TestString::value_type x = TestString::value_type(
        random_number(random_base)); // initialize before use
  ...
  WeakCheck(testString,
            test_insert_one<TestString>(testString, (int)testString.size()));

  WeakCheck(testString,
            test_insert_n<TestString>(testString, random_number(random_base)));
  WeakCheck(testString, test_insert_n<TestString>(
                            testString, random_number(random_base), 0));
  WeakCheck(testString,
            test_insert_n<TestString>(testString, random_number(random_base),
                                      (int)testString.size()));
  ...
  WeakCheck(testString,
            insert_range_tester(testString, insFirst, insFirst + insCnt));
  WeakCheck(testString, insert_range_at_begin_tester(testString, insFirst,
                                                     insFirst + insCnt));
  WeakCheck(testString, insert_range_at_end_tester(testString, insFirst,
                                                   insFirst + insCnt));

  ConstCheck(
      0, test_construct_pointer_range<TestString>(insFirst, insFirst + insCnt));
  delete[] insFirst;

  WeakCheck(testString, insert_range_tester(testString, testString2.begin(),
                                            testString2.end()));
  /*
  ...
  */
  ...
  ConstCheck(0,
             test_construct_n_instance<TestString>(random_number(random_base)));
