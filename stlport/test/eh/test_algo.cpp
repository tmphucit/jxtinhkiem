...
// 	constant value.
//
template <class T> struct less_by_reference {
...
struct test_stable_sort_1 {
  test_stable_sort_1(const SortBuffer &buf) : orig(buf) {
...
struct test_stable_sort_2 {
  test_stable_sort_2(const SortBuffer &buf) : orig(buf) {
...
struct test_inplace_merge_1 {
  test_inplace_merge_1(SortBuffer &buf) : orig(buf) {
    gTestController.SetCurrentTestName("inplace_merge #1()");
  }

  void operator()(SortBuffer &buf) const {
    EH_STD::inplace_merge(
        buf.begin(), buf.begin() + (buf.end() - buf.begin()) / 2, buf.end());
    assert_sorted_version(orig, buf);
  }
...
struct test_inplace_merge_2 {
  test_inplace_merge_2(SortBuffer &buf) : orig(buf) {
    gTestController.SetCurrentTestName("inplace_merge() #2");
  }

  void operator()(SortBuffer &buf) const {
    EH_STD::inplace_merge(buf.begin(),
                          buf.begin() + (buf.end() - buf.begin()) / 2,
