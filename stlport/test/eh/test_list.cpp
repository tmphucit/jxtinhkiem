...
inline sequence_container_tag container_category(const TestList &) {
...
struct test_list_sort {
  test_list_sort() { gTestController.SetCurrentTestName("list::sort()"); }
...
void test_list() {
  ...
  StrongCheck(testList,
              test_insert_one<TestList>(testList, (int)testList.size()));

  WeakCheck(testList,
            test_insert_n<TestList>(testList, random_number(random_base)));
  WeakCheck(testList,
            test_insert_n<TestList>(testList, random_number(random_base), 0));
  WeakCheck(testList,
            test_insert_n<TestList>(testList, random_number(random_base),
                                    (int)testList.size()));
  ...
  WeakCheck(testList,
            insert_range_tester(testList, insFirst, insFirst + insCnt));
  WeakCheck(testList, insert_range_at_begin_tester(testList, insFirst,
                                                   insFirst + insCnt));
  WeakCheck(testList,
            insert_range_at_end_tester(testList, insFirst, insFirst + insCnt));

  ConstCheck(
      0, test_construct_pointer_range<TestList>(insFirst, insFirst + insCnt));
  delete[] insFirst;
  ...
  ConstCheck(0,
             test_construct_n_instance<TestList>(random_number(random_base)));
