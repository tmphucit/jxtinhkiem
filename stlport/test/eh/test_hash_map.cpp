...
inline multimap_tag container_category(const TestMultiMap &) {
  return multimap_tag();
}
...
void test_hash_multimap() {
  ...
  WeakCheck(testMultiMap,
            insert_range_tester(testMultiMap, insFirst, insFirst + insCnt));
  ConstCheck(0, test_construct_pointer_range<TestMultiMap>(insFirst,
                                                           insFirst + insCnt));
  delete[] insFirst;

  WeakCheck(testMultiMap,
            insert_range_tester(testMultiMap, testMultiMap2.begin(),
                                testMultiMap2.end()));
...
typedef EH_STD::__hash_map__<TestClass, TestClass, ThrowHash, ThrowEqual,
                             eh_allocator(TestClass)>
    TestMap;

inline map_tag container_category(const TestMap &) { return map_tag(); }
...
void test_hash_map() {
  ...
  ConstCheck(
      0, test_construct_pointer_range<TestMap>(insFirst, insFirst + insCnt));
  delete[] insFirst;

  WeakCheck(testMap,
            insert_range_tester(testMap, testMap2.begin(), testMap2.end()));
