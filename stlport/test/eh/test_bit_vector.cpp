...
inline sequence_container_tag container_category(const BitVector &) {
...
struct test_BitVector_reserve {
  ...
  void operator()(BitVector &v) const { v.reserve(fAmount); }
...
#define __WORD_BIT (int(CHAR_BIT * sizeof(unsigned int)))
  ...
  StrongCheck(testVector,
              test_insert_one<BitVector>(testVector, (int)testVector.size()));

  StrongCheck(testVector,
              test_insert_n<BitVector>(testVector, random_number(random_base)));
  StrongCheck(testVector, test_insert_n<BitVector>(
  ...
  StrongCheck(testVector,
    ...
    delete[] insFirst;
#endif
  StrongCheck(testVector, insert_range_tester(testVector, testVector2.begin(),
                                              testVector2.end()));
  StrongCheck(testVector,
              insert_range_at_begin_tester(testVector, testVector2.begin(),
                                           testVector2.end()));
  StrongCheck(testVector,
              insert_range_at_end_tester(testVector, testVector2.begin(),
                                         testVector2.end()));
  StrongCheck(testVector, test_BitVector_reserve(testVector.capacity() +
                                                 random_number(50)));
  ...
  ConstCheck(0,
             test_construct_n_instance<BitVector>(random_number(random_base)));
