...
const unsigned long TestRope::_S_min_len[46] = {/* 0 */ 1,
                                                /* 1 */ 2,
                                                /* 2 */ 3,
                                                /* 3 */ 5,
                                                /* 4 */ 8,
...
inline sequence_container_tag container_category(const TestRope &) {
...
void test_rope() {
  ...
  while (testRope.size() < ropeSize) {
    TestRope::value_type x = TestRope::value_type(
  ...
  WeakCheck(testRope,
            test_insert_one<TestRope>(testRope, (int)testRope.size()));

  WeakCheck(testRope,
            test_insert_n<TestRope>(testRope, random_number(random_base)));
  WeakCheck(testRope,
            test_insert_n<TestRope>(testRope, random_number(random_base), 0));
  WeakCheck(testRope,
            test_insert_n<TestRope>(testRope, random_number(random_base),
                                    (int)testRope.size()));
  ...
  WeakCheck(testRope,
            insert_range_tester(testRope, insFirst, insFirst + insCnt));
  WeakCheck(testRope, insert_range_at_begin_tester(testRope, insFirst,
                                                   insFirst + insCnt));
  WeakCheck(testRope,
            insert_range_at_end_tester(testRope, insFirst, insFirst + insCnt));

  ConstCheck(
      0, test_construct_pointer_range<TestRope>(insFirst, insFirst + insCnt));
  delete[] insFirst;

  WeakCheck(testRope,
            insert_range_tester(testRope, testRope2.begin(), testRope2.end()));
  ...
  ConstCheck(0,
             test_construct_n_instance<TestRope>(random_number(random_base)));
