...
// fbp : DEC cxx is unable to compile it for some reason
      ...
      (defined(__GNUC__) && (__GNUC_MINOR__ < 8)))
...
template <class _Tp> class _STLP_CLASS_DECLSPEC EH_allocator;
...
public:
  ...
  template <class _Tp1>
  EH_allocator(const EH_allocator<_Tp1> &) _STLP_NOTHROW {}
#endif
  ...
  _Tp *allocate(size_type __n, const void * = 0) const {
    return __n != 0
               ? __REINTERPRET_CAST(value_type *, EH_STD::__new_alloc::allocate(
  ...
  void deallocate(pointer __p, size_type __n) const {
    ...
    if (__p != 0)
      EH_STD::__new_alloc::deallocate((void *)__p, __n * sizeof(value_type));
  }
  // backwards compatibility
  void deallocate(pointer __p) const {
    if (__p != 0)
      EH_STD::__new_alloc::deallocate((void *)__p, sizeof(value_type));
  }
  size_type max_size() const _STLP_NOTHROW {
    return size_t(-1) / sizeof(value_type);
  }
  void construct(pointer __p, const _Tp &__val) const {
    _STLP_STD::construct(__p, __val);
  }
  void destroy(pointer __p) const { _STLP_STD::destroy(__p); }
};

template <class _T1>
inline bool _STLP_CALL operator==(const EH_allocator<_T1> &,
...
__stl_alloc_rebind(EH_allocator<_Tp1> &__a, const _Tp2 *) {
  return (EH_allocator<_Tp2> &)(__a);
}
template <class _Tp1, class _Tp2>
inline EH_allocator<_Tp2>
    _STLP_CALL __stl_alloc_create(const EH_allocator<_Tp1> &, const _Tp2 *) {
  return EH_allocator<_Tp2>();
}
_STLP_END_NAMESPACE
