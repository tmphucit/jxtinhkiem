...
#include <iostream.h>
    ...
    defined(_MAC) && defined(__MWERKS__)
...
/*===================================================================================
        ...
        EFFECTS: Breaks into the debugger and shows the assertion. This
implementation is Mac-specific; others could be added for other platforms.
====================================================================================*/
extern "C" {
...
namespace // dwa 1/21/00 - must use unnamed namespace here to avoid conflict
          // under gcc using native streams
{
using EH_STD::cerr;
using EH_STD::endl;
} // namespace
...
static void usage(const char *name) {
  cerr << name
       << " Usage : leak_test [-n <iterations>] [-s <size>] [-l] [-e] "
          "[-q]/[-v] [-t] [test_name...]\n";
  ...
  cerr << "\t[test name [test name...]] : run only some of the tests by name "
          "(default==all tests):"
       << endl;
  cerr << "\t\tpossible test names are : algo vector bit_vector list slist "
          "deque set map hash_set hash_map rope string bitset valarray"
       << endl;
  EH_CSTD::exit(1);
}
