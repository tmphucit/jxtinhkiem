...
typedef std::hash_set<void *, ::hash_void, std::equal_to<void *>>
    allocation_set;
#endif
...
FastAllocator::Block *FastAllocator::mBlocks =
    (FastAllocator::Block *)EH_CSTD::calloc(sizeof(FastAllocator::Block),
                                            FastAllocator::kBlockCount);
...
static void *OperatorNew(size_t s) {
  ...
  if (gTestController.TrackingEnabled() &&
      gTestController.LeakDetectionEnabled() && !using_alloc_set) {
...
void _STLP_CALL operator delete[](void *ptr) throw() { operator delete(ptr); }
#endif
...
/*===================================================================================
        ...
        EFFECTS: Prints information about the current test. If err is false,
ends with an ellipsis, because the test is ongoing. If err is true an error is
being reported, and the output ends with an endl.
====================================================================================*/
