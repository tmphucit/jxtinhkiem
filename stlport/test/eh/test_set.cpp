...
inline multiset_tag container_category(const TestMultiSet &) {
  return multiset_tag();
}
...
void test_multiset() {
  ...
  const EH_STD::size_t setSize = random_number(random_base);
  ...
  WeakCheck(testMultiSet,
            insert_range_tester(testMultiSet, insFirst, insFirst + insCnt));
  ConstCheck(0, test_construct_pointer_range<TestMultiSet>(insFirst,
                                                           insFirst + insCnt));
  delete[] insFirst;
  WeakCheck(testMultiSet,
            insert_range_tester(testMultiSet, testMultiSet2.begin(),
                                testMultiSet2.end()));
...
typedef EH_STD::__set__<TestClass, ThrowCompare, eh_allocator(TestClass)>
    TestSet;
...
void test_set() {
  ...
  ConstCheck(
      0, test_construct_pointer_range<TestSet>(insFirst, insFirst + insCnt));
  delete[] insFirst;
  WeakCheck(testSet,
            insert_range_tester(testSet, testSet2.begin(), testSet2.end()));
