...
inline void VerifyInsertion(const C &original, const C &result,
  ...
  while (first2 != result.end()) {
    EH_STD::pair<DstIter, DstIter> p =
        EH_STD::mismatch(first1, original.end(), first2);
    if (p.second != result.end()) {
      SrcIter srcItem =
          EH_STD::find(SrcIter(firstNew), SrcIter(lastNew), *p.second);
...
inline void VerifyInsertion(const C &original, const C &result,
                            const SrcIter &firstNew, const SrcIter &lastNew,
                            EH_STD::size_t, set_tag) {
  VerifyInsertion(original, result, firstNew, lastNew, EH_STD::size_t(0),
...
#ifdef _MSC_VER
    const C &original, const C &result, SrcIter firstNew, SrcIter lastNew,
    EH_STD::size_t insPos, sequence_container_tag)
#else
...
inline void VerifyInsertion(const C &original, const C &result,
                            const SrcIter &firstNew, const SrcIter &lastNew,
                            EH_STD::size_t insPos) {
  EH_ASSERT(result.size() ==
            original.size() + CountNewItems(original, firstNew, lastNew,
                                            container_category(original)));
...
void VerifyInsertN(const C &original, const C &result, EH_STD::size_t insCnt,
  ...
  typename C::const_iterator p1 = original.begin();
  typename C::const_iterator p2 = result.begin();
  (void)val; //*TY 02/06/2000 - to suppress unused variable warning under
             // nondebug build
...
template <class C> void prepare_insert_n(C &, EH_STD::size_t) {}
...
template <class T> inline void MakeRandomValue(T &) {}

template <class C> struct test_insert_one {
...
template <class C> struct test_insert_n {
...
template <class C> struct test_insert_value {
  test_insert_value(const C &orig) : original(orig) {
    MakeRandomValue(fInsVal);
...
template <class C> struct test_insert_noresize {
  test_insert_noresize(const C &orig) : original(orig) {
    MakeRandomValue(fInsVal);
...
void do_insert_range(C &c_inst, Position offset, Iter first, Iter last,
...
template <class C, class Position, class Iter>
void do_insert_range(C &c, Position, Iter first, Iter last,
                     associative_container_tag) {
  c.insert(first, last);
}
...
template <class C, class Iter> struct test_insert_range {
  test_insert_range(const C &orig, Iter first, Iter last, int pos = -1)
      : fFirst(first), fLast(last), original(orig),
...
template <class C, class Iter>
test_insert_range<C, Iter> insert_range_tester(const C &orig, const Iter &first,
                                               const Iter &last) {
