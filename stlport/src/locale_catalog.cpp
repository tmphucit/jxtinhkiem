...
void *_Loc_time_create(const char *s) { return (void *)_Locale_time_create(s); }
...
char *_Loc_time_name(const void *l, char *s) { return _Locale_time_name(l, s); }
...
const char *_Loc_monetary_default(char *p) {
  return _Locale_monetary_default(p);
}
const char *_Loc_messages_default(char *p) {
...
template <class _Category_ptr> struct __destroy_fun {
...
typedef hash_map<const char *, pair<void *, size_t>, hash<const char *>,
                 __eqstr>
    Category_Map;

// Look up a category by name
static hash_map<const char *, pair<void *, size_t>, hash<const char *>, __eqstr>
    *ctype_hash;
static hash_map<const char *, pair<void *, size_t>, hash<const char *>, __eqstr>
    *numeric_hash;
static hash_map<const char *, pair<void *, size_t>, hash<const char *>, __eqstr>
...
static void *__acquire_category(const char *name, loc_create_func_t create_obj,
                                loc_default_name_func_t default_obj,
                                Category_Map **M) {
  ...
  pair<const key_type, pair<void *, size_t>> __e(
      name, pair<void *, size_t>((void *)0, size_t(0)));
  ...
  if (!*M)
    delete *M; //*TY 06/01/2000 - it forgets to generate dtor for Category_Map
               // class. This fake code forces to generate one.
#endif         //*TY 06/01/2000 -
...
static void __release_category(void *cat, loc_destroy_func_t destroy_fun,
                               loc_name_func_t get_name, Category_Map *M) {
  _STLP_auto_lock sentry(__category_hash_lock);
...
_Locale_monetary *_STLP_CALL __acquire_monetary(const char *name) {
  return __REINTERPRET_CAST(_Locale_monetary *,
                            __acquire_category(name, _Loc_monetary_create,
...
_Locale_messages *_STLP_CALL __acquire_messages(const char *name) {
  return __REINTERPRET_CAST(_Locale_messages *,
                            __acquire_category(name, _Loc_messages_create,
...
void _STLP_CALL __release_numeric(_Locale_numeric *cat) {
  __release_category(cat, _Loc_numeric_destroy, _Loc_numeric_name,
                     numeric_hash);
}
...
void _STLP_CALL __release_collate(_Locale_collate *cat) {
  __release_category(cat, _Loc_collate_destroy, _Loc_collate_name,
                     collate_hash);
}
void _STLP_CALL __release_monetary(_Locale_monetary *cat) {
  __release_category(cat, _Loc_monetary_destroy, _Loc_monetary_name,
                     monetary_hash);
}
void _STLP_CALL __release_messages(_Locale_messages *cat) {
  __release_category(cat, _Loc_messages_destroy, _Loc_messages_name,
                     messages_hash);
}
...
inline locale::facet *_Locale_insert(_Locale *__that, Facet *f) {
...
void _Stl_loc_combine_names(_Locale *L, const char *name1, const char *name2,
                            locale::category c) {
  if ((c & locale::all) == 0 || strcmp(name1, name2) == 0)
    ...
    _Locale_compose_name(composite_buf, ctype_buf, numeric_buf, time_buf,
...
locale::locale(const char *name) : _M_impl(0) {
  if (!name)
...
locale::locale(const locale &L1, const locale &L2, category c) : _M_impl(0) {
  ...
  if (L1.name() != nameless && L2.name() != nameless)
    _Stl_loc_combine_names(impl, L1._M_impl->name.c_str(),
                           L2._M_impl->name.c_str(), c);
  else {
    impl->name = "*";
  }
  ...
  if (c & monetary) {
    ...
    impl->insert(
        i2,
        _STLP_STD::money_get<char,
                             istreambuf_iterator<char, char_traits<char>>>::id);
    impl->insert(
        i2,
        _STLP_STD::money_put<char,
                             ostreambuf_iterator<char, char_traits<char>>>::id);
#ifndef _STLP_NO_WCHAR_T
    ...
    impl->insert(
        i2,
        _STLP_STD::money_get<
            wchar_t, istreambuf_iterator<wchar_t, char_traits<wchar_t>>>::id);
    impl->insert(
        i2,
        _STLP_STD::money_put<
            wchar_t, ostreambuf_iterator<wchar_t, char_traits<wchar_t>>>::id);
#endif
  }
  if (c & numeric) {
    ...
    impl->insert(
        i2,
        _STLP_STD::num_get<char,
                           istreambuf_iterator<char, char_traits<char>>>::id);
    impl->insert(
        i2,
        _STLP_STD::num_put<char,
                           ostreambuf_iterator<char, char_traits<char>>>::id);
#ifndef _STLP_NO_WCHAR_T
    impl->insert(i2, _STLP_STD::numpunct<wchar_t>::id);
    impl->insert(
        i2, num_get<wchar_t,
                    istreambuf_iterator<wchar_t, char_traits<wchar_t>>>::id);
    impl->insert(
        i2, num_put<wchar_t,
  ...
  if (c & time) {
    impl->insert(
        i2,
        _STLP_STD::time_get<char,
                            istreambuf_iterator<char, char_traits<char>>>::id);
    impl->insert(
        i2,
        _STLP_STD::time_put<char,
                            ostreambuf_iterator<char, char_traits<char>>>::id);
#ifndef _STLP_NO_WCHAR_T
    impl->insert(
        i2,
...
void _Locale::insert_numeric_facets(const char *pname) {
  ...
  num_get<wchar_t, istreambuf_iterator<wchar_t, char_traits<wchar_t>>> *wget =
      0;
  num_put<wchar_t, ostreambuf_iterator<wchar_t, char_traits<wchar_t>>> *wput =
      0;
#endif
  ...
  if (pname == 0 || pname[0] == 0 || strcmp(pname, "C") == 0) {
    ...
    this->insert(
        i2, num_put<char, ostreambuf_iterator<char, char_traits<char>>>::id);
    this->insert(
        i2, num_get<char, istreambuf_iterator<char, char_traits<char>>>::id);
#ifndef _STLP_NO_WCHAR_T
...
void _Locale::insert_time_facets(const char *pname) {
...
#ifndef _STLP_NO_WCHAR_T
  time_get<wchar_t, istreambuf_iterator<wchar_t, char_traits<wchar_t>>> *wget =
      0;
  time_put<wchar_t, ostreambuf_iterator<wchar_t, char_traits<wchar_t>>> *wput =
      0;
#endif
  ...
  if (pname == 0 || pname[0] == 0 || strcmp(pname, "C") == 0) {
    ...
    this->insert(
        i2, time_get<char, istreambuf_iterator<char, char_traits<char>>>::id);
    this->insert(
        i2, time_put<char, ostreambuf_iterator<char, char_traits<char>>>::id);
#ifndef _STLP_NO_WCHAR_T
    this->insert(
        i2, time_get<wchar_t,
                     istreambuf_iterator<wchar_t, char_traits<wchar_t>>>::id);
    ...
    _STLP_TRY {
      get = new time_get_byname<char,
                                istreambuf_iterator<char, char_traits<char>>>(
          pname);
      put = new time_put_byname<char,
                                ostreambuf_iterator<char, char_traits<char>>>(
          pname);
#ifndef _STLP_NO_WCHAR_T
      wget = new time_get_byname<
          wchar_t, istreambuf_iterator<wchar_t, char_traits<wchar_t>>>(pname);
      wput = new time_put_byname<
          wchar_t, ostreambuf_iterator<wchar_t, char_traits<wchar_t>>>(pname);
#endif
    }
#ifndef _STLP_NO_WCHAR_T
...
void _Locale::insert_monetary_facets(const char *pname) {
  ...
  money_get<wchar_t, istreambuf_iterator<wchar_t, char_traits<wchar_t>>> *wget =
      0;
  money_put<wchar_t, ostreambuf_iterator<wchar_t, char_traits<wchar_t>>> *wput =
      0;
#endif
  ...
  if (pname == 0 || pname[0] == 0 || strcmp(pname, "C") == 0) {
    ...
    this->insert(
        i2, money_get<char, istreambuf_iterator<char, char_traits<char>>>::id);
    this->insert(
        i2, money_put<char, ostreambuf_iterator<char, char_traits<char>>>::id);
#ifndef _STLP_NO_WCHAR_T
    this->insert(i2, moneypunct<wchar_t, false>::id);
    this->insert(i2, moneypunct<wchar_t, true>::id);
    this->insert(
        i2, money_get<wchar_t,
                      istreambuf_iterator<wchar_t, char_traits<wchar_t>>>::id);
    this->insert(
        i2, money_put<wchar_t,
                      ostreambuf_iterator<wchar_t, char_traits<wchar_t>>>::id);
#endif
  } else {
    _STLP_TRY {
