...
basic_streambuf<char, char_traits<char>>::basic_streambuf(FILE *__get,
                                                          FILE *__put)
    : _M_get(__get ? __get : FILE_CAST(&_M_default_get)),
      _M_put(__put ? __put : FILE_CAST(&_M_default_put)), _M_locale() {
...
basic_streambuf<char, char_traits<char>>::seekoff(off_type, ios_base::seekdir,
                                                  ios_base::openmode) {
  return pos_type(-1);
}

basic_streambuf<char, char_traits<char>>::pos_type
basic_streambuf<char, char_traits<char>>::seekpos(pos_type,
                                                  ios_base::openmode) {
  return pos_type(-1);
}
...
streamsize basic_streambuf<char, char_traits<char>>::showmanyc() { return 0; }

streamsize basic_streambuf<char, char_traits<char>>::xsgetn(char *s,
                                                            streamsize n) {
  streamsize result = 0;
...
streamsize basic_streambuf<char, char_traits<char>>::xsputn(const char *s,
                                                            streamsize n) {
  streamsize result = 0;
  const int_type eof = traits_type::eof();
...
streamsize basic_streambuf<char, char_traits<char>>::_M_xsputnc(char c,
                                                                streamsize n) {
  streamsize result = 0;
  const int_type eof = traits_type::eof();
...
basic_streambuf<char, char_traits<char>>::_M_snextc_aux() {
  ...
  if (_FILE_I_avail(_M_get) == 0)
    ...
    _FILE_I_set(_M_get, _FILE_I_begin(_M_get), _FILE_I_end(_M_get),
                _FILE_I_end(_M_get));
