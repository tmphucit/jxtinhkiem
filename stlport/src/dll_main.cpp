...
_STLP_DECLSPEC const char *_STLP_CALL __get_c_string(const string &__str) {
  return __str.c_str();
}
...
const char *bad_exception::what() const _STLP_NOTHROW {
...
template class _STLP_CLASS_DECLSPEC __malloc_alloc<0>;
    ...
    (defined(_STLP_PTHREADS) || defined(_STLP_UITHREADS) ||                    \
...
#if defined(_STLP_DEBUG) && !defined(__SUNPRO_CC)
template class _STLP_CLASS_DECLSPEC
    _Nondebug_string<char, char_traits<char>, allocator<char>>;
#endif
