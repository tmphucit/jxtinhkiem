...
ctype_byname<char>::~ctype_byname() { __release_ctype(_M_ctype); }
...
struct _Ctype_byname_w_is_mask {
  ...
  _Ctype_byname_w_is_mask(/* ctype_base::mask */ int m, _Locale_ctype *c)
      : M((int)m), M_ctp(c) {}
...
ctype_byname<wchar_t>::ctype_byname(const char *name, size_t refs)
    : ctype<wchar_t>(refs), _M_ctype(__acquire_ctype(name)) {
  if (!_M_ctype)
    locale::_M_throw_runtime_error();
}

ctype_byname<wchar_t>::~ctype_byname() { __release_ctype(_M_ctype); }
...
collate_byname<char>::collate_byname(const char *name, size_t refs)
    : collate<char>(refs), _M_collate(__acquire_collate(name)) {
  if (!_M_collate)
    locale::_M_throw_runtime_error();
}

collate_byname<char>::~collate_byname() { __release_collate(_M_collate); }

int collate_byname<char>::do_compare(const char *__low1, const char *__high1,
                                     const char *__low2,
                                     const char *__high2) const {
  return _Locale_strcmp(_M_collate, __low1, __high1 - __low1, __low2,
                        __high2 - __low2);
}
...
collate_byname<char>::do_transform(const char *low, const char *high) const {
  ...
  char &__c1 = *(buf.begin());
  char &__c2 = (n == (size_t)-1) ? *(buf.begin() + (high - low - 1))
                                 : *(buf.begin() + n);
...
collate_byname<wchar_t>::collate_byname(const char *name, size_t refs)
    : collate<wchar_t>(refs), _M_collate(__acquire_collate(name)) {
  if (!_M_collate)
    locale::_M_throw_runtime_error();
}

collate_byname<wchar_t>::~collate_byname() { __release_collate(_M_collate); }

int collate_byname<wchar_t>::do_compare(const wchar_t *low1,
                                        const wchar_t *high1,
                                        const wchar_t *low2,
                                        const wchar_t *high2) const {
  return _Locale_strwcmp(_M_collate, low1, high1 - low1, low2, high2 - low2);
}
...
collate_byname<wchar_t>::do_transform(const wchar_t *low,
  ...
  _Locale_strwxfrm(_M_collate, &buf.front(), n, low, high - low);
  wchar_t &__c1 = *(buf.begin());
  wchar_t &__c2 = (n == (size_t)-1) ? *(buf.begin() + (high - low - 1))
                                    : *(buf.begin() + n);
...
codecvt_byname<char, char, mbstate_t>::codecvt_byname(const char * /* name */,
                                                      size_t refs)
...
codecvt_byname<wchar_t, char, mbstate_t>::codecvt_byname(const char *name,
                                                         size_t refs)
    : codecvt<wchar_t, char, mbstate_t>(refs), _M_ctype(__acquire_ctype(name)) {
  if (!_M_ctype)
    locale::_M_throw_runtime_error();
}
...
codecvt_byname<wchar_t, char, mbstate_t>::do_out(
    state_type &state, const wchar_t *from, const wchar_t *from_end,
    const wchar_t *&from_next, char *to, char *to_limit, char *&to_next) const {
  while (from != from_end) {
    size_t chars_stored =
        _Locale_wctomb(_M_ctype, to, to_limit - to, *from, &state);
...
codecvt_byname<wchar_t, char, mbstate_t>::do_in(state_type &state,
  ...
  while (from != from_end) {
    size_t chars_read =
        _Locale_mbtowc(_M_ctype, to, from, from_end - from, &state);
...
codecvt<wchar_t, char, mbstate_t>::result
codecvt_byname<wchar_t, char, mbstate_t>::do_unshift(
    state_type &state, extern_type *to, extern_type *to_limit,
    extern_type *&to_next) const {
  to_next = to;
  size_t result =
      _Locale_unshift(_M_ctype, &state, to, to_limit - to, &to_next);
  if (result == (size_t)-1)
    return error;
...
int codecvt_byname<wchar_t, char, mbstate_t>::do_length(const state_type &,
                                                        const extern_type *from,
...
numpunct_byname<char>::numpunct_byname(const char *name, size_t refs)
    : numpunct<char>(refs), _M_numeric(__acquire_numeric(name)) {
  if (!_M_numeric)
    locale::_M_throw_runtime_error();
...
numpunct_byname<char>::~numpunct_byname() { __release_numeric(_M_numeric); }
...
numpunct_byname<wchar_t>::numpunct_byname(const char *name, size_t refs)
    : numpunct<wchar_t>(refs), _M_numeric(__acquire_numeric(name)) {
  if (!_M_numeric)
    locale::_M_throw_runtime_error();
...
numpunct_byname<wchar_t>::~numpunct_byname() { __release_numeric(_M_numeric); }
...
moneypunct_byname<char, true>::moneypunct_byname(const char *name, size_t refs)
...
moneypunct_byname<char, false>::moneypunct_byname(const char *name, size_t refs)
...
wstring moneypunct_byname<wchar_t, true>::do_curr_symbol() const {
  string str = _Locale_int_curr_symbol(_M_monetary);
    ...
    defined(__MRC__) ||                                                        \
    defined(__SC__) //*ty 05/26/2001 - added workaround for mpw
...
_Messages_impl::_Messages_impl(bool is_wide) : _M_message_obj(0), _M_map(0) {
  _M_delete = true;
  if (is_wide)
...
int _Messages_impl::do_open(const string &filename, const locale &L) const {
  int result =
      _M_message_obj ? _Locale_catopen(_M_message_obj, filename.c_str()) : -1;
...
string _Messages_impl::do_get(catalog cat, int set, int p_id,
                              const string &dfault) const {
  return _M_message_obj != 0 && cat >= 0
             ? string(_Locale_catgets(_M_message_obj, cat, set, p_id,
                                      dfault.c_str()))
             : dfault;
}
...
wstring _Messages_impl::do_get(catalog thecat, int set, int p_id,
...
messages<char>::messages(size_t refs, _Locale_messages *msg_obj)
    : _BaseFacet(refs), _M_impl(new _Messages_impl(false, msg_obj)) {}
...
messages<wchar_t>::messages(size_t refs, _Locale_messages *msg_obj)
    : _BaseFacet(refs), _M_impl(new _Messages_impl(true, msg_obj)) {}
