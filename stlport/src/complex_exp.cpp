...
_STLP_DECLSPEC complex<float> _STLP_CALL exp(const complex<float> &z) {
  ...
  return complex<float>(expx * _STLP_COSF(z._M_im), expx * _STLP_SINF(z._M_im));
}

_STLP_DECLSPEC complex<double> _STLP_CALL exp(const complex<double> &z) {
  double expx = _STLP_EXP(z._M_re);
  return complex<double>(expx * _STLP_COS(z._M_im), expx * _STLP_SIN(z._M_im));
}

_STLP_DECLSPEC complex<long double>
    _STLP_CALL exp(const complex<long double> &z) {
...
_STLP_DECLSPEC complex<long double>
    _STLP_CALL log10(const complex<long double> &z) {
...
_STLP_DECLSPEC complex<long double>
    _STLP_CALL log(const complex<long double> &z) {
  complex<long double> result;
...
_STLP_DECLSPEC complex<double> _STLP_CALL pow(const complex<double> &z_in,
  ...
  if (n < 0)
    ...
    return double(1.0) / z; //*TY 04/15/2000 - it incorrectly assign long double
                            // attribute to floating point literals
#endif //*TY 04/15/2000 -
  else
    return z;
}
...
_STLP_DECLSPEC complex<long double>
    _STLP_CALL pow(const long double &a, const complex<long double> &b) {
...
_STLP_DECLSPEC complex<long double>
    _STLP_CALL pow(const complex<long double> &z_in, int n) {
...
_STLP_DECLSPEC complex<long double>
    _STLP_CALL pow(const complex<long double> &a, const long double &b) {
