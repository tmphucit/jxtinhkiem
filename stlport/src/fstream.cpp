...
//  on HP-UX 11, this one contradicts with pthread.h on pthread_atfork, unless
//  we unset this
...
#if !defined(__MSL__) && !defined(__MRC__) && !defined(__SC__) &&              \
    !defined(_STLP_WINCE) //*TY 04/15/2000 - exclude mpw compilers also
...
#pragma unused(fd)
  return true; // each file is a regular file under mac os, isn't it? (we don't
               // have fstat())
...
_Filebuf_base::_Filebuf_base()
    : _M_file_id((_STLP_fd)-1), _M_openmode(0), _M_is_open(false),
...
bool _Filebuf_base::_M_open(const char *name, ios_base::openmode openmode,
  ...
  (void)permission; // currently unused		//*TY 02/26/2000 - added to
...
#endif
                       dwDesiredAccess, dwShareMode, 0, dwCreationDisposition,
                       permission, 0);
  ...
  if ((doTruncate && SetEndOfFile(file_no) == 0) ||
      ((openmode & ios_base::ate) &&
       SetFilePointer(file_no, 0, NULL, FILE_END) == -1)) {
    CloseHandle(file_no);
    return false;
  }
...
#elif defined(__MRC__) ||                                                      \
    defined(__SC__) //*TY 02/26/2000 - added support for MPW compilers
  return this->_M_open(name, openmode, 0);
#else
  return this->_M_open(name, openmode,
...
#elif (defined(_STLP_USE_WIN32_IO) && defined(_MSC_VER) &&                     \
       !defined(_STLP_WINCE)) ||                                               \
    (defined(__MINGW32__) && defined(__MSVCRT__))
...
ptrdiff_t _Filebuf_base::_M_read(char *buf, ptrdiff_t n) {
  ...
  ReadFile(_M_file_id, (LPVOID)buf, (DWORD)n, &NumberOfBytesRead, 0);
...
bool _Filebuf_base::_M_write(char *buf, ptrdiff_t n) {
  ...
  while (true) {
    ...
    if (_M_openmode & ios_base::binary) {
      ...
      WriteFile(_M_file_id, (LPVOID)buf, (DWORD)n, &NumberOfBytesWritten, 0);
      ...
      while ((nextblocksize > 0) &&
             (nextlf = (char *)memchr(nextblock, _STLP_LF, nextblocksize)) !=
                 0) {
...
void *_Filebuf_base::_M_mmap(streamoff offset, streamoff len) {
  ...
  _M_view_id = CreateFileMapping(
      _M_file_id, (PSECURITY_ATTRIBUTES)0, PAGE_READONLY, 0 /* len >> 32 */,
...
int _STLP_CALL _Underflow<char, char_traits<char>>::_M_doit(
    basic_filebuf<char, char_traits<char>> *__this) {
  ...
  if (__this->_M_base.__regular_file() && __this->_M_always_noconv &&
