...
_STLP_STD::streambuf *stdio_streambuf_base::setbuf(char *s, streamsize n) {
  _STLP_VENDOR_CSTD::setvbuf(_M_file, s, (s == 0 && n == 0) ? _IONBF : _IOFBF,
                             n);
  return this;
}
...
stdio_streambuf_base::seekoff(off_type off, ios_base::seekdir dir,
  ...
  if (_STLP_VENDOR_CSTD::fseek(_M_file, off, whence) == 0) {
    ...
    // added 21 june 00 mdb,rjf,wjs: glibc 2.2 changed fpos_t to be a struct
...
stdio_streambuf_base::pos_type stdio_streambuf_base::seekpos(
    pos_type pos, ios_base::openmode /* mode */) // dwa 4/27/00 - suppress
...
streamsize stdio_ostreambuf::showmanyc() { return -1; }
