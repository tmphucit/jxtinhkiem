...
_Locale::_Locale(const _Locale_impl &L)
    : _Locale_impl(L), _Refcount_Base(1),
      facets_vec((void **)L.facets, (void **)L.facets + L.size()) {
...
locale::facet *_Locale::insert(locale::facet *f, size_t index, bool do_incr) {
  if (f != 0 && index != 0) {
    if (index >= facets_vec.size()) {
      facets_vec.insert(facets_vec.end(), index - facets_vec.size() + 1,
                        (void *)0);
      facets = (locale::facet **)&facets_vec[0];
      _M_size = facets_vec.size();
    }
...
locale::locale(_Locale_impl *impl, bool do_copy) : _M_impl(0) {
  if (do_copy) {
