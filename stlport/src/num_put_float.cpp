...
// # if !(defined(_STLP_USE_GLIBC) || defined(__FreeBSD__) ||
// defined(__NetBSD__) || defined (_AIX) || defined(__MVS__) || defined
// (__OS400__) || defined (__QNXNTO__) || defined (__APPLE__) || defined
// (__DJGPP))
...
#include <cstdlib>
    ...
    defined(__DJGPP) || defined(_STLP_SCO_OPENSERVER) || defined(__NCR_SVR)
#include <float.h>
#endif
...
#define _ISUNORDERED(x, y)                                                     \
  (_IS32(x) && _IS32(y) ? _Isunorderedf(x, y) : _Isunordered(x, y))
...
#if defined(_AIX)
// Some OS'es only provide non-reentrant primitives, so we have to use
...
#define RETURN_CVT(ecvt, x, n, pt, sign, buf)                                  \
  strcpy(buf, ecvt(x, n, pt, sign));                                           \
  return buf;
#else
#define LOCK_CVT
...
#elif defined(__hpux) || defined(__DJGPP) ||                                   \
...
#endif
inline bool _Stl_is_neg_nan(double x) {
  return isnan(x) && (copysign(1., x) < 0);
}
...
inline bool _Stl_is_neg_nan(double x) {
  return _isnan(x) && _copysign(1., x) < 0;
    ...
    defined(__SC__) //*TY 02/24/2000 - added support for MPW
...
bool _Stl_is_neg_inf(double x) {
  return _Stl_is_inf(x) && (copysign(1., x) < 0);
}
bool _Stl_is_neg_nan(double x) { return isnan(x) && (copysign(1., x) < 0); }
#elif defined(__ISCPP__)
inline bool _Stl_is_nan_or_inf(double x) {
  return _fp_isINF(x) || _fp_isNAN(x);
}
...
inline char *_Stl_fcvtR(double x, int n, int *pt, int *sign, char *buf) {
  return fcvt(x, n, pt, sign);
    ...
    (defined(_PTHREADS_DRAFT4) || defined(PTHREAD_THREADS_MAX))
inline char *_Stl_qecvtR(long double x, int n, int *pt, int *sign, char *buf) {
  return (_ldecvt_r(*(long_double *)&x, n, pt, sign, buf, NDIG + 2) == 0 ? buf
                                                                         : 0);
}
inline char *_Stl_qfcvtR(long double x, int n, int *pt, int *sign, char *buf) {
  return (_ldfcvt_r(*(long_double *)&x, n, pt, sign, buf, NDIG + 2) == 0 ? buf
                                                                         : 0);
}
#else
inline char *_Stl_qecvtR(long double x, int n, int *pt, int *sign, char *buf) {
...
inline char *_Stl_ecvtR(double x, int n, int *pt, int *sign, char *buf) {
  return _ecvt(x, n, pt, sign);
}
inline char *_Stl_fcvtR(double x, int n, int *pt, int *sign, char *buf) {
  return _fcvt(x, n, pt, sign);
}
#ifndef _STLP_NO_LONG_DOUBLE
inline char *_Stl_qecvtR(long double x, int n, int *pt, int *sign, char *buf) {
  return _ecvt((double)x, n, pt, sign);
}
inline char *_Stl_qfcvtR(long double x, int n, int *pt, int *sign, char *buf) {
  return _fcvt((double)x, n, pt, sign);
}
#endif
#elif defined(__ISCPP__)
inline char *_Stl_ecvtR(double x, int n, int *pt, int *sign, char *buf) {
...
void __format_float_scientific(char *buf, const char *bp, int decpt, int sign,
                               bool is_zero, ios_base::fmtflags flags,
...
void __format_float_fixed(char *buf, const char *bp, int decpt, int sign,
                          bool /* x */, ios_base::fmtflags flags, int precision,
                          bool islong) {
  ...
  do {
    *buf++ = (char)((nn <= 0 || *bp == 0 || k >= maxfsig) ? '0' : (k++, *bp++));
  } while (--nn > 0);
...
void __format_nan_or_inf(char *buf, double x, ios_base::fmtflags flags) {
...
static inline void __format_float(char *buf, const char *bp, int decpt,
                                  int sign, max_double_type x,
                                  ios_base::fmtflags flags, int precision,
                                  bool islong) {
  ...
  if (_Stl_is_nan_or_inf((double)x)) { // Infinity or NaN
    ...
    case ios_base::fixed:
      __format_float_fixed(buf, bp, decpt, sign, true, flags, precision,
                           islong);
      break;
    ...
    default: // g format
      ...
      if (decpt < -3 || decpt > precision) {
        precision = kk - 1;
        __format_float_scientific(buf, bp, decpt, sign, x == 0, flags,
                                  precision, islong);
      } else {
        precision = kk - decpt;
        __format_float_fixed(buf, bp, decpt, sign, true, flags, precision,
...
static int fill_fmtbuf(char *fmtbuf, ios_base::fmtflags flags,
                       char long_modifier) {
  fmtbuf[0] = '%';
  int i = 1;
...
char *_STLP_CALL __write_float(char *buf, ios_base::fmtflags flags,
...
wchar_t *_STLP_CALL __convert_float_buffer(const char *first, const char *last,
                                           wchar_t *out,
...
void _STLP_CALL __adjust_float_buffer(char *first, char *last, char dot) {
