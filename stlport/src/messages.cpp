...
messages<char>::messages(_Messages *imp) : _BaseFacet(1), _M_impl(imp) {
  imp->_M_delete = false;
}

messages<char>::~messages() {
  if (_M_impl && _M_impl->_M_delete)
    delete _M_impl;
}
...
string messages<char>::do_get(catalog cat, int set, int p_id,
                              const string &dfault) const {
  return _M_impl->do_get(cat, set, p_id, dfault);
}

void messages<char>::do_close(catalog cat) const { _M_impl->do_close(cat); }
...
messages<wchar_t>::messages(_Messages *imp) : _BaseFacet(1), _M_impl(imp) {
  imp->_M_delete = false;
}
...
wstring messages<wchar_t>::do_get(catalog thecat, int set, int p_id,
                                  const wstring &dfault) const {
  return _M_impl->do_get(thecat, set, p_id, dfault);
}

void messages<wchar_t>::do_close(catalog cat) const { _M_impl->do_close(cat); }

wstring _Messages::do_get(catalog, int, int, const wstring &dfault) const {
  return dfault;
}
