...
#ifndef _STLP_NO_LONG_DOUBLE
_STLP_DECLSPEC complex<long double> _STLP_CALL polar(const long double &__rho,
                                                     const long double &__phi) {
  return complex<long double>(__rho * _STLP_COSL(__phi),
                              __rho * _STLP_SINL(__phi));
}
#endif

// Division

void _STLP_CALL complex<float>::_div(const float &__z1_r, const float &__z1_i,
...
void _STLP_CALL complex<float>::_div(const float &__z1_r, const float &__z2_r,
