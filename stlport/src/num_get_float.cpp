...
void _STLP_CALL _Initialize_get_float(const ctype<wchar_t> &ct, wchar_t &Plus,
                                      wchar_t &Minus, wchar_t &pow_e,
                                      wchar_t &pow_E, wchar_t *digits) {
...
#define ULL(x) x##ULL
    ...
    defined(__SC__) //*TY 02/25/2000 - added support for MPW compilers
...
#else //*TY 03/20/2000 - added support for SCpp which lacks native 64bit integer
      // type
...
static const short _Stl_twoexp[80] = {
    ...
    70,   74,   77,   80,   84,   87,   90,   183,   276,   369,
    462,  555,  648,  741,  834,  927,  1020, -93,   -186,  -279,
    -372, -465, -558, -651, -744, -837, -930, -1023, -1116, -1209};
...
void _Stl_norm_and_round(uint64 &p, int &norm, uint64 prodhi, uint64 prodlo) {
  ...
  if ((prodlo & _Stl_HIBITULL) != 0) {
    /* first guard bit a one */ //*TY 03/25/2000 - added explicit comparison to
                                // zero to avoid reliance to the implicit
                                // conversion from uint64 to bool
    ...
    bool b1 = ((p & 0x1) != 0);
    if (b1 ||
        prodlo != _Stl_HIBITULL) { //*TY 03/25/2000 - SCpp confuses on this
                                   // particular original boolean expression
#endif //*TY 03/25/2000 -
      /* round */
      p++;
      if (p == 0)
...
//*TY 04/06/2000 - powermac's 68K emulator utilizes apple's SANE floating point,
// which is not compatible with IEEE format.
_STLP_END_NAMESPACE
#include <fp.h>
_STLP_BEGIN_NAMESPACE
...
double _Stl_atod(char *buffer, int ndigit, int dexp) {
  ...
  if ((value >> 32) != 0) {
    nzero = 32;
  } //*TY 03/25/2000 - added explicit comparison to zero to avoid uint64 to bool
    // conversion operator
  if ((value >> (16 + nzero)) != 0) {
    nzero += 16;
  }
  ...
  /* Normalize */
  value <<= /*(uint64)*/ (
      64 - nzero); //*TY 03/25/2000 - removed extraneous cast to uint64
  bexp -= 64 - nzero;
...
#else
        guard = to_ulong((value >> 63) &
                         1); //*TY 03/25/2000 - use member function instead of
                             // problematic conversion operator utilization
#endif
        value = 0;
      } else {
...
long double _Stl_string_to_long_double(const char *s) {
...
void _STLP_CALL __string_to_float(const string &v, float &val) {
  val = _Stl_string_to_double(v.data());
}

void _STLP_CALL __string_to_float(const string &v, double &val) {
  val = _Stl_string_to_double(v.data());
}

#ifndef _STLP_NO_LONG_DOUBLE
void _STLP_CALL __string_to_float(const string &v, long double &val) {
