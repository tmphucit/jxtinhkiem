...
strstreambuf::strstreambuf(streamsize initial_capacity)
    : _M_alloc_fun(0), _M_free_fun(0), _M_dynamic(true), _M_frozen(false),
      _M_constant(false) {
...
strstreambuf::strstreambuf(__alloc_fn alloc_f, __free_fn free_f)
    : _M_alloc_fun(alloc_f), _M_free_fun(free_f), _M_dynamic(true),
...
strstreambuf::strstreambuf(char *get, streamsize n, char *put)
    : _M_alloc_fun(0), _M_free_fun(0), _M_dynamic(false), _M_frozen(false),
      _M_constant(false) {
  _M_setup(get, put, n);
}
...
strstreambuf::strstreambuf(unsigned char *get, streamsize n, unsigned char *put)
    : _M_alloc_fun(0), _M_free_fun(0), _M_dynamic(false), _M_frozen(false),
      _M_constant(false) {
  _M_setup(__REINTERPRET_CAST(char *, get), __REINTERPRET_CAST(char *, put), n);
}
...
strstreambuf::strstreambuf(const unsigned char *get, streamsize n)
    : _M_alloc_fun(0), _M_free_fun(0), _M_dynamic(false), _M_frozen(false),
      _M_constant(true) {
  _M_setup(__REINTERPRET_CAST(char *, __CONST_CAST(unsigned char *, get)), 0,
           n);
}
...
int strstreambuf::pcount() const { return int(pptr() ? pptr() - pbase() : 0); }
...
basic_streambuf<char, char_traits<char>> *strstreambuf::setbuf(char *,
                                                               streamsize) {
  return this;
}

strstreambuf::pos_type strstreambuf::seekoff(off_type off,
...
strstreambuf::pos_type strstreambuf::seekpos(pos_type pos,
                                             ios_base::openmode mode) {
...
ostrstream::ostrstream() : basic_ostream<char, char_traits<char>>(0), _M_buf() {
...
void ostrstream::freeze(bool freezeflag) { _M_buf.freeze(freezeflag); }

char *ostrstream::str() { return _M_buf.str(); }

int ostrstream::pcount() const { return _M_buf.pcount(); }

//----------------------------------------------------------------------
// Class strstream

strstream::strstream() : basic_iostream<char, char_traits<char>>(0), _M_buf() {
  basic_ios<char, char_traits<char>>::init(&_M_buf);
}
...
void strstream::freeze(bool freezeflag) { _M_buf.freeze(freezeflag); }

int strstream::pcount() const { return _M_buf.pcount(); }

char *strstream::str() { return _M_buf.str(); }
