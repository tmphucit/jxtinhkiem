...
function MoveNPC(nMap,nX,nY,nOffsetX,nOffsetY,nPrice,nFightState,nTermini)
    ...
    posy = floor((nY+nOffsetY/10)*16)
   		if (GetCash() >= nPrice) then
			...
			Say("Quy Khach khong co du "..nPrice.." luong", 1,"Thoat./Exit")
		end
end;
---------------------------------------------------------------------------------------Menpai and Skill--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
function bmxSay(TH,CF)
	...
	__next = function() page_current(page_current()+1) bmxSay(%TH,%CF) end
	__back = function()  page_current(page_current()-1) bmxSay(%TH,%CF) end
	...
	dostring("Say("..content..","..(choice_per_page+default_choice_per_page+_bmxGetn(bfc))..",".._bmxFStr(choice["_c"], choice_per_page, choice_begin)..",".._bmxFStr(choice["_d"], default_choice_per_page)..",".._bmxFStr(bfc)..");")
end
...
--Leerim
Task_LuuNgayThangNam = 255
function updateNgay()
	-- Msg2Player("FUCK "..Task_LuuNgayThangNam);
	local nNgay_Thang_Nam = tonumber(date("%d%m%y")) 
	-- Msg2Player(format("Ngay thang hien tai: %d",nNgay_Thang_Nam));
	-- Msg2Player("Ngay thang da luu: "..GetTask(Task_LuuNgayThangNam));
	if (GetTask(Task_LuuNgayThangNam) ~= nNgay_Thang_Nam) then
		SetTask(185,0) --VA
		SetTask(188,0) --PLD
		SetTask(191,0) --KyLan
		SetTask(193,0) --HuyHoang
		SetTask(189,0) --HuyHoang
		-- Msg2Player("VO DAY CHUA MAI");
		SetTask(Task_LuuNgayThangNam, nNgay_Thang_Nam)
		
		Msg2Player("H«m nay lµ ngµy :"..GetTask(Task_LuuNgayThangNam).."")
	end
end 