...
Msg2Player("§¼ng cÊp 100 míi cã thÓ t¹o lËp bang héi !")
return
end
...
if GetLevel() < 90 then
Talk(1,"","§¼ng cÊp 90 míi cã thÓ gia nhËp Bang Héi !")
return
end
...
if GetTaskTemp(tempbang) == 1000 then
	...
	Talk(1,"","Chøc n¨ng ch­a cËp nhËt !")
...
LoadTongMapMain()
	...
	Talk(1,"","ThuÕ suÊt chØ ®­îc nhËp tõ 1 - 20% ")
end
end
end
...
Say("<color=green>Qu¶n Lý Khu Vùc<color>: Mçi bang héi cã mét l·nh ®Þa riªng, ta lµ ng­êi qu¶n lý viÖc ph©n chia l·nh ®Þa cho c¸c bang héi",3,
...
Talk(1,"","B¹n kh«ng ®ñ 10.000 v¹n l­îng")
return
end
...
if SoThanhVien + 1 >= 80 then
	...
	Talk(1,"","Bang héi cña b¹n kh«ng ®ñ 80 thµnh viªn, kh«ng thÓ thiÕt lËp khu vùc Bang Héi !")
	return
end
...
if GetTongFigure() == 3 then
	...
	"N©ng cÊp Thî RÌn/nangcapthoren",
	"N©ng cÊp Boss Bang Héi/nangcapboss",
	"Rót C«ng Thµnh ChiÕn LÔ Bao /rutctclb",
	"Kick Ng­êi Ch¬i L¹/kicknguoichoi",
	"Tho¸t./no")
else
	Say("<color=green>Qu¶n Lý L·nh §Þa<color>: HiÖn t¹i ®ang cã <color=red>"..soluongthanhvienlanhdia().."<color> thµnh viªn trong L·nh §Þa Bang Héi. ",1,
	"Tho¸t./no")
...
if MAP_BANG_HOI[vitri][3] < 6 then
	if GetCash() >= tien then
		...
		if GetCash() == sl-tien then
			...
			Msg2SubWorld("<color=pink>Chóc mõng Bang <color=green>"..GetTongName().." <color=pink>®· th¨ng cÊp thµnh c«ng <color=yellow>Thî RÌn L·nh §Þa lªn cÊp "..(MAP_BANG_HOI[vitri][3]+1).."<color=pink>, x¸c suÊt th¨ng cÊp trang bÞ vµ th¨ng cÊp huyÒn tinh cña Bang Héi ®­îc t¨ng thªm: <color=yellow>"..(MAP_BANG_HOI[vitri][3]*2).."% ")	
