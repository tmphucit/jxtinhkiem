...
if (GetTask(SJTASKKEY) == GetMissionV(MS_KEY)) then
 if (GetTask(SJTASKVLAUE) == 1) then
    ...
    Say("金方人数太多了，你目前不能再加金方了！目前宋金人数为"..GetMSPlayerCount(1,1)..":"..GetMSPlayerCount(1,2),0);
    return 0;
    end
    if (GetCash() >= MS_SIGN_MONEY) then
       Pay(MS_SIGN_MONEY) 
    else
    Say("你的报名费不够啊，需要50万两！",0);
    return 0;
    end
...
if (GetTask(SJTASKKEY) == GetMissionV(MS_KEY)) then
 if (GetTask(SJTASKVLAUE) == 2) then
    ...
    Say("宋方人数太多了，你目前不能再加宋方了！目前宋金人数为"..GetMSPlayerCount(1,1)..":"..GetMSPlayerCount(1,2),0);
    return 0;
    end
    if (GetCash() >= MS_SIGN_MONEY) then
       Pay(MS_SIGN_MONEY) 
