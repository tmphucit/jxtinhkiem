Include("\\datascript\\kethon\\danhsachkethon.lua")
...
if check_tinh_trang_hon_nhan() == 0 then
	...
	if GetSex() == 0 then
		if GetTaskTemp(6) == 0 then
			Say("<color=green>NguyÖt L·o<color>: Ng­¬i ®Õn t×m ta cã viÖc g×? ",2,
			"Ta muèn cÇu h«n víi c« g¸i nµy, nhê ng­êi gióp ta to¹i nguyÖn /male_hurry",
			"Ta ch­a chuÈn bÞ kü, gÆp l¹i ng­êi sau !/no")
		else
			Say("<color=green>NguyÖt L·o<color>: ý trung nh©n cña ng­¬i vÉn ch­a tr¶ lêi ng­¬i, cã cÇn mêi c« ta nhanh nhanh mét chót kh«ng? ",2,
		...
		if GetTaskTemp(6) == 0 then
			...
			Say("<color=green>NguyÖt L·o<color>: anh ta ®ang ®îi ng­¬i tr¶ lêi k×a, ®· suy nghÜ kü lµ muèn cïng víi anh ta ®i suèt cuéc ®êi hay ch­a?",2,
	...
	Say("<color=green>NguyÖt L·o<color>: T×nh tr¹ng h«n nh©n cña 2 ng­êi vÉn æn chø ? ",2,
	"TEST 1/no",
	"TEST 2/no")	
end
end
