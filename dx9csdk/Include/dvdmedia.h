...
#define AMINTERLACE_IsInterlaced                                               \
...
#define AMCOPYPROTECT_RestrictDuplication                                      \
...
#define AMMPEG2_DVDLine21Field2                                                \
...
#define AMMPEG2_DVB_UserData                                                   \
  0x00000080 // if set, the MPEG-2 decoder must process DVB style user data
#define AMMPEG2_27MhzTimebase                                                  \
  0x00000100 // if set, the PTS,DTS timestamps advance at 27MHz rather than
             // 90KHz

#define AMMPEG2_WidescreenAnalogOut                                            \
  0x00000200 // if set and this stream is sent to an analog output, it should
...
#define AMCONTROL_USED                                                         \
  0x00000001 // Used to test if these flags are supported.  Set and test for
             // AcceptMediaType. If rejected, then you cannot use the AMCONTROL
             // flags (send 0 for dwReserved1)
#define AMCONTROL_PAD_TO_4x3                                                   \
  0x00000002 // if set means display the image in a 4x3 area
#define AMCONTROL_PAD_TO_16x9                                                  \
  0x00000004 // if set means display the image in a 16x9 area
...
typedef struct tagVIDEOINFOHEADER2 {
  ...
  union {
    ...
    DWORD dwReserved1; // for backward compatiblity (was "must be 0"; connection
                       // rejected otherwise)
  };
...
#define SIZE_MPEG2VIDEOINFO(pv)                                                \
  (FIELD_OFFSET(MPEG2VIDEOINFO, dwSequenceHeader[0]) + (pv)->cbSequenceHeader)
...
//===================================================================================
// flags for dwTypeSpecificFlags in AM_SAMPLE2_PROPERTIES which define type
// specific data in IMediaSample2
//===================================================================================

#define AM_VIDEO_FLAG_FIELD_MASK                                               \
  0x0003L // use this mask to check whether the sample is field1 or field2 or
          // frame
#define AM_VIDEO_FLAG_INTERLEAVED_FRAME                                        \
  0x0000L // the sample is a frame (remember to use AM_VIDEO_FLAG_FIELD_MASK
          // when using this)
#define AM_VIDEO_FLAG_FIELD1                                                   \
  0x0001L // the sample is field1 (remember to use AM_VIDEO_FLAG_FIELD_MASK when
          // using this)
#define AM_VIDEO_FLAG_FIELD2                                                   \
  0x0002L // the sample is the field2 (remember to use AM_VIDEO_FLAG_FIELD_MASK
          // when using this)
...
#define AM_VIDEO_FLAG_IPB_MASK                                                 \
  0x0030L // use this mask to check whether the sample is I, P or B
#define AM_VIDEO_FLAG_I_SAMPLE                                                 \
  0x0000L // I Sample (remember to use AM_VIDEO_FLAG_IPB_MASK when using this)
#define AM_VIDEO_FLAG_P_SAMPLE                                                 \
...
#define AM_VIDEO_FLAG_REPEAT_FIELD                                             \
  0x0040L // if set means display the field which has been displayed first again
          // after displaying both fields first. This bit is irrelavant for
          // 1FieldPerSample mode
...
typedef struct {
  REFERENCE_TIME StartInTime;  // stream time (input) at which to start decoding
                               // at this rate
  REFERENCE_TIME StartOutTime; // reference time (output) at which to start
                               // showing at this rate
  LONG Rate;                   // new rate * 10000 (decimal)
} AM_DVD_ChangeRate;

typedef LONGLONG AM_DVD_DecoderPosition;

typedef enum { DVD_DIR_FORWARD = 0, DVD_DIR_BACKWARD = 1 } DVD_PLAY_DIRECTION;
