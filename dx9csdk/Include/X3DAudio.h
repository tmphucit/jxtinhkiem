...
// speed of sound in meters per second for dry air at approximately 20C, used
// with X3DAudioInitialize
#define X3DAUDIO_SPEED_OF_SOUND 343.5f

// calculation control flags, used with X3DAudioCalculate
#define X3DAUDIO_CALCULATE_MATRIX                                              \
  0x00000001 // enable matrix coefficient table calculation
#define X3DAUDIO_CALCULATE_DELAY                                               \
  0x00000002 // enable delay time array calculation (stereo final mix only)
#define X3DAUDIO_CALCULATE_LPF_DIRECT                                          \
...
#define X3DAUDIO_CALCULATE_REVERB                                              \
  0x00000010 // enable reverb send level calculation
#define X3DAUDIO_CALCULATE_DOPPLER                                             \
  0x00000020 // enable doppler shift factor calculation
#define X3DAUDIO_CALCULATE_EMITTER_ANGLE                                       \
  0x00000040 // enable emitter-to-listener interior angle calculation

#define X3DAUDIO_CALCULATE_ZEROCENTER                                          \
  0x00010000 // do not position to front center speaker, center destination
             // channel will be zero in returned matrix coefficient table, used
             // only for matrix calculations and only for final mix formats that
             // have a front center channel
...
typedef UINT_PTR
    UNWORD; // unsigned natural machine word, bytesize platform specific
...
//      pPoints[k-1].DSPSetting +
//      ((pPoints[k].DSPSetting-pPoints[k-1].DSPSetting) /
//      (pPoints[k].Distance-pPoints[k-1].Distance)) *
//      (pPoints[k].Distance-pPoints[k-1].Distance) != NAN or infinite values
...
static const X3DAUDIO_DISTANCE_CURVE X3DAudioDefault_LinearCurve = {
    (X3DAUDIO_DISTANCE_CURVE_POINT *)&X3DAudioDefault_LinearCurvePoints[0], 2};
...
typedef struct X3DAUDIO_CONE {
  FLOAT32 InnerAngle; // inner cone angle in radians, must be within [0.0f,
                      // X3DAUDIO_2PI]
  FLOAT32 OuterAngle; // outer cone angle in radians, must be within
  ...
  FLOAT32 InnerLPF; // LPF (both direct and reverb paths) coefficient scaler
                    ...
                    // when used
  FLOAT32 OuterLPF; // LPF (both direct and reverb paths) coefficient scaler
                    // on/beyond outer cone, used only for LPF (both direct and
                    // reverb paths) calculations, must be within [0.0f, 1.0f]
                    // when used
  FLOAT32 InnerReverb; // reverb send level scaler on/within inner cone, used
                       // only for reverb calculations, must be within
                       // [0.0f, 2.0f] when used
  FLOAT32 OuterReverb; // reverb send level scaler on/beyond outer cone, used
                       // only for reverb calculations, must be within
                       // [0.0f, 2.0f] when used
} X3DAUDIO_CONE, *LPX3DAUDIO_CONE;
static const X3DAUDIO_CONE X3DAudioDefault_DirectionalCone = {
    X3DAUDIO_PI / 2, X3DAUDIO_PI, 1.0f, 0.708f, 1.0f, 0.75f, 0.708f, 1.0f};

// Listener:
// Defines a point of 3D audio reception.
typedef struct X3DAUDIO_LISTENER {
  X3DAUDIO_VECTOR OrientFront; // orientation of front direction, used only for
...
typedef struct X3DAUDIO_EMITTER {
  ...
  *pReverbCurve; // reverb send level distance curve, used only for reverb
  ...
  FLOAT32 CurveDistanceScaler; // curve distance scaler, used to scale
                               ...
                               // world units and/or exaggerate their effect,
...
typedef struct X3DAUDIO_DSP_SETTINGS {
  ...
  *pDelayTimes; // [in] delay time array, receives delays for each
  ...
  UINT32 DstChannelCount; // [in] number of destination channels, must equal
                          // number of channels on the final mix
  ...
  FLOAT32 DopplerFactor; // [out] doppler shift factor, scales resampler ratio
                         // for doppler shift effect, where the effective
                         // frequency = DopplerFactor * original frequency
  FLOAT32 EmitterToListenerAngle; // [out] emitter-to-listener interior angle,
                                  // expressed in radians with respect to the
                                  // emitter's front orientation

  FLOAT32 EmitterToListenerDistance; // [out] distance in user-defined world
                                     // units from the emitter base to listener
                                     // position, always calculated
...
X3DAUDIO_API_(void)
X3DAudioInitialize(UINT32 SpeakerChannelMask, FLOAT32 SpeedOfSound,
                   X3DAUDIO_HANDLE Instance);

// calculates DSP settings with respect to 3D parameters
X3DAUDIO_API_(void)
X3DAudioCalculate(const X3DAUDIO_HANDLE Instance,
                  const X3DAUDIO_LISTENER *pListener,
                  const X3DAUDIO_EMITTER *pEmitter, UINT32 Flags,
