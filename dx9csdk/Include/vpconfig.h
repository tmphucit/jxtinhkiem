...
public:
  ...
  (THIS_ IN OUT LPAMVPSIZE pamvpSize, OUT LPDWORD pdwMaxPixelsPerSecond) PURE;

  // informs the callee of the videoformats supported by the videoport
  STDMETHOD(InformVPInputFormats)
  (THIS_ IN DWORD dwNumFormats, IN LPDDPIXELFORMAT pDDPixelFormats) PURE;
  ...
  (THIS_ IN OUT LPDWORD pdwNumFormats, IN OUT LPDDPIXELFORMAT pddPixelFormats)
      PURE;
  ...
  (THIS_ OUT LPDIRECTDRAWSURFACE * ppddOverlaySurface) PURE;

  // sets the direct draw kernel handle
  STDMETHOD(SetDirectDrawKernelHandle)
  (THIS_ IN ULONG_PTR dwDDKernelHandle) PURE;
  ...
  (THIS_ IN DWORD cHandles, IN ULONG_PTR * rgDDKernelHandles) PURE;
  ...
  (THIS_ IN DWORD dwPitch, IN DWORD dwXOrigin, IN DWORD dwYOrigin) PURE;
};
...
// IVPVBIConfig
DECLARE_INTERFACE_(IVPVBIConfig, IVPBaseConfig){public : };
