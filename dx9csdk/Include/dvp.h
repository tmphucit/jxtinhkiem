...
DEFINE_GUID(IID_IDDVideoPortContainer, 0x6C142760, 0xA733, 0x11CE, 0xA5, 0x21,
            0x00, 0x20, 0xAF, 0x0B, 0xE5, 0x60);
DEFINE_GUID(IID_IDirectDrawVideoPort, 0xB36D93E0, 0x2B43, 0x11CF, 0xA2, 0xDE,
            0x00, 0xAA, 0x00, 0xB9, 0x33, 0x56);
DEFINE_GUID(IID_IDirectDrawVideoPortNotify, 0xA655FB94, 0x0589, 0x4E57, 0xB3,
            0x33, 0x56, 0x7A, 0x89, 0x46, 0x8C, 0x88);
...
DEFINE_GUID(DDVPTYPE_BROOKTREE, 0x1352A560L, 0xDA61, 0x11CF, 0x9B, 0x06, 0x00,
            0xA0, 0xC9, 0x03, 0xA3, 0xB8);
...
/*
 * These definitions are required to allow polymorphic structure members (i.e.
 * those that are referred to both as DWORDs and as pointers) to resolve into a
 * type of correct size to hold the largest of those two types (i.e. pointer) on
 * 64 bit systems. For 32 bit environments, ULONG_PTR resolves to a DWORD.
 */
#ifndef MAXULONG_PTR
...
typedef struct IDirectDrawVideoPortNotifyVtbl
...
DECLARE_INTERFACE_(IDDVideoPortContainer, IUnknown) {
  ...
  (THIS_ DWORD, LPDDVIDEOPORTDESC, LPDIRECTDRAWVIDEOPORT FAR *, IUnknown FAR *)
      PURE;
  STDMETHOD(EnumVideoPorts)
  (THIS_ DWORD, LPDDVIDEOPORTCAPS, LPVOID, LPDDENUMVIDEOCALLBACK) PURE;
  STDMETHOD(GetVideoPortConnectInfo)
  (THIS_ DWORD, LPDWORD, LPDDVIDEOPORTCONNECT) PURE;
...
typedef struct _DDVIDEOPORTCAPS {
  ...
  DWORD dwAlignVideoPortBoundary; // Byte restriction of placement within the
                                  // surface
  ...
  DWORD dwNumVBIAutoFlipSurfaces; // Max number of VBI autoflippable surfaces
                                  // allowed
  DWORD dwNumPreferredAutoflip; // Optimal number of autoflippable surfaces for
                                // hardware
  WORD wNumFilterTapsX; // Number of taps the prescaler uses in the X direction
                        // (0 - no prescale, 1 - replication, etc.)
  WORD wNumFilterTapsY; // Number of taps the prescaler uses in the Y direction
                        // (0 - no prescale, 1 - replication, etc.)
} DDVIDEOPORTCAPS;
...
typedef struct _DDVIDEOPORTINFO {
  ...
  DWORD dwPrescaleWidth;  // Determines pre-scaling/zooming in the X direction
                          // (optional).
  DWORD dwPrescaleHeight; // Determines pre-scaling/zooming in the Y direction
                          // (optional).
  ...
  DWORD dwVBIHeight;     // Specifies the number of lines of data within the
...
typedef struct _DDVIDEOPORTBANDWIDTH {
  ...
  DWORD dwYInterpolate; // Zoom factor at which overlay w/ Y interpolation is
                        // supported
  DWORD dwYInterpAndColorkey; // Zoom factor at which ovelray w/ Y interpolation
                              // and colorkeying is supported
  ULONG_PTR dwReserved1;      // Reserved for future use - set to zero
  ULONG_PTR dwReserved2;      // Reserved for future use - set to zero
} DDVIDEOPORTBANDWIDTH;
...
typedef struct _DDVIDEOPORTNOTIFY {
  ...
  UINT dwSurfaceIndex; // Index in the surface chain of the surface that
                       // received the sample
