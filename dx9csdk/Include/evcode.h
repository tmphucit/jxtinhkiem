...
//
//
// Note that for EC_WMT_EVENT events the wmsdk-based filters use the following
// structure for passing event parameters to the app:
...
//
// lParam1 is one of the enum WMT_STATUS messages listed below, sent by the
// WindowsMedia SDK lParam2 is specific to the lParam event
//
//     the following WMT_STATUS messages are sent for this event:
//         WMT_STARTED        - lParam2 is 0
//         WMT_CLOSED         - lParam2 is 0
//         WMT_INDEX_PROGRESS - lParam2 is a DWORD containing the progress
//         percent complete
//
...
//
// lParam1 is one of the enum WMT_STATUS messages listed below, sent by the
// WindowsMedia SDK lParam2 is a pointer an AM_WMT_EVENT_DATA structure where,
//                          hrStatus is the status code sent by the wmsdk
//                          pData is specific to the lParam1 event
//
//     the following WMT_STATUS messages are sent by the WMSDK Reader filter for
//     this event:
//         WMT_NO_RIGHTS        - pData is a pointer to a WCHAR string
//         containing a challenge URL WMT_ACQUIRE_LICENSE  - lParam2 is a
//         pointer to a WM_GET_LICENSE_DATA struct WMT_NO_RIGHTS_EX     -
//         lParam2 is a pointer to a WM_GET_LICENSE_DATA struct
//         WMT_NEEDS_INDIVIDUALIZATION - lParam2 is NULL
//         WMT_INDIVIDUALIZE    - lParam2 is a pointer to a
//         WM_INDIVIDUALIZE_STATUS struct
//     the WMSDK (V9) ASF Writer filter will send this event in response to a
//     wmsdk-signaled error during file
//         writing, along with the wmsdk WMT_STATUS error as the lParam1 and
//         hrStatus embedded in the AM_WMT_EVENT_DATA struct pointed to by the
//         lParam2 pointer.
//
