...
//
#define FCC(ch4)                                                               \
  ((((DWORD)(ch4) & 0xFF) << 24) | (((DWORD)(ch4) & 0xFF00) << 8) |            \
...
#define RIFFNEXT(pChunk)                                                       \
  (LPRIFFCHUNK)((LPBYTE)(pChunk) + sizeof(RIFFCHUNK) +                         \
...
#define NUMINDEXFILL(wLongsPerEntry)                                           \
...
#define Valid_SUPERINDEX(pi)                                                   \
  (*(DWORD *)(&((pi)->wLongsPerEntry)) == (4 | (AVI_INDEX_OF_INDEXES << 24)))
...
#define AVISTDINDEX_DELTAFRAME                                                 \
  (0x80000000) // Delta frames have the high bit set
