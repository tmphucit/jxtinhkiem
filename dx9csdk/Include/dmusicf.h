...
/* The following structures are used by the Tracks, and are the packed
 * structures */
...
typedef DMUS_CHORD_KEY
    DMUS_CHORD_PARAM; /* DMUS_CHORD_KEY defined in dmusici.h */
...
/* These flags determine the types of chords supported by a given variation in
 * DirectMusic */
/* mode.  The first seven flags (bits 1-7) are set if the variation supports
 * major chords  */
/* rooted in scale positions, so, e.g., if bits 1, 2, and 4 are set, the
 * variation         */
/* supports major chords rooted in the tonic, second, and fourth scale
 * positions.  The     */
/* next seven flags serve the same purpose, but for minor chords, and the
 * following seven  */
/* flags serve the same purpose for chords that are not major or minor (e.g.,
 * SUS 4        */
/* chords).  Bits 22, 23, and 24 are set if the variation supports chords rooted
 * in the    */
/* scale, chords rooted sharp of scale tones, and chords rooted flat of scale
 * tones,       */
/* respectively.  For example, to support a C# minor chord in the scale of C
 * Major,        */
/* bits 8 (for tonic minor) and 24 (for sharp) need to be set.  Bits 25, 26, an
 * 27 handle  */
/* chords that are triads, 6th or 7th chords, and chords with extensions,
 * respectively.    */
/* bits 28 and 29 handle chords that are followed by tonic and dominant chords,
 */
/* respectively. */
#define DMUS_VARIATIONF_MAJOR                                                  \
  0x0000007F /* Seven positions in the scale - major chords. */
#define DMUS_VARIATIONF_MINOR                                                  \
  0x00003F80 /* Seven positions in the scale - minor chords. */
#define DMUS_VARIATIONF_OTHER                                                  \
  0x001FC000 /* Seven positions in the scale - other chords. */
#define DMUS_VARIATIONF_ROOT_SCALE                                             \
  0x00200000 /* Handles chord roots in the scale. */
#define DMUS_VARIATIONF_ROOT_FLAT                                              \
  0x00400000 /* Handles flat chord roots (based on scale notes). */
#define DMUS_VARIATIONF_ROOT_SHARP                                             \
  0x00800000 /* Handles sharp chord roots (based on scale notes). */
#define DMUS_VARIATIONF_TYPE_TRIAD                                             \
  0x01000000 /* Handles simple chords - triads. */
#define DMUS_VARIATIONF_TYPE_6AND7                                             \
  0x02000000 /* Handles simple chords - 6 and 7. */
#define DMUS_VARIATIONF_TYPE_COMPLEX 0x04000000 /* Handles complex chords. */
#define DMUS_VARIATIONF_DEST_TO1                                               \
  0x08000000 /* Handles transitions to 1 chord. */
#define DMUS_VARIATIONF_DEST_TO5                                               \
  0x10000000 /* Handles transitions to 5 chord. */
#define DMUS_VARIATIONF_DEST_OTHER                                             \
  0x40000000 /* Handles transitions to chords other than 1 . */

/* legacy mask for variation modes */
#define DMUS_VARIATIONF_MODES 0xE0000000
/* Bits 29 and 31 of the variation flags are the Mode bits.  If both are 0, it's
 * IMA. */
...
/* These specify if the marker event signals whether to stop a variation or
start a pattern/variation (or both), and whether new variations must align with
a chord */
#define DMUS_MARKERF_START 0x1
#define DMUS_MARKERF_STOP 0x2
#define DMUS_MARKERF_CHORD_ALIGN 0x4

/* if this flag is set, variation settings in a playing pattern-based track's
state data will persist in the track after it stops playing */
...
typedef enum enumDMUS_VARIATIONT_TYPES {
  DMUS_VARIATIONT_SEQUENTIAL =
      0,                      /* Play sequential starting with variation 1. */
  DMUS_VARIATIONT_RANDOM = 1, /* Play randomly. */
  DMUS_VARIATIONT_RANDOM_START =
      2, /* Play sequential starting with a random variation. */
  DMUS_VARIATIONT_NO_REPEAT =
      3, /* Play randomly, but don't play the same variation twice. */
  DMUS_VARIATIONT_RANDOM_ROW = 4 /* Play randomly as a row: don't repeat any
                                    variation until all have played. */
...
typedef struct _DMUS_IO_PARTREF {
  ...
  BYTE bPriority;        /* 256 priority levels. Parts with lower priority */
                         ...
                         /* notes */
  BYTE
      bRandomVariation; /* when set, matching variations play in random order */
...
typedef struct _DMUS_IO_STYLECURVE {
  ...
  MUSIC_TIME mtResetDuration; /* how long after the end of the curve to reset
                                 the curve */
  short nTimeOffset;          /* offset from mtGridStart */
...
typedef struct _DMUS_IO_MOTIFSETTINGS {
  ...
  MUSIC_TIME mtLoopEnd; /* End of loop. Must be greater than mtLoopStart. Or, 0,
                           indicating loop full motif.  */
...
(
    ...
    (
        ...
        [<rsln-ck>]     // Optional chunk containing an array of variation
resolutions in Part
        [<anpn-ck>]     // Optional chunk containing an array of resolution
anticipations in Part
    )
    ...
    (
        ...
        <rhtm-ck>       // Chunk containing an array of rhythms for chord
matching
        [<UNFO-list>]   // Name, author, copyright info., comments
        [<mtfs-ck>]     // Motif settings chunk
        [<DMBD-form>]   // Optional band to be associated with the pattern (for
motifs) <pref-list>...  // Array of part reference id's
    )
...
(
    'DMPT'          // Pattern
    <styh-ck>       // Style header chunk
    <pttn-list>     // The pattern, in single pattern format (includes
DMUS_FOURCC_PART_LIST chunks)
)
*/
...
typedef enum enumDMUS_PATTERNT_TYPES {
  ...
  DMUS_PATTERNT_SEQUENTIAL =
      2, /* Play sequential starting with first matching pattern. */
  DMUS_PATTERNT_RANDOM_START =
      3, /* Play sequential starting with a random pattern. */
  DMUS_PATTERNT_NO_REPEAT =
      4, /* Play randomly, but don't play the same pattern twice. */
  DMUS_PATTERNT_RANDOM_ROW = 5 /* Play randomly as a row: don't repeat any
                                  pattern until all have played. */
} DMUS_PATTERNT_TYPES;
...
typedef struct _DMUS_IO_COMMAND {
  ...
  BYTE bRepeatMode;  /* Used to control selection of patterns with same groove
                        level  */
} DMUS_IO_COMMAND;
...
// <DMTL-form>      Tools are embedded in a graph. Theoretically, they can be
saved as individual files too. RIFF
(
    'DMTL'
    <tolh-ck>
...
#define DMUS_PORTCONFIGF_DRUMSON10                                             \
  1 /* This port configured for drums on channel 10. */
...
#define DMUS_BUFFERF_SHARED                                                    \
  1 /* Share this with other audio paths, instead of creating unique copies.   \
     */
#define DMUS_BUFFERF_DEFINED                                                   \
  2 /* Use one of the standard predefined buffers (see GUID_Buffer... in       \
       dmusici.h.) */
...
LIST
(
    'pchl'          // List container for one or more pchannel to buffer
assignments. <pchh-ck>...    // One or more pchannel to buffer assignment
headers and data.
    ...
    (
        ...
        <GUID>...                           // Array of GUIDs defining the
buffers they all connect to.
    )
)

LIST
(
    'dbfl'          // List container for one buffer and buffer attributes
header. <ddah-ck>       // Buffer attributes header.
    [<DSBC-form>]   // Buffer configuration. Not required when header uses a
...
typedef struct _DMUS_IO_BAND_ITEM_HEADER2 {
  MUSIC_TIME lBandTimeLogical; /* Position in track list. Time in the music with
                                  which band change is associated. */
  MUSIC_TIME lBandTimePhysical; /* Precise time band change will take effect.
                                   Should be close to logical time. */
...
(
    ...
    (
        ...
        (
            'lbnd'
            <bdih-ck> or <bd2h-ck>  // bdih is a legacy format.  bd2h is
preferred for new content. <DMBD-form> // Band
        )

            // <bdih-ck> or <bd2h-ck>       // band item header
            (
                <DMUS_IO_BAND_ITEM_HEADER> or <DMUS_IO_BAND_ITEM_HEADER2> //
Band item header
            )
*/
...
#define DMUS_IO_INST_BANKSELECT                                                \
  (1 << 1) /* dwPatch contains a valid Bank Select MSB and LSB part */
...
typedef struct _DMUS_IO_INSTRUMENT {
  ...
  DWORD dwAssignPatch;   /* MSB, LSB and Program change to assign to instrument
                            when downloading */
  DWORD dwNoteRanges[4]; /* 128 bits; one for each MIDI note instrument needs to
                            able to play */
...
/* This RIFF id and io struct have been added to allow wave files (and the wave
   object) to differentiate between streaming and one-shot waves, and to give a
   prefetch for streaming waves  */
...
/* This flag is included in DMUS_IO_WAVE_TRACK_HEADER.dwFlags.  If set, the
   track will get its variations from a pattern track, via
   GetParam(GUID_Variations). */
#define DMUS_WAVETRACKF_SYNC_VAR 0x1
/* This is also included in DMUS_IO_WAVE_TRACK_HEADER.dwFlags.  If set,
   variation control information will persist from one playback instance to the
   next.*/
#define DMUS_WAVETRACKF_PERSIST_CONTROL 0x2

typedef struct _DMUS_IO_WAVE_TRACK_HEADER {
  long lVolume;  /* Gain, in 1/100th of dB, to be applied to all waves.  Note:
                    All gain values should be negative. */
  DWORD dwFlags; /* Flags, including whether this track syncs to a pattern track
                    for its variations. */
} DMUS_IO_WAVE_TRACK_HEADER;

typedef struct _DMUS_IO_WAVE_PART_HEADER {
  long lVolume; /* Gain, in 1/100th of dB, to be applied to all waves in wave
                   part.  Note:  All gain values should be negative. */
...
typedef struct _DMUS_IO_WAVE_ITEM_HEADER {
  long lVolume; /* Gain, in 1/100th of dB.  Note:  All gain values should be
                   negative. */
  long lPitch;  /* Pitch offset in 1/100th of a semitone. */
  DWORD dwVariations;    /* Variation flags for which of 32 variations this wave
                            belongs to. */
  REFERENCE_TIME rtTime; /* Start time, in REFERENCE_TIME, if clock time track,
                            or MUSIC_TIME for music time track. */
  REFERENCE_TIME rtStartOffset; /* Distance into wave to start playback, in
...
LIST
{
    ...
    LIST
    {
        ...
        {
            'wavi'
            <wave-list>...  //  Array of waves; each wave is encapsulated in a
list
        }
...
/*  File io for DirectMusic Container file. This embeds a set of related files.
   And, in turn, it can be embedded within a segment or script file.
*/
...
typedef struct _DMUS_IO_CONTAINED_OBJECT_HEADER {
  ...
  FOURCC fccType;   /* list type if NULL ckid valid */
  /* Note that LIST:DMRF may be used for ckid and fccType in order to reference
     an object instead of embedding it within the container. */
} DMUS_IO_CONTAINED_OBJECT_HEADER;

#define DMUS_CONTAINED_OBJF_KEEP                                               \
  1 /* Keep the object cached in the loader after the container is released.   \
     */
...
(
    ...
    (
        'cobl'
        [<coba-ck>]         // Alias.  An alternative name by which this object
is known
                            // within the container.
        <cobh-ck>           // Required header, includes CLASS ID for object.
        [<data>] or <DMRF>  // Object data of the type specified in <cobh-ck>.
                            // If DMRF, it is a reference of where to find the
object.
                            // Otherwise, it could be any RIFF readable chunk in
the
                            //    exact same format as a file.  The object will
load
...
typedef struct _DMUS_IO_SEGMENT_HEADER {
  ...
  MUSIC_TIME mtPlayStart; /* Start of playback. By default, 0. */
  MUSIC_TIME mtLoopStart; /* Start of looping portion. By default, 0. */
  MUSIC_TIME mtLoopEnd; /* End of loop. Must be greater than dwPlayStart. Or, 0,
                           indicating loop full segment. */
  DWORD dwResolution;   /* Default resolution. */
...
#define DMUS_SEGIOF_REFLENGTH                                                  \
  1 /* Use the time in rtLength for the segment length. */
...
(
    ...
    [<vers-ck>]     // Optional version info
    [<UNFO-list>]   // Name, author, copyright info., comments
    [<DMCN-form>]   // Optional container of objects embedded in file. Must
precede tracklist. <trkl-list>     // List of Tracks
    [<DMTG-form>]   // Optional ToolGraph
    ...
    // <DMTK-form>      // Tracks can be embedded in a segment or stored as
separate files. RIFF
    (
        'DMTK'
        <trkh-ck>
        [<trkx-ck>]     // Optional track flags.
        [<guid-ck>]     // Optional GUID for track object instance (not to be
confused with Class id in track header)
        [<vers-ck>]     // Optional version info
...
#define DMUS_SIGNPOSTF_LETTER                                                  \
  (DMUS_SIGNPOSTF_A | DMUS_SIGNPOSTF_B | DMUS_SIGNPOSTF_C | DMUS_SIGNPOSTF_D | \
   DMUS_SIGNPOSTF_E | DMUS_SIGNPOSTF_F)
...
#define DMUS_SIGNPOSTF_ROOT                                                    \
  (DMUS_SIGNPOSTF_1 | DMUS_SIGNPOSTF_2 | DMUS_SIGNPOSTF_3 | DMUS_SIGNPOSTF_4 | \
   DMUS_SIGNPOSTF_5 | DMUS_SIGNPOSTF_6 | DMUS_SIGNPOSTF_7)
...
/* If set, when the script loads it will also load all the content in its
 * container. */
#define DMUS_SCRIPTIOF_DOWNLOAD_ALL_SEGMENTS (1 << 1)
/* If set and LOAD_ALL_CONTENT is also set, when the script initializes it will
   also download all the segments in its container. If set and LOAD_ALL_CONTENT
   is not set, when the script calls segment.Load on a segment then the segment
   will also be downloaded. If not set, the script must manually download and
   unload by calling segment.DownloadSoundData and segment.UnloadSoundData. */
...
(
    ...
    [<vers-ck>]         // Optional version info
    [<UNFO-list>]       // Name, author, copyright info., comments
    <scve-ck>           // Version of DirectMusic this script was authored to
run against <DMCN-form>         // Container of content referenced by the
script. <scla-ck>           // ActiveX scripting language in which the script is
written <scsr-ck> or <DMRF> // The script's source code.
                        // If scsr-ck, the source is embedding in the chunk.
                        // If DMRF, it is a reference of where to find a text
file with the source.
                        //    Class id (guidClassID in DMUS_IO_REFERENCE) must
be GUID_NULL because
                        //    this text file is not a DirectMusic object in its
own right.
)
...
/*
    ...
    (
        {
            <DMUS_IO_SYSEX_ITEM>
            <BYTE>...    // Array of bytes, length defined in the
   DMUS_IO_SYSEXITEM structure
        }...
    )
*/
...
/*  DirectX8 Time signature track. The track has been updated from DirectX7 to
   support a list of RIFF chunks. This will allow the time signature track to
...
#define DMUS_SEGMENTTRACKF_MOTIF                                               \
  1 /* interpret DMRF as link to style, and use snam as the name of a motif    \
       within the style */
...
(
    ...
    (
        ...
        <lseg-list>...  // Each segment is encapsulated in a list (that way it
can still be riff parsed.)
    )
        ...
        (
            ...
            [<snam-ck>]    // Name field.  Used with DMUS_SEGMENTTRACKF_MOTIF
flag.
        )
...
#define DMUS_IO_SCRIPTTRACKF_PREPARE                                           \
  (1 << 0) /* Fire event in advance of time stamp, at Prepare time.  This is   \
              the default because it leaves the script time to change the      \
              music happening at the target time. */
#define DMUS_IO_SCRIPTTRACKF_QUEUE                                             \
  (1 << 1) /* Fire event just before time stamp, at Queue time. */
#define DMUS_IO_SCRIPTTRACKF_ATTIME                                            \
  (1 << 2) /* Fire event right at the time stamp. */
...
#define DMUS_FOURCC_PARAMCONTROLTRACK_OBJECT_CHUNK                             \
  mmioFOURCC('p', 'r', 'o', 'h')
...
#define DMUS_FOURCC_PARAMCONTROLTRACK_CURVES_CHUNK                             \
  mmioFOURCC('p', 'r', 'c', 'c')

typedef struct _DMUS_IO_PARAMCONTROLTRACK_OBJECTHEADER {
  DWORD dwFlags;       /* Reserved.  Must be zero. */
  GUID guidTimeFormat; /* Time format to set the object to.  Must be
                          GUID_TIME_REFERNCE or GUID_TIME_MUSIC from medparam.h.
                        */
  /* Path for finding the object. These fields correspond to the first five
   * parameters of IDirectMusicSegmentState::GetObjectInPath. */
  DWORD dwPChannel;
  DWORD dwStage;
  DWORD dwBuffer;
  GUID guidObject;
...
typedef struct _DMUS_IO_PARAMCONTROLTRACK_CURVEINFO {
  ...
  float fltStartValue;
  float fltEndValue;
  DWORD dwCurveType; /* One of the items from the MP_CURVE_TYPE enum in
                        medparam.h */
        ...
        (
            ...
            (
                ...
                (
                    ...
                    <DMUS_IO_PARAMCONTROLTRACK_CURVEINFO>... // curves, sorted
   in order of mtTime
                )
*/
...
typedef struct _DSOUND_IO_DSBUFFERDESC {
  ...
  WORD nChannels;   /* No. of channels (rest of buffer format is determined by
                       owning sink) */
...
typedef struct _DSOUND_IO_3D {
  GUID guid3DAlgorithm; /* GUID identifying the 3D algorithm to use (defined in
                           dsound.h) */
  DS3DBUFFER ds3d;      /* Initial 3D parameters */
} DSOUND_IO_3D;

typedef struct _DSOUND_IO_DXDMO_HEADER {
  DWORD dwEffectFlags; /* Effect creation flags - equivalent to
                          DSEFFECTDESC::dwFlags */
  GUID guidDSFXClass;  /* GUID identifying the effect to use - corresponds to a
                          COM CLSID */
  GUID guidReserved;   /* Reserved - must be the null GUID */
  GUID guidSendBuffer; /* GUID identifying the buffer to send to if this is a
                          send effect */
  DWORD dwReserved;    /* Reserved - must be 0 */
...
(
    ...
    'dsbd'
    (
        <DSOUND_IO_DSBUFFERDESC>  // Creation parameters and initial settings
for the buffer
    )
    ...
    'bsid'
    (
        <DSOUND_IO_DSBUSID>  // The size of DSOUND_IO_DSBUSID is determined by
the chunk size
    )
...
// <DSFX-form>          // DMOs can be embedded in a buffer configuration or
stored as separate files RIFF
(
    'DSFX'
    ...
    'data'
    (
        <DSOUND_IO_DXDMO_DATA>  // Opaque data block used by the DMO to load
itself.
        // For our standard included DMOs, this is simply the structure accepted
by
        // the DMO's SetAllParameters() method - e.g. struct DSFXChorus for
Chorus.
    )
*/
