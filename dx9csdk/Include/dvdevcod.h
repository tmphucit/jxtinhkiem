...
typedef enum _tagDVD_ERROR {
  ...
  DVD_ERROR_InvalidDVD1_0Disc =
      3, // DVD-Video disc is incorrectly authored for v1.0
         //   of spec. Playback is stopped.
  DVD_ERROR_InvalidDiscRegion = 4, // The Disc cannot be played because the disc
                                   // is not authored to play in system region.
                                   // The region mismatch may be fixable by
                                   // changing the system region with dvdrgn.exe
  DVD_ERROR_LowParentalLevel =
      5, // Player parental level is lower than the lowest parental
...
typedef enum _tagDVD_WARNING {
  DVD_WARNING_InvalidDVD1_0Disc =
      1, // DVD-Video disc is incorrectly authored. Playback
         //   can continue, but unexpected behavior may occur.
  DVD_WARNING_FormatNotSupported =
      2, // A decoder would not support the current format.  Playback
  ...
  DVD_WARNING_IllegalNavCommand =
      3, // The internal DVD navigation command processor attempted to
...
typedef enum _tagDVD_PB_STOPPED {
  ...
  DVD_PB_STOPPED_NoBranch = 1, // The nav completed the current pgc and there
                               // was no more video and did not find any other
                               // branching instruction for subsequent playback.
  DVD_PB_STOPPED_NoFirstPlayDomain =
      2, // The disc does not contain an initial startup program.
  DVD_PB_STOPPED_StopCommand = 3, // The app issued a stop() command or a stop
                                  // command was authored on the disc.
  DVD_PB_STOPPED_Reset = 4, // The navigator was reset to the start of the disc
                            // (using ResetOnStop).
  DVD_PB_STOPPED_DiscEjected = 5, // The disc was ejected.
  DVD_PB_STOPPED_IllegalNavCommand =
      6, // An illegal nav command prevented playback from continuing.
...
// Raised from following domains: all
//
// Signaled when ever the current user subpicture stream number changes for the
// main title.  This can be changed automatically with a navigation command on
// disc as well as through IDVDAnnexJ. Subpicture stream numbers range from 0
// to 31.  Stream 0xffffffff indicates that no stream is selected.
...
// multiangle. Angle numbers range from 1 to 9.
...
// Parameters: ( DWORD, DWORD)
// lParam1 is an enum DVD_WARNING which notifies the app of some warning
// condition. lParam2 contains more specific information about the warning
// (warning dependent)
//
// Raised from following domains: all
//
...
// lParam1 is a BOOL which indicates the reason for the cancellation of
// ChapterPlayAutoStop lParam1 == 0 indicates successful completion of
// ChapterPlayAutoStop lParam1 == 1 indicates that ChapterPlayAutoStop is being
// cancelled as a result of another
//            IDVDControl call or the end of content has been reached & no more
...
//
//  Indicates that the DVD disc does not have a FP_PGC (First Play Program
//  Chain) and the DVD Navigator will not automatically load any PGC and start
//  playback.
//
...
//
//  Indicates that a rate change in playback has been initiated and the
//  parameter lParam1 indicates the new playback rate that is being used.
//
...
//             not available
//  lParam1 == 1 indicates that an angle block is being played back and angle
//  changes
//             can be performed.
//
...
// Parameters: none
// Sent when the nav detects that a disc was inserted and the nav begins
// playback The app does not need to take any action to start the playback.
//

#define EC_DVD_CURRENT_HMSF_TIME (EC_DVDBASE + 0x1a)
// Parameters: ( ULONG, ULONG )
// lParam2 contains a union of the DVD_TIMECODE_FLAGS
// lParam1 contains a DVD_HMSF_TIMECODE.  Assign lParam1 to a ULONG then cast
// the ULONG as a DVD_HMSF_TIMECODE to use its values.
//
// Raised from following domains: DVD_DOMAIN_Title
//
// Signaled at the beginning of every VOBU, which occurs every .4 to 1.0 sec.
...
// lParam1 is either TRUE (a karaoke track is being played) or FALSE (no karaoke
// data is being played).
//
#endif // __DVDEVCOD__
