...
typedef struct _D3DCAPS9 {
  ...
  float PixelShader1xMaxValue; // max value storable in registers of ps.1.x
                               // shaders
  ...
  UINT NumberOfAdaptersInGroup; // number of adapters in this adapter group
                                // (only if master)
  ...
  DWORD VertexTextureFilterCaps; // D3DPTFILTERCAPS for IDirect3DTexture9's for
                                 // texture, used in vertex shaders
  DWORD MaxVShaderInstructionsExecuted; // maximum number of vertex shader
                                        // instructions that can be executed
  DWORD MaxPShaderInstructionsExecuted; // maximum number of pixel shader
                                        // instructions that can be executed
...
#define D3DCAPS3_COPY_TO_VIDMEM                                                \
  0x00000100L /* Device can acclerate copies from sysmem to local vidmem */
#define D3DCAPS3_COPY_TO_SYSTEMMEM                                             \
  0x00000200L /* Device can acclerate copies from local vidmem to sysmem */
...
#define D3DDEVCAPS_TLVERTEXVIDEOMEMORY                                         \
  0x00000080L /* Device can use TL buffers from video memory */
...
#define D3DDEVCAPS_TEXTUREVIDEOMEMORY                                          \
  0x00000200L /* Device can texture from device memory */
#define D3DDEVCAPS_DRAWPRIMTLVERTEX                                            \
  0x00000400L /* Device can draw TLVERTEX primitives */
...
#define D3DDEVCAPS_DRAWPRIMITIVES2                                             \
  0x00002000L /* Device can support DrawPrimitives2 */
...
#define D3DDEVCAPS_DRAWPRIMITIVES2EX                                           \
  0x00008000L /* Device can support Extended DrawPrimitives2 i.e. DX7          \
                 compliant driver*/
#define D3DDEVCAPS_HWTRANSFORMANDLIGHT                                         \
  0x00010000L /* Device can support transformation and lighting in hardware    \
                 and DRAWPRIMITIVES2EX must be also */
#define D3DDEVCAPS_CANBLTSYSTONONLOCAL                                         \
  0x00020000L /* Device supports a Tex Blt from system memory to non-local     \
                 vidmem */
#define D3DDEVCAPS_HWRASTERIZATION                                             \
  0x00080000L /* Device has HW acceleration for rasterization */
#define D3DDEVCAPS_PUREDEVICE                                                  \
  0x00100000L /* Device supports D3DCREATE_PUREDEVICE */
#define D3DDEVCAPS_QUINTICRTPATCHES                                            \
  0x00200000L /* Device supports quintic Beziers and BSplines */
#define D3DDEVCAPS_RTPATCHES                                                   \
  0x00400000L /* Device supports Rect and Tri patches */
#define D3DDEVCAPS_RTPATCHHANDLEZERO                                           \
  0x00800000L /* Indicates that RT Patches may be drawn efficiently using      \
                 handle 0 */
...
#define D3DPMISCCAPS_TSSARGTEMP                                                \
  0x00000400L /* device supports D3DTA_TEMP for temporary register */
#define D3DPMISCCAPS_BLENDOP 0x00000800L /* device supports D3DRS_BLENDOP */
#define D3DPMISCCAPS_NULLREFERENCE                                             \
  0x00001000L /* Reference Device that doesnt render */
#define D3DPMISCCAPS_INDEPENDENTWRITEMASKS                                     \
  0x00004000L /* Device supports independent write masks for MET or MRT */
#define D3DPMISCCAPS_PERSTAGECONSTANT                                          \
  0x00008000L /* Device supports per-stage constants */
...
#define D3DPMISCCAPS_SEPARATEALPHABLEND                                        \
  0x00020000L /* Device supports separate blend settings for the alpha channel \
               */
#define D3DPMISCCAPS_MRTINDEPENDENTBITDEPTHS                                   \
  0x00040000L /* Device supports different bit depths for MRT */
#define D3DPMISCCAPS_MRTPOSTPIXELSHADERBLENDING                                \
  0x00080000L /* Device supports post-pixel shader operations for MRT */
#define D3DPMISCCAPS_FOGVERTEXCLAMPED                                          \
  0x00100000L /* Device clamps fog blend factor per vertex */
...
#define D3DPRASTERCAPS_COLORPERSPECTIVE                                        \
  0x00400000L /* Device iterates colors perspective correct */
...
#define D3DPBLENDCAPS_BLENDFACTOR                                              \
  0x00002000L /* Supports both D3DBLEND_BLENDFACTOR and                        \
...
#define D3DPTEXTURECAPS_PERSPECTIVE                                            \
  0x00000001L /* Perspective-correct texturing is supported */
#define D3DPTEXTURECAPS_POW2                                                   \
  0x00000002L /* Power-of-2 texture dimensions are required - applies to       \
                 non-Cube/Volume textures only. */
#define D3DPTEXTURECAPS_ALPHA                                                  \
  0x00000004L /* Alpha in texture pixels is supported */
#define D3DPTEXTURECAPS_SQUAREONLY                                             \
  0x00000020L /* Only square textures are supported */
#define D3DPTEXTURECAPS_TEXREPEATNOTSCALEDBYSIZE                               \
  0x00000040L /* Texture indices are not scaled by the texture size prior to   \
                 interpolation */
#define D3DPTEXTURECAPS_ALPHAPALETTE                                           \
  0x00000080L /* Device can draw alpha from texture palettes */
...
#define D3DPTEXTURECAPS_MIPMAP                                                 \
  0x00004000L /* Device can do mipmapped textures */
#define D3DPTEXTURECAPS_MIPVOLUMEMAP                                           \
  0x00008000L /* Device can do mipmapped volume textures */
#define D3DPTEXTURECAPS_MIPCUBEMAP                                             \
  0x00010000L /* Device can do mipmapped cube maps */
...
// FVFCaps
//
#define D3DFVFCAPS_TEXCOORDCOUNTMASK                                           \
  0x0000ffffL /* mask for texture coordinate count field */
...
#define D3DVTXPCAPS_DIRECTIONALLIGHTS                                          \
  0x00000008L /* device can do directional lights */
...
#define D3DVTXPCAPS_TEXGEN_SPHEREMAP                                           \
  0x00000100L /* device supports D3DTSS_TCI_SPHEREMAP */
...
#define D3DDEVCAPS2_STREAMOFFSET                                               \
  0x00000001L /* Device supports offsets in streams. Must be set by DX9        \
                 drivers */
#define D3DDEVCAPS2_DMAPNPATCH                                                 \
  0x00000002L /* Device supports displacement maps for N-Patches*/
#define D3DDEVCAPS2_ADAPTIVETESSRTPATCH                                        \
  0x00000004L /* Device supports adaptive tesselation of RT-patches*/
#define D3DDEVCAPS2_ADAPTIVETESSNPATCH                                         \
  0x00000008L /* Device supports adaptive tesselation of N-patches*/
#define D3DDEVCAPS2_CAN_STRETCHRECT_FROM_TEXTURES                              \
  0x00000010L /* Device supports StretchRect calls with a texture as the       \
                 source*/
#define D3DDEVCAPS2_PRESAMPLEDDMAPNPATCH                                       \
  0x00000020L /* Device supports presampled displacement maps for N-Patches */
#define D3DDEVCAPS2_VERTEXELEMENTSCANSHARESTREAMOFFSET                         \
  0x00000040L /* Vertex elements in a vertex declaration can share the same    \
                 stream offset */
