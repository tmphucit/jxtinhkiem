...
static const float aStereoLayout[] = {LEFT_AZIMUTH, RIGHT_AZIMUTH};
static const float a2Point1Layout[] = {LEFT_AZIMUTH, RIGHT_AZIMUTH,
                                       LOW_FREQUENCY_AZIMUTH};
static const float aQuadLayout[] = {FRONT_LEFT_AZIMUTH, FRONT_RIGHT_AZIMUTH,
                                    BACK_LEFT_AZIMUTH, BACK_RIGHT_AZIMUTH};
static const float a4Point1Layout[] = {FRONT_LEFT_AZIMUTH, FRONT_RIGHT_AZIMUTH,
                                       LOW_FREQUENCY_AZIMUTH, BACK_LEFT_AZIMUTH,
                                       BACK_RIGHT_AZIMUTH};
static const float a5Point1Layout[] = {
    FRONT_LEFT_AZIMUTH,    FRONT_RIGHT_AZIMUTH, FRONT_CENTER_AZIMUTH,
    LOW_FREQUENCY_AZIMUTH, BACK_LEFT_AZIMUTH,   BACK_RIGHT_AZIMUTH};
...
//
// PARAMETERS:
//  SpeakerChannelMask - [in]  speaker geometry configuration on the final mix,
//  specifies assignment of channels to speaker positions, defined as per
//  WAVEFORMATEXTENSIBLE.dwChannelMask, must be != 0
//                             Currently only SPEAKER_STEREO and SPEAKER_5POINT1
...
//  HResult error code
////
EXTERN_C HRESULT inline XACT3DInitialize(UINT32 SpeakerChannelMask,
                                         IXACTEngine *pEngine,
  ...
  if (SUCCEEDED(hr)) {
    XACTVARIABLEINDEX xactSpeedOfSoundID =
        pEngine->GetGlobalVariableIndex("SpeedOfSound");
...
//
// PARAMETERS:
//  X3DInstance        - [in]  X3DAudio instance (returned from
//  XACT3DInitialize) pListener          - [in]  point of 3D audio reception
//  pEmitter           - [in]  3D audio source
//  pDSPSettings       - [out] receives calculation results, applied to an XACT
//  cue via XACT3DApply
//
// RETURN VALUE:
//  HResult error code
////
EXTERN_C HRESULT inline XACT3DCalculate(X3DAUDIO_HANDLE X3DInstance,
  ...
  if (SUCCEEDED(hr)) {
    static X3DAUDIO_DISTANCE_CURVE_POINT DefaultCurvePoints[2] = {0.0f, 1.0f,
                                                                  1.0f, 1.0f};
    static X3DAUDIO_DISTANCE_CURVE DefaultCurve = {
        (X3DAUDIO_DISTANCE_CURVE_POINT *)&DefaultCurvePoints[0], 2};
...
//  HResult error code
////
EXTERN_C HRESULT inline XACT3DApply(X3DAUDIO_DSP_SETTINGS *pDSPSettings,
                                    IXACTCue *pCue) {
  HRESULT hr = S_OK;
  ...
  if (SUCCEEDED(hr)) {
    hr = pCue->SetMatrixCoefficients(pDSPSettings->SrcChannelCount,
  ...
  if (SUCCEEDED(hr)) {
    ...
    hr = pCue->SetVariable(xactDistanceID,
                           pDSPSettings->EmitterToListenerDistance);
  }
  if (SUCCEEDED(hr)) {
    XACTVARIABLEINDEX xactDopplerID =
        pCue->GetVariableIndex("DopplerPitchScalar");
    hr = pCue->SetVariable(xactDopplerID, pDSPSettings->DopplerFactor);
  }
  if (SUCCEEDED(hr)) {
    XACTVARIABLEINDEX xactOrientationID =
        pCue->GetVariableIndex("OrientationAngle");
    hr = pCue->SetVariable(xactOrientationID,
                           pDSPSettings->EmitterToListenerAngle *
                               (180.0f / X3DAUDIO_PI));
  }
