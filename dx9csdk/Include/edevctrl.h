...
typedef enum {
  KSPROPERTY_EXTDEVICE_ID, // ID (such as Symbolic Lin) that can uniquely idenfy
                           // this device
  KSPROPERTY_EXTDEVICE_VERSION, // Device model number and version (such AV/C
                                // VCR Subunit Spec. 2.01)
...
typedef enum {
  ...
  KSPROPERTY_EXTXPORT_MEDIUM_INFO,        // cassettte_type and
                                          // tape_grade_and_write_protect
  KSPROPERTY_EXTXPORT_STATE,              // Get/Set transport mode and state
  KSPROPERTY_EXTXPORT_STATE_NOTIFY,       // NOTIFY: Mode + State (Table 4-8)
  KSPROPERTY_EXTXPORT_TIMECODE_SEARCH,    // Request VCR subunit to search for a
                                          // specific timecode on the medium
  KSPROPERTY_EXTXPORT_ATN_SEARCH,         // Request VCR subunit to search for a
                                          // specific ATN on the medium
  KSPROPERTY_EXTXPORT_RTC_SEARCH,         // Request VCR subunit to search for a
                                  // specific RelativeTimeCounter on the medium
...
typedef struct {
  ...
  union {
    ...
    struct {
      ULONG PayloadSize;
      BYTE Payload[512]; // This is only for testing sending AVC command from
                         // User mode.
    } RawAVC;
...
typedef enum {
  ...
  KSPROPERTY_RTC_READER, // Relative time counter for the current medium
                         // position
...
typedef enum {
  ...
  KSEVENT_EXTDEV_OPERATION_MODE_UPDATE,  // Notify mode of operation change
                                         // (VCR,OFF,Camera)
  KSEVENT_EXTDEV_TRANSPORT_STATE_UPDATE, // XPrt state change
  KSEVENT_EXTDEV_NOTIFY_REMOVAL,         // Notify device removal
  KSEVENT_EXTDEV_NOTIFY_MEDIUM_CHANGE,   // Notify medium (tape) is removed or
                                         // added
