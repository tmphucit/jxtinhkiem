...
#define WAVEBANK_BANKNAME_LENGTH                                               \
  64 // Wave bank friendly name length, in characters
#define WAVEBANK_ENTRYNAME_LENGTH                                              \
  64 // Wave bank entry friendly name length, in characters

#define WAVEBANK_MAX_DATA_SEGMENT_SIZE                                         \
  0xFFFFFFFF // Maximum wave bank data segment size, in bytes
#define WAVEBANK_MAX_COMPACT_DATA_SEGMENT_SIZE                                 \
  0x001FFFFF // Maximum compact wave bank data segment size, in bytes
...
#define WAVEBANK_FLAGS_SYNC_DISABLED                                           \
  0x00040000 // Bank is disabled for audition sync
...
#define WAVEBANKENTRY_FLAGS_LOOPCACHE                                          \
  0x00000002 // One or more looping sounds use this wave
#define WAVEBANKENTRY_FLAGS_REMOVELOOPTAIL                                     \
  0x00000004 // Remove data after the end of the loop region
#define WAVEBANKENTRY_FLAGS_IGNORELOOP                                         \
  0x00000008 // Used internally when the loop region can't be used
...
#define WAVEBANKENTRY_XMACHANNELS_MAX                                          \
  6 // enough for 5.1 channel audio (cf. XAUDIOCHANNEL_SOURCEMAX)
...
#define WAVEBANK_ALIGNMENT_DVD                                                 \
  WAVEBANK_DVD_SECTOR_SIZE // DVD-optimized alignment
...
#define ADPCM_MINIWAVEFORMAT_BLOCKALIGN_CONVERSION_OFFSET 22
  DWORD BlockAlign() const {
    return wFormatTag != WAVEBANKMINIFORMAT_TAG_ADPCM
               ? wBlockAlign
               : (wBlockAlign +
                  ADPCM_MINIWAVEFORMAT_BLOCKALIGN_CONVERSION_OFFSET) *
                     nChannels;
  }
...
typedef struct WAVEBANKDATA {
  ...
  DWORD dwEntryMetaDataElementSize; // Size of each entry meta-data element, in
                                    // bytes
