...
DEFINE_GUID(IID_ID3DX10Sprite, 0xba0b762d, 0x8d28, 0x43ec, 0xb9, 0xdc, 0x2f,
            0x84, 0x44, 0x3b, 0x6, 0x14);
...
DECLARE_INTERFACE_(ID3DX10Sprite, IUnknown) {
  ...
  STDMETHOD(DrawSpritesBuffered)
  (THIS_ D3DX10_SPRITE * pSprites, UINT cSprites) PURE;
  STDMETHOD(Flush)(THIS) PURE;

  STDMETHOD(DrawSpritesImmediate)
  (THIS_ D3DX10_SPRITE * pSprites, UINT cSprites, UINT cbSprite, UINT flags)
      PURE;
  ...
  STDMETHOD(GetProjectionTransform)
  (THIS_ D3DXMATRIX * pProjectionTransform) PURE;
  STDMETHOD(SetProjectionTransform)
  (THIS_ D3DXMATRIX * pProjectionTransform) PURE;
...
HRESULT WINAPI D3DX10CreateSprite(ID3D10Device *pDevice, UINT cDeviceBufferSize,
...
DEFINE_GUID(IID_ID3DX10ThreadPump, 0xc93fecfa, 0x6967, 0x478a, 0xab, 0xbc, 0x40,
            0x2d, 0x90, 0x62, 0x1f, 0xcb);
...
DECLARE_INTERFACE_(ID3DX10ThreadPump, IUnknown) {
  ...
  // ID3DX10ThreadPump
  STDMETHOD(AddWorkItem)
  (THIS_ ID3DX10DataLoader * pDataLoader, ID3DX10DataProcessor * pDataProcessor,
  ...
  STDMETHOD(GetQueueStatus)
  (THIS_ UINT * pIoQueue, UINT * pProcessQueue, UINT * pDeviceQueue) PURE;
};

HRESULT WINAPI D3DX10CreateThreadPump(UINT cIoThreads, UINT cProcThreads,
...
DEFINE_GUID(IID_ID3DX10Font, 0xd79dbb70, 0x5f21, 0x4d36, 0xbb, 0xc2, 0xff, 0x52,
            0x5c, 0x21, 0x3c, 0xdc);
...
DECLARE_INTERFACE_(ID3DX10Font, IUnknown) {
  ...
  (THIS_ UINT Glyph, ID3D10ShaderResourceView * *ppTexture, RECT * pBlackBox,
   POINT * pCellInc) PURE;
  ...
  (THIS_ LPD3DX10SPRITE pSprite, LPCSTR pString, INT Count, LPRECT pRect,
   UINT Format, D3DXCOLOR Color) PURE;
  STDMETHOD_(INT, DrawTextW)
  (THIS_ LPD3DX10SPRITE pSprite, LPCWSTR pString, INT Count, LPRECT pRect,
   UINT Format, D3DXCOLOR Color) PURE;

#ifdef __cplusplus
#ifdef UNICODE
  STDMETHOD(GetDesc)(THIS_ D3DX10_FONT_DESCW * pDesc) {
    return GetDescW(pDesc);
  }
  STDMETHOD(PreloadText)(THIS_ LPCWSTR pString, INT Count) {
    return PreloadTextW(pString, Count);
  }
#else
  STDMETHOD(GetDesc)(THIS_ D3DX10_FONT_DESCA * pDesc) {
    return GetDescA(pDesc);
  }
  STDMETHOD(PreloadText)(THIS_ LPCSTR pString, INT Count) {
...
HRESULT WINAPI D3DX10CreateFontA(ID3D10Device *pDevice, INT Height, UINT Width,
                                 UINT Weight, UINT MipLevels, BOOL Italic,
                                 UINT CharSet, UINT OutputPrecision,
                                 UINT Quality, UINT PitchAndFamily,
                                 LPCSTR pFaceName, LPD3DX10FONT *ppFont);

HRESULT WINAPI D3DX10CreateFontW(ID3D10Device *pDevice, INT Height, UINT Width,
                                 UINT Weight, UINT MipLevels, BOOL Italic,
                                 UINT CharSet, UINT OutputPrecision,
                                 UINT Quality, UINT PitchAndFamily,
                                 LPCWSTR pFaceName, LPD3DX10FONT *ppFont);
