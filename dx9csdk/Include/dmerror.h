...
#define MAKE_DMHRESULTSUCCESS(code)                                            \
  MAKE_HRESULT(0, FACILITY_DIRECTMUSIC, (DMUS_ERRBASE + (code)))
#define MAKE_DMHRESULTERROR(code)                                              \
  MAKE_HRESULT(1, FACILITY_DIRECTMUSIC, (DMUS_ERRBASE + (code)))

/* DMUS_S_PARTIALLOAD
 *
 * The object could only load partially. This can happen if some components are
 * not registered properly, such as embedded tracks and tools. This can also
 * happen if some content is missing. For example, if a segment uses a DLS
 * collection that is not in the loader's current search directory.
 */
 ...
 * occurs when some instruments are on PChannels not supported by the
 * performance or port.
 */
 ...
 *
 * The segment object was unable to load all tracks from the IStream* object
 * data. This may be due to errors in the stream, or the tracks being
 * incorrectly registered on the client.
 */
 ...
 *
 * Segment initialization failed, most likely due to a critical memory
 * situation.
 */
...
/* misspelling in previous versions of DirectX preserved for backward
 * compatibility */
 ...
 * The pErrorInfo parameter (if supplied) was filled with information about the
 * error.
 */
 ...
 *
 * Loading of oleaut32.dll failed.  VBScript and other activeX scripting
 * languages require use of oleaut32.dll.  On platforms where oleaut32.dll is
 * not present, only the DirectMusicScript language, which doesn't require
 * oleaut32.dll can be used.
 */
#define DMUS_E_SCRIPT_CANTLOAD_OLEAUT32 MAKE_DMHRESULTERROR(0x0216)

/* DMUS_E_SCRIPT_LOADSCRIPT_ERROR
 *
 * An error occured while parsing a script loaded using LoadScript.  The script
 * that was loaded contains an error.
 */
 ...
 *
 * Scripts variables for content referenced or embedded in a script cannot be
 * set.
 */
 ...
 *
 * A script written in AudioVBScript could not be read because it contained a
 * statement that is not allowed by the AudioVBScript language.
 */
#define DMUS_E_AUDIOVBSCRIPT_SYNTAXERROR MAKE_DMHRESULTERROR(0x0223)

/* DMUS_E_AUDIOVBSCRIPT_RUNTIMEERROR
 *
 * A script routine written in AudioVBScript failed because an invalid operation
 * occurred.  For example, adding the number 3 to a segment object would produce
 * this error.  So would attempting to call a routine that doesn't exist.
 */
#define DMUS_E_AUDIOVBSCRIPT_RUNTIMEERROR MAKE_DMHRESULTERROR(0x0224)

/* DMUS_E_AUDIOVBSCRIPT_OPERATIONFAILURE
 *
 * A script routine written in AudioVBScript failed because a function outside
 * of a script failed to complete. For example, a call to PlaySegment that fails
 * to play because of low memory would return this error.
 */
 ...
 *
 * An audiopath failed to create because a requested buffer could not be
 * created.
 */
 ...
 *
 * An audiopath could not be used for playback because it lacked port
 * assignments.
 */
...
/* DMUS_E_NO_AUDIOPATH
 *
 * Attempt was made to play segment in audiopath mode and there was no
 * audiopath.
 */
 ...
 *
 * Attempt was made to create an audiopath that sends to a global effects buffer
 * which did not exist.
 */
