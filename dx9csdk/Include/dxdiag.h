...
// This identifier is passed to IDxDiagProvider::Initialize in order to ensure
// that an application was built against the correct header files. This number
// is incremented whenever a header (or other) change would require applications
// to be rebuilt. If the version doesn't match, IDxDiagProvider::Initialize will
// fail. (The number itself has no meaning.)
...
#define DXDIAG_E_INSUFFICIENT_BUFFER                                           \
  ((HRESULT)0x8007007AL) // HRESULT_FROM_WIN32(ERROR_INSUFFICIENT_BUFFER)
...
DEFINE_GUID(CLSID_DxDiagProvider, 0xA65B8071, 0x3BFE, 0x4213, 0x9A, 0x5B, 0x49,
            0x1D, 0xA4, 0x46, 0x1C, 0xA7);
...
DEFINE_GUID(IID_IDxDiagProvider, 0x9C6B4CB0, 0x23F8, 0x49CC, 0xA3, 0xED, 0x45,
            0xA5, 0x50, 0x00, 0xA6, 0xD2);

// {0x7D0F462F-0x4064-0x4862-BC7F-933E5058C10F}
DEFINE_GUID(IID_IDxDiagContainer, 0x7D0F462F, 0x4064, 0x4862, 0xBC, 0x7F, 0x93,
            0x3E, 0x50, 0x58, 0xC1, 0x0F);
...
DECLARE_INTERFACE_(IDxDiagContainer, IUnknown) {
  ...
  STDMETHOD(EnumChildContainerNames)
  (THIS_ DWORD dwIndex, LPWSTR pwszContainer, DWORD cchContainer) PURE;
  STDMETHOD(GetChildContainer)
  (THIS_ LPCWSTR pwszContainer, IDxDiagContainer * *ppInstance) PURE;
  STDMETHOD(GetNumberOfProps)(THIS_ DWORD * pdwCount) PURE;
  STDMETHOD(EnumPropNames)
  (THIS_ DWORD dwIndex, LPWSTR pwszPropName, DWORD cchPropName) PURE;
