...
typedef HRESULT(CALLBACK *LPD3DVALIDATECALLBACK)(LPVOID lpUserArg,
                                                 DWORD dwOffset);
typedef HRESULT(CALLBACK *LPD3DENUMTEXTUREFORMATSCALLBACK)(
    LPDDSURFACEDESC lpDdsd, LPVOID lpContext);
typedef HRESULT(CALLBACK *LPD3DENUMPIXELFORMATSCALLBACK)(
...
#if (defined __cplusplus) && (defined D3D_OVERLOADS)
  _D3DLVERTEX() {}
  _D3DLVERTEX(const D3DVECTOR &v, D3DCOLOR _color, D3DCOLOR _specular,
              float _tu, float _tv) {
    x = v.x;
    y = v.y;
    z = v.z;
...
#define D3DSTATUS_CLIPUNIONALL                                                 \
  (D3DSTATUS_CLIPUNIONLEFT | D3DSTATUS_CLIPUNIONRIGHT |                        \
 ...
 */
#define D3DSTATE_OVERRIDE(type)                                                \
  (D3DRENDERSTATETYPE)(((DWORD)(type) + D3DSTATE_OVERRIDE_BIAS))
...
typedef enum _D3DRENDERSTATETYPE {
  ...
  D3DRENDERSTATE_STIPPLEDALPHA =
      33, /* TRUE to enable stippled alpha (RGB device only) */
  ...
  D3DRENDERSTATE_STENCILFAIL =
      53, /* D3DSTENCILOP to do if stencil test fails */
  ...
  D3DRENDERSTATE_STENCILFUNC =
      56, /* D3DCMPFUNC fn.  Stencil Test passes if ((ref & mask) stencilfn
  ...
  D3DRENDERSTATE_TEXTUREHANDLE =
      1, /* Texture handle for legacy interfaces (Texture,Texture2) */
  ...
  D3DRENDERSTATE_TRANSLUCENTSORTINDEPENDENT =
      51, /* BOOL enable sort-independent transparency */
#endif    /* DIRECT3D_VERSION >= 0x0600 */
  ...
  //
  // retired renderstate names - the values are still used under new naming
  // conventions
  //
...
typedef enum _D3DTEXTURESTAGESTATETYPE {
  ...
  D3DTSS_MAGFILTER =
      16, /* D3DTEXTUREMAGFILTER filter to use for magnification */
  D3DTSS_MINFILTER =
      17, /* D3DTEXTUREMINFILTER filter to use for minification */
  D3DTSS_MIPFILTER = 18, /* D3DTEXTUREMIPFILTER filter to use between mipmaps
                            during minification */
...
#define D3DFVF_LVERTEX                                                         \
  (D3DFVF_XYZ | D3DFVF_RESERVED1 | D3DFVF_DIFFUSE | D3DFVF_SPECULAR |          \
   D3DFVF_TEX1)
