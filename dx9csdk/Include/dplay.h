...
typedef LPRGLPVOID PRGPVOID, LPRGPVOID, PRGLPVOID, PAPVOID, LPAPVOID, PALPVOID,
    LPALPVOID;
 ...
 */
DEFINE_GUID(IID_IDirectPlay2, 0x2b74f7c0, 0x9154, 0x11cf, 0xa9, 0xcd, 0x0, 0xaa,
            0x0, 0x68, 0x86, 0xe3);
DEFINE_GUID(IID_IDirectPlay2A, 0x9d460580, 0xa822, 0x11cf, 0x96, 0xc, 0x0, 0x80,
            0xc7, 0x53, 0x4e, 0x82);

DEFINE_GUID(IID_IDirectPlay3, 0x133efe40, 0x32dc, 0x11d0, 0x9c, 0xfb, 0x0, 0xa0,
            0xc9, 0xa, 0x43, 0xcb);
DEFINE_GUID(IID_IDirectPlay3A, 0x133efe41, 0x32dc, 0x11d0, 0x9c, 0xfb, 0x0,
            0xa0, 0xc9, 0xa, 0x43, 0xcb);

DEFINE_GUID(IID_IDirectPlay4, 0xab1c530, 0x4745, 0x11d1, 0xa7, 0xa1, 0x0, 0x0,
            0xf8, 0x3, 0xab, 0xfc);
DEFINE_GUID(IID_IDirectPlay4A, 0xab1c531, 0x4745, 0x11d1, 0xa7, 0xa1, 0x0, 0x0,
            0xf8, 0x3, 0xab, 0xfc);

// {D1EB6D20-8923-11d0-9D97-00A0C90A43CB}
DEFINE_GUID(CLSID_DirectPlay, 0xd1eb6d20, 0x8923, 0x11d0, 0x9d, 0x97, 0x0, 0xa0,
            0xc9, 0xa, 0x43, 0xcb);
...
DEFINE_GUID(DPSPGUID_IPX, 0x685bc400, 0x9d2c, 0x11cf, 0xa9, 0xcd, 0x0, 0xaa,
            0x0, 0x68, 0x86, 0xe3);

// GUID for TCP/IP service provider
// 36E95EE0-8577-11cf-960C-0080C7534E82
DEFINE_GUID(DPSPGUID_TCPIP, 0x36E95EE0, 0x8577, 0x11cf, 0x96, 0xc, 0x0, 0x80,
            0xc7, 0x53, 0x4e, 0x82);

// GUID for Serial service provider
// {0F1D6860-88D9-11cf-9C4E-00A0C905425E}
DEFINE_GUID(DPSPGUID_SERIAL, 0xf1d6860, 0x88d9, 0x11cf, 0x9c, 0x4e, 0x0, 0xa0,
            0xc9, 0x5, 0x42, 0x5e);

// GUID for Modem service provider
// {44EAA760-CB68-11cf-9C4E-00A0C905425E}
DEFINE_GUID(DPSPGUID_MODEM, 0x44eaa760, 0xcb68, 0x11cf, 0x9c, 0x4e, 0x0, 0xa0,
            0xc9, 0x5, 0x42, 0x5e);
...
typedef struct {
  ...
  DWORD dwMaxBufferSize;   // Maximum message size, in bytes,  for this service
                           // provider
 ...
 * flag allows applications to get the right behaviour without breaking apps
 * depending on old broken behavior.
 */
 ...
 */
typedef BOOL(FAR PASCAL *LPDPENUMPLAYERSCALLBACK2)(DPID dpId,
                                                   DWORD dwPlayerType,
                                                   LPCDPNAME lpName,
                                                   DWORD dwFlags,
                                                   LPVOID lpContext);
...
extern HRESULT WINAPI DirectPlayCreate(LPGUID lpGUID, LPDIRECTPLAY *lplpDP,
                                       IUnknown *pUnk);
...
DECLARE_INTERFACE_(IDirectPlay2, IUnknown) {
  ...
  (THIS_ LPDPSESSIONDESC2, DWORD, LPDPENUMSESSIONSCALLBACK2, LPVOID, DWORD)
      PURE;
...
#define IDirectPlay2_EnumGroups(p, a, b, c, d)                                 \
  (p)->lpVtbl->EnumGroups(p, a, b, c, d)
#define IDirectPlay2_EnumPlayers(p, a, b, c, d)                                \
  (p)->lpVtbl->EnumPlayers(p, a, b, c, d)
#define IDirectPlay2_EnumSessions(p, a, b, c, d, e)                            \
  (p)->lpVtbl->EnumSessions(p, a, b, c, d, e)
#define IDirectPlay2_GetCaps(p, a, b) (p)->lpVtbl->GetCaps(p, a, b)
#define IDirectPlay2_GetMessageCount(p, a, b)                                  \
  (p)->lpVtbl->GetMessageCount(p, a, b)
#define IDirectPlay2_GetGroupData(p, a, b, c, d)                               \
  (p)->lpVtbl->GetGroupData(p, a, b, c, d)
#define IDirectPlay2_GetGroupName(p, a, b, c)                                  \
  (p)->lpVtbl->GetGroupName(p, a, b, c)
#define IDirectPlay2_GetPlayerAddress(p, a, b, c)                              \
  (p)->lpVtbl->GetPlayerAddress(p, a, b, c)
#define IDirectPlay2_GetPlayerCaps(p, a, b, c)                                 \
  (p)->lpVtbl->GetPlayerCaps(p, a, b, c)
#define IDirectPlay2_GetPlayerData(p, a, b, c, d)                              \
  (p)->lpVtbl->GetPlayerData(p, a, b, c, d)
#define IDirectPlay2_GetPlayerName(p, a, b, c)                                 \
  (p)->lpVtbl->GetPlayerName(p, a, b, c)
#define IDirectPlay2_GetSessionDesc(p, a, b)                                   \
...
#define IDirectPlay2_Receive(p, a, b, c, d, e)                                 \
  (p)->lpVtbl->Receive(p, a, b, c, d, e)
#define IDirectPlay2_Send(p, a, b, c, d, e) (p)->lpVtbl->Send(p, a, b, c, d, e)
#define IDirectPlay2_SetGroupData(p, a, b, c, d)                               \
  (p)->lpVtbl->SetGroupData(p, a, b, c, d)
#define IDirectPlay2_SetGroupName(p, a, b, c)                                  \
  (p)->lpVtbl->SetGroupName(p, a, b, c)
#define IDirectPlay2_SetPlayerData(p, a, b, c, d)                              \
  (p)->lpVtbl->SetPlayerData(p, a, b, c, d)
#define IDirectPlay2_SetPlayerName(p, a, b, c)                                 \
  (p)->lpVtbl->SetPlayerName(p, a, b, c)
#define IDirectPlay2_SetSessionDesc(p, a, b)                                   \
...
#define IDirectPlay3_SetSessionDesc(p, a, b)                                   \
  (p)->lpVtbl->SetSessionDesc(p, a, b)
#define IDirectPlay3_AddGroupToGroup(p, a, b)                                  \
...
#define IDirectPlay3_DeleteGroupFromGroup(p, a, b)                             \
  (p)->lpVtbl->DeleteGroupFromGroup(p, a, b)
#define IDirectPlay3_EnumConnections(p, a, b, c, d)                            \
...
#define IDirectPlay3_InitializeConnection(p, a, b)                             \
  (p)->lpVtbl->InitializeConnection(p, a, b)
#define IDirectPlay3_SecureOpen(p, a, b, c, d)                                 \
...
#define IDirectPlay3_GetGroupParent(p, a, b)                                   \
  (p)->lpVtbl->GetGroupParent(p, a, b)
#define IDirectPlay3_GetPlayerAccount(p, a, b, c, d)                           \
  (p)->lpVtbl->GetPlayerAccount(p, a, b, c, d)
#define IDirectPlay3_GetPlayerFlags(p, a, b)                                   \
...
DECLARE_INTERFACE_(IDirectPlay4, IDirectPlay3) {
  ...
  STDMETHOD(SendEx)
  (THIS_ DPID, DPID, DWORD, LPVOID, DWORD, DWORD, DWORD, LPVOID, DWORD_PTR *)
      PURE;
...
typedef BOOL(PASCAL *LPDPENUMPLAYERSCALLBACK)(DPID dpId, LPSTR lpFriendlyName,
                                              LPSTR lpFormalName, DWORD dwFlags,
                                              LPVOID lpContext);
...
typedef BOOL(PASCAL *LPDPENUMSESSIONSCALLBACK)(LPDPSESSIONDESC lpDPSessionDesc,
                                               LPVOID lpContext,
                                               LPDWORD lpdwTimeOut,
                                               DWORD dwFlags);
...
DECLARE_INTERFACE_(IDirectPlay, IUnknown) {
  ...
  STDMETHOD(EnumGroups)
  (THIS_ DWORD_PTR, LPDPENUMPLAYERSCALLBACK, LPVOID, DWORD) PURE;
  STDMETHOD(EnumPlayers)
  (THIS_ DWORD_PTR, LPDPENUMPLAYERSCALLBACK, LPVOID, DWORD) PURE;
  STDMETHOD(EnumSessions)
...
#define IDirectPlay_EnumGroupPlayers(p, a, b, c, d)                            \
  (p)->lpVtbl->EnumGroupPlayers(p, a, b, c, d)
#define IDirectPlay_EnumGroups(p, a, b, c, d)                                  \
  (p)->lpVtbl->EnumGroups(p, a, b, c, d)
#define IDirectPlay_EnumPlayers(p, a, b, c, d)                                 \
  (p)->lpVtbl->EnumPlayers(p, a, b, c, d)
#define IDirectPlay_EnumSessions(p, a, b, c, d, e)                             \
  (p)->lpVtbl->EnumSessions(p, a, b, c, d, e)
#define IDirectPlay_GetCaps(p, a) (p)->lpVtbl->GetCaps(p, a)
#define IDirectPlay_GetMessageCount(p, a, b)                                   \
  (p)->lpVtbl->GetMessageCount(p, a, b)
#define IDirectPlay_GetPlayerCaps(p, a, b) (p)->lpVtbl->GetPlayerCaps(p, a, b)
#define IDirectPlay_GetPlayerName(p, a, b, c, d, e)                            \
...
DEFINE_GUID(IID_IDirectPlay, 0x5454e9a0, 0xdb65, 0x11ce, 0x92, 0x1c, 0x00, 0xaa,
            0x00, 0x6c, 0x49, 0x72);
