...
#define AMVA_QUERYRENDERSTATUSF_READ                                           \
  0x00000001 // Query for read
             // set this bit to 0
             // if query for update
...
typedef struct _tag_AMVAInternalMemInfo {
  DWORD dwScratchMemAlloc; // [out]    amount of scratch memory will the hal
                           // allocate for its private use
...
typedef struct _tag_AMVABeginFrameInfo {
  DWORD dwDestSurfaceIndex; // IN  destination buffer in which to decoding this
                            // frame
...
typedef struct _tag_AMVABUFFERINFO {
  ...
  DWORD dwDataOffset;  // [in]    offset of relevant data from the beginning of
                       // buffer
