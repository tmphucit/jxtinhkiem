...
DEFINE_GUID(IID_IDirectDraw, 0x6C14DB80, 0xA733, 0x11CE, 0xA5, 0x21, 0x00, 0x20,
            0xAF, 0x0B, 0xE5, 0x60);
...
DEFINE_GUID(IID_IDirectDrawClipper, 0x6C14DB85, 0xA733, 0x11CE, 0xA5, 0x21,
            0x00, 0x20, 0xAF, 0x0B, 0xE5, 0x60);
DEFINE_GUID(IID_IDirectDrawColorControl, 0x4B9F0EE0, 0x0D7E, 0x11D0, 0x9B, 0x06,
            0x00, 0xA0, 0xC9, 0x03, 0xA3, 0xB8);
DEFINE_GUID(IID_IDirectDrawGammaControl, 0x69C11C3E, 0xB46B, 0x11D1, 0xAD, 0x7A,
            0x00, 0xC0, 0x4F, 0xC2, 0x9B, 0x4E);
...
#endif
typedef BOOL(FAR PASCAL *LPDDENUMCALLBACKEXA)(GUID FAR *, LPSTR, LPSTR, LPVOID,
                                              HMONITOR);
typedef BOOL(FAR PASCAL *LPDDENUMCALLBACKEXW)(GUID FAR *, LPWSTR, LPWSTR,
                                              LPVOID, HMONITOR);
extern HRESULT WINAPI DirectDrawEnumerateExW(LPDDENUMCALLBACKEXW lpCallback,
                                             LPVOID lpContext, DWORD dwFlags);
extern HRESULT WINAPI DirectDrawEnumerateExA(LPDDENUMCALLBACKEXA lpCallback,
                                             LPVOID lpContext, DWORD dwFlags);
typedef HRESULT(WINAPI *LPDIRECTDRAWENUMERATEEXA)(
    LPDDENUMCALLBACKEXA lpCallback, LPVOID lpContext, DWORD dwFlags);
typedef HRESULT(WINAPI *LPDIRECTDRAWENUMERATEEXW)(
...
extern HRESULT WINAPI DirectDrawCreateEx(GUID FAR *lpGuid, LPVOID *lplpDD,
                                         REFIID iid, IUnknown FAR *pUnkOuter);
extern HRESULT WINAPI
DirectDrawCreateClipper(DWORD dwFlags, LPDIRECTDRAWCLIPPER FAR *lplpDDClipper,
                        IUnknown FAR *pUnkOuter);
#endif
/*
 * Flags for DirectDrawEnumerateEx
 * DirectDrawEnumerateEx supercedes DirectDrawEnumerate. You must use
 * GetProcAddress to obtain a function pointer (of type LPDIRECTDRAWENUMERATEEX)
 * to DirectDrawEnumerateEx. By default, only the primary display device is
 * enumerated. DirectDrawEnumerate is equivalent to
 * DirectDrawEnumerate(,,DDENUM_NONDISPLAYDEVICES)
 */
...
typedef HRESULT(FAR PASCAL *LPDDENUMSURFACESCALLBACK)(LPDIRECTDRAWSURFACE,
                                                      LPDDSURFACEDESC, LPVOID);
typedef HRESULT(FAR PASCAL *LPDDENUMSURFACESCALLBACK2)(LPDIRECTDRAWSURFACE4,
                                                       LPDDSURFACEDESC2,
                                                       LPVOID);
typedef HRESULT(FAR PASCAL *LPDDENUMSURFACESCALLBACK7)(LPDIRECTDRAWSURFACE7,
                                                       LPDDSURFACEDESC2,
                                                       LPVOID);
// #endif
...
typedef struct _DDBLTFX {
  ...
  DWORD dwZDestConstBitDepth; // Bit depth used to specify Z constant for
                              // destination
  union {
  ...
  DWORD dwAlphaEdgeBlendBitDepth; // Bit depth used to specify constant for
                                  // alpha edge blend
  DWORD dwAlphaEdgeBlend;         // Alpha for edge blending
  DWORD dwReserved;
  DWORD dwAlphaDestConstBitDepth; // Bit depth used to specify alpha constant
                                  // for destination
  union {
    DWORD dwAlphaDestConst;             // Constant to use as Alpha Channel
    LPDIRECTDRAWSURFACE lpDDSAlphaDest; // Surface to use as Alpha Channel
  } DUMMYUNIONNAMEN(3);
  DWORD dwAlphaSrcConstBitDepth; // Bit depth used to specify alpha constant for
                                 // source
  union {
...
/*
 * This structure represents the DDCAPS structure released in DirectDraw 1.0. It
 * is used internally by DirectDraw to interpret caps passed into ddraw by
 * drivers written prior to the release of DirectDraw 2.0. New applications
 * should use the DDCAPS structure defined below.
 */
typedef struct _DDCAPS_DX1 {
  ...
  DWORD dwMinOverlayStretch;   // minimum overlay stretch factor multiplied by
                               // 1000, eg 1000 == 1.0, 1300 == 1.3
  DWORD dwMaxOverlayStretch;   // maximum overlay stretch factor multiplied by
                               // 1000, eg 1000 == 1.0, 1300 == 1.3
  DWORD dwMinLiveVideoStretch; // OBSOLETE! This field remains for compatability
                               // reasons only
  DWORD dwMaxLiveVideoStretch; // OBSOLETE! This field remains for compatability
                               // reasons only
  DWORD dwMinHwCodecStretch;   // OBSOLETE! This field remains for compatability
                               // reasons only
  DWORD dwMaxHwCodecStretch;   // OBSOLETE! This field remains for compatability
                               // reasons only
...
/*
 * This structure is the DDCAPS structure as it was in version 2 and 3 of Direct
 * X. It is present for back compatability.
 */
typedef struct _DDCAPS_DX3 {
  ...
  DWORD dwMaxOverlayStretch;   // maximum overlay stretch factor multiplied by
                               // 1000, eg 1000 == 1.0, 1300 == 1.3
  DWORD dwMinLiveVideoStretch; // minimum live video stretch factor multiplied
                               // by 1000, eg 1000 == 1.0, 1300 == 1.3
  DWORD dwMaxLiveVideoStretch; // maximum live video stretch factor multiplied
                               // by 1000, eg 1000 == 1.0, 1300 == 1.3
  DWORD dwMinHwCodecStretch; // minimum hardware codec stretch factor multiplied
                             // by 1000, eg 1000 == 1.0, 1300 == 1.3
  DWORD dwMaxHwCodecStretch; // maximum hardware codec stretch factor multiplied
                             // by 1000, eg 1000 == 1.0, 1300 == 1.3
  DWORD dwReserved1;         // reserved
  DWORD dwReserved2;         // reserved
  DWORD dwReserved3;         // reserved
...
typedef struct _DDCAPS_DX5 {
  ...
  /* 88*/ DWORD
      dwMinOverlayStretch; // minimum overlay stretch factor multiplied by 1000,
                           // eg 1000 == 1.0, 1300 == 1.3
  /* 8c*/ DWORD
      dwMaxOverlayStretch; // maximum overlay stretch factor multiplied by 1000,
                           // eg 1000 == 1.0, 1300 == 1.3
  /* 90*/ DWORD
      dwMinLiveVideoStretch; // minimum live video stretch factor multiplied by
                             // 1000, eg 1000 == 1.0, 1300 == 1.3
  /* 94*/ DWORD
      dwMaxLiveVideoStretch; // maximum live video stretch factor multiplied by
                             // 1000, eg 1000 == 1.0, 1300 == 1.3
  /* 98*/ DWORD
      dwMinHwCodecStretch; // minimum hardware codec stretch factor multiplied
                           // by 1000, eg 1000 == 1.0, 1300 == 1.3
  /* 9c*/ DWORD
      dwMaxHwCodecStretch;   // maximum hardware codec stretch factor multiplied
                             // by 1000, eg 1000 == 1.0, 1300 == 1.3
  ...
  /*13c*/ DWORD dwNLVBCaps; // driver specific capabilities for non-local->local
                            // vidmem blts
  /*140*/ DWORD dwNLVBCaps2;    // more driver specific capabilities
                                // non-local->local vidmem blts
  /*144*/ DWORD dwNLVBCKeyCaps; // driver color key capabilities for
                                // non-local->local vidmem blts
  /*148*/ DWORD
      dwNLVBFXCaps; // driver FX capabilities for non-local->local blts
  /*14c*/ DWORD
      dwNLVBRops[DD_ROP_SPACE]; // ROPS supported for non-local->local blts
} DDCAPS_DX5;
...
typedef struct _DDPIXELFORMAT {
  ...
  union {
    ...
    DWORD dwZBufferBitDepth; // how many total bits/pixel in z buffer (including
                             // any stencil bits)
  ...
  union {
    ...
    DWORD dwStencilBitDepth;  // how many stencil bits (note:
                              // dwZBufferBitDepth-dwStencilBitDepth is total
                              // Z-only bits)
  ...
  union {
    ...
    struct {
      WORD wFlipMSTypes; // Multisample methods supported via flip for this
                         // D3DFORMAT
      WORD wBltMSTypes;  // Multisample methods supported via blt for this
                         // D3DFORMAT
    } MultiSampleCaps;
...
/*
 *  This is the structure within which DirectDraw returns data about the current
 * graphics driver and chipset
 */
...
typedef struct tagDDDEVICEIDENTIFIER {
  /*
   * These elements are for presentation to the user only. They should not be
   * used to identify particular drivers, since this is unreliable and many
   * different strings may be associated with the same device, and the same
   * driver from different vendors.
   */
  char szDriver[MAX_DDDEVICEID_STRING];
  char szDescription[MAX_DDDEVICEID_STRING];

  /*
   * This element is the version of the DirectDraw/3D driver. It is legal to do
   * <, > comparisons on the whole 64 bits. Caution should be exercised if you
   * use this element to identify problematic drivers. It is recommended that
   ...
   */
#ifdef _WIN32
  LARGE_INTEGER liDriverVersion; /* Defined for applications and other 32 bit
                                    components */
#else
  ...
  /*
   * These elements can be used to identify particular chipsets. Use with
   * extreme caution. dwVendorId     Identifies the manufacturer. May be zero if
   * unknown. dwDeviceId     Identifies the type of chipset. May be zero if
   * unknown. dwSubSysId     Identifies the subsystem, typically this means the
   * particular board. May be zero if unknown. dwRevision     Identifies the
   * revision level of the chipset. May be zero if unknown.
   */
  DWORD dwVendorId;
  DWORD dwDeviceId;
  DWORD dwSubSysId;
  ...
  /*
   * This element can be used to check changes in driver/chipset. This GUID is a
   * unique identifier for the driver/chipset pair. Use this element if you wish
   * to track changes to the driver/chipset in order to reprofile the graphics
   * subsystem. This element can also be used to identify particular problematic
   * drivers.
   */
...
/*
 * This flag causes GetDeviceIdentifier to return information about the host
 * (typically 2D) adapter in a system equipped with a stacked secondary 3D
 * adapter. Such an adapter appears to the application as if it were part of the
 * host adapter, but is typically physcially located on a separate card. The
 * stacked secondary's information is returned when GetDeviceIdentifier's
 * dwFlags field is zero, since this most accurately reflects the qualities of
 * the DirectDraw object involved.
 */
...
DECLARE_INTERFACE_(IDirectDraw, IUnknown) {
  ...
  (THIS_ DWORD, LPPALETTEENTRY, LPDIRECTDRAWPALETTE FAR *, IUnknown FAR *) PURE;
  ...
  (THIS_ DWORD, LPDDSURFACEDESC, LPVOID, LPDDENUMSURFACESCALLBACK) PURE;
...
#define IDirectDraw_CreateClipper(p, a, b, c)                                  \
  (p)->lpVtbl->CreateClipper(p, a, b, c)
#define IDirectDraw_CreatePalette(p, a, b, c, d)                               \
  (p)->lpVtbl->CreatePalette(p, a, b, c, d)
#define IDirectDraw_CreateSurface(p, a, b, c)                                  \
  (p)->lpVtbl->CreateSurface(p, a, b, c)
#define IDirectDraw_DuplicateSurface(p, a, b)                                  \
  (p)->lpVtbl->DuplicateSurface(p, a, b)
#define IDirectDraw_EnumDisplayModes(p, a, b, c, d)                            \
...
#define IDirectDraw_GetMonitorFrequency(p, a)                                  \
  (p)->lpVtbl->GetMonitorFrequency(p, a)
#define IDirectDraw_GetScanLine(p, a) (p)->lpVtbl->GetScanLine(p, a)
#define IDirectDraw_GetVerticalBlankStatus(p, a)                               \
  (p)->lpVtbl->GetVerticalBlankStatus(p, a)
#define IDirectDraw_Initialize(p, a) (p)->lpVtbl->Initialize(p, a)
#define IDirectDraw_RestoreDisplayMode(p) (p)->lpVtbl->RestoreDisplayMode(p)
#define IDirectDraw_SetCooperativeLevel(p, a, b)                               \
  (p)->lpVtbl->SetCooperativeLevel(p, a, b)
#define IDirectDraw_SetDisplayMode(p, a, b, c)                                 \
  (p)->lpVtbl->SetDisplayMode(p, a, b, c)
#define IDirectDraw_WaitForVerticalBlank(p, a, b)                              \
...
DECLARE_INTERFACE_(IDirectDraw4, IUnknown) {
  ...
  (THIS_ DWORD, LPPALETTEENTRY, LPDIRECTDRAWPALETTE FAR *, IUnknown FAR *) PURE;
  STDMETHOD(CreateSurface)
  (THIS_ LPDDSURFACEDESC2, LPDIRECTDRAWSURFACE4 FAR *, IUnknown FAR *) PURE;
  STDMETHOD(DuplicateSurface)
  (THIS_ LPDIRECTDRAWSURFACE4, LPDIRECTDRAWSURFACE4 FAR *) PURE;
  STDMETHOD(EnumDisplayModes)
  (THIS_ DWORD, LPDDSURFACEDESC2, LPVOID, LPDDENUMMODESCALLBACK2) PURE;
  STDMETHOD(EnumSurfaces)
  (THIS_ DWORD, LPDDSURFACEDESC2, LPVOID, LPDDENUMSURFACESCALLBACK2) PURE;
  STDMETHOD(FlipToGDISurface)(THIS) PURE;
  STDMETHOD(GetCaps)(THIS_ LPDDCAPS, LPDDCAPS) PURE;
...
DECLARE_INTERFACE_(IDirectDraw7, IUnknown) {
  ...
  (THIS_ DWORD, LPPALETTEENTRY, LPDIRECTDRAWPALETTE FAR *, IUnknown FAR *) PURE;
  STDMETHOD(CreateSurface)
  (THIS_ LPDDSURFACEDESC2, LPDIRECTDRAWSURFACE7 FAR *, IUnknown FAR *) PURE;
  STDMETHOD(DuplicateSurface)
  (THIS_ LPDIRECTDRAWSURFACE7, LPDIRECTDRAWSURFACE7 FAR *) PURE;
  STDMETHOD(EnumDisplayModes)
  (THIS_ DWORD, LPDDSURFACEDESC2, LPVOID, LPDDENUMMODESCALLBACK2) PURE;
  STDMETHOD(EnumSurfaces)
  (THIS_ DWORD, LPDDSURFACEDESC2, LPVOID, LPDDENUMSURFACESCALLBACK7) PURE;
  STDMETHOD(FlipToGDISurface)(THIS) PURE;
  STDMETHOD(GetCaps)(THIS_ LPDDCAPS, LPDDCAPS) PURE;
  STDMETHOD(GetDisplayMode)(THIS_ LPDDSURFACEDESC2) PURE;
  STDMETHOD(GetFourCCCodes)(THIS_ LPDWORD, LPDWORD) PURE;
...
#define IDirectDrawPalette_Initialize(p, a, b, c)                              \
  (p)->lpVtbl->Initialize(p, a, b, c)
#define IDirectDrawPalette_SetEntries(p, a, b, c, d)                           \
...
#define IDirectDrawClipper_GetClipList(p, a, b, c)                             \
  (p)->lpVtbl->GetClipList(p, a, b, c)
#define IDirectDrawClipper_GetHWnd(p, a) (p)->lpVtbl->GetHWnd(p, a)
#define IDirectDrawClipper_Initialize(p, a, b) (p)->lpVtbl->Initialize(p, a, b)
#define IDirectDrawClipper_IsClipListChanged(p, a)                             \
  (p)->lpVtbl->IsClipListChanged(p, a)
#define IDirectDrawClipper_SetClipList(p, a, b)                                \
  (p)->lpVtbl->SetClipList(p, a, b)
...
DECLARE_INTERFACE_(IDirectDrawSurface, IUnknown) {
  ...
  STDMETHOD(Blt)
  (THIS_ LPRECT, LPDIRECTDRAWSURFACE, LPRECT, DWORD, LPDDBLTFX) PURE;
  STDMETHOD(BltBatch)(THIS_ LPDDBLTBATCH, DWORD, DWORD) PURE;
  STDMETHOD(BltFast)
  (THIS_ DWORD, DWORD, LPDIRECTDRAWSURFACE, LPRECT, DWORD) PURE;
  ...
  (THIS_ LPRECT, LPDIRECTDRAWSURFACE, LPRECT, DWORD, LPDDOVERLAYFX) PURE;
...
#define IDirectDrawSurface_DeleteAttachedSurface(p, a, b)                      \
  (p)->DeleteAttachedSurface(a, b)
#define IDirectDrawSurface_EnumAttachedSurfaces(p, a, b)                       \
  (p)->EnumAttachedSurfaces(a, b)
#define IDirectDrawSurface_EnumOverlayZOrders(p, a, b, c)                      \
  (p)->EnumOverlayZOrders(a, b, c)
#define IDirectDrawSurface_Flip(p, a, b) (p)->Flip(a, b)
#define IDirectDrawSurface_GetAttachedSurface(p, a, b)                         \
  (p)->GetAttachedSurface(a, b)
...
DECLARE_INTERFACE_(IDirectDrawSurface2, IUnknown) {
  ...
  STDMETHOD(AddOverlayDirtyRect)(THIS_ LPRECT) PURE;
  STDMETHOD(Blt)
  (THIS_ LPRECT, LPDIRECTDRAWSURFACE2, LPRECT, DWORD, LPDDBLTFX) PURE;
  STDMETHOD(BltBatch)(THIS_ LPDDBLTBATCH, DWORD, DWORD) PURE;
  STDMETHOD(BltFast)
  (THIS_ DWORD, DWORD, LPDIRECTDRAWSURFACE2, LPRECT, DWORD) PURE;
  ...
  STDMETHOD(Unlock)(THIS_ LPVOID) PURE;
  STDMETHOD(UpdateOverlay)
  (THIS_ LPRECT, LPDIRECTDRAWSURFACE2, LPRECT, DWORD, LPDDOVERLAYFX) PURE;
  STDMETHOD(UpdateOverlayDisplay)(THIS_ DWORD) PURE;
...
#define IDirectDrawSurface2_UpdateOverlayZOrder(p, a, b)                       \
  (p)->lpVtbl->UpdateOverlayZOrder(p, a, b)
#define IDirectDrawSurface2_GetDDInterface(p, a)                               \
  (p)->lpVtbl->GetDDInterface(p, a)
...
DECLARE_INTERFACE_(IDirectDrawSurface3, IUnknown) {
  ...
  STDMETHOD(AddOverlayDirtyRect)(THIS_ LPRECT) PURE;
  STDMETHOD(Blt)
  (THIS_ LPRECT, LPDIRECTDRAWSURFACE3, LPRECT, DWORD, LPDDBLTFX) PURE;
  STDMETHOD(BltBatch)(THIS_ LPDDBLTBATCH, DWORD, DWORD) PURE;
  STDMETHOD(BltFast)
  (THIS_ DWORD, DWORD, LPDIRECTDRAWSURFACE3, LPRECT, DWORD) PURE;
  ...
  STDMETHOD(Unlock)(THIS_ LPVOID) PURE;
  STDMETHOD(UpdateOverlay)
  (THIS_ LPRECT, LPDIRECTDRAWSURFACE3, LPRECT, DWORD, LPDDOVERLAYFX) PURE;
  STDMETHOD(UpdateOverlayDisplay)(THIS_ DWORD) PURE;
...
#define IDirectDrawSurface3_SetSurfaceDesc(p, a, b)                            \
  (p)->lpVtbl->SetSurfaceDesc(p, a, b)
#else
...
DECLARE_INTERFACE_(IDirectDrawSurface4, IUnknown) {
  ...
  STDMETHOD(AddOverlayDirtyRect)(THIS_ LPRECT) PURE;
  STDMETHOD(Blt)
  (THIS_ LPRECT, LPDIRECTDRAWSURFACE4, LPRECT, DWORD, LPDDBLTFX) PURE;
  STDMETHOD(BltBatch)(THIS_ LPDDBLTBATCH, DWORD, DWORD) PURE;
  STDMETHOD(BltFast)
  (THIS_ DWORD, DWORD, LPDIRECTDRAWSURFACE4, LPRECT, DWORD) PURE;
  ...
  STDMETHOD(UpdateOverlay)
  (THIS_ LPRECT, LPDIRECTDRAWSURFACE4, LPRECT, DWORD, LPDDOVERLAYFX) PURE;
  STDMETHOD(UpdateOverlayDisplay)(THIS_ DWORD) PURE;
...
#define IDirectDrawSurface4_SetSurfaceDesc(p, a, b)                            \
  (p)->lpVtbl->SetSurfaceDesc(p, a, b)
#define IDirectDrawSurface4_SetPrivateData(p, a, b, c, d)                      \
  (p)->lpVtbl->SetPrivateData(p, a, b, c, d)
#define IDirectDrawSurface4_GetPrivateData(p, a, b, c)                         \
  (p)->lpVtbl->GetPrivateData(p, a, b, c)
#define IDirectDrawSurface4_FreePrivateData(p, a)                              \
  (p)->lpVtbl->FreePrivateData(p, a)
#define IDirectDrawSurface4_GetUniquenessValue(p, a)                           \
  (p)->lpVtbl->GetUniquenessValue(p, a)
#define IDirectDrawSurface4_ChangeUniquenessValue(p)                           \
  (p)->lpVtbl->ChangeUniquenessValue(p)
#else
...
DECLARE_INTERFACE_(IDirectDrawSurface7, IUnknown) {
  ...
  STDMETHOD(AddOverlayDirtyRect)(THIS_ LPRECT) PURE;
  STDMETHOD(Blt)
  (THIS_ LPRECT, LPDIRECTDRAWSURFACE7, LPRECT, DWORD, LPDDBLTFX) PURE;
  STDMETHOD(BltBatch)(THIS_ LPDDBLTBATCH, DWORD, DWORD) PURE;
  STDMETHOD(BltFast)
  (THIS_ DWORD, DWORD, LPDIRECTDRAWSURFACE7, LPRECT, DWORD) PURE;
  ...
  STDMETHOD(Unlock)(THIS_ LPRECT) PURE;
  STDMETHOD(UpdateOverlay)
  (THIS_ LPRECT, LPDIRECTDRAWSURFACE7, LPRECT, DWORD, LPDDOVERLAYFX) PURE;
  STDMETHOD(UpdateOverlayDisplay)(THIS_ DWORD) PURE;
...
#define IDirectDrawColorControl_GetColorControls(p, a)                         \
  (p)->lpVtbl->GetColorControls(p, a)
#define IDirectDrawColorControl_SetColorControls(p, a)                         \
...
#define IDirectDrawGammaControl_GetGammaRamp(p, a, b)                          \
  (p)->lpVtbl->GetGammaRamp(p, a, b)
#define IDirectDrawGammaControl_SetGammaRamp(p, a, b)                          \
...
typedef struct _DDSURFACEDESC2 {
  ...
  union {
    DWORD dwMipMapCount; // number of mip-map levels requestde
                         // dwZBufferBitDepth removed, use ddpfPixelFormat one
                         // instead
    DWORD dwRefreshRate; // refresh rate (used when display mode is described)
...
/*
 * Indicates that this surface is an overlay.  It may or may not be directly
 * visible depending on whether or not it is currently being overlayed onto the
 * primary surface.  DDSCAPS_VISIBLE can be used to determine whether or not it
 * is being overlayed at the moment.
 */
 ...
 * currently only valid in conjunction with the DDSCAPS_TEXTURE flag. The
 * surface will be created without any underlying video memory until loaded.
 */
...
/*
 * Indicates that the client would like this texture surface to be managed by
 * the DirectDraw/Direct3D runtime. Surfaces with this cap set must also have
 * DDSCAPS_TEXTURE set.
 */
 ...
 * The driver is free to optimize this surface via retiling and actual
 * compression. All calls to Lock() or Blts from this surface will fail.
 * Surfaces with this cap set must also have DDSCAPS_TEXTURE. This cap cannot be
 * used with DDSCAPS2_HINTDYNAMIC and DDSCAPS2_HINTSTATIC.
 */
...
/*
 * This macro may be used to specify all faces of a cube map at CreateSurface
 * time
 */
...
/*
 * This flag is an additional flag which is present on mipmap sublevels from DX7
 * onwards It enables easier use of GetAttachedSurface rather than
 * EnumAttachedSurfaces for surface constructs such as Cube Maps, wherein there
 * are more than one mipmap surface attached to the root surface. This caps bit
 * is ignored by CreateSurface
 */
...
/*
 * Indicates that all surfaces in this creation chain should be given an alpha
 * channel. This flag will be set on primary surface chains that may have no
 * explicit pixel format (and thus take on the format of the current display
 * mode). The driver should infer that all these surfaces have a format having
 * an alpha channel. (e.g. assume D3DFMT_A8R8G8B8 if the display mode is x888.)
 */
#define DDSCAPS2_ENABLEALPHACHANNEL 0x20000000L

/*
 * Indicates that all surfaces in this creation chain is extended primary
 * surface format. This flag will be set on extended primary surface chains that
 * always have explicit pixel format and the pixel format is typically GDI
 * (Graphics Device Interface) couldn't handle, thus only used with fullscreen
 * application. (e.g. D3DFMT_A2R10G10B10 format)
 */
#define DDSCAPS2_EXTENDEDFORMATPRIMARY 0x40000000L

/*
 * Indicates that all surfaces in this creation chain is additional primary
 * surface. This flag will be set on primary surface chains which must present
 * on the adapter id provided on dwCaps4. Typically this will be used to create
 ...
 * on DIRECTDRAWCAPS.dwAlignBoundaryDest boundaries of the surface,
...
/*
 * Indicates that DirectDraw will support only source rectangles that are
 * aligned on DIRECTDRAWCAPS.dwAlignBoundarySrc boundaries of the surface,
 * respectively. READ ONLY.
 */
...
/*
 * Indicates that DirectDraw will create video memory surfaces that have a
 * stride alignment equal to DIRECTDRAWCAPS.dwAlignStride.  READ ONLY.
 */
...
/*
 * Display hardware is capable of color space conversions during the blt
 * operation.
 */
...
/*
 * Indicates that unique DirectDrawPalettes can be created for
 * DirectDrawSurfaces other than the primary surface.
 */
...
/*
 * Driver supports bltting any FOURCC surface to another surface of the same
 * FOURCC
 */
 ...
 * DirectX manages the texture if this flag is OFF and surface has
 * DDSCAPS2_TEXTUREMANAGE on
 */
#define DDCAPS2_CANMANAGETEXTURE 0x00800000l

/*
 * The Direct3D texture manager uses this cap to decide whether to put managed
 * surfaces in non-local video memory. If the cap is set, the texture manager
 * will put managed surfaces in non-local vidmem. Drivers that cannot texture
 * from local vidmem SHOULD NOT set this cap.
 */
#define DDCAPS2_TEXMANINNONLOCALVIDMEM 0x01000000l

/*
 * Indicates that the driver supports DX7 type of stereo in at least one mode
 * (which may not necessarily be the current mode). Applications should use
 * IDirectDraw7 (or higher)
 * ::EnumDisplayModes and check the DDSURFACEDESC.ddsCaps.dwCaps2 field for the
 * presence of DDSCAPS2_STEREOSURFACELEFT to check if a particular mode supports
 * stereo. The application can also use IDirectDraw7(or higher)::GetDisplayMode
 ...
 * of textures. This is NOT meant to imply that hardware can write into AGP
 * memory. This operation is not currently supported.
 */
 ...
 * DirectX manages the resource if this flag is OFF and surface has
 * DDSCAPS2_TEXTUREMANAGE on
 */
 ...
 * 1,2,4, or 8.  The alpha value becomes more opaque as the alpha value
 * increases. (0 is transparent.) For Blt.
 */
 ...
 * Supports alpha only surfaces.  The bit depth of an alpha only surface can be
 * 1,2,4, or 8.  The alpha value becomes more opaque as the alpha value
 * increases. (0 is transparent.) For Overlays.
 */
...
/*
 * Enumerate the first surface that can be created which meets the search
 * criterion.
 */
 ...
 * that a particular mode will be enumerated only once.  This flag specifies
 * whether the refresh rate is taken into account when determining if a mode is
 * unique.
 */
 ...
 * modes such as mode 0x13 in addition to the usual ModeX modes (which are
 * always enumerated if the application has previously called
 * SetCooperativeLevel with the DDSCL_ALLOWMODEX flag set).
 */
 ...
 * several blts. All blts (even if not clipped) are tagged with
 * DDBLT_PRESENTATION and the last (even if not clipped) additionally with
 * DDBLT_LAST_PRESENTATION. Thus the true rule is that the driver must not
 * schedule a DDBLT_PRESENTATION blt if there are 3 or more DDBLT_PRESENTLAST
 * blts in the hardware pipe. If there are such blts in the pipe, the driver
 * should return DDERR_WASSTILLDRAWING until the oldest queued
 * DDBLT_LAST_PRESENTATION blts has been retired (i.e. the pixels have been
 * actually written to the primary surface). Once the oldest blt has been
 * retired, the driver is free to schedule the current blt. The goal is to
 * provide a mechanism whereby the device's hardware queue never gets more than
 * 3 frames ahead of the frames being generated by the application. When
 * excessive queueing occurs, applications become unusable because the
 * application visibly lags user input, and such problems make windowed
 * interactive applications impossible. Some drivers may not have sufficient
 * knowledge of their hardware's FIFO to know when a certain blt has been
 * retired. Such drivers should code cautiously, and simply not allow any frames
 * to be queued at all. DDBLT_LAST_PRESENTATION should cause such drivers to
 * return DDERR_WASSTILLDRAWING until the accelerator is completely finished-
 * exactly as if the application had called Lock on the source surface before
 * calling Blt. In other words, the driver is allowed and encouraged to generate
 * as much latency as it can, but never more than 3 frames worth. Implementation
 * detail: Drivers should count blts against the SOURCE surface, not against the
 * primary surface. This enables multiple parallel windowed application to
 * function more optimally. This flag is passed only to DX8 or higher drivers.
 *
 ...
 * ---------------------------------------------------------------
 *  2  0x00000004  DDBLT_EXTENDED_LINEAR_CONTENT DDBLT_ALPHADESTNEG 4 0x00000010
 * DDBLT_EXTENDED_PRESENTATION_STRETCHFACTOR    DDBLT_ALPHAEDGEBLEND
 *
 *
...
/*
 * Flip Interval Flags. These flags indicate how many vertical retraces to wait
 * between each flip. The default is one. DirectDraw will return
 * DDERR_WASSTILLDRAWING for each surface involved in the flip until the
 * specified number of vertical retraces has ocurred. Only effective if
 * DDCAPS2_FLIPINTERVAL is set. If that bit is not set, DDFLIP_INTERVALn has no
 * effect.
 */
...
/*
 * On IDirectDrawSurface7 and higher interfaces, the default is DDFLIP_WAIT. If
 * you wish to override the default and use time when the accelerator is busy
 * (as denoted by the DDERR_WASSTILLDRAWING return code) then use
 * DDFLIP_DONOTWAIT.
 */
...
/*
 * Indicates that ARGB scaling factors can be degraded to fit driver
 * capabilities.
 */
 ...
 *      When the application locks the surface, the driver is forced to
 * un-re-order the surface data before allowing the application to see the
 * surface contents. This flag is a hint to the driver that it can skip the
 * un-re-ordering process since the application plans to overwrite every single
 * pixel in the surface or locked rectangle (and so erase any un-re-ordered
 * pixels anyway). Applications should always set this flag when they intend to
 * overwrite the entire surface or locked rectangle.
 */
...
/*
 * On IDirectDrawSurface7 and higher interfaces, the default is DDLOCK_WAIT. If
 * you wish to override the default and use time when the accelerator is busy
 * (as denoted by the DDERR_WASSTILLDRAWING return code) then use
 * DDLOCK_DONOTWAIT.
 */
...
/*
 * This indicates that the driver should not update dirty rect information for
 * this lock.
 */
 ...
 * paltte-indexed surface when the surface had no palette AND the display mode
 * was not palette-indexed (in this case DirectDraw cannot select a proper
 * palette into the DC)
 */
...
/*
 * Device does not support optimized surfaces, therefore no video memory
 * optimized surfaces
 */
