...
#define TRUECOLOR(pbmi)                                                        \
  ((TRUECOLORINFO *)(((LPBYTE) & ((pbmi)->bmiHeader)) +                        \
                     (pbmi)->bmiHeader.biSize))
#define COLORS(pbmi)                                                           \
  ((RGBQUAD *)(((LPBYTE) & ((pbmi)->bmiHeader)) + (pbmi)->bmiHeader.biSize))
#define BITMASKS(pbmi)                                                         \
  ((DWORD *)(((LPBYTE) & ((pbmi)->bmiHeader)) + (pbmi)->bmiHeader.biSize))
...
#define DIBWIDTHBYTES(bi)                                                      \
  (DWORD) WIDTHBYTES((DWORD)(bi).biWidth *(DWORD)(bi).biBitCount)
...
#define RESET_PALETTE(pbmi)                                                    \
  (ZeroMemory((PVOID)(pbmi)->bmiColors, SIZE_PALETTE));

#if 0
// !!! This is the right way to do it, but may break existing code
#define RESET_MASKS(pbmi)                                                      \
  (ZeroMemory((PVOID)(((LPBYTE)(pbmi)->bmiHeader) + (pbmi)->bmiHeader.biSize,  \
                      SIZE_MASKS)))
#define RESET_HEADER(pbmi)                                                     \
  (ZeroMemory((PVOID)(pbmi), SIZE_PREHEADER + sizeof(BITMAPINFOHEADER)))
...
#define SIZE_MPEG1VIDEOINFO(pv)                                                \
  (FIELD_OFFSET(MPEG1VIDEOINFO, bSequenceHeader[0]) + (pv)->cbSequenceHeader)
