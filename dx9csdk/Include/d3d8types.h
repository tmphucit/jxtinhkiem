...
typedef enum _D3DRENDERSTATETYPE {
  ...
  D3DRS_POSITIONORDER = 172,    // NPatch position interpolation order.
                                // D3DORDER_LINEAR or D3DORDER_CUBIC (default)
  D3DRS_NORMALORDER = 173, // NPatch normal interpolation order. D3DORDER_LINEAR
                           // (default) or D3DORDER_QUADRATIC
...
typedef enum _D3DTEXTURESTAGESTATETYPE {
  ...
  D3DTSS_MIPFILTER = 18, /* D3DTEXTUREFILTER filter to use between mipmaps
                            during minification */
  D3DTSS_MIPMAPLODBIAS = 19, /* float Mipmap LOD bias */
  D3DTSS_MAXMIPLEVEL =
      20, /* DWORD 0..(n-1) LOD index of largest map to use (0 == largest) */
...
appear, the type of each element, and the vertex register into which to load an
element. Streams are allowed to include data which is not loaded into a vertex
register, thus allowing data which is not used for this shader to exist in the
vertex stream.  This skipped data is defined only by a count of DWORDs to skip
over, since the type information is irrelevant. The token sequence: Stream
Select: stream=0 Stream Data Definition (Load): type=FLOAT3; register=3 Stream
Data Definition (Load): type=FLOAT3; register=4 Stream Data Definition (Skip):
...
registers and the fourth skipped over.  Register 3 is loaded with the first
three DWORDs in each vertex interpreted as FLOAT data.  Register 4 is loaded
with the 4th, 5th, and 6th DWORDs interpreted as FLOAT data.  The next two
DWORDs (7th and 8th) are skipped over and not loaded into any vertex input
register.   Register 7 is loaded with the 9th and 10th DWORDS interpreted as
FLOAT data. Placing of tokens other than NOPs between the Stream Selector and
Stream Data Definition tokens is disallowed.
...
typedef enum _D3DVSD_TOKENTYPE {
  ...
  D3DVSD_TOKEN_STREAMDATA,  // stream data definition (map to vertex input
                            // memory)
...
#define D3DVSDT_SHORT2                                                         \
  0x06 // 2D signed short expanded to (value, value, 0., 1.)
...
typedef enum _D3DSHADER_PARAM_REGISTER_TYPE {
  ...
  D3DSPR_TEXCRDOUT =
      6 << D3DSP_REGTYPE_SHIFT,    // Texture Coordinate Output Register File
...
typedef enum _D3DVS_ADDRESSMODE_TYPE {
  ...
  D3DVS_ADDRMODE_RELATIVE =
      (1 << D3DVS_ADDRESSMODE_SHIFT),      // Relative to register A0
...
#define D3DDEVINFOID_RESOURCEMANAGER                                           \
  5                                /* Used with D3DDEVINFO_RESOURCEMANAGER */
