...
HCURSOR CDebugWinDlg::OnQueryDragIcon() { return (HCURSOR)m_hIcon; }
...
BOOL CDebugWinDlg::OnCopyData(CWnd *pWnd, COPYDATASTRUCT *pCopyDataStruct) {
  ...
  if (!m_bAllChannel) {
    if (*(char *)pCopyDataStruct->lpData == '[') {
      if (NULL ==
          (pStartString = strstr((const char *)pCopyDataStruct->lpData, "]")))
        return FALSE;
      char sChannel[200];
      memcpy(sChannel, (char *)(pCopyDataStruct->lpData) + 1,
             pStartString - pCopyDataStruct->lpData - 1);
      sChannel[pStartString - pCopyDataStruct->lpData - 1] = 0;

      if (LB_ERR == m_ChannelList.FindString(0, sChannel))
        return FALSE;
      else
        pStartString++;
    } else
  ...
  if (!m_bWriteTime)
    ...
    str.Format("[%s] %s  [0x%x]", tmpbuf, (char *)pCopyDataStruct->lpData,
               pWnd);
