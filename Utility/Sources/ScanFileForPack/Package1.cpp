...
void PutSampleFilePathInContainer(SampleFileExecuter &CollectFileExecute,
                                  FilePathContainer &Container) {
...
FilePathContainer ConstructFilePathSet(vector<FileHandle>::iterator _First,
                                       vector<FileHandle>::iterator _Last,
                                       auto_ptr<CodingMethod> &Coding) {
...
void ConstructionFolderArray(vector<string> &FolderArray, int Count,
                             char *FolderName[], char *RootFolder) {
  cout << "文件夹采集:" << endl;
...
FilePathContainer ProcessFolderFile(
    vector<string> &FolderArray, const string &RootName,
    KScanFile &SampleExecuter, FilePathContainer &Container,
    auto_ptr<CodingMethod> &SimpleCodingMethod,
    SampleProcess<vector<FileHandle>::iterator, vector<FileHandle>::iterator>
        *GetRepeatedProcessMethod) {
  ...
  vector<FileHandle>::iterator End = GetRepeatedProcessMethod->Process(
      Container.begin(), Container.end(), Temp.begin());
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    auto_ptr<CodingMethod> g_SimpleCodingMethod =
        auto_ptr<CodingMethod>(new CodingMethod);

    FilePathContainer g_Container(g_SimpleCodingMethod),
        g_Temp(g_SimpleCodingMethod);

    auto_ptr<KScanFile> g_ScanFile = auto_ptr<KScanFile>(new KScanFile());

    SampleProcess<vector<FileHandle>::iterator, vector<FileHandle>::iterator>
        *GetRepeatedProcessMethod =
            new SampleProcess<vector<FileHandle>::iterator,
                              vector<FileHandle>::iterator>();
    ...
    FilePathContainer ResultContainer = ProcessFolderFile(
        FolderArray, string(argv[1]), *g_ScanFile, g_Container,
        g_SimpleCodingMethod, GetRepeatedProcessMethod);
