...
FilePathContainer::FilePathContainer(auto_ptr<CodingMethod> &FileCoder)
    : m_Coder(FileCoder), m_FilePathSet() {}
...
void FilePathContainer::InsertFilePath(const string &FilePath) {
  ...
  try {
    ...
    string ErrorString =
        string("Error when InsertFile Path. system Info:") + Error.what();
...
size_t FilePathContainer::GetHandleCount() { return m_FilePathSet.size(); }
