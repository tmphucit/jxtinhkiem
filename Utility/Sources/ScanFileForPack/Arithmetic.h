...
inline OutputIterator SelectElemCountNotOne(InputIterator _First,
  ...
  if (_DestBeg == NULL) {
    throw exception(
        "OutputIterator must not be null when SelectElemCountNotOne");
  }
  ...
  try {
    ...
    string ErrorString =
        string("sort Error when SelectElemCountNotOne .system info:") +
        Error.what();
  ...
  for (Pointer = _First; Pointer != _Last; Pointer++) {
    ...
    try {
      ...
      string ErrorString =
          string("Error when SelectElemCountNotOne Get gather system info:") +
          Error.what();
      throw exception(ErrorString.c_str());
    }
  }
