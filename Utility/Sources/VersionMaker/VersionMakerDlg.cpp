...
CVersionMakerDlg::CVersionMakerDlg(CWnd *pParent /*=NULL*/)
    : CDialog(CVersionMakerDlg::IDD, pParent), m_BackUpStatus(-1) {
...
HCURSOR CVersionMakerDlg::OnQueryDragIcon() { return (HCURSOR)m_hIcon; }
...
void CVersionMakerDlg::OnOK() {
  ...
  BackupUpDateFile(GetUpdataDataFolderName().c_str(),
                   GetUpdateDataBackUpFolderName().c_str());
  // 拷贝本次更新内容
  SetBackUpStatus(FALSE);
  SetFileAttributeOfAFolder(GetUpdataDataFolderName().c_str(),
...
bool CVersionMakerDlg::MakeFileList() {
  ...
  sprintf(szFileListName, "%s%s\\FileList%04d", strBuf, KLEP_FILELIST,
          m_nCurVersion + 1);
...
void CopyFileCB(const char *szFileName, void *pParam) {
  ...
  if (!MakeSureDirectoryPathExists(szDesFullPath)) {
    string ErrorInfo = string("Error when MakeSureDirectoryPathExists(") +
                       string(szDesFullPath) + string(" )");
    DisplayErrorInfo(ErrorInfo);
  }

  if (!CopyFile(szFileName, szDesFullPath, FALSE)) {
    string ErrorInfo = string("Error when CopyFile(") + string(szFileName) +
                       string(",") + string(szDesFullPath) + string(")");
    DisplayErrorInfo(ErrorInfo);
...
void MakeFileListCB(const char *szFileName, void *pParam) {
  ...
  sprintf(szContent, "%d年%d月%d日%d时%d分%d秒%d毫秒", SysTime.wYear,
...
void CVersionMakerDlg::SortFileListInfo() {
  sort(m_NewFileListVector.begin(),
       &m_NewFileListVector[m_NewFileListVector.size()]);
}
...
void CVersionMakerDlg::OutputToFileLsit(const char *szFileListName) {
  ...
  for (theIterator = m_NewFileListVector.begin();
       theIterator != m_NewFileListVector.end(); theIterator++) {
...
void CVersionMakerDlg::MakeVersionInfo() {
  ...
  RunAutoBat(GetVersionDataFolderName(), GetToolsFolderName(),
             GetInfoFolderName(), GetIndexFolder());
}
...
void CVersionMakerDlg::MakeVersionInfo(int VersionNumber) {
  ...
  m_DesPath.GetWindowText(strBuf);
  sprintf(szOldFileList, "%s%s\\%s%04d", strBuf, KLEP_FILELIST, "FileList",
          VersionNumber);
  ...
  PickOutFileNameBetweenRootAndNotRoot(UpdateFileName, UpdateFileNameInRoot,
                                       UpdateFileNameNotInRoot, RootPath);

  if (VersionNumber == 0) {

    InRootFileProcess(UpdateFileNameInRoot, GetVersionDataFolderName());
  }
  InNotRootFileProcess(UpdateFileNameNotInRoot, VersionNumber,
                       GetUpdataDataFolderName());
}

bool CVersionMakerDlg::GetUpdateFileNameOnThisVersion(
    const char *szOldFileList, vector<string> &UpdateFileName) {
  ...
  if (false == ReadFileList(OldFileVector, szOldFileList)) {
    for (NewIterator = m_NewFileListVector.begin();
         NewIterator != m_NewFileListVector.end(); NewIterator++) {
  ...
  for (NewIterator = m_NewFileListVector.begin();
       NewIterator != m_NewFileListVector.end(); NewIterator++) {
    FindIterator =
        find(OldFileVector.begin(), OldFileVector.end(), *NewIterator);
...
bool CVersionMakerDlg::ReadFileList(KFileInfoVector &FileInfoVector,
                                    const char *szFileListName) {
  KLEFile File;
  KFileInfo FileInfo;
...
void CVersionMakerDlg::PickOutFileNameBetweenRootAndNotRoot(
    vector<string> &FileNameArray, vector<FilePath> &FileNameInRoot,
    vector<FilePath> &FileNameNotInRoot, const string &RootPath) {
  ...
  */
  ...
  string HalfPath;
  string FileName;
  for (FileNamePoint = FileNameArray.begin();
       FileNamePoint != FileNameArray.end(); FileNamePoint++) {
...
void CVersionMakerDlg::InRootFileProcess(
    vector<FilePath> &FileName, string &ThisVersionFileFolderName) const {
  ...
  for (FilePoint = FileName.begin(); FilePoint != FileName.end(); FilePoint++) {
    string VersionFileName =
        ThisVersionFileFolderName + "\\" + (*FilePoint).HalfPath;
    ...
    if (!CopyFile((*FilePoint).FullPath.c_str(), VersionFileName.c_str(),
                  FALSE)) {
      string ErrorInfo = string("Error when CopyFile(") +
                         (*FilePoint).FullPath + string(",") + VersionFileName +
                         string(",FALSE)");
      DisplayErrorInfo(ErrorInfo);
      throw exception(ErrorInfo.c_str());
    }
...
string CVersionMakerDlg::CreateVersionFolder(int nOldVersion) {
  ...
  VersionFileFolder << GetVersionDataFolderName() << "\\" << nOldVersion
                    << '\0';
...
void CVersionMakerDlg::InNotRootFileProcess(vector<FilePath> &FileName,
                                            int nOldVersion, string RootName) {
  ...
  if (FileName.size() > 0) {
    ...
    PackFullFileNameStream << PackFileName << nOldVersion << "-"
                           << m_nCurVersion + 1 << ".pak" << ends;
...
void CVersionMakerDlg::CreateVersionConfigFile() {
  string VersionConfigPath =
      GetVersionDataFolderName() + "\\" + VersionConfigFileName;
  //  确保目录创建成功
...
void CVersionMakerDlg::RunAutoBat(const string &DataPath,
  ...
  string Command = IndexGenExeFile + FilesPath + OutPath + " 1  " +
                   ServerUpdateFolder + " " + CommonIndexPath +
                   FilesCommentPath + infoPath;
...
void CVersionMakerDlg::DeleteADirectoryAllFile(const char *DirectoryName) {
  ...
  for (Pointer = FilenameSet.begin(); Pointer != FilenameSet.end(); Pointer++) {
    if (!SetFileAttributes((*Pointer).c_str(), FILE_ATTRIBUTE_NORMAL)) {
      string ErrorInfo = string("SetFileAttributes('") + *Pointer +
                         string("',FILE_ATTRIBUTE_NORMAL)");
      DisplayErrorInfo(ErrorInfo);
    }
...
void CVersionMakerDlg::BackupUpDateFile(const char *SrcFolder,
...
void CVersionMakerDlg::OnResume() {
  ...
  if (m_nCurVersion > 0 && m_BackUpStatus == 0) {
    ...
    BackupUpDateFile(GetUpdateDataBackUpFolderName().c_str(),
                     GetUpdataDataFolderName().c_str());
...
void CVersionMakerDlg::SetFileAttributeOfAFolder(const char *FolderName,
  ...
  for (Pointer = FilenameSet.begin(); Pointer != FilenameSet.end(); Pointer++) {
    if (!SetFileAttributes((*Pointer).c_str(), dwFileAttributes)) {
      ...
      string ErrorInfo =
          "SetFileAttributes('" + (*Pointer) + "'," + FileAttributes + ")";
      DisplayErrorInfo(ErrorInfo);
    }
  }
}
...
void CVersionMakerDlg::GetUpdateRelativeFile(vector<string> &FileNameS) const {
  try {
    string UpdateRelativeFilePath =
        GetInfoFolderName() + string(KLEP_UpdateRelativeFile);
...
bool CVersionMakerDlg::CheckFileNameInFileS(const string &FileSample,
                                            vector<string> &FileGroup) const {
  vector<string>::iterator Position;
  Position = find_if(FileGroup.begin(), FileGroup.end(),
                     CompStringNotMatchCase(FileSample));
