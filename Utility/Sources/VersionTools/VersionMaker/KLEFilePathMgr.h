...
public:
  ...
  static void ComposePath(char *szResult, const char *szFirstPath,
                          const char *szSecondPath);
  //!  由 Driver, Dir, 文件名, 文件扩展名构成一个全路径, 可传入空指针
  static void MakeFullPath(char *szFullPath, const char *szDriver,
                           const char *szDir, const char *szFile, char *szExt);
  //!  由 一个全路径分解为 Driver, Dir, 文件名, 文件扩展名, 可传入空指针
  static void SpilitFullPath(const char *szFullPath, char *szDriver,
