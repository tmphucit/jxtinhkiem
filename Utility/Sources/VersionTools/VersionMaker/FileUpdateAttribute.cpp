...
FileUpdateAttribute::FileUpdateAttribute(string &File)
    : Belong("-1"), CheckVersionMethod("5"), UpdateFileFlag("1"),
      LocalPath("%PROGRAMDIR%"), UpdateFileMethod(""), RemotePath(""),
      FileName(File), UpdateRelative("") {}

FileUpdateAttribute::~FileUpdateAttribute() {}

void FileUpdateAttribute::SetBelone(string &BeloneValue) {
  Belong = BeloneValue;
}

void FileUpdateAttribute::SetCheckVersionMethod(
    string &CheckVersionMethodValue) {
...
void FileUpdateAttribute::DumpTostream(iostream &InfoStream) {
  ...
  if (CheckVersionMethod != "") {
    InfoStream << ("CheckVersionMethod = " + CheckVersionMethod).c_str()
               << endl;
  }
