...
#if !defined(                                                                  \
    AFX_VERSIONMAKERDLG_H__885D3D82_372B_45A5_8D2A_8A9FC6384725__INCLUDED_)
...
protected:
  ...
  // Implementation
protected:
  virtual void InNotRootFileProcess(vector<FilePath> &FileName, int nOldVersion,
                                    string RootName);
  ...
  virtual void InRootFileProcess(vector<FilePath> &FileName,
                                 string &ThisVersionFileFolderName) const;
  HICON m_hIcon;
  CEdit m_DesPath;
  CEdit m_SrcPath;
  int m_nCurVersion;
  ...
  bool GetUpdateFileNameOnThisVersion(const char *szOldFileList,
                                      vector<string> &UpdateFileName);
  bool ReadFileList(KFileInfoVector &FileInfoVector,
...
private:
  bool CheckFileNameInFileS(const string &FileSample,
                            vector<string> &FileGroup) const;
  virtual void GetUpdateRelativeFile(vector<string> &FileNameS) const;
  void SetFileAttributeOfAFolder(const char *FolderName,
  ...
  void RunAutoBat(const string &DataPath, const string &ToolsPath,
                  const string &InfoPath, const string &IndexPath);
  string GetInfoFolderName() const;
  string GetToolsFolderName();
  void CreateVersionConfigFile();
  fstream FileComment;
  void PickOutFileNameBetweenRootAndNotRoot(vector<string> &FileNameArray,
                                            vector<FilePath> &FileNameInRoot,
                                            vector<FilePath> &FileNameNotInRoot,
