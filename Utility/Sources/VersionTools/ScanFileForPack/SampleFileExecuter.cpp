...
SampleFileExecuter::SampleFileExecuter() : m_FilePathSet() {}

SampleFileExecuter::~SampleFileExecuter() {}

int SampleFileExecuter::GetFilesCount() const { return m_FilePathSet.size(); }
...
void SampleFileExecuter::InsertFilePath(const string &FilePath) {
  try {
    ...
    string ErrorString =
        string("Error when SampleExecute Insert.system info: ") + Error.what();
    throw exception(ErrorString.c_str());
  }
}

void SampleFileExecuter::CleanResult() { m_FilePathSet.clear(); }

vector<string> SampleFileExecuter::GetResult() { return m_FilePathSet; }
