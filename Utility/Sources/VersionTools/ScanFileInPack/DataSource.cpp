...
BlockSource *DataSource::FindBlock(BlockSource &SourceBlockSource) {
  set<BlockSource>::iterator Position;
  Position = find(m_BlockSet.begin(), m_BlockSet.end(), SourceBlockSource);

  if (Position == m_BlockSet.end()) {
    return NULL;
  } else {
    return &(*Position);
  }
