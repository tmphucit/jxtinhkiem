...
void ReplaceAFileToOtherFile(const char *SourceFileName, char *DestFileName) {
  ...
  if (!DeleteFile(DestFileName)) {
    string ErrorInfo =
        string("DeleteFile(") + string(DestFileName) + string(")");
    DisplayErrorInfo(ErrorInfo);
    throw exception("error when delete old File");
  }

  if (!MoveFile(SourceFileName, DestFileName)) {
    string ErrorInfo = string("MoveFile(") + string(SourceFileName) +
                       string(",") + string(DestFileName) + string(")");
    DisplayErrorInfo(ErrorInfo);
...
string GetCurrentTempFileName() {
  ...
  if (!GetTempFileName(DirectoryName, "CQ", 0, TempFileName)) {
    string ErrorInfo =
        "GetTempFileName('" + string(DirectoryName) + "','CQ',0,TempFileName)";
    DisplayErrorInfo(ErrorInfo);
...
void DisplaySource(DataSource &File1Source) {
  ...
  for (int i = 0; i < File1Source.GetBlockCount(); i++) {
    cout << "ID :" << hex << setw(Width) << File1Source[i].GetID()
         << "    Length:" << dec << File1Source[i].GetLength()
         << "     CompressMethod:" << (int)File1Source[i].GetCompressMethod();
...
void find_file_in_package(Package &container, vector<string> &FileList,
                          DataSource &Result, CodingMethod &Coder,
                          ostream &out) {
  ...
  for (Pointer = FileList.begin(); Pointer != FileList.end(); Pointer++) {
    ...
    if (fResult == NULL) {
      ...
      cout << "找到文件: " << FileName.c_str() << hex
           << "  ID: " << fResult->GetID() << "     文件长度:" << dec
           << fResult->GetUnCompressBlockLength()
           << "   文件压缩长度:" << fResult->GetLength() << endl;
      out << "找到文件: " << FileName.c_str() << hex
          << "  ID: " << fResult->GetID() << "     文件长度:" << dec
          << fResult->GetUnCompressBlockLength()
          << "   文件压缩长度:" << fResult->GetLength() << endl;
...
void ConstructionFileList(vector<string> &FileList, int Count,
                          char *FileName[]) {
  cout << "文件名采集:" << endl;
  for (int j = 0; j < Count; j++) {
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    cout << "工程运行耗时:" << g_hour << "小时" << g_minute << "分钟"
         << g_Second << "秒" << endl;
