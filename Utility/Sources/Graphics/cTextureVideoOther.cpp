...
cTextureVideoOther::~cTextureVideoOther() { FinalCleanup(); }
...
long cTextureVideoOther::Upload(MediaEnum type) {
  ...
  case MEDIA_VIDEOOTHER: {
    ...
    HRESULT hr = D3DXCreateTextureFromFileEx(
        GetGraphics()->GetDevice3D(), str, D3DX_DEFAULT, D3DX_DEFAULT,
        D3DX_DEFAULT, m_stResA.stDevice.usage, m_stResA.stDevice.format,
        m_stResA.stDevice.pool, D3DX_FILTER_NONE, D3DX_FILTER_NONE, 0, &m_info,
        NULL, &m_pTextureVideo);
    // DX_ERROR(hr);
    if (FAILED(hr)) {
      ...
      HRESULT hr = D3DXCreateTextureFromFileEx(
          GetGraphics()->GetDevice3D(), str, D3DX_DEFAULT, D3DX_DEFAULT,
          D3DX_DEFAULT, m_stResA.stDevice.usage, m_stResA.stDevice.format,
          m_stResA.stDevice.pool, D3DX_FILTER_NONE, D3DX_FILTER_NONE, 0,
          &m_info, NULL, &m_pTextureVideo);
      DX_ERROR(hr);
    }
    return GetSize(type);
  }
...
void cTextureVideoOther::GetTextureSize(int frame, ULONG &width,
                                        ULONG &height) {
  ASSERT(frame == 0);
