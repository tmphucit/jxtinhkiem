...
HRESULT cPlaneHor::setParam(stObjParam &pm) {
  ...
  if (param.IsEMask(MASK_GROUNDOBJ) &&
      param.IsSetMask(stGroundObjParam::MASK_GROUNDOBJ_MOVING)) {
...
BOOL cPlaneHor::FillVertexInfoHor(int iResource)
{
        ...
        float fAngleXY = m_param.fAngleXY * c_fPI / 180.f;	//	底边在z
= 0的角度 float fAngleZ = m_param.fAngleZ  * c_fPI / 180.f;	//	斜边与z
= 0的角度 float fAngleRenderZ = c_fPI / 6; //	a渲染时平面与Z轴角度

        float sina = sin(fAngleRenderZ);
        float tgb1 = tan(fAngleXY);
        float tgr1 = fabs(sina * tgb1);					// for
example: a = 30, b = 45, then tgr = 0.5; float tgb2 = tan(c_fPI/2 - fAngleXY);
        float tgr2 = fabs(sina * tgb2);					// for
example: a = 30, b = 45, then tgr = 0.5;
        ...
        {
                fWidth = (width + fRightExtra - x) / cos(fAngleXY) /
c_ptScale.x; fHeight = (x - fLeftExtra) / cos(c_fPI / 2 - fAngleXY) /
c_ptScale.x; fZ = fHeight * tan(fAngleZ);
                ...
                for (int i=0; i<4; i++)
                {
                        v[i].tu = p[i].x / (width+fRightExtra-fLeftExtra) *
rc.Width() + rc.l; v[i].tv = p[i].y / height * rc.Height() + rc.t;
                }
        }
        else
        {
                ...
                fHeight = (width + fRightExtra - x) / cos(c_fPI / 2 - fAngleXY)
/ c_ptScale.x; fZ = fHeight * tan(fAngleZ);
                ...
                p[0].y = p[2].y - ((width+fRightExtra - x) * tgr2 + fZ *
c_ptScale.z); p[1].x = p[3].x + (width+fRightExtra - x); p[1].y = p[3].y -
((width+fRightExtra - x) * tgr2 + fZ * c_ptScale.z);
...
HRESULT cPlaneHor::DrawTrace() {
  ...
  if (GetMapedit() & Mapedit_Trace) {
    char sMsg[128];
    sprintf(sMsg, "(F6)Plane layer(%d) Height(%.1f)", m_param.nLayer,
            GetHeight());
