...
public:
  ...
  virtual LightingEnum GetLightingType() {
    return GetPlaneVerParam()->eLighting;
  };
  float GetLightedRadius() { return GetPlaneVerParam()->fLightedRadius; };
  float GetAngleXY() { return GetPlaneVerParam()->fAngleXY; }
  // 为了加云改了这一句，看来不行!
  //	SortEnum			GetSortType(){return
  // GetPlaneVerParam()->eSort;}
  ...
  float m_fK, m_fB;
  void GetLineFunction(float &k, float &b) {
    k = m_fK;
    b = m_fB;
  };
  void SetLineFunction(float k, float b) {
    m_fK = k;
    m_fB = b;
  }
  ...
  // lighting and mesh
  //	virtual bool		Lighting(cLight* p,const cPoint& p1,const
  // cPoint& p2,bool bLightit = true);	//传入的p1,p2是切分的点，
  //	//不判断受光
  //	virtual bool		Lighted(cLight* p, cPoint& p1, cPoint& p2);
  ////传出的p1,p2是挡光的seam
