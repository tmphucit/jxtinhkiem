...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CD3DApplication::Create(HINSTANCE hInstance, HWND hWnd, ULONG sx,
                                ULONG sy, bool bFullScreen) {
  HRESULT hr;
  ...
  if (m_hWnd == NULL) {
    ...
    m_hWnd = CreateWindow(
        _T("D3D Window"), m_strWindowTitle, m_dwWindowStyle, CW_USEDEFAULT,
        CW_USEDEFAULT, (rc.right - rc.left), (rc.bottom - rc.top), 0L,
        LoadMenu(hInstance, MAKEINTRESOURCE(IDR_MENU)), hInstance, 0L);
  }
  ...
  for (UINT iAdapter = 0; iAdapter < m_pD3D->GetAdapterCount(); iAdapter++) {
    ...
    for (UINT iDevice = 0; iDevice < dwNumDeviceTypes; iDevice++) {
      ...
      for (DWORD f = 0; f < dwNumFormats; f++) {
        ...
        if (pDevice->d3dCaps.DevCaps & D3DDEVCAPS_HWTRANSFORMANDLIGHT) {
          if (pDevice->d3dCaps.DevCaps & D3DDEVCAPS_PUREDEVICE) {
            ...
            if (SUCCEEDED(ConfirmDevice(&pDevice->d3dCaps, dwBehavior[f],
              ...
              //							MessageBox(m_hWnd,"use
              // hw pure device",NULL,MB_OK);
            }
          }
          ...
          if (FALSE == bFormatConfirmed[f]) {
            ...
            if (SUCCEEDED(ConfirmDevice(&pDevice->d3dCaps, dwBehavior[f],
              ...
              bFormatConfirmed[f] = TRUE;
              //							MessageBox(m_hWnd,"use
              // hw hardware_vp device",NULL,MB_OK);
            }
          }
        ...
        if (FALSE == bFormatConfirmed[f] || 1) {
          ...
          if (SUCCEEDED(ConfirmDevice(&pDevice->d3dCaps, dwBehavior[f],
            ...
            bFormatConfirmed[f] = TRUE;
            //						MessageBox(m_hWnd,"use
            // hw soft_vp device",NULL,MB_OK);
          }
        }

        // Find a suitable depth/stencil buffer format for this device/format
        if (bFormatConfirmed[f] && m_bUseDepthBuffer) {
          if (!FindDepthStencilFormat(iAdapter, pDevice->DeviceType, formats[f],
                                      &fmtDepthStencil[f])) {
            bFormatConfirmed[f] = FALSE;
          }
        }
      ...
      for (m = 0; m < pDevice->dwNumModes; m++) {
        if (pDevice->modes[m].Width == m_sx &&
            pDevice->modes[m].Height == m_sy) {
...
BOOL CD3DApplication::FindDepthStencilFormat(UINT iAdapter,
  ...
  if (m_dwMinDepthBits <= 16 && m_dwMinStencilBits == 0) {
    if (SUCCEEDED(m_pD3D->CheckDeviceFormat(iAdapter, DeviceType, TargetFormat,
      ...
      if (SUCCEEDED(m_pD3D->CheckDepthStencilMatch(
              iAdapter, DeviceType, TargetFormat, TargetFormat, D3DFMT_D16))) {
  ...
  if (m_dwMinDepthBits <= 24 && m_dwMinStencilBits <= 4) {
    if (SUCCEEDED(m_pD3D->CheckDeviceFormat(
            iAdapter, DeviceType, TargetFormat, D3DUSAGE_DEPTHSTENCIL,
  ...
  if (m_dwMinDepthBits <= 32 && m_dwMinStencilBits == 0) {
    if (SUCCEEDED(m_pD3D->CheckDeviceFormat(iAdapter, DeviceType, TargetFormat,
      ...
      if (SUCCEEDED(m_pD3D->CheckDepthStencilMatch(
              iAdapter, DeviceType, TargetFormat, TargetFormat, D3DFMT_D32))) {
...
HRESULT CD3DApplication::Initialize3DEnvironment() {
  ...
  D3DDeviceInfo *pDeviceInfo =
      &pAdapterInfo->devices[pAdapterInfo->dwCurrentDevice];
  ...
  // Create the device
  hr = m_pD3D->CreateDevice(m_dwAdapter, pDeviceInfo->DeviceType, m_hWndFocus,
...
HRESULT CD3DApplication::ForceWindowed() {
  ...
  D3DDeviceInfo *pDeviceInfoCur =
      &pAdapterInfoCur->devices[pAdapterInfoCur->dwCurrentDevice];
...
INT_PTR CALLBACK CD3DApplication::SelectDeviceProc(HWND hDlg, UINT msg,
  ...
  if (WM_COMMAND == msg) {
    ...
    if (IDOK == LOWORD(wParam)) {
      ...
      if (LOWORD(wParam) == IDC_ADAPTER_COMBO) {
        ...
        NewMultiSampleType = (D3DMULTISAMPLE_TYPE)ComboBox_GetItemData(
            hwndMultiSampleList, dwItem);
      }
    }
    // Keep the UI current
    bUpdateDlgControls = TRUE;
  ...
  if (bUpdateDlgControls) {
    ...
    for (DWORD a = 0; a < pd3dApp->m_dwNumAdapters; a++) {
      ...
      DWORD dwItem = ComboBox_AddString(
          hwndAdapterList,
    ...
    for (DWORD d = 0; d < pAdapter->dwNumDevices; d++) {
      // Add device name to the combo box
      DWORD dwItem =
          ComboBox_AddString(hwndDeviceList, pAdapter->devices[d].strDesc);
    ...
    for (DWORD m = 0; m < pDevice->dwNumModes; m++) {
      ...
      _stprintf(strMode, _T("%ld x %ld x %ld"), pDevice->modes[m].Width,
                pDevice->modes[m].Height, BitDepth);
    ...
    for (m = 0; m <= 16; m++) {
      ...
      if (SUCCEEDED(pd3dApp->m_pD3D->CheckDeviceMultiSampleType(
              dwNewAdapter, pDevice->DeviceType, fmt, bNewWindowed,
    ...
    EnableWindow(hwndMultiSampleList,
                 ComboBox_GetCount(hwndMultiSampleList) > 1);
    ...
    if (fTime - fLastTime > 1.0f) {
      ...
      if (m_bUseDepthBuffer) {
        ...
        D3DModeInfo *pModeInfo =
            &pDeviceInfo->modes[pDeviceInfo->dwCurrentMode];
