...
HRESULT CD3DFrame::RestoreDeviceObjects(LPDIRECT3DDEVICE8 pd3dDevice) {
  if (m_pMesh)
    m_pMesh->RestoreDeviceObjects(pd3dDevice);
  if (m_pChild)
    m_pChild->RestoreDeviceObjects(pd3dDevice);
  if (m_pNext)
    m_pNext->RestoreDeviceObjects(pd3dDevice);
  return S_OK;
}
...
HRESULT CD3DFrame::InvalidateDeviceObjects() {
  ...
  if (m_pChild)
    m_pChild->InvalidateDeviceObjects();
  if (m_pNext)
    m_pNext->InvalidateDeviceObjects();
  return S_OK;
}
...
HRESULT CD3DFrame::Render(LPDIRECT3DDEVICE8 pd3dDevice, BOOL bDrawOpaqueSubsets,
  ...
  // For pure devices, specify the world transform. If the world transform is
  // not specified on pure devices, this function will fail.
...
HRESULT CD3DFile::LoadFrame(LPDIRECT3DDEVICE8 pd3dDevice,
  ...
  if (*pGUID == TID_D3DRMFrame) {
    ...
    while (SUCCEEDED(pFileData->GetNextObject(&pChildObj))) {
      ...
      hr =
          pChildObj->QueryInterface(IID_IDirectXFileData, (VOID **)&pChildData);
      if (SUCCEEDED(hr)) {
...
// Desc:
//-----------------------------------------------------------------------------
HRESULT CD3DFile::CreateFromResource(LPDIRECT3DDEVICE8 pd3dDevice,
