...
template <class Container> HRESULT T_SaveList(Container &ilist, LPCSTR file) {
  int i = 0;
  int n = ilist.size();
  ...
  /*
          ...
          strBinaryFile = strBinaryFile.Left(strBinaryFile.GetLength() - 3) +
     "bin"; CFile f; if (n == 0) AppDeleteFolder(strBinaryFile); else if
     (f.Open(strBinaryFile,CFile::modeCreate | CFile::modeWrite))
          {
                  f.Write(&n,sizeof(n));
                  for (Container::iterator it = ilist.begin(); it!= ilist.end();
     ++it)
                  {
...
template <class _II, class _OI> inline void insert(_II _F, _II _L, _OI &_X) {
  for (; _F != _L; ++_F)
    _X.push_back(*_F);
}

template <class _II, class _OI, class _Pr>
inline void insert_if(_II _F, _II _L, _OI &_X, _Pr _P) {
  for (; _F != _L; ++_F)
    if (_P(*_F))
      _X.push_back(*_F);
}

template <class T> BOOL T_AddToList(list<T *> &ilist, T *p) {
...
template <class T> BOOL T_RemoveFromList(list<T *> &ilist, T *p) {
  if (!p->IsInGround() || p->IsChild())
    return FALSE;
  int size = ilist.size();
  ilist.remove(p);
...
template <class T> BOOL T_RemovePlaneFromList(list<T *> &ilist, T *p) {
  int size = ilist.size();
  ilist.remove(p);
  if (size == ilist.size() + 1)
    return TRUE;
...
template <class T> BOOL T_AddPlaneToList(list<T *> &ilist, T *p) {
  // hor已经在场景中了
  if (find(ilist.begin(), ilist.end(), p) != ilist.end())
    return FALSE;
  ilist.push_back(p);
...
template <class T> BOOL T_AddPlaneToListByLayer(list<T *> &ilist, T *p) {
  if (find(ilist.begin(), ilist.end(), p) != ilist.end())
    return FALSE;
