...
// com
/**************************************************************************/
ULONG cGround::AddRef() { return ++m_ulCount; }
...
// Graphics
//////////////////////////////////////////////////////////////////////
cScreen *cGround::GetcScreen() { return GetGraphics()->GetcScreen(); }

cMapSmall *cGround::GetcMapSmall() { return GetcScreen()->GetcMapSmall(); }

cMap *cGround::GetcMap() { return GetcScreen()->GetcMap(); }

iObj *cGround::GetiSelect() { return m_pSelect; }

cCamera *cGround::GetcCamera() { return GetGraphics()->GetcCamera(); }
...
HRESULT cGround::AskSave() {
  ...
  if (IsModify()) {
    if (AfxMessageBox("当前的文件已经改变，存储么？", MB_YESNO) == IDYES) {
      ...
      for (itcRegion it = m_listRegion.begin(); it != m_listRegion.end();
           ++it) {
        cRegion *p = (*it);
        p->Save(strFolder);
      }
...
HRESULT cGround::SaveStatic(CString &strFold) {
  for_all(m_listRegion,
          bind2nd(mem_fun1<HRESULT, cRegion, LPCSTR>(&cRegion::Save),
                  (LPCSTR)strFold));
  return S_OK;
}
...
BOOL cGround::FindDynamicNick(LPCSTR szNick) {
  for (itcGroundObj it = m_listDynamicObj.begin(); it != m_listDynamicObj.end();
       ++it) {
    cGroundObj *p = (*it);
...
HRESULT cGround::ClearDynamic() {
  ...
  for_all(m_listDynamicObj,
          bind2nd(mem_fun1<BOOL, cGroundObj, BOOL>(&cGroundObj::SetInGround),
                  FALSE));
  for_all(m_listDynamicObj,
          mem_fun<HRESULT, cGroundObj>(&cGroundObj::EndPainting));
...
void cGround::ComputeRegionPosBySize(int &x, int &y, int w, int h,
                                     const cRect &r) {
...
cPoint
cGround::GetCenterTileByRegion() // get center according to m_xRegion,m_yRegion
{
  cRect rcRegion;
...
HRESULT cGround::ClearFarestRegion(cPoint ptFocusTile) {
  ...
  for (itcRegion it = m_listRegion.begin(); it != m_listRegion.end(); ++it) {
    ...
    int x, y;
    pRegion->GetPos(x, y);
    float dis = fabs(x + 0.5f - ptFocusTile.x) + fabs(y + 0.5f - ptFocusTile.y);
    if (dis > maxdis) {
      maxdis = dis;
      itGet = it;
      bGet = true;
...
HRESULT cGround::UpdateRegion() {
  ...
  if (!IsRegionCoverScreen()) {
    OffsetRegion();
    GetcMap()->OnRegionUpdate(m_xRegion + m_wRegion / 2,
                              m_yRegion + m_hRegion / 2);
    // 统一来更新3*3中所有的图素
  }
...
int cGround::GetRegionNum() { return m_listRegion.size(); }
...
HRESULT cGround::RebuildTree() {
  ...
  for (itALine it = m_aLine.begin(); it != m_aLine.end(); it++) {
    ...
    for (itcPlaneVer it = line.listVer.begin(); it != line.listVer.end();
         ++it) {
      m_pTree->insert((*it));
    }
  }
  ...
  {
    for (itcPlaneVer it = m_listPlaneVerSeam.begin();
         it != m_listPlaneVerSeam.end(); ++it)
      m_pTree->insert((*it));
  }
  {
    for (itcPlaneVer it = m_listPlaneVerPoint.begin();
         it != m_listPlaneVerPoint.end(); ++it)
      m_pTree->insert((*it));
  }
  {
    for (itcGroundObj it = m_listDynamicObj.begin();
         it != m_listDynamicObj.end(); ++it) {
...
void cGround::AddLight(cLight *p) { m_listLight.push_back(p); }
...
void cGround::RemoveLight(cLight *p) { m_listLight.remove(p); }
...
BOOL cGround::GetLinkOffsetVer(cPoint &ptOffset, cPlaneVer *p, int eLink,
                               cPoint ptPlane, float &fMinDis) {
  BOOL b = FALSE;
  for (itcPlaneVer it = m_listPlaneVer.begin(); it != m_listPlaneVer.end();
       ++it) {
    cPlaneVer *pPlane = (*it);
    if (pPlane == p)
      continue;
...
BOOL cGround::GetLinkOffsetTile(cPoint &ptOffset, cPoint ptPlane,
                                float &fMinDis) {
...
BOOL cGround::GetLinkOffset(cPoint &ptOffset, cPlane *p, int eLink,
                            cPoint ptPlane, float &fMinDis) {
  if (eLink == LINK_TILE) {
    ...
    if (dynamic_cast<cPlaneVer *>(p) == NULL)
      return FALSE;
    return GetLinkOffsetVer(ptOffset, dynamic_cast<cPlaneVer *>(p), eLink,
                            ptPlane, fMinDis);
  }
  return FALSE;
}
...
ObstacleEnum cGround::CheckObstacle(cPoint pt, cPoint &vSpeed,
                                    bool bClockWise) {
  for (int x = 0; x < MAX_REGION_WIDTH; x++)
    for (int y = 0; y < MAX_REGION_HEIGHT; y++) {
      cRegion *pRegion = m_aRegion[x][y];
...
HRESULT cGround::GetiObstacle(stSelect &st, iObjList &tlist) {
  ...
  case SelectPoint_OneByScreen:
    if (rlist.empty()) {
      for (itcPlaneHor it = m_listMovingObj.begin();
           it != m_listMovingObj.end(); ++it) {
        cPlaneHor *p = (*it);
...
HRESULT cGround::GetiPlaneByRect(cRect rcGround, iObjList &tlist) { return -1; }

HRESULT cGround::GetiObstacleByRect(cRect rcGround, iObjList &tlist) {
  return -1;
}

HRESULT cGround::GetiLightByRect(cRect rcGround, iObjList &tlist) { return -1; }
...
void cGround::CheckDisplay() {
  ...
  for_all(m_listLight,
          bind2nd(mem_fun1<bool, cLight, const cRect *>(&cLight::CheckDisplay),
                  &rc));
  for_all(m_listPlaneVer,
          bind2nd(mem_fun1<bool, cPlane, const cRect *>(&cPlane::CheckDisplay),
                  &rc));
  for_all(m_listPlaneAboveVer,
          bind2nd(mem_fun1<bool, cPlane, const cRect *>(&cPlane::CheckDisplay),
                  &rc));
  for_all(m_listMovingObj, bind2nd(mem_fun1<bool, cPlaneHor, const cRect *>(
                                       &cPlaneHor::CheckDisplay),
                                   &rc));
  for_all(m_listDynamicObj, bind2nd(mem_fun1<bool, cGroundObj, const cRect *>(
                                        &cGroundObj::CheckDisplay),
                                    &rc));
}
...
void cGround::BeginLighting() {
  ...
  for_all(m_listLight,
          bind2nd(mem_fun1<bool, cLight, cColor>(&cLight::BeginLighting), cr));
  for_all(m_listPlaneVer,
          bind2nd(mem_fun1<bool, cPlane, cColor>(&cPlane::BeginLighting), cr));
  for_all(m_listPlaneAboveVer,
          bind2nd(mem_fun1<bool, cPlane, cColor>(&cPlane::BeginLighting), cr));
  for_all(m_listMovingObj,
          bind2nd(mem_fun1<bool, cPlaneHor, cColor>(&cPlaneHor::BeginLighting),
                  cr));
  for_all(m_listDynamicObj, bind2nd(mem_fun1<bool, cGroundObj, cColor>(
                                        &cGroundObj::BeginLighting),
                                    cr));
...
#pragma message(                                                               \
    ATTENTION("get light map of the ver and add ver to lights's obstacle"))
  ...
  for (itcGroundObj it2 = m_listDynamicObj.begin();
       it2 != m_listDynamicObj.end(); ++it2) {
...
void cGround::EndLighting() {
  ...
  for_all(m_listDynamicObj,
          mem_fun<bool, cGroundObj>(&cGroundObj::EndLighting));
...
void cGround::Paint() {
  ...
  next2:
    for (int x = 0; x < m_wRegion; x++)
      for (int y = 0; y < m_hRegion; y++) {
        ...
        //			if (p->m_pTexture == NULL ||
        //! p->m_bUpdateTexture) 				return ;
      }
  } else {
  next:
    for (int x = 0; x < m_wRegion; x++)
  ...
  if (GetGraphics()->IsTrace(iGraphics::TRACE_FPS)) {
    CString s;
    s.Format("FPS: %02d, CKD: %02d, BL: %02d, GLM: %02d, EL: %02d, PP: %02d, "
             "Flip: %02d.\n",
             GetGraphics()->GetFps(), t1 / f, t2 / f, t3 / f, t4 / f, t5 / f,
             t6 / f);
    GetGraphics()->Trace(s);
  }
  //*/
...
HRESULT cGround::CameraNotify(cCamera *p, eNotifyType eType, long lParam) {
  if (eType == NOTIFY_POSITION_CHANGED) {
    UpdateRegion();
    ClearFarestRegion();
    for (itSpecial it = m_listSpecial.begin(); it != m_listSpecial.end();
         ++it) {
      iSpecial *p = (*it);
...
HRESULT cGround::FinalCleanup() {
  ...
  for_all(
      m_listMovingObj,
      bind2nd(mem_fun1<BOOL, cPlaneHor, BOOL>(&cPlaneHor::SetInGround), FALSE));
...
HRESULT cGround::InitDeviceObjects() {
  ...
  for_all(m_listMovingObj,
          mem_fun<HRESULT, cPlaneHor>(&cPlaneHor::InitDeviceObjects));
  for_all(m_listSpecial,
          mem_fun<HRESULT, iSpecial>(&iSpecial::InitDeviceObjects));
  return S_OK;
};

HRESULT cGround::RestoreDeviceObjects() {
  for_all(m_listRegion,
          mem_fun<HRESULT, cRegion>(&cRegion::RestoreDeviceObjects));
  for_all(m_listMovingObj,
          mem_fun<HRESULT, cPlaneHor>(&cPlaneHor::RestoreDeviceObjects));
  for_all(m_listSpecial,
          mem_fun<HRESULT, iSpecial>(&iSpecial::RestoreDeviceObjects));
  return S_OK;
};

HRESULT cGround::InvalidateDeviceObjects() {
  for_all(m_listRegion,
          mem_fun<HRESULT, cRegion>(&cRegion::InvalidateDeviceObjects));
  for_all(m_listMovingObj,
          mem_fun<HRESULT, cPlaneHor>(&cPlaneHor::InvalidateDeviceObjects));
  for_all(m_listSpecial,
          mem_fun<HRESULT, iSpecial>(&iSpecial::InvalidateDeviceObjects));
  return S_OK;
};

HRESULT cGround::DeleteDeviceObjects() {
  for_all(m_listRegion,
          mem_fun<HRESULT, cRegion>(&cRegion::DeleteDeviceObjects));
  for_all(m_listMovingObj,
          mem_fun<HRESULT, cPlaneHor>(&cPlaneHor::DeleteDeviceObjects));
  for_all(m_listSpecial,
          mem_fun<HRESULT, iSpecial>(&iSpecial::DeleteDeviceObjects));
  return S_OK;
};
...
// 坐标变换
////////////////////////////////////////////////////////////////////////////
float cGround::GetGroundHeight(cPoint ptGround) { return 0; }
...
void cGround::SaveSpecial(LPCSTR szFile) { T_SaveList(m_listSpecial, szFile); }
...
BOOL cGround::FindWalkPath(std::list<cPoint> &ptlist, cPoint ptStart,
                           cPoint ptEnd) {
