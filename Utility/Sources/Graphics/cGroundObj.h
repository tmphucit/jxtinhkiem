...
public:
  ...
  virtual HRESULT LoadDynamic(LPCSTR szEvent) {
    return S_OK;
  }; // call it after call load!
  // param
  void SetGroupID(long idGroup) { GetGroundObjParam()->lGroupID = idGroup; }
  long GetGroupID() { return GetGroundObjParam()->lGroupID; }
  stGroundObjParam *GetGroundObjParam() {
    return (stGroundObjParam *)GetObjParam();
  };
  cPoint GetOffset() { return GetGroundObjParam()->ptOffset; };
  virtual void SetOffset(const cPoint &pt) {
