...
cMapSmall::~cMapSmall() { RELEASE(m_pImage); }
...
BOOL cMapSmall::VerifyTexture(BOOL bForceUpdate) {
  ...
  m_rcImage.right =
      m_rcImage.left + (rcGroundNow.Width()) * 32 / c_nWidthRegion;
  m_rcImage.bottom =
      m_rcImage.top + (rcGroundNow.Height()) * 32 / c_nHeightRegion;
  if (!bUpdate) {
...
HRESULT cMapSmall::Draw() {
  ...
  if (m_pImage != NULL) {
    ...
    // draw button
    rc = cRect(m_param.ptOffset.x, m_param.ptOffset.y, m_param.ptOffset.x + 8,
    ...
    rc = cRect(ptCenter.x - dx, ptCenter.y - dy, ptCenter.x + dx,
               ptCenter.y + dy);
    GetScreen()->DrawRect(rc, cColor(0, 255, 0));
  }
...
void cMapSmall::ScreenToGround(CPoint ptScreen, cPoint &ptGround) {
  ...
  cPoint ptOffsetGround = cPoint(ptOffsetScreen.x * c_nWidthRegion / 32,
                                 ptOffsetScreen.y * c_nHeightRegion / 32);
...
HRESULT cMapSmall::LoadBmp() {
  ...
  if (!m_pTexture) {
    ...
    HRESULT hr = GetGraphics()->GetDevice3D()->CreateTexture(
        c_nTextureWidth, c_nTextureHeight, 1, 0, stRes.stDevice.format,
...
HRESULT cMapSmall::OnCameraUpdate() { return S_OK; }
