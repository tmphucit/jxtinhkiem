...
////////////////////////////////////////////////////////////////////////////
// graphics
cGround *cGroundObj::GetGround() { return GetGraphics()->GetcGround(); }

cCamera *cGroundObj::GetCamera() { return GetGraphics()->GetcCamera(); }
...
void cGroundObj::Offset(cPoint pt) { SetOffset(pt); }
...
HRESULT cGroundObj::LoadPack(void **pParam) {
  ...
  //	if (GetOffset().x < c_nWidthRegion*2 && GetOffset().y <
  // c_nHeightRegion*2) 		m_bOffsetOfRegion = TRUE;
  // 但是对于有高度的物体，这个条件并不足够，y值相差可能很远
  if (fabs(GetOffset().x) < c_nWidthRegion * 2 ||
      fabs(GetOffset().y) < c_nHeightRegion * 2)
