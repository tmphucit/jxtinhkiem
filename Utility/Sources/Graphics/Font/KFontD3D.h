...
// FileDescription	:	字体类
// Revision Count	:
2002-9-4改写，改为基于d3d，去除文本控制标记的判断。wooy
*******************************************************************************/
#pragma once
#include "KCharSet.h"
#include "iFont.h"
...
public:
  ...
  // 在D3d Device由lost state恢复为operational state后KFont作d3d
  // Device相关内容的恢复操作。
...
private:
  ...
  LPDIRECT3DTEXTURE8 m_pTexture;       // 字体贴图句柄
  LPDIRECT3DDEVICE8 m_pd3dDevice;      // Direct 3d device接口的实例的指针
  unsigned long m_uOrigStateBlock;     // 文字绘制前D3d device的状态纪录
  unsigned long m_uDrawTextStateBlock; // 文字绘制时D3d device的状态纪录
