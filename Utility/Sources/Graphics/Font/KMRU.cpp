...
// Argumant		: unsigned short *pPositions
指向用于存储各字符在贴图中的位置的缓冲区
// Argumant		: unsigned short *pNewItems
指向用于存储新进入表中的项的缓冲区
// Argumant		: int& nNumNewItem
新新进入表中的项的数目
*****************************************************************************/
void KMRU::Commit(unsigned short *pString, int nCount,
                  unsigned short *pPositions, unsigned short *pNewItems,
                  int &nNumNewItem) {
  ...
  for (i = 0; i < nCount; i++) {
    // 取得字符的编码
    nCode = *pString++;
    if (!GetCharPos(nCode, 0, m_nItemCount, nDueSpace)) { // 字符没有在表中
      if (m_nItemCount < m_nMaxItemNum) // 看表中是否还有空的项
        nInsertSpace = m_nItemCount++;
      else
        nInsertSpace = GetMostLongestUnusedItem();
      m_pItemTable[nInsertSpace].Id = nCode;
      m_pItemTable[nInsertSpace].MRURecord = 0xff;
      // 调整新加入项在表中的位置
      if (nInsertSpace != nDueSpace)
        AdjustCharPos(nInsertSpace, nDueSpace);

      pNewItems[nNumNewItem * 2] = m_pItemTable[nDueSpace].Data;
      pNewItems[nNumNewItem * 2 + 1] = nCode;
...
void KMRU::AdjustCharPos(unsigned int nIndex, unsigned int nDueIndex) {
  ...
  if (nDueIndex < nIndex) { // 应该把项往前移
    for (i = nIndex; i > nDueIndex; i--)
      m_pItemTable[i] = m_pItemTable[i - 1];
    m_pItemTable[nDueIndex] = Temp;
  }
  if (nDueIndex > nIndex) { // 应该把项往后移
    for (i = nIndex; i < nDueIndex; i++)
      m_pItemTable[i] = m_pItemTable[i + 1];
    m_pItemTable[nDueIndex] = Temp;
  }
...
*****************************************************************************/
bool KMRU::GetCharPos(unsigned short Id, int nFrom, int nCount,
