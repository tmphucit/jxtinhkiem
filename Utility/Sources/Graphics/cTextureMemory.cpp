...
cTextureMemory::~cTextureMemory() { FinalCleanup(); }

//////////////////////////////////////////////////////////////////////
// COM
//////////////////////////////////////////////////////////////////////

ULONG cTextureMemory::AddRef() { return ++m_ulCount; }
...
HRESULT cTextureMemory::InitDeviceObjects() { return S_OK; };
...
HRESULT cTextureMemory::RestoreDeviceObjects() {
  ...
  /*
          ...
          HRESULT hr =
     D3DXSaveTextureToFile("d:\\temp.bmp",D3DXIFF_BMP,m_pTextureVideo,NULL);
          ASSERT(!FAILED(hr));
  */
  return S_OK;
};
...
HRESULT cTextureMemory::Create(const stResourceA &st, int width, int height) {
  ...
  HRESULT hr = GetGraphics()->GetDevice3D()->CreateTexture(
      width, height, 1, 0, m_stResA.stDevice.format, D3DPOOL_SYSTEMMEM,
      &(m_pTextureMemory));
...
HRESULT cTextureMemory::CreateTextureVideo() {
  ...
  HRESULT hr = GetGraphics()->GetDevice3D()->CreateTexture(
      desc.Width, desc.Height, 1, 0, desc.Format, D3DPOOL_DEFAULT,
      &(m_pTextureVideo));
  if (FAILED(hr)) {
    GetGraphics()->DXError(hr);
    return hr;
  ...
  ASSERT(!FAILED(hr));
  hr = GetGraphics()->GetDevice3D()->UpdateTexture(m_pTextureMemory,
                                                   m_pTextureVideo);
  ASSERT(!FAILED(hr));

  /*
          hr = D3DXSaveTextureToFile("d:\\texture.bmp", D3DXIFF_BMP
     ,m_pTextureMemory,NULL); ASSERT(!FAILED(hr));
  */
  return S_OK;
}
...
void cTextureMemory::ClearRect(RECT &rc) {
  ...
  char *buffer =
      (char *)lr.pBits + rc.top * lr.Pitch + rc.left * GetTextureBytes();
  for (int y = 0; y < height; y++) {
    ZeroMemory(buffer, width * GetTextureBytes());
    buffer += lr.Pitch;
  }
