...
void cCmdGroundFillRect::GetGroundBackFile() {
  ...
  pGround->Load(m_param.strDocument);
  HRESULT hr =
      pGround->BackFile(m_param.rect.left, m_param.rect.top, m_strBackGround);
  if (FAILED(hr))
...
void cCmdGroundFillRect::GetRegionBackFile() {
  stCommand_FillGround &m_param = GetParam();
  HRESULT hr = GetGround()->BackFile(m_param.rect.left, m_param.rect.top,
                                     m_strBackRegion);
  if (FAILED(hr))
...
void cCmdGroundFillRect::SetGroundBackFile() {
  ...
  iAutoGround *pGround = GetParam().pAutoGround;
  pGround->Load(m_param.strDocument);
  HRESULT hr = pGround->RestoreFile(m_param.rect.left, m_param.rect.top,
                                    m_strBackGround);
}

void cCmdGroundFillRect::SetRegionBackFile() {
  stCommand_FillGround &m_param = GetParam();
  HRESULT hr = GetGround()->RestoreFile(m_param.rect.left, m_param.rect.top,
                                        m_strBackRegion);
}
