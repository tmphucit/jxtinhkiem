...
HRESULT cCmdGroundChangeTile::TileToResource(const stAutoTile &autoTile,
                                             stTile &tile) {
...
eDoType cCmdGroundChangeTile::Do() {
  ...
  BOOL b;
  b = pGround->ChangeTile(m_param.xAutoGround, m_param.yAutoGround, sName,
                          m_param.bIgnore);
  if (!b)
    return DO_NOTHING;

  for (int x = m_param.xAutoGround - MAX_AUTOTILE / 2;
       x <= m_param.xAutoGround + MAX_AUTOTILE / 2; x++)
    for (int y = m_param.yAutoGround - MAX_AUTOTILE / 2;
         y <= m_param.yAutoGround + MAX_AUTOTILE / 2; y++) {
...
eDoType cCmdGroundChangeTile::Redo() {
  if (m_listAutoTileRedo.size() == 0 && m_listGroundTileRedo.size() == 0)
    return DO_NOTHING;
  for_all(m_listAutoTileRedo,
          mem_fun_ref<HRESULT, stChangeAutoTile>(&stChangeAutoTile::Do));
  for_all(m_listGroundTileRedo,
          mem_fun_ref<HRESULT, stChangeGroundTile>(&stChangeGroundTile::Do));
  return DO_CANUNDO;
}
...
eDoType cCmdGroundChangeTile::Undo() {
  if (m_listAutoTileUndo.size() == 0 && m_listGroundTileUndo.size() == 0)
    return DO_NOTHING;
  for_all(m_listAutoTileUndo,
          mem_fun_ref<HRESULT, stChangeAutoTile>(&stChangeAutoTile::Do));
  for_all(m_listGroundTileUndo,
          mem_fun_ref<HRESULT, stChangeGroundTile>(&stChangeGroundTile::Do));
  return DO_CANUNDO;
}
