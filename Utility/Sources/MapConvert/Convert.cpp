...
BOOL CMapConvertDlg::ConvertAObstacle(stObstacle &obs, int xRegion,
                                      int yRegion) {
  cPointList ptlist;
  if (obs.nPoint == 1) // 点
  {
...
void CMapConvertDlg::ModifyObstacle(int xRegion, int yRegion, int offx,
                                    int offy, ObstacleEnum e) {
  ASSERT(e > 0 && e < OBSTACLE_NUM);
  if (xRegion < 0 || xRegion >= MAX_REGION || yRegion < 0 ||
      yRegion >= MAX_REGION)
    return;
      ...
      for (i = 0; i < c_nWidthRegion; i++)
        for (j = 0; j < c_nHeightRegion; j++) {
          ...
          if (i > 0)
            ...
            left = GetObstacleNotGenerate(
                m_aRegion[x - 1][y]->aTile[c_nWidthRegion - 1][j]);
          ...
          if (j > 0)
            ...
            top = GetObstacleNotGenerate(
                m_aRegion[x][y - 1]->aTile[i][c_nHeightRegion - 1]);
...
void CMapConvertDlg::ConvertObstacleFile(int xRegion, int yRegion,
...
void CMapConvertDlg::SaveTrap() {
...

  {
    ...
    strScriptFile =
        m_strFile.Left(m_strFile.GetLength() - 4) + "\\" + BINFILE_TRAP_SCRIPT;
    CStdioFile f;
...
void CMapConvertDlg::ModifyTrap(int xRegion, int yRegion, int offx, int offy,
                                long e) {
  if (xRegion < 0 || xRegion >= MAX_REGION || yRegion < 0 ||
      yRegion >= MAX_REGION)
    return;
...
BOOL GetIniFile(const CString &m_strDocument, const CString &strEvent, long lID,
...
void CMapConvertDlg::ConvertTrapFile(int sx, int sy, const CString &szFile) {
  ...
  for (int i = 0; i < num; i++) {
    ...
    // if it is valid!
    if (param.ptOffset.x < c_nWidthRegion &&
        param.ptOffset.y < c_nHeightRegion) {
...
int on_convertnpc(LPCSTR szNpc) {
  ...
  CString strBox =
      strNpc.Left(strNpc.GetLength() - strName.GetLength()) + FILE_BOX;
  CString strTrap =
      strNpc.Left(strNpc.GetLength() - strName.GetLength()) + FILE_TRAP;
  CString strNpcTemp =
      strNpc.Left(strNpc.GetLength() - strName.GetLength()) + "temp.ini";
