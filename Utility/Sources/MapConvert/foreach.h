...
public:
  ...
  doonce m_func;
  class foreach (doonce f) : m_func(f) {
    m_strFindFolder = "*.", m_strFindFile = "*.*";
  };
  ...
  virtual bool is_foldervalid(LPCSTR szFolder) { return true; }
...
public:
  class foreachregion(doonce f) : foreach (f) {
    m_strFindFolder = "*.", m_strFindFile = "*.wor";
  };
  CString m_strFileInRegion;
  void SetFileInRegion(LPCSTR szFileInRegion) {
    m_strFileInRegion = szFileInRegion;
  }
