...
//  Comment     :   The commmon interface of get Kingsoft antivirus files
//  version
//
//////////////////////////////////////////////////////////////////////////////////////
...
static int inline _GetVersionStringFromInt(int nMajorVersion, int nMinorVersion,
...
int GetFileVersion(const char cszFileName[], DWORD *pdwProductVersionMS,
  ...
  *pdwProductVersionLS = 0;
  ...
  nRetCode = GetFileVersionInfo((char *)cszFileName, dwHandle, dwFileInfoSize,
                                pszFileInfo);
  if (!nRetCode)
    goto Exit0;
...
int GetProgramVersion(char szFileName[], char szVersion[], int nVersionLen) {
  ...
  nRetCode = GetFileVersion(szFileName, (DWORD *)&uMajorVersion,
                            (DWORD *)&uMinorVersion);
  if (nRetCode)
    nRetCode = _GetVersionStringFromInt(uMajorVersion, uMinorVersion, szVersion,
                                        g_cnVersionLen);
  else
...
int GetEngineFileVersion(char szFileName[], char szVersion[], int nVersionLen) {
  ...
  nRetCode = _GetVersionStringFromInt(dwMajorVersion, dwMinorVersion, szVersion,
                                      g_cnVersionLen);
...
int GetAllVersion() {
  ...
  nRetCode =
      GetSignFileVersion(szSignFileName, g_szSignDataVersion, g_cnVersionLen);

  // Get KAEngine.Dat version
  strcpy(szFileName, g_szProgramPath);
  strcat(szFileName, "KAEngine.DAT");
  nRetCode =
      GetEngineFileVersion(szFileName, g_szEngineVersion, g_cnVersionLen);

  strcpy(szFileName, g_szProgramPath);
  strcat(szFileName, "KAExtend.DAT");
  nRetCode =
      GetEngineFileVersion(szFileName, g_szKAExtendVersion, g_cnVersionLen);
  strcpy(szFileName, g_szProgramPath);
