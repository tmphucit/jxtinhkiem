...
void Color::RGBtoHSB(int red, int green, int blue, float *H, float *S,
                     float *B) {
   ...
   * variable * to the difference of the two as a percentage of the largest */
  if (largestColor != 0)
  ...
  if (saturation == 0.0F) {
    ...
    float redRatio =
        (float)(largestColor - red) / (float)(largestColor - lowestColor);
    float greenRatio =
        (float)(largestColor - green) / (float)(largestColor - lowestColor);
    float blueRatio =
        (float)(largestColor - blue) / (float)(largestColor - lowestColor);
    ...
    /* I don't know if this prevents us from being negative (as in the worst
     * outcome from the above * operations is -1, or if this does something
     * different).                                      */
    if (hue < 0.0F)
      hue++;
  }
...
float Color::GetH() { return m_H; }

float Color::GetS() { return m_S; }

float Color::GetB() { return m_B; }

int Color::GetRed() { return m_red; }

int Color::GetGreen() { return m_green; }

int Color::GetBlue() { return m_blue; }
