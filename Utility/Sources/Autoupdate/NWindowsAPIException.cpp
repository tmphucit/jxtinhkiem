...
NWindowsAPIException::NWindowsAPIException(const char *ThrowFileName,
                                           int ThrowLineNumber,
  ...
  LPVOID lpMsgBuf;
  FormatMessageA(dwFormatFlags, hModule, ErrorType,
                 MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), // Default language
                 (LPTSTR)&lpMsgBuf, 0, NULL);
  ErrorInfo = string("系统模块") + string(ModuleName) + string("内部错误:") +
