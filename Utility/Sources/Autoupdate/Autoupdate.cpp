...
#define bindconnect(p)                                                         \
  CFtpConnection *p = GetConnect();                                            \
  if (p == NULL)                                                               \
    return false;                                                              \
...
BOOL CAutoupdateApp::DownloadFile(stFile &st) {
  ...
  try {
    ...
    if (st.length_downloaded != 0) // resume
    {
      ...
      BOOL bIE5 = FALSE;
      BOOL bRet =
          FtpCommand(*pConnect,                // WinInet Connection handle
...
BOOL CAutoupdateApp::ExtractFile(const stFile *st) {
  ...
  CString s;
  s.Format("Failed when extract file %s, Please remove the file's readonly "
           "attribute, or the file may be corrupt, Autoupdate Abort!",
           st->strFileName);
  AfxMessageBox(s);
  /*
          cZip zip;
...
void CAutoupdateApp::RunGame() { OnExecute(m_strGame); }
...
// for can not get so much large file,size always < max_ulong, so use int not
// unsigned int!
BOOL CAutoupdateApp::DiskHaveSpace(int size) {
  ...
  BOOL b = GetDiskFreeSpaceEx(szDisk, &i64FreeBytesToCaller, &i64TotalBytes,
                              &i64FreeBytes);
  if (b) {
...
typedef BOOL (*FtpCommandFunc)(HINTERNET hConnect, BOOL fExpectResponse,
                               DWORD dwFlags, LPCTSTR lpszCommand,
                               DWORD_PTR dwContext, HINTERNET *phFtpCommand);

BOOL CAutoupdateApp::FtpCommand(HINTERNET hConnect, BOOL fExpectResponse,
                                DWORD dwFlags, LPCTSTR lpszCommand,
                                DWORD_PTR dwContext, HINTERNET *phFtpCommand,
                                BOOL *bIE5) {
  *bIE5 = TRUE;
  return ::FtpCommand(hConnect, fExpectResponse, dwFlags, lpszCommand,
  ...
  /*
      ...
      if (hinstLib != NULL)
      {
          ...
          if (fRunTimeLinkSuccess = (ProcAdd != NULL))
                  {
                          bRet =
     (ProcAdd)(hConnect,fExpectResponse,dwFlags,lpszCommand,dwContext,phFtpCommand);
                          *bIE5 = TRUE;
                  }
...
BOOL CAutoupdateApp::PauseResumeSupported() {
  ...
  //	BOOL bRet = pConnect->Command("REST
  // 0",CmdRespNone,FTP_TRANSFER_TYPE_ASCII);

  BOOL bIE5 = FALSE;
  HINTERNET hResponse;
  BOOL bRet = FtpCommand(hConnect, FALSE, FTP_TRANSFER_TYPE_ASCII, "REST 0",
                         0, // No context needed
                         &hResponse,
                         &bIE5); // The handle to read the response
  if (!bIE5)
