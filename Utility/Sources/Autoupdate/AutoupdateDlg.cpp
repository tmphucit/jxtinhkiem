...
BOOL CAutoupdateDlg::OnInitDialog() {
  ...
  sVersonInfo.Format(
      "From Client Verson: %d to Server Verson: %d support resume (%c)",
      theApp.m_nLocalVerson, theApp.m_nServerVerson, cResume);
...
void CAutoupdateDlg::Refresh() {
  ...
  case STATUS_DOWNLOADING:
    ...
    m_strProgress.Format(
        "all %d files downloaded %d file all %dK size downloaded %dK size",
        theApp.GetDownloadNum(), theApp.GetDownloadedNum(),
        theApp.GetDownloadSize() / 1024, theApp.GetDownloadedSize() / 1024);
    m_strProgressCurrent.Format("current file is %s size %dK downloaded %dK",
                                theApp.GetDownloadingFile(),
                                theApp.GetDownloadingSize() / 1024,
                                theApp.GetDownloadingDownloadedSize() / 1024);
    UpdateData(FALSE);

    m_progress.SetPos(
        GetPercent(theApp.GetDownloadedSize(), theApp.GetDownloadSize()));
    m_progressCurrent.SetPos(GetPercent(theApp.GetDownloadingDownloadedSize(),
                                        theApp.GetDownloadingSize()));
  ...
  case STATUS_EXTRACTING:
    ...
    m_strProgress.Format(
        "all %d files extracted %d file all %dK size extracted %dK size",
        theApp.GetExtractNum(), theApp.GetExtractedNum(),
        theApp.GetExtractSize() / 1024, theApp.GetExtractedSize() / 1024);
    m_strProgressCurrent.Format("current file is %s size %dK extracted %dK",
                                theApp.GetExtractingFile(),
                                theApp.GetExtractingSize() / 1024,
                                theApp.GetExtractingExtractedSize() / 1024);
    UpdateData(FALSE);

    m_progress.SetPos(
        GetPercent(theApp.GetExtractedSize(), theApp.GetExtractSize()));
    m_progressCurrent.SetPos(GetPercent(theApp.GetExtractingExtractedSize(),
                                        theApp.GetExtractingSize()));
  ...
  if (theApp.m_bFinished > 0) {
    ...
    m_strProgress.Format(
        "all %d files extracted %d file all %dK size extracted %dK size",
        theApp.GetExtractNum(), theApp.GetExtractNum(),
        theApp.GetExtractSize() / 1024, theApp.GetExtractSize() / 1024);
    m_strProgressCurrent.Empty();
    UpdateData(FALSE);
...
void CAutoupdateDlg::OnOK() { CDialog::OnOK(); }
