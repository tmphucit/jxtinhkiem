...
BOOL cStatusTerrainSquare::Create() {
  ...
  for (int i = 0; i < TYPE_NUM; i++) {
    ...
    m_aGround[i] = AutoMap_CreateAutoGround(m_aSize[i], sTerrain[i],
                                            sConnection[i], "spr");
...
void cStatusTerrainSquare::ChangeSelectTile(CPoint point, BOOL bFill,
                                            BOOL bIgnore) {
...
void cStatusTerrainSquare::ChangeTile(LPCSTR strSpr, CPoint point, BOOL bFill,
                                      BOOL bIgnore) {
  ...
  if (bFill) // || m_bAutoFillGround &&
             // GetAutoGround()->IsAllNull(ptTile.x,ptTile.y))
  {
...
BOOL cStatusTerrainSquare::OnLButtonDown(UINT nFlags, CPoint point) {
  ...
  if (!m_rcFillGround.IsRectNull()) {
    ...
    int x = pt.x / c_tWidthRegion;
    int y = pt.y / c_tHeightRegion;
    FillTile(CRect(max(x - m_rcFillGround.left, 0),
                   max(y - m_rcFillGround.top, 0), x + m_rcFillGround.right + 1,
                   y + m_rcFillGround.bottom + 1));
  } else {
...
BOOL cStatusTerrainSquare::OnLButtonDblClk(UINT nFlags, CPoint point){return
FALSE;} BOOL cStatusTerrainSquare::OnRButtonDown(UINT nFlags, CPoint
point){return FALSE;} BOOL cStatusTerrainSquare::OnRButtonUp(UINT nFlags, CPoint
point){return FALSE;} BOOL cStatusTerrainSquare::OnRButtonDblClk(UINT nFlags,
CPoint point){return FALSE;} BOOL cStatusTerrainSquare::OnTimer(UINT
nIDEvent){return FALSE;} BOOL cStatusTerrainSquare::OnSetCursor(CWnd* pWnd, UINT
nHitTest, UINT message){return FALSE;}
*/
