...
BOOL cStatusDynamic::ShowNpcSetting(iObj *pPlane,
                                    const CString &strTemplateFile,
                                    const CString &strSection,
                                    const CString &strObjFile) {
  char FilePath[200];
  CRect rc(0, 0, 300, 400);

  if (m_pSettingDlg)
    m_pSettingDlg->GetWindowRect(&rc);
  ...
  if (!m_pSettingDlg) {
    ...
    if (SysTabFile.Load(FilePath)) {
      ...
      m_pSettingDlg->m_ActiveIniForm.AddCtrl(
          "NpcResType", "StringList", TypeList.GetBuffer(TypeList.GetLength()),
          "", "");
    }
    ...
    if (TabFile.Load(FilePath)) {
      ...
      m_pSettingDlg->m_ActiveIniForm.AddCtrl(
          "动作", "IndexList", ActionList.GetBuffer(ActionList.GetLength()), "",
          "");
    ...
    m_pSettingDlg->m_ActiveIniForm.AddCtrl("RideHorse", "IndexList",
                                           "不骑马|骑马", "", "");
  ...
  // m_pSettingDlg->m_ActiveIniForm.Load((LPCSTR)strTemplateFile, 1,
  // (LPCSTR)strSection, TRUE);
  m_pSettingDlg->m_ActiveIniForm.Load("\\Settings\\Npcs.txt", 1,
                                      (LPCSTR)strSection, TRUE);
  ...
  m_pSettingDlg->SetWindowPos(&CWnd::wndTopMost, rc.left, rc.top, rc.Width(),
                              rc.Height(), 0);
  m_pSettingDlg->ShowWindow(SW_SHOW);
  return TRUE;
}

BOOL cStatusDynamic::ShowBoxSetting(iObj *pPlane,
                                    const CString &strTemplateFile,
                                    const CString &strSection,
                                    const CString &strObjFile) {
  ...
  m_pSettingDlg->m_ActiveIniForm.Load("\\Settings\\Obj\\ObjData.txt", 1,
                                      (LPCSTR)strSection, TRUE);
  m_pSettingDlg->m_TemplateFile = strTemplateFile;
...
BOOL cStatusDynamic::ShowTrapSetting(iObj *pPlane,
                                     const CString &strTemplateFile,
                                     const CString &strSection,
                                     const CString &strObjFile) {
  char FilePath[200];
  ...
  m_pSettingDlg->m_ActiveIniForm.ModifyData("Script", szScriptFile,
                                            szScriptFile);
  char szTrapId[20];
...
BOOL cStatusDynamic::ChangeTrapScript(iObj *pPlane) {
  ...
  if (filedlg.DoModal() == IDOK) {
    ...
    PathRelativePathTo(PathName, (LPCSTR)sWork, FILE_ATTRIBUTE_DIRECTORY,
...
BOOL cStatusDynamic::SetObjTemplate(iObj *pPlane, const CString &strTemplate,
                                    const CString &strSection) {
  CString strIni;
...
BOOL cStatusDynamic::GetObjTemplate(iObj *pPlane, CString &strTemplate,
                                    CString &strSection) {
  CString strIni;
  if (GetObjIniFile(pPlane, strIni)) {
...
/*
BOOL cStatusDynamic::GetObjAnimation(iObj* pPlane,CString& strAnimation, BOOL&
bAutoplay)
{
        CString strIni;
        bAutoplay = FALSE;
...
BOOL cStatusDynamic::GetObjScriptFile(iObj *p, CString &sScript) {
  ...
  if (!bFindScript) {
    ...
    if (TabFile.Load((LPSTR)(LPCSTR)strTemplateFile)) {
      ...
      case DYNAMIC_NPC: {
        ...
        if (TabFile.GetString((LPSTR)(LPCSTR)strSection, "ActionScript", "",
                              ActionScript, 100)) {
          sScript = ActionScript;
          bFindScript = TRUE;
        }
      ...
      case DYNAMIC_BOX: {
        if (TabFile.GetString((LPSTR)(LPCSTR)strSection, "ScriptName", "",
                              ActionScript, 100)) {
          sScript = ActionScript;
          bFindScript = TRUE;
        }
      ...
      case DYNAMIC_TRAP: {
        if (TabFile.GetString((LPSTR)(LPCSTR)strSection, "Script", "",
                              ActionScript, 100)) {
          sScript = ActionScript;
          bFindScript = TRUE;
        }
  ...
  if (sScript == "") {
    if (eType == DYNAMIC_TRAP) {
      ...
      return FALSE;
    } else if (AfxMessageBox("当前未设置脚本文件，是否需要自动为其生成脚本？",
                             MB_OKCANCEL) == IDOK) {
...
BOOL cStatusDynamic::AfterCreatePlane(iObj *pPlane,
                                      const CString &m_strSelectFile,
                                      const CString &m_strCopyDataFile) {
  if (pPlane == NULL)
    return FALSE;
  ...
  wrap_ptr<iItem> ptr(pItem);
  if (GetItemValue(pItem, "template", strTem) &&
      GetItemValue(pItem, "section", strSection)) {
