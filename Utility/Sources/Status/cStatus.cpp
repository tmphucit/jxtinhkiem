...
void cStatus::Clear() { m_strSelectFile.Empty(); }

/**************************************************************************/
// com interface
/**************************************************************************/
ULONG cStatus::AddRef() { return ++m_ulCount; }
...
BOOL cStatus::Create() { return TRUE; }

BOOL cStatus::Create(iGraphics *p, CWnd *pWnd,
...
bool cStatus::IsAltDown() { return IsKeyDown(VK_LMENU) || IsKeyDown(VK_RMENU); }
...
iGraphics *cStatus::GetGraphics() { return m_pGraphics; }

iGround *cStatus::GetGround() { return GetGraphics()->GetiGround(); }

iScreen *cStatus::GetScreen() { return GetGraphics()->GetiScreen(); }
...
iCamera *cStatus::GetCamera() { return GetGraphics()->GetiCamera(); }

iObj *cStatus::GetSelect() { return GetGround()->GetiSelect(); }

iObj *cStatus::GetMap() { return GetScreen()->GetiMap(); }

iObj *cStatus::GetMapSmall() { return GetScreen()->GetiMapSmall(); }
...
BOOL cStatus::OnLButtonDown(UINT nFlags, CPoint point) { return FALSE; }
...
BOOL cStatus::OnSetCursor(CWnd *pWnd, UINT nHitTest, UINT message) {
  return FALSE;
}
...
BOOL LoadBitmapFromBMPFile( LPCSTR szFileName, CBitmap& bmp)
{
        ...
        for (int j=0; j<bm.bmHeight*bm.bmWidth; j++)
        {
                ...
                BYTE g = *pByte++;
                BYTE b = *pByte++;
//		(*pWord++) = ((WORD)(r >> 3)) << 10 | ((WORD)(g >> 3)) << 5 | (b
>> 3);
                (*pWord++) = ((WORD)(r >> 3)) << 11 | ((WORD)(g >> 2)) << 5 | (b
>> 3);
        }
...
HRESULT Save16BMPFileTo24(const char *filename, CBitmap &bmp) {
  ...
  WORD *pSrc16 =
      (WORD *)(pExbuffer + (bi.biWidth * (bm.bmHeight - 1) * (16 / 8)));
  char *pDes24 = buffer;
  for (int i = 0; i < bm.bmHeight; i++) {
    for (int j = 0; j < bm.bmWidth; j++) {
      WORD w = (*pSrc16++);
  ...
  for (int i = rect.left; i <= rect.right; i++) {
    for (int j = rect.top; j <= rect.bottom; j++) {
      ...
      if (f.FindFile(sFile)) {
        ...
        dcMem1.StretchBlt((i - rect.left) * g_nMapSize,
                          (j - rect.top) * g_nMapSize, g_nMapSize, g_nMapSize,
                          &dcMem2, 0, 0, 512, 512, SRCCOPY);
      }
    }
  }
  // 合并所有的bmp
...
BOOL cStatus::OnMouseMove(UINT nFlags, CPoint point) { return TRUE; }
...
BOOL cStatus::CanRedo() { return GetCommandMachine()->CanRedo(); }

BOOL cStatus::CanUndo() { return GetCommandMachine()->CanUndo(); }
...
eMaskSetting cStatus::FindFileAndGetValue(LPCSTR szFile, LPCSTR szPath,
                                          stSetting &set) {
...
// app message
//////////////////////////////////////////////////////////////////////
void cStatus::BeginMacroCommand() { GetCommandMachine()->BeginMacroCommand(); }

void cStatus::EndMacroCommand() { GetCommandMachine()->EndMacroCommand(); }
...
void cStatus::ClearSelect() { GU_SetSelectType(GetSelect(), SELECT_NULL); }
...
BOOL cStatus::OnRightSelectedCommand(LPCTSTR szFile) { return FALSE; }
...
void cStatus::OnSelectedSetting(LPCTSTR szFile) { return; }
...
BOOL cStatus::OnLeftSelectedCommand(LPCTSTR szFile) { return FALSE; }
...
eMaskSetting cStatus::WantNameToID(const CString &s) {
  static CString s_strWantName[] = {
      "rect", "point", "bool",   "string", "color",
      "int",  "float", "cpoint", "clear",  "",
  };
...
BOOL cStatus::GetIniFile(const CString &strEvent, long lID,
                         const CString &sDynamic, CString &sini) {
  if (m_strDocument.IsEmpty())
    return FALSE;
