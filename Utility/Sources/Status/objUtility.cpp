...
void SetObjMoving(iObj *pObj, bool bMoving) {
  ...
  param.ClearMask();
  pObj->GetParam(param);
  if (param.eElement == GT_PLANEHORNORMAL ||
      param.eElement == GT_PLANEHORMESH) {
    stPlaneParam param;
    param.ClearMask();
    pObj->GetParam(param);
...
void CreateObstacle(iGraphics *pGraphics, iObj *&pObstacle,
  ...
  param.SetMask(MASK_OBSTACLE, stObstacleParam::MASK_OBSTACLE_TYPE |
                                   stObstacleParam::MASK_OBSTACLE_POINTINFO);
  pObstacle->setParam(param);
}

void CreatePlane(iGraphics *pGraphics, iObj *&pPlane, LPCSTR szFile,
                 int nFrame) {
  ElementEnum gtype;
  ...
  VERIFY(
      GetItemValue(pItem, GetTablePlaneName(TP_PLANETYPE), strValue, nFrame));
