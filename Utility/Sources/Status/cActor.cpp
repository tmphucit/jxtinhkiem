...
// com
/**************************************************************************/
ULONG cActor::AddRef() { return ++m_ulCount; }
...
void cActor::JumpTo(cPoint pt) {
  ...
  if (ptOffsetGround != pt) {
    ...
    float f =
        m_fSpeed / sqrt(m_vSpeed.x * m_vSpeed.x + m_vSpeed.y * m_vSpeed.y +
...
void cActor::ChangeAnimation(eAnimation eAni) { m_eAni = eAni; }
...
BOOL cActor::IsInGround() { return m_pObject->IsInGraphics(); }
...
void cActor::FormatSpeed(cPoint &m_vSpeed) {
  ...
  if (DistanceSquare(ptOffsetDec, m_ptGoal) <
      DistanceSquare(ptOffset, m_ptGoal)) {
...
BOOL cActor::AddSpeed(cPoint m_vSpeed) {
  float f = m_fSpeed / sqrt(m_vSpeed.x * m_vSpeed.x + m_vSpeed.y * m_vSpeed.y +
                            m_vSpeed.z * m_vSpeed.z);
  m_vSpeed.x *= f;
  m_vSpeed.y *= f;
  m_vSpeed.z *= f;
  ...
  if (IsSeamObtacle() &&
      !GetGraphics()->GetiGround()->CheckObstacle(
          ptOffset, m_vBlockSpeed, m_bClockWise) == OBSTACLE_NULL) {
    return FALSE;
  }
...
BOOL cActor::Run() {
  ...
  case STATUS_BLOCK: {
    ...
    if (++m_nBlock > 10) {
      ...
      ptOffset += m_vSpeed;
      cPoint ptSpeed;
      if (GetGraphics()->GetiGround()->CheckObstacle(
              ptOffset, ptSpeed, m_bClockWise) == OBSTACLE_NULL) {
        TurnToSpeed(m_vSpeed);
        m_eStatus = STATUS_WALK;
      }
    } else {
  ...
  case STATUS_WALK: {
    ...
    if (Distance(ptOffset, m_ptGoal) > 0.1f) {
      ptOffset += m_vSpeed;
      if (!GetGraphics()->GetiGround()->BlockObstacle(ptOffset,
                                                      OBSTACLE_BLOCKWALK)) {
        SetPosition(ptOffset);
        IncreaseFrame();
      } else {
...
void cActor::End() { m_eStatus = STATUS_NULL; }
...
void cActor::GetCurrentAnimation(CString &str) { str = m_strAni; }
...
void cActor::SetLightRange(float r) { GU_SetLightRange(m_pLight, r); }

float cActor::GetLightRadius() { return 0; }

void cActor::SetLightColor(cColor cr) { GU_ChangeObjColor(m_pLight, cr); }

cColor cActor::GetLightColor() { return cColor(0, 0, 0, 0); }

void cActor::SetCameraFollow(bool bFollow) { m_bMainActor = true; }
