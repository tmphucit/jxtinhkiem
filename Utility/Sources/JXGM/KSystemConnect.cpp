...
void STDMETHODCALLTYPE KSystemConnect::SetOption(bool bOnlinePlayer,
                                                 bool bRegisterUser,
                                                 DWORD nAutoTime) {
...
void KSystemConnect::P_PlayerInfo(void *pThis, const void *pData,
  ...
  if (pNPL && dataLength == sizeof(GM_GET_ROLE_DATA_SYNC)) {
    size_t nOffset = sizeof(pNPL->wLength) + sizeof(pNPL->AccountName) +
                     sizeof(pNPL->wGetID) + 1;
    p->m_pEvent->PlayerInfo(pNPL->AccountName, pNPL->wGetID,
                            ((char *)pData) + nOffset, dataLength - nOffset);
  }
}

void KSystemConnect::P_PlayerMessage(void *pThis, const void *pData,
                                     size_t dataLength) {}
