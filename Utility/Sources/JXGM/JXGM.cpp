...
BOOL APIENTRY DllMain(HANDLE hModule, DWORD ul_reason_for_call,
                      LPVOID lpReserved) {
  g_pFactroyFun = (pfnCreateClientInterface)g_theRainbowDll.GetProcAddress(
      "CreateInterface");
...
IClient *CreateClient(const size_t &maxFreeBuffers, const size_t &bufferSize) {
  ...
  if (SUCCEEDED(g_pFactroyFun(IID_IClientFactory,
                              reinterpret_cast<void **>(&pClientFactory)))) {
    pClientFactory->SetEnvironment(maxFreeBuffers, bufferSize);
