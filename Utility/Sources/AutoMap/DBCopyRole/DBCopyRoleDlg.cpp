...
void CDBCopyRoleDlg::OnBtnSearchrole() {
  ...
  size_t size;
  bool aSearchResult =
      DBLoad.SearchData(m_txtRoleName.GetBuffer(1), aBuffer, size);
  DBLoad.Close();
  if (!aSearchResult) {
    m_txtStatus = "找不到该角色！";
    UpdateData(false);
  ...
  int i;
  {
    ...
    TDBSkillData *pSkillData =
        (TDBSkillData *)(aBuffer + pRoleData->dwFSkillOffset);
