...
ULONG cAutoGround::AddRef() { return ++m_ulCount; }
...
BOOL cAutoGround::Save(LPCSTR strFile) {
  ...
  for (int i = 0; i < num; i++)
    for (int j = 0; j < num; j++) {
      ...
      GetFilePath(s, tx, ty);
      CFile file;
      VERIFY(AppOpenFile(file, s, CFile::modeCreate | CFile::modeWrite,
                         "autoground"));
      //		if (!AppOpenFile(file,FileName,CFile::modeRead |
      // CFile::shareDenyWrite,"image")) 		eGroundKind eKind =
      // m_tile.eKind[i][j]; 		file.Write(&eKind,sizeof(eGroundKind));
      file.Write(&tx, sizeof(tx));
      file.Write(&ty, sizeof(ty));
      int tiles = MAX_AUTOTILE / num * MAX_AUTOTILE / num;
      file.Write(&tiles, sizeof(tiles));
      for (int x = i * MAX_AUTOTILE / num; x < (i + 1) * MAX_AUTOTILE / num;
           x++)
        for (int y = j * MAX_AUTOTILE / num; y < (j + 1) * MAX_AUTOTILE / num;
             y++) {
...
HRESULT cAutoGround::SetConnectionFolder(LPCSTR szConnectionFolder,
                                         LPCSTR szExt) {
...
// kind
void cAutoGround::AddGroundKind(CString sKind) { m_aKind.push_back(sKind); }
...
int cAutoGround::GetGroundKindNum() { return m_aKind.size(); }
...
void cAutoGround::ResetTile(int x, int y, stAutoTile &tile) {
  ...
  file.Seek(sizeof(stHeader) + sizeof(tile) * (nx * GetRegionSize() + ny),
            CFile::begin);
...
bool cAutoGround::IsAllNull(int x, int y) {
  ...
  for (int tx = 0; tx < GetRegionSize(); tx++)
    for (int ty = 0; ty < GetRegionSize(); ty++) {
      if (!m_tile.aTile[tx + i * GetRegionSize()][ty + j * GetRegionSize()]
               .IsNull())
        return false;
    }
  return true;
...
void cAutoGround::SetAllGround(int x, int y, eGroundKind eKind) {
  ...
  VERIFY(
      AppOpenFile(file, s, CFile::modeCreate | CFile::modeWrite, "autoground"));
...
void DumpTile(cAutoTile *pLeave) {
  CString strKind[] = {
      "GROUND_CAOYUAN",  "GROUND_HUANGTU",  "GROUND_HEITU",
      "GROUND_KUCAO",    "GROUND_SATU",     "GROUND_SAXI",
      "GROUND_SAMO",     "GROUND_BIBO",     "GROUND_BOXUE",
      "GROUND_BINGMIAN", "GROUND_HONGTU",   "GROUND_HUANGSHUI",
      "GROUND_LUOYE",    "GROUND_HONGXI",   "GROUND_HUANGXI",
      "GROUND_CHANGCAO", "GROUND_QINGSHUI", "GROUND_DACAOYUAN",
      "GROUND_DABIBO",
  };
  CString strStyle[] = {
  ...
  afxDump << "pos(" << pLeave->m_tile.x << "," << pLeave->m_tile.y << ")"
          << " e0: " << strKind[pLeave->m_stTile.e0]
          << " e1: " << strKind[pLeave->m_stTile.e1]
          << " style: " << strStyle[pLeave->m_stTile.eStyle]
          << " chooses: " << pLeave->m_tileList.GetCount()
          << " choose: " << pLeave->m_nChoose << "\n";
#endif
}
...
bool cAutoGround::ChangeTile(int xCenter, int yCenter, float xWidth,
                             float yWidth, eGroundKind eKind) {
  int i, j;
