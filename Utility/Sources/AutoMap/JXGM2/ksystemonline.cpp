...
KSystemOnline::KSystemOnline() { m_pSystemConnect = NULL; }

KSystemOnline::~KSystemOnline() { Close(); }
...
HRESULT STDMETHODCALLTYPE KSystemOnline::OpenSystemConnect(
    DWORD nIP, short nPort, ISystemConnect **ppConnect) {
  if (ppConnect == NULL)
    return E_INVALIDARG;
  ...
  KSystemConnect *pSystemConnect = new KSystemConnect(nIP);
  if (pSystemConnect == NULL)
    return E_OUTOFMEMORY;

  HRESULT hr = E_FAIL;
  if (pSystemConnect->InitClient(50, 8192) &&
      pSystemConnect->ConnectTo(nIP, nPort)) {
...
HRESULT STDMETHODCALLTYPE KSystemOnline::OpenSystemConnect2(
    DWORD nIP, short nPort, ISystemConnect2 **ppConnect) {
  if (ppConnect == NULL)
    return E_INVALIDARG;
