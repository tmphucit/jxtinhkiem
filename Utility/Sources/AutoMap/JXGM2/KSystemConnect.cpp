...
bool KSystemConnect::NeedGMLogEvent(char *szAccount) {
  ...
  if (szAccount && szAccount[0] == 'g' && szAccount[0] == 'm' &&
      szAccount[0] == '-')
    return false;
...
void KSystemConnect::P_FreezeReturn(void *pThis, const void *pData,
                                    size_t dataLength) {
  KSystemConnect *p = (KSystemConnect *)(KConnect *)pThis;
  if (!p->m_pEvent)
    return;
  KAccountUserReturn *pReturn = (KAccountUserReturn *)(((BYTE *)pData) + 2);
  if (pReturn && dataLength - 2 == sizeof(KAccountUserReturn)) {
    p->m_pEvent->AccountUserReturn(pReturn->Operate, pReturn->Account,
                                   pReturn->nReturn);
  }
}

void KSystemConnect::P_UnlockReturn(void *pThis, const void *pData,
                                    size_t dataLength) {
  KSystemConnect *p = (KSystemConnect *)(KConnect *)pThis;
  if (!p->m_pEvent)
    return;
...
void KSystemConnect::P_ServerInfo(void *pThis, const void *pData,
  ...
  if (pReturn && dataLength - 2 == sizeof(KServerInfo)) {
    ...
    case server_OnlinePlayerCount:
      p->m_pEvent->OnlinePlayerCount(pReturn->Operate, pReturn->Account,
                                     pReturn->nValue);
      break;
    case server_PlayerWhere:
      // p->m_pEvent->PlayerAccountAtGW(pReturn->Account, pReturn->nValue, -1,
      // -1, -1, NULL);
      break;
...
void STDMETHODCALLTYPE KSystemConnect::GetOnlinePlayerCount(DWORD Operate,
                                                            char *szGWName) {
  KServerInfo user;
...
/*
void STDMETHODCALLTYPE KSystemConnect::GetPlayerAccountAtGW(DWORD Operate, char*
szAccount)
{
        KServerInfo user;
        user.Size = sizeof(KServerInfo);
...
void STDMETHODCALLTYPE KSystemConnect::GetPlayerAccountAtGWID(DWORD Operate,
char* szAccount)
{
        KServerInfo user;
        user.Size = sizeof(KServerInfo);
...
void STDMETHODCALLTYPE KSystemConnect::GetGWRelayID(DWORD Operate,
                                                    char *szGWName) {
  KServerInfo user;
  user.Size = sizeof(KServerInfo);
  user.Type = ServerInfo;
...
void STDMETHODCALLTYPE KSystemConnect::UnlockPlayerAccount(DWORD Operate,
                                                           char *szAccount) {
...
void STDMETHODCALLTYPE KSystemConnect::FreezePlayerAccount(DWORD Operate,
                                                           char *szAccount) {
  KAccountUser user;
...
void STDMETHODCALLTYPE KSystemConnect::ExecuteAction(char *szRole,
                                                     DWORD nRelayID,
                                                     char *szScript,
                                                     DWORD nReturn) {
  ...
  SendRelayRoleData(szRole, m_nIP, nRelayID, m_nRelyID, &exe, sizeof(exe),
                    szExe, nszLen);
}

void STDMETHODCALLTYPE KSystemConnect::ExecuteAction(DWORD nRelayID,
                                                     char *szScript,
                                                     DWORD nReturn,
                                                     DWORD nSendIP) {
  if (m_nRelyID == -1 || nRelayID == -1)
  ...
  if (nSendIP != 0) {
    if (nSendIP == INADDR_BROADCAST) {
      ...
      nszLen = sprintf(szExe, "GameExecute(\"%s\", \"dw %s\")", _ip2a(nSendIP),
                       szExe2);
    }
  } else
    strcpy(szExe, szExe2);
...
void KSystemConnect::P_MissingPlayer(void *pThis, const void *pData,
  ...
  if (pLoseWayData) {
    ...
    switch (((char *)(pData))[0]) {
    case pf_gamemaster:
      p->m_pEvent->LosePlayer(((GM_HEADER *)pData)->AccountName,
                              pLoseWayData->nFromIP);
      break;
    };
  }
}
...
void KSystemConnect::P_GWReturnMsg(void *pThis, const void *pData,
  ...
  case pf_playercommunity: {
    if (playercomm_channelchat == pEH->ProtocolID) {
      ...
      p->m_pEvent->PlayerMessage(
          pChatData->m_szAccountName, pChatData->m_szSourceName, szMessage,
          pChatData->SentenceLength, pRelayData->nFromIP);
...
DWORD KSystemConnect::GetIP() { return m_nIP; }

DWORD KSystemConnect::GetRelayID() { return m_nRelyID; }

void KSystemConnect::SetFlag(int nFlag) { m_nFlag = nFlag; }
