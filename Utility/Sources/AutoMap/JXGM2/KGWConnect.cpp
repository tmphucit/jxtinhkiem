...
void KGSConnect::P_PlayerLoginorOut(void *pThis, const void *pData,
                                    size_t dataLength) {
...
void KGSConnect::P_PlayerTrackingData(void *pThis, const void *pData,
                                      size_t dataLength) {
  KGSConnect *p = (KGSConnect *)(KConnect *)pThis;
  if (!p->m_pEvent)
    return;
  ...
  if (pNPL && dataLength == sizeof(GM_TRACK_SYNC)) {
    size_t nOffset = sizeof(pNPL->m_wLength) + sizeof(pNPL->m_szAccount) +
                     sizeof(pNPL->m_szRoleName) + 1;
    p->m_pEvent->PlayerTrackingData(pNPL->m_szAccount, pNPL->m_szRoleName,
                                    ((char *)pData) + nOffset,
                                    dataLength - nOffset);
  }
}

void KGSConnect::P_PlayerInfo(void *pThis, const void *pData,
                              size_t dataLength) {
  KGSConnect *p = (KGSConnect *)(KConnect *)pThis;
  if (!p->m_pEvent)
    return;
  ...
  if (pNPL && dataLength == sizeof(GM_GET_ROLE_DATA_SYNC)) {
    size_t nOffset = sizeof(pNPL->m_wLength) + sizeof(pNPL->m_szAccount) +
                     sizeof(pNPL->m_szRoleName) + sizeof(pNPL->m_wGetID) + 1;
    p->m_pEvent->PlayerInfo(pNPL->m_szAccount, pNPL->m_szRoleName,
                            pNPL->m_wGetID, ((char *)pData) + nOffset,
                            dataLength - nOffset);
  }
}

void KGSConnect::P_PlayerMessage(void *pThis, const void *pData,
                                 size_t dataLength) {}
...
void KGWConnect::P_PlayerLogin(void *pThis, const void *pData,
                               size_t dataLength) {}
...
HRESULT STDMETHODCALLTYPE KGWConnect::Login(char *szGMAccount, char *szPassword,
                                            IGWEvent *pEvent) {
  HRESULT hr = E_FAIL;
  m_pEvent = pEvent;
  return hr;
...
void STDMETHODCALLTYPE KGWConnect::ExecuteAction(char *szAccount,
                                                 char *szScript) {
  ...
  pSend->SendRelayData(szAccount, &exe, sizeof(GM_EXECUTE_COMMAND), szScript,
                       nszLen);
}

void STDMETHODCALLTYPE KGWConnect::EnableProtocol(char *szAccount,
  ...
  pSend->SendRelayData(szAccount, &exe, sizeof(GM_ENABLE_ACTION_COMMAND), pData,
                       dataLength);
}

void STDMETHODCALLTYPE KGWConnect::DisableProtocol(char *szAccount,
                                                   BYTE nProtocol, void *pData,
                                                   size_t dataLength) {
  KConnect *pSend = GetRelayConnect();
  if (!pSend)
  ...
  pSend->SendRelayData(szAccount, &exe, sizeof(GM_DISABLE_ACTION_COMMAND),
                       pData, dataLength);
}

void STDMETHODCALLTYPE KGWConnect::TrackingPlayer(char *szAccount,
                                                  bool bTrack) {
  KConnect *pSend = GetRelayConnect();
  if (!pSend)
    return;
...
void STDMETHODCALLTYPE KGWConnect::SetPlayerInfo(char *szAccount, WORD ninfo,
                                                 void *pData,
                                                 size_t dataLength) {
  KConnect *pSend = GetRelayConnect();
  if (!pSend)
  ...
  pSend->SendRelayData(szAccount, &exe, sizeof(GM_SET_ROLE_DATA_COMMAND), pData,
                       dataLength);
}

void STDMETHODCALLTYPE KGWConnect::SendMessage(char *szAccount, char *szRole,
                                               char *szMessage) {}
