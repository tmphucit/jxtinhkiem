...
void cFps::KeepFps(int frame) {
  ...
  if (new_time - pre_time < ms)
    WaitForSingleObject(GetCurrentProcess(),
                        ms - (new_time - pre_time)); // never singalled
  pre_time = new_time;
}

_declspec(dllexport) iFps *Common_CreateFps() { return new cFps; }
