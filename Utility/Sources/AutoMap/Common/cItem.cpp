...
void cItem::SetFlag(long lFlag) { m_lFlag |= lFlag; }
...
ULONG cItem::AddRef() { return ++m_ulCount; }
...
BOOL cItem::IsMain(LPCSTR szSection) { return stricmp(szSection, "MAIN") == 0; }
...
BOOL cItem::Save() {
  ...
  for (itSectionMap itS = m_mapSection.begin(); itS != m_mapSection.end();
       ++itS) {
    of << "[" << (*itS).first.c_str() << "]\n";
  ...
  /*
          for (itSectionArray it = m_aSection.begin(); it != m_aSection.end();
     ++it)
          {
                  if ((*it) == NULL)
                          continue;
...
BOOL cItem::SetValue(LPCSTR szItem, const CString &strValue,
                     stcSection &section) {
  m_bModify = true;
