...
// 输入参数：lana_num为网卡编号，一般地，从0开始，但在Windows
// 2000中并不一定是连续分配的
...
static int _GetCpuID(BYTE byaryCpuID[12]) {
  ...
  __try {
    __asm mov eax, 0x01 __asm cpuid __asm test edx,
        (0x01 << 18) __asm jz NoSerial __asm mov dwaryCPUID[2],
        eax __asm mov eax, 0x03 __asm cpuid __asm mov dwaryCPUID[1],
        edx __asm mov dwaryCPUID[0], ecx __asm jmp HaveSerial
...
KSystemConnect::KSystemConnect() {
  ...
  m_ProtocolTable[MAKEWORD(pf_gamemaster, gm_s2c_tracking)] =
      P_PlayerTrackingData;
...
void KSystemConnect::P_Logined(void *pThis, const void *pData,
                               size_t dataLength) {
...
void KSystemConnect::P_ServerInfo(void *pThis, const void *pData,
                                  size_t dataLength) {
  KSystemConnect *p = (KSystemConnect *)(KConnect *)pThis;
  ...
  if (pReturn && dataLength - 2 == sizeof(KServerInfo)) {
    ...
    case server_PlayerWhere:
      p->m_pEvent->PlayerAccountAtGW(pReturn->Account, pReturn->nValue, -1, -1,
                                     -1, NULL);
      break;
...
HRESULT STDMETHODCALLTYPE KSystemConnect::Login(char *szGMAccount,
                                                char *szPassword,
                                                ISystemEvent *pEvent) {
  HRESULT hr = E_FAIL;
  m_pEvent = pEvent;
...
void STDMETHODCALLTYPE KSystemConnect::Close() { Stop(); }
...
KSystemConnect::GetPlayerAccountAtGWRealTime(char *szAccount) {
...
void STDMETHODCALLTYPE KSystemConnect::UnlockPlayerAccount(char *szAccount,
                                                           char *szPassword) {
...
void KSystemConnect::P_PlayerTrackingData(void *pThis, const void *pData,
                                          size_t dataLength) {
  KSystemConnect *p = (KSystemConnect *)(KConnect *)pThis;
  if (!p->m_pEvent)
    return;
  ...
  if (pNPL && dataLength == sizeof(GM_TRACK_SYNC)) {
    ...
    p->m_pEvent->PlayerTrackingData(
        pNPL->AccountName, ((char *)pData) + nOffset, dataLength - nOffset);
  }
}

void KSystemConnect::P_PlayerInfo(void *pThis, const void *pData,
                                  size_t dataLength) {
  KSystemConnect *p = (KSystemConnect *)(KConnect *)pThis;
  if (!p->m_pEvent)
    return;
  GM_GET_ROLE_DATA_SYNC *pNPL = (GM_GET_ROLE_DATA_SYNC *)((char *)pData);
  p->m_pEvent->PlayerInfo(pNPL->AccountName, pNPL->wGetID, pNPL + 1,
                          pNPL->wLength);
}

void STDMETHODCALLTYPE KSystemConnect::ExecuteAction(char *szAccount,
                                                     char *szScript) {
  int nszLen = 0;
  if (szScript)
...
void STDMETHODCALLTYPE KSystemConnect::EnableProtocol(char *szAccount,
                                                      BYTE nProtocol,
                                                      void *pData,
                                                      size_t dataLength) {
  ...
  SendRelayData(szAccount, &exe, sizeof(GM_ENABLE_ACTION_COMMAND), pData,
                dataLength);
}

void STDMETHODCALLTYPE KSystemConnect::DisableProtocol(char *szAccount,
                                                       BYTE nProtocol,
                                                       void *pData,
                                                       size_t dataLength) {
  ...
  SendRelayData(szAccount, &exe, sizeof(GM_DISABLE_ACTION_COMMAND), pData,
                dataLength);
}

void STDMETHODCALLTYPE KSystemConnect::TrackingPlayer(char *szAccount,
                                                      bool bTrack) {
...
void STDMETHODCALLTYPE KSystemConnect::GetPlayerInfo(char *szAccount,
                                                     WORD ninfo) {
...
void STDMETHODCALLTYPE KSystemConnect::GetPlayerInfoFromDB(char *szRole,
                                                           DWORD dwRelayID,
                                                           WORD nInfo) {
  RELAY_DATA data;
...
void STDMETHODCALLTYPE KSystemConnect::SetPlayerInfoToDB(
    char *szRole, DWORD dwRelayID, WORD ninfo, void *pData, size_t dataLength) {
  RELAY_DATA *pRelayData = (RELAY_DATA *)_alloca(
      sizeof(RELAY_DATA) + sizeof(GM_SET_ROLE_DATA_COMMAND));
  ...
  GM_SET_ROLE_DATA_COMMAND *pGMData =
      (GM_SET_ROLE_DATA_COMMAND *)(pRelayData + 1);
...
void STDMETHODCALLTYPE KSystemConnect::GetRoleListFromDB(char *szAccount,
                                                         DWORD dwRelayID) {
  RELAY_DATA data;
  data.ProtocolFamily = pf_relay;
  data.ProtocolID = relay_c2c_data;
...
void STDMETHODCALLTYPE KSystemConnect::SetPlayerInfo(char *szAccount,
                                                     WORD ninfo, void *pData,
                                                     size_t dataLength) {
  ...
  SendRelayData(szAccount, &exe, sizeof(GM_SET_ROLE_DATA_COMMAND), pData,
                dataLength);
}

void STDMETHODCALLTYPE KSystemConnect::SendMessage(char *szAccount,
                                                   char *szRole,
                                                   char *szMessage) {
...
void KSystemConnect::P_MissingPlayer(void *pThis, const void *pData,
                                     size_t dataLength) {
  KSystemConnect *p = (KSystemConnect *)(KConnect *)pThis;
  if (!p->m_pEvent)
    return;
  ...
  if (pLoseWayData) {
    ...
    switch (((char *)(pData))[0]) {
    case pf_gamemaster:
      p->m_pEvent->PlayerAccountAtGW(((GM_HEADER *)pData)->AccountName, 0, -1,
                                     -1, -1, NULL);
      break;
    };
  }
...
void KSystemConnect::P_GWReturnMsg(void *pThis, const void *pData,
                                   size_t dataLength) {
  KSystemConnect *p = (KSystemConnect *)(KConnect *)pThis;
  if (!p->m_pEvent)
    return;
  ...
  case gm_s2c_findplayer: {
    ...
    p->m_pEvent->PlayerAccountAtGW(pHeader->AccountName, pRelayData->nFromIP,
                                   pGMData->nX, pGMData->nY, pGMData->nWorldID,
...
void KSystemConnect::P_AskwayProcess(void *pThis, const void *pData,
                                     size_t dataLength) {
  KSystemConnect *p = (KSystemConnect *)(KConnect *)pThis;
  if (!p->m_pEvent)
    return;
  ...
  case pf_playercommunity: {
    ...
    p->m_pEvent->PlayerMessage(pChatData->m_szAccountName,
                               pChatData->m_szSourceName,
                               (char *)(pChatData + 1),
                               pChatData->SentenceLength, pAskwayData->nFromIP);
  } break;
  case pf_gamemaster: {
    if (gm_s2c_getrolelist == pEH->ProtocolID) {
      ...
      p->m_pEvent->PlayerRoleList(pGMData->AccountName, pGMData->wRoleCount,
                                  (pGMData + 1));
      ...
      p->m_pEvent->PlayerAccountAtGW(
          pGMData->AccountName, pAskwayData->nFromIP, pGMData->nX, pGMData->nY,
          pGMData->nWorldID, pGMData->szCurrentRoleName);
    }
  } break;
  case pf_normal: {
...
void STDMETHODCALLTYPE KSystemConnect::BroadCastChat(DWORD dwRelayID,
                                                     char *szChatMsg) {
  RELAY_DATA data;
  data.ProtocolFamily = pf_relay;
  data.ProtocolID = relay_c2c_data;
  ...
  GM_BROADCAST_CHAT_CMD *msgChat =
      (GM_BROADCAST_CHAT_CMD *)_alloca(data.routeDateLength);
