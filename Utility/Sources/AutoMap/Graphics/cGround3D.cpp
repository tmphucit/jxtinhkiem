...
cGround3D::cGround3D(cGraphics *p) : cGround(p) { m_pVertexGround = NULL; }
...
HRESULT cGround3D::SetStates() {
  ...
  case 1:
    ...
    break;
  case 2:
    hr = GetDevice()->SetTextureStageState(0, D3DTSS_MINFILTER,
                                           D3DTEXF_ANISOTROPIC);
    hr = GetDevice()->SetTextureStageState(0, D3DTSS_MAGFILTER,
                                           D3DTEXF_ANISOTROPIC);
    break;
  default:
    break;
...
void cGround3D::DrawTriangle(stVertexGround *pV) { Draw(pV, 3); }

void cGround3D::DrawQuadrangle(stVertexGround *pV) { Draw(pV, 4); }
...
void cGround3D::Draw3Vertex(stVertexGround &v1, stVertexGround &v2,
                            stVertexGround &v3) {
  HRESULT hr;
  stVertexGround *m_pVB;
...
void cGround3D::Draw4Vertex(stVertexGround &v1, stVertexGround &v2,
                            stVertexGround &v3, stVertexGround &v4) {
  HRESULT hr;
  stVertexGround *m_pVB;
...
void cGround3D::DrawPointAsRect(std::list<cPoint> &ptlist, cColor cr) {
  ...
  hr = GetDevice()->DrawPrimitiveUP(D3DPT_LINELIST, nPoint / 2, aVertex,
                                    sizeof(stVertexGround));
  DX_ERROR(hr);
  ASSERT_HR(hr);
  //	delete aVertex;
}
...
void cGround3D::DrawPoint(std::list<cPoint> &ptlist, cColor cr) {
  ...
  for (std::list<cPoint>::iterator it = ptlist.begin(); it != ptlist.end();
    ...
    v.tv = 0.f;
    if (i == MAX_POINT) {
      hr = GetDevice()->DrawPrimitiveUP(D3DPT_POINTLIST, MAX_POINT,
                                        m_aVertexPoint, sizeof(stVertexGround));
      DX_ERROR(hr);
      ASSERT_HR(hr);
      left -= MAX_POINT;
      i = 0;
  ...
  if (left > 0) {
    hr = GetDevice()->DrawPrimitiveUP(D3DPT_POINTLIST, left, m_aVertexPoint,
                                      sizeof(stVertexGround));
    DX_ERROR(hr);
    ASSERT_HR(hr);
  }
...
void cGround3D::DrawPoint(cPoint *aPoint, int nPoint, cColor cr) {
  ...
  HRESULT hr;
  GetGraphics()->SetCommonTexture((cGraphics::eTexKind)COLOR_WHITE, 0);
  hr = GetDevice()->DrawPrimitiveUP(D3DPT_POINTLIST, nPoint, m_aVertexPoint,
                                    sizeof(stVertexGround));
...
void cGround3D::DrawLine(stVertexGround *pV, int n) {
  ...
  HRESULT hr;
  GetGraphics()->SetCommonTexture((cGraphics::eTexKind)COLOR_WHITE, 0);
  hr = GetDevice()->DrawPrimitiveUP(D3DPT_LINESTRIP, n - 1, pV,
                                    sizeof(stVertexGround));
  ASSERT_HR(hr);
}
...
void cGround3D::DrawPolygon(stVertexGround *pV, int num, cColor cr) {
  ...
  HRESULT hr;
  GetGraphics()->SetCommonTexture((cGraphics::eTexKind)COLOR_WHITE, 0);
  hr = GetDevice()->DrawPrimitiveUP(D3DPT_LINESTRIP, num, v,
                                    sizeof(stVertexGround));
  ASSERT_HR(hr);
}
