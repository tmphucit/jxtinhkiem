...
// Purpose		: 释放接口实例对象
*****************************************************************************/
void KFontD3D::Release() { delete this; }
...
void KFontD3D::TextOut(const char *pszText, int nCount /*= KF_ZERO_END*/,
  ...
  while (nPos < nCount) {
    ...
    if (bShowChar) {
      ...
      vCharPolys[n + 4].x = vCharPolys[n + 1].x =
          vCharPolys[n].x + m_nFontWidth;
      vCharPolys[n + 4].y = vCharPolys[n + 1].y = vCharPolys[n].y;

      vCharPolys[n + 3].x = vCharPolys[n + 2].x = vCharPolys[n].x;
      vCharPolys[n + 3].y = vCharPolys[n + 2].y =
          vCharPolys[n].y + m_nFontHeight;
...
*****************************************************************************/
void KFontD3D::RenderText(KFontVertex *pPolyVertices,
                          unsigned int nNumPolys) const {
  if (m_pd3dDevice && pPolyVertices) { // Set up renderstate
    ...
    // 绘制字符多边形
    m_pd3dDevice->DrawPrimitiveUP(D3DPT_TRIANGLELIST, nNumPolys, pPolyVertices,
                                  sizeof(KFontVertex));
