...
HRESULT cObstacle::GetParam(stObjParam &pm) { return cObj::GetParam(pm); }
  ...
  for (int i = 0; i < nPoint - 1; i++) {
    ...
    //		if (PointInRect(rect,p1) || PointInRect(rect,p1))
    {
      ...
      for (std::list<cPoint>::iterator it = ptlist.begin(); it != ptlist.end();
        ...
        p.x += 0.5f;
        p.y += 0.5f;
        if (std::find(m_listTile.begin(), m_listTile.end(), p) ==
            m_listTile.end())
...
HRESULT cObstacle::Draw() { return Draw(GetOffset()); }
...
HRESULT cObstacle::Draw(cPoint ptOffset) {
  ...
  if (GetMapedit() & Mapedit_Trace) {
    char sMsg[128];
    sprintf(sMsg, "(F7)Obstacle: pos(%.1f,%.1f,%.1f) ", GetOffset().x,
            GetOffset().y, GetOffset().z);
  ...
  if (m_param.nPoint == 1)
    ...
    //		DrawLine(ptOffset+m_param.aPoint[i],
    // ptOffset+m_param.aPoint[i+1],cr);
  }
  ...
  if (GetMapedit() & Mapedit_Select) {
    if (m_param.iPoint != -1) {
      ...
      if (m_param.nPoint != 1)
        DrawPoint(ptOffset + m_param.aPoint[m_param.iPoint],
                  cColor(0, 255, 255));
      else
        DrawPoint(ptOffset + m_param.aPoint[m_param.iPoint],
                  cColor(0, 255, 255), m_param.fRadius);
    }
  }
...
BOOL cObstacle::ContainedScreen(RECT rc) { return FALSE; }

BOOL cObstacle::ContainedGround(cRect rc) { return FALSE; }
...
ObstacleEnum cObstacle::CheckObstacle(cPoint pt, cPoint &vSpeed,
                                      bool bClockWise) {
  cPoint p1, p2;
  if (!IsBlock(pt, p1, p2))
