...
const CString c_aStrItem[] = {
    "elementtype", "image", "数目", "最低",       "最高", "雪花大小",
    "远近的缩放",  "颜色",  "位置", "覆盖的范围", "",
};

const CString *cSpecialFirefly::GetItemStringArray() { return c_aStrItem; }
...
void cSpecialFirefly::InitParam() {
  ...
  for (int i = 0; i < m_param.nNumber; i++) {
    m_aFly[i].ptPos =
        cPoint(pt.x + rand() % width / fmul, pt.y + rand() % height / fmul,
               pt.z + (rand() % z) / fmul);
    ...
    m_aFly[i].fSpeedDiv =
        sqrt(m_aFly[i].fSpeedVerMul * m_aFly[i].fSpeedVerMul + 1);
  }
...
void cSpecialFirefly::RunSnow(stFly &fly) // 单个的snow运动
{
  ...
  case STATUS_MOVE: {
    ...
    {
      cPoint vVer = cPoint(fly.ptSpeed.y * fly.fSpeedVerMul,
                           -fly.ptSpeed.x * fly.fSpeedVerMul, 0);
...
// draw
//////////////////////////////////////////////////////////////////////
cPoint cSpecialFirefly::GetDrawPoint() { return m_param.ptFocus; }
