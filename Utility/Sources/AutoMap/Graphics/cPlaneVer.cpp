...
// param
//////////////////////////////////////////////////////
bool cPlaneVer::IsSortPoint() { return GetVersortType() == VERSORT_POINT; }
...
void cPlaneVer::GetLightedPoint(cPoint &p) { p = GetOffset(); }

void cPlaneVer::GetLightingSeam(cPoint &p1, cPoint &p2) {
  GetBottomSeam(p1, p2);
}

void cPlaneVer::GetLightingPoint(cPoint &p) { p = GetOffset(); }

void cPlaneVer::GetSortSeam(cPoint &p1, cPoint &p2) { GetBottomSeam(p1, p2); }
...
float cPlaneVer::GetArea() {
  ...
  for (int j = 0; j < GetResNum(); j++) {
    ...
    float ww = (v[0].sx - v[1].sx) * (v[0].sx - v[1].sx) +
               (v[0].sy - v[1].sy) * (v[0].sy - v[1].sy);
    float w = sqrtf(ww);
    fArea += w * h;
  }
  return fArea;
...
HRESULT cPlaneVer::DrawTrace() {
  ...
  if (GetMapedit() & Mapedit_Trace) {
    char sMsg[128];
    sprintf(sMsg,
            "(F6)Plane 角度(%.1f) Height(%.1f) Versort(%d) "
            "SortOffY(%1.f),k(%.2f),b(%.2f)",
            m_param.fAngleXY, GetHeight(), GetVersortType(), GetSortOffY(),
            m_fK, m_fB);
...
BOOL cPlaneVer::FillVertexInfoVerFull(const stTexInfo &info, int iResource) {
  ...
  if (m_param.fAngleXY == 90) {
    CString strMsg;
    strMsg.Format("file: %s index %d 的在z=0的角度为90,不可能，请检查这个文件",
                  m_param.szTemplate, iResource);
