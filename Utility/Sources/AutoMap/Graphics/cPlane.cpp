...
BOOL cPlane::SetInGround(BOOL b) {
  ...
  for_all(m_listObstacle,
          bind2nd(mem_fun1<BOOL, cObstacle, BOOL>(&cObstacle::SetInGround), b));
  return b;
}
...
stVertexGround *cPlane::GetVG(int i) { return m_aVG; }
...
////////////////////////////////////////////////////////////////////
// draw 顺时针的四边形
void cPlane::GetSelectVG(stVertexGround v[4], int i) { GetQuadrangle(v, i); }
...
bool cPlane::EndLighting() { return true; }
...
HRESULT cPlane::DrawTrace() {
  ...
  if (GetMapedit() & Mapedit_Trace) {
    ...
    /*
                    sprintf(sMsg,"(F6)Plane Type(%s) Offset(%.1f,%.1f,%.1f)
       Center(%.1f,%.1f,%.1f) Region(%d,%d)", sType,
                            GetOffset().x,GetOffset().y,GetOffset().z,
                            ptCenter.x,ptCenter.y,ptCenter.z,
                            (int)ptRegion.x,(int)ptRegion.y);
    */
    sprintf(sMsg,
            "(F6)Plane Type(%s) Offset(%.1f,%.1f,%.1f) Kind(%d) Region(%d,%d)",
            sType, GetOffset().x, GetOffset().y, GetOffset().z, GetKind(),
            (int)ptRegion.x, (int)ptRegion.y);
    GetGraphics()->Trace(sMsg);
    if (strlen(m_param.szGroup) != 0) {
      sprintf(sMsg, "(F6)Plane Grp(%s)", m_param.szGroup);
      GetGraphics()->Trace(sMsg);
    }
    sprintf(sMsg, "(F6)Plane Tmp(%s(%d))", m_param.szTemplate,
            m_param.iTemplate);
    GetGraphics()->Trace(sMsg);
    sprintf(sMsg, "(F6)Plane Spr(%s(%d))", m_param.stResA.stRes.szResource,
            m_param.stResA.stRes.iFrame);
    GetGraphics()->Trace(sMsg);
  }
  return S_OK;
}
...
HRESULT cPlane::DrawNick() {
  ...
  GetGraphics()->GetDebugFont()->TextOut(s, s.GetLength(), pt.x, pt.y,
                                         0xff00ffff);
  return S_OK;
}
...
void cPlane::GetSprFile(CString &sValue) { sValue = GetResA(0).szResource; }
...
HRESULT cPlane::LoadFromTemplate(LPCSTR szFile) {
  // 检验该文件是否存在
  stTemplateOne *pTemplate = (stTemplateOne *)Common_GetPack1D(Pack1D_Plane)
                                 ->LocateToFrame(szFile, GetiFrame(0));
  if (pTemplate == NULL)
    return -1;
  ...
  if (nObstacle > 0) {
    ...
    for (int i = 0; i < nObstacle; i++) {
      ...
      param.SetMask(MASK_OBSTACLE, stObstacleParam::MASK_OBSTACLE_POINTINFO |
                                       stObstacleParam::MASK_OBSTACLE_TYPE);
...
BOOL cPlane::Contain(cPoint ptGround) { return false; }
...
cObstacle *cPlane::GetObstacle(CPoint ptScreen) {
  itcObstacle it = find_if(
      m_listObstacle.begin(), m_listObstacle.end(),
...
int cPlane::MC_GetImageInfo() {
  ...
  stFrameInfo info;
  pImage->GetFrameInfo(info, m_param.stResA.stRes.iFrame);
  m_param.hTex = info.height;
  if (m_param.ptCenter.x == -1 && m_param.ptCenter.y == -1) {
