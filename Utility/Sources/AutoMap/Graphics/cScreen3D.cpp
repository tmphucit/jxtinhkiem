...
cScreen3D::cScreen3D(cGraphics *p) : cScreen(p) { m_pVertexScreen = NULL; }
...
HRESULT cScreen3D::InitDeviceObjects() {
  ...
  hr = GetDevice()->CreateVertexBuffer(MAX_POINT * sizeof(stVertexScreen), 0,
                                       D3DFVF_SCREENVERTEX, D3DPOOL_MANAGED,
                                       &m_pVertexScreen);
  ASSERT(!FAILED(hr));
  return hr;
}
...
void cScreen3D::DrawRaw(iTexture* pTex,int frame, stVertexScreen* pV, int n)
{
        ...
        hr = GetDevice()->SetStreamSource(0, m_pVertexScreen,
sizeof(stVertexScreen)); ASSERT(!FAILED(hr)); hr = GetDevice()->SetVertexShader(
D3DFVF_SCREENVERTEX ); ASSERT(!FAILED(hr)); Draw(pTex,frame,pV,n); if (pData !=
NULL)
        {
...
void cScreen3D::DrawPoint(CPoint *aPoint, int nPoint, cColor cr) {
  ...
  hr = GetDevice()->DrawPrimitiveUP(D3DPT_POINTLIST, nPoint, m_aVertexPoint,
                                    sizeof(stVertexScreen) * nPoint);
  ASSERT_HR(hr);
}

void cScreen3D::DrawPoint(CPoint p, cColor cr) { DrawPoint(&p, 1, cr); }
...
HRESULT cScreen3D::SetVertexBuffer() {
  HRESULT hr =
      GetDevice()->SetStreamSource(0, m_pVertexScreen, sizeof(stVertexScreen));
  ASSERT(!FAILED(hr));
