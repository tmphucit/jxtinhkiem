...
// com interface
/**************************************************************************/
ULONG cCamera::AddRef() { return ++m_ulCount; }
...
cGround *cCamera::GetGround() { return GetGraphics()->GetcGround(); }
...
ULONG cCamera::UpdateCamera() {
  ...
  SetProjParams(m_param.fFOV, m_param.fAspect, m_param.fNearPlane,
                m_param.fFarPlane);
...
HRESULT cCamera::setParam(stParam &param) {
  ...
  if (param.mask & MASK_EYE || param.mask & MASK_TO || param.mask & MASK_UP ||
      param.mask & MASK_ANGLEX || param.mask & MASK_ANGLE ||
      param.mask & MASK_DIS) {
    ...
    if (param.mask & MASK_ANGLEX || param.mask & MASK_ANGLE ||
        param.mask & MASK_DIS || param.mask & MASK_TO) {
      float fx = m_param.ptTo.x + m_param.fDis *
                                      cos(m_param.fAngle * D3DX_PI / 180) *
                                      sin(m_param.fAngleX * D3DX_PI / 180);
      float fy = m_param.ptTo.y + m_param.fDis *
                                      cos(m_param.fAngle * D3DX_PI / 180) *
                                      cos(m_param.fAngleX * D3DX_PI / 180);
      float fz =
          m_param.ptTo.z + m_param.fDis * sin(m_param.fAngle * D3DX_PI / 180);
...
void cCamera::Trace() {
  char sMsg[128];
  sprintf(sMsg,
          "(F4)Camera: From(%.1f,%.1f,%.1f) To(%.1f,%.1f,%.1f) AngleXY(%.1f) "
          "AngleZ(%.1f) Scale(%.1f)",
          m_param.ptEye.x, m_param.ptEye.y, m_param.ptEye.z, m_param.ptTo.x,
          m_param.ptTo.y, m_param.ptTo.z, m_param.fAngleX, m_param.fAngle,
          m_param.fScale);
...
void cCamera::Init() {
  ...
  param.mask = iCamera::MASK_ANGLE | iCamera::MASK_DIS | iCamera::MASK_TO |
               iCamera::MASK_SCALE;
  param.fAngle = 30.f;
  param.fDis = 32.f;
  param.fScale = 1.f;
  // for i dont how to decrease the effect of the prespecive,so choose a stupid
  // way
...
void cCamera::GroundToScreen(cPoint &pt) { GroundToScreen(pt, pt); }
...
void cCamera::ScreenToGround(cPoint &pt) { ScreenToGround(pt, pt); }
