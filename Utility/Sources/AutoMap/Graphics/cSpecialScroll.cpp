...
cSpecialScroll::~cSpecialScroll() { Clear(); }
...
const CString *cSpecialScroll::GetItemStringArray() { return c_aStrItem; }
...
cRect cSpecialScroll::GetScrollRect() {
  cRect rect(m_param.ptStart.x, m_param.ptStart.y,
             m_param.ptStart.x + m_param.ptSize.x,
             m_param.ptStart.y + m_param.ptSize.y);
  return rect;
}
...
void cSpecialScroll::CreatePicture() {
  for (int i = 0; i < MAX_IMAGE; i++) {
    ...
    param.AddMask(stPictureParam::MASK_PICTURE_LAYER |
                  stPictureParam::MASK_PICTURE_RESOURCE);
