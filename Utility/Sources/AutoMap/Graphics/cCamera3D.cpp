...
HRESULT cCamera3D::InitDeviceObjects() {
  float fx =
      m_param.ptTo.x + m_param.fDis * sin(m_param.fAngleX * D3DX_PI / 180);
  float fy =
      m_param.ptTo.y + m_param.fDis * cos(m_param.fAngleX * D3DX_PI / 180);
  float fz =
      m_param.ptTo.z + m_param.fDis * tan(m_param.fAngle * D3DX_PI / 180);
...
HRESULT cCamera3D::UpdateMatrix() {
  ...
  m_matScreen = D3DXMATRIX(
      viewData.Width / 2, 0, 0, 0, 0, -(float)viewData.Height / 2, 0, 0, 0, 0,
      viewData.MaxZ - viewData.MinZ, 0, viewData.X + viewData.Width / 2,
      viewData.Y + viewData.Height / 2, viewData.MinZ, 1);
...
void cCamera3D::ScreenToGround(float x, float y, float z, float &tx, float &ty,
                               float &tz) {
  ...
  vPickRayDir.x =
      v3.x * m_matIView._11 + v3.y * m_matIView._21 + v3.z * m_matIView._31;
  vPickRayDir.y =
      v3.x * m_matIView._12 + v3.y * m_matIView._22 + v3.z * m_matIView._32;
  vPickRayDir.z =
      v3.x * m_matIView._13 + v3.y * m_matIView._23 + v3.z * m_matIView._33;
...
void cCamera3D::GroundToScreen(float tx, float ty, float tz, float &x, float &y,
                               float &z) {
  D3DXVECTOR4 v4;
