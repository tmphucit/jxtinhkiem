...
public:
  ...
  virtual void ScreenToGround(float x, float y, float z, float &tx, float &ty,
                              float &tz) = 0;
  virtual void ScreenToGround(stVertexGround p, stVertexGround &pt);
  virtual void ScreenToGround(cPoint p, cPoint &pt);
  virtual void ScreenToGround(CPoint p, cPoint &pt);
  virtual void ScreenToGround(POINT p, cPoint &pt) {
    ScreenToGround(CPoint(p.x, p.y), pt);
  };
  virtual void ScreenToGround(cPoint &pt);

  virtual void GroundToScreen(float tx, float ty, float tz, float &x, float &y,
                              float &z) = 0;
  virtual void GroundToScreen(stVertexGround p, stVertexGround &pt);
  virtual void GroundToScreen(cPoint p, cPoint &pt);
  virtual void GroundToScreen(cPoint p, CPoint &pt);
  virtual void GroundToScreen(cPoint p, POINT &pt) {
    CPoint point;
    GroundToScreen(p, point);
    pt.x = point.x;
    pt.y = point.y;
  };
