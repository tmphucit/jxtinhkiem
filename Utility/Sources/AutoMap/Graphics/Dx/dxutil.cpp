...
HRESULT DXUtil_FindMediaFile(TCHAR *strPath, TCHAR *strFilename) {
  ...
  // Build full path name from strFileName (strShortName will be just the leaf
  // filename)
  ...
  file = CreateFile(strPath, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING,
                    0, NULL);
  if (INVALID_HANDLE_VALUE != file) {
    CloseHandle(file);
    return S_OK;
...
FLOAT __stdcall DXUtil_Timer(TIMER_COMMAND command) {
  ...
  if (m_bUsingQPF) {
    ...
    if (m_llStopTime != 0 && command != TIMER_START &&
    ...
    if (command == TIMER_GETAPPTIME) {
      double fAppTime =
          (double)(qwTime.QuadPart - m_llBaseTime) / (double)m_llQPFTicksPerSec;
    ...
    // Get either the current time or the stop time, depending
    // on whether we're stopped and what command was sent
    if (m_fStopTime != 0.0 && command != TIMER_START &&
        command != TIMER_GETABSOLUTETIME)
      fTime = m_fStopTime;
    else
