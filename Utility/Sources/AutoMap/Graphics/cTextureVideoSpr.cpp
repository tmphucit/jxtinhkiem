...
bool stD3DTextureInfo::IsFrame(int frm) { return frame == frm; }

const int MAX_FRAME = 256;
cTextureVideoSpr::cTextureVideoSpr(cGraphics *pGraphics)
    : cTexture(pGraphics), m_listInfo(MAX_FRAME) {
  m_listInfo.assign(MAX_FRAME, NULL);
  m_pTexture = NULL;
}

cTextureVideoSpr::~cTextureVideoSpr() { FinalCleanup(); }
...
stD3DTextureInfo *cTextureVideoSpr::CreateD3DTextureInfo(int frame) {
  ...
  HRESULT hr = GetGraphics()->GetDevice3D()->CreateTexture(
      Width, Height, 0, 0, GetResD3DFormat(), D3DPOOL_MANAGED,
      &(pInfo->pTexture));
...
IDirect3DTexture8 *cTextureVideoSpr::GetD3DTexture(int frame) {
  ...
  //		bind2nd(
  // mem_fun1<bool,stD3DTextureInfo,int>(&stD3DTextureInfo::IsFrame),frame ));
...
HRESULT cTextureVideoSpr::Create(const stResourceA &st, int width, int height) {
  ...
  pInfo->hTexture = height;
  HRESULT hr = GetGraphics()->GetDevice3D()->CreateTexture(
      width, height, 1, 0, st.stDevice.format, D3DPOOL_MANAGED,
      &(pInfo->pTexture));
...
// device info
//////////////////////////////////////////////////////////////////////
HRESULT cTextureVideoSpr::InitDeviceObjects() { return S_OK; };

HRESULT cTextureVideoSpr::RestoreDeviceObjects() { return S_OK; };

HRESULT cTextureVideoSpr::InvalidateDeviceObjects() { return S_OK; };

HRESULT cTextureVideoSpr::DeleteDeviceObjects() { return S_OK; };
