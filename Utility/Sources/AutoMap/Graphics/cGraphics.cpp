...
HRESULT cGraphics::Trace() {
  ...
  if (1) {
    ...
    if (m_param.lTraceEnum & TRACE_MOUSE)
      Trace(s);
    /*	remark by wooy
                    sprintf(s,"fps %d, faces %d, planever %d, light %d timer
       %d", fps.GetNowFps(), m_nFaces, GetcGround()->GetPlaneDrawNum(),
    ...
    /*	remark by wooy
            sprintf(s,"cache regions:%d 地图(%d/%d)
       video(%d/%d),Staticspr(%d/%d),video(%d/%d),动画spr(%d/%d),video(%d,%d/%d)",
