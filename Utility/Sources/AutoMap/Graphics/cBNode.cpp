...
void insert_list_seam(NodeList &nlist, cBNode *p) {
  ...
  if (p->GetVersortType() == VERSORT_POINT) {
    ...
    for (itNode it = nlist.begin(); it != nlist.end(); ++it) {
      ...
      if (pt.x >= _cpp_min(p1.x, p2.x) - fsizex &&
          pt.x <= _cpp_max(p1.x, p2.x) + fsizex &&
          pt.y >= _cpp_min(p1.y, p2.y) - fsizesub &&
          pt.y <= _cpp_max(p1.y, p2.y) + fsizeadd) {
        pFather = *it;
        e = e2;
        if (e == PL_Up)
          break;
    ...
    for (itNode it = nlist.begin(); it != nlist.end(); ++it) {
      ...
      // 在该矩形之内
      float fsize = 0.2f;
      if ((pt1.x >= _cpp_min(p1.x, p2.x) - fsize &&
           pt1.x <= _cpp_max(p1.x, p2.x) + fsize &&
           pt1.y >= _cpp_min(p1.y, p2.y) && pt1.y <= _cpp_max(p1.y, p2.y)) &&
          (pt2.x >= _cpp_min(p1.x, p2.x) - fsize &&
           pt2.x <= _cpp_max(p1.x, p2.x) + fsize &&
...
// utility function
/////////////////////////////////////////////////////////////////////
long cBNode::get_value() { return (long)_plane; }
...
void cBNode::GetSortPoint(cPoint &pt) { _plane->GetSortPoint(pt); }

void cBNode::GetSortSeam(cPoint &p1, cPoint &p2) {
  _plane->GetSortSeam(p1, p2);
}

void cBNode::GetSortLine(float &k, float &b) { _plane->GetLineFunction(k, b); }
