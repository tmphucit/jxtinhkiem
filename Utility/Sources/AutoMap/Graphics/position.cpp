...
cPoint LineCrossLineAt(cPoint pt1, cPoint pt2, cPoint p1, cPoint p2) {
  ...
  //	p.y = -( (x1*y2 - x2*y1)(y4 - y3) - (x3*y4 - x4*y3)(y2 - y1) ) /  ( (x2
  //- x1) * (y4 - y3) - (x4 - x3)(y2 - y1) );
  ...
  if (x2 == x1 && x4 == x3) {
    ...
    p.y = ((x1 * y2 - x2 * y1) * (y4 - y3) - (x3 * y4 - x4 * y3) * (y2 - y1)) /
          ((y2 - y1) * (x4 - x3) - (y4 - y3) * (x2 - x1));
    p.x = ((x1 * y2 - x2 * y1) * (x4 - x3) - (x3 * y4 - x4 * y3) * (x2 - x1)) /
          ((y2 - y1) * (x4 - x3) - (y4 - y3) * (x2 - x1));
  }
...
float Distance(cPoint p1, cPoint p2) { return sqrtf(DistanceSquare(p1, p2)); }
...
float PointToSeamSquare(cPoint pt, cPoint p1, cPoint p2) {
  ...
  //	y((x2-x1)*(x2-x1) + (y2-y1)(y2-y1)) = (y0(y2-y1) + x0(x2-x1))(y2-y1) -
  //(x1y2 - x2y1)(x2-x1); 	x((x2-x1)*(x2-x1) + (y2-y1)(y2-y1)) = (y0(y2-y1)
  //+ x0(x2-x1))(x2-x1) + (x1y2 - x2y1)(y2-y1);
  float x, y;
  y = ((y0 * (y2 - y1) + x0 * (x2 - x1)) * (y2 - y1) -
       (x1 * y2 - x2 * y1) * (x2 - x1)) /
      ((x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1));
  x = ((y0 * (y2 - y1) + x0 * (x2 - x1)) * (x2 - x1) +
       (x1 * y2 - x2 * y1) * (y2 - y1)) /
      ((x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1));

  //	如果垂点在线段上
  if (x >= _cpp_min(p1.x, p2.x) && x <= _cpp_max(p1.x, p2.x) &&
      y >= _cpp_min(p1.y, p2.y) && y <= _cpp_max(p1.y, p2.y))
...
float PointToLine(cPoint pt, cPoint p1, cPoint p2) {
  ...
  float f = fabs(pt.y * (x2 - x1) - pt.x * (y2 - y1) + (x1 * y2 - x2 * y1)) /
...
bool IsPointInPolygon(cPoint pt, cPoint p[], int nPoint) {
  ...
  for (i = 0; i < nPoint; i++) {
    float f = (pt.y - point[i].y) * (point[i + 1].x - point[i].x) -
              (pt.x - point[i].x) * (point[i + 1].y - point[i].y);
    if (f == 0)
      return false;
    if (i == 0) {
      if (f > 0)
...
void GetSeamCrossTile(cPoint p1,cPoint p2,list<cPoint> & ptlist)
{
        ...
        {
                int x = p1.x;
                for (int y = ceil(_cpp_min(y2,y1)); y <= floor(_cpp_max(y2,y1));
y++) ptlist.push_back(cPoint(x,y)); return;
        }
        if (p1.y == p2.y)
        {
                int y = p1.y;
...
//	y(x2-x1) - x(y2-y1) + (x1y2 - x2y1) = 0
        ...
        for (int y = ceil(_cpp_min(y2,y1)); y <= floor(_cpp_max(y2,y1)); y++)
        {
                ...
                if ( ceil(fx) - fx < fMin)
                        ...
                        x = floor(fx);
                bool binsert = false;
                for (list<cPoint>::iterator it = list1.begin(); it !=
list1.end(); ++it)
                {
                        if ((*it).x > x)
                        {
        ...
        for (++it; it != list1.end(); ++it)
        {
                ...
                //
                cPoint pt =
cPoint((int)(_cpp_min(p0.x,p9.x)),(int)(_cpp_min(p0.y,p9.y))); if (
find(ptlist.begin(),ptlist.end(),pt) == ptlist.end() ) ptlist.push_back(pt); p0
= p9;
        }
}
*/
...
_declspec(dllexport) void GetSeamCrossTile(cPoint p1, cPoint p2,
  ...
  /*
          ...
          {
                  int x = p1.x;
                  for (int y = floor(_cpp_min(y2,y1)); y <
     ceil(_cpp_max(y2,y1)); y++) ptlist.push_back(cPoint(x,y)); return;
          }
  ...
  */
  ...
  for (int y = ceil(_cpp_min(y2, y1)); y <= floor(_cpp_max(y2, y1)); y++) {
    ...
    if (ceil(fx) - fx < fMin)
      ...
      x = floor(fx);
    bool binsert = false;
    for (std::list<cPoint>::iterator it = list1.begin(); it != list1.end();
         ++it) {
      if ((*it).x > x) {
        binsert = true;
        list1.insert(it, cPoint(x, y));
  ...
  */
  ...
  for (++it; it != list1.end(); ++it) {
    ...
    //
    cPoint pt =
        cPoint(floor(_cpp_min(p0.x, p9.x)), floor(_cpp_min(p0.y, p9.y)));
#ifdef FLOAT_TOOMUCHLARGE
    pt += ptOffset;
#endif
...
bool PointInSeamTile(cPoint p, cPoint p1, cPoint p2) {
  ...
  for (std::list<cPoint>::iterator it = ptlist.begin(); it != ptlist.end();
       ++it) {
    if (p == (*it))
      return true;
  }
