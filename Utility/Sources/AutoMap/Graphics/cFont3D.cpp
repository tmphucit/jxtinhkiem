...
cFont3D::cFont3D(cGraphics *p) : cFont(p) { m_pFont = NULL; }
...
// device
/**************************************************************************/
LPDIRECT3DDEVICE8 cFont3D::GetDevice() { return GetGraphics()->GetDevice3D(); }
...
HRESULT cFont3D::DrawText(const stParam &param, LPCSTR strText, int nLength) {
  ...
  return m_pFont->DrawText(param.ptPos.x, param.ptPos.y, cr, (LPSTR)strText,
                           param.lFlag);
}
