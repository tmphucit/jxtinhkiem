...
#if !defined(                                                                  \
    AFX_CTEXTURECACHE_H__ED1E8826_1389_4B94_AE1D_306B6D820412__INCLUDED_)
...
#include <hash_map>
typedef std::hash_map<std::string, stTexture *, std::hash<std::string>,
...
public:
  ...
  void SetTotalSize(MediaEnum type, ULONG size) {
    m_size[type].nTotalSize = size;
  };
  ...
  void SetMaxCount(MediaEnum type, int count) {
    m_size[type].nMaxCount = count;
  }
