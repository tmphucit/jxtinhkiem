...
ULONG cCommandMachine::AddRef() { return ++m_ulCount; }
...
BOOL cCommandMachine::FindCommandMessageString(iCommand *pCom, int e,
                                               CString &strMessage) {
...
HRESULT cCommandMachine::End() { return 0; }
...
void cCommandMachine::RemoveRedoCommand() {
  for_each(m_itCommand, m_listCommand.end(),
           mem_fun<ULONG, iCommand>(&iCommand::Release));
...
HRESULT cCommandMachine::Replay() { return RedoToEnd(); }
...
_declspec(dllexport) iCommandMachine *
Command_CreateCommandMachine(LPCSTR szAppName, iGraphics *p) {
  return new cCommandMachine(szAppName, p);
}

_declspec(dllexport) eCommandType CommandNameToID(const CString &s) {
  static CString s_strCommand[] = {"file_newdocument",
                                   "file_opendocument",
                                   "file_savedocument",
...
_declspec(dllexport) int OutputNameToID(const CString &s) {
  static CString s_strCommandMessageItem[] = {
      "ok",   "succeed", "forbid", "warnning", "error", "hint",
      "help", "chat",    "next",   "new",      ""};
