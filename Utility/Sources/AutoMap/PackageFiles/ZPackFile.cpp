...
ZMapFile::ZMapFile(const char *file_name) {
  ...
  m_hMap = 0;
  m_hFile =
      CreateFile(file_name, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE,
...
ZCache::ZCache(unsigned long size) {
  ...
  free_ptr = buffer;
  //	temp_size = size / 4;
  ////四分之一的临时内存缓冲区 	temp_buffer = new char[temp_size]; 	temp_ptr
  ///=
  // temp_buffer;
}
...
char *ZCache::getNode(unsigned char key_size,
                      unsigned long size) { // 得到一个空闲缓冲区数据块
  size += sizeof(unsigned long) + key_size; // 加上块头的长度和索引的长度
  unsigned long align = size % sizeof(unsigned long);
  if (align)
    size += sizeof(unsigned long) - align;
  if (size >= MAX_BLOCK_SIZE || size >= cache_size)
    return 0;
  char *result;
...
ZPackFile::ZPackFile(const char *file_name, ZCache *the_cache) {
  ...
  char *index_ptr =
      data_map->map(header.index_offset, header.count * sizeof(index_info));
  memcpy(&index_list[0], index_ptr, header.count * sizeof(index_info));
  //	index_map = new ZMapFile(file_name);
  //	index_list = (index_info *)index_map->map(header.index_offset,
  // header.count * sizeof(index_info)); 	if(!index_list) return;
  opened = true;
}
...
char *ZPackFile::getData(unsigned long id) {
  ...
  *(unsigned long *)node = id;
  if (_readData(node_index, node + sizeof(unsigned long)))
