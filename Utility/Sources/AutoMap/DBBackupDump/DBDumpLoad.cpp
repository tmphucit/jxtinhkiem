...
bool CDBDump::Open(char *aFilePath) { // 打开包文件
  if (m_IsOpened)
    return false;
  ...
  if (aFileFound != -1)
    return false; // 如果该文件存在就返回不成功

  m_output = fopen(aFilePath, "wb");
  if (!m_output)
    return false; // 如果打开该文件失败就返回不成功
...
bool CDBDump::AddData(char *key, const size_t keysize, char *aData,
                      const size_t size) { // 添加文件
  if (!m_IsOpened)
    return false;
...
bool CDBLoad::Open(char *aFilePath) { // 打开包文件
  if (m_IsOpened)
    return false;
  _finddata_t FindData;
  long aFileFound = _findfirst(aFilePath, &FindData);
  if (aFileFound == -1)
    return false; // 如果该文件不存在就返回不成功
...
bool CDBLoad::ReadData(char *key, size_t &keysize, char *aData,
                       size_t &size) { // 添加文件
  if (!m_IsOpened)
    return false;
...
void CDBLoad::GotoHead() { // 把文件指针移动到文件头
  if (!m_IsOpened)
    return;
  if (m_output)
...
bool CDBLoad::SearchData(char *key, char *aData, size_t &size) { // 搜索数据
