...
void DBTran() {
  ...
  while (aBuffer) {
    ...
    int aNewSize =
        sizeof(TRoleDataNew) + sizeof(TDBSkillData) * 3 - 1; // 新角色数据的大小
    ...
    {
      cout << endl
           << aRoleBuf->BaseInfo.szName << " 该人物" << aLevel
           << "级，设为80级。" << endl;
      aLevel = 80;
    }
    ++LevelCount[aLevel];
    ...
    aRoleBufNew->BaseInfo.icurinner =
        aRoleBufNew->BaseInfo.imaxinner; // 当前内力
    aRoleBufNew->BaseInfo.icurstamina =
        aRoleBufNew->BaseInfo.imaxstamina; // 当前体力
