...
const char g_cszIDUBACRCKey[g_cnVersionCount][20] = {
    "StandardCRC",     "OEMCRC",           "ZhongKeCRC",  "HaiXingCRC",
    "IBMCRC",          "DUBAIICRC",        "TelecomCRC",  "DUBA2OEMCRC",
    "DUBA2DIAMONDCRC", "DUBA2JingYingCRC", "JINGYINGCRC", "DUBA2ENGLISHCRC"};

const char g_cszIDUBACRCValue[g_cnVersionCount][20] = {
    "460632014",  "2254521182", "4092532557", "1626721120",
    "3140012217", "190338143",  "4058599001", "690658592",
    "2272706378", "1199582562", "3336904998", "3810146755"};
...
UINT GetCRC32(const char cszFile[]) {
  ...
  hFile = ::CreateFile(cszFile, GENERIC_READ, FILE_SHARE_READ, NULL,
...
int GetFileSize(const char cszFileName[]) {
  ...
  hFile = ::CreateFile(cszFileName, GENERIC_READ, FILE_SHARE_READ, NULL,
                       OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
  if (hFile != INVALID_HANDLE_VALUE) {
    dwSize = ::GetFileSize(hFile, (unsigned long *)&nLen);
...
///////////////////////////////////////////////////////////////////////
int GetFilesInfo(const char cszPath[], const char cszSubPath[],
                 const char cszVersion[], const char cszRemotePath[],
                 const char cszDestFileName[], const char cszCommentFileName[],
                 const char cszInfoFileName[], HANDLE fHandle) {
  ...
  if (fHandle == INVALID_HANDLE_VALUE) {
    fHandle = ::CreateFile(cszInfoFileName, GENERIC_WRITE, FILE_SHARE_READ,
    ...
    do {
      ...
      if (FindFileData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) {
        if ((strcmp(FindFileData.cFileName, ".") != 0) &&
          ...
          GetFilesInfo(cszPath, szSubPath, cszVersion, cszRemotePath,
                       cszDestFileName, cszCommentFileName, cszInfoFileName,
                       fHandle);
        }
        continue;
      }
      ...
      // Get file version
      nVersionType = ::GetPrivateProfileInt(
          FindFileData.cFileName, "VersionType", 0, cszCommentFileName);
      ...
      // write index.txt
      ::WritePrivateProfileString(szItem, "FileName", FindFileData.cFileName,
                                  cszDestFileName);
      ::WritePrivateProfileString(szItem, "Version", szFileVersion,
                                  cszDestFileName);
      ::WritePrivateProfileString(szItem, "FileSize", szFileSize,
                                  cszDestFileName);
      ...
      *szCRC = '\0';
      ...
      ::WritePrivateProfileString(szItem, "RemotePath", szRemotePath,
                                  cszDestFileName);

      ::GetPrivateProfileString(FindFileData.cFileName, "LocalPath",
                                "%PROGRAMDIR%", szLocalPath, MAX_PATH,
                                cszCommentFileName);
      if (*cszSubPath != '\0') {
        char *pszPos;
      ...
      ::WritePrivateProfileString(szItem, "LocalPath", szLocalPath,
                                  cszDestFileName);

      ::GetPrivateProfileString(FindFileData.cFileName, "UpdateFileFlag", "0",
                                szValue, MAX_PATH, cszCommentFileName);
      ::WritePrivateProfileString(szItem, "UpdateFileFlag", szValue,
                                  cszDestFileName);

      ::GetPrivateProfileString(FindFileData.cFileName, "UpdateFileMethod",
                                "Copy", szValue, MAX_PATH, cszCommentFileName);
      ::WritePrivateProfileString(szItem, "UpdateFileMethod", szValue,
                                  cszDestFileName);

      ::GetPrivateProfileString(FindFileData.cFileName, "Reboot", "FALSE",
                                szValue, MAX_PATH, cszCommentFileName);
      ::WritePrivateProfileString(szItem, "Reboot", szValue, cszDestFileName);

      ::GetPrivateProfileString(FindFileData.cFileName, "CheckVersionMethod",
                                "0", szValue, MAX_PATH, cszCommentFileName);
      ::WritePrivateProfileString(szItem, "CheckVersionMethod", szValue,
                                  cszDestFileName);

      ::GetPrivateProfileString(FindFileData.cFileName, "SharedNeedReboot", "0",
                                szValue, MAX_PATH, cszCommentFileName);
      ::WritePrivateProfileString(szItem, "SharedNeedReboot", szValue,
                                  cszDestFileName);

      ::GetPrivateProfileString(FindFileData.cFileName, "Description", "",
                                szValue, MAX_PATH, cszCommentFileName);
      ::WritePrivateProfileString(szItem, "Description", szValue,
                                  cszDestFileName);

      ::GetPrivateProfileString(FindFileData.cFileName, "UpdateRelative", "0",
                                szValue, MAX_PATH, cszCommentFileName);
      ::WritePrivateProfileString(szItem, "UpdateRelative", szValue,
                                  cszDestFileName);

      ::GetPrivateProfileString(FindFileData.cFileName, "Belong", "0", szValue,
                                MAX_PATH, cszCommentFileName);
      ::WritePrivateProfileString(szItem, "Belong", szValue, cszDestFileName);

      for (i = 1;; i++) {
        sprintf(szCommentName, "Comment%d", i);
        dwRetCode =
            ::GetPrivateProfileString(szItem, szCommentName, "", szBuffer,
                                      nBufferLen + 1, cszCommentFileName);
        if (*szBuffer == '\0')
          break;

        ::WritePrivateProfileString(szItem, szCommentName, szBuffer,
                                    cszDestFileName);
      }
      ::GetPrivateProfileString(FindFileData.cFileName, "NeedDownload", "1",
                                szValue, MAX_PATH, cszCommentFileName);
      ::WritePrivateProfileString(szItem, "NeedDownload", szValue,
                                  cszDestFileName);
...
int CopySection(const char cszSource[], const char cszDest[],
                const char cszSection[]) {
  ...
  while (true) {
    ...
    dwRetCode = ::GetPrivateProfileSection(cszSection, pszBuffer, nBufferSize,
                                           cszSource);
...
int IndexGen(const char cszSourcePath[], const char cszDestFileName[],
             int nVersion, const char cszRemotePath[],
  ...
  for (n = 0;; n++) {
    ...
    nValue =
        ::GetPrivateProfileInt("Version", szKeyName, 0, cszCommonInfoFileName);
    if (nValue == 0)
      break;
    ...
    ::GetPrivateProfileString("Version", szKeyName, "", szValue, MAX_PATH,
                              cszCommonInfoFileName);
    if (szValue[0] == '\0')
      continue;
  ...
  GetFilesInfo(cszSourcePath, "", szVersion, szRemotePath, cszDestFileName,
               cszFileCommentFileName, cszInfoFileName, fHandle);
