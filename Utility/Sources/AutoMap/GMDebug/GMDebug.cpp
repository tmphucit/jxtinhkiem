...
void STDMETHODCALLTYPE CSystemEvent::PlayerMessage(char *szAccountFrom,
                                                   char *szRole,
...
void STDMETHODCALLTYPE CSystemEvent::AccountUserReturn(DWORD Operate,
                                                       char *szAccount,
                                                       int nReturn) {
  if (m_pParent) {
    CString strError;
...
void STDMETHODCALLTYPE CSystemEvent::OnlinePlayerCount(DWORD Operate,
                                                       char *szGWName,
                                                       DWORD nCount) {
  if (m_pParent) {
    CString strError;
...
void STDMETHODCALLTYPE CSystemEvent::RegisterUserCount(DWORD Operate,
                                                       DWORD nCount) {
  if (m_pParent) {
    CString strError;
...
void CGMDebugApp::CloseConnect() {
  if (m_pISystemConnect) {
    ...
    if (m_hDllJXGM) {
      ...
      fnDestroySystemOnline = (DESTROYSYSTEMONLINE)::GetProcAddress(
          m_hDllJXGM, _T("DestroySystemOnline"));
...
BOOL CGMDebugApp::InitInstance() {
  ...
  fnCreateSystemOnline = (CREATESYSTEMONLINE)::GetProcAddress(
      m_hDllJXGM, _T("CreateSystemOnline2"));
...
#ifdef _DEBUG
  GetPrivateProfileString("Main", "IP", "192.168.20.15", szModulePath, MAX_PATH,
                          strIni);
#else
  GetPrivateProfileString("Main", "IP", "192.168.", szModulePath, MAX_PATH,
                          strIni);
#endif //_DEBUG

  Adlg.m_strAddress = szModulePath;

  GetPrivateProfileString("Main", "Port", "7777", szModulePath, MAX_PATH,
                          strIni);
  ...
  if (Adlg.DoModal() == IDOK) {
    ...
    if (m_nSystemPort == 0)
      goto Error2;
    if (FAILED(pISystemOnline->OpenSystemConnect2(m_dwSystemIP, m_nSystemPort,
                                                  &m_pISystemConnect)) ||
        m_pISystemConnect == NULL) {
      goto Error2;
    }
  ...
  GetPrivateProfileString("Main", "Account", "gm-kingsoft", szModulePath,
                          MAX_PATH, strIni);

  Ndlg.m_strName = szModulePath;
  Ndlg.m_strPassword = "gm-kingsoft";
  if (Ndlg.DoModal() == IDOK) {
    hr = m_pISystemConnect->Login((char *)(const char *)Ndlg.m_strName,
                                  (char *)(const char *)Ndlg.m_strPassword,
                                  &m_Event);
    if (FAILED(hr)) {
      goto Error3;
    }
  ...
  pDocTemplate = new CMultiDocTemplate(
      IDR_GMDEBUTYPE, RUNTIME_CLASS(CGMDebugDoc),
...
Error2: {
  CString str;
  str.Format("在地址 %s 找不到 S3ReleyServer !!!",
             Adlg.m_strAddress + ":" + Adlg.m_strPort);
  ::AfxMessageBox(str);
}
  return FALSE;
Error3: { ::AfxMessageBox("帐户密码错, 登入系统失败!!!"); }
  return FALSE;
}
...
void CGMDebugApp::NewMessage(const char *szAccountFrom, const char *szRole,
                             const char *szMessage, int nMsgLen, DWORD dwGWIP) {
  m_RelayDataCS.Lock();
  RELAYDATA data;
...
void CGMDebugApp::ExecuteAction(const char *szAccount, const char *szScript,
                                DWORD nReturn) {
  if (m_pISystemConnect) {
    m_pISystemConnect->ExecuteAction((char *)szAccount, (char *)szScript,
                                     nReturn);
  }
}

void CGMDebugApp::ExecuteAction(const char *szRole, DWORD nRelayID,
                                const char *szScript, DWORD nReturn) {
  if (m_pISystemConnect) {
    m_pISystemConnect->ExecuteAction((char *)szRole, nRelayID, (char *)szScript,
                                     nReturn);
  }
}

void CGMDebugApp::ExecuteAction(DWORD nRelayID, const char *szScript,
                                DWORD nReturn, DWORD nSendIP) {
  if (m_pISystemConnect) {
    m_pISystemConnect->ExecuteAction(nRelayID, (char *)szScript, nReturn,
                                     nSendIP);
  }
}
...
BOOL CGMDebugApp::OnIdle(LONG lCount) { return CWinApp::OnIdle(lCount); }
  ...
  } else // 最后才处理消息到达
  {
    ...
    if (m_Datas.GetCount() > 0) {
      ...
      if (!data.strAccount.IsEmpty())
        str.Format("Say(\"%s\", \"%s\", \"%s\")", data.strAccount, data.strRole,
                   data.strMessage);
      else
        str = data.strMessage;
      pDoc->Trace(str);
...
void CGMDebugApp::ConnectAndLogin() {
  ...
  hr = pISystemConnect->Login((char *)(const char *)m_strAccount,
                              (char *)(const char *)m_strPassword, &m_Event);
  if (FAILED(hr)) {
    goto ErrorExit;
  }
