...
//////////////////////////////////////////////////////////////////////
namespace PackageCQ {
DataSource::DataSource() : m_CompressMethod(0) {}
...
void DataSource::InsertBlock(const BlockSource &Block) {
  try {
    ...
    string ErrorString =
        string("Error when DataSource Insert Block system Info:") +
        string(Error.what());
    throw exception(ErrorString.c_str());
  }
}

int DataSource::GetBlockCount() const { return m_BlockSet.size(); }
...
DataSource::DataSource(int Compress) : m_CompressMethod(Compress) {}

void DataSource::SetCompressMethod(int CompressID) {
  m_CompressMethod = CompressID;
}

int DataSource::GetCompressMethod() { return m_CompressMethod; }
