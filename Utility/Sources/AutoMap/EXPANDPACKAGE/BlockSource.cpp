...
BlockSource::BlockSource(int ID, int Length, DataPointFromStream &Point)
    : m_BlockID(ID), m_UnCompressBlockLength(Length), m_BlockPoint(Point) {

  m_BlockLength =
      ComputeBlockSize(m_UnCompressBlockLength, Point.GetStreamPointToData());
}

BlockSource::~BlockSource() {}

int BlockSource::GetID() const { return m_BlockID; }

int BlockSource::GetLength() const { return m_BlockLength; }
...
const BlockSource &BlockSource::operator<<(iostream &DataStream) {
  ...
  if (!DataStream.eof()) {
    ...
    DataStream.read((char *)&m_UnCompressBlockLength,
                    sizeof(m_UnCompressBlockLength));
  ...
  m_BlockLength = ComputeBlockSize(m_UnCompressBlockLength,
                                   m_BlockPoint.GetStreamPointToData());
...
BlockSource::BlockSource() : m_BlockPoint(), m_BlockID(0), m_BlockLength(0) {}
...
const BlockSource &BlockSource::operator>>(iostream &DataStream) {
  ...
  DataStream.write((char *)&m_UnCompressBlockLength,
                   sizeof(m_UnCompressBlockLength));
...
BlockSource::BlockSource(DataPointFromStream &Point)
    : m_BlockLength(0), m_BlockID(0), m_BlockPoint(Point),
      m_UnCompressBlockLength(0) {}

unsigned BlockSource::ComputeBlockSize(int SourceLength, iostream &Data) {
  unsigned Length;
  int SliceCount = (SourceLength + SliceSize - 1) / (SliceSize);
