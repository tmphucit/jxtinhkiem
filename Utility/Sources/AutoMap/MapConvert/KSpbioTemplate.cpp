...
bool KSpbioTemplate::LoadFromIniSection(class KIniFile *pIni,
  ...
  for (int i = 0; i < 5; i++) {
    if (pIni->GetString(pszSection, l_PropUnits[i].szSection, "", Buffer, 80)) {
      ...
      for (int j = l_PropUnits[i].nStartMappingItem;
           j <= l_PropUnits[i].nEndMappingItem; j++) {
...
void KSpbioTemplate::Instance(KBuildinObj *pInstance, int &nCentreX,
                              int &nCentreY) {
  if (pInstance) {
    if (m_bSetSortManner)
      ...
      pInstance->Props =
          m_uProps | (pInstance->Props & SPBIO_P_SORTMANNER_MASK);
