...
bool IsGraphics(LPCSTR szFile) {
  ...
  if (s.Find("_group") != -1)
    return 0;
  if (s.Find("美术图素") == -1 && s.Find("地图动画") == -1)
    return 0;
  return 1;
}
...
void CMapConvertDlg::OnLength() {
  ...
  CString sMessage;
  sMessage.Format("max file length is %d great than %d file is %d",
                  g_max_length, c_nMaxLength, g_count);
  AfxMessageBox(sMessage);
...
void CMapConvertDlg::ReplaceIni(const CString &strFile, const CString &strSrc,
...
void CMapConvertDlg::ReplaceFolder(const CString &strFile,
...
void CMapConvertDlg::OnCheckspr() {
  ...
  int n =
      AfxMessageBox("是否确认maps目录下放了全部的地图文件?", MB_YESNOCANCEL);
  if (n == IDCANCEL)
    return;
  if (n == IDNO) {
