...
#if !defined(                                                                  \
    AFX_MAPCONVERTDLG_H__8727E092_1D51_47BB_9E59_D22AE8568179__INCLUDED_)
...
public:
  ...
  void ReplaceIni(const CString &strFile, const CString &strSrc,
                  const CString &strDes);
  void ReplaceFolder(const CString &strFile, const CString &strSrc,
                     const CString &strDes);
  ...
  void ClearObstacle();
  void ModifyObstacle(int xRegion, int yRegion, int offx, int offy,
                      ObstacleEnum e);
  void SaveObstacle();
  ...
  //	BOOL FindNextRegion();
  void PackFile(mc_doonce fun, LPCSTR szRaw, LPCSTR szPack,
                const CString &strFolder, const CRect &rect);
...
private:
  bool ComposeGroundFile(const char *pFile1, const char *pFile2,
                         const char *pFile3);
  bool ConvertBuildinObjFile(const char *pFile1, const char *pOldLightFile,
                             const char *pFile2);
  bool ConvertNpcFileBin(const char *pRootPath, const char *pFile1,
                         const char *pFile2, bool bServer);
  bool ConvertObjFileBin(const char *pRootPath, const char *pFile1,
                         const char *pFile2, bool bServer,
                         const char *pBioFile);
  bool ConvertTrapFileBin(const char *pRootPath, const char *pFile1,
                          const char *pFile2, int nLeft, int nTop);
  ...
  bool LoadALight(struct KBuildInLightInfo *pLight, class KIniFile *pFile,
                  int nObject);
  bool LoadANpc(const char *pRootPath, struct KSPNpc *pNpc,
                class KIniFile *pFile, int nNpc, bool bServer);
  bool LoadAObj(const char *pRootPath, struct KSPObj *pObj,
                class KIniFile *pFile, int nObj, KBuildinObj *pBio,
                bool bServer);
  bool LoadATrap(/*const char* pRootPath,*/ struct KSPTrap *pTrap,
                 class KIniFile *pFile, int nTrap, int nLeft, int nTop);
