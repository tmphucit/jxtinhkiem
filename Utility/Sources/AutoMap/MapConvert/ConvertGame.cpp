...
#define ABOVE_HEAD_OBJ_ATLEAST_HIGH 80 // ¸ß¿Õ¶ÔÏó×îµÍÀëµØ¾àÀëÒªÇó
...
bool LoadTreeList() {
  ...
  CString File;
  File = "\\ÓÎÏ·×ÊÔ´\\ÃÀÊõÍ¼ËØ\\³ÇÊÐ\\Á½ºþ\\ÊÒÍâ\\Ê÷Ä¾\\Ê÷Ä¾ÖØÐÄ.txt";
  AppGetFullPath(File);
  l_TreeListFile.Clear();
  if (!l_TreeListFile.Load(File)) {
    MessageBox(NULL, File, "´ò¿ªÎÄ¼þ´íÎó¡£", MB_OK);
    return false;
  }
  l_TreeListFile.GetInteger("List", "Count", 0, &l_nNumTree);
  if (l_nNumTree > MAX_CHECK_TREE) {
    l_nNumTree = 0;
    MessageBox(NULL, File, "ÎÄ¼þÄÚÁÐ³öµÄÊ÷ÊýÄ¿¹ý´ó£¬ÈÃ³ÌÐò¸Ä³ÌÐò¡£", MB_OK);
    return false;
  }
  ...
  for (int i = 0; i < l_nNumTree; i++) {
    itoa(i, szIndex, 10);
    l_TreeListFile.GetString("List", szIndex, "", l_szTreeImgName[i],
                             sizeof(l_szTreeImgName[i]));
...
bool GetTreeCentre(const char *pszTreeImg, int nFrame, int &nCentreX,
                   int &nCentreY) {
  if (pszTreeImg) {
    for (int i = 0; i < l_nNumTree; i++) {
...
bool CMapConvertDlg::LoadANpc(const char *pRootPath, struct KSPNpc *pNpc,
  ...
  pNpc->cReserved[0] = pNpc->cReserved[1] = pNpc->cReserved[2] = 0;
  return true;
}
  ...
  if ((pObject->Props & SPBIO_P_SORTMANNER_MASK) == SPBIO_P_SORTMANNER_TREE ||
    ...
    if (strstr(pObject->szImage,
      ...
      if ((pObject->Props & SPBIO_P_SORTMANNER_MASK) ==
        ...
        if (GetTreeCentre(pObject->szImage, pObject->nFrame, nCentreX,
                          nCentreY)) {
          str = pObject->szImage;
          AppGetFullPath(str);
          KSprite spr;
