...
int on_zip_ground(LPCSTR szFile) {
  ...
  f.Read(m_aTile,
         sizeof(stTile) * (c_tWidthRegion + 1) * (c_tHeightRegion + 1));
  ...
  for (int i = 0; i < c_tWidthRegion + 1; i++)
    for (int j = 0; j < c_tHeightRegion + 1; j++) {
      ...
      if (m_aTile[i][j].szResource[0] != 0) {
        m_aGroundTile[i][j].idResource =
            Template_GetTerrainIndex(m_aTile[i][j].szResource);
        //	ASSERT(m_aGroundTile[i][j].idResource != -1);
      }
    }

  g_pPack->Convert(sx, sy, &m_aGroundTile,
...
int on_zip_plane(LPCSTR szFile) {
  ...
  for (int i = 0; i < s_Plane.nPlane; i++) {
    ...
    if (!GetSectionValue(p, "regionoffset", param.ptOffset)) {
      ...
      if (fabs(param.ptOffset.x) < c_nWidthRegion * 2 ||
          fabs(param.ptOffset.y) < c_nHeightRegion * 2) {
        int n;
        n = 0;
      } else {
  ...
  g_pPack->Convert(sx, sy, &s_Plane,
                   sizeof(stPlaneVerParam) * s_Plane.nPlane +
                       sizeof(s_Plane.nPlane));
...
int on_zip_npc(LPCSTR szFile) {
  ...
  g_pPack->Convert(sx, sy, &s_Obj,
                   sizeof(stObjPack) * s_Obj.nObj + sizeof(s_Obj.nObj));
  return 0;
}
...
int on_zip_light(LPCSTR szFile) {
  ...
  g_pPack->Convert(sx, sy, &s_Light,
                   sizeof(stLightParam) * s_Light.nLight +
                       sizeof(s_Light.nLight));
...
int on_zip_obstacle(LPCSTR szFile) {
  ...
  g_pPack->Convert(sx, sy, &s_Obstacle,
                   sizeof(stObstacleParam) * s_Obstacle.nObstacle +
                       sizeof(s_Obstacle.nObstacle));

  return 0;
}

void CMapConvertDlg::PackFile(mc_doonce fun, LPCSTR szRaw, LPCSTR szPack,
...
void CMapConvertDlg::OnZipmap() {
  // FILE_PROPERTY,FILE_GROUND,FILE_PLANEVER,FILE_PLANEHORBELOWVER,FILE_LIGHT,FILE_NPC,FILE_OBSTACLE,
  // need pack
  if (!VerifyFile())
    return;
  ...
  if (m_bPlane) {
    ...
    PackFile(on_zip_planehor, FILE_PLANEHORBELOWVER, PKFILE_PLANEHORBELOWVER,
             strFolder, rect);
  ...
  if (m_bNpc) {
    ...
    PackFile(on_zip_npc, FILE_CLIENTONLYNPC, PKFILE_CLIENTONLYNPC, strFolder,
             rect);
  ...
  if (m_bObj) {
    ...
    PackFile(on_zip_npc, FILE_CLIENTONLYBOX, PKFILE_CLIENTONLYBOX, strFolder,
             rect);
