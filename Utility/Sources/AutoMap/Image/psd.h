...
#if !defined(                                                                  \
    AFX_TPSDFILECODE_H__9D2B4C1A_1D42_4364_A93E_65D59EF0F741__INCLUDED_)
...
template <class T = void *> struct SPSD_COMMemoryPolicy {
...
template <class T = void *> struct SPSD_NewMemoryPolicy {
  typedef T pointer_type;
...
template <class MEMORY_POLICY = SPSD_NewMemoryPolicy<>> class TFileIO {
public:
...
template <class CHANNEL_SURFACE,
...
public:
  // --------------------------------------------------------------------
  typedef TPSDFileLoader<CHANNEL_SURFACE, CHANNEL_COLLECTION, INPUT_POLICY,
                         SIZE_COLLECTION>
      this_type;
...
private:
  ...
  bool LoadChannelsHandler(INPUT &input, DECODER &decode,
                           SIZE_COLLECTION &dataSizeTable,
                           CHANNEL_COLLECTION *pChannelCollection,
...
public:
  ...
  CHANNEL_COLLECTION *LoadFromAbstraction(INPUT &input,
                                          const int nLoadChannelCountMax = 0) {
    ...
    if (COMPRESSION_NONE == compressionType) {
      ...
      if (!LoadChannelsHandler(input, ReadRaw8BitChannel(), dataSizeTable,
      ...
      // Call the handler
      // ------------------------------------------------------------
      if (!LoadChannelsHandler(input, Unpack8BitChannel(), dataSizeTable,
                               pChannelCollection, loadChannelCount))
        goto HANDLE_ERROR;
    }
  ...
  CHANNEL_COLLECTION *Load(const char *pInputName,
                           const int nLoadChannelCountMax = 0) {
    ...
    CHANNEL_COLLECTION *pChannels =
        LoadFromAbstraction(input, nLoadChannelCountMax);
