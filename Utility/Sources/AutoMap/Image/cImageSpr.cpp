...
inline void GetColorA4R4G4B4Format(WORD cr, BYTE &a, BYTE &r, BYTE &g,
                                   BYTE &b) {
  a = ((cr >> 12) << 4);
  r = ((((WORD)(cr << 4)) >> 12) << 4);
  g = ((((WORD)(cr << 8)) >> 12) << 4);
...
inline void SetColorA4R4G4B4Format(WORD &cr, BYTE a, BYTE r, BYTE g, BYTE b) {
  cr =
      ((WORD)(a >> 4)) << 12 | ((WORD)(r >> 4)) << 8 | (g >> 4) << 4 | (b >> 4);
}
...
//////////////////////////////////////////////////////////////////////
// info of a image
ULONG cImageSpr::GetSize() { return m_size; }
  ...
  for (int i = 0; i < 16; i++)
    for (int j = 0; j < 16; j++) {
      for (int a = 0; a < 16; a++) {
        ...
        int jj = (j << 4) + 8;
        int aa = (a << 4);
        g_aAlphaResult4444[(i << 8) | (j << 4) | (a)] =
            ((ii * (255 - aa) + jj * aa) / 255) >> 4;
      }
    }
}
...
HRESULT cImageSpr::RenderToA4R4G4B4Alpha2(stImageRender &render) {
  CString strMessage;
  strMessage.Format("%s RenderToA4R4G4B4Alpha2 frame %d ", m_strName,
                    render.nFrame);
...
HRESULT cImageSpr::RenderToA4R4G4B4Alpha(stImageRender &render) {
  ...
  if (render.rcSrc.Width() > info.width ||
      render.rcSrc.Height() > info.height) {
    ASSERT(0);
    return -1;
  }
...
BOOL cImageSpr::Save() {
  CFile File;
  if (!AppOpenFile(File, m_strName, CFile::modeCreate | CFile::modeWrite,
                   "image"))
    return FALSE;
...
BOOL cImageSpr::Load(LPCSTR FileName) {
  ...
  // open the file
  if (!AppOpenFile(File, FileName, CFile::modeRead | CFile::shareDenyWrite,
                   "image")) {
    if (!AppOpenFile(File, "\\system\\spr\\white.spr",
                     CFile::modeRead | CFile::shareDenyWrite, "image"))
      return FALSE;
  }
...
BOOL cImageSpr::LoadInfo(LPCSTR FileName) {
  ...
  if (!AppOpenFile(File, FileName, CFile::modeRead | CFile::shareDenyWrite,
                   "image")) {
    //		if
    //(!AppOpenFile(File,"\\system\\txt\\planedefault.spr",CFile::modeRead |
    // CFile::shareDenyWrite,"image"))
    return FALSE;
  }
...
HRESULT cImageSpr::SaveFile8888(const char *filename, stImageInfo *pInfo) {
  ...
  if (pInfo != NULL) {
    if (pInfo->nDirections != 0 && pInfo->nDirections != 1) {
      ...
      if (m_nFrameInfo % pInfo->nDirections != 0) {
        CString strErr;
        strErr.Format("'%s' 方向：%d 帧数：%d 该套图片不齐，请重新生成!",
                      filename, pInfo->nDirections, m_nFrameInfo);
  ...
  for (i = 0; i < m_nFrameInfo; i++) {
    ...
    for (int j = 0; j < sz; j++) {
      *pBitmap++ =
          ImageTree.QueryIndex(*(buffer + 2), *(buffer + 1), *(buffer + 0));
      // no alpha
