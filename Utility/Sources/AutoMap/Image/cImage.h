...
public:
  ...
  virtual BOOL DivideTo(UINT wDivide, UINT hDivide) {
    ASSERT(0);
    return false;
  };
  virtual HRESULT SetCenter(int nFrame, int x, int y) { return 0; };
  virtual BOOL DivideTo(LPCSTR szFile, stDivide &divide) {
    ASSERT(0);
    return 0;
  };
  ...
  virtual HRESULT CreateFile(const char *filename, const stImageInfo &info) {
    return 0;
  };
