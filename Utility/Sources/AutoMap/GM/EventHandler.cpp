...
void STDMETHODCALLTYPE KSystemEventHandler::GWLogin(char *szGWName, DWORD nIP) {
  ...
  // post message to dialog
  {
    ...
    ::PostThreadMessage((DWORD)_Module.dwThreadID, WM_JX_GW_LOGIN, 0,
                        (LONG)strOut.AllocSysString());
  }
}

void STDMETHODCALLTYPE KSystemEventHandler::GWSubLogout(DWORD nGWIP,
                                                        DWORD nSubIP) {
  for (int i = 0, nSize = (int)m_pExgSys->aryGameWorlds.size(); i < nSize;
       i++) {
    const GAMEWORLD &gw = m_pExgSys->aryGameWorlds[i];
    if (nGWIP == gw.dwIP) {
      // GameWorld found
      ::PostThreadMessage((DWORD)_Module.dwThreadID, WM_JX_SUB_LOGOUT, nSubIP,
                          (LONG)gw.strGameWorldName.AllocSysString());
      return;
    }
  }
  ASSERT(0); // not login ???
...
void STDMETHODCALLTYPE KSystemEventHandler::GWLogout(char *szGWName,
                                                     DWORD nIP) {
  for (int i = 0, nSize = (int)m_pExgSys->aryGameWorlds.size(); i < nSize;
       i++) {
    const GAMEWORLD &gw = m_pExgSys->aryGameWorlds[i];
    if (szGWName == gw.strGameWorldName && nIP == gw.dwIP) {
      ...
      {
        CString strOut(szGWName);
        ::PostThreadMessage((DWORD)_Module.dwThreadID, WM_JX_GW_LOGOFF, 0,
                            (LONG)strOut.AllocSysString());
      }
      return;
    }
...
void STDMETHODCALLTYPE KSystemEventHandler::OnlinePlayerCount(char *szGWName,
                                                              DWORD nCount) {
...
KSystemEventHandler::PlayerAccountAtGWID(char *szAccount, DWORD dwRelayID) {
#ifdef _DEBUG
  if (::WaitForSingleObject(m_pExgSys->hEventGetPlayerAtGWDone, 0) ==
      WAIT_OBJECT_0)
    TRACE("%s(%u): ISystemEvent::PlayerAccountAtGWID(...) called for no "
          "reason.\n",
          __FILE__, __LINE__);
#endif
...
void STDMETHODCALLTYPE KSystemEventHandler::PlayerRoleList(char *szAccount,
                                                           WORD wRoleCount,
                                                           void *p) {
#ifdef _DEBUG
  if (::WaitForSingleObject(m_pExgSys->hEventGetRoleListDone, 0) ==
      WAIT_OBJECT_0)
...
void STDMETHODCALLTYPE KSystemEventHandler::PlayerAccountAtGW(char *szAccount,
                                                              DWORD nIP, int nX,
                                                              int nY,
                                                              int nSubWorld,
                                                              char *szCurRole) {
  ...
  if (::WaitForSingleObject(m_pExgSys->hEventGetPlayerAtGWDone, 0) ==
      WAIT_OBJECT_0)
    TRACE(
        "%s(%u): ISystemEvent::PlayerAccountAtGW(...) called for no reason.\n",
        __FILE__, __LINE__);
#endif
    ...
    if (pos) {
      if (nIP != 0 && nIP != 0xFFFFFFFF) {
        ...
        if (nSubWorld != KGameMasterCore::s_nJailWorldID) {
          ...
          ::PostThreadMessage((DWORD)_Module.dwThreadID, WM_JX_DO_SCRIPT, 0,
                              (LONG)strOut.AllocSysString());
        } else {
          ...
          CString strRole = strOut.Mid(nSplit + 1, nSplit1 - nSplit - 1),
                  strGWName;
          for (int i = 0, nSize = (int)m_pExgSys->aryGameWorlds.size();
               i < nSize; i++) {
            const GAMEWORLD &gw = m_pExgSys->aryGameWorlds[i];
            if (nIP == gw.dwIP) {
              // GameWorld found
          ...
          if (strRole == szCurRole && strGWName == strOut.Mid(nSplit1 + 1))
            ::PostThreadMessage((DWORD)_Module.dwThreadID, WM_JX_JAIL_JUDGE, 0,
                                (LONG)strOut.AllocSysString());
        }
      }
...
void STDMETHODCALLTYPE KSystemEventHandler::GameWorldRelayID(char *szGWName,
                                                             DWORD nRelayID) {
#ifdef _DEBUG
  if (::WaitForSingleObject(m_pExgSys->hEventGetGWRelayIDDone, 0) ==
      WAIT_OBJECT_0)
    TRACE("%s(%u): ISystemEvent::GameWorldRelayID(\"%s\", ...) called for no "
          "return.\n",
...
void STDMETHODCALLTYPE KSystemEventHandler::PlayerTrackingData(
    char *szAccount, void *pData, size_t dataLength) {
  ASSERT(0); // todo...database link
}

void STDMETHODCALLTYPE KSystemEventHandler::PlayerInfo(char *szAccount,
                                                       WORD ninfo, void *pData,
                                                       size_t dataLength) {
#ifdef _DEBUG
  if (::WaitForSingleObject(m_pExgSys->hEventGetPlayerInfoDone, 0) ==
      WAIT_OBJECT_0)
...
void STDMETHODCALLTYPE KSystemEventHandler::PlayerMessage(char *szAccountFrom,
                                                          char *szRole,
                                                          char *szMessage,
                                                          int nMsgLen,
                                                          DWORD dwGWIP) {
  ...
  strOK += strMsg;
  ::PostThreadMessage((DWORD)_Module.dwThreadID, WM_JX_CHATING, 0,
                      (LONG)strOK.AllocSysString());
}
