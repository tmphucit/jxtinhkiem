...
KPlayerApiTool::KPlayerApiTool() { m_pGMCoreRef = NULL; }
...
STDMETHODIMP KPlayerApiTool::get_ActiveRole(IKRole **pVal) {
  ...
  HRESULT hr = m_pGMCoreRef->WhichGWIsPlayerInRealTime(
      gw, strRole, nX, nY, nWorldID, m_strAccountName);
  if (!SUCCEEDED(hr))
    strRole.Empty();
    ...
    hr = m_pGMCoreRef->GetRoleList(m_strAccountName, gw.strGameWorldName,
                                   lstRoles);
    KS_CHECK(hr);

    {
      if (lstRoles.Find(strRole)) {
        if (LookingForChild(strRole + "\n" + gw.strGameWorldName,
          ...
          if (pPLTool) {
            pPLTool->Init(m_pGMCoreRef, m_strAccountName, strRole,
                          gw.strGameWorldName);
...
STDMETHODIMP KPlayerApiTool::get_CountOfRolesInGW(BSTR strGameWorldName,
                                                  long *pVal) {
  AFX_MANAGE_STATE(AfxGetStaticModuleState())
...
STDMETHODIMP KPlayerApiTool::ItemRoleByNameInGW(BSTR strGameWorldName,
                                                BSTR strRoleName,
                                                IKRole **prop) {
  AFX_MANAGE_STATE(AfxGetStaticModuleState())
...
STDMETHODIMP KPlayerApiTool::ItemRoleByIndexInGW(BSTR strGameWorldName,
                                                 long Index, IKRole **prop) {
  AFX_MANAGE_STATE(AfxGetStaticModuleState())
