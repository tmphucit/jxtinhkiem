...
STDMETHODIMP CGMApp::get_TotalOnlineCount(long *prop) {
  ...
  TRY { *prop = m_pGameMaster->GetOnlineCount(""); }
  CATCH_ALL(e) {
    TRACE("%s(%u): Failed.\n", __FILE__, __LINE__);
    return E_FAIL;
  }
...
STDMETHODIMP CGMApp::get_TotalRegisteredCount(long *pVal) {
  ...
  TRY { *pVal = m_pGameMaster->GetRegisterCount(); }
  CATCH_ALL(e) {
    TRACE("%s(%u): Failed.\n", __FILE__, __LINE__);
    return E_FAIL;
  }
...
STDMETHODIMP CGMApp::get_IsGMSystemRunning(BOOL *pVal) {
  ...
  TRY { *pVal = m_pGameMaster->IsConnectionOK(); }
  CATCH_ALL(e) {
    TRACE("%s(%u): Failed.\n", __FILE__, __LINE__);
    return E_FAIL;
  }
...
STDMETHODIMP CGMApp::get_CountOfGameWorlds(long *pVal) {
  ...
  TRY { *pVal = m_pGameMaster->CountOfGameWorlds(); }
  CATCH_ALL(e) {
    TRACE("%s(%u): Failed.\n", __FILE__, __LINE__);
    return E_FAIL;
  }
...
STDMETHODIMP CGMApp::ItemGameWorldByIndex(long Index, IKGameWorld **prop) {
  ...
  KS_CHECK_BOOLEX(Index >= 0 && Index < m_pGameMaster->CountOfGameWorlds(),
                  hr = E_FAIL);
...
STDMETHODIMP CGMApp::FindPlayerInRealTime(BSTR strAccount,
                                          BSTR *pstrInGameWorld) {
  AFX_MANAGE_STATE(AfxGetStaticModuleState())
  ...
  HRESULT hr = m_pGameMaster->WhichGWIsPlayerInRealTime(gw, strRole, nX, nY,
                                                        nWorldID, strAcc);
  if (SUCCEEDED(hr))
    *pstrInGameWorld = gw.strGameWorldName.AllocSysString();
  return hr;
