...
KGameMasterCore::KGameMasterCore(ISystemOnline *pISO) {
  ...
  GetPrivateProfileString("System", "IP", "61.55.138.19",
                          strTmp.GetBuffer(STR_BUFFER_LEN), STR_BUFFER_LEN,
                          gINI_FILENAME);
  ...
  hr = m_pISystemConnect->Login((char *)(const char *)strAccount,
                                (char *)(const char *)strPassword,
                                m_pSysHandler);
  if (FAILED(hr)) {
...
HRESULT KGameMasterCore::GetRoleList(LPCSTR szAccount, LPCSTR szGWName,
                                     CStringList &lstRoleNames) {
  HRESULT hr = S_OK;
  ...
  if (::WaitForSingleObject(m_ExgSYS.hEventGetRoleListDone, SYSTEM_TIMEOUT) !=
      WAIT_OBJECT_0) {
    // time out
    ::SetEvent(m_ExgSYS.hEventGetRoleListDone);
    TRACE("%s(%u): KGameMasterCore::GetRoleList(..., szAccountName == \"%s\") "
          "time out.\n",
...
KGameMasterCore::~KGameMasterCore() {
  ...
  if (m_pConnSQL) {
    S3PDBSQLServerConnectionPool *pDB =
        S3PDBSQLServerConnectionPool::Instance();
...
HRESULT KGameMasterCore::WhichGWIsPlayerIn(GAMEWORLD &gw, LPCSTR szAccount) {
  ...
  if (::WaitForSingleObject(m_ExgSYS.hEventGetPlayerAtGWDone, SYSTEM_TIMEOUT) !=
      WAIT_OBJECT_0) {
    // time out
    ::SetEvent(m_ExgSYS.hEventGetPlayerAtGWDone);
    TRACE("%s(%u): KGameMasterCore::WhichGWIsPlayerIn(..., szAccountName == "
          "\"%s\") time out.\n",
          __FILE__, __LINE__, szAccount);
    hr = E_FAIL;
    goto KS_EXIT;
  ...
  if (0 == m_ExgSYS.dwIPGWIncludePlayer ||
      0xFFFFFFFF == m_ExgSYS.dwIPGWIncludePlayer) {
...
BOOL KGameMasterCore::WhichRelayIsTheGameWorld(DWORD &dwRelayID,
                                               LPCSTR szGWName) {
  HRESULT hr = S_OK;

  dwRelayID = 0xFFFFFFFF;

  KS_CHECK_BOOLEX(m_pISystemConnect && m_bConnected && szGWName &&
                      strlen(szGWName),
                  hr = E_FAIL);
  ...
  if (::WaitForSingleObject(m_ExgSYS.hEventGetGWRelayIDDone, SYSTEM_TIMEOUT) !=
      WAIT_OBJECT_0) {
    // time out
    ::SetEvent(m_ExgSYS.hEventGetGWRelayIDDone);
    TRACE("%s(%u): KGameMasterCore::WhichRelayIsTheGameWorld(..., szGWName == "
          "\"%s\") time out.\n",
...
HRESULT KGameMasterCore::WhichGWIsPlayerInRealTime(GAMEWORLD &gw,
                                                   CString &rstrCurRole,
                                                   int &rnCurX, int &rnCurY,
                                                   int &rnCurWorldID,
                                                   LPCSTR szAccount) {
  HRESULT hr = S_OK;
  int i, nSize;
  ...
  if (::WaitForSingleObject(m_ExgSYS.hEventGetPlayerAtGWDone, SYSTEM_TIMEOUT) !=
    ...
    ::SetEvent(m_ExgSYS.hEventGetPlayerAtGWDone);
    m_ExgSYS.strAccount.Empty();
    TRACE("%s(%u): KGameMasterCore::WhichGWIsPlayerInRealTime(..., "
          "szAccountName == \"%s\") time out.\n",
          __FILE__, __LINE__, szAccount);
    hr = E_FAIL;
    goto KS_EXIT;
  }
...
int KGameMasterCore::PingSystem() {
  ...
  if (::WaitForSingleObject(m_ExgSYS.hEventPingDone, PING_TIMEOUT) !=
      WAIT_OBJECT_0) {
    // time out
...
HRESULT KGameMasterCore::SendMessage(LPCSTR szAccount, LPCSTR szRole,
                                     LPCSTR szMessage) {
  HRESULT hr = S_OK;
  KS_CHECK_BOOLEX((m_bConnected && m_pISystemConnect), hr = E_FAIL);

  m_pISystemConnect->SendMessage((char *)szAccount, (char *)szRole,
                                 (char *)szMessage);
...
void KGameMasterCore::DoJailProcess(DWORD dwTickCount) {
  ...
  if (pCon->QuerySql(strSQL, &pResult)) {
    ...
    if (pResult && pResult->num_rows() > 0) {
      ...
      // 清除过多的无效等待队列
      if (m_ExgSYS.lstJailWaitingForFree.GetCount() >
          pResult->num_rows() * 2 + 128) {
      ...
      for (int i = 0, nSize = pResult->num_rows(); i < nSize;
        ...
        if (pResult->get_field_data(1, &AccountName, sizeof(_variant_t)) &&
          ...
          ASSERT(VT_BSTR == AccountName.vt && VT_BSTR == RoleName.vt &&
                 VT_BSTR == GWName.vt);
          // 将该用户加入等待处理队列
          strTmp.Format("%s\n%s\n%s", CString(((LPCWSTR)(_bstr_t)AccountName)),
          ...
          m_pISystemConnect->GetPlayerAccountAtGWRealTime(
              (char *)(LPCSTR)strTmp);
        }
      }
...
void KGameMasterCore::Breathe(DWORD dwTickCount) {
  S3PDBAccessConnectionPool *pDB = S3PDBAccessConnectionPool::Instance();
  S3PDBSQLServerConnectionPool *pDBSQL =
      S3PDBSQLServerConnectionPool::Instance();
...
HRESULT KGameMasterCore::PutInJail(LPCSTR szAccount, LPCSTR szRole,
                                   LPCSTR szGWName, int nMinutes) {
  if (nMinutes <= 0 || nMinutes > 120 || strlen(szRole) <= 0 ||
      strlen(szAccount) <= 0 || strlen(szGWName) <= 0)
    return E_FAIL;
  ...
  if (gw.strGameWorldName == szGWName && szRole == strRole) {
    // 如果该用户在线，立即关入天牢
    strRole.Format("ds NewWorld(%d, %d, %d)", KGameMasterCore::s_nJailWorldID,
                   KGameMasterCore::s_nJailX, KGameMasterCore::s_nJailY);
    hr = ExecutePlayer(szAccount, strRole);
  } else {
    // 用户不在线，更新进入游戏点
    VERIFY(
        SetPlayerPosInDB(szRole, szGWName, s_nJailWorldID, s_nJailX, s_nJailY));
  }

  sprintf(strSQL,
          "select count(*) from JXJail where (AccountName='%s' and "
          "RoleName='%s' and GameWorldName='%s')",
          szAccount, szRole, szGWName);
  if (pCon->QuerySql(strSQL, &pResult)) {
    ASSERT(pResult);
    _variant_t var;
    ...
    if ((long)var > 0) {
      sprintf(strSQL,
              "update JXJail set JailMinutesLeft=%d where (AccountName='%s' "
              "and RoleName='%s' and GameWorldName='%s')",
              nMinutes, szAccount, szRole, szGWName);
      KS_CHECK_BOOLEX(pCon->Do(strSQL), hr = E_FAIL);
    } else {
      sprintf(strSQL,
              "insert into JXJail (AccountName, RoleName, JailMinutesLeft, "
              "OriginX, OriginY, OriginWorld, GameWorldName) values ('%s', "
              "'%s', %d, %d, %d, %d, '%s')",
...
HRESULT KGameMasterCore::FreeFromJail(LPCSTR szAccount, LPCSTR szRole,
                                      LPCSTR szGWName) {
  ...
  sprintf(strSQL,
          "select * from JXJail where (AccountName='%s' and RoleName='%s' and "
          "JailMinutesLeft > 0 and GameWorldName='%s')",
          szAccount, szRole, szGWName);
  if (pCon->QuerySql(strSQL, &pResult)) {
    ASSERT(pResult);
    _variant_t var;
    if (pResult->num_rows()) {
      ...
      sprintf(strSQL,
              "delete from JXJail where (AccountName='%s' and RoleName='%s' "
              "and GameWorldName='%s')",
              szAccount, szRole, szGWName);
      KS_CHECK_BOOLEX(pCon->Do(strSQL), hr = E_FAIL);
    }
...
HRESULT KGameMasterCore::JailJudge(LPCSTR szAccount, LPCSTR szRole,
                                   LPCSTR szGWName) {
  ...
  sprintf(strSQL,
          "select * from JXJail where (AccountName='%s' and RoleName='%s' and "
          "GameWorldName='%s' and JailMinutesLeft > 0)",
          szAccount, szRole, szGWName);
  if (pCon->QuerySql(strSQL, &pResult)) {
    ASSERT(pResult);
    _variant_t var;
    if (pResult->num_rows()) {
      ...
      if (nMinitesLeft <= 0)
        ...
        sprintf(
            strSQL,
            "update JXJail set JailMinutesLeft=%d where (AccountName='%s' and "
            "RoleName='%s' and GameWorldName='%s' and JailMinutesLeft > 0)",
            nMinitesLeft, szAccount, szRole, szGWName);
        KS_CHECK_BOOLEX(pCon->Do(strSQL), hr = E_FAIL);
      }
    }
  }
...
HRESULT KGameMasterCore::SaveMessageToDB(LPCSTR szAccount, LPCSTR szRole,
  ...
  sprintf(strSQL,
          "insert into question (ddatetime, playerAccount, playerRole, gwcode, "
          "question, isAnswer) values ('%s', '%s', '%s', '%s', '%s', 0)",
...
HRESULT KGameMasterCore::SaveDisconnectionToDB(LPCSTR szGameWorld,
                                               LPCSTR szSubIP) {
  HRESULT hr = S_OK;
...
HRESULT KGameMasterCore::BroadCastChatToGameWorlds(LPCSTR szGWName,
                                                   LPCSTR szCharMessage) {
  HRESULT hr = S_OK;
  ...
  if (lstGWNames.GetCount()) {
    ...
    for (POSITION pos = lstGWNames.GetHeadPosition(); pos;) {
      KS_CHECK_BOOLEX(
          WhichRelayIsTheGameWorld(dwRelayID, lstGWNames.GetNext(pos)),
          hr = E_FAIL);
...
BOOL KGameMasterCore::SetPlayerPosInDB(LPCSTR szRole, LPCSTR szGWName,
  ...
  m_pISystemConnect->SetPlayerInfoToDB((char *)szRole, dwRelayID,
                                       gm_role_entergame_position, &data,
                                       sizeof(data));
...
BOOL KGameMasterCore::GetPlayerPosInDB(LPCSTR szRole, LPCSTR szGWName,
  ...
  m_pISystemConnect->GetPlayerInfoFromDB((char *)szRole, dwRelayID,
                                         gm_role_entergame_position);

  if (::WaitForSingleObject(m_ExgSYS.hEventGetPlayerInfoDone, SYSTEM_TIMEOUT) !=
      WAIT_OBJECT_0) {
    // time out
    ::SetEvent(m_ExgSYS.hEventGetPlayerInfoDone);
    TRACE("%s(%u): KGameMasterCore::GetPlayerPosInDB(..., szRoleName == "
          "\"%s\") time out.\n",
