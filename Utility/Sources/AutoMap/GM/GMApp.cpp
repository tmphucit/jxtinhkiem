...
BOOL CGMApp::InitInstance() {
  ...
  S3PDBAccessConnectionPool *pConnectPool =
      S3PDBAccessConnectionPool::Instance();
  KS_CHECK_BOOLEX(pConnectPool, hr = E_FAIL);

  br = pConnectPool->Init(strPath + GM_DB_NAME, 10);
  if (!br) {
    CString str;
    str.Format("在当前位置 \"%s\" 找不到运行所需文件 \"%s\" !!!", szModulePath,
               GM_DB_NAME);
    ::AfxMessageBox(str);
    goto KS_EXIT;
  }
...

  {
    ...
    GetPrivateProfileString("DataBase", "DBAddress", "192.168.20.84", szBuffer,
                            sizeof(szBuffer), gINI_FILENAME);
    database.strServer = szBuffer;
    GetPrivateProfileString("DataBase", "DBName", "cs", szBuffer,
                            sizeof(szBuffer), gINI_FILENAME);
    database.strDataBase = szBuffer;
    GetPrivateProfileString("DataBase", "UserName",
                            "8UGFSDUHGxWCA~TH1jkNrx84Zz080AF7", szBuffer,
                            sizeof(szBuffer), gINI_FILENAME); // blackbay
    ::SimplyDecryptPassword(szBufDec, szBuffer);
    database.strUser = szBufDec;
    GetPrivateProfileString("DataBase", "Password",
                            "bLdZsppHf_ij_HzMJAUnrb_VVdaZ4OAY", szBuffer,
                            sizeof(szBuffer), gINI_FILENAME); // thekingofworld
    ::SimplyDecryptPassword(szBufDec, szBuffer);
    ...
    if (!br) {
      CString str;
      str.Format("在地址 \"%s\" 无法打开 SQL 数据库 \"%s\" !!!",
                 database.strServer.c_str(), database.strDataBase.c_str());
      ::AfxMessageBox(str);
    }
  }
  ...
  fnCreateSystemOnline = (CREATESYSTEMONLINE)::GetProcAddress(
      m_hDllJXGM, _T("CreateSystemOnline"));
