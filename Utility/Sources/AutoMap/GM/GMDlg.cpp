...
//	ÎÄ¼þÃû			:	GMDlg.cpp
//	´´½¨ÈË			:	ÍõÎ÷±´
//	´´½¨Ê±¼ä		:	2003-6-4 14:59:49
//	ÎÄ¼þËµÃ÷		:	²âÊÔ¶Ô»°¿òÀàµÄÊµÏÖ
//////////////////////////////////////////////////////////////////////////////////////
...
CGMDlg::CGMDlg(CWnd *pParent /*=NULL*/) : CDialog(CGMDlg::IDD, pParent) {
  m_pGMCoreRef = NULL;
...
#define GM_AUTO_REPLY                                                          \
  "(GM×Ô¶¯»Ø¸´)ÓÎÏ·¹ÜÀíÔ±ÒÑ¾­ÊÕµ½ÄúµÄÍ¶Ëß£¬ÇëÄÍÐÄµÈºò´¦Àí......"
...
BOOL CGMDlg::OnInitDialog() {
  ...
  GetPrivateProfileString("System", "GMAutoReply",
                          ...
                          g_szGMReturnMessage, sizeof(g_szGMReturnMessage),
                          gINI_FILENAME);
...
HCURSOR CGMDlg::OnQueryDragIcon() { return (HCURSOR)m_hIcon; }
...
void CGMDlg::OnFindPlayer() {
  ...
  if (FAILED(hr)) {
    ShowInfoString("ÔÚÓÎÏ·ÊÀ½çÀïÑ°ÕÒÍæ¼ÒÊ§°Ü !!!");
    goto KS_EXIT;
  }
  ...
  if (gw.strGameWorldName.GetLength()) {
    ...
    CString str;
    str.Format("Ê¹ÓÃÕËºÅ\"%s\"µÄÍæ¼ÒÕýÔÚÓÎÏ·ÊÀ½ç\"%s\"Àï.\n(×¢£ºÊ¹ÓÃµÄÊÇ¿ìËÙ²éÕ"
               "Ò£¬²»ÄÜ±£Ö¤ÍêÈ«ÕýÈ·)",
               m_strAccount, gw.strGameWorldName);
    ShowInfoString(str);
  } else {
    if (gw.dwIP == 0)
      ShowInfoString("¸ÃÓÃ»§²»ÔÚÏß £¡£¡");
    else
      ShowInfoString("¸ÃÕËºÅ²»´æÔÚ £¡£¡£¡");
  }
...
void CGMDlg::OnSystemDisconnect(WPARAM wParam, LPARAM lParam) {
  ...
  ShowInfoString(
      "GMÏµÍ³ºÍÖÐ×ª·þÎñÆ÷Ê§È¥ÁªÏµ £¡£¡£¡\nÇë¼ì²éÏßÂ·ºóÖØÐÂÆô¶¯GM.exe.");
#ifdef _DEBUG
  DisableDialog();
#endif
}
...
void CGMDlg::OnJailJudge(WPARAM wParam, LPARAM lParam) {
  ...
  if (nSplit >= 0 && nSplit1 > nSplit)
    m_pGMCoreRef->JailJudge(strAccount.Left(nSplit),
                            strAccount.Mid(nSplit + 1, nSplit1 - nSplit - 1),
                            strAccount.Mid(nSplit1 + 1));
}
...
void CGMDlg::OnDoScript(WPARAM wParam, LPARAM lParam) {
  ...
  int nSplit = strAccount.Find('\n');
  if (-1 != nSplit)
    m_pGMCoreRef->ExecutePlayer(strAccount.Left(nSplit),
                                strAccount.Mid(nSplit + 1));
}

void CGMDlg::OnSystemPing(WPARAM wParam, LPARAM lParam) {
  m_nBadPingCount = 0; // ping Ê§°Ü¼ÆÊýÇåÁã
}
...
void CGMDlg::OnChat(WPARAM wParam, LPARAM lParam) {
  ...
  if (nSplit >= 0 && nSplit2 > nSplit && nSplit3 > nSplit2) {
    ...
    m_pGMCoreRef->SaveMessageToDB(strChat, ((LPCSTR)strChat) + nSplit + 1,
                                  ((LPCSTR)strChat) + nSplit2 + 1,
                                  ((LPCSTR)strChat) + nSplit3 + 1);
#ifdef _DEBUG
    ...
    if (strlen(g_szGMReturnMessage))
      m_pGMCoreRef->SendMessage((LPCSTR)strChat, (LPCSTR)(lpChat + nSplit + 1),
                                g_szGMReturnMessage);
...
void CGMDlg::OnSubLogout(WPARAM wParam, LPARAM lParam) {
  ...
  CString strOut;
  strOut.Format("ÓÎÏ·ÊÀ½ç\"%s\"µÄ·þÎñÆ÷×éÖÐIPµØÖ·Îª\"%s\"µÄ»úÆ÷¶Ï¿ªÁ¬½Ó¡£\nÓÐ¿É"
                "ÄÜÊÇ³öÏÖ¹ÊÕÏ£¬Çë×¢Òâ£¡£¡",
                strGWName, _ip2a(wParam));
...
void CGMDlg::OnGetGwName() {
  ...
  if (LB_ERR != nSel)
    ...
    ShowInfoString("ÇëÑ¡ÔñÒ»¸öÓÎÏ·ÊÀ½ç.....");
  UpdateData(FALSE);
}
...
void CGMDlg::OnGetGwIp() {
  ...
  if (LB_ERR != m_lstboxGameWorlds.GetCurSel()) {
    ...
    if (m_pGMCoreRef->GetGWByName(gw, m_strGWName)) {
      ...
      ShowInfoString("Ö¸¶¨µÄÓÎÏ·ÊÀ½ç²»´æÔÚ?@!#%!#");
    UpdateData(FALSE);
  }
}
...
void CGMDlg::OnFindPlayer2() {
  ...
  hr = m_pGMCoreRef->WhichGWIsPlayerInRealTime(gw, m_strRoleName, nX, nY,
                                               nWorld, m_strAccount);
  m_strPlayerPos.Format("[%d]: %d, %d", nWorld, nX, nY);
  if (FAILED(hr)) {
    ShowInfoString("ÔÚÓÎÏ·ÊÀ½çÀï*ÊµÊ±*Ñ°ÕÒÍæ¼ÒÊ§°Ü !!!");
    goto KS_EXIT;
  }
  ...
  if (gw.strGameWorldName.GetLength()) {
    m_lstboxGameWorlds.SelectString(0, gw.strGameWorldName);
    CString str;
    str.Format("Ê¹ÓÃÕËºÅ\"%s\"µÄÍæ¼ÒÕýÔÚÓÎÏ·ÊÀ½ç\"%s\"Àï.(ÊµÊ±)", m_strAccount,
               gw.strGameWorldName);
    ShowInfoString(str);
  } else {
    if (gw.dwIP == 0)
...
void CGMDlg::OnTimer(UINT nIDEvent) {
  ...
  // ·¢ÏÖ ping Ê§°Ü£¬×·¼Ó ping
  // ÒòÎªÍøÂçµ×²ã±£Ö¤Á¬½Ó£¬ËùÒÔÕâÀïÈ¥µô [wxb 2003-7-30]
  /*	if (IS_GM_INTERCOUNT_REACHED_ADD(m_dwGMTimerTick, GM_INTERCOUNT_PING,
                                  (PING_TIMEOUT / GM_TIMER_INTERVAL) *
     m_nBadPingCount) && m_nBadPingCount > 0)
          {
                  if (m_nBadPingCount > PING_MAX_RETRY)
                  {
                          PostMessage(WM_JX_SYS_DISCONNCECT, 0, 0);	//¶ÏÏßÁË
                          goto KS_EXIT;
                  }
                  else
                  {
...
void CGMDlg::OnSendChat() {
  ...
  if (m_strMsgToBeSent.GetLength()) {
    HRESULT hr = m_pGMCoreRef->SendMessage(m_strAccount, m_strRoleName,
                                           m_strMsgToBeSent);
#ifdef _DEBUG
    if (SUCCEEDED(hr)) {
      if (m_strAllChatMsgs.GetLength())
...
void CGMDlg::OnGetRoleList() {
  ...
  if (FAILED(hr)) {
    str.Format("ÔÚÓÎÏ·ÊÀ½ç\"%s\"²éÑ¯ÕËºÅ\"%s\"µÄ½ÇÉ«Ê§°Ü £¡£¡", m_strGWName,
               m_strAccount);
  } else {
    str.Format("ÔÚÓÎÏ·ÊÀ½ç\"%s\"ÖÐÊôÓÚÕËºÅ\"%s\"µÄ½ÇÉ«ÈçÏÂ£º\n", m_strGWName,
               m_strAccount);
    if (lstRoles.GetCount() == 0)
      str += "¸ÃÕËºÅ´ÓÎ´½øÈë¹ý¸ÃÓÎÏ·ÊÀ½ç£¬Ã»ÓÐÈÎºÎ½ÇÉ«¡£";
...
void CGMDlg::OnBroadcast() {
  ...
  HRESULT hr =
      m_pGMCoreRef->BroadCastChatToGameWorlds(m_strGWName, m_strMsgToBeSent);
#ifdef _DEBUG
  if (SUCCEEDED(hr)) {
    if (m_strAllChatMsgs.GetLength())
      m_strAllChatMsgs += "\r\n";
    if (m_strGWName.GetLength())
      m_strAllChatMsgs += "{ÏµÍ³¹«¸æ}: ";
    else {
      CString str;
      str.Format("{ÓÎÏ·ÊÀ½ç<%s>¹«¸æ}", m_strGWName);
...
void CGMDlg::OnGetPosFromDb() {
  ...
  if (m_pGMCoreRef->GetPlayerPosInDB(m_strRoleName, m_strGWName, nWorld, nX,
                                     nY))
    m_strPlayerPos.Format("[%d]: %d, %d", nWorld, nX, nY);
  else
    m_strPlayerPos = "»ñÈ¡Ê§°Ü!!!";
  UpdateData(FALSE);
}
