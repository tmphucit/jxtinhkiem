...
enum enumGMGetRoleCmds {
  gm_role_entergame_position =
      0, // 获取玩家进入游戏的位置，返回 GM_GET_ROLE_DATA_SUB_ENTER_POS
};
...
protected:
  ...
  BOOL SetPlayerPosInDB(LPCSTR szRole, LPCSTR szGWName, int nWorldID, int nX,
                        int nY);
  BOOL GetPlayerPosInDB(LPCSTR szRole, LPCSTR szGWName, int &nWorldID, int &nX,
                        int &nY);
...
public:
  ...
  HRESULT GetRoleList(LPCSTR szAccount, LPCSTR szGWName,
                      CStringList &lstRoleNames);
  HRESULT PutInJail(LPCSTR szAccount, LPCSTR szRole, LPCSTR szGWName,
                    int nMinutes);
  HRESULT JailJudge(LPCSTR szAccount, LPCSTR szRole, LPCSTR szGWName);
  HRESULT SaveMessageToDB(LPCSTR szAccount, LPCSTR szRole, LPCSTR szGameWorld,
                          LPCSTR szChat);
