// S3PDB_MSAccess_Connection.cpp: implementation of the
// S3PDB_MSAccess_Connection class.
//
//////////////////////////////////////////////////////////////////////
  ...
  if (NULL == m_pCon) {
    ...
    if ((SUCCEEDED(hr)) && (NULL != pCon)) {
      ...
      try {
        ...
        TRACE("[S3PDB_MSAccess_Connection::OpenConnect]COM error: %s",
              e.ErrorMessage());
      }
    }
  }
...
bool S3PDB_MSAccess_Connection::OpenConnect(_LPDATABASEINFO) { return false; }

bool S3PDB_MSSQLServer_Connection::OpenConnect(LPCSTR) { return false; }
  ...
  if (NULL == m_pCon) {
    ...
    if ((SUCCEEDED(hr)) && (NULL != pCon)) {
      ...
      try {
        ...
        TRACE("[S3PDBConnectionPool::OpenConnect]COM error: %s",
              e.ErrorMessage());
      }
    }
  }
    ...
    if (m_pCmd) {
      ...
      if (GetFreeResult(&pResult, &pRes)) {
        if (pRes) {
          try {
            HRESULT hr =
                pRes->Open(lpszSql, _variant_t((IDispatch *)(m_pCon), true),
                           adOpenDynamic, adLockOptimistic, adCmdText);
            if (SUCCEEDED(hr)) {
              bRet = TRUE;
            }
          } catch (_com_error &e) {
            TRACE("[S3PDB_MSAccess_Connection::Do COM error: %s",
                  e.ErrorMessage());
          }
        }
...
bool S3PDB_MSAccess_Connection::DoSql(const char *lpszSql, _Recordset *pRes) {
  ...
  if ((NULL != m_pCon) && (NULL != lpszSql) && (NULL != pRes)) {
    try {
      HRESULT hr = pRes->Open(lpszSql, _variant_t((IDispatch *)(m_pCon), true),
...
#endif
      TRACE("[S3PDB_MSAccess_Connection::DoSql COM error: %s",
            e.ErrorMessage());
      bRet = FALSE;
    }
  }
...
bool S3PDB_MSAccess_Connection::QuerySql(const char *lpszSql,
...
bool S3PDB_MSAccess_Connection::GetFreeResult(S3PResultVBC **ppResult,
                                              _Recordset **ppRes) {
