...
CString g_strContent[] = {"tga", "bmp", "psd", "tgb", ""};
/////////////////////////////////////////////////////////////////////////////
// CDirTree
...
CDirTree::~CDirTree() { SaveQuickKey(); }
...
void CDirTree::SaveQuickKey() {
  if (m_strQuickKeyFile.IsEmpty())
    return;
  CStdioFile file;
  if (!AppOpenFile(file, m_strQuickKeyFile,
                   CFile::modeCreate | CFile::modeWrite, "dirtree"))
    return;
  for (int i = 0; i < 10; i++) {
...
HTREEITEM CDirTree::AddChildFolder(HTREEITEM hitem, const CString &sFolder,
                                   LPCSTR szPath) {
  TVINSERTSTRUCT t;
...
BOOL CDirTree::SelectNode(const CString &file) {
  ...
  if (file.Find('.') != -1) {
    m_strFile = file;
    GetParent()->PostMessage(WM_DIRTREE_LEFTSELECTED, (long)(LPCSTR)m_strFile,
                             (long)this);
  }
...
int CDirTree::GetImageInfo(const CString &strName) {
  if (m_bGetIconByFile) {
    ...
    if (!b) {
      CString strErr;
      strErr =
          "file " + strName + " format is not valid, not found item 'icon=' ";
...
void CDirTree::InsertFolder(const HTREEITEM father, const CString &strPath,
                            int layer) {
...
void CDirTree::OnRButtonDown(UINT nFlags, CPoint point) {
  ...
  if (h == hItem) {
    m_strFile = GetSelectedFile();
    GetParent()->PostMessage(WM_DIRTREE_RIGHTSELECTED, (long)(LPCSTR)m_strFile,
                             (long)this);
  }
}
...
void CDirTree::DeleteSelect() {
  ...
  if (h == NULL)
    return;
  if (h == GetRootItem() || GetParentItem(h) == GetRootItem() ||
      GetParentItem(GetParentItem(h)) == GetRootItem()) {
  ...
  strExt.MakeLower();
  if (strExt == ".txt") // if it is a txt file and asscial with a spr, delete
                        // the spr also!
  {
...
eIconType IconNameToID(const CString &s) {
  static CString s_strIconName[] = {
      "folder",  "folderopen", "normal",         "npc",      "box",
      "trap",    "camera",     "light",          "terra",    "diamond",
      "road",    "doodad",     "doodad_complex", "group",    "obstacle",
      "special", "setting",    "command",        "template",
