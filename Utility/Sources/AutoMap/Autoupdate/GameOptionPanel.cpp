...
void GameOptionPanel::OnOk() {
  ...
  try {
    ...
    auto_ptr<ClientGameOptionProcess> ClientGameOptionProcesser(
        new ClientGameOptionProcess());
    ...
    if (find(OptionS.begin(), OptionS.end(), ClientGameOptionID) ==
        OptionS.end()) {
    ...
    // copy(istream_iterator<string>(TextBuffer),
    // istream_iterator<string>(),ostream_iterator<string>(ConfigFileOut,"\n"));
    ...
    copy(NewOptionS.begin(), NewOptionS.end(),
         ostream_iterator<string>(ConfigFileOut, "\n"));
...
BOOL GameOptionPanel::OnInitDialog() {
  ...
  try {
    ...
    if (Represent3DModule == NULL) {
      string ErrorInfo =
          string("调入动态库'") + Represent3DModuleFileName + string("'出错");
      throw exception(ErrorInfo.c_str());
    }

    fnRepresentIsModuleRecommended Check_Represent;
    Check_Represent = (fnRepresentIsModuleRecommended)GetProcAddress(
        Represent3DModule, Check_Represent_FunctionName.c_str());

    if (Check_Represent == NULL) {
      FreeLibrary(Represent3DModule);
      string ErrorInfo = string("取得函数地址'") +
                         Check_Represent_FunctionName + string("'出错");
      throw exception(ErrorInfo.c_str());
    }
    ...
    if (!FreeLibrary(Represent3DModule)) {
      string ErrorInfo =
          string("释放模块'") + Represent3DModuleFileName + string("'出错");
      throw exception(ErrorInfo.c_str());
    }
