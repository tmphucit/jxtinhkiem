...
void DisplayErrorInfo(string &ErrorInfo) {
  ...
  // ...
  // Display the string.
  MessageBox(NULL, ErrorInfo.c_str(), (LPCTSTR)lpMsgBuf,
...
int __stdcall RefreshStatus(int nCurrentStatus, long lParam) {
  ...
  case defUPDATE_STATUS_DOWNLOADING:
    ...
    Info.nTotalPos = ConnectionProgress +
                     ((100 - ConnectionProgress) * (4 / 5)) * lParam / 100;
    Info.nCurrentPos = lParam;
    break;
  case defUPDATE_STATUS_DOWNLOADING_FILE:
    ...
    if (pFileinfo) {
      CString strFile;
      strFile.Format("%s --- %d KB(%d KB)", pFileinfo->strFileName,
                     pFileinfo->dwFileSize / 1024,
                     pFileinfo->dwFileDownloadedSize / 1024);
      strncpy(Info.szCurrentStatusInfo, strFile, MAX_PATH);
      Info.nCurrentPos = (int)(pFileinfo->dwFileDownloadedSize * 100.0 /
...
UINT AutoUpdateDLL(char *pSite) {
  ...
  UpdateSet.nVersion = ::GetPrivateProfileInt("Version", "Version", 0,
  ...
  if (g_Update_Init == NULL || g_Update_UnInit == NULL ||
      g_Update_Start == NULL)
    goto Exit0;
...
UINT UpdateThread(LPVOID p) { return AutoUpdateDLL((char *)p); }
...
BOOL CAutoupdateDlg::OnInitDialog() {
  ...
  if (!m_strWebURL.IsEmpty()) {
    ...
    //		if(result != S_OK) {
    //			result = m_web.Navigate("http://www.jxonline.net", NULL,
    // NULL, NULL, NULL);
    //		}
  }
...
BOOL CAutoupdateDlg::InitAutoUpdate() {
  ...
  IniFile.GetString("main", "gameurl", "http://www.kingsoft.net/", szWebURL,
                    MAX_PATH);
...
HCURSOR CAutoupdateDlg::OnQueryDragIcon() { return (HCURSOR)m_hIcon; }
  ...
  if (pInfo->nFinish < 0) {
    ...
    if (pInfo->nTotalPos >= 0) {
      ...
      if (pInfo->nTotalPos < ConnectionProgress) {
        TotalProgress = m_nCurrentHost * m_nConnectionStep +
                        m_nConnectionStep *
...
// DEL HRGN CAutoupdateDlg::BitmapToRegionFromHSBRange(HBITMAP& hBmp, Color
// cTransparentColor,float Hrange , float SRange,float Brange) DEL { DEL
// HRGN hRgn = NULL; DEL DEL 	if (hBmp) DEL 	{ DEL 		// Create a
// memory DC inside which we will scan the bitmap content DEL 		HDC
// hMemDC = CreateCompatibleDC(NULL); DEL 		if (hMemDC) DEL
// { DEL 			// Get bitmap size DEL 			BITMAP
// bm; DEL 			GetObject(hBmp, sizeof(bm), &bm); DEL DEL
// // Create a 32 bits depth bitmap and select it into the memory DC DEL
// BITMAPINFOHEADER RGB32BITSBITMAPINFO = { DEL
// sizeof(BITMAPINFOHEADER),	// biSize DEL
// bm.bmWidth,					// biWidth; DEL
// bm.bmHeight,				// biHeight; DEL
// 1,							// biPlanes; DEL
// 32,							// biBitCount DEL
// BI_RGB,						// biCompression; DEL
// 0,							// biSizeImage; DEL
// 0,							// biXPelsPerMeter; DEL
// 0,							// biYPelsPerMeter; DEL
// 0,							// biClrUsed; DEL
// 0							// biClrImportant; DEL
// }; DEL 			VOID * pbits32; DEL 			HBITMAP
// hbm32 = CreateDIBSection(hMemDC, (BITMAPINFO *)&RGB32BITSBITMAPINFO,
// DIB_RGB_COLORS, &pbits32, NULL, 0); DEL 			if (hbm32) DEL
// { DEL 				HBITMAP holdBmp =
// (HBITMAP)SelectObject(hMemDC, hbm32); DEL DEL
// // Create a DC just to copy the bitmap into the memory DC DEL
// HDC hDC = CreateCompatibleDC(hMemDC); DEL 				if (hDC)
// DEL 				{
// DEL 					// Get how many bytes per row we have
// for the bitmap bits (rounded up to 32 bits) DEL
// BITMAP bm32; DEL 					GetObject(hbm32,
// sizeof(bm32), &bm32); DEL 					while
// (bm32.bmWidthBytes % 4) DEL
// bm32.bmWidthBytes++; DEL DEL 					// Copy
// the bitmap into the memory DC DEL 					HBITMAP
// holdBmp = (HBITMAP)SelectObject(hDC, hBmp); DEL
// BitBlt(hMemDC, 0, 0, bm.bmWidth, bm.bmHeight, hDC, 0, 0, SRCCOPY); DEL DEL
// // For better performances, we will use the ExtCreateRegion() function to
// create the DEL 					// region. This function
// take a RGNDATA structure on entry. We will add rectangles by DEL
// // amount of ALLOC_UNIT number in this structure. DEL #define ALLOC_UNIT
// 100 DEL 					DWORD maxRects = ALLOC_UNIT; DEL
// HANDLE hData = GlobalAlloc(GMEM_MOVEABLE, sizeof(RGNDATAHEADER) +
// (sizeof(RECT) * maxRects)); DEL 					RGNDATA
// *pData = (RGNDATA *)GlobalLock(hData); DEL
// pData->rdh.dwSize = sizeof(RGNDATAHEADER); DEL
// pData->rdh.iType = RDH_RECTANGLES; DEL
// pData->rdh.nCount = pData->rdh.nRgnSize = 0; DEL
// SetRect(&pData->rdh.rcBound, MAXLONG, MAXLONG, 0, 0); DEL DEL
// // Keep on hand highest and lowest values for the "transparent" pixels DEL
// float lH  = cTransparentColor.GetH() - Hrange/2; DEL
// float hH  = cTransparentColor.GetH() + Hrange/2; DEL DEL
// float lS = cTransparentColor.GetS() - SRange/2; DEL
// float lB = cTransparentColor.GetB() - Brange/2; DEL DEL
// float hS = cTransparentColor.GetS() + SRange/2; DEL
// float hB = cTransparentColor.GetS() + Brange/2; DEL DEL
// // Scan each bitmap row from bottom to top (the bitmap is inverted
// vertically) DEL 					BYTE *p32 = (BYTE
// *)bm32.bmBits + (bm32.bmHeight - 1) * bm32.bmWidthBytes; DEL
// for (int y = 0; y < bm.bmHeight; y++) DEL { DEL
// // Scan each bitmap pixel from left to right DEL
// for (int x = 0; x < bm.bmWidth; x++) DEL
// { DEL 							// Search for a
// continuous range of "non transparent pixels" DEL
// int x0 = x; DEL 							LONG *p
// = (LONG *)p32 + x; DEL
// while (x < bm.bmWidth) DEL { DEL
// /*	BYTE b = GetRValue(*p); DEL
// if (b >= lr && b <= hr) DEL { DEL
// b = GetGValue(*p); DEL
// if (b >= lg && b <= hg) DEL { DEL
// b = GetBValue(*p); DEL
// if (b >= lb && b <= hb) DEL
// // This pixel is "transparent" DEL
// break; DEL 							} DEL DEL DEL
// } DEL 								*/ DEL
// BYTE  r = GetRValue(*p); DEL
// BYTE  g = GetGValue(*p); DEL
// BYTE  b = GetBValue(*p); DEL DEL
// Color color(r,g,b); DEL DEL DEL
// if(((color.GetH() > lH) && (color.GetH() < hH)) && (color.GetS() > lS &&
// color.GetS() < hS ) && (color.GetB() > lB && color.GetB() < hB ) ) DEL
// { DEL
// //透明的颜色 DEL
// break; DEL 								} DEL
// DEL 								p++;
...
// DEL 							{
// DEL 								// Add the
// pixels (x0, y) to (x, y+1) as a new rectangle in the region DEL
// if (pData->rdh.nCount >= maxRects) DEL
// { DEL
// GlobalUnlock(hData); DEL
// maxRects += ALLOC_UNIT; DEL
// hData = GlobalReAlloc(hData, sizeof(RGNDATAHEADER) + (sizeof(RECT) *
// maxRects), GMEM_MOVEABLE); DEL
// pData = (RGNDATA *)GlobalLock(hData); DEL
// } DEL 								RECT *pr
// = (RECT *)&pData->Buffer; DEL
// SetRect(&pr[pData->rdh.nCount], x0, y, x, y+1); DEL
// if (x0 < pData->rdh.rcBound.left) DEL
// pData->rdh.rcBound.left = x0; DEL
// if (y < pData->rdh.rcBound.top) DEL
// pData->rdh.rcBound.top = y; DEL
// if (x > pData->rdh.rcBound.right) DEL
// pData->rdh.rcBound.right = x; DEL
// if (y+1 > pData->rdh.rcBound.bottom) DEL
// pData->rdh.rcBound.bottom = y+1; DEL
// pData->rdh.nCount++; DEL DEL
// // On Windows98, ExtCreateRegion() may fail if the number of rectangles is
// too DEL 								// large
// (ie: > 4000). Therefore, we have to create the region by multiple steps. DEL
// if (pData->rdh.nCount == 2000) DEL
// { DEL
// HRGN h = ExtCreateRegion(NULL, sizeof(RGNDATAHEADER) + (sizeof(RECT) *
// maxRects), pData); DEL
// if (hRgn) DEL { DEL
// CombineRgn(hRgn, hRgn, h, RGN_OR); DEL
// DeleteObject(h); DEL
// } DEL else DEL
// hRgn = h; DEL
// pData->rdh.nCount = 0; DEL
// SetRect(&pData->rdh.rcBound, MAXLONG, MAXLONG, 0, 0); DEL
// } DEL 							} DEL
// } DEL DEL 						// Go to next row
// (remember, the bitmap is inverted vertically) DEL
// p32 -= bm32.bmWidthBytes; DEL 					} DEL
// DEL 					// Create or extend the region with the
// remaining rectangles DEL 					HRGN h =
// ExtCreateRegion(NULL, sizeof(RGNDATAHEADER) + (sizeof(RECT) * maxRects),
// pData); DEL 					if (hRgn) DEL
// { DEL 						CombineRgn(hRgn, hRgn,
// h, RGN_OR); DEL 						DeleteObject(h);
// DEL 					}
// DEL 					else
// DEL 						hRgn = h;
// DEL
...
// DEL  void CAutoupdateDlg::DrawAText(CDC * DeviceHandle,int nHeight, int
// nWidth, int nOrientation, int nWeight, DWORD dwItalic, DWORD dwUnderline,
// DWORD dwStrikeOut, const CString &CSrFace, COLORREF colColor, COLORREF
// colBackColor, int nBackMode,const CString &CSTextString,CRect& Range) DEL  {
// DEL 	 HFONT  TextFont = CreateFont(nHeight , nWidth , nOrientation
// ,nOrientation , nWeight, dwItalic, dwUnderline, dwStrikeOut, DEFAULT_CHARSET
// ,OUT_DEFAULT_PRECIS,CLIP_DEFAULT_PRECIS,ANTIALIASED_QUALITY,DEFAULT_PITCH
// ,CSrFace); DEL DEL DEL 	 HGDIOBJ  OldFont      =
// DeviceHandle->SelectObject(TextFont); DEL 	 COLORREF OldBkColor   =
// DeviceHandle->SetBkColor(colBackColor); DEL 	 COLORREF OldTextColor =
// DeviceHandle->SetTextColor(colColor); DEL DEL 	 int      OldTextMode  =
// DeviceHandle->SetBkMode(nBackMode); DEL DEL DEL DEL DEL
// DeviceHandle->DrawText(CSTextString,Range,DT_LEFT); DEL DEL
// DeviceHandle->SetBkMode(OldTextMode); DEL
// DeviceHandle->SetTextColor(OldTextColor); DEL
// DeviceHandle->SetBkColor(OldBkColor); DEL
// DeviceHandle->SelectObject(OldFont); DEL DEL 	 DeleteObject(TextFont);
// DEL
// DEL  }
...
// DEL  HRGN CAutoupdateDlg::BitmapToRegionFromAlphaValueRange(HBITMAP &hBmp,
// unsigned char ColorValueRange) DEL  { DEL 	 HRGN hRgn = NULL; DEL DEL
// if (hBmp) DEL 	 { DEL 		 // Create a memory DC inside which we
// will scan the bitmap content DEL 		 HDC hMemDC =
// CreateCompatibleDC(NULL); DEL 		 if (hMemDC) DEL { DEL
// // Get bitmap size DEL 			 BITMAP bm; DEL
// GetObject(hBmp, sizeof(bm), &bm); DEL DEL 			 // Create a 32
// bits depth bitmap and select it into the memory DC DEL
// BITMAPINFOHEADER RGB32BITSBITMAPINFO = { DEL
// sizeof(BITMAPINFOHEADER),	// biSize DEL
// bm.bmWidth,					// biWidth; DEL
// bm.bmHeight,				// biHeight; DEL
...
// (HBITMAP)SelectObject(hMemDC, hbm32); DEL DEL
// // Create a DC just to copy the bitmap into the memory DC DEL
// HDC hDC = CreateCompatibleDC(hMemDC); DEL 				 if
// (hDC) DEL 				 { DEL
// // Get how many bytes per row we have for the bitmap bits (rounded up to 32
// bits) DEL 					 BITMAP bm32; DEL
// GetObject(hbm32, sizeof(bm32), &bm32); DEL
// while (bm32.bmWidthBytes % 4) DEL
// bm32.bmWidthBytes++; DEL DEL 					 // Copy
// the bitmap into the memory DC DEL 					 HBITMAP
// holdBmp = (HBITMAP)SelectObject(hDC, hBmp); DEL
// BitBlt(hMemDC, 0, 0, bm.bmWidth, bm.bmHeight, hDC, 0, 0, SRCCOPY); DEL DEL
// // For better performances, we will use the ExtCreateRegion() function to
// create the DEL 					 // region. This
// function take a RGNDATA structure on entry. We will add rectangles by DEL
// // amount of ALLOC_UNIT number in this structure. DEL #define ALLOC_UNIT
// 1 DEL 					 DWORD maxRects = ALLOC_UNIT;
// DEL 					 HANDLE hData =
// GlobalAlloc(GMEM_MOVEABLE, sizeof(RGNDATAHEADER) + (sizeof(RECT) *
// maxRects)); DEL 					 RGNDATA *pData =
// (RGNDATA *)GlobalLock(hData); DEL
// pData->rdh.dwSize = sizeof(RGNDATAHEADER); DEL
// pData->rdh.iType = RDH_RECTANGLES; DEL
// pData->rdh.nCount = pData->rdh.nRgnSize = 0; DEL
// SetRect(&pData->rdh.rcBound, MAXLONG, MAXLONG, 0, 0); DEL DEL DEL
// long MaxColor = ColorValueRange; DEL
// // Scan each bitmap row from bottom to top (the bitmap is inverted
// vertically) DEL 					 BYTE *p32 = (BYTE
// *)bm32.bmBits + (bm32.bmHeight - 1) * bm32.bmWidthBytes; DEL
// for (int y = 0; y < bm.bmHeight; y++) DEL { DEL
...
// DEL 								 {
// DEL 									 // Add
// the pixels (x0, y) to (x, y+1) as a new rectangle in the region DEL
// if (pData->rdh.nCount >= maxRects) DEL
// { DEL
// GlobalUnlock(hData); DEL
// maxRects += ALLOC_UNIT; DEL
...
// // On Windows98, ExtCreateRegion() may fail if the number of rectangles is
// too DEL
// // large (ie: > 4000). Therefore, we have to create the region by multiple
// steps. DEL
// if (pData->rdh.nCount == 2000) DEL
// { DEL
// HRGN h = ExtCreateRegion(NULL, sizeof(RGNDATAHEADER) + (sizeof(RECT) *
...
// } DEL 								 } DEL
// } DEL DEL 							 // Go to next
// row (remember, the bitmap is inverted vertically) DEL
// p32 -= bm32.bmWidthBytes; DEL } DEL DEL
// // Create or extend the region with the remaining rectangles DEL
// HRGN h = ExtCreateRegion(NULL, sizeof(RGNDATAHEADER) + (sizeof(RECT) *
// maxRects), pData); DEL 						 if
// (hRgn) DEL 						 { DEL
// CombineRgn(hRgn, hRgn, h, RGN_OR); DEL
// DeleteObject(h); DEL 						 } DEL
// else DEL 							 hRgn = h; DEL
// DEL 						 // Clean up
// DEL 						 GlobalFree(hData);
// DEL 						 SelectObject(hDC, holdBmp);
...
// DEL 					 }
// DEL
// DEL 					 DeleteObject(SelectObject(hMemDC,
// holdBmp)); DEL 			} DEL DEL
// DeleteDC(hMemDC); DEL 		} DEL 	} DEL 	return hRgn; DEL DEL DEL
// }
...
/*void CAutoupdateDlg::OnMouseMove(UINT nFlags, CPoint point)
{
        ...
        CPoint WindowMove = point - m_MouseClickPosition ;
                ...
                this->MoveWindow(WindowMove.x
,WindowMove.y,WindowRect.right,WindowRect.bottom,TRUE);
...
void CAutoupdateDlg::OnRegister() {
  ...
  ShellExecute(NULL, "open", "\\\\s\\自动更新\\剑侠情缘3网上交易平台.htm", NULL,
               NULL, SW_SHOW);
}
...
void CAutoupdateDlg::LoadBitMapFromInstance(CBitmap &Dest, HINSTANCE Hinstance,
                                            WORD wResourceID) {
...
// DEL void CAutoupdateDlg::DrawInfoOnStaticPositionToDC(CDC * DeviceHandle,WORD
// CtlID, CString& Text) DEL { DEL 	    CWnd* pWnd; DEL 		CRect
// rcText; DEL 		pWnd = GetDlgItem(CtlID); DEL
// pWnd->GetWindowRect(&rcText); DEL 		ScreenToClient( &rcText ); DEL
// //ClientToInfoRegion(rcText); DEL
// DrawAText(DeviceHandle,12,6,0,100,FALSE,FALSE,FALSE,"仿宋",RGB(102,101,98),0,TRANSPARENT,Text,rcText);
// DEL
// DEL }
