...
void CNetClientApp::OnQuit() { GetWorld()->Client_Quit(); }

iClient *AppGetClient() { return theApp.GetClient(); }

cWorld *AppGetWorld() { return theApp.GetWorld(); }

void OutputString(LPCSTR szAccount, LPCSTR szMessage) {
  theApp.OutputString(szAccount, szMessage);
}

void EnableNetStatus(NetStatus e, BOOL b) { theApp.EnableNetStatus(e, b); }
