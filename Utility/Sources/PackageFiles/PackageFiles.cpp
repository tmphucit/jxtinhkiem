...
char
    compress_buffer[COMPRESS_BUF_SIZE]; // 10M的压缩缓冲区，存放所有的帧，一次写
  ...
  if (map.m_Ptr) {
    ...
    if (!bSPR || map.m_Size < FRAME_SIZE) {
      if (compress_type == TYPE_UCL) {
        ...
        //*)&index_list[index].compress_size, map.m_Ptr, map.m_Size, 9, 0, 30);
      }
      if (r)
        return false;
      ...
      for (frame_index = 0; frame_index < head->Frames; frame_index++) {
        ...
        if (frame_info[frame_index].Length >= 256) { // 小于256字节的不压缩
          if (compress_type == TYPE_UCL) {
            r = ucl_nrv2b_99_compress((BYTE *)frame_data +
                                          frame_info[frame_index].Offset,
                                      frame_info[frame_index].Length,
                                      (BYTE *)ptr, &size, NULL, 10, NULL, NULL);
          } else if (compress_type == TYPE_BZIP2) {
            //						size =
            // COMPRESS_BUF_SIZE;
            // r = BZ2_bzBuffToBuffCompress(ptr, &size, frame_data +
...
void addDirectory(FILE *output, const char *rootDir,
                  const char *subDir = NULL) {
  char szRealDir[MAX_PATH];
  if (subDir)
    sprintf(szRealDir, "%s\\%s", rootDir, subDir);
  else {
    strcpy(szRealDir, rootDir);
    root_length = strlen(rootDir) - 1;
    while (rootDir[root_length] != '\\')
      root_length--;
  }
  if (chdir(szRealDir))
    return;
...
__declspec(dllexport) bool pack(vector<string> &FileList,
                                const string &RootPath, string &PackName) {
  try {
    ...
    if (output == NULL) {
      string ErrorInfo = string("Error when Create Pack File! Expression is ") +
                         string("fopen('") + PackName + string("',wb)");
