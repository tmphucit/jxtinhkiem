...
COutputList::COutputList() { m_bCreate = false; }
...
CString g_aStatusString[] = {
    "OK",   "Succeed", "Failure", "Warnning", "Error",
    "Hint", "Help",    "Chat",    "Next",
};
...
BOOL COutputList::Create() {
  ...
  */
  ...
  CRect rect;
  GetWindowRect(&rect);
  int rt = InsertColumn(0, strItem1, LVCFMT_LEFT, statusWidth, 0);
  ASSERT(rt != -1);
  rt = InsertColumn(1, strItem2, LVCFMT_LEFT, rect.Width() - statusWidth - 10,
                    1);
  ASSERT(rt != -1);

  for (itOutputItem it = m_listOutputItem.begin(); it != m_listOutputItem.end();
       ++it)
    MessageItem(*it);
