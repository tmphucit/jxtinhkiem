...
CWorldEditorView::CWorldEditorView() : CFormView(CWorldEditorView::IDD) {
...
#ifdef _DEBUG
void CWorldEditorView::AssertValid() const { CFormView::AssertValid(); }

void CWorldEditorView::Dump(CDumpContext &dc) const { CFormView::Dump(dc); }
...
BOOL CWorldEditorView::OnRightSelectedFile(LPCSTR szFile) { return TRUE; }
...
BOOL CWorldEditorView::OnSelectedFile(LPCSTR szFile) {
  ...
  if (!m_staticSpr.SetSprFile(sValue)) {
    CString strMsg;
    strMsg.Format("file %s 和 file %s 的关联出现错误,无法使用该文件!", szFile,
                  (LPCSTR)sValue);
...
LRESULT CWorldEditorView::WindowProc(UINT message, WPARAM wParam,
                                     LPARAM lParam) {
  CDirTree *pTree = NULL;
  switch (message) {
...
HRESULT CWorldEditorView::OutputMessage(LPCSTR szMessage) { return 0; }
