...
public:
  ...
  // simple interface
  BOOL Unzip(LPCTSTR szFileName, LPCTSTR szFolder = NULL,
             BOOL bIgnoreFilePath = FALSE);

  // works with prior opened zip
  BOOL Unzip(BOOL bIgnoreFilePath =
                 FALSE); // unzips to output folder or sub folder with zip name
  BOOL UnzipTo(LPCTSTR szFolder,
               BOOL bIgnoreFilePath = FALSE); // unzips to specified folder
  ...
  BOOL UnzipFile(int nFile, LPCTSTR szFolder = NULL,
                 BOOL bIgnoreFilePath = FALSE);
