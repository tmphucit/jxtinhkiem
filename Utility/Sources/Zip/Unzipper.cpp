...
BOOL CUnzipper::Unzip(LPCTSTR szFileName, LPCTSTR szFolder,
...
BOOL CUnzipper::GetFileInfo(UZ_FileInfo &info) {
  ...
  if (UNZ_OK != unzGetCurrentFileInfo(m_uzFile, &uzfi, info.szFileName,
                                      MAX_PATH, NULL, 0, info.szComment,
                                      MAX_COMMENT))
    return FALSE;
...
BOOL CUnzipper::UnzipFile(LPCTSTR szFolder, BOOL bIgnoreFilePath) {
  ...
  HANDLE hOutputFile = ::CreateFile(szFilePath, GENERIC_WRITE, 0, NULL,
...
BOOL CUnzipper::SetFileModTime(LPCTSTR szFilePath, DWORD dwDosDate) {
  HANDLE hFile = CreateFile(szFilePath, GENERIC_READ | GENERIC_WRITE, 0, NULL,
  ...
  if (bRes)
    bRes = DosDateTimeToFileTime((WORD)(dwDosDate >> 16), (WORD)dwDosDate,
                                 &ftLocal);
...
DWORD CUnzipper::GetUncompressedSize() {
  int n = GetFileCount();
  DWORD size = 0;
  UZ_FileInfo info;
  for (int i = 0; i < n; i++) {
    GotoFile(i);
    GetFileInfo(info);
    size += info.dwUncompressedSize;
  }
  return size;
}

bool CUnzipper::LocateFile(const char *name) {
  if (!m_uzFile)
    return FALSE;

  int ret;

  ret = unzLocateFile(m_uzFile, name, 0);

  return (ret == UNZ_OK);
}

int CUnzipper::GetFileLength() {
  if (!m_uzFile)
    return -1;

  unz_file_info fi;

  unzGetCurrentFileInfo(m_uzFile, &fi, NULL, 0, NULL, 0, NULL, 0);

  return fi.uncompressed_size;
}

//*bufsize file size, return read size
int CUnzipper::ReadFromFile(void *buf, int *bufsize) {
  if (!m_uzFile)
    return -1;
  ...
  int cb;
  cb = unzReadCurrentFile(m_uzFile, buf, *bufsize);

  if (cb != len) {
    //"error while reading zip file, maybe IO error or buf not large enough";
  }
