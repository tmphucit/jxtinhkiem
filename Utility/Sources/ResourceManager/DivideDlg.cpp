...
void CDivideDlg::SaveDivide(stDivide &divide) {
  ...
  sName += ".div";
  CFile file;
  VERIFY(AppOpenFile(file, sName, CFile::modeCreate | CFile::modeWrite,
                     "dividedlg"));
...
void CDivideDlg::OnDivide() {
  ...
  for (int i = 0; i < divide.nPoint - 1; i++) {
    ...
    if (fabs(divide.aPoint[i + 1].y - divide.aPoint[i].y) + divide.nHeight >
        c_nMaxHeight) {
      CString s;
...
void CDivideDlg::Divide(stDivide &divide) {
  ...
  for (int y = 0; y <= h; y++)
    for (int x = 0; x < divide.nPoint - 1; x++) {
      ...
      SetItemValue(pItemTxt, GetTablePlaneName(TP_PLANETYPE), "垂直于地表",
                   index);
      SetItemValue(pItemTxt, GetTablePlaneName(TP_LIGHTING), "按照切分受光",
                   index);
      SetItemValue(pItemTxt, GetTablePlaneName(TP_LIGHTED), "按照底边挡光",
                   index);
      SetItemValue(pItemTxt, GetTablePlaneName(TP_ANGLEZ), "90", index);
      SetItemValue(pItemTxt, GetTablePlaneName(TP_KIND), "组件的垂直部分",
                   index);
      ...
      cPoint pt = aPoint[x];
      //		pt.z = ((h-y) * c_nMaxHeight - (height -
      // max(divide.aPoint[i].y,divide.aPoint[i+1].y))) / c_ptScale.z;
  ...
  aPoint[i].y = (divide.aPoint[i].y - divide.aPoint[0].y) / c_ptScale.y;
  aPoint[i + 1].x =
      ((aPoint[0].x + aPoint[0].y) + (aPoint[i].x - aPoint[i].y)) / 2.f;
  aPoint[i + 1].y =
      ((aPoint[0].x + aPoint[0].y) - (aPoint[i].x - aPoint[i].y)) / 2.f;
  aPoint[i + 2] = aPoint[0];
  ...
  if (bShadow) {
    SetItemValue(pItemTxt, GetTablePlaneName(TP_PLANETYPE), "平行于地表",
                 nPoint);
...
void CDivideDlg::OnCancel() { CDialog::OnCancel(); }
...
void CDivideDlg::OnUp() { OffsetPoint(CPoint(0, -1)); }

void CDivideDlg::OnLeft() { OffsetPoint(CPoint(-1, 0)); }

void CDivideDlg::OnDown() { OffsetPoint(CPoint(0, 1)); }

void CDivideDlg::OnRight() { OffsetPoint(CPoint(1, 0)); }
...
void CDivideDlg::OnSelchangeGeometry() { UpdatePointArray(); }

void CDivideDlg::OnEditchangeHeight() { UpdatePointArray(); }

void CDivideDlg::OnSelchangeHeight() { UpdatePointArray(); }
...
void CDivideDlg::OnSelchangeObstacle() { UpdatePointArray(); }
