...
void CPingThread::SetTimeOut(int nTimeOut) { m_nPingTimeOut = nTimeOut; }

void CPingThread::SetContext(ULONG ulContext) { m_ulContextWith = ulContext; }
...
int CPingThread::StartPingThread() {
  ...
  m_hPingThread =
      ::CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)&PingThreadProc,
                     (LPVOID)this, 0, &m_ulPingThreadId);
...
HANDLE CPingThread::GetThreadHandle() { return m_hPingThread; }
...
int CPingThread::DoStatusNotify(ULONG ulOccurEvent, ULONG ulOccurParam) {
  ...
  if (m_piCallback != NULL) {
    nResult =
        m_piCallback->PingStatus(m_ulContextWith, ulOccurEvent, ulOccurParam);
  }
...
ULONG CPingThread::MainExecution() {
  ...
  while (!nPingStop) {
    ...
    nRetCode =
        m_piCallback->GetNextUrl(&ulContextWith, sizeof(szPingUrl), szPingUrl);
    if (nRetCode <= 0) {
      if (0 == nRetCode) {
        ASSERT(FALSE);
        continue;
    ...
    while (nPingLeft-- > 0) {
      ...
      if (PINGRESULT_SUCCESS == nRetCode) {
        ...
        DoStatusNotify(PINGEVENT_OCCUROVER, nPingUsedTime);
        continue;
      }
      if (PINGRESULT_TIMEOUT == nRetCode) {
        nPingUsedTime += nPingTime;

        DoStatusNotify(PINGEVENT_TIMEOUT, nPingUsedTime);
        continue;
      }
