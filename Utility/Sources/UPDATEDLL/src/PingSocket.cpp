...
CPingSocket::CPingSocket() : CRawSocket() {
...
int CPingSocket::PingToAddress(const INET_ADDRESS *psaRemoteAddr,
                               int &nTimeOut) {
  ...
  EchoRequest.icmpHdr.Checksum =
      inet_chksum((unsigned short *)pSendBuf, nSendSize);
  ...
  while (nTimeLeft > 0) {
    ...
    if ((nRetCode >= (sizeof(IPHDR) + sizeof(ICMPHDR))) &&
      ...
      if ((nRetCode >= (sizeof(IPHDR) + sizeof(PING_ECHOREQUEST))) &&
...
int CPingSocket::GetLastPingTime() { return m_nLastPingTime; }

int CPingSocket::OnPingDataSend(int *pnBufSize, unsigned char **ppbyDataSend) {
  return true;
}

int CPingSocket::OnPingDataReceive(int *pnBufSize,
                                   unsigned char **pbyDataReceive) {
  return true;
}
...
void CPingSocket::OnErrorHandler() { DoEventNotify(PINGEVENT_OCCURERROR); }
