...
CString GetLogPathName() {
  ...
  MkDirEx(sPathName);
      ...
      sPathName = (CString)g_PathTable.szModulePath + "LOGS\\" +
  (CString)szUserName + "Update.LOG";
  */
...
CString GetCurTimeString(BOOL bYear) {
  ...
  if (bYear) {
    sMes.Format("%04d-%02d-%02d %02d:%02d:%02d", now.GetYear(), now.GetMonth(),
                now.GetDay(), now.GetHour(), now.GetMinute(), now.GetSecond());
  } else {
    sMes.Format("%02d:%02d:%02d", now.GetHour(), now.GetMinute(),
                now.GetSecond());
  }
...
BOOL KSaveLog::WriteLogEnd() { return TRUE; }
