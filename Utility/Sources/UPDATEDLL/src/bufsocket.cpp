...
//			析构函数
////////////////////////////////////////////////////////////////////////////////
CBufSocket::~CBufSocket() { Close(TRUE); }
...
BOOL CBufSocket::InitLibrary(int nMajorVer /*= 2*/, int nMinorVer /*= 2*/) {
  ...
  if (LOBYTE(WSD.wVersion) != nMinorVer || HIBYTE(WSD.wVersion) != nMajorVer) {
    WSACleanup();
    return FALSE;
  }
...
//	返回值：void
////////////////////////////////////////////////////////////////////////////////
void CBufSocket::CleanupLibrary() { WSACleanup(); }
...
//	返回值：int，最近一次的操作的错误码
////////////////////////////////////////////////////////////////////////////////
int CBufSocket::GetLastError(void) { return m_nErrorCode; }
...
//	返回值：void
////////////////////////////////////////////////////////////////////////////////
void CBufSocket::SetLastError(int nErrorCode) { m_nErrorCode = nErrorCode; }
  ...
  if (!bHardClose) // 优雅关闭 Graceful close
  {
    ...
    do {
      if (hSocketEvent != NULL) {
        ...
        WSAWaitForMultipleEvents(1, &hSocketEvent, TRUE,
                                 DEFAULT_SHUTDOWN_RECV_TIMEOUT, TRUE);
        // 清除网络事件
...
BOOL CBufSocket::SetSocketOption(SOCKET hSocket) {
  ...
  // setsockopt(hSocket, SOL_SOCKET, SO_KEEPALIVE, (const char *)
  // &nActivate,sizeof(nActivate));
  return TRUE;
}
...
int CBufSocket::RecvOnce(SOCKET hSocket, OUT char *pszBuffer, int nBufferSize) {
  ...
  return ((WSARecv(hSocket, &WSABuff, 1, &dwRecvBytes, &dwFlags, NULL, NULL) ==
           SOCKET_SUCCESS)
              ? (int)dwRecvBytes
              : -WSAGetLastError());
}
...
//		<0			: 失败(SOCKET_FAIL: -1)
////////////////////////////////////////////////////////////////////////////////
int CBufSocket::RecvData(SOCKET hSocket, OUT char *pszBuffer, int nBufferSize,
                         DWORD dwTimeout) {
  ...
  for (;;) {
    ...
    // dwWaitResult = WSAWaitForMultipleEvents(1, &hReadEvent, TRUE,dwTimeout,
    // TRUE);
...
//	返回值：int，接收到的字节数
////////////////////////////////////////////////////////////////////////////////
int CBufSocket::Recv(OUT char *pszBuffer, int nBufferSize,
                     DWORD dwTimeout /*= DEFAULT_RECV_TIMEOUT*/) {
  int nRecvBytes = 0, nRecvCurrent = 0;

  while (nRecvBytes < nBufferSize) {
    nRecvCurrent = RecvData(m_hSocket, (pszBuffer + nRecvBytes),
                            (nBufferSize - nRecvBytes), dwTimeout);
...
//		<0			: 失败，错误码的相反数
////////////////////////////////////////////////////////////////////////////////
int CBufSocket::SendOnce(SOCKET hSocket, const char *pszBuffer,
                         int nBufferSize) {
  DWORD dwSendBytes = 0;
  WSABUF WSABuff;
  ...
  return ((WSASend(hSocket, &WSABuff, 1, &dwSendBytes, 0, NULL, NULL) ==
           SOCKET_SUCCESS)
              ? (int)dwSendBytes
              : -WSAGetLastError());
}
...
//		<0			: 失败(SOCKET_FAIL:-1)
////////////////////////////////////////////////////////////////////////////////
int CBufSocket::SendData(SOCKET hSocket, const char *pszBuffer, int nBufferSize,
                         DWORD dwTimeout) {
  ...
  for (;;) {
    ...
    // 等待事件发生
    DWORD dwWaitResult =
        WSAWaitForMultipleEvents(1, &hWriteEvent, TRUE, dwTimeout, TRUE);
...
//	返回值：int，已经成功发送的字节数
////////////////////////////////////////////////////////////////////////////////
int CBufSocket::Send(const char *pszBuffer, int nBufferSize,
                     DWORD dwTimeout /*= DEFAULT_SEND_TIMEOUT*/) {
  int nSendBytes = 0, nSendCurrent = 0;

  while (nSendBytes < nBufferSize) {
    nSendCurrent = SendData(m_hSocket, (pszBuffer + nSendBytes),
                            (nBufferSize - nSendBytes), dwTimeout);
    if (nSendCurrent < 0)
      return (nSendBytes);
...
//				DWORD dwTimeout /*= DEFAULT_RECV_TIMEOUT*/ )
//	用
// 途：从缓冲区读取一行(包括单换行0x0A)(注意，结尾的回车换行不包括在其中)
//	对全局变量的影响：无
//	参  数：
//		pszBuffer   : 接收缓冲区
//		nBufferSize : 缓冲大小
...
////////////////////////////////////////////////////////////////////////////////
int CBufSocket::BSDGetString(OUT char *pszBuffer, int nBufferSize,
                             DWORD dwTimeout /*= DEFAULT_RECV_TIMEOUT*/) {
  if (m_pBSD == NULL)
    return (SOCKET_FAIL);
...
//		<0			: 失败(SOCKET_FAIL:-1)
////////////////////////////////////////////////////////////////////////////////
int CBufSocket::BSDGetData(OUT char *pszBuffer, int nBufferSize,
                           DWORD dwTimeout /*= DEFAULT_RECV_TIMEOUT*/) {
  if (m_pBSD == NULL)
    return (SOCKET_FAIL);
  ...
  {
    nHeadSize =
        Min(m_pBSD->nBufferSize - m_pBSD->nReadIndex, m_pBSD->nBytesInBuffer);
    memcpy(pszBuffer + nReadBytes, m_pBSD->pszBuffer + m_pBSD->nReadIndex,
           nHeadSize);
  ...
  {
    nHeadSize =
        Min(m_pBSD->nBufferSize - m_pBSD->nReadIndex, nBufferSize - nReadBytes);
    memcpy(pszBuffer + nReadBytes, m_pBSD->pszBuffer + m_pBSD->nReadIndex,
           nHeadSize);
...
BOOL CBufSocket::GetSockName(OUT char *pszIP, OUT int &nPort) {
  ...
  if (getsockname(m_hSocket, (SOCKADDR *)&sockAddr, &nSockAddrLen) ==
      SOCKET_ERROR) {
    SetLastError(WSAGetLastError());
    return FALSE;
  }
...
//	用
// 途：域名解析线程，引入该函数的目的是为了避免NT下对域名解析的CACHE造成的问题
//	对全局变量的影响：无
//	参  数：
//		pParam : 待解析的域名
  ...
  if (bFixNtDNS) {
    ...
    if (GetVersionEx(&osVersion)) {
      if (osVersion.dwPlatformId == VER_PLATFORM_WIN32_NT) {
        DWORD dwThreadId = 0;
        HANDLE hThread =
            CreateThread(NULL, 0, DNSThread, (LPVOID)pszName, 0, &dwThreadId);
        if (hThread != NULL) {
...
//	返回值：BOOL
////////////////////////////////////////////////////////////////////////////////
BOOL CBufSocket::Connect(SOCKET hSocket, const struct sockaddr *pSocketAddr,
  ...
  // 注册FD_CONNECT事件
  if (WSAEventSelect(hSocket, (WSAEVENT)hConnectEvent, FD_CONNECT) ==
      SOCKET_ERROR) {
  ...
  int nConnectResult =
      WSAConnect(hSocket, pSocketAddr, nAddrLen, NULL, NULL, NULL, NULL);
  int nConnectError = WSAGetLastError();

  if ((nConnectResult == SOCKET_ERROR) && (nConnectError == WSAEWOULDBLOCK)) {
    DWORD dwWaitResult =
        WSAWaitForMultipleEvents(1, &hConnectEvent, TRUE, dwTimeout, TRUE);
...
//	返回值：BOOL
////////////////////////////////////////////////////////////////////////////////
BOOL CBufSocket::Connect(const char *pszServer, int nPort,
                         DWORD dwTimeout /*= DEFAULT_CONNECT_TIMEOUT*/,
                         BOOL bFixNtDNS /*= FALSE*/) {
  SOCKADDR_IN sockAddr;
  ZeroMemory(&sockAddr, sizeof(sockAddr));
...
//	返回值：SOCKET
////////////////////////////////////////////////////////////////////////////////
SOCKET CBufSocket::GetSocket() { return m_hSocket; }
...
//	返回值：SOCKET
////////////////////////////////////////////////////////////////////////////////
SOCKET CBufSocket::Accept(struct sockaddr *pSocketAddr, int *nAddrLen,
                          DWORD dwTimeout /*= DEFAULT_ACCEPT_TIMEOUT*/) {
  ...
  // 注册FD_ACCEPT事件
  if (WSAEventSelect(m_hSocket, (WSAEVENT)hAcceptEvent, FD_ACCEPT) ==
      SOCKET_ERROR) {
  ...
  if ((hSocketAccept == INVALID_SOCKET) && (nConnectError == WSAEWOULDBLOCK)) {
    ...
    // 阻塞
    DWORD dwWaitResult =
        WSAWaitForMultipleEvents(1, &hAcceptEvent, TRUE, dwTimeout, TRUE);
    ...
    if (dwWaitResult == WSA_WAIT_EVENT_0) {
      ...
      ///////////////////////////////////////////////////////////////
      WSANETWORKEVENTS NetEvent;
      if (WSAEnumNetworkEvents(m_hSocket, hAcceptEvent, &NetEvent) ==
          SOCKET_ERROR)
        SetLastError(WSAGetLastError());
...
SOCKET CBufSocket::Accept(struct sockaddr *pSocketAddr, int *nAddrLen,
  ...
  if ((hSocketAccept == INVALID_SOCKET) && (nConnectError == WSAEWOULDBLOCK)) {
    // 阻塞
    DWORD dwWaitResult =
        WSAWaitForMultipleEvents(2, hEvent, FALSE, dwTimeout, TRUE);
    if (dwWaitResult == WSA_WAIT_EVENT_0) {
      //////////////////////////////////////////////////////////////
      ///	注意：即使 dwWaitResult == WSA_WAIT_EVENT0 ，也应该
      ///			进一步检查网络是否发生错误
...
//	返回值：BOOL
////////////////////////////////////////////////////////////////////////////////
BOOL CBufSocket::Bind(SOCKET hSocket, const struct sockaddr *pSocketAddr,
                      int nAddrLen) {
...
//	返回值：int
////////////////////////////////////////////////////////////////////////////////
int CBufSocket::RecvFrom(SOCKET hSocket, struct sockaddr *pFrom, int nFromlen,
                         char *pszBuffer, int nBufferSize, DWORD dwTimeout) {
  HANDLE hReadEvent = CreateEvent(NULL, FALSE, FALSE, NULL);
  if (hReadEvent == NULL) {
    SetLastError((int)GetLastError());
    return (SOCKET_ERROR);
  ...
  for (;;) {
    ...
    DWORD dwWaitResult =
        WSAWaitForMultipleEvents(1, &hReadEvent, TRUE, dwTimeout, TRUE);
    ...
    if (WSARecvFrom(hSocket, &WSABuff, 1, &dwRecvBytes, &dwFlags, pFrom,
                    &nAddrLen, NULL, NULL) == SOCKET_SUCCESS)
      break;
...
//	返回值：int
////////////////////////////////////////////////////////////////////////////////
int CBufSocket::SendTo(SOCKET hSocket, const struct sockaddr *pTo, int nToLen,
                       const char *pszBuffer, int nBufferSize,
                       DWORD dwTimeout) {
  HANDLE hWriteEvent = CreateEvent(NULL, FALSE, FALSE, NULL);
  if (hWriteEvent == NULL) {
    SetLastError((int)GetLastError());
  ...
  for (;;) {
    if (WSASendTo(hSocket, &WSABuff, 1, &dwSendBytes, dwFlags, pTo, nToLen,
                  NULL, NULL) == SOCKET_SUCCESS)
      break;
...
//	返回值：BOOL
////////////////////////////////////////////////////////////////////////////////
BOOL CBufSocket::RecvFrom(OUT char *pszFrom, OUT int &nFromPort,
                          OUT char *pszBuffer, IN int nBufferSize,
                          IN DWORD dwTimeout /*= DEFAULT_RECV_TIMEOUT*/) {
  SOCKADDR_IN sockAddrFrom;
  ZeroMemory(&sockAddrFrom, sizeof(sockAddrFrom));

  // 假设缓冲区足够大,不考虑缓冲区不够的情形
  if (RecvFrom(m_hSocket, (SOCKADDR *)&sockAddrFrom, sizeof(sockAddrFrom),
               pszBuffer, nBufferSize, dwTimeout) <= 0)
    return FALSE;
...
//	返回值：BOOL
////////////////////////////////////////////////////////////////////////////////
BOOL CBufSocket::SendTo(const char *pszTo, int nToPort, const char *pszBuffer,
                        int nBufferSize,
                        DWORD dwTimeout /*= DEFAULT_SEND_TIMEOUT*/) {
  ...
  if (SendTo(m_hSocket, (SOCKADDR *)&sockAddrTo, sizeof(sockAddrTo), pszBuffer,
             nBufferSize, dwTimeout) < nBufferSize)
    return FALSE;
...
//	返回值：BOOL
////////////////////////////////////////////////////////////////////////////////
BOOL CBufSocket::SetSockOpt(int nOptionName, const void *lpOptionValue,
                            int nOptionLen, int nLevel /*= SOL_SOCKET*/) {
  if (setsockopt(m_hSocket, nLevel, nOptionName, (const char *)lpOptionValue,
...
//	返回值：BOOL
////////////////////////////////////////////////////////////////////////////////
BOOL CBufSocket::GetSockOpt(int nOptionName, void *lpOptionValue,
                            int *lpOptionLen, int nLevel /*= SOL_SOCKET*/) {
  if (getsockopt(m_hSocket, nLevel, nOptionName, (char *)lpOptionValue,
                 lpOptionLen) == SOCKET_ERROR) {
    SetLastError(WSAGetLastError());
    return FALSE;
  }
  return TRUE;
