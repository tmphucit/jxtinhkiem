...
//	参  数：
//		pszDest     :
// 字符串指针,用于保存打包数据(必须预先分配足够的内存空间) 		pPacket
// : 数据包指针(待打包的数据包) 		nPacketType : 包类型
// 返回值：BOOL
////////////////////////////////////////////////////////////////////////////////
BOOL CSocksPacket::Pack(OUT char *pszDest, PSOCKSREQPACKET pPacket,
                        int nPacketType) {
  ...
  case PACKET_SOCKS5REQ:
    ...
    case ATYP_HOSTNAME:
      ...
      /*
              ...
              szAddr[0] = (char)nLen;			//
         第一个字节保存域名的长度 for( int i =0;i<nLen;i++)		//
         从第二个字节开始才是真正的域名 szAddr[i+1] = DM[i]; szAddr[nLen+1] =
         '\0';
...
int CSocksPacket::PacketSize(PSOCKSREQPACKET pPacket, int nPacketType) {
  ...
  case PACKET_SOCKS4REQ:
    ...
    break;
  case PACKET_SOCKS4AREQ:
    nLen = 10 + strlen(pPacket->socks4aReq.pszUserID) +
           strlen(pPacket->socks4aReq.pszDestHostName);
    break;
  case PACKET_SOCKS5AUTHREQ:
    nLen = 2 + (BYTE)pPacket->socks5AuthReq.byNAuth;
    break;
  case PACKET_SOCKS5AUTHPASSWDREQ:
    nLen = 3 + strlen(pPacket->socks5AuthPasswdReq.pszUserName) +
           strlen(pPacket->socks5AuthPasswdReq.pszPasswd);
    break;
  case PACKET_SOCKS5REQ:
    nLen = 6;
    switch (pPacket->socks5Req.byAtyp) {
...
//	返回值：BOOL
////////////////////////////////////////////////////////////////////////////////
BOOL CSocksPacket::SendPacket(PSOCKSREQPACKET pPacket, int nPacketType,
                              DWORD dwTimeout /* = SOP_SEND_TIMEOUT*/) {
  // 计算包大小
...
//	返回值：BOOL
////////////////////////////////////////////////////////////////////////////////
BOOL CSocksPacket::SendSocks4Req(BYTE byCmd, WORD wDestPort, DWORD dwDestIP,
                                 const char *pszUserID /* = NULL */,
                                 DWORD dwTimeout /* = SOP_SEND_TIMEOUT */) {
  SOCKSREQPACKET pack;
...
//	返回值：BOOL
////////////////////////////////////////////////////////////////////////////////
BOOL CSocksPacket::SendSocks4aReq(BYTE byCmd, WORD wDestPort,
                                  const char *pszDestHostName,
                                  const char *pszUserID /* = NULL */,
                                  DWORD dwTimeout /* = SOP_SEND_TIMEOUT */) {
  SOCKSREQPACKET pack;
...
//	返回值：BOOL
////////////////////////////////////////////////////////////////////////////////
BOOL CSocksPacket::SendSocks5AuthReq(BYTE byNAuth, const char *pszAuth,
                                     DWORD dwTimeout /* = SOP_SEND_TIMEOUT */) {
  SOCKSREQPACKET pack;
...
//	返回值：BOOL
////////////////////////////////////////////////////////////////////////////////
BOOL CSocksPacket::SendSocks5AuthPasswdReq(
    const char *pszUserName, const char *pszPassword,
    DWORD dwTimeout /* = SOP_SEND_TIMEOUT */) {
  SOCKSREQPACKET pack;
  pack.socks5AuthPasswdReq.byVer = 0x01;
  pack.socks5AuthPasswdReq.byUserNameLen =
      (BYTE)((pszUserName == NULL) ? 0 : strlen(pszUserName));
  pack.socks5AuthPasswdReq.pszUserName = pszUserName;
  pack.socks5AuthPasswdReq.byPasswdLen =
      (BYTE)((pszPassword == NULL) ? 0 : strlen(pszPassword));
...
//			ATYP_HOSTNAME	pszDestAddr =
//[1BYTE:(HostName)Length]+[HostName] 		pszDestAddr : 目标地址
//wDestPort   : 目标端口 		dwTimeout   : 超时 	返回值：BOOL
////////////////////////////////////////////////////////////////////////////////
BOOL CSocksPacket::SendSocks5Req(BYTE byCmd, BYTE byAtyp,
                                 const char *pszDestAddr, WORD wDestPort,
                                 DWORD dwTimeout /* = SOP_SEND_TIMEOUT */) {
  SOCKSREQPACKET pack;
...
//	返回值：BOOL
////////////////////////////////////////////////////////////////////////////////
BOOL CSocksPacket::RecvPacket(OUT PSOCKSREPPACKET pPacket, int nPacketType,
                              DWORD dwTimeout /*= SOP_RECV_TIMEOUT*/) {
  int nRecv, n, nCount;
  WORD wData = 0;
  DWORD dwData = 0;
...
//	返回值：BOOL
////////////////////////////////////////////////////////////////////////////////
BOOL CSocksPacket::RecvPacket(int nPacketType,
                              DWORD dwTimeout /*= SOP_RECV_TIMEOUT*/) {
