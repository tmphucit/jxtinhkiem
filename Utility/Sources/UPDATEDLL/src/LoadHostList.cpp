...
int KHostList::SaveHostListFileInfo(const char cszDestFileName[],
                                    const char cszSrcFileName[]) {
...
int KHostList::IsHostListFileChanged(const char cszDestFileName[],
                                     const char cszSrcFileName[]) {
  char szFileName[MAX_PATH] = {0};
  int nRetCode = 0;
  UINT uCRC32 = 0;
...
void KHostList::AddHostList(KHOST_LIST *pSrcHostList,
                            KHOST_LIST **ppDestHostList) {
...
int KHostList::LoadOrdHostList(const char cszFileName[],
                               KHOST_LIST **ppListHost) {
  int nResult = 0;
  int i;
  ...
  if (FileExists(cszFileName)) {
    for (i = 0;; i++) {
      ...
      dwSize = MAX_PATH;
      dwRetCode = GetPrivateProfileString("HostList", szHostURLKey, "",
                                          szHostURL, dwSize, cszFileName);
      if (*szHostURL == '\0')
        break;

      dwSize = MAX_PATH;
      dwRetCode = GetPrivateProfileString("HostList", szHostNameKey, "",
                                          szHostName, dwSize, cszFileName);
      if (*szHostName == '\0')
        break;
...
int KHostList::LoadSrcHostList(const char cszFileName[],
                               KHOST_LIST **ppHostList) {
  int nResult = false;
...
KHostList::KHostList(const char cszFileName[]) { LoadList(cszFileName); }

KHostList::~KHostList() { ReleaseList(); }

void KHostList::ReleaseList() { ReleaseHostList(&m_pHostList); }

int KHostList::LoadList(const char cszFileName[]) {
  strcpy(m_szFileName, cszFileName);
  m_nSVRChangedFlag =
      IsHostListFileChanged(m_szFileName, defHOST_LIST_FILE_SVR);
  m_nEXTChangedFlag =
      IsHostListFileChanged(m_szFileName, defHOST_LIST_FILE_EXT);
...
int KHostList::SaveList(char *pszFileName /* = NULL */) {
  ...
  while (pHostList) {
    ...
    WritePrivateProfileString("HostList", szHostNameKey, pHostList->szName,
                              pszSaveFileName);
    WritePrivateProfileString("HostList", szHostURLKey, pHostList->szURL,
                              pszSaveFileName);
