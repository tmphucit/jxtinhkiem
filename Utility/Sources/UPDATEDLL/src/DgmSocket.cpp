...
int CDgmSocket::GetPrivateMode() { return m_nInPrivate; }
...
int CDgmSocket::SetRemoteAddress(const INET_ADDRESS *psaRemoteAddr) {
  ...
  if (0 == psaRemoteAddr->sin_addr.s_addr) {
    if (!m_nInPrivate)
      ...
      nRetCode = ::connect(m_hSckHandle, (sockaddr *)psaRemoteAddr,
                           sizeof(INET_ADDRESS));
    ...
    if ((m_saRemoteAddr.sin_family == psaRemoteAddr->sin_family) &&
        (m_saRemoteAddr.sin_addr.s_addr == psaRemoteAddr->sin_addr.s_addr))
      nResult = true;
    else {
      nResult = ::connect(m_hSckHandle, (sockaddr *)psaRemoteAddr,
                          sizeof(INET_ADDRESS));
      nRetCode = CheckError(nRetCode);
      if (0 == nRetCode) {
        m_saRemoteAddr = *psaRemoteAddr;
