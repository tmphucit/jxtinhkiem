...
void CDownloadFile::SetTimeout(ULONG ulSendTimeout, ULONG ulRecvTimeout,
                               ULONG ulConnTimeout) {
...
void CDownloadFile::SetProxy(int nProxyMode, LPCTSTR lpszProxyHost,
                             int nProxyPort, LPCTSTR lpszProxyUserName,
  ...
  m_FtpDownload.SetProxy(lpszProxyHost, nProxyPort,
                         (nProtocolMode != FTP_PROXY_NONE),
                         (lpszProxyUserName != NULL), lpszProxyUserName,
                         lpszProxyPassword, nProtocolMode);
  ...
  m_HttpDownload.SetProxy(lpszProxyHost, nProxyPort,
                          (nProtocolMode != HTTP_PROXY_NONE),
                          (lpszProxyUserName != NULL), lpszProxyUserName,
                          lpszProxyPassword, nProtocolMode);
}

int CDownloadFile::StartDownload(LPCTSTR lpszUrlPath,
                                 BOOL bEnableResume /*= TRUE*/,
                                 LPCTSTR lpszSaveAs /*= NULL*/) {
  if (m_hdowning != NULL) {
...
ULONG CDownloadFile::MainExecution() {
  while (1) {
    ...
    Sleep(1);
    if (m_hstartdown &&
        WaitForSingleObject(m_hstartdown, 0) == WAIT_OBJECT_0) // ¿ªÊ¼down
    {
...
BOOL CDownloadFile::DownOneFile() {
  ...
  switch (m_nDownProtocol) {
  case DOWN_PROTOCOL_HTTP:
    ulResult = (ULONG)m_HttpDownload.Download(m_strDownUrlPath,
                                              m_strSaveFilePath, m_nForceDown);
  ...
  case DOWN_PROTOCOL_FTP:
    ulResult = (ULONG)m_FtpDownload.Download(m_strDownUrlPath,
                                             m_strSaveFilePath, m_nForceDown);
  ...
  if (m_ulNotifyCode != -1) {
    ::SendMessage(m_hNotifyWnd, m_ulNotifyMsg, MSG_DOWNLOAD_RESULT,
                  m_ulNotifyCode);
  }
