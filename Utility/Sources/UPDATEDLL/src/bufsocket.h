...
static const int DEFAULT_BLOCKED_SNDRCV_SLEEP =
    100; // 100毫秒(发生阻塞时等待[睡眠]时间)
...
public:
  ...
  static DWORD GetIP(const char *pszName,
                     BOOL bFixNtDNS = FALSE); // Used to Fix NT DNS Problem
...
public:
  ...
  BOOL Create(int nAddressFamily = AF_INET, int nType = SOCK_STREAM,
              int nProtocol = 0, int nBufferSize = DEFAULT_BUFFER_SIZE,
              SOCKET hSocket = INVALID_SOCKET,
              DWORD dwFlags = WSA_FLAG_OVERLAPPED);

  void Close(BOOL bHardClose = FALSE);
  int Recv(OUT char *pszBuffer, int nBufferSize,
           DWORD dwTimeout = DEFAULT_RECV_TIMEOUT);
  int Send(char const *pszBuffer, int nBufferSize,
           DWORD dwTimeout = DEFAULT_SEND_TIMEOUT);

  BOOL Connect(char const *pszServer, int nPort,
               DWORD dwTimeout = DEFAULT_CONN_TIMEOUT, BOOL bFixNtDNS = FALSE);
  int BSDGetChar(DWORD dwTimeout = DEFAULT_RECV_TIMEOUT);
  int BSDGetString(OUT char *pszBuffer, int nBufferSize,
                   DWORD dwTimeout = DEFAULT_RECV_TIMEOUT);
  int BSDGetData(OUT char *pszBuffer, int nBufferSize,
                 DWORD dwTimeout = DEFAULT_RECV_TIMEOUT);
  ...
  SOCKET Accept(struct sockaddr *pSocketAddr, int *nAddrLen,
                DWORD dwTimeout = DEFAULT_ACCEPT_TIMEOUT);
  SOCKET Accept(struct sockaddr *pSocketAddr, int *nAddrLen, HANDLE hEndEvent,
                DWORD dwTimeout = DEFAULT_ACCEPT_TIMEOUT);
  // Add in 1.1 >

  // Add in 1.2 <
  BOOL RecvFrom(OUT char *pszFrom, OUT int &nFromPort, OUT char *pszBuffer,
                IN int nBufferSize, IN DWORD dwTimeout = DEFAULT_RECV_TIMEOUT);
  BOOL SendTo(const char *pszTo, int nToPort, const char *pszBuffer,
              int nBufferSize, DWORD dwTimeout = DEFAULT_SEND_TIMEOUT);
...
private:
  ...
  BOOL Connect(SOCKET hSocket, const struct sockaddr *pSocketAddr, int iAddrLen,
               DWORD dwTimeout);
  int RecvOnce(SOCKET hSocket, OUT char *pszBuffer, int nBufferSize);
  int RecvData(SOCKET hSocket, OUT char *pszBuffer, int nBufferSize,
               DWORD dwTimeout);
  int SendOnce(SOCKET hSocket, char const *pszBuffer, int nBufferSize);
  int SendData(SOCKET hSocket, char const *pszBuffer, int nBufferSize,
               DWORD dwTimeout);
  ...
  // Add in 1.2 <
  int RecvFrom(SOCKET hSocket, struct sockaddr *pFrom, int nFromlen,
               char *pszBuffer, int nBufferSize, DWORD dwTimeout);
  int SendTo(SOCKET hSocket, const struct sockaddr *pTo, int nToLen,
             char const *pszBuffer, int nBufferSize, DWORD dwTimeout);
  // Add in 1.2 >
