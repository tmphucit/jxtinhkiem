...
static const char *g_pszCheckHostArray[] = {
    "www.duba.net", "gz3.duba.net", "gz.duba.net", "sh.duba.net",
    "bj.duba.net",  "gz2.duba.net", "gz4.duba.net"

};
static const int g_nCheckPortArray[] = {
    defCHECK_SERVER_PORT, defCHECK_SERVER_PORT, defCHECK_SERVER_PORT,
    defCHECK_SERVER_PORT, defCHECK_SERVER_PORT, defCHECK_SERVER_PORT,
    defCHECK_SERVER_PORT

};

static const int g_nCheckServerCount =
    sizeof(g_pszCheckHostArray) / sizeof(const char *);
...
int CUpdateDLLApp::Init(CONST KUPDATE_SETTING &UpdateSetting) {
  ...
  g_UpdateData.ProxySetting.nProxyMethod =
      UpdateSetting.ProxySetting.nProxyMethod;
  g_UpdateData.ProxySetting.nHostPort = UpdateSetting.ProxySetting.nHostPort;
  strcpy(g_UpdateData.ProxySetting.szHostAddr,
         UpdateSetting.ProxySetting.szHostAddr);
  strcpy(g_UpdateData.ProxySetting.szPassword,
         UpdateSetting.ProxySetting.szPassword);
  strcpy(g_UpdateData.ProxySetting.szUserName,
         UpdateSetting.ProxySetting.szUserName);
  ...
  nRetCode = m_wndMessage.Create(
      ::AfxRegisterWndClass(CS_DBLCLKS | CS_VREDRAW, 0,
                            (HBRUSH)::GetStockObject(BLACK_BRUSH)),
      "KingSoft Update Notify Window", WS_OVERLAPPEDWINDOW & ~WS_VISIBLE, rect,
...
int CUpdateDLLApp::CheckNeedUpdate() {
  ...
  if (g_UpdateData.nMethod == defUPDATE_METHOD_LAN) {
    m_strIndexTempFile =
        (CString)g_PathTable.szDownloadDestPath + defINDEX_FILE_NAME;
...
int CUpdateDLLApp::Download() {
  ...
  m_DownloadFile.SetNotifyWnd(m_wndMessage.GetSafeHwnd(),
                              WM_DOWNNOTIFY_DEFAULT);
...
ULONG CUpdateDLLApp::OnDownResult(ULONG ulOverResult) {
  ...
  if (DOWNINDEX_STEPINDEX == m_nProcessStep) {
    ...
    case DOWN_RESULT_SAMEAS:
      ...
      // Check file CRC
      nCurDownOK = CheckFileCRC(atoi(m_pCurDownItem->szCRC),
      ...
      if (!nCurDownOK) {
        if (!g_UpdateData.bAutoTryNextHost) // if (!ExistsOtherHost())
        {
          ...
          sLog.Format(sFormat, m_pCurDownItem->szFileName,
                      m_pCurDownItem->szDownloadTmpFileName);
...
int CUpdateDLLApp::ProcessNextStep(int nDownStep) {
  ...
  case DOWNINDEX_STEPCHKSNUDP:
    ...
    nRetCode = ::WaitForMultipleObjects(1, &Handle, true, INFINITE);
    if (WAIT_OBJECT_0 == nRetCode)
      UnInitCheckThread();
  ...
  case DOWNINDEX_STEPCHKSNHTTP:
    ...
    do {
      nRetCode = ::MsgWaitForMultipleObjects(1, &m_hExitNotify, false, INFINITE,
                                             QS_ALLINPUT);
      if (WAIT_OBJECT_0 == nRetCode)
        break;
  ...
  case DOWNINDEX_STEPINDEX:
    ...
    m_DownloadFile.SetTimeout(10000, 24000, 24000);
    nResult =
        m_DownloadFile.StartDownload(strDownUrl, false, m_strIndexTempFile);
    break;
...
int CUpdateDLLApp::DownNextItem(int nRedownCurrent /*=false*/) {
  ...
  if (m_pCurDownItem != NULL) {
    ...
    if (m_pfnCallBackProc) {
      ...
      if (m_pfnCallBackProc(defUPDATE_STATUS_DOWNLOADING,
                            nIndex / nCount * 100)) {
        m_nResultCode = defUPDATE_RESULT_CANCEL;
        goto Exit0;
      }
    ...
    nRetCode =
        m_DownloadFile.StartDownload(strDownUrlPath, m_nCurEnableResume,
                                     m_pCurDownItem->szDownloadTmpFileName);
    ASSERT(nRetCode);
...
int CUpdateDLLApp::GetCheckInfo(ULONG ulContextWith, int *pnSendSize,
  ...
  if ((NULL == pnSendSize) || (NULL == ppbySendInfo))
    goto Exit0;
...
int CUpdateDLLApp::CheckRecvInfo(ULONG ulContextWith, int nRecvSize,
  ...
  nRetCode = strncmp(pServerInfo->szFlag, defCHECK_CURRENT_SERVERFLAG,
...
void CUpdateDLLApp::CheckResult(ULONG ulContextWith, int nSvrResult) {
  int nRetCode = false;
  CString sMsg;
  ...
  if (DOWNINDEX_STEPCHKSNHTTP == m_nProcessStep) {
    if (pDownStatus->dwFileDownloadedSize >= 20) {
      ...
      TRY {
        ...
        nRetCode = file.Open(m_strCgiTempFile + ".tmp",
                             CFile::modeRead | CFile::shareDenyNone);
        if (!nRetCode) {
          nRetCode = file.Open(m_strCgiTempFile,
                               CFile::modeRead | CFile::shareDenyNone);
          if (!nRetCode) {
            ASSERT(false);
            goto Exit0;
