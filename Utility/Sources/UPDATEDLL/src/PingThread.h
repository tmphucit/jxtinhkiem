...
public:
  virtual int GetNextUrl(ULONG *pulContextWith, int nUrlBufSize,
                         char *pszUrlTextBuf) = 0;
  virtual int PingStatus(ULONG ulContextWith, ULONG ulStatusType,
...
class CPingThread : public CPingSocket {
public:
  CPingThread();
  virtual ~CPingThread();
