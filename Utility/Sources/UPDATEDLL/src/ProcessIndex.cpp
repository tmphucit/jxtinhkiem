...
int CheckSectionIsUpdateItem(const char cszFileName[],
                             const char cszItemName[]) {
  DWORD dwRetCode = 0;
  int nRetCode = 0;
  char szUpdateFileFlag[10];

  dwRetCode = ::GetPrivateProfileString(cszItemName, "UpdateFileFlag", "",
                                        szUpdateFileFlag, 10, cszFileName);
  if (!dwRetCode)
    return 0;
...
int GetActuallyPath(char szPath[], const char cszBasePath[]) {
  ...
  if (!(((*szPath = '\\') || (*szPath = '/')) || // Maybe '/' in Unix
        ((strlen(szPath) > 2) && (*(szPath + 1) == ':') &&
         ((*(szPath + 2) == '\\') || (*(szPath + 2) == '/'))))) // if is sub-dir
  {
    char szTemp[MAX_PATH] = {0};
    strcpy(szTemp, szPath);
    strcpy(szPath, cszBasePath);
...
int CProcessIndex::GetItemInfo(PKUPDATE_ITEM pUpdateItem, int nUpdateMethod,
                               char szLocalPath[]) {
  int nResult = false;
  DWORD dwRetCode = 0;
  int nRetCode = 0;
  ...
  dwRetCode = ::GetPrivateProfileString(pUpdateItem->szItemName, "FileName", "",
                                        pUpdateItem->szFileName, MAX_PATH,
                                        m_szTxtFileName);
  KAV_PROCESS_ERROR(dwRetCode);

  pUpdateItem->nFileSize = (int)::GetPrivateProfileInt(
      pUpdateItem->szItemName, "FileSize", 0, m_szTxtFileName);
  KAV_PROCESS_ERROR(pUpdateItem->nFileSize);

  pUpdateItem->KCheckVersionMethod =
      (KCHECKVERSIONMETHOD)::GetPrivateProfileInt(
          pUpdateItem->szItemName, "CheckVersionMethod", 0, m_szTxtFileName);

  dwRetCode = ::GetPrivateProfileString(pUpdateItem->szItemName, "SNMask", "",
                                        pUpdateItem->szSNMask, defSNMaskSize,
                                        m_szTxtFileName);

  // --------------- Process Version As File Date------------------
  dwRetCode = ::GetPrivateProfileString(pUpdateItem->szItemName, "Version", "",
                                        szValue, 50, m_szTxtFileName);
  KAV_PROCESS_ERROR(dwRetCode);

  strcpy(pUpdateItem->szFileDate, szValue);
  dwRetCode =
      ::GetPrivateProfileString(pUpdateItem->szItemName, "CRC", "",
                                pUpdateItem->szCRC, 20, m_szTxtFileName);
  KAV_PROCESS_ERROR(dwRetCode);
  ...
  if ((pUpdateItem->KCheckVersionMethod == enumCVM_TIME) ||
    ...
    nRetCode = GetVersionFromString(szValue, &(pUpdateItem->uMajorVersion),
                                    &(pUpdateItem->uMinorVersion));
  ...
  dwRetCode = ::GetPrivateProfileString(pUpdateItem->szItemName, "Reboot", "",
                                        szValue, 50, m_szTxtFileName);
  KAV_PROCESS_ERROR(dwRetCode);
  ...
  dwRetCode =
      ::GetPrivateProfileString(pUpdateItem->szItemName, "UpdateRelative", "",
                                szValue, 50, m_szTxtFileName);
  ...
  pUpdateItem->nBelong = ::GetPrivateProfileInt(pUpdateItem->szItemName,
                                                "Belong", -1, m_szTxtFileName);

  dwRetCode =
      ::GetPrivateProfileString(pUpdateItem->szItemName, "SharedNeedReboot",
                                "0", szValue, 50, m_szTxtFileName);
  if (!stricmp(szValue, "TRUE") || !stricmp(szValue, "1"))
    pUpdateItem->nSharedNeedReboot = 1;
  else
    pUpdateItem->nSharedNeedReboot = 0;

  dwRetCode = ::GetPrivateProfileString(pUpdateItem->szItemName, "RemotePath",
                                        "", pUpdateItem->szRemotePath, MAX_PATH,
                                        m_szTxtFileName);
  KAV_PROCESS_ERROR(dwRetCode);
  ...
  dwRetCode = ::GetPrivateProfileString(
      pUpdateItem->szItemName, "LocalPath", "%PROGRAMDIR%",
      pUpdateItem->szLocalPath, MAX_PATH, m_szTxtFileName);
  KAV_PROCESS_ERROR(dwRetCode);
  nRetCode =
      GetActuallyPath(pUpdateItem->szLocalPath, g_PathTable.szUpdateDestPath);
  ...
  dwRetCode = ::GetPrivateProfileString(pUpdateItem->szItemName, "Description",
                                        "", pUpdateItem->szDescription, 200,
                                        m_szTxtFileName);
  dwRetCode = ::GetPrivateProfileString(pUpdateItem->szItemName, "ApplyVersion",
                                        "", pUpdateItem->szApplyVersion,
                                        defApplyVersionSize, m_szTxtFileName);

  if (nUpdateMethod == defUPDATE_METHOD_INTERNET) {
    nRetCode = CheckFileCRC(atoi(pUpdateItem->szCRC),
  ...
  for (nComment = 1;; nComment++) {
    ...
    dwRetCode =
        ::GetPrivateProfileString(pUpdateItem->szItemName, szCommentName, "",
                                  szBuffer, nBufferLen + 1, m_szTxtFileName);
    if (*szBuffer == '\0')
      break;
  ...
  dwRetCode = ::GetPrivateProfileString(pUpdateItem->szItemName, "CallBy", "",
                                        pUpdateItem->szCallBy, MAX_PATH,
                                        m_szTxtFileName);

  *szValue = '\0';
  dwRetCode =
      ::GetPrivateProfileString(pUpdateItem->szItemName, "FileType", "Program",
                                szValue, 50, m_szTxtFileName);
  pUpdateItem->nItemType = -1;
  dwRetCode = ::GetPrivateProfileString(pUpdateItem->szItemName, "NeedDownload",
                                        "1", szValue, 50, m_szTxtFileName);
  if (!stricmp(szValue, "TRUE") || !stricmp(szValue, "1"))
  ...
  dwRetCode = ::GetPrivateProfileString(
      pUpdateItem->szItemName, "UpdateFileMethod", "Copy",
      pUpdateItem->szUpdateFileMethod, MAX_PATH, m_szTxtFileName);
...
int _CheckVersionByRes(const char cszFileName[], char szDate[]) {
  ...
  if (File.Open(cszFileName, CFile::modeRead | CFile::shareDenyNone, NULL)) {
    ...
    if ((uReadLen == sizeof(DOSHeader)) &&
      ...
      if ((uReadLen == sizeof(VXDHeader)) &&
          (IMAGE_OS2_SIGNATURE_LE == VXDHeader.e32_magic)) {
        char szBuf[1024];
...
int CProcessIndex::InitModuleExitInfo() {
  ...
  for (i = 0;; i++) {
    ...
    ::GetPrivateProfileString("ExitType", szKeyName, "", szFileName, MAX_PATH,
                              m_szTxtFileName);
    if (szFileName[0] == '\0')
      break;
    ...
    ::GetPrivateProfileString("ExitType", szKeyName, "", szReleaseFileName,
                              MAX_PATH, m_szTxtFileName);
    if (szReleaseFileName[0] == '\0')
      strcpy(szReleaseFileName, szFileName);

    strcpy(pModuleExitInfoList->MouleExitInfo.szReleaseFileName,
           szReleaseFileName);

    sprintf(szKeyName, "ExitType%d", i);
    pModuleExitInfoList->MouleExitInfo.nExitType =
        ::GetPrivateProfileInt("ExitType", szKeyName, 0, m_szTxtFileName);
    ...
    case defEXIT_TYPE_RPC:
      ...
      ::GetPrivateProfileString("ExitType", szKeyName, "",
                                pModuleExitInfoList->MouleExitInfo.szStopParam,
                                MAX_PATH, m_szTxtFileName);
      sprintf(szKeyName, "StartParam%d", i);
      ::GetPrivateProfileString("ExitType", szKeyName, "",
                                pModuleExitInfoList->MouleExitInfo.szStartParam,
                                MAX_PATH, m_szTxtFileName);
      ...
      if ((pModuleExitInfoList->MouleExitInfo.nExitType == defEXIT_TYPE_IPC) ||
        ...
        pModuleExitInfoList->MouleExitInfo.uID =
            ::GetPrivateProfileInt("ExitType", szKeyName, 0, m_szTxtFileName);
      }
...
int CProcessIndex::AddItemInfoToList(const char cszSectionName[],
...
int CProcessIndex::AnalysisUpdateFiles(int nMethod, char szLocalPath[],
                                       int nMainVersion) {
  int nResult = false;
  int nRetCode = 0;
  ...
  ::GetPrivateProfileString("COMMENT", "Readme", "", szBuffer, nBufferLen,
                            m_szTxtFileName);
  ...
  dwRetCode = ::GetPrivateProfileSectionNames(szSectionNames, nSectionNamesSize,
                                              m_szTxtFileName);
  KAV_PROCESS_ERROR(dwRetCode);
  ...
  while (*pszSectionName) {
    ...
    nRetCode =
        AddItemInfoToList(pszSectionName, nMethod, szLocalPath, nMainVersion);
...
int CProcessIndex::GetExitType(PKModuleExitInfo pModuleExitInfo) {
  ...
  while (pModuleExitInfoList) {
    if (stricmp(pModuleExitInfo->szFileName,
                pModuleExitInfoList->MouleExitInfo.szFileName) == 0) {
      sprintf(pModuleExitInfo->szReleaseFileName, "%s%s",
              g_PathTable.szUpdateDestPath,
              pModuleExitInfoList->MouleExitInfo.szReleaseFileName);
      ...
      strcpy(pModuleExitInfo->szStartParam,
             pModuleExitInfoList->MouleExitInfo.szStartParam);
      strcpy(pModuleExitInfo->szStopParam,
             pModuleExitInfoList->MouleExitInfo.szStopParam);
      break;
    }
...
bool CProcessIndex::IsVersionError() { return m_bVersionError; }

bool CProcessIndex::IsNeedUpdateSelf() { return m_bNeedUpdateSelfFirst; }

bool CProcessIndex::IsNotUpdateItem() { return (m_pUpdateItemList == NULL); }
...
int CProcessIndex::InitVersionInfo() {
  ...
  for (i = 0; i < defVersion_ValueSize; i++) {
    ...
    ::GetPrivateProfileString("Version", szKeyName, "",
                              m_Version_Value[i].szVersionStr, MAX_PATH,
                              m_szTxtFileName);
    ...
    GetPrivateProfileString("Version", szKeyName, "0", szValue, 70,
                            m_szTxtFileName);
...
int CProcessIndex::CheckIsNeedUpdate(PKUPDATE_ITEM pUpdateItem,
                                     int nMainVersion) {
  int nSignMaxNum = 0;
  int nRetCode = 0;
  ...
  if (pUpdateItem->KCheckVersionMethod == enumCVM_CRC) {
    pUpdateItem->bNeedUpdate =
        !CheckFileCRC(atoi(pUpdateItem->szCRC), szLocalFullFileName); // CRC
    goto Exit0;
  }
  ...
  if ((pUpdateItem->KCheckVersionMethod == enumCVM_SIGN) &&
    ...
    nRetCode =
        GetKAVFileVersion(szLocalFullFileName, pUpdateItem->KCheckVersionMethod,
                          &dwMajorVersion, &dwMinorVersion);
    if (!nRetCode) {
      nRetCode =
          _CheckVersionByRes(szLocalFullFileName, pUpdateItem->szFileDate);
      if (nRetCode < 0)
        pUpdateItem->bNeedUpdate = true;
      else if (nRetCode == 0)
        goto Exit1;
  ...
  pUpdateItem->bNeedUpdate = ((pUpdateItem->uMajorVersion > dwMajorVersion) ||
...
Exit1:
  ...
  if (nSignFileFlag) {
    ...
    if (nRetCode == 0)
      ...
      pUpdateItem->bNeedUpdate =
          !CheckFileCRC(atoi(pUpdateItem->szCRC), szLocalFullFileName);
  } else
    pUpdateItem->bNeedUpdate =
        !CheckFileCRC(atoi(pUpdateItem->szCRC), szLocalFullFileName);
...
CString CProcessIndex::GetVirusNames() { return m_sVirusNames; }

CString CProcessIndex::GetReadme() { return m_sReadme; }

CString CProcessIndex::GetAnnounce() { return m_sAnnounce; }

int CProcessIndex::GetHost(char szHostName[], int *pnNameSize, char szHostURL[],
                           int *pnURLSize) {
  int nResult = false;
...
int CProcessIndex::GetPath(char szPath[], int nSize) {
  ...
  ::GetPrivateProfileString("COMMENT", "PathName", "", szPath, nSize,
                            m_szTxtFileName);
