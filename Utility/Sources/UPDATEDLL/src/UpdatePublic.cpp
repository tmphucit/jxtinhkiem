...
int ProcessIndexFile(const char cszFileName[]) {
  ...
  nRetCode = g_ProcessIndex.AnalysisUpdateFiles(g_UpdateData.nMethod,
...
int KAVSetupInstallFile(const char cszCleanedFileName[],
                        const char cszLockedFileName[]) {
  int nResult = false;
  int nRetCode = false;
  ...
  *pszPos = '\0';
  ...
  szBuf +=
      (CString)szLockedFileShortName + "=" + (CString)szTempFileName + "\r\n";
...
int ProcessSharingFile(const char cszDestFileName[],
                       const char cszTempFileName[]) {
  int nResult = false;
  int nRetCode = 0;
  ...
  case defOSPLATFORM_WINNT: // WINNT/Windows2000
    nRetCode = ::SetFileAttributes(cszDestFileName, FILE_ATTRIBUTE_ARCHIVE |
    ...
    nRetCode =
        ::MoveFileEx(szLocalTempFileName, cszDestFileName,
  ...
  case defOSPLATFORM_WINME:   // Windows ME
    ...
    if (g_UpdateData.nOSPlatVersion == defOSPLATFORM_WIN9598) {
      ...
      nRetCode = File.Open(sKAVMOVE_BAT_FileName,
      ...
      if (FileExists(sKAVMove_EXE_FileName)) {
        if (cszTempFileName[0] != '\0')
          sBuffer = "@if exist " + sKAVMove_EXE_FileName + " " +
                    sKAVMove_EXE_FileName + " " + sCleanedFileShortName + " " +
                    sLockedFileShortName + "\r\n";
        else
          sBuffer = "@if exist " + sKAVMove_EXE_FileName + " " +
                    sKAVMove_EXE_FileName + " " + sLockedFileShortName + "\r\n";
      } else {
        if (cszTempFileName[0] != '\0') {
          sBuffer = "@if exist " + sLockedFileShortName + " " +
                    sAttrib_FileName + " -s -h -r " + sLockedFileShortName +
                    "\r\n";
          sBuffer += "@if exist " + sCleanedFileShortName + " copy /y " +
                     sCleanedFileShortName + " " + sLockedFileShortName +
                     "\r\n";
          sBuffer += "@if exist " + sCleanedFileShortName + " del " +
                     sCleanedFileShortName + "\r\n";
        } else {
          sBuffer = "if exist " + sLockedFileShortName + " " +
                    sAttrib_FileName + " -s -h -r " + sLockedFileShortName +
                    "\r\n";
          sBuffer += "@if exist " + sLockedFileShortName + " del " +
                     sLockedFileShortName + "\r\n";
        }
      }
      ...
      ::SetFileAttributes(cszDestFileName, FILE_ATTRIBUTE_NORMAL);
      szCopyIniFileName =
          (CString)g_PathTable.szTempPath + defUPDATE_FILES_INI_FILE_NAME;

      strcpy(szLockedFileShortName, strrchr(cszDestFileName, '\\') + 1);
      if (cszTempFileName) {
        ::WritePrivateProfileString(szLockedFileShortName, "src",
                                    cszTempFileName, szCopyIniFileName);
        ::WritePrivateProfileString(szLockedFileShortName, "dst",
                                    cszDestFileName, szCopyIniFileName);
      } else {
        ::WritePrivateProfileString(szLockedFileShortName, "src",
                                    cszDestFileName, szCopyIniFileName);
      }
...
int UpdateSelf() {
  ...
  while (pUpdateItem) {
    ...
    WritePrivateProfileString("UpdateSelf", szKeyName,
    ...
    WritePrivateProfileString("UpdateSelf", szKeyName, szDstFile,
                              szUpdateSelfInfoFile);
  ...
  WritePrivateProfileString("UpdateSelf", "Parameter", szParameter,
                            szUpdateSelfInfoFile);

  strcpy(szDstFile, g_PathTable.szUpdateDestPath);
  strcat(szDstFile, g_UpdateData.szExecuteProgram);
  WritePrivateProfileString("UpdateSelf", "FullPathName", szDstFile,
                            szUpdateSelfInfoFile);
...
int UpdateFile_Copy(const KUPDATE_ITEM &UpdateItem) {
  ...
  nRetCode =
      ::CopyFile(UpdateItem.szDownloadTmpFileName, szLocalFileName, false);
  if (nRetCode) {
  ...
  _GetTempFileName(szLocalTempFileName);
  nRetCode =
      ::CopyFile(UpdateItem.szDownloadTmpFileName, szLocalTempFileName, false);
  if (!nRetCode) {
...
int UpdateFile_Package(const KUPDATE_ITEM &UpdateItem) {
  ...
  if (FileExists(szLocalFileName)) {
    ...
    if (hModule) {
      COMBINAPACKAGE pFunCombinaPackage =
          (COMBINAPACKAGE)GetProcAddress(hModule, "CombinatPackage");
      if (pFunCombinaPackage) {
        nResult = pFunCombinaPackage(UpdateItem.szDownloadTmpFileName,
                                     szLocalFileName);
        if (!nResult) {
          g_UpdateData.bUpdateFailed = true;
          sFormat = defIDS_UPDATE_FAILED;
...
static int _SetRebootProcess_WIN9X() {
  ...
  sCommandString = "\r\n@if exist " + sKAVMOVE_BAT_FileName + " call " +
                   sKAVMOVE_BAT_FileName + "\r\n";
  sCommandString += "if exist " + sKAVMOVE_BAT_FileName + " del " +
                    sKAVMOVE_BAT_FileName + "\r\n";
  sCommandString += "if exist " + sKAVMOVE_EXE_FileName + " del " +
                    sKAVMOVE_EXE_FileName + "\r\n";
  ...
  for (int i = 0; i < 2; i++) {
    nRetCode = File.Open(
        sProcessFileArray[i],
        CFile::modeNoTruncate | CFile::modeCreate | CFile::modeReadWrite, NULL);
    if (nRetCode) {
...
int UpdateFiles() {
  ...
  while (pUpdateItem) {
    if ((pUpdateItem->DownloadStatus == enumDOWNLOADSTATUS_DOWNLOADED) &&
          ...
          pLastUpdateItem == NULL) // the first last copy
      {
        ...
        memmove(pLastUpdateItem->szUpdateFileMethod,
                pLastUpdateItem->szUpdateFileMethod + 5, MAX_PATH - 5);
