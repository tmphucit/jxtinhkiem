...
int CCheckThread::SetCheckCallback(IKCheckCallback *piCallback,
                                   ULONG ulContextWith) {
  int nResult = false;
  int nRetCode = false;
...
int CCheckThread::StartThread() {
  ...
  if ((NULL == m_hKillEvent) || (NULL == m_piCallback)) {
    ASSERT(FALSE);
    goto Exit0;
  }
...
ULONG CCheckThread::MainExecution() {
  ...
  nRetCode = m_piCallback->GetCheckInfo(m_ulContextWith, &nSendBufSize,
                                        &pbySendBuffer);
  if ((!nRetCode) || (nSendBufSize <= 0) || (NULL == pbySendBuffer))
    goto Exit0;
  ...
  for (nAddrIndex = 0; nAddrIndex < nCheckServerCount; nAddrIndex++) {
    ...
    nRetCode =
        CheckSocket.SetRemoteAddress(pcszCheckServerAddr, nCheckServerPort);
    if (!nRetCode)
      continue;
    ...
    while (nTryTimes-- > 0) {
      ...
      if (nDataReady > 0) {
        ...
        nCheckResult = m_piCallback->CheckRecvInfo(m_ulContextWith, nRetCode,
                                                   byRecvBuffer);
