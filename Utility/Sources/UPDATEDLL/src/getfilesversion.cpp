...
int GetKAVFileVersion(LPCTSTR lpszFileName,
  ...
  *pdwMinorVersion = 0;
  ...
  case enumCVM_ENGINE: // Engine file
    nResult =
        GetEngineFileVersion(lpszFileName, pdwMajorVersion, pdwMinorVersion);
    break;
  case enumCVM_SIGN: // Sign file
    nResult =
        GetSignFileVersion(lpszFileName, pdwMajorVersion, pdwMinorVersion);
    break;
...
int GetEngineFileVersion(const char cszFileName[], DWORD *pdwMajorVersion,
  ...
  *pdwMinorVersion = 0;
  ...
  nRetCode = ::ReadFile(hFile, (void *)&Header, nKAVEngineFileHeaderSize,
                        (DWORD *)&nReadSize, NULL);
...
int GetSignFileVersion(const char cszFileName[], DWORD *pdwMajorVersion,
  ...
  *pdwMinorVersion = 0;
  ...
  nRetCode = ::ReadFile(hFile, (void *)&Header, nKAVSignFileHeaderSize,
                        (DWORD *)&nReadSize, NULL);
...
int GetLocalSignVersion(DWORD *pdwMajorVersion, DWORD *pdwMinorVersion) {
  ...
  nRetCode = GetSignFileVersion(szSignFileName, (unsigned long *)&uMajorVersion,
                                (unsigned long *)&uMinorVersion);
  ...
  *pdwMinorVersion = uMinorVersion;
  ...
  while (true) {
    ...
    if (!nRetCode) {
      ...
      nRetCode = GetSignFileVersion(szFileName, (unsigned long *)&uMajorVersion,
                                    (unsigned long *)&uMinorVersion);
      if ((uMajorVersion == 0) && (uMinorVersion == 0))
        break;
...
int GetLocalSignVersion(char szVersion[]) {
  ...
  GetLocalSignVersion((unsigned long *)&uMajorVersion,
                      (unsigned long *)&uMinorVersion);

  GetVersionStringFromInt(uMajorVersion, uMinorVersion, szVersion,
                          defVERSION_SIZE);
...
int ExistUpdFileByNum(int nNum) {
  ...
  memcpy(szLocalPathName + nLocalPathNameLen - 4 - nRetCode, szSignVersion,
         nRetCode);
...
int IsSignFile(const char szFileName[]) {
  ...
  nRetCode = ::ReadFile(hFile, (void *)&KAVSignFileHeader,
                        nKAVSignFileHeaderSize, (DWORD *)&nReadSize, NULL);
  KAV_PROCESS_ERROR(!(!nRetCode || (nReadSize != nKAVSignFileHeaderSize)));

  nRetCode = strnicmp(KAVSignFileHeader.cszDescription,
                      "Kingsoft AntiVirus Database", 27);
  if (!nRetCode)
    nResult = true;
...
int GetNormalVersion(const char cszPathName[], char szVersion[]) {
  ...
  nRetCode = GetFileVersion(cszPathName, (DWORD *)&uMajorVersion,
                            (DWORD *)&uMinorVersion);
  if (nRetCode)
    nRetCode = GetVersionStringFromInt(uMajorVersion, uMinorVersion, szVersion,
                                       defVERSION_SIZE);
  else
...
int GetEngineVersion(const char cszPathName[], char szVersion[]) {
  ...
  nRetCode = GetEngineFileVersion(cszPathName, (DWORD *)&uMajorVersion,
                                  (DWORD *)&uMinorVersion);
  if (!nRetCode)
    return nRetCode;
...
int GetSignVersion(const char cszPathName[], char szVersion[]) {
  ...
  nRetCode = GetSignFileVersion(cszPathName, (DWORD *)&uMajorVersion,
                                (DWORD *)&uMinorVersion);
...
int GetVirusCountFromSignFile(const char cszPathName[]) {
  ...
  hFile = ::CreateFile(cszPathName, GENERIC_READ, FILE_SHARE_READ, NULL,
                       OPEN_EXISTING,
