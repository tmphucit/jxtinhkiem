...
int KClosedProgram::CloseProgram() {
  ...
  Hinstance = ShellExecute(NULL, "open", m_kProcessInfo.szFileName,
                           m_kProcessInfo.szCloseParam, "", SW_MINIMIZE);
  if ((int)Hinstance <= 32)
    return 0;
...
int KClosedProgram::StartProgram() {
  ...
  Hinstance = ShellExecute(NULL, "open", m_kProcessInfo.szFileName,
                           m_kProcessInfo.szStartParam, "", SW_MINIMIZE);
  if ((int)Hinstance <= 32)
    return false;
...
//----------------------------------------------------------------------------
KCloseProgramMgr::KCloseProgramMgr() { m_pClosedProgramList = NULL; }
