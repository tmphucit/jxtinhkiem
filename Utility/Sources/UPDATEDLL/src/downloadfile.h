...
#if !defined(                                                                  \
    AFX_DOWNLOADFILE_H__E7F63D02_EA09_4DD2_8D47_AC9A4FD8A98A__INCLUDED_)
...
#define DOWN_RESULT_SAMEAS                                                     \
  HTTP_RESULT_SAMEAS // 要下载的文件已经存在并且与远程文件一致，不用下载
...
public:
  void SetTimeout(ULONG ulSendTimeout, ULONG ulRecvTimeout,
                  ULONG ulConnTimeout);
  void SetProxy(int nProxyMode, LPCTSTR lpszProxyHost, int nProxyPort,
                LPCTSTR lpszProxyUserName, LPCTSTR lpszProxyPassword);

public:
  int StartDownload(LPCTSTR lpszUrlPath, BOOL nEnableResume = TRUE,
