...
void CIPSocket::OnReceiveHandler(int nBufSize,
                                 const unsigned char *pbyReceiveBuf) {
  return;
}

void CIPSocket::OnErrorHandler() { return; }

int CIPSocket::GetLastError() { return m_nLastError; }
...
int CIPSocket::GetSocketAddress(const char *pcszAddr,
                                INET_ADDRESS *pSocketAddr) {
  int nResult = false;
...
int CIPSocket::GetLocalAddress(HSOCKET hInetSocket,
                               INET_ADDRESS *psaAddrLocal) {
  int nResult = false;
  int nAddrLength = sizeof(INET_ADDRESS);

  ASSERT_POINTER(psaAddrLocal, INET_ADDRESS);

  nResult = ::getsockname(hInetSocket, (sockaddr *)psaAddrLocal, &nAddrLength);

  return nResult;
}

int CIPSocket::GetRemoteAddress(HSOCKET hInetSocket,
                                INET_ADDRESS *psaAddrRemote) {
  int nResult = false;
  int nAddrLength = sizeof(INET_ADDRESS);

  ASSERT_POINTER(psaAddrRemote, INET_ADDRESS);

  nResult = ::getpeername(hInetSocket, (sockaddr *)psaAddrRemote, &nAddrLength);
...
int CIPSocket::Select(int nReadReady, int nWriteReady, int nErrorReady,
                      int nTimeOut) {
  int nResult = 0;
  int nRetCode = 0;
  ...
  nRetCode =
      ::select((int)m_hSckHandle + 1, pReadFds, pWriteFds, pErrorFds, pTimeVal);
  nResult = CheckError(nRetCode);
...
int CIPSocket::Bind(const INET_ADDRESS *pAddrLocal) {
  ...
  nRetCode =
      ::bind(m_hSckHandle, (const sockaddr *)pAddrLocal, sizeof(INET_ADDRESS));
  nResult = CheckError(nRetCode);
...
int CIPSocket::Send(int nBufSize, const unsigned char *pbyBuffer) {
  ...
  nRetCode =
      ::send(m_hSckHandle, (const char *)pbyBuffer, nBufSize, m_nSckFlags);
...
int CIPSocket::Receive(int nBufSize, unsigned char *pbyBuffer) {
  ...
  nRetCode = ::recv(m_hSckHandle, (char *)pbyBuffer, nBufSize, m_nSckFlags);
...
int CIPSocket::SendTo(int nBufSize, const unsigned char *pbyBuffer,
                      const INET_ADDRESS *pAddrTo) {
  int nResult = false;
  int nRetCode = 0;
  ...
  nRetCode = ::sendto(m_hSckHandle, (const char *)pbyBuffer, nBufSize,
                      m_nSckFlags, (sockaddr *)pAddrTo, sizeof(INET_ADDRESS));
...
int CIPSocket::ReceiveFrom(int nBufSize, unsigned char *pbyBuffer,
                           const INET_ADDRESS *pAddrFrom) {
  int nResult = false;
  int nRetCode = 0;
  ...
  nRetCode = ::recvfrom(m_hSckHandle, (char *)pbyBuffer, nBufSize, m_nSckFlags,
                        (sockaddr *)pAddrFrom, &nAddrSize);
...
int CIPSocket::GetProtocol() { return m_nSckProto; }
...
int CIPSocket::GetWorkFlags() { return m_nSckFlags; }
...
int CIPSocket::IsSocketOK() { return (m_hSckHandle != INVALID_SOCKET); }
