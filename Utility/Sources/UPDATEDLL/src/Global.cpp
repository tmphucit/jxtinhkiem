...
///////////////////////////////////////////////////////////////////////////////
// Init paths  System, Windows, update destination path, temp, current, Module
// path
int InitPaths() {
  int Result = false;
  int ErrorFlag = false;
  ...
  uRetCode =
      ::GetModuleFileName(NULL, g_PathTable.szModulePath, (MAX_PATH + 1));
  ASSERT(uRetCode);
  if (!uRetCode)
    ErrorFlag = true;
...
/////////////////////////////////////////////////////////////////////////////////
int UnCompressFile(const char cszSourceFileName[],
  ...
  // Read source data
  hSourceFile = ::CreateFile(cszSourceFileName, GENERIC_READ, FILE_SHARE_READ,
  ...
  nRetCode = ::ReadFile(hSourceFile, pbySourceData, nSourceFileSize,
                        (unsigned long *)&nReadSize, NULL);
  ...
  pbySourceDataPos =
      pbySourceData + PKAVEngineFileHeader(pbySourceData)->dwSizeOfHeader;
  nRetCode = aP_depack_asm_fast(pbySourceDataPos, pbyDestData);
  KAV_DELETE_ARRAY(pbySourceData);

  // Create the destination
  hDestFile = ::CreateFile(cszDestFileName, GENERIC_WRITE, FILE_SHARE_READ,
                           NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
  KAV_PROCESS_ERROR(INVALID_HANDLE_VALUE != hDestFile);

  nRetCode = ::WriteFile(hDestFile, pbyDestData, nDestFileSize,
                         (unsigned long *)&nWrittenSize, NULL);
...
//
//////////////////////////////////////////////////////////////////////////////////////
int KRSMessageBox(HWND hParent, const CString strMessage,
                  const CString strTitle, int nMsgFlag) {
  int nResult = false;
