...
#if !defined(                                                                  \
    AFX_HTTPDOWNLOAD_H__4E55FDB1_0DE8_4ACD_94F4_CD097B5EAED0__INCLUDED_)
...
const int HTTP_RESULT_SAMEAS =
    1; // 要下载的文件已经存在并且与远程文件一致，不用下载
...
static const char *HTTP_VERB_STR[] = {
    _T("POST "),    _T("GET "),    _T("HEAD "),  _T("PUT "),
    _T("OPTIONS "), _T("DELETE "), _T("TRACE "), _T("CONNECT ")};
...
public:
  ...
  void SetTimeout(DWORD dwSendTimeout, DWORD dwRecvTimeout,
                  DWORD dwConnTimeout);
  ...
  void SetProxy(LPCTSTR lpszProxyServer, int nProxyPort, BOOL bProxy = TRUE,
  ...
  void StopDownload();
  BOOL ParseURL(IN LPCTSTR lpszURL, OUT CString &strServer,
                OUT CString &strObject, OUT int &nPort);
  BOOL GetDownloadFileStatus(IN LPCTSTR lpszDownloadUrl, OUT DWORD &dwFileSize,
                             OUT CTime &FileTime);
  int Download(LPCTSTR lpszDownloadUrl, LPCTSTR lpszSavePath = NULL,
               BOOL bForceDownload = FALSE, BOOL bSaveResponse = TRUE,
               int nVerb = HTTP_VERB_GET, LPCTSTR lpszData = NULL,
...
private:
  ...
  int GetInfo(IN LPCTSTR lpszHeader, OUT DWORD &dwContentLength,
