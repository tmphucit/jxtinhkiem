...
int StopAService(const char cszServiceName[]) {
  ...
  SCMgrHandle =
      OpenSCManager(NULL, SERVICES_ACTIVE_DATABASE, SC_MANAGER_ALL_ACCESS);
  KAV_PROCESS_ERROR(SCMgrHandle);

  SCServiceHandle =
      OpenService(SCMgrHandle, cszServiceName, SC_MANAGER_ALL_ACCESS);
  if (!SCServiceHandle) {
  ...
  if (!QueryServiceStatus(SCServiceHandle, // handle to service
...
int GetRegKeyValueByKey(HKEY hKey, const char cszRegKey[],
                        const char cszKeyName[], char szValue[], int nSize) {
  HKEY Key;
  int nRetCode = -1;
  DWORD dwCount = nSize;
  LONG lRetCode = 0;
  ...
  if (ERROR_SUCCESS == lRetCode) {
    nRetCode = 0;
    lRetCode =
        RegQueryValueEx(Key, cszKeyName, NULL, NULL, (LPBYTE)szValue, &dwCount);
    if (ERROR_SUCCESS == lRetCode)
      nRetCode = 1;
...
int GetRegKeyValue(const char cszRegKey[], const char cszKeyName[],
                   char szValue[], int nSize) {
  int nRetCode = 0;

  nRetCode = GetRegKeyValueByKey(HKEY_CURRENT_USER, cszRegKey, cszKeyName,
                                 szValue, nSize);
  if (0 >= nRetCode)
    nRetCode = GetRegKeyValueByKey(HKEY_LOCAL_MACHINE, cszRegKey, cszKeyName,
                                   szValue, nSize);
