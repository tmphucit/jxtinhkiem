...
#if !defined(                                                                  \
    AFX_PROCESSINDEX_H__11959E05_CF66_4A40_81F4_D13ED5311C27__INCLUDED_)
...
typedef struct tagKModuleExitInfo {
  ...
  char szReleaseFileName[MAX_PATH]; // Release Module Name : File Name or
                                    // service name
  int nExitType;
  unsigned uID;
...
private:
  ...
  int CheckIsNeedUpdate(PKUPDATE_ITEM pUpdateItem, int nMainVersion);
  int GetItemInfo(PKUPDATE_ITEM pUpdateItem, int nUpdateMethod,
                  char szLocalPath[]);

  int AddItemInfoToList(const char cszSectionName[], int nUpdateMethod,
                        char szLocalPath[], int nMainVersion);

public:
  int Uninit();
  int Init(const char cszFileName[], const char cszTxtFileName[]);
  int AnalysisUpdateFiles(int nMethod, char szLocalPath[], int nMainVersion);

  int GetExitType(PKModuleExitInfo pModuleExitInfo);
  int GetDownloadFileTypeInfo(CString &sMsg, int nDownloadFileType);
  int GetHost(char szHostName[], int *pnNameSize, char szHostURL[],
              int *pnURLSize);
