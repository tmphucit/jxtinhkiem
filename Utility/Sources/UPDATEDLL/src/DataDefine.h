...
#define defITEMTYPE_INCSIGN                                                    \
  0x01 // Increment sign files(KAVXXXXX.UPD¡¢KAVXXXXX.DAT)
...
typedef struct tagKAVSignFileHeader {
  char cszDescription[0x50]; //"Kingsoft AntiVirus Database. Copyright (c) 1999,
                             // 2000 Kingsoft AntiVirus Lab.\x1A"

  DWORD dwVersion;         // dat file version
  DWORD dwDateStamp;       // Update Date, High 16 bit is year, Low 16 bit is
                           // Month(High 8 bit),Day(Low 8 bit)
...
typedef enum _KCHECKVERSIONMETHOD {
  ...
  enumCVM_ENGINE = 1,
  enumCVM_SIGN = 2,
  enumCVM_CRC = 3, // new method, Check Version by CRC32(in old update program,
                   // download directly);
...
typedef struct tagKUPDATE_ITEM {
  ...
  char szUpdateFileMethod[MAX_PATH]; // Module name of Call by  :
                                     // KAV9X.EXE,KAVPlus.EXE
