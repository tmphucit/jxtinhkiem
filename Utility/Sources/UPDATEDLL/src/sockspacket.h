...
static const DWORD UNKNOWN_IP =
    0x10000000; // 特殊的IP地址,指明要SOCKS4A PROXY解析域名 0.0.0.1
...
public:
  BOOL SendSocks4Req(BYTE byCmd, WORD wDestPort, DWORD dwDestIP,
                     const char *pszUserID = NULL,
                     DWORD dwTimeout = SOP_SEND_TIMEOUT);
  BOOL SendSocks4aReq(BYTE byCmd, WORD wDestPort, const char *pszDestHostName,
                      const char *pszUserID = NULL,
                      DWORD dwTimeout = SOP_SEND_TIMEOUT);
  BOOL SendSocks5AuthReq(BYTE byNAuth, const char *pszAuth,
                         DWORD dwTimeout = SOP_SEND_TIMEOUT);
  BOOL SendSocks5AuthPasswdReq(const char *pszUserName, const char *pszPassword,
                               DWORD dwTimeout = SOP_SEND_TIMEOUT);
  BOOL SendSocks5Req(BYTE byCmd, BYTE byAtyp, const char *pszDestAddr,
                     WORD wDestPort, DWORD dwTimeout = SOP_SEND_TIMEOUT);
  BOOL RecvPacket(OUT PSOCKSREPPACKET pPacket, int nPacketType,
                  DWORD dwTimeout = SOP_RECV_TIMEOUT);
  BOOL IsSocksOK(PSOCKSREPPACKET pPacket, int nPacketType);
  BOOL SendPacket(PSOCKSREQPACKET pPacket, int nPacketType,
                  DWORD dwTimeout = SOP_SEND_TIMEOUT);
