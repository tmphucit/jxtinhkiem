...
// 用于BASE64编码、解码的静态常量
CString CHttpDownload::m_strBase64TAB =
...
CHttpDownload::CHttpDownload() {
  ...
  // 重试机制
  m_nRetryType = HTTP_RETRY_NONE; // 重试类型(0:不重试 1:重试一定次数
                                  // 2:总是重试)
...
//	返回值：void
////////////////////////////////////////////////////////////////////////////////
void CHttpDownload::CloseSocket() { m_cBufSocket.Close(TRUE); }
...
//				int nVerb				/*=
// HTTP_VERB_GET*/, 				LPCTSTR lpszData
// /*= NULL*/, 				LPTSTR lpszFtpURL
///*= NULL*/) 	用  途：下载入口，调用此函数来开始下载过程
///对全局变量的影响：无 	参
// 数： 		lpszDownloadUrl : 待下载的网络URL
// lpszSavePath    : 本地保存路径(可以是全路径或者仅仅是目录，目录需以\结尾)
// bForceDownload  : 是否强制下载，不管文件是否与远程一致
// bSaveResponse   : 是否保存服务器的返回 		nVerb           : HTTP命令 		lpszData :
//要在发送请求时传给服务器的数据(用于POST) 		lpszFtpURL		:
//如果重定向到FTP,此指针用来保存FTP的URL 	返回值：int 		HTTP_RESULT_STOP :
// 用户停止下载(调用了StopDownload函数) 		HTTP_RESULT_FAIL
// : 下载失败 		HTTP_RESULT_SUCCESS		: 下载成功 		HTTP_RESULT_SAMEAS
//: 本地已有完全一样的文件，不用再下载 		HTTP_RESULT_REDIRECT_FTP:
// 重定向到FTP
////////////////////////////////////////////////////////////////////////////////
int CHttpDownload::Download(LPCTSTR lpszDownloadUrl,
                            LPCTSTR lpszSavePath /*= NULL*/,
                            BOOL bForceDownload /*= FALSE*/,
                            BOOL bSaveResponse /*= TRUE*/,
  ...
  while (bSendRequestAgain) {
    ...
    if (!m_bForceDownload) // 非强制下载，检查Last-Modified
    {
      ...
      if (CFile::GetStatus(m_strSavePath, fileStatus)) {
        // 可能会存在1秒的误差
        if ((fileStatus.m_mtime - m_TimeLastModified <= 2 &&
             m_TimeLastModified - fileStatus.m_mtime <= 2) &&
            (DWORD)fileStatus.m_size == m_dwFileSize)
          return HTTP_RESULT_SAMEAS;
      }
    }

    CFile fileDown;
    if (!fileDown.Open(m_strTempSavePath,
    ...
    if (bSendOnce && m_bNotify) {
      ...
      if (WaitForSingleObject(m_hStopEvent, 0) != WAIT_OBJECT_0)
        ::SendMessage(m_hNotifyWnd, m_nNotifyMessage, MSG_HTTPDOWNLOAD_STATUS,
                      (LPARAM)&DownloadStatus);
      else
        return HTTP_RESULT_STOP;
    ...
    do {
      ...
      int nRet =
          m_cBufSocket.BSDGetData(szReadBuf, READ_BUFFER_SIZE, m_dwRecvTimeout);
...
//		HTTP_REQUEST_ERROR			:
// 发生一般网络错误，可以重试 		HTTP_REQUEST_SUCCESS		:
// 请求成功 		HTTP_REQUEST_FAIL			: 发生验证错误，不用重试
//		HTTP_REQUEST_REDIRECT_FTP	: 请求被重定向到FTP
////////////////////////////////////////////////////////////////////////////////
int CHttpDownload::SendRequest(int nVerb /*= HTTP_VERB_GET*/,
                               LPTSTR lpszFtpURL /* = NULL*/) {
  // 判断HTTP命令是否合法
  ...
  while (TRUE) {
    ...
    if (m_bSaveResponse) {
      ...
      if (CFile::GetStatus(m_strTempSavePath, fileDownStatus) &&
          !m_bForceDownload) {
    ...
    // 发送请求
    nRet = m_cBufSocket.Send((LPCTSTR)strSend, strSend.GetLength(),
                             m_dwSendTimeout);
    ...
    while (bAgain) {
      ...
      //////////////////////////////////////
      nRet = GetInfo(strHeader, dwContentLength, dwStatusCode,
                     m_TimeLastModified, strCookie, lpszFtpURL);
      switch (nRet) {
      case HTTP_FAIL:
        return HTTP_REQUEST_FAIL;
        break;
      ...
      case HTTP_SUCCESS:
        ...
        if (m_bSaveResponse) {
          ...
          if (strRange.IsEmpty()) {
            ...
            {
              m_dwFileSize = (dwContentLength == 0)
                                 ? 0
                                 : (m_dwFileDownloadedSize + dwContentLength);
...
//		nProxyPort          : 代理服务器的端口
//		bProxy              :
// 是否使用代理(该值为FALSE的时候，前两个参数将被忽略)
// bProxyAuthorization : 代理是否需要验证 		lpszProxyUsername   :
// 代理服务器验证的用户名 		lpszProxyPassword   : 代理服务器验证的口令 		nProxyType
//: 代理服务器的类型 	返回值：void
////////////////////////////////////////////////////////////////////////////////
void CHttpDownload::SetProxy(LPCTSTR lpszProxyServer, int nProxyPort,
                             BOOL bProxy /*=TRUE*/,
                             BOOL bProxyAuthorization /*= FALSE*/,
                             LPCTSTR lpszProxyUsername /*= NULL*/,
                             LPCTSTR lpszProxyPassword /*= NULL*/,
...
//	返回值：void
////////////////////////////////////////////////////////////////////////////////
void CHttpDownload::SetAuthorization(LPCTSTR lpszUsername, LPCTSTR lpszPassword,
                                     BOOL bAuthorization /* = TRUE*/) {
...
//	返回值：void
////////////////////////////////////////////////////////////////////////////////
void CHttpDownload::SetNotifyWnd(HWND hNotifyWnd, int nNotifyMsg,
                                 BOOL bNotify /*= TRUE */) {
...
//	返回值：void
////////////////////////////////////////////////////////////////////////////////
void CHttpDownload::SetTimeout(DWORD dwSendTimeout, DWORD dwRecvTimeout,
                               DWORD dwConnTimeout) {
  if (dwSendTimeout > 0)
...
//	对全局变量的影响：无
//	参  数：
//		lpszUserAgent : 新的UserAgent(例如：Mozilla/4.0 (compatible;
// MSIE 5.0; Windows 98) ) 	返回值：void
////////////////////////////////////////////////////////////////////////////////
...
//		nRetryDelay : 重试延迟
//		nRetryMax   :
// 最大重试次数(nRetryType为HTTP_RETRY_TIMES时，此值才有意义) 	返回值：void
////////////////////////////////////////////////////////////////////////////////
...
//	返回值：BOOL
////////////////////////////////////////////////////////////////////////////////
BOOL CHttpDownload::GetDownloadFileStatus(IN LPCTSTR lpszDownloadUrl,
                                          OUT DWORD &dwFileSize,
                                          OUT CTime &FileTime) {
  // 检验要下载的URL是否为空
  m_strDownloadUrl = lpszDownloadUrl;
...
//	返回值：BOOL
////////////////////////////////////////////////////////////////////////////////
BOOL CHttpDownload::ParseURL(IN LPCTSTR lpszURL, OUT CString &strServer,
                             OUT CString &strObject, OUT int &nPort) {
...
//		HTTP_SUCCESS	: 成功
////////////////////////////////////////////////////////////////////////////////
int CHttpDownload::GetInfo(IN LPCTSTR lpszHeader, OUT DWORD &dwContentLength,
                           OUT DWORD &dwStatusCode, OUT CTime &TimeLastModified,
                           OUT CString &strCookie, LPTSTR lpszFtpURL) {
  dwContentLength = 0;
  dwStatusCode = 0;
  ...
  // 检查返回码
  if (dwStatusCode >= 300 &&
      dwStatusCode < 400) // 首先检测一下服务器的应答是否为重定向
  {
...
//	返回值：int，解码后的长度
////////////////////////////////////////////////////////////////////////////////
int CHttpDownload::Base64Decode(IN LPCTSTR lpszDecoding,
                                OUT CString &strDecoded) {
  int nIndex = 0;
  int nDigit;
  int nDecode[256];
...
//	返回值：int，编码后的长度
////////////////////////////////////////////////////////////////////////////////
int CHttpDownload::Base64Encode(IN LPCTSTR lpszEncoding,
                                OUT CString &strEncoded) {
  int nDigit;
  int nNumBits = 6;
  int nIndex = 0;
...
//	返回值：int
////////////////////////////////////////////////////////////////////////////////
int CHttpDownload::MakeConnection(CBufSocket *pBufSocket, CString strServer,
                                  int nPort) {
  ...
  case HTTP_PROXY_NONE:
    Check(m_bStopDownload, HTTP_REQUEST_STOP);
    Check(!pBufSocket->Connect(strServer, nPort, m_dwConnTimeout, TRUE),
          HTTP_REQUEST_ERROR);
    break;
  case HTTP_PROXY_HTTPGET:
    Check(m_bStopDownload, HTTP_REQUEST_STOP);
    Check(!pBufSocket->Connect(m_strProxyServer, m_nProxyPort, m_dwConnTimeout,
                               TRUE),
          HTTP_REQUEST_ERROR);
    break;
  case HTTP_PROXY_SOCKS4A:
    ...
    if (dwIP == INADDR_NONE) {
      Check(m_bStopDownload, HTTP_REQUEST_STOP);
      Check(!pBufSocket->Connect(m_strProxyServer, m_nProxyPort,
                                 m_dwConnTimeout, TRUE),
            HTTP_REQUEST_ERROR);
      Check(m_bStopDownload, HTTP_REQUEST_STOP);
      Check(!cSocks.SendSocks4aReq(CMD_CONNECT, nPort, strServer,
                                   m_strProxyUsername, m_dwSendTimeout),
            HTTP_REQUEST_ERROR);

      Check(m_bStopDownload, HTTP_REQUEST_STOP);
      Check(!cSocks.RecvPacket(PACKET_SOCKS4AREP, m_dwRecvTimeout),
            HTTP_REQUEST_ERROR);
      Check(!cSocks.IsSocksOK(),
            HTTP_REQUEST_FAIL); // 请求有错误，重试可能是没有用的
  ...
  case HTTP_PROXY_SOCKS4:
    ...
    Check(!pBufSocket->Connect(m_strProxyServer, m_nProxyPort, m_dwConnTimeout,
          ...
          HTTP_REQUEST_ERROR);
    Check(m_bStopDownload, HTTP_REQUEST_STOP);
    Check(!cSocks.SendSocks4Req(CMD_CONNECT, nPort, dwIP, m_strProxyUsername,
                                m_dwSendTimeout),
          HTTP_REQUEST_ERROR);

    Check(m_bStopDownload, HTTP_REQUEST_STOP);
    Check(!cSocks.RecvPacket(PACKET_SOCKS4REP, m_dwRecvTimeout),
          HTTP_REQUEST_ERROR);
    Check(!cSocks.IsSocksOK(),
          HTTP_REQUEST_FAIL); // 请求有错误，重试可能是没有用的
    break;
  ...
  case HTTP_PROXY_SOCKS5:
    ...
    Check(m_bStopDownload, HTTP_REQUEST_STOP);
    Check(!cSocks.SendSocks5AuthReq(byAuth, (LPCTSTR)strAuth, m_dwSendTimeout),
          HTTP_REQUEST_ERROR);

    Check(m_bStopDownload, HTTP_REQUEST_STOP);

    ZeroMemory(&pack, sizeof(SOCKSREPPACKET));
    Check(!cSocks.RecvPacket(&pack, PACKET_SOCKS5AUTHREP, m_dwRecvTimeout),
          HTTP_REQUEST_ERROR);
    Check(!cSocks.IsSocksOK(&pack, PACKET_SOCKS5AUTHREP),
          HTTP_REQUEST_FAIL); // 请求有错误，重试可能是没有用的
    ...
    case AUTH_PASSWD:
      ...
      Check(m_bStopDownload, HTTP_REQUEST_STOP);
      Check(!cSocks.SendSocks5AuthPasswdReq(
                m_strProxyUsername, m_strProxyPassword, m_dwSendTimeout),
            HTTP_REQUEST_ERROR);
      Check(m_bStopDownload, HTTP_REQUEST_STOP);

      Check(!cSocks.RecvPacket(PACKET_SOCKS5AUTHPASSWDREP, m_dwRecvTimeout),
            HTTP_REQUEST_ERROR);
      Check(!cSocks.IsSocksOK(),
            HTTP_REQUEST_FAIL); // 请求有错误，重试可能是没有用的
      break;
    ...
    Check(m_bStopDownload, HTTP_REQUEST_STOP);
    Check(!cSocks.SendSocks5Req(CMD_CONNECT, byAtyp, strAddr, nPort,
                                m_dwSendTimeout),
          HTTP_REQUEST_ERROR);
    Check(m_bStopDownload, HTTP_REQUEST_STOP);
    Check(!cSocks.RecvPacket(PACKET_SOCKS5REP, m_dwRecvTimeout),
          HTTP_REQUEST_ERROR);
    Check(!cSocks.IsSocksOK(),
          HTTP_REQUEST_FAIL); // 请求有错误，重试可能是没有用的
    break;
...
//	返回值：BOOL
////////////////////////////////////////////////////////////////////////////////
BOOL CHttpDownload::GetIEProxy(CString &strProxyServer, int &nProxyPort,
                               int &nProxyType) {
  // 读取注册表
  HKEY hRegKey;
  DWORD dwRet = RegOpenKeyEx(
      HKEY_CURRENT_USER,
      "Software\\Microsoft\\Windows\\CurrentVersion\\Internet Settings", 0L,
  ...
  TCHAR szAddr[256] = {0};
  DWORD dwLen = 256;
  dwRet = RegQueryValueEx(hRegKey, "ProxyServer", NULL, NULL, (LPBYTE)szAddr,
                          &dwLen);
  if (dwRet != ERROR_SUCCESS) {
    RegCloseKey(hRegKey);
    return FALSE;
...
//	返回值：void
////////////////////////////////////////////////////////////////////////////////
void CHttpDownload::SetCookie(LPCTSTR lpszCookie,
                              BOOL bUseIECookie /*= FALSE*/) {
  if (bUseIECookie) {
    m_bUseIECookie = TRUE;
    /*
...
BOOL CHttpDownload::GetIECookie(CString &strCookie, LPCTSTR lpszURL) {
  /*
     ...
     Folders\ Cookies = ***
          ...
          But this way need the Wininet.dll which i don't want to use,How can i
     do? OR: You can call : CInternetSession::GetCookie(***,***,strCookie);
...
BOOL CHttpDownload::IsValidFileName(LPCTSTR lpszFileName) {
  /*
     ...
     characters:
          +  ,  ;  =  [  ]
          Short file names may not include a space, although a long file name
     may. No space is allowed preceding the vertical bar (|) separator for the
     short file name/long file name syntax. If a space exists after the
     separator, then the long file name must have a space at the beginning of
     the file name. No full-path syntax is allowed.
  */
