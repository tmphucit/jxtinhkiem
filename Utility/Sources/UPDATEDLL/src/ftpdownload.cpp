...
//	返回值：BOOL
////////////////////////////////////////////////////////////////////////////////
BOOL CFtpDownload::GetIEProxy(CString &strProxyServer, int &nProxyPort,
                              int &nProxyType) {
  // 读取注册表
  HKEY hRegKey;
...
// 设置代理
void CFtpDownload::SetProxy(LPCTSTR lpszProxyServer, int nProxyPort,
                            BOOL bProxy /*=TRUE*/,
                            BOOL bProxyAuthorization /*=FALSE*/,
                            LPCTSTR lpszProxyUsername /*=NULL*/,
                            LPCTSTR lpszProxyPassword /*=NULL*/,
                            int nProxyType /*=FTP_PROXY_NONE*/) {
...
// 设置超时
void CFtpDownload::SetTimeout(DWORD dwSendTimeout, DWORD dwRecvTimeout,
                              DWORD dwConnTimeout) {
  if (dwSendTimeout > 0)
...
// 设置用户名和口号
void CFtpDownload::SetAuthorization(LPCTSTR lpszUsername, LPCTSTR lpszPassword,
                                    BOOL byAuthorization) {
...
// 设置PASV模式
void CFtpDownload::SetPasv(BOOL bPasv /*= TRUE*/) { m_bPasv = bPasv; }
...
BOOL CFtpDownload::CreateDataSocket(SOCKET hSocket /*= INVALID_SOCKET */) {
  CloseDataSocket();
  return m_cDataSocket.Create(AF_INET, SOCK_STREAM, 0, READ_BUFFER_SIZE,
                              hSocket);
}

// 关闭控制SOCKET
void CFtpDownload::CloseControlSocket() { m_cControlSocket.Close(TRUE); }

// 关闭数据SOCKET
void CFtpDownload::CloseDataSocket() { m_cDataSocket.Close(TRUE); }

// 从URL里面拆分出Server和Object来
BOOL CFtpDownload::ParseURL(LPCTSTR lpszURL, CString &strServer,
                            CString &strObject, int &nPort) {
...
// 获得返回码
BOOL CFtpDownload::GetReplyCode(LPCTSTR lpszResponse, DWORD &dwReplyCode,
                                BOOL &bMultiLine) {
...
BOOL CFtpDownload::GetDataReply(CString &strReply) {
  ...
  if (m_cDataSocket.BSDGetString(szReadBuf, 1024, m_dwRecvTimeout) !=
      SOCKET_SUCCESS)
    return FALSE;
...
BOOL CFtpDownload::GetReply(CString &strReply, DWORD &dwReplyCode) {
  ...
  // 先接收一行
  ZeroMemory(szReadBuf, 1025);
  if (m_cControlSocket.BSDGetString(szReadBuf, 1024, m_dwRecvTimeout) !=
      SOCKET_SUCCESS)
    return FALSE;
...
BOOL CFtpDownload::SendCommand(LPCTSTR lpszCmd, LPCTSTR lpszParameter) {
  ...
  if (m_cControlSocket.Send(strCmd, strCmd.GetLength(), m_dwSendTimeout) !=
      strCmd.GetLength())
    return FALSE;
...
// 从Pasv的返回信息中获得IP和端口
BOOL CFtpDownload::GetPasvIP(LPCTSTR lpszReply, CString &strPasvIP,
                             int &nPasvPort) {
...
// 获取文件时间和大小
BOOL CFtpDownload::GetInfo(LPCTSTR lpszReply, DWORD &dwSize,
  ...
  //-r-xr-xr-x   1 0        0         986400 Jun 16  1998
  /// pub/Networks/Socket/Winsock/ws2setup.exe
...
int CFtpDownload::SendRequest(BOOL bPasv /*= TRUE*/) {
  ...
  while (TRUE) {
    ...
    Check(m_bStopDownload, FTP_REQUEST_STOP);
    Check(!GetReply(strReply, dwReplyCode),
          FTP_REQUEST_ERROR); // 先获取服务器的Greeting信息
    // seawind
    Check(m_bStopDownload, FTP_REQUEST_STOP);
    ...
    } else // 不采用PASV
    {
      ...
      if (m_nProxyType == FTP_PROXY_SOCKS4 ||
        ...
        Check(m_bStopDownload, FTP_REQUEST_STOP);
        nRet = MakeConnection(&m_cDataSocket, m_strServer, m_nPort, TRUE,
                              &dwListenIP, &nListenPort);
        Check(nRet != FTP_REQUEST_SUCCESS, nRet);
        strListenIP.Format("%d,%d,%d,%d", (dwListenIP >> 24) & 0xFF,
                           (dwListenIP >> 16) & 0xFF, (dwListenIP >> 8) & 0xFF,
                           (dwListenIP) & 0xFF);
      } else {
        // 没有代理
        Check(m_bStopDownload, FTP_REQUEST_STOP);
        ...
        Check(!m_cControlSocket.GetSockName(szListenIP, nListenPort),
              FTP_REQUEST_ERROR);

        ZeroMemory(szTemp, 128);
        Check(!cListenSocket.GetSockName(szTemp, nListenPort),
              FTP_REQUEST_ERROR);
      ...
      if ((dwReplyCode == 125) || (dwReplyCode == 150)) {
        if (m_nProxyType == FTP_PROXY_SOCKS4 ||
          ...
          // ACCEPT
          SOCKET hSocket =
              cListenSocket.Accept(NULL, 0, m_hStopEvent, WSA_INFINITE);
    ...
    // seawind
    Check(m_bStopDownload, FTP_REQUEST_STOP);
    Check((dwReplyCode != 125 && dwReplyCode != 150 && dwReplyCode != 110),
          FTP_REQUEST_FAIL);
...
// 开始下载
int CFtpDownload::Download(LPCTSTR lpszDownloadUrl, LPCTSTR lpszSavePath,
...
ReDownload:
  ...
  if (bSendOnce && m_bNotify) {
    ...
    if (WaitForSingleObject(m_hStopEvent, 0) != WAIT_OBJECT_0)
      ::SendMessage(m_hNotifyWnd, m_nNotifyMessage, MSG_FTPDOWNLOAD_STATUS,
  ...
  do {
    ...
    int nRet =
        m_cDataSocket.BSDGetData(szReadBuf, READ_BUFFER_SIZE, m_dwRecvTimeout);
...
// 建立连接
int CFtpDownload::MakeConnection(CBufSocket *pBufSocket, CString strServer,
                                 int nPort, BOOL bBind /*= FALSE*/,
                                 LPDWORD lpdwIP /*= NULL*/,
                                 LPINT lpnPort /*= NULL*/) {
  ...
  // 检查是否为BIND命令
  if (bBind &&
      (m_nProxyType == FTP_PROXY_SOCKS4 || m_nProxyType == FTP_PROXY_SOCKS4A ||
       m_nProxyType == FTP_PROXY_SOCKS5)) {
    byCmd = CMD_BIND;
  ...
  case FTP_PROXY_SOCKS4A:
    ...
    if (dwIP == INADDR_NONE) {
      ...
      Check(m_bStopDownload, FTP_REQUEST_STOP);
      Check(!cSocks.SendSocks4aReq(byCmd, nPort, strServer, m_strProxyUsername,
                                   m_dwSendTimeout),
            FTP_REQUEST_ERROR);
      Check(m_bStopDownload, FTP_REQUEST_STOP);

      ZeroMemory(&pack, sizeof(SOCKSREPPACKET));
      Check(!cSocks.RecvPacket(&pack, PACKET_SOCKS4AREP, m_dwRecvTimeout),
            FTP_REQUEST_ERROR);
      Check(!cSocks.IsSocksOK(&pack, PACKET_SOCKS4AREP),
            FTP_REQUEST_FAIL); // 请求有错误，重试可能是没有用的
  ...
  case FTP_PROXY_SOCKS4:
    ...
    Check(m_bStopDownload, FTP_REQUEST_STOP);
    Check(!cSocks.SendSocks4Req(byCmd, nPort, dwIP, m_strProxyUsername,
                                m_dwSendTimeout),
          FTP_REQUEST_ERROR);

    ZeroMemory(&pack, sizeof(SOCKSREPPACKET));
    Check(!cSocks.RecvPacket(&pack, PACKET_SOCKS4REP, m_dwRecvTimeout),
          FTP_REQUEST_ERROR);
    Check(!cSocks.IsSocksOK(&pack, PACKET_SOCKS4REP),
          FTP_REQUEST_FAIL); // 请求有错误，重试可能是没有用的
  ...
  case FTP_PROXY_SOCKS5:
    ...
    case AUTH_PASSWD:
      ...
      ZeroMemory(&pack, sizeof(SOCKSREPPACKET));
      Check(!cSocks.RecvPacket(&pack, PACKET_SOCKS5AUTHPASSWDREP,
                               m_dwRecvTimeout),
            FTP_REQUEST_ERROR);
      Check(!cSocks.IsSocksOK(&pack, PACKET_SOCKS5AUTHPASSWDREP),
            FTP_REQUEST_FAIL); // 请求有错误，重试可能是没有用的
      break;
    case AUTH_GSSAPI:
    case AUTH_CHAP:
    ...
    Check(m_bStopDownload, FTP_REQUEST_STOP);
    Check(!cSocks.SendSocks5Req(byCmd, byAtyp, strAddr, nPort, m_dwSendTimeout),
          FTP_REQUEST_ERROR);
    Check(m_bStopDownload, FTP_REQUEST_STOP);

    ZeroMemory(&pack, sizeof(SOCKSREPPACKET));
    Check(!cSocks.RecvPacket(&pack, PACKET_SOCKS5REP, m_dwRecvTimeout),
          FTP_REQUEST_ERROR);
    Check(!cSocks.IsSocksOK(&pack, PACKET_SOCKS5REP),
          FTP_REQUEST_FAIL); // 请求有错误，重试可能是没有用的
