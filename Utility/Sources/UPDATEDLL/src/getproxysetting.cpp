...
static int GetIEProxySetString(int nProxySetSize, char *pszProxySetString) {
  ...
  lRetCode = RegOpenKeyEx(HKEY_CURRENT_USER, g_szRegistryIESettingPath, 0L,
  ...
  lRetCode = RegQueryValueEx(hRegKey, "ProxyServer", NULL, NULL,
                             (LPBYTE)pszProxySetString, &dwLength);
  if (lRetCode != ERROR_SUCCESS)
    goto Exit0;
...
int GetIEProxySocks5(int nSvrNameSize, char *pszProxySvrName,
                     int *pnProxyPort) {
  int nResult = false;
  int nRetCode = 0;
...
int IsIEProxyEnable() {
  ...
  lRetCode = RegQueryValueEx(hRegKey, "ProxyEnable", NULL, NULL,
