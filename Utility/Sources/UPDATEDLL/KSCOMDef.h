...
#define BEGIN_INTERFACE                                                        \
  virtual void __KS_VC_COMPATIBILITY_GCC__a() {}                               \
  virtual void __KS_VC_COMPATIBILITY_GCC__b() {}
#endif
...
inline int KSCOM_EQUAL_UUID(const UUID &cruuid1, const UUID &cruuid2) {
  typedef int *PINT32;
  return (((((PINT32)&cruuid1)[0]) == (((PINT32)&cruuid2)[0])) &&
...
KSCOMRESULT KSCOMSTDMETHOD KSCoInitialize(void);

KSCOMRESULT KSCOMSTDMETHOD KSCoUnInitialize(void);

KSCOMRESULT KSCOMSTDMETHOD KSCoCreateInstance(const UUID &cruuidCLSID,
                                              IUnknown *pUnkOuter,
                                              const UUID &cruuidIID,
                                              void **ppvObject);

KSCOMRESULT KSCOMSTDMETHOD KSCoGetClassObject(const UUID &cruuidCLSID,
                                              const UUID &cruuidIID,
                                              void **ppvClassObject);

KSCOMRESULT KSCOMSTDMETHOD KSCoRegisterClassObject(const UUID &cruuidCLSID,
                                                   const UUID &cruuidIID,
                                                   IUnknown *pUnkInterface,
                                                   KSCOMHANDLE *phRegister);

KSCOMRESULT KSCOMSTDMETHOD KSCoUnRegisterClassObject(KSCOMHANDLE hRegister);

KSCOMRESULT KSCOMSTDMETHOD KSCoRegisterComponent(
    const UUID &cruuidCLSID, const UUID &cruuidIID,
    TKSCOM_REGISTER_CLASS_OBJECT_MODE RegisterMode, int nClassObjectSize,
    void *pvClassObject, KSCOMHANDLE *phRegister);

KSCOMRESULT KSCOMSTDMETHOD KSCoUnRegisterComponent(KSCOMHANDLE hRegister);

KSCOMRESULT KSCOMSTDMETHOD KSCoFreeUnusedLibraries(void);

// =========================================================================
// Function for Running Object Table Object
// =========================================================================
KSCOMRESULT KSCOMSTDMETHOD KSGetActiveObject(const UUID &cruuidCLSID,
                                             IUnknown **ppUnkActiveObject);

KSCOMRESULT KSCOMSTDMETHOD KSRegisterActiveObject(IUnknown *pUnkActiveObject,
                                                  const UUID &cruuidCLSID,
                                                  KSCOMHANDLE *phRegister);

KSCOMRESULT KSCOMSTDMETHOD KSUnRegisterActiveObject(KSCOMHANDLE hRegister);
