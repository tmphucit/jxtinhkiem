...
public:
  ...
  virtual ~cTimer() {};
  LARGE_INTEGER c1;
  void SetTimer() { BOOL b = QueryPerformanceCounter(&c1); }
  DWORD GetTimeMS() {
    LARGE_INTEGER c2;
    LARGE_INTEGER freq;
...
public:
  cTimerFunction(LPCSTR szFunctionName, DWORD nTimeRange = 10,
                 BOOL bAutoSettimer = TRUE)
      : cTimer(bAutoSettimer) {
...
public:
  cTimerLimit(DWORD ms, BOOL bAutoSettimer = TRUE) : cTimer(bAutoSettimer) {
    m_nMs = ms;
  } // ms ∫¡√Î
  DWORD m_nMs;
  virtual ~cTimerLimit() {
