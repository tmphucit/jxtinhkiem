...
public:
  ...
  HRESULT CalcCenterOrgForCurrentView(const SIZE *pImgSize, POINT *pPtOrg,
                                      BOOL bSetOrigin = FALSE);
  VOID SetDefaultText(const TCHAR *pctszDefaultText, DWORD length);
  VOID SetColors(COLORREF crFore, COLORREF crBack, COLORREF crHighlight,
                 COLORREF crHighlightLine);
  VOID GetColors(COLORREF *pCrFore, COLORREF *pCrBack, COLORREF *pCrHighlight,
                 COLORREF *pCrHighlightLine);
...
protected:
  ...
  VOID CalcSizes(const DIDEVICEIMAGEINFO *prgImageInfoArray,
                 DWORD dwNumElements, const LPDWORD pNumViews,
                 LPDWORD pNumCallouts, LPDWORD pNumDistinctObjID);
  HRESULT RenderView(HDC hdc, VOID *pBits, INT width, INT height);

  HRESULT InitCustomViews(LPDIRECTINPUTDEVICE8 pDevice,
...
struct DIDCfgViewInfo {
  ...
  DIDCfgViewInfo()
      : bIsInit(FALSE), iCurrentViewID(DIDCV_INVALID_ID), iNumTotalViews(0),
        dwNumUniqueCallouts(0), bCanBeCollapsed(FALSE), bIsCollapsed(FALSE) {}
};
...
struct DidcvCustomViewInfo {
  ...
  DidcvCustomViewInfo() : dwType(0), dwCount(0), dwSize(0), rgObjData(NULL) {}
...
public:
  ...
  virtual void Render(HDC hdc, VOID *pBits, INT width, INT height,
                      const POINT *pPtOrigin) = 0;
...
public:
  ...
  static DidcvCustomView *Create(const DidcvCustomViewInfo *pInfo,
                                 DWORD dwStartIndex, DWORD *pFinishIndex);
  static BOOL CalcImageInfo(DWORD index, LPRECT rcOverlay, LPDWORD pNumPoints,
                            LPPOINT rgptCalloutLine, LPRECT rcCalloutRect);
  static DWORD CalcNumViews(DWORD dwCount);

private:
  // constructor
  DidcvCustomView(const DidcvCustomViewInfo *pInfo, DWORD dwStartIndex,
                  DWORD dwEndIndex);
...
protected:
  // table holding references to views, indexed by the internal unique
  // identifier
...
public:
  ...
  // draw the overlay
  DWORD DrawOverlay(HDC hDC, VOID *lpBits, INT width, INT height,
                    const POINT *pptOrigin);
...
//-----------------------------------------------------------------------------
template <class Item> class GwArray {
public:
  // constructors
  GwArray() {
    this->Alloc(GW_ARRAY_DEFAULT_SIZE);
    m_size = 0;
  }
  GwArray(UINT initCap) {
    this->Alloc(initCap);
    m_size = 0;
  }
  ~GwArray() {
...
public:
  ...
  BOOL EnumObjects(LPDIRECTINPUTDEVICE8 pDevice,
                   LPDIENUMDEVICEOBJECTSCALLBACK pCallback, LPVOID pvRef,
                   DWORD dwMapOnly);
  BOOL CalcCanBeCollapsed();

  // allocation
  BOOL SetCapacity(DWORD dwNumCallouts, DWORD dwNumUniqueObjID,
                   DWORD dwNumViews, BOOL bDeleteContent = TRUE);
  void TrimArrays();
  void CleanUp();

protected:
  // helper functions
  DidcvCalloutSet *Find(const GwArray<DidcvCalloutSet *> &array,
...
struct DidcvCalloutState {
  ...
  // --- member functions ---
  DidcvCalloutState(DWORD f = 0, BOOL c = FALSE, BOOL o = FALSE, BOOL h = FALSE,
                    BOOL e = FALSE, BOOL d = FALSE)
...
// utility functions
void DidcvPolyLineArrow(HDC hDC, const POINT *rgpt, INT nPoints,
                        BOOL bDoShadow = FALSE);
...
#define SPOINT_INITIALIZERS                                                    \
  p(u.p), s(u.s), a(((int *)(void *)u.a)), x(u.p.x), y(u.p.y), cx(u.s.cx),     \
      cy(u.s.cy)
      : SPOINT_INITIALIZERS {
    x = y = 0;
  }
  ...
  SPOINT(int, POINT *r)
      : p(*r), s(*((SIZE *)(void *)r)), a(((int *)(void *)r)), x(r->x), y(r->y),
        cx(r->x), cy(r->y) {}

  SPOINT(const SPOINT &sp) : SPOINT_INITIALIZERS { p = sp.p; }

  SPOINT(int b, int c) : SPOINT_INITIALIZERS {
    x = b;
    y = c;
  }

  SPOINT(const POINT &point) : SPOINT_INITIALIZERS { p = point; }

  SPOINT(const SIZE &size) : SPOINT_INITIALIZERS { s = size; }
...
#if (WINVER >= 0x400)
typedef WINGDIAPI BOOL(WINAPI *DIDCV_ALPHABLEND)(HDC, int, int, int, int, HDC,
                                                 int, int, int, int,
                                                 BLENDFUNCTION);
#else
...
public:
  ...
  static BOOL Blend(HDC hDC, INT xStart, INT yStart, INT width, INT height,
                    HBITMAP hbitmap, const SIZE *lpSize);
  static BOOL Blend(VOID *lpDestBits, INT xStart, INT yStart, INT destWidth,
                    INT destHeight, VOID *lpSrcBits, INT srcWidth,
                    INT srcHeight);
