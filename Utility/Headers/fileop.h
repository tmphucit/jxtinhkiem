...
_declspec(dllexport) BOOL AppCopyFile(LPCSTR sSrc, LPCSTR sDes,
...
_declspec(dllexport) BOOL AppFindFile(LPCSTR sFileName, LPCSTR sDir,
                                      CString &sPathName);

_declspec(dllexport) BOOL AppGetPathDialog(CString &strPath,
                                           LPCSTR szRoot = NULL);
_declspec(dllexport) BOOL AppGetFileDialog(CString &strFile,
                                           LPCSTR szPath = NULL,
                                           LPCSTR szFilter = "*.*",
                                           BOOL Load = TRUE);

_declspec(dllexport) BOOL AppOpenFile(CFile &file, LPCSTR szFile, long flag,
                                      LPCSTR szLocate);
