...
cRegion::~cRegion() { FinalCleanup(); }

////////////////////////////////////////////////////////////////////
// com
////////////////////////////////////////////////////////////////////
ULONG cRegion::AddRef() { return ++m_ulCount; }
...
cCamera *cRegion::GetCamera() { return GetGraphics()->GetcCamera(); }

cGround *cRegion::GetGround() { return GetGraphics()->GetcGround(); }

cScreen *cRegion::GetScreen() { return GetGraphics()->GetcScreen(); }

LPDIRECT3DDEVICE8 cRegion::GetDevice() { return GetGraphics()->GetDevice3D(); }
...
void cRegion::ClearModify() { m_lModify = 0; }

BOOL cRegion::IsModify(long mask) { return m_lModify & mask; };

void cRegion::ClearModify(long mask) { m_lModify &= (~mask); }

void cRegion::AddModify(long mask) { m_lModify |= mask; }
...
void cRegion::GetRegionFolder(CString &strFolder) { strFolder = m_strName; }
...
HRESULT cRegion::LoadForMap(LPCSTR szFolder) {
  ...
  for_all(m_listPlane,
          bind2nd(mem_fun1<bool, cGroundObj, CPoint>(&cGroundObj::SetRegion),
                  ptRegion));
  //	for_all(m_listLight,bind2nd(mem_fun1<bool,cGroundObj,CPoint>(&cGroundObj::SetRegion),ptRegion));
  for_all(m_listObstacle,
          bind2nd(mem_fun1<bool, cGroundObj, CPoint>(&cGroundObj::SetRegion),
                  ptRegion));
  for_all(m_listClientNpc,
          bind2nd(mem_fun1<bool, cGroundObj, CPoint>(&cGroundObj::SetRegion),
                  ptRegion));

  for_all(m_listPlane,
          mem_fun<HRESULT, cPlane>(cPlane::FillVertexInfoNotCallGraphics));
...
HRESULT cRegion::Load(LPCSTR szFolder) {
  ...
  for_all(m_listPlane,
          bind2nd(mem_fun1<bool, cGroundObj, CPoint>(&cGroundObj::SetRegion),
                  ptRegion));
  for_all(m_listPlaneVerBelowGround,
          bind2nd(mem_fun1<bool, cGroundObj, CPoint>(&cGroundObj::SetRegion),
                  ptRegion));
  for_all(m_listLight,
          bind2nd(mem_fun1<bool, cGroundObj, CPoint>(&cGroundObj::SetRegion),
  ...
  for_all(m_listPlane,
          bind2nd(mem_fun1<BOOL, cPlane, BOOL>(cPlane::SetInGround), true));
  for_all(m_listPlaneVerBelowGround,
          bind2nd(mem_fun1<BOOL, cPlane, BOOL>(cPlane::SetInGround), true));
  for_all(m_listLight,
          bind2nd(mem_fun1<BOOL, cLight, BOOL>(cLight::SetInGround), true));
  for_all(
      m_listObstacle,
      bind2nd(mem_fun1<BOOL, cObstacle, BOOL>(cObstacle::SetInGround), true));
  for_all(m_listClientNpc,
          bind2nd(mem_fun1<BOOL, cPlane, BOOL>(cPlane::SetInGround), true));
...
HRESULT cRegion::Save(LPCSTR szFolder) {
  ...
  hFile = CreateFile(szTemp, GENERIC_WRITE, FILE_SHARE_READ, NULL, OPEN_ALWAYS,
...
HRESULT cRegion::LoadTerrain() {
  ...
  if (pPack != NULL) {
    ...
    if (pOff != NULL) {
      ...
      memcpy(m_aGroundTile, pOff,
             sizeof(stTerrainTile) * (c_tWidthRegion + 1) *
                 (c_tHeightRegion + 1));
      for (int i = 0; i < c_tWidthRegion + 1; i++)
        for (int j = 0; j < c_tHeightRegion + 1; j++) {
          //				m_aTile[i][j].v.sx =
          // m_aGroundTile[i][j].ptPos.x;
          ...
          if (m_aTile[i][j].idResource != -1) {
            sprintf(m_aTile[i][j].szResource, "%s\\%04d.spr",
                    FOLDER_SPR_TERRAIN, m_aTile[i][j].idResource);
          } else
...
HRESULT cRegion::LoadLight() {
  ...
  if (pPack != NULL) {
    ...
    if (pOffset == NULL)
      return S_OK;
    return T_LoadPackList<cLightList, cLight>(m_listLight, GetGraphics(),
                                              pOffset);
  }
...
HRESULT cRegion::LoadObstacle() {
  ...
  if (pPack != NULL) {
    ...
    if (pOffset == NULL)
      return S_OK;
    return T_LoadPackList<cObstacleList, cObstacle>(m_listObstacle,
                                                    GetGraphics(), pOffset);
  }
  T_LoadList<cObstacleList, cObstacle>(m_listObstacle, file, GetGraphics());
  return S_OK;
  /*
          for (itcObstacle it = m_listObstacle.begin(); it !=
     m_listObstacle.end(); ++it)
          {
                  cObstacle* p = (*it);
                  p->ClearModify();
          }
...
HRESULT cRegion::SavePlaneVer() {
  ...
  cPlaneList rlist;
  insert_if(
      m_listPlane.begin(), m_listPlane.end(), rlist,
      bind2nd(mem_fun1<BOOL, cPlane, SortEnum>(&cPlane::IsSortType), SORT_VER));
...
HRESULT cRegion::SavePlaneHor() {
  ...
  CString strFile = m_strName + "\\" + FILE_PLANEHORBELOWVER;
  cPlaneList rlist;
  insert_if(m_listPlane.begin(), m_listPlane.end(), rlist,
            bind2nd(mem_fun1<BOOL, cPlane, SortEnum>(&cPlane::IsSortType),
                    SORT_HORGROUND));
  insert_if(m_listPlane.begin(), m_listPlane.end(), rlist,
            bind2nd(mem_fun1<BOOL, cPlane, SortEnum>(&cPlane::IsSortType),
                    SORT_HORBELOWVER));
...
HRESULT cRegion::SaveLight() {
  ...
  //	{
  //		for (itcLight it = m_listLight.begin(); it != m_listLight.end();
  //++it)
  //		{
  //			cLight* p = (*it);
  //			if (p->IsModify())
...
HRESULT cRegion::SaveObstacle() {
  ...
  {
    for (itcObstacle it = m_listObstacle.begin(); it != m_listObstacle.end();
         ++it) {
      cObstacle *p = (*it);
      p->ClearModify();
    }
...
HRESULT cRegion::SaveClientOnlyNpc() {
  ...
  for (itcPlane it = m_listClientNpc.begin(); it != m_listClientNpc.end();
       ++it) {
    cPlane *p = (*it);
...
HRESULT cRegion::BeginPainting() {
  ...
  for_all(m_listPlaneVerBelowGround,
          mem_fun<HRESULT, cPlane>(&cPlane::BeginPainting));
...
HRESULT cRegion::EndPainting() {
  ...
  for_all(m_listPlaneVerBelowGround,
          mem_fun<HRESULT, cPlane>(&cPlane::EndPainting));
...
void cRegion::CreateTerrainTexture() {
  if (m_pTexture == NULL) {
    ...
    SetResource(pos);
    pos.stDevice.eMedia = MEDIA_MEMORY;
    iTexture *pTexture = GetGraphics()->CreateiTexture(
        pos, c_nWidthRegionTexture, c_nHeightRegionTexture);
...
#pragma message(ATTENTION("when use 2 在yes那里掉图素"))
  ...
  CRect rcTexture(rect.left * c_nWidthTile, rect.top * c_nHeightTile,
                  rect.right * c_nWidthTile, rect.bottom * c_nHeightTile);
...
void cRegion::DrawHor(cPlaneHor *pPlane, const cRect &rect) {
  ...
  // 直接转换成为int会有一个像素的偏差.
  render.ptDes =
      CPoint(GetInt(ptDes.x * c_ptScale.x), GetInt(ptDes.y * c_ptScale.x / 2));
  render.nFrameDes = 0;
  render.pTexSrc = pTex;
  render.rcSrc =
      CRect(GetInt(rcSrc.l * c_ptScale.x), GetInt(rcSrc.t * c_ptScale.x / 2),
...
bool cRegion::BeginLighting(cColor color) {
  ...
  for_all(
      m_listPlaneVerBelowGround,
      bind2nd(mem_fun1<bool, cPlane, cColor>(&cPlane::BeginLighting), color));
  return true;
}
...
HRESULT cRegion::InitDeviceObjects() {
  ...
  for_all(m_listPlaneVerBelowGround,
          mem_fun<HRESULT, cPlane>(&cPlane::InitDeviceObjects));
  for_all(m_listClientNpc, mem_fun<HRESULT, cPlane>(&cPlane::EndPainting));
  if (m_pMesh)
...
HRESULT cRegion::RestoreDeviceObjects() {
  ...
  for_all(m_listPlaneVerBelowGround,
          mem_fun<HRESULT, cPlane>(&cPlane::RestoreDeviceObjects));
  for_all(m_listClientNpc,
          mem_fun<HRESULT, cPlane>(&cPlane::RestoreDeviceObjects));
  if (m_pMesh)
...
HRESULT cRegion::InvalidateDeviceObjects() {
  ...
  for_all(m_listLight,
          mem_fun<HRESULT, cLight>(&cLight::InvalidateDeviceObjects));
  for_all(m_listPlane,
          mem_fun<HRESULT, cPlane>(&cPlane::InvalidateDeviceObjects));
  for_all(m_listPlaneVerBelowGround,
          mem_fun<HRESULT, cPlane>(&cPlane::InvalidateDeviceObjects));
  for_all(m_listClientNpc,
          mem_fun<HRESULT, cPlane>(&cPlane::InvalidateDeviceObjects));
...
HRESULT cRegion::DeleteDeviceObjects() {
  ...
  for_all(m_listPlaneVerBelowGround,
          mem_fun<HRESULT, cPlane>(&cPlane::DeleteDeviceObjects));
  for_all(m_listClientNpc,
          mem_fun<HRESULT, cPlane>(&cPlane::DeleteDeviceObjects));
  if (m_pMesh)
...
HRESULT cRegion::FinalCleanup() {
  ...
  for_all(m_listLight,
          bind2nd(mem_fun1<BOOL, cLight, BOOL>(&cLight::SetInGround), FALSE));
  ...
  for_all(
      m_listObstacle,
      bind2nd(mem_fun1<BOOL, cObstacle, BOOL>(&cObstacle::SetInGround), FALSE));
  for_all(m_listObstacle, mem_fun<ULONG, cObstacle>(&cObstacle::Release));
  m_listObstacle.clear();

  for_all(m_listPlane,
          bind2nd(mem_fun1<BOOL, cPlane, BOOL>(&cPlane::SetInGround), FALSE));
  ...
  for_all(m_listPlaneVerBelowGround,
          bind2nd(mem_fun1<BOOL, cPlane, BOOL>(&cPlane::SetInGround), FALSE));
  for_all(m_listPlaneVerBelowGround,
          mem_fun<HRESULT, cPlane>(&cPlane::FinalCleanup));
...
BOOL cRegion::AddLinkHorFromRegion(cRegion *pRegion) {
  ...
  */
  ...
  cRect rc;
  //	for (itcPlaneHor it = plist.begin(); it != plist.end(); ++it)
  for (itcPlane it = pRegion->m_listPlane.begin();
       it != pRegion->m_listPlane.end(); ++it) {
    cPlaneHor *p = dynamic_cast<cPlaneHor *>(*it);
    if (p == NULL)
      continue;
    p->GetGroundRect(rc);
...
BOOL cRegion::AddLinkHor(cPlaneHor *p) {
  for (itcPlaneHor it = m_listLinkPlaneHor.begin();
       it != m_listLinkPlaneHor.end(); ++it) {
    if ((*it) == (p)) // p 不应该在列表中
      return FALSE;
  }
  p->AddRef();
...
HRESULT cRegion::PaintHorBelow() { return S_OK; }

HRESULT cRegion::PaintVer(int yTile) { return S_OK; }

HRESULT cRegion::PaintVer() { return S_OK; }

HRESULT cRegion::PaintHorAbove() { return S_OK; }
...
HRESULT cRegion::DrawTrace() {
  if (m_param.lMapeditEnum & Mapedit_Select) {
    ...
    char s[256];
    sprintf(s,
            "(F3)Region: wind %.2f,%.2f,%.2f CR %d,%d Color %d,%d,%d ani %d "
            "speed %d degree %.1f",
            pt.x, pt.y, pt.z, m_x, m_y, m_param.crColor.r, m_param.crColor.g,
            m_param.crColor.b, m_param.eAnimation, m_param.nSpeed,
            m_param.fDegree);
    GetGraphics()->Trace(s);
  }
  return S_OK;
...
HRESULT cRegion::PaintOthers() {
  ...
  for_all(m_listPlaneVerBelowGround,
          mem_fun<HRESULT, cPlane>(&cPlane::DrawSelect));
  for_all(m_listPlaneVerBelowGround,
          mem_fun<HRESULT, cPlane>(&cPlane::DrawTrace));
...
HRESULT cRegion::FilterByY(SelectFilterParamEnum param, cPlaneList &listIn,
                           cPlaneList &listOut) {
  int y = INT_MIN;
  cPlane *pGet = NULL;
...
HRESULT cRegion::FilterByZ(SelectFilterParamEnum param, cPlaneList &listIn,
                           cPlaneList &listOut) {
  int z = INT_MIN;
  cPlane *pGet = NULL;
  for (itcPlane it = listIn.begin(); it != listIn.end(); ++it) {
...
HRESULT cRegion::FilterByDisplay(SelectFilterParamEnum param,
                                 cPlaneList &listIn, cPlaneList &listOut) {
...
HRESULT cRegion::FilterByOffset(SelectFilterParamEnum param, cPlaneList &listIn,
                                cPlaneList &listOut, POINT point) {
  cPoint ptGround;
...
HRESULT cRegion::FilterByArea(SelectFilterParamEnum param, cPlaneList &listIn,
                              cPlaneList &listOut) {
...
HRESULT cRegion::GetiLight(stSelect &st, cLightList &tlist) {
  ...
  case SelectPoint_RectByDiamond: {
    insert_if(ilist.begin(), ilist.end(), jlist,
              bind2nd(mem_fun1<BOOL, cLight, cRect>(&cLight::ContainedDiamond),
                      st.rcGround));
  } break;
  }
...
HRESULT cRegion::GetiObstacle(stSelect &st, cObstacleList &tlist) {
  ...
  case SelectPoint_OneByScreen: {
    itcObstacle it =
        find_if(ilist.begin(), ilist.end(),
                bind2nd(mem_fun1<BOOL, cObstacle, POINT>(&cObstacle::Contain),
                        st.ptScreen));
    if (it != ilist.end()) {
      jlist.push_back(*it);
    } else {
  ...
  case SelectPoint_RectByDiamond: {
    insert_if(
        ilist.begin(), ilist.end(), jlist,
        bind2nd(mem_fun1<BOOL, cObstacle, cRect>(&cObstacle::ContainedDiamond),
                st.rcGround));
    // insert
...
HRESULT cRegion::GetiPlane(stSelect &st, cPlaneList &tlist) {
  ...
  case SelectPoint_RectByGround:
    insert_if(ilist.begin(), ilist.end(), jlist,
              bind2nd(mem_fun1<BOOL, cPlane, cRect>(&cPlane::ContainedGround),
                      st.rcGround));
    break;
  case SelectPoint_RectByDiamond:
    insert_if(ilist.begin(), ilist.end(), jlist,
              bind2nd(mem_fun1<BOOL, cPlane, cRect>(&cPlane::ContainedDiamond),
                      st.rcGround));
    break;
  }
  ...
  if (jlist.empty()) {
    //		CString s;
    //		s.Format("there are %d plane in the list, but not
    // found",ilist.size()); 		AfxMessageBox(s);
    return 0;
  }
...
ObstacleEnum cRegion::CheckObstacle(cPoint pt, cPoint &vSpeed,
                                    bool bClockWise) {
  for (itcObstacle it = m_listObstacle.begin(); it != m_listObstacle.end();
       ++it) {
    cObstacle *p = (*it);
...
int cRegion::MC_GetLinkInfo(LPCSTR szFolder) {
  ...
  for (int x = -1; x <= 1; x++)
    for (int y = -1; y <= 1; y++) {
      ...
      for (itcPlane it = region.m_listPlane.begin();
           it != region.m_listPlane.end(); it++) {
        cPlaneHor *p = dynamic_cast<cPlaneHor *>(*it);
        if (p != NULL) {
          p->UpdateVertexInfo();
